...
function budgetKey(project: string): string {
  return `pmm-node-budget:${project}`;
}
...
export function GraphTab({ project }: GraphTabProps) {
  ...
  const [leftWidth, setLeftWidth] = useState(() => loadWidth("pmm-left-w", 260));
  const [rightWidth, setRightWidth] = useState(() => loadWidth("pmm-right-w", 280));
