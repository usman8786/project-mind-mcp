# install.ps1 - One-line installer for project-mind-mcp (Windows).
#
...
$InstallDir = "$env:LOCALAPPDATA\Programs\project-mind-mcp"
$BinName = "project-mind-mcp.exe"
$WindowsArchiveNames = @(
    $BinName,
    "LICENSE",
    "install.ps1",
...
$BaseUrl = if ($env:PMM_DOWNLOAD_URL) { $env:PMM_DOWNLOAD_URL } else { "https://github.com/$Repo/releases/latest/download" }
...
Write-Host "project-mind-mcp installer (Windows)"
...
# Build download URL
if ($Variant -eq "ui") {
    $Archive = "project-mind-mcp-ui-windows-$Arch.zip"
} else {
    $Archive = "project-mind-mcp-windows-$Arch.zip"
}
$Url = "$BaseUrl/$Archive"

# Download
$TmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "pmm-install-$(Get-Random)"
New-Item -ItemType Directory -Path $TmpDir -Force | Out-Null

Write-Host "Downloading $Archive..."
try {
    Invoke-PmmDownload -Url $Url -OutFile "$TmpDir\$Archive"
} catch {
...
try {
    Invoke-PmmDownload -Url $ChecksumUrl -OutFile "$TmpDir\checksums.txt"
...
if (Test-Path -LiteralPath $Dest -PathType Leaf) {
    ...
    if (-not $renamed) {
        ...
        Write-Host "       project-mind-mcp sessions and coding agents, then re-run." -ForegroundColor Red
        Remove-Item -Recurse -Force $TmpDir -ErrorAction SilentlyContinue
        exit 1
    }
...
Write-Host ""
Write-Host "Done! Restart your terminal and coding agent to start using project-mind-mcp."
