"""Downloads project-mind-mcp on first run, then runs its native entry point."""
...
REPO = "usman8786/project-mind-mcp"
_WINDOWS_BINARY_NAME = "project-mind-mcp.exe"
...
def _validate_url_scheme(url: str) -> None:
    ...
    if (
        ...
        sys.exit(
            f"project-mind-mcp: refusing to fetch invalid, credentialed, or "
            f"non-https URL: {url}"
        )
...
def _download_https(url: str, dest: str, max_bytes: int = 0) -> None:
    ...
    for redirect_count in range(_MAX_REDIRECTS + 1):
        ...
        request = urllib.request.Request(
            current_url, headers={"User-Agent": "project-mind-mcp-installer"}
        )
        try:
...
def _safe_extract_tar(tf, dest: str) -> None:
    ...
    for member in tf.getmembers():
        if member.issym() or member.islnk():
            sys.exit(
                f"project-mind-mcp: refusing unsafe tar entry "
...
def _safe_extract_zip(zf, dest: str, archive_names=(), extract_names=()) -> None:
    ...
    for info in zf.infolist():
        ...
        if (
            ...
            sys.exit(
                f"project-mind-mcp: refusing unsafe zip entry "
        ...
        if name not in required_set or info.is_dir():
            sys.exit(
                f"project-mind-mcp: archive must contain only the exact "
    ...
    if missing or len(seen) != len(required):
        sys.exit(
            f"project-mind-mcp: archive must contain exactly one of each "
...
def _verify_checksum(archive_path: str, archive_name: str, version: str) -> None:
    ...
    try:
        ...
        with open(tmp_path, encoding="utf-8") as f:
            for line in f:
                ...
                if len(digest) != 64 or any(
                    ...
                    sys.exit(
                        f"project-mind-mcp: invalid SHA256 checksum for "
                        f"{archive_name}"
                    )
                if expected is not None and expected != digest:
                    sys.exit(
                        f"project-mind-mcp: conflicting SHA256 checksums for "
                        f"{archive_name}"
                    )
                expected = digest
        if expected is None:
            sys.exit(
                f"project-mind-mcp: no checksum for {archive_name} in checksums.txt"
            )
        h = hashlib.sha256()
        ...
        if expected != actual:
            sys.exit(
                f"project-mind-mcp: CHECKSUM MISMATCH for {archive_name}\n"
                f"  expected: {expected}\n"
                f"  actual:   {actual}"
            )
        print("project-mind-mcp: checksum verified.", file=sys.stderr)
    except SystemExit:
        raise
    except Exception as exc:
        sys.exit(f"project-mind-mcp: checksum verification failed: {exc}")
    finally:
...
def _version() -> str:
    try:
        ...
        return version("project-mind-mcp")
    except Exception:
        return "0.8.1"
...
def _os_name() -> str:
    ...
    if p == "win32":
        return "windows"
    sys.exit(f"project-mind-mcp: unsupported platform: {p}")
...
def _arch() -> str:
    ...
    if m in ("x86_64", "amd64"):
        return "amd64"
    sys.exit(f"project-mind-mcp: unsupported architecture: {m}")
...
def _download(version: str) -> Path:
    ...
    # Opt into the UI build (embedded graph visualization) with PMM_VARIANT=ui.
    # Default is the standard (headless) build. Mirrors install.sh --ui.
    ui = "ui-" if os.environ.get("PMM_VARIANT", "").lower() == "ui" else ""
    archive = f"project-mind-mcp-{ui}{os_name}-{arch}{variant}.{ext}"
    ...
    print(
        f"project-mind-mcp: downloading v{version} for {os_name}/{arch}...",
        file=sys.stderr,
    )

    with tempfile.TemporaryDirectory() as tmp:
        tmp_archive = os.path.join(tmp, f"pmm.{ext}")
        try:
            _download_https(url, tmp_archive)
        except (OSError, RuntimeError, urllib.error.URLError) as e:
            sys.exit(
                f"project-mind-mcp: download failed ({e})\n"
                f"URL: {url}\n"
        ...
        bin_name = (
            _WINDOWS_BINARY_NAME if os_name == "windows" else "project-mind-mcp"
        )
        ...
        for name in extraction_names:
            ...
            if not extracted_path.is_file():
                sys.exit(
                    f"project-mind-mcp: required binary not found after "
                    f"extraction: {name}"
                )
        ...
        try:
            ...
            sys.exit(f"project-mind-mcp: {exc}")
    ...
    if sys.platform != "win32":
        ...
        if result.returncode != 0 and mutation is not None:
            package_command = (
                ...
                if mutation == "update"
                else "python -m pip uninstall project-mind-mcp"
            )
            print(
                f'This PyPI Windows copy is portable. Use "{package_command}" '
                f'for package maintenance, or run "project-mind-mcp '
