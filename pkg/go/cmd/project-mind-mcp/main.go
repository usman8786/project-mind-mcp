// project-mind-mcp — Go installer wrapper.
//
...
// Install:
//
//	go install github.com/usman8786/project-mind-mcp/pkg/go/cmd/project-mind-mcp@latest
package main
...
const (
	...
	version             = "0.8.1"
	windowsLauncherName = "project-mind-mcp.exe"
	windowsPayloadName  = "project-mind-mcp.payload.exe"
	...
	windowsPairLockName           = ".project-mind-mcp-pair.lock"
...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "project-mind-mcp: %v\n", err)
		os.Exit(1)
	}
...
func printPortableMutationGuidance(args []string) {
	...
	packageCommand := "go install github.com/usman8786/project-mind-mcp/pkg/go/cmd/project-mind-mcp@latest"
	if action == "uninstall" {
		packageCommand = "Remove-Item (Get-Command project-mind-mcp).Source"
	}
	fmt.Fprintf(
		os.Stderr,
...
func cacheDir() string {
	if d := os.Getenv("PMM_CACHE_DIR"); d != "" {
		return d
	}
	switch runtime.GOOS {
	case "windows":
		if d := os.Getenv("LOCALAPPDATA"); d != "" {
			return filepath.Join(d, "project-mind-mcp")
		}
	case "darwin":
		if home, err := os.UserHomeDir(); err == nil {
			return filepath.Join(home, "Library", "Caches", "project-mind-mcp")
		}
	}
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".cache", "project-mind-mcp")
	}
	return filepath.Join(os.TempDir(), "project-mind-mcp")
}
...
func download(dest string) error {
	...
	archive := fmt.Sprintf("project-mind-mcp-%s-%s%s.%s", platform, arch, portable, ext)
	...
	fmt.Fprintf(os.Stderr, "project-mind-mcp: downloading v%s for %s/%s...\n", version, platform, arch)

	tmp, err := os.MkdirTemp("", "pmm-install-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(tmp)

	archivePath := filepath.Join(tmp, "pmm."+ext)
	...
	operationErr := func() error {
		for _, name := range names {
			stagePath := filepath.Join(
				dstDir, ".pmm-pair-stage-"+transaction+"-"+name)
		...
		for _, name := range names {
			...
			backup := filepath.Join(
				dstDir, ".pmm-pair-backup-"+transaction+"-"+name)
