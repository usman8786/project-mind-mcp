...
test('Windows npm shim probes and executes the cached launcher', () => {
  ...
  for (const call of observed.calls) {
    assert.equal(path.basename(call.executable), 'project-mind-mcp.exe');
    assert.notEqual(
...
test('Windows npm shim keeps package-manager guidance after launcher refusal', () => {
  ...
  assert.equal(path.basename(observed.calls[1].executable), 'project-mind-mcp.exe');
  assert.match(observed.stderr, /npm install project-mind-mcp@latest/);
...
test('non-Windows npm shim keeps its native payload execution path', () => {
  ...
  assert.equal(path.basename(observed.calls[0].executable), 'project-mind-mcp');
});
