...
const REPO = 'usman8786/project-mind-mcp';
...
const WINDOWS_BINARY_NAME = 'project-mind-mcp.exe';
...
const WINDOWS_PAIR_LOCK_NAME = '.project-mind-mcp-pair.lock';
...
function installWindowsBinaryAtomically(sourceDir, destDir, verifier = verifyCandidate) {
  ...
  try {
    ...
    try {
      for (const name of [WINDOWS_BINARY_NAME]) {
        const staged = path.join(destDir, `.pmm-pair-stage-${transaction}-${name}`);
      ...
      for (const name of [WINDOWS_BINARY_NAME]) {
        ...
        const backup = path.join(destDir, `.pmm-pair-backup-${transaction}-${name}`);
...
function extractZipOnWindows(archivePath, destPath, requiredNames, extractNames) {
  ...
  const script = [
    ...
    '$zip = [System.IO.Compression.ZipFile]::OpenRead($env:PMM_NPM_ARCHIVE_PATH)',
    "$seen = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::OrdinalIgnoreCase)",
    "$requiredNames = @($env:PMM_NPM_REQUIRED_NAMES.Split('|'))",
    '$targetCounts = @{}',
    ...
    "$extractNames = @($env:PMM_NPM_EXTRACT_NAMES.Split('|'))",
    '$extractZip = [System.IO.Compression.ZipFile]::OpenRead($env:PMM_NPM_ARCHIVE_PATH)',
    'try { foreach ($extractName in $extractNames) { ' +
      '$entry = @($extractZip.Entries | Where-Object { $_.FullName -ceq $extractName })[0]; ' +
      '[System.IO.Compression.ZipFileExtensions]::ExtractToFile(' +
      '$entry, (Join-Path $env:PMM_NPM_DEST_PATH $extractName), $false) ' +
  ...
  execFileSync('powershell', [
    ...
    env: {
      ...process.env,
      PMM_NPM_ARCHIVE_PATH: archivePath,
      PMM_NPM_DEST_PATH: destPath,
      PMM_NPM_REQUIRED_NAMES: requiredNames.join('|'),
      PMM_NPM_EXTRACT_NAMES: extractNames.join('|'),
    },
    stdio: 'inherit',
    windowsHide: true,
  });
...
async function verifyChecksum(archivePath, archiveName) {
  ...
  try {
    ...
    process.stdout.write('project-mind-mcp: checksum verified.\n');
  } finally {
...
async function main() {
  ...
  const ui = (process.env.PMM_VARIANT || '').toLowerCase() === 'ui' ? 'ui-' : '';
  const archive = `project-mind-mcp-${ui}${platform}-${arch}${variant}.${ext}`;
  const url = `https://github.com/${REPO}/releases/download/v${VERSION}/${archive}`;

  const uiLabel = ui ? '(ui) ' : '';
  process.stdout.write(`project-mind-mcp: downloading v${VERSION} ${uiLabel}for ${platform}/${arch}...\n`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pmm-install-'));
  const tmpArchive = path.join(tmpDir, `pmm.${ext}`);
  ...
  try {
    ...
    process.stdout.write('project-mind-mcp: ready.\n');
    if (platform === 'windows') {
      process.stdout.write(
        'Windows package cache is portable. Run "project-mind-mcp install --yes" ' +
        'to create the managed launcher (use "npx project-mind-mcp install --yes" ' +
        'for a local npm install). Package updates/removal remain ' +
        '"npm install project-mind-mcp@latest" and ' +
        '"npm uninstall project-mind-mcp" (add -g for a global install).\n',
      );
    }
  } finally {
...
if (require.main === module || module.parent == null) {
  main().catch((err) => {
    process.stderr.write(`\nproject-mind-mcp: install failed — ${err.message}\n`);
