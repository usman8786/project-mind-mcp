...
const binName = isWindows ? 'project-mind-mcp.exe' : 'project-mind-mcp';
...
if (!cacheReady()) {
  ...
  process.stderr.write('project-mind-mcp: binary not found, downloading...\n');
  const installResult = spawnSync(process.execPath, [path.join(__dirname, 'install.js')], {
    stdio: 'inherit',
  });
  if (installResult.status !== 0 || !cacheReady()) {
    process.stderr.write(
      'project-mind-mcp: download failed.\n' +
      'Try reinstalling: npm install -g project-mind-mcp\n'
    );
    process.exit(1);
  }
}
...
if (result.error) {
  process.stderr.write(`project-mind-mcp: ${result.error.message}\n`);
  process.exit(1);
}
...
if (mutation) {
  ...
  process.stderr.write(
    ...
    '"project-mind-mcp install --yes" once for a managed install with its ' +
