...
if [[ -d "$SKILLS_DIR" ]]; then
    ...
    while IFS= read -r skill_file; do
        ...
        if grep -oE 'https?://[^\s"'"'"']+' "$skill_file" 2>/dev/null | grep -v 'github.com/usman8786' | grep -v 'localhost' | grep -v '127.0.0.1' > /tmp/sec_skill_urls 2>/dev/null; then
