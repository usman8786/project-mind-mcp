...
MANIFEST_PATHS="$(mktemp "${TMPDIR:-/tmp}/pmm-vendored-paths.XXXXXX")" || {
    echo "BLOCKED: cannot create checksum verification workspace"
    exit 1
}
VENDORED_FILES="$(mktemp "${TMPDIR:-/tmp}/pmm-vendored-files.XXXXXX")" || {
    rm -f "$MANIFEST_PATHS"
    echo "BLOCKED: cannot create vendored inventory workspace"
    exit 1
}
VENDORED_SYMLINKS="$(mktemp "${TMPDIR:-/tmp}/pmm-vendored-links.XXXXXX")" || {
...
# Also scan tree-sitter grammars (internal/pmm/vendored/) — 650MB, 20M lines.
# Use fixed-string grep for each pattern to avoid slow regex on the large tree.
if [[ -d "$ROOT/internal/pmm/vendored" ]]; then
    GRAMMAR_FAIL=false
    for pattern in 'system(' 'popen(' 'execl(' 'execv(' 'fork(' 'connect(' 'socket(' 'sendto(' 'dlopen(' 'LoadLibrary('; do
        HITS="$(grep -rl -F "$pattern" "$ROOT/internal/pmm/vendored/" --include='*.c' --include='*.h' 2>/dev/null | head -3 || true)"
        if [[ -n "$HITS" ]]; then
