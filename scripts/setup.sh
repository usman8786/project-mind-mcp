...
# project-mind-mcp setup script (macOS + Linux)
...
BINARY_NAME="project-mind-mcp"
SOURCE_DIR="$HOME/.local/share/project-mind-mcp"
...
download_binary() {
    ...
    local asset="project-mind-mcp-${platform}.tar.gz"
...
build_from_source() {
    ...
    (cd "$SOURCE_DIR" && scripts/build.sh && cp build/c/project-mind-mcp "${INSTALL_DIR}/${BINARY_NAME}")
...
configure_claude() {
    ...
    printf "%s" "${BOLD}Configure Claude Code to use project-mind-mcp? [y/N] ${RESET}"
    read -r answer
    if [[ ! "$answer" =~ ^[Yy]$ ]]; then
        echo ""
...
JSONEOF
    ...
    if command -v jq &>/dev/null; then
        ...
        if [ -f "$settings_file" ]; then
            local tmp
            tmp=$(mktemp)
            jq --argjson entry "$mcp_entry" '.mcpServers["project-mind-mcp"] = $entry' "$settings_file" > "$tmp"
            mv "$tmp" "$settings_file"
        else
            echo "{}" | jq --argjson entry "$mcp_entry" '.mcpServers["project-mind-mcp"] = $entry' > "$settings_file"
        fi
...
if os.path.exists(path):
    with open(path) as f:
        data = json.load(f)
data.setdefault('mcpServers', {})['project-mind-mcp'] = json.loads('$mcp_entry')
...
echo ""
echo "${BOLD}project-mind-mcp installer${RESET}"
echo ""
...
info "  rm -rf ~/.cache/project-mind-mcp/  # graph database"
