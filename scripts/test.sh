...
Environment:
  PMM_TEST_SEQUENTIAL=1   Single-process runner instead of the parallel harness.
  PMM_RUN_HANG_TEST=1     Opt-in C++ index-hang regression (#410, needs prod).
  PMM_NO_CCACHE=1         Disable the content-verified compiler cache.
  PMM_TEST_SHARD/_LEG     Set by CI's sharded legs; leave unset locally.
...
if [ "$TSAN" -eq 1 ]; then
    ...
    make -j"$NPROC" -f Makefile.pmm "$BUILD_DIR/test-runner-tsan" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
    make -f Makefile.pmm test-tsan ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
    exit "$?"
fi
...
if [ -n "$SUITES" ]; then
    ...
    make -j"$NPROC" -f Makefile.pmm "$BUILD_DIR/test-runner" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
...
# PMM_TEST_SEQUENTIAL=1 restores the single-process runner.
make -j"$NPROC" -f Makefile.pmm "$BUILD_DIR/test-runner" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
if [ "${PMM_TEST_SEQUENTIAL:-0}" = "1" ]; then
    make -f Makefile.pmm test ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
else
    make -f Makefile.pmm test-par ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
fi
...
# Opt-in via PMM_RUN_HANG_TEST=1 (it needs the prod binary, which the ASan unit
# run above does not build). Skipped by default so the fast unit run stays fast.
if [ "${PMM_RUN_HANG_TEST:-0}" = "1" ]; then
...
make -j"$NPROC" -f Makefile.pmm pmm TEST_SEAMS=1 ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
WATCHDOG_BINARY="$ROOT/$BUILD_DIR/project-mind-mcp"
PMM_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_parent_watchdog.sh"
...
PMM_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_worker_watchdog.sh"
