...
Legs (default: quick,query-leak — the release-gating sequence):
  ...
  query-leak   PMM_SOAK_MODE=query-leak, --skip-crash-test, results in
...
Examples:
  scripts/soak-legs.sh build/c/project-mind-mcp 10          # both legs, 10m each
  scripts/soak-legs.sh --legs quick build/c/project-mind-mcp 15   # ASan-style
EOF
}
...
# keys its native-Windows handling — cygpath'd PMM_CACHE_DIR, coproc stdio —
...
run_leg() {
    ...
    log="$(mktemp "${TMPDIR:-/tmp}/pmm-soak-leg-XXXXXX")"
    ...
    quick)
        ...
        ;;
    query-leak)
        PMM_SOAK_MODE=query-leak RESULTS_DIR=soak-results-query-leak \
