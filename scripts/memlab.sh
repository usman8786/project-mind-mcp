...
if [[ "$BINARY" == *.exe ]] && command -v cygpath >/dev/null 2>&1 &&
    ...
    WIN_ROOT=$(mktemp -d "$(cygpath "$USERPROFILE")/pmm-memlab.XXXXXX")
    ...
    cp "$BINARY" "$WIN_ROOT/project-mind-mcp.exe"
    BINARY="$WIN_ROOT/project-mind-mcp.exe"
    WORK="$WIN_ROOT"
...
if command -v cygpath >/dev/null 2>&1 && ! command -v winepath >/dev/null 2>&1; then
    mkdir -p "$WORK/cache"
    export PMM_CACHE_DIR="$(cygpath -w "$WORK/cache")"
else
    export PMM_CACHE_DIR="$WORK/cache"
fi
export PMM_MEM_PROFILE=1
export PMM_MEM_PROFILE_OUT="$PROFILE_OUT"
export PMM_MEM_CENSUS=1
export PMM_LOG_LEVEL=info
export PMM_LOG_FORMAT=text
mkdir -p "$PMM_CACHE_DIR"
...
RC=$?
# With PMM_CACHE_DIR set the process logs to its own file rather than stderr,
...
if [ "$CENSUS" -eq 0 ]; then
    echo "WARN: no census samples — check PMM_MEM_CENSUS wiring" >&2
fi
if [ "$SITES" -eq 0 ]; then
