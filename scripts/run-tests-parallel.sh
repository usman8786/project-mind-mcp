...
#   jobs defaults to PMM_TEST_PAR_JOBS, then the CPU count.

set -uo pipefail

RUNNER="${1:?usage: run-tests-parallel.sh <path-to-test-runner> [jobs]}"
JOBS="${2:-${PMM_TEST_PAR_JOBS:-}}"
...
# PMM_TEST_SHARD="i/N" runs this invocation's deterministic slice of the
...
SHARD_INDEX=1
SHARD_TOTAL=1
if [ -n "${PMM_TEST_SHARD:-}" ]; then
    if ! printf '%s' "$PMM_TEST_SHARD" | grep -qE '^[0-9]+/[0-9]+$'; then
        echo "FAIL: PMM_TEST_SHARD must be i/N, got '$PMM_TEST_SHARD'" >&2
        exit 1
    fi
    SHARD_INDEX="${PMM_TEST_SHARD%%/*}"
    SHARD_TOTAL="${PMM_TEST_SHARD##*/}"
    if [ "$SHARD_TOTAL" -lt 1 ] || [ "$SHARD_INDEX" -lt 1 ] ||
        [ "$SHARD_INDEX" -gt "$SHARD_TOTAL" ]; then
        echo "FAIL: PMM_TEST_SHARD out of range: $PMM_TEST_SHARD" >&2
        exit 1
    fi
fi
...
# against a mis-plumbed PMM_TEST_SHARD (two jobs running the same slice
...
run_wave() {
    ...
    if ! python3 "$SCHEDULER" \
        ...
        --jobs "$jobs" \
        --timeout "${PMM_SUITE_TIMEOUT:-900}" \
        --slow-timeout "${PMM_SUITE_TIMEOUT_SLOW:-3600}" \
        --kill-grace 15 \
        "$RUNNER"; then
...
# so a small fixed overlap (PMM_TAIL_JOBS, default 2) is safe and converts
...
run_wave "$FLEX_FILE" "${PMM_TAIL_JOBS:-2}"
