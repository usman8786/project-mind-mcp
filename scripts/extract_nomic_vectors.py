...
def write_tokens_h(path: str, tokens: list):
    ...
    with open(path, "w") as f:
        ...
        f.write("#ifndef PMM_NOMIC_TOKENS_H\n")
        f.write("#define PMM_NOMIC_TOKENS_H\n\n")
        f.write(f"static const char *PRETRAINED_TOKENS[{len(tokens)}] = {{\n")
        for t in tokens:
            escaped = t.replace("\\", "\\\\").replace('"', '\\"')
            f.write(f'"{escaped}",\n')
        f.write("};\n\n")
        f.write("#endif /* PMM_NOMIC_TOKENS_H */\n")
 ...
 * License: Apache 2.0
 */
#ifndef PMM_NOMIC_VECTORS_H
#define PMM_NOMIC_VECTORS_H
...
def main():
    ...
    print()
    print("next steps:")
    print(f"  1. update Makefile.pmm: change UNIXCODER_BLOB_SRC path to vendored/nomic/")
    print(f"  2. update #include in semantic.c: \"vendored/nomic/code_vectors.h\"")
    print(f"  3. arch -arm64 make -j12 -f Makefile.pmm clean-c && arch -arm64 make -j12 -f Makefile.pmm")
