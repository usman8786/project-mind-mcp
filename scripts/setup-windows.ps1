# project-mind-mcp setup script (Windows)
...
$BinaryName = "project-mind-mcp"
$InstallDir = Join-Path $env:LOCALAPPDATA "project-mind-mcp"
...
function Configure-ClaudeCode($McpConfig) {
    Write-Host ""
    $answer = Read-Host "Configure Claude Code to use project-mind-mcp? [y/N]"
    ...
    if ($answer -match '^[Yy]$') {
        ...
        $settings["mcpServers"]["project-mind-mcp"] = $McpConfig
        ...
        Write-Host ""
        $snippet = @{ mcpServers = @{ "project-mind-mcp" = $McpConfig } }
...
Write-Host ""
Write-Host "project-mind-mcp installer (Windows)" -ForegroundColor White
Write-Host ""
...
if ($FromSource) {
    ...
    # Clone or update
    Write-Host ""
    $sourceDir = "/home/$wslUser/.local/share/project-mind-mcp"
    try {
    ...
    Invoke-WSL "mkdir -p /home/$wslUser/.local/bin && cd $sourceDir && scripts/build.sh && cp build/c/project-mind-mcp $wslBinaryPath"
    ...
    Write-Host "    wsl.exe -- rm -rf ~/.cache/project-mind-mcp/"
    ...
    $release = Invoke-RestMethod -Uri $releaseUrl -Headers @{ "User-Agent" = "project-mind-mcp-setup" }
    ...
    $asset = "project-mind-mcp-windows-amd64.zip"
    ...
    Write-Host "    Remove-Item -Recurse -Force `"$env:LOCALAPPDATA\project-mind-mcp`"  # graph database"
}
