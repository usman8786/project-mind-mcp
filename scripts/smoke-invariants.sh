...
#
# A comprehensive, fast, portable smoke battery for the project-mind-mcp
...
#
# Usage:
#   scripts/smoke-invariants.sh <binary>        # e.g. build/c/project-mind-mcp(.exe)
#
# Portability notes:
...
inv_help() {
    ...
    if printf '%s' "$RB_OUT" | grep -qiE 'usage|project-mind-mcp'; then
        pass "help"
    else
...
GOEOF
    ...
    # Project name derivation mirrors pmm_project_name_from_path: every char not
...
inv_second_index_inprocess() {
    local base
    base=$(mktemp -d "${TMPDIR:-/tmp}/pmm_inv773.XXXXXX")
    ...
    run_bounded 120 env PMM_INDEX_SUPERVISOR=0 sh -c 'exec "$1" < "$2"' _ "$BINARY" "$infile"
    local rc="$RB_RC"
    local out="$RB_OUT"
    rm -rf "$base"
    if [ "$rc" -gt 128 ]; then
...
inv_every_tool() {
    ...
    CALLS=(
        ...
        "delete_project|{\"project\":\"__pmm_smoke_nonexistent__\"}"
    )
    ...
    local ureq="{\"jsonrpc\":\"2.0\",\"id\":$((MCP_ID++)),\"method\":\"tools/call\",\"params\":{\"name\":\"__pmm_no_such_tool__\",\"arguments\":{}}}"
...
inv_malformed_input() {
    ...
    BADLINES=(
        ...
        '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"index_repository","arguments":{"repo_path":"/pmm/does/not/exist/xyz"}}}'
...
inv_nonexistent_repo_cli() {
    cli_call 30 --json index_repository '{"repo_path":"/pmm/definitely/not/here/zzz"}'
    if [ "$CLI_RC" -eq 124 ]; then
...
# (PMM_TEST_CRASH_ON) so the guard is honest: with the supervisor OFF the crash
...
inv_crasher_skipped_cli() {
    ...
    export PMM_TEST_CRASH_ON=crash_me
    export PMM_INDEX_SUPERVISOR=0
    cli_call 60 index_repository "{\"repo_path\":\"$cn\"}"
    local base_rc="$CLI_RC"
    unset PMM_INDEX_SUPERVISOR
...
# progress (external-scanner infinite loop, modelled by PMM_TEST_HANG_ON's busy-
...
# PMM_INDEX_WORKER_TIMEOUT_S the run must COMPLETE (rc<128, not 124), report
...
inv_hanger_skipped_cli() {
    ...
    export PMM_TEST_HANG_ON=hang_me
    export PMM_INDEX_SUPERVISOR=0
    ...
    export PMM_INDEX_WORKER_TIMEOUT_S=5
    cli_call 90 index_repository "{\"repo_path\":\"$hn\"}"
    local sup_rc="$CLI_RC"
    local sup_out="$CLI_OUT"
    unset PMM_INDEX_WORKER_TIMEOUT_S
    unset PMM_TEST_HANG_ON
