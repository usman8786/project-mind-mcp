<#
.SYNOPSIS
    Run the native-Windows product-surface test suite for project-mind-mcp.
...
.DESCRIPTION
    ...
    Two categories of test:
      ...
      GUARDS      - regression guards for Windows bugs already fixed on main.
                    ...
                    regressed and fails this runner.
                      ...
                      * test_non_ascii_cache_dump.py guards #996 (writer pmm_fopen)
    ...
    under the mandatory coordination daemon PMM_INDEX_SUPERVISOR=0 is a
...
.PARAMETER Target
    Makefile.pmm target used when building: 'pmm-with-ui' (default; needed for the
    drive-picker guard's embedded HTTP UI) or 'pmm' (no UI - the drive guard then
...
.EXAMPLE
    pwsh -File scripts/test-windows.ps1 -GuardsOnly -Binary build\c\project-mind-mcp.exe
#>
[CmdletBinding()]
param(
    [string]$Binary,
    [ValidateSet("pmm-with-ui", "pmm")]
    [string]$Target = "pmm-with-ui",
    [switch]$GuardsOnly,
    [string]$Make = "make"
)
...
function Resolve-Binary {
    ...
    $built = Join-Path $repoRoot "build\c\project-mind-mcp.exe"
    if (Test-Path $built) { return $built }
    Write-Host "Building $Target via Makefile.pmm ..." -ForegroundColor Cyan
    & $Make "-j" "-f" "Makefile.pmm" $Target "SANITIZE=" "TMP=$tmp" "TEMP=$tmp" "TMPDIR=$tmp" | Out-Host
...
try {
    ...
    $guardRoot = Join-Path $userProfile ("pmm-windows-guards-root-" + [guid]::NewGuid().ToString("N"))
    ...
    $guardBundle = Join-Path $guardRoot ("pmm-windows-guards-" + [guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $guardBundle | Out-Null
    $guardBin = Join-Path $guardBundle "project-mind-mcp.exe"
...
# UI (build target pmm-with-ui); against a non-UI binary it reports a precondition
...
if ($guardSkips.Count -gt 0) {
    ...
    Write-Host "needs a UI build (-Target pmm-with-ui, the default)." -ForegroundColor Yellow
}
