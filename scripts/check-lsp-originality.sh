...
#
# WHY THIS EXISTS
#   The LSP layer (internal/pmm/lsp/) is an original C implementation whose
...
if [ "$total_hits" -gt 0 ] || [ "$structural_hits" -gt 0 ]; then
  if [ "$total_hits" -gt 0 ]; then
    ...
    echo "$TAG with internal/pmm/lsp/. A hit is NOT proof of copying (common phrases collide)"
