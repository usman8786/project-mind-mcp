...
per-grammar license list is internal/pmm/vendored/grammars/MANIFEST.md.
"""
...
version = sys.argv[1]
n_grammars = len(
    [d for d in glob.glob("internal/pmm/vendored/grammars/*") if os.path.isdir(d)]
)
sbom = {
    "spdxVersion": "SPDX-2.3",
    "dataLicense": "CC0-1.0",
    "SPDXID": "SPDXRef-DOCUMENT",
    "name": f"project-mind-mcp-{version}",
    "documentNamespace": f"https://github.com/usman8786/project-mind-mcp/releases/{version}",
    "creationInfo": {
        "created": datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "creators": ["Tool: project-mind-mcp-release-pipeline"],
    },
    "packages": [
