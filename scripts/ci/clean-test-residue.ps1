<#
.SYNOPSIS
Sweep pmm test residue from a Windows test host and assert a runner-like free
...
Sweeps only pmm-prefixed directories under the profile and TEMP. The checkout
(C:\pmm) is not under either, and is never touched.
...
# non-representable characters, deep .pmm-retired/generations nesting - whose
...
foreach ($root in $roots) {
    ...
    Get-ChildItem -LiteralPath $root -Directory -Filter 'pmm-*' -ErrorAction SilentlyContinue |
        ForEach-Object {
