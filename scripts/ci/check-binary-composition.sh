...
# processes and file placement — and PMM_TEST_* in a shipped binary is exactly
# the "debug/injection hooks" shape AV heuristics score on.
SEAM_NEEDLES=(
    'PMM_TEST_WORKER_DESCENDANT_PID_FILE'
...
CANARY_NEEDLE='project-mind-mcp'
...
check_file() {
    ...
    # "project-mind-mcp", so the parent directory is what tells them apart.
    ...
    standard) is_ui=0 ;;
    auto)
        # The UI archive is project-mind-mcp-ui-<os>-<arch>, but the binary
        # inside it is just "project-mind-mcp" — so the whole path decides,
    ...
    # Sweep for seams nobody thought to pin: a new PMM_TEST_* env var added to
    ...
    seam_allowed='PMM_TEST_CRASH_ON PMM_TEST_HANG_ON PMM_TEST_WINDOWS_USER_PATH_RUN_ID'
    seam_unexpected=''
    for found in $(LC_ALL=C grep -a -o -E 'PMM_TEST_[A-Za-z0-9_]+' "$file" 2>/dev/null |
        sort -u || true); do
    ...
    if [ -n "$seam_unexpected" ]; then
        report FAIL A2-no-test-seams "$token" \
            "unexpected PMM_TEST_* seam(s):$seam_unexpected (allowlist: $seam_allowed)"
    else
        report PASS A2-no-test-seams "$token" 'no PMM_TEST_* seams beyond the smoke allowlist'
    fi
    ...
    # exercised so that the day the split lands, flipping PMM_CHECK_UI_ABSENT=1
    ...
    if [ "$is_ui" -eq 1 ]; then
        printf 'n/a  %-22s %s: UI artifact, HTTP server ships here by design\n' \
            A5-no-ui-http "$token"
    elif [ "${PMM_CHECK_UI_ABSENT:-0}" = "1" ]; then
        ...
        printf 'INFO %-22s %s: %d/%d UI/HTTP needles present (no-UI split pending; set PMM_CHECK_UI_ABSENT=1 to enforce)\n' \
