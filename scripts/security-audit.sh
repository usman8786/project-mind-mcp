...
FUNC_LIST="system popen pmm_popen execl fork"
...
while IFS= read -r file; do
    ...
    for func in $FUNC_LIST; do
        ...
        # 'popen(' must NOT match 'pmm_popen(' — use [^a-z] negative class
        case "$func" in
            pmm_popen) pattern="pmm_popen(" ;;
...
while IFS= read -r file; do
    ...
    DANGER_LINES=$(grep -n 'system(\|popen(\|pmm_popen(\|connect(\|execl(' "$file" 2>/dev/null \
