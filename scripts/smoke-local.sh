...
#   <binary>  product binary to smoke (e.g. build/c/project-mind-mcp)
...
Environment:
  PMM_SMOKE_ARTIFACT_DIR   Release mode: an EXTRACTED release artifact
...
smoke-linux-portable (with PMM_SMOKE_ARTIFACT_DIR) · compose smoke services.
...
WORK_DIR=$(mktemp -d "${TMPDIR:-/tmp}/pmm-smoke-server-XXXXXX")
...
cp "$BINARY" "$FIXTURE_DIR/project-mind-mcp"
if [ -n "$ARTIFACT_DIR" ]; then
...
EXPECTED_ARTIFACT="project-mind-mcp${SUFFIX}-${OS}-${ARCH}.tar.gz"
tar -czf "$FIXTURE_DIR/$EXPECTED_ARTIFACT" -C "$FIXTURE_DIR" \
    project-mind-mcp LICENSE install.sh THIRD_PARTY_NOTICES.md
if [ -n "$SUFFIX" ]; then
    cp "$FIXTURE_DIR/$EXPECTED_ARTIFACT" \
        "$FIXTURE_DIR/project-mind-mcp-${OS}-${ARCH}.tar.gz"
fi
...
if [ "$OS" = "linux" ]; then
    cp "$FIXTURE_DIR/$EXPECTED_ARTIFACT" \
        "$FIXTURE_DIR/project-mind-mcp${SUFFIX}-${OS}-${ARCH}-portable.tar.gz"
    if [ -n "$SUFFIX" ]; then
        cp "$FIXTURE_DIR/project-mind-mcp${SUFFIX}-${OS}-${ARCH}-portable.tar.gz" \
            "$FIXTURE_DIR/project-mind-mcp-${OS}-${ARCH}-portable.tar.gz"
    fi
fi
