...
Archive contents (defined here, canonical) — ONE binary per platform:
  unix:    project-mind-mcp LICENSE install.sh THIRD_PARTY_NOTICES.md (.tar.gz)
  windows: project-mind-mcp.exe LICENSE install.ps1
...
NAME="project-mind-mcp${SUFFIX}-${GOOS}-${GOARCH}"
...
if [ "$GOOS" = "windows" ]; then
    ...
    PAYLOAD="$BUILD_DIR/project-mind-mcp"
    [ -f "${PAYLOAD}.exe" ] && PAYLOAD="${PAYLOAD}.exe"
    [ -f "$PAYLOAD" ] || { echo "package-release: build first; missing $PAYLOAD" >&2; exit 2; }
    PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pmm-package.XXXXXX")"
    trap 'rm -rf "$PACK_DIR"' EXIT
    cp "$PAYLOAD" "$PACK_DIR/project-mind-mcp.exe"
    strip_release_binary "$PACK_DIR/project-mind-mcp.exe" || exit 2
    ...
    bash scripts/ci/check-binary-composition.sh --variant="$VARIANT" \
        "$PACK_DIR/project-mind-mcp.exe" || exit 2
    ...
    [ -f "$BUILD_DIR/project-mind-mcp" ] ||
        { echo "package-release: build first; missing $BUILD_DIR/project-mind-mcp" >&2; exit 2; }
    strip_release_binary "$BUILD_DIR/project-mind-mcp" || exit 2
    bash scripts/ci/check-binary-composition.sh --variant="$VARIANT" \
        "$BUILD_DIR/project-mind-mcp" || exit 2
