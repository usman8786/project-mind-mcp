...
[![GitHub Release](https://img.shields.io/github/v/release/usman8786/project-mind-mcp?style=flat&color=blue)](https://github.com/usman8786/project-mind-mcp/releases/latest)
[![License](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![CI](https://img.shields.io/github/actions/workflow/status/usman8786/project-mind-mcp/dry-run.yml?label=CI)](https://github.com/usman8786/project-mind-mcp/actions/workflows/dry-run.yml)
[![Tests](https://img.shields.io/badge/tests-6768_passing-brightgreen)](https://github.com/usman8786/project-mind-mcp)
[![Languages](https://img.shields.io/badge/languages-158-orange)](https://github.com/usman8786/project-mind-mcp)
[![Hybrid LSP](https://img.shields.io/badge/Hybrid_LSP-10_languages-blue)](#hybrid-lsp)
[![Agents](https://img.shields.io/badge/agent_surfaces-43-purple)](https://github.com/usman8786/project-mind-mcp)
[![Pure C](https://img.shields.io/badge/pure_C-zero_dependencies-blue)](https://github.com/usman8786/project-mind-mcp)
[![Platform](https://img.shields.io/badge/macOS_%7C_Linux_%7C_Windows-supported-lightgrey)](https://github.com/usman8786/project-mind-mcp/releases/latest)
...
> **Provenance** — This project is a rebranded fork of the open-source [codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp) (MIT). Original copyright: DeusData. Project Mind is maintained by [@usman8786](https://github.com/usman8786).
...
> **Research** — Upstream design and benchmarks are described in the preprint [*Codebase-Memory: Tree-Sitter-Based Knowledge Graphs for LLM Code Exploration via MCP*](https://arxiv.org/abs/2603.27277) (arXiv:2603.27277).
...
<p align="center">
  <img src="docs/graph-ui-screenshot.png" alt="Graph visualization UI showing the project-mind-mcp knowledge graph" width="800">
  <br>
  <em>Built-in 3D graph visualization (UI variant) — explore your knowledge graph at localhost:9749</em>
</p>

## Why project-mind-mcp
...
Invoke-WebRequest -Uri https://raw.githubusercontent.com/usman8786/project-mind-mcp/main/install.ps1 -OutFile install.ps1
...
1. **Download** the archive for your platform from the [latest release](https://github.com/usman8786/project-mind-mcp/releases/latest):
   - `project-mind-mcp-<os>-<arch>.tar.gz` (macOS/Linux) or `.zip` (Windows) — standard
   - `project-mind-mcp-ui-<os>-<arch>.tar.gz` / `.zip` — with graph visualization

2. **Extract and install** (each archive includes `install.sh` or `install.ps1`):

   macOS / Linux:
   ```bash
   tar xzf project-mind-mcp-*.tar.gz
   ./install.sh
   ```

   Windows (PowerShell):
   ```powershell
   Expand-Archive project-mind-mcp-windows-amd64.zip -DestinationPath .
   Unblock-File .\install.ps1
   .\install.ps1
   ```
...
PMM automatically shares one per-account coordination daemon across Claude Code, Codex, OpenCode, and every other configured client. There is no opt-in setting for MCP servers or hook clients: the first daemon-backed PMM session starts it, each session registers its own work, and the final session shuts it down. The daemon owns long-lived background services such as watchers, shared indexing jobs, and the optional UI. Closing one session cancels work owned only by that session, while work still needed by another session continues.

The detached daemon does not depend on an MCP frontend's stderr. It keeps owner-only durable records under the canonical `${PMM_CACHE_DIR}/logs` directory (default `~/.cache/project-mind-mcp/logs`):

| File | Contents |
|------|----------|
| `pmm-daemon.log` | Daemon lifecycle, watcher/indexing, UI, resource, and error events. |
...
All active PMM processes must run the exact same version, executable build, coordination ABI, and canonical cache root. Equivalent `PMM_CACHE_DIR` aliases resolve to the same root; a genuinely different root is rejected while any PMM process is active. MCP servers, hooks, one-shot CLI commands, temporary index workers, and the daemon share a crash-safe OS admission barrier; starting an ordinary conflicting process fails before doing work and records an explicit conflict in `${PMM_CACHE_DIR}/logs/daemon-conflicts.ndjson`.

The native `install`, `update`, and `uninstall` commands are the deliberate exception to that conflict rule. Download, verification, and private same-filesystem staging happen first so a bad candidate never disrupts active work. Activation then publishes account-wide maintenance intent, asks the daemon and every temporary local operation to cancel, and waits to a finite deadline for all coordinated PMM processes to exit. It holds the admission and lifetime barriers exclusively while changing the active binary, configuration, PATH, or indexes. New PMM work cannot enter during this window. Activation progress and results are recorded in `${PMM_CACHE_DIR}/logs/activation-events.ndjson`, and a successful command tells you to restart open coding-agent sessions so they launch the activated build.

Package-manager setup (npm, PyPI, or Go) only verifies and atomically publishes that package's private cached binary; it does not replace the active native installation and therefore does not stop running PMM sessions. When that cached binary is executed, it still enters the same exact-build admission barrier. The shell and PowerShell installers invoke the verified candidate's native `install` command, so they do receive the full account-wide activation guarantee.
...
- **npm:** `PMM_VARIANT=ui npm install -g project-mind-mcp`
- **PyPI:** `PMM_VARIANT=ui pip install project-mind-mcp`
- **Manual:** download the `project-mind-mcp-ui-<os>-<arch>` archive

Then run it:

```bash
project-mind-mcp --ui=true --port=9749
```
...
```bash
project-mind-mcp config set auto_index true
```
...
**Updates run from the install script on every platform, not from inside the running binary.** `project-mind-mcp update` validates your flags and then prints the exact command to run:
...
Why it works this way. On Windows it is a hard requirement — a running executable cannot replace its own image, so the swap has to happen from a process that is not the binary being replaced. On macOS and Linux it is a deliberate choice: an in-process updater is structurally a downloader (fetch an archive, verify it, unpack it, mark a file executable, run it), and shipping that composite in every binary to serve a command most people run a handful of times is a poor trade. The release archives now carry no download URLs at all, and **pmm makes no network request of its own accord** — it does not check for new versions in the background, and nothing phones home. You find out about releases from the install script, your package manager, or GitHub.

If PowerShell refuses to run the script because the file came from the internet, `Unblock-File` it first.

Installed through **npm or pip**? Update with your package manager on every platform (`npm install -g project-mind-mcp@latest` / `pip install -U project-mind-mcp`).

### Uninstall

```bash
project-mind-mcp uninstall
```
...
- **BM25 full-text search** via SQLite FTS5 with `pmm_camel_split` tokenizer (camelCase / snake_case aware)
...
- **Single static binary, zero infrastructure**: SQLite-backed, persists to `~/.cache/project-mind-mcp/`
- **Auto-sync**: Background watcher detects file changes and re-indexes automatically
- **Route nodes**: REST endpoints are first-class graph entities
- **CLI mode**: `project-mind-mcp cli search_graph '{"project": "my-project", "name_pattern": ".*Handler.*"}'`
...
`.project-mind-mcp/graph.db.zst` is a zstd-compressed snapshot of the knowledge graph that lives next to your source. When you index, the artifact is written or refreshed; when a teammate clones the repo and runs `project-mind-mcp` for the first time, the artifact is decompressed and incremental indexing fills in their local diff.
...
- **Optional**: never committed unless you want it. Add `.project-mind-mcp/` to `.gitignore` if you prefer everyone to reindex from scratch.
...
project-mind-mcp is a **structural analysis backend** — it builds and queries the knowledge graph. It does **not** include an LLM. Instead, it relies on your MCP client (Claude Code, or any MCP-compatible agent) to be the intelligence layer.
...
project-mind-mcp: executes graph query, returns structured results
...
**Token efficiency**: Five structural queries consumed ~3,400 tokens via project-mind-mcp versus ~412,000 tokens via file-by-file grep exploration — a **99.2% reduction**.

## Troubleshooting & Diagnostics

project-mind-mcp runs **100% locally and collects no telemetry** — your code, queries, environment, and usage never leave your machine. That privacy guarantee also means that when you hit something we can't reproduce on our side (a slow memory climb over hours, a performance regression, a leak that only appears after days of real use), **we have no data at all unless you choose to send it.** Here is how to capture it yourself.

### Capture a diagnostics log

Set `PMM_DIAGNOSTICS=1` before the first daemon-backed MCP session starts, then reproduce the problem (let it run as long as it takes — a slow leak needs time to show in the trend). The shared daemon captures this setting from the session that starts it. If it is already running, close all daemon-backed sessions so it exits before changing the setting. The daemon creates a fresh owner-private `pmm-diagnostics-<pid>-<random>` directory below the system temp directory (`$TMPDIR` or `/tmp` on macOS/Linux, `%TEMP%` on Windows). The exact paths are recorded by the `diagnostics.start` event in `${PMM_CACHE_DIR}/logs/pmm-daemon.log`:
...
|----------|----------|---------------|
| macOS (Apple Silicon) | `project-mind-mcp-darwin-arm64.tar.gz` | `project-mind-mcp-ui-darwin-arm64.tar.gz` |
| macOS (Intel) | `project-mind-mcp-darwin-amd64.tar.gz` | `project-mind-mcp-ui-darwin-amd64.tar.gz` |
| Linux (x86_64) | `project-mind-mcp-linux-amd64.tar.gz` | `project-mind-mcp-ui-linux-amd64.tar.gz` |
| Linux (ARM64) | `project-mind-mcp-linux-arm64.tar.gz` | `project-mind-mcp-ui-linux-arm64.tar.gz` |
| Windows (x86_64) | `project-mind-mcp-windows-amd64.zip` | `project-mind-mcp-ui-windows-amd64.zip` |
...
```bash
curl -fsSL https://raw.githubusercontent.com/usman8786/project-mind-mcp/main/scripts/setup.sh | bash
```

**Windows (PowerShell):**

```powershell
irm https://raw.githubusercontent.com/usman8786/project-mind-mcp/main/scripts/setup-windows.ps1 | iex
```
...
```bash
yay -S project-mind-mcp-bin
```

```bash
paru -S project-mind-mcp-bin
```

The `project-mind-mcp-bin` package is available at: https://aur.archlinux.org/packages/project-mind-mcp-bin

### Install via Claude Code

```
You: "Install this MCP server: https://github.com/usman8786/project-mind-mcp"
```
...
```bash
git clone https://github.com/usman8786/project-mind-mcp.git
cd project-mind-mcp
scripts/build.sh                    # standard binary
scripts/build.sh --with-ui          # with graph visualization
# Binary at: build/c/project-mind-mcp   (project-mind-mcp.exe on Windows)
```
...
```json
{
  "mcpServers": {
    "project-mind-mcp": {
      "command": "/path/to/project-mind-mcp",
      "args": []
    }
  }
}
...
Restart your agent. Verify with `/mcp` — you should see `project-mind-mcp` with 15 tools.
...
| OpenHands | Detected | `.openhands/mcp.json` | Shared `.agents/skills/project-mind/SKILL.md` |
...
| Warp | Detected, skill only | UI, Warp Drive, or per invocation (manual) | Shared `~/.agents/skills/project-mind/SKILL.md` |
...
| Amp | Detected | `~/.config/agents/skills/project-mind/mcp.json` | Colocated skill + `~/.config/amp/AGENTS.md`; no plugin |
...
| Continue / cn | Conditional | Existing `~/.continue/config.yaml` or `$PMM_CONTINUE_CONFIG_PATH` | MCP only |
| Visual Studio | Conditional, Windows | `~/.mcp.json` | MCP only |
| TRAE | Conditional | Existing `$PMM_TRAE_CONFIG_PATH` | MCP only |
| Roo Code | Conditional | Existing `$PMM_ROO_CONFIG_PATH` | MCP only |
...
| Sourcegraph Cody | Explicit opt-in | Existing `$PMM_CODY_CONFIG_PATH` | MCP only |
...
OpenClaw reinjects the `Codebase Knowledge Graph (project-mind-mcp)` AGENTS
...
The installed Claude shim is named `pmm-code-discovery-gate` for backward
...
```bash
project-mind-mcp cli index_repository --repo-path /path/to/repo
project-mind-mcp cli list_projects

# Use the "name" returned by list_projects as the project value.
project-mind-mcp cli search_graph --project my-project --name-pattern '.*Handler.*' --label Function
project-mind-mcp cli trace_path --project my-project --function-name Search --direction both
project-mind-mcp cli query_graph --project my-project --query 'MATCH (f:Function) RETURN f.name LIMIT 5'

# Force human-readable progress without contaminating stdout.
project-mind-mcp cli --progress index_repository --repo-path /path/to/repo
project-mind-mcp cli search_graph --project my-project --label Function | jq '.results[].name'
```
...
Layered: hardcoded patterns (`.git`, `node_modules`, etc.) → `.gitignore` hierarchy → `.pmmignore` (project-specific, gitignore syntax). Symlinks are always skipped.

See [docs/pmmignore.md](docs/pmmignore.md) for the full `.pmmignore` how-to: syntax, precedence across the ignore layers, and negation semantics.

## Configuration

```bash
project-mind-mcp config list                          # show all settings
project-mind-mcp config set auto_index true           # auto-index on session start
project-mind-mcp config set auto_index_limit 50000    # max files for auto-index
project-mind-mcp config set auto_watch false          # don't register background git watcher (default: true)
project-mind-mcp config reset auto_index              # reset to default
```
...
|----------|---------|-------------|
| `PMM_ALLOWED_ROOT` | *(unset)* | Restrict `index_repository` to paths within this directory. When set, a `repo_path` that resolves (after symlink / `..` resolution) outside this root is refused; unset imposes no restriction. Useful when the server may be driven by an untrusted caller, e.g. agentic or multi-tenant deployments. |
| `PMM_CACHE_DIR` | `~/.cache/project-mind-mcp` | Override the database storage directory. All project indexes and config are stored here. One account can use only one canonical cache root at a time; close active PMM sessions/commands before switching it. |
| `PMM_DIAGNOSTICS` | `false` | Set to `1` or `true` to enable the shared daemon's periodic `snapshot.json` and retained `trajectory.ndjson` below a fresh owner-private directory in the system temp directory. Exact paths are logged by `diagnostics.start`. |
| `PMM_DOWNLOAD_URL` | *(GitHub releases)* | Override the download URL for updates. Used for testing or self-hosted deployments. |
| `PMM_LOG_LEVEL` | `info` | Set the minimum log level. Accepted values (case-insensitive): `debug`, `info`, `warn`, `error`, `none` — or their numeric equivalents `0`–`4` matching the internal enum. Thin-frontend messages go to that session's stderr; detached daemon events go to `${PMM_CACHE_DIR}/logs/pmm-daemon.log`. Stdout is reserved for MCP JSON-RPC. |
| `PMM_WORKERS` | *(detected)* | Override the parallel-indexing worker count returned by `pmm_default_worker_count`. Useful inside containers where `sysconf(_SC_NPROCESSORS_ONLN)` reports host CPUs rather than the cgroup's effective quota. Range 1–256; invalid values are ignored with a warning. |
| `PMM_MEM_BUDGET_MB` | *(detected)* | Override the in-memory graph budget with an explicit cap in MiB, taking precedence over the `ram_fraction × total_RAM` default. Useful on bare-metal hosts without a cgroup limit, or to pin a budget *below* the cgroup limit so headroom is left for sibling processes. Must be a positive integer; it is clamped to detected total RAM (logged as `mem.budget.clamped`), and non-numeric or non-positive values are ignored with a warning (`mem.budget.env.invalid`). |
| `PMM_DUMP_VERIFY_MIN_RATIO` | `0.5` | After indexing, compare persisted SQLite node count to the in-memory dump count. When persisted nodes fall below this fraction of committed nodes (and committed > 50), `index_repository` returns `status:"degraded"` instead of silent `indexed`. Range 0–1; set `0` to disable. Invalid values are ignored with a warning. |

Environment used by daemon-owned components—such as diagnostics, daemon logging, and process-wide indexing resource limits—is captured from the first daemon-backed session that starts the daemon. Later sessions join that process and cannot replace those values. To change them, close all daemon-backed sessions, update the relevant agent configurations consistently, and restart a session. `PMM_ALLOWED_ROOT` remains session-specific, a conflicting `PMM_CACHE_DIR` is rejected, and one-shot CLI commands read their own environment without starting the daemon.

```bash
# Store indexes in a custom directory
export PMM_CACHE_DIR=~/my-projects/pmm-data
```
...
```json
// .project-mind-mcp.json
{"extra_extensions": {".blade.php": "php", ".mjs": "javascript"}}
```

**Global** (applies to all projects):
```json
// ~/.config/project-mind-mcp/config.json  (or $XDG_CONFIG_HOME/...)
...
SQLite databases stored at `~/.cache/project-mind-mcp/`. Persists across restarts (WAL mode, ACID-safe). To reset: `rm -rf ~/.cache/project-mind-mcp/`.
...
project-mind-mcp ships a **lightweight C implementation of language type-resolution algorithms, structurally inspired by and compatible with major language servers** (tsserver / typescript-go, pyright, gopls, Roslyn, Eclipse JDT, rust-analyzer), embedded directly into the static binary. No language server process, no per-project setup, no API key. We call this layer **Hybrid LSP**: it runs alongside tree-sitter on every parse and refines `CALLS`, `USAGE`, and `RESOLVED_CALLS` edges with type information, so the resulting graph mirrors what an IDE "Go to Definition" would resolve.
...
src/
  ...
  discover/           File discovery (.gitignore, .pmmignore, symlink handling)
...
internal/pmm/         Vendored tree-sitter grammars (158 languages) + AST extraction engine
```
...
- **SLSA Level 3** — cryptographic build provenance generated by the trusted GitHub Actions build workflow; verify with `gh attestation verify <file> --repo usman8786/project-mind-mcp --signer-workflow usman8786/project-mind-mcp/.github/workflows/_build.yml`
