...
# install.sh — One-line installer for project-mind-mcp.
#
# Usage:
...
#
# Environment:
#   PMM_DOWNLOAD_URL  Override base URL for downloads (for testing)
...
VARIANT="standard"
SKIP_CONFIG=false
PMM_DOWNLOAD_URL="${PMM_DOWNLOAD_URL:-https://github.com/${REPO}/releases/latest/download}"
...
if [[ "$PMM_DOWNLOAD_URL" == https://* ]]; then
    PMM_DOWNLOAD_LOOPBACK=false
elif is_loopback_http_url "$PMM_DOWNLOAD_URL"; then
    PMM_DOWNLOAD_LOOPBACK=true
else
    echo "error: refusing non-HTTPS download URL: $PMM_DOWNLOAD_URL" >&2
    exit 1
fi
...
download_file() {
    ...
    local destination="$2"
    local progress="$3"
    if [ "$PMM_DOWNLOAD_LOOPBACK" = true ]; then
...
echo "project-mind-mcp installer"
echo "  os:      $OS"
echo "  arch:    $ARCH"
echo "  variant: $VARIANT"
echo "  target:  $INSTALL_DIR/project-mind-mcp"
echo ""
...
if [ "$VARIANT" = "ui" ]; then
    ARCHIVE="project-mind-mcp-ui-${OS}-${ARCH}${PORTABLE}.${EXT}"
else
    ARCHIVE="project-mind-mcp-${OS}-${ARCH}${PORTABLE}.${EXT}"
fi

URL="${PMM_DOWNLOAD_URL}/${ARCHIVE}"
...
# Checksum verification is mandatory. Activation must never stop running PMM
# sessions for a candidate whose published digest was not positively verified.
CHECKSUM_URL="${PMM_DOWNLOAD_URL}/checksums.txt"
...
DEST="$INSTALL_DIR/project-mind-mcp"
...
echo ""
echo "Done! Restart your coding agent to start using project-mind-mcp."
