...
# reason to throttle it. The PMM_LOCAL_CI_CPUS knob survives only for the
...
first — PMM_SKIP_PREFLIGHT=1 skips it, PMM_PREFLIGHT_ARGS=--deep widens it.
...
Legs:
  ...
  perf            arm64 incremental-perf leg (PMM_SKIP_PERF unset)
  ...
  soak-linux      both CI soak legs (quick + #581 query-leak) via
                  scripts/soak-legs.sh; PMM_SOAK_MINUTES per leg (default 10)
  soak-windows    delegates to the real-Windows VM (win.sh soak 10)

Environment:
  PMM_SOAK_MINUTES     soak-linux duration per leg (default 10)
  PMM_LOCAL_CI_CPUS    opt-in CPU cap for smoke/soak starvation checks only
                       (the regular suite always runs full-parallel)
  PMM_SKIP_PREFLIGHT=1 / PMM_PREFLIGHT_ARGS=--deep   preflight controls
...
# product failure deep inside an install path. PMM_PREFLIGHT_ARGS=--deep also
# reclaims the build cache; PMM_SKIP_PREFLIGHT=1 opts out for a quick re-run.
if [ "${PMM_SKIP_PREFLIGHT:-0}" != "1" ]; then
    # shellcheck disable=SC2086  # deliberate word-splitting of opt-in flags
    "$ROOT/scripts/ci/preflight-docker.sh" ${PMM_PREFLIGHT_ARGS:-}
fi

case "${1:-full}" in
    full)
        echo "=== Linux arm64: test + build ==="
        $COMPOSE run --rm -e PMM_SKIP_PERF=1 test
    ...
    smoke)
        ...
        ;;
    soak-linux)
        # Both CI legs (quick + #581 query-leak). Duration via PMM_SOAK_MINUTES,
        ...
        # the fast ladder.
        echo "=== Linux arm64: soak (quick + query-leak, ${PMM_SOAK_MINUTES:-10}m each) ==="
        $COMPOSE run --rm soak
        ;;
    portable)
        echo "=== Linux portable: Alpine static build + smoke ==="
    ...
    portable-test)
        ...
        $COMPOSE run --rm -e PMM_SKIP_PERF=1 test-portable
        ;;
    windows)
    ...
    amd64)
        ...
        $COMPOSE run --rm -e PMM_SKIP_PERF=1 test-amd64
