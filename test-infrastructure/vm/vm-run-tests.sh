...
Environment:
  PMM_VM_RUNNER      sanitizer-variant runner path (ubsan/trap-ubsan builds);
                     switches to direct-runner mode for those iteration tools.
  PMM_VM_SOAK_BINARY product binary for --soak (default build/c/project-mind-mcp.exe)
  PMM_VM_TEST_LOG    VM-side log path (default /tmp/win-test.log)
...
RUNNER="${PMM_VM_RUNNER:-}"
LOG="${PMM_VM_TEST_LOG:-/tmp/win-test.log}"
...
if [ "$1" = "--soak" ]; then
    ...
    binary="${PMM_VM_SOAK_BINARY:-build/c/project-mind-mcp.exe}"
    [ -x "$binary" ] || { echo "ERROR: binary '$binary' missing — build first" >&2; exit 2; }
    artifact="$binary"
elif [ -n "$RUNNER" ]; then
    # Explicit PMM_VM_RUNNER = the sanitizer-variant iteration mode (ubsan /
...
# profile carry no Authenticated-Users ACE, but C:\pmm (like CI's workspace
...
# Only an explicit PMM_VM_RUNNER (sanitizer-variant builds) bypasses test.sh.
if [ -n "$RUNNER" ]; then
    if [ "$1" = "--par" ]; then
