...
Host-local config (never committed): `~/.claude/pmm-vm/config` with
`PMM_VM_HOST=<ip>`, `PMM_VM_USER=<user>`, and the
`PMM_VM_HOST_KEY_SHA256=<SHA256:...>` fingerprint printed by the bootstrap.
`PMM_VM_BRANCH=<branch>` is optional; both drivers otherwise use the current
...
`~/.claude/pmm-vm/id_ed25519` (generate: `ssh-keygen -t ed25519 -N "" -C
claude-pmm-vm -f ~/.claude/pmm-vm/id_ed25519`; include the public half beside
...
7. Get `windows-bootstrap.ps1` into the VM via the ISO trick in its header
   ...
   run it, note the printed ip/user, write `~/.claude/pmm-vm/config`,
...
vm/win.sh sh "cd /c/pmm && gdb ..."   # anything, interactively
vm/win.sh push-file src/cli/cli.c /c/pmm/src/cli/cli.c   # WIP iteration
```
