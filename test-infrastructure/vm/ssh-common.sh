...
pmm_vm_require_safe_branch() {
    local branch="${1-}"
    if [[ ! "$branch" =~ ^[A-Za-z0-9][A-Za-z0-9._/-]*$ ]] ||
        ! git check-ref-format --branch "$branch" >/dev/null 2>&1; then
        echo "FATAL: PMM_VM_BRANCH is not a safe Git branch name: $branch" >&2
        return 1
    fi
}

pmm_vm_prepare_known_hosts() {
    local host="$1"
    local expected="$2"
    local actual

    if [[ ! "$expected" =~ ^SHA256:[A-Za-z0-9+/]{43}$ ]]; then
        echo "FATAL: PMM_VM_HOST_KEY_SHA256 is missing or malformed." >&2
        echo "  Copy the SHA256:... Ed25519 fingerprint printed by windows-bootstrap.ps1." >&2
        return 1
    fi

    PMM_VM_KNOWN_HOSTS="$(mktemp "${TMPDIR:-/tmp}/pmm-vm-known-hosts.XXXXXX")"
    chmod 600 "$PMM_VM_KNOWN_HOSTS"
    if ! ssh-keyscan -T 10 -t ed25519 "$host" >"$PMM_VM_KNOWN_HOSTS" 2>/dev/null; then
        echo "FATAL: could not read the Windows VM Ed25519 host key at $host." >&2
        pmm_vm_cleanup_known_hosts
        return 1
    fi
    actual="$(ssh-keygen -lf "$PMM_VM_KNOWN_HOSTS" -E sha256 2>/dev/null | awk '{print $2}')"
...
pmm_vm_cleanup_known_hosts() {
    if [ -n "${PMM_VM_KNOWN_HOSTS:-}" ]; then
        rm -f -- "$PMM_VM_KNOWN_HOSTS"
        PMM_VM_KNOWN_HOSTS=""
    fi
}
...
# Usage: pmm_vm_run_powershell '<source>' "${SSH[@]}"
pmm_vm_run_powershell() {
    local source="${1-}"
    shift || true
    if [ "$#" -eq 0 ]; then
...
pmm_vm_sync_windows_clock() {
    ...
    for attempt in 1 2 3; do
        ...
        if ! guest_output="$(pmm_vm_run_powershell "$powershell_source" "$@")"; then
...
pmm_vm_write_untracked_manifest() {
    local root="$1"
    local manifest="$2"
    local symlinks
    local entry
    ...
    symlinks="$(mktemp "${TMPDIR:-/tmp}/pmm-vm-symlinks.XXXXXX")"
    : >"$manifest"
