...
# machine). Config: ~/.claude/pmm-vm/config (PMM_VM_HOST, PMM_VM_USER),
# key: ~/.claude/pmm-vm/id_ed25519. Provision first: provision-windows.sh.
#
# Usage:
...
temp root). Config: ~/.claude/pmm-vm/config (PMM_VM_HOST, PMM_VM_USER,
PMM_VM_HOST_KEY_SHA256); key: ~/.claude/pmm-vm/id_ed25519; first-time setup:
...
Environment:
  PMM_VM_BRANCH          branch for update (default: current local branch)
  PMM_VM_MIN_FREE_GB     preflight free-disk floor (default 14, runner spec)
  PMM_VM_SKIP_PREFLIGHT=1  bootstrap-only escape (checkout predates the script)
...
CONFIG="${HOME}/.claude/pmm-vm/config"
KEY="${HOME}/.claude/pmm-vm/id_ed25519"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# The fixed host-local config is intentionally outside this repository.
# shellcheck source=/dev/null
[ -f "$CONFIG" ] && . "$CONFIG"
HOST="${PMM_VM_HOST:?set PMM_VM_HOST in ~/.claude/pmm-vm/config}"
USER_="${PMM_VM_USER:-test}"
HOST_KEY="${PMM_VM_HOST_KEY_SHA256:?set PMM_VM_HOST_KEY_SHA256 in ~/.claude/pmm-vm/config}"
LOCAL_BRANCH="$(git -C "$ROOT" branch --show-current)"
BRANCH="${PMM_VM_BRANCH:-${LOCAL_BRANCH:-main}}"
...
pmm_vm_require_safe_branch "$BRANCH"
pmm_vm_prepare_known_hosts "$HOST" "$HOST_KEY"
WIN_MANIFEST=""
WIN_ARCHIVE=""
WIN_PATCH=""
win_cleanup() {
    ...
    pmm_vm_cleanup_known_hosts
}
trap win_cleanup EXIT
SSH_OPTIONS=(-i "$KEY" -o IdentitiesOnly=yes -o HostKeyAlgorithms=ssh-ed25519 \
             -o StrictHostKeyChecking=yes -o UserKnownHostsFile="$PMM_VM_KNOWN_HOSTS" \
...
# PMM_VM_SKIP_PREFLIGHT=1 exists for ONE case: bootstrapping a VM whose
...
vm_preflight() {
    [ "${PMM_VM_SKIP_PREFLIGHT:-0}" = "1" ] && {
        echo "=== win.sh: preflight SKIPPED (PMM_VM_SKIP_PREFLIGHT=1) ==="
        return 0
    }
    vm_cmd "powershell -NoProfile -ExecutionPolicy Bypass -File \
C:\\pmm\\scripts\\ci\\clean-test-residue.ps1 -MinFreeGB ${PMM_VM_MIN_FREE_GB:-14}"
    ...
    vm_cmd "powershell -NoProfile -ExecutionPolicy Bypass -File \
C:\\pmm\\scripts\\ci\\ensure-defender.ps1"
}

cmd="${1:-status}"; shift || true
case "$cmd" in
status | help | -h | --help) ;;
*) pmm_vm_sync_windows_clock "${SSH[@]}" ;;
esac
...
status)
    ...
    vm clangarm64 "cd /c/pmm 2>/dev/null && git log --oneline -1 && ls -la build/c/project-mind-mcp.exe build/c/test-runner.exe 2>/dev/null || echo 'repo/build missing — run provision-windows.sh'"
    ;;
update)
    vm clangarm64 "cd /c/pmm && git fetch origin ${BRANCH} && git reset --hard FETCH_HEAD && git clean -fdx && git log --oneline -1"
    exec "$0" build
    ;;
sync)
    local_head="$(git -C "$ROOT" rev-parse --verify HEAD)"
    WIN_MANIFEST="$(mktemp "${TMPDIR:-/tmp}/pmm-vm-manifest.XXXXXX")"
    WIN_ARCHIVE="$(mktemp "${TMPDIR:-/tmp}/pmm-vm-worktree.XXXXXX.tar")"
    WIN_PATCH="$(mktemp "${TMPDIR:-/tmp}/pmm-vm-worktree.XXXXXX.patch")"
    git -C "$ROOT" diff --binary --full-index HEAD -- >"$WIN_PATCH"
    pmm_vm_write_untracked_manifest "$ROOT" "$WIN_MANIFEST"
    COPYFILE_DISABLE=1 tar --no-xattrs --no-mac-metadata \
        -C "$ROOT" --null -T "$WIN_MANIFEST" -cf "$WIN_ARCHIVE"
    remote_head="$(vm clangarm64 "cd /c/pmm && git rev-parse --verify HEAD")"
    ...
    vm clangarm64 \
        "cd /c/pmm && git reset --hard '$local_head' && git clean -fdx"
    if [ -s "$WIN_PATCH" ]; then
        "${SSH[@]}" \
            'C:\msys64\msys2_shell.cmd -defterm -no-start -clangarm64 -c "cd /c/pmm && git apply --binary --whitespace=nowarn -"' \
            <"$WIN_PATCH"
    fi
    if [ -s "$WIN_MANIFEST" ]; then
        "${SSH[@]}" \
            'C:\msys64\msys2_shell.cmd -defterm -no-start -clangarm64 -c "cd /c/pmm && tar -xf -"' \
            <"$WIN_ARCHIVE"
    fi
    vm clangarm64 "cd /c/pmm && git status --short --branch"
    win_cleanup
    exec "$0" build
    ;;
build)
    ...
    vm clangarm64 "cd /c/pmm && scripts/build.sh CC=clang CXX=clang++ > /tmp/win-build.log 2>&1 && echo BUILD_OK || (echo BUILD_FAIL; tail -20 /tmp/win-build.log; exit 1)"
    ;;
test)
    ...
    vm clangarm64 "cd /c/pmm && bash test-infrastructure/vm/vm-run-tests.sh $*"
    ;;
guards)
    ...
    # MSYS2's mingw node.
    vm clangarm64 "export PATH=/c/node:\$PATH && cd /c/pmm && scripts/build.sh --with-ui CC=clang CXX=clang++ SANITIZE= BUILD_DIR=build/guards"
    ...
    vm_cmd "cd /d C:\\pmm && set PATH=C:\\msys64\\clangarm64\\bin;C:\\msys64\\usr\\bin;%PATH%&& powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\test-windows.ps1 -GuardsOnly -Binary build\\guards\\project-mind-mcp.exe -Make C:\\msys64\\usr\\bin\\make.exe"
    ;;
smoke-install)
    ...
    vm clangarm64 "cd /c/pmm && scripts/build.sh CC=clang CXX=clang++ && bash test-infrastructure/vm/vm-smoke.sh"
    ;;
smoke-artifact)
    ...
    vm clangarm64 "cd /c/pmm && bash scripts/ci/smoke-artifact.sh windows arm64 CC=clang CXX=clang++"
    ;;
soak)
    duration="${1:-10}"
    case "$duration" in
    ...
    vm clangarm64 "cd /c/pmm && PMM_VM_TEST_LOG=/tmp/win-soak.log bash \
...
ubsan-build)
    ...
    vm clang64 "cd /c/pmm && make -j${JOBS} -f Makefile.pmm CC=clang CXX=clang++ SANITIZE='-fsanitize=undefined -fno-omit-frame-pointer' build/c/test-runner > /tmp/win-ubsan-build.log 2>&1 && echo UBSAN_BUILD_OK || (echo UBSAN_BUILD_FAIL; tail -20 /tmp/win-ubsan-build.log; exit 1)"
    ;;
ubsan-test)
    [ $# -ge 1 ] || { echo "usage: win.sh ubsan-test <suite...>" >&2; exit 2; }
    vm clang64 "cd /c/pmm && PMM_VM_TEST_LOG=/tmp/win-ubsan-test.log bash test-infrastructure/vm/vm-run-tests.sh $*"
    ;;
trap-ubsan-build)
...
trap-ubsan-test)
    ...
    vm clangarm64 "cd /c/pmm && PMM_VM_RUNNER=build/trap-ubsan/test-runner PMM_VM_TEST_LOG=/tmp/win-trap-ubsan-test.log bash test-infrastructure/vm/vm-run-tests.sh $*"
    ;;
pageheap)
...
test-par)
    ...
    vm clangarm64 "cd /c/pmm && PMM_VM_TEST_LOG=/tmp/win-test-par.log bash test-infrastructure/vm/vm-run-tests.sh --par"
    ;;
help | -h | --help)
    print_help
    ;;
