...
Environment:
  ...
  PMM_SMOKE_ARTIFACT_DIR
                  Release mode: an EXTRACTED windows release artifact
                  (project-mind-mcp.exe + LICENSE + install.ps1 +
...
ARTIFACT_DIR="${PMM_SMOKE_ARTIFACT_DIR:-}"
if [ -n "$ARTIFACT_DIR" ]; then
    ARTIFACT_DIR="$(cd "$ARTIFACT_DIR" && pwd)"
    for required in project-mind-mcp.exe \
    ...
    BINARY_SRC="$ARTIFACT_DIR/project-mind-mcp.exe"
else
    BINARY_SRC="build/c/project-mind-mcp.exe"
...
cp "$SMOKE_DIR/project-mind-mcp.exe" "$FIXTURE_DIR/"
...
EXPECTED_ARTIFACT="project-mind-mcp${SUFFIX}-windows-${SMOKE_ARCH}.zip"
(
    cd "$FIXTURE_DIR"
    zip -q "$EXPECTED_ARTIFACT" \
        project-mind-mcp.exe LICENSE install.ps1 THIRD_PARTY_NOTICES.md
    if [ -n "$SUFFIX" ]; then
        cp "$EXPECTED_ARTIFACT" "project-mind-mcp-windows-${SMOKE_ARCH}.zip"
    fi
...
env \
    ...
    -u GLAB_CONFIG_DIR \
    -u KIMI_CODE_HOME \
    -u PMM_CONTINUE_CONFIG_PATH \
    -u PMM_TRAE_CONFIG_PATH \
    -u PMM_ROO_CONFIG_PATH \
    -u PMM_CODY_CONFIG_PATH \
    -u PMM_TEST_WINDOWS_USER_PATH_RUN_ID \
    ...
    PMM_TEST_WINDOWS_USER_PATH_RUN_ID="$PATH_RUN_ID" \
    ...
    scripts/smoke-test.sh "$SMOKE_DIR/project-mind-mcp.exe"
