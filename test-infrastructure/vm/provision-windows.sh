...
#   2. Host config in ~/.claude/pmm-vm/config (KEY=VALUE), values printed by
#      windows-bootstrap.ps1 at the end of its run:
#        PMM_VM_HOST=<vm ip>
#        PMM_VM_USER=<vm user>
#        PMM_VM_HOST_KEY_SHA256=<SHA256:... host-key fingerprint>
#      Key at ~/.claude/pmm-vm/id_ed25519 (see README.md to generate your own;
...
REPO_URL="${PMM_VM_REPO:-https://github.com/usman8786/project-mind-mcp.git}"
...
trap pmm_vm_cleanup_known_hosts EXIT
...
vm_powershell() { pmm_vm_run_powershell "$1" "${SSH[@]}"; }
...
step "4/6 repo clone/update -> /c/pmm @ ${BRANCH}"
if vm "clangarm64" "test -d /c/pmm/.git && echo REPO_PRESENT" | grep -q REPO_PRESENT; then
    vm "clangarm64" "cd /c/pmm && git fetch origin ${BRANCH} && git reset --hard FETCH_HEAD && git clean -fdx && git log --oneline -1"
else
    vm "clangarm64" "git clone --branch ${BRANCH} --single-branch --depth 200 ${REPO_URL} /c/pmm && cd /c/pmm && git log --oneline -1"
fi

step "5/6 build: native ARM64 binary + test-runner (no ASan on arm64)"
vm "clangarm64" "cd /c/pmm && make -j${JOBS} -f Makefile.pmm CC=clang CXX=clang++ SANITIZE= pmm build/c/test-runner > /tmp/provision-build.log 2>&1 && echo BUILD_OK || (echo BUILD_FAIL; tail -15 /tmp/provision-build.log; exit 1)"

step "6/6 smoke: binary + test-runner start"
vm "clangarm64" "cd /c/pmm && ./build/c/project-mind-mcp.exe --version && ./build/c/test-runner --list-suites | head -3"
