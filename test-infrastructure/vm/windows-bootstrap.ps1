...
#   cp ~/.claude/pmm-vm/id_ed25519.pub /tmp/cbmiso/host-key.pub
...
#   hdiutil makehybrid -iso -joliet -default-volume-name CBMSETUP -o ~/pmm-setup.iso /tmp/cbmiso
...
New-NetFirewallRule -Name pmm-sshd -DisplayName "OpenSSH (pmm test VM)" `
...
Write-Host "On the host, write ~/.claude/pmm-vm/config with:"
Write-Host "  PMM_VM_HOST=$ip"
Write-Host ("  PMM_VM_USER=" + $env:USERNAME)
Write-Host "  PMM_VM_HOST_KEY_SHA256=<copy the SHA256:... value above>"
