 ...
 * Cypher queries against the pmm_store graph database.
 *
 * Supported syntax:
...
/* Tokenize a Cypher query string. Caller must call pmm_lex_free(). */
...
typedef struct {
    pmm_node_pattern_t *nodes;
    int node_count;
    pmm_rel_pattern_t *rels;
    int rel_count;
} pmm_pattern_t;
...
typedef struct pmm_expr pmm_expr_t;
struct pmm_expr {
    pmm_expr_type_t type;
    pmm_condition_t cond; /* leaf (EXPR_CONDITION only) */
    pmm_expr_t *left;     /* AND/OR/XOR left; NOT child */
    pmm_expr_t *right;    /* AND/OR/XOR right; NULL for NOT */
};

typedef struct {
    pmm_expr_t *root; /* expression tree (NULL = use legacy conditions) */
    /* Legacy flat model — kept during migration, removed after Phase 2 */
    pmm_condition_t *conditions;
    int count;
...
typedef struct {
    pmm_expr_t *when_expr; /* condition */
    const char *then_val;  /* result if true */
} pmm_case_branch_t;

typedef struct {
    pmm_case_branch_t *branches;
    int branch_count;
...
typedef struct {
    ...
    pmm_case_expr_t *kase; /* CASE expression (NULL if not CASE) */
    pmm_func_arg_t *args;  /* args for a multi-argument function (NULL if none) */
    int arg_count;
} pmm_return_item_t;

typedef struct {
    pmm_return_item_t *items;
    int count;
    bool distinct;
    bool star;             /* RETURN * */
...
/* Full query AST */
typedef struct pmm_query pmm_query_t;
struct pmm_query {
    pmm_pattern_t *patterns; /* array of patterns (first = main MATCH) */
    int pattern_count;
    bool *pattern_optional;              /* pattern_optional[i] = true → OPTIONAL MATCH */
    pmm_where_clause_t *where;           /* NULL if no WHERE */
    pmm_return_clause_t *with_clause;    /* WITH clause (NULL if none) */
    pmm_where_clause_t *post_with_where; /* WHERE after WITH */
    pmm_return_clause_t *ret;            /* NULL if no RETURN */
    pmm_query_t *union_next;             /* next query in UNION chain (NULL if none) */
...
#define pmm_query_pattern(q) ((q)->patterns[0])
...
/* Parse tokens into AST. Caller must call pmm_parse_free(). */
int pmm_parse(const pmm_token_t *tokens, int token_count, pmm_parse_result_t *out);
