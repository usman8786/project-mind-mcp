 ...
 *
 * Translates a subset of Cypher into SQL queries against pmm_store.
...
static void lex_push(pmm_lex_result_t *r, pmm_token_type_t type, const char *text, int pos) {
    if (r->count >= r->capacity) {
        r->capacity = r->capacity ? r->capacity * PAIR_LEN : PMM_SZ_32;
        r->tokens = safe_realloc(r->tokens, r->capacity * sizeof(pmm_token_t));
    }
    r->tokens[r->count++] = (pmm_token_t){.type = type, .text = heap_strdup(text), .pos = pos};
}

static void lex_push_n(pmm_lex_result_t *r, pmm_token_type_t type, const char *start, size_t len,
                       int pos) {
    if (r->count >= r->capacity) {
        r->capacity = r->capacity ? r->capacity * PAIR_LEN : PMM_SZ_32;
        r->tokens = safe_realloc(r->tokens, r->capacity * sizeof(pmm_token_t));
    ...
    r->tokens[r->count++] =
        (pmm_token_t){.type = type, .text = heap_strndup(start, len), .pos = pos};
}
...
static void lex_string_literal(const char *input, int len, int *pos, char quote,
                               pmm_lex_result_t *out) {
    int start = *pos;
    char buf[PMM_SZ_4K];
    int blen = 0;
    const int max_blen = PMM_SZ_4K - 1;
...
static pmm_token_type_t keyword_lookup(const char *word) {
...
static bool lex_try_two_char(const char *input, int len, int *i, pmm_lex_result_t *out) {
    static const struct {
        char c1, c2;
        pmm_token_type_t type;
        const char *text;
...
static pmm_token_type_t lex_single_char(char c) {
    switch (c) {
    case '(':
        return TOK_LPAREN;
    case ')':
...
static bool lex_try_ident(const char *input, int len, int *i, pmm_lex_result_t *out) {
    char c = input[*i];
    ...
    word[wlen] = '\0';
    pmm_token_type_t type = keyword_lookup(word);
    lex_push_n(out, type, input + start, *i - start, start);
    return true;
}

/* Try to lex a number starting at position i. Returns true if matched. */
static bool lex_try_number(const char *input, int len, int *i, pmm_lex_result_t *out) {
    char c = input[*i];
...
int pmm_lex(const char *input, pmm_lex_result_t *out) {
    ...
    while (i < len) {
        ...
        pmm_token_type_t stype = lex_single_char(c);
...
void pmm_lex_free(pmm_lex_result_t *r) {
    if (!r) {
        return;
    }
...
typedef struct {
    const pmm_token_t *tokens;
    int count;
    int pos;
    char error[PMM_SZ_512];
} parser_t;

static const pmm_token_t *peek(parser_t *p) {
    if (p->pos >= p->count) {
...
static const pmm_token_t *advance(parser_t *p) {
    if (p->pos >= p->count) {
...
static bool check(parser_t *p, pmm_token_type_t type) {
    return peek(p)->type == type;
}

static bool match(parser_t *p, pmm_token_type_t type) {
    if (check(p, type)) {
        advance(p);
        return true;
    }
...
static const pmm_token_t *expect(parser_t *p, pmm_token_type_t type) {
    if (check(p, type)) {
        return advance(p);
    }
...
static int parse_props(parser_t *p, pmm_prop_filter_t **out, int *count) {
    *out = NULL;
    *count = 0;
    ...
    int n = 0;
    pmm_prop_filter_t *arr = malloc(cap * sizeof(pmm_prop_filter_t));
    if (!arr) {
        return PMM_NOT_FOUND;
    }

    while (!check(p, TOK_RBRACE) && !check(p, TOK_EOF)) {
        const pmm_token_t *key = expect(p, TOK_IDENT);
        if (!key) {
            free(arr);
            return PMM_NOT_FOUND;
        }
        ...
        const pmm_token_t *val = expect(p, TOK_STRING);
        if (!val) {
            free(arr);
            return PMM_NOT_FOUND;
        }
        ...
        if (n >= cap) {
            ...
            void *tmp = realloc(arr, new_cap * sizeof(pmm_prop_filter_t));
            if (!tmp) {
                for (int i = 0; i < n; i++) {
...
static int parse_node(parser_t *p, pmm_node_pattern_t *out) {
    memset(out, 0, sizeof(*out));
    ...
    if (match(p, TOK_COLON)) {
        const pmm_token_t *label = expect(p, TOK_IDENT);
        if (!label) {
            return PMM_NOT_FOUND;
        }
        char lbuf[PMM_SZ_256];
        int ll = snprintf(lbuf, sizeof(lbuf), "%s", label->text);
        while (match(p, TOK_PIPE)) {
            const pmm_token_t *alt = expect(p, TOK_IDENT);
            if (!alt) {
                return PMM_NOT_FOUND;
            }
...
static void parse_hop_range(parser_t *p, int *min_hops, int *max_hops) {
    if (check(p, TOK_NUMBER)) {
        int val = (int)strtol(peek(p)->text, NULL, PMM_DECIMAL_BASE);
        advance(p);
        if (match(p, TOK_DOTDOT)) {
            *min_hops = val;
            *max_hops =
                check(p, TOK_NUMBER) ? (int)strtol(advance(p)->text, NULL, PMM_DECIMAL_BASE) : 0;
        } else {
            /* *N means 1..N */
            *min_hops = SKIP_ONE;
            *max_hops = val;
...
static int parse_rel_types(parser_t *p, pmm_rel_pattern_t *out) {
    int cap = CYP_INIT_CAP4;
    int n = 0;
    ...
    const pmm_token_t *t = expect(p, TOK_IDENT);
    if (!t) {
        free(types);
        return PMM_NOT_FOUND;
    }
...
static int parse_rel_bracket(parser_t *p, pmm_rel_pattern_t *out) {
    /* Optional variable */
...
static int parse_rel(parser_t *p, pmm_rel_pattern_t *out) {
    memset(out, 0, sizeof(*out));
...
static void expr_free(pmm_expr_t *e) {
    enum { EXPR_FREE_STACK = 128 };
    pmm_expr_t *stack[EXPR_FREE_STACK];
    int top = 0;
    if (e) {
        stack[top++] = e;
    }
    while (top > 0) {
        pmm_expr_t *cur = stack[--top];
...
static pmm_expr_t *expr_leaf(pmm_condition_t c) {
    pmm_expr_t *e = calloc(PMM_ALLOC_ONE, sizeof(pmm_expr_t));
    e->type = EXPR_CONDITION;
    e->cond = c;
    return e;
}

static pmm_expr_t *expr_binary(pmm_expr_type_t type, pmm_expr_t *left, pmm_expr_t *right) {
    pmm_expr_t *e = calloc(PMM_ALLOC_ONE, sizeof(pmm_expr_t));
    e->type = type;
    e->left = left;
    e->right = right;
...
static pmm_expr_t *expr_not(pmm_expr_t *child) {
    pmm_expr_t *e = calloc(PMM_ALLOC_ONE, sizeof(pmm_expr_t));
    e->type = EXPR_NOT;
    e->left = child;
    return e;
...
static const char *unsupported_clause_error(pmm_token_type_t type) {
    switch (type) {
    case TOK_CREATE:
...
static pmm_expr_t *parse_or_expr(parser_t *p);
/* Multi-arg scalar function support, shared with the RETURN-item parser (#874) */
static bool is_multiarg_func_call(parser_t *p);
static int parse_multiarg_func_item(parser_t *p, pmm_return_item_t *item);

/* Free a multi-arg function argument array. */
static void func_args_free(pmm_func_arg_t *args, int count) {
...
static void func_item_fields_free(pmm_return_item_t *item) {
...
static void cond_func_fields_free(pmm_condition_t *c) {
...
static pmm_expr_t *parse_in_list(parser_t *p, pmm_condition_t *c) {
    advance(p);
    c->op = heap_strdup("IN");
    if (!c->op) {
...
static void free_one_node_pattern(pmm_node_pattern_t *n) {
...
static void free_one_rel_pattern(pmm_rel_pattern_t *r) {
...
static pmm_expr_t *parse_exists_predicate(parser_t *p, bool negated) {
    advance(p); /* EXISTS */
    if (!match(p, TOK_LBRACE)) {
        snprintf(p->error, sizeof(p->error), "expected '{' after EXISTS at pos %d", peek(p)->pos);
        return NULL;
    }
    pmm_node_pattern_t anchor = {0};
    pmm_rel_pattern_t rel = {0};
    pmm_node_pattern_t far_node = {0};
...
static pmm_expr_t *parse_condition_op(parser_t *p, pmm_condition_t *c) {
    /* IS NULL / IS NOT NULL */
    if (check(p, TOK_IS)) {
        advance(p);
        if (match(p, TOK_NOT)) {
    ...
    /* IN [...] */
    if (check(p, TOK_IN)) {
        pmm_expr_t *e = parse_in_list(p, c);
        if (!e) {
 ...
 * Returns PMM_NOT_FOUND on error, 0 when an operator/value should follow, and
 * COND_LHS_COMPLETE when the condition is already complete (label test). */
static int parse_condition_lhs(parser_t *p, pmm_condition_t *c) {
    ...
    const pmm_token_t *var = expect(p, TOK_IDENT);
    if (!var) {
        return PMM_NOT_FOUND;
    }
    ...
    if (check(p, TOK_COLON)) {
        advance(p);
        const pmm_token_t *lbl = expect(p, TOK_IDENT);
        if (!lbl) {
            return PMM_NOT_FOUND;
        }
    ...
    if (match(p, TOK_DOT)) {
        const pmm_token_t *prop = expect(p, TOK_IDENT);
        if (!prop) {
            return PMM_NOT_FOUND;
        }
        c->variable = heap_strdup(var->text);
...
static pmm_expr_t *parse_condition_expr(parser_t *p) {
...
/* Atom: ( expr ) | condition */
static pmm_expr_t *parse_atom_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    if (match(p, TOK_LPAREN)) {
        pmm_expr_t *e = parse_or_expr(p);
        expect(p, TOK_RPAREN);
        return e;
    }
...
/* NOT: NOT atom | atom */
static pmm_expr_t *parse_not_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    if (match(p, TOK_NOT)) {
        pmm_expr_t *child = parse_not_expr(p);
...
/* AND: not (AND not)* */
static pmm_expr_t *parse_and_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    pmm_expr_t *left = parse_not_expr(p);
    if (!left) {
        return NULL;
    }
    while (check(p, TOK_AND)) {
        advance(p);
        pmm_expr_t *right = parse_not_expr(p);
        if (!right) {
            expr_free(left);
            return NULL;
        }
...
/* XOR: and (XOR and)* */
static pmm_expr_t *parse_xor_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    pmm_expr_t *left = parse_and_expr(p);
    if (!left) {
        return NULL;
    }
    while (check(p, TOK_XOR)) {
        advance(p);
        pmm_expr_t *right = parse_and_expr(p);
        if (!right) {
            expr_free(left);
            return NULL;
        }
...
/* OR: xor (OR xor)* */
static pmm_expr_t *parse_or_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    pmm_expr_t *left = parse_xor_expr(p);
    if (!left) {
        return NULL;
    }
    while (check(p, TOK_OR)) {
        advance(p);
        pmm_expr_t *right = parse_xor_expr(p);
        if (!right) {
            expr_free(left);
            return NULL;
        }
...
static int parse_where(parser_t *p, pmm_where_clause_t **out) {
    if (!match(p, TOK_WHERE)) {
        *out = NULL;
        return 0;
    }

    pmm_where_clause_t *w = calloc(PMM_ALLOC_ONE, sizeof(pmm_where_clause_t));
...
static const char *str_func_name(pmm_token_type_t t) {
    switch (t) {
    case TOK_TOLOWER:
        return "toLower";
    case TOK_TOUPPER:
...
static const char *parse_value_literal(parser_t *p) {
    ...
    if (check(p, TOK_IDENT)) {
        char buf[PMM_SZ_256];
        const pmm_token_t *v = advance(p);
        if (match(p, TOK_DOT)) {
            const pmm_token_t *pr = expect(p, TOK_IDENT);
...
static pmm_case_expr_t *parse_case_expr(parser_t *p) {
    /* CASE already consumed */
    pmm_case_expr_t *kase = calloc(PMM_ALLOC_ONE, sizeof(pmm_case_expr_t));
    if (!kase) {
        return NULL;
    }
    int bcap = CYP_INIT_CAP4;
    kase->branches = malloc(bcap * sizeof(pmm_case_branch_t));
    if (!kase->branches) {
        free(kase);
        return NULL;
    }
    ...
    while (check(p, TOK_WHEN)) {
        advance(p);
        pmm_expr_t *when = parse_or_expr(p);
        ...
        if (kase->branch_count >= bcap) {
            ...
            void *tmp = realloc(kase->branches, new_bcap * sizeof(pmm_case_branch_t));
            if (!tmp) {
                expr_free(when);
        ...
        kase->branches[kase->branch_count++] =
            (pmm_case_branch_t){.when_expr = when, .then_val = then_val};
    }
...
static int parse_var_dot_prop(parser_t *p, pmm_return_item_t *item) {
    const pmm_token_t *var = expect(p, TOK_IDENT);
    if (!var) {
        return PMM_NOT_FOUND;
    }
...
static int parse_named_func_item(parser_t *p, pmm_return_item_t *item) {
...
static int parse_func_arg(parser_t *p, pmm_func_arg_t *arg) {
...
static int parse_multiarg_func_item(parser_t *p, pmm_return_item_t *item) {
    ...
    expect(p, TOK_LPAREN);
    int cap = CYP_INIT_CAP4;
    item->args = malloc((size_t)cap * sizeof(pmm_func_arg_t));
    item->arg_count = 0;
    while (!check(p, TOK_RPAREN) && !check(p, TOK_EOF)) {
        ...
        if (item->arg_count >= cap) {
            cap *= PAIR_LEN;
            item->args = safe_realloc(item->args, (size_t)cap * sizeof(pmm_func_arg_t));
        }
...
static int parse_aggregate_item(parser_t *p, pmm_return_item_t *item) {
    pmm_token_type_t ft = peek(p)->type;
    advance(p);
    expect(p, TOK_LPAREN);
...
static int parse_string_func_item(parser_t *p, pmm_return_item_t *item) {
    pmm_token_type_t ft = peek(p)->type;
    advance(p);
    expect(p, TOK_LPAREN);
    if (parse_var_dot_prop(p, item) < 0) {
...
static int parse_return_item(parser_t *p, pmm_return_item_t *item) {
    ...
    /* Optional AS alias */
    if (match(p, TOK_AS)) {
        const pmm_token_t *alias = expect(p, TOK_IDENT);
        if (alias) {
...
static void parse_order_by_clause(parser_t *p, pmm_return_clause_t *r) {
    expect(p, TOK_BY);
    char order_buf[PMM_SZ_256];
...
static int parse_return_or_with(parser_t *p, pmm_return_clause_t **out, bool is_with) {
    pmm_token_type_t tok = (int)is_with ? TOK_WITH : TOK_RETURN;
    ...
    pmm_return_clause_t *r = calloc(PMM_ALLOC_ONE, sizeof(pmm_return_clause_t));
    ...
    r->items = malloc(cap * sizeof(pmm_return_item_t));
    ...
    do {
        ...
        if (r->count >= cap) {
            cap *= PAIR_LEN;
            r->items = safe_realloc(r->items, cap * sizeof(pmm_return_item_t));
        }
...
tail:
    ...
    /* Optional SKIP */
    if (match(p, TOK_SKIP)) {
        const pmm_token_t *num = expect(p, TOK_NUMBER);
        if (num) {
            r->skip = (int)strtol(num->text, NULL, PMM_DECIMAL_BASE);
        }
    }
    ...
    if (match(p, TOK_LIMIT)) {
        const pmm_token_t *num = expect(p, TOK_NUMBER);
        if (num) {
            r->limit = (int)strtol(num->text, NULL, PMM_DECIMAL_BASE);
        }
    }
...
/* Parse RETURN clause */
static int parse_return(parser_t *p, pmm_return_clause_t **out) {
    return parse_return_or_with(p, out, false);
}

/* Parse a single MATCH pattern into pat */
static int parse_match_pattern(parser_t *p, pmm_pattern_t *pat) {
    memset(pat, 0, sizeof(*pat));
    int node_cap = CYP_INIT_CAP4;
    int rel_cap = CYP_INIT_CAP4;
    pat->nodes = malloc(node_cap * sizeof(pmm_node_pattern_t));
    pat->rels = calloc(rel_cap, sizeof(pmm_rel_pattern_t));
    ...
    while (check(p, TOK_DASH) || check(p, TOK_LT)) {
        if (pat->rel_count >= rel_cap) {
            rel_cap *= PAIR_LEN;
            pat->rels = safe_realloc(pat->rels, rel_cap * sizeof(pmm_rel_pattern_t));
        }
        ...
        if (pat->node_count >= node_cap) {
            node_cap *= PAIR_LEN;
            pat->nodes = safe_realloc(pat->nodes, node_cap * sizeof(pmm_node_pattern_t));
        }
...
static void parse_unwind_clause(parser_t *p, pmm_query_t *q) {
    advance(p);
...
static int parse_match_chain(parser_t *p, pmm_query_t *q, int *pat_cap) {
    while (check(p, TOK_MATCH) || check(p, TOK_OPTIONAL)) {
        ...
        if (q->pattern_count >= *pat_cap) {
            *pat_cap *= PAIR_LEN;
            q->patterns = safe_realloc(q->patterns, *pat_cap * sizeof(pmm_pattern_t));
...
static int parse_post_where(parser_t *p, pmm_query_t *q, // NOLINT(misc-no-recursion)
                            int *pat_cap) {
    ...
    if (check(p, TOK_UNION)) {
        ...
        pmm_parse_result_t sub = {0};
        if (pmm_parse(&p->tokens[p->pos], p->count - p->pos, &sub) < 0) {
            if (sub.error) {
...
int pmm_parse(const pmm_token_t *tokens, int token_count, // NOLINT(misc-no-recursion)
              pmm_parse_result_t *out) {
    memset(out, 0, sizeof(*out));
    ...
    pmm_query_t *q = calloc(PMM_ALLOC_ONE, sizeof(pmm_query_t));
    ...
    q->patterns = malloc(pat_cap * sizeof(pmm_pattern_t));
...
void pmm_parse_free(pmm_parse_result_t *r) {
    if (!r) {
        return;
    }
    pmm_query_free(r->query);
    free(r->error);
    memset(r, 0, sizeof(*r));
}
...
static void free_pattern(pmm_pattern_t *pat) {
    for (int i = 0; i < pat->node_count; i++) {
        pmm_node_pattern_t *n = &pat->nodes[i];
        safe_str_free(&n->variable);
        safe_str_free(&n->label);
        for (int j = 0; j < n->prop_count; j++) {
            safe_str_free(&n->props[j].key);
    ...
    for (int i = 0; i < pat->rel_count; i++) {
        pmm_rel_pattern_t *r = &pat->rels[i];
        safe_str_free(&r->variable);
        for (int j = 0; j < r->type_count; j++) {
            safe_str_free(&r->types[j]);
        }
...
static void free_where(pmm_where_clause_t *w) {
    if (!w) {
        return;
    }
    expr_free(w->root);
...
static void free_case_expr(pmm_case_expr_t *k) {
    if (!k) {
        return;
    }
...
static void free_return_clause(pmm_return_clause_t *r) {
    if (!r) {
        return;
    }
    for (int i = 0; i < r->count; i++) {
...
void pmm_query_free(pmm_query_t *q) {
    while (q) {
        pmm_query_t *next = q->union_next;
...
int pmm_cypher_parse(const char *query, pmm_query_t **out, char **error) {
    *out = NULL;
    *error = NULL;

    pmm_lex_result_t lr = {0};
    if (pmm_lex(query, &lr) < 0 || lr.error) {
        *error = heap_strdup(lr.error ? lr.error : "lex error");
        pmm_lex_free(&lr);
        return PMM_NOT_FOUND;
    }

    pmm_parse_result_t pr = {0};
    if (pmm_parse(lr.tokens, lr.count, &pr) < 0) {
...
typedef struct {
    ...
    pmm_node_t var_nodes[CYP_MAX_VARS];  /* node data */
    int var_count;
    const char *edge_var_names[CYP_MAX_EDGE_VARS]; /* variable names (edges) */
    pmm_edge_t edge_vars[CYP_MAX_EDGE_VARS];       /* edge data */
    int edge_var_count;
    pmm_store_t *store; /* for computing in_degree/out_degree on demand */
} binding_t;

/* Return a string field from a node by property name.  NULL-safe. */
static const char *node_string_field(const pmm_node_t *n, const char *prop) {
    static const struct {
        const char *key;
        size_t offset;
    } fields[] = {
        {"name", offsetof(pmm_node_t, name)},
        {"qualified_name", offsetof(pmm_node_t, qualified_name)},
        /* Aliases: field-eval agents reach for the short names, and a miss
         * used to return a silent empty column costing a round-trip. */
        {"qn", offsetof(pmm_node_t, qualified_name)},
        {"label", offsetof(pmm_node_t, label)},
        {"file_path", offsetof(pmm_node_t, file_path)},
        {"file", offsetof(pmm_node_t, file_path)},
        {"path", offsetof(pmm_node_t, file_path)},
    };
...
static void node_fields_free(pmm_node_t *n); /* defined below; used by the stub re-fetch */

static const char *node_prop(const pmm_node_t *n, const char *prop, pmm_store_t *store) {
    if (!n || !prop) {
        return "";
    }
    ...
    if (store && (strcmp(prop, "in_degree") == 0 || strcmp(prop, "out_degree") == 0)) {
        int in_deg = 0;
        int out_deg = 0;
        pmm_store_node_degree(store, n->id, &in_deg, &out_deg);
        int val = (strcmp(prop, "in_degree") == 0) ? in_deg : out_deg;
        snprintf(out, PMM_SZ_512, "%d", val);
        return out;
    }
    ...
    if (n->properties_json && n->properties_json[0] == '{') {
        const char *v = json_extract_prop(n->properties_json, prop, out, PMM_SZ_512);
        if (v && v[0]) {
            return v;
        }
    }
    ...
    if (store && n->id > 0 && !n->file_path && !n->label) {
        pmm_node_t full = {0};
        if (pmm_store_find_node_by_id(store, n->id, &full) == PMM_STORE_OK) {
            const char *res = NULL;
            const char *rv = node_string_field(&full, prop);
            if (rv && rv[0]) {
                snprintf(out, PMM_SZ_512, "%s", rv);
                res = out;
            } else if (strcmp(prop, "start_line") == 0) {
                snprintf(out, PMM_SZ_512, "%d", full.start_line);
                res = out;
            } else if (strcmp(prop, "end_line") == 0) {
                snprintf(out, PMM_SZ_512, "%d", full.end_line);
                res = out;
            } else if (full.properties_json && full.properties_json[0] == '{') {
                const char *jv = json_extract_prop(full.properties_json, prop, out, PMM_SZ_512);
                if (jv && jv[0]) {
                    res = out;
                }
            }
 ...
 * in the same row). */
static const char *edge_prop(const pmm_edge_t *e, const char *prop) {
    if (!e || !prop) {
        return "";
    }
    ...
    static PMM_TLS char ebufs[CYP_BUF_8][PMM_SZ_512];
    static PMM_TLS int ebuf_idx = 0;
    char *buf = ebufs[ebuf_idx++ & CYP_EBUF_MASK];
    json_extract_prop(e->properties_json, prop, buf, PMM_SZ_512);
    return buf;
}

/* Find an edge variable in a binding */
static pmm_edge_t *binding_get_edge(binding_t *b, const char *var) {
...
static pmm_node_t *binding_get(binding_t *b, const char *var) {
...
static void node_deep_copy(pmm_node_t *dst, const pmm_node_t *src) {
    *dst = *src;
...
static void node_fields_free(pmm_node_t *n) {
    if (!n) {
        return;
    }
...
static void edge_deep_copy(pmm_edge_t *dst, const pmm_edge_t *src) {
    *dst = *src;
    dst->project = heap_strdup(src->project);
...
static void edge_fields_free(pmm_edge_t *e) {
...
static void binding_set_edge(binding_t *b, const char *var, const pmm_edge_t *edge) {
...
static void binding_set(binding_t *b, const char *var, const pmm_node_t *node) {
    /* Check existing — free old fields first */
    for (int i = 0; i < b->var_count; i++) {
        if (strcmp(b->var_names[i], var) == 0) {
            node_fields_free(&b->var_nodes[i]);
...
static const char *eval_multiarg_func(binding_t *b, const pmm_return_item_t *item, char *buf,
                                      size_t bufsz);

/* Resolve the actual property value for a condition from a binding */
static const char *resolve_condition_value(const pmm_condition_t *c, binding_t *b) {
    /* Multi-arg scalar function LHS: coalesce(f.depth, 0) >= 2 (#874).
    ...
    if (c->func) {
        static _Thread_local char func_buf[PMM_SZ_512];
        pmm_return_item_t item = {0};
    ...
    pmm_edge_t *e = binding_get_edge(b, c->variable);
    if (e) {
        return edge_prop(e, c->property);
    }
    pmm_node_t *n = binding_get(b, c->variable);
    if (!n) {
...
static bool eval_comparison_op(const char *op, const char *actual, const char *expected) {
    ...
    if (strcmp(op, "=~") == 0) {
        pmm_regex_t re;
        if (pmm_regcomp(&re, expected, PMM_REG_EXTENDED | PMM_REG_NOSUB) != 0) {
            return false;
        }
        int rc = pmm_regexec(&re, actual, 0, NULL, 0);
        pmm_regfree(&re);
        return rc == 0;
    }
...
static bool eval_condition(const pmm_condition_t *c, binding_t *b) {
    ...
    if (strcmp(c->op, "EXISTS") == 0) {
        ...
        if (n && b->store) {
            ...
            if (c->exists_dir != 1) { /* outbound or any */
                if (c->value) {
                    pmm_store_find_edges_by_source_type(b->store, n->id, c->value, &edges, &cnt);
                } else {
                    pmm_store_find_edges_by_source(b->store, n->id, &edges, &cnt);
                }
                result = cnt > 0;
                pmm_store_free_edges(edges, cnt);
            }
            if (!result && c->exists_dir != 0) { /* inbound or any */
                edges = NULL;
                cnt = 0;
                if (c->value) {
                    pmm_store_find_edges_by_target_type(b->store, n->id, c->value, &edges, &cnt);
                } else {
                    pmm_store_find_edges_by_target(b->store, n->id, &edges, &cnt);
                }
                result = cnt > 0;
                pmm_store_free_edges(edges, cnt);
            }
...
static bool eval_expr(const pmm_expr_t *e, binding_t *b) { // NOLINT(misc-no-recursion)
    if (!e) {
        return true;
    }
    switch (e->type) {
...
static bool eval_where(const pmm_where_clause_t *w, binding_t *b) {
    if (!w) {
        return true;
    }
    if (w->root) {
...
static bool check_inline_props(const pmm_node_t *n, const pmm_prop_filter_t *props, int count,
                               pmm_store_t *store) {
    for (int i = 0; i < count; i++) {
        const char *actual = node_prop(n, props[i].key, store);
        if (looks_like_regex(props[i].value)) {
            pmm_regex_t re;
            if (pmm_regcomp(&re, props[i].value, PMM_REG_EXTENDED | PMM_REG_NOSUB) == 0) {
                bool matched = pmm_regexec(&re, actual, 0, NULL, 0) == 0;
                pmm_regfree(&re);
                if (!matched) {
                    return false;
                }
...
static void cypher_deadline_arm(void) {
    ...
    g_cypher_deadline_ms = pmm_now_ms() + (uint64_t)budget;
}
...
static bool cypher_deadline_exceeded(void) {
    ...
    if (pmm_now_ms() >= g_cypher_deadline_ms) {
...
void pmm_cypher_test_set_deadline_ms(int64_t budget_ms) {
...
static const char *binding_get_virtual(binding_t *b, const char *var, const char *prop) {
    ...
    pmm_edge_t *e = binding_get_edge(b, var);
    if (e) {
    ...
    pmm_node_t *n = binding_get(b, var);
    if (n) {
        if (prop) {
...
static const char *apply_string_func(const char *func, const char *val, char *buf, size_t buf_sz) {
    ...
    if (strcmp(func, "toInteger") == 0) {
        char *end = NULL;
        long long v = strtoll(val, &end, PMM_DECIMAL_BASE);
        if (end == val) {
...
static const char *eval_case_expr(const pmm_case_expr_t *k, binding_t *b) {
    if (!k) {
        return "";
    }
    for (int i = 0; i < k->branch_count; i++) {
 ...
 * container is freed. */
static void scan_alternation_labels(pmm_store_t *store, const char *project, const char *labels,
                                    pmm_node_t **out_nodes, int *out_count) {
    *out_nodes = NULL;
    *out_count = 0;
    int cap = 0;
    ...
    for (char *tok = strtok_r(copy, "|", &save); tok; tok = strtok_r(NULL, "|", &save)) {
        pmm_node_t *part = NULL;
        int pc = 0;
        pmm_store_find_nodes_by_label(store, project, tok, &part, &pc);
        if (pc > 0 && part) {
            if (*out_count + pc > cap) {
                cap = (*out_count + pc) * PAIR_LEN;
                *out_nodes = safe_realloc(*out_nodes, (size_t)cap * sizeof(pmm_node_t));
            }
            memcpy(*out_nodes + *out_count, part, (size_t)pc * sizeof(pmm_node_t));
            *out_count += pc;
        }
...
static void scan_pattern_nodes(pmm_store_t *store, const char *project, int max_rows,
                               pmm_node_pattern_t *first, pmm_node_t **out_nodes, int *out_count) {
    if (first->label && strchr(first->label, '|')) {
        scan_alternation_labels(store, project, first->label, out_nodes, out_count);
    } else if (first->label) {
        pmm_store_find_nodes_by_label(store, project, first->label, out_nodes, out_count);
    } else {
        pmm_search_params_t params = {.project = project,
                                      .min_degree = CYP_FOUND_NONE,
                                      .max_degree = CYP_FOUND_NONE,
                                      .limit = max_rows * CYP_GROWTH_10};
        pmm_search_output_t sout = {0};
        pmm_store_search(store, &params, &sout);
        *out_count = sout.count;
        *out_nodes = malloc(sout.count * sizeof(pmm_node_t));
...
static void process_edges(pmm_store_t *store, pmm_edge_t *edges, int edge_count, bool inbound,
                          const pmm_node_pattern_t *target_node, binding_t *b, const char *to_var,
    ...
    pmm_node_t *bound_to = binding_get(b, to_var);
    ...
    for (int ei = 0; ei < edge_count && *new_count < max_new; ei++) {
        ...
        pmm_node_t found = {0};
        if (pmm_store_find_node_by_id(store, tid, &found) != PMM_STORE_OK) {
            continue;
        }
 ...
 * during the CURRENT execution; pmm_cypher_execute turns it into
...
static void expand_var_length(pmm_store_t *store, pmm_rel_pattern_t *rel,
                              pmm_node_pattern_t *target_node, binding_t *b, pmm_node_t *src,
     ...
     * verbatim, driving pmm_store_bfs to an unbounded hop count (#887). WARN on
     * clamp — never a silent truncation. */
    int depth_cap = pmm_cypher_max_depth();
    ...
    if (max_depth > depth_cap) {
        ...
        pmm_log_warn("cypher.depth_capped", "requested", req_buf, "cap", cap_buf);
    ...
    pmm_store_bfs(store, src->id, dir, rel->types, rel->type_count, max_depth, PMM_PERCENT, &tr);
    for (int v = 0; v < tr.visited_count && *new_count < max_new; v++) {
        pmm_node_hop_t *hop = &tr.visited[v];
...
static void expand_fixed_length(pmm_store_t *store, pmm_rel_pattern_t *rel,
                                pmm_node_pattern_t *target_node, binding_t *b, pmm_node_t *src,
                                const char *to_var, binding_t *new_bindings, int *new_count,
                                int max_new, int *match_count) {
    ...
    if (rel->type_count > 0) {
        for (int ti = 0; ti < rel->type_count; ti++) {
            ...
            int edge_count = 0;
            if (is_inbound) {
                pmm_store_find_edges_by_target_type(store, src->id, rel->types[ti], &edges,
                                                    &edge_count);
            } else {
                pmm_store_find_edges_by_source_type(store, src->id, rel->types[ti], &edges,
                                                    &edge_count);
            }
        ...
        int edge_count = 0;
        if (is_inbound) {
            pmm_store_find_edges_by_target(store, src->id, &edges, &edge_count);
        } else {
            pmm_store_find_edges_by_source(store, src->id, &edges, &edge_count);
        }
        process_edges(store, edges, edge_count, is_inbound, target_node, b, to_var, rel_var,
                      new_bindings, new_count, max_new, match_count);
        pmm_store_free_edges(edges, edge_count);
...
static void expand_pattern_rels(pmm_store_t *store, pmm_pattern_t *pat, binding_t **bindings,
    ...
    for (int ri = 0; ri < pat->rel_count; ri++) {
        ...
        pmm_rel_pattern_t *rel = &pat->rels[ri];
        pmm_node_pattern_t *target_node = &pat->nodes[ri + SKIP_ONE];
        ...
        for (int bi = 0; bi < *bind_count; bi++) {
            ...
            pmm_node_t *src = binding_get(b, *var_name);
            if (!src) {
                continue;
            }
...
static int rb_find_order_column(const result_builder_t *rb, const pmm_return_clause_t *ret) {
...
static void rb_apply_order_by(result_builder_t *rb, const pmm_return_clause_t *ret) {
    if (!ret->order_by) {
        return;
    }
    ...
    for (int i = 0; i < rb->row_count - SKIP_ONE; i++) {
        for (int j = 0; j < rb->row_count - i - SKIP_ONE; j++) {
            int cmp;
            if (numeric) {
                cmp = (int)strtol(rb->rows[j][order_col], NULL, PMM_DECIMAL_BASE) -
                      (int)strtol(rb->rows[j + SKIP_ONE][order_col], NULL, PMM_DECIMAL_BASE);
            } else {
...
static const char *node_keys_list(const pmm_node_t *n, char *buf, size_t buf_sz) {
    const struct {
        const char *k;
        bool present;
    } ks[] = {
...
static const char *eval_func_arg(binding_t *b, const pmm_func_arg_t *a) {
    if (a->literal) {
        return a->literal;
    }
...
static const char *eval_multiarg_func(binding_t *b, const pmm_return_item_t *item, char *buf,
    ...
    if (strcmp(f, "substring") == 0 && n >= 2) {
        ...
        long start = strtol(eval_func_arg(b, &item->args[1]), NULL, PMM_DECIMAL_BASE);
        ...
        if (n >= 3) {
            long len = strtol(eval_func_arg(b, &item->args[2]), NULL, PMM_DECIMAL_BASE);
            if (len < 0) {
                len = 0;
            }
            if ((size_t)len < take) {
    ...
    if ((strcmp(f, "left") == 0 || strcmp(f, "right") == 0) && n >= 2) {
        const char *s = eval_func_arg(b, &item->args[0]);
        long k = strtol(eval_func_arg(b, &item->args[1]), NULL, PMM_DECIMAL_BASE);
        if (k < 0) {
            k = 0;
        }
        size_t slen = strlen(s);
...
static const char *project_item(binding_t *b, pmm_return_item_t *item, char *func_buf,
                                size_t buf_sz) {
    if (item->kase) {
    ...
    if (item->func) {
        if (strcmp(item->func, "labels") == 0) {
            pmm_node_t *n = binding_get(b, item->variable);
            if (n && n->label) {
        ...
        if (strcmp(item->func, "type") == 0) {
            pmm_edge_t *e = binding_get_edge(b, item->variable);
...
static void with_sort_skip_limit(const pmm_return_clause_t *wc, binding_t *vbindings, int *vcount) {
    if (wc->order_by) {
...
static const char *resolve_item_alias(const pmm_return_item_t *item, char *name_buf,
                                      size_t buf_sz) {
    if (item->alias) {
        return item->alias;
    }
...
static int with_agg_build_key(pmm_return_clause_t *wc, binding_t *b, char *key, size_t key_sz) {
    int kl = 0;
...
static int with_agg_find_or_create(with_agg_t **aggs, int *agg_cnt, int *agg_cap,
                                   pmm_return_clause_t *wc, binding_t *b, const char *key) {
    ...
    for (int ci = 0; ci < wc->count; ci++) {
        ...
        if (!wc->items[ci].property && wc->items[ci].variable) {
            pmm_node_t *gn = binding_get(b, wc->items[ci].variable);
            if (gn) {
...
static void with_agg_accumulate(with_agg_t *agg, pmm_return_clause_t *wc, binding_t *b) {
    for (int ci = 0; ci < wc->count; ci++) {
        if (!wc->items[ci].func) {
            continue;
        }
...
static void with_add_vbinding_var(binding_t *vb, const char *alias, const char *val) {
    pmm_node_t vn = {.name = heap_strdup(val), .qualified_name = heap_strdup(alias)};
...
static void execute_with_aggregate(pmm_return_clause_t *wc, binding_t *bindings, int bind_count,
                                   binding_t **vbindings, int *vcount) {
    int agg_cap = PMM_SZ_256;
...
static void execute_with_simple(pmm_return_clause_t *wc, binding_t *bindings, int bind_count,
                                binding_t *vbindings, int *vcount) {
    for (int bi = 0; bi < bind_count; bi++) {
        binding_t vb = {0};
...
static void filter_bindings_where(const pmm_where_clause_t *where, binding_t *vbindings,
                                  int *vcount) {
    int kept = 0;
...
static void with_proj_key(pmm_return_clause_t *wc, binding_t *b, char *key, size_t key_sz) {
    int kl = 0;
    key[0] = '\0';
    char name_buf[PMM_SZ_256];
    for (int ci = 0; ci < wc->count; ci++) {
...
static void with_apply_distinct(pmm_return_clause_t *wc, binding_t *vbindings, int *vcount) {
    int kept = 0;
    for (int i = 0; i < *vcount; i++) {
        char key[PMM_SZ_1K];
...
static void execute_with_clause(pmm_query_t *q, binding_t **bindings_ptr, int *bind_count_ptr) {
    pmm_return_clause_t *wc = q->with_clause;
    if (!wc) {
        return;
    }
...
static int collect_pattern_vars(pmm_query_t *q, const char **vars, int max_vars) {
    int vc = 0;
...
static void build_star_columns(result_builder_t *rb, const char **vars, int vc) {
    ...
    const char *col_names[PMM_SZ_128];
    for (int v = 0; v < vc; v++) {
        char buf[PMM_SZ_128];
...
static void project_star_var(binding_t *b, const char *var, const char **vals) {
    pmm_edge_t *edge = binding_get_edge(b, var);
    if (edge) {
...
static void execute_return_star(pmm_query_t *q, binding_t *bindings, int bind_count, int max_rows,
                                result_builder_t *rb) {
    const char *vars[PMM_SZ_32];
    int vc = collect_pattern_vars(q, vars, PMM_SZ_32);
    build_star_columns(rb, vars, vc);
    for (int bi = 0; bi < bind_count && rb->row_count < max_rows; bi++) {
        const char *vals[PMM_SZ_128];
...
static void ret_agg_accumulate(ret_agg_entry_t *entry, pmm_return_clause_t *ret, binding_t *b) {
    for (int ci = 0; ci < ret->count; ci++) {
...
static void ret_agg_build_key(pmm_return_clause_t *ret, binding_t *b, char *key, size_t key_sz,
                              const char **vals, char valbufs[][PMM_SZ_512]) {
    int klen = 0;
    for (int ci = 0; ci < ret->count; ci++) {
        if (ret->items[ci].func) {
            vals[ci] = "0";
        ...
        const char *v = project_item(b, &ret->items[ci], valbufs[ci], PMM_SZ_512);
        if (v != valbufs[ci]) {
            snprintf(valbufs[ci], PMM_SZ_512, "%s", v ? v : "");
        }
        vals[ci] = valbufs[ci];
...
static void ret_agg_emit_row(pmm_return_clause_t *ret, ret_agg_entry_t *agg, result_builder_t *rb) {
    const char *row[PMM_SZ_32];
    char bufs[PMM_SZ_32][PMM_SZ_64];
    for (int ci = 0; ci < ret->count; ci++) {
        if (!ret->items[ci].func) {
...
static void execute_return_agg(pmm_return_clause_t *ret, binding_t *bindings, int bind_count,
                               result_builder_t *rb) {
    int agg_cap = PMM_SZ_256;
    ...
    for (int bi = 0; bi < bind_count; bi++) {
        ...
        char key[PMM_SZ_1K] = "";
        const char *vals[PMM_SZ_32];
        char valbufs[PMM_SZ_32][PMM_SZ_512];
...
static void build_return_columns(result_builder_t *rb, pmm_return_clause_t *ret) {
    const char *col_names[PMM_SZ_32];
    for (int i = 0; i < ret->count && i < PMM_SZ_32; i++) {
        pmm_return_item_t *item = &ret->items[i];
        if (item->alias) {
...
static void execute_return_simple(pmm_return_clause_t *ret, binding_t *bindings, int bind_count,
    ...
    for (int bi = 0; bi < bind_count && rb->row_count < proj_cap; bi++) {
        const char *vals[PMM_SZ_32];
        char func_bufs[PMM_SZ_32][PMM_SZ_512];
        for (int ci = 0; ci < ret->count; ci++) {
            vals[ci] =
...
static void execute_default_projection(pmm_pattern_t *pat0, binding_t *bindings, int bind_count,
                                       int max_rows, result_builder_t *rb) {
    ...
    for (int bi = 0; bi < bind_count && rb->row_count < max_rows; bi++) {
        ...
        for (int v = 0; v < vc; v++) {
            pmm_node_t *n = binding_get(&bindings[bi], vars[v]);
...
static void cross_join_nodes(binding_t **bindings, int *bind_count, pmm_node_t *extra_nodes,
...
static void cross_join_with_rels(pmm_store_t *store, pmm_pattern_t *patn, binding_t **bindings,
                                 int *bind_count, pmm_node_t *extra_nodes, int extra_count,
...
static void expand_from_bound_terminal(pmm_store_t *store, pmm_pattern_t *patn,
                                       binding_t **bindings, int *bind_count, const char *start_var,
                                       bool opt) {
    pmm_rel_pattern_t *rel = &patn->rels[0];
    const pmm_node_pattern_t *start_node = &patn->nodes[0];
    ...
    for (int bi = 0; bi < *bind_count && new_count < max_new; bi++) {
        binding_t *b = &(*bindings)[bi];
        pmm_node_t *term = binding_get(b, patn->nodes[1].variable ? patn->nodes[1].variable : "");
        int match_count = 0;
        if (term) {
            for (int ti = 0;
                ...
                if (rel->type_count > 0) {
                    if (scan_targets) {
                        pmm_store_find_edges_by_target_type(store, term->id, rel->types[ti], &edges,
                                                            &edge_count);
                    } else {
                        pmm_store_find_edges_by_source_type(store, term->id, rel->types[ti], &edges,
                                                            &edge_count);
                    }
                } else if (scan_targets) {
                    pmm_store_find_edges_by_target(store, term->id, &edges, &edge_count);
                } else {
                    pmm_store_find_edges_by_source(store, term->id, &edges, &edge_count);
                }
                for (int ei = 0; ei < edge_count && new_count < max_new; ei++) {
                    int64_t sid = scan_targets ? edges[ei].source_id : edges[ei].target_id;
                    pmm_node_t found = {0};
                    if (pmm_store_find_node_by_id(store, sid, &found) != PMM_STORE_OK) {
                        continue;
                    }
...
static void expand_additional_patterns(pmm_store_t *store, pmm_query_t *q, const char *project,
                                       int max_rows, binding_t **bindings, int *bind_count,
                                       int *bind_cap) {
    for (int pi = SKIP_ONE; pi < q->pattern_count; pi++) {
        pmm_pattern_t *patn = &q->patterns[pi];
        ...
        pmm_store_free_nodes(extra_nodes, extra_count);
    }
}

/* Project RETURN clause results */
static void execute_return_clause(pmm_query_t *q, pmm_return_clause_t *ret, binding_t *bindings,
...
static int execute_single(pmm_store_t *store, pmm_query_t *q, const char *project, int max_rows,
                          result_builder_t *rb) {
    pmm_pattern_t *pat0 = &q->patterns[0];

    /* Step 1: Scan initial nodes */
    pmm_node_t *scanned = NULL;
    int scan_count = 0;
    ...
    free(bindings);
    pmm_store_free_nodes(scanned, scan_count);
    return 0;
}

/* ── Main entry point ─────────────────────────────────────────── */

int pmm_cypher_execute(pmm_store_t *store, const char *query, const char *project, int max_rows,
                       pmm_cypher_result_t *out) {
    memset(out, 0, sizeof(*out));
    ...
    pmm_query_t *q = NULL;
    char *err = NULL;
    if (pmm_cypher_parse(query, &q, &err) < 0) {
        out->error = err;
        return PMM_NOT_FOUND;
    }
...
void pmm_cypher_result_free(pmm_cypher_result_t *r) {
    if (!r) {
        return;
    }
    free(r->warning);
