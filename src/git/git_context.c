...
#endif
    /* Double quotes work for POSIX shells and cmd.exe. pmm_validate_shell_arg()
...
static bool json_append_string(char *buf, int buf_size, int *off, const char *name,
    ...
    int actual = pmm_json_escape(escaped, needed + 1, value ? value : "");
