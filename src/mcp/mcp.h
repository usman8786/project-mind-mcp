/*
 * mcp.h — MCP (Model Context Protocol) server for project-mind-mcp.
 *
...
typedef struct pmm_store pmm_store_t; /* from store/store.h */
struct pmm_watcher;                   /* from watcher/watcher.h */
struct pmm_config;                    /* from cli/cli.h */
...
typedef enum {
    ...
    PMM_MCP_TOOL_PROFILE_ANALYSIS = 1,
    PMM_MCP_TOOL_PROFILE_SCOUT = 2,
} pmm_mcp_tool_profile_t;
...
typedef char *(*pmm_mcp_index_executor_fn)(void *context, const char *repo_path,
...
typedef bool (*pmm_mcp_project_mutation_begin_fn)(void *context, const char *project);
typedef void (*pmm_mcp_project_mutation_end_fn)(void *context, const char *project);
 ...
 * PMM_ALLOWED_ROOT. Returns false when the context is invalid or cannot be
 * copied. */
...
typedef enum {
    PMM_MCP_SUPERVISED_RESULT_FALLBACK = 0,
    PMM_MCP_SUPERVISED_RESULT_SUCCESS,
    PMM_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE,
    PMM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL,
} pmm_mcp_supervised_result_disposition_t;
...
struct pmm_pipeline; /* forward decl */

/* Get the currently active pipeline (for signal handler cancellation).
 * Returns NULL if no pipeline is running. */
struct pmm_pipeline *pmm_mcp_server_active_pipeline(pmm_mcp_server_t *srv);
