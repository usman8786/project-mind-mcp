 ...
 */
#ifndef PMM_INDEX_SUPERVISOR_H
#define PMM_INDEX_SUPERVISOR_H

#include <stdbool.h>
#include <stddef.h>
#include <stdatomic.h>

#include "foundation/subprocess.h" /* pmm_proc_outcome_t */
...
#define PMM_INDEX_WORKER_SINGLE_THREAD_ARG "--index-worker-single-thread"
#define PMM_INDEX_WORKER_MARKER_ARG "--index-worker-marker"
#define PMM_INDEX_WORKER_QUARANTINE_ARG "--index-worker-quarantine"
#define PMM_INDEX_WORKER_MEMORY_BUDGET_ARG "--index-worker-memory-budget-bytes"
#define PMM_INDEX_WORKER_BUILD_ARG "--index-worker-build"
#define PMM_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH 64U
#define PMM_INDEX_WORKER_BUILD_FINGERPRINT_SIZE 65U
...
typedef struct {
    ...
    size_t memory_budget_bytes;
} pmm_index_worker_invocation_t;

typedef enum {
    PMM_INDEX_WORKER_ARGV_NOT_WORKER = 0,
    PMM_INDEX_WORKER_ARGV_VALID,
    PMM_INDEX_WORKER_ARGV_INVALID,
    PMM_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE,
    PMM_INDEX_WORKER_ARGV_BUILD_MISMATCH,
} pmm_index_worker_argv_status_t;
 ...
 * project-mind-mcp binary calls this (first thing in main(), before any
 * subcommand dispatch, so MCP server + CLI + HTTP paths are all covered).
 * EMBEDDERS of pmm_mcp_handle_tool (test binaries, future library users) never
 ...
 * this process called pmm_index_supervisor_mark_host() (i.e. it IS the real
 ...
 * entry to pmm_index_worker_start (including calls made by the synchronous
...
typedef struct {
    pmm_proc_outcome_t outcome; /* how the worker ended */
...
} pmm_index_worker_result_t;

/* Daemon-owned, nonblocking supervisor for one contained worker process tree. */
typedef struct pmm_index_worker_handle pmm_index_worker_handle_t;

typedef enum {
    PMM_INDEX_WORKER_POLL_ERROR = -1,
    PMM_INDEX_WORKER_POLL_RUNNING = 0,
    PMM_INDEX_WORKER_POLL_TERMINAL = 1,
} pmm_index_worker_poll_t;
 ...
 * and applied after exec by pmm_index_set_worker_role_options(). */
 ...
 *   - single_thread   → PMM_INDEX_SINGLE_THREAD=1
 *   - marker_file     → PMM_INDEX_MARKER_FILE
 *   - quarantine_file → PMM_INDEX_QUARANTINE_FILE
