...
#include "foundation/compat_fs.h" /* pmm_mkdir_p, pmm_fopen */
#include "foundation/constants.h"
#include "foundation/log.h"
#include "foundation/platform.h" /* pmm_safe_getenv, path normalization */
#include "foundation/profile.h"  /* pmm_profile_active (keep worker log under PMM_PROFILE) */
#include "ui/http_server.h"      /* pmm_http_server_resolve_binary_path */
...
_Static_assert(PMM_INDEX_WORKER_BUILD_FINGERPRINT_SIZE == PMM_DAEMON_BUILD_FINGERPRINT_SIZE,
               "worker and daemon build fingerprint sizes must match");

/* ── Worker-role state ────────────────────────────────────────────── */

static bool g_worker_active = false;
static char g_worker_response_out[PMM_SZ_4K] = {0};
static size_t g_worker_memory_budget_bytes = 0;
static bool g_build_fingerprint_capture_attempted = false;
static char g_build_fingerprint[PMM_INDEX_WORKER_BUILD_FINGERPRINT_SIZE] = {0};

void pmm_index_set_worker_role(bool is_worker, const char *response_out) {
...
static void worker_set_local_env(const char *name, const char *value) {
    if (value && value[0]) {
        (void)pmm_setenv(name, value, 1);
    } else {
        (void)pmm_unsetenv(name);
    }
}

void pmm_index_set_worker_role_options(bool is_worker, const char *response_out, bool single_thread,
                                       const char *marker_file, const char *quarantine_file,
                                       size_t memory_budget_bytes) {
    pmm_index_set_worker_role(is_worker, response_out);
    g_worker_memory_budget_bytes = is_worker ? memory_budget_bytes : 0;
    if (!is_worker) {
        return;
    }
    worker_set_local_env("PMM_INDEX_SINGLE_THREAD", single_thread ? "1" : NULL);
    worker_set_local_env("PMM_INDEX_MARKER_FILE", marker_file);
    worker_set_local_env("PMM_INDEX_QUARANTINE_FILE", quarantine_file);
}

bool pmm_index_worker_active(void) {
    return g_worker_active;
}

const char *pmm_index_worker_response_out(void) {
    return g_worker_response_out[0] ? g_worker_response_out : NULL;
}

size_t pmm_index_worker_memory_budget_bytes(void) {
    return g_worker_memory_budget_bytes;
}

static bool worker_fingerprint_valid(const char *fingerprint);

bool pmm_index_supervisor_capture_build_fingerprint(void) {
    if (g_build_fingerprint_capture_attempted) {
        return g_build_fingerprint[0] != '\0';
    }
    g_build_fingerprint_capture_attempted = true;
#if defined(PMM_CLI_ENABLE_TEST_API)
    ...
    char test_fingerprint[PMM_INDEX_WORKER_BUILD_FINGERPRINT_SIZE];
    const char *test_value = pmm_safe_getenv("PMM_TEST_BUILD_FINGERPRINT", test_fingerprint,
...
#endif
    char captured[PMM_INDEX_WORKER_BUILD_FINGERPRINT_SIZE] = {0};
    if (!pmm_daemon_runtime_process_build_fingerprint((uint64_t)worker_getpid(), captured)) {
        return false;
    }
...
const char *pmm_index_supervisor_build_fingerprint(void) {
    return g_build_fingerprint[0] ? g_build_fingerprint : NULL;
}

static bool worker_fingerprint_valid(const char *fingerprint) {
    if (!fingerprint || strlen(fingerprint) != PMM_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH) {
        return false;
    }
    for (size_t i = 0; i < PMM_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH; i++) {
...
pmm_index_worker_argv_status_t pmm_index_worker_parse_process_argv(
    int argc, char *const argv[], pmm_index_worker_invocation_t *invocation_out) {
    if (invocation_out) {
    ...
    if (!invocation_out || argc < 9 || !argv || !argv[0] || !argv[0][0] || !argv[1] ||
        ...
        return PMM_INDEX_WORKER_ARGV_INVALID;
    }

    pmm_index_worker_invocation_t parsed = {
        .expected_build_fingerprint = argv[4],
        .args_json = argv[6],
        .response_out = argv[8],
    };
    int next = 9;
    if (next < argc && argv[next] && strcmp(argv[next], PMM_INDEX_WORKER_MEMORY_BUDGET_ARG) == 0) {
        if (next + 1 >= argc ||
    ...
    if (next < argc && argv[next] && strcmp(argv[next], PMM_INDEX_WORKER_SINGLE_THREAD_ARG) == 0) {
        parsed.single_thread = true;
        next++;
    }
    if (next < argc && argv[next] && strcmp(argv[next], PMM_INDEX_WORKER_MARKER_ARG) == 0) {
    ...
    if (next < argc && argv[next] && strcmp(argv[next], PMM_INDEX_WORKER_QUARANTINE_ARG) == 0) {
        if (next + 1 >= argc || !argv[next + 1] || !argv[next + 1][0]) {
            return PMM_INDEX_WORKER_ARGV_INVALID;
        }
    ...
    if (!g_build_fingerprint[0]) {
        return PMM_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE;
    }
    if (strcmp(parsed.expected_build_fingerprint, g_build_fingerprint) != 0) {
        return PMM_INDEX_WORKER_ARGV_BUILD_MISMATCH;
    }
    *invocation_out = parsed;
    return PMM_INDEX_WORKER_ARGV_VALID;
}

const char *pmm_index_worker_argv_status_message(pmm_index_worker_argv_status_t status) {
    switch (status) {
    case PMM_INDEX_WORKER_ARGV_VALID:
        return "worker invocation accepted";
    case PMM_INDEX_WORKER_ARGV_INVALID:
        return "invalid internal worker arguments";
    case PMM_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE:
        return "worker executable fingerprint could not be verified";
    case PMM_INDEX_WORKER_ARGV_BUILD_MISMATCH:
        return "worker executable build conflicts with its supervisor; close all PMM sessions "
               "and retry";
    case PMM_INDEX_WORKER_ARGV_NOT_WORKER:
    default:
...
int pmm_index_supervisor_spawn_count(void) {
...
int pmm_index_supervisor_spawn_st_count(void) {
...
void pmm_index_supervisor_mark_host(void) {
    g_host_marked = true;
}

bool pmm_index_supervisor_should_wrap(void) {
    if (!g_host_marked) {
    ...
    /* Supervision is a safety boundary for the physical PMM host. An ambient
     * variable must never turn a long-lived MCP/CLI parent into the indexer. */
    return true;
}

static bool supervisor_disable_requested(void) {
    char supervisor_setting[PMM_SZ_32] = {0};
    return pmm_safe_getenv("PMM_INDEX_SUPERVISOR", supervisor_setting, sizeof(supervisor_setting),
                           NULL) &&
 ...
 * PMM_INDEX_WORKER_TIMEOUT_S override (seconds → ms) tightens it for tests. */
static int worker_quiet_timeout_ms(void) {
    enum { DEFAULT_QUIET_TIMEOUT_MS = 900000 }; /* 15 min with no progress */
    char timeout_seconds[PMM_SZ_32] = {0};
    if (pmm_safe_getenv("PMM_INDEX_WORKER_TIMEOUT_S", timeout_seconds, sizeof(timeout_seconds),
                        NULL) &&
        timeout_seconds[0]) {
...
static char *slurp_worker_response(const char *path, worker_response_read_status_t *status_out) {
    *status_out = WORKER_RESPONSE_READ_ERROR;
    ...
    if ((uint64_t)n > (uint64_t)PMM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
...
enum {
    INDEX_WORKER_PATH_CAP = PMM_SZ_4K,
...
struct pmm_index_worker_handle {
    pmm_subprocess_t *process;
    char response_path[INDEX_WORKER_PATH_CAP];
    char log_path[INDEX_WORKER_PATH_CAP];
    pmm_proc_log_cb log_callback;
    void *log_context;
    long relay_tail_pos;
    bool process_terminal;
    pmm_proc_result_t process_result;
 ...
 * left undelivered, matching pmm_proc_log_cb's completed-line contract. */
static bool worker_relay_log(pmm_index_worker_handle_t *handle) {
    if (!handle || !handle->log_callback) {
        return true;
    }

#ifdef _WIN32
    FILE *log = pmm_fopen(handle->log_path, "rb");
#else
...
/* pmm_mkstemp's Windows compatibility implementation uses an internal buffer.
...
static void worker_result_init(pmm_index_worker_result_t *result) {
...
static bool worker_cache_dir(char out[INDEX_WORKER_PATH_CAP]) {
    ...
    int written = snprintf(out, INDEX_WORKER_PATH_CAP, "%s/.cache/project-mind-mcp", home);
    if (written <= 0 || written >= INDEX_WORKER_PATH_CAP) {
        return false;
    }
    pmm_normalize_path_sep(out);
...
static bool worker_result_succeeded(const pmm_index_worker_result_t *result) {
    return result && result->outcome == PMM_PROC_CLEAN && !result->cancellation_requested &&
           result->tree_quiesced && !result->supervision_failed;
}

static void worker_terminal_log(pmm_index_worker_handle_t *handle) {
    char signal_text[16];
    char exit_text[16];
    ...
    pmm_log_info("index.supervisor.reap", "outcome", pmm_proc_outcome_str(handle->result.outcome),
                 "exit_code", exit_text, "signal", signal_text);
    if (handle->result.response_rejected) {
        pmm_log_error("index.supervisor.response_rejected", "reason", "payload_too_large", "log",
                      handle->log_path);
    } else if (handle->result.supervision_failed || !handle->result.tree_quiesced) {
        pmm_log_error("index.supervisor.containment_failed", "outcome",
                      pmm_proc_outcome_str(handle->result.outcome), "log", handle->log_path);
    } else if (handle->result.cancellation_requested) {
        pmm_log_warn("index.supervisor.worker_cancelled", "outcome",
                     pmm_proc_outcome_str(handle->result.outcome), "log", handle->log_path);
    } else if (handle->result.outcome == PMM_PROC_CLEAN && !pmm_profile_active) {
        (void)pmm_unlink(handle->log_path);
    } else if (handle->result.outcome == PMM_PROC_CLEAN) {
        pmm_log_info("index.supervisor.profile_log", "log", handle->log_path);
    } else {
        pmm_log_warn("index.supervisor.worker_failed", "outcome",
                     pmm_proc_outcome_str(handle->result.outcome), "exit_code", exit_text, "log",
                     handle->log_path);
    }
}

int pmm_index_worker_start_with_log(const char *args_json, size_t memory_budget_bytes,
                                    bool single_thread, const char *marker_file,
                                    const char *quarantine_file, pmm_proc_log_cb log_callback,
                                    void *log_context, pmm_index_worker_handle_t **handle_out) {
    if (handle_out) {
        *handle_out = NULL;
    }
    ...
    if (g_host_marked && !g_worker_active && supervisor_disable_requested()) {
        ...
        pmm_log_error("index.supervisor.disable_refused", "action", "fail_closed");
        return -1;
    }
    const char *build_fingerprint = pmm_index_supervisor_build_fingerprint();
    if (!build_fingerprint) {
        pmm_log_error("index.supervisor.build_fingerprint_unavailable", "action", "refuse_spawn");
        return -1;
    }

    char self[INDEX_WORKER_PATH_CAP] = {0};
    if (!pmm_http_server_resolve_binary_path(NULL, self, sizeof(self)) || !self[0]) {
        pmm_log_error("index.supervisor.no_self_path", "action", "fail_closed");
        return -1;
    }
    pmm_index_worker_handle_t *handle = calloc(1, sizeof(*handle));
    if (!handle) {
        return -1;
    }
    ...
    if (!worker_unique_file(handle->response_path, sizeof(handle->response_path), "response") ||
        ...
        (void)pmm_unlink(handle->response_path);
        (void)pmm_unlink(handle->log_path);
        free(handle);
        return -1;
    }
    ...
    if (pmm_subprocess_spawn(&options, &handle->process) != 0) {
        (void)pmm_unlink(handle->response_path);
        (void)pmm_unlink(handle->log_path);
        free(handle);
        pmm_log_error("index.supervisor.spawn_failed", "action", "fail_closed");
        return -1;
    }
    *handle_out = handle;
    return 0;
...
int pmm_index_worker_start(const char *args_json, size_t memory_budget_bytes, bool single_thread,
                           const char *marker_file, const char *quarantine_file,
                           pmm_index_worker_handle_t **handle_out) {
    return pmm_index_worker_start_with_log(args_json, memory_budget_bytes, single_thread,
                                           marker_file, quarantine_file, NULL, NULL, handle_out);
}

pmm_index_worker_poll_t pmm_index_worker_poll(pmm_index_worker_handle_t *handle,
    ...
    if (!handle->process_terminal) {
        pmm_proc_result_t process_result;
        pmm_proc_poll_t state = pmm_subprocess_poll(handle->process, &process_result);
    ...
    const pmm_proc_result_t *process_result = &handle->process_result;
    ...
    if (worker_result_succeeded(&handle->result)) {
        ...
        if (response_status == WORKER_RESPONSE_READ_TOO_LARGE) {
            ...
            handle->result.outcome = PMM_PROC_EXIT_NONZERO;
...
bool pmm_index_worker_request_cancel(pmm_index_worker_handle_t *handle) {
    return handle && !atomic_load_explicit(&handle->terminal, memory_order_acquire) &&
           pmm_subprocess_request_cancel(handle->process);
}

const char *pmm_index_worker_response_path(const pmm_index_worker_handle_t *handle) {
    return handle ? handle->response_path : NULL;
}

const char *pmm_index_worker_log_path(const pmm_index_worker_handle_t *handle) {
    return handle ? handle->log_path : NULL;
}

void pmm_index_worker_destroy(pmm_index_worker_handle_t *handle) {
    if (!handle) {
        return;
    }
    if (!atomic_load_explicit(&handle->terminal, memory_order_acquire)) {
        pmm_log_error("index.supervisor.destroy_running", "action", "ignored");
        return;
    }
    pmm_subprocess_destroy(handle->process);
    free(handle->result.response);
    handle->result.response = NULL;
    free(handle);
}

int pmm_index_spawn_worker_with_log_cancel(const char *args_json, bool single_thread,
                                           const char *marker_file, const char *quarantine_file,
                                           pmm_proc_log_cb log_callback, void *log_context,
    ...
    pmm_index_worker_handle_t *handle = NULL;
    if (pmm_index_worker_start_with_log(args_json, 0, single_thread, marker_file, quarantine_file,
                                        log_callback, log_context, &handle) != 0) {
        return -1;
    }
    const pmm_index_worker_result_t *cached = NULL;
    ...
    for (;;) {
        if (!cancellation_forwarded && cancel_requested &&
            ...
            cancellation_forwarded = pmm_index_worker_request_cancel(handle);
        }
        pmm_index_worker_poll_t state = pmm_index_worker_poll(handle, &cached);
        if (state == PMM_INDEX_WORKER_POLL_TERMINAL) {
            break;
        }
        if (state == PMM_INDEX_WORKER_POLL_ERROR) {
            (void)pmm_index_worker_request_cancel(handle);
        }
        const struct timespec pause = {0, INDEX_WORKER_SYNC_POLL_NS};
        (void)pmm_nanosleep(&pause, NULL);
    }
    *result = *cached;
    result->response = cached->response ? pmm_strdup(cached->response) : NULL;
    pmm_index_worker_destroy(handle);
    return 0;
}

int pmm_index_spawn_worker_with_log(const char *args_json, bool single_thread,
                                    const char *marker_file, const char *quarantine_file,
                                    pmm_proc_log_cb log_callback, void *log_context,
                                    pmm_index_worker_result_t *result) {
    return pmm_index_spawn_worker_with_log_cancel(args_json, single_thread, marker_file,
                                                  quarantine_file, log_callback, log_context, NULL,
                                                  result);
}

int pmm_index_spawn_worker(const char *args_json, bool single_thread, const char *marker_file,
                           const char *quarantine_file, pmm_index_worker_result_t *result) {
    return pmm_index_spawn_worker_with_log(args_json, single_thread, marker_file, quarantine_file,
                                           NULL, NULL, result);
}

void pmm_index_worker_result_free(pmm_index_worker_result_t *result) {
    if (result) {
