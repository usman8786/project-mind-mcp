...
/* Write through the descriptor pmm_mkstemp returned rather than reopening its
...
int pmm_jsonrpc_parse(const char *line, pmm_jsonrpc_request_t *out) {
    memset(out, 0, sizeof(*out));
    out->id = PMM_NOT_FOUND;
...
void pmm_jsonrpc_request_free(pmm_jsonrpc_request_t *r) {
    if (!r) {
        return;
    }
...
char *pmm_jsonrpc_format_response(const pmm_jsonrpc_response_t *resp) {
...
char *pmm_jsonrpc_format_error(int64_t id, int code, const char *message) {
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
char *pmm_mcp_text_result(const char *text, bool is_error) {
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
bool pmm_mcp_cancel_request_matches(const char *params_json, int64_t active_id,
...
static const tool_def_t TOOLS[] = {
    {"index_repository", "Index repository",
     ...
     "deliberately NOT indexed (gitignore/.pmmignore/skip-lists) — by design, not failures.",
     ...
     "\"Write compressed artifact to .project-mind-mcp/graph.db.zst for team sharing. "
    ...
    {"index_status", "Index status",
     ...
     "BY DESIGN (gitignore/.pmmignore/skip-lists): deliberate and deterministic, not failures; "
...
static const char *mcp_tool_profile_name(pmm_mcp_tool_profile_t profile) {
    return profile == PMM_MCP_TOOL_PROFILE_SCOUT ? "scout" : "analysis";
}

int pmm_mcp_parse_tool_profile_args(int argc, const char *const argv[const],
                                    pmm_mcp_tool_profile_t *profile_out) {
    if (argc < 0 || !argv || !profile_out) {
        return -1;
    }
    *profile_out = PMM_MCP_TOOL_PROFILE_ALL;
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "--tool-profile=analysis") == 0) {
            *profile_out = PMM_MCP_TOOL_PROFILE_ANALYSIS;
            continue;
        }
        if (strcmp(arg, "--tool-profile=scout") == 0) {
            *profile_out = PMM_MCP_TOOL_PROFILE_SCOUT;
            continue;
        }
...
bool pmm_mcp_tool_profile_allows_http(pmm_mcp_tool_profile_t profile) {
    return profile == PMM_MCP_TOOL_PROFILE_ALL;
}

static int mcp_allowed_tool_count(pmm_mcp_tool_profile_t profile) {
    int count = 0;
...
static char *pmm_mcp_tools_list_range(pmm_mcp_tool_profile_t profile, int offset, int limit,
...
char *pmm_mcp_tools_list(void) {
    return pmm_mcp_tools_list_range(PMM_MCP_TOOL_PROFILE_ALL, 0, TOOL_COUNT, false);
}
...
const char *pmm_mcp_tool_input_schema(const char *tool_name) {
    if (!tool_name) {
        return NULL;
    }
    for (int i = 0; i < TOOL_COUNT; i++) {
...
static char *pmm_mcp_tools_list_page(pmm_mcp_tool_profile_t profile, const char *params_json) {
    bool has_cursor = false;
    int offset = mcp_tools_cursor_offset(params_json, &has_cursor);
    if (!has_cursor) {
        return pmm_mcp_tools_list_range(profile, 0, TOOL_COUNT, false);
    }
    return pmm_mcp_tools_list_range(profile, offset, MCP_TOOLS_PAGE_SIZE, true);
}
...
static char *pmm_mcp_prompts_list(void) {
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
static char *pmm_mcp_prompt_get(const char *params_json, char **error_json) {
    *error_json = NULL;
...
static char *pmm_mcp_initialize_response_for_profile(const char *params_json,
                                                     pmm_mcp_tool_profile_t profile) {
    ...
    yyjson_mut_obj_add_str(doc, impl, "name", "project-mind-mcp");
    yyjson_mut_obj_add_str(doc, impl, "version", pmm_cli_get_version());
...
char *pmm_mcp_initialize_response(const char *params_json) {
    return pmm_mcp_initialize_response_for_profile(params_json, PMM_MCP_TOOL_PROFILE_ALL);
}
...
char *pmm_mcp_get_tool_name(const char *params_json) {
    yyjson_doc *doc = yyjson_read(params_json, strlen(params_json), 0);
    if (!doc) {
        return NULL;
    }
...
char *pmm_mcp_get_arguments(const char *params_json) {
    yyjson_doc *doc = yyjson_read(params_json, strlen(params_json), 0);
    if (!doc) {
        return NULL;
    }
...
char *pmm_mcp_get_string_arg(const char *args_json, const char *key) {
...
static char *canonicalize_repo_path_if_exists(char *repo_path) {
    ...
    if (pmm_canonical_path(repo_path, real, sizeof(real))) {
        pmm_normalize_path_sep(real);
...
static char *normalize_project_arg(char *project) {
    ...
    char *normalized = pmm_project_name_from_path(project);
    if (normalized) {
        free(project);
        return normalized;
    }
...
bool pmm_validate_project_name(const char *project);

/* #1025: agents naturally pass the repo FOLDER name ("project-mind-mcp"),
...
static char *resolve_project_tail(char *project) {
    if (!project || !pmm_validate_project_name(project)) {
        return project;
    }
    char dir[PMM_SZ_1K];
    cache_dir(dir, sizeof(dir));
    char exact[PMM_SZ_2K];
    snprintf(exact, sizeof(exact), "%s/%s.db", dir, project);
    if (pmm_file_exists(exact)) {
        return project; /* exact name — untouched fast path */
    }
    size_t plen = strlen(project);
    char match[PMM_SZ_1K] = "";
    int matches = 0;
    pmm_dir_t *d = pmm_opendir(dir);
    if (!d) {
        return project;
    }
    pmm_dirent_t *entry;
    while ((entry = pmm_readdir(d)) != NULL) {
    ...
    pmm_closedir(d);
    if (matches == 1) {
        pmm_log_info("mcp.project_tail_resolved", "passed", project, "resolved", match);
        free(project);
...
static char *get_project_arg(const char *args_json) {
    char *p = pmm_mcp_get_string_arg(args_json, "project");
    if (!p) {
        p = pmm_mcp_get_string_arg(args_json, "project_name");
    }
    if (!p) {
        p = pmm_mcp_get_string_arg(args_json, "project_id");
    }
    if (!p) {
        p = pmm_mcp_get_string_arg(args_json, "projectName");
    }
    return resolve_project_tail(normalize_project_arg(p));
}

int pmm_mcp_get_int_arg(const char *args_json, const char *key, int default_val) {
    yyjson_doc *doc = yyjson_read(args_json, strlen(args_json), 0);
    if (!doc) {
        return default_val;
    }
...
bool pmm_mcp_get_bool_arg(const char *args_json, const char *key) {
    yyjson_doc *doc = yyjson_read(args_json, strlen(args_json), 0);
    if (!doc) {
        return false;
    }
...
struct pmm_mcp_server {
    pmm_store_t *store;     /* currently open project store (or NULL) */
    ...
    char session_root[PMM_SZ_1K];     /* detected project root path */
    char session_project[PMM_SZ_256]; /* derived project name */
    ...
    struct pmm_watcher *watcher;      /* external watcher ref (not owned) */
    struct pmm_config *config;        /* external config ref (not owned) */
    pmm_mcp_index_executor_fn index_executor;
    void *index_executor_context;
    pmm_proc_log_cb index_log_callback;
    void *index_log_context;
    pmm_mcp_project_mutation_begin_fn mutation_begin;
    pmm_mcp_project_mutation_end_fn mutation_end;
    void *mutation_context;
    pmm_mcp_quarantine_test_hook_fn quarantine_test_hook;
    void *quarantine_test_context;
    pmm_mcp_command_test_hook_fn command_test_hook;
    void *command_test_context;
    pmm_thread_t autoindex_tid;
    ...
    pmm_mutex_t request_scope_mutex;
    unsigned int request_scope_depth;
    atomic_int pipeline_cancel_requested;
    pmm_pipeline_t *active_pipeline; /* non-NULL while index_repository runs */
    ...
    pmm_mcp_tool_profile_t tool_profile;
};

pmm_mcp_server_t *pmm_mcp_server_new(const char *store_path) {
    pmm_mcp_server_t *srv = calloc(PMM_ALLOC_ONE, sizeof(*srv));
    if (!srv) {
        return NULL;
    }
    pmm_mutex_init(&srv->request_scope_mutex);
    ...
    if (store_path) {
        srv->store = pmm_store_open(store_path);
        srv->current_project = heap_strdup(store_path);
    } else {
        srv->store = pmm_store_open_memory();
    }
    srv->owns_store = true;
    srv->tool_profile = PMM_MCP_TOOL_PROFILE_ALL;
    srv->background_tasks = true;

    return srv;
}

void pmm_mcp_server_set_tool_profile(pmm_mcp_server_t *srv, pmm_mcp_tool_profile_t profile) {
    if (srv) {
        srv->tool_profile = profile;
    }
}

pmm_store_t *pmm_mcp_server_store(pmm_mcp_server_t *srv) {
    return srv ? srv->store : NULL;
}

void pmm_mcp_server_set_project(pmm_mcp_server_t *srv, const char *project) {
    if (!srv) {
        return;
    }
...
void pmm_mcp_server_set_watcher(pmm_mcp_server_t *srv, struct pmm_watcher *w) {
    if (srv) {
        srv->watcher = w;
    }
}

void pmm_mcp_server_set_config(pmm_mcp_server_t *srv, struct pmm_config *cfg) {
    if (srv) {
        srv->config = cfg;
    }
}

bool pmm_mcp_server_set_session_context(pmm_mcp_server_t *srv, const char *session_root,
    ...
    char *project = pmm_project_name_from_path(session_root);
...
const char *pmm_mcp_server_session_root(const pmm_mcp_server_t *srv) {
    return srv ? srv->session_root : NULL;
}

const char *pmm_mcp_server_session_project(const pmm_mcp_server_t *srv) {
    return srv ? srv->session_project : NULL;
}

const char *pmm_mcp_server_allowed_root(const pmm_mcp_server_t *srv) {
    return srv ? srv->allowed_root : NULL;
}

void pmm_mcp_server_set_background_tasks(pmm_mcp_server_t *srv, bool enabled) {
    if (srv) {
        srv->background_tasks = enabled;
    }
}

void pmm_mcp_server_set_index_executor(pmm_mcp_server_t *srv, pmm_mcp_index_executor_fn executor,
                                       void *context) {
    if (srv) {
...
void pmm_mcp_server_set_index_log_callback(pmm_mcp_server_t *srv, pmm_proc_log_cb callback,
                                           void *context) {
    if (srv) {
...
void pmm_mcp_server_set_project_mutation_guard(pmm_mcp_server_t *srv,
                                               pmm_mcp_project_mutation_begin_fn begin,
                                               pmm_mcp_project_mutation_end_fn end, void *context) {
    if (!srv) {
        return;
    }
...
static bool mcp_project_mutation_begin(pmm_mcp_server_t *srv, const char *project) {
    return !srv->mutation_begin || srv->mutation_begin(srv->mutation_context, project);
}

static void mcp_project_mutation_end(pmm_mcp_server_t *srv, const char *project) {
    if (srv->mutation_end) {
        srv->mutation_end(srv->mutation_context, project);
    }
}

void pmm_mcp_server_free(pmm_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    if (srv->autoindex_active) {
        pmm_thread_join(&srv->autoindex_tid);
    }
    if (srv->owns_store && srv->store) {
        pmm_store_close(srv->store);
    }
    free(srv->current_project);
    free(srv->allowed_root);
    free(srv->active_request_id_str);
    pmm_mutex_destroy(&srv->request_scope_mutex);
    free(srv);
}

/* ── Idle store eviction ──────────────────────────────────────── */

void pmm_mcp_server_evict_idle(pmm_mcp_server_t *srv, int timeout_s) {
    if (!srv || !srv->store) {
        return;
    }
...
bool pmm_mcp_server_has_cached_store(pmm_mcp_server_t *srv) {
    return (srv && srv->store != NULL) != 0;
}

bool pmm_mcp_server_release_pristine_memory_store(pmm_mcp_server_t *srv) {
    const char *db_path = srv && srv->store ? pmm_store_db_path(srv->store) : NULL;
...
pmm_pipeline_t *pmm_mcp_server_active_pipeline(pmm_mcp_server_t *srv) {
    return srv ? srv->active_pipeline : NULL;
}

bool pmm_mcp_server_cancel_active(pmm_mcp_server_t *srv) {
    if (!srv) {
        return false;
    }
    pmm_mutex_lock(&srv->request_scope_mutex);
    bool active = srv->request_scope_depth != 0;
    if (active) {
        atomic_store_explicit(&srv->pipeline_cancel_requested, 1, memory_order_release);
    }
    pmm_mutex_unlock(&srv->request_scope_mutex);
    return active;
}

bool pmm_mcp_server_request_scope_begin(pmm_mcp_server_t *srv) {
    if (!srv) {
        return false;
    }
    pmm_mutex_lock(&srv->request_scope_mutex);
...
void pmm_mcp_server_request_scope_end(pmm_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    pmm_mutex_lock(&srv->request_scope_mutex);
...
static bool mcp_request_cancelled(const pmm_mcp_server_t *srv) {
    return srv && atomic_load_explicit(&srv->pipeline_cancel_requested, memory_order_acquire) != 0;
}

void pmm_mcp_server_set_quarantine_test_hook(pmm_mcp_server_t *srv,
                                             pmm_mcp_quarantine_test_hook_fn hook, void *context) {
    if (!srv) {
        return;
    }
...
void pmm_mcp_server_set_command_test_hook(pmm_mcp_server_t *srv, pmm_mcp_command_test_hook_fn hook,
                                          void *context) {
    if (!srv) {
        return;
    }
...
static const char *cache_dir(char *buf, size_t bufsz) {
    const char *dir = pmm_resolve_cache_dir();
    if (!dir) {
        dir = pmm_tmpdir();
    }
...
static const char *project_db_path(const char *project, char *buf, size_t bufsz) {
    if (!pmm_validate_project_name(project)) {
        buf[0] = '\0';
        return buf;
    }
    char dir[PMM_SZ_1K];
 ...
 * handle is transferred to the caller (who must pmm_store_close it). On failure
...
static pmm_store_t *resolve_store_fallback_scan(const char *project);
...
static bool reserve_unique_corrupt_pending(const char *path, char *pending, size_t pending_size,
    ...
    for (unsigned int attempt = 0; attempt < 128; attempt++) {
        uint64_t token = pmm_now_ns() ^ ((uint64_t)(unsigned int)getpid() << 32) ^
        ...
        if (pmm_file_exists(backup)) {
            continue;
        }
#ifdef _WIN32
        wchar_t *wide = pmm_path_to_wide(pending);
...
static void discard_corrupt_pending(const char *pending) {
    ...
    (void)pmm_remove_db_sidecars(pending);
    (void)pmm_unlink(pending);
}

#ifndef _WIN32
static bool sync_parent_directory(const char *path) {
    char directory[PMM_SZ_2K];
...
#ifdef _WIN32
    wchar_t *wide_pending = pmm_path_to_wide(pending);
    wchar_t *wide_backup = pmm_path_to_wide(backup);
...
static bool quarantine_step_allowed(pmm_mcp_server_t *srv, const char *step) {
...
static bool quarantine_corrupt_store(pmm_mcp_server_t *srv, const char *project, const char *path,
                                     char *backup_out, size_t backup_out_size) {
    char backup[PMM_SZ_2K];
    char pending[PMM_SZ_2K];
    if (!reserve_unique_corrupt_pending(path, pending, sizeof(pending), backup, sizeof(backup))) {
        pmm_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
                      "cannot reserve unique backup");
        return false;
    }

    if (pmm_store_backup_path(path, pending) != PMM_STORE_OK ||
        pmm_store_prepare_path_for_replace(pending) != PMM_STORE_OK) {
    ...
    pmm_store_t *snapshot = pmm_store_open_path_query(pending);
    if (!snapshot) {
        discard_corrupt_pending(pending);
        pmm_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
    ...
    if (pmm_unlink(path) != 0 && errno != ENOENT) {
        pmm_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
                      "backup complete; live database removal failed", "backup", backup);
        return false;
    }
    if (pmm_remove_db_sidecars(path) != 0) {
        pmm_log_error("store.auto_clean_sidecars", "project", project, "path", path, "reason",
...
static pmm_store_t *resolve_store_internal(pmm_mcp_server_t *srv, const char *project,
    ...
    srv->store = pmm_store_open_path_query(path);
    if (srv->store) {
        /* Check DB integrity — back up (never silently delete) a corrupt DB */
        if (!pmm_store_check_integrity(srv->store)) {
            pmm_store_close(srv->store);
            srv->store = NULL;
            ...
            srv->store = pmm_store_open_path_query(path);
            bool current_valid = srv->store && pmm_store_check_integrity(srv->store);
            if (!current_valid) {
                pmm_store_close(srv->store);
                srv->store = NULL;
                char backup[PMM_SZ_2K] = {0};
                bool quarantined =
                    quarantine_corrupt_store(srv, project, path, backup, sizeof(backup));
                pmm_log_error("store.auto_clean", "project", project, "path", path, "action",
        ...
        pmm_project_t proj_verify = {0};
        if (pmm_store_get_project(srv->store, project, &proj_verify) == PMM_STORE_OK) {
            pmm_project_free_fields(&proj_verify);
            srv->owns_store = true;
            free(srv->current_project);
    ...
    pmm_store_t *scanned = resolve_store_fallback_scan(project);
    if (scanned) {
        srv->store = scanned;
        srv->owns_store = true;
        free(srv->current_project);
...
static pmm_store_t *resolve_store(pmm_mcp_server_t *srv, const char *project) {
...
static void free_node_contents(pmm_node_t *n);
...
static int collect_db_project_names(const char *dir_path, char *out, size_t out_sz) {
    int count = 0;
    int offset = 0;
    pmm_dir_t *d = pmm_opendir(dir_path);
    if (!d) {
        return 0;
    }
    pmm_dirent_t *entry;
...
static void add_git_context_json(yyjson_mut_doc *doc, yyjson_mut_val *obj, const char *root_path) {
    pmm_git_context_t ctx = {0};
    (void)pmm_git_context_resolve(root_path, &ctx);
...
static char *build_project_list_error(const char *reason) {
    char dir_path[PMM_SZ_1K];
    cache_dir(dir_path, sizeof(dir_path));

    char projects[PMM_SZ_4K] = "";
...
#define REQUIRE_STORE(store, project)                     \
    do {                                                  \
        if (!(store)) {                                   \
            ...
            char *_res = pmm_mcp_text_result(_err, true); \
            free(_err);                                   \
            free(project);                                \
            return _res;                                  \
        }                                                 \
...
static bool project_has_adr(pmm_store_t *store, const char *project, const char *root_path) {
    if (store && project) {
        pmm_adr_t adr;
        memset(&adr, 0, sizeof(adr));
        if (pmm_store_adr_get(store, project, &adr) == PMM_STORE_OK) {
            pmm_store_adr_free(&adr);
            return true;
        }
    }
    ...
    char adr_path[PMM_SZ_4K];
    snprintf(adr_path, sizeof(adr_path), "%s/.project-mind-mcp/adr.md", root_path);
    struct stat adr_st;
 ...
 * "tmp-" (pmm_project_name_from_path: "/tmp/bench/..." → "tmp-bench-...";
...
static bool db_internal_project_name(const char *full_path, char *name_out, size_t name_sz,
    ...
    pmm_store_t *st = pmm_store_open_path_query(full_path);
    if (!st) {
        return false; /* nonexistent / unreadable */
    }
    pmm_project_t *projs = NULL;
    int n = 0;
    bool ok = false;
    if (pmm_store_list_projects(st, &projs, &n) == PMM_STORE_OK) {
    ...
    pmm_store_free_projects(projs, n);
    if (ok && out_store) {
...
static void build_project_json_entry(yyjson_mut_doc *doc, yyjson_mut_val *arr, const char *dir_path,
    ...
    char project_name[PMM_SZ_1K];
    pmm_store_t *pstore = NULL;
    ...
    int edges = 0;
    if (!metadata_only) {
        nodes = pmm_store_count_nodes(pstore, project_name);
        edges = pmm_store_count_edges(pstore, project_name);
    }
    char root_path_buf[PMM_SZ_1K] = "";
    pmm_project_t proj = {0};
    if (pmm_store_get_project(pstore, project_name, &proj) == PMM_STORE_OK) {
        if (proj.root_path) {
            snprintf(root_path_buf, sizeof(root_path_buf), "%s", proj.root_path);
        }
        pmm_project_free_fields(&proj);
    }
    pmm_store_close(pstore);
    ...
    if (!metadata_only && root_path_buf[0]) {
        pmm_git_context_t gctx = {0};
        (void)pmm_git_context_resolve(root_path_buf, &gctx);
        if (gctx.is_git && gctx.branch) {
            yyjson_mut_obj_add_strcpy(doc, p, "branch", gctx.branch);
        }
        pmm_git_context_free(&gctx);
    }
    if (!metadata_only) {
...
static char *handle_list_projects(pmm_mcp_server_t *srv, const char *args) {
    (void)srv;
    ...
    if (!d) {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 dir_path);
        yyjson_mut_doc_free(doc);
        return pmm_mcp_text_result(msg, true);
    }
    ...
    while ((entry = pmm_readdir(d)) != NULL) {
        ...
        char full_path[PMM_SZ_2K];
        snprintf(full_path, sizeof(full_path), "%s/%s", dir_path, name);
        int64_t size_bytes = pmm_file_size(full_path);
        if (size_bytes < 0) {
            continue;
        }
    ...
    char *result = pmm_mcp_text_result(json, false);
    free(json);
    return result;
}
 ...
 * resolve_store uses pmm_store_open_path_query (no SQLITE_OPEN_CREATE), so
...
static char *verify_project_indexed(pmm_store_t *store, const char *project) {
    pmm_project_t proj_check = {0};
    if (pmm_store_get_project(store, project, &proj_check) != PMM_STORE_OK) {
        char *err = build_project_list_error("project not indexed — run index_repository first");
        char *res = pmm_mcp_text_result(err, true);
        free(err);
        return res;
    }
    pmm_project_free_fields(&proj_check);
    return NULL;
}

static bool sg_field_blocked(const char *f); /* internal-only fields, defined with search_graph */

static char *handle_get_graph_schema(pmm_mcp_server_t *srv, const char *args) {
    ...
    pmm_store_get_schema(store, project, &schema);
    ...
    /* Check ADR presence */
    pmm_project_t proj_info = {0};
    if (pmm_store_get_project(store, project, &proj_info) == 0 && proj_info.root_path) {
        ...
        pmm_project_free_fields(&proj_info);
    }
...
static void enrich_add_bfs(yyjson_mut_doc *doc, yyjson_mut_val *arr, pmm_traverse_result_t *tr) {
...
static yyjson_mut_val *enrich_connected(yyjson_mut_doc *doc, pmm_store_t *store, int64_t node_id,
    ...
    pmm_traverse_result_t tr_in = {0};
    pmm_store_bfs(store, node_id, "inbound", et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr_in);
    enrich_add_bfs(doc, conn, &tr_in);
    pmm_store_traverse_free(&tr_in);

    pmm_traverse_result_t tr_out = {0};
    pmm_store_bfs(store, node_id, "outbound", et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr_out);
    enrich_add_bfs(doc, conn, &tr_out);
    pmm_store_traverse_free(&tr_out);
...
static void enrich_connected_joined(pmm_store_t *store, int64_t node_id, const char *relationship,
    ...
    for (int d = 0; d < 2; d++) {
        pmm_traverse_result_t tr = {0};
        pmm_store_bfs(store, node_id, dirs[d], et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr);
...
static char *bm25_file_pattern_like(const char *file_pattern) {
    ...
    char *like = pmm_glob_to_like(file_pattern);
...
static char *bm25_search(pmm_store_t *store, const char *project, const char *query,
                         const char *file_pattern, int limit, int offset, bool toon) {
    sqlite3 *db = pmm_store_get_db(store);
    if (!db) {
        return NULL;
    }
    ...
    if (toon) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            pmm_tree_row_begin(&rows);
            pmm_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_QN), true);
            pmm_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_LABEL),
                              false);
            pmm_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_FILE), false);
            pmm_tree_cell_str(&rows, lines, false);
            pmm_tree_cell_real(&rows, sqlite3_column_double(stmt, BM25_COL_RANK), false);
            pmm_tree_row_end(&rows);
            emitted++;
        }
        ...
        pmm_sb_t sb;
        pmm_sb_init(&sb);
        pmm_tree_scalar_int(&sb, "total", total);
        pmm_tree_scalar_str(&sb, "search_mode", "bm25");
        static const char *const cols[] = {"qn", "label", "file", "lines", "rank"};
        pmm_tree_table_header(&sb, "results", emitted, cols, 5);
        char *rows_text = pmm_sb_finish(&rows);
        pmm_sb_append(&sb, rows_text ? rows_text : "");
        free(rows_text);
        pmm_tree_scalar_bool(&sb, "has_more", total > offset + emitted);
        return pmm_sb_finish(&sb);
    }
...
static void emit_semantic_results(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                  pmm_vector_result_t *vresults, int vcount) {
 ...
 * *out_vcount (caller frees via pmm_store_free_vector_results when vcount>0).
 * Returns true if semantic_query was provided as a non-array (type error —
 * caller should surface to the user). */
static bool run_semantic_query_core(const char *args, pmm_store_t *store, const char *project,
                                    int limit, pmm_vector_result_t **out_vresults, int *out_vcount,
                                    bool *out_present) {
    ...
    *out_vcount = 0;
    ...
    if (sq_val && !yyjson_is_arr(sq_val)) {
        ...
        pmm_vector_result_t *vresults = NULL;
        int vcount = 0;
        int sem_limit = limit > 0 ? limit : PMM_SZ_16;
        if (pmm_store_vector_search(store, project, keywords, ki, sem_limit, &vresults, &vcount) ==
                PMM_STORE_OK &&
            vcount > 0) {
...
static bool run_semantic_query(yyjson_mut_doc *doc, yyjson_mut_val *root, const char *args,
                               pmm_store_t *store, const char *project, int limit) {
    pmm_vector_result_t *vresults = NULL;
    int vcount = 0;
    bool type_error =
    ...
    if (vcount > 0) {
        ...
        pmm_store_free_vector_results(vresults, vcount);
    }
    return type_error;
}
...
static void sg_toon_extra_cells(pmm_sb_t *sb, const char *props_json, const char *const *fields,
                                int nfields) {
    yyjson_doc *pd =
    ...
    for (int f = 0; f < nfields; f++) {
        ...
        if (v && yyjson_is_str(v)) {
            pmm_tree_cell_str(sb, yyjson_get_str(v), false);
        } else if (v && yyjson_is_bool(v)) {
            pmm_tree_cell_bool(sb, yyjson_get_bool(v), false);
        } else if (v && yyjson_is_int(v)) {
            pmm_tree_cell_int(sb, yyjson_get_int(v), false);
        } else if (v && yyjson_is_real(v)) {
            pmm_tree_cell_real(sb, yyjson_get_real(v), false);
        } else {
            pmm_tree_cell_str(sb, "", false);
        }
    }
    if (pd) {
        yyjson_doc_free(pd);
...
static void emit_search_results_toon(pmm_sb_t *sb, const pmm_search_output_t *out, int offset,
                                     const char *const *fields, int nfields, bool detail_ids) {
    pmm_tree_scalar_int(sb, "total", out->total);
    if (detail_ids) {
        ...
        pmm_tree_table_header(sb, "results", out->count, id_cols, 1);
        for (int i = 0; i < out->count; i++) {
            pmm_tree_row_begin(sb);
            pmm_tree_cell_str(sb, out->results[i].node.qualified_name, true);
            pmm_tree_row_end(sb);
        }
        pmm_tree_scalar_bool(sb, "has_more", out->total > offset + out->count);
        return;
    }
    ...
    pmm_tree_table_header(sb, "results", out->count, cols, ncols);
    for (int i = 0; i < out->count; i++) {
        const pmm_search_result_t *sr = &out->results[i];
        char lines[PMM_SZ_32];
        sg_lines_str(lines, sizeof(lines), sr->node.start_line, sr->node.end_line);
        pmm_tree_row_begin(sb);
        pmm_tree_cell_str(sb, sr->node.qualified_name, true);
        pmm_tree_cell_str(sb, sr->node.label, false);
        pmm_tree_cell_str(sb, sr->node.file_path, false);
        pmm_tree_cell_str(sb, lines, false);
        pmm_tree_cell_int(sb, sr->in_degree, false);
        pmm_tree_cell_int(sb, sr->out_degree, false);
...
static int sg_cmp_by_qn(const void *pa, const void *pb) {
    const pmm_search_result_t *a = (const pmm_search_result_t *)pa;
    const pmm_search_result_t *b = (const pmm_search_result_t *)pb;
    const char *qa = a->node.qualified_name ? a->node.qualified_name : "";
    const char *qb = b->node.qualified_name ? b->node.qualified_name : "";
    return strcmp(qa, qb);
}

static void emit_search_results_tree(pmm_sb_t *sb, pmm_search_output_t *out, int offset,
                                     const char *const *fields, int nfields, pmm_store_t *store,
                                     const char *relationship, bool include_connected) {
    char buf[PMM_SZ_512];
    char extra_cols[PMM_SZ_256] = "";
    for (int f = 0; f < nfields; f++) {
    ...
    pmm_sb_append(sb, buf);
    /* Sort by qn so same-prefix rows are adjacent (module clustering). */
    if (out->count > 1) {
        qsort(out->results, (size_t)out->count, sizeof(pmm_search_result_t), sg_cmp_by_qn);
    }
    char cur_group[PMM_SZ_1K] = "";
    for (int i = 0; i < out->count; i++) {
        const pmm_search_result_t *sr = &out->results[i];
        ...
        if (nfields > 0) {
            ...
            for (int f = 0; f < nfields; f++) {
                ...
                if (v && yyjson_is_str(v)) {
                    ...
                    pmm_tree_cell_str(sb, "", false); /* emits "-" */
                }
            }
            if (pd) {
                yyjson_doc_free(pd);
...
static void emit_search_results_tree_json(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                          pmm_search_output_t *out, int offset,
                                          const char *const *fields, int nfields,
                                          pmm_store_t *store, const char *relationship,
...
static void emit_semantic_results_toon(pmm_sb_t *sb, const pmm_vector_result_t *vresults,
                                       int vcount) {
    static const char *const cols[] = {"qn", "label", "file", "score"};
    pmm_tree_table_header(sb, "semantic", vcount, cols, 4);
    for (int v = 0; v < vcount; v++) {
        pmm_tree_row_begin(sb);
        pmm_tree_cell_str(sb, vresults[v].qualified_name, true);
        pmm_tree_cell_str(sb, vresults[v].label, false);
        pmm_tree_cell_str(sb, vresults[v].file_path, false);
        pmm_tree_cell_real(sb, vresults[v].score, false);
        pmm_tree_row_end(sb);
    }
}

static char *handle_search_graph(pmm_mcp_server_t *srv, const char *args) {
    ...
    pmm_mem_phase_mark("handler.args");
    char *project = get_project_arg(args);
    pmm_mem_phase_mark("handler.resolve_store");
    pmm_store_t *store = resolve_store(srv, project);
    pmm_mem_phase_mark("handler.body");
    REQUIRE_STORE(store, project);
    ...
    char *format_arg = pmm_mcp_get_string_arg(args, "format");
    ...
    if (query && query[0]) {
        int q_limit = pmm_mcp_get_int_arg(args, "limit", BM25_DEFAULT_LIMIT);
        int q_offset = pmm_mcp_get_int_arg(args, "offset", 0);
        char *q_file_pattern = pmm_mcp_get_string_arg(args, "file_pattern");
        char *bm25_json =
        ...
        if (bm25_json) {
            free(query);
            free(project);
            char *result = pmm_mcp_text_result(bm25_json, false);
            free(bm25_json);
            return result;
        }
    }
    ...
    char *label = pmm_mcp_get_string_arg(args, "label");
    char *name_pattern = pmm_mcp_get_string_arg(args, "name_pattern");
    char *qn_pattern = pmm_mcp_get_string_arg(args, "qn_pattern");
    char *file_pattern = pmm_mcp_get_string_arg(args, "file_pattern");
    char *relationship = pmm_mcp_get_string_arg(args, "relationship");
    bool exclude_entry_points = pmm_mcp_get_bool_arg(args, "exclude_entry_points");
    bool include_connected = pmm_mcp_get_bool_arg(args, "include_connected");
    int limit = pmm_mcp_get_int_arg(args, "limit", PMM_DEFAULT_SEARCH_LIMIT);
    int offset = pmm_mcp_get_int_arg(args, "offset", 0);
    int min_degree = pmm_mcp_get_int_arg(args, "min_degree", PMM_NOT_FOUND);
    int max_degree = pmm_mcp_get_int_arg(args, "max_degree", PMM_NOT_FOUND);
    ...
    if (relationship && !validate_edge_type(relationship)) {
        ...
        free(file_pattern);
        free(relationship);
        return pmm_mcp_text_result("relationship must be uppercase letters and underscores", true);
    }
    ...
    if (!legacy_json) {
        ...
        char *sg_detail = pmm_mcp_get_string_arg(args, "detail");
        ...
        if (!sq_type_error) {
            ...
            bool has_filters = label || name_pattern || qn_pattern || file_pattern ||
                               ...
                               min_degree != PMM_NOT_FOUND || max_degree != PMM_NOT_FOUND;
            bool semantic_only = sq_present && !has_filters;

            pmm_sb_t sb;
            pmm_sb_init(&sb);
            pmm_search_output_t tout = {0};
            if (!semantic_only) {
                pmm_store_search(store, &params, &tout);
            ...
            if (vcount > 0) {
                ...
                static const char *const sem_cols[] = {"qn", "label", "file", "score"};
                pmm_tree_table_header(&sb, "semantic", 0, sem_cols, 4);
                pmm_tree_scalar_str(&sb, "hint",
            ...
            free(file_pattern);
            free(relationship);
            char *text = pmm_sb_finish(&sb);
            char *result = pmm_mcp_text_result(text ? text : "out of memory", text == NULL);
            free(text);
            return result;
        }
    ...
    pmm_store_search(store, &params, &out);
...
static char *handle_query_graph(pmm_mcp_server_t *srv, const char *args) {
    char *query = pmm_mcp_get_string_arg(args, "query");
    char *project = get_project_arg(args);
    pmm_store_t *store = resolve_store(srv, project);
    int max_rows = pmm_mcp_get_int_arg(args, "max_rows", 0);
    ...
    char *graph_arg = pmm_mcp_get_string_arg(args, "graph");
    bool missed_graph = graph_arg && strcmp(graph_arg, "missed") == 0;
    free(graph_arg);

    if (!query) {
        free(project);
        return pmm_mcp_text_result("query is required", true);
    }
    if (missed_graph && !project) {
        free(query);
        return pmm_mcp_text_result("project is required when graph=\"missed\"", true);
    }
    if (!store) {
    ...
    char covproj[PMM_SZ_512];
    const char *cypher_project = project;
    if (missed_graph) {
        pmm_store_coverage_shadow_project(covproj, sizeof(covproj), project);
        cypher_project = covproj;
    }

    pmm_cypher_result_t result = {0};
    int rc = pmm_cypher_execute(store, query, cypher_project, max_rows, &result);

    if (rc < 0) {
        char *err_msg = result.error ? result.error : "query execution failed";
        char *resp = pmm_mcp_text_result(err_msg, true);
        pmm_cypher_result_free(&result);
        free(query);
        free(project);
        return resp;
    }
    ...
    char *qg_format = pmm_mcp_get_string_arg(args, "format");
    ...
    if (!qg_legacy_json) {
        pmm_sb_t sb;
        pmm_sb_init(&sb);
        pmm_tree_table_header(&sb, "rows", result.row_count, (const char *const *)result.columns,
                              result.col_count);
        for (int r = 0; r < result.row_count; r++) {
            pmm_tree_row_begin(&sb);
            for (int c = 0; c < result.col_count; c++) {
                pmm_tree_cell_str(&sb, result.rows[r][c], c == 0);
            }
            pmm_tree_row_end(&sb);
        }
        pmm_tree_scalar_int(&sb, "total", result.row_count);
        if (result.warning) {
            pmm_tree_scalar_str(&sb, "warning", result.warning);
        }
    ...
    char *res = pmm_mcp_text_result(json ? json : "out of memory", json == NULL);
    free(json);
    return res;
}
...
static void add_coverage_report(yyjson_mut_doc *doc, yyjson_mut_val *root, pmm_store_t *store,
                                const char *project) {
    pmm_coverage_row_t *rows = NULL;
    int count = 0;
    (void)pmm_store_coverage_get(store, project, &rows, &count);
    ...
    pmm_store_free_coverage(rows, count);
    ...
    if (ni_dir_n > 0 || ni_file_n > 0) {
        yyjson_mut_obj_add_str(doc, ni, "note",
                               ...
                               "gitignore/.pmmignore/skip-lists (see each file's reason). Not an "
...
bool pmm_path_within_root(const char *root_path, const char *abs_path); /* defined below */
...
#ifdef __APPLE__
    return ((int64_t)st->st_mtimespec.tv_sec * (int64_t)PMM_NSEC_PER_SEC) +
           (int64_t)st->st_mtimespec.tv_nsec;
#elif defined(_WIN32)
    return (int64_t)st->st_mtime * (int64_t)PMM_NSEC_PER_SEC;
#else
    return ((int64_t)st->st_mtim.tv_sec * (int64_t)PMM_NSEC_PER_SEC) + (int64_t)st->st_mtim.tv_nsec;
#endif
}

static const char *coverage_path_freshness(pmm_store_t *store, const char *project,
    ...
    *outside = false;
    ...
    if (!pmm_path_within_root(root_path, abs_path)) {
        *outside = true;
        return "outside_project";
    }

    pmm_file_hash_t hash = {0};
    int rc = pmm_store_get_file_hash(store, project, rel_path, &hash);
    if (rc == PMM_STORE_NOT_FOUND) {
        return "not_tracked";
    }
    if (rc != PMM_STORE_OK) {
        return "unavailable";
    ...
    pmm_store_clear_file_hash(&hash);
...
static void coverage_add_row_json(yyjson_mut_doc *doc, yyjson_mut_val *array,
                                  const pmm_coverage_row_t *row, const char *requested_path) {
    yyjson_mut_val *item = yyjson_mut_obj(doc);
...
static const char *coverage_status(const pmm_coverage_row_t *rows, int count,
...
static char *handle_check_index_coverage(pmm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    pmm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
    ...
    pmm_project_t proj = {0};
    bool have_project = pmm_store_get_project(store, project, &proj) == PMM_STORE_OK;
    pmm_coverage_meta_t meta = {0};
    bool have_meta = pmm_store_coverage_meta_get(store, project, &meta) == PMM_STORE_OK;
    ...
    if (paths) {
        yyjson_arr_foreach(paths, idx, max, value) {
            ...
            pmm_coverage_row_t *rows = NULL;
            int row_count = 0;
            int cov_rc = pmm_store_coverage_get_path(store, project, rel, &rows, &row_count);
            bool lookup_ok = cov_rc == PMM_STORE_OK || cov_rc == PMM_STORE_NOT_FOUND;
            if (!lookup_ok) {
                row_count = 0;
            ...
            pmm_store_free_coverage(rows, row_count);
            yyjson_mut_arr_add_val(path_results, item);
        }
    }
    yyjson_mut_obj_add_val(doc, root, "paths", path_results);

    int scope_limit = pmm_mcp_get_int_arg(args, "scope_limit", COVERAGE_SCOPE_DEFAULT_LIMIT);
    int scope_offset = pmm_mcp_get_int_arg(args, "scope_offset", 0);
    if (scope_limit < 1) {
        scope_limit = 1;
    ...
    if (scopes) {
        yyjson_arr_foreach(scopes, idx, max, value) {
            ...
            pmm_coverage_row_t *rows = NULL;
            int row_count = 0;
            int cov_rc = pmm_store_coverage_get_scope(store, project, scope, &rows, &row_count);
            bool lookup_ok = cov_rc == PMM_STORE_OK || cov_rc == PMM_STORE_NOT_FOUND;
            if (!lookup_ok) {
                row_count = 0;
                yyjson_mut_obj_add_str(doc, item, "coverage_lookup", "error");
    ...
    yyjson_doc_free(adoc);
    if (have_meta) {
        pmm_store_coverage_meta_clear(&meta);
    }
    if (have_project) {
...
static char *handle_index_status(pmm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    pmm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
     ...
     * stays lean. */
    bool verbose = pmm_mcp_get_bool_arg(args, "verbose");
    ...
    if (project) {
        int nodes = pmm_store_count_nodes(store, project);
        int edges = pmm_store_count_edges(store, project);
        yyjson_mut_obj_add_str(doc, root, "project", project);
        yyjson_mut_obj_add_int(doc, root, "nodes", nodes);
        yyjson_mut_obj_add_int(doc, root, "edges", edges);
        yyjson_mut_obj_add_str(doc, root, "status", nodes > 0 ? "ready" : "empty");
        pmm_project_t proj_info = {0};
        if (pmm_store_get_project(store, project, &proj_info) == PMM_STORE_OK) {
...
static char *handle_delete_project(pmm_mcp_server_t *srv, const char *args) {
    ...
    if (!mcp_project_mutation_begin(srv, name)) {
        free(name);
        return pmm_mcp_text_result("project operation cancelled or blocked by an active index",
                                   true);
    }
    ...
    if (srv->watcher) {
        pmm_watcher_unwatch(srv->watcher, name);
    }

    pmm_mem_collect(); /* return freed pages to OS after closing database */
    ...
    char *result = pmm_mcp_text_result(json, is_error);
    free(json);
    return result;
}
 ...
 * pmm_store_arch_aspect_in_overview in store.c), or the aspect name itself. */
...
static bool aspect_wanted(yyjson_doc *aspects_doc, yyjson_val *aspects_arr, const char *name) {
    ...
    while ((val = yyjson_arr_iter_next(&iter)) != NULL) {
        ...
        if (strcmp(s, "overview") == 0 && pmm_store_arch_aspect_in_overview(name)) {
            return true;
        }
        if (strcmp(s, name) == 0) {
            return true;
...
static void append_cross_repo_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                      const pmm_schema_info_t *schema) {
 ...
 * PMM_STORE_OK, or PMM_STORE_ERR on failure (all outs zeroed). */
enum { ARCH_SCC_MAX_EDGES = 400000, ARCH_SCC_MAX_CYCLES = 100, ARCH_SCC_MEMBERS_SHOWN = 20 };

static int arch_compute_cycles(pmm_store_t *store, const char *project, int64_t ***out_members,
    ...
    *edges_truncated = false;
    ...
    int64_t *tgt = NULL;
    int ecount = 0;
    if (pmm_store_fetch_call_edges(store, project, ARCH_SCC_MAX_EDGES, &src, &tgt, &ecount,
                                   edges_truncated) != PMM_STORE_OK) {
    ...
    *scanned_edges = ecount;
    ...
    if (!scc_build(src, tgt, ecount, &g)) {
        free(src);
        free(tgt);
        return PMM_STORE_OK; /* no edges = no cycles, not an error */
    }
    free(src);
    free(tgt);
...
static void arch_node_qn(pmm_store_t *store, int64_t id, char *out, size_t outsz) {
    pmm_node_t n = {0};
    if (pmm_store_find_node_by_id(store, id, &n) == PMM_STORE_OK && n.qualified_name) {
...
static char *handle_get_architecture(pmm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *scope_path = pmm_mcp_get_string_arg(args, "path");
    pmm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
    ...
    /* Build a C string array from aspects for pmm_store_get_architecture.
    ...
    for (int i = 0; i < aspects_strs_count; i++) {
        if (!aspect_is_valid(aspects_strs[i])) {
            ...
            snprintf(msg, sizeof(msg), "Unknown aspect '%s'. Valid: %s.", aspects_strs[i],
                     valid_list);
            char *err = pmm_mcp_text_result(msg, true);
            free(project);
            free(scope_path);
            if (aspects_doc) {
    ...
    pmm_store_get_schema_counts_scoped(store, project, scope_path, &schema);

    pmm_architecture_info_t arch = {0};
    pmm_store_get_architecture(store, project, scope_path, aspects_strs, aspects_strs_count, &arch);

    int node_count = pmm_store_count_nodes_scoped(store, project, scope_path);
    int edge_count = pmm_store_count_edges_scoped(store, project, scope_path);
    char norm_path[PMM_SZ_512];
    bool path_scoped = pmm_store_normalize_arch_path(scope_path, norm_path, sizeof(norm_path));

    /* Response encoding: tree tables by default; format:"json" emits the
     * same model as structured JSON ({cols, rows} per section). */
    char *arch_format = pmm_mcp_get_string_arg(args, "format");
    ...
    if (!arch_legacy_json) {
        ...
        pmm_sb_init(&sb);
        if (project) {
            pmm_tree_scalar_str(&sb, "project", project);
        }
        if (default_summary) {
            pmm_tree_scalar_str(&sb, "aspects_hint",
        ...
        if (path_scoped) {
            pmm_tree_scalar_str(&sb, "path", norm_path);
            pmm_tree_scalar_int(&sb, "root_total_nodes", pmm_store_count_nodes(store, project));
            pmm_tree_scalar_int(&sb, "root_total_edges", pmm_store_count_edges(store, project));
            pmm_tree_scalar_int(&sb, "scoped_total_nodes", node_count);
            pmm_tree_scalar_int(&sb, "scoped_total_edges", edge_count);
        }
        pmm_tree_scalar_int(&sb, "total_nodes", node_count);
        pmm_tree_scalar_int(&sb, "total_edges", edge_count);

        if (aspect_wanted(aspects_doc, aspects_arr, "structure") && schema.node_label_count > 0) {
            static const char *const lcols[] = {"label", "count"};
            pmm_tree_table_header(&sb, "node_labels", schema.node_label_count, lcols, 2);
            for (int i = 0; i < schema.node_label_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, schema.node_labels[i].label, true);
                pmm_tree_cell_int(&sb, schema.node_labels[i].count, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (aspect_wanted(aspects_doc, aspects_arr, "dependencies") && schema.edge_type_count > 0) {
            static const char *const tcols[] = {"type", "count"};
            pmm_tree_table_header(&sb, "edge_types", schema.edge_type_count, tcols, 2);
            for (int i = 0; i < schema.edge_type_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, schema.edge_types[i].type, true);
                pmm_tree_cell_int(&sb, schema.edge_types[i].count, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (aspect_wanted(aspects_doc, aspects_arr, "routes") && schema.rel_pattern_count > 0) {
            static const char *const pcols[] = {"pattern"};
            pmm_tree_table_header(&sb, "relationship_patterns", schema.rel_pattern_count, pcols, 1);
            for (int i = 0; i < schema.rel_pattern_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, schema.rel_patterns[i], true);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.language_count > 0) {
            static const char *const gcols[] = {"language", "files"};
            pmm_tree_table_header(&sb, "languages", arch.language_count, gcols, 2);
            for (int i = 0; i < arch.language_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.languages[i].language, true);
                pmm_tree_cell_int(&sb, arch.languages[i].file_count, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.package_count > 0) {
            static const char *const kcols[] = {"name", "nodes", "fan_in", "fan_out"};
            pmm_tree_table_header(&sb, "packages", arch.package_count, kcols, 4);
            for (int i = 0; i < arch.package_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.packages[i].name, true);
                pmm_tree_cell_int(&sb, arch.packages[i].node_count, false);
                pmm_tree_cell_int(&sb, arch.packages[i].fan_in, false);
                pmm_tree_cell_int(&sb, arch.packages[i].fan_out, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.entry_point_count > 0) {
            ...
            pmm_tree_table_header(&sb, "entry_points", arch.entry_point_count, ecols, 2);
            for (int i = 0; i < arch.entry_point_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.entry_points[i].qualified_name, true);
                pmm_tree_cell_str(&sb, arch.entry_points[i].file, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.route_count > 0) {
            static const char *const rcols[] = {"method", "path", "handler"};
            pmm_tree_table_header(&sb, "routes", arch.route_count, rcols, 3);
            for (int i = 0; i < arch.route_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.routes[i].method, true);
                pmm_tree_cell_str(&sb, arch.routes[i].path, false);
                pmm_tree_cell_str(&sb, arch.routes[i].handler, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.hotspot_count > 0) {
            static const char *const hcols[] = {"qn", "fan_in"};
            pmm_tree_table_header(&sb, "hotspots", arch.hotspot_count, hcols, 2);
            for (int i = 0; i < arch.hotspot_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.hotspots[i].qualified_name, true);
                pmm_tree_cell_int(&sb, arch.hotspots[i].fan_in, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.boundary_count > 0) {
            static const char *const bcols[] = {"from", "to", "calls"};
            pmm_tree_table_header(&sb, "boundaries", arch.boundary_count, bcols, 3);
            for (int i = 0; i < arch.boundary_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.boundaries[i].from, true);
                pmm_tree_cell_str(&sb, arch.boundaries[i].to, false);
                pmm_tree_cell_int(&sb, arch.boundaries[i].call_count, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.service_count > 0) {
            static const char *const scols[] = {"from", "to", "type", "count"};
            pmm_tree_table_header(&sb, "services", arch.service_count, scols, 4);
            for (int i = 0; i < arch.service_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.services[i].from, true);
                pmm_tree_cell_str(&sb, arch.services[i].to, false);
                pmm_tree_cell_str(&sb, arch.services[i].type, false);
                pmm_tree_cell_int(&sb, arch.services[i].count, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.layer_count > 0) {
            static const char *const ycols[] = {"name", "layer", "reason"};
            pmm_tree_table_header(&sb, "layers", arch.layer_count, ycols, 3);
            for (int i = 0; i < arch.layer_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.layers[i].name, true);
                pmm_tree_cell_str(&sb, arch.layers[i].layer, false);
                pmm_tree_cell_str(&sb, arch.layers[i].reason, false);
                pmm_tree_row_end(&sb);
            }
        }
        if (arch.cluster_count > 0) {
            ...
            pmm_tree_table_header(&sb, "clusters", arch.cluster_count, ccols, 7);
            for (int i = 0; i < arch.cluster_count; i++) {
                const pmm_cluster_info_t *c = &arch.clusters[i];
                char joined[PMM_SZ_1K];
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_int(&sb, c->id, true);
                pmm_tree_cell_str(&sb, c->label, false);
                pmm_tree_cell_int(&sb, c->members, false);
                pmm_tree_cell_real(&sb, c->cohesion, false);
                arch_join_list(joined, sizeof(joined), c->top_nodes, c->top_node_count);
                pmm_tree_cell_str(&sb, joined, false);
        ...
        if (arch.file_tree_count > 0) {
            ...
            pmm_tree_table_header(&sb, "file_tree", arch.file_tree_count, fcols, 3);
            for (int i = 0; i < arch.file_tree_count; i++) {
                pmm_tree_row_begin(&sb);
                pmm_tree_cell_str(&sb, arch.file_tree[i].path, true);
                pmm_tree_cell_str(&sb, arch.file_tree[i].type, false);
                pmm_tree_cell_int(&sb, arch.file_tree[i].children, false);
                pmm_tree_row_end(&sb);
            }
        }
        /* Cross-repo edge summary (mirrors append_cross_repo_summary). */
        {
            ...
            if (cross_total > 0) {
                pmm_tree_scalar_int(&sb, "cross_repo_links_total", cross_total);
            }
        }
        ...
        if (aspect_explicitly_named(aspects_arr, "cycles")) {
            ...
            if (arch_compute_cycles(store, project, &members, &sizes, &ncyc, &total_cyc, &scanned,
                                    &etrunc) == PMM_STORE_OK) {
                pmm_tree_scalar_int(&sb, "call_edges_scanned", scanned);
                pmm_tree_scalar_int(&sb, "cycles_total", total_cyc);
                if (etrunc) {
                    pmm_tree_scalar_bool(&sb, "cycles_partial", true);
                    pmm_tree_scalar_str(&sb, "cycles_hint",
                                        "call graph exceeded the scan budget; cycle list may be "
                                        "incomplete");
                }
                if (total_cyc > ncyc) {
                    char omit[PMM_SZ_128];
                    snprintf(omit, sizeof(omit), "cycles_omitted: %d  (showing the first %d)\n",
                             total_cyc - ncyc, ncyc);
                    pmm_sb_append(&sb, omit);
                }
                char hdr[PMM_SZ_128];
                snprintf(hdr, sizeof(hdr),
                         "cycles: %d  (rows: size members; circular CALLS dependencies)\n", ncyc);
                pmm_sb_append(&sb, hdr);
        ...
        pmm_store_architecture_free(&arch);
        pmm_store_schema_free(&schema);
        if (aspects_doc) {
            yyjson_doc_free(aspects_doc);
        }
    ...
    if (path_scoped) {
        ...
        int root_nodes = pmm_store_count_nodes(store, project);
        int root_edges = pmm_store_count_edges(store, project);
...
static const char *bfs_edge_args_for_hop(pmm_traverse_result_t *tr, int64_t hop_node_id,
                                         size_t *out_len) {
...
static void bfs_to_toon_table(pmm_sb_t *sb, const char *key, pmm_traverse_result_t *tr,
    ...
    pmm_tree_table_header(sb, key, visible, cols, ncols);
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        pmm_tree_row_begin(sb);
        pmm_tree_cell_str(sb, tr->visited[i].node.qualified_name, true);
        pmm_tree_cell_int(sb, tr->visited[i].hop, false);
        if (risk_labels) {
            pmm_tree_cell_str(sb, pmm_risk_label(pmm_hop_to_risk(tr->visited[i].hop)), false);
        }
        if (include_tests) {
            pmm_tree_cell_bool(sb, test, false);
        }
        if (data_flow) {
            size_t alen = 0;
            const char *ea = bfs_edge_args_for_hop(tr, tr->visited[i].node.id, &alen);
            if (ea && alen > 0 && alen < PMM_SZ_1K) {
                char abuf[PMM_SZ_1K];
                memcpy(abuf, ea, alen);
                abuf[alen] = '\0';
                pmm_tree_cell_str(sb, abuf, false);
            } else {
                pmm_tree_cell_str(sb, "", false);
            }
        }
...
static char *snippet_suggestions(const char *input, pmm_node_t *nodes, int count);
...
static long node_resolution_score(const pmm_node_t *n) {
...
static bool node_is_real_callable_def(const pmm_node_t *n) {
    if (!n->label) {
        return false;
    }
    if (strcmp(n->label, "Function") != 0 && strcmp(n->label, "Method") != 0) {
...
static int pick_resolved_node(const pmm_node_t *nodes, int count, bool *ambiguous) {
    *ambiguous = false;
    if (count <= 1) {
        return 0;
    }
...
static int node_hop_cmp_hop_id(const void *pa, const void *pb) {
    const pmm_node_hop_t *a = (const pmm_node_hop_t *)pa;
    const pmm_node_hop_t *b = (const pmm_node_hop_t *)pb;
    if (a->hop != b->hop) {
 ...
 * transfers into *out, freed by pmm_store_traverse_free. */
static void bfs_union_same_name(pmm_store_t *store, const pmm_node_t *nodes, int node_count,
                                const char *direction, const char **edge_types, int edge_type_count,
                                int depth, int limit, pmm_traverse_result_t *out) {
    memset(out, 0, sizeof(*out));
    int vcap = 0, ecap = 0;
    for (int k = 0; k < node_count; k++) {
        pmm_traverse_result_t tr = {0};
        pmm_store_bfs(store, nodes[k].id, direction, edge_types, edge_type_count, depth, limit,
                      &tr);
        for (int i = 0; i < tr.visited_count; i++) {
            bool dup = false;
            ...
            if (out->visited_count >= vcap) {
                vcap = vcap ? vcap * 2 : 8;
                out->visited = safe_realloc(out->visited, vcap * sizeof(pmm_node_hop_t));
            }
        ...
        for (int i = 0; i < tr.edge_count; i++) {
            ...
            if (out->edge_count >= ecap) {
                ecap = ecap ? ecap * 2 : 8;
                out->edges = safe_realloc(out->edges, ecap * sizeof(pmm_edge_info_t));
            }
            out->edges[out->edge_count++] = tr.edges[i];
            memset(&tr.edges[i], 0, sizeof(tr.edges[i])); /* ownership moved */
        }
        pmm_store_traverse_free(&tr); /* frees only the un-moved (root + dup) fields */
    }
    ...
    if (out->visited_count > 1) {
        qsort(out->visited, (size_t)out->visited_count, sizeof(pmm_node_hop_t),
              node_hop_cmp_hop_id);
    }
}
...
static int trace_watermark_index(const pmm_traverse_result_t *tr, int hop, int64_t node_id) {
    for (int i = 0; i < tr->visited_count; i++) {
...
static yyjson_mut_val *bfs_to_tree_json(yyjson_mut_doc *doc, pmm_traverse_result_t *tr,
                                        bool risk_labels, bool include_tests, bool data_flow) {
    ...
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        if (risk_labels) {
            yyjson_mut_arr_add_str(doc, row, pmm_risk_label(pmm_hop_to_risk(tr->visited[i].hop)));
        }
        if (data_flow) {
            size_t alen = 0;
            const char *ea = bfs_edge_args_for_hop(tr, tr->visited[i].node.id, &alen);
...
static void bfs_to_tree_table(pmm_sb_t *sb, const char *key, pmm_traverse_result_t *tr,
                              bool include_tests) {
    int visible = 0;
    for (int i = 0; i < tr->visited_count; i++) {
        if (!include_tests && is_test_file(tr->visited[i].node.file_path)) {
    ...
    if (tr->visited_count > 1) {
        qsort(tr->visited, (size_t)tr->visited_count, sizeof(pmm_node_hop_t), tree_hop_cmp_qn);
    }
    char cur_group[PMM_SZ_1K] = "";
    for (int i = 0; i < tr->visited_count; i++) {
        if (!include_tests && is_test_file(tr->visited[i].node.file_path)) {
...
/* Clamp a client-supplied traversal depth to the MCP ceiling (pmm_mcp_max_depth),
 * WARN-logging when it does so — never a silent truncation (#887). An unclamped
 * `depth` would drive the shared pmm_store_bfs to an arbitrary hop count. */
static int clamp_mcp_depth(int depth, const char *tool) {
    int cap = pmm_mcp_max_depth();
    if (depth > cap) {
        char req_buf[16];
        char cap_buf[16];
        snprintf(req_buf, sizeof(req_buf), "%d", depth);
        snprintf(cap_buf, sizeof(cap_buf), "%d", cap);
        pmm_log_warn("mcp.depth_capped", "tool", tool, "requested", req_buf, "cap", cap_buf);
        return cap;
    }
    return depth;
}

static char *handle_trace_call_path(pmm_mcp_server_t *srv, const char *args) {
    char *func_name = pmm_mcp_get_string_arg(args, "function_name");
    char *project = get_project_arg(args);
    pmm_store_t *store = resolve_store(srv, project);
    char *direction = pmm_mcp_get_string_arg(args, "direction");
    char *mode = pmm_mcp_get_string_arg(args, "mode");
    char *param_name = pmm_mcp_get_string_arg(args, "parameter_name");
    int depth = pmm_mcp_get_int_arg(args, "depth", MCP_DEFAULT_DEPTH);
     ...
     * depth clamp, #887). */
    int trace_limit = pmm_mcp_get_int_arg(args, "limit", MCP_BFS_LIMIT);
    if (trace_limit < 1) {
        trace_limit = 1;
    }
    ...
    bool risk_labels = pmm_mcp_get_bool_arg(args, "risk_labels");
    bool include_tests = pmm_mcp_get_bool_arg(args, "include_tests");
    ...
    if (!func_name) {
        ...
        free(mode);
        free(param_name);
        return pmm_mcp_text_result("function_name is required", true);
    }
    if (!store) {
        char *_err = build_project_list_error("project not found or not indexed");
        char *_res = pmm_mcp_text_result(_err, true);
    ...
    char generation[96];
    (void)pmm_store_generation(store, generation, sizeof(generation));
    char *cursor_arg = pmm_mcp_get_string_arg(args, "cursor");
    ...
    if (cursor_arg && cursor_arg[0]) {
        uint64_t qh =
            trace_params_hash(project, func_name, direction ? direction : "both", mode,
                              pmm_mcp_get_int_arg(args, "depth", MCP_DEFAULT_DEPTH), include_tests,
                              pmm_mcp_get_int_arg(args, "limit", MCP_BFS_LIMIT));
        ...
        if (cerr) {
            ...
            free(mode);
            free(param_name);
            return pmm_mcp_text_result(cerr, true);
        }
        have_cursor = true;
    }
    free(cursor_arg);
    ...
    if (strcmp(direction, "inbound") != 0 && strcmp(direction, "outbound") != 0 &&
        ...
        free(mode);
        free(param_name);
        return pmm_mcp_text_result(errbuf, true);
    }
     ...
     * fields cleanly and pmm_store_free_nodes will free them. */
    pmm_node_t *nodes = NULL;
    int node_count = 0;
    pmm_store_find_nodes_by_name(store, project, func_name, &nodes, &node_count);

    if (node_count == 0) {
        pmm_node_t qn_node = {0};
        if (pmm_store_find_node_by_qn(store, project, func_name, &qn_node) == PMM_STORE_OK) {
            nodes = malloc(sizeof(pmm_node_t));
            if (nodes) {
                nodes[0] = qn_node;
                node_count = 1;
            } else {
    ...
    if (node_count == 0) {
        ...
        free(mode);
        free(param_name);
        pmm_store_free_nodes(nodes, 0);
        return pmm_mcp_text_result(hint, true);
    }
    ...
    if (trace_ambiguous) {
        ...
        free(mode);
        free(param_name);
        pmm_store_free_nodes(nodes, node_count);
        return result;
    }

    /* Response encoding: tree tables by default; format:"json" emits the
     * same grouped model as structured JSON. */
    char *trace_format = pmm_mcp_get_string_arg(args, "format");
    ...
    if (!trace_legacy_json) {
        pmm_sb_t sb;
        pmm_sb_init(&sb);
        pmm_tree_scalar_str(&sb, "function", func_name);
        pmm_tree_scalar_str(&sb, "direction", direction);
        if (mode) {
            pmm_tree_scalar_str(&sb, "mode", mode);
        }
        ...
        if (do_outbound) {
            pmm_tree_scalar_int(&sb, "callees_total", out_total);
            if (flat_trace) {
        ...
        if (do_inbound) {
            pmm_tree_scalar_int(&sb, "callers_total", in_total);
            if (flat_trace) {
        ...
        if (more_rows) {
            pmm_tree_scalar_bool(&sb, "truncated", true);
            if (next_tok[0]) {
                pmm_tree_scalar_str(&sb, "next", next_tok);
                pmm_tree_scalar_str(&sb, "hint",
...
static char *read_file_lines(const char *path, int start, int end) {
    FILE *fp = pmm_fopen(path, "r");
    if (!fp) {
        return NULL;
    }

    size_t cap = PMM_SZ_4K;
    char *buf = malloc(cap);
    size_t len = 0;
    buf[0] = '\0';
...
static char *project_root_from_store(pmm_store_t *store, const char *project) {
    if (!store || !project) {
        return NULL;
    }
    pmm_project_t proj = {0};
    if (pmm_store_get_project(store, project, &proj) != PMM_STORE_OK) {
        return NULL;
    }
...
static char *get_project_root(pmm_mcp_server_t *srv, const char *project) {
...
static char *handle_cross_repo_mode(pmm_mcp_server_t *srv, const char *repo_path,
                                    const char *name_override, const char *args) {
    if (name_override && name_override[0] && !pmm_validate_project_name(name_override)) {
        return pmm_mcp_text_result("invalid project name", true);
    }
    char *project = name_override && name_override[0] ? heap_strdup(name_override)
                                                      : pmm_project_name_from_path(repo_path);
    if (!project) {
        return pmm_mcp_text_result("cannot derive project name", true);
    }
    ...
    if (target_count > MCP_MAX_CROSS_REPO_TARGETS) {
        yyjson_doc_free(jdoc);
        free(project);
        return pmm_mcp_text_result("too many cross-repo target projects", true);
    }
    ...
    if (!targets || !lease_keys) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return pmm_mcp_text_result("failed to allocate cross-repo project leases", true);
    }
    size_t idx;
    size_t max;
    yyjson_val *val;
    ...
    yyjson_arr_foreach(tp_arr, idx, max, val) {
        ...
        if (!target || !target[0] || strlen(target) >= PMM_SZ_256 ||
            (strcmp(target, "*") != 0 && !pmm_validate_project_name(target))) {
            invalid_target = true;
            break;
        }
        targets[ti++] = target;
    ...
    if (invalid_target || ti != tp_count) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return pmm_mcp_text_result("target_projects must contain valid project names or '*'", true);
    }
    if (all_projects && tp_count != 1) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return pmm_mcp_text_result("target_projects wildcard '*' must be the only entry", true);
    }
    if (!all_projects) {
    ...
    if (held_count != lease_count || cancelled) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return pmm_mcp_text_result("cross-repo operation cancelled or blocked by active indexing",
                                   true);
    }

    pmm_cross_repo_result_t result = pmm_cross_repo_match_cancellable(
    ...
    yyjson_mut_doc_free(doc);
    free(project);
    char *out = pmm_mcp_text_result(json, result.cancelled);
    free(json);
    return out;
}
...
static void try_artifact_bootstrap(const char *project_name, const char *repo_path) {
    char db_buf[PMM_SZ_1K];
    project_db_path(project_name, db_buf, sizeof(db_buf));
    if (pmm_file_size(db_buf) < 0 && pmm_artifact_exists(repo_path)) {
        pmm_log_info("index.artifact_bootstrap", "project", project_name);
        pmm_artifact_import(repo_path, db_buf);
    }
}
...
static void add_not_indexed_files_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                          pmm_pipeline_t *p) {
    pmm_ignored_file_t *ignored = NULL;
    int stored = 0;
    int total = 0;
    pmm_pipeline_get_ignored(p, &ignored, &stored, &total);
    ...
    yyjson_mut_obj_add_str(doc, ni, "note",
                           "Excluded by design (gitignore/.pmmignore/skip-lists); examples only — "
...
static bool is_parse_partial(const pmm_file_error_t *e) {
...
static void add_skipped_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                const pmm_file_error_t *errs, int count, const char *logfile) {
    int skips = 0;
    for (int i = 0; i < count; i++) {
...
static void add_parse_partial_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                      const pmm_file_error_t *errs, int count) {
    int partials = 0;
    for (int i = 0; i < count; i++) {
        if (is_parse_partial(&errs[i])) {
            partials++;
 ...
 *   $PMM_INDEX_LOG (override) else <cache_dir>/logs/<project>-<epoch>.log
 * Returns true and fills out_path on success. */
static bool write_skip_logfile(const char *project, const pmm_file_error_t *errs, int count,
                               char *out_path, size_t out_sz) {
    if (!errs || count <= 0) {
        return false;
    }
    char path[PMM_SZ_1K];
    const char *override = getenv("PMM_INDEX_LOG");
    if (override && override[0]) {
        snprintf(path, sizeof(path), "%s", override);
    } else {
        const char *cdir = pmm_resolve_cache_dir();
        if (!cdir) {
            return false;
        }
        char logdir[PMM_SZ_1K];
        snprintf(logdir, sizeof(logdir), "%s/logs", cdir);
        pmm_mkdir_p(logdir, 0755);
        snprintf(path, sizeof(path), "%s/%s-%lld.log", logdir, project ? project : "index",
                 (long long)time(NULL));
    }
    FILE *f = pmm_fopen(path, "wb");
    if (!f) {
        pmm_log_warn("index.logfile_open_fail", "path", path);
        return false;
    }
    int partials = 0;
    for (int i = 0; i < count; i++) {
    ...
    (void)fprintf(f, "# project-mind-mcp index coverage report\n");
...
static bool build_index_success_response(pmm_mcp_server_t *srv, yyjson_mut_doc *doc,
                                         yyjson_mut_val *root, const char *project_name,
                                         const char *repo_path, bool persistence, pmm_pipeline_t *p,
                                         char **excluded_dirs, int excluded_count,
                                         const pmm_file_error_t *file_errors, int file_error_count,
                                         const char *logfile) {
    ...
    int exp_nodes = -1;
    int exp_edges = -1;
    pmm_pipeline_get_committed_counts(p, &exp_nodes, &exp_edges);

    const double ratio = pmm_dump_verify_min_ratio();
    const int min_floor = PMM_DUMP_VERIFY_MIN_FLOOR;

    pmm_store_t *store = resolve_store(srv, project_name);
    int nodes = 0;
    int edges = 0;
    bool degraded = false;
    ...
    if (!store) {
        ...
        nodes = pmm_store_count_nodes(store, project_name);
        edges = pmm_store_count_edges(store, project_name);
        if (nodes < 0) {
            degraded = true;
            nodes = 0;
            edges = edges >= 0 ? edges : 0;
        } else if (pmm_dump_verify_is_degraded(exp_nodes, nodes, ratio, min_floor)) {
            (void)pmm_store_checkpoint(store);
            int nodes2 = pmm_store_count_nodes(store, project_name);
            int edges2 = pmm_store_count_edges(store, project_name);
            if (nodes2 >= 0) {
                nodes = nodes2;
            }
            if (edges2 >= 0) {
            ...
            degraded = pmm_dump_verify_is_degraded(exp_nodes, nodes, ratio, min_floor);
        }
    }
    ...
    if (degraded) {
        if (!store) {
            ...
            pmm_log_warn("dump.verify", "reason", "store_missing", "expected_nodes",
            ...
            pmm_log_warn("dump.verify", "expected_nodes", exp_buf, "persisted_nodes", got_buf);
        }
    }
    ...
    bool has_artifact = pmm_artifact_exists(repo_path);
    ...
    if (persistence && has_artifact) {
        yyjson_mut_obj_add_str(doc, root, "artifact_hint",
                               "Persistent artifact written to .project-mind-mcp/graph.db.zst. "
...
static char *build_worker_failure_response(const char *args, pmm_proc_outcome_t outcome) {
    char *repo_path = pmm_mcp_get_string_arg(args, "repo_path");
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
    yyjson_mut_obj_add_str(doc, root, "status", "error");
    yyjson_mut_obj_add_str(doc, root, "outcome", pmm_proc_outcome_str(outcome));
    const char *hint = NULL;
    if (outcome == PMM_PROC_SPAWN_FAILED) {
        hint = "Indexing worker could not be started. Supervision is mandatory in a PMM "
               "host, so no in-process fallback was attempted.";
    } else if (outcome == PMM_PROC_HANG) {
        hint = "Indexing worker timed out (a file made no progress). The worker was "
               "terminated and the server survived. Re-run to retry.";
    } else if (outcome == PMM_PROC_CRASH) {
    ...
    } else if (outcome == PMM_PROC_CLEAN) {
    ...
    yyjson_mut_doc_free(doc);
    free(repo_path);
    char *result = pmm_mcp_text_result(json, true);
    free(json);
    return result;
}

static char *build_worker_unsafe_terminal_response(const char *args, pmm_proc_outcome_t outcome,
    ...
    yyjson_mut_doc_free(doc);
    free(repo_path);
    char *response = pmm_mcp_text_result(json, true);
    free(json);
    return response;
}
 ...
 * to invalidate. */
static void invalidate_cached_store(pmm_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    if (srv->owns_store && srv->store) {
 ...
 * per line — see pmm_index_mark_start/done) into the crash/hang SUSPECT set:
    ...
    *out_n = 0;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (line[0] == 'S') {
            ...
            open_paths[open_n++] = pmm_strdup(rel);
        } else {
...
pmm_mcp_supervised_result_disposition_t pmm_mcp_supervised_result_disposition(
    int spawn_result, const pmm_index_worker_result_t *worker_result) {
    if (spawn_result != 0 || !worker_result || worker_result->outcome == PMM_PROC_SPAWN_FAILED) {
        return PMM_MCP_SUPERVISED_RESULT_FALLBACK;
    }
    if (worker_result->cancellation_requested || !worker_result->tree_quiesced ||
        worker_result->supervision_failed) {
        return PMM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL;
    }
    if (worker_result->outcome == PMM_PROC_CLEAN) {
        return worker_result->response ? PMM_MCP_SUPERVISED_RESULT_SUCCESS
                                       : PMM_MCP_SUPERVISED_RESULT_FALLBACK;
    }
    return PMM_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE;
}
 ...
 * A physical PMM host never falls back to its in-process pipeline: an initial
 * start/protocol failure is returned as an explicit error response. */
static char *index_run_supervised(pmm_mcp_server_t *srv, const char *args) {
    invalidate_cached_store(srv);

    /* First attempt: normal parallel run. */
    pmm_index_worker_result_t wr;
    int rc = pmm_index_spawn_worker_with_log_cancel(
    ...
    pmm_mcp_supervised_result_disposition_t disposition =
        pmm_mcp_supervised_result_disposition(rc, &wr);

    if (disposition == PMM_MCP_SUPERVISED_RESULT_FALLBACK) {
        pmm_proc_outcome_t outcome = wr.outcome;
        pmm_index_worker_result_free(&wr);
        invalidate_cached_store(srv);
        return build_worker_failure_response(args, outcome);
    }
    if (disposition == PMM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
        char *failure =
    ...
    if (disposition == PMM_MCP_SUPERVISED_RESULT_SUCCESS) {
     ...
     * time. */
    pmm_proc_outcome_t last_outcome = wr.outcome;
    pmm_index_worker_result_free(&wr);

    char marker_path[PMM_SZ_1K];
    char quarantine_path[PMM_SZ_1K];
    ...
    FILE *qinit = pmm_fopen(quarantine_path, "wb");
    if (qinit) {
        (void)fclose(qinit);
    }

    int cap = 100;
    const char *cap_env = getenv("PMM_INDEX_MAX_RESTARTS");
    ...
    for (int i = 0; i < cap; i++) {
        pmm_index_worker_result_t wr2;
        int rc2 = pmm_index_spawn_worker_with_log_cancel(
        ...
        pmm_mcp_supervised_result_disposition_t recovery_disposition =
            pmm_mcp_supervised_result_disposition(rc2, &wr2);
        if (recovery_disposition == PMM_MCP_SUPERVISED_RESULT_FALLBACK) {
            last_outcome = wr2.outcome;
            pmm_index_worker_result_free(&wr2);
            break; /* spawn failed mid-recovery — give up */
        }
        if (recovery_disposition == PMM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
            last_outcome = wr2.outcome;
        ...
        if (recovery_disposition == PMM_MCP_SUPERVISED_RESULT_SUCCESS) {
            resp = wr2.response; /* transfer ownership to caller */
            wr2.response = NULL;
            pmm_index_worker_result_free(&wr2);
            break; /* good files indexed; quarantined files reported as crash/hang */
        }
        if (wr2.outcome == PMM_PROC_CRASH || wr2.outcome == PMM_PROC_HANG) {
            last_outcome = wr2.outcome;
            pmm_index_worker_result_free(&wr2);
            ...
            const char *phase = (last_outcome == PMM_PROC_HANG) ? "hang" : "crash";
            int sus_n = 0;
            ...
            if (!suspects || sus_n == 0) {
                ...
                pmm_log_warn("index.supervisor.unattributable", "action", "give_up");
                break;
            }
            if (prev_suspects) {
                ...
                if (!supervisor_append_quarantine(quarantine_path, pick, phase)) {
                    pmm_log_warn("index.supervisor.quarantine_write_fail", "path", pick);
                    supervisor_free_suspects(suspects, sus_n);
                    break;
                }
                quarantined++;
                ...
                pmm_log_warn("index.file_quarantined", "path", pick, "outcome", phase, "attempt",
                             attempt_buf);
            }
    ...
    if (!resp && !unsafe_terminal && quarantined > 0) {
        pmm_index_worker_result_t wrp;
        int rcp = pmm_index_spawn_worker_with_log_cancel(
            args, /*single_thread=*/false, NULL, quarantine_path,
            srv ? srv->index_log_callback : NULL, srv ? srv->index_log_context : NULL,
            srv ? &srv->pipeline_cancel_requested : NULL, &wrp);
        pmm_mcp_supervised_result_disposition_t partial_disposition =
            pmm_mcp_supervised_result_disposition(rcp, &wrp);
        if (partial_disposition == PMM_MCP_SUPERVISED_RESULT_SUCCESS) {
            resp = wrp.response; /* transfer ownership to caller */
            wrp.response = NULL;
            char qn[MCP_FIELD_SIZE];
            snprintf(qn, sizeof(qn), "%d", quarantined);
            pmm_log_error("index.supervisor.partial", "quarantined", qn, "outcome",
                          pmm_proc_outcome_str(last_outcome));
        } else if (partial_disposition == PMM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
            last_outcome = wrp.outcome;
            unsafe_terminal = true;
            terminal_cancelled = wrp.cancellation_requested;
        }
        pmm_index_worker_result_free(&wrp);
    }
...
static char *index_run_supervised_path(pmm_mcp_server_t *srv, const char *root_path) {
    if (!root_path || !root_path[0]) {
        return NULL;
    }
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
...
char *pmm_mcp_index_run_supervised_path(const char *root_path) {
...
static bool resolve_session_repo_path(pmm_mcp_server_t *srv, char **repo_path) {
...
static char *handle_index_repository(pmm_mcp_server_t *srv, const char *args) {
    char *repo_path = pmm_mcp_get_string_arg(args, "repo_path");
    char *mode_str = pmm_mcp_get_string_arg(args, "mode");
    char *name_override = pmm_mcp_get_string_arg(args, "name");
    pmm_normalize_path_sep(repo_path);

    if (!repo_path) {
        free(mode_str);
        free(name_override);
        return pmm_mcp_text_result("repo_path is required", true);
    }
    ...
    if (!resolve_session_repo_path(srv, &repo_path)) {
        ...
        free(name_override);
        free(repo_path);
        return pmm_mcp_text_result("failed to resolve repo_path", true);
    }
     ...
     * standalone server retains the process-wide PMM_ALLOWED_ROOT fallback. */
    const char *allowed_root =
        srv->allowed_root_policy_set ? srv->allowed_root : getenv("PMM_ALLOWED_ROOT");
    if (allowed_root && allowed_root[0] && repo_path &&
        !pmm_path_within_root(allowed_root, repo_path)) {
        free(mode_str);
        free(name_override);
        free(repo_path);
        return pmm_mcp_text_result("repo_path is outside the allowed root", true);
    }
    ...
    char *mutation_project =
        pmm_project_name_from_path(name_override && name_override[0] ? name_override : repo_path);
    if (!mutation_project) {
        free(repo_path);
        free(mode_str);
        free(name_override);
        return pmm_mcp_text_result("could not resolve index project name", true);
    }
    ...
    if (pmm_index_supervisor_should_wrap()) {
        char *worker_args = index_args_with_repo_path(args, repo_path);
        if (!worker_args) {
            ...
            free(mode_str);
            free(name_override);
            return pmm_mcp_text_result("failed to prepare supervised index request", true);
        }
    ...
    if (!mcp_project_mutation_begin(srv, mutation_project)) {
        ...
        free(mode_str);
        free(name_override);
        return pmm_mcp_text_result("index operation blocked by another mutation for this project",
                                   true);
    }
    if (mcp_request_cancelled(srv)) {
        ...
        free(mode_str);
        free(name_override);
        return pmm_mcp_text_result("index operation cancelled for this request", true);
    }

    pmm_index_mode_t mode = PMM_MODE_FULL;
    if (mode_str && strcmp(mode_str, "fast") == 0) {
        mode = PMM_MODE_FAST;
    } else if (mode_str && strcmp(mode_str, "moderate") == 0) {
        mode = PMM_MODE_MODERATE;
    }
    free(mode_str);

    bool persistence = pmm_mcp_get_bool_arg(args, "persistence");

    pmm_pipeline_t *p = pmm_pipeline_new(repo_path, NULL, mode);
    if (!p) {
        mcp_project_mutation_end(srv, mutation_project);
        free(mutation_project);
        free(name_override);
        free(repo_path);
        return pmm_mcp_text_result("failed to create pipeline", true);
    }
    if (name_override && name_override[0] && !pmm_pipeline_set_project_name(p, name_override)) {
        pmm_pipeline_free(p);
        mcp_project_mutation_end(srv, mutation_project);
        free(mutation_project);
        free(name_override);
    ...
    free(name_override);
    pmm_pipeline_set_persistence(p, persistence);

    char *project_name = heap_strdup(pmm_pipeline_project_name(p));
    ...
    pmm_pipeline_lock();
    pmm_pipeline_bind_cancel_flag(p, &srv->pipeline_cancel_requested);
    srv->active_pipeline = p;
    int rc = pmm_pipeline_run(p);
     ...
     * JSON doc, so they need only outlive that call, not pmm_pipeline_free. */
    char **excluded_dirs = NULL;
    int excluded_count = 0;
    pmm_pipeline_get_excluded(p, &excluded_dirs, &excluded_count);
    ...
    pmm_file_error_t *file_errors = NULL;
    int file_error_count = 0;
    pmm_pipeline_get_file_errors(p, &file_errors, &file_error_count);

    pmm_mem_collect(); /* return mimalloc pages to OS after large indexing */
    ...
    if (pmm_index_worker_active()) {
        pmm_log_info("index.worker.fast_exit", "skip", "pipeline_free");
    } else {
        pmm_pipeline_free(p);
    }
    free(project_name);
    ...
    char *result = pmm_mcp_text_result(json, rc != 0);
    free(json);
    return result;
}
...
static void copy_node(const pmm_node_t *src, pmm_node_t *dst) {
    dst->id = src->id;
 ...
 * (>= PMM_SZ_4K bytes); returns true on success. Isolating the per-OS resolver
 * keeps pmm_path_within_root's control flow unconditional: the previous `#ifdef`
...
static bool resolve_canonical_path(const char *path, char *out, size_t out_sz) {
    /* pmm_canonical_path: realpath on POSIX; an opened handle plus
    ...
    if (!pmm_canonical_path(path, out, out_sz)) {
        return false;
    }
#ifdef _WIN32
    pmm_normalize_path_sep(out);
#endif
    return true;
}
...
#ifdef _WIN32
    wchar_t *wide_root = pmm_utf8_to_wide(root_path);
    wchar_t *wide_candidate = pmm_utf8_to_wide(candidate_path);
...
bool pmm_path_within_root(const char *root_path, const char *abs_path) {
    if (!root_path || !abs_path) {
        return false;
    }
    char real_root[PMM_SZ_4K];
    char real_file[PMM_SZ_4K];
...
static void add_snippet_coverage_note(yyjson_mut_doc *doc, yyjson_mut_val *root_obj,
                                      pmm_store_t *store, const pmm_node_t *node) {
    if (!node->file_path || !node->file_path[0] || !node->project) {
        return;
    }
    pmm_coverage_row_t *rows = NULL;
    int count = 0;
    if (pmm_store_coverage_get_path(store, node->project, node->file_path, &rows, &count) !=
        PMM_STORE_OK) {
        return;
    }
    for (int i = 0; i < count; i++) {
...
static char *build_snippet_response(pmm_mcp_server_t *srv, pmm_node_t *node,
                                    const char *match_method, bool include_neighbors,
                                    pmm_node_t *alternatives, int alt_count) {
    ...
    pmm_store_t *store = srv->store;
    int in_deg = 0;
    int out_deg = 0;
    pmm_store_node_degree(store, node->id, &in_deg, &out_deg);
    ...
    if (include_neighbors) {
        pmm_store_node_neighbor_names(store, node->id, MCP_DEFAULT_LIMIT, &nb_callers,
...
static char *handle_get_code_snippet(pmm_mcp_server_t *srv, const char *args) {
    char *qn = pmm_mcp_get_string_arg(args, "qualified_name");
    char *project = get_project_arg(args);
    bool include_neighbors = pmm_mcp_get_bool_arg(args, "include_neighbors");

    if (!qn) {
        free(project);
        return pmm_mcp_text_result("qualified_name is required", true);
    }
    ...
    pmm_node_t node = {0};
    int rc = pmm_store_find_node_by_qn(store, effective_project, qn, &node);
    if (rc == PMM_STORE_OK) {
    ...
    pmm_node_t *suffix_nodes = NULL;
    int suffix_count = 0;
    pmm_store_find_nodes_by_qn_suffix(store, effective_project, qn, &suffix_nodes, &suffix_count);

    if (suffix_count == SKIP_ONE) {
        copy_node(&suffix_nodes[0], &node);
        pmm_store_free_nodes(suffix_nodes, suffix_count);
...
typedef struct {
    char file[PMM_SZ_512];
    int line;
    char content[PMM_SZ_1K];
} grep_match_t;

/* Deduped result: one per containing graph node */
typedef struct {
    int64_t node_id; /* 0 = raw match (no containing node) */
    char node_name[PMM_SZ_256];
    char qualified_name[PMM_SZ_512];
    char label[PMM_SZ_64];
    char file[PMM_SZ_512];
    int start_line;
    int end_line;
    int in_degree;
...
static int aggregate_search_dirs(search_result_t *sr, int sr_count, char dir_names[][PMM_SZ_128],
    ...
    for (int di = 0; di < sr_count; di++) {
        ...
        if (found >= 0) {
            ...
            snprintf(dir_names[dir_n], PMM_SZ_128, "%s", top);
...
static yyjson_mut_val *build_dir_distribution(yyjson_mut_doc *doc, search_result_t *sr,
    ...
    char dir_names[PMM_SZ_64][PMM_SZ_128];
    int dir_counts[PMM_SZ_64];
    int dir_n = aggregate_search_dirs(sr, sr_count, dir_names, dir_counts, PMM_SZ_64);
    for (int d = 0; d < dir_n; d++) {
...
static char *assemble_search_output_toon(search_result_t *sr, int sr_count, grep_match_t *raw,
    ...
    pmm_tree_table_header(&sb, "results", output_count, cols, 7);
    for (int ri = 0; ri < output_count; ri++) {
        ...
        pmm_tree_row_begin(&sb);
        pmm_tree_cell_str(&sb, r->qualified_name, true);
        pmm_tree_cell_str(&sb, r->label, false);
        pmm_tree_cell_str(&sb, r->file, false);
        pmm_tree_cell_str(&sb, lines, false);
        pmm_tree_cell_str(&sb, matches, false);
        pmm_tree_cell_int(&sb, r->in_degree, false);
        pmm_tree_cell_int(&sb, r->out_degree, false);
        pmm_tree_row_end(&sb);
    }
    ...
    if (raw_output > 0) {
        ...
        pmm_tree_table_header(&sb, "raw", raw_output, rcols, 3);
        for (int ri = 0; ri < raw_output; ri++) {
            pmm_tree_row_begin(&sb);
            pmm_tree_cell_str(&sb, raw[ri].file, true);
            pmm_tree_cell_int(&sb, raw[ri].line, false);
            pmm_tree_cell_str(&sb, raw[ri].content, false);
            pmm_tree_row_end(&sb);
        }
    }
    ...
    if (dir_n > 0) {
        ...
        pmm_tree_table_header(&sb, "dirs", dir_n, dcols, 2);
        for (int d = 0; d < dir_n; d++) {
            pmm_tree_row_begin(&sb);
            pmm_tree_cell_str(&sb, dir_names[d], true);
            pmm_tree_cell_int(&sb, dir_counts[d], false);
            pmm_tree_row_end(&sb);
        }
    }

    pmm_tree_scalar_int(&sb, "total_grep_matches", gm_count);
    pmm_tree_scalar_int(&sb, "total_results", sr_count);
    pmm_tree_scalar_int(&sb, "raw_match_count", raw_count);
    pmm_tree_scalar_int(&sb, "elapsed_ms", (long long)elapsed_ms);
    if (warn_literal_pipe) {
        pmm_tree_scalar_str(&sb, "warning",
    ...
    if (elapsed_ms >= SEARCH_SLOW_MS) {
        pmm_tree_scalar_str(&sb, "warning_slow",
...
static char *assemble_search_output(search_result_t *sr, int sr_count, grep_match_t *raw,
    ...
    if (elapsed_ms >= SEARCH_SLOW_MS) {
        ...
        pmm_log_warn("search.slow", "elapsed_ms", ems); /* visibility in logs */
    }
...
static grep_match_t *collect_grep_matches(FILE *fp, const char *root_path, size_t root_len,
                                          bool has_path_filter, pmm_regex_t *path_regex,
                                          int grep_limit, int *out_count) {
    int gm_cap = PMM_SZ_64;
    int gm_count = 0;
...
#ifdef _WIN32
        pmm_normalize_path_sep(line);
#endif
        const char *path = line;
        const char *file = strip_root_prefix(path, root_path, root_len);

        if (has_path_filter && pmm_regexec(path_regex, file, 0, NULL, 0) != PMM_REG_OK) {
            continue;
        }

        safe_grow(gm, gm_count, gm_cap, PAIR_LEN);
        snprintf(gm[gm_count].file, sizeof(gm[0].file), "%s", file);
        gm[gm_count].line = (int)strtol(sep1 + SKIP_ONE, NULL, PMM_DECIMAL_BASE);
...
static int find_tightest_node(pmm_node_t *nodes, int count, int line) {
    int best = PMM_NOT_FOUND;
...
static void add_to_search_results(search_result_t **sr, int *sr_count, int *sr_cap, pmm_node_t *n,
                                  int line) {
    for (int j = 0; j < *sr_count; j++) {
        if ((*sr)[j].node_id == n->id) {
            if ((*sr)[j].match_count < PMM_SZ_64) {
...
static void classify_grep_hit(grep_match_t *hit, pmm_node_t *file_nodes, int file_node_count,
    ...
    if (best >= 0) {
        ...
        if (*raw_count >= *raw_cap) {
            *raw_cap = (*raw_cap == 0) ? PMM_SZ_32 : *raw_cap * PAIR_LEN;
...
/* Free a file_nodes array returned from pmm_store_find_nodes_by_file. */
static void free_file_nodes(pmm_node_t *nodes, int count) {
    for (int j = 0; j < count; j++) {
...
static void classify_all_grep_hits(grep_match_t *gm, int gm_count, pmm_store_t *store,
    ...
    while (i < gm_count) {
        ...
        pmm_node_t *file_nodes = NULL;
        int file_node_count = 0;
        if (store) {
            pmm_store_find_nodes_by_file(store, project, cur_file, &file_nodes, &file_node_count);
        }
 ...
 *
 * `fl` is the caller's already-open binary stream on the descriptor pmm_mkstemp
...
static bool write_scoped_filelist(pmm_mcp_server_t *srv, const char *project, const char *root_path,
                                  FILE *fl, bool has_path_filter, pmm_regex_t *path_regex,
                                  int *out_written) {
    *out_written = 0;
    pmm_store_t *pre_store = resolve_store(srv, project);
    if (!pre_store) {
        return false;
    }
    char **indexed_files = NULL;
    int indexed_count = 0;
    if (pmm_store_list_files(pre_store, project, &indexed_files, &indexed_count) != PMM_STORE_OK ||
        indexed_count == 0) {
        return false;
    }
    bool ok = false;
...
#ifdef _WIN32
                pmm_normalize_path_sep(indexed_files[fi]);
#endif
                if (pmm_regexec(path_regex, indexed_files[fi], 0, NULL, 0) != PMM_REG_OK) {
                    continue;
                }
            }
 ...
 * "<tmp>/pmm_search_<pid>.pat" and its ".files" companion — opened with a plain
 ...
 * them. Now both live inside a directory created by pmm_mkdtemp (0700 on POSIX,
 * an explicit owner-only DACL on Windows) under an unguessable XXXXXX suffix,
 * and each file is created by pmm_mkstemp — O_CREAT|O_EXCL at mode 0600, so the
 * create fails rather than following anything already at the name. Every write
 * goes through the descriptor pmm_mkstemp returned; neither path is ever
 * reopened by name.
 *
 * Sizing: pmm_mkdtemp copies its expanded result back into `dir`, and its own
 * internal buffer is PMM_SZ_512, so `dir` must be at least that big to receive
 * it. The two file paths are `dir` plus a short basename. */
typedef struct {
    char dir[PMM_SZ_512];
    char pattern_path[PMM_SZ_1K];
    char filelist_path[PMM_SZ_1K];
...
static FILE *search_scratch_file(const char *dir, const char *basename, char *path_out,
    ...
    int descriptor = pmm_mkstemp(path_out);
    if (descriptor < 0) {
        path_out[0] = '\0';
        return NULL;
    }
...
static void search_scratch_close(search_scratch_t *scratch) {
    ...
    if (scratch->pattern_path[0] != '\0') {
        (void)pmm_unlink(scratch->pattern_path);
        scratch->pattern_path[0] = '\0';
    }
    if (scratch->filelist_path[0] != '\0') {
        (void)pmm_unlink(scratch->filelist_path);
        scratch->filelist_path[0] = '\0';
    }
    if (scratch->dir[0] != '\0') {
        (void)pmm_rmdir(scratch->dir);
        scratch->dir[0] = '\0';
    }
}
...
static bool search_scratch_open(search_scratch_t *scratch, const char *pattern) {
    ...
    int written =
        snprintf(scratch->dir, sizeof(scratch->dir), "%s/pmm-search-XXXXXX", pmm_tmpdir());
    if (written <= 0 || (size_t)written >= sizeof(scratch->dir) || !pmm_mkdtemp(scratch->dir)) {
        scratch->dir[0] = '\0';
        return false;
    }
...
static bool compile_path_filter(const char *filter, pmm_regex_t *re) {
    if (!filter || !filter[0]) {
        return false;
    }
    return pmm_regcomp(re, filter, PMM_REG_EXTENDED | PMM_REG_NOSUB) == PMM_REG_OK;
}

static char *handle_search_code(pmm_mcp_server_t *srv, const char *args) {
    char *pattern = pmm_mcp_get_string_arg(args, "pattern");
    char *project = get_project_arg(args);
    char *file_pattern = pmm_mcp_get_string_arg(args, "file_pattern");
    char *path_filter = pmm_mcp_get_string_arg(args, "path_filter");
    char *mode_str = pmm_mcp_get_string_arg(args, "mode");
    int limit = pmm_mcp_get_int_arg(args, "limit", MCP_DEFAULT_LIMIT);
    int context_lines = pmm_mcp_get_int_arg(args, "context", 0);
    bool use_regex = pmm_mcp_get_bool_arg(args, "regex");
    uint64_t search_t0 = pmm_now_ms();
    ...
    if (!pattern) {
        free(project);
        free(file_pattern);
        return pmm_mcp_text_result("pattern is required", true);
    }
    ...
    if (!validate_search_args(root_path, file_pattern)) {
        if (has_path_filter) {
            pmm_regfree(&path_regex);
        }
        free(root_path);
        free(pattern);
        free(project);
        free(file_pattern);
        return pmm_mcp_text_result("path or file_pattern contains invalid characters", true);
    }
    ...
    if (use_regex) {
        pmm_regex_t probe;
        if (pmm_regcomp(&probe, pattern, PMM_REG_EXTENDED | PMM_REG_NOSUB) != PMM_REG_OK) {
            if (has_path_filter) {
                pmm_regfree(&path_regex);
            }
            free(root_path);
    ...
    if (!search_scratch_open(&scratch, pattern)) {
        ...
        free(project);
        free(file_pattern);
        return pmm_mcp_text_result(errmsg, true);
    }
    ...
    if (scoped && scoped_written == 0) {
        ...
        if (!fp) {
            ...
            free(project);
            free(file_pattern);
            return pmm_mcp_text_result("search failed", true);
        }
    ...
    if (store && sr_count > 0) {
        ...
        if (pmm_store_batch_count_degrees(store, ids, sr_count, "CALLS", in_degs, out_degs) ==
            PMM_STORE_OK) {
            for (int j = 0; j < sr_count; j++) {
    ...
    char *sc_format = pmm_mcp_get_string_arg(args, "format");
    ...
    if (mode == 0 && !sc_legacy_json) {
        char *toon_text =
            assemble_search_output_toon(sr, sr_count, raw, raw_count, gm_count, limit,
                                        pat_has_pipe && !use_regex, pmm_now_ms() - search_t0);
        result = pmm_mcp_text_result(toon_text ? toon_text : "out of memory", toon_text == NULL);
        free(toon_text);
    } else {
...
static bool mcp_command_output_path(char out[PMM_SZ_2K]) {
    char directory[PMM_SZ_1K];
    const char *cache = pmm_resolve_cache_dir();
    int written;
    if (cache && cache[0]) {
        written = snprintf(directory, sizeof(directory), "%s/logs", cache);
        if (written <= 0 || written >= (int)sizeof(directory) || !pmm_mkdir_p(directory, 0700)) {
            return false;
        }
    } else {
        written = snprintf(directory, sizeof(directory), "%s", pmm_tmpdir());
        if (written <= 0 || written >= (int)sizeof(directory)) {
            return false;
        }
    }
    written = snprintf(out, PMM_SZ_2K, "%s/.mcp-command-XXXXXX", directory);
    if (written <= 0 || written >= PMM_SZ_2K) {
        out[0] = '\0';
        return false;
    }
    int descriptor = pmm_mkstemp(out);
    if (descriptor < 0) {
        out[0] = '\0';
        return false;
    }
 ...
 * COMSPEC. pmm_subprocess receives this as lpApplicationName and validates the
 * same absolute cmd.exe path before using the dedicated payload serializer. */
static bool mcp_resolve_windows_cmd(char out[PMM_SZ_4K]) {
    if (!out) {
        return false;
    }
    out[0] = '\0';
    ...
    char *candidate = pmm_wide_to_utf8(system_directory);
    if (!candidate) {
        return false;
    }
    bool resolved = pmm_canonical_path(candidate, out, PMM_SZ_4K) != 0;
    free(candidate);
    return resolved;
}
#endif

static int mcp_run_shell_command_cancellable(pmm_mcp_server_t *srv, const char *command,
                                             char output_path[PMM_SZ_2K],
                                             pmm_proc_result_t *result_out) {
...
#ifdef _WIN32
    char shell[PMM_SZ_4K];
    if (!mcp_resolve_windows_cmd(shell)) {
        (void)pmm_unlink(output_path);
        output_path[0] = '\0';
        return -1;
    }
#else
    ...
    pmm_proc_opts_t options = {
        ...
        .quiet_timeout_ms = 0,
        .cancel_grace_ms = PMM_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS,
        .delete_log_on_exit = false,
    };
    pmm_subprocess_t *process = NULL;
    if (pmm_subprocess_spawn(&options, &process) != 0) {
        (void)pmm_unlink(output_path);
        output_path[0] = '\0';
        return -1;
    }

    pmm_proc_poll_t state;
    for (;;) {
        if (mcp_request_cancelled(srv)) {
            (void)pmm_subprocess_request_cancel(process);
        }
        state = pmm_subprocess_poll(process, result_out);
        if (state != PMM_PROC_POLL_RUNNING) {
            break;
        }
        pmm_usleep(10000);
    }
    bool contained = state == PMM_PROC_POLL_TERMINAL && result_out->tree_quiesced &&
                     !result_out->supervision_failed;
    pmm_subprocess_destroy(process);
...
static void detect_collect_seeds(pmm_store_t *store, const char *project, const char *file,
                                 int64_t **seeds, int *n, int *cap) {
    pmm_node_t *nodes = NULL;
    int ncount = 0;
    pmm_store_find_nodes_by_file(store, project, file, &nodes, &ncount);
...
static int detect_module_rollup(const pmm_traverse_result_t *impact, char mods[][PMM_SZ_128],
    ...
    for (int i = 0; i < impact->visited_count; i++) {
        ...
        if (j == nmods) {
            if (nmods < DETECT_MODCAP) {
                snprintf(mods[nmods], PMM_SZ_128, "%s", m);
                mcnt[nmods] = 1;
                nmods++;
            } else {
                (*overflow)++;
...
static void detect_emit_impacted_tree(pmm_sb_t *sb, pmm_traverse_result_t *tr, int limit) {
    pmm_tree_scalar_int(sb, "impacted_total", tr->visited_count);
    ...
    pmm_sb_append(sb, hdr);
    if (shown > 1) {
        qsort(tr->visited, (size_t)shown, sizeof(pmm_node_hop_t), tree_hop_cmp_qn);
    }
    char cur_group[PMM_SZ_1K] = "";
    for (int i = 0; i < shown; i++) {
        const char *qn =
    ...
    if (shown < tr->visited_count) {
        ...
        pmm_sb_append(sb, more);
    }
}

static char *handle_detect_changes(pmm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *base_branch = pmm_mcp_get_string_arg(args, "base_branch");
    char *since = pmm_mcp_get_string_arg(args, "since");
    char *scope = pmm_mcp_get_string_arg(args, "scope");
    int depth = pmm_mcp_get_int_arg(args, "depth", MCP_DEFAULT_BFS_DEPTH);
    ...
    if (!pmm_validate_shell_arg(base_branch) || base_branch[0] == '-' ||
        !validate_windows_cmd_interpolation_arg(base_branch)) {
        free(project);
        free(base_branch);
        free(scope);
        return pmm_mcp_text_result("base_branch contains invalid characters", true);
    }
    ...
    if (!validate_search_path_arg(root_path) ||
        ...
        free(base_branch);
        free(scope);
        return pmm_mcp_text_result("project path contains invalid characters", true);
    }
    ...
    char output_path[PMM_SZ_2K] = {0};
    pmm_proc_result_t git_result = {0};
    ...
    if (git_cancelled) {
        ...
        free(base_branch);
        free(scope);
        return pmm_mcp_text_result("detect_changes cancelled for this request", true);
    }
    if (git_run != 0) {
        (void)pmm_unlink(output_path);
        char errmsg[PMM_SZ_256];
    ...
    FILE *fp = pmm_fopen(output_path, "rb");
    if (!fp) {
        (void)pmm_unlink(output_path);
        free(root_path);
        free(project);
        free(base_branch);
        free(scope);
        return pmm_mcp_text_result("git diff failed: contained output could not be read", true);
    }
    ...
    char *fmt = pmm_mcp_get_string_arg(args, "format");
    ...
    int imp_limit = pmm_mcp_get_int_arg(args, "limit", MCP_DEFAULT_IMPACT_LIMIT);
    if (imp_limit < 1) {
        imp_limit = 1;
    }
    ...
    char merge_base[64] = "";
    {
        ...
        snprintf(mbcmd, sizeof(mbcmd), "git -C '%s' merge-base '%s' HEAD 2>/dev/null", root_path,
                 base_branch);
#endif
        char mb_output_path[PMM_SZ_2K] = {0};
        pmm_proc_result_t mb_result = {0};
        ...
        FILE *mbfp =
            mb_run == 0 && mb_result.exit_code == 0 ? pmm_fopen(mb_output_path, "rb") : NULL;
        ...
        if (mb_output_path[0]) {
            (void)pmm_unlink(mb_output_path);
        }
    ...
    pmm_traverse_result_t impact = {0};
    bool truncated = false;
    if (want_symbols && seed_count > 0) {
        (void)pmm_store_bfs_multi(store, seeds, seed_count, direction, NULL, 0, depth,
    ...
    if (!legacy_json) {
        pmm_sb_t sb;
        pmm_sb_init(&sb);
        pmm_tree_scalar_str(&sb, "base", base_branch);
        if (merge_base[0]) {
            pmm_tree_scalar_str(&sb, "merge_base", merge_base);
        }
        pmm_tree_scalar_str(&sb, "direction", direction);
        if (is_error) {
            char hint_buf[PMM_SZ_256];
            snprintf(hint_buf, sizeof(hint_buf),
                     "git diff exited with status %d. Check that branch '%s' exists.", git_status,
                     base_branch);
            pmm_tree_scalar_str(&sb, "hint", hint_buf);
        }
    ...
    pmm_store_traverse_free(&impact);
    for (int i = 0; i < file_count; i++) {
        free(files[i]);
    }
    free(files);
    ...
    char *result = pmm_mcp_text_result(out_str, is_error);
    free(out_str);
    return result;
}
...
/* Read the legacy file-based ADR (<root>/.project-mind-mcp/adr.md), used by
...
static char *handle_manage_adr(pmm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *mode_str = pmm_mcp_get_string_arg(args, "mode");
    char *content = pmm_mcp_get_string_arg(args, "content");
    ...
    if (project) {
        ...
        if (mcp_request_cancelled(srv)) {
            ...
            free(mode_str);
            free(content);
            return pmm_mcp_text_result("project operation cancelled for this request", true);
        }
    }
    ...
    pmm_store_t *resolved = resolve_store_internal(srv, project, mutation_held);
    if (!resolved) {
        char *err = build_no_store_error(project);
        char *res = pmm_mcp_text_result(err, true);
        free(err);
     ...
     * to exist via resolve_store, so pmm_store_open_path won't create a ghost
     * DB). For an in-memory / embedded store (db_path == NULL) the resolved
     * store is already writable — use it directly. */
    pmm_store_t *store = resolved;
    pmm_store_t *owned_rw = NULL;
    const char *resolved_db_path = pmm_store_db_path(resolved);
    if (resolved_db_path) {
        ...
        if (!rw_path) {
            ...
            free(mode_str);
            free(content);
            return pmm_mcp_text_result("failed to allocate ADR store path", true);
        }
        invalidate_cached_store(srv);
        owned_rw = pmm_store_open_path(rw_path);
        free(rw_path);
        if (!owned_rw) {
            char *err = build_no_store_error(project);
            char *res = pmm_mcp_text_result(err, true);
     ...
     * <root>/.project-mind-mcp/adr.md. If the store has no ADR yet but that
     * legacy file exists, import it so nothing is lost on upgrade. */
    pmm_adr_t adr;
    memset(&adr, 0, sizeof(adr));
    bool have_adr = (pmm_store_adr_get(store, project, &adr) == PMM_STORE_OK);
    if (!have_adr) {
        char *root_path = project_root_from_store(store, project);
        char *legacy = adr_read_legacy_file(root_path);
        free(root_path);
        if (legacy) {
            if (pmm_store_adr_store(store, project, legacy) == PMM_STORE_OK) {
                have_adr = (pmm_store_adr_get(store, project, &adr) == PMM_STORE_OK);
            }
            free(legacy);
        }
    }
    ...
    if ((strcmp(mode_str, "update") == 0 || strcmp(mode_str, "store") == 0) && content) {
        if (pmm_store_adr_store(store, project, content) == PMM_STORE_OK) {
...
static char *handle_ingest_traces(pmm_mcp_server_t *srv, const char *args) {
    (void)srv;
...
static char *dispatch_tool(pmm_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    if (!tool_name) {
        return pmm_mcp_text_result("missing tool name", true);
    }
    if (srv && !mcp_tool_allowed(srv->tool_profile, tool_name)) {
        char message[PMM_SZ_256];
        ...
        return pmm_mcp_text_result(message, true);
    }
 ...
 * process lifetime. */
static void release_request_store(pmm_mcp_server_t *srv) {
    if (!srv || !srv->owns_store || !srv->store || !pmm_store_db_path(srv->store)) {
        return;
    }
    pmm_store_close(srv->store);
    srv->store = NULL;
...
char *pmm_mcp_handle_tool(pmm_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    /* Phase marks bracket the WHOLE request with no unlabelled gap, so growth
     * cannot hide between them (PMM_MEM_PHASES=1; see foundation/mem.h). The
    ...
    pmm_mem_phase_mark("request.scope_begin");
    bool request_scope = !srv || pmm_mcp_server_request_scope_begin(srv);
    if (!request_scope) {
        release_request_store(srv);
        pmm_mem_phase_mark("idle");
        return pmm_mcp_text_result("request cancellation scope unavailable", true);
    }
    pmm_mem_phase_mark("request.dispatch_tool");
    char *result = dispatch_tool(srv, tool_name, args_json);
    pmm_mem_phase_mark("request.scope_end");
    if (srv) {
        pmm_mcp_server_request_scope_end(srv);
    }
    pmm_mem_phase_mark("request.release_store");
    release_request_store(srv);
    pmm_mem_phase_mark("idle");
    ...
    pmm_mem_census_log("mcp.request");
    return result;
}
...
static void detect_session(pmm_mcp_server_t *srv) {
    ...
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        ...
        if (strcmp(cwd, "/") != 0 && (home == NULL || strcmp(cwd, home) != 0)) {
            ...
            pmm_log_info("session.root.cwd", "path", cwd);
        }
    }

    /* Derive project name from path — must match pmm_project_name_from_path
    ...
    if (srv->session_root[0]) {
        char *pname = pmm_project_name_from_path(srv->session_root);
        if (pname) {
...
static bool auto_watch_enabled(pmm_mcp_server_t *srv) {
    if (!srv->config) {
        return true; /* default on */
    }
    return pmm_config_get_bool(srv->config, PMM_CONFIG_AUTO_WATCH, true);
}

/* Register the session project with the background watcher for ongoing
 * change detection — unless auto_watch is disabled. */
static void register_watcher_if_enabled(pmm_mcp_server_t *srv) {
    if (!srv->watcher || srv->session_project[0] == '\0' || srv->session_root[0] == '\0') {
        return;
    }
    if (!auto_watch_enabled(srv)) {
        pmm_log_info("watcher.register.skipped", "reason", "auto_watch_off", "project",
                     srv->session_project);
        return;
    }
    pmm_watcher_watch(srv->watcher, srv->session_project, srv->session_root);
}

/* Background auto-index thread function */
static void *autoindex_thread(void *arg) {
    pmm_mcp_server_t *srv = (pmm_mcp_server_t *)arg;

    pmm_log_info("autoindex.start", "project", srv->session_project, "path", srv->session_root);
    ...
    if (pmm_index_supervisor_should_wrap()) {
        ...
        if (resp) {
            free(resp);
            pmm_log_info("autoindex.done", "project", srv->session_project, "mode", "supervised");
        ...
        pmm_log_error("autoindex.supervision_failed", "project", srv->session_project, "action",
                      "fail_closed");
        return NULL;
    }

    pmm_pipeline_t *p = pmm_pipeline_new(srv->session_root, NULL, PMM_MODE_FULL);
    if (!p) {
        pmm_log_warn("autoindex.err", "msg", "pipeline_create_failed");
        return NULL;
    }
    ...
    pmm_pipeline_free(p);
    pmm_mem_collect(); /* return mimalloc pages to OS after indexing (in-process only) */

    if (rc == 0) {
        pmm_log_info("autoindex.done", "project", srv->session_project);
        register_watcher_if_enabled(srv);
    } else {
        pmm_log_warn("autoindex.err", "msg", "pipeline_run_failed");
    }
    return NULL;
}

bool pmm_mcp_auto_index_within_file_limit(const char *root_path, int file_limit,
                                          int *file_count_out) {
    if (file_count_out) {
        *file_count_out = -1;
    }
    ...
    pmm_discover_opts_t options = {
        .mode = PMM_MODE_FULL,
        .ignore_file = NULL,
        .max_file_size = 0,
    };
    int count = -1;
    pmm_discover_status_t status = pmm_discover_count_bounded(
        root_path, &options, file_limit, pmm_now_ms() + AUTO_INDEX_COUNT_TIMEOUT_MS, &count);
    if (file_count_out) {
        *file_count_out = status == PMM_DISCOVER_LIMIT_EXCEEDED
                              ? (file_limit < INT_MAX ? file_limit + 1 : INT_MAX)
                              : count;
    }
    return status == PMM_DISCOVER_OK;
}

/* Start auto-indexing if configured and project not yet indexed. */
static void maybe_auto_index(pmm_mcp_server_t *srv) {
    ...
    const char *home = pmm_get_home_dir();
    if (home) {
        char db_check[PMM_SZ_1K];
        snprintf(db_check, sizeof(db_check), "%s/%s.db", pmm_resolve_cache_dir(),
                 srv->session_project);
        if (pmm_file_size(db_check) >= 0) {
            /* Already indexed → register watcher for change detection */
            pmm_log_info("autoindex.skip", "reason", "already_indexed", "project",
                         srv->session_project);
            register_watcher_if_enabled(srv);
            return;
        }
    ...
    int file_limit = PMM_MCP_DEFAULT_AUTO_INDEX_LIMIT;
    if (srv->config) {
        auto_index = pmm_config_get_bool(srv->config, PMM_CONFIG_AUTO_INDEX, false);
        file_limit = pmm_config_get_int(srv->config, PMM_CONFIG_AUTO_INDEX_LIMIT,
                                        PMM_MCP_DEFAULT_AUTO_INDEX_LIMIT);
    }

    if (!auto_index) {
        pmm_log_info("autoindex.skip", "reason", "disabled", "hint",
                     "run: project-mind-mcp config set auto_index true");
        return;
    }

    /* Quick tracked-file count check to avoid OOM on massive repos. */
    int file_count = -1;
    if (!pmm_mcp_auto_index_within_file_limit(srv->session_root, file_limit, &file_count)) {
        char files[32];
        (void)snprintf(files, sizeof(files), "%d", file_count);
        pmm_log_warn("autoindex.skip", "reason",
                     file_count >= 0 ? "too_many_files" : "unsafe_or_unavailable_path", "files",
                     files, "limit", PMM_CONFIG_AUTO_INDEX_LIMIT);
        return;
    }

    /* Launch auto-index in background */
    if (pmm_thread_create(&srv->autoindex_tid, 0, autoindex_thread, srv) == 0) {
...
bool pmm_mcp_jsonrpc_response_prepend_notice(char **response_io, const char *notice) {
...
char *pmm_mcp_server_handle(pmm_mcp_server_t *srv, const char *line) {
    pmm_jsonrpc_request_t req = {0};
    if (pmm_jsonrpc_parse(line, &req) < 0) {
        return pmm_jsonrpc_format_error(0, JSONRPC_PARSE_ERROR, "Parse error");
    }

    /* Notifications (no id) → handle cancellation, then no response */
    if (!req.has_id) {
        if (req.method && strcmp(req.method, "notifications/cancelled") == 0) {
            if (pmm_mcp_cancel_request_matches(req.params_raw, srv->active_request_id,
                                               srv->active_request_id_str) &&
                pmm_mcp_server_cancel_active(srv)) {
                pmm_log_info("mcp.cancelled", "match", "true");
            }
        }
        pmm_jsonrpc_request_free(&req);
        return NULL;
    }

    if (!pmm_mcp_server_request_scope_begin(srv)) {
        int64_t request_id = req.id;
        pmm_jsonrpc_request_free(&req);
        return pmm_jsonrpc_format_error(request_id, JSONRPC_INTERNAL_ERROR,
                                        "Request cancellation scope unavailable");
    }

    struct timespec req_t0;
    pmm_clock_gettime(CLOCK_MONOTONIC, &req_t0);
    char *result_json = NULL;
    char *request_error_json = NULL;
    bool request_logged = false;

    if (strcmp(req.method, "initialize") == 0) {
        result_json = pmm_mcp_initialize_response_for_profile(req.params_raw, srv->tool_profile);
        detect_session(srv);
        if (srv->background_tasks && srv->tool_profile == PMM_MCP_TOOL_PROFILE_ALL) {
        ...
        result_json = pmm_mcp_prompts_list();
    } else if (strcmp(req.method, "prompts/get") == 0) {
        result_json = pmm_mcp_prompt_get(req.params_raw, &request_error_json);
    } else if (strcmp(req.method, "tools/list") == 0) {
        result_json = pmm_mcp_tools_list_page(srv->tool_profile, req.params_raw);
    } else if (strcmp(req.method, "tools/call") == 0) {
        char *tool_name = req.params_raw ? pmm_mcp_get_tool_name(req.params_raw) : NULL;
        char *tool_args =
            req.params_raw ? pmm_mcp_get_arguments(req.params_raw) : heap_strdup("{}");
        srv->active_request_id = req.id;
        free(srv->active_request_id_str);
        srv->active_request_id_str = req.id_str ? heap_strdup(req.id_str) : NULL;

        struct timespec t0;
        pmm_clock_gettime(CLOCK_MONOTONIC, &t0);
        result_json = pmm_mcp_handle_tool(srv, tool_name, tool_args);
        srv->active_request_id = PMM_NOT_FOUND;
        free(srv->active_request_id_str);
        srv->active_request_id_str = NULL;
        struct timespec t1;
        pmm_clock_gettime(CLOCK_MONOTONIC, &t1);
        ...
        pmm_jsonrpc_response_t err_resp = {
            .id = req.id,
            .id_str = req.id_str,
            .error_json = err_obj,
        };
        char *err = pmm_jsonrpc_format_response(&err_resp);
        struct timespec t1;
        pmm_clock_gettime(CLOCK_MONOTONIC, &t1);
        long long dur_us = ((long long)(t1.tv_sec - req_t0.tv_sec) * MCP_S_TO_US) +
                           ((long long)(t1.tv_nsec - req_t0.tv_nsec) / MCP_MS_TO_US);
        pmm_log_mcp_request(req.method, NULL, true, dur_us);
        pmm_mcp_server_request_scope_end(srv);
        pmm_jsonrpc_request_free(&req);
        return err;
    }
    ...
    if (!request_logged) {
        ...
        long long dur_us = ((long long)(t1.tv_sec - req_t0.tv_sec) * MCP_S_TO_US) +
                           ((long long)(t1.tv_nsec - req_t0.tv_nsec) / MCP_MS_TO_US);
        pmm_log_mcp_request(req.method, NULL, false, dur_us);
    }

    pmm_jsonrpc_response_t resp = {
        .id = req.id,
        .id_str = req.id_str,
        .result_json = result_json,
    };
    char *out = pmm_jsonrpc_format_response(&resp);
    free(result_json);
    pmm_mcp_server_request_scope_end(srv);
    pmm_jsonrpc_request_free(&req);
    return out;
...
static bool parse_content_length(const char *line, size_t *out) {
    ...
    errno = 0;
    char *end = NULL;
    unsigned long long parsed = strtoull(cursor, &end, PMM_DECIMAL_BASE);
...
int pmm_mcp_read_message(FILE *in, char **message, bool *content_length_framed) {
...
static int poll_for_input_unix(pmm_mcp_server_t *srv, int fd, FILE *in) {
    ...
    if (saved_flags < 0) {
        ...
        if (pr == 0) {
            pmm_mcp_server_evict_idle(srv, STORE_IDLE_TIMEOUT_S);
            return 0;
        }
        return SKIP_ONE;
    }
...
int pmm_mcp_server_run(pmm_mcp_server_t *srv, FILE *in, FILE *out) {
    int fd = pmm_fileno(in);
    ...
    for (;;) {
        ...
        if (pmm_mcp_read_message(in, &message, &content_length_framed) <= 0) {
            break;
        }

        char *resp = pmm_mcp_server_handle(srv, message);
        free(message);
        if (resp) {
...
bool pmm_parse_file_uri(const char *uri, char *out_path, int out_size) {
