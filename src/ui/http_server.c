 ...
 *   POST /rpc         → JSON-RPC dispatch via own pmm_mcp_server_t
 ...
 * Has its own pmm_mcp_server_t with a separate SQLite connection (WAL reader).
 */
...
#include "foundation/subprocess.h" /* pmm_build_win_cmdline — shared MS-CRT arg quoting */
#include "foundation/win_utf8.h"   /* pmm_utf8_to_wide — CreateProcessW wide cmdline (#423/#20) */
...
#define MAX_BODY_SIZE PMM_HTTP_MAX_BODY
...
static void update_cors(const pmm_http_req_t *req, int port) {
...
static void handle_ui_config(pmm_http_conn_t *c, const pmm_http_req_t *req) {
    const char *lang = NULL;
    char cache_dir[1024];
    snprintf(cache_dir, sizeof(cache_dir), "%s", pmm_resolve_cache_dir());
    pmm_config_t *cfg = pmm_config_open(cache_dir);
    if (cfg) {
        const char *pinned = pmm_config_get(cfg, PMM_CONFIG_UI_LANG, "auto");
    ...
    pmm_http_replyf(c, 200, g_cors_json, "{\"lang\":\"%s\",\"upstream_issues_url\":\"%s\"}",
                    lang_buf, "https://github.com/usman8786/project-mind-mcp/issues/new");
}
...
struct pmm_http_server {
    pmm_httpd_t *listener;
    pmm_mcp_server_t *mcp;       /* own MCP server instance (read-only) */
    struct pmm_watcher *watcher; /* external watcher ref (not owned) */
    pmm_http_index_executor_fn index_executor;
    void *index_executor_context;
    pmm_http_project_mutation_begin_fn mutation_begin;
    pmm_http_project_mutation_end_fn mutation_end;
...
static bool serve_embedded(pmm_http_conn_t *c, const char *path) {
    const pmm_embedded_file_t *f = pmm_embedded_lookup(path);
    if (!f)
        return false;
    ...
    pmm_http_reply_buf(c, 200, hdrs, f->data, (size_t)f->size);
    return true;
}
 ...
 * libgit2 is initialized once at process start by pmm_alloc_init() (which also
...
static void handle_repo_info(pmm_http_conn_t *c, const pmm_http_req_t *req) {
    char project[256] = {0};
    if (!pmm_http_query_param(req->query, "project", project, (int)sizeof(project)) ||
        project[0] == '\0') {
        pmm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project parameter\"}");
        return;
    }

    char db_path[1024];
    db_path_for_project(project, db_path, sizeof(db_path));
    if (db_path[0] == '\0' || !pmm_file_exists(db_path)) {
        pmm_http_replyf(c, 404, g_cors_json, "{\"error\":\"project not found\"}");
        return;
    }
    ...
    if (pmm_store_get_project(store, project, &proj) == PMM_STORE_OK && proj.root_path) {
    ...
    char branch[256] = {0};
    if (root_path[0]) {
        pmm_git_context_t gctx;
        memset(&gctx, 0, sizeof(gctx));
        if (pmm_git_context_resolve(root_path, &gctx) == 0 && gctx.branch) {
    ...
    char *remote_safe = pmm_ui_git_strip_credentials(remote); /* never echo secrets */
    char *web_base = pmm_ui_git_web_base(remote);
    ...
    pmm_json_escape(esc_root, (int)sizeof(esc_root), root_path);
    pmm_json_escape(esc_branch, (int)sizeof(esc_branch), branch);
    pmm_json_escape(esc_remote, (int)sizeof(esc_remote), remote_safe ? remote_safe : "");
    pmm_json_escape(esc_web, (int)sizeof(esc_web), web_base ? web_base : "");
    pmm_json_escape(esc_blob, (int)sizeof(esc_blob), blob_base);

    pmm_http_replyf(c, 200, g_cors_json,
...
static pmm_mutex_t g_log_mutex;

enum { PMM_LOG_MUTEX_UNINIT = 0, PMM_LOG_MUTEX_INITING = 1, PMM_LOG_MUTEX_INITED = 2 };
static atomic_int g_log_mutex_init = PMM_LOG_MUTEX_UNINIT;

/* Safe for concurrent callers: only publishes INITED after pmm_mutex_init()
...
void pmm_ui_log_init(void) {
    ...
    if (state == PMM_LOG_MUTEX_INITED)
        return;

    state = PMM_LOG_MUTEX_UNINIT;
    if (atomic_compare_exchange_strong(&g_log_mutex_init, &state, PMM_LOG_MUTEX_INITING)) {
        pmm_mutex_init(&g_log_mutex);
        atomic_store(&g_log_mutex_init, PMM_LOG_MUTEX_INITED);
        return;
    }

    /* Another thread is initializing — spin until done */
    while (atomic_load(&g_log_mutex_init) != PMM_LOG_MUTEX_INITED) {
        pmm_usleep(1000); /* 1ms */
    }
}
...
static void handle_logs(pmm_http_conn_t *c, const pmm_http_req_t *req) {
    char lines_str[16] = {0};
    int max_lines = 100;
    if (pmm_http_query_param(req->query, "lines", lines_str, (int)sizeof(lines_str))) {
    ...
    if (!buf) {
        pmm_mutex_unlock(&g_log_mutex);
        pmm_http_replyf(c, 500, g_cors, "oom");
        return;
    }
    ...
    pmm_http_replyf(c, 200, g_cors_json, "%s", buf);
    free(buf);
}
...
/* GET /api/processes — list project-mind-mcp processes via ps */
static void handle_processes(pmm_http_conn_t *c) {
    char buf[8192];
    int pos = 0;
...
static void append_roots_json(char *buf, size_t bufsz, int *pos) {
    ...
    for (int i = 0; i < 26; i++) {
         ...
         * offline volume) fails the same pmm_is_dir gate browse applies, and
...
static void handle_browse(pmm_http_conn_t *c, const pmm_http_req_t *req) {
     ...
     * gate, exactly as the MCP repo_path handler and pmm_project_name_from_path
     * already do — otherwise a real D:/ directory is rejected (#548). */
    pmm_normalize_path_sep(path);

    if (!pmm_is_dir(path)) {
        pmm_http_replyf(c, 400, g_cors_json, "{\"error\":\"not a directory\"}");
        return;
    }

    DIR *dir = opendir(path);
    if (!dir) {
        pmm_http_replyf(c, 403, g_cors_json, "{\"error\":\"cannot open directory\"}");
        return;
    }
...
static void handle_adr_get(pmm_http_conn_t *c, const pmm_http_req_t *req) {
    char name[256] = {0};
    ...
    pmm_store_t *store = pmm_store_open_path_query(db_path);
    if (!store) {
        pmm_http_replyf(c, 200, g_cors_json, "{\"has_adr\":false}");
        return;
    }

    pmm_adr_t adr;
    memset(&adr, 0, sizeof(adr));
    if (pmm_store_adr_get(store, name, &adr) == PMM_STORE_OK && adr.content) {
...
static void handle_adr_save(pmm_http_server_t *srv, pmm_http_conn_t *c, const pmm_http_req_t *req) {
    ...
    if (!v_proj || !yyjson_is_str(v_proj) || !v_content || !yyjson_is_str(v_content)) {
        yyjson_doc_free(doc);
        pmm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project or content\"}");
        return;
    }
    ...
    if (srv->mutation_begin && !srv->mutation_begin(srv->mutation_context, proj)) {
        yyjson_doc_free(doc);
        pmm_http_replyf(c, 423, g_cors_json,
    ...
    int rc = pmm_store_adr_store(store, proj, content);
    pmm_store_close(store);
    if (mutation_held) {
        srv->mutation_end(srv->mutation_context, proj);
    }
    ...
    if (rc == PMM_STORE_OK) {
        pmm_http_replyf(c, 200, g_cors_json, "{\"saved\":true}");
    } else {
        pmm_http_replyf(c, 500, g_cors_json, "{\"error\":\"save failed\"}");
    }
}
...
static bool resolve_self_executable(char *out, size_t outsz) {
    ...
    char *utf8 = pmm_wide_to_utf8(wbuf);
    if (!utf8) {
        return false;
    }
...
bool pmm_http_server_resolve_binary_path(const char *argv0, char *out, size_t outsz) {
    ...
    if (argv0 && argv0[0]) {
        ...
        wchar_t *wargv0 = pmm_utf8_to_wide(argv0);
...
void pmm_http_server_set_binary_path(const char *path) {
    if (path) {
        if (!pmm_http_server_resolve_binary_path(path, g_binary_path, sizeof(g_binary_path))) {
            g_binary_path[0] = '\0';
        }
    }
}
...
static void *index_thread_fn(void *arg) {
    index_job_t *job = arg;
    pmm_log_info("ui.index.start", "path", job->root_path);
    pmm_http_server_t *server = job->server;
    ...
    pmm_log_info("ui.index.done", "path", job->root_path, "rc", result == 0 ? "ok" : "err");
    atomic_store_explicit(&job->completed, 1, memory_order_release);
    return NULL;
}

/* POST /api/index — body: {"root_path": "/abs/path", "project_name": "..."} */
static void handle_index_start(pmm_http_server_t *server, pmm_http_conn_t *c,
                               const pmm_http_req_t *req) {
    if (!server || !server->index_executor) {
        pmm_http_replyf(c, 503, g_cors_json,
    ...
    if (!v_path || !yyjson_is_str(v_path)) {
        yyjson_doc_free(doc);
        pmm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing root_path\"}");
        return;
    }
    ...
    if (!pmm_is_dir(rpath)) {
        yyjson_doc_free(doc);
        pmm_http_replyf(c, 400, g_cors_json, "{\"error\":\"directory not found\"}");
        return;
    }
    ...
    if (slot < 0) {
        yyjson_doc_free(doc);
        pmm_http_replyf(c, 429, g_cors_json, "{\"error\":\"all index slots busy\"}");
        return;
    }

    index_job_t *job = &server->index_jobs[slot];
    if (job->thread_started) {
        if (pmm_thread_join(&job->thread) != 0) {
            pmm_http_replyf(c, 503, g_cors_json, "{\"error\":\"index worker unavailable\"}");
            return;
        }
    ...
    if (pmm_thread_create(&job->thread, 0, index_thread_fn, job) != 0) {
        atomic_store(&job->status, 3);
        atomic_store_explicit(&job->completed, 1, memory_order_release);
        snprintf(job->error_msg, sizeof(job->error_msg), "thread creation failed");
        pmm_http_replyf(c, 500, g_cors_json, "{\"error\":\"thread creation failed\"}");
        return;
    }
    job->thread_started = true;

    pmm_http_replyf(c, 202, g_cors_json, "{\"status\":\"indexing\",\"slot\":%d,\"path\":\"%s\"}",
                    slot, job->root_path);
}

/* GET /api/index-status — returns status of all index jobs */
static void handle_index_status(pmm_http_server_t *server, pmm_http_conn_t *c) {
    char buf[2048] = "[";
    int pos = 1;
    for (int i = 0; i < MAX_INDEX_JOBS; i++) {
        int st = atomic_load(&server->index_jobs[i].status);
...
static void unwatch_project(pmm_http_server_t *srv, const char *name) {
    if (srv && srv->watcher) {
...
static void handle_delete_project(pmm_http_server_t *srv, pmm_http_conn_t *c,
                                  const pmm_http_req_t *req) {
    char name[256] = {0};
    if (!pmm_http_query_param(req->query, "name", name, (int)sizeof(name)) || name[0] == '\0') {
        pmm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing name\"}");
        return;
    }
    ...
    if (unlink(db_path) != 0) {
        ...
        pmm_http_replyf(c, 500, g_cors_json, "{\"error\":\"failed to delete\"}");
        return;
    }
    ...
    unwatch_project(srv, name);
    pmm_log_info("ui.project.deleted", "name", name);
    if (mutation_held) {
        srv->mutation_end(srv->mutation_context, name);
    }
    pmm_http_replyf(c, 200, g_cors_json, "{\"deleted\":true}");
}

/* GET /api/project-health?name=X — checks db integrity */
static void handle_project_health(pmm_http_conn_t *c, const pmm_http_req_t *req) {
    char name[256] = {0};
    if (!pmm_http_query_param(req->query, "name", name, (int)sizeof(name)) || name[0] == '\0') {
        pmm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing name\"}");
        return;
    ...
    if (!pmm_file_exists(db_path)) {
        pmm_http_replyf(c, 200, g_cors_json, "{\"status\":\"missing\"}");
        return;
    }

    pmm_store_t *store = pmm_store_open_path_query(db_path);
    if (!store) {
        pmm_http_replyf(c, 200, g_cors_json, "{\"status\":\"corrupt\",\"reason\":\"cannot open\"}");
        return;
    }

    int node_count = pmm_store_count_nodes(store, name);
    int edge_count = pmm_store_count_edges(store, name);
    pmm_store_close(store);

    int64_t size = pmm_file_size(db_path);
...
static int find_cross_repo_targets(pmm_store_t *store, const char *project, char **out,
                                   int max_out) {
...
static double layout_radius(const pmm_layout_result_t *r) {
    if (!r || r->node_count == 0)
        return 0.0;
    double max_r2 = 0.0;
...
static bool attach_missed_graph(yyjson_mut_doc *mdoc, yyjson_mut_val *mroot, pmm_store_t *store,
                                const char *project, double primary_radius) {
    char covproj[512];
    pmm_store_coverage_shadow_project(covproj, sizeof(covproj), project);
    pmm_layout_result_t *ml = pmm_layout_compute(store, covproj, PMM_LAYOUT_OVERVIEW, NULL, 0, 0);
    if (!ml) {
        return false;
    }
    if (ml->node_count == 0) {
    ...
    char *mjson = pmm_layout_to_json(ml);
    pmm_layout_free(ml);
    if (!mjson) {
        return false;
    }
...
static void handle_layout(pmm_http_conn_t *c, const pmm_http_req_t *req) {
    char project[256] = {0};
    char max_str[32] = {0};
    char graph_str[32] = {0};
    ...
    if (pmm_http_query_param(req->query, "max_nodes", max_str, (int)sizeof(max_str))) {
        int v = atoi(max_str);
        if (v > 0)
            max_nodes = v;
    }
    ...
    if (pmm_http_query_param(req->query, "graph", graph_str, (int)sizeof(graph_str))) {
        missed_graph = strcmp(graph_str, "missed") == 0;
    }
    char scoped_project[320];
    if (missed_graph) {
        pmm_store_coverage_shadow_project(scoped_project, sizeof(scoped_project), project);
    } else {
    ...
    pmm_layout_result_t *layout =
        pmm_layout_compute(store, scoped_project, PMM_LAYOUT_OVERVIEW, NULL, 0, max_nodes);
    ...
    if (!layout) {
        pmm_store_close(store);
        pmm_http_replyf(c, 500, g_cors_json, "{\"error\":\"layout computation failed\"}");
        return;
    }
    ...
    char *primary_json = pmm_layout_to_json(layout);
    pmm_layout_free(layout);
    if (!primary_json) {
        pmm_store_close(store);
        pmm_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON serialization failed\"}");
        return;
    }
    ...
    if (linked_count == 0 && missed_graph) {
        pmm_store_close(store);
        pmm_http_replyf(c, 200, g_cors_json, "%s", primary_json);
        free(primary_json);
        return;
    }
    ...
    if (!pdoc) {
        pmm_store_close(store);
        pmm_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON parse failed\"}");
        return;
    }
    ...
    for (int li = 0; li < linked_count; li++) {
        ...
        if (!pmm_file_exists(lp_path)) {
            free(linked[li]);
            continue;
        }

        pmm_store_t *lp_store = pmm_store_open_path_query(lp_path);
        if (!lp_store) {
            free(linked[li]);
            continue;
        }
        ...
        pmm_layout_result_t *lp_layout =
            pmm_layout_compute(lp_store, linked[li], PMM_LAYOUT_OVERVIEW, NULL, 0, max_nodes);

        if (!lp_layout) {
            pmm_store_close(lp_store);
            free(linked[li]);
            continue;
        }

        double sat_radius = layout_radius(lp_layout);
        char *lp_json = pmm_layout_to_json(lp_layout);
        pmm_layout_free(lp_layout);
        if (!lp_json) {
            pmm_store_close(lp_store);
            free(linked[li]);
            continue;
        ...
        struct sqlite3 *src_db = pmm_store_get_db(store);
        struct sqlite3 *lp_db = pmm_store_get_db(lp_store);
        if (src_db && lp_db) {
    ...
    if (final_json) {
        pmm_http_replyf(c, 200, g_cors_json, "%s", final_json);
        free(final_json);
    } else {
        pmm_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON write failed\"}");
    }
}
...
static void handle_rpc(pmm_http_conn_t *c, const pmm_http_req_t *req, pmm_mcp_server_t *mcp) {
    ...
    if (!rpc_is_allowed_for_ui(req->body, req->body_len)) {
        pmm_http_replyf(c, 403, g_cors_json,
    ...
    char *response = pmm_mcp_server_handle(mcp, req->body);

    if (response) {
        pmm_http_replyf(c, 200, g_cors_json, "%s", response);
        free(response);
    } else {
        pmm_http_replyf(c, 204, g_cors, "%s", "");
    }
}
...
static bool request_passes_http_security(pmm_http_server_t *srv, pmm_http_conn_t *c,
                                         const pmm_http_req_t *req) {
    if (req->http_minor == 1 && req->host[0] == '\0') {
        pmm_http_replyf(c, 400, "", "%s", "{\"error\":\"Host header required\"}");
        return false;
    }
    if (req->host[0] != '\0' && !host_is_this_server(req->host, srv->port)) {
        pmm_http_replyf(c, 403, "", "%s", "{\"error\":\"forbidden host\"}");
        return false;
    }
    if (req->origin[0] != '\0' &&
        (req->host[0] == '\0' || !origin_is_same_server(req->origin, srv->port) ||
         !origin_matches_host(req->origin, req->host, srv->port))) {
        pmm_http_replyf(c, 403, "", "%s", "{\"error\":\"forbidden origin\"}");
        return false;
    }
    ...
    if (route_is_protected(req->path) && is_post && !content_type_is_json(req->content_type)) {
        pmm_http_replyf(c, 415, g_cors_json, "%s", "{\"error\":\"application/json required\"}");
        return false;
    }
    return true;
}

static void dispatch_request(pmm_http_server_t *srv, pmm_http_conn_t *c,
                             const pmm_http_req_t *req) {
    ...
    if (is_get && pmm_http_path_match(req->path, "/api/project-health*")) {
        handle_project_health(c, req);
        return;
    }

    /* GET /api/processes → list running project-mind-mcp processes */
    if (is_get && pmm_http_path_match(req->path, "/api/processes")) {
        handle_processes(c);
        return;
    }

    /* GET /api/logs → recent log lines */
    if (is_get && pmm_http_path_match(req->path, "/api/logs*")) {
        handle_logs(c, req);
        return;
    }

    /* GET / → index.html (no-cache so browser always gets latest) */
    if (pmm_http_path_match(req->path, "/")) {
        const pmm_embedded_file_t *f = pmm_embedded_lookup("/index.html");
        if (f) {
            char html_hdrs[1024];
            snprintf(html_hdrs, sizeof(html_hdrs),
                     "%sContent-Type: text/html\r\nCache-Control: no-cache\r\n" PMM_UI_CSP, g_cors);
            pmm_http_reply_buf(c, 200, html_hdrs, f->data, (size_t)f->size);
            return;
        }
        pmm_http_replyf(c, 404, g_cors, "no frontend embedded");
        return;
    }
    ...
    pmm_http_replyf(c, 404, g_cors, "not found");
}
...
static char *http_read_only_index_rejected(void *context, const char *repo_path,
    ...
    (void)repo_path;
    (void)args_json;
    return pmm_mcp_text_result("UI RPC indexing is disabled; use the coordinated /api/index route",
                               true);
}

pmm_http_server_t *pmm_http_server_new(int port) {
    pmm_http_server_t *srv = calloc(1, sizeof(*srv));
    if (!srv)
        return NULL;
    ...
    srv->mcp = pmm_mcp_server_new(NULL);
    if (!srv->mcp) {
        pmm_log_error("ui.http.mcp_fail", "reason", "cannot create MCP instance");
        free(srv);
        return NULL;
    }
    pmm_mcp_server_set_background_tasks(srv->mcp, false);
    pmm_mcp_server_set_index_executor(srv->mcp, http_read_only_index_rejected, srv);

    /* Bind to localhost only (httpd refuses anything else by construction) */
    srv->listener = pmm_httpd_listen(port);
    if (!srv->listener) {
        char port_str[16];
        snprintf(port_str, sizeof(port_str), "%d", port);
        pmm_log_warn("ui.unavailable", "port", port_str, "reason", "in_use", "hint",
                     "use --port=N to override");
        pmm_mcp_server_free(srv->mcp);
        free(srv);
        return NULL;
    }

    srv->port = pmm_httpd_port(srv->listener);
    ...
    pmm_log_info("ui.serving", "url", url, "port", port_str);
    ...
    for (int i = 0; i < MAX_INDEX_JOBS; i++) {
        if (srv->index_jobs[i].thread_started) {
            if (!atomic_load_explicit(&srv->index_jobs[i].completed, memory_order_acquire))
                return false;
            if (pmm_thread_join(&srv->index_jobs[i].thread) != 0)
                return false;
            srv->index_jobs[i].thread_started = false;
        }
    }
    if (!pmm_httpd_close(srv->listener))
        return false;
    pmm_mcp_server_free(srv->mcp);
    free(srv);
    return true;
...
void pmm_http_server_stop(pmm_http_server_t *srv) {
    if (srv) {
        ...
        pmm_httpd_interrupt(srv->listener);
    }
}
...
void pmm_http_server_run(pmm_http_server_t *srv) {
    ...
    while (!atomic_load(&srv->stop_flag)) {
        pmm_http_conn_t *conn = pmm_httpd_accept(srv->listener, 200);
        if (!conn)
            continue; /* timeout — re-check stop flag */

        uint64_t request_start_ms = pmm_now_ms();
        pmm_http_req_t req;
        int rc = pmm_httpd_read_request(conn, &req);
        if (rc == 0) {
            if (atomic_load(&srv->stop_flag)) {
                pmm_http_req_free(&req);
                pmm_httpd_conn_close(conn);
                break;
            }
            dispatch_request(srv, conn, &req);
            pmm_log_http_request("graph_ui", req.method, req.path, pmm_http_conn_status(conn),
                                 (int64_t)(pmm_now_ms() - request_start_ms), req.body_len,
                                 pmm_http_conn_response_bytes(conn));
            pmm_http_req_free(&req);
        } else if (rc > 0) {
            /* Parse/transport error with a known HTTP status (400/408/411/413/431).
             * No CORS reflection here — the request was never parsed. */
            pmm_http_replyf(conn, rc, "", "bad request");
            pmm_log_http_request("graph_ui", "", "", pmm_http_conn_status(conn),
                                 (int64_t)(pmm_now_ms() - request_start_ms), 0,
                                 pmm_http_conn_response_bytes(conn));
        }
        pmm_httpd_conn_close(conn);
    }
...
pmm_httpd_activity_t pmm_http_server_activity_for_test(pmm_http_server_t *srv) {
    return srv ? pmm_httpd_activity_for_test(srv->listener) : PMM_HTTPD_ACTIVITY_IDLE;
}
...
void pmm_http_server_set_recv_deadline_ms(pmm_http_server_t *srv, int ms) {
    if (srv && srv->listener_ok) {
        pmm_httpd_set_recv_deadline_ms(srv->listener, ms);
    }
}
...
void pmm_http_server_set_project_mutation_guard(pmm_http_server_t *srv,
    ...
    pmm_mcp_server_set_project_mutation_guard(srv->mcp, begin, end, begin ? context : NULL);
}
