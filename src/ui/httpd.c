...
#include <winsock2.h>
#include <ws2tcpip.h>
typedef SOCKET pmm_sock_t;
#define PMM_SOCK_BAD INVALID_SOCKET
#define pmm_sock_close closesocket
#else
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
#include <time.h>
#include <unistd.h>
typedef int pmm_sock_t;
#define PMM_SOCK_BAD (-1)
#define pmm_sock_close close
#endif

/* Suppress SIGPIPE on send: Linux has the per-call flag, macOS has the
 * per-socket option (set at accept), Windows has no signals. */
#ifdef MSG_NOSIGNAL
#define PMM_SEND_FLAGS MSG_NOSIGNAL
#else
#define PMM_SEND_FLAGS 0
#endif

/* A non-reading peer may occupy the sequential event loop only briefly. */
#define PMM_HTTP_SEND_DEADLINE_MS 1000
#define PMM_HTTP_SEND_SLICE_BYTES (64U * 1024U)
#ifdef _WIN32
...
#define PMM_HTTP_WAIT_SLICE_MS 50
#endif
...
struct pmm_httpd {
    ...
    pmm_mutex_t active_mutex;
    struct pmm_http_conn *active;
    bool interrupted;
    int send_buffer_for_test;
    int send_deadline_for_test_ms;
};

struct pmm_http_conn {
    pmm_sock_t fd;
    pmm_httpd_t *owner;
    int recv_deadline_ms;
...
static int wait_ready(pmm_sock_t fd, bool writing, int timeout_ms) {
#ifdef _WIN32
    fd_set ready;
    fd_set errors;
    FD_ZERO(&ready);
...
static int wait_readable(pmm_sock_t fd, int timeout_ms) {
...
static bool socket_set_nonblocking(pmm_sock_t fd) {
#ifdef _WIN32
    u_long enabled = 1;
...
static void socket_shutdown(pmm_sock_t fd) {
#ifdef _WIN32
...
static bool connection_interrupted(pmm_http_conn_t *connection) {
    pmm_httpd_t *owner = connection->owner;
    if (!owner)
        return false;
    pmm_mutex_lock(&owner->active_mutex);
    bool interrupted = owner->interrupted;
    pmm_mutex_unlock(&owner->active_mutex);
    return interrupted;
}
...
static int wait_connection_ready(pmm_http_conn_t *connection, bool writing, int64_t deadline) {
    for (;;) {
...
#ifdef _WIN32
        if (timeout_ms > PMM_HTTP_WAIT_SLICE_MS)
            timeout_ms = PMM_HTTP_WAIT_SLICE_MS;
#endif
...
static int send_all(pmm_http_conn_t *connection, const void *data, size_t len, int64_t deadline) {
    const char *p = data;
    size_t off = 0;
    while (off < len) {
        ...
        if (available > PMM_HTTP_SEND_SLICE_BYTES)
            available = PMM_HTTP_SEND_SLICE_BYTES;
#ifdef _WIN32
        int n = send(connection->fd, p + off, (int)available, PMM_SEND_FLAGS);
#else
        ssize_t n = send(connection->fd, p + off, available, PMM_SEND_FLAGS);
#endif
...
pmm_httpd_t *pmm_httpd_listen(int port) {
    ...
    pmm_sock_t fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd == PMM_SOCK_BAD)
        return NULL;
    ...
    pmm_httpd_t *d = calloc(1, sizeof(*d));
    if (!d) {
        pmm_sock_close(fd);
        return NULL;
    }
    ...
    d->recv_deadline_ms = PMM_HTTP_RECV_DEADLINE_MS;
    pmm_mutex_init(&d->active_mutex);
    return d;
}
...
void pmm_httpd_set_recv_deadline_ms(pmm_httpd_t *d, int ms) {
    if (d && ms > 0) {
        pmm_mutex_lock(&d->active_mutex);
        d->recv_deadline_ms = ms;
        pmm_mutex_unlock(&d->active_mutex);
    }
}
...
bool pmm_httpd_close(pmm_httpd_t *d) {
    ...
    pmm_mutex_unlock(&d->active_mutex);
    pmm_sock_close(d->fd);
    pmm_mutex_destroy(&d->active_mutex);
    free(d);
    return true;
}
...
pmm_httpd_activity_t pmm_httpd_activity_for_test(pmm_httpd_t *d) {
    if (!d)
        return PMM_HTTPD_ACTIVITY_IDLE;
    pmm_mutex_lock(&d->active_mutex);
    pmm_httpd_activity_t activity = PMM_HTTPD_ACTIVITY_IDLE;
    if (d->active) {
        activity = atomic_load_explicit(&d->active->response_started, memory_order_acquire)
                       ? PMM_HTTPD_ACTIVITY_RESPONDING
                       : PMM_HTTPD_ACTIVITY_READING_REQUEST;
    }
    pmm_mutex_unlock(&d->active_mutex);
    return activity;
}
...
pmm_http_conn_t *pmm_httpd_accept(pmm_httpd_t *d, int timeout_ms) {
    ...
    pmm_sock_t cfd = accept(d->fd, NULL, NULL);
    if (cfd == PMM_SOCK_BAD)
        return NULL;
    ...
    pmm_http_conn_t *c = calloc(1, sizeof(*c));
    if (!c) {
        pmm_sock_close(cfd);
        return NULL;
    }
...
int pmm_http_parse_head(const char *data, size_t len, pmm_http_req_t *req, size_t *body_offset,
    ...
    *content_length = 0;
    ...
    size_t scan = len < PMM_HTTP_MAX_HEAD ? len : PMM_HTTP_MAX_HEAD;
    ...
    if (head_end == 0)
        return len >= PMM_HTTP_MAX_HEAD ? 431 : PMM_HTTP_NEED_MORE;
    ...
    while (p < head_stop) {
        ...
        if (header_name_is(p, nlen, "origin")) {
            ...
            for (const char *v = val; *v; v++) {
                ...
                if (cl > (uint64_t)PMM_HTTP_MAX_BODY)
                    return 413;
            }
...
int pmm_httpd_read_request(pmm_http_conn_t *c, pmm_http_req_t *req) {
    ...
    char *head = malloc(PMM_HTTP_MAX_HEAD);
    if (!head)
        return -1;
    ...
    for (;;) {
...
#ifdef _WIN32
        int n = recv(c->fd, head + have, (int)(PMM_HTTP_MAX_HEAD - have), 0);
#else
        ssize_t n = recv(c->fd, head + have, PMM_HTTP_MAX_HEAD - have, 0);
#endif
        if (n < 0 && socket_would_block())
            continue;
        if (n <= 0) {
        ...
        rc = pmm_http_parse_head(head, have, req, &body_off, &clen);
        if (rc == 0)
            break;
        if (rc != PMM_HTTP_NEED_MORE) {
            free(head);
            return rc;
        }
        if (have >= PMM_HTTP_MAX_HEAD) {
            free(head);
            return 431;
        }
    }
...
void pmm_http_replyf(pmm_http_conn_t *c, int status, const char *extra_headers, const char *fmt,
    ...
    pmm_http_reply_buf(c, status, extra_headers, body, (size_t)need);
    if (body != stack_buf)
        free(body);
}
