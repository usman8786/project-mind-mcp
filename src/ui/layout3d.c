...
/* pmm_store_batch_count_degrees builds a bound "?,?,..." IN clause into a fixed
...
/* Ceiling for a caller-requested node budget. PMM_UI_MAX_RENDER_NODES lowers
...
static int render_node_limit(void) {
    const char *raw = getenv("PMM_UI_MAX_RENDER_NODES");
    if (!raw || !raw[0]) {
...
static octree_node_t *octree_new(float ox, float oy, float oz, float half) {
    octree_node_t *n = calloc(PMM_ALLOC_ONE, sizeof(*n));
    if (!n)
        return NULL;
    n->ox = ox;
    n->oy = oy;
...
static void free_edge_array(pmm_edge_t *edges, int count) {
    if (!edges)
        return;
...
pmm_layout_result_t *pmm_layout_compute(pmm_store_t *store, const char *project,
    ...
    pmm_search_output_t search_out;
    memset(&search_out, 0, sizeof(search_out));
    if (pmm_store_search(store, &params, &search_out) != PMM_STORE_OK)
        return calloc(PMM_ALLOC_ONE, sizeof(pmm_layout_result_t));

    int n = search_out.count, total_count = search_out.total;
    if (n == 0) {
        pmm_store_search_free(&search_out);
        pmm_layout_result_t *r = calloc(PMM_ALLOC_ONE, sizeof(*r));
        if (r)
    ...
    int mapped = 0;
    int edge_cap = PMM_SZ_256;
    pmm_edge_t *all_edges = malloc((size_t)edge_cap * sizeof(pmm_edge_t));
    int *es = malloc((size_t)edge_cap * sizeof(int));
    int *ed = malloc((size_t)edge_cap * sizeof(int));
    pmm_schema_info_t schema;
    memset(&schema, 0, sizeof(schema));
    if (deg && all_edges && es && ed &&
        pmm_store_get_schema(store, project, &schema) == PMM_STORE_OK) {
        for (int t = 0; t < schema.edge_type_count; t++) {
            pmm_edge_t *te = NULL;
            int tc = 0;
            if (pmm_store_find_edges_by_type(store, project, schema.edge_types[t].type, &te, &tc) ==
                PMM_STORE_OK) {
                for (int e = 0; e < tc; e++) {
                    ...
                    if (si >= 0 && di >= 0) {
                        if (mapped >= edge_cap) {
                            ...
                            pmm_edge_t *te2 = realloc(all_edges, (size_t)nc * sizeof(pmm_edge_t));
                        ...
                        memset(&te[e], 0, sizeof(pmm_edge_t));
                        es[mapped] = si;
                        ed[mapped] = di;
                        deg[si]++;
                        deg[di]++;
    ...
    pmm_layout_result_t *result = calloc(PMM_ALLOC_ONE, sizeof(*result));
    if (!result || !bodies) {
        free(bodies);
        free(deg);
        free(es);
        free(ed);
        free(cdepth);
        pmm_layout_free(result);
        free_edge_array(all_edges, mapped);
        pmm_store_search_free(&search_out);
        return NULL;
    }
    result->nodes = calloc((size_t)n, sizeof(pmm_layout_node_t));
    result->node_count = n;
    ...
    if (node_ids && in_calls && in_usage && deg_dummy) {
        ...
        for (int off = 0; off < n; off += DEAD_DEGREE_CHUNK) {
            ...
            pmm_store_batch_count_degrees(store, node_ids + off, cnt, "CALLS", in_calls + off,
                                          deg_dummy + off);
            pmm_store_batch_count_degrees(store, node_ids + off, cnt, "USAGE", in_usage + off,
                                          deg_dummy + off);
        }
    }

    for (int i = 0; i < n; i++) {
        const pmm_node_t *sn = &search_out.results[i].node;
        const char *fp = sn->file_path ? sn->file_path : "";

        /* Cluster key = first 3 dir components */
        char ck[PMM_SZ_256] = {0};
        {
            const char *p = fp;
            int sl = 0, ki = 0;
            while (*p && ki < 255) {
        ...
        bool testish = nf.is_test || (sn->file_path && pmm_is_test_file_path(sn->file_path));
    ...
    /* 8. Output edges */
    if (mapped > 0) {
        result->edges = calloc((size_t)mapped, sizeof(pmm_layout_edge_t));
...
void pmm_layout_free(pmm_layout_result_t *r) {
    if (!r)
        return;
...
char *pmm_layout_to_json(const pmm_layout_result_t *r) {
    if (!r)
        return NULL;
    ...
    if (!json) {
        ...
        pmm_log_error("layout.json.fail", "code", code, "msg",
