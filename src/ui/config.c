 ...
 *
 * Config file: ~/.cache/project-mind-mcp/config.json
...
void pmm_ui_config_load(pmm_ui_config_t *cfg) {
    ...
    cfg->ui_enabled = PMM_UI_DEFAULT_ENABLED;
    cfg->ui_port = PMM_UI_DEFAULT_PORT;
    ...
    free(buffer);
    if (!doc) {
        pmm_log_warn("ui.config.corrupt", "path", path);
...
bool pmm_ui_config_save(const pmm_ui_config_t *cfg) {
    if (!cfg || cfg->ui_port <= 0 || cfg->ui_port > 65535) {
        pmm_log_error("ui.config.write_fail", "reason", "invalid_config");
        return false;
    }
    char path[PMM_SZ_1K];
    pmm_ui_config_path(path, (int)sizeof(path));
    ...
    if (directory_ready && !pmm_is_dir(dir)) {
        directory_ready = pmm_mkdir_p(dir, 0750) || pmm_is_dir(dir);
    }
    if (!directory_ready) {
        pmm_log_error("ui.config.write_fail", "path", path, "reason", "create_directory");
        return false;
    }
    ...
    if (!json) {
        pmm_log_error("ui.config.write_fail", "reason", "serialize");
        return false;
    }
    ...
    free(json);
    if (!saved) {
        pmm_log_error("ui.config.write_fail", "path", path, "reason", "atomic_publish");
        return false;
    }

    pmm_log_debug("ui.config.saved", "path", path);
    return true;
}
