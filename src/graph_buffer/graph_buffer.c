 ...
 * The buffer frees everything in pmm_gbuf_free().
 */
...
#define EDGE_KEY_BUF PMM_SZ_256

/* Per-type or per-key edge list stored in hash tables as values */
typedef PMM_DYN_ARRAY(const pmm_gbuf_edge_t *) edge_ptr_array_t;

/* Per-label or per-name node list */
typedef PMM_DYN_ARRAY(const pmm_gbuf_node_t *) node_ptr_array_t;
...
struct pmm_gbuf {
    ...
    PMM_DYN_ARRAY(pmm_gbuf_node_t *) nodes;

    /* Primary index: QN → pmm_gbuf_node_t* */
    CBMHashTable *node_by_qn;
    /* Primary index: "id" string → pmm_gbuf_node_t* */
    ...
    PMM_DYN_ARRAY(pmm_gbuf_edge_t *) edges;

    /* Edge dedup index: "srcID:tgtID:type" → pmm_gbuf_edge_t* */
     ...
     * freed by free_node_strings/free_edge_strings — only once in pmm_gbuf_free. */
    CBMHashTable *intern_pool;

    /* Vector storage for semantic embeddings (filled by pass_semantic_edges,
     * consumed by pmm_write_db during dump). */
...
static const char *gb_intern(pmm_gbuf_t *gb, const char *s) {
    const char *key = s ? s : "{}";
    const char *found = pmm_ht_get(gb->intern_pool, key);
    if (found) {
        return found;
    }
    char *copy = strdup(key);
    if (copy) {
        pmm_ht_set(gb->intern_pool, copy, copy); /* key == value == owned copy */
    }
    return copy;
}
...
static node_ptr_array_t *get_or_create_node_array(CBMHashTable *ht, const char *key) {
    node_ptr_array_t *arr = pmm_ht_get(ht, key);
    if (!arr) {
        arr = calloc(PMM_ALLOC_ONE, sizeof(node_ptr_array_t));
        pmm_ht_set(ht, strdup(key), arr);
    }
    return arr;
}
...
static edge_ptr_array_t *get_or_create_edge_array(CBMHashTable *ht, const char *key) {
    edge_ptr_array_t *arr = pmm_ht_get(ht, key);
    if (!arr) {
        arr = calloc(PMM_ALLOC_ONE, sizeof(edge_ptr_array_t));
        pmm_ht_set(ht, strdup(key), arr);
    }
    return arr;
}
 ...
 * (pool-owned) — NOT freed here; the pool frees them once in pmm_gbuf_free. */
static void free_node_strings(pmm_gbuf_node_t *n) {
    free(n->name);
 ...
 * freed here; the pool frees it once in pmm_gbuf_free. */
static void free_edge_strings(pmm_gbuf_edge_t *e) {
    free(e->properties_json);
}

/* Allocate the next buffer-local or shared-atomic ID. */
static int64_t alloc_next_id(pmm_gbuf_t *gb) {
    if (gb->shared_ids) {
...
static void unindex_edge(pmm_gbuf_t *gb, const pmm_gbuf_edge_t *e) {
    char key[EDGE_KEY_BUF];

    make_edge_key(key, sizeof(key), e->source_id, e->target_id, e->type, e->properties_json);
    const char *ekey = pmm_ht_get_key(gb->edge_by_key, key);
    pmm_ht_delete(gb->edge_by_key, key);
    free((void *)ekey);

    make_src_type_key(key, sizeof(key), e->source_id, e->type);
    remove_edge_from_ptr_array(pmm_ht_get(gb->edges_by_source_type, key), e->id);

    make_src_type_key(key, sizeof(key), e->target_id, e->type);
    remove_edge_from_ptr_array(pmm_ht_get(gb->edges_by_target_type, key), e->id);

    remove_edge_from_ptr_array(pmm_ht_get(gb->edges_by_type, e->type), e->id);
}

/* Cascade-delete all edges touching nodes in deleted_set. */
static void cascade_delete_edges(pmm_gbuf_t *gb, CBMHashTable *deleted_set) {
    int write_idx = 0;
    for (int i = 0; i < gb->edges.count; i++) {
        pmm_gbuf_edge_t *e = gb->edges.items[i];
        char src_id[PMM_SZ_32];
        char tgt_id[PMM_SZ_32];
        make_id_key(src_id, sizeof(src_id), e->source_id);
        make_id_key(tgt_id, sizeof(tgt_id), e->target_id);

        if (pmm_ht_get(deleted_set, src_id) || pmm_ht_get(deleted_set, tgt_id)) {
            unindex_edge(gb, e);
            free_edge_strings(e);
            free(e);
        } else {
...
static void register_node_in_indexes(pmm_gbuf_t *gb, pmm_gbuf_node_t *node) {
    pmm_ht_set(gb->node_by_qn, node->qualified_name, node);

    if (node->id >= gb->by_id_cap) {
        int64_t nc = gb->by_id_cap > 0 ? gb->by_id_cap : PMM_SZ_1K;
        while (nc <= node->id) {
            nc *= 2;
        }
        pmm_gbuf_node_t **grown = realloc(gb->by_id, (size_t)nc * sizeof(*grown));
        if (grown) {
    ...
    pmm_da_push(by_label, (const pmm_gbuf_node_t *)node);

    node_ptr_array_t *by_name =
        get_or_create_node_array(gb->nodes_by_name, node->name ? node->name : "");
    pmm_da_push(by_name, (const pmm_gbuf_node_t *)node);
}

/* Push an edge pointer into a dynamic array (wraps macro to reduce CC contribution). */
static void edge_array_push(edge_ptr_array_t *arr, const pmm_gbuf_edge_t *edge) {
    pmm_da_push(arr, edge);
}

/* Index an edge by one key into a hash table bucket. */
static void index_edge_by_key(CBMHashTable *ht, const char *key, pmm_gbuf_edge_t *edge) {
    edge_ptr_array_t *arr = get_or_create_edge_array(ht, key);
    edge_array_push(arr, (const pmm_gbuf_edge_t *)edge);
}

/* Register an edge in secondary indexes (source_type, target_type, type). */
static void register_edge_in_indexes(pmm_gbuf_t *gb, pmm_gbuf_edge_t *edge) {
    char key[EDGE_KEY_BUF];
...
static void rebuild_edge_secondary_indexes(pmm_gbuf_t *gb) {
    pmm_ht_foreach(gb->edges_by_source_type, free_edge_array, NULL);
    pmm_ht_free(gb->edges_by_source_type);
    pmm_ht_foreach(gb->edges_by_target_type, free_edge_array, NULL);
    pmm_ht_free(gb->edges_by_target_type);
    pmm_ht_foreach(gb->edges_by_type, free_edge_array, NULL);
    pmm_ht_free(gb->edges_by_type);

    gb->edges_by_source_type = pmm_ht_create(PMM_SZ_256);
    gb->edges_by_target_type = pmm_ht_create(PMM_SZ_256);
    gb->edges_by_type = pmm_ht_create(PMM_SZ_32);
...
static void release_gbuf_indexes(pmm_gbuf_t *gb) {
    pmm_ht_free(gb->node_by_qn);
    gb->node_by_qn = NULL;
    free(gb->by_id);
    gb->by_id = NULL;
    gb->by_id_cap = 0;
    pmm_ht_foreach(gb->nodes_by_label, free_node_array, NULL);
    pmm_ht_free(gb->nodes_by_label);
    gb->nodes_by_label = NULL;
    pmm_ht_foreach(gb->nodes_by_name, free_node_array, NULL);
    pmm_ht_free(gb->nodes_by_name);
    gb->nodes_by_name = NULL;
    pmm_ht_foreach(gb->edge_by_key, free_key_only, NULL);
    pmm_ht_free(gb->edge_by_key);
    gb->edge_by_key = NULL;
    pmm_ht_foreach(gb->edges_by_source_type, free_edge_array, NULL);
    pmm_ht_free(gb->edges_by_source_type);
...
pmm_gbuf_t *pmm_gbuf_new(const char *project, const char *root_path) {
    pmm_gbuf_t *gb = calloc(PMM_ALLOC_ONE, sizeof(pmm_gbuf_t));
    if (!gb) {
        return NULL;
    }
    ...
    gb->node_by_qn = pmm_ht_create(PMM_SZ_256);
    gb->by_id = NULL;
    gb->by_id_cap = 0;
    gb->nodes_by_label = pmm_ht_create(PMM_SZ_32);
    gb->nodes_by_name = pmm_ht_create(PMM_SZ_256);

    gb->edge_by_key = pmm_ht_create(PMM_SZ_512);
    gb->edges_by_source_type = pmm_ht_create(PMM_SZ_256);
    gb->edges_by_target_type = pmm_ht_create(PMM_SZ_256);
    gb->edges_by_type = pmm_ht_create(PMM_SZ_32);

    gb->intern_pool = pmm_ht_create(PMM_SZ_1K);
...
pmm_gbuf_t *pmm_gbuf_new_shared_ids(const char *project, const char *root_path,
    ...
    pmm_gbuf_t *gb = pmm_gbuf_new(project, root_path);
    if (gb && id_source) {
...
void pmm_gbuf_free(pmm_gbuf_t *gb) {
    ...
    for (int i = 0; i < gb->nodes.count; i++) {
        pmm_gbuf_node_t *n = gb->nodes.items[i];
        free_node_strings(n);
        free(n);
    }
    pmm_da_free(&gb->nodes);
    ...
    if (gb->intern_pool) {
        pmm_ht_foreach(gb->intern_pool, free_key_only, NULL);
        pmm_ht_free(gb->intern_pool);
    }
...
int64_t pmm_gbuf_upsert_node(pmm_gbuf_t *gb, const char *label, const char *name,
    ...
    pmm_gbuf_node_t *existing = pmm_ht_get(gb->node_by_qn, qualified_name);
    if (existing) {
         ...
         * label/name — pmm_gbuf_find_by_label/name then missed or mis-listed it,
        ...
        if (label_changed) {
            remove_node_from_ptr_array(
                pmm_ht_get(gb->nodes_by_label, existing->label ? existing->label : ""),
                existing->id);
        }
        if (name_changed) {
            remove_node_from_ptr_array(
                pmm_ht_get(gb->nodes_by_name, existing->name ? existing->name : ""), existing->id);
        }
        ...
        if (label_changed) {
            ...
            pmm_da_push(by_label, (const pmm_gbuf_node_t *)existing);
        }
        if (name_changed) {
            node_ptr_array_t *by_name =
                get_or_create_node_array(gb->nodes_by_name, existing->name ? existing->name : "");
            pmm_da_push(by_name, (const pmm_gbuf_node_t *)existing);
        }
        return existing->id;
    }

    /* Heap-allocate a new node (pointer stays stable across array growth) */
    pmm_gbuf_node_t *node = calloc(PMM_ALLOC_ONE, sizeof(pmm_gbuf_node_t));
    if (!node) {
        return 0;
    }
...
const pmm_gbuf_node_t *pmm_gbuf_find_by_qn(const pmm_gbuf_t *gb, const char *qn) {
    ...
    return pmm_ht_get(gb->node_by_qn, qn);
}
...
int pmm_gbuf_find_by_label(const pmm_gbuf_t *gb, const char *label, const pmm_gbuf_node_t ***out,
    ...
    node_ptr_array_t *arr = pmm_ht_get(gb->nodes_by_label, label ? label : "");
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int pmm_gbuf_find_by_name(const pmm_gbuf_t *gb, const char *name, const pmm_gbuf_node_t ***out,
    ...
    node_ptr_array_t *arr = pmm_ht_get(gb->nodes_by_name, name ? name : "");
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int pmm_gbuf_node_count(const pmm_gbuf_t *gb) {
    ...
    return gb ? (int)pmm_ht_count(gb->node_by_qn) : 0;
}
...
int pmm_gbuf_delete_by_label(pmm_gbuf_t *gb, const char *label) {
    ...
    node_ptr_array_t *arr = pmm_ht_get(gb->nodes_by_label, label);
    if (!arr || arr->count == 0) {
        return 0;
    }

    /* Build hash set of deleted node IDs for O(1) lookup */
    CBMHashTable *deleted_set = pmm_ht_create(arr->count);
    for (int i = 0; i < arr->count; i++) {
        const pmm_gbuf_node_t *n = arr->items[i];

        char id_buf[PMM_SZ_32];
        make_id_key(id_buf, sizeof(id_buf), n->id);
        pmm_ht_set(deleted_set, strdup(id_buf), intptr_to_ptr(SKIP_ONE));

        /* Remove from primary indexes */
        pmm_ht_delete(gb->node_by_qn, n->qualified_name);
    ...
    pmm_ht_foreach(deleted_set, free_key_only, NULL);
    pmm_ht_free(deleted_set);
    return 0;
}
...
int pmm_gbuf_delete_by_file(pmm_gbuf_t *gb, const char *file_path) {
    ...
    CBMHashTable *deleted_set = pmm_ht_create(PMM_SZ_64);
    int deleted_count = 0;
    int scanned = 0;
    ...
    for (int i = 0; i < gb->nodes.count; i++) {
        ...
        if (!n->qualified_name || !pmm_ht_get(gb->node_by_qn, n->qualified_name)) {
            continue;
        }
        ...
        remove_node_from_ptr_array(pmm_ht_get(gb->nodes_by_label, n->label), n->id);
        remove_node_from_ptr_array(pmm_ht_get(gb->nodes_by_name, n->name), n->id);
        ...
        /* NULL out QN so dump's liveness check (pmm_ht_get by QN) fails
    ...
    pmm_ht_free(deleted_set);
    {
        char s_buf[PMM_SZ_16];
        char d_buf[PMM_SZ_16];
        snprintf(s_buf, sizeof(s_buf), "%d", scanned);
        snprintf(d_buf, sizeof(d_buf), "%d", deleted_count);
        pmm_log_info("gbuf.delete_by_file", "file", file_path, "scanned", s_buf, "deleted", d_buf);
    }
...
int pmm_gbuf_load_from_db(pmm_gbuf_t *gb, const char *db_path, const char *project) {
    ...
    if (sqlite3_prepare_v2(db, "SELECT MAX(id) FROM nodes WHERE project = ?", PMM_NOT_FOUND, &stmt,
                           NULL) != SQLITE_OK) {
        pmm_store_close(store);
        return PMM_NOT_FOUND;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        int64_t new_id = pmm_gbuf_upsert_node(gb, label, name, qn, fp, sl, el, props);
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (new_src > 0 && new_tgt > 0) {
            pmm_gbuf_insert_edge(gb, new_src, new_tgt, type, props);
        }
    }
    sqlite3_finalize(stmt);
...
void pmm_gbuf_foreach_node(const pmm_gbuf_t *gb, pmm_gbuf_node_visitor_fn fn, void *userdata) {
    ...
    for (int i = 0; i < gb->nodes.count; i++) {
        const pmm_gbuf_node_t *n = gb->nodes.items[i];
        if (n->qualified_name && pmm_ht_get(gb->node_by_qn, n->qualified_name)) {
            fn(n, userdata);
        }
    }
...
int64_t pmm_gbuf_insert_edge(pmm_gbuf_t *gb, int64_t source_id, int64_t target_id, const char *type,
    ...
    pmm_gbuf_edge_t *existing = pmm_ht_get(gb->edge_by_key, key);
    if (existing) {
    ...
    pmm_gbuf_edge_t *edge = calloc(PMM_ALLOC_ONE, sizeof(pmm_gbuf_edge_t));
    if (!edge) {
        return 0;
    }
    ...
    pmm_da_push(&gb->edges, edge);

    /* Dedup index */
    pmm_ht_set(gb->edge_by_key, strdup(key), edge);
...
int pmm_gbuf_find_edges_by_source_type(const pmm_gbuf_t *gb, int64_t source_id, const char *type,
    ...
    edge_ptr_array_t *arr = pmm_ht_get(gb->edges_by_source_type, key);
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int pmm_gbuf_find_edges_by_target_type(const pmm_gbuf_t *gb, int64_t target_id, const char *type,
    ...
    edge_ptr_array_t *arr = pmm_ht_get(gb->edges_by_target_type, key);
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int pmm_gbuf_find_edges_by_type(const pmm_gbuf_t *gb, const char *type,
    ...
    edge_ptr_array_t *arr = pmm_ht_get(gb->edges_by_type, type);
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
static void merge_update_existing(pmm_gbuf_t *dst, pmm_gbuf_node_t *existing,
                                  const pmm_gbuf_node_t *sn, CBMHashTable **remap) {
    /* Same guard as pmm_gbuf_upsert_node: a per-file "Module" def coming from a
    ...
    if (!module_on_container) {
         ...
         * pmm_gbuf_upsert_node exactly. Distinct source entities can share a
        ...
        if (sn_wins) {
            ...
            if (label_changed) {
                remove_node_from_ptr_array(
                    pmm_ht_get(dst->nodes_by_label, existing->label ? existing->label : ""),
                    existing->id);
            }
            if (name_changed) {
                remove_node_from_ptr_array(
                    pmm_ht_get(dst->nodes_by_name, existing->name ? existing->name : ""),
                    existing->id);
            }
    ...
    if (sn->id != existing->id) {
        if (!*remap) {
            *remap = pmm_ht_create(PMM_SZ_32);
        }
        char key[PMM_SZ_32];
        make_id_key(key, sizeof(key), sn->id);
        int64_t *val = malloc(sizeof(int64_t));
        *val = existing->id;
        pmm_ht_set(*remap, strdup(key), val);
    }
}

/* Copy a non-colliding src node into dst with its original ID. */
static void merge_copy_new_node(pmm_gbuf_t *dst, const pmm_gbuf_node_t *sn) {
    pmm_gbuf_node_t *node = calloc(PMM_ALLOC_ONE, sizeof(pmm_gbuf_node_t));
    if (!node) {
        return;
    }
...
static void merge_remap_edges(pmm_gbuf_t *dst, pmm_gbuf_t *src, CBMHashTable *remap) {
    for (int i = 0; i < src->edges.count; i++) {
        pmm_gbuf_edge_t *se = src->edges.items[i];
        ...
        if (remap) {
            ...
            int64_t *remapped = pmm_ht_get(remap, key);
            if (remapped) {
                new_src = *remapped;
            }

            make_id_key(key, sizeof(key), se->target_id);
            remapped = pmm_ht_get(remap, key);
            if (remapped) {
                new_tgt = *remapped;
            }
        }

        pmm_gbuf_insert_edge(dst, new_src, new_tgt, se->type, se->properties_json);
    }
}
...
int pmm_gbuf_merge(pmm_gbuf_t *dst, pmm_gbuf_t *src) {
    ...
    for (int i = 0; i < src->nodes.count; i++) {
        pmm_gbuf_node_t *sn = src->nodes.items[i];
        if (!sn->qualified_name) {
            continue;
        }

        /* Skip nodes deleted from QN index */
        if (!pmm_ht_get(src->node_by_qn, sn->qualified_name)) {
            continue;
        }

        pmm_gbuf_node_t *existing = pmm_ht_get(dst->node_by_qn, sn->qualified_name);
        if (existing) {
    ...
    if (remap) {
        pmm_ht_foreach(remap, free_remap_entry, NULL);
        pmm_ht_free(remap);
    }
...
static CBMDumpNode *build_dump_nodes(pmm_gbuf_t *gb, int live_count, int64_t *temp_to_final,
                                     int64_t max_temp_id, int *out_count,
                                     pmm_gbuf_node_t ***src_out) {
    ...
    pmm_gbuf_node_t **src = malloc(cap * sizeof(pmm_gbuf_node_t *));
    int idx = 0;
...
static CBMDumpEdge *build_dump_edges(pmm_gbuf_t *gb, const int64_t *temp_to_final,
...
static void remap_sort_dedup_vectors(pmm_gbuf_t *gb, const int64_t *temp_to_final,
                                     int64_t max_temp_id) {
    int remapped = 0;
    int dropped = 0;
    for (int i = 0; i < gb->dump_vector_count; i++) {
    ...
    if (dropped > 0) {
        char r_buf[PMM_SZ_16];
        char d_buf[PMM_SZ_16];
        snprintf(r_buf, sizeof(r_buf), "%d", remapped);
        snprintf(d_buf, sizeof(d_buf), "%d", dropped);
        pmm_log_info("dump.vectors.remap", "remapped", r_buf, "dropped", d_buf);
    }
...
static void log_dump_summary(int node_count, int edge_count) {
    ...
    pmm_log_info("gbuf.dump", "nodes", b1, "edges", b2);
}
...
static int count_live_nodes(pmm_gbuf_t *gb) {
    int count = 0;
    for (int i = 0; i < gb->nodes.count; i++) {
        pmm_gbuf_node_t *n = gb->nodes.items[i];
        if (n->qualified_name && pmm_ht_get(gb->node_by_qn, n->qualified_name)) {
...
static void generate_iso_timestamp(char *buf, size_t buf_size) {
    ...
    struct tm tm_buf;
    struct tm *tm_val = pmm_gmtime_r(&now, &tm_buf);
...
static void release_and_remap_vectors(pmm_gbuf_t *gb, const int64_t *temp_to_final,
                                      int64_t max_temp_id) {
    PMM_PROF_START(t_vec_remap);
    remap_sort_dedup_vectors(gb, temp_to_final, max_temp_id);
    PMM_PROF_END_N("dump", "5_vector_remap_sort", t_vec_remap, gb->dump_vector_count);
}
...
int pmm_gbuf_dump_to_sqlite(pmm_gbuf_t *gb, const char *path) {
    ...
    PMM_PROF_START(t_count);
    int live_count = count_live_nodes(gb);
    PMM_PROF_END_N("dump", "1_count_live_nodes", t_count, live_count);

    PMM_PROF_START(t_build_nodes);
    ...
    PMM_PROF_START(t_release_idx);
    release_gbuf_indexes(gb);
    PMM_PROF_END("dump", "4_release_gbuf_indexes", t_release_idx);

    char indexed_at[PMM_SZ_64];
    ...
    pmm_db_writer_t *w = pmm_writer_open(path);
    if (!w) {
        free(src_nodes);
        free(dump_nodes);
        free(temp_to_final);
    ...
    for (int off = 0; off < node_idx; off += DUMP_PARTITION_NODES) {
        ...
        rc = pmm_writer_append_nodes(w, &dump_nodes[off], chunk);
        if (rc != 0) {
            break;
        }
        free_heavy = free_heavy || (pmm_mem_budget() > 0 && pmm_mem_over_budget());
    ...
    PMM_PROF_END_N("dump", "2b_stream_append_nodes", t_append, node_idx);
    ...
    if (rc == 0) {
        PMM_PROF_START(t_build_edges);
        dump_edges =
            build_dump_edges(gb, temp_to_final, max_temp_id, &edge_idx, &url_paths, &local_names);
        PMM_PROF_END_N("dump", "3_build_dump_edges", t_build_edges, edge_idx);
    ...
    PMM_PROF_START(t_finalize);
    int frc = pmm_writer_finalize(w, gb->project, gb->root_path, indexed_at, dump_nodes, node_idx,
                                  dump_edges, edge_idx, gb->dump_vectors, gb->dump_vector_count,
                                  gb->dump_token_vecs, gb->dump_token_vec_count);
    PMM_PROF_END_N("dump", "6_write_db_finalize", t_finalize, node_idx + edge_idx);
    if (rc == 0) {
        rc = frc;
    }
...
int pmm_gbuf_flush_to_store(pmm_gbuf_t *gb, pmm_store_t *store) {
    ...
    /* Upsert project */
    pmm_store_upsert_project(store, gb->project, gb->root_path);

    /* Begin bulk mode */
    pmm_store_begin_bulk(store);
    pmm_store_drop_indexes(store);
    pmm_store_begin(store);

    /* Delete existing project data */
    pmm_store_delete_edges_by_project(store, gb->project);
    pmm_store_delete_nodes_by_project(store, gb->project);
    ...
    for (int i = 0; i < gb->nodes.count; i++) {
        ...
        int64_t real_id = pmm_store_upsert_node(store, &sn);
    ...
    for (int i = 0; i < gb->edges.count; i++) {
        ...
        pmm_store_insert_edge(store, &se);
    }

    pmm_store_commit(store);
    pmm_store_create_indexes(store);
    pmm_store_end_bulk(store);
