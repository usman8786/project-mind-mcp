 ...
 */
#ifndef PMM_STR_UTIL_H
#define PMM_STR_UTIL_H
 ...
 *
 * Usage: PMM_SNPRINTF_APPEND(buf, sizeof(buf), off, "fmt %s", arg);
 * Requires: <stdio.h> included by caller. */
#define PMM_SNPRINTF_APPEND(buf, sz, off, ...)                                       \
    do {                                                                             \
        if ((off) >= 0 && (off) < (int)(sz)) {                                       \
            int _pmm_r = snprintf((buf) + (off), (sz) - (size_t)(off), __VA_ARGS__); \
            if (_pmm_r > 0)                                                          \
                (off) += _pmm_r;                                                     \
            if ((off) >= (int)(sz))                                                  \
                (off) = (int)(sz) - 1;                                               \
