...
struct pmm_dir {
    ...
    wchar_t wide_pattern[PMM_PATH_MAX];
    pmm_dirent_t entry;
    bool first;
    bool done;
};
...
pmm_dir_t *pmm_opendir(const char *path) {
    ...
    if (wlen == 0 || wlen + 2 >= PMM_PATH_MAX) {
        free(wpath);
        return NULL;
    }

    pmm_dir_t *d = (pmm_dir_t *)calloc(PMM_ALLOC_ONE, sizeof(pmm_dir_t));
    if (!d) {
        free(wpath);
        return NULL;
    }
...
pmm_dirent_t *pmm_readdir(pmm_dir_t *d) {
    ...
    char *u8 = pmm_wide_to_utf8(d->find_data.cFileName);
    if (!u8) {
        d->done = true;
        return NULL;
    }
    size_t nlen = strlen(u8);
    if (nlen >= PMM_DIRENT_NAME_MAX) {
        nlen = PMM_DIRENT_NAME_MAX - SKIP_ONE;
    }
 ...
 * falling back would silently re-arm the deadlock. pmm_popen logs a structured
 * warning and returns NULL instead (every call site handles NULL). */

enum { PMM_POPEN_MAX = 16 };
static struct {
    FILE *fp;
    HANDLE proc;
} g_popen_tab[PMM_POPEN_MAX];
 ...
 * pmm_popen(..., "r") stream came from the isolated spawn. Test-only
 * observable; not synchronized across threads. */
static volatile LONG g_popen_last_isolated = 0;

int pmm_popen_last_was_isolated(void) {
    return (int)g_popen_last_isolated;
}

static BOOL CALLBACK pmm_popen_init(PINIT_ONCE once, PVOID param, PVOID *ctx) {
    (void)once;
    (void)param;
    (void)ctx;
...
static wchar_t *pmm_resolve_comspec(void) {
...
static FILE *pmm_popen_isolated(const char *cmd, const char **stage, DWORD *gle) {
    *stage = "";
    *gle = 0;
    InitOnceExecuteOnce(&g_popen_once, pmm_popen_init, NULL, NULL);
    ...
    wchar_t *app = pmm_resolve_comspec();
...
FILE *pmm_popen(const char *cmd, const char *mode) {
    ...
    if (mode && mode[0] == 'r' && mode[1] == '\0') {
        const char *stage = "";
        DWORD gle = 0;
        FILE *fp = pmm_popen_isolated(cmd, &stage, &gle);
        g_popen_last_isolated = (fp != NULL);
        if (!fp) {
            char glebuf[PMM_SZ_16];
            char errnobuf[PMM_SZ_16];
            snprintf(glebuf, sizeof(glebuf), "%lu", (unsigned long)gle);
            snprintf(errnobuf, sizeof(errnobuf), "%d", errno);
            pmm_log_warn("compat.popen_isolated_failed", "stage", stage, "gle", glebuf, "errno",
                         errnobuf);
        }
        return fp;
    }
...
FILE *pmm_fopen(const char *path, const char *mode) {
    ...
    wchar_t *wmode = pmm_utf8_to_wide(mode);
    if (!wmode) {
        free(wpath);
        return NULL;
    }
 ...
 * DACL on a directory pmm just created. Under the Administrators-default-owner
...
static void pmm_windows_stamp_dir_owner(const wchar_t *path) {
    HANDLE token = NULL;
...
static bool pmm_windows_mkdir_component(wchar_t *path) {
    ...
    if (created) {
        pmm_windows_stamp_dir_owner(path);
    }
    return true;
}
...
bool pmm_mkdir_p(const char *path, int mode) {
    ...
    for (wchar_t *p = tmp + start; ok && *p; p++) {
        if (*p == L'/' || *p == L'\\') {
            ...
            wchar_t separator = *p;
            *p = L'\0';
            ok = pmm_windows_mkdir_component(tmp);
            *p = separator;
        }
    }
    if (ok) {
 ...
 * bytes are treated as UTF-8 and converted to wide via pmm_utf8_to_wide,
...
wchar_t *pmm_build_cmdline(const char *const *argv) {
     ...
     * whole via pmm_utf8_to_wide. Every character the quoting logic acts
    ...
    *w = '\0';
    ...
    wchar_t *out = pmm_utf8_to_wide(buf);
    free(buf);
    return out;
}
...
int pmm_exec_no_shell(const char *const *argv) {
    ...
    wchar_t *cmdline = pmm_build_cmdline(argv);
    if (!cmdline) {
        return PMM_NOT_FOUND;
    }
    ...
    DWORD exit_code = (DWORD)PMM_NOT_FOUND;
...
static int pmm_open_directory_component(int parent, const char *component, int flags) {
...
bool pmm_mkdir_p(const char *path, int mode) {
    ...
    while (ok && *cursor) {
        ...
        if (cursor[0] != '\0' && strcmp(cursor, ".") != 0) {
            int next = pmm_open_directory_component(directory, cursor, flags);
            if (next < 0 && errno == ENOENT) {
                if (mkdirat(directory, cursor, (mode_t)mode) != 0 && errno != EEXIST) {
                    ok = false;
                } else {
                    next = pmm_open_directory_component(directory, cursor, flags);
                }
            }
            if (ok && next < 0) {
                ok = false;
...
int pmm_exec_no_shell(const char *const *argv) {
    ...
    return PMM_NOT_FOUND; /* killed by signal */
}
...
int pmm_canonical_path(const char *path, char *out, size_t out_sz) {
    ...
    char *utf8 = pmm_wide_to_utf8(wfull);
    free(wfull);
    if (!utf8) {
        return 0;
    }
...
#ifdef _WIN32
    wchar_t *wsrc = pmm_path_to_wide(src);
    wchar_t *wdst = pmm_path_to_wide(dst);
    int ret = PMM_NOT_FOUND;
    if (wsrc && wdst) {
