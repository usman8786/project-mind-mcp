...
static uint64_t pmm_platform_scale_fraction(uint64_t remainder, uint64_t multiplier,
                                            uint64_t divisor) {
    uint64_t quotient = 0;
    uint64_t reduced = 0;
    uint64_t mask = 1;
...
uint64_t pmm_platform_scale_counter_ns(uint64_t counter, uint64_t frequency) {
    ...
    if (whole_seconds > UINT64_MAX / PMM_NSEC_PER_SEC) {
        return UINT64_MAX;
    }
    uint64_t whole_ns = whole_seconds * PMM_NSEC_PER_SEC;
    uint64_t fraction_ns = pmm_platform_scale_fraction(remainder, PMM_NSEC_PER_SEC, frequency);
...
static void pmm_canonicalize_drive(char *path) {
...
void *pmm_mmap_read(const char *path, size_t *out_size) {
    if (!path || !out_size) {
        return NULL;
    }
    *out_size = 0;
...
void pmm_munmap(void *addr, size_t size) {
    (void)size;
    if (addr) {
...
uint64_t pmm_now_ns(void) {
    LARGE_INTEGER freq, count;
    QueryPerformanceFrequency(&freq);
    QueryPerformanceCounter(&count);
    return pmm_platform_scale_counter_ns((uint64_t)count.QuadPart, (uint64_t)freq.QuadPart);
}

#define PMM_USEC_PER_SEC 1000000ULL

uint64_t pmm_now_ms(void) {
    return pmm_now_ns() / PMM_USEC_PER_SEC;
}
...
bool pmm_file_exists(const char *path) {
    wchar_t *wpath = pmm_path_to_wide(path);
    if (!wpath) {
        return false;
    }
...
bool pmm_is_dir(const char *path) {
    wchar_t *wpath = pmm_path_to_wide(path);
    if (!wpath) {
        return false;
    }
...
int64_t pmm_file_size(const char *path) {
    wchar_t *wpath = pmm_path_to_wide(path);
    if (!wpath) {
...
char *pmm_normalize_path_sep(char *path) {
    if (path) {
        for (char *p = path; *p; p++) {
            if (*p == '\\') {
                *p = '/';
...
static void pmm_timebase_initialize(void) {
...
uint64_t pmm_now_ns(void) {
    (void)pthread_once(&timebase_once, pmm_timebase_initialize);
...
#ifdef _WIN32
#include <stdlib.h>
#define PMM_ENVIRON _environ
#elif defined(__APPLE__)
#include <crt_externs.h>
#define PMM_ENVIRON (*_NSGetEnviron())
#else
extern char **environ;
#define PMM_ENVIRON environ
#endif
...
const char *pmm_safe_getenv(const char *name, char *buf, size_t buf_sz, const char *fallback) {
     ...
     * PMM_CACHE_DIR) arrives here either mojibake'd or with unrepresentable
     ...
     * UTF-8-path convention the rest of the codebase (pmm_fopen, _wmkdir,
     * SQLite VFS) already assumes. */
    {
        wchar_t wname[PMM_SZ_256];
        int wn = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, wname, PMM_SZ_256);
        if (wn > 0) {
            ...
            char *utf8 = pmm_wide_to_utf8(wval);
            free(wval);
            if (!utf8) {
                return NULL;
            }
...
const char *pmm_get_home_dir(void) {
    static PMM_TLS char buf[PMM_SZ_1K];
    char tmp[PMM_SZ_256] = "";

    pmm_safe_getenv("HOME", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
    ...
    pmm_safe_getenv("USERPROFILE", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        pmm_normalize_path_sep(buf);
        return buf;
...
const char *pmm_app_config_dir(void) {
    static PMM_TLS char buf[PMM_SZ_1K];
    char tmp[PMM_SZ_256] = "";
#ifdef _WIN32
    pmm_safe_getenv("APPDATA", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        pmm_normalize_path_sep(buf);
        return buf;
    ...
    pmm_safe_getenv("XDG_CONFIG_HOME", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        return buf;
    }
...
const char *pmm_app_local_dir(void) {
#ifdef _WIN32
    static PMM_TLS char buf[PMM_SZ_1K];
    char tmp[PMM_SZ_256] = "";
    pmm_safe_getenv("LOCALAPPDATA", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        pmm_normalize_path_sep(buf);
        return buf;
    ...
    return NULL;
#else
    return pmm_app_config_dir();
#endif
}

/* ── Cache directory ────────────────────────── */

const char *pmm_resolve_cache_dir(void) {
    static PMM_TLS char buf[PMM_SZ_4K];
    static const char missing[] = "\x1f"
                                  "PMM_CACHE_DIR_MISSING"
                                  "\x1f";
    const char *configured = pmm_safe_getenv("PMM_CACHE_DIR", buf, sizeof(buf), missing);
    if (!configured) {
    ...
    int written = snprintf(buf, sizeof(buf), "%s/.cache/project-mind-mcp", home);
