...
enum { DEFAULT_CORES = 1, MIN_WORKERS = 1, PMM_WORKERS_MAX = 256 };
...
static pmm_system_info_t detect_system_macos(void) {
    pmm_system_info_t info;
...
static pmm_system_info_t detect_system_bsd(void) {
    pmm_system_info_t info;
    memset(&info, 0, sizeof(info));
...
int pmm_detect_cgroup_cpus(const char *cgroup_root) {
    ...
    long quota = strtol(buf, NULL, PMM_DECIMAL_BASE);
    if (quota <= 0) {
        return -1;
    }
    ...
    long period = strtol(buf, NULL, PMM_DECIMAL_BASE);
    if (period <= 0) {
        return -1;
    }
...
size_t pmm_detect_cgroup_mem(const char *cgroup_root) {
    char path[PMM_PATH_MAX];
    char buf[PMM_SZ_64];
    ...
    if (read_small_file(path, buf, sizeof(buf)) > 0) {
        ...
        char *end = NULL;
        unsigned long long n = strtoull(buf, &end, PMM_DECIMAL_BASE);
        if (end == buf || n == 0) {
            return 0;
        }
        return (size_t)n;
...
static pmm_system_info_t detect_system_linux(void) {
    pmm_system_info_t info;
    memset(&info, 0, sizeof(info));
    ...
    int cg_cpus = pmm_detect_cgroup_cpus("/sys/fs/cgroup");
    ...
    size_t cg_ram = pmm_detect_cgroup_mem("/sys/fs/cgroup");
...
#ifdef _WIN32
static pmm_system_info_t detect_system_windows(void) {
    pmm_system_info_t info;
    memset(&info, 0, sizeof(info));
...
static pmm_system_info_t cached_info;

pmm_system_info_t pmm_system_info(void) {
    if (!info_cached) {
#ifdef _WIN32
...
int pmm_default_worker_count(bool initial) {
    /* PMM_WORKERS env override (clamped to [1, PMM_WORKERS_MAX]).
     ...
     * Same precedence shape as other PMM_* env overrides:
     * explicit override > implicit detection. */
    char buf[PMM_SZ_32];
    if (pmm_safe_getenv("PMM_WORKERS", buf, sizeof(buf), NULL) != NULL) {
        long n = strtol(buf, NULL, PMM_DECIMAL_BASE);
        if (n >= MIN_WORKERS && n <= PMM_WORKERS_MAX) {
            return (int)n;
        }
        pmm_log_warn("workers.env.invalid", "value", buf, "fallback", "sysconf");
    }
