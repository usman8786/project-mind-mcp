...
#define MB_DIVISOR ((size_t)(PMM_SZ_1K * PMM_SZ_1K))
...
static size_t os_rss(void) {
    ...
    return rss_pages * (ps > 0 ? (size_t)ps : PMM_SZ_4K);
#endif
}
...
static void check_pressure(size_t rss) {
    ...
    if (over && !was) {
        ...
        char rss_mb[PMM_SZ_32];
        char budget_mb[PMM_SZ_32];
        char pct_str[PMM_SZ_16];
        ...
        snprintf(pct_str, sizeof(pct_str), "%zu",
                 g_budget > 0 ? (rss * PMM_PERCENT) / g_budget : 0);
        pmm_log_warn("mem.pressure.warn", "rss_mb", rss_mb, "budget_mb", budget_mb, "pct", pct_str);
    } else if (!over && was) {
        ...
        snprintf(pct_str, sizeof(pct_str), "%zu",
                 g_budget > 0 ? (rss * PMM_PERCENT) / g_budget : 0);
        pmm_log_info("mem.pressure.ok", "rss_mb", rss_mb, "budget_mb", budget_mb, "pct", pct_str);
    }
}
...
static void mem_setup_fatal(const char *what, const char *detail) {
    pmm_log_error("mem.allocator.setup_failed", "check", what, "detail", detail);
...
double pmm_mem_ram_fraction_for_total(size_t total_ram_bytes) {
...
pmm_mem_budget_t pmm_mem_resolve_budget_capped(size_t total_ram, double ram_fraction,
                                               const char *budget_mb, size_t hard_cap_bytes) {
    pmm_mem_budget_t result = pmm_mem_resolve_budget(total_ram, ram_fraction, budget_mb);
...
void pmm_mem_init_with_cap(double ram_fraction, size_t hard_cap_bytes) {
    int expected = 0;
    ...
    if (_mi_preloading()) {
        ...
        pmm_log_warn("mem.allocator.preloading_completed", "still_preloading", "false", "detail",
                     "allocator was still preloading, so arena creation and purging were "
                     "disabled; process init completed explicitly");
    } else {
        pmm_log_info("mem.allocator.preloading", "state", "already_complete");
    }
#endif
    ...
    pmm_mem_ownership_audit_t audit;
    pmm_mem_audit_ownership(&audit);
    if (!audit.all_owned) {
        ...
        char classes[PMM_SZ_256];
        int length = 0;
        for (int i = 0;
             i < PMM_MEM_OWNERSHIP_CLASSES && length >= 0 && (size_t)length < sizeof(classes);
             i++) {
        ...
        char owned_str[PMM_SZ_32];
        snprintf(owned_str, sizeof(owned_str), "%d/%d", audit.owned_count, audit.probed_count);
        pmm_log_warn("mem.allocator.not_owned", "owned_classes", owned_str, "unowned_bytes",
        ...
        pmm_log_info("mem.allocator.owned", "classes", "all");
    }

    /* PMM_MEM_BUDGET_MB env override (memory analogue of PMM_WORKERS).
     ...
     * testable pmm_mem_resolve_budget(); this path only reads the env and
     * emits the log/warn lines. */
    pmm_system_info_t info = pmm_system_info();

    char env_buf[PMM_SZ_32];
    const char *env = pmm_safe_getenv("PMM_MEM_BUDGET_MB", env_buf, sizeof(env_buf), NULL);
    pmm_mem_budget_t resolved =
        pmm_mem_resolve_budget_capped(info.total_ram, ram_fraction, env, hard_cap_bytes);
    ...
    if (resolved.invalid) {
        pmm_log_warn("mem.budget.env.invalid", "value", env, "fallback", "ram_fraction");
    } else if (resolved.clamped) {
        char cap_mb[PMM_SZ_32];
        snprintf(cap_mb, sizeof(cap_mb), "%zu", info.total_ram / MB_DIVISOR);
        pmm_log_warn("mem.budget.clamped", "requested_mb", env, "cap_mb", cap_mb);
    }
    if (resolved.hard_capped) {
        char cap_bytes[PMM_SZ_32];
        snprintf(cap_bytes, sizeof(cap_bytes), "%zu", hard_cap_bytes);
        pmm_log_info("mem.budget.worker_cap", "cap_bytes", cap_bytes);
    }

    char budget_mb[PMM_SZ_32];
    char ram_mb[PMM_SZ_32];
    snprintf(budget_mb, sizeof(budget_mb), "%zu", g_budget / MB_DIVISOR);
    snprintf(ram_mb, sizeof(ram_mb), "%zu", info.total_ram / MB_DIVISOR);
    pmm_log_info("mem.init", "budget_mb", budget_mb, "total_ram_mb", ram_mb, "source",
                 resolved.source);
}

void pmm_mem_init(double ram_fraction) {
    pmm_mem_init_with_cap(ram_fraction, 0);
}

size_t pmm_mem_rss(void) {
     ...
     * (pmm_mem_init) to reduce upfront memory. So on Linux "current_rss" is a
     ...
     * silently blinding pmm_mem_over_budget()'s backpressure to real memory
...
size_t pmm_mem_peak_rss(void) {
    size_t peak_rss = 0;
    mi_process_info(NULL, NULL, NULL, NULL, &peak_rss, NULL, NULL, NULL);
    /* Peak RSS is by definition >= current RSS. On Linux pmm_mem_rss() reads the
...
size_t pmm_mem_budget(void) {
    return g_budget;
}

void pmm_mem_set_budget_for_tests(size_t bytes) {
    g_budget = bytes;
}

bool pmm_mem_over_budget(void) {
    size_t rss = pmm_mem_rss();
    check_pressure(rss);
    return rss > g_budget;
}

size_t pmm_mem_worker_budget(int num_workers) {
    if (num_workers <= 0) {
...
void pmm_mem_collect(void) {
    mi_collect(true);
}

/* ── Memory map (see mem.h for how to read the triple) ─────────────── */

static const size_t MEM_MAP_BUCKET_LIMITS[PMM_MEM_MAP_BUCKETS] = {
    64, 256, 1024, 4096, 16384, 65536, 1048576, 0, /* 0 = open-ended tail */
};

size_t pmm_mem_map_bucket_limit(int bucket) {
    if (bucket < 0 || bucket >= PMM_MEM_MAP_BUCKETS) {
        return 0;
    }
...
static int mem_map_bucket_of(size_t block_size) {
    for (int i = 0; i < PMM_MEM_MAP_BUCKETS - 1; i++) {
        if (block_size <= MEM_MAP_BUCKET_LIMITS[i]) {
            return i;
        }
    }
    return PMM_MEM_MAP_BUCKETS - 1;
}
...
static bool mem_map_collect_impl(pmm_mem_map_t *out, bool walk_allocator);

bool pmm_mem_map_collect_os(pmm_mem_map_t *out) {
    return mem_map_collect_impl(out, false);
}

bool pmm_mem_map_collect(pmm_mem_map_t *out) {
...
static bool mem_map_collect_impl(pmm_mem_map_t *out, bool walk_allocator) {
    ...
    out->os_rss_bytes = current_rss ? current_rss : pmm_mem_rss();
...
void pmm_mem_audit_ownership(pmm_mem_ownership_audit_t *out) {
    ...
    static const size_t PROBES[PMM_MEM_OWNERSHIP_CLASSES] = {
        64, 4096, 65536, 262144, 1048576, 8388608,
    };
    for (int i = 0; i < PMM_MEM_OWNERSHIP_CLASSES; i++) {
...
static pmm_mutex_t g_mem_phase_mutex;
...
static bool mem_phase_enabled(void) {
    ...
    char buf[PMM_SZ_16];
    bool on = pmm_safe_getenv("PMM_MEM_PHASES", buf, sizeof(buf), NULL) != NULL && buf[0] == '1';
    if (on) {
        pmm_mutex_init(&g_mem_phase_mutex);
    }
    atomic_store_explicit(&g_mem_phase_enabled, on ? 1 : 0, memory_order_release);
    return on;
}

void pmm_mem_phase_mark(const char *label) {
    if (!mem_phase_enabled()) {
        return;
    }
    size_t now = mem_phase_committed();
    pmm_mutex_lock(&g_mem_phase_mutex);
    if (g_mem_phase_open) {
        int slot = -1;
    ...
    pmm_mutex_unlock(&g_mem_phase_mutex);
}

void pmm_mem_phase_reset(void) {
    if (!mem_phase_enabled()) {
        return;
    }
    pmm_mutex_lock(&g_mem_phase_mutex);
...
int pmm_mem_phase_report_json(char *out, size_t size) {
    if (!out || size == 0) {
        return 0;
    }
    out[0] = '\0';
...
bool pmm_mem_census_enabled(void) {
    int state = atomic_load_explicit(&g_mem_census_enabled, memory_order_acquire);
    if (state >= 0) {
        return state == 1;
    }
    char buf[PMM_SZ_16];
    bool on = pmm_safe_getenv("PMM_MEM_CENSUS", buf, sizeof(buf), NULL) != NULL && buf[0] == '1';
    atomic_store_explicit(&g_mem_census_enabled, on ? 1 : 0, memory_order_release);
    return on;
}

bool pmm_mem_win_census(pmm_mem_win_census_t *out) {
    if (!out) {
        return false;
    }
    memset(out, 0, sizeof(*out));
    ...
    while (VirtualQuery(address, &mbi, sizeof(mbi)) == sizeof(mbi)) {
        ...
        if (mbi.State == MEM_COMMIT) {
            ...
            case MEM_PRIVATE:
                ...
                if (mbi.RegionSize < (size_t)PMM_SZ_64K) {
                    out->private_small_bytes += mbi.RegionSize;
                    out->private_small_count++;
                } else if (mbi.RegionSize < (size_t)PMM_SZ_1K * PMM_SZ_1K) {
                    ...
                    if (out->large_tracked < PMM_MEM_TRACKED_REGIONS) {
                ...
                if (mbi.RegionSize >= (size_t)PMM_SZ_1K * PMM_SZ_1K) {
...
void pmm_mem_census_log(const char *tag) {
    if (!pmm_mem_census_enabled()) {
        return;
    }
    pmm_mem_win_census_t census;
    ...
    (void)pmm_mem_win_census(&census);
    ...
    char priv_kb[PMM_SZ_32];
    char crt_kb[PMM_SZ_32];
    char busy_kb[PMM_SZ_32];
    char mapped_kb[PMM_SZ_32];
    char rss_kb[PMM_SZ_32];
    char biggest_kb[PMM_SZ_32];
    char regions[PMM_SZ_32];
    char base[PMM_SZ_32];
    char big_regions[PMM_SZ_32];
    char psmall[PMM_SZ_32];
    char pmed[PMM_SZ_32];
    char plarge[PMM_SZ_32];
    char pcounts[PMM_SZ_64];
    char large_map[PMM_SZ_256];
    char heaps[PMM_SZ_32];
    (void)snprintf(priv_kb, sizeof(priv_kb), "%zu", census.committed_private / PMM_SZ_1K);
    (void)snprintf(crt_kb, sizeof(crt_kb), "%zu", census.crt_heap_committed / PMM_SZ_1K);
    (void)snprintf(busy_kb, sizeof(busy_kb), "%zu", census.crt_heap_busy / PMM_SZ_1K);
    (void)snprintf(mapped_kb, sizeof(mapped_kb), "%zu", census.committed_mapped / PMM_SZ_1K);
    (void)snprintf(rss_kb, sizeof(rss_kb), "%zu", pmm_mem_rss() / PMM_SZ_1K);
    (void)snprintf(biggest_kb, sizeof(biggest_kb), "%zu",
                   census.largest_private_region / PMM_SZ_1K);
    ...
    (void)snprintf(psmall, sizeof(psmall), "%zu", census.private_small_bytes / PMM_SZ_1K);
    (void)snprintf(pmed, sizeof(pmed), "%zu", census.private_medium_bytes / PMM_SZ_1K);
    (void)snprintf(plarge, sizeof(plarge), "%zu", census.private_large_bytes / PMM_SZ_1K);
    ...
    for (unsigned r = 0; r < census.large_tracked; r++) {
        ...
        (void)snprintf(one, sizeof(one), "%s%p:%zuM", r == 0 ? "" : ",", census.large_base[r],
                       census.large_size[r] / ((size_t)PMM_SZ_1K * PMM_SZ_1K));
    ...
    pmm_mem_map_t map;
    (void)pmm_mem_map_collect(&map);
    char map_live_kb[PMM_SZ_32];
    char map_area_kb[PMM_SZ_32];
    (void)snprintf(map_live_kb, sizeof(map_live_kb), "%zu", map.live_bytes / PMM_SZ_1K);
    (void)snprintf(map_area_kb, sizeof(map_area_kb), "%zu", map.area_committed_bytes / PMM_SZ_1K);
    pmm_log_info("mem.census", "at", tag ? tag : "?", "mi_area_kb", map_area_kb, "mi_live_kb",
    ...
    if (tag && strcmp(tag, "connection.exit") == 0) {
        ...
        char stats_path[PMM_SZ_1K];
        if (pmm_safe_getenv("PMM_MEM_STATS_OUT", stats_path, sizeof(stats_path), NULL) != NULL) {
            FILE *stats = pmm_fopen(stats_path, "wb");
            if (stats) {
                mi_stats_print_out(mem_stats_writer, stats);
                (void)fclose(stats);
            }
        }
        char phases[PMM_SZ_1K];
        if (pmm_mem_phase_report_json(phases, sizeof(phases)) > 0) {
            pmm_log_info("mem.phases", "table", phases);
        }
    }
}
