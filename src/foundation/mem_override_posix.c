...
void *__wrap_malloc(size_t size) {
    ...
    pmm_mem_profile_alloc(block, size);
    return block;
}

void *__wrap_calloc(size_t count, size_t size) {
    void *block = mi_calloc(count, size);
    pmm_mem_profile_alloc(block, count * size);
    return block;
}

void *__wrap_realloc(void *block, size_t size) {
    if (block) {
        pmm_mem_profile_free(block);
    }
    void *grown = mi_realloc(block, size);
    pmm_mem_profile_alloc(grown, size);
    return grown;
}
...
char *__wrap_strdup(const char *text) {
    ...
    if (copy) {
        pmm_mem_profile_alloc(copy, mi_usable_size(copy));
    }
    return copy;
}
