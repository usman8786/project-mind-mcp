...
char *pmm_mkdtemp(char *tmpl) {
     ...
     * Callers must provide buffers >= PMM_SZ_256 bytes (all test code does). */
    static char buf[PMM_SZ_512];
    ...
    wchar_t *wide_template = pmm_utf8_to_wide(buf);
    if (!wide_template || !_wmktemp(wide_template)) {
        free(wide_template);
        return NULL;
    }
    char *expanded = pmm_wide_to_utf8(wide_template);
    free(wide_template);
    ...
    if (!win_mkdtemp_private_create(buf)) {
        ...
        wchar_t *wide_directory = pmm_utf8_to_wide(buf);
    ...
    /* Copy result back — callers now use char[PMM_SZ_256]+ buffers */
    strcpy(tmpl, buf);
    return tmpl;
}
#endif
...
#ifdef _WIN32
    wchar_t *wide = pmm_path_to_wide(path);
    char *narrow = wide ? pmm_wide_to_utf8(wide) : NULL;
    free(wide);
    if (!narrow) {
        return false;
    }
...
int pmm_mkstemp(char *tmpl) {
    /* Rewrite /tmp/ to %TEMP%\ like pmm_mkdtemp */
     ...
     * PMM_CACHE_DIR, which users may place at non-ASCII paths; the ANSI CRT
    ...
    char *expanded_for_open = pmm_wide_to_utf8(wide_template);
    wchar_t *wide_open = expanded_for_open ? pmm_path_to_wide(expanded_for_open) : NULL;
