...
struct pmm_private_lock_directory {
    int fd;
    char *path;
    dev_t device;
    ino_t inode;
...
struct pmm_private_file_lock {
    int fd;
    pid_t owner_pid;
    pmm_private_file_lock_mode_t mode;
    struct pmm_private_file_lock *next_tracked;
    bool unlocked;
...
struct pmm_private_fork_condition {
...
static pmm_private_file_lock_t *private_tracked_locks;
...
static void private_atfork_child(void) {
    pmm_private_file_lock_t *lock = private_tracked_locks;
    while (lock) {
        if (lock->fd >= 0) {
...
bool pmm_private_file_lock_fork_guard_enter(void) {
    return private_atfork_ensure() && pthread_mutex_lock(&private_fork_mutex) == 0;
}

void pmm_private_file_lock_fork_guard_leave(void) {
    (void)pthread_mutex_unlock(&private_fork_mutex);
}

pmm_private_fork_condition_t *pmm_private_fork_condition_new(void) {
    pmm_private_fork_condition_t *condition = calloc(1, sizeof(*condition));
...
void pmm_private_fork_condition_free(pmm_private_fork_condition_t *condition) {
    if (!condition) {
        return;
    }
...
void pmm_private_fork_condition_broadcast_while_guarded(pmm_private_fork_condition_t *condition) {
    if (condition) {
        (void)pthread_cond_broadcast(&condition->value);
    }
}

pmm_private_fork_wait_status_t pmm_private_fork_condition_wait_until_while_guarded(
    pmm_private_fork_condition_t *condition, uint64_t deadline_ms) {
    if (!condition) {
        return PMM_PRIVATE_FORK_WAIT_ERROR;
    }
    int status;
    ...
    if (status == 0) {
        return PMM_PRIVATE_FORK_WAIT_SIGNALED;
    }
    return status == ETIMEDOUT ? PMM_PRIVATE_FORK_WAIT_TIMEOUT : PMM_PRIVATE_FORK_WAIT_ERROR;
}
...
static bool private_directory_revalidate(const pmm_private_lock_directory_t *directory) {
    ...
    return directory && directory->owner_pid == getpid() && directory->fd >= 0 &&
           ...
           pmm_macos_extended_acl_fd_is_empty(directory->fd);
}
...
static bool private_file_revalidate(const pmm_private_lock_directory_t *directory,
    ...
    return private_directory_revalidate(directory) && fstat(fd, &by_handle) == 0 &&
           ...
           by_path.st_ino == by_handle.st_ino && pmm_macos_extended_acl_fd_is_empty(fd) &&
           (!expected ||
            (expected->st_dev == by_handle.st_dev && expected->st_ino == by_handle.st_ino));
}

static pmm_private_file_lock_status_t private_open_failure_status(
    const pmm_private_lock_directory_t *directory, const char *base_name) {
    struct stat status;
    if (fstatat(directory->fd, base_name, &status, AT_SYMLINK_NOFOLLOW) == 0) {
        return PMM_PRIVATE_FILE_LOCK_UNSAFE;
    }
    return errno == ENOENT ? PMM_PRIVATE_FILE_LOCK_IO : PMM_PRIVATE_FILE_LOCK_UNSAFE;
}
...
static int private_release_unlock(pmm_private_file_lock_t *lock) {
...
static int private_release_close(pmm_private_file_lock_t *lock, bool *attempted_out) {
    *attempted_out = false;
...
pmm_private_file_lock_status_t pmm_private_lock_directory_adopt_posix(
    int directory_fd, const char *stable_path, pmm_private_lock_directory_t **directory_out) {
    if (directory_out) {
        *directory_out = NULL;
    }
    ...
    if (fstat(directory_fd, &status) != 0 || lstat(stable_path, &by_path) != 0 ||
        ...
        status.st_ino != by_path.st_ino || !pmm_macos_extended_acl_fd_is_empty(directory_fd)) {
        return PMM_PRIVATE_FILE_LOCK_UNSAFE;
    }
    pmm_private_lock_directory_t *directory = calloc(1, sizeof(*directory));
    if (directory) {
...
pmm_private_file_lock_status_t pmm_private_file_lock_try_acquire(
    ...
    if (!directory || !lock_out || !private_base_name_valid(base_name) ||
        (mode != PMM_PRIVATE_FILE_LOCK_SH && mode != PMM_PRIVATE_FILE_LOCK_EX)) {
        return PMM_PRIVATE_FILE_LOCK_UNSAFE;
    }
    ...
    if (fd < 0) {
        pmm_private_file_lock_status_t status = private_open_failure_status(directory, base_name);
    ...
    pmm_private_file_lock_t *lock = calloc(1, sizeof(*lock));
    if (!lock) {
        (void)close(fd);
        pmm_private_file_lock_fork_guard_leave();
        return PMM_PRIVATE_FILE_LOCK_IO;
    ...
    int operation = mode == PMM_PRIVATE_FILE_LOCK_SH ? LOCK_SH : LOCK_EX;
    if (private_flock_set(fd, operation | LOCK_NB) != 0) {
        int lock_error = errno;
        (void)close(fd);
        free(lock);
        pmm_private_file_lock_fork_guard_leave();
        return lock_error == EWOULDBLOCK || lock_error == EAGAIN ? PMM_PRIVATE_FILE_LOCK_BUSY
    ...
    if (forced_cleanup || !private_file_revalidate(directory, base_name, fd, &initial)) {
        pmm_private_file_lock_status_t failure_status =
            forced_cleanup ? PMM_PRIVATE_FILE_LOCK_IO : PMM_PRIVATE_FILE_LOCK_UNSAFE;
        lock->test_fail_unlock_once = fail_cleanup_unlock;
        lock->test_fail_close_once = fail_cleanup_close;
        *lock_out = lock;
        pmm_private_file_lock_fork_guard_leave();
        pmm_private_file_lock_status_t cleanup_status = pmm_private_file_lock_release(lock_out);
        return cleanup_status == PMM_PRIVATE_FILE_LOCK_OK ? failure_status
                                                          : PMM_PRIVATE_FILE_LOCK_IO;
    }
    *lock_out = lock;
    pmm_private_file_lock_fork_guard_leave();
...
static bool private_lock_is_tracked(const pmm_private_file_lock_t *lock) {
    for (const pmm_private_file_lock_t *cursor = private_tracked_locks; cursor;
...
static bool private_payload_fd_valid(const pmm_private_file_lock_t *lock, struct stat *status_out) {
    struct stat status;
    bool valid = lock && lock->fd >= 0 && !lock->unlocked && lock->owner_pid == getpid() &&
                 ...
                 pmm_macos_extended_acl_fd_is_empty(lock->fd);
    if (valid && status_out) {
        *status_out = status;
    }
    return valid;
...
pmm_private_file_lock_status_t pmm_private_file_lock_payload_read(pmm_private_file_lock_t *lock,
...
pmm_private_file_lock_status_t pmm_private_file_lock_payload_write(pmm_private_file_lock_t *lock,
                                                                   const void *buffer,
                                                                   size_t length) {
    if (!lock || !buffer || length == 0 || length > PRIVATE_FILE_LOCK_PAYLOAD_CAP ||
        lock->mode != PMM_PRIVATE_FILE_LOCK_EX) {
        return PMM_PRIVATE_FILE_LOCK_UNSAFE;
    }
    if (!pmm_private_file_lock_fork_guard_enter()) {
        return PMM_PRIVATE_FILE_LOCK_IO;
    ...
    return valid ? PMM_PRIVATE_FILE_LOCK_OK : PMM_PRIVATE_FILE_LOCK_IO;
}
...
pmm_private_file_lock_status_t pmm_private_file_lock_release(pmm_private_file_lock_t **lock_io) {
    ...
    pmm_private_file_lock_t *lock = *lock_io;
    ...
    pmm_private_file_lock_t **cursor = &private_tracked_locks;
    ...
    *lock_io = NULL;
    free(lock);
    return close_status == 0 ? PMM_PRIVATE_FILE_LOCK_OK : PMM_PRIVATE_FILE_LOCK_IO;
}
...
bool pmm_private_file_lock_is_cloexec_for_test(const pmm_private_file_lock_t *lock) {
    if (!lock || lock->fd < 0) {
        return false;
    }
...
bool pmm_private_file_lock_unlock_complete(const pmm_private_file_lock_t *lock) {
    return lock && lock->unlocked;
}

bool pmm_private_lock_directory_fail_post_acquire_cleanup_for_test(
    pmm_private_lock_directory_t *directory, bool fail_unlock, bool fail_close) {
...
bool pmm_private_file_lock_fail_next_release_step_for_test(
    pmm_private_file_lock_t *lock, pmm_private_file_lock_release_step_t step) {
    if (!lock) {
        return false;
    }
    if (step == PMM_PRIVATE_FILE_LOCK_RELEASE_UNLOCK) {
        lock->test_fail_unlock_once = true;
        return true;
    }
    if (step == PMM_PRIVATE_FILE_LOCK_RELEASE_CLOSE) {
...
unsigned int pmm_private_file_lock_release_step_attempts_for_test(
    const pmm_private_file_lock_t *lock, pmm_private_file_lock_release_step_t step) {
    if (!lock) {
        return 0;
    }
    if (step == PMM_PRIVATE_FILE_LOCK_RELEASE_UNLOCK) {
...
bool pmm_private_file_lock_fail_close_after_consuming_for_test(pmm_private_file_lock_t *lock) {
    if (!lock) {
        return false;
    }
...
int pmm_private_file_lock_native_fd_for_test(const pmm_private_file_lock_t *lock) {
...
static pmm_private_file_lock_status_t private_win_path_from_utf8(const char *path,
                                                                 wchar_t **wide_out) {
    *wide_out = NULL;
...
static bool private_win_directory_handle_valid(pmm_private_lock_directory_t *directory,
...
static bool private_win_directory_path_matches(pmm_private_lock_directory_t *directory) {
    HANDLE probe =
...
static bool private_win_directory_revalidate(pmm_private_lock_directory_t *directory) {
    BY_HANDLE_FILE_INFORMATION information;
...
static wchar_t *private_win_file_path(const pmm_private_lock_directory_t *directory,
...
static bool private_win_file_handle_valid(pmm_private_lock_directory_t *directory, HANDLE handle,
                                          BY_HANDLE_FILE_INFORMATION *information_out) {
    BY_HANDLE_FILE_INFORMATION information;
...
static bool private_win_file_revalidate(pmm_private_lock_directory_t *directory,
...
static pmm_private_file_lock_status_t private_win_open_failure_status(const wchar_t *path) {
    DWORD attributes = GetFileAttributesW(path);
    return attributes == INVALID_FILE_ATTRIBUTES ? PMM_PRIVATE_FILE_LOCK_IO
                                                 : PMM_PRIVATE_FILE_LOCK_UNSAFE;
}

pmm_private_file_lock_status_t pmm_private_lock_directory_adopt_windows(
    void *directory_handle, const char *stable_path, pmm_private_lock_directory_t **directory_out) {
    if (directory_out) {
        *directory_out = NULL;
    }
    ...
    pmm_private_file_lock_status_t path_status =
        private_win_path_from_utf8(stable_path, &wide_path);
    if (path_status != PMM_PRIVATE_FILE_LOCK_OK) {
        return path_status;
    }
    private_win_security_t security;
    ...
    pmm_private_lock_directory_t candidate;
...
pmm_private_file_lock_status_t pmm_private_file_lock_try_acquire(
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        pmm_private_file_lock_status_t status = private_win_open_failure_status(path);
        pmm_private_file_lock_fork_guard_leave();
        free(path);
        return status;
    }
    ...
    if (!native_locked) {
        ...
        *lock_out = lock;
        pmm_private_file_lock_fork_guard_leave();
        free(path);
        pmm_private_file_lock_status_t failure_status = lock_error == ERROR_LOCK_VIOLATION
                                                            ? PMM_PRIVATE_FILE_LOCK_BUSY
                                                            : PMM_PRIVATE_FILE_LOCK_IO;
        pmm_private_file_lock_status_t cleanup_status = pmm_private_file_lock_release(lock_out);
        return cleanup_status == PMM_PRIVATE_FILE_LOCK_OK ? failure_status
                                                          : PMM_PRIVATE_FILE_LOCK_IO;
...
static bool private_win_payload_handle_valid(const pmm_private_file_lock_t *lock) {
    BY_HANDLE_FILE_INFORMATION information;
...
static bool private_win_release_unlock(pmm_private_file_lock_t *lock) {
    lock->test_unlock_attempts++;
    if (lock->test_fail_unlock_once) {
        lock->test_fail_unlock_once = false;
...
static bool private_win_release_close(pmm_private_file_lock_t *lock) {
    lock->test_close_attempts++;
    if (lock->test_fail_close_once) {
        lock->test_fail_close_once = false;
        SetLastError(ERROR_GEN_FAILURE);
...
bool pmm_private_lock_directory_fail_lock_attempt_cleanup_for_test(
...
pmm_private_fork_wait_status_t pmm_private_fork_condition_wait_until_while_guarded(
    ...
    return GetLastError() == ERROR_TIMEOUT ? PMM_PRIVATE_FORK_WAIT_TIMEOUT
                                           : PMM_PRIVATE_FORK_WAIT_ERROR;
}
