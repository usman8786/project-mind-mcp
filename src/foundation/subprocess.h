 ...
 */
#ifndef PMM_SUBPROCESS_H
#define PMM_SUBPROCESS_H

#include <stdbool.h>
#include <stddef.h> /* size_t (pmm_build_win_cmdline) */

/* How a supervised child ended. */
typedef enum {
    PMM_PROC_CLEAN = 0,    /* exited with code 0 */
    PMM_PROC_EXIT_NONZERO, /* exited with a nonzero code (a graceful failure) */
    PMM_PROC_CRASH,        /* died from a fault: POSIX SIGSEGV/BUS/ILL/FPE/ABRT/SYS,
                            * or a Windows NTSTATUS exception exit code (>= 0xC0000000) */
    PMM_PROC_HANG,         /* made no progress within the quiet-timeout; we killed it */
    PMM_PROC_KILLED,       /* terminated by a non-fault signal we did not initiate */
    PMM_PROC_SPAWN_FAILED  /* fork/exec/CreateProcess failed — no child ever ran */
} pmm_proc_outcome_t;

typedef struct {
    pmm_proc_outcome_t outcome;
...
typedef void (*pmm_proc_log_cb)(const char *line, void *ud);
...
typedef struct {
    ...
    pmm_proc_log_cb on_log_line;     /* optional per-line callback */
    ...
    int cancel_grace_ms;             /* graceful tree-termination window; <= 0 uses the finite
                                      * PMM_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS */
    bool delete_log_on_exit;         /* unlink log_file after reaping */
} pmm_proc_opts_t;

#define PMM_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS 1000
#define PMM_SUBPROCESS_MAX_CANCEL_GRACE_MS 1000
#define PMM_SUBPROCESS_FORCE_SETTLE_MS 1000
...
typedef struct pmm_subprocess pmm_subprocess_t;

typedef enum {
    PMM_PROC_POLL_ERROR = -1, /* invalid handle/arguments */
    PMM_PROC_POLL_RUNNING = 0,
    PMM_PROC_POLL_TERMINAL = 1
} pmm_proc_poll_t;
 ...
 * but the terminal outcome remains PMM_PROC_HANG. */
 ...
 * (out->outcome == PMM_PROC_SPAWN_FAILED). */
 ...
 * is unit-tested on Linux/macOS CI, and so both spawn sites (pmm_subprocess_run and
 ...
 * command-language payload. Unlike pmm_build_win_cmdline(), payload is not a C
