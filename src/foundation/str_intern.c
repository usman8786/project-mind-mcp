...
CBMInternPool *pmm_intern_create(void) {
    CBMInternPool *p = (CBMInternPool *)calloc(PMM_ALLOC_ONE, sizeof(*p));
    if (!p) {
        return NULL;
    }
    pmm_arena_init(&p->arena);
    p->capacity = PMM_SZ_256;
    p->mask = p->capacity - SKIP_ONE;
    p->buckets = (InternEntry *)calloc(p->capacity, sizeof(InternEntry));
    if (!p->buckets) {
        pmm_arena_destroy(&p->arena);
        free(p);
        return NULL;
    }
    return p;
...
const char *pmm_intern_n(CBMInternPool *pool, const char *s, size_t len) {
    ...
    /* Resize at 70% load */
    if (pool->count * PMM_DECIMAL_BASE >= pool->capacity * INTERN_LOAD_NUM) {
        intern_resize(pool);
    ...
    char *copy = pmm_arena_strndup(&pool->arena, s, len);
    if (!copy) {
        return NULL;
    }
...
const char *pmm_intern(CBMInternPool *pool, const char *s) {
    ...
    return pmm_intern_n(pool, s, strlen(s));
}
