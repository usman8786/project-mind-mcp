...
pmm_query_stats_t g_query_stats = {0};
static atomic_bool g_diag_stop = false;
static atomic_bool g_diag_done = false;
static pmm_thread_t g_diag_thread;
static bool g_diag_started = false;
static bool g_diag_abandoned = false;
static time_t g_start_time = 0;
static pmm_private_lock_directory_t *g_diag_directory = NULL;
#ifndef _WIN32
static int g_diag_directory_fd = -1;
#endif
static char g_diag_directory_path[PMM_PATH_MAX] = "";
static char g_diag_path[PMM_PATH_MAX] = "";
static char g_diag_ndjson_path[PMM_PATH_MAX] = "";
 ...
 * committed bytes cannot tell apart. Off unless PMM_MEM_STATS=1. */
...
#ifdef _WIN32
    /* _wopen mirrors pmm_mkstemp's Windows contract — the ANSI CRT interprets
...
static void diag_write_allocator_stats(void) {
    char flag[PMM_SZ_16];
    if (pmm_safe_getenv("PMM_MEM_STATS", flag, sizeof(flag), NULL) == NULL || flag[0] != '1') {
        return;
    }
    char path[PMM_PATH_MAX];
    ...
    int written =
        snprintf(path, sizeof(path), "%s/pmm-allocator-stats-%d.txt", pmm_tmpdir(), (int)getpid());
...
#endif
    pmm_private_lock_directory_close(g_diag_directory);
    g_diag_directory = NULL;
    if (remove_directory && g_diag_directory_path[0] != '\0') {
        (void)pmm_rmdir(g_diag_directory_path);
    }
}

/* Diagnostics placement honors the documented contract ($TMPDIR, /tmp
 * fallback) without touching pmm_tmpdir(), whose many other callers keep
...
static const char *diag_tmp_base(char *buffer, size_t size) {
    ...
    return pmm_tmpdir();
#else
    pmm_safe_getenv("TMPDIR", buffer, size, NULL);
    if (buffer[0] != '\0') {
        return buffer;
    }
    return "/tmp";
...
static bool diag_directory_prepare(void) {
    char tmp_base[PMM_PATH_MAX];
    char directory_template[PMM_PATH_MAX];
    int written =
        snprintf(directory_template, sizeof(directory_template), "%s/pmm-diagnostics-%d-XXXXXX",
                 diag_tmp_base(tmp_base, sizeof(tmp_base)), (int)getpid());
    if (written <= 0 || (size_t)written >= sizeof(directory_template) ||
        !pmm_mkdtemp(directory_template)) {
        return false;
    }
    written =
    ...
    if (written <= 0 || (size_t)written >= sizeof(g_diag_directory_path)) {
        (void)pmm_rmdir(directory_template);
        return false;
    }

#ifdef _WIN32
    wchar_t *wide_directory = pmm_path_to_wide(g_diag_directory_path);
    HANDLE handle =
        wide_directory
    ...
    free(wide_directory);
    pmm_private_file_lock_status_t status = PMM_PRIVATE_FILE_LOCK_IO;
    if (handle != INVALID_HANDLE_VALUE) {
        status = pmm_private_lock_directory_adopt_windows(handle, g_diag_directory_path,
                                                          &g_diag_directory);
    ...
    pmm_private_file_lock_status_t status = PMM_PRIVATE_FILE_LOCK_IO;
    if (same_directory) {
        status = pmm_private_lock_directory_adopt_posix(validation_fd, g_diag_directory_path,
                                                        &g_diag_directory);
        if (status == PMM_PRIVATE_FILE_LOCK_OK) {
            validation_fd = -1;
    ...
    if (status != PMM_PRIVATE_FILE_LOCK_OK || !diag_set_output_paths()) {
...
static bool diag_write_file(const char *base_name, const char *data, size_t length, bool append) {
    ...
    wchar_t *wide_path = pmm_path_to_wide(path);
...
static bool diag_posix_file_valid(int descriptor) {
    ...
    return descriptor >= 0 && fstat(descriptor, &state) == 0 && S_ISREG(state.st_mode) &&
           ...
           pmm_macos_extended_acl_fd_is_empty(descriptor);
}
...
static void diag_wait_for_interval(void) {
    uint64_t deadline = pmm_now_ms() + DIAG_INTERVAL_MS;
...
static void write_diagnostics(void) {
    ...
    pmm_mem_map_t map;
    (void)pmm_mem_map_collect_os(&map);
    size_t residual =
        map.os_committed_bytes > map.live_bytes ? map.os_committed_bytes - map.live_bytes : 0;
    char buckets[PMM_SZ_512];
    int bucket_length = 0;
    for (int i = 0;
         i < PMM_MEM_MAP_BUCKETS && bucket_length >= 0 && (size_t)bucket_length < sizeof(buckets);
         i++) {
        int written =
            snprintf(buckets + bucket_length, sizeof(buckets) - (size_t)bucket_length,
                     "%s{\"limit\": %zu, \"bytes\": %zu, \"blocks\": %zu}", i == 0 ? "" : ", ",
                     pmm_mem_map_bucket_limit(i), map.bucket_bytes[i], map.bucket_blocks[i]);
        if (written < 0) {
            break;
        }
...
bool pmm_diag_start(void) {
    ...
    pmm_safe_getenv("PMM_DIAGNOSTICS", env_buf, sizeof(env_buf), NULL);
    ...
    if (pmm_thread_create(&g_diag_thread, 0, diag_thread_fn, NULL) != 0) {
        diag_directory_close(true);
        return false;
    }

    g_diag_started = true;
    char interval[PMM_SZ_32];
    (void)snprintf(interval, sizeof(interval), "%d", DIAG_INTERVAL_MS / 1000);
    /* Discovery must survive PMM_LOG_LEVEL suppression and paths containing
     * spaces: a control record is the only announced copy of these paths. */
    pmm_log_control("diagnostics.start", "snapshot", g_diag_path, "trajectory", g_diag_ndjson_path,
                    "interval_s", interval);
    return true;
}
...
void pmm_diag_stop(void) {
    ...
    uint64_t deadline = pmm_now_ms() + DIAG_STOP_TIMEOUT_MS;
    while (!atomic_load_explicit(&g_diag_done, memory_order_acquire) && pmm_now_ms() < deadline) {
        diag_sleep_ms(DIAG_WAIT_SLICE_MS);
    }
    ...
    if (completed) {
        ...
        (void)pmm_thread_detach(&g_diag_thread);
