 ...
 *   Matches tree-sitter SubtreeHeapData (PMM_SZ_64 bytes). Backed by
 ...
 *
 * Usage:
 *   pmm_slab_install();         // once, before any parsing
 *   ... parse files ...
 *   pmm_slab_destroy_thread();  // on thread exit — frees owned memory
 */
#ifndef PMM_SLAB_ALLOC_H
#define PMM_SLAB_ALLOC_H
 ...
 * Only safe after pmm_destroy_thread_parser() has been called. */
void pmm_slab_reset_thread(void);
...
void pmm_slab_destroy_thread(void);
...
void pmm_slab_reclaim(void);

/* Test/diagnostic API: direct access to the slab allocator.
 * Use these to unit test slab (≤64B) and heap (>64B) paths. */
void *pmm_slab_test_malloc(size_t size);
void pmm_slab_test_free(void *ptr);
void *pmm_slab_test_realloc(void *ptr, size_t size);
void *pmm_slab_test_calloc(size_t count, size_t size);

#endif /* PMM_SLAB_ALLOC_H */
