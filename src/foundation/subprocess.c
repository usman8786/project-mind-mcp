...
#include "compat.h" /* pmm_nanosleep */
#include "compat_fs.h"
#include "log.h"
#include "platform.h" /* pmm_now_ms */
...
#ifdef _WIN32
#include <windows.h>
#include "win_utf8.h" /* pmm_utf8_to_wide — spawn the worker with a wide command line so a
...
#define PMM_WIN_CRASH_CODE_MIN 0xC0000000u
#define PMM_WIN_CONTROL_C_EXIT 0xC000013Au

#ifndef _WIN32
static bool pmm_is_fault_signal(int sig) {
    switch (sig) {
    case SIGSEGV:
    case SIGBUS:
    case SIGILL:
...
pmm_proc_outcome_t pmm_proc_classify(bool exited_normally, int exit_code, int term_signal,
                                     bool timed_out) {
    if (timed_out) {
        return PMM_PROC_HANG;
    }
    if (!exited_normally) {
        /* POSIX signal death. */
#ifndef _WIN32
        if (pmm_is_fault_signal(term_signal)) {
            return PMM_PROC_CRASH;
        }
#else
        (void)term_signal;
#endif
        return PMM_PROC_KILLED;
    }
    ...
    if ((unsigned)exit_code == PMM_WIN_CONTROL_C_EXIT) {
        return PMM_PROC_KILLED;
    }
    if ((unsigned)exit_code >= PMM_WIN_CRASH_CODE_MIN) {
        return PMM_PROC_CRASH;
    }
    return (exit_code == 0) ? PMM_PROC_CLEAN : PMM_PROC_EXIT_NONZERO;
}

const char *pmm_proc_outcome_str(pmm_proc_outcome_t o) {
    switch (o) {
    case PMM_PROC_CLEAN:
        return "clean";
    case PMM_PROC_EXIT_NONZERO:
        return "exit_nonzero";
    case PMM_PROC_CRASH:
        return "crash";
    case PMM_PROC_HANG:
        return "hang";
    case PMM_PROC_KILLED:
        return "killed";
    case PMM_PROC_SPAWN_FAILED:
    default:
        return "spawn_failed";
    }
}
...
static pmm_tail_result_t pmm_tail_log(const char *log_file, long *tail_pos, pmm_proc_log_cb cb,
                                      void *ud, bool final) {
    pmm_tail_result_t result = {.status = PMM_TAIL_ERROR, .progressed = false};
    if (!log_file || !tail_pos) {
        return result;
    }
#ifdef _WIN32
    FILE *lf = pmm_fopen(log_file, "r");
#else
    ...
    if (fseek(lf, *tail_pos, SEEK_SET) == 0) {
        ...
        enum {
            PMM_TAIL_MAX_LINES_PER_POLL = 64,
            PMM_TAIL_MAX_BYTES_PER_POLL = 64 * 1024,
        };
        while (delivered_lines < PMM_TAIL_MAX_LINES_PER_POLL &&
               delivered_bytes < PMM_TAIL_MAX_BYTES_PER_POLL) {
            long before = ftell(lf);
            if (!fgets(line, sizeof(line), lf)) {
                result.status = ferror(lf) ? PMM_TAIL_ERROR : PMM_TAIL_CAUGHT_UP;
                break;
            }
            ...
            if (complete) {
                ...
                *tail_pos = ftell(lf);
                ...
                result.status = PMM_TAIL_CAUGHT_UP;
                break;
            }
        }
        if (result.status == PMM_TAIL_ERROR && !ferror(lf)) {
...
static size_t pmm_cmdline_put(char *buf, size_t cap, size_t pos, char c, bool *ovf) {
    if (pos + 1 >= cap) {
        *ovf = true;
        return pos;
    }
...
static size_t pmm_cmdline_append_arg(char *buf, size_t cap, size_t pos, const char *arg, bool first,
                                     bool *ovf) {
    if (!first) {
        pos = pmm_cmdline_put(buf, cap, pos, ' ', ovf);
    }
    pos = pmm_cmdline_put(buf, cap, pos, '"', ovf);
    for (const char *p = arg; *p;) {
        ...
        if (*p == '"') {
            ...
            pos = pmm_cmdline_put(buf, cap, pos, *p, ovf);
            p++;
        }
    }
    pos = pmm_cmdline_put(buf, cap, pos, '"', ovf);
 ...
 * only runs on Windows. Shared by pmm_subprocess_spawn_win AND the UI http_server
...
bool pmm_build_win_cmdline(char *buf, size_t cap, const char *const *argv) {
    if (!buf || cap == 0 || !argv) {
        return false;
    }
    size_t pos = 0;
    ...
    for (int i = 0; argv[i]; i++) {
        pos = pmm_cmdline_append_arg(buf, cap, pos, argv[i], i == 0, &ovf);
        if (ovf) {
...
static bool pmm_ascii_case_equal(const char *left, const char *right) {
    if (!left || !right) {
        return false;
    }
    while (*left && *right) {
...
static bool pmm_win_cmd_path_is_absolute(const char *path) {
    if (!path) {
        return false;
    }
    ...
    return pmm_ascii_case_equal(basename, "cmd.exe");
}

bool pmm_build_win_cmd_payload(char *buf, size_t cap, const char *cmd_executable,
                               const char *payload) {
    if (!buf || cap == 0) {
        return false;
    }
    buf[0] = '\0';
    if (!pmm_win_cmd_path_is_absolute(cmd_executable) || !payload || !payload[0]) {
        return false;
    }
...
enum { PMM_SUBPROCESS_ARGV_LIMIT = 4096 };

typedef enum {
    PMM_SUBPROCESS_ACTIVE = 0,
    PMM_SUBPROCESS_CANCEL_REQUESTED,
    PMM_SUBPROCESS_DRAINING,
    PMM_SUBPROCESS_TERMINAL,
} pmm_subprocess_lifecycle_t;

struct pmm_subprocess {
    char *bin;
    char **argv;
    size_t argc;
    char *windows_cmd_payload;
    char *log_file;
    pmm_proc_log_cb on_log_line;
    void *log_ud;
...
static void pmm_subprocess_result_init(pmm_proc_result_t *result) {
    result->outcome = PMM_PROC_SPAWN_FAILED;
    result->exit_code = -1;
...
static void pmm_subprocess_free_config(pmm_subprocess_t *process) {
    if (!process) {
        return;
    }
    if (process->argv) {
...
static pmm_subprocess_t *pmm_subprocess_copy_opts(const pmm_proc_opts_t *opts) {
    if (!opts || !opts->bin || !opts->bin[0]) {
        return NULL;
    }
#ifdef _WIN32
    if (opts->windows_cmd_payload &&
        (!opts->windows_cmd_payload[0] || opts->argv || !pmm_win_cmd_path_is_absolute(opts->bin))) {
        return NULL;
    }
#else
    if (opts->windows_cmd_payload) {
    ...
    if (opts->argv) {
        argc = 0;
        while (argc < PMM_SUBPROCESS_ARGV_LIMIT && opts->argv[argc]) {
            argc++;
        }
        if (argc == 0 || argc == PMM_SUBPROCESS_ARGV_LIMIT) {
            return NULL;
        }
    }

    pmm_subprocess_t *process = (pmm_subprocess_t *)calloc(1, sizeof(*process));
    if (!process) {
        return NULL;
    }
    process->bin = pmm_strdup(opts->bin);
    process->windows_cmd_payload =
        opts->windows_cmd_payload ? pmm_strdup(opts->windows_cmd_payload) : NULL;
    ...
    if (!process->bin || !process->argv ||
        ...
        pmm_subprocess_free_config(process);
        return NULL;
    }
    for (size_t i = 0; i < argc; i++) {
        const char *arg = opts->argv ? opts->argv[i] : opts->bin;
        process->argv[i] = pmm_strdup(arg);
        if (!process->argv[i]) {
            pmm_subprocess_free_config(process);
            return NULL;
        }
    ...
    if (opts->log_file) {
        process->log_file = pmm_strdup(opts->log_file);
        if (!process->log_file) {
            pmm_subprocess_free_config(process);
            return NULL;
        }
    ...
    process->cancel_grace_ms =
        opts->cancel_grace_ms > 0 ? opts->cancel_grace_ms : PMM_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS;
    if (process->cancel_grace_ms > PMM_SUBPROCESS_MAX_CANCEL_GRACE_MS) {
        process->cancel_grace_ms = PMM_SUBPROCESS_MAX_CANCEL_GRACE_MS;
    }
    process->delete_log_on_exit = opts->delete_log_on_exit;
    atomic_init(&process->lifecycle, PMM_SUBPROCESS_ACTIVE);
    pmm_subprocess_result_init(&process->result);
    return process;
}

static pmm_tail_result_t pmm_subprocess_poll_log(pmm_subprocess_t *process, bool final) {
    pmm_tail_result_t result = pmm_tail_log(process->log_file, &process->tail_pos,
                                            process->on_log_line, process->log_ud, final);
    if (result.progressed) {
        process->last_activity_ms = pmm_now_ms();
    }
    return result;
}

static bool pmm_subprocess_cancellation_requested(const pmm_subprocess_t *process) {
    return atomic_load_explicit(&process->lifecycle, memory_order_acquire) ==
           PMM_SUBPROCESS_CANCEL_REQUESTED;
}

static void pmm_subprocess_delete_log(pmm_subprocess_t *process) {
    if (!process->log_file || !process->delete_log_on_exit) {
        return;
    }
#ifdef _WIN32
    wchar_t *path = pmm_path_to_wide(process->log_file);
    if (path) {
...
static bool pmm_subprocess_begin_terminal_transition(pmm_subprocess_t *process) {
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    for (;;) {
        if (lifecycle == PMM_SUBPROCESS_DRAINING) {
            return true;
        }
        if (lifecycle == PMM_SUBPROCESS_TERMINAL) {
            return false;
        }
        int desired = PMM_SUBPROCESS_DRAINING;
        if (atomic_compare_exchange_weak_explicit(&process->lifecycle, &lifecycle, desired,
                                                  memory_order_acq_rel, memory_order_acquire)) {
            process->result.cancellation_requested = lifecycle == PMM_SUBPROCESS_CANCEL_REQUESTED;
            return true;
        }
    }
}

static pmm_proc_poll_t pmm_subprocess_publish_terminal(pmm_subprocess_t *process,
                                                       pmm_proc_result_t *out, bool delete_log) {
    if (delete_log) {
        pmm_subprocess_delete_log(process);
    }
    atomic_store_explicit(&process->lifecycle, PMM_SUBPROCESS_TERMINAL, memory_order_release);
    if (out) {
        *out = process->result;
    }
    return PMM_PROC_POLL_TERMINAL;
}

static pmm_proc_poll_t pmm_subprocess_finish(pmm_subprocess_t *process, pmm_proc_result_t *out) {
    if (!pmm_subprocess_begin_terminal_transition(process)) {
        return PMM_PROC_POLL_ERROR;
    }
    process->result.tree_quiesced = true;
    process->result.supervision_failed = false;
    if (process->log_file && process->on_log_line) {
        return PMM_PROC_POLL_RUNNING;
    }
    return pmm_subprocess_publish_terminal(process, out, true);
}

static pmm_proc_poll_t pmm_subprocess_finish_failed(pmm_subprocess_t *process,
                                                    pmm_proc_result_t *out) {
    if (!pmm_subprocess_begin_terminal_transition(process)) {
        return PMM_PROC_POLL_ERROR;
    }
    ...
    if (process->timed_out) {
        process->result.outcome = PMM_PROC_HANG;
    } else if (process->result.outcome == PMM_PROC_SPAWN_FAILED) {
        process->result.outcome = PMM_PROC_KILLED;
    }
    process->containment_failed = true;
    return pmm_subprocess_publish_terminal(process, out, false);
}

#ifdef _WIN32

static void pmm_win_close_spawn_handles(HANDLE nul, HANDLE log, LPPROC_THREAD_ATTRIBUTE_LIST attrs,
                                        bool attrs_init) {
    if (attrs) {
        if (attrs_init) {
...
static int pmm_subprocess_spawn_win(pmm_subprocess_t *process) {
    char cmdline[8192];
    bool built =
        process->windows_cmd_payload
            ? pmm_build_win_cmd_payload(cmdline, sizeof(cmdline), process->bin,
                                        process->windows_cmd_payload)
            : pmm_build_win_cmdline(cmdline, sizeof(cmdline), (const char *const *)process->argv);
    if (!built) {
        return -1;
    }
    wchar_t *wbin = pmm_utf8_to_wide(process->bin);
    wchar_t *wcmdline = pmm_utf8_to_wide(cmdline);
    if (!wbin || !wcmdline) {
        free(wbin);
        free(wcmdline);
        return -1;
    ...
    if (process->log_file) {
        wchar_t *wlog = pmm_path_to_wide(process->log_file);
        if (wlog) {
    ...
    if (!attrs_ready) {
        pmm_win_close_spawn_handles(nul, log, attrs, attrs_init);
        CloseHandle(job);
        free(wbin);
        free(wcmdline);
        return -1;
...
static bool pmm_win_job_active(pmm_subprocess_t *process, bool *known) {
...
static void pmm_win_begin_termination(pmm_subprocess_t *process, uint64_t now) {
...
static void pmm_win_force_tree(pmm_subprocess_t *process, uint64_t now) {
...
static void pmm_win_capture_root(pmm_subprocess_t *process, DWORD code) {
    ...
    if (process->timed_out) {
        process->result.outcome = PMM_PROC_HANG;
               ...
               (pmm_subprocess_cancellation_requested(process) && code == PMM_WIN_CONTROL_C_EXIT)) {
        process->result.outcome = PMM_PROC_KILLED;
    } else {
        process->result.outcome = pmm_proc_classify(true, (int)code, 0, false);
    }
}

static pmm_proc_poll_t pmm_subprocess_poll_win(pmm_subprocess_t *process, pmm_proc_result_t *out) {
    ...
    if (!process->root_reaped) {
        ...
        if (waited == WAIT_OBJECT_0) {
            ...
            pmm_win_capture_root(process, code);
        } else if (waited == WAIT_FAILED) {
            DWORD code = STILL_ACTIVE;
            if (GetExitCodeProcess(process->process, &code) && code != STILL_ACTIVE) {
                pmm_win_capture_root(process, code);
            } else {
                pmm_win_begin_termination(process, now);
                pmm_win_force_tree(process, now);
            }
        }
    }

    bool job_known = false;
    bool job_active = pmm_win_job_active(process, &job_known);
    if (!process->termination_started) {
        if (pmm_subprocess_cancellation_requested(process)) {
            pmm_win_begin_termination(process, now);
    ...
    if (process->force_started_ms != 0 &&
        now - process->force_started_ms >= PMM_SUBPROCESS_FORCE_SETTLE_MS &&
        (!job_known || job_active || !process->root_reaped)) {
        return pmm_subprocess_finish_failed(process, out);
    }
    if (process->root_reaped && !job_active) {
        return pmm_subprocess_finish(process, out);
    }
    return PMM_PROC_POLL_RUNNING;
}
...
static void pmm_posix_reset_child_signals(void) {
...
static void pmm_posix_child_exec(pmm_subprocess_t *process, int input, int output, long max_fd) {
    if (setpgid(0, 0) < 0) {
        _exit(127);
    }
    pmm_posix_reset_child_signals();
...
static int pmm_posix_fd_at_least_three(int fd) {
...
static int pmm_subprocess_spawn_posix(pmm_subprocess_t *process) {
    int input_flags = O_RDONLY;
#ifdef O_CLOEXEC
    input_flags |= O_CLOEXEC;
#endif
    int input = pmm_posix_fd_at_least_three(open("/dev/null", input_flags));
...
#endif
    int output = pmm_posix_fd_at_least_three(open(target, output_flags, 0600));
    if (input < 0 || output < 0) {
        if (input >= 0) {
            (void)close(input);
        }
    ...
    if (pid == 0) {
        pmm_posix_child_exec(process, input, output, max_fd);
    }
    (void)close(input);
    (void)close(output);
...
static bool pmm_posix_group_active(pmm_subprocess_t *process) {
...
static void pmm_posix_begin_termination(pmm_subprocess_t *process, uint64_t now) {
    if (process->termination_started) {
        return;
    }
    process->termination_started = true;
...
static void pmm_posix_force_tree(pmm_subprocess_t *process, uint64_t now) {
    if (process->force_sent) {
        return;
    }
    if (process->force_started_ms == 0) {
...
static void pmm_posix_capture_root(pmm_subprocess_t *process, int status) {
    process->root_reaped = true;
    if (WIFEXITED(status)) {
        process->result.exit_code = WEXITSTATUS(status);
        process->result.term_signal = 0;
        process->result.outcome =
            pmm_proc_classify(true, process->result.exit_code, 0, process->timed_out);
    } else if (WIFSIGNALED(status)) {
        process->result.exit_code = -1;
        process->result.term_signal = WTERMSIG(status);
        process->result.outcome =
            pmm_proc_classify(false, -1, process->result.term_signal, process->timed_out);
    } else {
        process->result.exit_code = -1;
        process->result.term_signal = 0;
        process->result.outcome = process->timed_out ? PMM_PROC_HANG : PMM_PROC_KILLED;
    }
}

static pmm_proc_poll_t pmm_subprocess_poll_posix(pmm_subprocess_t *process,
                                                 pmm_proc_result_t *out) {
    uint64_t now = pmm_now_ms();
    ...
    if (!process->root_reaped) {
        ...
        if (waited == process->pid) {
            pmm_posix_capture_root(process, status);
            ...
            process->result.exit_code = -1;
            process->result.term_signal = 0;
            pmm_posix_begin_termination(process, now);
        }
    }

    bool group_active = pmm_posix_group_active(process);
    if (!process->termination_started) {
        if (pmm_subprocess_cancellation_requested(process)) {
            pmm_posix_begin_termination(process, now);
        } else if (!process->root_reaped && process->quiet_timeout_ms > 0 &&
    ...
    if (process->termination_started && group_active && !process->force_sent &&
        now - process->termination_started_ms >= (uint64_t)process->cancel_grace_ms) {
        pmm_posix_force_tree(process, now);
    }
    group_active = pmm_posix_group_active(process);
...
int pmm_subprocess_spawn(const pmm_proc_opts_t *opts, pmm_subprocess_t **out) {
    if (!out) {
        return -1;
    }
    *out = NULL;
    pmm_subprocess_t *process = pmm_subprocess_copy_opts(opts);
    if (!process) {
        return -1;
    }
#ifdef _WIN32
    int spawn_rc = pmm_subprocess_spawn_win(process);
#else
    int spawn_rc = pmm_subprocess_spawn_posix(process);
#endif
    if (spawn_rc != 0) {
        pmm_subprocess_free_config(process);
        return -1;
...
pmm_proc_poll_t pmm_subprocess_poll(pmm_subprocess_t *process, pmm_proc_result_t *out) {
    if (!process) {
        return PMM_PROC_POLL_ERROR;
    }
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    ...
    if (lifecycle == PMM_SUBPROCESS_DRAINING) {
        pmm_tail_result_t tail = pmm_subprocess_poll_log(process, true);
        if (tail.status == PMM_TAIL_MORE) {
            return PMM_PROC_POLL_RUNNING;
        }
        if (tail.status == PMM_TAIL_ERROR) {
            pmm_log_error("subprocess.log_drain_failed", "reason", "io_error");
            return pmm_subprocess_publish_terminal(process, out, false);
        }
        return pmm_subprocess_publish_terminal(process, out, true);
    }
    ...
    (void)pmm_subprocess_poll_log(process, false);
#ifdef _WIN32
    return pmm_subprocess_poll_win(process, out);
#else
    return pmm_subprocess_poll_posix(process, out);
#endif
}

bool pmm_subprocess_request_cancel(pmm_subprocess_t *process) {
    if (!process) {
        return false;
    }
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    for (;;) {
        if (lifecycle == PMM_SUBPROCESS_CANCEL_REQUESTED) {
            return true;
        }
        if (lifecycle != PMM_SUBPROCESS_ACTIVE) {
            return false;
        }
        int desired = PMM_SUBPROCESS_CANCEL_REQUESTED;
        if (atomic_compare_exchange_weak_explicit(&process->lifecycle, &lifecycle, desired,
                                                  memory_order_acq_rel, memory_order_acquire)) {
            return true;
        }
...
void pmm_subprocess_destroy(pmm_subprocess_t *process) {
...
int pmm_subprocess_run(const pmm_proc_opts_t *opts, pmm_proc_result_t *out) {
    pmm_proc_result_t local;
    if (!out) {
        out = &local;
    }
    pmm_subprocess_result_init(out);

    pmm_subprocess_t *process = NULL;
    if (pmm_subprocess_spawn(opts, &process) != 0) {
        return -1;
    }
    for (;;) {
        pmm_proc_poll_t state = pmm_subprocess_poll(process, out);
        if (state == PMM_PROC_POLL_TERMINAL) {
