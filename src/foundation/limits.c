...
long pmm_max_file_bytes(void) {
    ...
    const char *raw = getenv("PMM_MAX_FILE_BYTES");
    if (raw && raw[0]) {
        errno = 0;
        char *end = NULL;
 ...
 * Read fresh each call (see pmm_max_file_bytes rationale — cheap, test-friendly,
...
int pmm_cypher_max_depth(void) {
    ...
    return env_positive_int("PMM_CYPHER_MAX_DEPTH", 10);
}
...
int pmm_mcp_max_depth(void) {
    ...
    return env_positive_int("PMM_MCP_MAX_DEPTH", 15);
}
