...
static pmm_mutex_t g_profile_mutex;
static profile_site_t g_sites[PROFILE_SITES];
static profile_pointer_t g_pointers[PROFILE_POINTERS];
static pmm_mem_profile_totals_t g_totals;
static atomic_int g_profile_enabled = -1;
static size_t g_profile_min = PROFILE_DEFAULT_MIN;

size_t pmm_mem_profile_threshold(void) {
    (void)pmm_mem_profile_enabled();
    return g_profile_min;
}

bool pmm_mem_profile_enabled(void) {
    int state = atomic_load_explicit(&g_profile_enabled, memory_order_acquire);
    if (state >= 0) {
        return state == 1;
    }
    char buf[PMM_SZ_32];
    bool on = pmm_safe_getenv("PMM_MEM_PROFILE", buf, sizeof(buf), NULL) != NULL && buf[0] == '1';
    if (on) {
        char min_buf[PMM_SZ_32];
        if (pmm_safe_getenv("PMM_MEM_PROFILE_MIN", min_buf, sizeof(min_buf), NULL) != NULL) {
            long parsed = strtol(min_buf, NULL, 10);
            if (parsed > 0) {
                g_profile_min = (size_t)parsed;
            }
        }
        pmm_mutex_init(&g_profile_mutex);
    }
...
void pmm_mem_profile_alloc(void *block, size_t size) {
    pmm_mem_profile_alloc_at(block, size, NULL);
}

void pmm_mem_profile_alloc_at(void *block, size_t size, void *caller) {
    if (!block || g_profile_reentrant) {
        return;
    }
    g_profile_reentrant = true;
    if (!pmm_mem_profile_enabled() || size < g_profile_min) {
    ...
    if (count <= 0) {
        pmm_mutex_lock(&g_profile_mutex);
        g_totals.capture_failed++;
        pmm_mutex_unlock(&g_profile_mutex);
        g_profile_reentrant = false;
        return;
    }
    pmm_mutex_lock(&g_profile_mutex);
...
void pmm_mem_profile_free(void *block) {
    if (!block || g_profile_reentrant) {
        return;
    }
    g_profile_reentrant = true;
    if (!pmm_mem_profile_enabled()) {
        g_profile_reentrant = false;
        return;
    }
    pmm_mutex_lock(&g_profile_mutex);
...
void pmm_mem_profile_totals(pmm_mem_profile_totals_t *out) {
    if (!out) {
        return;
    }
...
bool pmm_mem_profile_dump(const char *path, const char *label) {
    if (!path || !pmm_mem_profile_enabled()) {
        return false;
    }
    FILE *out = pmm_fopen(path, "ab");
    if (!out) {
        return false;
    }
    pmm_mutex_lock(&g_profile_mutex);
