...
struct pmm_lock_registry {
    pmm_private_lock_directory_t *directory;
    pmm_private_fork_condition_t *condition;
    pmm_mutex_t mutex;
    ...
    uint64_t owner_pid;
    bool closing;
    pmm_lock_registry_stage_hook_fn stage_hook;
    void *stage_context;
    pmm_lock_registry_release_handle_t test_release_fault_handle;
    pmm_private_file_lock_release_step_t test_release_fault_step;
    bool test_release_fault_armed;
    pmm_lock_registry_abort_failure_t test_abort_failure;
    bool test_abort_failure_armed;
    atomic_uint_fast64_t test_condition_wait_calls;
    atomic_size_t test_condition_waiters_now;
    struct pmm_lock_registry *next_live;
    struct pmm_lock_registry *next_retired;
};

struct pmm_lock_lease {
    pmm_lock_registry_t *registry;
    lock_registry_entry_t *entry;
    lock_registry_waiter_t waiter;
    pmm_private_file_lock_mode_t mode;
    pmm_private_file_lock_t *turn;
    pmm_private_file_lock_t *rw;
    uint64_t owner_pid;
    bool active;
    bool cleanup_only;
...
/* Protected by pmm_private_file_lock_fork_guard_enter(). Besides serializing
 ...
 * freed mutex. */
static pmm_lock_registry_t *lock_registry_live;
...
static pmm_lock_registry_t *lock_registry_retired;
...
static bool lock_registry_is_live_unlocked(const pmm_lock_registry_t *registry) {
    for (const pmm_lock_registry_t *cursor = lock_registry_live; cursor;
...
static bool lock_registry_lock(pmm_lock_registry_t *registry) {
    if (!registry || !pmm_private_file_lock_fork_guard_enter()) {
        return false;
    }
    if (!lock_registry_is_live_unlocked(registry)) {
        pmm_private_file_lock_fork_guard_leave();
        return false;
    }
    pmm_mutex_lock(&registry->mutex);
    if (registry->closing || registry->owner_pid != lock_registry_current_pid()) {
        pmm_mutex_unlock(&registry->mutex);
        pmm_private_file_lock_fork_guard_leave();
        return false;
    }
    return true;
...
static void lock_registry_unlock(pmm_lock_registry_t *registry) {
    pmm_mutex_unlock(&registry->mutex);
    pmm_private_file_lock_fork_guard_leave();
}

/* Requires the global fork guard and registry mutex, in that order. */
static void lock_registry_broadcast_locked(pmm_lock_registry_t *registry) {
    pmm_private_fork_condition_broadcast_while_guarded(registry->condition);
}
...
static pmm_private_fork_wait_status_t lock_registry_wait_locked(pmm_lock_registry_t *registry,
    ...
    pmm_mutex_unlock(&registry->mutex);
    pmm_private_fork_wait_status_t status =
        pmm_private_fork_condition_wait_until_while_guarded(registry->condition, deadline_ms);
    pmm_mutex_lock(&registry->mutex);
...
static bool lock_registry_park_native(pmm_lock_registry_t *registry, uint64_t deadline_ms,
                                      const pmm_lock_cancel_token_t *cancel_token) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
    pmm_private_fork_wait_status_t status = PMM_PRIVATE_FORK_WAIT_SIGNALED;
    ...
    return status != PMM_PRIVATE_FORK_WAIT_ERROR;
}
...
static bool lock_registry_should_stop(uint64_t deadline_ms,
    ...
    return deadline_ms != UINT64_MAX && pmm_now_ms() >= deadline_ms;
}

static void lock_registry_notify(pmm_lock_registry_t *registry, pmm_private_file_lock_mode_t mode,
                                 pmm_lock_registry_stage_t stage) {
    if (!lock_registry_lock(registry)) {
        return;
    }
    pmm_lock_registry_stage_hook_fn hook = registry->stage_hook;
    if (!hook) {
        lock_registry_unlock(registry);
        return;
    }
...
static lock_registry_entry_t *lock_registry_find_entry(pmm_lock_registry_t *registry,
...
static void lock_registry_waiter_push(pmm_lock_registry_t *registry, lock_registry_entry_t *entry,
                                      lock_registry_waiter_t *waiter) {
    waiter->next = NULL;
    waiter->queued = true;
...
static bool lock_registry_waiter_remove(pmm_lock_registry_t *registry, lock_registry_entry_t *entry,
                                        lock_registry_waiter_t *waiter) {
    if (!waiter->queued) {
        return false;
    }
...
static void lock_registry_apply_release_fault(pmm_lock_registry_t *registry,
    ...
    pmm_private_file_lock_release_step_t step = registry->test_release_fault_step;
    if (inject) {
        registry->test_release_fault_armed = false;
    }
    lock_registry_unlock(registry);
    if (inject) {
        (void)pmm_private_file_lock_fail_next_release_step_for_test(lock, step);
    }
}

/* Rollback and writer release are deliberately rw-before-turn. */
static pmm_private_file_lock_status_t lock_registry_release_native(
    pmm_lock_registry_t *registry, pmm_private_file_lock_t **rw_io,
    pmm_private_file_lock_t **turn_io) {
    pmm_private_file_lock_status_t result = PMM_PRIVATE_FILE_LOCK_OK;
    lock_registry_apply_release_fault(registry, PMM_LOCK_REGISTRY_RELEASE_RW, *rw_io);
    if (*rw_io && pmm_private_file_lock_release(rw_io) != PMM_PRIVATE_FILE_LOCK_OK) {
        result = PMM_PRIVATE_FILE_LOCK_IO;
        if (*rw_io && !pmm_private_file_lock_unlock_complete(*rw_io)) {
            return result;
        }
    }
    lock_registry_apply_release_fault(registry, PMM_LOCK_REGISTRY_RELEASE_TURN, *turn_io);
    if (*turn_io && pmm_private_file_lock_release(turn_io) != PMM_PRIVATE_FILE_LOCK_OK) {
        result = PMM_PRIVATE_FILE_LOCK_IO;
    }
    return result;
}
...
bool pmm_lock_registry_resource_names(const char *resource_key,
    ...
    char digest[PMM_SHA256_HEX_LEN + 1];
    pmm_sha256_hex(resource_key, length, digest);
    int turn_written = snprintf(turn_out, PMM_LOCK_REGISTRY_NAME_CAP, "pmm-%s.turn", digest);
    int rw_written = snprintf(rw_out, PMM_LOCK_REGISTRY_NAME_CAP, "pmm-%s.rw", digest);
    return turn_written > 0 && turn_written < (int)PMM_LOCK_REGISTRY_NAME_CAP && rw_written > 0 &&
           rw_written < (int)PMM_LOCK_REGISTRY_NAME_CAP;
}

pmm_lock_registry_t *pmm_lock_registry_new(pmm_private_lock_directory_t *directory) {
    if (!directory) {
        return NULL;
    }
    pmm_lock_registry_t *registry = calloc(1, sizeof(*registry));
    if (!registry) {
        return NULL;
    }
    registry->directory = directory;
    registry->owner_pid = lock_registry_current_pid();
    pmm_mutex_init(&registry->mutex);
    registry->condition = pmm_private_fork_condition_new();
    if (!registry->condition) {
        pmm_mutex_destroy(&registry->mutex);
        free(registry);
        return NULL;
    }
    if (!pmm_private_file_lock_fork_guard_enter()) {
        pmm_private_fork_condition_free(registry->condition);
        pmm_mutex_destroy(&registry->mutex);
        free(registry);
        return NULL;
    }
...
pmm_private_file_lock_status_t pmm_lock_registry_request_cancel(pmm_lock_registry_t *registry,
...
static void lock_registry_retain_waiter_cleanup(pmm_lock_lease_t *lease,
                                                pmm_private_file_lock_t **rw_io,
                                                pmm_private_file_lock_t **turn_io,
                                                pmm_lock_lease_t **lease_out) {
    lease->rw = *rw_io;
    lease->turn = *turn_io;
...
static bool lock_registry_abort_lock(pmm_lock_registry_t *registry) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
    bool fail_lock = registry->test_abort_failure_armed &&
                     registry->test_abort_failure == PMM_LOCK_REGISTRY_ABORT_FAIL_LOCK;
    if (fail_lock) {
...
static pmm_private_file_lock_status_t lock_registry_abort_attempt(
    pmm_lock_registry_t *registry, lock_registry_entry_t *entry, lock_registry_waiter_t *waiter,
    pmm_private_file_lock_t **rw_io, pmm_private_file_lock_t **turn_io,
    pmm_private_file_lock_status_t requested_status, pmm_lock_lease_t *lease,
    pmm_lock_lease_t **lease_out) {
    ...
    bool fail_remove = registry->test_abort_failure_armed &&
                       registry->test_abort_failure == PMM_LOCK_REGISTRY_ABORT_FAIL_REMOVE;
    if (fail_remove) {
        registry->test_abort_failure_armed = false;
    }
    ...
    if (!has_native) {
        free(lease);
        return owns_attempt ? requested_status : PMM_PRIVATE_FILE_LOCK_IO;
    }
    pmm_lock_lease_t *cleanup = lease;
    if (pmm_lock_lease_release(&cleanup) != PMM_PRIVATE_FILE_LOCK_OK) {
        *lease_out = cleanup;
        return PMM_PRIVATE_FILE_LOCK_IO;
    }
    return owns_attempt ? requested_status : PMM_PRIVATE_FILE_LOCK_IO;
...
static pmm_private_file_lock_status_t lock_registry_attempt_native(
    pmm_lock_registry_t *registry, lock_registry_entry_t *entry, lock_registry_waiter_t *waiter,
    uint64_t deadline_ms, const pmm_lock_cancel_token_t *cancel_token, bool try_once,
    pmm_lock_lease_t *lease, pmm_lock_lease_t **lease_out) {
    pmm_private_file_lock_t *turn = NULL;
    pmm_private_file_lock_t *rw = NULL;
    ...
    while (!rw) {
        ...
        if (!turn) {
            pmm_private_file_lock_mode_t turn_mode = waiter->mode == PMM_PRIVATE_FILE_LOCK_SH
                                                         ? PMM_PRIVATE_FILE_LOCK_EX
                                                         : PMM_PRIVATE_FILE_LOCK_SH;
            status = pmm_private_file_lock_try_acquire(registry->directory, entry->turn_name,
                                                       turn_mode, &turn);
            if (status == PMM_PRIVATE_FILE_LOCK_BUSY) {
                lock_registry_notify(registry, waiter->mode, PMM_LOCK_REGISTRY_STAGE_TURN_BUSY);
                if (try_once) {
                    break;
                }
            ...
            lock_registry_notify(registry, waiter->mode, PMM_LOCK_REGISTRY_STAGE_TURN_HELD);
        }
        ...
        status = pmm_private_file_lock_try_acquire(registry->directory, entry->rw_name,
                                                   waiter->mode, &rw);
        if (status == PMM_PRIVATE_FILE_LOCK_BUSY) {
            lock_registry_notify(registry, waiter->mode, PMM_LOCK_REGISTRY_STAGE_RW_BUSY);
            if (try_once) {
                break;
            }
            if (!lock_registry_park_native(registry, deadline_ms, cancel_token)) {
        ...
        if (waiter->mode == PMM_PRIVATE_FILE_LOCK_SH) {
            status = pmm_private_file_lock_release(&turn);
            if (status != PMM_PRIVATE_FILE_LOCK_OK) {
                break;
            }
        }
        lock_registry_notify(registry, waiter->mode, PMM_LOCK_REGISTRY_STAGE_NATIVE_READY);
    }

    if (!rw || status != PMM_PRIVATE_FILE_LOCK_OK) {
    ...
    for (;;) {
        if (!try_once && lock_registry_should_stop(deadline_ms, cancel_token)) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               PMM_PRIVATE_FILE_LOCK_BUSY, lease, lease_out);
        }
        if (!lock_registry_lock(registry)) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               PMM_PRIVATE_FILE_LOCK_IO, lease, lease_out);
        }
        bool owns_attempt = entry->attempt_waiter == waiter && entry->waiter_head == waiter;
        bool local_state_ready = waiter->mode == PMM_PRIVATE_FILE_LOCK_SH
                                     ? !entry->writer_active
        ...
        pmm_private_fork_wait_status_t wait_status = PMM_PRIVATE_FORK_WAIT_SIGNALED;
        if (removed) {
            entry->attempt_waiter = NULL;
            if (waiter->mode == PMM_PRIVATE_FILE_LOCK_SH) {
        ...
        if (wait_status == PMM_PRIVATE_FORK_WAIT_ERROR) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               PMM_PRIVATE_FILE_LOCK_IO, lease, lease_out);
        }
    }
...
static pmm_private_file_lock_status_t lock_registry_acquire_internal(
    pmm_lock_registry_t *registry, const char *resource_key, pmm_private_file_lock_mode_t mode,
    uint64_t deadline_ms, const pmm_lock_cancel_token_t *cancel_token, bool try_once,
    pmm_lock_lease_t **lease_out) {
    if (lease_out) {
        *lease_out = NULL;
    ...
    if (mode != PMM_PRIVATE_FILE_LOCK_SH && mode != PMM_PRIVATE_FILE_LOCK_EX) {
        return PMM_PRIVATE_FILE_LOCK_UNSAFE;
    }
    char turn_name[PMM_LOCK_REGISTRY_NAME_CAP];
    char rw_name[PMM_LOCK_REGISTRY_NAME_CAP];
    if (!pmm_lock_registry_resource_names(resource_key, turn_name, rw_name)) {
        return PMM_PRIVATE_FILE_LOCK_UNSAFE;
    }
    if (!try_once && lock_registry_should_stop(deadline_ms, cancel_token)) {
        return PMM_PRIVATE_FILE_LOCK_BUSY;
    }

    lock_registry_entry_t *candidate = calloc(1, sizeof(*candidate));
    pmm_lock_lease_t *lease = calloc(1, sizeof(*lease));
    if (!candidate || !lease) {
        free(candidate);
        free(lease);
        return PMM_PRIVATE_FILE_LOCK_IO;
...
pmm_private_file_lock_status_t pmm_lock_registry_acquire(
    pmm_lock_registry_t *registry, const char *resource_key, pmm_private_file_lock_mode_t mode,
    uint64_t deadline_ms, const pmm_lock_cancel_token_t *cancel_token,
    pmm_lock_lease_t **lease_out) {
    return lock_registry_acquire_internal(registry, resource_key, mode, deadline_ms, cancel_token,
                                          false, lease_out);
}

pmm_private_file_lock_status_t pmm_lock_registry_try_acquire(pmm_lock_registry_t *registry,
...
static pmm_private_file_lock_status_t lock_registry_cleanup_lease_release(
    pmm_lock_lease_t **lease_io) {
    pmm_lock_lease_t *lease = *lease_io;
    pmm_lock_registry_t *registry = lease->registry;
...
pmm_private_file_lock_status_t pmm_lock_lease_release(pmm_lock_lease_t **lease_io) {
    ...
    } else if (lease->mode == PMM_PRIVATE_FILE_LOCK_SH) {
        native_state_valid = lease->rw && !lease->turn;
    } else if (lease->mode == PMM_PRIVATE_FILE_LOCK_EX) {
    ...
    if (valid && lease->mode == PMM_PRIVATE_FILE_LOCK_SH) {
        valid = entry->active_readers > 0;
    } else if (valid && lease->mode == PMM_PRIVATE_FILE_LOCK_EX) {
    ...
    if (!lease->critical_released) {
        ...
        if (lease->rw && !pmm_private_file_lock_unlock_complete(lease->rw)) {
            return status;
        }
    ...
    if (lease->mode == PMM_PRIVATE_FILE_LOCK_SH) {
        entry->active_readers--;
    } else {
    ...
    pmm_private_file_lock_status_t result =
        lease->release_error ? PMM_PRIVATE_FILE_LOCK_IO : PMM_PRIVATE_FILE_LOCK_OK;
    *lease_io = NULL;
    free(lease);
    return result;
}

pmm_private_file_lock_status_t pmm_lock_registry_free(pmm_lock_registry_t **registry_io) {
    if (!registry_io || !*registry_io || !pmm_private_file_lock_fork_guard_enter()) {
        return PMM_PRIVATE_FILE_LOCK_IO;
    }
    pmm_lock_registry_t *registry = *registry_io;
    pmm_lock_registry_t **live_cursor = &lock_registry_live;
...
bool pmm_lock_registry_is_retired_for_test(const pmm_lock_registry_t *registry) {
    ...
    bool retired = false;
    for (const pmm_lock_registry_t *cursor = lock_registry_retired; cursor;
...
bool pmm_lock_lease_fail_next_release_step_for_test(pmm_lock_lease_t *lease,
    ...
    pmm_private_file_lock_t *lock = handle == PMM_LOCK_REGISTRY_RELEASE_RW     ? lease->rw
                                    : handle == PMM_LOCK_REGISTRY_RELEASE_TURN ? lease->turn
                                                                               : NULL;
    return pmm_private_file_lock_fail_next_release_step_for_test(lock, step);
}
...
bool pmm_lock_lease_has_release_handle_for_test(const pmm_lock_lease_t *lease,
    ...
    if (handle == PMM_LOCK_REGISTRY_RELEASE_RW) {
        return lease->rw != NULL;
    }
    if (handle == PMM_LOCK_REGISTRY_RELEASE_TURN) {
...
bool pmm_lock_lease_fail_close_after_consuming_for_test(pmm_lock_lease_t *lease,
    ...
    pmm_private_file_lock_t *lock = handle == PMM_LOCK_REGISTRY_RELEASE_RW     ? lease->rw
                                    : handle == PMM_LOCK_REGISTRY_RELEASE_TURN ? lease->turn
                                                                               : NULL;
    return pmm_private_file_lock_fail_close_after_consuming_for_test(lock);
}
#endif
...
bool pmm_lock_registry_fail_next_native_release_step_for_test(
    ...
    pmm_private_file_lock_release_step_t step) {
    if ((handle != PMM_LOCK_REGISTRY_RELEASE_RW && handle != PMM_LOCK_REGISTRY_RELEASE_TURN) ||
        (step != PMM_PRIVATE_FILE_LOCK_RELEASE_UNLOCK &&
         step != PMM_PRIVATE_FILE_LOCK_RELEASE_CLOSE) ||
        !lock_registry_lock(registry)) {
        return false;
    }
...
bool pmm_lock_registry_fail_next_abort_bookkeeping_for_test(
    ...
    if ((failure != PMM_LOCK_REGISTRY_ABORT_FAIL_LOCK &&
         failure != PMM_LOCK_REGISTRY_ABORT_FAIL_REMOVE) ||
        !lock_registry_lock(registry)) {
        return false;
    }
    bool idle = registry->waiter_count == 0 && registry->active_lease_count == 0 &&
...
bool pmm_lock_registry_set_stage_hook_for_test(pmm_lock_registry_t *registry,
                                               pmm_lock_registry_stage_hook_fn hook,
                                               void *context) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
