...
#define PMM_DEFAULT_STACK_SIZE ((size_t)8 * PMM_SZ_1K * PMM_SZ_1K)
#include <string.h>
 ...
 *
 * PMM_MI_THREAD_DONE=0 disables it, so one binary can demonstrate both
...
static bool thread_release_heap_enabled(void) {
    ...
    if (state < 0) {
        char buf[PMM_SZ_16];
        state =
            (pmm_safe_getenv("PMM_MI_THREAD_DONE", buf, sizeof(buf), NULL) != NULL && buf[0] == '0')
                ? 0
                : 1;
    }
    return state == 1;
...
static void NTAPI pmm_thread_detach_callback(PVOID handle, DWORD reason, PVOID reserved) {
    (void)handle;
    (void)reserved;
...
static const void *const pmm_tls_anchor __attribute__((used)) = &_tls_used;
__attribute__((section(".CRT$XLB"), used)) PIMAGE_TLS_CALLBACK pmm_thread_detach_tls_cb =
    pmm_thread_detach_callback;
...
int pmm_thread_create(pmm_thread_t *t, size_t stack_size, void *(*fn)(void *), void *arg) {
    if (stack_size == 0) {
        stack_size = PMM_DEFAULT_STACK_SIZE;
    }
