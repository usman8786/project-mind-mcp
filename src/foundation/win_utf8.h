#ifndef PMM_WIN_UTF8_H
#define PMM_WIN_UTF8_H
...
static inline wchar_t *pmm_utf8_to_wide(const char *utf8) {
    if (!utf8) {
        return NULL;
    }
...
static inline wchar_t *pmm_path_to_wide(const char *utf8_path) {
    wchar_t *wide = pmm_utf8_to_wide(utf8_path);
    if (!wide) {
        return NULL;
    }
...
static inline char *pmm_wide_to_utf8(const wchar_t *wide) {
    if (!wide) {
        return NULL;
    }
