...
static CBMLogLevel g_log_level = PMM_LOG_INFO;
static CBMLogFormat g_log_format = PMM_LOG_FORMAT_TEXT;
static pmm_log_sink_fn g_log_sink = NULL;
static CBMLogSinkMode g_log_sink_mode = PMM_LOG_SINK_REPLACE;

/* PMM_LOG_LEVEL support — distilled from #414 (closes #413, thanks @santanusinha). */
void pmm_log_init_from_env(void) {
    ...
    const char *raw = getenv("PMM_LOG_LEVEL");
    if (raw && raw[0] != '\0') {
        ...
        if (raw[i] == '\0') { /* fully consumed — candidate textual match */
            for (size_t lvl = 0; lvl < sizeof(names) / sizeof(names[0]); lvl++) {
                if (strcmp(lower, names[lvl]) == 0) {
                    pmm_log_set_level((CBMLogLevel)lvl);
                    goto parse_format;
                }
            }
        }
        ...
        char *end = NULL;
        long n = strtol(raw, &end, PMM_DECIMAL_BASE);
        if (end != raw && *end == '\0' && n >= PMM_LOG_DEBUG && n <= PMM_LOG_NONE) {
            pmm_log_set_level((CBMLogLevel)n);
        }
    }
...
parse_format:;
    const char *fmt = getenv("PMM_LOG_FORMAT");
    if (fmt && fmt[0] != '\0') {
        char lower_fmt[8];
        size_t i = 0;
...
void pmm_log_set_sink(pmm_log_sink_fn fn) {
    pmm_log_set_sink_ex(fn, PMM_LOG_SINK_REPLACE);
}
...
static void emit_line(const char *line) {
    if (g_log_sink) {
        g_log_sink(line);
        if (g_log_sink_mode == PMM_LOG_SINK_REPLACE) {
            return;
        }
    }
...
void pmm_log(CBMLogLevel level, const char *msg, ...) {
    ...
    char line_buf[PMM_SZ_4K];
    size_t pos = 0;
    va_list args;
    va_start(args, msg);

    if (g_log_format == PMM_LOG_FORMAT_JSON) {
...
void pmm_log_control_record(const char *msg, ...) {
     ...
     * PMM_LOG_LEVEL suppression. Always JSON, independent of PMM_LOG_FORMAT,
...
void pmm_log_int(CBMLogLevel level, const char *msg, const char *key, int64_t value) {
    char value_buf[PMM_SZ_32];
    snprintf(value_buf, sizeof(value_buf), "%" PRId64, value);
    pmm_log(level, msg, key ? key : "?", value_buf, NULL);
}
...
void pmm_log_mcp_request(const char *method, const char *tool_name, bool is_error,
                         int64_t duration_us) {
    char duration_ms[PMM_SZ_32];
    snprintf(duration_ms, sizeof(duration_ms), "%" PRId64, duration_us / 1000);
    if (tool_name && tool_name[0] != '\0') {
        pmm_log(is_error ? PMM_LOG_WARN : PMM_LOG_INFO, "mcp.request", "protocol", "jsonrpc",
...
void pmm_log_http_request(const char *component, const char *method, const char *path, int status,
    ...
    char safe_path[PMM_SZ_1K];
    char status_buf[PMM_SZ_16];
    char duration_buf[PMM_SZ_32];
    char request_buf[PMM_SZ_32];
    char response_buf[PMM_SZ_32];
    ...
    CBMLogLevel level = PMM_LOG_INFO;
    if (status >= 500) {
        level = PMM_LOG_ERROR;
    } else if (status >= 400) {
        level = PMM_LOG_WARN;
    ...
    pmm_log(level, "http.request", "component", component ? component : "", "method",
