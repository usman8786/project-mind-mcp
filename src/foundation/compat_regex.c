...
_Static_assert(sizeof(regex_t) <= PMM_SZ_256,
               "pmm_regex_t opaque buffer too small for TRE regex_t");

static int translate_flags_tre(int flags) {
    int tre_flags = 0;
    if (flags & PMM_REG_EXTENDED)
        tre_flags |= REG_EXTENDED;
    if (flags & PMM_REG_ICASE)
        tre_flags |= REG_ICASE;
    if (flags & PMM_REG_NOSUB)
        tre_flags |= REG_NOSUB;
    if (flags & PMM_REG_NEWLINE)
...
int pmm_regexec(const pmm_regex_t *r, const char *str, int nmatch, pmm_regmatch_t *matches,
    ...
    if (nmatch <= 0 || !matches) {
        ...
        return rc == 0 ? PMM_REG_OK : PMM_REG_NOMATCH;
    }
    regmatch_t pmatch[PMM_SZ_32];
    int n = nmatch > PMM_SZ_32 ? PMM_SZ_32 : nmatch;
    int rc = tre_regexec(re, str, (size_t)n, pmatch, eflags);
    if (rc != 0)
        return PMM_REG_NOMATCH;
    for (int i = 0; i < n; i++) {
 ...
 * If this fires, increase pmm_regex_t.opaque size. */
_Static_assert(sizeof(regex_t) <= PMM_SZ_256, "pmm_regex_t opaque buffer too small for regex_t");
