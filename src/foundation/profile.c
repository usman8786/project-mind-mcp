...
bool pmm_profile_active = false;

void pmm_profile_init(void) {
    const char *env = getenv("PMM_PROFILE");
    if (env && env[0] != '\0' && env[0] != '0') {
        pmm_profile_active = true;
    }
}
...
void pmm_profile_log_elapsed(const char *phase, const char *sub, const struct timespec *start,
    ...
    if (items > 0 && us > 0) {
        ...
        pmm_log_info("prof", "phase", phase, "sub", sub, "ms", ms_buf, "us", us_buf, "items",
        ...
        pmm_log_info("prof", "phase", phase, "sub", sub, "ms", ms_buf, "us", us_buf);
    }
}
