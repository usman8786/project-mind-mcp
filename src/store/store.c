 ...
 *
 * Implements the opaque pmm_store_t handle with prepared statement caching,
...
enum {
     ...
     * PMM_SZ_1K; percent-encoding can triple it, plus the "file://" prefix,
...
static void store_set_error(pmm_store_t *s, const char *msg) {
    snprintf(s->errbuf, sizeof(s->errbuf), "%s", msg);
}

static void store_set_error_sqlite(pmm_store_t *s, const char *prefix) {
    snprintf(s->errbuf, sizeof(s->errbuf), "%s: %s", prefix, sqlite3_errmsg(s->db));
}

static int exec_sql(pmm_store_t *s, const char *sql) {
    if (!s || !s->db) {
...
static sqlite3_stmt *prepare_cached(pmm_store_t *s, sqlite3_stmt **slot, const char *sql) {
    if (!s || !s->db) {
        return NULL;
    }
    if (*slot) {
    ...
    int rc = sqlite3_prepare_v2(s->db, sql, PMM_NOT_FOUND, slot, NULL);
    if (rc != SQLITE_OK) {
...
static int init_schema(pmm_store_t *s) {
    const char *ddl =
         ...
         * built sqlite_autoindex_edges_1 (internal/pmm/sqlite_writer.c) —
     ...
     * the column — the widened upsert in pmm_store_insert_edge can neither
     ...
     * reindex). Read-only query opens skip init_schema and keep working. */
    {
        ...
        if (sqlite3_prepare_v2(s->db, "SELECT local_name_gen FROM edges LIMIT 0;", PMM_NOT_FOUND,
                               &probe, NULL) != SQLITE_OK) {
            pmm_log_warn("store.schema", "result", "incompatible", "missing",
     ...
     * because we feed it `pmm_camel_split(name)` at insert time but want
...
static int create_user_indexes(pmm_store_t *s) {
    const char *sql =
...
int64_t pmm_store_resolve_mmap_size(void) {
    ...
    char buf[ST_BUF_64];
    if (pmm_safe_getenv("PMM_SQLITE_MMAP_SIZE", buf, sizeof(buf), NULL) == NULL) {
...
static int configure_pragmas(pmm_store_t *s, bool in_memory, bool read_only) {
    int rc;
    ...
    if (in_memory) {
        ...
        snprintf(mmap_sql, sizeof(mmap_sql), "PRAGMA mmap_size = %lld;",
                 (long long)pmm_store_resolve_mmap_size());
         ...
         * all PASSIVE (they never ftruncate — see pmm_store_checkpoint's SIGBUS
...
#define PMM_STORE_DENOM_EPS_D 1e-10
#define PMM_STORE_DENOM_EPS_F 1e-10F
#define PMM_STORE_INT8_MAX 127.0F
#define PMM_STORE_UNIT_POS_F 1.0F
#define PMM_STORE_UNIT_POS_D 1.0
...
static sqlite3_destructor_type pmm_sqlite_transient_destructor(void) {
    static const volatile intptr_t raw = -1;
    sqlite3_destructor_type dtor = NULL;
    memcpy(&dtor, (const void *)&raw, sizeof(dtor));
    return dtor;
}
#define PMM_SQLITE_TRANSIENT (pmm_sqlite_transient_destructor())
...
static void sqlite_camel_split(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    if (!input || !input[0]) {
        sqlite3_result_text(ctx, input ? input : "", SQLITE_AUTO_LEN, PMM_SQLITE_TRANSIENT);
        return;
    }
    ...
    if (len < 0 || len >= (int)sizeof(buf)) {
        ...
        sqlite3_result_text(ctx, input, SQLITE_AUTO_LEN, PMM_SQLITE_TRANSIENT);
        return;
    }
    ...
    buf[len] = '\0';
    sqlite3_result_text(ctx, buf, len, PMM_SQLITE_TRANSIENT);
}
 ...
 * value), so pmm_regcomp is called exactly once per statement instead of once per row. */
static void sqlite_regexp(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    pmm_regex_t *re = (pmm_regex_t *)sqlite3_get_auxdata(ctx, 0);
    if (!re) {
        re = malloc(sizeof(pmm_regex_t));
        if (!re) {
            sqlite3_result_error_nomem(ctx);
            return;
        }
        if (pmm_regcomp(re, pattern, PMM_REG_EXTENDED | PMM_REG_NOSUB) != 0) {
            free(re);
            sqlite3_result_error(ctx, "invalid regex", PMM_NOT_FOUND);
            return;
        }
        sqlite3_set_auxdata(ctx, 0, re, regex_free_cb);
    }

    sqlite3_result_int(ctx, pmm_regexec(re, text, 0, NULL, 0) == 0 ? SKIP_ONE : 0);
}
...
static void sqlite_iregexp(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    if (!re) {
        ...
        if (pmm_regcomp(re, pattern, PMM_REG_EXTENDED | PMM_REG_NOSUB | PMM_REG_ICASE) != 0) {
            free(re);
            sqlite3_result_error(ctx, "invalid regex", PMM_NOT_FOUND);
            return;
        }
...
static void sqlite_cosine_i8(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    sqlite3_result_double(ctx, denom > PMM_STORE_DENOM_EPS_D ? (double)dot / denom : 0.0);
}
...
static pmm_store_t *store_open_internal(const char *path, bool in_memory, bool create) {
    pmm_store_t *s = calloc(PMM_ALLOC_ONE, sizeof(pmm_store_t));
    if (!s) {
        return NULL;
    }
    ...
    if (path && !in_memory) {
        if (!pmm_path_for_file_api(path, open_path, sizeof(open_path))) {
            free(s);
            return NULL;
        }
    ...
    sqlite3_create_function(s->db, "pmm_cosine_i8", ST_COL_2, SQLITE_UTF8 | SQLITE_DETERMINISTIC,
                            NULL, sqlite_cosine_i8, NULL, NULL);
    /* camelCase splitter for FTS5 BM25 indexing */
    sqlite3_create_function(s->db, "pmm_camel_split", SKIP_ONE, SQLITE_UTF8 | SQLITE_DETERMINISTIC,
                            NULL, sqlite_camel_split, NULL, NULL);

    if (configure_pragmas(s, in_memory, false) != PMM_STORE_OK || init_schema(s) != PMM_STORE_OK ||
        create_user_indexes(s) != PMM_STORE_OK) {
        sqlite3_close(s->db);
...
pmm_store_t *pmm_store_open_path_query(const char *db_path) {
    ...
    char open_path[4096];
    if (!pmm_path_for_file_api(db_path, open_path, sizeof(open_path))) {
        free(s);
        return NULL;
    }
    ...
    if (configure_pragmas(s, false, true) != PMM_STORE_OK) {
        sqlite3_close(s->db);
        safe_str_free(&s->db_path);
        free(s);
        return NULL;
...
bool pmm_store_check_integrity_deep(pmm_store_t *s) {
    if (!pmm_store_check_integrity(s)) {
        return false;
    }
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, "PRAGMA quick_check(1);", PMM_NOT_FOUND, &stmt, NULL) !=
        SQLITE_OK) {
        return false;
    }
    bool ok = false;
...
bool pmm_store_check_integrity(pmm_store_t *s) {
    ...
    sqlite3_stmt *stmt = NULL;
    int rc =
        sqlite3_prepare_v2(s->db, "SELECT count(*) FROM projects;", PMM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        return false;
    }
...
sqlite3 *pmm_store_get_db(pmm_store_t *s) {
    return s ? s->db : NULL;
}
...
int pmm_store_begin_bulk(pmm_store_t *s) {
    ...
    return exec_sql(s, "PRAGMA cache_size = -65536;"); /* PMM_SZ_64 MB */
}
...
int pmm_store_checkpoint(pmm_store_t *s) {
    ...
    if (wal_frames >= ST_WAL_STARVE_WARN_FRAMES && checkpointed < wal_frames) {
        ...
        pmm_log_warn("store.wal.starving", "wal_frames", frames_buf, "checkpointed", ckpt_buf,
                     "hint",
...
static int prepare_sqlite_for_publish(sqlite3 *db) {
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(db, "PRAGMA journal_mode=DELETE;", PMM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        return PMM_STORE_ERR;
    }
    ...
    sqlite3_finalize(stmt);
    return delete_mode ? PMM_STORE_OK : PMM_STORE_ERR;
}
...
int pmm_store_backup_path(const char *source_path, const char *staging_path) {
    ...
    if (pmm_remove_db_sidecars(staging_path) != 0) {
        return PMM_STORE_ERR;
    }
    ...
    int result = PMM_STORE_ERR;
    if (backup) {
        int step_rc = sqlite3_backup_step(backup, PMM_NOT_FOUND);
...
int pmm_store_dump_to_file(pmm_store_t *s, const char *dest_path) {
    ...
    rc = sqlite3_backup_step(bk, PMM_NOT_FOUND); /* copy ALL pages in one shot */
    ...
    pmm_remove_db_sidecars(dest_path);
    if (pmm_rename_replace(tmp_path, dest_path) != 0) {
...
int pmm_store_generation(pmm_store_t *s, char *buf, size_t bufsz) {
    ...
    if (sqlite3_prepare_v2(s->db,
                           ...
                           PMM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
        return PMM_STORE_OK; /* pre-migration DB: stable "legacy" generation */
    }
    if (sqlite3_step(stmt) == SQLITE_ROW) {
...
int pmm_store_get_project(pmm_store_t *s, const char *name, pmm_project_t *out) {
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->root_path = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_2));
        return PMM_STORE_OK;
    }
    return PMM_STORE_NOT_FOUND;
}
...
int pmm_store_list_projects(pmm_store_t *s, pmm_project_t **out, int *count) {
    ...
    int n = 0;
    pmm_project_t *arr = malloc(cap * sizeof(pmm_project_t));

    int scan_rc1;
    while ((scan_rc1 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_project_t));
        }
        ...
        arr[n].root_path = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_2));
        n++;
    }
    if (scan_rc1 != SQLITE_DONE) { /* SCANCHK:1:stmt */
        store_set_error_sqlite(s, "row scan aborted");
        pmm_store_free_projects(arr, n);
        *out = NULL;
        *count = 0;
        return PMM_STORE_ERR;
    }
...
int pmm_store_delete_project(pmm_store_t *s, const char *name) {
    ...
    if (exec_sql(s, "BEGIN;") != PMM_STORE_OK) {
        return PMM_STORE_ERR;
    }
    ...
    for (size_t i = 0; i < sizeof(cleanup_sql) / sizeof(cleanup_sql[0]); i++) {
        ...
        if (sqlite3_prepare_v2(s->db, cleanup_sql[i], PMM_NOT_FOUND, &cleanup, NULL) != SQLITE_OK) {
...
static void scan_node(sqlite3_stmt *stmt, pmm_node_t *n) {
    n->id = sqlite3_column_int64(stmt, 0);
    n->project = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
    n->label = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_2));
    n->name = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_3));
    n->qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_4));
    n->file_path = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_5));
    n->start_line = sqlite3_column_int(stmt, PMM_SZ_6);
    n->end_line = sqlite3_column_int(stmt, PMM_SZ_7);
...
int pmm_store_find_node_by_qn(pmm_store_t *s, const char *project, const char *qn,
                              pmm_node_t *out) {
    if (!s || !s->db) {
        return PMM_STORE_ERR;
    }
...
int pmm_store_find_nodes_by_name_any(pmm_store_t *s, const char *name, pmm_node_t **out,
    ...
    int n = 0;
    pmm_node_t *arr = malloc(cap * sizeof(pmm_node_t));
    int scan_rc2;
    while ((scan_rc2 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_node_t));
        }
...
int pmm_store_find_node_ids_by_qns(pmm_store_t *s, const char *project, const char **qns,
    ...
    for (int i = 0; i < qn_count; i++) {
        ...
        int rc = pmm_store_find_node_by_qn(s, project, qns[i], &node);
        if (rc == PMM_STORE_OK) {
            out_ids[i] = node.id;
            found++;
            pmm_node_free_fields(&node);
...
static int find_nodes_generic(pmm_store_t *s, sqlite3_stmt **slot, const char *sql,
                              const char *project, const char *val, pmm_node_t **out, int *count) {
    if (!s || !s->db) {
        *out = NULL;
        *count = 0;
        return PMM_STORE_ERR;
...
int pmm_store_upsert_node_batch(pmm_store_t *s, const pmm_node_t *nodes, int count,
    ...
    for (int i = 0; i < count; i++) {
        int64_t id = pmm_store_upsert_node(s, &nodes[i]);
        if (id == PMM_STORE_ERR) {
            exec_sql(s, "ROLLBACK;");
            return PMM_STORE_ERR;
        }
...
int64_t pmm_store_insert_edge(pmm_store_t *s, const pmm_edge_t *e) {
    ...
    sqlite3_bind_int64(stmt, PMM_SZ_3, e->target_id);
...
static void scan_edge(sqlite3_stmt *stmt, pmm_edge_t *e) {
    e->id = sqlite3_column_int64(stmt, 0);
    e->project = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
    e->source_id = sqlite3_column_int64(stmt, PMM_SZ_2);
    e->target_id = sqlite3_column_int64(stmt, PMM_SZ_3);
    e->type = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_4));
    e->properties_json = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_5));
}

/* Generic: find multiple edges by a filter. */
static int find_edges_generic(pmm_store_t *s, sqlite3_stmt **slot, const char *sql,
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    pmm_edge_t *arr = malloc(cap * sizeof(pmm_edge_t));

    int scan_rc4;
    while ((scan_rc4 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_edge_t));
        }
...
int pmm_store_fetch_call_edges(pmm_store_t *s, const char *project, int max_edges,
    ...
    bind_text(stmt, SKIP_ONE, project);
    sqlite3_bind_int(stmt, 2, max_edges > 0 ? max_edges + SKIP_ONE : PMM_NOT_FOUND);
...
int pmm_store_find_edges_by_source(pmm_store_t *s, int64_t source_id, pmm_edge_t **out,
                                   int *count) {
...
int pmm_store_find_edges_by_target(pmm_store_t *s, int64_t target_id, pmm_edge_t **out,
                                   int *count) {
...
int pmm_store_insert_edge_batch(pmm_store_t *s, const pmm_edge_t *edges, int count) {
    ...
    exec_sql(s, "BEGIN IMMEDIATE;");
    for (int i = 0; i < count; i++) {
        int64_t id = pmm_store_insert_edge(s, &edges[i]);
        if (id == PMM_STORE_ERR) {
            exec_sql(s, "ROLLBACK;");
            return PMM_STORE_ERR;
        }
...
int pmm_store_upsert_file_hash(pmm_store_t *s, const char *project, const char *rel_path,
    ...
    sqlite3_bind_int64(stmt, PMM_SZ_4, mtime_ns);
    sqlite3_bind_int64(stmt, PMM_SZ_5, size);
...
int pmm_store_get_file_hashes(pmm_store_t *s, const char *project, pmm_file_hash_t **out,
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    pmm_file_hash_t *arr = malloc(cap * sizeof(pmm_file_hash_t));

    int scan_rc5;
    while ((scan_rc5 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_file_hash_t));
        }
        ...
        arr[n].sha256 = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_2));
        arr[n].mtime_ns = sqlite3_column_int64(stmt, PMM_SZ_3);
        arr[n].size = sqlite3_column_int64(stmt, PMM_SZ_4);
        n++;
    }
    if (scan_rc5 != SQLITE_DONE) { /* SCANCHK:5:stmt */
        store_set_error_sqlite(s, "row scan aborted");
        pmm_store_free_file_hashes(arr, n);
        *out = NULL;
        *count = 0;
        return PMM_STORE_ERR;
    }
...
int pmm_store_get_file_hash(pmm_store_t *s, const char *project, const char *rel_path,
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->size = sqlite3_column_int64(stmt, PMM_SZ_4);
        sqlite3_finalize(stmt);
        if (!out->project || !out->rel_path || !out->sha256) {
            pmm_store_clear_file_hash(out);
            return PMM_STORE_ERR;
        }
        return PMM_STORE_OK;
    }
...
static int cov_rebuild_shadow_graph(pmm_store_t *s, const char *project) {
    ...
    for (int w = 0; w < 2; w++) {
        ...
        if (sqlite3_prepare_v2(s->db, wipes[w], PMM_NOT_FOUND, &del, NULL) != SQLITE_OK) {
    ...
    int count = 0;
    if (pmm_store_coverage_get(s, project, &rows, &count) != PMM_STORE_OK) {
    ...
    if (pmm_store_upsert_project(s, covproj, "") != PMM_STORE_OK) {
        pmm_store_free_coverage(rows, count);
        return PMM_STORE_ERR;
    }

    pmm_node_t root = {.project = covproj,
                       .label = "Project",
                       .name = project,
                       .qualified_name = covproj,
                       .properties_json = "{}"};
    int64_t root_id = pmm_store_upsert_node(s, &root);
    if (root_id <= 0) {
        pmm_store_free_coverage(rows, count);
        return PMM_STORE_ERR;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (char *p = pathbuf; *p; p++) {
            ...
            *p = '\0';
            ...
            pmm_node_t folder = {.project = covproj,
                                 .label = "Folder",
                                 .name = seg ? seg + 1 : pathbuf,
                                 .qualified_name = pathbuf,
                                 .file_path = pathbuf,
                                 .properties_json = "{}"};
            int64_t fid = pmm_store_upsert_node(s, &folder);
            *p = '/';
            if (fid <= 0) {
                pmm_store_free_coverage(rows, count);
                return PMM_STORE_ERR;
            }
            pmm_edge_t e = {.project = covproj,
                            .source_id = parent,
                            .target_id = fid,
                            .type = "CONTAINS_FOLDER",
                            .properties_json = "{}"};
            if (pmm_store_insert_edge(s, &e) <= 0) {
                pmm_store_free_coverage(rows, count);
                return PMM_STORE_ERR;
            }
            parent = fid;
        ...
        pmm_node_t file = {.project = covproj,
                           .label = "File",
                           .name = base ? base + 1 : rel,
                           .qualified_name = rel,
                           .file_path = rel,
                           .properties_json = props};
        int64_t file_id = pmm_store_upsert_node(s, &file);
        if (file_id <= 0) {
            pmm_store_free_coverage(rows, count);
            return PMM_STORE_ERR;
        }
...
int pmm_store_coverage_replace_ex(pmm_store_t *s, const char *project,
    ...
    sqlite3_stmt *del = NULL;
    if (sqlite3_prepare_v2(s->db, "DELETE FROM index_coverage WHERE project = ?1;", PMM_NOT_FOUND,
                           &del, NULL) != SQLITE_OK) {
    ...
    if (sqlite3_prepare_v2(
            ...
            "VALUES (?1, ?2, ?3, ?4);",
            PMM_NOT_FOUND, &ins, NULL) != SQLITE_OK) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        bind_text(ins, PMM_SZ_4, rows[i].detail ? rows[i].detail : "");
    ...
    if (sqlite3_prepare_v2(s->db,
                           ...
                           PMM_NOT_FOUND, &prune, NULL) != SQLITE_OK) {
    ...
    if (meta) {
        ...
        if (sqlite3_prepare_v2(
                ...
                PMM_NOT_FOUND, &up_meta, NULL) != SQLITE_OK) {
        ...
        bind_text(up_meta, PMM_SZ_4, recorded_at);
        bind_text(up_meta, PMM_SZ_5, recording_status);
        ...
        if (sqlite3_prepare_v2(s->db, "DELETE FROM index_coverage_meta WHERE project = ?1;",
                               PMM_NOT_FOUND, &del_meta, NULL) != SQLITE_OK) {
    ...
    if (cov_rebuild_shadow_graph(s, project) != PMM_STORE_OK) {
        (void)exec_sql(s, "ROLLBACK;");
        return PMM_STORE_ERR;
    }
    return exec_sql(s, "COMMIT;");
...
int pmm_store_coverage_replace(pmm_store_t *s, const char *project, const pmm_coverage_row_t *rows,
                               int count) {
    return pmm_store_coverage_replace_ex(s, project, rows, count, NULL);
}

static int coverage_query_rows(pmm_store_t *s, const char *project, const char *selector,
                               const char *sql, pmm_coverage_row_t **out, int *count) {
    if (!out || !count) {
        return PMM_STORE_ERR;
    }
    *out = NULL;
    *count = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, sql, PMM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    pmm_coverage_row_t *arr = malloc((size_t)cap * sizeof(*arr));
    if (!arr) {
        sqlite3_finalize(stmt);
        return PMM_STORE_ERR;
    }
    ...
    while ((scan_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        if (!arr[n].rel_path || !arr[n].kind || !arr[n].detail) {
            sqlite3_finalize(stmt);
            pmm_store_free_coverage(arr, n + 1);
            return PMM_STORE_ERR;
        }
        n++;
    }
...
int pmm_store_coverage_meta_get(pmm_store_t *s, const char *project, pmm_coverage_meta_t *out) {
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->recording_status = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_4));
        out->ignored_files_stored = sqlite3_column_int(stmt, PMM_SZ_5);
        ...
        if (!out->project || !out->generation || !out->index_mode || !out->recorded_at ||
            ...
            pmm_store_coverage_meta_clear(out);
            return PMM_STORE_ERR;
        }
        return PMM_STORE_OK;
    }
...
int pmm_store_coverage_get(pmm_store_t *s, const char *project, pmm_coverage_row_t **out,
    ...
    *count = 0;
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    pmm_coverage_row_t *arr = malloc(cap * sizeof(pmm_coverage_row_t));
    int scan_rc6;
    while ((scan_rc6 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_coverage_row_t));
        }
        arr[n].rel_path = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].kind = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].detail = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_2));
        n++;
    }
...
int pmm_store_find_nodes_by_file_overlap(pmm_store_t *s, const char *project, const char *file_path,
    ...
    *count = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    int rc = sqlite3_prepare_v2(s->db, sql, PMM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    pmm_node_t *nodes = malloc(cap * sizeof(pmm_node_t));
    int scan_rc7;
    while ((scan_rc7 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            nodes = safe_realloc(nodes, cap * sizeof(pmm_node_t));
        }
        memset(&nodes[n], 0, sizeof(pmm_node_t));
...
int pmm_store_find_nodes_by_qn_suffix(pmm_store_t *s, const char *project, const char *suffix,
    ...
    *count = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    int rc =
        sqlite3_prepare_v2(s->db, project ? sql_with_project : sql_any, PMM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
...
void pmm_store_node_degree(pmm_store_t *s, int64_t node_id, int *in_deg, int *out_deg) {
    ...
    *out_deg = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, in_sql, PMM_NOT_FOUND, &stmt, NULL) == SQLITE_OK) {
    ...
    if (sqlite3_prepare_v2(s->db, out_sql, PMM_NOT_FOUND, &stmt, NULL) == SQLITE_OK) {
        sqlite3_bind_int64(stmt, SKIP_ONE, node_id);
        if (sqlite3_step(stmt) == SQLITE_ROW) {
...
int pmm_store_list_files(pmm_store_t *s, const char *project, char ***out, int *count) {
    ...
    *count = 0;
    ...
    sqlite3_bind_text(stmt, SKIP_ONE, project, PMM_NOT_FOUND, SQLITE_STATIC);
...
static int query_neighbor_names(sqlite3 *db, const char *sql, int64_t node_id, int limit,
    ...
    *out_count = 0;
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db, sql, PMM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
...
static int count_degrees_direction(pmm_store_t *s, const int64_t *node_ids, int id_count,
...
int pmm_store_upsert_file_hash_batch(pmm_store_t *s, const pmm_file_hash_t *hashes, int count) {
    ...
    int rc = pmm_store_begin(s);
    if (rc != PMM_STORE_OK) {
        return rc;
    }

    for (int i = 0; i < count; i++) {
        rc = pmm_store_upsert_file_hash(s, hashes[i].project, hashes[i].rel_path, hashes[i].sha256,
...
int pmm_store_find_edges_by_url_path(pmm_store_t *s, const char *project, const char *keyword,
    ...
    *count = 0;
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    pmm_edge_t *edges = malloc(cap * sizeof(pmm_edge_t));
    int scan_rc12;
    while ((scan_rc12 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            edges = safe_realloc(edges, cap * sizeof(pmm_edge_t));
        }
        memset(&edges[n], 0, sizeof(pmm_edge_t));
...
int pmm_store_restore_from(pmm_store_t *dst, pmm_store_t *src) {
    ...
    int rc = sqlite3_backup_step(bk, PMM_NOT_FOUND); /* copy all pages */
    sqlite3_backup_finish(bk);
...
static void search_apply_degree_filter(char *sql, size_t sql_sz, const pmm_search_params_t *p) {
    bool has_degree_filter = (p->min_degree >= 0 || p->max_degree >= 0);
    if (!has_degree_filter) {
        return;
    }
    char inner_sql[PMM_SZ_4K];
 ...
 * pmm_extract_like_hints bails on alternation, so no false negatives. */
static void where_add_like_hints(const char *column, const char *pattern, char *where, int where_sz,
    ...
    int nhints = pmm_extract_like_hints(pattern, hints, ST_LIKE_HINT_MAX);
    char bind_buf[PMM_SZ_64];
...
static int search_where_basic(const pmm_search_params_t *params, char *where, int where_sz,
                              int *wlen, int *nparams, search_bind_t *binds, int *bind_idx,
                              search_like_pool_t *pool) {
    char bind_buf[PMM_SZ_64];
    ...
    if (params->file_pattern) {
        char *lp = pmm_glob_to_like(params->file_pattern);
...
static void search_where_advanced(const pmm_search_params_t *params, char *where, int where_sz,
                                  int *wlen, int *nparams, search_bind_t *binds, int *bind_idx) {
    if (params->relationship) {
        char rel_clause[PMM_SZ_256];
...
static int search_build_where(const pmm_search_params_t *params, char *where, int where_sz,
...
int pmm_store_search(pmm_store_t *s, const pmm_search_params_t *params, pmm_search_output_t *out) {
    ...
    int rc = sqlite3_prepare_v2(s->db, count_sql, PMM_NOT_FOUND, &cnt_stmt, NULL);
    if (rc == SQLITE_OK) {
    ...
    rc = sqlite3_prepare_v2(s->db, sql, PMM_NOT_FOUND, &main_stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    pmm_search_result_t *results = malloc(cap * sizeof(pmm_search_result_t));

    int scan_rc13;
    while ((scan_rc13 = sqlite3_step(main_stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            results = safe_realloc(results, cap * sizeof(pmm_search_result_t));
        }
        memset(&results[n], 0, sizeof(pmm_search_result_t));
        scan_node(main_stmt, &results[n].node);
        results[n].in_degree = sqlite3_column_int(main_stmt, ST_COL_9);
        results[n].out_degree = sqlite3_column_int(main_stmt, PMM_DECIMAL_BASE);
        n++;
    }
...
void pmm_store_search_free(pmm_search_output_t *out) {
    ...
    for (int i = 0; i < out->count; i++) {
        pmm_search_result_t *r = &out->results[i];
...
static int bfs_collect_edges(pmm_store_t *s, int64_t start_id, const pmm_node_hop_t *visited,
                             int visited_count, const char *types_clause, const char **edge_types,
                             int edge_type_count, pmm_edge_info_t **out_edges,
                             int *out_edge_count) {
    *out_edges = NULL;
    *out_edge_count = 0;
    ...
    if (sqlite3_exec(s->db,
        ...
        return PMM_STORE_OK; /* best-effort: nodes without edges beat an error */
    }
    sqlite3_stmt *ins = NULL;
    if (sqlite3_prepare_v2(s->db, "INSERT OR IGNORE INTO bfs_ids(id) VALUES (?1)", PMM_NOT_FOUND,
                           &ins, NULL) != SQLITE_OK) {
        return PMM_STORE_OK;
    }
    ...
    int rc = sqlite3_prepare_v2(s->db, edge_sql, PMM_NOT_FOUND, &estmt, NULL);
    if (rc != SQLITE_OK) {
        *out_edges = NULL;
        *out_edge_count = 0;
        return PMM_STORE_OK;
    ...
    int en = 0;
    pmm_edge_info_t *edges = malloc(ecap * sizeof(pmm_edge_info_t));

    int scan_rc14;
    while ((scan_rc14 = sqlite3_step(estmt)) == SQLITE_ROW) {
        if (en >= ecap) {
            ecap *= ST_GROWTH;
            edges = safe_realloc(edges, ecap * sizeof(pmm_edge_info_t));
        }
        ...
        edges[en].type = heap_strdup((const char *)sqlite3_column_text(estmt, PMM_SZ_2));
        ...
        edges[en].properties_json = heap_strdup((const char *)sqlite3_column_text(estmt, PMM_SZ_5));
        en++;
    }
...
int pmm_store_bfs(pmm_store_t *s, int64_t start_id, const char *direction, const char **edge_types,
    ...
    pmm_node_t root = {0};
    int rc = pmm_store_find_node_by_id(s, start_id, &root);
    if (rc != PMM_STORE_OK) {
        return rc;
    }
    out->root = root;

    char types_clause[PMM_SZ_512];
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(s->db, sql, PMM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    pmm_node_hop_t *visited = malloc(cap * sizeof(pmm_node_hop_t));

    int scan_rc15;
    while ((scan_rc15 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            visited = safe_realloc(visited, cap * sizeof(pmm_node_hop_t));
        }
...
int pmm_store_bfs_multi(pmm_store_t *s, const int64_t *seed_ids, int seed_count,
    ...
    sqlite3_stmt *ins = NULL;
    if (sqlite3_prepare_v2(s->db, "INSERT OR IGNORE INTO bfs_seeds(id) VALUES (?1)", PMM_NOT_FOUND,
                           &ins, NULL) != SQLITE_OK) {
    ...
    if (n > max_results) {
        ...
        n = max_results;
        pmm_node_free_fields(&visited[n].node);
        if (truncated) {
            *truncated = true;
        }
    }
...
void pmm_store_traverse_free(pmm_traverse_result_t *out) {
    ...
    for (int i = 0; i < out->visited_count; i++) {
        pmm_node_hop_t *h = &out->visited[i];
...
pmm_risk_level_t pmm_hop_to_risk(int hop) {
    switch (hop) {
    case SKIP_ONE:
        return PMM_RISK_CRITICAL;
    case ST_COL_2:
        return PMM_RISK_HIGH;
    case ST_COL_3:
        return PMM_RISK_MEDIUM;
    default:
        return PMM_RISK_LOW;
    }
}
...
const char *pmm_risk_label(pmm_risk_level_t level) {
    switch (level) {
    case PMM_RISK_CRITICAL:
        return "CRITICAL";
    case PMM_RISK_HIGH:
        return "HIGH";
    case PMM_RISK_MEDIUM:
        return "MEDIUM";
    case PMM_RISK_LOW:
    default:
        return "LOW";
...
pmm_impact_summary_t pmm_build_impact_summary(const pmm_node_hop_t *hops, int hop_count,
    ...
    pmm_impact_summary_t s = {0};
    for (int i = 0; i < hop_count; i++) {
        switch (pmm_hop_to_risk(hops[i].hop)) {
        case PMM_RISK_CRITICAL:
            s.critical++;
            break;
        case PMM_RISK_HIGH:
...
int pmm_deduplicate_hops(const pmm_node_hop_t *hops, int hop_count, pmm_node_hop_t **out,
    ...
    *out_count = 0;
    ...
    pmm_node_hop_t *result = malloc(hop_count * sizeof(pmm_node_hop_t));
    int n = 0;
    ...
    *out = safe_realloc(result, n * sizeof(pmm_node_hop_t));
    *out_count = n;
    return PMM_STORE_OK;
}
...
static void schema_discover_props(sqlite3 *db, const char *sql, const char *project,
    ...
    if (sqlite3_prepare_v2(db, sql, PMM_NOT_FOUND, &pstmt, NULL) == SQLITE_OK) {
...
bool pmm_store_arch_path_scoped(const char *path) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512 + 4];
...
int pmm_store_count_edges_scoped(pmm_store_t *s, const char *project, const char *path) {
    ...
    char like[PMM_SZ_512];
    if (!arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like))) {
        return pmm_store_count_edges(s, project);
    }
    const char *sql =
...
static int get_schema_impl(pmm_store_t *s, const char *project, pmm_schema_info_t *out,
                           bool with_props) {
    memset(out, 0, sizeof(*out));
    if (!s || !s->db) {
        return PMM_NOT_FOUND;
    ...
    /* Node labels */
    {
        ...
        int cap = ST_INIT_CAP_8;
        int n = 0;
        pmm_label_count_t *arr = malloc(cap * sizeof(pmm_label_count_t));
        if (!arr) {
            sqlite3_finalize(stmt);
            return PMM_NOT_FOUND;
        }
        ...
        while ((scan_rc16 = sqlite3_step(stmt)) == SQLITE_ROW) {
            if (n >= cap) {
                ...
                void *tmp = realloc(arr, new_cap * sizeof(pmm_label_count_t));
                if (!tmp) {
                    for (int i = 0; i < n; i++) {
    ...
    /* Edge types */
    {
        ...
        int cap = ST_INIT_CAP_8;
        int n = 0;
        pmm_type_count_t *arr = malloc(cap * sizeof(pmm_type_count_t));
        if (!arr) {
            sqlite3_finalize(stmt);
            pmm_store_schema_free(out);
            return PMM_NOT_FOUND;
        ...
        while ((scan_rc17 = sqlite3_step(stmt)) == SQLITE_ROW) {
            if (n >= cap) {
                int new_cap = cap * ST_GROWTH;
                void *tmp = realloc(arr, new_cap * sizeof(pmm_type_count_t));
                if (!tmp) {
                    for (int i = 0; i < n; i++) {
...
int pmm_store_get_schema_counts_scoped(pmm_store_t *s, const char *project, const char *path,
    ...
    char sqlbuf[ST_SQL_BUF];
    {
        ...
        snprintf(sqlbuf, sizeof(sqlbuf), "%s%s GROUP BY label ORDER BY COUNT(*) DESC;", base,
                 arch_path_scope_sql());
        sqlite3_stmt *stmt = NULL;
        if (sqlite3_prepare_v2(s->db, sqlbuf, PMM_NOT_FOUND, &stmt, NULL) != SQLITE_OK || !stmt) {
            if (stmt) {
                sqlite3_finalize(stmt);
            }
            return PMM_NOT_FOUND;
...

    {
        ...
        sqlite3_stmt *stmt = NULL;
        if (sqlite3_prepare_v2(s->db, esql, PMM_NOT_FOUND, &stmt, NULL) != SQLITE_OK || !stmt) {
            if (stmt) {
                sqlite3_finalize(stmt);
            }
            pmm_store_schema_free(out);
...
const char *pmm_qn_to_package(const char *qn) {
    ...
    static PMM_TLS char buf[PMM_SZ_256];
...
static const char *file_ext(const char *path) {
    ...
    static PMM_TLS char buf[PMM_SZ_16];
    int len = (int)strlen(dot);
    if (len >= (int)sizeof(buf)) {
        return NULL;
    }
    for (int i = 0; i < len; i++) {
        buf[i] = (char)((dot[i] >= 'A' && dot[i] <= 'Z') ? dot[i] + PMM_SZ_32 : dot[i]);
    }
    buf[len] = '\0';
    return buf;
}
...
static int arch_languages(pmm_store_t *s, const char *project, const char *path,
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, sqlbuf, PMM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    const char *lang_names[PMM_SZ_64];
    int lang_counts[PMM_SZ_64];
    int nlang = 0;
    ...
    if (nlang > PMM_DECIMAL_BASE) {
        nlang = ST_MAX_LANG;
    }

    out->languages = (nlang > 0) ? calloc(nlang, sizeof(pmm_language_count_t)) : NULL;
...
static int arch_entry_points(pmm_store_t *s, const char *project, const char *path,
                             pmm_architecture_info_t *out) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    pmm_entry_point_t *arr = calloc(cap, sizeof(pmm_entry_point_t));
    int scan_rc21;
    while ((scan_rc21 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_entry_point_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].file = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_2));
        n++;
    }
...
static char *extract_json_string_prop(const char *json, const char *key, int key_len) {
    ...
    if (!end || end - m >= PMM_SZ_256) {
        return NULL;
    }
    char vbuf[PMM_SZ_256];
    memcpy(vbuf, m, end - m);
    vbuf[end - m] = '\0';
    return heap_strdup(vbuf);
}

static int arch_routes(pmm_store_t *s, const char *project, const char *path,
                       pmm_architecture_info_t *out) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    pmm_route_info_t *arr = calloc(cap, sizeof(pmm_route_info_t));
    int scan_rc22;
    while ((scan_rc22 = sqlite3_step(stmt)) == SQLITE_ROW) {
        const char *name = (const char *)sqlite3_column_text(stmt, 0);
        const char *props = (const char *)sqlite3_column_text(stmt, SKIP_ONE);
        const char *fp = (const char *)sqlite3_column_text(stmt, PMM_SZ_2);
        if (pmm_is_test_file_path(fp)) {
            continue;
        }
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_route_info_t));
        }
...
static int arch_hotspots(pmm_store_t *s, const char *project, const char *path,
                         pmm_architecture_info_t *out) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    pmm_hotspot_t *arr = calloc(cap, sizeof(pmm_hotspot_t));
    int scan_rc23;
    while ((scan_rc23 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_hotspot_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].fan_in = sqlite3_column_int(stmt, PMM_SZ_2);
        n++;
    }
...
static int arch_boundaries(pmm_store_t *s, const char *project, const char *path,
                           pmm_cross_pkg_boundary_t **out_arr, int *out_count) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    if (sqlite3_prepare_v2(s->db, nsqlbuf, PMM_NOT_FOUND, &nstmt, NULL) != SQLITE_OK) {
    ...
    while ((scan_rc24 = sqlite3_step(nstmt)) == SQLITE_ROW) {
        ...
        npkgs[nn] = heap_strdup(pmm_qn_to_package(qn));
        nn++;
    }
    ...
    sqlite3_stmt *estmt = NULL;
    if (sqlite3_prepare_v2(s->db, esql, PMM_NOT_FOUND, &estmt, NULL) != SQLITE_OK) {
        for (int i = 0; i < nn; i++) {
            free(npkgs[i]);
        }
        free(nids);
    ...
    pmm_cross_pkg_boundary_t *result =
        (bn > 0) ? calloc(bn, sizeof(pmm_cross_pkg_boundary_t)) : NULL;
    for (int i = 0; i < bn; i++) {
...
static int arch_packages_from_qn(pmm_store_t *s, const char *project, const char *path,
                                 pmm_package_summary_t **out_arr, int *out_count) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, qsqlbuf, PMM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    pmm_package_summary_t *arr = (np > 0) ? calloc(np, sizeof(pmm_package_summary_t)) : NULL;
    for (int i = 0; i < np; i++) {
        arr[i].name = pnames[i];
...
static int arch_packages(pmm_store_t *s, const char *project, const char *path,
                         pmm_architecture_info_t *out) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    pmm_package_summary_t *arr = calloc(cap, sizeof(pmm_package_summary_t));
    int scan_rc27;
    while ((scan_rc27 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(pmm_package_summary_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
...
static void classify_layer(const char *pkg, int in, int out_deg, bool has_routes,
    ...
    static PMM_TLS char reason_buf[PMM_SZ_128];
...
static int collect_pkg_names(pmm_store_t *s, const char *sql, const char *project, const char *path,
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW && count < max_pkgs) {
        const char *qn = (const char *)sqlite3_column_text(stmt, 0);
        pkgs[count++] = heap_strdup(pmm_qn_to_package(qn));
    }
    sqlite3_finalize(stmt);
    return count;
}

static int arch_layers(pmm_store_t *s, const char *project, const char *path,
                       pmm_architecture_info_t *out) {
    /* Get boundaries for fan analysis */
    pmm_cross_pkg_boundary_t *boundaries = NULL;
    int bcount = 0;
    ...
    int nepkgs = collect_pkg_names(s,
                                   ...
                                   project, path, entry_pkgs, PMM_SZ_32);

    /* Compute fan-in/out per package */
    char *all_pkgs[PMM_SZ_64];
    int fan_in[PMM_SZ_64];
    int fan_out[PMM_SZ_64];
    int npkgs = 0;
    ...
    out->layers = (npkgs > 0) ? calloc(npkgs, sizeof(pmm_package_layer_t)) : NULL;
    ...
    for (int i = SKIP_ONE; i < npkgs; i++) {
        ...
        while (j > 0 && strcmp(out->layers[j].name, out->layers[j - SKIP_ONE].name) < 0) {
            pmm_package_layer_t tmp = out->layers[j];
...
static pmm_file_tree_entry_t make_tree_entry(const char *path, char **files, int fn,
                                             char **dir_paths, const int *dir_child_counts,
                                             int dn) {
    pmm_file_tree_entry_t e = {0};
...
static void push_tree_entry(pmm_file_tree_entry_t **entries, int *en, int *ecap,
                            pmm_file_tree_entry_t e) {
    if (*en >= *ecap) {
        *ecap *= ST_GROWTH;
        *entries = safe_realloc(*entries, *ecap * sizeof(pmm_file_tree_entry_t));
    }
    (*entries)[(*en)++] = e;
}
...
static void arch_collect_entries(char **dir_paths, int *dir_child_counts, char ***dir_children,
                                 int dn, char **files, int fn, pmm_file_tree_entry_t **entries_out,
                                 int *en_out) {
    int ecap = PMM_SZ_64;
    int en = 0;
    pmm_file_tree_entry_t *entries = calloc(ecap, sizeof(pmm_file_tree_entry_t));
    ...
    for (int i = SKIP_ONE; i < en; i++) {
        ...
        while (j > 0 && strcmp(entries[j].path, entries[j - SKIP_ONE].path) < 0) {
            pmm_file_tree_entry_t tmp = entries[j];
...
static int arch_file_tree(pmm_store_t *s, const char *project, const char *path,
                          pmm_architecture_info_t *out) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
...
static void louvain_build_weights(const int64_t *nodes, int n, const pmm_louvain_edge_t *edges,
...
static void lg_free(pmm_lg_t *g) {
    free(g->off);
    free(g->nbr);
    free(g->w);
    free(g->k);
 ...
 * comm[] in place. */
static void leiden_move(const pmm_lg_t *g, int *comm, double gamma, double twom) {
    int n = g->n;
...
static int leiden_refine(const pmm_lg_t *g, const int *comm, double gamma, double twom,
                         int *refined) {
    int n = g->n;
    double *stot = calloc((size_t)n, sizeof(double));
    double *acc = calloc((size_t)n, sizeof(double));
...
static int leiden_aggregate(const pmm_lg_t *g, const int *refined, int r_count, const int *comm,
                            pmm_lg_t *out, int *seed) {
    int n = g->n;
    ...
    for (int r = 0; r < r_count; r++) {
        seed[r] = PMM_NOT_FOUND;
    }
    for (int i = 0; i < n; i++) {
        int r = refined[i];
        k2[r] += g->k[i];
        gcount[r]++;
        if (seed[r] == PMM_NOT_FOUND) {
            seed[r] = comm[i];
        }
    }
    gstart[0] = 0;
...
int pmm_leiden(const int64_t *nodes, int node_count, const pmm_louvain_edge_t *edges,
    ...
    pmm_louvain_result_t *result = malloc((size_t)n * sizeof(*result));
    if (!result) {
        return PMM_NOT_FOUND;
    }
    for (int i = 0; i < n; i++) {
    ...
    pmm_lg_t g;
    int built = (wn > 0) ? lg_build(n, wsi, wdi, ww, wn, &g) : PMM_NOT_FOUND;
    free(wsi);
    free(wdi);
    free(ww);
    if (built != PMM_STORE_OK) {
    ...
    for (int level = 0; level < LEIDEN_MAX_LEVELS; level++) {
        ...
        if (!seed || leiden_aggregate(&g, refined, r_count, comm, &g2, seed) != PMM_STORE_OK) {
            free(seed);
            free(refined);
            break;
        }
...
int pmm_louvain(const int64_t *nodes, int node_count, const pmm_louvain_edge_t *edges,
    ...
    return pmm_leiden(nodes, node_count, edges, edge_count, 1.0, out, out_count);
}

/* ── Architecture: community clusters via Leiden ───────────────── */

enum {
    PMM_CLUSTER_TOP_N = 12,       /* report at most this many clusters */
    PMM_CLUSTER_MAX_TOPNODES = 5, /* representative node names per cluster */
    PMM_CLUSTER_MAX_PKGS = 5,     /* packages listed per cluster */
    PMM_CLUSTER_MIN_MEMBERS = 2,  /* skip singletons */
    PMM_CLUSTER_NODE_CAP = 8000   /* bound the work for very large graphs */
};
...
/* Index of node id within the id-sorted ids[], or PMM_NOT_FOUND. */
static int cluster_id_index(const int64_t *ids, int n, int64_t id) {
    ...
    return hit ? (int)(hit - ids) : PMM_NOT_FOUND;
}
...
static void cluster_build_one(pmm_cluster_info_t *ci, int c, int n, const int *comm,
    ...
    /* Top nodes by degree. */
    int top_idx[PMM_CLUSTER_MAX_TOPNODES];
    int top_deg[PMM_CLUSTER_MAX_TOPNODES];
    int tn = 0;
    for (int i = 0; i < n; i++) {
        if (comm[i] != c) {
            continue;
        ...
        if (pos < PMM_CLUSTER_MAX_TOPNODES) {
            int last = (tn < PMM_CLUSTER_MAX_TOPNODES) ? tn : PMM_CLUSTER_MAX_TOPNODES - 1;
            ...
            top_idx[pos] = i;
            top_deg[pos] = d;
            if (tn < PMM_CLUSTER_MAX_TOPNODES) {
                tn++;
            }
        }
    }
    ...
    const char *pkgs[PMM_CLUSTER_MAX_PKGS];
    int pkg_counts[PMM_CLUSTER_MAX_PKGS];
    int pc = 0;
    for (int i = 0; i < n; i++) {
        if (comm[i] == c) {
            cluster_add_pkg(pkgs, pkg_counts, &pc, PMM_CLUSTER_MAX_PKGS,
                            pmm_qn_to_top_package(qns[i]));
        }
    }
    if (pc > 0) {
...
static int arch_clusters(pmm_store_t *s, const char *project, const char *path,
                         pmm_architecture_info_t *out) {
    char norm[PMM_SZ_512];
    char like[PMM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    if (sqlite3_prepare_v2(s->db, nsqlbuf, PMM_NOT_FOUND, &st, NULL) != SQLITE_OK) {
        return PMM_STORE_OK; /* clusters are best-effort */
    }
    bind_text(st, SKIP_ONE, project);
    if (scoped) {
        arch_bind_path_scope(st, ST_COL_2, ST_COL_3, norm, like);
        sqlite3_bind_int(st, ST_COL_4, PMM_CLUSTER_NODE_CAP);
    } else {
        sqlite3_bind_int(st, PMM_SZ_2, PMM_CLUSTER_NODE_CAP);
    }
    int cap = ST_INIT_CAP_8;
    int n = 0;
    ...
    while ((scan_rc29 = sqlite3_step(st)) == SQLITE_ROW) {
        ...
        qns[n] = heap_strdup((const char *)sqlite3_column_text(st, PMM_SZ_2));
        n++;
    }
    ...
    pmm_louvain_edge_t *edges = malloc((size_t)n * sizeof(pmm_louvain_edge_t));
    int *esrc = malloc((size_t)n * sizeof(int));
    int *edst = malloc((size_t)n * sizeof(int));
    int *degree = calloc((size_t)n, sizeof(int));
    int ne = 0;
    const char *esql = "SELECT source_id, target_id FROM edges WHERE project=?1 AND type='CALLS'";
    if (sqlite3_prepare_v2(s->db, esql, PMM_NOT_FOUND, &st, NULL) == SQLITE_OK) {
        int ecap = n;
        bind_text(st, SKIP_ONE, project);
        int scan_rc30;
        while ((scan_rc30 = sqlite3_step(st)) == SQLITE_ROW) {
            ...
            if (ne >= ecap) {
                ecap *= ST_GROWTH;
                edges = safe_realloc(edges, (size_t)ecap * sizeof(pmm_louvain_edge_t));
    ...
    pmm_louvain_result_t *res = NULL;
    int rn = 0;
    int *comm = NULL;
    int C = 0;
    if (pmm_leiden(ids, n, edges, ne, 1.0, &res, &rn) == PMM_STORE_OK && res && rn == n) {
    ...
    if (comm && C > 0) {
        ...
        pmm_cluster_info_t *clusters =
            malloc((size_t)PMM_CLUSTER_TOP_N * sizeof(pmm_cluster_info_t));
        int cc = 0;
        for (int r = 0; r < C && cc < PMM_CLUSTER_TOP_N; r++) {
            int c = rank[r].comm;
            if (members[c] < PMM_CLUSTER_MIN_MEMBERS) {
...
static bool want_aspect(const char **aspects, int aspect_count, const char *name) {
    ...
    for (int i = 0; i < aspect_count; i++) {
        ...
        if (strcmp(aspects[i], "overview") == 0 && pmm_store_arch_aspect_in_overview(name)) {
            return true;
        }
...
int pmm_store_get_architecture(pmm_store_t *s, const char *project, const char *path,
    ...
    if (want_aspect(aspects, aspect_count, "boundaries")) {
        pmm_cross_pkg_boundary_t *barr = NULL;
        int bcount = 0;
...
static void adr_save_section(pmm_adr_sections_t *result, char *section_name, char *buf,
                             int buf_len) {
...
int pmm_adr_validate_content(const char *content, char *errbuf, int errbuf_size) {
    pmm_adr_sections_t sections = pmm_adr_parse_sections(content);
    char missing[PMM_SZ_256] = "";
    int mlen = 0;
    int nmissing = 0;
    ...
    pmm_adr_sections_free(&sections);
...
int pmm_store_adr_get(pmm_store_t *s, const char *project, pmm_adr_t *out) {
    ...
    out->created_at = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_2));
    out->updated_at = heap_strdup((const char *)sqlite3_column_text(stmt, PMM_SZ_3));
    sqlite3_finalize(stmt);
    return PMM_STORE_OK;
}
...
int pmm_store_adr_update_sections(pmm_store_t *s, const char *project, const char **keys,
    ...
    /* Get existing ADR */
    pmm_adr_t existing;
    int rc = pmm_store_adr_get(s, project, &existing);
    if (rc != PMM_STORE_OK) {
    ...
    pmm_adr_sections_t sections = pmm_adr_parse_sections(existing.content);
    ...
    /* Render merged */
    char *merged = pmm_adr_render(&sections);
    pmm_adr_sections_free(&sections);

    /* Check length */
    if ((int)strlen(merged) > PMM_ADR_MAX_LENGTH) {
        char msg[PMM_SZ_128];
        snprintf(msg, sizeof(msg), "merged ADR exceeds %d chars (%d chars)", PMM_ADR_MAX_LENGTH,
                 (int)strlen(merged));
    ...
    /* Store merged */
    rc = pmm_store_adr_store(s, project, merged);
    free(merged);
    if (rc != PMM_STORE_OK) {
        return rc;
    }
...
void pmm_store_free_projects(pmm_project_t *projects, int count) {
    ...
    for (int i = 0; i < count; i++) {
        pmm_project_free_fields(&projects[i]);
    }
    free(projects);
}
...
static bool vs_load_enriched_vector(pmm_store_t *s, const char *project, const char *token,
                                    float *out) {
    ...
    if (sqlite3_step(tv_stmt) == SQLITE_ROW) {
        ...
        if (vec && vec_len == VS_VEC_DIM) {
            for (int d = 0; d < VS_VEC_DIM; d++) {
                out[d] = (float)vec[d] / PMM_STORE_INT8_MAX;
            }
            found = true;
        }
    }
...
static void vs_fill_sparse_random(const char *token, float *out) {
    ...
    for (int i = 0; i < VS_SPARSE_NNZE; i++) {
        ...
        float sign = (h & SKIP_ONE) ? PMM_STORE_UNIT_POS_F : -PMM_STORE_UNIT_POS_F;
        out[pos] += sign;
    }
}
...
static int8_t vs_clamp_int8(float v) {
    if (v > PMM_STORE_INT8_MAX) {
        return (int8_t)PMM_STORE_INT8_MAX;
    }
    if (v < -PMM_STORE_INT8_MAX) {
        return (int8_t)-PMM_STORE_INT8_MAX;
    }
...
static bool vs_normalize_and_quantize(const float *src, int8_t *dst) {
    ...
    mag = sqrtf(mag);
    if (mag < PMM_STORE_DENOM_EPS_F) {
        return false;
    }
    float inv = PMM_STORE_UNIT_POS_F / mag;
    for (int d = 0; d < VS_VEC_DIM; d++) {
        dst[d] = vs_clamp_int8(src[d] * inv * PMM_STORE_INT8_MAX);
    }
    return true;
}
...
static int vs_build_keyword_vectors(pmm_store_t *s, const char *project, const char **keywords,
...
static double vs_min_cosine_score(const int8_t *node_vec, int node_vec_len,
    ...
    double min_score = PMM_STORE_UNIT_POS_D;
    for (int k = 0; k < actual_kw; k++) {
        ...
        double cos_k = denom > PMM_STORE_DENOM_EPS_D ? (double)dot / denom : 0.0;
        if (cos_k < min_score) {
            min_score = cos_k;
        }
    }
...
static pmm_vector_result_t *vs_append_result(pmm_vector_result_t *results, int *count, int *cap,
                                             sqlite3_stmt *stmt,
                                             const int8_t (*kw_vecs)[VS_VEC_DIM], int actual_kw) {
    if (*count >= *cap) {
        int nc = *cap < PMM_SZ_16 ? PMM_SZ_16 : *cap * ST_COL_2;
        pmm_vector_result_t *grown = realloc(results, (size_t)nc * sizeof(pmm_vector_result_t));
        if (!grown) {
            return NULL;
        }
        results = grown;
...
int pmm_store_vector_search(pmm_store_t *s, const char *project, const char **keywords,
    ...
    *out_count = 0;
    ...
    const char *sql = "SELECT n.id, n.name, n.qualified_name, n.file_path, n.label,"
                      "       pmm_cosine_i8(v.vector, ?1) as score, v.vector"
                      " FROM node_vectors v"
    ...
    int fetch_limit = (limit > 0 ? limit : PMM_SZ_16) * ST_COL_5;
...

    {
        ...
        pmm_log_info("vector_search.exec", "kw_count", kw_buf, "fetch_limit", fl_buf, "project",
                     project);
    }

    pmm_vector_result_t *results = NULL;
    int count = 0;
    int cap = 0;
    int step_rc = 0;
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        pmm_vector_result_t *grown =
    ...
    if (step_rc != SQLITE_DONE) {
        ...
        pmm_log_warn("vector_search.step_error", "rc", rc_buf, "msg", sqlite3_errmsg(s->db));
    }
    {
        char cnt_buf[VS_STR_BUF];
        snprintf(cnt_buf, sizeof(cnt_buf), "%d", count);
        pmm_log_info("vector_search.done", "candidates", cnt_buf);
    }
    sqlite3_finalize(stmt);
    ...
    for (int i = 0; i < count - SKIP_ONE; i++) {
        for (int j = i + SKIP_ONE; j < count; j++) {
            if (results[j].score > results[i].score) {
                pmm_vector_result_t tmp = results[i];
                results[i] = results[j];
                results[j] = tmp;
            }
        }
    ...
    int final_limit = limit > 0 ? limit : PMM_SZ_16;
