...
static float g_rsq_diag[PMM_RSQ_DIM];
enum {
...
static void rsq_fwht(float *v) {
    for (int len = 1; len < PMM_RSQ_DIM; len <<= 1) {
        for (int i = 0; i < PMM_RSQ_DIM; i += len << 1) {
...
void pmm_rsq_encode(const float *v, pmm_rsq_code_t *out) {
    ...
    float rot[PMM_RSQ_DIM];
    for (int d = 0; d < PMM_RSQ_IN_DIM; d++) {
        rot[d] = v[d] * g_rsq_diag[d];
    }
    for (int d = PMM_RSQ_IN_DIM; d < PMM_RSQ_DIM; d++) {
        rot[d] = 0.0F;
    }
    rsq_fwht(rot);
    ...
    /* Per-vector scalar quantization over [lo, hi] at PMM_RSQ_BITS. */
    float range = hi - lo;
    float step = range > 0.0F ? range / (float)PMM_RSQ_LEVELS : 1.0F;
    out->offset = lo;
    out->scale = step;
...
float pmm_rsq_ip(const pmm_rsq_code_t *a, const pmm_rsq_code_t *b) {
    ...
    int64_t dot = 0;
    for (int i = 0; i < PMM_RSQ_CODE_BYTES; i++) {
