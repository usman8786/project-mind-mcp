 ...
 */
#ifndef PMM_SEMANTIC_ROTSQ_H
#define PMM_SEMANTIC_ROTSQ_H

#include <stdint.h>

enum {
    PMM_RSQ_IN_DIM = 768,                 /* input dimension (PMM_SEM_DIM) */
    PMM_RSQ_DIM = 1024,                   /* padded pow2 rotation dimension */
    PMM_RSQ_BITS = 4,                     /* bits per coordinate */
    PMM_RSQ_LEVELS = 15,                  /* (1 << PMM_RSQ_BITS) - 1 */
    PMM_RSQ_CODE_BYTES = PMM_RSQ_DIM / 2, /* two 4-bit codes per byte */
};

typedef struct {
    uint8_t codes[PMM_RSQ_CODE_BYTES]; /* packed 4-bit codes, little nibble first */
    float scale;                       /* per-vector dequant scale  */
    float offset;                      /* per-vector dequant offset */
    int32_t code_sum;                  /* Σ codes (for the IP expansion) */
} pmm_rsq_code_t;

/* Encode a PMM_RSQ_IN_DIM float vector (zero-padded to PMM_RSQ_DIM, rotated,
 * quantized). Deterministic: the rotation is fixed at build time. */
void pmm_rsq_encode(const float *v, pmm_rsq_code_t *out);

/* Estimated inner product of the two ORIGINAL vectors from their codes.
 * Deterministic pure function of the codes. */
float pmm_rsq_ip(const pmm_rsq_code_t *a, const pmm_rsq_code_t *b);

/* Dequantize a code into the ROTATED space (PMM_RSQ_DIM floats). Note: this
 ...
 * coordinates. */
void pmm_rsq_decode(const pmm_rsq_code_t *c, float *out);

#endif /* PMM_SEMANTIC_ROTSQ_H */
