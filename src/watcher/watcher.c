...
typedef struct {
    ...
    pmm_subprocess_t *active_git;
    /* Room for Git's 64-hex SHA-256 object ID plus newline/NUL while reading. */
    char last_head[PMM_SZ_128]; /* git HEAD hash (committed baseline) */
    ...
    uint64_t first_missing_ms;  /* pmm_now_ms() of the streak's first miss (0 = no streak) */
    ...
    char pending_head[PMM_SZ_128]; /* HEAD observed at check time */
} project_state_t;
...
struct pmm_watcher {
    pmm_store_t *store;
    pmm_index_fn index_fn;
    void *user_data;
    CBMHashTable *projects; /* name → project_state_t* */
    pmm_mutex_t projects_lock;
    ...
    pmm_mutex_t coordination_lock;
    pmm_watcher_project_mutation_begin_fn mutation_begin;
    pmm_watcher_project_mutation_end_fn mutation_end;
    pmm_watcher_project_pruned_fn project_pruned;
...
#define PRUNE_GRACE_DEFAULT_S 600 /* 10 min; override: PMM_WATCHER_PRUNE_GRACE_S */
...
int pmm_watcher_poll_interval_ms(int file_count) {
    int ms = POLL_BASE_MS + ((file_count / POLL_FILE_STEP) * PMM_MSEC_PER_SEC);
    if (ms > POLL_MAX_MS) {
        ms = POLL_MAX_MS;
    }
    return ms;
...
static bool watcher_git_output_create(watcher_git_output_t *output) {
    ...
    int written =
        snprintf(output->path, sizeof(output->path), "%s/pmm-watcher-git-XXXXXX", pmm_tmpdir());
    if (written <= 0 || written >= (int)sizeof(output->path)) {
        output->path[0] = '\0';
        return false;
    }
    int descriptor = pmm_mkstemp(output->path);
    if (descriptor < 0) {
        output->path[0] = '\0';
        return false;
    }
...
static bool watcher_windows_git_candidate(const wchar_t *entry, size_t entry_length,
    ...
    if (entry_length == 0U || entry_length >= PMM_SZ_4K) {
        return false;
    }
    wchar_t directory[PMM_SZ_4K];
    ...
    wchar_t candidate[PMM_SZ_4K];
    int written =
        swprintf(candidate, PMM_SZ_4K, separator ? L"%lsgit.exe" : L"%ls\\git.exe", directory);
    if (written <= 0 || written >= PMM_SZ_4K) {
        return false;
    }
    ...
    char *utf8 = regular ? pmm_wide_to_utf8(normalized) : NULL;
    size_t utf8_length = utf8 ? strlen(utf8) : 0U;
    bool valid = utf8 && utf8_length > 0U && utf8_length < PMM_SZ_4K;
    if (valid) {
...
static bool watcher_resolve_git_executable(char output[PMM_SZ_4K]) {
    output[0] = '\0';
...
static watcher_git_status_t watcher_git_run(pmm_watcher_t *w, project_state_t *state,
    ...
    pmm_mutex_lock(&w->projects_lock);
    if (state->active_git) {
        publication_conflict = true;
        cancelled = pmm_subprocess_request_cancel(process);
    } else {
    ...
    pmm_mutex_unlock(&w->projects_lock);

    uint64_t started_at = pmm_now_ms();
    ...
    for (;;) {
        ...
        if (!cancelled && output_limit > 0) {
            int64_t observed_size = pmm_file_size(output->path);
            if (observed_size > 0 && (uint64_t)observed_size > output_limit) {
                output_exceeded = true;
                cancelled = pmm_subprocess_request_cancel(process);
            }
        }
        pmm_proc_poll_t polled = pmm_subprocess_poll(process, &result);
        if (polled == PMM_PROC_POLL_TERMINAL) {
            break;
        }
        if (polled == PMM_PROC_POLL_ERROR) {
            /* Valid owned handles have no ERROR transition. Fail closed, but
             * continue polling so the contained tree is never abandoned. */
            cancelled = pmm_subprocess_request_cancel(process) || cancelled;
        }
        pmm_usleep(WATCHER_GIT_POLL_US);
    }
    ...
    } else if (result.outcome != PMM_PROC_CLEAN) {
        status = WATCHER_GIT_COMMAND_FAILED;
    }

    if (status == WATCHER_GIT_OK && output_limit > 0) {
        int64_t final_size = pmm_file_size(output->path);
        if (final_size < 0) {
            status = WATCHER_GIT_SUPERVISION_FAILED;
    ...
    if (status == WATCHER_GIT_DEADLINE || status == WATCHER_GIT_OUTPUT_LIMIT ||
        status == WATCHER_GIT_SUPERVISION_FAILED) {
        pmm_log_warn("watcher.git.failed", "project", state->project_name, "reason",
                     status == WATCHER_GIT_DEADLINE
                         ? "deadline"
...
static watcher_git_status_t git_repo_status(pmm_watcher_t *w, project_state_t *state) {
    const char *argv[] = {"git", "-C", state->root_path, "rev-parse", "--git-dir", NULL};
    return watcher_git_run(w, state, argv, 0, NULL);
}

static watcher_git_status_t git_head(pmm_watcher_t *w, project_state_t *state, char *out,
                                     size_t out_size) {
    if (!out || out_size < 2) {
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
    ...
    FILE *file = pmm_fopen(output.path, "rb");
...
static watcher_git_status_t git_dirty_signature(pmm_watcher_t *w, project_state_t *state,
    ...
    *signature_out = 0;
    ...
    FILE *fp = pmm_fopen(output.path, "rb");
    if (!fp) {
        watcher_git_output_cleanup(&output);
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
...
static watcher_git_status_t git_file_count(pmm_watcher_t *w, project_state_t *state,
                                           int *count_out) {
    if (!count_out) {
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
...
static project_state_t *state_new(const char *name, const char *root_path) {
    project_state_t *s = calloc(PMM_ALLOC_ONE, sizeof(*s));
    if (!s) {
        return NULL;
    }
...
static bool defer_state_free(pmm_watcher_t *w, project_state_t *s) {
    if (w->pending_free_count >= w->pending_free_cap) {
        ...
        if (!tmp) {
            pmm_log_warn("watcher.unwatch.oom", "project", s->project_name);
            return false;
        }
        w->pending_free = tmp;
...
static root_status_t root_status(const char *root_path, int *out_errno) {
    ...
    *out_errno = errno;
    return pmm_watcher_root_missing_errno(errno) ? ROOT_MISSING : ROOT_UNCERTAIN;
}

/* Sustained-absence window (seconds) before a missing root may be pruned.
 * Generous default: 10 minutes. Override with PMM_WATCHER_PRUNE_GRACE_S
 ...
 * same convention as pmm_max_file_bytes in limits.c. */
static long prune_grace_s(void) {
    const char *raw = getenv("PMM_WATCHER_PRUNE_GRACE_S");
    if (raw && raw[0]) {
        errno = 0;
        char *end = NULL;
...
static const char *itoa_buf(int v) {
    static PMM_TLS char buf[PMM_SZ_32];
...
static bool watcher_unlink_cached_file(const char *project_name, const char *path,
    ...
    char errno_text[PMM_SZ_32];
    snprintf(errno_text, sizeof(errno_text), "%d", unlink_errno);
    pmm_log_warn("watcher.root_prune_delete_failed", "project", project_name, "artifact", artifact,
                 "path", path, "errno", errno_text);
    return false;
}

static bool delete_cached_project_db(const char *project_name) {
    if (!pmm_validate_project_name(project_name)) {
        return false;
    }
...
pmm_watcher_t *pmm_watcher_new(pmm_store_t *store, pmm_index_fn index_fn, void *user_data) {
    pmm_watcher_t *w = calloc(PMM_ALLOC_ONE, sizeof(*w));
    if (!w) {
        return NULL;
    }
    w->store = store;
    ...
    w->projects = pmm_ht_create(PMM_SZ_32);
    if (!w->projects) {
        free(w);
        return NULL;
    }
    pmm_mutex_init(&w->projects_lock);
    pmm_mutex_init(&w->coordination_lock);
...
void pmm_watcher_free(pmm_watcher_t *w) {
     ...
     * In production the caller should pmm_watcher_stop() + join first. */
    atomic_store(&w->stopped, 1);
    pmm_mutex_lock(&w->coordination_lock);
    pmm_mutex_lock(&w->projects_lock);
    pmm_ht_foreach(w->projects, free_state_entry, NULL);
    pmm_ht_free(w->projects);
    for (int i = 0; i < w->pending_free_count; i++) {
        state_free(w->pending_free[i]);
    }
    free(w->pending_free);
    pmm_mutex_unlock(&w->projects_lock);
    pmm_mutex_unlock(&w->coordination_lock);
    pmm_mutex_destroy(&w->projects_lock);
    pmm_mutex_destroy(&w->coordination_lock);
    free(w);
}
...
bool pmm_watcher_watch(pmm_watcher_t *w, const char *project_name, const char *root_path) {
    ...
    if (!s) {
        pmm_log_warn("watcher.watch.oom", "project", project_name, "path", root_path);
        return false;
    }
    ...
    project_state_t *old = pmm_ht_get(w->projects, project_name);
    ...
    if (old) {
        ...
        pmm_ht_delete(w->projects, project_name);
    }
    pmm_ht_set(w->projects, s->project_name, s);
    bool registered = pmm_ht_get(w->projects, project_name) == s;
    if (!registered && old) {
        ...
        pmm_ht_set(w->projects, old->project_name, old);
        if (pmm_ht_get(w->projects, project_name) == old) {
            w->pending_free[--w->pending_free_count] = NULL;
        } else {
            atomic_store_explicit(&old->registered, false, memory_order_release);
            if (old->active_git) {
                (void)pmm_subprocess_request_cancel(old->active_git);
            }
        }
    }
    if (registered && old) {
    ...
    if (!registered) {
        state_free(s);
        pmm_log_warn("watcher.watch.failed", "project", project_name, "reason", "registration");
        return false;
    }
    pmm_log_info("watcher.watch", "project", project_name, "path", root_path);
    return true;
}
...
void pmm_watcher_unwatch(pmm_watcher_t *w, const char *project_name) {
    ...
    bool removed = false;
    pmm_mutex_lock(&w->projects_lock);
    project_state_t *s = pmm_ht_get(w->projects, project_name);
    if (s && defer_state_free(w, s)) {
        ...
        if (s->active_git) {
            (void)pmm_subprocess_request_cancel(s->active_git);
        }
        pmm_ht_delete(w->projects, project_name);
        removed = true;
    }
    pmm_mutex_unlock(&w->projects_lock);
    if (removed) {
        pmm_log_info("watcher.unwatch", "project", project_name);
    }
}
...
int pmm_watcher_watch_count(pmm_watcher_t *w) {
    ...
    pmm_mutex_lock(&w->projects_lock);
    int count = (int)pmm_ht_count(w->projects);
    pmm_mutex_unlock(&w->projects_lock);
    return count;
}
...
static bool init_baseline(pmm_watcher_t *w, project_state_t *s) {
    struct stat st;
    if (stat(s->root_path, &st) != 0) {
        pmm_log_warn("watcher.root_gone", "project", s->project_name, "path", s->root_path);
    ...
    if (s->is_git) {
        ...
        s->interval_ms = pmm_watcher_poll_interval_ms(s->file_count);
        pmm_log_info("watcher.baseline", "project", s->project_name, "strategy", "git", "files",
                     s->file_count > 0 ? "yes" : "0");
    } else {
        pmm_log_info("watcher.baseline", "project", s->project_name, "strategy", "none");
    }
...
static bool check_changes(pmm_watcher_t *w, project_state_t *s, bool *changed_out) {
    if (!changed_out) {
        return false;
    }
    *changed_out = false;
...
static void prune_missing_project(pmm_watcher_t *w, project_state_t *s) {
    if (!w || !s || !s->project_name) {
        return;
    }

    char *project_name = pmm_strdup(s->project_name);
    char *root_path = pmm_strdup(s->root_path);
    ...
    pmm_mutex_lock(&w->projects_lock);
    project_state_t *current = pmm_ht_get(w->projects, project_name);
    /* Deferred free (same discipline as pmm_watcher_unwatch): this state
    ...
    bool still_eligible =
        ...
        now_ms - current->first_missing_ms >= (uint64_t)prune_grace_s() * PMM_MSEC_PER_SEC &&
    ...
    if (removed) {
        pmm_log_info("watcher.root_pruned", "project", project_name);
    }
    free(project_name);
    free(root_path);
}
...
static void poll_project(const char *key, void *val, void *ud) {
    ...
    if (rs == ROOT_UNCERTAIN) {
        ...
        pmm_log_warn("watcher.root_stat_error", "project", s->project_name, "path", s->root_path,
    ...
    if (rs == ROOT_MISSING) {
        ...
        pmm_log_warn("watcher.root_missing", "project", s->project_name, "path", s->root_path,
                     "polls", itoa_buf(s->missing_root_count));
        if (s->missing_root_count >= MISSING_ROOT_DELETE_AFTER &&
            now_ms - s->first_missing_ms >= (uint64_t)prune_grace_s() * PMM_MSEC_PER_SEC) {
            prune_missing_project(ctx->w, s);
        }
        return;
    }
    if (s->missing_root_count > 0) {
        pmm_log_info("watcher.root_restored", "project", s->project_name, "path", s->root_path);
        s->missing_root_count = 0;
        s->first_missing_ms = 0;
    }
    ...
    pmm_log_info("watcher.changed", "project", s->project_name, "strategy", "git");
    if (ctx->w->index_fn) {
        ...
        if (rc == 0) {
            ...
            pmm_log_info("watcher.index.retry", "project", s->project_name);
        } else {
            pmm_log_warn("watcher.index.err", "project", s->project_name);
        }
    }
...
int pmm_watcher_poll_once(pmm_watcher_t *w) {
    ...
    pmm_ht_foreach(w->projects, snapshot_project, &sc);
    pmm_mutex_unlock(&w->projects_lock);
...
static void cancel_active_git_entry(const char *key, void *value, void *user_data) {
    ...
    if (state && state->active_git) {
        (void)pmm_subprocess_request_cancel(state->active_git);
    }
}
...
void pmm_watcher_stop(pmm_watcher_t *w) {
    if (w) {
        ...
        pmm_mutex_lock(&w->projects_lock);
        pmm_ht_foreach(w->projects, cancel_active_git_entry, NULL);
        for (int i = 0; i < w->pending_free_count; i++) {
...
int pmm_watcher_run(pmm_watcher_t *w, int base_interval_ms) {
    ...
    pmm_log_info("watcher.start", "interval_ms", base_interval_ms > 999 ? "multi-sec" : "fast");
    ...
    while (!atomic_load(&w->stopped)) {
        ...
        while (slept < base_interval_ms && !atomic_load(&w->stopped)) {
            ...
            pmm_usleep((unsigned)chunk * PMM_MSEC_PER_SEC);
            slept += chunk;
        }
    }

    pmm_log_info("watcher.stop");
    return 0;
}
