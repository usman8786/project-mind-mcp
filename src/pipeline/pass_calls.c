 ...
 *   1. Re-extract calls (pmm_extract_file)
...
static char *read_file(const char *path, int *out_len) {
    ...
    if (size <= 0 || size > pmm_max_file_bytes()) { /* generous, env-configurable cap (B4) */
        (void)fclose(f);
        return NULL;
    }

    /* +pad: tree-sitter lexer lookahead reads past EOF; keep it in-bounds */
    enum { PMM_TS_LOOKAHEAD_PAD = 16 };
    char *buf = malloc((size_t)size + PMM_TS_LOOKAHEAD_PAD);
    if (!buf) {
        (void)fclose(f);
        return NULL;
    }
    ...
    memset(buf + nread, 0, PMM_TS_LOOKAHEAD_PAD);
    *out_len = (int)nread;
    return buf;
}
...
static const char *itoa_log(int val) {
    static PMM_TLS char bufs[PC_RING][PMM_SZ_32];
...
static char *extract_local_name_from_json(const char *props_json) {
    ...
    return pmm_strndup(start, end - start);
}

static int build_import_map(pmm_pipeline_ctx_t *ctx, const char *rel_path,
    ...
    *out_count = 0;
    ...
    if (result && result->imports.count > 0) {
        ...
        for (int i = 0; i < result->imports.count; i++) {
            ...
            char *target_qn = pmm_pipeline_fqn_module(ctx->project_name, imp->module_path);
            const pmm_gbuf_node_t *target = pmm_gbuf_find_by_qn(ctx->gbuf, target_qn);
            free(target_qn);
            if (!target) {
                continue;
            }
    ...
    char *file_qn = pmm_pipeline_fqn_compute(ctx->project_name, rel_path, "__file__");
    ...
    int edge_count = 0;
    int rc = pmm_gbuf_find_edges_by_source_type(ctx->gbuf, file_node->id, "IMPORTS", &edges,
                                                &edge_count);
    ...
    for (int i = 0; i < edge_count; i++) {
        ...
        const pmm_gbuf_node_t *target = pmm_gbuf_find_by_id(ctx->gbuf, e->target_id);
        if (!target) {
            continue;
        }
...
static void handle_route_registration(pmm_pipeline_ctx_t *ctx, const CBMCall *call,
                                      const pmm_gbuf_node_t *source_node, const char *module_qn,
    ...
    snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", method ? method : "ANY",
             pmm_route_canon_path(call->first_string_arg, cpath, sizeof(cpath)));
    char route_props[PMM_SZ_256];
    snprintf(route_props, sizeof(route_props), "{\"method\":\"%s\"}", method ? method : "ANY");
    int64_t route_id = pmm_gbuf_upsert_node(ctx->gbuf, "Route", call->first_string_arg, route_qn,
                                            "", 0, 0, route_props);
    char esc_cn[PMM_SZ_256]; /* sliced source text: escape quotes/newlines */
    char esc_fa[PMM_SZ_256];
    pmm_json_escape(esc_cn, sizeof(esc_cn), call->callee_name);
    pmm_json_escape(esc_fa, sizeof(esc_fa), call->first_string_arg);
    char props[PMM_SZ_512];
    snprintf(props, sizeof(props),
             "{\"callee\":\"%s\",\"url_path\":\"%s\",\"via\":\"route_registration\"}", esc_cn,
             esc_fa);
    pmm_gbuf_insert_edge(ctx->gbuf, source_node->id, route_id, "CALLS", props);
    if (call->second_arg_name != NULL && call->second_arg_name[0] != '\0') {
        pmm_resolution_t hres = pmm_registry_resolve(ctx->registry, call->second_arg_name,
                                                     module_qn, imp_keys, imp_vals, imp_count);
        if (hres.qualified_name != NULL && hres.qualified_name[0] != '\0') {
            const pmm_gbuf_node_t *handler = pmm_gbuf_find_by_qn(ctx->gbuf, hres.qualified_name);
            if (handler != NULL) {
                char hprops[PMM_SZ_1K]; /* must exceed escaped value + wrapper or snprintf cuts the
                                           closing brace */
                char esc_h[PMM_SZ_512];
                pmm_json_escape(esc_h, sizeof(esc_h), hres.qualified_name);
                snprintf(hprops, sizeof(hprops), "{\"handler\":\"%s\"}", esc_h);
                pmm_gbuf_insert_edge(ctx->gbuf, handler->id, route_id, "HANDLES", hprops);
            }
        }
    }
}
...
static int64_t create_svc_route_node(pmm_pipeline_ctx_t *ctx, const char *url, pmm_svc_kind_t svc,
    ...
    if (svc == PMM_SVC_HTTP) {
        ...
        qpath = pmm_route_canon_path(url, cpath, sizeof(cpath));
    } else {
    ...
    return pmm_gbuf_upsert_node(ctx->gbuf, "Route", url, route_qn, "", 0, 0, route_props);
}
...
static void calls_append_args(char *props, size_t cap, const CBMCall *call) {
    ...
    for (int i = 0; i < call->arg_count; i++) {
        ...
        char esc_e[PMM_SZ_256];
        pmm_json_escape(esc_e, sizeof(esc_e), a->expr ? a->expr : "");
        char one[PMM_SZ_512];
        if (a->value) {
            char esc_v[PMM_SZ_256];
...
static void calls_emit_edge(pmm_gbuf_t *gbuf, int64_t src, int64_t tgt, const char *type,
                            char *props, size_t cap, const CBMCall *call) {
    if (call && call->start_line > 0 && strcmp(type, "CALLS") == 0) {
        size_t len = strlen(props);
        if (len >= SKIP_ONE && props[len - SKIP_ONE] == '}' && len + PMM_SZ_32 < cap) {
    ...
    pmm_gbuf_insert_edge(gbuf, src, tgt, type, props);
}

static void emit_http_async_edge(pmm_pipeline_ctx_t *ctx, const CBMCall *call,
                                 const pmm_gbuf_node_t *source, const pmm_gbuf_node_t *target,
                                 const pmm_resolution_t *res, pmm_svc_kind_t svc,
    ...
    bool is_topic = (url_or_topic && url_or_topic[0] != '\0' && svc == PMM_SVC_ASYNC &&
    ...
    if (!is_url && !is_topic) {
        ...
        char esc_callee[PMM_SZ_256];
        pmm_json_escape(esc_callee, sizeof(esc_callee), call->callee_name);
        char props[PMM_SZ_512];
        snprintf(props, sizeof(props),
    ...
    char esc_callee[PMM_SZ_256];
    char esc_url[PMM_SZ_256];
    pmm_json_escape(esc_callee, sizeof(esc_callee), call->callee_name);
    pmm_json_escape(esc_url, sizeof(esc_url), url_or_topic);
...
static void emit_classified_edge(pmm_pipeline_ctx_t *ctx, const CBMCall *call,
    ...
    char esc_c2[PMM_SZ_256];
    pmm_json_escape(esc_c2, sizeof(esc_c2), call->callee_name);
    char props[PMM_SZ_512];
    snprintf(props, sizeof(props),
             "{\"callee\":\"%s\",\"confidence\":%.2f,\"strategy\":\"%s\",\"candidates\":%d}",
...
static const pmm_gbuf_node_t *calls_find_source(pmm_pipeline_ctx_t *ctx, const char *rel,
                                                const char *enclosing_qn) {
    const pmm_gbuf_node_t *src = NULL;
    if (enclosing_qn) {
        src = pmm_gbuf_find_by_qn(ctx->gbuf, enclosing_qn);
    ...
    if (!src) {
        char *fqn = pmm_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
        src = pmm_gbuf_find_by_qn(ctx->gbuf, fqn);
        free(fqn);
    }
    return src;
}
...
static int resolve_single_call(pmm_pipeline_ctx_t *ctx, CBMCall *call,
    ...
    const pmm_gbuf_node_t *source_node = calls_find_source(ctx, rel, call->enclosing_func_qn);
    if (!source_node) {
        return 0;
    }

    /* LSP-resolved calls take precedence over registry-textual matching.
     * Unique-tail fallbacks are JVM-only (see pmm_pipeline_lsp_allow_tail_match). */
    bool allow_tail = pmm_pipeline_lsp_allow_tail_match(lang);
    const CBMResolvedCall *lsp = pmm_pipeline_find_lsp_resolution(lsp_calls, call, allow_tail);
    if (lsp) {
        const pmm_gbuf_node_t *target_node =
            pmm_pipeline_lsp_target_node(ctx->gbuf, ctx->project_name, lsp->callee_qn, allow_tail);
    ...
    if (csvc == PMM_SVC_HTTP || csvc == PMM_SVC_ASYNC) {
        ...
        bool chas_url = cu && cu[0] != '\0' &&
                        (cu[0] == '/' || strstr(cu, "://") != NULL ||
                         (csvc == PMM_SVC_ASYNC && strlen(cu) > PAIR_LEN));
        if (chas_url) {
    ...
    pmm_resolution_t res = pmm_registry_resolve(ctx->registry, call->callee_name, module_qn,
    ...
    if (!res.qualified_name || res.qualified_name[0] == '\0') {
        ...
        if (pmm_service_pattern_route_method(call->callee_name) != NULL && call->first_string_arg &&
            call->first_string_arg[0] == '/') {
            handle_route_registration(ctx, call, source_node, module_qn, imp_keys, imp_vals,
                                      imp_count);
            return SKIP_ONE;
        }
        pmm_svc_kind_t esvc = pmm_service_pattern_match(call->callee_name);
        if (esvc == PMM_SVC_NONE && pmm_service_pattern_is_global_fetch(call->callee_name)) {
            esvc = PMM_SVC_HTTP;
        }
        if (esvc == PMM_SVC_HTTP || esvc == PMM_SVC_ASYNC) {
            const char *u = call->first_string_arg;
            bool has_url_or_topic = u && u[0] != '\0' &&
                                    (u[0] == '/' || strstr(u, "://") != NULL ||
                                     (esvc == PMM_SVC_ASYNC && strlen(u) > PAIR_LEN));
            if (has_url_or_topic) {
                pmm_resolution_t svc_res = {.qualified_name = call->callee_name,
                                            .confidence = PC_SVC_PATTERN_CONF,
                                            .strategy = "service_pattern",
    ...
    if (pmm_perl_suppress_generic_match(lang == PMM_LANG_PERL, call->is_method, call->callee_name,
                                        res.strategy)) {
        return 0;
    }
     ...
     * pmm_gbuf_find_by_qn returns NULL. The edge target for such calls is a
    ...
    if (svc == PMM_SVC_HTTP || svc == PMM_SVC_ASYNC) {
        ...
        bool has_url_or_topic = u && u[0] != '\0' &&
                                (u[0] == '/' || strstr(u, "://") != NULL ||
                                 (svc == PMM_SVC_ASYNC && strlen(u) > PAIR_LEN));
        if (has_url_or_topic) {
    ...
    const pmm_gbuf_node_t *target_node = pmm_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
    if (!target_node || source_node->id == target_node->id) {
        return 0;
    }
...
static CBMReturnTypeTable *build_return_type_table(const pmm_gbuf_t *gbuf) {
    if (!gbuf) {
        return NULL;
    }
    const pmm_gbuf_node_t **method_nodes = NULL;
    int method_count = 0;
    if (pmm_gbuf_find_by_label(gbuf, "Method", &method_nodes, &method_count) != 0 ||
    ...
    for (int i = 0; i < method_count && rtt->count < PMM_RETURN_TYPE_TABLE_CAP; i++) {
        const pmm_gbuf_node_t *n = method_nodes[i];
...
static CBMFileResult *calls_get_or_extract(pmm_pipeline_ctx_t *ctx, int idx,
                                           const pmm_file_info_t *fi, bool *owned) {
    *owned = false;
    ...
    CBMFileResult *r = pmm_extract_file_ex(src, slen, fi->language, ctx->project_name, fi->rel_path,
                                           PMM_EXTRACT_BUDGET, NULL, NULL, ctx->macro_table,
...
int pmm_pipeline_pass_calls(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files, int file_count) {
    pmm_log_info("pass.start", "pass", "calls", "files", itoa_log(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        char *module_qn = pmm_pipeline_fqn_module_dir(ctx->project_name, rel,
    ...
    pmm_log_info("pass.done", "pass", "calls", "total", itoa_log(total_calls), "resolved",
    ...
    pmm_pipeline_pass_fastapi_depends(ctx, files, file_count);
...
static int scan_depends_in_sig(pmm_pipeline_ctx_t *ctx, const pmm_regex_t *re, const char *sig,
                               const CBMDefinition *def, const char *module_qn, const char **ik,
                               const char **iv, int ic) {
    int count = 0;
    pmm_regmatch_t match[PC_REGEX_GRP];
    const char *scan = sig;
    while (pmm_regexec(re, scan, PC_REGEX_GRP, match, 0) == 0) {
        int ref_len = match[SKIP_ONE].rm_eo - match[SKIP_ONE].rm_so;
        char func_ref[PMM_SZ_256];
        ...
        func_ref[ref_len] = '\0';
        pmm_resolution_t res = pmm_registry_resolve(ctx->registry, func_ref, module_qn, ik, iv, ic);
        if (res.qualified_name && res.qualified_name[0] != '\0') {
            const pmm_gbuf_node_t *sn = pmm_gbuf_find_by_qn(ctx->gbuf, def->qualified_name);
            const pmm_gbuf_node_t *tn = pmm_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
            if (sn && tn && sn->id != tn->id) {
                pmm_gbuf_insert_edge(ctx->gbuf, sn->id, tn->id, "CALLS",
...
void pmm_pipeline_pass_fastapi_depends(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files,
                                       int file_count) {
    pmm_regex_t depends_re;
    if (pmm_regcomp(&depends_re, "Depends\\(([A-Za-z_][A-Za-z0-9_.]*)", PMM_REG_EXTENDED) != 0) {
        return;
    }
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        char *module_qn = pmm_pipeline_fqn_module_dir(ctx->project_name, files[i].rel_path,
                                                      pc_module_is_dir(files[i].language));
    ...
    pmm_regfree(&depends_re);
    if (edge_count > 0) {
        pmm_log_info("pass.fastapi_depends", "edges", itoa_log(edge_count));
    }
}
