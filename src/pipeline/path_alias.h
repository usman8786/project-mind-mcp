#ifndef PMM_PATH_ALIAS_H
#define PMM_PATH_ALIAS_H
 ...
 *
 * Lookup is two-step:
 *   1. pmm_path_alias_find_for_file picks the nearest ancestor scope for
 *      a given source file (longest matching dir_prefix wins).
 *   2. pmm_path_alias_resolve runs the chosen scope's alias map against
...
typedef struct {
    char *dir_prefix;
    pmm_path_alias_map_t *map;
} pmm_path_alias_scope_t;
 ...
 * linear scan. */
typedef struct {
    pmm_path_alias_scope_t *scopes;
    int count;
} pmm_path_alias_collection_t;
 ...
 * pmm_path_alias_collection_free. */
...
/* Free a collection produced by pmm_load_path_aliases. NULL-safe. */
