...
static char *pkgmap_read_file(const char *path, int *out_len) {
    ...
    if (size <= 0 || size > (long)PMM_SZ_1K * PMM_SZ_1K) { /* 1MB cap for manifests */
        (void)fclose(f);
        return NULL;
    }
...
static void pkg_entries_push(pmm_pkg_entries_t *e, char *pkg_name, char *entry_rel) {
    if (e->count >= e->cap) {
        int new_cap = e->cap == 0 ? PKGMAP_INIT_CAP : e->cap * SKIP_ONE * PAIR_LEN;
        pmm_pkg_entry_t *tmp = realloc(e->items, new_cap * sizeof(pmm_pkg_entry_t));
        if (!tmp) {
            free(pkg_name);
            free(entry_rel);
            return;
...
static char *path_dirname(const char *rel_path) {
    ...
    return pmm_strndup(rel_path, (size_t)(last - rel_path));
}
...
static char *strip_extension(const char *path) {
    ...
    for (size_t i = len; i > 0; i--) {
        if (path[i - SKIP_ONE] == '.') {
            return pmm_strndup(path, i - SKIP_ONE);
        }
        if (path[i - SKIP_ONE] == '/') {
            break;
        }
...
static char *extract_quoted(const char *p, const char *end) {
    ...
    return pmm_strndup(start, (size_t)(p - start));
}
...
static void parse_go_mod(const char *source, int source_len, const char *rel_path,
    ...
    char *module_path = pmm_strndup(start, (size_t)(val - start));
    char *dir = path_dirname(rel_path);
...
static void extract_psr4(yyjson_val *root, const char *dir, pmm_pkg_entries_t *entries) {
...
static void parse_pubspec_yaml(const char *source, int source_len, const char *rel_path,
                               pmm_pkg_entries_t *entries) {
    pmm_yaml_node_t *root = pmm_yaml_parse(source, source_len);
    if (!root) {
        return;
    }
    const char *name = pmm_yaml_get_str(root, "name");
    if (name && name[0] != '\0') {
        char *dir = path_dirname(rel_path);
...
static char *xml_tag_content(const char *p, const char *end) {
    ...
    return pmm_strndup(s, (size_t)(p - s));
}
...
static void parse_mix_exs(const char *source, int source_len, const char *rel_path,
    ...
    char *app_name = pmm_strndup(start, (size_t)(val - start));
    char *dir = path_dirname(rel_path);
    char entry[PKGMAP_PATH_BUF];
...
CBMHashTable *pmm_pkgmap_build(pmm_pkg_entries_t *worker_entries, int worker_count,
    ...
    CBMHashTable *map = pmm_ht_create(PKGMAP_HT_INIT);
    int merged = 0;

    for (int w = 0; w < worker_count; w++) {
        pmm_pkg_entries_t *we = &worker_entries[w];
        for (int i = 0; i < we->count; i++) {
            /* Convert entry_rel to QN: project.dir.parts */
            char *qn = pmm_pipeline_fqn_module(project_name, we->items[i].entry_rel);
            if (!qn) {
                continue;
            }

            /* Check for duplicate — first wins */
            if (pmm_ht_has(map, we->items[i].pkg_name)) {
                free(qn);
                continue;
            }
    ...
    pmm_log_info("pkgmap.build", "entries", pkgmap_itoa(merged));
    return map;
}

/* Returns true if basename is a package manifest we know how to parse.
 * Used by the filesystem walker; pmm_pkgmap_try_parse is the source of
 ...
 * matched by the shared pmm_should_skip_dir helper so we don't walk
 * node_modules, .git, build, etc. Returns the number of manifests
 * parsed, accumulated across the whole walk.
 *
 * Cross-platform: uses the portable pmm_opendir/pmm_readdir/pmm_closedir
...
static int pkgmap_walk_dir(const char *abs_dir, const char *rel_dir, pmm_pkg_entries_t *entries,
                           int depth, char **excluded_dirs, int excluded_count) {
    if (depth >= PKGMAP_WALK_MAX_DEPTH) {
        pmm_log_info("pkgmap.walk", "depth_cap", rel_dir && rel_dir[0] ? rel_dir : ".");
        return 0;
    }
    pmm_dir_t *dir = pmm_opendir(abs_dir);
    if (!dir) {
        return 0;
    }
    int parsed = 0;
    ...
    while ((entry = pmm_readdir(dir)) != NULL) {
        ...
        if (S_ISDIR(st.st_mode)) {
            if (pmm_should_skip_dir(name, PMM_MODE_FULL) ||
                pmm_pipeline_relpath_is_excluded(rel_path, excluded_dirs, excluded_count)) {
                continue;
            }
#ifdef _WIN32
        ...
        if (pmm_pkgmap_try_parse(name, rel_path, source, source_len, entries)) {
            parsed++;
        }
        free(source);
    }
...
int pmm_pkgmap_scan_repo(const char *repo_path, pmm_pkg_entries_t *entries, char **excluded_dirs,
    ...
    pmm_log_info("pkgmap.scan_repo", "manifests", pkgmap_itoa(parsed));
    return parsed;
}
...
CBMHashTable *pmm_pkgmap_build_from_files(const pmm_file_info_t *files, int file_count,
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        pmm_pkgmap_try_parse(basename, files[i].rel_path, source, source_len, &entries);
        free(source);
    }

    CBMHashTable *map = pmm_pkgmap_build(&entries, SKIP_ONE, project_name);
    pmm_pkg_entries_free(&entries);
    return map;
}

/* Variant of pmm_pkgmap_build_from_files that ALSO walks the repo
...
CBMHashTable *pmm_pkgmap_build_from_repo(const char *repo_path, const pmm_file_info_t *files,
    ...
    int from_walk = pmm_pkgmap_scan_repo(repo_path, &entries, excluded_dirs, excluded_count);
    pmm_log_info("pkgmap.scan", "manifests_from_files", pkgmap_itoa(from_files),
...
void pmm_pkgmap_free(CBMHashTable *pkgmap) {
    ...
    pmm_ht_foreach(pkgmap, pkgmap_free_entry, NULL);
    pmm_ht_free(pkgmap);
}
...
static char *resolve_slash_prefix(CBMHashTable *map, const char *module_path) {
    ...
    for (char *slash = buf + strlen(buf) - SKIP_ONE; slash > buf; slash--) {
        ...
        *slash = '\0';
        const char *base_qn = (const char *)pmm_ht_get(map, buf);
        if (!base_qn) {
            continue;
        }
...
static char *resolve_dot_prefix(CBMHashTable *map, const char *module_path,
    ...
    for (char *dot = buf + strlen(buf) - SKIP_ONE; dot > buf; dot--) {
        ...
        *dot = '\0';
        ...
        free(buf);
        return pmm_pipeline_fqn_module(project_name, result);
    }
    free(buf);
    return NULL;
}
...
static char *resolve_backslash_prefix(CBMHashTable *map, const char *module_path,
    ...
    for (char *bs = buf + strlen(buf) - SKIP_ONE; bs > buf; bs--) {
        ...
        *bs = '\0';
        ...
        const char *base_dir = (const char *)pmm_ht_get(map, prefix);
        if (!base_dir) {
            continue;
        }
        ...
        free(buf);
        return pmm_pipeline_fqn_module(project_name, path_result);
    }
    free(buf);
    return NULL;
}
...
char *pmm_pipeline_resolve_module(const pmm_pipeline_ctx_t *ctx, const char *source_rel,
    ...
    if (!ctx || !module_path) {
        return pmm_pipeline_fqn_module(ctx ? ctx->project_name : NULL, module_path);
    }

    /* 1. Try relative import resolution (existing logic) */
    char *resolved = pmm_pipeline_resolve_relative_import(source_rel, module_path);
    if (resolved) {
        char *qn = pmm_pipeline_fqn_module(ctx->project_name, resolved);
        free(resolved);
        return qn;
    }
    ...
    if (ctx->path_aliases && source_rel) {
        const pmm_path_alias_map_t *amap =
            pmm_path_alias_find_for_file(ctx->path_aliases, source_rel);
        if (amap) {
            char *aliased = pmm_path_alias_resolve(amap, module_path);
            if (aliased) {
                char *qn = pmm_pipeline_fqn_module(ctx->project_name, aliased);
                free(aliased);
                return qn;
            }
        }
    ...
    CBMHashTable *pkgmap = pmm_pipeline_get_pkgmap();
    if (!pkgmap) {
        return pmm_pipeline_fqn_module(ctx->project_name, module_path);
    }

    /* 3. Exact lookup */
    const char *mapped_qn = (const char *)pmm_ht_get(pkgmap, module_path);
    if (mapped_qn) {
...
static const pmm_gbuf_node_t *resolve_exact_file_node(const pmm_pipeline_ctx_t *ctx,
    ...
    const pmm_gbuf_node_t *best = NULL;
    for (int ni = 0; ni < 2; ni++) {
        ...
        const pmm_gbuf_node_t **hits = NULL;
        int hit_count = 0;
        if (pmm_gbuf_find_by_name(ctx->gbuf, name, &hits, &hit_count) != 0 || !hits) {
            continue;
        }

        for (int i = 0; i < hit_count; i++) {
            const pmm_gbuf_node_t *cand = hits[i];
...
static const pmm_gbuf_node_t *resolve_header_include(const pmm_pipeline_ctx_t *ctx,
    ...
    const pmm_gbuf_node_t *exact = resolve_exact_file_node(ctx, base, source_file_qn);
    if (exact) {
        return exact;
    }
...
static const pmm_gbuf_node_t *resolve_sibling_file(const pmm_pipeline_ctx_t *ctx,
                                                   const char *source_rel,
                                                   const char *source_file_qn,
                                                   const char *module_path) {
    if (!module_path || !module_path[0]) {
    ...
    /* 2. SCSS partial: dir/[subdir/]_<basename>.scss (underscore-prefixed). */
    {
        ...
        char *dpart = slash ? pmm_strndup(base, (size_t)(slash - base)) : strdup("");
        if (dpart && bn[0] != '_') {
    ...
    for (int i = 0; i < ncand; i++) {
        char *qn = pmm_pipeline_fqn_module(ctx->project_name, cands[i]);
        if (!qn) {
            continue;
        }
        const pmm_gbuf_node_t *n = pmm_gbuf_find_by_qn(ctx->gbuf, qn);
        free(qn);
...
const pmm_gbuf_node_t *pmm_pipeline_resolve_import_node(const pmm_pipeline_ctx_t *ctx,
    ...
    const pmm_gbuf_node_t *header_target =
    ...
    char *target_qn = pmm_pipeline_resolve_module(ctx, source_rel, imp->module_path);
    const pmm_gbuf_node_t *target = target_qn ? pmm_gbuf_find_by_qn(ctx->gbuf, target_qn) : NULL;
    free(target_qn);
    if (target) {
        return target;
    }
    ...
    if (namespace_map) {
        ...
        for (;;) {
            ...
            const char *list = (const char *)pmm_ht_get(namespace_map, norm);
            for (const char *seg = list; seg && *seg;) {
                ...
                if (len > 0 && len < sizeof(qbuf)) {
                    memcpy(qbuf, seg, len);
                    qbuf[len] = '\0';
                    const pmm_gbuf_node_t *n = pmm_gbuf_find_by_qn(ctx->gbuf, qbuf);
    ...
    }
    {
        ...
        for (int ci = 0; ci < ncands; ci++) {
            const pmm_gbuf_node_t **hits = NULL;
            int n = 0;
            if (pmm_gbuf_find_by_name(ctx->gbuf, cands[ci], &hits, &n) == 0 && hits) {
     ...
     * remaining path (and successive prefixes) to a Module/File node. */
    {
        ...
        if (body[0]) {
            ...
            for (;;) {
                char *rqn = pmm_pipeline_resolve_module(ctx, source_rel, work);
                const pmm_gbuf_node_t *n = rqn ? pmm_gbuf_find_by_qn(ctx->gbuf, rqn) : NULL;
                free(rqn);
                if (n && import_targetable_label(n->label) &&
                    (!source_file_qn || !n->qualified_name ||
                     strcmp(n->qualified_name, source_file_qn) != 0)) {
...
CBMHashTable *pmm_pipeline_namespace_map_build(const char *project_name,
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!map) {
            map = pmm_ht_create(PMM_SZ_64);
            if (!map) {
                return NULL;
            }
        }
        char *file_qn = pmm_pipeline_fqn_compute(project_name, rels[i], "__file__");
        if (!file_qn) {
            continue;
        }
        /* Normalize the namespace key to dot-separated form so it matches the
         * dot-normalized lookups in pmm_pipeline_resolve_import_node (PHP uses
         * '\\', some grammars '::' or '/'). */
        ...
        if (!pmm_ht_has(map, key)) {
            pmm_ht_set(map, key, file_qn); /* map owns key + file_qn */
        } else {
            ...
            const char *stored_key = pmm_ht_get_key(map, key);
            const char *cur = (const char *)pmm_ht_get(map, key);
            char *combined = NULL;
            if (stored_key && cur) {
                ...
                if (combined) {
                    ...
                    void *prev = pmm_ht_set(map, stored_key, combined);
...
static void ns_map_free_entry(const char *key, void *value, void *ud) {
    (void)ud;
    free((void *)key); /* strdup'd in pmm_pipeline_namespace_map_build */
    free(value);
}
...
void pmm_pipeline_namespace_map_free(CBMHashTable *map) {
    ...
    pmm_ht_foreach(map, ns_map_free_entry, NULL);
    pmm_ht_free(map);
}
