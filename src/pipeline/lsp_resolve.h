 ...
 *
 * Both pipeline paths (sequential pmm_pipeline_pass_calls and parallel
 * pmm_parallel_extract → resolve_file_calls) need to look up an
 ...
 */
#ifndef PMM_PIPELINE_LSP_RESOLVE_H
#define PMM_PIPELINE_LSP_RESOLVE_H
 ...
 * (Go, C/C++, Python, PHP). */
#define PMM_LSP_CONFIDENCE_FLOOR 0.6f
 ...
 * for them. */
static inline const char *pmm_lsp_bare_segment(const char *name) {
    if (!name) {
        return name;
    }
    const char *seg = name;
...
static inline const char *pmm_pipeline_qn_class_method_tail(const char *qn) {
    if (!qn) {
        return NULL;
    }
...
static inline const char *pmm_pipeline_call_callee_leaf(const char *callee_name) {
    return pmm_lsp_bare_segment(callee_name);
}
...
static inline bool pmm_pipeline_lsp_allow_tail_match(CBMLanguage lang) {
    return lang == PMM_LANG_JAVA || lang == PMM_LANG_KOTLIN;
}

static inline int pmm_pipeline_qn_class_method_tail_eq(const char *qn, const char *tail) {
    const char *qt = pmm_pipeline_qn_class_method_tail(qn);
 ...
 *      (JVM callers only, see pmm_pipeline_lsp_allow_tail_match), a
 ...
 * bare last segment by pmm_lsp_bare_segment before matching.
 *
 * The pointer returned aliases into `arr` and stays valid as long as the
 * underlying CBMFileResult is alive. */
static inline const CBMResolvedCall *pmm_pipeline_find_lsp_resolution(
    ...
    for (int i = 0; i < arr->count; i++) {
        ...
        if (rc->confidence < PMM_LSP_CONFIDENCE_FLOOR) {
            continue;
        }
        if (strcmp(rc->caller_qn, call->enclosing_func_qn) != 0) {
            continue;
        }
        const char *short_name = pmm_lsp_bare_segment(rc->callee_qn);
        ...
        const char *call_short = pmm_lsp_bare_segment(call->callee_name);
        if (strcmp(short_name, call_short) != 0) {
            ...
            if (!(rc->reason && rc->strategy &&
                  ...
                  strcmp(pmm_lsp_bare_segment(rc->reason), call_short) == 0)) {
                continue;
            }
        }
    ...
    const char *call_tail = pmm_pipeline_qn_class_method_tail(call->enclosing_func_qn);
    if (!call_tail) {
        return NULL;
    }
    ...
    for (int i = 0; i < arr->count; i++) {
        ...
        if (!pmm_pipeline_qn_class_method_tail_eq(rc->caller_qn, call_tail)) {
            continue;
        }
        if (best_tail) {
            return NULL;
 ...
 *      pmm_pipeline_lsp_allow_tail_match), use the exact node-name index
...
static inline const pmm_gbuf_node_t *pmm_pipeline_lsp_target_node(const pmm_gbuf_t *gbuf,
    ...
    const pmm_gbuf_node_t *direct = pmm_gbuf_find_by_qn(gbuf, callee_qn);
    if (direct) {
        return direct;
    }
    if (project_name && project_name[0]) {
        ...
        if (!(strncmp(callee_qn, project_name, proj_len) == 0 && callee_qn[proj_len] == '.')) {
            ...
            if (written > 0 && (size_t)written < sizeof(buf)) {
                const pmm_gbuf_node_t *prefixed = pmm_gbuf_find_by_qn(gbuf, buf);
                if (prefixed) {
                    return prefixed;
                }
            }
    ...
    const char *callee_tail = pmm_pipeline_qn_class_method_tail(callee_qn);
    if (!callee_tail) {
        return NULL;
    }
    const pmm_gbuf_node_t **hits = NULL;
    int hit_count = 0;
    if (pmm_gbuf_find_by_name(gbuf, short_name, &hits, &hit_count) != 0 || hit_count == 0) {
        return NULL;
    }
    ...
    for (int i = 0; i < hit_count; i++) {
        ...
        if (!pmm_pipeline_qn_class_method_tail_eq(cand->qualified_name, callee_tail)) {
            continue;
        }
        if (match) {
            return NULL;
...
#endif /* PMM_PIPELINE_LSP_RESOLVE_H */
