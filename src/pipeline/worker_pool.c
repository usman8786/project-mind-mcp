...
#define PMM_WORKER_STACK_SIZE ((size_t)8 * PMM_SZ_1K * PMM_SZ_1K)

/* ── Serial fallback ─────────────────────────────────────────────── */

static void run_serial(int count, pmm_parallel_fn fn, void *ctx) {
...
static void run_pthreads(int count, pmm_parallel_fn fn, void *ctx, int nworkers) {
    ...
    pmm_thread_t *threads = (pmm_thread_t *)malloc((size_t)nworkers * sizeof(pmm_thread_t));
    if (!threads) {
    ...
    for (int i = 0; i < nworkers; i++) {
        if (pmm_thread_create(&threads[i], PMM_WORKER_STACK_SIZE, pthread_worker, &wa) != 0) {
...
void pmm_parallel_for(int count, pmm_parallel_fn fn, void *ctx, pmm_parallel_for_opts_t opts) {
    ...
    if (nworkers <= 0) {
        nworkers = pmm_default_worker_count(true);
    }
    if (nworkers < WP_MIN) {
        nworkers = SKIP_ONE;
    }
