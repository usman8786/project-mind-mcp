...
enum { PMM_TLD_MAX_DEPTH = 256 }; /* recursion-depth cap (cycle/stack guard) */
...
static int json_get_int(const char *json, const char *key, int dflt) {
    ...
    return (int)strtol(p, NULL, PMM_DECIMAL_BASE);
}
...
static void append_complexity_props(pmm_gbuf_node_t *node, int tld, bool recursive) {
    ...
    bool empty = (olen == 2); /* "{}" */
    char *neu = malloc(olen + PMM_SZ_64);
    if (!neu) {
        return;
    }
    memcpy(neu, old, olen - 1); /* copy without trailing '}' */
    int w =
        snprintf(neu + (olen - 1), PMM_SZ_64, "%s\"transitive_loop_depth\":%d,\"recursive\":%s}",
...
static int tld_dfs(const pmm_gbuf_t *gb, int64_t id, const int *loop_depth, int *tld, char *state,
    ...
    int ne = 0;
    pmm_gbuf_find_edges_by_source_type(gb, id, "CALLS", &edges, &ne);
    for (int i = 0; i < ne; i++) {
...
static void seed_loop_depths(const pmm_gbuf_t *gb, const char *label, int *loop_depth,
                             bool *recursive, pmm_gbuf_node_t **nptr, int64_t maxid) {
    const pmm_gbuf_node_t **nodes = NULL;
    int count = 0;
    if (pmm_gbuf_find_by_label(gb, label, &nodes, &count) != 0) {
        return;
    }
...
void pmm_pipeline_pass_complexity(pmm_pipeline_ctx_t *ctx) {
    ...
    int64_t maxid = pmm_gbuf_next_id(gb) - 1;
    if (maxid < 1) {
        return;
    }
    ...
    pmm_gbuf_node_t **nptr = calloc(sz, sizeof(pmm_gbuf_node_t *));
    ...
    pmm_log_info("pass.complexity", "functions", itoa_cx(updated));
