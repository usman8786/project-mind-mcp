...
static const char *itoa_log(int val) {
    static PMM_TLS char bufs[GH_RING][PMM_SZ_32];
...
static int parse_git_log(const char *repo_path, commit_t **out, int *out_count) {
    ...
    *out_count = 0;
     ...
     * safe here because pmm_validate_shell_arg (above) rejects ", $, `, \ and the
...
static void collect_coupling_cb(const char *pair_key, void *val, void *ud) {
    ...
    int *count_a = pmm_ht_get(cctx->file_counts, file_a_buf);
    int *count_b = pmm_ht_get(cctx->file_counts, file_b);
    if (!count_a || !count_b) {
        return;
    }
    ...
    pmm_change_coupling_t *cc = &cctx->out[cctx->out_count++];
    ...
    long long *ts = pmm_ht_get(cctx->pair_timestamps, pair_key);
...
int pmm_compute_change_coupling(const pmm_commit_files_t *commits, int commit_count,
    ...
    CBMHashTable *file_counts = pmm_ht_create(PMM_SZ_1K);
    CBMHashTable *pair_counts = pmm_ht_create(PMM_SZ_2K);
    ...
    CBMHashTable *pair_timestamps = pmm_ht_create(PMM_SZ_2K);
    ...
    for (int c = 0; c < commit_count; c++) {
        ...
        for (int i = 0; i < commits[c].count; i++) {
            int *val = pmm_ht_get(file_counts, commits[c].files[i]);
            if (val) {
                (*val)++;
            } else {
                int *nv = malloc(sizeof(int));
                *nv = SKIP_ONE;
                pmm_ht_set(file_counts, strdup(commits[c].files[i]), nv);
            }
        }
        ...
        for (int i = 0; i < commits[c].count; i++) {
            for (int j = i + SKIP_ONE; j < commits[c].count; j++) {
                ...
                int *val = pmm_ht_get(pair_counts, pk);
                if (val) {
                    (*val)++;
                    long long *ts = pmm_ht_get(pair_timestamps, pk);
                    ...
                    *nv = SKIP_ONE;
                    ...
                    pmm_ht_set(pair_counts, pk, nv);
                    long long *nts = malloc(sizeof(long long));
                    *nts = commits[c].timestamp;
                    pmm_ht_set(pair_timestamps, pk2, nts);
                }
            }
        }
    }
    ...
    pmm_ht_foreach(pair_counts, collect_coupling_cb, &cctx);

    pmm_ht_foreach(pair_counts, free_counter, NULL);
    pmm_ht_free(pair_counts);
    pmm_ht_foreach(pair_timestamps, free_counter, NULL);
    pmm_ht_free(pair_timestamps);
    pmm_ht_foreach(file_counts, free_counter, NULL);
    pmm_ht_free(file_counts);
...
int pmm_pipeline_githistory_compute(const char *repo_path, pmm_githistory_result_t *result) {
    ...
    pmm_commit_files_t *cf = calloc((size_t)commit_count, sizeof(pmm_commit_files_t));
    if (!cf) {
        for (int c = 0; c < commit_count; c++) {
    ...
    pmm_change_coupling_t *couplings = malloc(MAX_COUPLINGS * sizeof(pmm_change_coupling_t));
    int coupling_count = pmm_compute_change_coupling(cf, commit_count, couplings, MAX_COUPLINGS);
     ...
     * gets the couplings. */
    pmm_file_temporal_t *ft_arr = malloc(MAX_FILE_TEMPORAL * sizeof(pmm_file_temporal_t));
    if (ft_arr) {
        int ft_count = 0;
        CBMHashTable *file_idx = pmm_ht_create(PMM_SZ_1K);
        for (int c = 0; c < commit_count; c++) {
            ...
            for (int f = 0; f < cf[c].count; f++) {
                ...
                int *idx = pmm_ht_get(file_idx, fp);
                if (idx) {
                    ...
                    *nidx = new_idx;
                    pmm_ht_set(file_idx, strdup(fp), nidx);
                }
            }
        }
        pmm_ht_foreach(file_idx, free_counter, NULL);
        pmm_ht_free(file_idx);
...
int pmm_pipeline_githistory_apply(pmm_pipeline_ctx_t *ctx, const pmm_githistory_result_t *result) {
    ...
    for (int i = 0; i < result->count; i++) {
        const pmm_change_coupling_t *cc = &result->couplings[i];

        char *qn_a = pmm_pipeline_fqn_compute(ctx->project_name, cc->file_a, "__file__");
        char *qn_b = pmm_pipeline_fqn_compute(ctx->project_name, cc->file_b, "__file__");

        const pmm_gbuf_node_t *node_a = pmm_gbuf_find_by_qn(ctx->gbuf, qn_a);
        const pmm_gbuf_node_t *node_b = pmm_gbuf_find_by_qn(ctx->gbuf, qn_b);
        ...
        pmm_gbuf_insert_edge(ctx->gbuf, node_a->id, node_b->id, "FILE_CHANGES_WITH", props);
        edge_count++;
    }
    ...
    for (int i = 0; i < result->file_temporal_count; i++) {
        const pmm_file_temporal_t *ft = &result->file_temporal[i];
        char *qn = pmm_pipeline_fqn_compute(ctx->project_name, ft->file_path, "__file__");
        const pmm_gbuf_node_t *node = pmm_gbuf_find_by_qn(ctx->gbuf, qn);
        free(qn);
        if (!node) {
            continue;
        }
        ...
        char ext_escaped[PMM_SZ_64];
        pmm_json_escape(ext_escaped, (int)sizeof(ext_escaped), ext ? ext : "");
        ...
        pmm_gbuf_upsert_node(ctx->gbuf, node->label, node->name, node->qualified_name,
...
int pmm_pipeline_pass_githistory(pmm_pipeline_ctx_t *ctx) {
    pmm_log_info("pass.start", "pass", "githistory");

    pmm_githistory_result_t result = {0};
    pmm_pipeline_githistory_compute(ctx->repo_path, &result);

    int edge_count = 0;
    if (result.count > 0 || result.file_temporal_count > 0) {
        edge_count = pmm_pipeline_githistory_apply(ctx, &result);
    }

    free(result.couplings);
    free(result.file_temporal);

    pmm_log_info("pass.done", "pass", "githistory", "commits", itoa_log(result.commit_count),
