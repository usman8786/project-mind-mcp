...
#define PMM_NS_PER_MS 1000000.0
...
static double elapsed_ms(struct timespec start) {
    ...
    return (s * PMM_MS_PER_SEC) + (ns / PMM_NS_PER_MS);
}
...
#ifdef __APPLE__
    return ((int64_t)st->st_mtimespec.tv_sec * PMM_NS_PER_SEC) + (int64_t)st->st_mtimespec.tv_nsec;
#elif defined(_WIN32)
    return (int64_t)st->st_mtime * PMM_NS_PER_SEC;
#else
    return ((int64_t)st->st_mtim.tv_sec * PMM_NS_PER_SEC) + (int64_t)st->st_mtim.tv_nsec;
#endif
}
...
static bool *classify_files(pmm_file_info_t *files, int file_count, pmm_file_hash_t *stored,
    ...
    CBMHashTable *ht =
        pmm_ht_create(stored_count > 0 ? (size_t)stored_count * PAIR_LEN : PMM_SZ_64);
    for (int i = 0; i < stored_count; i++) {
        pmm_ht_set(ht, stored[i].rel_path, &stored[i]);
    }

    for (int i = 0; i < file_count; i++) {
        pmm_file_hash_t *h = pmm_ht_get(ht, files[i].rel_path);
        if (!h) {
            /* New file */
            changed[i] = true;
            n_changed++;
 ...
 * claude-connectors/project-mind-index-repository-is-destructive-...
 ...
 *   - snprintf truncation (combined path ≥ PMM_SZ_4K) → preserve. We can't
...
static int find_deleted_files(const char *repo_path, pmm_file_info_t *files, int file_count,
                              pmm_file_hash_t *stored, int stored_count, char ***out_deleted,
                              pmm_file_hash_t **out_mode_skipped, int *out_mode_skipped_count) {
    *out_deleted = NULL;
    ...
    *out_mode_skipped_count = 0;
    ...
    if (!repo_path) {
        ...
        pmm_log_error("incremental.err", "msg", "find_deleted_files_null_repo_path");
        return 0;
    }

    CBMHashTable *current = pmm_ht_create((size_t)file_count * PAIR_LEN);
    for (int i = 0; i < file_count; i++) {
        pmm_ht_set(current, files[i].rel_path, &files[i]);
    }

    int del_count = 0;
    int del_cap = PMM_SZ_64;
    char **deleted = malloc((size_t)del_cap * sizeof(char *));
    if (!deleted) {
        pmm_log_error("incremental.err", "msg", "find_deleted_files_oom");
        pmm_ht_free(current);
        return 0;
    }

    int ms_count = 0;
    int ms_cap = PMM_SZ_64;
    pmm_file_hash_t *mode_skipped = malloc((size_t)ms_cap * sizeof(pmm_file_hash_t));
    if (!mode_skipped) {
        pmm_log_error("incremental.err", "msg", "find_deleted_files_oom_ms");
        free(deleted);
        pmm_ht_free(current);
        return 0;
    }
    ...
    for (int i = 0; i < stored_count; i++) {
        if (pmm_ht_get(current, stored[i].rel_path)) {
        ...
        if (n < 0 || n >= (int)sizeof(abs_path)) {
            ...
            pmm_log_warn("incremental.path_truncated", "rel_path", stored[i].rel_path);
            preserve = true;
        } else {
            struct stat st;
            if (stat(abs_path, &st) == 0) {
                ...
                pmm_log_warn("incremental.stat_uncertain", "rel_path", stored[i].rel_path, "errno",
                             itoa_buf(errno));
                preserve = true;
            }
        }
        ...
        if (preserve) {
            ...
            if (ms_count >= ms_cap) {
                ms_cap *= PAIR_LEN;
                pmm_file_hash_t *tmp = realloc(mode_skipped, (size_t)ms_cap * sizeof(*tmp));
                if (!tmp) {
                    pmm_log_error("incremental.err", "msg", "find_deleted_files_realloc_oom_ms");
                    break;
                }
                mode_skipped = tmp;
            }
            ...
            if (!rp || (stored[i].sha256 && !sh)) {
                 ...
                 * orphaned-node bug. */
                pmm_log_error("incremental.err", "msg", "find_deleted_files_strdup_oom", "rel_path",
                              stored[i].rel_path);
                free(rp);
                free(sh);
                break;
        ...
        if (del_count >= del_cap) {
            ...
            if (!tmp) {
                pmm_log_error("incremental.err", "msg", "find_deleted_files_realloc_oom");
                break;
            }
            deleted = tmp;
        }
...
static void free_mode_skipped(pmm_file_hash_t *ms, int count) {
    if (!ms) {
        return;
    }
 ...
 *
 * The purge step (pmm_gbuf_delete_by_file) removes a changed file's nodes,
 ...
 *   - pmm_gbuf_insert_edge dedups, so re-linking an edge the resolver already
...
/* pmm_gbuf_foreach_edge visitor: snapshot inbound cross-file edges into
 * changed files so they survive the purge and can be re-linked afterward. */
static void incr_capture_inbound_edge(const pmm_gbuf_edge_t *edge, void *userdata) {
    pmm_edge_capture_t *cap = (pmm_edge_capture_t *)userdata;
    if (incr_edge_type_is_recomputed(edge->type)) {
        return;
    }
    const pmm_gbuf_node_t *src = pmm_gbuf_find_by_id(cap->gbuf, edge->source_id);
    const pmm_gbuf_node_t *tgt = pmm_gbuf_find_by_id(cap->gbuf, edge->target_id);
    ...
    if (!pmm_ht_get(cap->changed_paths, tgt->file_path) ||
        pmm_ht_get(cap->changed_paths, src->file_path)) {
        return;
    }
    if (cap->count >= cap->cap) {
        int ncap = (cap->cap > 0) ? cap->cap * PAIR_LEN : PMM_SZ_64;
        pmm_saved_edge_t *tmp = realloc(cap->items, (size_t)ncap * sizeof(*tmp));
        if (!tmp) {
            pmm_log_warn("incremental.edge_snapshot_oom", "captured", itoa_buf(cap->count));
            return; /* best-effort: stop capturing, keep what we have */
        }
        cap->items = tmp;
        cap->cap = ncap;
    }
    pmm_saved_edge_t *s = &cap->items[cap->count];
...
static int incr_restore_inbound_edges(pmm_gbuf_t *gbuf, pmm_edge_capture_t *cap) {
    int restored = 0;
    for (int i = 0; i < cap->count; i++) {
        pmm_saved_edge_t *s = &cap->items[i];
        const pmm_gbuf_node_t *src = pmm_gbuf_find_by_qn(gbuf, s->source_qn);
        const pmm_gbuf_node_t *tgt = pmm_gbuf_find_by_qn(gbuf, s->target_qn);
        if (src && tgt) {
            pmm_gbuf_insert_edge(gbuf, src->id, tgt->id, s->type, s->props);
            restored++;
        }
    }
    return restored;
...
static void incr_free_edge_capture(pmm_edge_capture_t *cap) {
    for (int i = 0; i < cap->count; i++) {
...
static int persist_hashes(pmm_store_t *store, const char *project, pmm_file_info_t *files,
                          int file_count, const pmm_file_hash_t *mode_skipped,
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        int rc = pmm_store_upsert_file_hash(store, project, files[i].rel_path, "",
                                            stat_mtime_ns(&st), st.st_size);
        if (rc != PMM_STORE_OK) {
            pmm_log_warn("incremental.persist_hash_failed", "scope", "current", "rel_path",
    ...
    if (mode_skipped) {
        for (int i = 0; i < mode_skipped_count; i++) {
            int rc =
                pmm_store_upsert_file_hash(store, project, mode_skipped[i].rel_path,
                                           mode_skipped[i].sha256 ? mode_skipped[i].sha256 : "",
                                           mode_skipped[i].mtime_ns, mode_skipped[i].size);
            if (rc != PMM_STORE_OK) {
                pmm_log_warn("incremental.persist_hash_failed", "scope", "mode_skipped", "rel_path",
    ...
    if (current_failed > 0 || ms_failed > 0) {
        pmm_log_warn("incremental.persist_summary", "current_failed", itoa_buf(current_failed),
...
static bool incr_label_is_registry_symbol(const char *label) {
    ...
    return label && (strcmp(label, "Function") == 0 || strcmp(label, "Method") == 0 ||
                     pmm_label_is_type_like(label) || strcmp(label, "Variable") == 0 ||
                     strcmp(label, "Field") == 0);
}

/* Callback for pmm_gbuf_foreach_node: seed the registry with the existing
...
static void registry_visitor(const pmm_gbuf_node_t *node, void *userdata) {
    pmm_registry_t *r = (pmm_registry_t *)userdata;
    if (!incr_label_is_registry_symbol(node->label)) {
        return;
    }
    pmm_registry_add(r, node->name, node->qualified_name, node->label);
}

/* Run parallel or sequential extract+resolve for changed files. */
static void run_extract_resolve(pmm_pipeline_ctx_t *ctx, pmm_file_info_t *changed_files, int ci) {
    struct timespec t;

    /* Per-file LSP always runs (every mode). Cross-file LSP stays disabled in
     * incremental regardless (pmm_parallel_resolve is called with NULL
     * cross_registries below). */

#define MIN_FILES_FOR_PARALLEL_INCR 50
    int worker_count = pmm_default_worker_count(true);
    bool use_parallel = (worker_count > SKIP_ONE && ci > MIN_FILES_FOR_PARALLEL_INCR);

    if (use_parallel) {
        pmm_log_info("incremental.mode", "mode", "parallel", "workers", itoa_buf(worker_count),
                     "changed", itoa_buf(ci));

        _Atomic int64_t shared_ids;
        atomic_init(&shared_ids, pmm_gbuf_next_id(ctx->gbuf));

        CBMFileResult **cache = (CBMFileResult **)calloc(ci, sizeof(CBMFileResult *));
        if (cache) {
            pmm_clock_gettime(CLOCK_MONOTONIC, &t);
            pmm_parallel_extract(ctx, changed_files, ci, cache, &shared_ids, worker_count);
            pmm_gbuf_set_next_id(ctx->gbuf, atomic_load(&shared_ids));
            pmm_log_info("pass.timing", "pass", "incr_extract", "elapsed_ms",
                         itoa_buf((int)elapsed_ms(t)));

            pmm_clock_gettime(CLOCK_MONOTONIC, &t);
            pmm_build_registry_from_cache(ctx, changed_files, ci, cache);
            pmm_log_info("pass.timing", "pass", "incr_registry", "elapsed_ms",
                         itoa_buf((int)elapsed_ms(t)));

            /* Incremental skips cross-file LSP precondition build — it
             * would need all_defs from the full project, not just the
             * changed slice. Per-file LSP (run inside pmm_extract_file)
            ...
            pmm_clock_gettime(CLOCK_MONOTONIC, &t);
            pmm_parallel_resolve(ctx, changed_files, ci, cache, &shared_ids, worker_count, NULL, 0,
                                 NULL, NULL /* module_def_index */,
                                 NULL /* cross_registries — incremental skips Tier 2 prebuild */);
            pmm_gbuf_set_next_id(ctx->gbuf, atomic_load(&shared_ids));
            pmm_log_info("pass.timing", "pass", "incr_resolve", "elapsed_ms",
                         itoa_buf((int)elapsed_ms(t)));

            for (int j = 0; j < ci; j++) {
                if (cache[j]) {
                    pmm_free_result(cache[j]);
                }
            }
            free(cache);
        }
    } else {
        pmm_log_info("incremental.mode", "mode", "sequential", "changed", itoa_buf(ci));
        pmm_pipeline_pass_definitions(ctx, changed_files, ci);
        pmm_pipeline_pass_calls(ctx, changed_files, ci);
        pmm_pipeline_pass_usages(ctx, changed_files, ci);
        pmm_pipeline_pass_semantic(ctx, changed_files, ci);
    }
}

/* Run post-extraction passes (tests, decorator tags, configlink). */
static void run_postpasses(pmm_pipeline_ctx_t *ctx, pmm_file_info_t *changed_files, int ci,
                           const char *project) {
    struct timespec t;
    ...
    pmm_clock_gettime(CLOCK_MONOTONIC, &t);
    pmm_pipeline_pass_decorator_tags(ctx->gbuf, project);
    pmm_log_info("pass.timing", "pass", "incr_decorator_tags", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

    pmm_clock_gettime(CLOCK_MONOTONIC, &t);
    pmm_pipeline_pass_configlink(ctx);
    pmm_log_info("pass.timing", "pass", "incr_configlink", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

    /* SIMILAR_TO + SEMANTICALLY_RELATED edges only in moderate/full modes */
    if (ctx->mode <= PMM_MODE_MODERATE) {
        pmm_clock_gettime(CLOCK_MONOTONIC, &t);
        pmm_pipeline_pass_similarity(ctx);
        pmm_log_info("pass.timing", "pass", "incr_similarity", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));

        pmm_clock_gettime(CLOCK_MONOTONIC, &t);
        pmm_pipeline_pass_semantic_edges(ctx);
        pmm_log_info("pass.timing", "pass", "incr_semantic_edges", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
    }
}
...
static int dump_and_persist(pmm_gbuf_t *gbuf, const char *db_path, const char *project,
                            pmm_file_info_t *files, int file_count,
                            const pmm_file_hash_t *mode_skipped, int mode_skipped_count,
                            const pmm_coverage_row_t *cov, int cov_count,
                            const pmm_coverage_meta_t *meta_template) {
    struct timespec t;
    pmm_clock_gettime(CLOCK_MONOTONIC, &t);

    if ((pmm_unlink(db_path) != 0 && errno != ENOENT) || pmm_remove_db_sidecars(db_path) != 0) {
        pmm_log_error("incremental.err", "msg", "clear_staging_failed", "path", db_path);
        return PMM_STORE_ERR;
    }

    int dump_rc = pmm_gbuf_dump_to_sqlite(gbuf, db_path);
    pmm_log_info("incremental.dump", "rc", itoa_buf(dump_rc), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    if (dump_rc != 0) {
        return dump_rc;
    }
    ...
    if (!hash_store) {
        pmm_log_error("incremental.err", "msg", "open_staging_after_dump", "path", db_path);
        return PMM_STORE_ERR;
    }
    int rc =
    ...
    bool have_project_info =
        pmm_store_get_project(hash_store, project, &project_info) == PMM_STORE_OK;
    pmm_coverage_meta_t meta = meta_template ? *meta_template : (pmm_coverage_meta_t){0};
    meta.generation = have_project_info ? project_info.indexed_at : NULL;
    meta.hash_records_complete = rc == PMM_STORE_OK;
    if (pmm_store_coverage_replace_ex(hash_store, project, cov, cov_count, &meta) != PMM_STORE_OK) {
        pmm_log_error("incremental.err", "msg", "persist_coverage", "project", project);
        rc = PMM_STORE_ERR;
    }
    ...
    if (pmm_store_exec(hash_store, "INSERT INTO nodes_fts(nodes_fts) VALUES('delete-all');") !=
        PMM_STORE_OK) {
        rc = PMM_STORE_ERR;
    }
...
int pmm_pipeline_run_incremental(pmm_pipeline_t *p, const char *db_path, pmm_file_info_t *files,
    ...
    if (!store) {
        pmm_log_error("incremental.err", "msg", "open_db_failed", "path", db_path);
    ...
    int stored_count = 0;
    pmm_store_get_file_hashes(store, project, &stored, &stored_count);
    ...
    char **deleted = NULL;
    pmm_file_hash_t *mode_skipped = NULL;
    int mode_skipped_count = 0;
    int deleted_count =
        find_deleted_files(pmm_pipeline_repo_path(p), files, file_count, stored, stored_count,
                           &deleted, &mode_skipped, &mode_skipped_count);

    pmm_log_info("incremental.classify", "changed", itoa_buf(n_changed), "unchanged",
    ...
    if (n_changed == 0 && deleted_count == 0) {
        pmm_log_info("incremental.noop", "reason", "no_changes");
        free(is_changed);
        free(deleted);
        free_mode_skipped(mode_skipped, mode_skipped_count);
        pmm_store_free_file_hashes(stored, stored_count);
    ...
    pmm_coverage_row_t *old_cov = NULL;
    int old_cov_count = 0;
    if (pmm_store_coverage_get(store, project, &old_cov, &old_cov_count) != PMM_STORE_OK) {
        pmm_log_error("incremental.err", "msg", "coverage_read_failed", "project", project);
        pmm_store_free_coverage(old_cov, old_cov_count);
        free(is_changed);
        ...
        free_mode_skipped(mode_skipped, mode_skipped_count);
        pmm_store_close(store);
        return PMM_PIPELINE_ABORT_PRESERVE_DB;
    }

    /* Build list of changed files */
    pmm_file_info_t *changed_files =
        (n_changed > 0) ? malloc((size_t)n_changed * sizeof(pmm_file_info_t)) : NULL;
    int ci = 0;
    for (int i = 0; i < file_count; i++) {
        if (is_changed[i]) {
    ...
    pmm_log_info("incremental.reparse", "files", itoa_buf(ci));

    struct timespec t;

    /* Step 1: Load existing graph into RAM */
    pmm_clock_gettime(CLOCK_MONOTONIC, &t);
    pmm_gbuf_t *existing = pmm_gbuf_new(project, pmm_pipeline_repo_path(p));
    int load_rc = pmm_gbuf_load_from_db(existing, db_path, project);
    pmm_log_info("incremental.load_db", "rc", itoa_buf(load_rc), "nodes",
                 itoa_buf(pmm_gbuf_node_count(existing)), "edges",
                 itoa_buf(pmm_gbuf_edge_count(existing)), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

    if (load_rc != 0) {
        pmm_log_error("incremental.err", "msg", "load_db_failed");
        pmm_gbuf_free(existing);
        free(changed_files);
        for (int i = 0; i < deleted_count; i++) {
            free(deleted[i]);
        }
    ...
    pmm_edge_capture_t edge_cap = {0};
    edge_cap.gbuf = existing;
    {
        CBMHashTable *changed_paths = pmm_ht_create(ci > 0 ? (size_t)ci * PAIR_LEN : PMM_SZ_64);
        for (int i = 0; i < ci; i++) {
            pmm_ht_set(changed_paths, changed_files[i].rel_path, &changed_files[i]);
        }
        edge_cap.changed_paths = changed_paths;
        pmm_clock_gettime(CLOCK_MONOTONIC, &t);
        pmm_gbuf_foreach_edge(existing, incr_capture_inbound_edge, &edge_cap);
        edge_cap.changed_paths = NULL;
        pmm_ht_free(changed_paths); /* keys borrowed from changed_files; not freed here */
    }
    pmm_log_info("incremental.edge_snapshot", "captured", itoa_buf(edge_cap.count), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

    /* Step 2: Purge stale nodes */
    pmm_clock_gettime(CLOCK_MONOTONIC, &t);
    for (int i = 0; i < ci; i++) {
        pmm_gbuf_delete_by_file(existing, changed_files[i].rel_path);
    }
    for (int i = 0; i < deleted_count; i++) {
        pmm_gbuf_delete_by_file(existing, deleted[i]);
        free(deleted[i]);
    }
    free(deleted);
    pmm_log_info("incremental.purge", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    /* Step 3-5: Registry + extract + resolve */
    pmm_registry_t *registry = pmm_registry_new();
    pmm_clock_gettime(CLOCK_MONOTONIC, &t);
    pmm_gbuf_foreach_node(existing, registry_visitor, registry);
    pmm_log_info("incremental.registry_seed", "symbols", itoa_buf(pmm_registry_size(registry)),
    ...
    pmm_path_alias_collection_t *path_aliases =
        pmm_load_path_aliases_excluded(pmm_pipeline_repo_path(p), excluded_dirs, excluded_count);

    pmm_pipeline_ctx_t ctx = {
        .project_name = project,
        .repo_path = pmm_pipeline_repo_path(p),
        .gbuf = existing,
        .registry = registry,
        .cancelled = pmm_pipeline_cancelled_ptr(p),
        .pipeline = p, /* so passes can record per-file skips (Track B) */
        .mode = pmm_pipeline_get_mode(p),
    ...
    for (int i = 0; i < ci; i++) {
        char *file_qn = pmm_pipeline_fqn_compute(project, changed_files[i].rel_path, "__file__");
        if (file_qn) {
            ...
            pmm_gbuf_upsert_node(existing, "File", basename, file_qn, rel, 0, 0, props);
            free(file_qn);
        }
    }

    run_extract_resolve(&ctx, changed_files, ci);
    pmm_pipeline_pass_k8s(&ctx, changed_files, ci);
    ...
    pmm_file_error_t *run_errs = NULL;
    int run_err_count = 0;
    pmm_pipeline_get_file_errors(p, &run_errs, &run_err_count);
    char **run_excluded = NULL;
    int run_excluded_count = 0;
    pmm_pipeline_get_excluded(p, &run_excluded, &run_excluded_count);
    pmm_ignored_file_t *run_ignored = NULL;
    int run_ignored_count = 0;
    int run_ignored_total = 0;
    pmm_pipeline_get_ignored(p, &run_ignored, &run_ignored_count, &run_ignored_total);
    pmm_coverage_row_t *cov = NULL;
    int cov_n = 0;
    int cov_cap = old_cov_count + run_err_count + run_excluded_count + run_ignored_count;
    if (cov_cap > 0) {
        cov = (pmm_coverage_row_t *)malloc((size_t)cov_cap * sizeof(*cov));
    }
    bool coverage_rows_available = cov_cap == 0 || cov != NULL;
    if (cov) {
        CBMHashTable *changed_set = pmm_ht_create(ci > 0 ? (size_t)ci * PAIR_LEN : PMM_SZ_64);
        for (int i = 0; i < ci; i++) {
            pmm_ht_set(changed_set, changed_files[i].rel_path, &changed_files[i]);
        }
        for (int i = 0; i < old_cov_count; i++) {
            ...
            if (!by_design && old_cov[i].rel_path &&
                !pmm_ht_get(changed_set, old_cov[i].rel_path)) {
                cov[cov_n++] = old_cov[i];
            }
        }
        pmm_ht_free(changed_set);
    ...
    pmm_log_info("incremental.edge_relink", "relinked", itoa_buf(relinked), "captured",
    ...
    pmm_pipeline_set_committed_counts(p, pmm_gbuf_node_count(existing),
                                      pmm_gbuf_edge_count(existing));
    int index_mode = pmm_pipeline_get_mode(p);
    ...
    pmm_log_info("incremental.done", "elapsed_ms", itoa_buf((int)elapsed_ms(t0)));
    return persist_rc;
}
