 ...
 * shape each language LSP's pmm_run_X_lsp_cross expects, then merges
 ...
 * extraction-side pmm_lang_module_is_dir() in internal/pmm/helpers.c so the
...
static char *pxc_read_file(const char *path, int *out_len) {
    ...
    if (size <= 0 || size > (long)PXC_MAX_FILE_BYTES_FACTOR * (long)PMM_SZ_1K * (long)PMM_SZ_1K) {
        (void)fclose(f);
        return NULL;
    }
    /* +pad: tree-sitter lexer lookahead reads past EOF; keep it in-bounds */
    enum { PMM_TS_LOOKAHEAD_PAD = 16 };
    char *buf = (char *)malloc((size_t)size + PMM_TS_LOOKAHEAD_PAD);
    if (!buf) {
        (void)fclose(f);
        return NULL;
    }
...
static const char *pxc_map_label(const char *label) {
    if (!label)
        return NULL;
    if (pmm_label_is_type_like(label) || strcmp(label, "Protocol") == 0 ||
...
static const char *pxc_join_pipe(CBMArena *arena, const char *const *items) {
    ...
    char *buf = (char *)pmm_arena_alloc(arena, bufsz);
    if (!buf)
        return NULL;
    char *p = buf;
...
static const char *pxc_jvm_type_qn(CBMArena *arena, const char *namespace_name,
    ...
    return pmm_arena_sprintf(arena, "%s.%s", namespace_name, short_name);
}
...
static const char *pxc_jvm_def_qn(CBMArena *arena, const CBMDefinition *src,
    ...
    if (strcmp(label, "Method") == 0 || strcmp(label, "Function") == 0 ||
        ...
        if (src->parent_class && src->parent_class[0]) {
            return pmm_arena_sprintf(arena, "%s.%s.%s", namespace_name,
                                     pxc_last_component(src->parent_class), src->name);
        }
        return pmm_arena_sprintf(arena, "%s.%s", namespace_name, src->name);
    }
    return pmm_arena_sprintf(arena, "%s.%s", namespace_name, src->name);
}
...
static const char *pxc_infer_jvm_namespace(CBMArena *arena, const char *rel_path,
    ...
    const char *lang_root = lang == PMM_LANG_KOTLIN ? "kotlin/" : "java/";
    if (strncmp(rel_path, "src/main/", 9) == 0 &&
        ...
        const char *needle = lang == PMM_LANG_KOTLIN ? "/kotlin/" : "/java/";
...
CBMLSPDef *pmm_pxc_collect_all_defs(CBMFileResult **cache, const pmm_file_info_t *files,
    ...
    for (int fi = 0; fi < file_count; fi++) {
        if (!cache[fi])
            continue;
        if (!def_modules[fi]) {
            def_modules[fi] = pmm_pipeline_fqn_module_dir(project_name, files[fi].rel_path,
...
static int pxc_build_import_map(const pmm_gbuf_t *gbuf, const char *project_name,
...
void pmm_pxc_ts_modes(CBMLanguage lang, const char *rel_path, bool *out_js, bool *out_jsx,
                      bool *out_dts) {
    *out_js = (lang == PMM_LANG_JAVASCRIPT);
    *out_jsx = (lang == PMM_LANG_TSX);
    *out_dts = false;
    if (!rel_path)
        return;
    size_t rl = strlen(rel_path);
    if (lang == PMM_LANG_JAVASCRIPT && rl >= 4 && strcmp(rel_path + rl - 4, ".jsx") == 0) {
        *out_jsx = true;
    }
    if (lang == PMM_LANG_TYPESCRIPT && rl >= 5 && strcmp(rel_path + rl - 5, ".d.ts") == 0) {
        *out_dts = true;
    }
}
...
bool pmm_pxc_has_cross_lsp(CBMLanguage lang) {
    ...
    case PMM_LANG_TSX:
    case PMM_LANG_PHP:
    case PMM_LANG_CSHARP: /* tier-2 prebuilt registry path (pass_parallel.c) */
    case PMM_LANG_JAVA:   /* fallback pmm_pxc_run_one path */
    case PMM_LANG_KOTLIN: /* fallback pmm_pxc_run_one path */
    case PMM_LANG_RUST:   /* fallback pmm_pxc_run_one path (manifest-aware) */
        return true;
    default:
        return false;
    }
...
static void pxc_append_results(CBMArena *dst_arena, CBMResolvedCallArray *dst_calls,
    ...
    CBMArena keys;
    pmm_arena_init(&keys);
    CBMHashTable *seen = pmm_ht_create((uint32_t)(dst_calls->count + src_out->count + 1));

    for (int i = 0; i < dst_calls->count; i++) {
        const CBMResolvedCall *rc = &dst_calls->items[i];
        if (rc->caller_qn && rc->callee_qn) {
            char *k = pmm_arena_sprintf(&keys, "%s\x1f%s", rc->caller_qn, rc->callee_qn);
            if (k) {
                pmm_ht_set(seen, k, (void *)1);
            }
        }
    }
    ...
    for (int j = 0; j < src_out->count; j++) {
        ...
        if (!src->caller_qn || !src->callee_qn)
            continue;
        char *k = pmm_arena_sprintf(&keys, "%s\x1f%s", src->caller_qn, src->callee_qn);
        if (k && pmm_ht_has(seen, k))
            continue;
        if (k) {
            pmm_ht_set(seen, k, (void *)1);
        }
        CBMResolvedCall dst;
        memset(&dst, 0, sizeof(dst));
        dst.caller_qn = pmm_arena_strdup(dst_arena, src->caller_qn);
        dst.callee_qn = pmm_arena_strdup(dst_arena, src->callee_qn);
        dst.strategy = src->strategy ? pmm_arena_strdup(dst_arena, src->strategy) : NULL;
        dst.confidence = src->confidence;
        dst.reason = src->reason ? pmm_arena_strdup(dst_arena, src->reason) : NULL;
        pmm_resolvedcall_push(dst_calls, dst_arena, dst);
    }

    pmm_ht_free(seen);
    pmm_arena_destroy(&keys);
}

/* ── Rust workspace manifest (Cargo.toml) for cross-CRATE resolution ──
 *
 * pmm_pxc_run_one's signature is shared with the parallel pass
 ...
 * sequential driver (pmm_pipeline_pass_lsp_cross, below) sets up once per
...
void pmm_pxc_set_rust_manifest(const CBMCargoManifest *m) {
...
static CBMRustLSPDef *pxc_lspdefs_to_rust(CBMArena *arena, const CBMLSPDef *defs, int def_count) {
    if (!defs || def_count <= 0)
        return NULL;
    CBMRustLSPDef *out =
        (CBMRustLSPDef *)pmm_arena_alloc(arena, (size_t)def_count * sizeof(CBMRustLSPDef));
    if (!out)
        return NULL;
...
void pmm_pxc_run_one(CBMLanguage lang, CBMFileResult *r, const char *source, int source_len,
    ...
    CBMArena scratch;
    pmm_arena_init(&scratch);
    CBMResolvedCallArray out;
    memset(&out, 0, sizeof(out));

    switch (lang) {
    case PMM_LANG_GO:
        pmm_run_go_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                             imp_qns, imp_count, tree, &out);
        break;
    case PMM_LANG_C:
    case PMM_LANG_CPP:
    case PMM_LANG_CUDA: {
        bool cpp_mode = (lang != PMM_LANG_C);
        ...
        pmm_run_c_lsp_cross(&scratch, source, source_len, module_qn, cpp_mode, defs, def_count,
                            NULL, NULL, 0, tree, &out);
        break;
    }
    case PMM_LANG_PYTHON:
        pmm_run_py_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                             imp_qns, imp_count, tree, &out);
        break;
    case PMM_LANG_PHP:
        pmm_run_php_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                              imp_qns, imp_count, tree, &out);
        break;
    case PMM_LANG_JAVA:
        pmm_run_java_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                               imp_qns, imp_count, tree, &out);
        break;
    case PMM_LANG_KOTLIN:
        pmm_run_kotlin_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count,
    ...
    case PMM_LANG_RUST: {
        ...
        pmm_run_rust_lsp_cross_with_manifest(&scratch, source, source_len, module_qn, rdefs,
    ...
    pmm_arena_destroy(&scratch);
}

/* Variant of pmm_pxc_run_one for TS/JS/JSX/TSX with explicit dialect
 * flags. Same scratch-arena lifecycle as pmm_pxc_run_one. */
void pmm_pxc_run_one_ts(CBMFileResult *r, const char *source, int source_len, const char *module_qn,
    ...
    pmm_run_ts_lsp_cross(&scratch, source, source_len, module_qn, js_mode, jsx_mode, dts_mode, defs,
...
void pmm_pxc_dispatch_file(CBMLanguage lang, CBMFileResult *result, const char *source,
    ...
    CBMTypeRegistry *prebuilt =
        cross_registries ? pmm_pxc_registry_for_lang(cross_registries, lang) : NULL;
    if (prebuilt) {
        switch (lang) {
        case PMM_LANG_GO:
            /* Tier 3 (metadata-driven): pure lookup over the Tier-1
             * lsp_unresolved entries — no parse, no AST walk. */
            pmm_go_fast_resolve_qualified_calls(result, prebuilt, imp_keys, imp_vals, imp_count);
            used_prebuilt = true;
            break;
        case PMM_LANG_PYTHON:
            pmm_run_py_lsp_cross_with_registry(&result->arena, source, source_len, def_module,
        ...
        case PMM_LANG_CPP:
        case PMM_LANG_CUDA:
            pmm_run_c_lsp_cross_with_registry(
                &result->arena, source, source_len, def_module, (lang != PMM_LANG_C), prebuilt,
                imp_keys, imp_vals, imp_count, result->cached_tree, &result->resolved_calls);
            used_prebuilt = true;
            break;
        case PMM_LANG_CSHARP:
            pmm_run_cs_lsp_cross_with_registry(&result->arena, source, source_len, def_module,
        ...
        case PMM_LANG_TSX: {
            ...
            bool jsx;
            bool dts;
            pmm_pxc_ts_modes(lang, rel, &js, &jsx, &dts);
            CBMLSPDef *ts_defs = all_defs;
            int ts_def_count = all_def_count;
            CBMLSPDef *ts_filtered = NULL;
            if (module_def_index) {
                int fc = 0;
                ts_filtered = pmm_pxc_filter_defs_for_file(module_def_index, all_defs, lang,
            ...
            pmm_run_ts_lsp_cross_with_registry(&result->arena, source, source_len, def_module, js,
    ...
    if (module_def_index && lang != PMM_LANG_RUST) {
        int filtered_count = 0;
        filtered =
            pmm_pxc_filter_defs_for_file(module_def_index, all_defs, lang, result->namespace_name,
    ...
    if (lang == PMM_LANG_RUST) {
        CBMTypeRegistry *shared = rust_shared_get ? rust_shared_get(rust_shared_ctx) : NULL;
        if (shared) {
            pmm_run_rust_lsp_cross_with_registry(&result->arena, source, source_len, def_module,
                                                 shared, imp_keys, imp_vals, imp_count,
                                                 result->cached_tree, pmm_pxc_get_rust_manifest(),
                                                 &result->resolved_calls,
                                                 /*result=*/NULL);
        } else {
            pmm_pxc_run_one(lang, result, source, source_len, def_module, file_defs, file_def_count,
                            imp_keys, imp_vals, imp_count);
        }
    } else if (lang == PMM_LANG_JAVASCRIPT || lang == PMM_LANG_TYPESCRIPT || lang == PMM_LANG_TSX) {
        bool js;
        bool jsx;
        bool dts;
        pmm_pxc_ts_modes(lang, rel, &js, &jsx, &dts);
        pmm_pxc_run_one_ts(result, source, source_len, def_module, file_defs, file_def_count,
...
static bool pxc_build_rust_manifest(const pmm_pipeline_ctx_t *ctx, CBMArena *marena,
    ...
    pmm_cargo_parse(marena, toml, toml_len, out_m);
...
int pmm_pipeline_pass_lsp_cross(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files,
    ...
    pmm_log_info("pass.start", "pass", "lsp_cross", "files", itoa_buf(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        if (cache[i] && files[i].language == PMM_LANG_RUST) {
            have_rust = true;
            break;
        }
    }
    ...
    if (have_rust) {
        pmm_arena_init(&cargo_arena);
        have_manifest = pxc_build_rust_manifest(ctx, &cargo_arena, &cargo_manifest);
        pmm_pxc_set_rust_manifest(have_manifest ? &cargo_manifest : NULL);
    }

    /* Per-file module QN cache so we don't recompute it once per def + once
     * per call. pmm_pipeline_fqn_module mallocs; freed at end. */
    char **def_modules = (char **)calloc((size_t)file_count, sizeof(char *));
    if (!def_modules) {
        pmm_log_error("pass.err", "pass", "lsp_cross", "phase", "alloc");
        return 0;
    }

    int def_count = 0;
    CBMLSPDef *all_defs = pmm_pxc_collect_all_defs(cache, files, file_count, ctx->project_name,
    ...
    if (all_defs) {
        ...
        cross_registries.go = pmm_go_build_cross_registry(xa, all_defs, def_count);
        cross_registries.python = pmm_py_build_cross_registry(xa, all_defs, def_count);
        cross_registries.c = pmm_c_build_cross_registry(xa, all_defs, def_count);
        cross_registries.cs = pmm_cs_build_cross_registry(xa, all_defs, def_count);
        cross_registries.ts = pmm_ts_build_cross_registry(xa, all_defs, def_count);
    }
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (!def_modules[i]) {
            def_modules[i] = pmm_pipeline_fqn_module_dir(ctx->project_name, files[i].rel_path,
         ...
         * failure mode). */
        pmm_index_mark_start(files[i].rel_path);
        pmm_pxc_dispatch_file(lang, cache[i], source, source_len, files[i].rel_path, def_modules[i],
                              &cross_registries, module_def_index, all_defs, def_count, imp_keys,
                              imp_vals, imp_count, NULL, NULL);
        pmm_index_mark_done(files[i].rel_path);
        per_lang_calls++;
        processed++;
    ...
    if (have_rust) {
        pmm_pxc_set_rust_manifest(NULL);
        pmm_arena_destroy(&cargo_arena);
    }
    (void)have_manifest;

    pmm_log_info("pass.done", "pass", "lsp_cross", "files_processed", itoa_buf(processed),
...
/* pmm_ht_foreach callback: free each pxc_module_entry_t. */
...
static pxc_module_entry_t *pxc_module_entry_get_or_create(CBMHashTable *ht, const char *key) {
    ...
    pxc_module_entry_t *e = (pxc_module_entry_t *)pmm_ht_get(ht, key);
    if (e) {
        return e;
    }
...
static bool pxc_is_jvm_lang(CBMLanguage lang) {
    return lang == PMM_LANG_JAVA || lang == PMM_LANG_KOTLIN;
}
...
static void pxc_mark_module_defs(const CBMModuleDefIndex *idx, bool *selected,
    ...
    pxc_module_entry_t *e = (pxc_module_entry_t *)pmm_ht_get(idx->ht, module_qn);
...
CBMModuleDefIndex *pmm_pxc_build_module_def_index(CBMLSPDef *all_defs, int def_count) {
    ...
    CBMHashTable *ht = pmm_ht_create(64);
    CBMHashTable *namespace_ht = pmm_ht_create(64);
    if (!ht || !namespace_ht) {
        pmm_ht_free(ht);
        pmm_ht_free(namespace_ht);
        return NULL;
    }
    ...
    if (!idx) {
        pmm_ht_foreach(ht, pxc_module_entry_free_cb, NULL);
        pmm_ht_free(ht);
        pmm_ht_foreach(namespace_ht, pxc_module_entry_free_cb, NULL);
        pmm_ht_free(namespace_ht);
        return NULL;
    }
    idx->ht = ht;
...
void pmm_pxc_free_module_def_index(CBMModuleDefIndex *idx) {
    ...
    if (idx->ht) {
        pmm_ht_foreach(idx->ht, pxc_module_entry_free_cb, NULL);
        pmm_ht_free(idx->ht);
    }
    if (idx->namespace_ht) {
        pmm_ht_foreach(idx->namespace_ht, pxc_module_entry_free_cb, NULL);
        pmm_ht_free(idx->namespace_ht);
    }
    free(idx);
}
...
CBMLSPDef *pmm_pxc_filter_defs_for_file(const CBMModuleDefIndex *idx, CBMLSPDef *all_defs,
    ...
    if (pxc_is_jvm_lang(caller_lang) && caller_namespace && caller_namespace[0] &&
        ...
        pxc_module_entry_t *e =
            (pxc_module_entry_t *)pmm_ht_get(idx->namespace_ht, caller_namespace);
