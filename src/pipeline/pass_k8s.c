 ...
 * supported)
 *
 * Depends on: pass_infrascan.c (pmm_is_kustomize_file, pmm_is_k8s_manifest, pmm_infra_qn),
 *             extraction layer (pmm.h), graph_buffer, pipeline internals.
 */
...
static void handle_kustomize(pmm_pipeline_ctx_t *ctx, const char *path, const char *rel_path,
                             CBMFileResult *result) {
    /* Emit Module node for this kustomize overlay file */
    char *mod_qn = pmm_infra_qn(ctx->project_name, rel_path, "kustomize", NULL);
    if (!mod_qn) {
        return;
    }

    int64_t mod_id = pmm_gbuf_upsert_node(ctx->gbuf, "Module", k8s_basename(rel_path), mod_qn,
    ...
    if (!res) {
        ...
        if (source) {
            res = pmm_extract_file(source, src_len, PMM_LANG_KUSTOMIZE, ctx->project_name, rel_path,
                                   PMM_EXTRACT_BUDGET, NULL, NULL);
            free(source);
            allocated = true;
        }
    }
    ...
    if (res) {
        for (int j = 0; j < res->imports.count; j++) {
            ...
            char *target_qn =
                pmm_pipeline_fqn_compute(ctx->project_name, imp->module_path, "__file__");
            if (!target_qn) {
                continue;
            }
            ...
            if (target) {
                pmm_gbuf_insert_edge(ctx->gbuf, mod_id, target->id, "IMPORTS",
                                     "{\"via\":\"kustomize\"}");
                import_count++;
            }
        }
    ...
    pmm_log_info("pass.k8s.kustomize", "file", rel_path, "imports", itoa_k8s(import_count));
}
...
static void k8s_link_selectors(pmm_pipeline_ctx_t *ctx, const k8s_record_array_t *recs) {
    int edges = 0;
        ...
        for (int j = 0; j < recs->count; j++) {
            ...
            if (k8s_selector_matches(svc, wl)) {
                ...
                snprintf(props, sizeof(props),
                         ...
                         svc->name, wl->name);
                pmm_gbuf_insert_edge(ctx->gbuf, svc->node_id, wl->node_id, "INFRA_MAPS", props);
                edges++;
            }
        }
    }
    if (edges > 0) {
        pmm_log_info("pass.k8s.selectors", "linked", itoa_k8s(edges));
    }
}
...
static void handle_k8s_manifest(pmm_pipeline_ctx_t *ctx, const char *path, const char *rel_path,
                                const char *source, int src_len, k8s_record_t *rec) {
    (void)path; /* retained for symmetry; source is always provided now */
    int resource_count = 0;

    CBMFileResult *res = pmm_extract_file(source, src_len, PMM_LANG_K8S, ctx->project_name,
                                          rel_path, PMM_EXTRACT_BUDGET, NULL, NULL);
    if (!res) {
        return;
    }
    ...
    const pmm_gbuf_node_t *file_node = file_qn ? pmm_gbuf_find_by_qn(ctx->gbuf, file_qn) : NULL;
    free(file_qn);
    ...
    for (int d = 0; d < res->defs.count; d++) {
        ...
        int64_t node_id =
            pmm_gbuf_upsert_node(ctx->gbuf, "Resource", def->name, def->qualified_name, rel_path,
    ...
    pmm_log_info("pass.k8s.manifest", "file", rel_path, "resources", itoa_k8s(resource_count));
}
...
static void handle_helm_chart(pmm_pipeline_ctx_t *ctx, const char *rel_path, const char *source) {
    pmm_helm_chart_t hc;
    if (pmm_parse_helm_chart(source, &hc) != 0) {
        return;
    }

    const char *cname = hc.chart_name[0] ? hc.chart_name : k8s_basename(rel_path);
    char *chart_qn = pmm_infra_qn(ctx->project_name, rel_path, "helm-chart", NULL);
    if (!chart_qn) {
        return;
    }
    int64_t chart_id = pmm_gbuf_upsert_node(ctx->gbuf, "Chart", cname, chart_qn, rel_path, SKIP_ONE,
                                            0, "{\"source\":\"helm\"}");
    free(chart_qn);
    ...
    if (file_node && chart_id > 0) {
        pmm_gbuf_insert_edge(ctx->gbuf, file_node->id, chart_id, "DEFINES", "{}");
    }
    free(file_qn);
    ...
    for (int i = 0; i < hc.dep_count && chart_id > 0; i++) {
        ...
        char dep_qn[PMM_SZ_512];
        snprintf(dep_qn, sizeof(dep_qn), "%s.__helm_dep__.%s", ctx->project_name, hc.deps[i]);
        int64_t dep_id =
            pmm_gbuf_upsert_node(ctx->gbuf, "Chart", hc.deps[i], dep_qn, rel_path, SKIP_ONE, 0,
                                 "{\"source\":\"helm\",\"external\":true}");
        if (dep_id > 0) {
            pmm_gbuf_insert_edge(ctx->gbuf, chart_id, dep_id, "DEPENDS_ON", "{}");
            dep_edges++;
        }
    }
    pmm_log_info("pass.k8s.helm", "file", rel_path, "deps", itoa_k8s(dep_edges));
}
...
static int emit_dep_edge(pmm_pipeline_ctx_t *ctx, const pmm_gbuf_node_t *src, const char *rel_path,
    ...
    char dep_props[PMM_SZ_256];
    snprintf(dep_props, sizeof(dep_props), "{\"source\":\"%s\",\"external\":true}", ecosystem);
    int64_t dep_id =
        pmm_gbuf_upsert_node(ctx->gbuf, "Package", name, dep_qn, rel_path, SKIP_ONE, 0, dep_props);
    if (dep_id > 0 && dep_id != src->id) {
        pmm_gbuf_insert_edge(ctx->gbuf, src->id, dep_id, "DEPENDS_ON", "{}");
        return 1;
    }
    return 0;
}
...
static int parse_gomod_deps(pmm_pipeline_ctx_t *ctx, const pmm_gbuf_node_t *src,
...
static int parse_requirements_deps(pmm_pipeline_ctx_t *ctx, const pmm_gbuf_node_t *src,
                                   const char *rel_path, const char *source) {
    int edges = 0;
    const char *p = source;
    while (p && *p) {
...
static void handle_dep_manifest(pmm_pipeline_ctx_t *ctx, const char *rel_path, const char *source,
                                const char *ecosystem) {
    if (!source) {
        return;
    }
    char *file_qn = pmm_pipeline_fqn_compute(ctx->project_name, rel_path, "__file__");
    const pmm_gbuf_node_t *src = file_qn ? pmm_gbuf_find_by_qn(ctx->gbuf, file_qn) : NULL;
    free(file_qn);
    if (!src) {
        return;
    }
    ...
    pmm_log_info("pass.k8s.depmanifest", "file", rel_path, "deps", itoa_k8s(dep_edges));
}

/* ── Pass entry point ────────────────────────────────────────────── */

int pmm_pipeline_pass_k8s(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files, int file_count) {
    pmm_log_info("pass.start", "pass", "k8s", "files", itoa_k8s(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (is_gomod_file(base) || lang == PMM_LANG_GOMOD || is_requirements_file(base)) {
            int dep_len = 0;
        ...
        } else if (pmm_is_kustomize_file(base)) {
            handle_kustomize(ctx, path, rel, cached);
            kustomize_count++;
        } else if (lang == PMM_LANG_YAML || lang == PMM_LANG_K8S) {
            ...
            if (source) {
                if (is_helm_chart_file(base)) {
                    ...
                    helm_count++;
                } else if (pmm_is_k8s_manifest(base, source)) {
                    /* Always re-extract with PMM_LANG_K8S regardless of any cached
    ...
    pmm_log_info("pass.done", "pass", "k8s", "kustomize", itoa_k8s(kustomize_count), "manifests",
