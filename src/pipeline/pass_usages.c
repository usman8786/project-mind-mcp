...
static int build_import_map_from_cache(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result,
...
static int build_import_map_from_edges(pmm_pipeline_ctx_t *ctx, const char *rel_path,
                                       const char ***out_keys, const char ***out_vals,
                                       int *out_count) {
...
static const pmm_gbuf_node_t *find_enclosing_node(pmm_pipeline_ctx_t *ctx, const char *func_qn,
                                                  const char *rel_path) {
    const pmm_gbuf_node_t *node = NULL;
    if (func_qn && func_qn[0]) {
        node = pmm_gbuf_find_by_qn(ctx->gbuf, func_qn);
        ...
        if (pmm_pipeline_node_is_dir_container(node)) {
            node = NULL;
        }
    }
    if (!node) {
...
static int resolve_usage_edges(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result,
    ...
    for (int u = 0; u < result->usages.count; u++) {
        ...
        const pmm_gbuf_node_t *src = find_enclosing_node(ctx, usage->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }

        pmm_resolution_t res = pmm_registry_resolve(ctx->registry, usage->ref_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }

        const pmm_gbuf_node_t *tgt = pmm_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
        ...
        char esc_ref[PMM_SZ_256];
        pmm_json_escape(esc_ref, sizeof(esc_ref), usage->ref_name);
        char uprops[PMM_SZ_512];
        snprintf(uprops, sizeof(uprops), "{\"callee\":\"%s\"}", esc_ref);
        pmm_gbuf_insert_edge(ctx->gbuf, src->id, tgt->id, "USAGE", uprops);
        resolved++;
    }
    return resolved;
}
...
static int resolve_throw_edges(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result,
                               const char *rel, const char *module_qn, const char **imp_keys,
                               const char **imp_vals, int imp_count) {
    int resolved = 0;
    for (int t = 0; t < result->throws.count; t++) {
        ...
        const pmm_gbuf_node_t *src = find_enclosing_node(ctx, thr->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }

        const char *edge_type = is_checked_exception(thr->exception_name) ? "THROWS" : "RAISES";
        pmm_resolution_t res = pmm_registry_resolve(ctx->registry, thr->exception_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);

        const pmm_gbuf_node_t *tgt = NULL;
        if (res.qualified_name && res.qualified_name[0]) {
            tgt = pmm_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
        }
        if (!tgt || src->id == tgt->id) {
            continue;
        }

        pmm_gbuf_insert_edge(ctx->gbuf, src->id, tgt->id, edge_type, "{}");
        resolved++;
    }
    return resolved;
}
...
static int resolve_rw_edges(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result, const char *rel,
    ...
    for (int r = 0; r < result->rw.count; r++) {
        ...
        const pmm_gbuf_node_t *src = find_enclosing_node(ctx, rw->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }

        pmm_resolution_t res = pmm_registry_resolve(ctx->registry, rw->var_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }
...
int pmm_pipeline_pass_usages(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files,
                             int file_count) {
    pmm_log_info("pass.start", "pass", "usages", "files", itoa_log(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (!result) {
            ...
            result = pmm_extract_file(source, source_len, files[i].language, ctx->project_name, rel,
    ...
    pmm_log_info("pass.done", "pass", "usages", "usage", itoa_log(usage_resolved), "throws",
