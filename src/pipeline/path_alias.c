...
enum {
    PMM_PATH_ALIAS_MAX_ENTRIES = 256, /* per single config file       */
    PMM_PATH_ALIAS_MAX_FILES = 256,   /* config files per repo walk   */
    PMM_PATH_ALIAS_MAX_FILE_BYTES = 64 * 1024,
    PMM_PATH_ALIAS_MAX_DEPTH = 32, /* directory recursion depth    */
};
 ...
 * pmm_pipeline_fqn_module tokenizes on '/' without collapsing them
...
static int cmp_alias_entry_by_specificity(const void *a, const void *b) {
    const pmm_path_alias_t *ea = a;
    const pmm_path_alias_t *eb = b;
...
static int cmp_scope_by_specificity(const void *a, const void *b) {
    const pmm_path_alias_scope_t *sa = a;
    const pmm_path_alias_scope_t *sb = b;
...
static pmm_path_alias_map_t *load_tsconfig_file(const char *abs_path, const char *dir_prefix) {
    FILE *f = pmm_fopen(abs_path, "r");
    if (!f) {
        return NULL;
    }
    fseek(f, 0, SEEK_END);
    long len = ftell(f);
    fseek(f, 0, SEEK_SET);
    if (len <= 0 || len > PMM_PATH_ALIAS_MAX_FILE_BYTES) {
        fclose(f);
        return NULL;
    }
    ...
    pmm_path_alias_map_t *map = calloc(1, sizeof(*map));
    if (!map) {
        yyjson_doc_free(doc);
        return NULL;
    }
    ...
    if (paths_obj && yyjson_is_obj(paths_obj)) {
        ...
        bool capped = obj_size > PMM_PATH_ALIAS_MAX_ENTRIES;
        int capacity = (int)(capped ? (size_t)PMM_PATH_ALIAS_MAX_ENTRIES : obj_size);
        if (capacity > 0) {
            map->entries = calloc((size_t)capacity, sizeof(pmm_path_alias_t));
            if (!map->entries) {
                free(map->base_url);
                free(map);
                yyjson_doc_free(doc);
            ...
            while ((key = yyjson_obj_iter_next(&iter)) != NULL && map->count < capacity) {
                ...
                pmm_path_alias_t *entry = &map->entries[map->count];
                const char *star = strchr(alias_pattern, '*');
                if (star) {
                    entry->has_wildcard = true;
                    entry->alias_prefix =
                        pmm_strndup(alias_pattern, (size_t)(star - alias_pattern));
                ...
                if (tstar) {
                    char *pre = pmm_strndup(target_pattern, (size_t)(tstar - target_pattern));
            ...
            if (capped) {
                pmm_log_warn("path_alias.entries.cap_hit", "config", abs_path, "kept",
                             /* itoa via thread-local buffer would be tidier; keep simple */
                             "256_of_more");
            }
            qsort(map->entries, (size_t)map->count, sizeof(pmm_path_alias_t),
...
void pmm_path_alias_collection_free(pmm_path_alias_collection_t *coll) {
    if (!coll) {
        return;
    }
    for (int i = 0; i < coll->count; i++) {
        ...
        if (coll->scopes[i].map) {
            pmm_path_alias_map_t *map = coll->scopes[i].map;
...
char *pmm_path_alias_resolve(const pmm_path_alias_map_t *map, const char *module_path) {
    if (!map || !module_path) {
        return NULL;
    }
    ...
    for (int i = 0; i < map->count; i++) {
        const pmm_path_alias_t *e = &map->entries[i];
...
static void find_alias_files(const char *abs_dir, const char *rel_dir, alias_config_hit_t *out,
                             ...
                             int excluded_count) {
    if (*count >= max_count || depth > PMM_PATH_ALIAS_MAX_DEPTH) {
        return;
    }
    pmm_dir_t *d = pmm_opendir(abs_dir);
    if (!d) {
        return;
    }
    ...
    pmm_dirent_t *ent;
    while ((ent = pmm_readdir(d)) != NULL && *count < max_count) {
        if (!ent->is_dir) {
            continue;
        }
        ...
        char child_abs[PMM_SZ_512];
        char child_rel[PMM_SZ_256];
        ...
        if (pmm_pipeline_relpath_is_excluded(child_rel, excluded_dirs, excluded_count)) {
            continue;
        }
...
pmm_path_alias_collection_t *pmm_load_path_aliases_excluded(const char *repo_path,
                                                            char **excluded_dirs,
                                                            int excluded_count) {
    if (!repo_path) {
        return NULL;
    }
    alias_config_hit_t *hits = calloc(PMM_PATH_ALIAS_MAX_FILES, sizeof(*hits));
    if (!hits) {
        return NULL;
    }
    int count = 0;
    find_alias_files(repo_path, "", hits, &count, PMM_PATH_ALIAS_MAX_FILES, 0, excluded_dirs,
                     excluded_count);
    if (count >= PMM_PATH_ALIAS_MAX_FILES) {
        pmm_log_warn("path_alias.files.cap_hit", "repo", repo_path, "kept", "256_of_more");
    }
    if (count == 0) {
        free(hits);
        return NULL;
    ...
    pmm_path_alias_collection_t *coll = calloc(1, sizeof(*coll));
    if (!coll) {
        free(hits);
        return NULL;
    }
    coll->scopes = calloc((size_t)count, sizeof(pmm_path_alias_scope_t));
    if (!coll->scopes) {
        free(coll);
        free(hits);
        return NULL;
    ...
    for (int i = 0; i < count; i++) {
        pmm_path_alias_map_t *map = load_tsconfig_file(hits[i].abs, hits[i].rel);
        if (!map) {
            continue;
        }
    ...
    qsort(coll->scopes, (size_t)coll->count, sizeof(pmm_path_alias_scope_t),
          cmp_scope_by_specificity);
    return coll;
}

pmm_path_alias_collection_t *pmm_load_path_aliases(const char *repo_path) {
    return pmm_load_path_aliases_excluded(repo_path, NULL, 0);
}

const pmm_path_alias_map_t *pmm_path_alias_find_for_file(const pmm_path_alias_collection_t *coll,
                                                         const char *rel_path) {
    if (!coll || !rel_path) {
        return NULL;
    }
