...
#include "foundation/str_util.h" /* pmm_validate_shell_arg — git shell-out hardening */
...
static _Thread_local char g_export_error[PMM_SZ_512];

static const char *itoa_buf(int v) {
    static _Thread_local char bufs[ART_RING][PMM_SZ_32];
...
static int artifact_export_fail(const char *stage, const char *path, const char *err, int err_no) {
    ...
    if (path && err_no != 0) {
        pmm_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "errno",
                      itoa_buf(err_no), "path", path);
    } else if (path) {
        pmm_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "path", path);
    } else if (err_no != 0) {
        pmm_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "errno",
                      itoa_buf(err_no));
    } else {
        pmm_log_error("artifact.export", "stage", safe_stage, "err", safe_err);
    }
...
/* Build path: <repo>/.project-mind-mcp/<name> into caller-owned buf. */
static bool artifact_path(char *buf, size_t bufsz, const char *repo_path, const char *name) {
    int n = snprintf(buf, bufsz, "%s/%s/%s", repo_path, PMM_ARTIFACT_DIR, name);
 ...
 * git shell-out): pmm_validate_shell_arg rejects quote / backslash / substitution
...
bool pmm_artifact_repo_path_is_shell_safe(const char *repo_path) {
    if (!pmm_validate_shell_arg(repo_path)) {
        return false;
    }
#ifdef _WIN32
...
/* Get current git HEAD hash. buf must be >= PMM_SZ_64. Returns false on error. */
static bool git_head_hash(const char *repo_path, char *buf, size_t bufsz) {
    char cmd[PMM_SZ_1K];
    if (!pmm_artifact_repo_path_is_shell_safe(repo_path)) {
        buf[0] = '\0';
        return false;
    }
    int n =
...
static int read_metadata_version(const char *repo_path) {
    char meta_path[PMM_SZ_4K];
    artifact_path(meta_path, sizeof(meta_path), repo_path, PMM_ARTIFACT_META);
    ...
    int version = ver ? yyjson_get_int(ver) : PMM_NOT_FOUND;
    yyjson_doc_free(doc);
    return version;
}
...
static int write_metadata(const char *repo_path, const char *project_name, int nodes, int edges,
    ...
    yyjson_mut_obj_add_int(doc, root, "schema_version", PMM_ARTIFACT_SCHEMA_VERSION);
    ...
    char meta_path[PMM_SZ_4K];
    if (!artifact_path(meta_path, sizeof(meta_path), repo_path, PMM_ARTIFACT_META)) {
        free(json);
...
static void ensure_gitattributes(const char *repo_path) {
    ...
    if (fd < 0) {
        if (errno != EEXIST) {
            pmm_log_warn("artifact.gitattributes.open path=%s err=%s", ga_path, strerror(errno));
        }
        ...
        if (fp) {
            ...
            (void)fputs("# Auto-generated by project-mind-mcp\n"
                        "# Prevent merge conflicts on compressed artifact\n" PMM_ARTIFACT_FILENAME
                        " binary merge=ours\n",
                        fp);
            (void)fclose(fp);
        } else {
 ...
 * pmm_mkstemp would — sqlite has to be the one that creates it. Containing it in
 * a directory only this user can enter buys the same protection: pmm_mkdtemp
 ...
 * "<tmp>/pmm_artifact_tmp.db" was vulnerable on both counts — another local user
 * could pre-plant a symlink there to redirect the copy, and two concurrent
 * exports collided on the one name. */
typedef struct {
    char dir[PMM_SZ_512]; /* pmm_mkdtemp copies its result back into this buffer */
    char db[PMM_SZ_4K];
...
static bool artifact_snapshot_tmp_open(artifact_snapshot_tmp_t *tmp) {
    tmp->dir[0] = '\0';
    tmp->db[0] = '\0';
    int written = snprintf(tmp->dir, sizeof(tmp->dir), "%s/pmm-artifact-XXXXXX", pmm_tmpdir());
    if (written <= 0 || (size_t)written >= sizeof(tmp->dir) || !pmm_mkdtemp(tmp->dir)) {
        tmp->dir[0] = '\0';
        return false;
    }
    written = snprintf(tmp->db, sizeof(tmp->db), "%s/snapshot.db", tmp->dir);
    if (written <= 0 || (size_t)written >= sizeof(tmp->db)) {
        (void)pmm_rmdir(tmp->dir);
        tmp->dir[0] = '\0';
        return false;
    }
    return true;
...
static char *prepare_snapshot_db(const char *db_path, size_t *out_size, bool strip_indexes) {
    ...
    if (!artifact_snapshot_tmp_open(&tmp)) {
        artifact_export_fail("prepare_snapshot_dir", pmm_tmpdir(), "private_tmpdir_failed", errno);
        return NULL;
    }
...
int pmm_artifact_export(const char *db_path, const char *repo_path, const char *project_name,
    ...
    if (!db_data || db_size == 0) {
        free(db_data);
        if (pmm_artifact_export_last_error()) {
            return PMM_NOT_FOUND;
        }
    ...
    /* Compress with zstd */
    size_t bound = pmm_zstd_compress_bound((int)db_size);
    ...
    int clen = pmm_zstd_compress(db_data, (int)db_size, compressed, (int)bound, compression_level);
    free(db_data);
    ...
    char zst_path[PMM_SZ_4K];
    if (!artifact_path(zst_path, sizeof(zst_path), repo_path, PMM_ARTIFACT_FILENAME)) {
        free(compressed);
    ...
    int nodes = 0;
    int edges = 0;
    pmm_store_t *count_store = pmm_store_open_path(db_path);
    if (count_store) {
        nodes = pmm_store_count_nodes(count_store, project_name);
        edges = pmm_store_count_edges(count_store, project_name);
        pmm_store_close(count_store);
    }
    ...
    pmm_log_info("artifact.export", "quality", quality == PMM_ARTIFACT_BEST ? "best" : "fast",
...
int pmm_artifact_import(const char *repo_path, const char *cache_db_path) {
    ...
    if (version < 0 || version > PMM_ARTIFACT_SCHEMA_VERSION) {
        pmm_log_info("artifact.import", "skip", "schema_version_mismatch", "artifact_ver",
                     itoa_buf(version), "current_ver", itoa_buf(PMM_ARTIFACT_SCHEMA_VERSION));
        return PMM_NOT_FOUND;
    }
    ...
    if (original_size == 0) {
        pmm_log_error("artifact.import", "err", "missing_original_size");
        return PMM_NOT_FOUND;
    }

    /* Read compressed artifact */
    char zst_path[PMM_SZ_4K];
    artifact_path(zst_path, sizeof(zst_path), repo_path, PMM_ARTIFACT_FILENAME);

    size_t clen = 0;
    char *compressed = read_file_alloc(zst_path, &clen);
    if (!compressed) {
        pmm_log_error("artifact.import", "err", "read_artifact");
        return PMM_NOT_FOUND;
    }
    ...
    size_t frame_size = pmm_zstd_frame_content_size(compressed, clen);
    if (frame_size == 0 || frame_size > ART_MAX_DECOMPRESSED_BYTES || frame_size != original_size) {
        free(compressed);
        pmm_log_error("artifact.import", "err", "bad_decompressed_size");
        return PMM_NOT_FOUND;
    }
    ...
    int64_t dlen = pmm_zstd_decompress(compressed, clen, decompressed, frame_size);
    free(compressed);

    if (dlen <= 0 || (size_t)dlen != frame_size) {
        free(decompressed);
        pmm_log_error("artifact.import", "err", "zstd_decompress");
        return PMM_NOT_FOUND;
    }

    /* Write to temp file, then rename for atomicity */
    char tmp_path[PMM_SZ_4K];
    ...
    if (last_slash) {
        *last_slash = '\0';
        pmm_mkdir_p(cache_dir, ART_DIR_PERMS);
    }
    ...
    if (wrc != 0) {
        if (ioerr.err_no != 0) {
            pmm_log_error("artifact.import", "err", "write_temp_db", "detail", ioerr.err, "errno",
                          itoa_buf(ioerr.err_no), "path", tmp_path);
        } else {
            pmm_log_error("artifact.import", "err", "write_temp_db", "detail", ioerr.err, "path",
                          tmp_path);
        }
        return PMM_NOT_FOUND;
    }

    /* Open with pmm_store_open_path to auto-create missing indexes + FTS5 */
    pmm_store_t *store = pmm_store_open_path(tmp_path);
    if (!store) {
        pmm_log_error("artifact.import", "err", "open_imported_db");
        pmm_unlink(tmp_path);
        return PMM_NOT_FOUND;
    }
    ...
    if (!pmm_store_check_integrity_deep(store)) {
        pmm_log_error("artifact.import", "err", "integrity_check_failed");
    ...
    pmm_remove_db_sidecars(cache_db_path);
    if (rename(tmp_path, cache_db_path) != 0) {
        pmm_log_error("artifact.import", "err", "rename_to_cache");
        pmm_unlink(tmp_path);
        return PMM_NOT_FOUND;
    }
    ...
    pmm_log_info("artifact.import", "db", cache_db_path, "size_mb",
...
bool pmm_artifact_exists(const char *repo_path) {
    ...
    int version = read_metadata_version(repo_path);
    return version >= 0 && version <= PMM_ARTIFACT_SCHEMA_VERSION;
}
