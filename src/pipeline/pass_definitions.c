 ...
 *   2. Call pmm_extract_file() to get defs, calls, imports
 ...
 *
 * Depends on: extraction layer (pmm.h), graph_buffer, pipeline internals
 */
...
static const char *itoa_log(int val) {
    static PMM_TLS char bufs[PD_RING][PMM_SZ_32];
...
static void process_def(pmm_pipeline_ctx_t *ctx, const CBMDefinition *def, const char *rel) {
    if (!def->qualified_name || !def->name) {
        return;
    }
    char props[PMM_SZ_2K];
    build_def_props(props, sizeof(props), def);
    int64_t node_id = pmm_gbuf_upsert_node(
    ...
    if (file_node && node_id > 0) {
        pmm_gbuf_insert_edge(ctx->gbuf, file_node->id, node_id, "DEFINES", "{}");
    }
    free(file_qn);
    if (def->parent_class && def->label && strcmp(def->label, "Method") == 0) {
        ...
        if (parent && node_id > 0) {
            pmm_gbuf_insert_edge(ctx->gbuf, parent->id, node_id, "DEFINES_METHOD", "{}");
        }
    }
}

/* Create Channel nodes + EMITS / LISTENS_ON edges for one file's channels.
 * Mirrors the parallel path in pmm_build_registry_from_cache — keep in sync. */
/* Find the source node for a channel edge: enclosing function or file node. */
static const pmm_gbuf_node_t *find_channel_source(pmm_pipeline_ctx_t *ctx, const CBMChannel *ch,
                                                  const char *rel) {
...
static void create_channel_edges_for_file(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result,
                                          const char *rel) {
    for (int j = 0; j < result->channels.count; j++) {
        ...
        const pmm_gbuf_node_t *src_node = find_channel_source(ctx, ch, rel);
...
static int create_env_configures_for_file(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result,
    ...
    for (int j = 0; j < result->env_accesses.count; j++) {
        ...
        if (!src) {
            if (!file_qn) {
                file_qn = pmm_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
                file_node = pmm_gbuf_find_by_qn(ctx->gbuf, file_qn);
            }
            src = file_node;
        }
        if (src && src->id != env_id) {
            pmm_gbuf_insert_edge(ctx->gbuf, src->id, env_id, "CONFIGURES",
                                 "{\"strategy\":\"env_access\"}");
            count++;
        }
    }
...
static int create_import_edges_for_file(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result,
...
int pmm_pipeline_pass_definitions(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files,
                                  int file_count) {
    pmm_log_info("pass.start", "pass", "definitions", "files", itoa_log(file_count));
     ...
     * here so the first pmm_extract_file below recreates a fresh parser. */
    ...
    for (int i = 0; i < file_count; i++) {
         ...
         * but hit the hard guard inside pmm_extract_file, so they no-op without
         * re-crashing and without duplicating the skip. No-op unless
         * PMM_INDEX_QUARANTINE_FILE is set. */
        if (pmm_index_is_quarantined(rel)) {
            const char *phase = pmm_index_quarantine_phase(rel);
            if (!phase) {
                phase = "crash";
            }
            const char *reason =
                (strcmp(phase, "hang") == 0) ? "quarantined after hang" : "quarantined after crash";
            pmm_pipeline_add_file_error(ctx->pipeline, rel, reason, phase);
            errors++;
            continue;
        }
        ...
        if (!source) {
            ...
            if (rst == PMM_READ_OVERSIZED) {
                ...
                pmm_pipeline_add_file_error(ctx->pipeline, rel, reason, "oversized");
                pmm_log_warn("index.file_oversized", "path", rel, "size_mb",
                ...
                pmm_pipeline_add_file_error(ctx->pipeline, rel, "read failed", "read");
            }
            /* PMM_READ_EMPTY: benign 0-byte file — nothing to index, not reported. */
            continue;
        }

        /* ObjectScript Studio Export XML: transcode each <Class> to UDL and
         * extract it as PMM_LANG_OBJECTSCRIPT_UDL. The XML→UDL mapping is 1:1,
         * so the same UDL extractor handles the result. These files are not
         * cached (their defs/edges are emitted directly here). */
        if (lang == PMM_LANG_OBJECTSCRIPT_EXPORT) {
            CBMArena export_arena;
            pmm_arena_init(&export_arena);
            int class_count = 0;
            char **udl_strings =
                pmm_iris_export_to_udl(&export_arena, source, source_len, &class_count);
            free(source);
            for (int ci = 0; ci < class_count; ci++) {
                CBMFileResult *xr = pmm_extract_file_ex(
                    udl_strings[ci], (int)strlen(udl_strings[ci]), PMM_LANG_OBJECTSCRIPT_UDL,
                    ctx->project_name, rel, PMM_EXTRACT_BUDGET, NULL, NULL, ctx->macro_table, NULL);
                if (!xr) {
                    continue;
                }
            ...
            pmm_arena_destroy(&export_arena);
            continue;
        }

        /* Extract */
        CBMFileResult *result = pmm_extract_file_ex(
            source, source_len, lang, ctx->project_name, rel, PMM_EXTRACT_BUDGET, NULL,
            NULL /* no extra defines or include paths */, ctx->macro_table, NULL);
        free(source);

        if (!result) {
            errors++;
            pmm_pipeline_add_file_error(ctx->pipeline, rel, "extract failed", "extract");
            continue;
        }
        ...
        if (result->has_error) {
            pmm_pipeline_add_file_error(ctx->pipeline, rel,
    ...
    if (local_cache) {
        ...
        CBMHashTable *namespace_map =
            pmm_pipeline_namespace_map_build(ctx->project_name, local_cache, rels, file_count);
        free(rels);
        for (int i = 0; i < file_count; i++) {
            if (pmm_pipeline_check_cancel(ctx)) {
                break;
    ...
    pmm_log_info("pass.done", "pass", "definitions", "defs", itoa_log(total_defs), "calls",
