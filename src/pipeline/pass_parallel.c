 ...
 *
 * Depends on: worker_pool, graph_buffer (shared IDs + merge), extraction (pmm.h)
 */
 ...
 * (source_reread fallback). Both caps are env-overridable — PMM_RETAIN_TOTAL_MB
 * / PMM_RETAIN_PER_FILE_MB raise or lower the auto-derived defaults directly
...
#include "pipeline/pass_lsp_cross.h" /* pmm_pxc_* helpers for fused cross-file LSP */
#include "pipeline/lsp_resolve.h"
#include "helpers.h" /* pmm_kind_in_set_free_cache — per-worker-thread cache teardown */
...
/* Parse a positive MB-valued retention env knob (PMM_RETAIN_*_MB) into bytes.
 * Follows the limits.c strtol convention: unset / unparseable / non-positive
 * → return 0 so the caller keeps its derived default. */
static size_t pmm_retain_env_bytes(const char *name) {
 ...
 * the ceiling. */
static size_t pmm_parallel_extract_default_total_cap(void) {
    size_t cap = pmm_mem_budget();
    if (cap > 0) {
        cap /= 8;
    } else {
...
static size_t pmm_parallel_extract_default_per_file_cap(size_t total_cap) {
 ...
 * explicit caller opts > PMM_RETAIN_*_MB env knobs > RAM-derived defaults.
...
static pmm_parallel_extract_opts_t pmm_parallel_extract_resolve_opts(
    const pmm_parallel_extract_opts_t *opts) {
    pmm_parallel_extract_opts_t resolved = {
        .retain_sources = true,
        .retain_sources_set = true,
        .retain_total_budget_bytes = pmm_parallel_extract_default_total_cap(),
        .retain_per_file_max_bytes = 0,
    };

    size_t env_total = pmm_retain_env_bytes("PMM_RETAIN_TOTAL_MB");
    if (env_total > 0) {
        resolved.retain_total_budget_bytes = env_total;
    }

    resolved.retain_per_file_max_bytes =
        pmm_parallel_extract_default_per_file_cap(resolved.retain_total_budget_bytes);
    size_t env_per_file = pmm_retain_env_bytes("PMM_RETAIN_PER_FILE_MB");
    if (env_per_file > 0) {
...
static uint64_t extract_now_ns(void) {
    struct timespec ts;
    pmm_clock_gettime(CLOCK_MONOTONIC, &ts);
...
static char *read_file(const char *path, int *out_len, long *out_size,
                       pmm_read_status_t *out_status) {
    if (out_size) {
        *out_size = 0;
    }
    if (out_status) {
        *out_status = PMM_READ_OK;
    }
    FILE *f = pmm_fopen(path, "rb");
    if (!f) {
        if (out_status) {
            *out_status = PMM_READ_OPEN_FAIL;
        }
        return NULL;
    }
    ...
    if (size <= 0) {
        (void)fclose(f);
        if (out_status) {
            *out_status = PMM_READ_EMPTY;
        }
        return NULL;
    }
    if (size > pmm_max_file_bytes()) { /* generous, env-configurable cap (B4) */
        (void)fclose(f);
        if (out_status) {
            *out_status = PMM_READ_OVERSIZED;
        }
        return NULL;
    }
 ...
 * pmm_file_error_t array in the existing sequential merge loop. */
typedef struct {
    pmm_file_error_t *items;
    int count;
    int cap;
} pp_err_list_t;
...
static void pp_err_add(pp_err_list_t *list, const char *path, const char *reason,
    ...
    if (list->count >= list->cap) {
        ...
        pmm_file_error_t *grown =
            (pmm_file_error_t *)realloc(list->items, (size_t)ncap * sizeof(*grown));
        if (!grown) {
...
static const char *itoa_log(int val) {
    static PMM_TLS char bufs[PP_RING][PMM_SZ_32];
...
static void build_def_props(char *buf, size_t bufsize, const CBMDefinition *def) {
    ...
    if (def->fingerprint && def->fingerprint_k > 0 &&
        pos + PMM_MINHASH_HEX_LEN + PMM_MINHASH_JSON_OVERHEAD < bufsize) {
        char fp_hex[PMM_MINHASH_HEX_BUF];
        pmm_minhash_to_hex((const pmm_minhash_t *)def->fingerprint, fp_hex, sizeof(fp_hex));
        append_json_string(buf, bufsize, &pos, "fp", fp_hex);
    }

    /* AST structural profile — append if present and buffer has room. */
    if (def->structural_profile && pos + PMM_AST_PROFILE_BUF < bufsize) {
    ...
    if (def->body_tokens && pos + PMM_SZ_512 < bufsize) {
...
static int build_import_map(const pmm_gbuf_t *gbuf, const char *project_name, const char *rel_path,
                            const char ***out_keys, const char ***out_vals, int *out_count) {
    *out_keys = NULL;
    *out_vals = NULL;
    *out_count = 0;

    char *file_qn = pmm_pipeline_fqn_compute(project_name, rel_path, "__file__");
    const pmm_gbuf_node_t *file_node = pmm_gbuf_find_by_qn(gbuf, file_qn);
    free(file_qn);
    if (!file_node) {
        return 0;
    }

    const pmm_gbuf_edge_t **edges = NULL;
    int edge_count = 0;
    int rc =
        pmm_gbuf_find_edges_by_source_type(gbuf, file_node->id, "IMPORTS", &edges, &edge_count);
    ...
    for (int i = 0; i < edge_count; i++) {
        const pmm_gbuf_edge_t *e = edges[i];
        const pmm_gbuf_node_t *target = pmm_gbuf_find_by_id(gbuf, e->target_id);
        ...
        if (start) {
            ...
            if (end && end > start) {
                keys[count] = pmm_strndup(start, end - start);
 ...
 * (Java/Go package). MUST match pmm_lang_module_is_dir() (internal/pmm/helpers.c)
...
static bool pp_module_is_dir(CBMLanguage lang) {
    return lang == PMM_LANG_JAVA || lang == PMM_LANG_GO;
}
...
static const char *resolve_as_class(const pmm_registry_t *reg, const char *name,
                                    const char *module_qn, const char **imp_keys,
                                    const char **imp_vals, int imp_count) {
    pmm_resolution_t res =
        pmm_registry_resolve(reg, name, module_qn, imp_keys, imp_vals, imp_count);
    ...
    const char *label = pmm_registry_label_of(reg, res.qualified_name);
    if (!pmm_label_is_type_like(label)) {
        return NULL;
    }
...
#define PMM_CACHE_LINE PMM_SZ_128

typedef struct __attribute__((aligned(PMM_CACHE_LINE))) {
    pmm_gbuf_t *local_gbuf;
    int nodes_created;
    int errors;
    char _pad[PMM_CACHE_LINE - sizeof(pmm_gbuf_t *) - (PP_ESC_SPACE * sizeof(int))];
} extract_worker_state_t;

typedef struct {
    const pmm_file_info_t *files;
    ...
    pmm_pkg_entries_t *pkg_entries; /* per-worker manifest arrays (separate allocation) */
...
static void insert_def_into_gbuf(extract_worker_state_t *ws, const pmm_file_info_t *fi,
                                 CBMDefinition *def) {
    char props[PMM_SZ_2K];
    build_def_props(props, sizeof(props), def);
    int64_t func_id =
        pmm_gbuf_upsert_node(ws->local_gbuf, def->label ? def->label : "Function", def->name,
    ...
    if (def->route_path && def->route_path[0] != '\0') {
        ...
        char route_qn[PMM_ROUTE_QN_SIZE];
        char cpath[PMM_SZ_256];
        snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", rm,
                 pmm_route_canon_path(def->route_path, cpath, sizeof(cpath)));
        char rprops[PMM_SZ_256];
        snprintf(rprops, sizeof(rprops), "{\"method\":\"%s\",\"source\":\"decorator\"}", rm);
        int64_t route_id =
            pmm_gbuf_upsert_node(ws->local_gbuf, "Route", def->route_path, route_qn,
                                 def->file_path ? def->file_path : fi->rel_path, 0, 0, rprops);
        char hprops[PMM_SZ_512];
        char esc_h[PMM_SZ_512];
        pmm_json_escape(esc_h, sizeof(esc_h), def->qualified_name);
        snprintf(hprops, sizeof(hprops), "{\"handler\":\"%s\"}", esc_h);
        pmm_gbuf_insert_edge(ws->local_gbuf, func_id, route_id, "HANDLES", hprops);
    }
}

static void log_extract_fail(int pos, uint64_t ms, const char *path) {
    if (pos < PP_LOG_THRESH) {
        pmm_log_warn("parallel.extract.file.fail", "pos", itoa_log(pos), "elapsed_ms",
...
static void log_extract_done(int pos, uint64_t ms, int defs, const char *path) {
    if (pos < PP_LOG_THRESH || ms > PP_TIMER_THRESH) {
        pmm_log_info("parallel.extract.file.done", "pos", itoa_log(pos), "elapsed_ms",
...
static void extract_worker(int worker_id, void *ctx_ptr) {
    ...
    /* Lazy gbuf creation */
    if (!ws->local_gbuf) {
        ws->local_gbuf = pmm_gbuf_new_shared_ids(ec->project_name, ec->repo_path, ec->shared_ids);
    }
    ...
    while (SKIP_ONE) {
        ...
        if (pmm_mem_budget() > 0) {
            ...
            if (over && !futile) {
                ...
                int bp = 0;
                for (; bp < PP_BACKPRESSURE_MAX_SPINS && pmm_mem_over_budget() &&
                       !atomic_load_explicit(ec->cancelled, memory_order_relaxed);
                     bp++) {
                    struct timespec nap = {0, PP_BACKPRESSURE_NAP_NS};
                    pmm_nanosleep(&nap, NULL);
                }
                if (bp == PP_BACKPRESSURE_MAX_SPINS && pmm_mem_over_budget()) {
                    ...
                    if (atomic_exchange_explicit(&ec->bp_futile, 1, memory_order_relaxed) == 0) {
                        pmm_log_warn("mem.backpressure.futile", "action", "soft_overshoot");
                    }
                }
            } else if (!over && futile) {
        ...
        const pmm_file_info_t *fi = &ec->files[file_idx];
         ...
         * PMM_INDEX_QUARANTINE_FILE. Covers the parallel path; the supervisor's
         * single-threaded recovery run instead takes the sequential path
         * (pass_definitions.c), and pmm_extract_file's hard guard backstops both. */
        if (pmm_index_is_quarantined(fi->rel_path)) {
            const char *phase = pmm_index_quarantine_phase(fi->rel_path);
            if (!phase) {
                phase = "crash";
            }
            const char *reason =
        ...
        if (!source) {
            ...
            if (rst == PMM_READ_OVERSIZED) {
                ...
                if (atomic_fetch_add_explicit(&ec->oversized_warned, SKIP_ONE,
                    ...
                    pmm_log_warn("index.file_oversized", "path", fi->rel_path, "size_mb",
                                 itoa_log((int)(file_size / (PMM_SZ_1K * PMM_SZ_1K))), "cap_mb",
                                 itoa_log((int)(cap / (PMM_SZ_1K * PMM_SZ_1K))));
                }
            } else if (rst == PMM_READ_OPEN_FAIL || rst == PMM_READ_OOM) {
                pp_err_add(errs, fi->rel_path, "read failed", "read");
            }
            /* PMM_READ_EMPTY: benign 0-byte file — not reported. */
            continue;
        }
        ...
        if (sort_pos < PP_LOG_THRESH) { /* first 2 rounds of workers = most interesting */
            pmm_log_info("parallel.extract.file.start", "pos", itoa_log(sort_pos), "size_kb",
                         itoa_log(source_len / PMM_SZ_1K), "path", fi->rel_path);
        }
        ...
        if (fi->language == PMM_LANG_OBJECTSCRIPT_EXPORT) {
            CBMArena ea;
            pmm_arena_init(&ea);
            int cc = 0;
            char **udls = pmm_iris_export_to_udl(&ea, source, source_len, &cc);
            for (int ci = 0; ci < cc; ci++) {
                CBMFileResult *xr =
                    pmm_extract_file_ex(udls[ci], (int)strlen(udls[ci]), PMM_LANG_OBJECTSCRIPT_UDL,
                                        ec->project_name, fi->rel_path, PMM_EXTRACT_BUDGET, NULL,
            ...
            pmm_arena_destroy(&ea);
            free_source(source);
            continue;
        }

        CBMFileResult *result = pmm_extract_file_ex(
            source, source_len, fi->language, ec->project_name, fi->rel_path, PMM_EXTRACT_BUDGET,
        ...
        pmm_free_tree(result);

        /* Detect and parse manifest files for package map */
        {
            const char *bn = strrchr(fi->rel_path, '/');
            pmm_pkgmap_try_parse(bn ? bn + SKIP_ONE : fi->rel_path, fi->rel_path, source,
        ...
        if (ec->retain_sources && source_len > 0) {
            ...
            if ((size_t)source_len > ec->retain_per_file_max_bytes) {
                ...
                if ((size_t)(prior + (int64_t)source_len) <= ec->retain_total_budget_bytes) {
                    char *copy = (char *)pmm_arena_alloc(&result->arena, (size_t)source_len + 1);
                    if (copy) {
            ...
            if (dropped &&
                ...
                pmm_log_warn("index.retain_capped", "path", fi->rel_path ? fi->rel_path : "?",
        ...
        if ((sort_pos + SKIP_ONE) % PP_LOG_INTERVAL == 0 || sort_pos + SKIP_ONE == ec->file_count) {
            pmm_log_info("parallel.extract.progress", "done", itoa_log(sort_pos + SKIP_ONE),
                         "total", itoa_log(ec->file_count));
        }

        /* Reclaim all slab + tier2 memory between files.
         *
         * After pmm_free_tree(result), all tree nodes are on free lists.
         ...
         *
         * get_thread_parser() in pmm_extract_file will create a fresh
        ...
        pmm_destroy_thread_parser();
        pmm_slab_reclaim();
        pmm_mem_collect();
    }

    /* Final cleanup (parser already destroyed in loop, just slab state) */
    pmm_slab_destroy_thread();
    pmm_kind_in_set_free_cache(); /* free this worker thread's node-type bitset cache */
}

static void merge_pkg_entries(pmm_pipeline_ctx_t *ctx, pmm_pkg_entries_t *pkg_entries,
                              int worker_count) {
    if (!pkg_entries) {
        return;
    }
    ...
    pmm_pkgmap_scan_repo(ctx->repo_path, &pkg_entries[0], ctx->excluded_dirs, ctx->excluded_count);
    pmm_pipeline_set_pkgmap(pmm_pkgmap_build(pkg_entries, worker_count, ctx->project_name));
    for (int i = 0; i < worker_count; i++) {
        pmm_pkg_entries_free(&pkg_entries[i]);
    }
    free(pkg_entries);
}
...
static void log_extract_mem_stats(int worker_count) {
    if (pmm_mem_budget() > 0) {
        size_t mb = (size_t)PMM_SZ_1K * PMM_SZ_1K;
        pmm_log_info("parallel.extract.mem", "rss_mb", itoa_log((int)(pmm_mem_rss() / mb)),
                     "peak_mb", itoa_log((int)(pmm_mem_peak_rss() / mb)), "budget_mb",
                     itoa_log((int)(pmm_mem_budget() / mb)), "per_worker_mb",
                     itoa_log((int)(pmm_mem_worker_budget(worker_count) / mb)));
    }
}

/* Forward declaration: macro table builder lives in pipeline.c (shared path). */
CBMMacroTable *pmm_build_macro_table_from_files(const pmm_file_info_t *files, int count,
                                                const char *repo_path);
...
int pmm_parallel_extract_ex(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files, int file_count,
    ...
    pmm_parallel_extract_opts_t resolved_opts = pmm_parallel_extract_resolve_opts(opts);

    if (file_count == 0) {
        return 0;
    }

    pmm_log_info("parallel.extract.start", "files", itoa_log(file_count), "workers",
                 itoa_log(worker_count));
    {
        size_t mb = (size_t)PMM_SZ_1K * PMM_SZ_1K;
        pmm_log_info("parallel.extract.retention", "retain_sources",
    ...
    if (pmm_mem_budget() > 0) {
        size_t worker_budget = pmm_mem_worker_budget(worker_count);
        pmm_log_info("parallel.mem.budget", "total_mb",
                     itoa_log((int)(pmm_mem_budget() / ((size_t)PMM_SZ_1K * PMM_SZ_1K))),
                     "per_worker_mb",
                     itoa_log((int)(worker_budget / ((size_t)PMM_SZ_1K * PMM_SZ_1K))));
    }

    /* Sub-phase: Ensure extraction library is initialized */
    PMM_PROF_START(t_init);
    pmm_init();
    ...
    pmm_destroy_thread_parser();
    pmm_slab_install();
    PMM_PROF_END("parallel_extract", "1_init_libs", t_init);

    /* Sub-phase: Sort files by descending size for tail-latency reduction */
    PMM_PROF_START(t_sort);
    ...
    PMM_PROF_END_N("parallel_extract", "2_sort_files", t_sort, file_count);

    /* Allocate per-worker state (cache-line aligned via posix_memalign) */
    extract_worker_state_t *workers = NULL;
    if (pmm_aligned_alloc((void **)&workers, PMM_CACHE_LINE,
    ...
    pmm_pkg_entries_t *pkg_entries = calloc((size_t)worker_count, sizeof(pmm_pkg_entries_t));
    if (!pkg_entries) {
        pmm_aligned_free(workers);
        free(sorted);
        return PMM_NOT_FOUND;
    }
    ...
    CBMMacroTable *pp_macro_table =
        pmm_build_macro_table_from_files(files, file_count, ctx->repo_path);
    ...
    PMM_PROF_START(t_dispatch);
    pmm_parallel_for_opts_t parallel_opts = {.max_workers = worker_count, .force_pthreads = false};
    pmm_parallel_for(worker_count, extract_worker, &ec, parallel_opts);
    PMM_PROF_END_N("parallel_extract", "3_dispatch_workers_parallel", t_dispatch, file_count);
    ...
    for (int i = 0; i < worker_count; i++) {
        if (workers[i].local_gbuf) {
            pmm_gbuf_merge(ctx->gbuf, workers[i].local_gbuf);
            total_nodes += workers[i].nodes_created;
            total_errors += workers[i].errors;
            pmm_gbuf_free(workers[i].local_gbuf);
        }
    }
    PMM_PROF_END_N("parallel_extract", "4_merge_gbufs_seq", t_merge, total_nodes);
    ...
    if (err_lists) {
        for (int i = 0; i < worker_count; i++) {
            for (int j = 0; j < err_lists[i].count; j++) {
                pmm_pipeline_add_file_error(ctx->pipeline, err_lists[i].items[j].path,
    ...
    pmm_aligned_free(workers);
    free(sorted);
    pmm_macro_table_free(pp_macro_table); /* ObjectScript macro table (NULL-safe) */
    ...
    pmm_log_info("parallel.extract.done", "nodes", itoa_log(total_nodes), "errors",
...
int pmm_parallel_extract(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files, int file_count,
                         CBMFileResult **result_cache, _Atomic int64_t *shared_ids,
                         int worker_count) {
    return pmm_parallel_extract_ex(ctx, files, file_count, result_cache, shared_ids, worker_count,
                                   NULL);
}
...
static int register_and_link_def(pmm_pipeline_ctx_t *ctx, const CBMDefinition *def, const char *rel,
                                 int *reg_entries) {
    int edges = 0;
    ...
    if (strcmp(def->label, "Function") == 0 || strcmp(def->label, "Method") == 0 ||
        pmm_label_is_type_like(def->label) || strcmp(def->label, "Variable") == 0 ||
        strcmp(def->label, "Field") == 0) {
        pmm_registry_add(ctx->registry, def->name, def->qualified_name, def->label);
        (*reg_entries)++;
    }
    char *file_qn = pmm_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
    const pmm_gbuf_node_t *file_node = pmm_gbuf_find_by_qn(ctx->gbuf, file_qn);
    const pmm_gbuf_node_t *def_node = pmm_gbuf_find_by_qn(ctx->gbuf, def->qualified_name);
    if (file_node && def_node) {
        pmm_gbuf_insert_edge(ctx->gbuf, file_node->id, def_node->id, "DEFINES", "{}");
        edges++;
    }
    free(file_qn);
    if (def->parent_class && strcmp(def->label, "Method") == 0) {
        const pmm_gbuf_node_t *parent = pmm_gbuf_find_by_qn(ctx->gbuf, def->parent_class);
        if (parent && def_node) {
            pmm_gbuf_insert_edge(ctx->gbuf, parent->id, def_node->id, "DEFINES_METHOD", "{}");
        }
    }
    return edges;
}
...
static int create_imports_edges(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result,
                                const char *rel, CBMHashTable *namespace_map) {
    int count = 0;
    char *file_qn = pmm_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
    const pmm_gbuf_node_t *source_node = pmm_gbuf_find_by_qn(ctx->gbuf, file_qn);
    if (!source_node) {
        free(file_qn);
        return 0;
    }
    for (int j = 0; j < result->imports.count; j++) {
        ...
        const pmm_gbuf_node_t *target =
            pmm_pipeline_resolve_import_node(ctx, rel, file_qn, imp, namespace_map);
        if (target && target->id != source_node->id) {
            char esc_ln[PMM_SZ_128];
            pmm_json_escape(esc_ln, sizeof(esc_ln), imp->local_name ? imp->local_name : "");
            char imp_props[PMM_SZ_256];
            snprintf(imp_props, sizeof(imp_props), "{\"local_name\":\"%s\"}", esc_ln);
            pmm_gbuf_insert_edge(ctx->gbuf, source_node->id, target->id, "IMPORTS", imp_props);
            count++;
        }
    }
    free(file_qn);
...
static const pmm_gbuf_node_t *find_channel_src(pmm_pipeline_ctx_t *ctx, const CBMChannel *ch,
                                               const char *rel) {
    const pmm_gbuf_node_t *node = NULL;
    if (ch->enclosing_func_qn && ch->enclosing_func_qn[0]) {
        node = pmm_gbuf_find_by_qn(ctx->gbuf, ch->enclosing_func_qn);
    }
    if (!node) {
        char *file_qn = pmm_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
        node = pmm_gbuf_find_by_qn(ctx->gbuf, file_qn);
        free(file_qn);
    }
    return node;
}
...
static void create_channel_edges(pmm_pipeline_ctx_t *ctx, const CBMFileResult *result,
                                 const char *rel) {
    for (int j = 0; j < result->channels.count; j++) {
        ...
        char channel_qn[PMM_SZ_512];
        snprintf(channel_qn, sizeof(channel_qn), "__channel__%s__%s",
                 ch->transport ? ch->transport : "unknown", ch->channel_name);
        char esc_cn[PMM_SZ_256];
        pmm_json_escape(esc_cn, sizeof(esc_cn), ch->channel_name);
        char channel_props[PMM_SZ_512];
        snprintf(channel_props, sizeof(channel_props), "{\"transport\":\"%s\",\"name\":\"%s\"}",
                 ch->transport ? ch->transport : "unknown", esc_cn);
        int64_t channel_id = pmm_gbuf_upsert_node(ctx->gbuf, "Channel", ch->channel_name,
                                                  channel_qn, "", 0, 0, channel_props);
        const pmm_gbuf_node_t *src_node = find_channel_src(ctx, ch, rel);
        if (src_node && channel_id > 0) {
            const char *edge_type = ch->direction == PMM_CHANNEL_EMIT ? "EMITS" : "LISTENS_ON";
            char edge_props[PMM_SZ_128];
            snprintf(edge_props, sizeof(edge_props), "{\"transport\":\"%s\"}",
                     ch->transport ? ch->transport : "unknown");
            pmm_gbuf_insert_edge(ctx->gbuf, src_node->id, channel_id, edge_type, edge_props);
        }
    }
}
...
int pmm_build_registry_from_cache(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files,
    ...
    pmm_log_info("parallel.registry.start", "files", itoa_log(file_count));
    ...
    CBMHashTable *namespace_map =
        pmm_pipeline_namespace_map_build(ctx->project_name, result_cache, rels, file_count);
    free(rels);

    for (int i = 0; i < file_count; i++) {
        if (pmm_pipeline_check_cancel(ctx)) {
            pmm_pipeline_namespace_map_free(namespace_map);
            return PMM_NOT_FOUND;
        }
    ...
    pmm_log_info("parallel.registry.done", "entries", itoa_log(reg_entries), "defines",
...
typedef struct __attribute__((aligned(PMM_CACHE_LINE))) {
    ...
    int lsp_overrides;
    char _pad[PMM_CACHE_LINE - sizeof(pmm_gbuf_t *) - ((PP_RING + 1) * sizeof(int))];
...
typedef struct {
    ...
    CBMFileResult **result_cache;
    const pmm_gbuf_t *main_gbuf;    /* READ-ONLY during Phase 4 */
    const pmm_registry_t *registry; /* READ-ONLY during Phase 4 */
    _Atomic int64_t *shared_ids;
    _Atomic int *cancelled;
    _Atomic int next_file_idx;
     ...
     * match the existing pmm_run_X_lsp_cross callee signatures the
     ...
     * When non-NULL, the fused worker calls pmm_pxc_filter_defs_for_file
     ...
     * pmm_run_X_lsp_cross_with_registry — skip per-file build entirely.
     ...
     * pmm_parallel_for synchronization barrier at the end. */
...
#define PMM_ARG_JSON_GUARD PMM_SZ_32
...
static int format_call_arg(char *buf, size_t bufsize, const CBMCallArg *a, const char *expr) {
    char esc_k[PMM_SZ_128];
    char esc_e[PMM_SZ_128];
    char esc_v[PMM_SZ_128];
    pmm_json_escape(esc_e, sizeof(esc_e), expr);
    if (a->keyword && a->value) {
        pmm_json_escape(esc_k, sizeof(esc_k), a->keyword);
        pmm_json_escape(esc_v, sizeof(esc_v), a->value);
...
static void finalize_and_emit(pmm_gbuf_t *gbuf, int64_t src_id, int64_t tgt_id,
                              const char *edge_type, char *props, int n, const CBMCall *call) {
    if (n > 0 && (size_t)n < PMM_SZ_2K - PP_ESC_SPACE) {
        size_t pos = append_args_json(props, PMM_SZ_2K, (size_t)n, call);
        if (call->start_line > 0 && strcmp(edge_type, "CALLS") == 0 &&
            pos < PMM_SZ_2K - PP_LINE_MARGIN) {
            int ln = snprintf(props + pos, PMM_SZ_2K - pos, ",\"line\":%d", call->start_line);
            if (ln > 0) {
                pos += (size_t)ln;
            }
        }
        if (pos < PMM_SZ_2K - SKIP_ONE) {
            props[pos] = '}';
            props[pos + SKIP_ONE] = '\0';
        }
    }
    pmm_gbuf_insert_edge(gbuf, src_id, tgt_id, edge_type, props);
}

/* Build Route node QN and properties for HTTP/async service edges. */
static int64_t build_service_route(pmm_gbuf_t *gbuf, const char *arg, const char *method,
                                   const char *broker, pmm_svc_kind_t svc) {
    char route_qn[PMM_ROUTE_QN_SIZE];
    const char *prefix;
    char cpath[PMM_SZ_256];
    ...
    return pmm_gbuf_upsert_node(gbuf, "Route", arg, route_qn, "", 0, 0, route_props);
}

/* Emit HTTP_CALLS or ASYNC_CALLS edge via Route node. */
static void emit_http_async_service_edge(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source,
                                         const CBMCall *call, const pmm_resolution_t *res,
                                         pmm_svc_kind_t svc, const char *arg) {
    const char *edge_type = (svc == PMM_SVC_HTTP) ? "HTTP_CALLS" : "ASYNC_CALLS";
    const char *method =
        (svc == PMM_SVC_HTTP) ? pmm_service_pattern_http_method(call->callee_name) : NULL;
    const char *broker =
        (svc == PMM_SVC_ASYNC) ? pmm_service_pattern_broker(res->qualified_name) : NULL;

    int64_t route_id = build_service_route(gbuf, arg, method, broker, svc);

    char esc_c[PMM_SZ_256];
    char esc_a[PMM_SZ_256];
    pmm_json_escape(esc_c, sizeof(esc_c), call->callee_name);
    pmm_json_escape(esc_a, sizeof(esc_a), arg);
    char props[PMM_SZ_2K];
...
static void emit_config_edge(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source,
                             const pmm_gbuf_node_t *target, const CBMCall *call,
                             const pmm_resolution_t *res, const char *arg) {
    ...
    char esc_c[PMM_SZ_256];
    char esc_k[PMM_SZ_256];
    pmm_json_escape(esc_c, sizeof(esc_c), call->callee_name);
    pmm_json_escape(esc_k, sizeof(esc_k), arg ? arg : "");
    char props[PMM_SZ_2K];
...
static void emit_normal_calls_edge(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source,
                                   const pmm_gbuf_node_t *target, const CBMCall *call,
                                   const pmm_resolution_t *res) {
...
static void emit_route_registration(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source,
                                    const CBMCall *call, const char *route_path,
                                    const char *handler_ref, const char *module_qn,
                                    const pmm_registry_t *registry, const pmm_gbuf_t *main_gbuf,
                                    const char **ik, const char **iv, int ic) {
    const char *method = pmm_service_pattern_route_method(call->callee_name);
    char rqn[PMM_ROUTE_QN_SIZE];
    char cpath[PMM_SZ_256];
    snprintf(rqn, sizeof(rqn), "__route__%s__%s", method ? method : "ANY",
             pmm_route_canon_path(route_path, cpath, sizeof(cpath)));
    char rp[PMM_SZ_256];
    snprintf(rp, sizeof(rp), "{\"method\":\"%s\"}", method ? method : "ANY");
    int64_t rid = pmm_gbuf_upsert_node(gbuf, "Route", route_path, rqn, "", 0, 0, rp);
    char esc_cn[PMM_SZ_256]; /* sliced source text: escape quotes/newlines */
    char esc_rp[PMM_SZ_512];
    pmm_json_escape(esc_cn, sizeof(esc_cn), call->callee_name);
    pmm_json_escape(esc_rp, sizeof(esc_rp), route_path);
    char props[PMM_SZ_1K];
    snprintf(props, sizeof(props),
             "{\"callee\":\"%s\",\"url_path\":\"%s\",\"via\":\"route_registration\"}", esc_cn,
             esc_rp);
    pmm_gbuf_insert_edge(gbuf, source->id, rid, "CALLS", props);
    if (handler_ref && handler_ref[0] != '\0') {
        pmm_resolution_t hres = pmm_registry_resolve(registry, handler_ref, module_qn, ik, iv, ic);
        if (hres.qualified_name && hres.qualified_name[0] != '\0') {
            const pmm_gbuf_node_t *h = pmm_gbuf_find_by_qn(main_gbuf, hres.qualified_name);
            if (h) {
                char hp[PMM_SZ_1K]; /* must exceed escaped value + wrapper or snprintf cuts the
                                       closing brace */
                char esc_h2[PMM_SZ_512];
                pmm_json_escape(esc_h2, sizeof(esc_h2), hres.qualified_name);
                snprintf(hp, sizeof(hp), "{\"handler\":\"%s\"}", esc_h2);
                pmm_gbuf_insert_edge(gbuf, h->id, rid, "HANDLES", hp);
            }
        }
    }
}
...
static void detect_url_in_args(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source,
                               const CBMCall *call) {
    for (int ai = 0; ai < call->arg_count; ai++) {
        const CBMCallArg *ca = &call->args[ai];
        ...
        snprintf(route_qn, sizeof(route_qn), "__route__ANY__%s",
                 pmm_route_canon_path(norm, cpath, sizeof(cpath)));
        int64_t route_id = pmm_gbuf_upsert_node(gbuf, "Route", norm, route_qn, "", 0, 0,
                                                "{\"source\":\"arg_url\"}");
        char esc_c[PMM_SZ_256];
        char esc_n[PMM_SZ_256];
        pmm_json_escape(esc_c, sizeof(esc_c), call->callee_name);
        pmm_json_escape(esc_n, sizeof(esc_n), norm);
        char eprops[PMM_SZ_512];
        snprintf(eprops, sizeof(eprops),
                 "{\"callee\":\"%s\",\"url_path\":\"%s\",\"via\":\"arg_url\"}", esc_c, esc_n);
        pmm_gbuf_insert_edge(gbuf, source->id, route_id, "HTTP_CALLS", eprops);
        break;
    }
}
...
bool extract_grpc_service_method(const char *callee, char *service, size_t srv_sz, char *method,
    ...
    const char *s = raw;
    if (strncmp(s, "pb.New", PMM_SZ_6) == 0) {
        s += PMM_SZ_6;
    } else if (strncmp(s, "pb.", PMM_SZ_3) == 0 || strncmp(s, "New", PMM_SZ_3) == 0) {
        s += PMM_SZ_3;
    }
...
static void emit_grpc_edge(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source, const CBMCall *call,
                           const pmm_resolution_t *res) {
    char service[PMM_SZ_256];
    char method[PMM_SZ_256];
    ...
    char route_qn[PMM_SZ_512];
    snprintf(route_qn, sizeof(route_qn), "__grpc__%s/%s", service, method);

    char route_name[PMM_SZ_256];
    snprintf(route_name, sizeof(route_name), "%s/%s", service, method);

    int64_t route_id = pmm_gbuf_upsert_node(gbuf, "Route", route_name, route_qn, "", 0, 0,
                                            "{\"source\":\"grpc\"}");
    ...
    pmm_gbuf_insert_edge(gbuf, source->id, route_id, "GRPC_CALLS", props);
}

/* Emit GRAPHQL_CALLS edge. Extract operation from first string arg if available. */
static void emit_graphql_edge(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source, const CBMCall *call,
                              const pmm_resolution_t *res) {
    ...
    if (strncmp(p, "query ", PMM_SZ_6) == 0) {
        p += PMM_SZ_6;
    } else if (strncmp(p, "mutation ", PMM_SZ_8) == 0) {
        p += PMM_SZ_8;
    }
    ...
    int64_t route_id =
        pmm_gbuf_upsert_node(gbuf, "Route", p, route_qn, "", 0, 0, "{\"source\":\"graphql\"}");
    ...
    pmm_gbuf_insert_edge(gbuf, source->id, route_id, "GRAPHQL_CALLS", props);
}

/* Emit TRPC_CALLS edge. Extract procedure path from callee chain. */
static void emit_trpc_edge(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source, const CBMCall *call,
                           const pmm_resolution_t *res) {
    ...
    int64_t route_id =
        pmm_gbuf_upsert_node(gbuf, "Route", p, route_qn, "", 0, 0, "{\"source\":\"trpc\"}");
    ...
    snprintf(props, sizeof(props), "{\"callee\":\"%s\",\"procedure\":\"%s\",\"confidence\":%.2f}",
             esc_c, p, res->confidence);
    pmm_gbuf_insert_edge(gbuf, source->id, route_id, "TRPC_CALLS", props);
}
...
static void emit_service_edge(pmm_gbuf_t *gbuf, const pmm_gbuf_node_t *source,
                              const pmm_gbuf_node_t *target, const CBMCall *call,
                              const pmm_resolution_t *res, const char *module_qn,
                              const pmm_registry_t *registry, const pmm_gbuf_t *main_gbuf,
                              const char **imp_keys, const char **imp_vals, int imp_count,
                              bool suppress_plain_calls) {
    pmm_svc_kind_t svc = pmm_service_pattern_match(res->qualified_name);
    ...
    if (svc == PMM_SVC_NONE && pmm_service_pattern_route_method(call->callee_name) != NULL) {
        svc = PMM_SVC_ROUTE_REG;
    }
    ...
    if (svc == PMM_SVC_NONE && res->qualified_name) {
    ...
    bool has_topic = (arg && arg[0] != '\0' && svc == PMM_SVC_ASYNC && strlen(arg) > PP_ESC_SPACE);

    if ((svc == PMM_SVC_HTTP || svc == PMM_SVC_ASYNC) && (has_url || has_topic)) {
        emit_http_async_service_edge(gbuf, source, call, res, svc, arg);
    } else if (svc == PMM_SVC_GRPC) {
        emit_grpc_edge(gbuf, source, call, res);
    } else if (svc == PMM_SVC_GRAPHQL) {
        emit_graphql_edge(gbuf, source, call, res);
    } else if (svc == PMM_SVC_TRPC) {
        emit_trpc_edge(gbuf, source, call, res);
    } else if (svc == PMM_SVC_CONFIG) {
...
static const pmm_gbuf_node_t *find_source_node(const pmm_gbuf_t *gbuf, const char *project,
                                               const char *rel, const char *enclosing_qn) {
    const pmm_gbuf_node_t *src = NULL;
    if (enclosing_qn) {
        src = pmm_gbuf_find_by_qn(gbuf, enclosing_qn);
        ...
        if (pmm_pipeline_node_is_dir_container(src)) {
            src = NULL;
        }
    }
    if (!src) {
        char *file_qn = pmm_pipeline_fqn_compute(project, rel, "__file__");
        src = pmm_gbuf_find_by_qn(gbuf, file_qn);
        free(file_qn);
    }
    return src;
}
...
static void try_field_type_hint(resolve_ctx_t *rc, pmm_resolution_t *res, const char *callee_name,
                                int64_t source_id) {
...
static void resolve_file_calls(resolve_ctx_t *rc, resolve_worker_state_t *ws, CBMFileResult *result,
     ...
     * "caller_qn|callee_short" for O(1) lookup. pmm_pipeline_find_lsp_
    ...
    if (result->calls.count > 0 && result->resolved_calls.count > 0) {
        lsp_idx = pmm_ht_create((uint32_t)result->resolved_calls.count * 2u + 16u);
        if (lsp_idx) {
            for (int i = 0; i < result->resolved_calls.count; i++) {
                ...
                if (!rc_e->caller_qn || !rc_e->callee_qn ||
                    rc_e->confidence < PMM_LSP_CONFIDENCE_FLOOR) {
                    continue;
                }
    ...
    for (int c = 0; c < result->calls.count; c++) {
        ...
        const pmm_gbuf_node_t *source_node =
         ...
         * Same helper + same PMM_LSP_CONFIDENCE_FLOOR as the sequential
         ...
         * fallbacks are JVM-only (see pmm_pipeline_lsp_allow_tail_match). */
        bool allow_tail = pmm_pipeline_lsp_allow_tail_match(lang);
        pmm_resolution_t res = {0};
        const CBMResolvedCall *lsp = NULL;
        _rc_t0 = extract_now_ns();
        if (lsp_idx && call->enclosing_func_qn) {
            const char *call_leaf = pmm_pipeline_call_callee_leaf(call->callee_name);
            char key[1024];
            int kn = call_leaf
                         ? snprintf(key, sizeof(key), "%s|%s", call->enclosing_func_qn, call_leaf)
                         : -1;
            if (kn > 0 && kn < (int)sizeof(key)) {
                lsp = (const CBMResolvedCall *)pmm_ht_get(lsp_idx, key);
            }
        }
        if (!lsp) {
            ...
            lsp = pmm_pipeline_find_lsp_resolution(&result->resolved_calls, call, allow_tail);
        }
        atomic_fetch_add_explicit(&rc->time_ns_rc_lsp_lookup, extract_now_ns() - _rc_t0,
                                  memory_order_relaxed);
        _rc_t0 = extract_now_ns();
        const pmm_gbuf_node_t *lsp_target = NULL;
        if (lsp) {
             ...
             * project name. */
            lsp_target = pmm_pipeline_lsp_target_node(rc->main_gbuf, rc->project_name,
        ...
        if (!res.qualified_name || !res.qualified_name[0]) {
            res = pmm_registry_resolve(rc->registry, call->callee_name, module_qn, imp_keys,
                                       imp_vals, imp_count);
        }
        ...
        if (pmm_perl_suppress_generic_match(lang == PMM_LANG_PERL, call->is_method,
        ...
        bool is_tsjs =
            lang == PMM_LANG_JAVASCRIPT || lang == PMM_LANG_TYPESCRIPT || lang == PMM_LANG_TSX;
        bool tsjs_drop_plain_call =
            pmm_tsjs_suppress_weak_method_match(is_tsjs, call->is_method, res.strategy);
        ...
        pmm_svc_kind_t csvc = pmm_service_pattern_match(call->callee_name);
        if (csvc == PMM_SVC_HTTP || csvc == PMM_SVC_ASYNC) {
            const char *cu = call->first_string_arg;
            bool chas_url = cu && cu[0] != '\0' &&
                            (cu[0] == '/' || strstr(cu, "://") != NULL ||
                             (csvc == PMM_SVC_ASYNC && strlen(cu) > PP_ESC_SPACE));
            if (chas_url) {
                pmm_resolution_t svc_res = {.qualified_name = call->callee_name,
        ...
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            if (pmm_service_pattern_route_method(call->callee_name) != NULL) {
                pmm_resolution_t fake_res = {.qualified_name = call->callee_name,
                                             .confidence = PP_HALF_CONF,
                                             .strategy = "callee_suffix"};
                emit_service_edge(ws->local_edge_buf, source_node, source_node, call, &fake_res,
                                  module_qn, rc->registry, rc->main_gbuf, imp_keys, imp_vals,
                                  imp_count, false);
            } else if (pmm_service_pattern_is_global_fetch(call->callee_name)) {
                 ...
                 * from res->qualified_name via pmm_service_pattern_match, which
        ...
        if (!target_node || source_node->id == target_node->id) {
            ...
            pmm_svc_kind_t psvc = pmm_service_pattern_match(res.qualified_name);
            if ((psvc == PMM_SVC_HTTP || psvc == PMM_SVC_ASYNC) && !target_node) {
                const char *u = call->first_string_arg;
                bool url_or_topic = u && u[0] != '\0' &&
                                    (u[0] == '/' || strstr(u, "://") != NULL ||
                                     (psvc == PMM_SVC_ASYNC && strlen(u) > PP_ESC_SPACE));
                if (url_or_topic) {
    ...
    if (lsp_idx) {
        pmm_ht_foreach(lsp_idx, lsp_idx_free_key, NULL);
        pmm_ht_free(lsp_idx);
    }
}
...
static void resolve_file_usages(resolve_ctx_t *rc, resolve_worker_state_t *ws,
    ...
    for (int u = 0; u < result->usages.count; u++) {
        ...
        const pmm_gbuf_node_t *src =
            find_source_node(rc->main_gbuf, rc->project_name, rel, usage->enclosing_func_qn);
        if (!src) {
            continue;
        }
        pmm_resolution_t res = pmm_registry_resolve(rc->registry, usage->ref_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }
        const pmm_gbuf_node_t *tgt = pmm_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
        if (!tgt || src->id == tgt->id) {
            continue;
        }
        char uprops[PMM_SZ_256];
        char esc_ref[PMM_SZ_256]; /* sliced source text: escape quotes/newlines */
        pmm_json_escape(esc_ref, sizeof(esc_ref), usage->ref_name);
        snprintf(uprops, sizeof(uprops), "{\"callee\":\"%s\"}", esc_ref);
        pmm_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, "USAGE", uprops);
        ws->usages_resolved++;
    }
}
...
static void resolve_file_throws(resolve_ctx_t *rc, resolve_worker_state_t *ws,
    ...
    for (int t = 0; t < result->throws.count; t++) {
        ...
        pmm_resolution_t res = pmm_registry_resolve(rc->registry, thr->exception_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }
        ...
        pmm_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, edge_type, "{}");
    }
}
...
static void resolve_file_rw(resolve_ctx_t *rc, resolve_worker_state_t *ws, CBMFileResult *result,
    ...
    for (int r = 0; r < result->rw.count; r++) {
        ...
        pmm_resolution_t res = pmm_registry_resolve(rc->registry, rw->var_name, module_qn, imp_keys,
                                                    imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }
        ...
        pmm_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, etype, "{}");
    }
}
...
static void resolve_def_inherits(resolve_ctx_t *rc, resolve_worker_state_t *ws,
                                 const CBMDefinition *def, const pmm_gbuf_node_t *node,
    ...
    for (int b = 0; def->base_classes[b]; b++) {
        ...
        const pmm_gbuf_node_t *bn = pmm_gbuf_find_by_qn(rc->main_gbuf, bqn);
        if (bn && node->id != bn->id) {
            ...
            pmm_gbuf_insert_edge(ws->local_edge_buf, node->id, bn->id,
                                 pmm_semantic_base_edge_type(bn), "{}");
...
static void resolve_def_decorators(resolve_ctx_t *rc, resolve_worker_state_t *ws,
    ...
    for (int dc = 0; def->decorators[dc]; dc++) {
        ...
        pmm_resolution_t res = pmm_registry_resolve(rc->registry, fn, mq, ik, iv, ic);
        if ((!res.qualified_name || res.qualified_name[0] == '\0') && !strchr(fn, '.')) {
            ...
            char with_suffix[PMM_SZ_256];
            int wn = snprintf(with_suffix, sizeof(with_suffix), "%sAttribute", fn);
            if (wn > 0 && (size_t)wn < sizeof(with_suffix)) {
                res = pmm_registry_resolve(rc->registry, with_suffix, mq, ik, iv, ic);
            }
        }
        const pmm_gbuf_node_t *dn = NULL;
        if (res.qualified_name && res.qualified_name[0] != '\0') {
            dn = pmm_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
        }
        int64_t dn_id = 0;
        if (dn) {
            dn_id = dn->id;
            ...
            char syn_qn[PMM_SZ_512];
            snprintf(syn_qn, sizeof(syn_qn), "<decorator:%s>", fn);
            dn_id =
                pmm_gbuf_upsert_node(ws->local_edge_buf, "Decorator", fn, syn_qn, "", 0, 0, "{}");
        }
        if (dn_id != 0 && node->id != dn_id) {
            ...
            char esc_dec[PMM_SZ_256];
            pmm_json_escape(esc_dec, sizeof(esc_dec), def->decorators[dc]);
            char dp[PMM_SZ_512];
            snprintf(dp, sizeof(dp), "{\"decorator\":\"%s\"}", esc_dec);
            pmm_gbuf_insert_edge(ws->local_edge_buf, node->id, dn_id, "DECORATES", dp);
            ...
            pmm_gbuf_insert_edge(ws->local_edge_buf, node->id, dn_id, "USAGE", "{}");
            ws->semantic_resolved++;
        }
    }
}
...
static void resolve_file_semantic(resolve_ctx_t *rc, resolve_worker_state_t *ws,
    ...
    for (int d = 0; d < result->defs.count; d++) {
        ...
        const pmm_gbuf_node_t *node = pmm_gbuf_find_by_qn(rc->main_gbuf, def->qualified_name);
        if (!node) {
            continue;
        }
    ...
    for (int t = 0; t < result->impl_traits.count; t++) {
        ...
        const pmm_gbuf_node_t *tn = pmm_gbuf_find_by_qn(rc->main_gbuf, tqn);
        const pmm_gbuf_node_t *sn = pmm_gbuf_find_by_qn(rc->main_gbuf, sqn);
        if (tn && sn && tn->id != sn->id) {
            pmm_gbuf_insert_edge(ws->local_edge_buf, sn->id, tn->id, "IMPLEMENTS", "{}");
            ws->semantic_resolved++;
        }
    }
}
...
static CBMTypeRegistry *pp_rust_shared_registry(resolve_ctx_t *rc) {
    ...
    if (!rc->all_defs || rc->def_count <= 0)
        return NULL;
    pmm_mutex_lock(&rc->rust_shared_mu);
    p = atomic_load_explicit(&rc->rust_shared_reg, memory_order_relaxed);
    if (!p) {
        pmm_arena_init(&rc->rust_shared_arena);
        rc->rust_shared_arena_live = true;
        p = pmm_rust_build_cross_registry(&rc->rust_shared_arena, rc->all_defs, rc->def_count);
        if (p) {
            char sb[96];
            snprintf(sb, sizeof(sb), "types=%d funcs=%d", p->type_count, p->func_count);
            pmm_log_info("cross_lsp.rust_registry", "scale", sb);
        }
        atomic_store_explicit(&rc->rust_shared_reg, p, memory_order_release);
    }
    pmm_mutex_unlock(&rc->rust_shared_mu);
    return p;
}
...
static void resolve_worker(int worker_id, void *ctx_ptr) {
    ...
    if (!ws->local_edge_buf) {
        ws->local_edge_buf =
            pmm_gbuf_new_shared_ids(rc->project_name, rc->repo_path, rc->shared_ids);
    }
    ...
    while (SKIP_ONE) {
         ...
         * calls exist. */
        bool jvm_cross_lsp = (lang == PMM_LANG_JAVA || lang == PMM_LANG_KOTLIN);
        bool cross_lsp_eligible =
            (rc->all_defs && rc->def_count > 0 && pmm_pxc_has_cross_lsp(lang) &&
         ...
         * through pmm_registry_resolve. Same callee_name appears in
        ...
        pmm_registry_reach_cache_begin(result->calls.count + result->usages.count + 64);
        ...
        pmm_registry_import_map_cache_begin(imp_keys, imp_vals, imp_count);

        /* THE BIG ONE: per-file cache of pmm_registry_resolve results.
        ...
        pmm_registry_resolve_cache_begin(result->calls.count + result->usages.count + 64);

        char *module_qn =
            pmm_pipeline_fqn_module_dir(rc->project_name, rel, pp_module_is_dir(lang));

        /* ── Cross-file LSP (FUSED) ─────────────────────────────
         * Runs BEFORE resolve_file_calls so its additions to
         * result->resolved_calls are picked up by
         * pmm_pipeline_find_lsp_resolution when calls become CALLS
        ...
        if (cross_lsp_eligible) {
            ...
            if ((!lsp_source || lsp_source_len <= 0) && rc->files[file_idx].path) {
                 ...
                 * by read_file's pmm_max_file_bytes cap), freed below. */
            ...
            if (lsp_source && lsp_source_len > 0) {
                ...
                pmm_index_mark_start(rel);
                pmm_pxc_dispatch_file(lang, result, lsp_source, lsp_source_len, rel, def_module,
                 ...
                 * pmm_extract_file on this thread will recreate the parser. */
                pmm_destroy_thread_parser();
                pmm_slab_reclaim();
                ...
                if (lsp_elapsed_ms > PP_TIMER_THRESH) {
                    pmm_log_info("parallel.resolve.lsp_cross.slow", "elapsed_ms",
                 ...
                 * unaffected. Deliberately NOT recorded as a pmm_file_error —
        ...
        pmm_registry_reach_cache_end();
        pmm_registry_import_map_cache_end();
        pmm_registry_resolve_cache_end();
...
int pmm_parallel_resolve(pmm_pipeline_ctx_t *ctx, const pmm_file_info_t *files, int file_count,
    ...
    pmm_log_info("parallel.resolve.start", "files", itoa_log(file_count), "workers",
                 itoa_log(worker_count));
    ...
    pmm_mutex_init(&rc.rust_shared_mu);
    rc.rust_shared_arena_live = false;

    /* Sub-phase: Dispatch resolve workers (per-file call/usage resolution, PARALLEL) */
    PMM_PROF_START(t_resolve_dispatch);
    pmm_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    pmm_parallel_for(worker_count, resolve_worker, &rc, opts);
    PMM_PROF_END_N("parallel_resolve", "1_dispatch_workers_parallel", t_resolve_dispatch,
                   file_count);
    ...
    if (rc.rust_shared_arena_live) {
        pmm_arena_destroy(&rc.rust_shared_arena);
        rc.rust_shared_arena_live = false;
    }
    pmm_mutex_destroy(&rc.rust_shared_mu);

    /* Sub-phase: Merge all local edge bufs into main gbuf (SEQUENTIAL) */
    PMM_PROF_START(t_resolve_merge);
    int total_calls = 0;
    int total_usages = 0;
    ...
    for (int i = 0; i < worker_count; i++) {
        if (workers[i].local_edge_buf) {
            pmm_gbuf_merge(ctx->gbuf, workers[i].local_edge_buf);
            ...
            pmm_gbuf_free(workers[i].local_edge_buf);
        }
    }
    PMM_PROF_END_N("parallel_resolve", "2_merge_edge_bufs_seq", t_resolve_merge,
                   total_calls + total_usages);

    pmm_aligned_free(workers);

    /* Go-style implicit interface satisfaction (needs full graph, serial) */
    int go_impl = pmm_pipeline_implements_go(ctx);
    ...
    total_lsp_overrides += pmm_pipeline_override_explicit(ctx);
    ...
    pmm_log_info("parallel.resolve.done", "calls", itoa_log(total_calls), "usages",
    ...
    pmm_log_info("parallel.resolve.phase_summary", "total_loop_cpu_ms", loop_buf, "files_visited",
                 visits_buf);
    ...
    snprintf(cal_buf, sizeof(cal_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_calls, memory_order_relaxed) /
                                  1000000ULL));
    pmm_log_info("parallel.resolve.phase_ms_a", "import_map", imp_buf, "cross_lsp", xls_buf,
    ...
    snprintf(sem_buf, sizeof(sem_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_semantic, memory_order_relaxed) /
                                  1000000ULL));
    pmm_log_info("parallel.resolve.phase_ms_b", "resolve_usages", use_buf, "resolve_throws",
    ...
    snprintf(emt_buf, sizeof(emt_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_rc_emit, memory_order_relaxed) /
                                  1000000ULL));
    pmm_log_info("parallel.resolve.calls_breakdown", "find_source", src_buf, "lsp_lookup", lsp_buf,
                 "resolve", rsv_buf);
    pmm_log_info("parallel.resolve.calls_breakdown2", "field_hint", hnt_buf, "find_target", tgt_buf,
                 "emit_edge", emt_buf);
    return 0;
}
