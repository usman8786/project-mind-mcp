...
static const char PROJECT_SET_KEY[] = "pmm-project-set-v1";

struct pmm_project_lock_manager {
    pmm_private_lock_directory_t *directory;
    pmm_lock_registry_t *registry;
};

struct pmm_project_lock_lease {
    pmm_lock_lease_t *project;
    pmm_lock_lease_t *project_set;
};
...
static bool project_lock_key(const char *project, char out[PROJECT_LOCK_KEY_CAP]) {
    ...
    static const char prefix[] = "pmm-project-v1:";
...
pmm_project_lock_manager_t *pmm_project_lock_manager_new(
    ...
    pmm_project_lock_manager_t *manager = calloc(1, sizeof(*manager));
    if (manager) {
        manager->directory = directory;
        manager->registry = pmm_lock_registry_new(directory);
    }
...
pmm_private_file_lock_status_t pmm_project_lock_lease_release(pmm_project_lock_lease_t **lease_io) {
    ...
    pmm_project_lock_lease_t *lease = *lease_io;
    pmm_private_file_lock_status_t result = PMM_PRIVATE_FILE_LOCK_OK;
    if (lease->project) {
        result = pmm_lock_lease_release(&lease->project);
        if (lease->project) {
            return PMM_PRIVATE_FILE_LOCK_IO;
        }
    }
    if (lease->project_set) {
        pmm_private_file_lock_status_t set_status = pmm_lock_lease_release(&lease->project_set);
        if (set_status != PMM_PRIVATE_FILE_LOCK_OK) {
            result = set_status;
        }
        if (lease->project_set) {
            return PMM_PRIVATE_FILE_LOCK_IO;
...
static pmm_private_file_lock_status_t project_lock_failed_acquire(
    pmm_project_lock_lease_t *lease, pmm_private_file_lock_status_t status,
    pmm_project_lock_lease_t **lease_out) {
    if (!lease->project && !lease->project_set) {
        free(lease);
        return status;
    }
    pmm_project_lock_lease_t *cleanup = lease;
    pmm_private_file_lock_status_t cleanup_status = pmm_project_lock_lease_release(&cleanup);
    if (cleanup) {
        *lease_out = cleanup;
        return PMM_PRIVATE_FILE_LOCK_IO;
    }
    return cleanup_status == PMM_PRIVATE_FILE_LOCK_OK ? status : PMM_PRIVATE_FILE_LOCK_IO;
}

static pmm_private_file_lock_status_t project_lock_acquire_internal(
    pmm_project_lock_manager_t *manager, const char *project, uint64_t deadline_ms,
    const pmm_lock_cancel_token_t *cancel_token, bool try_once,
    pmm_project_lock_lease_t **lease_out) {
    if (lease_out) {
        *lease_out = NULL;
    }
    ...
    pmm_project_lock_lease_t *lease = calloc(1, sizeof(*lease));
    if (!lease) {
        return PMM_PRIVATE_FILE_LOCK_IO;
    }
    pmm_private_file_lock_status_t status =
        try_once ? pmm_lock_registry_try_acquire(manager->registry, PROJECT_SET_KEY,
                                                 wildcard ? PMM_PRIVATE_FILE_LOCK_EX
                                                          : PMM_PRIVATE_FILE_LOCK_SH,
                                                 &lease->project_set)
                 : pmm_lock_registry_acquire(manager->registry, PROJECT_SET_KEY,
                                             wildcard ? PMM_PRIVATE_FILE_LOCK_EX
                                                      : PMM_PRIVATE_FILE_LOCK_SH,
    ...
    if (!wildcard) {
        status = try_once ? pmm_lock_registry_try_acquire(manager->registry, project_key,
                                                          PMM_PRIVATE_FILE_LOCK_EX, &lease->project)
                          : pmm_lock_registry_acquire(manager->registry, project_key,
                                                      PMM_PRIVATE_FILE_LOCK_EX, deadline_ms,
...
pmm_private_file_lock_status_t pmm_project_lock_request_cancel(pmm_project_lock_manager_t *manager,
    ...
    return manager ? pmm_lock_registry_request_cancel(manager->registry, token)
...
pmm_private_file_lock_status_t pmm_project_lock_manager_free(
    ...
    pmm_project_lock_manager_t *manager = *manager_io;
    pmm_private_file_lock_status_t status = pmm_lock_registry_free(&manager->registry);
    if (status != PMM_PRIVATE_FILE_LOCK_OK) {
        return status;
    }
