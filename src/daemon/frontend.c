...
enum {
    ...
    FRONTEND_MAINTENANCE_GRACE_MS =
        PMM_SUBPROCESS_MAX_CANCEL_GRACE_MS + PMM_SUBPROCESS_FORCE_SETTLE_MS + 1000,
...
struct pmm_daemon_maintenance_monitor {
    pmm_thread_t thread;
    pmm_version_cohort_manager_t *manager;
    pmm_daemon_maintenance_cancel_fn cancel;
    void *cancel_context;
...
static void *frontend_maintenance_monitor_worker(void *opaque) {
    pmm_daemon_maintenance_monitor_t *monitor = opaque;
    while (!atomic_load_explicit(&monitor->stopping, memory_order_acquire)) {
        pmm_version_cohort_maintenance_presence_t presence =
            pmm_version_cohort_maintenance_presence_terminal(monitor->manager);
        if (presence == PMM_VERSION_COHORT_MAINTENANCE_ABSENT) {
            pmm_usleep(FRONTEND_MAINTENANCE_POLL_MS * 1000U);
            continue;
        }
...
pmm_daemon_maintenance_monitor_t *pmm_daemon_maintenance_monitor_start(
    pmm_version_cohort_manager_t *manager, pmm_daemon_maintenance_cancel_fn cancel,
    void *cancel_context, int exit_code, const char *participant) {
    if (!manager || exit_code < 0 || !participant || !participant[0]) {
        return NULL;
    }
    pmm_daemon_maintenance_monitor_t *monitor = calloc(1, sizeof(*monitor));
    if (!monitor) {
        return NULL;
    }
    ...
    if (written <= 0 || written >= (int)sizeof(monitor->participant) ||
        pmm_thread_create(&monitor->thread, 0, frontend_maintenance_monitor_worker, monitor) != 0) {
        free(monitor);
        return NULL;
    }
    return monitor;
...
bool pmm_daemon_maintenance_monitor_stop(pmm_daemon_maintenance_monitor_t **monitor_io) {
    if (!monitor_io || !*monitor_io) {
        return false;
    }
    pmm_daemon_maintenance_monitor_t *monitor = *monitor_io;
    atomic_store_explicit(&monitor->stopping, true, memory_order_release);
    if (pmm_thread_join(&monitor->thread) != 0) {
        return false;
    }
    free(monitor);
    *monitor_io = NULL;
...
static void frontend_exit_for_maintenance(frontend_state_t *state) {
    pmm_version_cohort_maintenance_presence_t presence =
        pmm_version_cohort_maintenance_presence_terminal(state->cohort_manager);
    if (presence == PMM_VERSION_COHORT_MAINTENANCE_ABSENT) {
        return;
    }
    ...
    _Exit(presence == PMM_VERSION_COHORT_MAINTENANCE_REQUESTED ? EXIT_SUCCESS : EXIT_FAILURE);
}
...
static bool frontend_pop_begin(frontend_state_t *state, frontend_item_t *item) {
    ...
    if (!state->stopping && state->count > 0) {
        ...
        bool token_ready = queued->cancelled || pmm_daemon_runtime_client_application_token_reserve(
...
static void frontend_end_request(frontend_state_t *state, bool failed) {
    ...
    state->active_request_token = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
...
static bool frontend_write_cancelled_response(FILE *out, const frontend_item_t *item) {
    ...
    pmm_jsonrpc_response_t response = {
        .id = item->id,
        .id_str = item->id_str,
        .error_json = cancelled_error,
        .error_code = -32800,
    };
    char *encoded = pmm_jsonrpc_format_response(&response);
    bool written =
...
static bool frontend_parse_cancellation(const char *message, pmm_jsonrpc_request_t *request_out) {
    if (!message || !request_out) {
        return false;
    }
    memset(request_out, 0, sizeof(*request_out));
    if (pmm_jsonrpc_parse(message, request_out) != 0) {
        return false;
    }
    bool cancellation = !request_out->has_id && request_out->method &&
                        strcmp(request_out->method, "notifications/cancelled") == 0;
    if (!cancellation) {
        pmm_jsonrpc_request_free(request_out);
    }
    return cancellation;
}

bool pmm_daemon_frontend_is_cancellation_notification(const char *message) {
    pmm_jsonrpc_request_t request = {0};
    bool cancellation = frontend_parse_cancellation(message, &request);
    if (cancellation) {
        pmm_jsonrpc_request_free(&request);
    }
    return cancellation;
}

bool pmm_daemon_frontend_cancellation_matches_request(const char *message, int64_t active_id,
                                                      const char *active_id_str) {
    pmm_jsonrpc_request_t request = {0};
    bool cancellation = frontend_parse_cancellation(message, &request);
    bool matches = cancellation &&
                   pmm_mcp_cancel_request_matches(request.params_raw, active_id, active_id_str);
    if (cancellation) {
        pmm_jsonrpc_request_free(&request);
    }
    return matches;
...
static frontend_cancellation_route_t frontend_route_cancellation(
    ...
    pmm_daemon_runtime_application_token_t *request_token_out) {
    if (request_token_out) {
        *request_token_out = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    pmm_jsonrpc_request_t request = {0};
    ...
    if (state->in_request && state->active_has_id &&
        state->active_request_token != PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
        pmm_mcp_cancel_request_matches(request.params_raw, state->active_id,
                                       state->active_id_str)) {
        if (request_token_out) {
        ...
        for (size_t offset = 0; offset < state->count; offset++) {
            ...
            if (item->has_id &&
                pmm_mcp_cancel_request_matches(request.params_raw, item->id, item->id_str)) {
                item->cancelled = true;
...
static void *frontend_worker(void *opaque) {
    ...
    for (;;) {
        ...
        if (!frontend_pop_begin(state, &item)) {
            ...
            pmm_usleep(FRONTEND_IDLE_WAIT_US);
            continue;
        }
        ...
        if (item.cancelled) {
            ...
            pmm_daemon_runtime_application_status_t status =
                pmm_daemon_application_client_mcp_tagged(state->client, item.request_token,
                                                         item.message, &response, &response_length,
                                                         FRONTEND_REQUEST_TIMEOUT_MS);
            if (status == PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED) {
                failed = !frontend_write_cancelled_response(state->out, &item);
            } else {
                failed = status != PMM_DAEMON_RUNTIME_APPLICATION_OK;
            }
            if (status == PMM_DAEMON_RUNTIME_APPLICATION_OK && !failed && response &&
                response_length > 0) {
...
static bool frontend_enqueue(frontend_state_t *state, char *message, bool content_length_framed) {
    ...
    char *id_str = NULL;
    pmm_jsonrpc_request_t request = {0};
    if (pmm_jsonrpc_parse(message, &request) == 0) {
        has_id = request.has_id;
        id = request.id;
        if (request.id_str) {
            id_str = pmm_strdup(request.id_str);
        }
...
static bool frontend_stop_begin(frontend_state_t *state) {
    ...
    return pmm_daemon_runtime_client_close_begin(state->client);
}
...
static bool frontend_cancel_for_maintenance(void *opaque) {
    ...
    return pmm_daemon_runtime_client_application_cancel(state->client, request_token) ==
           PMM_DAEMON_RUNTIME_CANCEL_ACCEPTED;
}

int pmm_daemon_frontend_mcp_run(pmm_daemon_runtime_client_t *client,
                                pmm_version_cohort_manager_t *cohort_manager, FILE *in, FILE *out) {
    ...
    pmm_mutex_init(&state.mutex);
    atomic_init(&state.completed_items, 0);
    pmm_daemon_maintenance_monitor_t *maintenance_monitor = pmm_daemon_maintenance_monitor_start(
        cohort_manager, frontend_cancel_for_maintenance, &state, EXIT_SUCCESS, "MCP frontend");
    if (!maintenance_monitor) {
        pmm_mutex_destroy(&state.mutex);
        (void)pmm_daemon_runtime_client_close(client, FRONTEND_CLOSE_TIMEOUT_MS);
        return -1;
    }
    pmm_thread_t worker;
    if (pmm_thread_create(&worker, 0, frontend_worker, &state) != 0) {
        if (!pmm_daemon_maintenance_monitor_stop(&maintenance_monitor)) {
            _Exit(EXIT_FAILURE);
        }
        pmm_mutex_destroy(&state.mutex);
        (void)pmm_daemon_runtime_client_close(client, FRONTEND_CLOSE_TIMEOUT_MS);
    ...
    for (;;) {
        ...
        int read_status = pmm_mcp_read_message(in, &message, &content_length_framed);
        if (read_status <= 0) {
        ...
        pmm_daemon_runtime_application_token_t cancel_token =
            PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
        ...
        if (cancellation != FRONTEND_CANCELLATION_NONE) {
            ...
            if (cancellation == FRONTEND_CANCELLATION_ACTIVE) {
                pmm_daemon_runtime_cancel_result_t cancelled =
                    pmm_daemon_runtime_client_application_cancel(state.client, cancel_token);
                if (cancelled == PMM_DAEMON_RUNTIME_CANCEL_ERROR) {
                    result = -1;
    ...
    if (clean_eof && !close_begun) {
        ...
        uint64_t drain_deadline = pmm_now_ms() + FRONTEND_EOF_DRAIN_MS;
        while (!frontend_worker_is_done(&state) && pmm_now_ms() < drain_deadline) {
            pmm_usleep(FRONTEND_WAIT_US);
            uint64_t now_drained =
                atomic_load_explicit(&state.completed_items, memory_order_acquire);
            if (now_drained != drained) {
                drained = now_drained;
                drain_deadline = pmm_now_ms() + FRONTEND_EOF_DRAIN_MS;
            }
        }
    }
    if (!close_begun) {
    ...
    if (!frontend_worker_is_done(&state)) {
        ...
        if (pmm_thread_create(&watchdog_thread, 0, frontend_join_watchdog, &watchdog) != 0) {
            _Exit(EXIT_FAILURE);
        }
        watchdog_started = true;
    }
    if (pmm_thread_join(&worker) != 0) {
        /* Preserve every object the worker may still reference. */
        _Exit(EXIT_FAILURE);
    }
    if (watchdog_started) {
        atomic_store_explicit(&watchdog.complete, true, memory_order_release);
        if (pmm_thread_join(&watchdog_thread) != 0) {
            _Exit(EXIT_FAILURE);
        }
    }
    if (!pmm_daemon_maintenance_monitor_stop(&maintenance_monitor)) {
    ...
    if (close_begun) {
        (void)pmm_daemon_runtime_client_close_finish(state.client, FRONTEND_CLOSE_TIMEOUT_MS);
    } else {
        result = -1;
    }
