/* version_cohort.h — Crash-safe exact-build admission across PMM processes. */
#ifndef PMM_DAEMON_VERSION_COHORT_H
#define PMM_DAEMON_VERSION_COHORT_H
...
typedef struct pmm_version_cohort_manager pmm_version_cohort_manager_t;
typedef struct pmm_version_cohort_lease pmm_version_cohort_lease_t;
typedef struct pmm_version_cohort_daemon_claim pmm_version_cohort_daemon_claim_t;

typedef enum {
    PMM_VERSION_COHORT_OK = 0,
    PMM_VERSION_COHORT_CONFLICT = 1,
    PMM_VERSION_COHORT_BUSY = 2,
    PMM_VERSION_COHORT_UNSAFE = 3,
    PMM_VERSION_COHORT_IO = 4,
} pmm_version_cohort_status_t;

typedef enum {
    PMM_VERSION_COHORT_DAEMON_ABSENT = 0,
    PMM_VERSION_COHORT_DAEMON_COORDINATED = 1,
    PMM_VERSION_COHORT_DAEMON_UNCOORDINATED = 2,
    PMM_VERSION_COHORT_DAEMON_UNSAFE = 3,
    PMM_VERSION_COHORT_DAEMON_IO = 4,
} pmm_version_cohort_daemon_presence_t;

typedef enum {
    PMM_VERSION_COHORT_MAINTENANCE_ABSENT = 0,
    PMM_VERSION_COHORT_MAINTENANCE_REQUESTED = 1,
    PMM_VERSION_COHORT_MAINTENANCE_UNSAFE = 2,
    PMM_VERSION_COHORT_MAINTENANCE_IO = 3,
} pmm_version_cohort_maintenance_presence_t;
...
typedef enum {
    PMM_VERSION_COHORT_QUIESCE_NOT_NEEDED = 0,
    PMM_VERSION_COHORT_QUIESCE_REQUESTED = 1,
    PMM_VERSION_COHORT_QUIESCE_REFUSED = 2,
    PMM_VERSION_COHORT_QUIESCE_ERROR = 3,
} pmm_version_cohort_quiesce_result_t;

typedef pmm_version_cohort_quiesce_result_t (*pmm_version_cohort_quiesce_fn)(void *context);
 ...
 * pmm_now_ms() deadline; UINT64_MAX waits indefinitely. Every non-NULL
...
/* Coordinated install/update/uninstall barrier for modern PMM participants.
...
#endif /* PMM_DAEMON_VERSION_COHORT_H */
