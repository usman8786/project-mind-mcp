...
enum {
    ...
    RENDEZVOUS_REQUEST_BUILD_OFFSET =
        RENDEZVOUS_REQUEST_VERSION_OFFSET + PMM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    ...
    RENDEZVOUS_RESPONSE_ACTIVE_BUILD_OFFSET =
        RENDEZVOUS_RESPONSE_ACTIVE_VERSION_OFFSET + PMM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    RENDEZVOUS_RESPONSE_REQUESTED_VERSION_OFFSET =
        RENDEZVOUS_RESPONSE_ACTIVE_BUILD_OFFSET + PMM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
    RENDEZVOUS_RESPONSE_REQUESTED_BUILD_OFFSET =
        RENDEZVOUS_RESPONSE_REQUESTED_VERSION_OFFSET + PMM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    RENDEZVOUS_RESPONSE_MESSAGE_OFFSET =
        RENDEZVOUS_RESPONSE_REQUESTED_BUILD_OFFSET + PMM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
...
_Static_assert(RENDEZVOUS_REQUEST_BUILD_OFFSET + PMM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP ==
                   PMM_DAEMON_RENDEZVOUS_REQUEST_SIZE,
               "rendezvous request layout changed");
_Static_assert(RENDEZVOUS_RESPONSE_MESSAGE_OFFSET + PMM_DAEMON_RENDEZVOUS_MESSAGE_CAP ==
                   PMM_DAEMON_RENDEZVOUS_RESPONSE_SIZE,
               "rendezvous response layout changed");
_Static_assert(ACTIVATION_REQUEST_IDENTITY_OFFSET + PMM_DAEMON_RENDEZVOUS_REQUEST_SIZE ==
...
_Static_assert(PMM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP == PMM_DAEMON_VERSION_TEXT_SIZE,
               "service version capacity requires a new rendezvous strategy");
_Static_assert(PMM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP == PMM_DAEMON_BUILD_FINGERPRINT_SIZE,
               "service fingerprint capacity requires a new rendezvous strategy");
_Static_assert(PMM_DAEMON_RENDEZVOUS_MESSAGE_CAP == PMM_DAEMON_CONFLICT_MESSAGE_SIZE,
               "service message capacity requires a new rendezvous strategy");
_Static_assert(PMM_DAEMON_RUNTIME_CONNECT_ERROR == 0 && PMM_DAEMON_RUNTIME_CONNECT_ACCEPTED == 1 &&
                   PMM_DAEMON_RUNTIME_CONNECT_CONFLICT == 2 &&
                   PMM_DAEMON_RUNTIME_CONNECT_REJECTED == 3,
               "rendezvous connect status values changed");
_Static_assert(PMM_DAEMON_HELLO_INVALID == 0 && PMM_DAEMON_HELLO_COMPATIBLE == 1 &&
                   PMM_DAEMON_HELLO_VERSION_CONFLICT == 2 && PMM_DAEMON_HELLO_BUILD_CONFLICT == 3 &&
                   PMM_DAEMON_HELLO_PROTOCOL_ABI_CONFLICT == 4 &&
                   PMM_DAEMON_HELLO_STORE_ABI_CONFLICT == 5 &&
                   PMM_DAEMON_HELLO_FEATURE_ABI_CONFLICT == 6,
               "rendezvous hello status values changed");
_Static_assert(PMM_DAEMON_RENDEZVOUS_FRAME_VERSION == 1 && PMM_DAEMON_RENDEZVOUS_ABI == 1 &&
                   PMM_DAEMON_FRAME_REQUEST == 1 && PMM_DAEMON_FRAME_RESPONSE == 2 &&
...
typedef struct pmm_daemon_runtime_worker pmm_daemon_runtime_worker_t;
...
struct pmm_daemon_runtime_service {
    ...
    char semantic_version[PMM_DAEMON_VERSION_TEXT_SIZE];
    ...
    pmm_daemon_ipc_participant_guard_t *owned_participant_guard;
};

struct pmm_daemon_runtime_worker {
    pmm_daemon_runtime_service_t *service;
    pmm_mutex_t send_mutex;
    pmm_thread_t thread;
    bool thread_started;
    bool in_use;
    bool joining;
    ...
    pmm_daemon_runtime_application_session_t *application_session;
    bool application_session_opened;
    bool application_cancelled;
    atomic_bool disconnecting;

    pmm_thread_t application_thread;
    bool application_thread_started;
    atomic_bool application_thread_done;
    pmm_daemon_runtime_application_token_t application_request_token;
    pmm_daemon_runtime_application_token_t last_application_request_token;
    uint8_t *application_request;
    uint32_t application_request_length;
};

struct pmm_daemon_runtime_client {
    pmm_mutex_t exchange_mutex;
    pmm_mutex_t send_mutex;
    pmm_mutex_t state_mutex;
    pmm_daemon_ipc_connection_t *connection;
    pmm_daemon_client_id_t client_id;
    ...
    pmm_daemon_runtime_application_token_t next_application_token;
    pmm_daemon_runtime_application_token_t last_started_application_token;
    pmm_daemon_runtime_application_token_t active_application_token;
    pmm_daemon_runtime_application_token_t pending_cancel_token;
...
static pmm_daemon_hello_status_t runtime_rendezvous_compare(const char *active_version,
    ...
    pmm_daemon_build_identity_t requested = {
    ...
    pmm_daemon_hello_status_t status = pmm_daemon_hello_compare(&active, &requested, conflict_out);
    if (status != PMM_DAEMON_HELLO_INVALID && status != PMM_DAEMON_HELLO_COMPATIBLE &&
        status != PMM_DAEMON_HELLO_VERSION_CONFLICT && status != PMM_DAEMON_HELLO_BUILD_CONFLICT) {
        if (conflict_out) {
            memset(conflict_out, 0, sizeof(*conflict_out));
            conflict_out->status = PMM_DAEMON_HELLO_INVALID;
        }
        return PMM_DAEMON_HELLO_INVALID;
    }
    return status;
}
...
bool pmm_daemon_runtime_hello_request_encode(uint8_t out[PMM_DAEMON_RENDEZVOUS_REQUEST_SIZE],
    ...
    pmm_daemon_conflict_t validation;
    if (runtime_rendezvous_compare(identity->semantic_version, identity->build_fingerprint,
                                   ...
                                   &validation) != PMM_DAEMON_HELLO_COMPATIBLE) {
        return false;
    }
    memset(out, 0, PMM_DAEMON_RENDEZVOUS_REQUEST_SIZE);
    runtime_put_u32(out + RENDEZVOUS_REQUEST_ABI_OFFSET, PMM_DAEMON_RENDEZVOUS_ABI);
    return runtime_wire_string_encode(out + RENDEZVOUS_REQUEST_VERSION_OFFSET,
                                      PMM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
                                      identity->semantic_version, false) &&
           runtime_wire_string_encode(out + RENDEZVOUS_REQUEST_BUILD_OFFSET,
                                      PMM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
                                      identity->build_fingerprint, false);
}

static bool runtime_rendezvous_request_decode(
    const uint8_t payload[PMM_DAEMON_RENDEZVOUS_REQUEST_SIZE],
    char version[PMM_DAEMON_VERSION_TEXT_SIZE], char build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!payload || !version || !build ||
        runtime_get_u32(payload + RENDEZVOUS_REQUEST_ABI_OFFSET) != PMM_DAEMON_RENDEZVOUS_ABI ||
        !runtime_wire_string_decode(payload + RENDEZVOUS_REQUEST_VERSION_OFFSET,
                                    PMM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP, version, false) ||
        !runtime_wire_string_decode(payload + RENDEZVOUS_REQUEST_BUILD_OFFSET,
                                    PMM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP, build, false)) {
        return false;
    }
    pmm_daemon_conflict_t validation;
...
static bool runtime_activation_action_valid(pmm_daemon_runtime_activation_action_t action) {
    return action == PMM_DAEMON_RUNTIME_ACTIVATION_INSTALL ||
           action == PMM_DAEMON_RUNTIME_ACTIVATION_UPDATE ||
           action == PMM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL;
}

static const char *runtime_activation_action_text(pmm_daemon_runtime_activation_action_t action) {
    switch (action) {
...
static bool runtime_activation_request_encode(
    uint8_t out[PMM_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE],
    ...
    memset(out, 0, PMM_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE);
    runtime_put_u32(out + ACTIVATION_REQUEST_ACTION_OFFSET, (uint32_t)action);
    return pmm_daemon_runtime_hello_request_encode(out + ACTIVATION_REQUEST_IDENTITY_OFFSET,
                                                   identity);
}

static bool runtime_activation_request_decode(
    const uint8_t payload[PMM_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE],
    pmm_daemon_runtime_activation_action_t *action_out, char version[PMM_DAEMON_VERSION_TEXT_SIZE],
    char build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!payload || !action_out || !version || !build) {
        return false;
    }
    uint32_t raw_action = runtime_get_u32(payload + ACTIVATION_REQUEST_ACTION_OFFSET);
    if (raw_action < (uint32_t)PMM_DAEMON_RUNTIME_ACTIVATION_INSTALL ||
        raw_action > (uint32_t)PMM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL ||
    ...
    pmm_daemon_runtime_activation_action_t action =
        (pmm_daemon_runtime_activation_action_t)raw_action;
    *action_out = action;
    return true;
}

static bool runtime_activation_response_encode(
    uint8_t out[PMM_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE],
    const pmm_daemon_runtime_activation_result_t *result) {
    if (!out || !result) {
        return false;
    }
    memset(out, 0, PMM_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE);
    runtime_put_u32(out + ACTIVATION_RESPONSE_ABI_OFFSET, PMM_DAEMON_RENDEZVOUS_ABI);
...
static bool runtime_activation_response_decode(
    const uint8_t payload[PMM_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE],
    pmm_daemon_runtime_activation_result_t *result_out) {
    if (!payload || !result_out ||
        runtime_get_u32(payload + ACTIVATION_RESPONSE_ABI_OFFSET) != PMM_DAEMON_RENDEZVOUS_ABI) {
        return false;
    }
...
static bool runtime_process_image_reference_acquire(
    ...
    ok = ok && runtime_windows_file_snapshot(file, &file_before, &size_before) &&
         (!fingerprint || pmm_daemon_build_fingerprint_native_file((uintptr_t)file, fingerprint)) &&
    ...
    ok = ok && fd >= 0 && fstat(fd, &file_before) == 0 && S_ISREG(file_before.st_mode) &&
         ...
         (!fingerprint || pmm_daemon_build_fingerprint_native_file((uintptr_t)fd, fingerprint)) &&
    ...
    bool ok = image_fd >= 0 && fstat(image_fd, &image_before) == 0 &&
              ...
              (!fingerprint ||
               pmm_daemon_build_fingerprint_native_file((uintptr_t)image_fd, fingerprint)) &&
...
static bool runtime_hello_response_encode(uint8_t out[PMM_DAEMON_RENDEZVOUS_RESPONSE_SIZE],
                                          const pmm_daemon_runtime_connect_result_t *result) {
    if (!out || !result) {
        return false;
    }
    memset(out, 0, PMM_DAEMON_RENDEZVOUS_RESPONSE_SIZE);
    ...
    if (!runtime_wire_string_encode(out + RENDEZVOUS_RESPONSE_ACTIVE_VERSION_OFFSET,
        ...
        !runtime_wire_string_encode(out + RENDEZVOUS_RESPONSE_MESSAGE_OFFSET,
                                    PMM_DAEMON_RENDEZVOUS_MESSAGE_CAP, result->message, true)) {
        return false;
    }
    return true;
}
...
static bool runtime_hello_response_decode(
    const uint8_t payload[PMM_DAEMON_RENDEZVOUS_RESPONSE_SIZE],
    pmm_daemon_runtime_connect_result_t *result) {
    if (!payload || !result) {
        return false;
    }
    memset(result, 0, sizeof(*result));
    result->status = (pmm_daemon_runtime_connect_status_t)runtime_get_u32(
        payload + RENDEZVOUS_RESPONSE_CONNECT_STATUS_OFFSET);
    result->hello_status = (pmm_daemon_hello_status_t)runtime_get_u32(
    ...
    result->conflict.status = (pmm_daemon_hello_status_t)runtime_get_u32(
    ...
    if (result->status == PMM_DAEMON_RUNTIME_CONNECT_ACCEPTED) {
        return result->hello_status == PMM_DAEMON_HELLO_COMPATIBLE &&
               result->conflict.status == PMM_DAEMON_HELLO_COMPATIBLE &&
               result->client_id != PMM_DAEMON_CLIENT_ID_INVALID &&
               result->authenticated_process_id != 0;
    }
    if (result->status == PMM_DAEMON_RUNTIME_CONNECT_CONFLICT) {
        char expected[PMM_DAEMON_RENDEZVOUS_MESSAGE_CAP];
        return result->hello_status >= PMM_DAEMON_HELLO_VERSION_CONFLICT &&
               result->hello_status <= PMM_DAEMON_HELLO_FEATURE_ABI_CONFLICT &&
               result->conflict.status == result->hello_status &&
               result->client_id == PMM_DAEMON_CLIENT_ID_INVALID &&
               result->authenticated_process_id == 0 &&
               pmm_daemon_conflict_format(&result->conflict, expected, sizeof(expected)) &&
               strcmp(result->message, expected) == 0;
    }
    return result->status == PMM_DAEMON_RUNTIME_CONNECT_REJECTED &&
           result->hello_status == PMM_DAEMON_HELLO_INVALID &&
           result->conflict.status == PMM_DAEMON_HELLO_INVALID &&
           result->client_id == PMM_DAEMON_CLIENT_ID_INVALID &&
           result->authenticated_process_id == 0;
}

static bool runtime_send_hello_response(pmm_daemon_ipc_connection_t *connection,
                                        const pmm_daemon_runtime_connect_result_t *result) {
    uint8_t payload[PMM_DAEMON_RENDEZVOUS_RESPONSE_SIZE];
    return runtime_hello_response_encode(payload, result) &&
           pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_RESPONSE,
                                     PMM_DAEMON_RUNTIME_OP_HELLO, payload,
                                     (uint32_t)sizeof(payload));
}

static bool runtime_worker_send_frame(pmm_daemon_runtime_worker_t *worker,
                                      pmm_daemon_frame_type_t type,
                                      pmm_daemon_runtime_operation_t operation, const void *payload,
                                      uint32_t length) {
    if (!worker) {
        return false;
    }
    pmm_mutex_lock(&worker->send_mutex);
    bool sent =
        worker->connection &&
        pmm_daemon_ipc_send_frame(worker->connection, type, (uint16_t)operation, payload, length);
    pmm_mutex_unlock(&worker->send_mutex);
    return sent;
}

static bool runtime_worker_send_status(pmm_daemon_runtime_worker_t *worker,
                                       pmm_daemon_runtime_operation_t operation, bool success) {
    uint8_t payload[STATUS_RESPONSE_SIZE];
    runtime_put_u32(payload, success ? 1U : 0U);
    return runtime_worker_send_frame(worker, PMM_DAEMON_FRAME_RESPONSE, operation, payload,
                                     (uint32_t)sizeof(payload));
}

static bool runtime_application_status_is_callback_result(
    pmm_daemon_runtime_application_status_t status) {
    return status == PMM_DAEMON_RUNTIME_APPLICATION_OK ||
           status == PMM_DAEMON_RUNTIME_APPLICATION_REJECTED ||
           status == PMM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR ||
           status == PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED;
}

static bool runtime_worker_send_application_response(
    pmm_daemon_runtime_worker_t *worker, pmm_daemon_runtime_application_token_t request_token,
    pmm_daemon_runtime_application_status_t status, const uint8_t *response,
    uint32_t response_length, bool suppress_when_disconnecting) {
    if (!worker || request_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
        status <= PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR ||
        status > PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED ||
        response_length > PMM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
        (response_length > 0 && !response)) {
        return false;
    }
    uint64_t wire_length = APPLICATION_RESPONSE_PREFIX_SIZE + (uint64_t)response_length;
    if (wire_length > PMM_DAEMON_MAX_FRAME_SIZE || wire_length > UINT32_MAX) {
        return false;
    }
    ...
    bool sent = !disconnected && worker->connection &&
                pmm_daemon_ipc_send_frame(worker->connection, PMM_DAEMON_FRAME_RESPONSE,
...
static void runtime_result_rejected(pmm_daemon_runtime_connect_result_t *result,
                                    const char *message) {
    memset(result, 0, sizeof(*result));
    result->status = PMM_DAEMON_RUNTIME_CONNECT_REJECTED;
    result->hello_status = PMM_DAEMON_HELLO_INVALID;
    result->conflict.status = PMM_DAEMON_HELLO_INVALID;
    (void)snprintf(result->message, sizeof(result->message), "%s", message);
}

static void runtime_service_begin_stopping_locked(pmm_daemon_runtime_service_t *service,
                                                  uint64_t deadline, bool emergency,
                                                  const char *reason) {
    if (service->state == PMM_DAEMON_RUNTIME_SERVICE_RUNNING) {
        service->state = PMM_DAEMON_RUNTIME_SERVICE_STOPPING;
        ...
        pmm_log_info("daemon.runtime_stopping", "reason", reason ? reason : "unspecified");
    } else if (service->state == PMM_DAEMON_RUNTIME_SERVICE_STOPPING &&
...
static void runtime_service_begin_stopping(pmm_daemon_runtime_service_t *service,
                                           uint32_t timeout_ms, bool emergency,
                                           const char *reason) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    pmm_mutex_lock(&service->mutex);
    runtime_service_begin_stopping_locked(service, deadline, emergency, reason);
    pmm_mutex_unlock(&service->mutex);
}

static void runtime_service_interrupt_connections_except(pmm_daemon_runtime_service_t *service,
                                                         pmm_daemon_runtime_worker_t *except_worker,
    ...
    for (size_t i = 0; i < service->worker_capacity; i++) {
        pmm_daemon_runtime_worker_t *worker = &service->workers[i];
        if (worker != except_worker && worker->in_use && worker->connection &&
            (activation_owner || force || !worker->final_response_inflight)) {
            pmm_daemon_ipc_connection_interrupt(worker->connection);
        }
    }
    pmm_mutex_unlock(&service->mutex);
}

static void runtime_service_interrupt_connections(pmm_daemon_runtime_service_t *service) {
    runtime_service_interrupt_connections_except(service, NULL, false);
}

static void runtime_worker_disconnect(pmm_daemon_runtime_worker_t *worker) {
    pmm_daemon_runtime_service_t *service = worker->service;
    pmm_daemon_client_id_t client_id = PMM_DAEMON_CLIENT_ID_INVALID;
    ...
    (void)pmm_daemon_client_disconnected(service->coordinator, client_id, pmm_now_ms());
    ...
    if (pmm_daemon_coordinator_state(service->coordinator) == PMM_DAEMON_COORDINATOR_STOPPING) {
        runtime_service_begin_stopping(service, service->shutdown_timeout_ms, false,
                                       "coordinator_stopping");
    }
}

static bool runtime_worker_service_running(pmm_daemon_runtime_worker_t *worker) {
    pmm_daemon_runtime_service_t *service = worker->service;
    pmm_mutex_lock(&service->mutex);
    bool running = service->state == PMM_DAEMON_RUNTIME_SERVICE_RUNNING;
    pmm_mutex_unlock(&service->mutex);
    return running;
}

static bool runtime_worker_admit(pmm_daemon_runtime_worker_t *worker,
                                 pmm_daemon_client_id_t *client_id_out) {
    pmm_daemon_runtime_service_t *service = worker->service;
    pmm_daemon_client_id_t client_id = PMM_DAEMON_CLIENT_ID_INVALID;
    pmm_mutex_lock(&service->mutex);
    if (service->state == PMM_DAEMON_RUNTIME_SERVICE_RUNNING) {
        client_id = pmm_daemon_client_connected(service->coordinator, pmm_now_ms());
        if (client_id != PMM_DAEMON_CLIENT_ID_INVALID) {
    ...
    *client_id_out = client_id;
    return client_id != PMM_DAEMON_CLIENT_ID_INVALID;
}

static bool runtime_worker_commit_admission(pmm_daemon_runtime_worker_t *worker) {
    pmm_daemon_runtime_service_t *service = worker->service;
    pmm_mutex_lock(&service->mutex);
    bool committed = service->state == PMM_DAEMON_RUNTIME_SERVICE_RUNNING && worker->admitted &&
...
static bool runtime_worker_handle_subscribe(pmm_daemon_runtime_worker_t *worker,
    ...
    if (key_length == 0 || key_length > PMM_DAEMON_RUNTIME_PROJECT_KEY_MAX ||
        length != SUBSCRIBE_REQUEST_PREFIX_SIZE + key_length) {
        return false;
    }
    char project_key[PMM_DAEMON_RUNTIME_PROJECT_KEY_MAX + 1];
    ...
    pmm_daemon_subscription_id_t subscription_id = PMM_DAEMON_SUBSCRIPTION_ID_INVALID;
    pmm_daemon_subscription_result_t result = pmm_daemon_job_subscribe(
    ...
    return runtime_worker_send_frame(worker, PMM_DAEMON_FRAME_RESPONSE,
                                     PMM_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE, response,
                                     (uint32_t)sizeof(response));
}

static bool runtime_worker_handle_unsubscribe(pmm_daemon_runtime_worker_t *worker,
                                              const uint8_t *payload, uint32_t length) {
    if (!runtime_worker_service_running(worker) || !payload || length != UNSUBSCRIBE_REQUEST_SIZE) {
        return false;
    }
    pmm_daemon_subscription_id_t subscription_id = runtime_get_u64(payload);
    bool removed = pmm_daemon_job_unsubscribe(worker->service->coordinator, worker->client_id,
                                              subscription_id);
    return runtime_worker_send_status(worker, PMM_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE, removed);
}

static void *runtime_application_worker(void *opaque) {
    pmm_daemon_runtime_worker_t *worker = opaque;
    pmm_daemon_runtime_service_t *service = worker->service;
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    pmm_daemon_runtime_application_status_t status = service->application.request(
    ...
    bool valid_response =
        ...
        (status == PMM_DAEMON_RUNTIME_APPLICATION_OK || (response == NULL && response_length == 0));
...
static bool runtime_worker_reap_application(pmm_daemon_runtime_worker_t *worker, bool wait) {
    ...
    if (pmm_thread_join(&worker->application_thread) != 0) {
        return false;
    }
    ...
    worker->application_request_token = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
    return true;
}

static bool runtime_worker_handle_application(pmm_daemon_runtime_worker_t *worker,
                                              const uint8_t *payload, uint32_t length) {
    if (!payload || length < APPLICATION_REQUEST_PREFIX_SIZE) {
        return false;
    }
    pmm_daemon_runtime_application_token_t request_token = runtime_get_u64(payload);
    uint32_t request_length = runtime_get_u32(payload + 8);
    if (request_length > PMM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
        length != APPLICATION_REQUEST_PREFIX_SIZE + request_length ||
        request_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
    ...
    if (!service->application.request) {
        return runtime_worker_send_application_response(
            worker, request_token, PMM_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE, NULL, 0, false);
    }
    (void)runtime_worker_reap_application(worker, false);
    if (worker->application_thread_started) {
        return runtime_worker_send_application_response(
            worker, request_token, PMM_DAEMON_RUNTIME_APPLICATION_BUSY, NULL, 0, false);
    }
    ...
    if (request_length > 0) {
        ...
        if (!request_copy) {
            return runtime_worker_send_application_response(
                worker, request_token, PMM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR, NULL, 0,
                false);
        }
    ...
    atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
    if (pmm_thread_create(&worker->application_thread, RUNTIME_WORKER_STACK_SIZE,
                          runtime_application_worker, worker) != 0) {
        free(worker->application_request);
        worker->application_request = NULL;
        worker->application_request_length = 0;
        return runtime_worker_send_application_response(
            worker, request_token, PMM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR, NULL, 0, false);
    }
    worker->application_thread_started = true;
    return true;
}

static bool runtime_worker_handle_application_cancel(pmm_daemon_runtime_worker_t *worker,
                                                     const uint8_t *payload, uint32_t length) {
    if (!worker || !payload || length != APPLICATION_CANCEL_REQUEST_SIZE) {
        return false;
    }
    pmm_daemon_runtime_application_token_t request_token = runtime_get_u64(payload);
    if (request_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID) {
        return false;
    }
...
static bool runtime_worker_handle_close_intent(pmm_daemon_runtime_worker_t *worker,
                                               uint32_t length) {
    if (length != 0) {
        return false;
    }
...
static bool runtime_worker_handle_disconnect(pmm_daemon_runtime_worker_t *worker, uint32_t length) {
    if (length != 0) {
        return false;
    }
    pmm_daemon_runtime_service_t *service = worker->service;
    ...
    pmm_mutex_unlock(&service->mutex);
    runtime_worker_disconnect(worker);
    bool sent = runtime_worker_send_status(worker, PMM_DAEMON_RUNTIME_OP_DISCONNECT, true);
    pmm_mutex_lock(&service->mutex);
...
static void runtime_worker_finish(pmm_daemon_runtime_worker_t *worker) {
    pmm_daemon_runtime_service_t *service = worker->service;
    runtime_worker_disconnect(worker);
    ...
    if (worker->connection) {
         ...
         * it before the handle closes; see pmm_daemon_ipc_connection_drain. */
        pmm_daemon_ipc_connection_drain(worker->connection, RUNTIME_WORKER_DRAIN_TIMEOUT_MS);
    }
    pmm_mutex_lock(&service->mutex);
    if (worker->connection) {
        pmm_daemon_ipc_connection_close(worker->connection);
...
static bool runtime_activation_peer_matches_claim(pmm_daemon_runtime_service_t *service,
    ...
    char peer_fingerprint[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];
    return pmm_daemon_runtime_process_build_fingerprint(process_id, peer_fingerprint) &&
...
static bool runtime_control_request_decode(const uint8_t *payload, uint32_t length,
    ...
    if (!payload || length != PMM_DAEMON_CONTROL_REQUEST_SIZE ||
        payload[PMM_DAEMON_CONTROL_REQUEST_SIZE - 1U] != 0U) {
        return false;
    }
    memcpy(build_out, payload, PMM_DAEMON_BUILD_FINGERPRINT_SIZE);
    return strlen(build_out) == PMM_DAEMON_BUILD_FINGERPRINT_SIZE - 1U;
}

static void runtime_control_collect_clients_locked(pmm_daemon_runtime_service_t *service,
                                                   pmm_daemon_runtime_worker_t *except,
                                                   uint8_t *count_out,
                                                   uint32_t pids[PMM_DAEMON_CONTROL_CLIENT_CAP]) {
    uint8_t count = 0U;
    for (size_t index = 0; index < service->worker_capacity; index++) {
        pmm_daemon_runtime_worker_t *candidate = &service->workers[index];
        if (!candidate->in_use || !candidate->admission_committed || candidate == except) {
            continue;
        }
        if (count < PMM_DAEMON_CONTROL_CLIENT_CAP) {
            pids[count] = (uint32_t)candidate->peer_process_id;
        }
        count++;
    }
    *count_out =
        count > PMM_DAEMON_CONTROL_CLIENT_CAP ? (uint8_t)PMM_DAEMON_CONTROL_CLIENT_CAP : count;
}

static void runtime_worker_handle_status(pmm_daemon_runtime_worker_t *worker,
                                         const uint8_t *payload, uint32_t length) {
    pmm_daemon_runtime_service_t *service = worker->service;
    char requested_build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool peer_verified =
    ...
    uint8_t response[PMM_DAEMON_STATUS_RESPONSE_SIZE];
    memset(response, 0, sizeof(response));
    if (peer_verified) {
        uint8_t count = 0U;
        uint32_t pids[PMM_DAEMON_CONTROL_CLIENT_CAP] = {0};
        pmm_mutex_lock(&service->mutex);
        response[0] = 1U;
        response[1] =
            (uint8_t)((service->permanent ? 0x01U : 0U) |
                      (service->state != PMM_DAEMON_RUNTIME_SERVICE_RUNNING ? 0x02U : 0U));
        ...
        response[8] = count;
        for (size_t index = 0; index < PMM_DAEMON_CONTROL_CLIENT_CAP; index++) {
        ...
        (void)snprintf((char *)response + 44U, PMM_DAEMON_BUILD_FINGERPRINT_SIZE, "%s",
    ...
    (void)runtime_worker_send_frame(worker, PMM_DAEMON_FRAME_RESPONSE, PMM_DAEMON_RUNTIME_OP_STATUS,
                                    response, (uint32_t)sizeof(response));
    runtime_worker_finish(worker);
}

static void runtime_worker_handle_stop(pmm_daemon_runtime_worker_t *worker, const uint8_t *payload,
                                       uint32_t length) {
    pmm_daemon_runtime_service_t *service = worker->service;
    char requested_build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool peer_verified =
        runtime_control_request_decode(payload, length, requested_build) &&
        runtime_activation_peer_matches_claim(service, worker->peer_process_id, requested_build);
    uint8_t response[PMM_DAEMON_STOP_RESPONSE_SIZE];
    memset(response, 0, sizeof(response));
    bool accepted = false;
    if (peer_verified) {
        uint8_t count = 0U;
        ...
        if (service->committed_clients > 0) {
            ...
            response[1] = 0x02U;
        } else if (service->state == PMM_DAEMON_RUNTIME_SERVICE_RUNNING) {
    ...
    if (accepted) {
        ...
        pmm_log_info("daemon.stop_requested", "requester_pid", requester_pid, "requester_build",
                     requested_build);
        runtime_service_interrupt_connections_except(worker->service, worker, true);
    }
    (void)runtime_worker_send_frame(worker, PMM_DAEMON_FRAME_RESPONSE, PMM_DAEMON_RUNTIME_OP_STOP,
                                    response, (uint32_t)sizeof(response));
    if (accepted) {
        pmm_daemon_runtime_service_t *stop_service = worker->service;
        runtime_worker_finish(worker);
        pmm_mutex_lock(&stop_service->mutex);
        worker->final_response_inflight = false;
        stop_service->activation_response_inflight = false;
        pmm_mutex_unlock(&stop_service->mutex);
...
static void runtime_worker_handle_activation_shutdown(pmm_daemon_runtime_worker_t *worker,
                                                      const uint8_t *payload, uint32_t length) {
    pmm_daemon_runtime_service_t *service = worker->service;
    pmm_daemon_runtime_activation_result_t result = {0};
    pmm_daemon_runtime_activation_action_t action = 0;
    char requested_version[PMM_DAEMON_VERSION_TEXT_SIZE] = {0};
    char requested_build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool request_valid =
        length == PMM_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE && payload &&
    ...
    uint8_t response[PMM_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE];
    ...
    if (result.accepted) {
        ...
        pmm_log_info("daemon.activation_shutdown", "requester_pid", requester_pid,
    ...
    bool response_sent =
        encoded && runtime_worker_send_frame(worker, PMM_DAEMON_FRAME_RESPONSE,
                                             PMM_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN, response,
                                             (uint32_t)sizeof(response));
    if (result.accepted && !response_sent) {
        pmm_log_error("daemon.activation_shutdown_ack_failed", "shutdown", "accepted",
                      "requester_build", requested_build, "action",
...
static void *runtime_connection_worker(void *opaque) {
    ...
    char requested_build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];

    int received = pmm_daemon_ipc_receive_frame_bounded(
        worker->connection, service->request_timeout_ms,
        PMM_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE, &frame, &payload);
    if (received != 1 || frame.type != PMM_DAEMON_FRAME_REQUEST) {
        free(payload);
        runtime_worker_finish(worker);
        return NULL;
    }
    if (frame.flags == PMM_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN) {
        runtime_worker_handle_activation_shutdown(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags == PMM_DAEMON_RUNTIME_OP_STATUS) {
        runtime_worker_handle_status(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags == PMM_DAEMON_RUNTIME_OP_STOP) {
        runtime_worker_handle_stop(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags != PMM_DAEMON_RUNTIME_OP_HELLO ||
        frame.length != PMM_DAEMON_RENDEZVOUS_REQUEST_SIZE ||
    ...
    pmm_daemon_runtime_connect_result_t hello_result;
    memset(&hello_result, 0, sizeof(hello_result));
    pmm_daemon_hello_status_t hello_status = runtime_rendezvous_compare(
    ...
    if (hello_status != PMM_DAEMON_HELLO_COMPATIBLE) {
        if (hello_status == PMM_DAEMON_HELLO_INVALID) {
            runtime_worker_finish(worker);
            return NULL;
        }
        hello_result.status = PMM_DAEMON_RUNTIME_CONNECT_CONFLICT;
        hello_result.hello_status = hello_status;
        (void)pmm_daemon_conflict_format(&hello_result.conflict, hello_result.message,
                                         sizeof(hello_result.message));
        if (!pmm_daemon_conflict_log_append(service->conflict_log_path, &hello_result.conflict,
            ...
            pmm_log_error("daemon.conflict_log_append_failed", "path", service->conflict_log_path);
        }
    ...
    if (!peer_image_verified) {
        ...
        peer_image_fingerprinted =
            pmm_daemon_runtime_process_build_fingerprint(worker->peer_process_id, peer_fingerprint);
    ...
    if (!peer_image_verified) {
        pmm_log_error("daemon.client_image_rejected", "reason",
    ...
    if (!runtime_worker_admit(worker, &client_id)) {
        runtime_result_rejected(&hello_result, "PMM daemon is stopping");
        (void)runtime_send_hello_response(worker->connection, &hello_result);
        runtime_worker_finish(worker);
        return NULL;
    }
    if (service->application.request) {
        ...
        if (!worker->application_session) {
            runtime_result_rejected(&hello_result, "PMM daemon session initialization failed");
            pmm_mutex_lock(&service->mutex);
            worker->final_response_inflight = true;
            pmm_mutex_unlock(&service->mutex);
            runtime_worker_disconnect(worker);
    ...
    hello_result.status = PMM_DAEMON_RUNTIME_CONNECT_ACCEPTED;
    hello_result.hello_status = PMM_DAEMON_HELLO_COMPATIBLE;
    hello_result.conflict.status = PMM_DAEMON_HELLO_COMPATIBLE;
    ...
    while (keep_running && runtime_worker_service_running(worker)) {
        ...
        payload = NULL;
        received = pmm_daemon_ipc_receive_frame(worker->connection, PMM_DAEMON_IPC_WAIT_FOREVER,
                                                &frame, &payload);
        if (received != 1 || frame.type != PMM_DAEMON_FRAME_REQUEST) {
            ...
            pmm_log_info("daemon.connection_end", "received", received_text, "frame_type",
                         received == 1 ? "non_request" : "none");
            break;
        }
        switch (frame.flags) {
        case PMM_DAEMON_RUNTIME_OP_HEARTBEAT: {
            bool valid = frame.length == 0 &&
                         pmm_daemon_client_heartbeat(service->coordinator, client_id, pmm_now_ms());
            keep_running =
                valid && runtime_worker_send_status(worker, PMM_DAEMON_RUNTIME_OP_HEARTBEAT, true);
            break;
        }
        case PMM_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE:
            keep_running = runtime_worker_handle_subscribe(worker, payload, frame.length);
            break;
        case PMM_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE:
            keep_running = runtime_worker_handle_unsubscribe(worker, payload, frame.length);
            break;
        case PMM_DAEMON_RUNTIME_OP_APPLICATION_REQUEST:
            keep_running = runtime_worker_handle_application(worker, payload, frame.length);
            break;
        case PMM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL:
            keep_running = runtime_worker_handle_application_cancel(worker, payload, frame.length);
            break;
        case PMM_DAEMON_RUNTIME_OP_CLOSE_INTENT:
            keep_running = runtime_worker_handle_close_intent(worker, frame.length);
            break;
        case PMM_DAEMON_RUNTIME_OP_DISCONNECT:
            keep_running = false;
...
static void runtime_worker_reset_after_join(pmm_daemon_runtime_worker_t *worker) {
    free(worker->application_request);
    worker->thread_started = false;
    worker->in_use = false;
    worker->joining = false;
    worker->connection = NULL;
    worker->client_id = PMM_DAEMON_CLIENT_ID_INVALID;
    ...
    worker->application_thread_started = false;
    worker->application_request_token = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    worker->last_application_request_token = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    worker->application_request = NULL;
    worker->application_request_length = 0;
...
static void runtime_reap_completed_workers(pmm_daemon_runtime_service_t *service) {
    for (size_t i = 0; i < service->worker_capacity; i++) {
        pmm_daemon_runtime_worker_t *worker = &service->workers[i];
        pmm_mutex_lock(&service->mutex);
        ...
        int joined = pmm_thread_join(&worker->thread);
        pmm_mutex_lock(&service->mutex);
        if (joined == 0) {
...
static pmm_daemon_runtime_worker_t *runtime_find_free_worker_locked(
    pmm_daemon_runtime_service_t *service) {
    for (size_t i = 0; i < service->worker_capacity; i++) {
...
static void runtime_reject_inline(pmm_daemon_ipc_connection_t *connection, const char *message) {
    pmm_daemon_runtime_connect_result_t result;
    ...
    pmm_daemon_ipc_connection_drain(connection, RUNTIME_REJECT_DRAIN_TIMEOUT_MS);
    pmm_daemon_ipc_connection_close(connection);
}

static void runtime_accept_connection(pmm_daemon_runtime_service_t *service,
                                      pmm_daemon_ipc_connection_t *connection) {
    uint64_t peer_pid = pmm_daemon_ipc_connection_peer_pid(connection);
    if (peer_pid == 0) {
        pmm_daemon_ipc_connection_close(connection);
        return;
    }
    ...
    pmm_daemon_runtime_worker_t *worker =
    ...
    if (worker) {
        ...
        atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
        service->active_connections++;
        int created = pmm_thread_create(&worker->thread, RUNTIME_WORKER_STACK_SIZE,
    ...
    if (!running) {
        runtime_reject_inline(connection, "PMM daemon is stopping");
    } else {
        runtime_reject_inline(connection, "PMM daemon connection capacity reached");
    }
}

static bool runtime_service_can_exit(pmm_daemon_runtime_service_t *service, uint64_t now_ms) {
    bool can_exit = false;
    pmm_mutex_lock(&service->mutex);
    if (service->state == PMM_DAEMON_RUNTIME_SERVICE_STOPPING && service->active_connections == 0) {
        bool drained = pmm_daemon_should_exit(service->coordinator, now_ms);
        if (!drained && service->activation_shutdown_requested) {
            ...
            drained = pmm_daemon_active_clients(service->coordinator) == 0 &&
                      pmm_daemon_active_jobs(service->coordinator) == 0 &&
                      pmm_daemon_active_watches(service->coordinator) == 0;
        }
        bool deadline_reached =
            service->stop_deadline_ms != 0 && now_ms >= service->stop_deadline_ms;
        can_exit = drained || deadline_reached || service->emergency_stop;
        if (can_exit) {
            service->state = PMM_DAEMON_RUNTIME_SERVICE_EXITED;
        }
    }
    pmm_mutex_unlock(&service->mutex);
    return can_exit;
...
static void *runtime_accept_loop(void *opaque) {
    pmm_daemon_runtime_service_t *service = opaque;
    for (;;) {
        runtime_reap_completed_workers(service);
        pmm_mutex_lock(&service->mutex);
        pmm_daemon_runtime_service_state_t state = service->state;
        pmm_mutex_unlock(&service->mutex);
        if (state == PMM_DAEMON_RUNTIME_SERVICE_EXITED) {
            break;
        }
        if (state == PMM_DAEMON_RUNTIME_SERVICE_STOPPING) {
            runtime_service_interrupt_connections(service);
            if (runtime_service_can_exit(service, pmm_now_ms())) {
                break;
            }
        ...
        int accepted =
            pmm_daemon_ipc_accept(service->listener, RUNTIME_ACCEPT_POLL_MS, &connection);
...
static bool runtime_participant_guard_release_complete(
    pmm_daemon_ipc_participant_guard_t **guard_io, uint32_t timeout_ms) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    while (guard_io && *guard_io) {
        (void)pmm_daemon_ipc_participant_guard_release(guard_io);
        if (!*guard_io) {
            return true;
        }
...
static _Noreturn void runtime_cleanup_fail_stop(const char *component) {
    ...
    pmm_log_error("daemon.forced_shutdown", "component", component);
    (void)fflush(stdout);
    (void)fflush(stderr);
#ifdef _WIN32
...
static void runtime_startup_lock_release_complete(pmm_daemon_ipc_startup_lock_t **lock_io,
                                                  uint32_t timeout_ms) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
...
static void runtime_service_destroy_unstarted(pmm_daemon_runtime_service_t *service) {
    if (!service) {
        return;
    }
    pmm_daemon_ipc_listener_close(service->listener);
    pmm_daemon_coordinator_free(service->coordinator);
    ...
    for (size_t i = 0; i < service->worker_mutexes_initialized; i++) {
        pmm_mutex_destroy(&service->workers[i].send_mutex);
    }
    free(service->workers);
    pmm_mutex_destroy(&service->mutex);
    free(service);
}
...
pmm_daemon_runtime_service_t *pmm_daemon_runtime_service_start_reserved(
    ...
    uint8_t validation[PMM_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    char active_process_fingerprint[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];
    runtime_process_image_reference_t active_image;
    ...
    if (!reservation_io || !*reservation_io || !config || !config->endpoint ||
        ...
        config->request_timeout_ms == PMM_DAEMON_IPC_WAIT_FOREVER ||
        config->shutdown_timeout_ms == 0 ||
        config->shutdown_timeout_ms == PMM_DAEMON_IPC_WAIT_FOREVER ||
        !runtime_application_callbacks_valid(&config->application) ||
        !pmm_daemon_runtime_hello_request_encode(validation, &config->identity)) {
        pmm_log_error("daemon.runtime.start_failed", "stage", "config_validation");
        return NULL;
    }
    ...
    if (!runtime_process_image_reference_acquire(runtime_current_process_id(), &active_image,
        ...
        pmm_log_error("daemon.runtime.start_failed", "stage", "active_image_identity");
        (void)runtime_process_image_reference_release(&active_image);
        return NULL;
    }

    pmm_daemon_runtime_service_t *service = calloc(1, sizeof(*service));
    if (!service) {
        pmm_log_error("daemon.runtime.start_failed", "stage", "service_allocation");
        (void)runtime_process_image_reference_release(&active_image);
        return NULL;
    }
    service->active_image = active_image;
    runtime_process_image_reference_init(&active_image);
    pmm_mutex_init(&service->mutex);
    ...
    service->coordinator = pmm_daemon_coordinator_new(config->lease_timeout_ms);
    if (service->coordinator && config->permanent) {
        pmm_daemon_coordinator_set_permanent(service->coordinator, true);
    }
    ...
    service->state = PMM_DAEMON_RUNTIME_SERVICE_STARTING;

    if (!service->workers || !service->coordinator || !service->conflict_log_path ||
        !copied_identity) {
        pmm_log_error("daemon.runtime.start_failed", "stage", "service_initialization");
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
    for (size_t i = 0; i < service->worker_capacity; i++) {
        service->workers[i].service = service;
        pmm_mutex_init(&service->workers[i].send_mutex);
    ...
    service->listener = pmm_daemon_ipc_listen_reserved(config->endpoint, reservation_io);
    if (!service->listener) {
        pmm_log_error("daemon.runtime.start_failed", "stage", "listener_handoff");
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
    service->state = PMM_DAEMON_RUNTIME_SERVICE_RUNNING;
    if (pmm_thread_create(&service->accept_thread, RUNTIME_WORKER_STACK_SIZE, runtime_accept_loop,
                          service) != 0) {
        pmm_log_error("daemon.runtime.start_failed", "stage", "accept_thread");
        service->state = PMM_DAEMON_RUNTIME_SERVICE_EXITED;
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
...
pmm_daemon_runtime_service_t *pmm_daemon_runtime_service_start(
    ...
    pmm_daemon_ipc_startup_lock_t *startup_lock = NULL;
    if (pmm_daemon_ipc_startup_lock_try_acquire(config->endpoint, &startup_lock) != 1) {
        return NULL;
    }
    int generation =
        pmm_daemon_ipc_generation_probe_under_startup_lock(config->endpoint, startup_lock);
    if (generation != 0 || !pmm_daemon_ipc_startup_lock_prepare_handoff(startup_lock)) {
        runtime_startup_lock_release_complete(&startup_lock, config->shutdown_timeout_ms);
        return NULL;
    }
    pmm_daemon_ipc_participant_guard_t *participant_guard = NULL;
    if (pmm_daemon_ipc_participant_guard_try_join(config->endpoint, &participant_guard) != 1) {
        runtime_startup_lock_release_complete(&startup_lock, config->shutdown_timeout_ms);
        return NULL;
    }
    pmm_daemon_ipc_lifetime_reservation_t *reservation = NULL;
    if (pmm_daemon_ipc_lifetime_reservation_try_acquire(config->endpoint, &reservation) != 1) {
    ...
    pmm_daemon_runtime_service_t *service =
        pmm_daemon_runtime_service_start_reserved(config, &reservation);
...
pmm_daemon_runtime_service_state_t pmm_daemon_runtime_service_state(
    pmm_daemon_runtime_service_t *service) {
    if (!service) {
        return PMM_DAEMON_RUNTIME_SERVICE_EXITED;
    }
    pmm_mutex_lock(&service->mutex);
    pmm_daemon_runtime_service_state_t state = service->state;
    pmm_mutex_unlock(&service->mutex);
...
size_t pmm_daemon_runtime_service_job_subscribers(pmm_daemon_runtime_service_t *service,
    ...
    return service ? pmm_daemon_job_subscribers(service->coordinator, project_key) : 0;
}

uint64_t pmm_daemon_runtime_service_client_process_id(pmm_daemon_runtime_service_t *service,
                                                      pmm_daemon_client_id_t client_id) {
    if (!service || client_id == PMM_DAEMON_CLIENT_ID_INVALID) {
        return 0;
    }
...
static bool runtime_wait_for_count(pmm_daemon_runtime_service_t *service, size_t expected,
    ...
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    for (;;) {
        size_t actual = connections ? pmm_daemon_runtime_service_active_connections(service)
                                    : pmm_daemon_runtime_service_active_clients(service);
        if (actual == expected) {
            return true;
        }
        if (pmm_now_ms() >= deadline) {
...
bool pmm_daemon_runtime_service_wait_exited(pmm_daemon_runtime_service_t *service,
    ...
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    for (;;) {
        if (pmm_daemon_runtime_service_state(service) == PMM_DAEMON_RUNTIME_SERVICE_EXITED &&
...
bool pmm_daemon_runtime_service_job_reaped(pmm_daemon_runtime_service_t *service,
                                           const char *project_key) {
    return service && pmm_daemon_job_reaped(service->coordinator, project_key, pmm_now_ms());
}
...
bool pmm_daemon_runtime_service_stop(pmm_daemon_runtime_service_t *service, uint32_t timeout_ms) {
    ...
    if (pmm_daemon_runtime_service_state(service) == PMM_DAEMON_RUNTIME_SERVICE_EXITED) {
        return pmm_daemon_runtime_service_wait_exited(service, timeout_ms);
    }
...
bool pmm_daemon_runtime_service_free(pmm_daemon_runtime_service_t *service) {
    if (!service ||
        pmm_daemon_runtime_service_state(service) != PMM_DAEMON_RUNTIME_SERVICE_EXITED) {
        return false;
    }
    if (service->accept_thread_started && !service->accept_thread_joined) {
        if (pmm_thread_join(&service->accept_thread) != 0) {
            return false;
        }
    ...
    for (size_t i = 0; i < service->worker_capacity; i++) {
        ...
        if (worker->thread_started) {
            if (!atomic_load_explicit(&worker->done, memory_order_acquire) ||
                pmm_thread_join(&worker->thread) != 0) {
                return false;
            }
            runtime_worker_reset_after_join(worker);
        }
...
bool pmm_daemon_runtime_request_activation_shutdown(
    ...
    uint8_t request[PMM_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE];
    if (!endpoint || !identity || !result_out || timeout_ms == PMM_DAEMON_IPC_WAIT_FOREVER ||
        !runtime_activation_request_encode(request, action, identity)) {
        return false;
    }
    pmm_daemon_ipc_connection_t *connection = pmm_daemon_ipc_connect(endpoint, timeout_ms);
    ...
    pmm_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received = sent ? pmm_daemon_ipc_receive_frame_bounded(
                              connection, timeout_ms, PMM_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE,
                              &frame, &payload)
                        : 0;
    bool valid = received == 1 && frame.type == PMM_DAEMON_FRAME_RESPONSE &&
                 frame.flags == PMM_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN &&
                 frame.length == PMM_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE &&
...
static bool runtime_control_request_send(const pmm_daemon_ipc_endpoint_t *endpoint,
                                         const pmm_daemon_build_identity_t *identity,
                                         pmm_daemon_runtime_operation_t operation,
                                         uint32_t timeout_ms, uint32_t response_size,
                                         uint8_t **payload_out) {
    *payload_out = NULL;
    if (!endpoint || !identity || !identity->build_fingerprint ||
        timeout_ms == PMM_DAEMON_IPC_WAIT_FOREVER) {
        return false;
    }
    uint8_t request[PMM_DAEMON_CONTROL_REQUEST_SIZE];
    memset(request, 0, sizeof(request));
    int written = snprintf((char *)request, sizeof(request), "%s", identity->build_fingerprint);
    if (written <= 0 || (size_t)written != PMM_DAEMON_BUILD_FINGERPRINT_SIZE - 1U) {
        return false;
    }
    pmm_daemon_ipc_connection_t *connection = pmm_daemon_ipc_connect(endpoint, timeout_ms);
    bool sent =
        connection && pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST, operation,
                                                request, (uint32_t)sizeof(request));
    pmm_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received = sent ? pmm_daemon_ipc_receive_frame_bounded(connection, timeout_ms,
...
bool pmm_daemon_runtime_request_status(const pmm_daemon_ipc_endpoint_t *endpoint,
    ...
    uint8_t *payload = NULL;
    if (!runtime_control_request_send(endpoint, identity, PMM_DAEMON_RUNTIME_OP_STATUS, timeout_ms,
                                      PMM_DAEMON_STATUS_RESPONSE_SIZE, &payload)) {
        return false;
    }
    ...
    status_out->client_count = payload[8] > PMM_DAEMON_CONTROL_CLIENT_CAP
                                   ? (uint8_t)PMM_DAEMON_CONTROL_CLIENT_CAP
                                   : payload[8];
    for (size_t index = 0; index < PMM_DAEMON_CONTROL_CLIENT_CAP; index++) {
        status_out->client_pids[index] = runtime_control_read_u32(payload + 12U + index * 4U);
    }
    memcpy(status_out->build_fingerprint, payload + 44U, PMM_DAEMON_BUILD_FINGERPRINT_SIZE);
    status_out->build_fingerprint[PMM_DAEMON_BUILD_FINGERPRINT_SIZE - 1U] = '\0';
...
bool pmm_daemon_runtime_request_stop(const pmm_daemon_ipc_endpoint_t *endpoint,
    ...
    memset(result_out, 0, sizeof(*result_out));
    uint8_t *payload = NULL;
    if (!runtime_control_request_send(endpoint, identity, PMM_DAEMON_RUNTIME_OP_STOP, timeout_ms,
                                      PMM_DAEMON_STOP_RESPONSE_SIZE, &payload)) {
        return false;
    }
    ...
    result_out->client_count = payload[4] > PMM_DAEMON_CONTROL_CLIENT_CAP
                                   ? (uint8_t)PMM_DAEMON_CONTROL_CLIENT_CAP
                                   : payload[4];
    for (size_t index = 0; index < PMM_DAEMON_CONTROL_CLIENT_CAP; index++) {
...
pmm_daemon_runtime_client_t *pmm_daemon_runtime_client_connect(
    ...
    uint8_t request[PMM_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    if (!endpoint || !identity || !result_out || timeout_ms == PMM_DAEMON_IPC_WAIT_FOREVER ||
        !pmm_daemon_runtime_hello_request_encode(request, identity)) {
        return NULL;
    }
    pmm_daemon_ipc_connection_t *connection = pmm_daemon_ipc_connect(endpoint, timeout_ms);
    if (!connection || !pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST,
                                                  PMM_DAEMON_RUNTIME_OP_HELLO, request,
    ...
    uint8_t *payload = NULL;
    int received = pmm_daemon_ipc_receive_frame_bounded(
        connection, timeout_ms, PMM_DAEMON_RENDEZVOUS_RESPONSE_SIZE, &frame, &payload);
    bool valid = received == 1 && frame.type == PMM_DAEMON_FRAME_RESPONSE &&
                 frame.flags == PMM_DAEMON_RUNTIME_OP_HELLO &&
                 frame.length == PMM_DAEMON_RENDEZVOUS_RESPONSE_SIZE &&
                 runtime_hello_response_decode(payload, result_out);
    free(payload);
    if (!valid || result_out->status != PMM_DAEMON_RUNTIME_CONNECT_ACCEPTED) {
        pmm_daemon_ipc_connection_close(connection);
        return NULL;
    }

    pmm_daemon_runtime_client_t *client = calloc(1, sizeof(*client));
    if (!client) {
        pmm_daemon_ipc_connection_close(connection);
        memset(result_out, 0, sizeof(*result_out));
        return NULL;
    }
    pmm_mutex_init(&client->exchange_mutex);
    pmm_mutex_init(&client->send_mutex);
    pmm_mutex_init(&client->state_mutex);
...
pmm_daemon_client_id_t pmm_daemon_runtime_client_id(const pmm_daemon_runtime_client_t *client) {
    return client ? client->client_id : PMM_DAEMON_CLIENT_ID_INVALID;
}
...
static bool runtime_client_exchange(pmm_daemon_runtime_client_t *client,
                                    pmm_daemon_runtime_operation_t operation, const void *request,
                                    uint32_t request_length, uint32_t timeout_ms,
                                    uint8_t **response_out, uint32_t expected_response_length) {
    *response_out = NULL;
    if (timeout_ms == PMM_DAEMON_IPC_WAIT_FOREVER) {
        return false;
    }
    pmm_mutex_lock(&client->exchange_mutex);
    pmm_mutex_lock(&client->state_mutex);
    if (!client->usable || client->closing || !client->connection) {
        pmm_mutex_unlock(&client->state_mutex);
        pmm_mutex_unlock(&client->exchange_mutex);
        return false;
    }
    pmm_daemon_ipc_connection_t *connection = client->connection;
    client->exchange_active = true;
    pmm_mutex_unlock(&client->state_mutex);

    pmm_mutex_lock(&client->send_mutex);
    bool sent = pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST, (uint16_t)operation,
                                          request, request_length);
    pmm_mutex_unlock(&client->send_mutex);
    pmm_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received =
        sent ? pmm_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &payload) : -1;
    bool valid = sent && received == 1 && frame.type == PMM_DAEMON_FRAME_RESPONSE &&
...
pmm_daemon_subscription_result_t pmm_daemon_runtime_client_job_subscribe(
    ...
    if (subscription_id_out) {
        *subscription_id_out = PMM_DAEMON_SUBSCRIPTION_ID_INVALID;
    }
    size_t key_length = 0;
    if (!client || !subscription_id_out ||
        !runtime_bounded_length(project_key, PMM_DAEMON_RUNTIME_PROJECT_KEY_MAX + 1, &key_length) ||
        key_length == 0 || key_length > PMM_DAEMON_RUNTIME_PROJECT_KEY_MAX) {
        return PMM_DAEMON_SUBSCRIPTION_REJECTED;
    }
    uint8_t request[SUBSCRIBE_REQUEST_PREFIX_SIZE + PMM_DAEMON_RUNTIME_PROJECT_KEY_MAX];
    runtime_put_u32(request, (uint32_t)key_length);
    memcpy(request + SUBSCRIBE_REQUEST_PREFIX_SIZE, project_key, key_length);
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, PMM_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE, request,
                                 (uint32_t)(SUBSCRIBE_REQUEST_PREFIX_SIZE + key_length), timeout_ms,
                                 &response, SUBSCRIBE_RESPONSE_SIZE)) {
        return PMM_DAEMON_SUBSCRIPTION_REJECTED;
    }
    pmm_daemon_subscription_result_t result =
        (pmm_daemon_subscription_result_t)runtime_get_u32(response);
    pmm_daemon_subscription_id_t subscription_id = runtime_get_u64(response + 4);
    free(response);
    if ((result != PMM_DAEMON_SUBSCRIPTION_STARTED && result != PMM_DAEMON_SUBSCRIPTION_JOINED) ||
...
bool pmm_daemon_runtime_client_job_unsubscribe(pmm_daemon_runtime_client_t *client,
                                               ...
                                               uint32_t timeout_ms) {
    if (!client || subscription_id == PMM_DAEMON_SUBSCRIPTION_ID_INVALID) {
        return false;
    }
    uint8_t request[UNSUBSCRIBE_REQUEST_SIZE];
    runtime_put_u64(request, subscription_id);
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, PMM_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE, request,
...
bool pmm_daemon_runtime_client_heartbeat(pmm_daemon_runtime_client_t *client, uint32_t timeout_ms) {
    ...
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, PMM_DAEMON_RUNTIME_OP_HEARTBEAT, NULL, 0, timeout_ms,
...
bool pmm_daemon_runtime_client_application_token_reserve(
    ...
    if (token_out) {
        *token_out = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    if (!client || !token_out) {
        return false;
    }
    ...
    bool valid = client->usable && !client->closing && client->connection &&
                 client->active_application_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
...
pmm_daemon_runtime_cancel_result_t pmm_daemon_runtime_client_application_cancel(
    ...
    if (!client || request_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID) {
        return PMM_DAEMON_RUNTIME_CANCEL_ERROR;
    }
    ...
    if (client->active_application_token == request_token) {
    ...
    } else if (client->active_application_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
        ...
        pmm_mutex_unlock(&client->state_mutex);
        return PMM_DAEMON_RUNTIME_CANCEL_STALE;
    }
    pmm_mutex_unlock(&client->state_mutex);
    if (!dispatch) {
        return PMM_DAEMON_RUNTIME_CANCEL_ACCEPTED;
    }
    ...
    pmm_mutex_unlock(&client->state_mutex);
    bool sent = !dispatch || pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST,
                                                       PMM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL,
    ...
    if (!dispatch) {
        ...
        return already_sent ? PMM_DAEMON_RUNTIME_CANCEL_ACCEPTED
                            : (unavailable ? PMM_DAEMON_RUNTIME_CANCEL_ERROR
                                           : PMM_DAEMON_RUNTIME_CANCEL_STALE);
    }
    if (!sent) {
        pmm_mutex_lock(&client->state_mutex);
        client->usable = false;
        pmm_mutex_unlock(&client->state_mutex);
        pmm_daemon_ipc_connection_interrupt(connection);
        return PMM_DAEMON_RUNTIME_CANCEL_ERROR;
    }
    return PMM_DAEMON_RUNTIME_CANCEL_ACCEPTED;
}
...
pmm_daemon_runtime_application_status_t pmm_daemon_runtime_client_application_request_tagged(
    ...
    if (!client || !response_out || !response_length_out ||
        request_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
        timeout_ms == PMM_DAEMON_IPC_WAIT_FOREVER ||
        request_length > PMM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
    ...
    if (client->pending_cancel_token != request_token) {
        client->pending_cancel_token = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    pmm_mutex_unlock(&client->state_mutex);
    ...
    pmm_mutex_unlock(&client->state_mutex);
    bool sent =
        request_can_send && pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST,
                                                      PMM_DAEMON_RUNTIME_OP_APPLICATION_REQUEST,
    ...
    pmm_mutex_unlock(&client->state_mutex);
    if (send_cancel) {
        sent = pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST,
                                         PMM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL, cancel_wire,
    ...
    int received =
        sent ? pmm_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &payload) : -1;
    bool protocol_valid = sent && received == 1 && frame.type == PMM_DAEMON_FRAME_RESPONSE &&
                          frame.flags == PMM_DAEMON_RUNTIME_OP_APPLICATION_REQUEST &&
    ...
    uint32_t response_length = 0;
    if (protocol_valid) {
        pmm_daemon_runtime_application_token_t response_token = runtime_get_u64(payload);
        status = (pmm_daemon_runtime_application_status_t)runtime_get_u32(payload + 8);
        ...
        protocol_valid = response_token == request_token &&
                         ...
                         (status == PMM_DAEMON_RUNTIME_APPLICATION_OK || response_length == 0);
    }
...
pmm_daemon_runtime_application_status_t pmm_daemon_runtime_client_application_request(
    ...
    if (!pmm_daemon_runtime_client_application_token_reserve(client, &request_token)) {
        return PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    }
    return pmm_daemon_runtime_client_application_request_tagged(client, request_token, request,
...
bool pmm_daemon_runtime_client_close_begin(pmm_daemon_runtime_client_t *client) {
    ...
    pmm_daemon_ipc_connection_t *connection = client->connection;
    pmm_daemon_runtime_application_token_t cancel_token = client->active_application_token;
    bool send_cancel = client->close_interrupted_exchange && connection &&
                       client->application_request_sent && !client->application_cancel_sent &&
                       cancel_token != PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (send_cancel) {
        client->application_cancel_sent = true;
    }
    pmm_mutex_unlock(&client->state_mutex);
    ...
    if (send_cancel) {
        ...
        (void)pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST,
    ...
    if (client->close_interrupted_exchange && connection) {
        ...
        (void)pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST,
                                        PMM_DAEMON_RUNTIME_OP_CLOSE_INTENT, NULL, 0);
        pmm_daemon_ipc_connection_interrupt(connection);
    }
    pmm_mutex_unlock(&client->send_mutex);
    return true;
...
bool pmm_daemon_runtime_client_close_finish(pmm_daemon_runtime_client_t *client,
    ...
    pmm_mutex_lock(&client->state_mutex);
    bool can_disconnect = timeout_ms != PMM_DAEMON_IPC_WAIT_FOREVER &&
    ...
    if (can_disconnect) {
        pmm_mutex_lock(&client->send_mutex);
        disconnect_sent = pmm_daemon_ipc_send_frame(connection, PMM_DAEMON_FRAME_REQUEST,
                                                    PMM_DAEMON_RUNTIME_OP_DISCONNECT, NULL, 0);
        pmm_mutex_unlock(&client->send_mutex);
    }
    if (disconnect_sent) {
        pmm_daemon_frame_t frame = {0};
        uint8_t *response = NULL;
        int received = pmm_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &response);
        acknowledged = received == 1 && frame.type == PMM_DAEMON_FRAME_RESPONSE &&
                       frame.flags == PMM_DAEMON_RUNTIME_OP_DISCONNECT &&
    ...
    pmm_daemon_ipc_connection_close(connection);
    pmm_mutex_unlock(&client->exchange_mutex);
    pmm_mutex_destroy(&client->state_mutex);
    pmm_mutex_destroy(&client->send_mutex);
    pmm_mutex_destroy(&client->exchange_mutex);
    free(client);
    return acknowledged;
}

bool pmm_daemon_runtime_client_close(pmm_daemon_runtime_client_t *client, uint32_t timeout_ms) {
    if (!pmm_daemon_runtime_client_close_begin(client)) {
        return false;
    }
    return pmm_daemon_runtime_client_close_finish(client, timeout_ms);
}
