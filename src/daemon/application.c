/*
 * application.c — Daemon-owned PMM application sessions and thin-client wire.
 */
...
typedef struct pmm_daemon_application_watch pmm_daemon_application_watch_t;
typedef struct pmm_daemon_application_session pmm_daemon_application_session_t;
typedef struct pmm_daemon_application_job pmm_daemon_application_job_t;
typedef struct pmm_daemon_application_mutation pmm_daemon_application_mutation_t;
typedef struct pmm_daemon_application_watch_job_subscription
    pmm_daemon_application_watch_job_subscription_t;
...
struct pmm_daemon_application_watch {
    char *project;
    char *root;
    size_t subscribers;
    pmm_daemon_application_watch_t *next;
};

struct pmm_daemon_application_session {
    pmm_daemon_application_t *application;
    pmm_mcp_server_t *mcp;
    ...
    bool request_active;
    pmm_daemon_runtime_application_token_t active_request_token;
    pmm_daemon_runtime_application_token_t request_cancel_token;
    pmm_daemon_application_watch_t *watch;
    pmm_daemon_application_job_t *active_job;
    bool active_job_subscribed;
    pmm_daemon_application_job_t *auto_index_job;
    ...
    pmm_daemon_application_session_t *next;
};

struct pmm_daemon_application_job {
    pmm_daemon_application_t *application;
    char *project_key;
    char *root_path;
    char *args_json;
    char *response;
    pmm_daemon_application_worker_t worker;
    pmm_thread_t thread;
    size_t subscribers;
    ...
    pmm_daemon_application_job_t *next;
};
...
struct pmm_daemon_application_watch_job_subscription {
    pmm_daemon_application_session_t *session;
    pmm_daemon_application_job_t *job;
    pmm_daemon_application_watch_job_subscription_t *next;
};

struct pmm_daemon_application_mutation {
    char *project_key;
    pmm_project_lock_lease_t *project_lock_lease;
    bool releasing;
    pmm_daemon_application_mutation_t *next;
};

struct pmm_daemon_application {
    pmm_mutex_t mutex;
    struct pmm_watcher *watcher;
    struct pmm_config *config;
    pmm_daemon_application_session_t *sessions;
    pmm_daemon_application_watch_t *watches;
    pmm_daemon_application_job_t *jobs;
    pmm_daemon_application_watch_job_subscription_t *watch_job_subscriptions;
    pmm_daemon_application_mutation_t *mutations;
    pmm_daemon_application_worker_ops_t worker_ops;
    pmm_daemon_application_update_ops_t update_ops;
    ...
    pmm_daemon_application_update_worker_t update_worker;
    pmm_thread_t update_thread;
    ...
    bool stopping;
    /* See pmm_daemon_application_set_permanent. */
    bool permanent;
};

static void application_job_unsubscribe_locked(pmm_daemon_application_job_t *job);
...
static bool application_watch_job_subscribe_late_session_locked(
    pmm_daemon_application_session_t *session, pmm_daemon_application_watch_t *watch);
static bool application_unique_recovery_file(char out[APPLICATION_PATH_CAP], const char *kind);
static bool application_update_reap(pmm_daemon_application_t *application, bool wait,
                                    uint32_t timeout_ms);
static void *application_job_thread(void *opaque);
static char *application_auto_index_args(const char *root_path);
static pmm_daemon_application_job_t *application_job_subscribe_locked(
    pmm_daemon_application_t *application, const char *project_key, const char *root_path,
...
void pmm_daemon_application_fail_next_job_thread_start_for_test(void) {
...
int pmm_daemon_application_background_initializes_for_test(void) {
...
int pmm_daemon_application_busy_queue_waits_for_test(void) {
    return atomic_load_explicit(&g_application_busy_queue_waits_for_test, memory_order_acquire);
}

void pmm_daemon_application_hold_job_before_start_for_test(bool hold) {
    atomic_store_explicit(&g_application_hold_job_before_start_for_test, hold,
                          memory_order_release);
}

static int application_job_thread_create(pmm_thread_t *thread, void *context) {
    if (atomic_exchange_explicit(&g_application_fail_next_job_thread_start_for_test, false,
                                 memory_order_acq_rel)) {
        return -1;
    }
    return pmm_thread_create(thread, APPLICATION_JOB_THREAD_STACK, application_job_thread, context);
}

static bool application_request_cancelled_locked(const pmm_daemon_application_session_t *session) {
    return session &&
           (session->session_cancelled ||
            (session->request_active &&
             session->active_request_token != PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
...
static void application_project_lock_release_fully(pmm_project_lock_lease_t **lease) {
...
static int application_worker_start_default(void *context, const char *args_json,
    ...
    (void)context;
    pmm_index_worker_handle_t *worker = NULL;
    int result = pmm_index_worker_start(args_json, memory_budget_bytes, false, marker_file,
                                        quarantine_file, &worker);
    *worker_out = worker;
    return result;
}

static pmm_index_worker_poll_t application_worker_poll_default(
    void *context, pmm_daemon_application_worker_t worker,
    const pmm_index_worker_result_t **result_out) {
    (void)context;
    return pmm_index_worker_poll((pmm_index_worker_handle_t *)worker, result_out);
}

static bool application_worker_cancel_default(void *context,
                                              pmm_daemon_application_worker_t worker) {
    (void)context;
    return pmm_index_worker_request_cancel((pmm_index_worker_handle_t *)worker);
}

static const char *application_worker_log_path_default(void *context,
                                                       pmm_daemon_application_worker_t worker) {
    (void)context;
    return pmm_index_worker_log_path((pmm_index_worker_handle_t *)worker);
}

static void application_worker_destroy_default(void *context,
                                               pmm_daemon_application_worker_t worker) {
    (void)context;
    pmm_index_worker_destroy((pmm_index_worker_handle_t *)worker);
}
...
static pmm_daemon_application_watch_t *application_find_watch_locked(
    pmm_daemon_application_t *application, const char *project) {
    for (pmm_daemon_application_watch_t *watch = application->watches; watch; watch = watch->next) {
...
static void application_remove_watch_entry_locked(pmm_daemon_application_t *application,
                                                  pmm_daemon_application_watch_t *watch,
                                                  bool unregister_physical_watch) {
    pmm_daemon_application_watch_t **cursor = &application->watches;
    ...
    *cursor = watch->next;
    for (pmm_daemon_application_session_t *session = application->sessions; session;
    ...
    if (unregister_physical_watch && application->watcher) {
        pmm_watcher_unwatch(application->watcher, watch->project);
    }
    free(watch->project);
    free(watch->root);
    free(watch);
...
static void application_remove_watch_locked(pmm_daemon_application_t *application,
                                            pmm_daemon_application_watch_t *watch) {
    application_remove_watch_entry_locked(application, watch, true);
}

static void application_release_session_watch_locked(pmm_daemon_application_session_t *session) {
    pmm_daemon_application_watch_t *watch = session->watch;
    if (!watch) {
        return;
    }
    application_watch_job_unsubscribe_session_locked(session);
...
static void application_refresh_watch_locked(pmm_daemon_application_session_t *session) {
    pmm_daemon_application_t *application = session->application;
    if (!application->watcher || !session->context_set ||
        session->tool_profile != PMM_MCP_TOOL_PROFILE_ALL || session->hook_event ||
        session->hook_dialect || session->auto_index_subscribed) {
        return;
    }
    const char *project = pmm_mcp_server_session_project(session->mcp);
    const char *root = pmm_mcp_server_session_root(session->mcp);
    if (!project || !project[0] || !root || !root[0]) {
        return;
    }
    bool enabled = !application->config ||
                   pmm_config_get_bool(application->config, PMM_CONFIG_AUTO_WATCH, true);
    ...
    pmm_daemon_application_watch_t *watch = application_find_watch_locked(application, project);
    ...
    if (watch) {
        if (strcmp(watch->root, root) == 0) {
            if (!application_watch_job_subscribe_late_session_locked(session, watch)) {
                pmm_log_warn("daemon.watch.late_owner_allocation_failed", "project", project,
                             "action", "retry");
                return;
            }
            watch->subscribers++;
            ...
            pmm_log_warn("daemon.watch.project_collision", "project", project, "existing_root",
    ...
    if (!pmm_watcher_watch(application->watcher, project, root)) {
        pmm_log_warn("daemon.watch.registration_failed", "project", project, "action", "retry");
        free(watch->project);
        free(watch->root);
        free(watch);
        return;
...
static void application_refresh_watch(pmm_daemon_application_session_t *session) {
    if (!session || !session->application) {
        return;
    }
    pmm_mutex_lock(&session->application->mutex);
    application_refresh_watch_locked(session);
    pmm_mutex_unlock(&session->application->mutex);
}

static void application_job_free(pmm_daemon_application_job_t *job) {
    if (!job) {
        return;
    }
...
static void application_jobs_reap_completed(pmm_daemon_application_t *application) {
    for (;;) {
        pmm_daemon_application_job_t *reap = NULL;
        pmm_mutex_lock(&application->mutex);
        pmm_daemon_application_job_t **cursor = &application->jobs;
        while (*cursor) {
        ...
        pmm_mutex_unlock(&application->mutex);
        if (!reap) {
            return;
        }
        if (reap->thread_started) {
            (void)pmm_thread_join(&reap->thread);
        }
...
static bool application_cache_dir(char out[APPLICATION_PATH_CAP]) {
    ...
    if (pmm_safe_getenv("PMM_CACHE_DIR", configured, sizeof(configured), NULL) && configured[0]) {
    ...
    if (!pmm_safe_getenv("HOME", home, sizeof(home), NULL) || !home[0]) {
        (void)pmm_safe_getenv("USERPROFILE", home, sizeof(home), NULL);
    }
    if (!home[0]) {
        return false;
    }
    int written = snprintf(out, APPLICATION_PATH_CAP, "%s/.cache/project-mind-mcp", home);
    if (written <= 0 || written >= APPLICATION_PATH_CAP) {
        return false;
    }
    pmm_normalize_path_sep(out);
...
static void application_recovery_files_remove(const char *marker_path,
    ...
    if (quarantine_path && quarantine_path[0]) {
        (void)pmm_unlink(quarantine_path);
    }
}

static bool application_truncate_file(const char *path) {
    FILE *file = pmm_fopen(path, "wb");
    return file && fclose(file) == 0;
}

static bool application_job_cancel_requested(pmm_daemon_application_job_t *job);

static char **application_read_suspects(pmm_daemon_application_job_t *job, const char *path,
                                        int *count_out, bool *cancelled_out) {
    *count_out = 0;
    *cancelled_out = false;
    int64_t marker_size = pmm_file_size(path);
...
static int application_max_restarts(void) {
    char value[32] = {0};
    if (!pmm_safe_getenv("PMM_INDEX_MAX_RESTARTS", value, sizeof(value), NULL) || !value[0]) {
...
static void application_attempt_init(application_attempt_t *attempt) {
    ...
    attempt->result.outcome = PMM_PROC_SPAWN_FAILED;
...
static bool application_job_cancel_requested(pmm_daemon_application_job_t *job) {
    pmm_daemon_application_t *application = job->application;
    pmm_mutex_lock(&application->mutex);
...
static bool application_mutation_conflicts_locked(pmm_daemon_application_t *application,
                                                  const char *project_key) {
    for (pmm_daemon_application_mutation_t *mutation = application->mutations; mutation;
...
static bool application_job_reserves_project_locked(pmm_daemon_application_t *application,
                                                    const char *project_key) {
    for (pmm_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static bool application_mutation_begin_internal(pmm_daemon_application_t *application,
                                                pmm_daemon_application_session_t *session,
                                                const char *project_key, bool wait) {
    if (!application || !project_key || !project_key[0]) {
        return false;
    }
    pmm_daemon_application_mutation_t *reserved = NULL;
    for (;;) {
        pmm_mutex_lock(&application->mutex);
        bool cancelled =
        ...
        if (!cancelled && !busy) {
            pmm_daemon_application_mutation_t *mutation = calloc(1, sizeof(*mutation));
            if (mutation) {
                mutation->project_key = pmm_strdup(project_key);
            }
        ...
        pmm_usleep(APPLICATION_JOB_POLL_US);
    }
    ...
    for (;;) {
        ...
        pmm_private_file_lock_status_t status =
            wait ? pmm_project_lock_acquire(application->project_locks, project_key, deadline, NULL,
                                            &lease)
                 : pmm_project_lock_try_acquire(application->project_locks, project_key, &lease);
        if (status == PMM_PRIVATE_FILE_LOCK_OK && lease) {
            ...
            if (cancelled) {
                ...
                status = PMM_PRIVATE_FILE_LOCK_BUSY;
            } else {
        ...
        bool cancelled =
            application->stopping || (session && application_request_cancelled_locked(session));
        if (status != PMM_PRIVATE_FILE_LOCK_BUSY || cancelled || !wait) {
            pmm_daemon_application_mutation_t **cursor = &application->mutations;
            ...
            if (status != PMM_PRIVATE_FILE_LOCK_BUSY) {
                pmm_log_error("daemon.project_lock_failed", "project", project_key, "action",
                              "refuse_mutation");
            }
            return false;
        }
...
void pmm_daemon_application_project_mutation_end(pmm_daemon_application_t *application,
    ...
    pmm_mutex_lock(&application->mutex);
    pmm_daemon_application_mutation_t *mutation = application->mutations;
...
static bool application_watcher_mutation_begin(void *context, const char *project) {
    return pmm_daemon_application_project_mutation_try_begin(context, project);
}

static void application_watcher_mutation_end(void *context, const char *project) {
    pmm_daemon_application_project_mutation_end(context, project);
}

static void application_watcher_project_pruned(void *context, const char *project) {
    pmm_daemon_application_t *application = context;
...
static bool application_session_mutation_begin(void *context, const char *project) {
    pmm_daemon_application_session_t *session = context;
    return session &&
...
static void application_session_mutation_end(void *context, const char *project) {
    pmm_daemon_application_session_t *session = context;
    if (session) {
        pmm_daemon_application_project_mutation_end(session->application, project);
    }
}

static bool application_job_wait_for_mutations(pmm_daemon_application_job_t *job) {
    pmm_daemon_application_t *application = job->application;
    for (;;) {
        pmm_mutex_lock(&application->mutex);
        bool cancelled = job->cancel_requested || application->stopping;
...
static void application_cancelled_result(pmm_index_worker_result_t *result) {
    memset(result, 0, sizeof(*result));
    result->outcome = PMM_PROC_KILLED;
    result->exit_code = -1;
    result->cancellation_requested = true;
    result->tree_quiesced = true;
}

static application_attempt_status_t application_job_run_attempt(pmm_daemon_application_job_t *job,
    ...
    pmm_daemon_application_worker_t worker = NULL;
    application_tmp_lock();
    ...
    const pmm_index_worker_result_t *borrowed = NULL;
    for (;;) {
        pmm_index_worker_poll_t state =
            application->worker_ops.poll(application->worker_ops.context, worker, &borrowed);
        if (state == PMM_INDEX_WORKER_POLL_TERMINAL) {
            break;
        }
        pmm_mutex_lock(&application->mutex);
        bool cancel_pending =
        ...
        pmm_mutex_unlock(&application->mutex);
        if (cancel_pending || state == PMM_INDEX_WORKER_POLL_ERROR) {
            (void)application->worker_ops.cancel(application->worker_ops.context, worker);
        }
        pmm_usleep(APPLICATION_JOB_POLL_US);
    }
    ...
    if (borrowed) {
        ...
        attempt->result.response = borrowed->response ? pmm_strdup(borrowed->response) : NULL;
...
static char *application_job_failure_response(const pmm_index_worker_result_t *result,
                                              const char *log_path) {
    char message[1024];
    if (result && result->cancellation_requested) {
        ...
        (void)snprintf(message, sizeof(message),
                       ...
                       pmm_proc_outcome_str(result->outcome), log_path ? log_path : "unavailable");
    } else if (result) {
        (void)snprintf(message, sizeof(message),
                       "index worker ended with %s (exit=%d, signal=%d); inspect log: %s",
                       pmm_proc_outcome_str(result->outcome), result->exit_code,
...
static pmm_mcp_supervised_result_disposition_t application_attempt_disposition(
    pmm_daemon_application_job_t *job, application_attempt_t *attempt) {
    if (application_job_cancel_requested(job)) {
        if (!attempt->has_result) {
            application_cancelled_result(&attempt->result);
            attempt->has_result = true;
    ...
    return pmm_mcp_supervised_result_disposition(0, attempt->has_result ? &attempt->result : NULL);
}

static void application_record_attempt(const application_attempt_t *attempt,
                                       pmm_index_worker_result_t *last_result,
...
static void application_record_cancelled(pmm_index_worker_result_t *last_result,
                                         bool *have_last_result,
                                         char last_log[APPLICATION_PATH_CAP]) {
...
static bool application_result_is_attributable_failure(
    const application_attempt_t *attempt, pmm_mcp_supervised_result_disposition_t disposition) {
    return disposition == PMM_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE && attempt->has_result &&
           (attempt->result.outcome == PMM_PROC_CRASH || attempt->result.outcome == PMM_PROC_HANG);
}
...
static application_attempt_decision_t application_consume_attempt(
    pmm_daemon_application_job_t *job, application_attempt_t *attempt,
    application_job_execution_t *execution, pmm_proc_outcome_t *failure_outcome) {
...
static bool application_recovery_record_suspects(
    pmm_daemon_application_job_t *job, const char *marker_path, const char *quarantine_path,
    pmm_proc_outcome_t outcome, int recovery_index, char ***previous_suspects, int *previous_count,
    ...
    if (!suspects || suspect_count == 0) {
        application_free_suspects(suspects, suspect_count);
        pmm_log_warn("daemon.index.recovery_unattributable", "action", "stop");
        return false;
    }
    if (*previous_suspects) {
        ...
        const char *phase = outcome == PMM_PROC_HANG ? "hang" : "crash";
        if (!application_append_quarantine(quarantine_path, pick, phase)) {
            pmm_log_warn("daemon.index.quarantine_write_fail", "path", pick);
            application_free_suspects(suspects, suspect_count);
            return false;
        }
        (*quarantined)++;
        ...
        pmm_log_warn("daemon.index.file_quarantined", "path", pick, "outcome", phase, "attempt",
                     attempt_text);
    }
...
static void application_job_try_partial(pmm_daemon_application_job_t *job,
    ...
    pmm_proc_outcome_t failure_outcome = PMM_PROC_SPAWN_FAILED;
    ...
    if (decision == APPLICATION_ATTEMPT_DECISION_SUCCESS) {
        ...
        pmm_log_warn("daemon.index.recovery_partial", "quarantined", quarantined_text);
    }
}

static void application_job_recover(pmm_daemon_application_job_t *job,
                                    application_job_execution_t *execution) {
    if (application_job_cancel_requested(job)) {
        application_job_execution_cancel(execution);
        return;
...
static void application_auto_index_retry_pending_locked(pmm_daemon_application_t *application) {
    ...
    for (pmm_daemon_application_session_t *session = application->sessions; session;
        ...
        const char *project = pmm_mcp_server_session_project(session->mcp);
        const char *root_path = pmm_mcp_server_session_root(session->mcp);
        ...
        pmm_daemon_application_job_t *retry = application_job_subscribe_locked(
        ...
        if (retry) {
            ...
            session->auto_index_retry_pending = false;
            pmm_log_info("daemon.autoindex.admission_retried", "project", project);
            continue;
        }
...
static void application_job_publish(pmm_daemon_application_job_t *job,
                                    application_job_execution_t *execution) {
...
static void *application_job_thread(void *opaque) {
    pmm_daemon_application_job_t *job = opaque;
    application_job_execution_t execution;
...
static pmm_daemon_application_job_t *application_find_job_locked(
    pmm_daemon_application_t *application, const char *project_key) {
    for (pmm_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static pmm_daemon_application_job_t *application_find_active_job_locked(
    pmm_daemon_application_t *application, const char *project_key) {
    for (pmm_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static char *application_index_project_key(const char *root_path, const char *args_json) {
    char *override = pmm_mcp_get_string_arg(args_json, "name");
    char *key = pmm_project_name_from_path(override && override[0] ? override : root_path);
    free(override);
    return key;
}

static size_t application_active_job_count_locked(pmm_daemon_application_t *application) {
    size_t count = 0;
    for (pmm_daemon_application_job_t *job = application->jobs; job; job = job->next) {
        if (!job->terminal) {
            count++;
...
static pmm_daemon_application_job_t *application_job_subscribe_locked(
    ...
    *status_out = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
    ...
    if (application_active_job_count_locked(application) >= application->physical_job_limit) {
        ...
        pmm_log_warn("daemon.index.admission_busy", "limit", limit, "project", project_key);
    ...
    if (application_job_thread_create(&job->thread, job) == 0) {
        ...
        application_job_free(job);
        *status_out = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
        pmm_log_warn("daemon.index.thread_start_failed", "action", "retry");
        return NULL;
    }
    *status_out = APPLICATION_JOB_SUBSCRIBE_OK;
    return job;
...
static pmm_daemon_application_job_t *application_job_subscribe(
    pmm_daemon_application_t *application, const char *project_key, const char *root_path,
    const char *args_json, application_job_subscribe_status_t *status_out) {
    application_jobs_reap_completed(application);
    pmm_mutex_lock(&application->mutex);
    pmm_daemon_application_job_t *job = application_job_subscribe_locked(
...
static pmm_daemon_application_job_t *application_auto_index_release_locked(
    pmm_daemon_application_session_t *session) {
    if (!session || !session->auto_index_job || !session->auto_index_subscribed) {
        return NULL;
    }
    pmm_daemon_application_job_t *job = session->auto_index_job;
    session->auto_index_job = NULL;
    session->auto_index_subscribed = false;
...
static void application_auto_index_cancel_join(pmm_daemon_application_t *application,
                                               pmm_daemon_application_job_t *job) {
    if (!application || !job) {
        return;
    }
...
static void application_update_cancel_locked(pmm_daemon_application_t *application) {
...
static bool application_update_owner_release_locked(pmm_daemon_application_session_t *session) {
...
static void application_update_publish_terminal_locked(pmm_daemon_application_t *application,
                                                       const char *latest_version,
                                                       bool completed_generation) {
    if (!application->update_cancel_requested && application_update_version_valid(latest_version) &&
        pmm_compare_versions(latest_version, pmm_cli_get_version()) > 0) {
        (void)snprintf(application->update_notice, sizeof(application->update_notice),
                       "Update available: %s -> %s -- run: project-mind-mcp update  |  "
                       "Enjoying project-mind-mcp? Please leave a star: "
                       "https://github.com/usman8786/project-mind-mcp",
                       pmm_cli_get_version(), latest_version);
        pmm_log_info("update.available", "current", pmm_cli_get_version(), "latest",
                     latest_version);
    }
    for (pmm_daemon_application_session_t *session = application->sessions; session;
         session = session->next) {
...
static void *application_update_thread(void *opaque) {
    pmm_daemon_application_t *application = opaque;
    pmm_daemon_application_update_worker_t worker = NULL;
    ...
    for (;;) {
        pmm_daemon_application_update_poll_t status =
            application->update_ops.poll(application->update_ops.context, worker, &latest_version);
        if (status != PMM_DAEMON_APPLICATION_UPDATE_POLL_RUNNING) {
            if (status == PMM_DAEMON_APPLICATION_UPDATE_POLL_ERROR) {
                latest_version = NULL;
            }
            break;
        }
        ...
        pmm_mutex_unlock(&application->mutex);
        pmm_usleep(APPLICATION_UPDATE_POLL_US);
    }
...
static void application_update_subscribe_locked(pmm_daemon_application_session_t *session) {
    pmm_daemon_application_t *application = session->application;
    ...
    if (pmm_thread_create(&application->update_thread, APPLICATION_JOB_THREAD_STACK,
    ...
    application->update_thread_done = false;
    pmm_log_warn("daemon.update.thread_start_failed", "action", "retry");
}
...
static bool application_update_reap(pmm_daemon_application_t *application, bool wait,
    ...
    for (;;) {
        ...
        pmm_mutex_unlock(&application->mutex);
        if (join) {
            bool joined = pmm_thread_join(&application->update_thread) == 0;
            pmm_mutex_lock(&application->mutex);
            if (joined) {
...
static void application_background_initialize_impl(pmm_daemon_application_session_t *session) {
    ...
    bool auto_index = application->config &&
                      pmm_config_get_bool(application->config, PMM_CONFIG_AUTO_INDEX, false);
    int auto_index_limit =
        application->config ? pmm_config_get_int(application->config, PMM_CONFIG_AUTO_INDEX_LIMIT,
    ...
    bool within_auto_index_limit =
        ...
        pmm_mcp_auto_index_within_file_limit(root_path, auto_index_limit, &tracked_files);
    if (auto_index_candidate && !within_auto_index_limit) {
        char files[32];
        (void)snprintf(files, sizeof(files), "%d", tracked_files);
        pmm_log_warn("daemon.autoindex.skipped", "project", project, "reason",
    ...
    if (attempt_auto_index && args) {
        ...
        if (job) {
            ...
            pmm_log_warn("daemon.autoindex.admission_failed", "project", project, "action",
...
static void application_background_initialize(pmm_daemon_application_session_t *session) {
...
static void application_update_notice_inject(pmm_daemon_application_session_t *session,
                                             char **response_io) {
    pmm_daemon_application_t *application = session->application;
    pmm_mutex_lock(&application->mutex);
    if (session->background_eligible && !session->update_notice_delivered &&
        ...
        pmm_mcp_jsonrpc_response_prepend_notice(response_io, application->update_notice)) {
...
static bool application_watch_job_subscription_exists_locked(
    pmm_daemon_application_t *application, pmm_daemon_application_session_t *session,
    pmm_daemon_application_job_t *job) {
    for (pmm_daemon_application_watch_job_subscription_t *subscription =
...
static bool application_watch_job_subscribe_late_session_locked(
    ...
    pmm_daemon_application_watch_job_subscription_t *subscription =
...
static bool application_watch_job_subscribe_sessions_locked(pmm_daemon_application_t *application,
                                                            pmm_daemon_application_watch_t *watch,
                                                            pmm_daemon_application_job_t *job,
                                                            size_t *matched_out) {
    *matched_out = 0;
    ...
    pmm_daemon_application_watch_job_subscription_t *pending = NULL;
    for (pmm_daemon_application_session_t *session = application->sessions; session;
         session = session->next) {
        ...
        if (!subscription) {
            while (pending) {
                pmm_daemon_application_watch_job_subscription_t *next = pending->next;
                free(pending);
                pending = next;
            }
            return false;
    ...
    while (pending) {
        pmm_daemon_application_watch_job_subscription_t *subscription = pending;
...
static void application_watch_job_unsubscribe_session_locked(
    ...
    pmm_daemon_application_watch_job_subscription_t **cursor =
        &session->application->watch_job_subscriptions;
    while (*cursor) {
        pmm_daemon_application_watch_job_subscription_t *subscription = *cursor;
...
static void application_watch_job_unsubscribe_job_locked(pmm_daemon_application_t *application,
                                                         pmm_daemon_application_job_t *job) {
    pmm_daemon_application_watch_job_subscription_t **cursor =
        &application->watch_job_subscriptions;
    while (*cursor) {
...
static char *application_job_wait_for_session(pmm_daemon_application_session_t *session,
                                              pmm_daemon_application_job_t *job) {
    pmm_daemon_application_t *application = session->application;
    for (;;) {
        pmm_mutex_lock(&application->mutex);
        if (session->active_job != job || !session->active_job_subscribed) {
            pmm_mutex_unlock(&application->mutex);
            return pmm_mcp_text_result("index operation cancelled for this session", true);
        }
        if (job->terminal) {
            ...
            return response ? response
                            : pmm_mcp_text_result("index coordinator lost its result", true);
        }
        pmm_mutex_unlock(&application->mutex);
        pmm_usleep(APPLICATION_JOB_POLL_US);
    }
...
static char *application_index_execute(void *context, const char *root_path,
    ...
    if (!project_key) {
        return pmm_mcp_text_result("failed to derive index project identity", true);
    }
    application_job_subscribe_status_t subscribe_status = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
    pmm_daemon_application_job_t *job = NULL;
    for (;;) {
    ...
    if (session->active_job) {
        application_job_unsubscribe_locked(job);
        pmm_mutex_unlock(&session->application->mutex);
        return pmm_mcp_text_result("this session already has an active index operation", true);
    }
...
static pmm_daemon_runtime_application_session_t *application_session_open(
    void *context, pmm_daemon_client_id_t client_id, uint64_t authenticated_process_id) {
    pmm_daemon_application_t *application = context;
    if (!application || client_id == PMM_DAEMON_CLIENT_ID_INVALID ||
        authenticated_process_id == 0) {
        return NULL;
    }
    pmm_daemon_application_session_t *session = calloc(1, sizeof(*session));
    if (!session) {
        return NULL;
    }
    session->mcp = pmm_mcp_server_new(NULL);
    if (!session->mcp || !pmm_mcp_server_release_pristine_memory_store(session->mcp)) {
        pmm_mcp_server_free(session->mcp);
        free(session);
        return NULL;
    }
    pmm_mcp_server_set_background_tasks(session->mcp, false);
    pmm_mcp_server_set_config(session->mcp, application->config);
    pmm_mcp_server_set_index_executor(session->mcp, application_index_execute, session);
    pmm_mcp_server_set_project_mutation_guard(session->mcp, application_session_mutation_begin,
                                              application_session_mutation_end, session);
    session->tool_profile = PMM_MCP_TOOL_PROFILE_ALL;
...
static pmm_daemon_runtime_application_status_t application_set_context(
    pmm_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length) {
    if (session->context_set || request_length < APPLICATION_CONTEXT_HEADER_SIZE) {
        return PMM_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ...
    if (request[5] > 1 || root_length == 0 || expected != request_length ||
        ...
        profile_value > (uint8_t)PMM_MCP_TOOL_PROFILE_SCOUT ||
        (profile_value != (uint8_t)PMM_MCP_TOOL_PROFILE_ALL &&
         (event_length != 0 || dialect_length != 0))) {
        return PMM_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    pmm_mcp_tool_profile_t tool_profile = (pmm_mcp_tool_profile_t)profile_value;
    ...
    if (!root || (allowed_present && !allowed) || (event_length && !hook_event) ||
        ...
        !pmm_hook_augment_invocation_supported(hook_event, hook_dialect)) {
        free(root);
        free(allowed);
        free(hook_event);
        free(hook_dialect);
    ...
    bool canonical = pmm_canonical_path(root, canonical_root, sizeof(canonical_root));
    if (canonical && allowed_present) {
        canonical = pmm_canonical_path(allowed, canonical_allowed, sizeof(canonical_allowed));
    }
    struct stat root_status;
    canonical =
        canonical && stat(canonical_root, &root_status) == 0 && S_ISDIR(root_status.st_mode);
    bool set =
        canonical && pmm_mcp_server_set_session_context(session->mcp, canonical_root,
    ...
    pmm_mcp_server_set_tool_profile(session->mcp, tool_profile);
...
static pmm_daemon_runtime_application_status_t application_mcp_request(
    pmm_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length,
    ...
    pmm_jsonrpc_request_t parsed = {0};
    bool parsed_ok = pmm_jsonrpc_parse(message, &parsed) == 0;
    ...
    char *response = pmm_mcp_server_handle(session->mcp, message);
    free(message);
    ...
    if (parsed_ok) {
        pmm_jsonrpc_request_free(&parsed);
    }
    if (response) {
...
static pmm_daemon_runtime_application_status_t application_tool_request(
    pmm_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length,
    uint8_t **response_out, uint32_t *response_length_out) {
    ...
    char *response = pmm_mcp_handle_tool(session->mcp, tool, args);
    free(tool);
    free(args);
    if (!response) {
        return PMM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
...
static pmm_daemon_runtime_application_status_t application_set_ui_config(
    pmm_daemon_application_t *application, pmm_daemon_application_session_t *session,
    const uint8_t *request, uint32_t request_length) {
    const uint8_t valid_mask =
        PMM_DAEMON_APPLICATION_UI_CONFIG_ENABLED | PMM_DAEMON_APPLICATION_UI_CONFIG_PORT;
    if (!session || !session->context_set || session->tool_profile != PMM_MCP_TOOL_PROFILE_ALL ||
    ...
    bool enabled_present = (update_mask & PMM_DAEMON_APPLICATION_UI_CONFIG_ENABLED) != 0;
    bool port_present = (update_mask & PMM_DAEMON_APPLICATION_UI_CONFIG_PORT) != 0;
    ...
    if (application->stopping) {
        pmm_mutex_unlock(&application->mutex);
        return PMM_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE;
    }
    pmm_ui_config_t config;
    pmm_ui_config_load(&config);
    if (enabled_present) {
    ...
    bool saved = pmm_ui_config_save(&config);
    pmm_mutex_unlock(&application->mutex);
    return saved ? PMM_DAEMON_RUNTIME_APPLICATION_OK : PMM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
}

static pmm_daemon_runtime_application_status_t application_request_dispatch(
    pmm_daemon_application_t *application, pmm_daemon_application_session_t *session,
    const uint8_t *request, uint32_t request_length, uint8_t **response_out,
    uint32_t *response_length_out) {
    switch ((pmm_daemon_application_request_kind_t)request[0]) {
    case PMM_DAEMON_APPLICATION_REQUEST_SET_CONTEXT:
        return application_set_context(session, request, request_length);
    case PMM_DAEMON_APPLICATION_REQUEST_MCP:
        return application_mcp_request(session, request, request_length, response_out,
                                       response_length_out);
    case PMM_DAEMON_APPLICATION_REQUEST_TOOL:
        return application_tool_request(session, request, request_length, response_out,
                                        response_length_out);
    case PMM_DAEMON_APPLICATION_REQUEST_SET_UI_CONFIG:
        return application_set_ui_config(application, session, request, request_length);
    case PMM_DAEMON_APPLICATION_REQUEST_HOOK_AUGMENT: {
        if (!session->context_set || request_length <= 1) {
            return PMM_DAEMON_RUNTIME_APPLICATION_REJECTED;
        }
        ...
        char *response = pmm_hook_augment_process_for(session->mcp, input, session->hook_event,
                                                      session->hook_dialect);
        free(input);
        if (response) {
            size_t response_length = strlen(response);
...
static pmm_daemon_runtime_application_status_t application_request(
    void *context, pmm_daemon_runtime_application_session_t *opaque_session,
    ...
    pmm_daemon_application_t *application = context;
    pmm_daemon_application_session_t *session = (pmm_daemon_application_session_t *)opaque_session;
    if (response_out) {
        *response_out = NULL;
    }
    ...
    if (!application || !session || session->application != application ||
        request_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID || !request ||
    ...
    if (session->request_cancel_token != request_token) {
        session->request_cancel_token = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    session->request_active = true;
    session->active_request_token = request_token;
    bool mcp_scope_started = pmm_mcp_server_request_scope_begin(session->mcp);
    ...
    pmm_daemon_runtime_application_status_t status =
        cancelled_before_entry
            ? PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED
    ...
    pmm_mcp_server_request_scope_end(session->mcp);
    session->request_active = false;
    session->active_request_token = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (session->request_cancel_token == request_token) {
        session->request_cancel_token = PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    ...
    bool activate_background =
        !cancelled && status == PMM_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    if (!cancelled && status == PMM_DAEMON_RUNTIME_APPLICATION_OK &&
...
static void application_cancel_jobs_locked(pmm_daemon_application_t *application) {
    for (pmm_daemon_application_job_t *job = application->jobs; job; job = job->next) {
        if (!job->terminal) {
            job->cancel_requested = true;
        }
...
static void application_request_cancel(void *context,
                                       pmm_daemon_runtime_application_session_t *opaque_session,
    ...
    if (!session->session_cancelled &&
        ...
        if (active_match && session->active_job && session->active_job_subscribed) {
            pmm_daemon_application_job_t *job = session->active_job;
            session->active_job = NULL;
            session->active_job_subscribed = false;
            application_job_unsubscribe_locked(job);
        }
        if (active_match) {
            ...
            (void)pmm_mcp_server_cancel_active(session->mcp);
        }
    }
    pmm_mutex_unlock(&application->mutex);
}
...
static void application_session_cancel(void *context,
    ...
    if (application && session && session->application == application) {
        ...
        pmm_daemon_application_job_t *join_auto_index = NULL;
        pmm_mutex_lock(&application->mutex);
        ...
        for (pmm_daemon_application_session_t *other = application->sessions;
...
static void application_session_close(void *context,
    ...
    pmm_daemon_application_job_t *join_auto_index = NULL;
    pmm_mutex_lock(&application->mutex);
    pmm_daemon_application_session_t **cursor = &application->sessions;
...
pmm_daemon_application_t *pmm_daemon_application_new(
    const pmm_daemon_application_config_t *config) {
    pmm_daemon_application_t *application = calloc(1, sizeof(*application));
    if (!application) {
        return NULL;
    }
    pmm_mutex_init(&application->mutex);
    application->physical_job_limit = APPLICATION_DEFAULT_PHYSICAL_JOB_LIMIT;
    size_t aggregate_memory_budget_bytes = pmm_mem_budget();
    if (config) {
    ...
    if (!application->worker_ops.start) {
        application->worker_ops = (pmm_daemon_application_worker_ops_t){
            .context = NULL,
    ...
    if (!application->worker_ops.poll || !application->worker_ops.cancel ||
        ...
        pmm_mutex_destroy(&application->mutex);
        free(application);
        return NULL;
    }
    ...
    if (application->watcher) {
        pmm_watcher_set_project_mutation_guard(
...
bool pmm_daemon_application_shutdown(pmm_daemon_application_t *application, uint32_t timeout_ms) {
    ...
    for (;;) {
        ...
        for (pmm_daemon_application_session_t *session = application->sessions; all_done && session;
             session = session->next) {
            if (session->request_active) {
                all_done = false;
            }
...
bool pmm_daemon_application_free_with_timeout(pmm_daemon_application_t *application,
    ...
    if (!pmm_daemon_application_shutdown(application, timeout_ms)) {
         ...
         * resolved. */
        pmm_log_error("daemon.application.free_busy", "action", "retain");
        return false;
    }
    if (application->watcher) {
        ...
        pmm_watcher_set_project_mutation_guard(application->watcher, NULL, NULL, NULL, NULL);
    }
    pmm_mutex_lock(&application->mutex);
    pmm_daemon_application_session_t *sessions = application->sessions;
    application->sessions = NULL;
    pmm_daemon_application_watch_t *watches = application->watches;
    application->watches = NULL;
    pmm_daemon_application_job_t *jobs = application->jobs;
    application->jobs = NULL;
    pmm_daemon_application_watch_job_subscription_t *watch_job_subscriptions =
        application->watch_job_subscriptions;
    application->watch_job_subscriptions = NULL;
    pmm_daemon_application_mutation_t *mutations = application->mutations;
    application->mutations = NULL;
    pmm_mutex_unlock(&application->mutex);
    while (sessions) {
        pmm_daemon_application_session_t *next = sessions->next;
        pmm_mcp_server_free(sessions->mcp);
        free(sessions);
        sessions = next;
    }
    while (watches) {
        pmm_daemon_application_watch_t *next = watches->next;
        if (application->watcher) {
            pmm_watcher_unwatch(application->watcher, watches->project);
        }
    ...
    while (watch_job_subscriptions) {
        pmm_daemon_application_watch_job_subscription_t *next = watch_job_subscriptions->next;
        free(watch_job_subscriptions);
        watch_job_subscriptions = next;
    }
    while (jobs) {
        pmm_daemon_application_job_t *next = jobs->next;
        if (jobs->thread_started) {
            (void)pmm_thread_join(&jobs->thread);
        }
        application_job_free(jobs);
        jobs = next;
    }
    while (mutations) {
        pmm_daemon_application_mutation_t *next = mutations->next;
...
bool pmm_daemon_application_free(pmm_daemon_application_t *application) {
    return pmm_daemon_application_free_with_timeout(application, 3000);
}
...
static pmm_daemon_runtime_application_status_t application_client_exchange_tagged(
    ...
    pmm_daemon_runtime_application_status_t status =
        request_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID
            ? pmm_daemon_runtime_client_application_request(client, request, request_length,
                                                            &response, &response_length, timeout_ms)
            : pmm_daemon_runtime_client_application_request_tagged(client, request_token, request,
                                                                   request_length, &response,
                                                                   &response_length, timeout_ms);
    free(request);
    if (status != PMM_DAEMON_RUNTIME_APPLICATION_OK) {
        free(response);
        return status;
    }
    if (response) {
...
static pmm_daemon_runtime_application_status_t application_client_exchange(
    pmm_daemon_runtime_client_t *client, uint8_t *request, uint32_t request_length,
    uint8_t **response_out, uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_exchange_tagged(client, PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID,
...
pmm_daemon_runtime_application_status_t pmm_daemon_application_client_set_context(
    ...
    uint64_t total = (uint64_t)APPLICATION_CONTEXT_HEADER_SIZE + root_length + allowed_length +
                     event_length + dialect_length;
    if (tool_profile < PMM_MCP_TOOL_PROFILE_ALL || tool_profile > PMM_MCP_TOOL_PROFILE_SCOUT ||
        (tool_profile != PMM_MCP_TOOL_PROFILE_ALL && (event_length != 0 || dialect_length != 0)) ||
        !pmm_hook_augment_invocation_supported(hook_event, hook_dialect) ||
        root_length > UINT32_MAX || allowed_length > UINT32_MAX || event_length > UINT32_MAX ||
        dialect_length > UINT32_MAX || total > PMM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return PMM_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ...
    pmm_daemon_runtime_application_status_t status = application_client_exchange(
        client, request, (uint32_t)total, &unexpected, &unexpected_length, timeout_ms);
    if (status == PMM_DAEMON_RUNTIME_APPLICATION_OK && (unexpected || unexpected_length != 0)) {
...
pmm_daemon_runtime_application_status_t pmm_daemon_application_client_set_ui_config(
    ...
    request[0] = PMM_DAEMON_APPLICATION_REQUEST_SET_UI_CONFIG;
...
static pmm_daemon_runtime_application_status_t application_client_text_request_tagged(
    pmm_daemon_runtime_client_t *client, pmm_daemon_runtime_application_token_t request_token,
    pmm_daemon_application_request_kind_t kind, const char *text, uint8_t **response_out,
    uint32_t *response_length_out, uint32_t timeout_ms) {
    if (!client || !text || !text[0]) {
        return PMM_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    size_t text_length = strlen(text);
    if (text_length + 1U > PMM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return PMM_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
...
static pmm_daemon_runtime_application_status_t application_client_text_request(
    pmm_daemon_runtime_client_t *client, pmm_daemon_application_request_kind_t kind,
    ...
    return application_client_text_request_tagged(
        client, PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID, kind, text, response_out,
        response_length_out, timeout_ms);
}
...
pmm_daemon_runtime_application_status_t pmm_daemon_application_client_mcp(
    ...
    uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_text_request(client, PMM_DAEMON_APPLICATION_REQUEST_MCP, message,
...
pmm_daemon_runtime_application_status_t pmm_daemon_application_client_mcp_tagged(
    ...
    return application_client_text_request_tagged(client, request_token,
                                                  PMM_DAEMON_APPLICATION_REQUEST_MCP, message,
                                                  response_out, response_length_out, timeout_ms);
}
...
pmm_daemon_runtime_application_status_t pmm_daemon_application_client_tool(
    ...
    if (tool_length > UINT32_MAX || total > PMM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return PMM_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
...
pmm_daemon_runtime_application_status_t pmm_daemon_application_client_hook_augment(
    ...
    uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_text_request(client, PMM_DAEMON_APPLICATION_REQUEST_HOOK_AUGMENT,
                                           input_json, response_out, response_length_out,
                                           timeout_ms);
}

static int application_background_index(pmm_daemon_application_t *application,
    ...
    struct stat root_status;
    if (!pmm_canonical_path(root_path, canonical_root, sizeof(canonical_root)) ||
    ...
    char *default_project = pmm_project_name_from_path(canonical_root);
    bool custom_project =
...
size_t pmm_daemon_application_job_subscribers(pmm_daemon_application_t *application,
    ...
    pmm_mutex_lock(&application->mutex);
    pmm_daemon_application_job_t *job = application_find_job_locked(application, project_key);
...
bool pmm_daemon_application_session_retains_store_for_test(
    const pmm_daemon_runtime_application_session_t *opaque_session) {
    const pmm_daemon_application_session_t *session =
        (const pmm_daemon_application_session_t *)opaque_session;
    return session && session->mcp && pmm_mcp_server_store(session->mcp) != NULL;
}
