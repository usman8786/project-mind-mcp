/*
 * host.c — Lifecycle owner for the mandatory per-account PMM daemon.
 */
...
typedef struct {
    void *context;
    void (*config_load)(void *context, pmm_ui_config_t *config_out);
    pmm_http_server_t *(*server_new)(void *context, int port);
    void (*server_configure)(void *context, pmm_http_server_t *server, host_state_t *host);
    void (*server_stop)(void *context, pmm_http_server_t *server);
    bool (*server_free)(void *context, pmm_http_server_t *server);
    int (*thread_start)(void *context, pmm_thread_t *thread, void *(*entry)(void *),
                        void *entry_context);
    int (*thread_join)(void *context, pmm_thread_t *thread);
} host_http_ops_t;

struct host_state {
    /* Mirrors pmm_daemon_host_config_t.permanent for prepare-time consumers. */
    bool permanent;
    pmm_daemon_application_t *application;
    pmm_watcher_t *watcher;
    pmm_store_t *watch_store;
    pmm_config_t *runtime_config;
    pmm_project_lock_manager_t *project_locks;
    pmm_http_server_t *http;
    pmm_thread_t watcher_thread;
    pmm_thread_t http_thread;
...
static pmm_mutex_t g_host_log_mutex;
...
static bool host_log_open(char conflict_log_out[HOST_PATH_CAP]) {
    ...
    g_host_log_file =
        pmm_daemon_ipc_private_log_open(logs, "pmm-daemon.log", HOST_OPERATION_LOG_CAP);
    if (!g_host_log_file) {
        conflict_log_out[0] = '\0';
        return false;
    }
    pmm_mutex_init(&g_host_log_mutex);
    g_host_log_mutex_initialized = true;
    pmm_log_set_sink(host_log_sink);
    return true;
}
...
static void host_log_close(void) {
    ...
    if (g_host_log_mutex_initialized) {
        pmm_mutex_unlock(&g_host_log_mutex);
        pmm_mutex_destroy(&g_host_log_mutex);
...
static void host_cleanup_release_until_complete(pmm_daemon_host_cleanup_release_for_test_fn release,
...
void pmm_daemon_host_cleanup_release_until_complete_for_test(
    pmm_daemon_host_cleanup_release_for_test_fn release, void *context) {
...
static bool host_cohort_lease_release_once(void *context) {
    pmm_version_cohort_lease_t **lease = context;
    if (!lease || !*lease) {
        return true;
    }
    (void)pmm_version_cohort_lease_release(lease);
    return *lease == NULL;
}

static bool host_cohort_manager_free_once(void *context) {
    pmm_version_cohort_manager_t **manager = context;
    if (!manager || !*manager) {
        return true;
    }
    (void)pmm_version_cohort_manager_free(manager);
    return *manager == NULL;
}

static void host_cohort_close(pmm_version_cohort_lease_t **lease,
...
static bool host_daemon_claim_release_once(void *context) {
    pmm_version_cohort_daemon_claim_t **claim = context;
    if (!claim || !*claim) {
        return true;
    }
    (void)pmm_version_cohort_daemon_claim_release(claim);
    return *claim == NULL;
}

static void host_daemon_claim_close(pmm_version_cohort_daemon_claim_t **claim) {
...
static bool host_participant_guard_release_once(void *context) {
    pmm_daemon_ipc_participant_guard_t **guard = context;
    if (!guard || !*guard) {
        return true;
    }
    (void)pmm_daemon_ipc_participant_guard_release(guard);
    return *guard == NULL;
}

static void host_participant_guard_close(pmm_daemon_ipc_participant_guard_t **guard) {
...
static int host_lifetime_reservation_acquire(
    ...
    do {
        int status = pmm_daemon_ipc_lifetime_reservation_try_acquire(endpoint, reservation_out);
        if (status != 0) {
            return status;
        }
        pmm_usleep(1000);
    ...
    return 0;
#else
    return pmm_daemon_ipc_lifetime_reservation_try_acquire(endpoint, reservation_out);
#endif
}

static void *host_watcher_thread(void *opaque) {
    pmm_watcher_run(opaque, HOST_WATCH_INTERVAL_MS);
    return NULL;
}

static void *host_http_thread(void *opaque) {
    pmm_http_server_run(opaque);
    return NULL;
}
...
static int host_watcher_index(const char *project_name, const char *root_path, void *opaque) {
    ...
    return host && host->application
               ? pmm_daemon_application_watcher_index(project_name, root_path, host->application)
               : -1;
}
...
static int host_ui_index(void *opaque, const char *root_path, const char *project_name) {
    host_state_t *host = opaque;
    return host && host->application
               ? pmm_daemon_application_index(host->application, project_name ? project_name : "",
                                              root_path)
               : -1;
}
...
static bool host_ui_mutation_begin(void *opaque, const char *project) {
    host_state_t *host = opaque;
    return host && host->application &&
           pmm_daemon_application_project_mutation_try_begin(host->application, project);
}

static void host_ui_mutation_end(void *opaque, const char *project) {
    host_state_t *host = opaque;
    if (host && host->application) {
        pmm_daemon_application_project_mutation_end(host->application, project);
    }
}

static void host_http_config_load_default(void *context, pmm_ui_config_t *config_out) {
    (void)context;
    pmm_ui_config_load(config_out);
}

static pmm_http_server_t *host_http_server_new_default(void *context, int port) {
    (void)context;
    return pmm_http_server_new(port);
}

static void host_http_server_configure_default(void *context, pmm_http_server_t *server,
                                               host_state_t *host) {
    (void)context;
    pmm_http_server_set_watcher(server, host->watcher);
    pmm_http_server_set_index_executor(server, host_ui_index, host);
    pmm_http_server_set_project_mutation_guard(server, host_ui_mutation_begin, host_ui_mutation_end,
                                               host);
}

static void host_http_server_stop_default(void *context, pmm_http_server_t *server) {
    (void)context;
    pmm_http_server_stop(server);
}

static bool host_http_server_free_default(void *context, pmm_http_server_t *server) {
    (void)context;
    return pmm_http_server_free(server);
}

typedef int (*host_http_thread_create_fn)(void *context, pmm_thread_t *thread,
                                          void *(*entry)(void *), void *entry_context);

static int host_http_thread_create_default(void *context, pmm_thread_t *thread,
                                           void *(*entry)(void *), void *entry_context) {
    (void)context;
    return pmm_thread_create(thread, 0, entry, entry_context);
}

static int host_http_thread_schedule_create(pmm_thread_t *thread, void *(*entry)(void *),
                                            pmm_http_server_t *server,
                                            host_http_thread_create_fn create,
                                            void *create_context) {
    if (!server || !create || !pmm_http_server_schedule_run(server))
        return -1;
    int rc = create(create_context, thread, entry, server);
    if (rc != 0 && !pmm_http_server_cancel_scheduled_run(server))
        return -1;
    return rc;
}

static int host_http_thread_start_default(void *context, pmm_thread_t *thread,
                                          void *(*entry)(void *), void *entry_context) {
    (void)context;
...
static int host_http_thread_join_default(void *context, pmm_thread_t *thread) {
    (void)context;
    return pmm_thread_join(thread);
}
...
static void host_http_schedule_retry(host_state_t *host, uint64_t now_ms, const char *reason) {
    ...
    pmm_log_warn("ui.retry_scheduled", "reason", reason, "delay_ms", delay_text);
}
...
static void host_http_reconcile_at(host_state_t *host, uint64_t now_ms, bool force_config_load) {
    ...
    if (!host->http_assets_available) {
        if (config_changed) {
            pmm_log_warn("ui.no_assets", "hint", "rebuild with: make -f Makefile.pmm pmm-with-ui");
        }
...
static bool host_background_join(host_state_t *host) {
    ...
    if (host->watcher_started) {
        if (pmm_thread_join(&host->watcher_thread) != 0) {
            return false;
        }
...
static bool host_project_lock_manager_free_once(void *context) {
    pmm_project_lock_manager_t **manager = context;
    if (!manager || !*manager) {
        return true;
    }
    (void)pmm_project_lock_manager_free(manager);
    return *manager == NULL;
}
...
static void host_state_free(host_state_t *host) {
    ...
    if (host->application) {
        if (!pmm_daemon_application_free(host->application)) {
    ...
    pmm_watcher_free(host->watcher);
    host->watcher = NULL;
    pmm_store_close(host->watch_store);
    host->watch_store = NULL;
    pmm_config_close(host->runtime_config);
...
static bool host_state_prepare(host_state_t *host, const pmm_daemon_ipc_endpoint_t *endpoint) {
    host->http_ops = &g_host_http_default_ops;
    host->http_assets_available = PMM_EMBEDDED_FILE_COUNT > 0;
    size_t aggregate_memory_budget_bytes = pmm_mem_budget();
    const char *cache = pmm_resolve_cache_dir();
    if (!cache || !cache[0]) {
        pmm_log_error("daemon.runtime_config_open_failed", "reason", "cache_dir_unavailable");
        return false;
    }
    host->runtime_config = pmm_config_open(cache);
    if (!host->runtime_config) {
        pmm_log_error("daemon.runtime_config_open_failed", "reason", "config_db_unavailable");
        return false;
    }
    host->watch_store = pmm_store_open_memory();
    host->project_locks = pmm_project_lock_manager_new(endpoint);
    host->watcher = pmm_watcher_new(host->watch_store, host_watcher_index, host);
    pmm_daemon_application_config_t application_config = {
    ...
    host->application = pmm_daemon_application_new(&application_config);
    if (host->application && host->permanent) {
        pmm_daemon_application_set_permanent(host->application, true);
    }
...
bool pmm_daemon_host_state_prepare_for_test(const pmm_daemon_ipc_endpoint_t *endpoint) {
    if (!endpoint) {
        return false;
    }
    host_state_t host = {0};
...
static void host_http_test_config_load(void *opaque, pmm_ui_config_t *config_out) {
    ...
    config_out->ui_port = change_port ? PMM_UI_DEFAULT_PORT + 1 : PMM_UI_DEFAULT_PORT;
}

static pmm_http_server_t *host_http_test_server_new(void *opaque, int port) {
    (void)port;
    host_http_reconcile_test_context_t *context = opaque;
    context->server_create_attempts++;
    return context->server_create_attempts <= context->create_failures
               ? NULL
               : (pmm_http_server_t *)context;
}

static void host_http_test_server_configure(void *opaque, pmm_http_server_t *server,
                                            host_state_t *host) {
    (void)opaque;
    (void)server;
    (void)host;
...
static void host_http_test_server_stop(void *opaque, pmm_http_server_t *server) {
    if (server) {
...
// cppcheck-suppress constParameterCallback
static bool host_http_test_server_free(void *opaque, pmm_http_server_t *server) {
    host_http_reconcile_test_context_t *context = opaque;
    if (server) {
...
static int host_http_test_thread_start(void *opaque, pmm_thread_t *thread, void *(*entry)(void *),
                                       void *entry_context) {
    (void)thread;
    (void)entry;
    (void)entry_context;
...
static int host_http_test_thread_join(void *opaque, pmm_thread_t *thread) {
    (void)thread;
    host_http_reconcile_test_context_t *context = opaque;
...
bool pmm_daemon_host_http_reconcile_sequence_for_test(
    const uint64_t *timestamps_ms, size_t timestamp_count, size_t create_failures,
    size_t thread_start_failures, pmm_daemon_host_http_reconcile_test_result_t *result_out) {
    ...
    *result_out = (pmm_daemon_host_http_reconcile_test_result_t){
...
bool pmm_daemon_host_http_reconcile_free_refusal_for_test(
    pmm_daemon_host_http_free_refusal_test_result_t *result_out) {
    if (!result_out) {
        return false;
    }
    host_http_reconcile_test_context_t context = {
    ...
    *result_out = (pmm_daemon_host_http_free_refusal_test_result_t){
...
static int host_http_thread_create_failure_for_test(
    void *opaque, pmm_thread_t *thread, void *(*entry)(void *),
    ...
    if (!test || entry_context != test->server)
        return -1;
    test->free_refused_while_scheduled = !pmm_http_server_free(test->server);
    return -1;
}

bool pmm_daemon_host_http_thread_create_failure_lifecycle_for_test(void) {
    pmm_http_server_t *server = pmm_http_server_new(0);
    if (!server)
        return false;
    host_http_thread_create_failure_test_t test = {
        .server = server,
    };
    pmm_thread_t unused_thread = {0};
    ...
    bool final_free_succeeded = free_refused && pmm_http_server_free(server);
    return rc != 0 && free_refused && final_free_succeeded;
}

static bool host_background_start(host_state_t *host) {
    if (pmm_thread_create(&host->watcher_thread, 0, host_watcher_thread, host->watcher) != 0) {
        return false;
    }
    host->watcher_started = true;

    host_http_reconcile_at(host, pmm_now_ms(), true);
    return true;
}

static bool host_wait_for_lifetime(pmm_daemon_runtime_service_t *service,
                                   atomic_int *stop_requested, host_state_t *host, bool permanent) {
    uint64_t initial_deadline = pmm_now_ms() + HOST_INITIAL_CLIENT_TIMEOUT_MS;
    uint64_t stopping_deadline = 0;
    for (;;) {
        pmm_daemon_runtime_service_state_t state = pmm_daemon_runtime_service_state(service);
        if (state == PMM_DAEMON_RUNTIME_SERVICE_EXITED) {
            pmm_log_info("daemon.lifetime_end", "reason", "runtime_exited");
            return true;
        }
        if (state == PMM_DAEMON_RUNTIME_SERVICE_STOPPING) {
            if (stopping_deadline == 0) {
                stopping_deadline = pmm_now_ms() + HOST_RUNTIME_SHUTDOWN_MS;
            }
            if (pmm_now_ms() >= stopping_deadline) {
                return false;
            }
            (void)pmm_daemon_runtime_service_wait_exited(service, HOST_WAIT_TICK_MS);
            continue;
        }
        ...
        bool admitted = pmm_daemon_runtime_service_clients_admitted_total(service) > 0;
        ...
        if (stop || (!permanent && !admitted && pmm_now_ms() >= initial_deadline)) {
            pmm_log_info("daemon.lifetime_end", "reason",
                         stop ? "stop_requested" : "initial_window_expired");
            return pmm_daemon_runtime_service_stop(service, HOST_RUNTIME_SHUTDOWN_MS);
        }
        host_http_reconcile_at(host, pmm_now_ms(), false);
        (void)pmm_daemon_runtime_service_wait_exited(service, HOST_WAIT_TICK_MS);
    }
}

static bool host_application_shutdown(host_state_t *host) {
    if (pmm_daemon_application_shutdown(host->application, HOST_APPLICATION_SHUTDOWN_MS)) {
        return true;
    }
    pmm_log_error("daemon.shutdown_timeout", "component", "operations");
    return false;
}

static bool host_runtime_stop_free(pmm_daemon_runtime_service_t *service) {
    if (pmm_daemon_runtime_service_state(service) != PMM_DAEMON_RUNTIME_SERVICE_EXITED) {
        if (!pmm_daemon_runtime_service_stop(service, HOST_RUNTIME_SHUTDOWN_MS)) {
            pmm_log_error("daemon.shutdown_timeout", "component", "runtime");
            return false;
        }
    }
    if (!pmm_daemon_runtime_service_free(service)) {
        pmm_log_error("daemon.cleanup_timeout", "component", "runtime");
        return false;
    }
    return true;
}
...
_Noreturn void pmm_daemon_host_force_terminate_for_test(const char *component) {
    char conflict_log[HOST_PATH_CAP];
    pmm_ui_log_init();
...
int pmm_daemon_host_run(const pmm_daemon_host_config_t *config) {
    ...
    char conflict_log[HOST_PATH_CAP];
    if (!host_log_open(conflict_log)) {
        (void)fprintf(stderr, "project-mind: daemon log path is not private or safe\n");
        return -1;
    }
    pmm_version_cohort_manager_t *cohort_manager = pmm_version_cohort_manager_new(config->endpoint);
    ...
    pmm_version_cohort_status_t cohort_status =
        cohort_manager
            ? pmm_version_cohort_acquire(cohort_manager, &config->identity, cohort_deadline,
                                         &cohort_lease, &cohort_conflict)
            : PMM_VERSION_COHORT_IO;
    if (cohort_status != PMM_VERSION_COHORT_OK) {
        pmm_log_error("daemon.start_failed", "component", "cohort");
        ...
        (void)fprintf(stderr, "project-mind: %s\n",
    ...
    if (pmm_daemon_ipc_participant_guard_try_join(config->endpoint, &participant_guard) != 1) {
        pmm_log_error("daemon.start_failed", "component", "participant");
        (void)fprintf(stderr, "project-mind: daemon participant admission failed\n");
        host_participant_guard_close(&participant_guard);
        host_cohort_close(&cohort_lease, &cohort_manager);
        host_log_close();
        return -1;
    }
    pmm_version_cohort_daemon_claim_t *daemon_claim = NULL;
    uint64_t claim_deadline = host_deadline_after(HOST_DAEMON_CLAIM_TIMEOUT_MS);
    pmm_version_cohort_status_t claim_status =
        pmm_version_cohort_daemon_claim_acquire(cohort_manager, &daemon_claim);
    while (claim_status == PMM_VERSION_COHORT_BUSY && pmm_now_ms() < claim_deadline) {
        ...
        pmm_usleep(HOST_WAIT_TICK_MS * 1000);
        claim_status = pmm_version_cohort_daemon_claim_acquire(cohort_manager, &daemon_claim);
    }
    if (claim_status != PMM_VERSION_COHORT_OK) {
        pmm_log_error("daemon.start_failed", "component", "claim");
        host_participant_guard_close(&participant_guard);
    ...
    pmm_daemon_ipc_lifetime_reservation_t *lifetime_reservation = NULL;
    if (host_lifetime_reservation_acquire(config->endpoint, &lifetime_reservation) != 1) {
        pmm_log_error("daemon.start_failed", "component", "lifetime");
        pmm_daemon_ipc_lifetime_reservation_release(lifetime_reservation);
        host_participant_guard_close(&participant_guard);
        host_daemon_claim_close(&daemon_claim);
        host_cohort_close(&cohort_lease, &cohort_manager);
        host_log_close();
    ...
    pmm_http_server_set_binary_path(config->executable_path);
    pmm_index_supervisor_mark_host();

    host_state_t host = {0};
    host.permanent = config->permanent;
    if (!host_state_prepare(&host, config->endpoint)) {
        pmm_log_error("daemon.start_failed", "component", "application");
        host_state_free(&host);
        host_log_close();
        pmm_daemon_ipc_lifetime_reservation_release(lifetime_reservation);
        host_participant_guard_close(&participant_guard);
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(host.application);
    pmm_daemon_runtime_service_config_t runtime_config = {
    ...
    pmm_daemon_runtime_service_t *service =
        pmm_daemon_runtime_service_start_reserved(&runtime_config, &lifetime_reservation);
    pmm_daemon_ipc_lifetime_reservation_release(lifetime_reservation);
    lifetime_reservation = NULL;
    if (!service) {
        pmm_log_error("daemon.start_failed", "component", "runtime");
        host_state_free(&host);
        host_log_close();
        host_participant_guard_close(&participant_guard);
        host_daemon_claim_close(&daemon_claim);
    ...
    if (!host_background_start(&host)) {
        pmm_log_error("daemon.start_failed", "component", "background");
    ...
    (void)snprintf(memory_budget, sizeof(memory_budget), "%zu", pmm_mem_budget());
    (void)snprintf(physical_job_limit, sizeof(physical_job_limit), "%zu",
                   pmm_daemon_application_physical_job_limit(host.application));
    (void)snprintf(worker_memory_budget, sizeof(worker_memory_budget), "%zu",
                   pmm_daemon_application_worker_memory_budget_bytes(host.application));
    pmm_log_info("daemon.start", "version", config->identity.semantic_version, "pid", process_id,
                 "cache_fingerprint",
