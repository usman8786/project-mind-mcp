...
pmm_daemon_process_role_t pmm_daemon_process_role(int argc, char *const argv[]) {
    if (argc <= 0 || !argv || !argv[0] || argv[0][0] == '\0') {
        return PMM_DAEMON_PROCESS_INVALID;
    }

    int daemon_arg = bootstrap_find_arg(argc, argv, PMM_DAEMON_INTERNAL_ARG);
    if (daemon_arg >= 0) {
        ...
        if (argc == 2 && daemon_arg == 1) {
            return PMM_DAEMON_PROCESS_DAEMON;
        }
        if (argc == 3 && daemon_arg == 1 && bootstrap_arg_is(argv[2], PMM_DAEMON_PERMANENT_ARG)) {
            return PMM_DAEMON_PROCESS_DAEMON;
        }
        return PMM_DAEMON_PROCESS_INVALID;
    }
    ...
    if (worker_arg >= 0) {
        return bootstrap_worker_argv_exact(argc, argv) ? PMM_DAEMON_PROCESS_WORKER
                                                       : PMM_DAEMON_PROCESS_INVALID;
    }
    ...
    for (int arg = 1; arg < argc; arg++) {
        if (bootstrap_arg_is(argv[arg], "cli")) {
            if (bootstrap_has_help_after(argc, argv, arg + 1)) {
                return PMM_DAEMON_PROCESS_STATELESS;
            }
            return PMM_DAEMON_PROCESS_LOCAL_CLI;
        }
        if (bootstrap_arg_is(argv[arg], "hook-augment")) {
            return PMM_DAEMON_PROCESS_HOOK_CLIENT;
        }
        if (bootstrap_arg_is(argv[arg], "config")) {
            return bootstrap_has_help_after(argc, argv, arg + 1) ? PMM_DAEMON_PROCESS_STATELESS
                                                                 : PMM_DAEMON_PROCESS_LOCAL_CLI;
        }
        /* Placed after the `cli` check on purpose: `pmm cli search "daemon
    ...
    return PMM_DAEMON_PROCESS_MCP_CLIENT;
}

bool pmm_daemon_process_role_requires_client(pmm_daemon_process_role_t role) {
    return role == PMM_DAEMON_PROCESS_MCP_CLIENT || role == PMM_DAEMON_PROCESS_HOOK_CLIENT;
}

pmm_daemon_ipc_endpoint_t *pmm_daemon_bootstrap_endpoint_new(const char *runtime_parent) {
    char key[PMM_DAEMON_KEY_SIZE];
    if (!pmm_daemon_rendezvous_key(key)) {
        return NULL;
    }
    return pmm_daemon_ipc_endpoint_new(key, runtime_parent);
}
...
bool pmm_daemon_bootstrap_launch_spec_init(const char *executable_path,
    ...
    spec_out->argv[1] = PMM_DAEMON_INTERNAL_ARG;
    spec_out->argc = 2U;
...
bool pmm_daemon_bootstrap_launch_spec_init_permanent(const char *executable_path,
                                                     pmm_daemon_bootstrap_launch_spec_t *spec_out) {
    if (!pmm_daemon_bootstrap_launch_spec_init(executable_path, spec_out)) {
        return false;
    }
    spec_out->argv[2] = PMM_DAEMON_PERMANENT_ARG;
    spec_out->argc = 3U;
    return true;
}
...
static _Noreturn void bootstrap_cleanup_fail_stop(const char *component) {
    (void)fprintf(stderr,
                  "project-mind-mcp: coordination cleanup failed (%s); "
...
static void bootstrap_startup_lock_release_complete(const pmm_daemon_bootstrap_ops_t *ops,
                                                    pmm_daemon_bootstrap_lock_t *lock_io) {
...
static void bootstrap_result_reset(pmm_daemon_bootstrap_result_t *result,
                                   pmm_daemon_process_role_t role) {
    memset(result, 0, sizeof(*result));
    result->status = pmm_daemon_process_role_requires_client(role) ? PMM_DAEMON_BOOTSTRAP_FAILED
                                                                   : PMM_DAEMON_BOOTSTRAP_BYPASSED;
}

static pmm_daemon_bootstrap_status_t bootstrap_finish_probe(
    pmm_daemon_bootstrap_probe_status_t probe, pmm_daemon_runtime_client_t *client,
    const pmm_daemon_runtime_connect_result_t *connect_result,
    const pmm_daemon_bootstrap_ops_t *ops, pmm_daemon_bootstrap_result_t *result) {
    if (connect_result) {
        result->connect_result = *connect_result;
    }
    result->client = client;
    if (probe == PMM_DAEMON_BOOTSTRAP_PROBE_CONNECTED && client) {
        result->status = PMM_DAEMON_BOOTSTRAP_CONNECTED;
        return result->status;
    }
    result->client = NULL;
    if (probe == PMM_DAEMON_BOOTSTRAP_PROBE_CONFLICT) {
        result->status = PMM_DAEMON_BOOTSTRAP_CONFLICT;
        (void)snprintf(result->message, sizeof(result->message), "%s",
                       connect_result && connect_result->message[0]
                           ...
                           : "PMM could not start because a conflicting PMM process is active; "
                             "close all PMM sessions and commands, then retry. If a permanent "
                             "daemon from another build is running, `project-mind-mcp daemon "
                             "stop` retires it");
    ...
    return PMM_DAEMON_BOOTSTRAP_FAILED;
}

static pmm_daemon_bootstrap_probe_status_t bootstrap_probe(
    const pmm_daemon_bootstrap_config_t *config, const pmm_daemon_bootstrap_ops_t *ops,
    pmm_daemon_runtime_client_t **client_out, pmm_daemon_runtime_connect_result_t *connect_result) {
...
static bool bootstrap_probe_is_finishable(pmm_daemon_bootstrap_probe_status_t probe) {
    return probe == PMM_DAEMON_BOOTSTRAP_PROBE_CONNECTED ||
           probe == PMM_DAEMON_BOOTSTRAP_PROBE_CONFLICT;
}

static bool bootstrap_probe_is_waitable(pmm_daemon_bootstrap_probe_status_t probe) {
    return probe == PMM_DAEMON_BOOTSTRAP_PROBE_UNAVAILABLE ||
           probe == PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED ||
           probe == PMM_DAEMON_BOOTSTRAP_PROBE_TERMINAL;
}

static bool bootstrap_config_valid(const pmm_daemon_bootstrap_config_t *config,
...
pmm_daemon_bootstrap_status_t pmm_daemon_bootstrap_execute_with_ops(
    ...
    pmm_daemon_process_role_t role = config ? config->role : PMM_DAEMON_PROCESS_INVALID;
    bootstrap_result_reset(result_out, role);
    if (!pmm_daemon_process_role_requires_client(role)) {
        return role == PMM_DAEMON_PROCESS_INVALID ? PMM_DAEMON_BOOTSTRAP_FAILED
                                                  : PMM_DAEMON_BOOTSTRAP_BYPASSED;
    }
    ...
    pmm_daemon_bootstrap_probe_status_t probe =
    ...
    if (!bootstrap_probe_is_waitable(probe)) {
        probe = PMM_DAEMON_BOOTSTRAP_PROBE_ERROR;
    }

    pmm_daemon_bootstrap_lock_t startup_lock = NULL;
    bool lock_acquired = false;
    bool generation_observed = probe == PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED ||
                               probe == PMM_DAEMON_BOOTSTRAP_PROBE_TERMINAL;
    while (pmm_now_ms() < deadline) {
        if (!bootstrap_probe_is_waitable(probe)) {
            break;
        }
        if (probe == PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED ||
            probe == PMM_DAEMON_BOOTSTRAP_PROBE_TERMINAL) {
        ...
        if (bootstrap_probe_is_finishable(probe) || probe == PMM_DAEMON_BOOTSTRAP_PROBE_ERROR) {
            break;
        }
        if (probe != PMM_DAEMON_BOOTSTRAP_PROBE_UNAVAILABLE) {
            probe = PMM_DAEMON_BOOTSTRAP_PROBE_ERROR;
            break;
        }
        ...
        bool spec_ready =
            config->spawn_permanent
                ? pmm_daemon_bootstrap_launch_spec_init_permanent(config->executable_path, &spec)
                : pmm_daemon_bootstrap_launch_spec_init(config->executable_path, &spec);
    ...
    result_out->status = PMM_DAEMON_BOOTSTRAP_FAILED;
    if (generation_observed) {
        (void)snprintf(result_out->message, sizeof(result_out->message),
                       "PMM daemon is active or starting but could not accept this client "
                       "within %u ms",
                       config->startup_timeout_ms);
    } else {
        (void)snprintf(result_out->message, sizeof(result_out->message),
                       "PMM daemon could not start within %u ms", config->startup_timeout_ms);
    }
    if (ops->visible_diagnostic) {
        ops->visible_diagnostic(ops->context, result_out->message);
    }
...
pmm_daemon_bootstrap_probe_status_t pmm_daemon_bootstrap_classify_failed_connect(
    ...
    if (!connect_result) {
        return PMM_DAEMON_BOOTSTRAP_PROBE_ERROR;
    }
    if (connect_result->status == PMM_DAEMON_RUNTIME_CONNECT_CONFLICT) {
        return PMM_DAEMON_BOOTSTRAP_PROBE_CONFLICT;
    }
    if (connect_result->status == PMM_DAEMON_RUNTIME_CONNECT_REJECTED) {
    ...
    return connect_result->status == PMM_DAEMON_RUNTIME_CONNECT_ERROR
...
static pmm_daemon_bootstrap_probe_status_t bootstrap_production_probe(
    void *context, const pmm_daemon_ipc_endpoint_t *endpoint,
    ...
    pmm_version_cohort_daemon_presence_t claim =
        pmm_version_cohort_daemon_claim_presence(production->cohort->manager);
    if (claim == PMM_VERSION_COHORT_DAEMON_ABSENT) {
    ...
    } else if (claim != PMM_VERSION_COHORT_DAEMON_COORDINATED) {
        return PMM_DAEMON_BOOTSTRAP_PROBE_ERROR;
    }

    *client_out = pmm_daemon_runtime_client_connect(endpoint, identity, timeout_ms, result_out);
    if (*client_out) {
    ...
    claim = pmm_version_cohort_daemon_claim_presence(production->cohort->manager);
    if (claim == PMM_VERSION_COHORT_DAEMON_COORDINATED) {
        return pmm_daemon_bootstrap_classify_failed_connect(result_out, 1);
    }
    if (claim != PMM_VERSION_COHORT_DAEMON_ABSENT) {
        return PMM_DAEMON_BOOTSTRAP_PROBE_ERROR;
    }
    int lifetime_status = pmm_daemon_ipc_lifetime_reservation_probe(endpoint);
    return pmm_daemon_bootstrap_classify_failed_connect(result_out, lifetime_status);
}

static pmm_version_cohort_status_t bootstrap_production_cohort_acquire(
    void *context, const pmm_daemon_ipc_endpoint_t *endpoint,
    ...
    *cohort_out = NULL;
    ...
    if (cohort) {
        cohort->manager = pmm_version_cohort_manager_new(endpoint);
    }
    if (!cohort || !cohort->manager) {
        free(cohort);
        return PMM_VERSION_COHORT_IO;
    }
    pmm_version_cohort_status_t status = pmm_version_cohort_acquire(
        cohort->manager, identity, deadline_ms, &cohort->lease, conflict_out);
    if (status == PMM_VERSION_COHORT_CONFLICT) {
        (void)pmm_version_cohort_log_conflict(conflict_out);
    }
    if (status == PMM_VERSION_COHORT_OK || cohort->lease) {
        *cohort_out = cohort;
        if (context) {
            bootstrap_production_context_t *production = context;
    ...
    pmm_private_file_lock_status_t cleanup = PMM_PRIVATE_FILE_LOCK_OK;
    while (cohort->manager) {
        cleanup = pmm_version_cohort_manager_free(&cohort->manager);
        if (!cohort->manager) {
            break;
        }
        if (pmm_now_ms() >= cleanup_deadline) {
            bootstrap_cleanup_fail_stop("cohort_manager_cleanup");
        }
        pmm_usleep(1000);
    }
    free(cohort);
    return cleanup == PMM_PRIVATE_FILE_LOCK_OK ? status : PMM_VERSION_COHORT_IO;
}

static void bootstrap_production_cohort_release(void *context,
                                                pmm_daemon_bootstrap_cohort_t opaque) {
    bootstrap_production_context_t *production = context;
    ...
    uint64_t cleanup_deadline = bootstrap_deadline_after(BOOTSTRAP_COORDINATION_CLEANUP_MS);
    while (cohort->lease) {
        (void)pmm_version_cohort_lease_release(&cohort->lease);
        if (!cohort->lease) {
            break;
        }
        if (pmm_now_ms() >= cleanup_deadline) {
    ...
    while (cohort->manager) {
        (void)pmm_version_cohort_manager_free(&cohort->manager);
        if (!cohort->manager) {
            break;
        }
        if (pmm_now_ms() >= cleanup_deadline) {
...
static int bootstrap_production_lock(void *context, const pmm_daemon_ipc_endpoint_t *endpoint,
                                     pmm_daemon_bootstrap_lock_t *lock_out) {
    (void)context;
    pmm_daemon_ipc_startup_lock_t *lock = NULL;
    int status = pmm_daemon_ipc_startup_lock_try_acquire(endpoint, &lock);
    *lock_out = lock;
    return status;
}

static bool bootstrap_production_unlock(void *context, pmm_daemon_bootstrap_lock_t *lock_io) {
    (void)context;
    if (!lock_io) {
        return false;
    }
    pmm_daemon_ipc_startup_lock_t *lock = *lock_io;
    bool released = pmm_daemon_ipc_startup_lock_release(&lock);
    *lock_io = lock;
    return released;
}

static bool bootstrap_production_handoff(void *context, pmm_daemon_bootstrap_lock_t lock) {
    (void)context;
    return pmm_daemon_ipc_startup_lock_prepare_handoff((pmm_daemon_ipc_startup_lock_t *)lock);
}

#ifdef _WIN32
static bool bootstrap_production_spawn(void *context,
                                       const pmm_daemon_bootstrap_launch_spec_t *spec) {
    bootstrap_production_context_t *production = context;
    if (production) {
    ...
    if (!pmm_build_win_cmdline(command_line, sizeof(command_line), spec->argv)) {
        if (production) {
            production->spawn_error = ERROR_INVALID_PARAMETER;
        }
        return false;
    }
    wchar_t *application = pmm_utf8_to_wide(spec->executable_path);
    wchar_t *command = pmm_utf8_to_wide(command_line);
...
static void bootstrap_daemon_grandchild(const pmm_daemon_bootstrap_launch_spec_t *spec) {
    (void)umask(077);
    sigset_t empty;
    (void)sigemptyset(&empty);
...
static void bootstrap_production_diagnostic(void *context, const char *message) {
    ...
    if (production && production->spawn_error != ERROR_SUCCESS) {
        (void)fprintf(stderr, "project-mind-mcp: %s (daemon launch error %lu)\n",
    ...
    if (production && production->spawn_error != 0) {
        (void)fprintf(stderr, "project-mind-mcp: %s (daemon launch: %s)\n",
                      message ? message : "daemon startup failed",
...
pmm_daemon_bootstrap_status_t pmm_daemon_bootstrap_execute(
    ...
    return pmm_daemon_bootstrap_execute_with_ops(config, &ops, result_out);
}
