...
#define AGENT_ENTRY_KEY "project-mind-mcp"
#define AGENT_MAX_CONFIG_BYTES (8U * 1024U * 1024U)
#define AGENT_JSON_MAX_DEPTH 64U

static int agent_install_callback(pmm_agent_client_id_t id, const char *config_path,
                                  const char *binary_path);
static int agent_remove_callback(pmm_agent_client_id_t id, const char *config_path,
                                 const char *binary_path);

static const pmm_agent_client_profile_t agent_profiles[PMM_AGENT_CLIENT_COUNT] = {
    {PMM_AGENT_CLIENT_QODER, "qoder", "Qoder CLI", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_AGENT | PMM_AGENT_CAP_HOOK, "qodercli",
     agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_KIMI, "kimi", "Kimi Code CLI", PMM_AGENT_STABLE,
    ...
    {PMM_AGENT_CLIENT_GITLAB_DUO, "gitlab-duo", "GitLab Duo CLI", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_HOOK, "duo", agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_ROVO_DEV, "rovo-dev", "Rovo Dev CLI", PMM_AGENT_STABLE,
    ...
    {PMM_AGENT_CLIENT_AMP, "amp", "Amp", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL, "amp",
     agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_DEVIN, "devin", "Devin CLI / Local", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_HOOK,
     "devin", agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_TABNINE, "tabnine", "Tabnine", PMM_AGENT_STABLE, PMM_AGENT_CAP_MCP, "tabnine",
     agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_CONTINUE, "continue", "Continue / cn", PMM_AGENT_CONDITIONAL,
     PMM_AGENT_CAP_MCP, "cn", agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_VISUAL_STUDIO, "visual-studio", "Visual Studio", PMM_AGENT_CONDITIONAL,
     PMM_AGENT_CAP_MCP, "devenv", agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_TRAE, "trae", "TRAE", PMM_AGENT_CONDITIONAL, PMM_AGENT_CAP_MCP, NULL,
     agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_ROO_CODE, "roo-code", "Roo Code", PMM_AGENT_CONDITIONAL, PMM_AGENT_CAP_MCP,
     NULL, agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_AMAZON_Q, "amazon-q", "Amazon Q Developer IDE", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_MCP, NULL, agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_CODEBUDDY, "codebuddy", "CodeBuddy Code CLI", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_AGENT,
     "codebuddy", agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_IBM_BOB_IDE, "ibm-bob-ide", "IBM Bob IDE", PMM_AGENT_CONDITIONAL,
     PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL, NULL,
     agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_IBM_BOB_SHELL, "ibm-bob-shell", "IBM Bob Shell", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS, "bob", agent_install_callback,
     agent_remove_callback},
    {PMM_AGENT_CLIENT_POCHI, "pochi", "Pochi", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_AGENT,
     "pochi", agent_install_callback, agent_remove_callback},
    {PMM_AGENT_CLIENT_PI, "pi", "Pi", PMM_AGENT_STABLE,
     PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL, "pi", NULL, NULL},
    {PMM_AGENT_CLIENT_SOURCEGRAPH_CODY, "sourcegraph-cody", "Sourcegraph Cody", PMM_AGENT_OPT_IN,
     PMM_AGENT_CAP_MCP, NULL, agent_install_callback, agent_remove_callback},
};

size_t pmm_agent_client_count(void) {
    return PMM_AGENT_CLIENT_COUNT;
}

const pmm_agent_client_profile_t *pmm_agent_client_at(size_t index) {
    return index < PMM_AGENT_CLIENT_COUNT ? &agent_profiles[index] : NULL;
}

const pmm_agent_client_profile_t *pmm_agent_client_by_id(pmm_agent_client_id_t id) {
    return (unsigned)id < PMM_AGENT_CLIENT_COUNT ? &agent_profiles[id] : NULL;
}
...
static bool agent_path_exists(const pmm_agent_client_resolve_options_t *options, const char *path) {
    return options->path_exists ? options->path_exists(path, options->probe_context)
                                : pmm_file_exists(path);
}

static bool agent_command_exists(const pmm_agent_client_resolve_options_t *options,
                                 const char *command) {
...
static int agent_rovo_override(const char *config_path, const char *home_dir, bool windows,
                               char *path_out, size_t path_out_size) {
    FILE *file = pmm_fopen(config_path, "rb");
    if (!file) {
        return 1;
    }
...
int pmm_agent_client_resolve_path(pmm_agent_client_id_t id,
    ...
    if (!pmm_agent_client_by_id(id) || !options || !path_out || path_out_size == 0U ||
    ...
    case PMM_AGENT_CLIENT_KIMI:
        return agent_join_path(
            ...
            options->kimi_code_home && options->kimi_code_home[0] ? "mcp.json"
                                                                  : ".kimi-code/mcp.json");
    case PMM_AGENT_CLIENT_GITLAB_DUO:
    ...
    case PMM_AGENT_CLIENT_ROVO_DEV: {
    ...
    case PMM_AGENT_CLIENT_AMP:
        ...
        return agent_join_path(path_out, path_out_size, options->home_dir,
                               ".config/agents/skills/project-mind/mcp.json");
    case PMM_AGENT_CLIENT_DEVIN:
        if (options->is_windows && options->appdata_dir && options->appdata_dir[0]) {
            return agent_join_path(path_out, path_out_size, options->appdata_dir,
                                   "devin/config.json");
        }
    ...
    case PMM_AGENT_CLIENT_TABNINE:
        return agent_join_path(path_out, path_out_size, options->home_dir,
                               ".tabnine/mcp_servers.json");
    case PMM_AGENT_CLIENT_CONTINUE: {
    ...
    case PMM_AGENT_CLIENT_ROO_CODE:
    ...
    case PMM_AGENT_CLIENT_AMAZON_Q: {
    ...
    case PMM_AGENT_CLIENT_CODEBUDDY: {
...
static int agent_client_marker_path(pmm_agent_client_id_t id,
...
bool pmm_agent_client_detect(pmm_agent_client_id_t id,
                             const pmm_agent_client_resolve_options_t *options) {
    const pmm_agent_client_profile_t *profile = pmm_agent_client_by_id(id);
    if (!profile || !options) {
        return false;
    }
    char path[1024];
    int resolved = pmm_agent_client_resolve_path(id, options, path, sizeof(path));
    if (id == PMM_AGENT_CLIENT_CONTINUE || id == PMM_AGENT_CLIENT_TRAE ||
        id == PMM_AGENT_CLIENT_ROO_CODE || id == PMM_AGENT_CLIENT_SOURCEGRAPH_CODY) {
        return resolved == 0 && agent_path_exists(options, path);
    }
    if (id == PMM_AGENT_CLIENT_VISUAL_STUDIO) {
...
bool pmm_agent_client_cleanup_candidate(pmm_agent_client_id_t id,
                                        const pmm_agent_client_resolve_options_t *options) {
    if (pmm_agent_client_detect(id, options)) {
        return true;
    }
    if (id != PMM_AGENT_CLIENT_VISUAL_STUDIO || !options || !options->is_windows) {
        return false;
    }
    char path[1024];
    return pmm_agent_client_resolve_path(id, options, path, sizeof(path)) == 0 &&
...
static char *agent_json_canonical(pmm_agent_client_id_t id, const char *binary_path) {
    char *escaped = agent_json_escape(binary_path);
    if (!escaped) {
        return NULL;
    }
    const char *extra = "";
    if (id == PMM_AGENT_CLIENT_GITLAB_DUO || id == PMM_AGENT_CLIENT_VISUAL_STUDIO) {
        extra = ", \"type\": \"stdio\"";
    } else if (id == PMM_AGENT_CLIENT_ROVO_DEV) {
...
static const char *agent_json_section(pmm_agent_client_id_t id) {
    if (id == PMM_AGENT_CLIENT_AMP) {
        return NULL;
    }
    if (id == PMM_AGENT_CLIENT_VISUAL_STUDIO) {
        return "servers";
    }
    if (id == PMM_AGENT_CLIENT_SOURCEGRAPH_CODY) {
        return "cody.mcpServers";
    }
    if (id == PMM_AGENT_CLIENT_POCHI) {
        return "mcp";
    }
    return "mcpServers";
}

static int agent_json_edit(pmm_agent_client_id_t id, const char *config_path,
                           const char *binary_path, bool remove) {
    char *canonical = agent_json_canonical(id, binary_path);
    if (!canonical) {
        return PMM_AGENT_EDIT_ERROR;
    }
    char *document = NULL;
    size_t length = 0U;
    int read_result = pmm_json_like_read_document(config_path, &document, &length);
    if (read_result < 0) {
        free(canonical);
        return PMM_AGENT_EDIT_ERROR;
    }
    ...
    if (find_result == 0 && !agent_json_owned(document, entry_start, entry_end, canonical)) {
        free(document);
        free(canonical);
        return PMM_AGENT_EDIT_FOREIGN;
    }
    if ((remove && find_result == 1) || (!remove && find_result == 0)) {
        free(document);
        free(canonical);
        return PMM_AGENT_EDIT_OK;
    }
    const char *section = agent_json_section(id);
    const char *path[1] = {section};
    int edit_result =
        remove
            ? pmm_json_like_remove_entry_if_unchanged(config_path, path, section ? 1U : 0U,
                                                      AGENT_ENTRY_KEY, document, length)
            : pmm_json_like_upsert_entry_if_unchanged(config_path, path, section ? 1U : 0U,
                                                      AGENT_ENTRY_KEY, canonical,
                                                      read_result == 1 ? NULL : document, length);
    free(document);
    free(canonical);
    return edit_result == 0 ? PMM_AGENT_EDIT_OK : PMM_AGENT_EDIT_ERROR;
}
...
static int agent_continue_edit(const char *config_path, const char *binary_path, bool remove) {
    char *document = NULL;
    size_t length = 0U;
    if (pmm_json_like_read_document(config_path, &document, &length) != 0 || !document) {
        free(document);
        return PMM_AGENT_EDIT_NOT_APPLICABLE;
    }
    char *quoted = NULL;
    if (pmm_yaml_encode_double_quoted_scalar(binary_path, &quoted) != 0) {
        free(document);
        return PMM_AGENT_EDIT_ERROR;
    }
    ...
    int canonical_written = snprintf(canonical, canonical_size,
                                     "  - name: project-mind-mcp%s    command: %s%s"
                                     "    args: []%s",
                                     eol, quoted, eol, eol);
    free(quoted);
    ...
    int result = updated && pmm_text_write_owned_document_if_unchanged(config_path, updated,
                                                                       document, length) == 0
                     ? PMM_AGENT_EDIT_OK
...
static bool agent_json_client(pmm_agent_client_id_t id) {
    switch (id) {
    case PMM_AGENT_CLIENT_QODER:
    case PMM_AGENT_CLIENT_KIMI:
    case PMM_AGENT_CLIENT_GITLAB_DUO:
...
int pmm_agent_client_install_mcp(pmm_agent_client_id_t id, const char *config_path,
    ...
    if (!config_path || config_path[0] == '\0' || !binary_path || binary_path[0] == '\0' ||
        !pmm_agent_client_by_id(id)) {
        return PMM_AGENT_EDIT_ERROR;
    }
    if (id == PMM_AGENT_CLIENT_CONTINUE) {
...
static int agent_install_callback(pmm_agent_client_id_t id, const char *config_path,
                                  const char *binary_path) {
    return pmm_agent_client_install_mcp(id, config_path, binary_path);
}

static int agent_remove_callback(pmm_agent_client_id_t id, const char *config_path,
                                 const char *binary_path) {
    return pmm_agent_client_remove_mcp(id, config_path, binary_path);
}
