...
static bool tier_valid(pmm_graph_tier_t tier) {
    return tier >= PMM_GRAPH_TIER_SCOUT && tier < PMM_GRAPH_TIER_COUNT;
}

static bool access_valid(pmm_graph_access_t access) {
    return access >= PMM_GRAPH_ACCESS_DIRECT && access < PMM_GRAPH_ACCESS_COUNT;
}

static bool dialect_valid(pmm_graph_profile_dialect_t dialect) {
    return dialect >= PMM_GRAPH_DIALECT_CLAUDE && dialect < PMM_GRAPH_DIALECT_COUNT;
}

bool pmm_graph_dialect_direct_capable(pmm_graph_profile_dialect_t dialect) {
    switch (dialect) {
    case PMM_GRAPH_DIALECT_AUGMENT:
    case PMM_GRAPH_DIALECT_CURSOR:
    case PMM_GRAPH_DIALECT_ROVO:
    case PMM_GRAPH_DIALECT_POCHI:
        return false;
    default:
...
const char *pmm_graph_tier_slug(pmm_graph_tier_t tier) {
    static const char *const slugs[PMM_GRAPH_TIER_COUNT] = {
        "project-mind-scout",
        "project-mind",
...
const char *pmm_graph_tier_display_name(pmm_graph_tier_t tier) {
    static const char *const names[PMM_GRAPH_TIER_COUNT] = {
        "Project Mind Scout",
        "Project Mind Verify",
        "Project Mind Auditor",
    };
...
static const char *profile_description(pmm_graph_tier_t tier, pmm_graph_access_t access) {
    static const char *const direct[PMM_GRAPH_TIER_COUNT] = {
    ...
    static const char *const handoff[PMM_GRAPH_TIER_COUNT] = {
    ...
    return access == PMM_GRAPH_ACCESS_DIRECT ? direct[tier] : handoff[tier];
}
...
char *pmm_render_graph_prompt(pmm_graph_tier_t tier, pmm_graph_access_t access) {
    ...
    profile_buffer_init(&buffer);
    if (access == PMM_GRAPH_ACCESS_DIRECT) {
        switch (tier) {
        case PMM_GRAPH_TIER_SCOUT:
        ...
        profile_buffer_append(
            &buffer,
            "Use project-mind-mcp in the exact graph project. Use only read-only graph and "
...
static void tier_tool_set(pmm_graph_tier_t tier, const profile_tool_t **tools, size_t *count) {
    if (tier == PMM_GRAPH_TIER_SCOUT) {
        *tools = scout_tools;
...
static const char *tier_server_profile(pmm_graph_tier_t tier) {
    return tier == PMM_GRAPH_TIER_SCOUT ? "scout" : "analysis";
}

static const char *dialect_tool_prefix(pmm_graph_profile_dialect_t dialect) {
    switch (dialect) {
    case PMM_GRAPH_DIALECT_CLAUDE:
    case PMM_GRAPH_DIALECT_QWEN:
    case PMM_GRAPH_DIALECT_QODER:
    case PMM_GRAPH_DIALECT_CODEBUDDY:
    case PMM_GRAPH_DIALECT_FACTORY:
        return "mcp__project-mind-mcp__";
    case PMM_GRAPH_DIALECT_CODEX:
        return "";
    case PMM_GRAPH_DIALECT_GEMINI:
        return "mcp_project-mind-mcp_";
    case PMM_GRAPH_DIALECT_COPILOT:
        return "project-mind-mcp/";
    case PMM_GRAPH_DIALECT_OPENCODE:
    case PMM_GRAPH_DIALECT_KILO:
    case PMM_GRAPH_DIALECT_VIBE:
        return "project-mind-mcp_";
    case PMM_GRAPH_DIALECT_KIRO:
        return "@project-mind-mcp/";
    default:
        return NULL;
    }
...
static bool tool_identifier(pmm_graph_profile_dialect_t dialect, profile_tool_t tool, char *output,
                            size_t output_size) {
...
static bool append_yaml_mcp_tools(profile_buffer_t *buffer, pmm_graph_profile_dialect_t dialect,
                                  pmm_graph_tier_t tier) {
...
static bool append_csv_mcp_tools(profile_buffer_t *buffer, pmm_graph_profile_dialect_t dialect,
                                 pmm_graph_tier_t tier) {
    const profile_tool_t *tools = NULL;
    size_t count = 0U;
    tier_tool_set(tier, &tools, &count);
...
static bool append_toml_mcp_tools(profile_buffer_t *buffer, pmm_graph_profile_dialect_t dialect,
                                  pmm_graph_tier_t tier, bool leading_items) {
    const profile_tool_t *tools = NULL;
    size_t count = 0U;
    tier_tool_set(tier, &tools, &count);
    for (size_t i = 0U; i < count; i++) {
...
static char *render_kiro_profile(pmm_graph_tier_t tier, pmm_graph_access_t access,
                                 const char *binary_path, const char *prompt) {
    if (access == PMM_GRAPH_ACCESS_DIRECT && (!binary_path || !binary_path[0])) {
        return NULL;
    }
    ...
    if (ok && access == PMM_GRAPH_ACCESS_DIRECT) {
        const profile_tool_t *tier_tools = NULL;
        size_t count = 0U;
        tier_tool_set(tier, &tier_tools, &count);
        for (size_t i = 0U; ok && i < count; i++) {
            char identifier[160];
            ok = tool_identifier(PMM_GRAPH_DIALECT_KIRO, tier_tools[i], identifier,
                                 sizeof(identifier)) &&
...
static bool render_profile_text(profile_buffer_t *buffer, pmm_graph_profile_dialect_t dialect,
                                pmm_graph_tier_t tier, pmm_graph_access_t access,
                                const char *prompt) {
    const char *slug = pmm_graph_tier_slug(tier);
    const char *display = pmm_graph_tier_display_name(tier);
    const char *description = profile_description(tier, access);
    bool direct = access == PMM_GRAPH_ACCESS_DIRECT;
    switch (dialect) {
    case PMM_GRAPH_DIALECT_CLAUDE:
    ...
    case PMM_GRAPH_DIALECT_KILO:
        ...
        return true;
    case PMM_GRAPH_DIALECT_JUNIE:
        if (!profile_buffer_append(buffer, "---\nname: \"") ||
            !profile_buffer_append(buffer, slug) ||
            !profile_buffer_append(buffer, "\"\ndescription: \"") ||
            !profile_buffer_append(buffer, description) ||
            !profile_buffer_append(buffer, "\"\ntools: [\"Read\", \"Grep\", \"Glob\"]\n") ||
            (direct && !profile_buffer_append(buffer, "mcpServers: [\"project-mind-")) ||
...
char *pmm_render_graph_profile(pmm_graph_profile_dialect_t dialect, pmm_graph_tier_t tier,
    ...
    if (!dialect_valid(dialect) || !tier_valid(tier) || !access_valid(access) ||
        (access == PMM_GRAPH_ACCESS_DIRECT && !pmm_graph_dialect_direct_capable(dialect))) {
        return NULL;
    }
    char *prompt = pmm_render_graph_prompt(tier, access);
    if (!prompt) {
        return NULL;
    }
    if (dialect == PMM_GRAPH_DIALECT_KIRO) {
