...
static PMM_TLS pmm_toml_precommit_test_hook_t toml_precommit_test_hook = NULL;
static PMM_TLS void *toml_precommit_test_context = NULL;
static PMM_TLS pmm_toml_precommit_test_hook_t toml_prepublish_test_hook = NULL;
static PMM_TLS void *toml_prepublish_test_context = NULL;
#endif
...
#endif
    if (!failed && TOML_SYNC(pmm_fileno(file)) != 0) {
        failed = 1;
    }
    if (fclose(file) != 0) {
        failed = 1;
...
static int toml_edit_owned_named_array_table(const char *file_path, const char *table_name,
    ...
    if (!toml_named_inputs_valid(file_path, table_name, identity_key, identity_value) ||
        ...
        return PMM_TOML_OWNED_EDIT_ERROR;
    }
    toml_body_spec_t spec;
    ...
    if (scan.matching_count == 1) {
        ...
        if (!is_canonical) {
            free(existing);
            return PMM_TOML_OWNED_EDIT_FOREIGN;
        }
        if (!remove) {
            free(existing);
            return PMM_TOML_OWNED_EDIT_OK;
        }
    } else if (remove) {
        free(existing);
        return PMM_TOML_OWNED_EDIT_OK;
    ...
    toml_buffer_dispose(&output);
    free(existing);
    return result == TOML_EDIT_OK ? PMM_TOML_OWNED_EDIT_OK : PMM_TOML_OWNED_EDIT_ERROR;
}
...
static int toml_legacy_command_is_owned(const char *data, const toml_assignment_t *assignment,
    ...
    *owned = 0;
    ...
    static const char binary_name[] = "project-mind-mcp";
    static const char windows_binary_name[] = "project-mind-mcp.exe";
