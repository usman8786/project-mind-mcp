 ...
 *
 * Port of Go cmd/project-mind-mcp/ install/update logic.
...
#include "mcp/mcp.h" // pmm_mcp_tool_input_schema — CLI flag parser + per-tool --help
...
static int pmm_shell_quote_word(const char *value, char *out, size_t out_size);
static int pmm_powershell_quote_word(const char *value, char *out, size_t out_size);
...
static void (*pmm_sqlite_transient_fn(void))(void *) {
    uintptr_t bits = (uintptr_t)CLI_ERR;
    void (*fp)(void *) = NULL;
    memcpy(&fp, &bits, sizeof(fp));
    return fp;
}
#define pmm_sqlite_transient (pmm_sqlite_transient_fn())
...
#define DECOMPRESS_MAX_BYTES ((size_t)500 * CLI_BUF_1K * PMM_SZ_1K)
...
static const char CLI_ACTIVATION_REFUSED_MESSAGE[] =
    "error: active PMM sessions and operations could not be stopped safely; "
...
static const char CLI_ACTIVATION_MUTATION_FAILED_MESSAGE[] =
    "error: activation failed while PMM sessions were stopped; filesystem "
...
typedef struct {
    ...
    pmm_version_cohort_lease_t *cohort_lease;
    pmm_daemon_ipc_startup_lock_t *startup_lock;
    pmm_daemon_build_identity_t identity;
    char source_build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];
    char canonical_cache[CLI_BUF_4K];
    char cache_fingerprint[PMM_SHA256_HEX_LEN + 1];
    char *original_cache_environment;
    pmm_daemon_runtime_activation_action_t action;
    pmm_daemon_runtime_activation_result_t daemon_result;
...
static pmm_cli_activation_ops_t g_cli_activation_test_ops;
static bool g_cli_activation_test_ops_set = false;
static const char *g_cli_activation_runtime_parent_for_test = NULL;

static void cli_activation_diagnostic(const pmm_cli_activation_ops_t *ops, const char *message) {
...
int pmm_cli_activation_guard_with_ops(const pmm_cli_activation_ops_t *ops,
    ...
    pmm_cli_activation_lock_t mutation_lease = NULL;
...
bool pmm_cli_activation_test_ops_installed(void) {
...
static const char *cli_activation_action_text(pmm_daemon_runtime_activation_action_t action) {
    switch (action) {
    case PMM_DAEMON_RUNTIME_ACTIVATION_INSTALL:
        return "install";
    case PMM_DAEMON_RUNTIME_ACTIVATION_UPDATE:
        return "update";
    case PMM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL:
        return "uninstall";
    default:
        return "activation";
    }
...
static bool cli_activation_log_event(cli_activation_production_context_t *context,
    ...
    bool encoded =
        yyjson_mut_obj_add_str(document, root, "event", "pmm.activation") &&
        ...
        yyjson_mut_obj_add_str(document, root, "source_version", PMM_VERSION) &&
    ...
    if (written) {
        int descriptor = pmm_fileno(context->activation_log);
#ifdef _WIN32
...
static int cli_activation_startup_lock_acquire(cli_activation_production_context_t *context) {
    ...
    do {
        pmm_daemon_ipc_startup_lock_t *lock = NULL;
        int status = pmm_daemon_ipc_startup_lock_try_acquire(context->endpoint, &lock);
        if (status == 1 && lock) {
        ...
        pmm_usleep(CLI_ACTIVATION_RETRY_US);
    } while (pmm_now_ms() < context->control_deadline_ms);
    return 0;
}
...
static void cli_activation_startup_lock_release_complete(
    ...
    while (context && context->startup_lock) {
        (void)pmm_daemon_ipc_startup_lock_release(&context->startup_lock);
        if (!context->startup_lock) {
            return;
        }
...
static void cli_activation_merge_daemon_result(
    cli_activation_production_context_t *context,
    const pmm_daemon_runtime_activation_result_t *result) {
...
static pmm_version_cohort_quiesce_result_t cli_activation_request_quiescence(void *opaque) {
    cli_activation_production_context_t *context = opaque;
    if (!context || !context->endpoint) {
        return PMM_VERSION_COHORT_QUIESCE_ERROR;
    }
    ...
    pmm_daemon_runtime_activation_result_t result = {0};
    if (pmm_daemon_runtime_request_activation_shutdown(
    ...
    context->shutdown_requested = true;
    return PMM_VERSION_COHORT_QUIESCE_REQUESTED;
}
...
static void cli_activation_release_cleanup_lease(cli_activation_production_context_t *context,
    ...
    while (lease_io && *lease_io && pmm_now_ms() < cleanup_deadline) {
        if (pmm_version_cohort_lease_release(lease_io) == PMM_PRIVATE_FILE_LOCK_OK) {
            return;
        }
        pmm_usleep(CLI_ACTIVATION_RETRY_US);
    }
...
static int cli_activation_production_reserve(void *opaque, pmm_cli_activation_lock_t *lease_out) {
    cli_activation_production_context_t *context = opaque;
    if (lease_out) {
        *lease_out = NULL;
    }
    ...
    pmm_version_cohort_quiesce_result_t quiesce = PMM_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    pmm_version_cohort_lease_t *lease = NULL;
    ...
    pmm_daemon_runtime_activation_result_t eager_result = {0};
    if (pmm_daemon_runtime_request_activation_shutdown(
            context->endpoint, &context->identity, context->action,
    ...
    int generation = pmm_daemon_ipc_generation_probe_under_startup_lock(context->endpoint,
...
static void cli_activation_production_release(void *opaque, pmm_cli_activation_lock_t lease) {
    cli_activation_production_context_t *context = opaque;
    if (!context) {
        return;
    }
    ...
    pmm_version_cohort_lease_t *cohort_lease = (pmm_version_cohort_lease_t *)lease;
...
static bool cli_activation_production_context_init(cli_activation_production_context_t *context,
    ...
    context->control_deadline_ms = cli_activation_deadline_after(CLI_ACTIVATION_CONTROL_TIMEOUT_MS);
    const char *original_cache_environment = getenv("PMM_CACHE_DIR");
    ...
    const char *requested_cache = pmm_resolve_cache_dir();
    bool cache_ready = requested_cache && requested_cache[0] &&
                       pmm_canonical_path(requested_cache, context->canonical_cache,
                                          sizeof(context->canonical_cache));
    if (!cache_ready && requested_cache && requested_cache[0] &&
        pmm_mkdir_p(requested_cache, 0700)) {
        cache_ready = pmm_canonical_path(requested_cache, context->canonical_cache,
                                         sizeof(context->canonical_cache));
    }
    if (!cache_ready || !pmm_is_dir(context->canonical_cache) ||
        !pmm_daemon_ipc_private_directory_secure(context->canonical_cache)) {
        return false;
    }
    pmm_normalize_path_sep(context->canonical_cache);
    if (pmm_setenv("PMM_CACHE_DIR", context->canonical_cache, 1) != 0) {
        return false;
    }
    context->cache_environment_overridden = true;
    pmm_sha256_hex(context->canonical_cache, strlen(context->canonical_cache),
                   context->cache_fingerprint);
    context->endpoint = pmm_daemon_bootstrap_endpoint_new(g_cli_activation_runtime_parent_for_test);
    context->cohort_manager =
        context->endpoint ? pmm_version_cohort_manager_new(context->endpoint) : NULL;
    const char *captured_build = pmm_index_supervisor_build_fingerprint();
    bool build_ready = captured_build && captured_build[0]
                           ? snprintf(context->source_build, sizeof(context->source_build), "%s",
                                      captured_build) > 0
                           : pmm_daemon_runtime_process_build_fingerprint(
    ...
    context->identity = (pmm_daemon_build_identity_t){
    ...
    context->activation_log = pmm_daemon_ipc_private_log_open(log_dir, "activation-events.ndjson",
...
static void cli_activation_production_context_close(cli_activation_production_context_t *context) {
    ...
    while (context->cohort_manager && pmm_now_ms() < manager_deadline) {
        if (pmm_version_cohort_manager_free(&context->cohort_manager) == PMM_PRIVATE_FILE_LOCK_OK) {
            break;
        }
        pmm_usleep(CLI_ACTIVATION_RETRY_US);
    }
    ...
    pmm_daemon_ipc_endpoint_free(context->endpoint);
    context->endpoint = NULL;
    if (context->cache_environment_overridden) {
        int restore_result =
            context->original_cache_environment_present
                ? pmm_setenv("PMM_CACHE_DIR", context->original_cache_environment, 1)
...
static int cli_activation_guard(pmm_daemon_runtime_activation_action_t action,
                                const char *target_version, const char *target_build,
                                pmm_cli_activation_mutation_fn mutation, void *mutation_context) {
    if (g_cli_activation_test_ops_set) {
        return pmm_cli_activation_guard_with_ops(&g_cli_activation_test_ops, mutation,
                                                 mutation_context);
    }
    ...
    printf("Stopping active PMM sessions and operations for %s...\n",
    ...
    int rc = pmm_cli_activation_guard_with_ops(&ops, mutation, mutation_context);
    if (rc == CLI_OK) {
...
#define TAR_BINARY_NAME "project-mind-mcp"
#define TAR_BINARY_NAME_LEN 19
#define TAR_BLOCK_SIZE PMM_SZ_512 /* tar record alignment */
...
const char *pmm_detect_shell_rc(const char *home_dir) {
    ...
    const char *shell = pmm_safe_getenv("SHELL", shell_buf, sizeof(shell_buf), "");
    if (!shell) {
        shell = "";
    }
...
static bool find_in_path(const char *name, char *out, size_t out_sz) {
    ...
    if (!pmm_safe_getenv("PATH", path_copy, sizeof(path_copy), NULL)) {
        return false;
    }
    char *saveptr;
...
static bool pmm_agent_cli_exists(const char *name, const char *home_dir) {
    const char *actual_home = pmm_get_home_dir();
    if (actual_home && home_dir && strcmp(actual_home, home_dir) == 0) {
        return pmm_find_cli(name, home_dir)[0] != '\0';
    }
 ...
 * copying the running binary onto itself during install (pmm_copy_file would
 * truncate it, since it opens the destination "wb" before reading the source). */
static bool pmm_same_file(const char *a, const char *b) {
    struct stat sa;
    struct stat sb;
    ...
    pmm_normalize_path_sep(na);
    pmm_normalize_path_sep(nb);
...
typedef struct {
    char fingerprint[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];
...
static bool cli_binary_fingerprint_validator(const char *target_path, void *opaque) {
    ...
    char actual[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];
    return validator && target_path && pmm_daemon_build_fingerprint_file(target_path, actual) &&
           strcmp(actual, validator->fingerprint) == 0;
}

static int cli_activation_transaction_abort(pmm_activation_transaction_t **transaction_io) {
    if (!transaction_io || !*transaction_io) {
        return CLI_OK;
    }
    return pmm_activation_transaction_close(transaction_io) == PMM_ACTIVATION_TRANSACTION_OK
               ? CLI_OK
               : CLI_ERR;
}
...
void pmm_cli_set_activation_cleanup_failure_for_test(bool enabled);
int pmm_cli_activation_abort_cleanup_probe_for_test(void);
#endif

static void cli_activation_transaction_abort_or_fail_stop(
    pmm_activation_transaction_t **transaction_io, const char *component) {
...
int pmm_cli_activation_abort_cleanup_probe_for_test(void) {
    pmm_activation_transaction_t *transaction = NULL;
...
static int cli_activation_transaction_commit_validated(pmm_activation_transaction_t *transaction,
                                                       const cli_binary_validator_t *validator,
                                                       int mode) {
    if (!transaction) {
        return CLI_ERR;
    }
    pmm_activation_transaction_status_t status = pmm_activation_transaction_commit(
        transaction, validator ? cli_binary_fingerprint_validator : NULL, (void *)validator);
    if (status != PMM_ACTIVATION_TRANSACTION_OK) {
        return CLI_ERR;
    }
#ifndef _WIN32
    const char *target_path = pmm_activation_transaction_target_path(transaction);
    if (!target_path || chmod(target_path, (mode_t)mode) != 0) {
        (void)pmm_activation_transaction_rollback(transaction);
        return CLI_ERR;
    }
#else
    (void)mode;
...
static int cli_activation_transaction_finalize_close(
    ...
    pmm_activation_transaction_status_t status =
        pmm_activation_transaction_finalize(*transaction_io);
    if (status != PMM_ACTIVATION_TRANSACTION_OK && status != PMM_ACTIVATION_TRANSACTION_DEFERRED) {
        (void)cli_activation_transaction_abort(transaction_io);
        return CLI_ERR;
    }
    if (status == PMM_ACTIVATION_TRANSACTION_DEFERRED) {
        const char *deferred = pmm_activation_transaction_deferred_path(*transaction_io);
        pmm_log_warn("cli.activation_backup_cleanup_deferred", "path",
...
static int cli_activation_transaction_commit_removal(pmm_activation_transaction_t *transaction) {
    return transaction && pmm_activation_transaction_commit(transaction, NULL, NULL) ==
                              PMM_ACTIVATION_TRANSACTION_OK
               ? CLI_OK
               : CLI_ERR;
}

static bool cli_activation_transaction_expected_build(pmm_activation_transaction_t *transaction,
                                                      cli_binary_validator_t *validator) {
    const char *staged = pmm_activation_transaction_staged_path(transaction);
    return staged && validator && pmm_daemon_build_fingerprint_file(staged, validator->fingerprint);
}
...
int pmm_copy_binary_to_target(const char *src, const char *dst) {
    ...
    if (pmm_same_file(src, dst)) {
        return CLI_OK;
    }
    pmm_activation_transaction_t *transaction = NULL;
    pmm_activation_transaction_status_t status =
        pmm_activation_transaction_stage_file(dst, src, &transaction);
    cli_binary_validator_t validator;
    if (status != PMM_ACTIVATION_TRANSACTION_OK || !transaction ||
...
int pmm_replace_binary(const char *path, const unsigned char *data, int len, int mode) {
    ...
    pmm_activation_transaction_t *transaction = NULL;
    pmm_activation_transaction_status_t status =
        pmm_activation_transaction_stage_bytes(path, data, (size_t)len, &transaction);
    cli_binary_validator_t validator;
    if (status != PMM_ACTIVATION_TRANSACTION_OK || !transaction ||
        !cli_activation_transaction_expected_build(transaction, &validator)) {
        (void)cli_activation_transaction_abort(&transaction);
...
static const char skill_content[] =
    ...
    "---\n"
    "\n"
    "# Project Mind — Knowledge Graph Tools\n"
    "\n"
    ...
    "| Project specs / docs | `search_project_docs(query=\"...\")` |\n"
    "| Attach a local spec | `attach_project_doc(path=\"/abs/path\")` |\n"
    "\n"
    "## Exploration Workflow\n"
    "1. `list_projects` — check if project is indexed\n"
    "2. `search_project_docs` — pull requirements/API contracts when implementing features\n"
...
static const char codex_instructions_content[] =
    ...
    "\n"
    "This project uses project-mind-mcp to maintain a knowledge graph of the codebase.\n"
...
static const char *old_skill_names[] = {
    "project-mind-exploring",
    "project-mind-tracing",
    "project-mind-quality",
    "project-mind-reference",
};
enum { OLD_SKILL_COUNT = 4 };

static const pmm_skill_t skills[PMM_SKILL_COUNT] = {
    {"project-mind", skill_content},
};
...
static int mkdirp(const char *path, int mode) {
    return (int)pmm_mkdir_p(path, mode) ? 0 : CLI_ERR;
}
 ...
 * directory symlink; on Windows pmm_rmdir removes only an empty directory or
 * the directory link itself and never traverses its target. */
static bool pmm_remove_empty_directory(const char *path, bool dry_run) {
    struct stat state;
#ifndef _WIN32
    ...
    pmm_dir_t *directory = pmm_opendir(path);
    if (!directory) {
        return false;
    }
    bool empty = true;
    pmm_dirent_t *entry;
    while ((entry = pmm_readdir(directory)) != NULL) {
        if (strcmp(entry->name, ".") != 0 && strcmp(entry->name, "..") != 0) {
            empty = false;
            break;
        }
    }
    pmm_closedir(directory);
    return empty && (dry_run || pmm_rmdir(path) == 0);
}
...
int pmm_install_skills(const char *skills_dir, bool force, bool dry_run) {
    ...
    for (int i = 0; i < OLD_SKILL_COUNT; i++) {
        ...
        (void)pmm_remove_empty_directory(old_path, dry_run);
    }

    for (int i = 0; i < PMM_SKILL_COUNT; i++) {
        ...
        if (pmm_text_write_owned_document(file_path, skills[i].content) == 0) {
            count++;
        }
    }
    return count;
...
int pmm_remove_skills(const char *skills_dir, bool dry_run) {
    ...
    for (int i = 0; i < PMM_SKILL_COUNT; i++) {
        ...
        if (pmm_text_remove_owned_document(file_path, skills[i].content) == 0) {
            (void)pmm_rmdir(skill_path); /* only succeeds when no user files remain */
            count++;
        }
    }
    return count;
...
bool pmm_remove_old_monolithic_skill(const char *skills_dir, bool dry_run) {
    ...
    char old_path[CLI_BUF_1K];
    snprintf(old_path, sizeof(old_path), "%s/project-mind-mcp", skills_dir);
    return pmm_remove_empty_directory(old_path, dry_run);
}
...
typedef enum {
    PMM_JSON_MCP_STANDARD,
    PMM_JSON_MCP_OPENCLAW,
    PMM_JSON_MCP_VSCODE,
    PMM_JSON_MCP_LOCAL_ARRAY,
    PMM_JSON_MCP_CLINE,
    PMM_JSON_MCP_COPILOT,
    PMM_JSON_MCP_FACTORY,
    PMM_JSON_MCP_CRUSH,
} pmm_json_mcp_schema_t;

static bool pmm_json_mcp_command_is_array(pmm_json_mcp_schema_t schema) {
    return schema == PMM_JSON_MCP_LOCAL_ARRAY;
}

static const char *pmm_json_mcp_required_type(pmm_json_mcp_schema_t schema) {
    switch (schema) {
    case PMM_JSON_MCP_VSCODE:
    case PMM_JSON_MCP_FACTORY:
    case PMM_JSON_MCP_CRUSH:
        return "stdio";
    case PMM_JSON_MCP_LOCAL_ARRAY:
    case PMM_JSON_MCP_COPILOT:
        return "local";
    case PMM_JSON_MCP_STANDARD:
    case PMM_JSON_MCP_OPENCLAW:
    case PMM_JSON_MCP_CLINE:
        return NULL;
    }
    return NULL;
}

static const char PMM_DEFAULT_MCP_SERVER_NAME[] = "project-mind-mcp";
static const char PMM_ANALYSIS_MCP_SERVER_NAME[] = "project-mind-analysis";
static const char PMM_SCOUT_MCP_SERVER_NAME[] = "project-mind-scout";
static const char PMM_ANALYSIS_PROFILE_ARGUMENT[] = "--tool-profile=analysis";
static const char PMM_SCOUT_PROFILE_ARGUMENT[] = "--tool-profile=scout";

static char *pmm_build_json_mcp_entry(const char *binary_path, pmm_json_mcp_schema_t schema,
                                      const char *argument) {
    ...
    bool command_is_array = pmm_json_mcp_command_is_array(schema);
    ...
    const char *type = pmm_json_mcp_required_type(schema);
    if (ok && type) {
...
static size_t pmm_json_mcp_ownership_fields(pmm_json_mcp_schema_t schema, const char *argument,
                                            pmm_json_like_object_field_t fields[3]) {
    fields[0] = (pmm_json_like_object_field_t){
        .key = "command",
        .shape = pmm_json_mcp_command_is_array(schema) ? PMM_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY
                                                       : PMM_JSON_LIKE_VALUE_STRING,
        .expected_string = NULL,
        .flags = PMM_JSON_LIKE_FIELD_REQUIRED | PMM_JSON_LIKE_FIELD_CAPTURE_STRING,
    };
    fields[1] = (pmm_json_like_object_field_t){
        .key = "args",
        .shape =
            argument ? PMM_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY : PMM_JSON_LIKE_VALUE_EMPTY_ARRAY,
        .expected_string = argument,
        .flags = argument ? PMM_JSON_LIKE_FIELD_REQUIRED : 0U,
    };
    const char *type = pmm_json_mcp_required_type(schema);
    if (!type) {
        return 2U;
    }
    fields[2] = (pmm_json_like_object_field_t){
        .key = "type",
        .shape = PMM_JSON_LIKE_VALUE_STRING,
        .expected_string = type,
        .flags = PMM_JSON_LIKE_FIELD_REQUIRED,
    };
    return 3U;
}
...
static PMM_TLS const char *g_previous_managed_mcp_command = NULL;

static bool pmm_json_mcp_owned_command(const char *command, const char *expected_binary,
    ...
    return strcmp(command, "project-mind-mcp") == 0 || strcmp(command, "project-mind-mcp.exe") == 0;
}
...
enum { PMM_JSON_MCP_OWNERSHIP_STALE = 100 };

#ifdef _WIN32
typedef enum {
    PMM_JSON_MCP_COMMAND_UNKNOWN = 0,
    PMM_JSON_MCP_COMMAND_PRESENT,
    PMM_JSON_MCP_COMMAND_MISSING,
} pmm_json_mcp_command_availability_t;
#endif

#if defined(_WIN32) || defined(PMM_CLI_ENABLE_TEST_API)
static bool pmm_json_mcp_command_path_probe_safe_for_platform(const char *command, bool windows) {
...
#ifdef _WIN32
static bool pmm_json_mcp_command_path_probe_safe(const char *command) {
    return pmm_json_mcp_command_path_probe_safe_for_platform(command, true);
}
#endif

#ifdef PMM_CLI_ENABLE_TEST_API
static PMM_TLS int *g_mcp_command_path_probe_counter = NULL;

bool pmm_mcp_command_path_probe_safe_for_testing(const char *command, bool windows) {
    return pmm_json_mcp_command_path_probe_safe_for_platform(command, windows);
}
...
#ifdef _WIN32
static bool pmm_json_mcp_windows_path_character_forbidden(wchar_t character) {
    return character < L' ' || character == L':' || character == L'*' || character == L'?' ||
           character == L'"' || character == L'<' || character == L'>' || character == L'|';
}

static bool pmm_json_mcp_windows_path_syntax_valid(const wchar_t *path) {
    ...
    for (size_t index = component_start; index <= length; index++) {
        if (index < length && path[index] != L'\\') {
            if (pmm_json_mcp_windows_path_character_forbidden(path[index])) {
                return false;
            }
            continue;
        }
...
static wchar_t *pmm_json_mcp_windows_path_from_utf8(const char *path) {
    ...
    if (!pmm_json_mcp_windows_path_syntax_valid(wide)) {
        free(wide);
        return NULL;
    }
    return wide;
...
static bool pmm_json_mcp_windows_handle_is_local(HANDLE handle, wchar_t expected_drive,
...
static pmm_json_mcp_command_availability_t pmm_json_mcp_probe_windows_command_path(
    const char *path) {
    wchar_t *wide = pmm_json_mcp_windows_path_from_utf8(path);
    if (!wide) {
        return PMM_JSON_MCP_COMMAND_UNKNOWN;
    }
    ...
    if (length == 3U) {
        free(wide);
        return PMM_JSON_MCP_COMMAND_PRESENT;
    }
    ...
    pmm_json_mcp_command_availability_t availability = PMM_JSON_MCP_COMMAND_UNKNOWN;
    ...
    if (!pmm_json_mcp_windows_handle_is_local(root_handle, wide[0], &root_information) ||
    ...
    for (size_t index = 3U; index <= length; index++) {
        ...
        BY_HANDLE_FILE_INFORMATION information;
        bool local = pmm_json_mcp_windows_handle_is_local(component, wide[0], &information);
        if (local) {
        ...
        if (error == ERROR_FILE_NOT_FOUND || error == ERROR_PATH_NOT_FOUND) {
            availability = PMM_JSON_MCP_COMMAND_MISSING;
            break;
        }
        if (error != ERROR_SUCCESS || !local ||
            (!final_component && (information.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0)) {
            availability = PMM_JSON_MCP_COMMAND_UNKNOWN;
            break;
        }
        if (final_component) {
            availability = PMM_JSON_MCP_COMMAND_PRESENT;
            break;
        }
    }
...
#ifdef _WIN32
static pmm_json_mcp_command_availability_t pmm_json_mcp_probe_command_path(const char *path) {
#ifdef PMM_CLI_ENABLE_TEST_API
    if (g_mcp_command_path_probe_counter) {
        (*g_mcp_command_path_probe_counter)++;
    }
#endif
    return pmm_json_mcp_probe_windows_command_path(path);
}
...
static pmm_json_mcp_command_availability_t pmm_json_mcp_command_availability(const char *command) {
    if (!pmm_json_mcp_command_path_probe_safe(command) || strchr(command, '$') ||
        strchr(command, '%')) {
        return PMM_JSON_MCP_COMMAND_UNKNOWN;
    }
    pmm_json_mcp_command_availability_t availability = pmm_json_mcp_probe_command_path(command);
    if (availability != PMM_JSON_MCP_COMMAND_MISSING) {
        return availability;
    }
    ...
    if (!basename[0] || strchr(basename, '.')) {
        return PMM_JSON_MCP_COMMAND_MISSING;
    }
...
static int pmm_json_mcp_snapshot_ownership(const char *document, size_t document_length,
                                           const char *const *object_path, size_t path_len,
                                           pmm_json_mcp_schema_t schema, const char *entry_name,
                                           const char *argument, const char *expected_binary,
                                           const char *previous_managed_binary) {
    pmm_json_like_object_field_t fields[3];
    size_t field_count = pmm_json_mcp_ownership_fields(schema, argument, fields);
    char *command = NULL;
    int result = pmm_json_like_match_object_entry(document, document_length, object_path, path_len,
                                                  entry_name, fields, field_count, &command);
    if (result == PMM_JSON_LIKE_OBJECT_MATCH &&
        !pmm_json_mcp_owned_command(command, expected_binary, previous_managed_binary)) {
#ifdef _WIN32
        result = pmm_json_mcp_command_availability(command) == PMM_JSON_MCP_COMMAND_MISSING
                     ? PMM_JSON_MCP_OWNERSHIP_STALE
                     : PMM_JSON_LIKE_OBJECT_MISMATCH;
#else
        result = PMM_JSON_LIKE_OBJECT_MISMATCH;
#endif
    }
    free(command);
    return result;
...
static int pmm_upsert_json_named_mcp(const char *binary_path, const char *config_path,
                                     const char *const *object_path, size_t path_len,
                                     pmm_json_mcp_schema_t schema, const char *entry_name,
                                     const char *argument) {
    ...
    int read_result = pmm_json_like_read_document(config_path, &document, &document_length);
    if (read_result < 0) {
        return CLI_ERR;
    }
    if (read_result == 0) {
        int ownership = pmm_json_mcp_snapshot_ownership(
        ...
        if (ownership != PMM_JSON_LIKE_OBJECT_MATCH && ownership != PMM_JSON_LIKE_OBJECT_MISSING &&
            ownership != PMM_JSON_MCP_OWNERSHIP_STALE) {
            free(document);
            return CLI_ERR;
        }
    }

    char *entry = pmm_build_json_mcp_entry(binary_path, schema, argument);
    if (!entry) {
        free(document);
        return CLI_ERR;
    }
    int edit_result = pmm_json_like_upsert_entry_if_unchanged(
...
static int pmm_upsert_json_mcp(const char *binary_path, const char *config_path,
                               const char *const *object_path, size_t path_len,
                               pmm_json_mcp_schema_t schema) {
    return pmm_upsert_json_named_mcp(binary_path, config_path, object_path, path_len, schema,
                                     PMM_DEFAULT_MCP_SERVER_NAME, NULL);
}

static int pmm_remove_json_named_mcp(const char *config_path, const char *const *object_path,
                                     size_t path_len, pmm_json_mcp_schema_t schema,
    ...
    int ownership =
        pmm_json_mcp_snapshot_ownership(document, document_length, object_path, path_len, schema,
                                        entry_name, argument, expected_binary, NULL);
    if (ownership == PMM_JSON_LIKE_OBJECT_MISSING || ownership == PMM_JSON_LIKE_OBJECT_MISMATCH ||
        ownership == PMM_JSON_MCP_OWNERSHIP_STALE) {
        free(document);
        return CLI_OK;
    }
    if (ownership != PMM_JSON_LIKE_OBJECT_MATCH) {
        free(document);
        return CLI_ERR;
    }
    int edit_result = pmm_json_like_remove_entry_if_unchanged(
        config_path, object_path, path_len, entry_name, document, document_length);
    free(document);
    return edit_result == 0 ? CLI_OK : CLI_ERR;
}

static int pmm_remove_json_mcp(const char *config_path, const char *const *object_path,
                               size_t path_len, pmm_json_mcp_schema_t schema,
                               const char *expected_binary) {
    return pmm_remove_json_named_mcp(config_path, object_path, path_len, schema,
                                     PMM_DEFAULT_MCP_SERVER_NAME, NULL, expected_binary);
}
...
int pmm_install_editor_mcp(const char *binary_path, const char *config_path) {
    ...
    return pmm_upsert_json_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_STANDARD);
}
...
int pmm_install_editor_mcp_with_previous_for_testing(const char *binary_path,
    ...
    int result = pmm_install_editor_mcp(binary_path, config_path);
...
int pmm_remove_editor_mcp(const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_STANDARD, NULL);
}

int pmm_remove_editor_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_STANDARD, binary_path);
}
...
int pmm_install_openclaw_mcp(const char *binary_path, const char *config_path) {
    ...
    return pmm_upsert_json_mcp(binary_path, config_path, path, 2U, PMM_JSON_MCP_OPENCLAW);
}

int pmm_remove_openclaw_mcp(const char *config_path) {
    static const char *const path[] = {"mcp", "servers"};
    return pmm_remove_json_mcp(config_path, path, 2U, PMM_JSON_MCP_OPENCLAW, NULL);
}

int pmm_remove_openclaw_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp", "servers"};
    return pmm_remove_json_mcp(config_path, path, 2U, PMM_JSON_MCP_OPENCLAW, binary_path);
}

static const char pmm_openclaw_compaction_section[] = "Codebase Knowledge Graph (project-mind-mcp)";

static int pmm_upsert_openclaw_compaction(const char *config_path) {
    static const char *const path[] = {"agents", "defaults", "compaction"};
    return pmm_json_like_add_unique_string_at_path(config_path, path, 3U, "postCompactionSections",
                                                   pmm_openclaw_compaction_section) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int pmm_remove_openclaw_compaction(const char *config_path) {
    static const char *const path[] = {"agents", "defaults", "compaction"};
    return pmm_json_like_remove_string_at_path(config_path, path, 3U, "postCompactionSections",
                                               pmm_openclaw_compaction_section) == 0
               ? CLI_OK
               : CLI_ERR;
}
...
int pmm_install_vscode_mcp(const char *binary_path, const char *config_path) {
    ...
    return pmm_upsert_json_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_VSCODE);
}

int pmm_remove_vscode_mcp(const char *config_path) {
    static const char *const path[] = {"servers"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_VSCODE, NULL);
}

int pmm_remove_vscode_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"servers"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_VSCODE, binary_path);
}
...
static void pmm_claude_config_dir(const char *home_dir, char *out, size_t out_sz) {
    if (out_sz == 0) {
        return;
    }
    out[0] = '\0';
    char env_buf[CLI_BUF_1K];
    const char *env = pmm_safe_getenv("CLAUDE_CONFIG_DIR", env_buf, sizeof(env_buf), NULL);
    if (env && env[0]) {
...
static void pmm_claude_user_root(const char *home_dir, char *out, size_t out_sz) {
    if (out_sz == 0) {
        return;
    }
    out[0] = '\0';
...
static void pmm_codex_config_dir(const char *home_dir, char *out, size_t out_sz) {
    if (out_sz == 0) {
        return;
    }
    out[0] = '\0';
    char env_buf[CLI_BUF_1K];
    const char *env = pmm_safe_getenv("CODEX_HOME", env_buf, sizeof(env_buf), NULL);
    if (env && env[0]) {
        snprintf(out, out_sz, "%s", env);
    } else if (home_dir && home_dir[0]) {
...
static void pmm_zed_config_dir(const char *home_dir, char *out, size_t out_sz) {
    if (out_sz == 0) {
        return;
    }
    out[0] = '\0';
...
#else
    char env_buf[CLI_BUF_1K];
    const char *xdg = pmm_safe_getenv("XDG_CONFIG_HOME", env_buf, sizeof(env_buf), NULL);
    if (xdg && xdg[0]) {
...
static void pmm_zed_instructions_path(const char *home_dir, char *out, size_t out_sz) {
#ifdef _WIN32
...
static bool pmm_expand_user_path(const char *home_dir, const char *value, char *out,
                                 size_t out_sz) {
...
static void pmm_env_home_dir(const char *env_name, const char *home_dir, const char *fallback,
                             char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = pmm_safe_getenv(env_name, env_buf, sizeof(env_buf), NULL);
    out[0] = '\0';
    if (custom && custom[0] && pmm_expand_user_path(home_dir, custom, out, out_sz)) {
        return;
    }
...
static void pmm_kiro_home_dir(const char *home_dir, char *out, size_t out_sz) {
    pmm_env_home_dir("KIRO_HOME", home_dir, ".kiro", out, out_sz);
}

static void pmm_hermes_home_dir(const char *home_dir, char *out, size_t out_sz) {
    pmm_env_home_dir("HERMES_HOME", home_dir, ".hermes", out, out_sz);
}

static void pmm_qwen_home_dir(const char *home_dir, char *out, size_t out_sz) {
    pmm_env_home_dir("QWEN_HOME", home_dir, ".qwen", out, out_sz);
}

static void pmm_cline_root_dir(const char *home_dir, char *out, size_t out_sz) {
...
static void pmm_cline_data_dir(const char *home_dir, char *out, size_t out_sz) {
    pmm_env_home_dir("CLINE_DATA_DIR", home_dir, ".cline/data", out, out_sz);
}

static bool pmm_openclaw_internal_home(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = pmm_safe_getenv("OPENCLAW_HOME", env_buf, sizeof(env_buf), NULL);
    if (!custom || !custom[0]) {
        int written = snprintf(out, out_sz, "%s", home_dir);
        return written > 0 && (size_t)written < out_sz;
    }
    return pmm_expand_user_path(home_dir, custom, out, out_sz);
}

static bool pmm_openclaw_state_dir(const char *home_dir, char *out, size_t out_sz) {
    char internal_home[CLI_BUF_1K];
    if (!pmm_openclaw_internal_home(home_dir, internal_home, sizeof(internal_home))) {
        return false;
    }
    char env_buf[CLI_BUF_1K];
    const char *custom = pmm_safe_getenv("OPENCLAW_STATE_DIR", env_buf, sizeof(env_buf), NULL);
    if (custom && custom[0]) {
        return pmm_expand_user_path(internal_home, custom, out, out_sz);
    }
    char profile_buf[CLI_BUF_256];
    const char *profile =
        pmm_safe_getenv("OPENCLAW_PROFILE", profile_buf, sizeof(profile_buf), NULL);
...
static bool pmm_openclaw_config_path(const char *home_dir, char *out, size_t out_sz) {
    char internal_home[CLI_BUF_1K];
    if (!pmm_openclaw_internal_home(home_dir, internal_home, sizeof(internal_home))) {
        return false;
    }
    char env_buf[CLI_BUF_1K];
    const char *custom = pmm_safe_getenv("OPENCLAW_CONFIG_PATH", env_buf, sizeof(env_buf), NULL);
    if (custom && custom[0]) {
        return pmm_expand_user_path(internal_home, custom, out, out_sz);
    }
    char state_dir[CLI_BUF_1K];
    if (!pmm_openclaw_state_dir(home_dir, state_dir, sizeof(state_dir))) {
        return false;
    }
...
static bool pmm_openclaw_workspace_path(const char *home_dir, const char *config_path, char *out,
                                        size_t out_sz) {
    char internal_home[CLI_BUF_1K];
    if (!pmm_openclaw_internal_home(home_dir, internal_home, sizeof(internal_home))) {
        return false;
    ...
    int lookup =
        pmm_json_like_get_string_at_path(config_path, defaults_path, 2U, "workspace", &configured);
    if (lookup == 0) {
        bool ok = pmm_expand_user_path(internal_home, configured, out, out_sz);
        free(configured);
        return ok;
    }
    free(configured);
    ...
    int include_lookup =
        pmm_json_like_get_string_at_path(config_path, root_path, 0U, "$include", &include_value);
    free(include_value);
    if (include_lookup != 1) {
        return false;
    }
    ...
    const char *workspace =
        pmm_safe_getenv("OPENCLAW_WORKSPACE_DIR", env_buf, sizeof(env_buf), NULL);
    if (workspace && workspace[0]) {
        return pmm_expand_user_path(internal_home, workspace, out, out_sz);
    }
...
static void pmm_opencode_config_path(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = pmm_safe_getenv("OPENCODE_CONFIG", env_buf, sizeof(env_buf), NULL);
    if (custom && custom[0]) {
...
static void pmm_copilot_config_dir(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = pmm_safe_getenv("COPILOT_HOME", env_buf, sizeof(env_buf), NULL);
...
static void pmm_crush_config_path(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = pmm_safe_getenv("CRUSH_GLOBAL_CONFIG", env_buf, sizeof(env_buf), NULL);
    if (custom && custom[0]) {
        snprintf(out, out_sz, "%s", custom);
        return;
    }
...
static void pmm_goose_config_dir(const char *home_dir, char *out, size_t out_sz) {
#ifdef _WIN32
...
static void pmm_vibe_config_dir(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = pmm_safe_getenv("VIBE_HOME", env_buf, sizeof(env_buf), NULL);
    snprintf(out, out_sz, "%s", custom && custom[0] ? custom : "");
    if ((!custom || !custom[0]) && home_dir && home_dir[0]) {
...
static bool pmm_hook_script_name_safe(const char *script_name) {
...
static int pmm_build_claude_hook_command(const char *script_name, const char *config_dir,
                                         bool windows, char *out, size_t out_sz) {
    if (!pmm_hook_script_name_safe(script_name) || !out || out_sz == 0U) {
        return CLI_ERR;
    }
    out[0] = '\0';
    if (windows) {
    ...
    if (config_dir && config_dir[0]) {
        ...
        return written > 0 && (size_t)written < sizeof(path)
                   ? pmm_shell_quote_word(path, out, out_sz)
                   : CLI_ERR;
    }
...
static int pmm_resolve_hook_command(const char *script_name, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *env = pmm_safe_getenv("CLAUDE_CONFIG_DIR", env_buf, sizeof(env_buf), NULL);
#ifdef _WIN32
    return pmm_build_claude_hook_command(script_name, env, true, out, out_sz);
#else
    return pmm_build_claude_hook_command(script_name, env, false, out, out_sz);
#endif
}
...
int pmm_resolve_claude_hook_command_for_testing(const char *script_name, bool windows,
    ...
    char env_buf[CLI_BUF_1K];
    const char *env = pmm_safe_getenv("CLAUDE_CONFIG_DIR", env_buf, sizeof(env_buf), NULL);
    return pmm_build_claude_hook_command(script_name, env, windows, command, command_size);
}
#endif
...
static int pmm_resolve_previous_hook_command(const char *script_name, char *out, size_t out_sz) {
    if (!pmm_hook_script_name_safe(script_name) || !out || out_sz == 0U) {
        return CLI_ERR;
    }
    char env_buf[CLI_BUF_1K];
...
static int pmm_resolve_released_hook_command(const char *script_name, char *out, size_t out_sz) {
...
pmm_detected_agents_t pmm_detect_agents(const char *home_dir) {
    ...
    pmm_claude_config_dir(home_dir, path, sizeof(path));
    agents.claude_code = path[0] != '\0' && dir_exists(path);

    pmm_codex_config_dir(home_dir, path, sizeof(path));
    agents.codex = path[0] != '\0' && dir_exists(path);

    snprintf(path, sizeof(path), "%s/.gemini/antigravity-cli", home_dir);
    agents.antigravity = dir_exists(path) || pmm_agent_cli_exists("antigravity", home_dir);

    snprintf(path, sizeof(path), "%s/.gemini/settings.json", home_dir);
    agents.gemini = pmm_file_exists(path) || pmm_agent_cli_exists("gemini", home_dir);

    pmm_zed_config_dir(home_dir, path, sizeof(path));
    agents.zed = dir_exists(path);

    pmm_opencode_config_path(home_dir, path, sizeof(path));
    agents.opencode = pmm_file_exists(path) || pmm_agent_cli_exists("opencode", home_dir);
    if (!agents.opencode) {
    ...
    if (!agents.opencode) {
        ...
        const char *config_dir =
            pmm_safe_getenv("OPENCODE_CONFIG_DIR", env_buf, sizeof(env_buf), NULL);
        agents.opencode = config_dir && config_dir[0] && dir_exists(config_dir);
    }

    agents.aider = pmm_agent_cli_exists("aider", home_dir);
    ...
    agents.kilocode = agents.kilocode || dir_exists(path) || pmm_agent_cli_exists("kilo", home_dir);
    ...
    agents.augment = dir_exists(path) || pmm_agent_cli_exists("auggie", home_dir);

    char openclaw_config[CLI_BUF_1K];
    char openclaw_state[CLI_BUF_1K];
    bool has_openclaw_config =
        pmm_openclaw_config_path(home_dir, openclaw_config, sizeof(openclaw_config));
    bool has_openclaw_state =
        pmm_openclaw_state_dir(home_dir, openclaw_state, sizeof(openclaw_state));
    agents.openclaw = (has_openclaw_config && pmm_file_exists(openclaw_config)) ||
                      (has_openclaw_state && dir_exists(openclaw_state)) ||
                      pmm_agent_cli_exists("openclaw", home_dir);

    pmm_kiro_home_dir(home_dir, path, sizeof(path));
    agents.kiro = path[0] && (dir_exists(path) || pmm_agent_cli_exists("kiro-cli", home_dir) ||
                              pmm_agent_cli_exists("kiro", home_dir));

    /* Junie (JetBrains): ~/.junie/ */
    snprintf(path, sizeof(path), "%s/.junie", home_dir);
    agents.junie = dir_exists(path);

    pmm_hermes_home_dir(home_dir, path, sizeof(path));
    agents.hermes = path[0] && (dir_exists(path) || pmm_agent_cli_exists("hermes", home_dir));

    snprintf(path, sizeof(path), "%s/.openhands", home_dir);
    agents.openhands = dir_exists(path) || pmm_agent_cli_exists("openhands", home_dir);

    char cline_root[CLI_BUF_1K];
    char cline_data[CLI_BUF_1K];
    pmm_cline_root_dir(home_dir, cline_root, sizeof(cline_root));
    pmm_cline_data_dir(home_dir, cline_data, sizeof(cline_data));
    agents.cline = (cline_root[0] && dir_exists(cline_root)) ||
                   (cline_data[0] && dir_exists(cline_data)) ||
                   pmm_agent_cli_exists("cline", home_dir);
...
#endif
    agents.warp = agents.warp || pmm_agent_cli_exists("oz", home_dir) ||
                  pmm_agent_cli_exists("oz-preview", home_dir) ||
                  pmm_agent_cli_exists("warp-cli", home_dir);

    pmm_qwen_home_dir(home_dir, path, sizeof(path));
    agents.qwen = path[0] && (dir_exists(path) || pmm_agent_cli_exists("qwen", home_dir));

    pmm_copilot_config_dir(home_dir, path, sizeof(path));
    ...
    agents.copilot_cli = pmm_file_exists(copilot_mcp) || pmm_file_exists(copilot_instructions) ||
                         pmm_agent_cli_exists("copilot", home_dir);

    snprintf(path, sizeof(path), "%s/.factory", home_dir);
    agents.factory_droid = dir_exists(path) || pmm_agent_cli_exists("droid", home_dir);

    pmm_crush_config_path(home_dir, path, sizeof(path));
    agents.crush = pmm_file_exists(path) || pmm_agent_cli_exists("crush", home_dir);
    if (!agents.crush) {
        snprintf(path, sizeof(path), "%s/.config/crush", home_dir);
        agents.crush = dir_exists(path);
    }

    pmm_goose_config_dir(home_dir, path, sizeof(path));
    agents.goose = dir_exists(path) || pmm_agent_cli_exists("goose", home_dir);

    pmm_vibe_config_dir(home_dir, path, sizeof(path));
    agents.mistral_vibe = dir_exists(path) || pmm_agent_cli_exists("vibe", home_dir);
...
static const char agent_instructions_content[] =
    "# Project Mind\n"
    "\n"
    "## Codebase Knowledge Graph (project-mind-mcp)\n"
    "\n"
    "This project uses project-mind-mcp to maintain a knowledge graph of the codebase.\n"
...
static const char legacy_augment_verify_agent_content[] =
    ...
    "graph.\n"
    "---\n"
    "Use project-mind-mcp for structural discovery. Start with search_graph, continue with "
...
static const char legacy_gemini_verify_agent_content[] =
    ...
    "  - read_file\n"
    "  - grep_search\n"
    "  - mcp_project-mind-mcp_search_graph\n"
    "  - mcp_project-mind-mcp_trace_path\n"
    "  - mcp_project-mind-mcp_get_code_snippet\n"
    "  - mcp_project-mind-mcp_query_graph\n"
    "  - mcp_project-mind-mcp_get_architecture\n"
    "  - mcp_project-mind-mcp_search_code\n"
    "  - mcp_project-mind-mcp_get_graph_schema\n"
    "  - mcp_project-mind-mcp_list_projects\n"
    "  - mcp_project-mind-mcp_index_status\n"
    "  - mcp_project-mind-mcp_detect_changes\n"
    "---\n"
    "Use project-mind-mcp for structural discovery. Start with search_graph, continue with "
    "trace_path, and retrieve exact definitions with get_code_snippet. Use query_graph or "
...
#define LEGACY_PMM_GRAPH_PROFILE_GUIDANCE                                                       \
    "Use project-mind-mcp for read-only structural discovery. Start with search_graph, "        \
...
#define LEGACY_PMM_GRAPH_HANDOFF_GUIDANCE                                                       \
...
static const char legacy_claude_verify_agent_content[] =
    ...
    "  - Grep\n"
    "  - Glob\n"
    "  - mcp__project-mind-mcp__search_graph\n"
    "  - mcp__project-mind-mcp__trace_path\n"
    "  - mcp__project-mind-mcp__get_code_snippet\n"
    "  - mcp__project-mind-mcp__query_graph\n"
    "  - mcp__project-mind-mcp__get_architecture\n"
    "  - mcp__project-mind-mcp__search_code\n"
    "  - mcp__project-mind-mcp__get_graph_schema\n"
    "  - mcp__project-mind-mcp__list_projects\n"
    "  - mcp__project-mind-mcp__index_status\n"
    "  - mcp__project-mind-mcp__detect_changes\n"
    "mcpServers: [project-mind-mcp]\n"
    "permissionMode: plan\n"
    "skills: [project-mind]\n"
    "---\n" LEGACY_PMM_GRAPH_PROFILE_GUIDANCE;
...
static const char legacy_cursor_verify_agent_content[] =
    ...
    "model: inherit\n"
    "readonly: true\n"
    "---\n" LEGACY_PMM_GRAPH_HANDOFF_GUIDANCE;
...
static const char legacy_copilot_verify_agent_content[] =
    ...
    "  - read\n"
    "  - search\n"
    "  - project-mind-mcp/search_graph\n"
    "  - project-mind-mcp/trace_path\n"
    "  - project-mind-mcp/get_code_snippet\n"
    "  - project-mind-mcp/get_graph_schema\n"
    "  - project-mind-mcp/get_architecture\n"
    "  - project-mind-mcp/search_code\n"
    "  - project-mind-mcp/query_graph\n"
    "  - project-mind-mcp/list_projects\n"
    "  - project-mind-mcp/index_status\n"
    "  - project-mind-mcp/detect_changes\n"
    "---\n" LEGACY_PMM_GRAPH_PROFILE_GUIDANCE;
...
static const char legacy_kilo_verify_agent_content[] =
    ...
    "permission:\n"
    "  \"*\": deny\n"
    "  \"project-mind-mcp_search_graph\": ask\n"
    "  \"project-mind-mcp_trace_path\": ask\n"
    "  \"project-mind-mcp_get_code_snippet\": ask\n"
    "  \"project-mind-mcp_query_graph\": ask\n"
    "  \"project-mind-mcp_get_architecture\": ask\n"
    "  \"project-mind-mcp_search_code\": ask\n"
    "  \"project-mind-mcp_get_graph_schema\": ask\n"
    "  \"project-mind-mcp_list_projects\": ask\n"
    "  \"project-mind-mcp_index_status\": ask\n"
    "  \"project-mind-mcp_detect_changes\": ask\n"
    "---\n"
...
static const char legacy_vibe_verify_agent_content[] =
    "agent_type = \"subagent\"\n"
    "display_name = \"Project Mind\"\n"
    "description = \"Read-only knowledge-graph specialist for structure, dependencies, and call "
    "chains.\"\n"
    "safety = \"safe\"\n"
    "system_prompt_id = \"project-mind\"\n"
    "enabled_tools = [\"project-mind-mcp_search_graph\", "
    "\"project-mind-mcp_trace_path\", \"project-mind-mcp_get_code_snippet\", "
    "\"project-mind-mcp_query_graph\", \"project-mind-mcp_get_architecture\", "
    "\"project-mind-mcp_search_code\", \"project-mind-mcp_get_graph_schema\", "
    "\"project-mind-mcp_list_projects\", \"project-mind-mcp_index_status\", "
    "\"project-mind-mcp_detect_changes\"]\n";

static const char legacy_vibe_verify_prompt_content[] =
    "Use the project-mind graph: search_graph first for structural discovery, trace_path for "
...
static char *pmm_build_legacy_kiro_verify_agent_content(const char *binary_path) {
    ...
    yyjson_mut_doc_set_root(doc, root);
    bool ok =
        yyjson_mut_obj_add_str(doc, root, "name", "project-mind") &&
        yyjson_mut_obj_add_str(
            doc, root, "description",
            "Read-only code structure and call-chain investigation with the knowledge graph.") &&
        yyjson_mut_obj_add_str(
            doc, root, "prompt",
            "Use project-mind-mcp for structural discovery. Start with search_graph, use "
        ...
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/search_graph") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/trace_path") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/get_code_snippet") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/query_graph") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/get_architecture") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/search_code") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/get_graph_schema") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/list_projects") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/index_status") &&
        yyjson_mut_arr_add_str(doc, tools, "@project-mind-mcp/detect_changes") &&
        ...
        yyjson_mut_obj_add_val(doc, servers, "project-mind-mcp", server) &&
...
static const char legacy_codebuddy_verify_agent_content[] =
    ...
    "tools: mcp__project-mind-mcp__search_graph,mcp__project-mind-mcp__trace_path,"
    "mcp__project-mind-mcp__get_code_snippet,mcp__project-mind-mcp__query_graph,"
    "mcp__project-mind-mcp__get_architecture,mcp__project-mind-mcp__search_code,"
    "mcp__project-mind-mcp__get_graph_schema\n"
    "model: inherit\n"
    "permissionMode: plan\n"
    "skills: project-mind\n"
...
static const char legacy_pochi_verify_agent_content[] =
    "---\n"
    "name: project-mind\n"
    "description: Analyze code structure, dependencies, and call chains from project-mind "
...
#undef LEGACY_PMM_GRAPH_PROFILE_GUIDANCE
#undef LEGACY_PMM_GRAPH_HANDOFF_GUIDANCE
 ...
 * exact same discovery priority, expressed as runnable `project-mind-mcp
...
static const char aider_instructions_content[] =
    ...
    "   project-mind-mcp cli search_graph "
    "'{\"project\":\"<name>\",\"name_pattern\":\".*Foo.*\"}'\n"
    "2. Who calls X / what does X call:\n"
    "   project-mind-mcp cli trace_path "
    "'{\"project\":\"<name>\",\"function_name\":\"Foo\",\"direction\":\"both\"}'\n"
    "3. Read a specific function/class:\n"
    "   project-mind-mcp cli get_code_snippet "
    "'{\"project\":\"<name>\",\"qualified_name\":\"<qn>\"}'\n"
    "4. Complex patterns (Cypher):\n"
    "   project-mind-mcp cli query_graph '{\"project\":\"<name>\",\"query\":\"MATCH ...\"}'\n"
    "5. Project overview:\n"
    "   project-mind-mcp cli get_architecture '{\"project\":\"<name>\"}'\n"
    "\n"
    "First use in a repo: project-mind-mcp cli index_repository '{\"repo_path\":\"<abs "
    "path>\"}'\n"
    "List indexed projects (for <name>): project-mind-mcp cli list_projects '{}'\n"
    "\n"
    "## When to fall back to grep/glob\n"
    "- Searching for string literals, error messages, config values\n"
    "- Searching non-code files (Dockerfiles, shell scripts, configs)\n"
...
#define CMM_MARKER_START "<!-- project-mind-mcp:start -->"
#define CMM_MARKER_END "<!-- project-mind-mcp:end -->"
...
int pmm_upsert_instructions(const char *path, const char *content) {
    ...
    return pmm_text_upsert_managed_block(path, CMM_MARKER_START, CMM_MARKER_END, content) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int pmm_upsert_windsurf_rules(const char *path, const char *content) {
    if (!path || !content || ensure_parent_dir(path) != CLI_OK) {
        return CLI_ERR;
    }
    return pmm_text_upsert_managed_block_limited(path, CMM_MARKER_START, CMM_MARKER_END, content,
...
int pmm_remove_instructions(const char *path) {
    ...
    return pmm_text_remove_managed_block(path, CMM_MARKER_START, CMM_MARKER_END) == 0 ? CLI_OK
                                                                                      : CLI_ERR;
}

/* ── Codex MCP config (TOML) ─────────────────────────────────── */

#define CODEX_CMM_TABLE "mcp_servers.project-mind-mcp"
#define CODEX_CMM_SECTION "[" CODEX_CMM_TABLE "]"
#define CODEX_MCP_BEGIN "# >>> project-mind-mcp MCP >>>"
#define CODEX_MCP_END "# <<< project-mind-mcp MCP <<<"
 ...
 * remains fail-closed. */
static int pmm_remove_codex_legacy_mcp(const char *config_path) {
    return pmm_toml_remove_legacy_table(config_path, CODEX_CMM_TABLE, CODEX_MCP_BEGIN,
                                        CODEX_MCP_END);
}
...
int pmm_upsert_codex_mcp(const char *binary_path, const char *config_path) {
    ...
    if (pmm_toml_escape_basic_string(binary_path, escaped, sizeof(escaped)) != 0) {
        return CLI_ERR;
    }
    ...
    if (written < 0 || (size_t)written >= sizeof(block) ||
        pmm_remove_codex_legacy_mcp(config_path) != 0) {
        return CLI_ERR;
    }
    return pmm_toml_upsert_managed_block(config_path, CODEX_MCP_BEGIN, CODEX_MCP_END, block) == 0
               ? CLI_OK
               : CLI_ERR;
}
...
int pmm_remove_codex_mcp(const char *config_path) {
    if (!config_path ||
        pmm_toml_remove_managed_block(config_path, CODEX_MCP_BEGIN, CODEX_MCP_END) != 0) {
        return CLI_ERR;
    }
    return pmm_remove_codex_legacy_mcp(config_path) >= 0 ? CLI_OK : CLI_ERR;
}

static int pmm_remove_codex_mcp_owned(const char *binary_path, const char *config_path) {
    (void)binary_path;
    return pmm_remove_codex_mcp(config_path);
}
...
#define CODEX_HOOK_BEGIN "# >>> project-mind-mcp SessionStart >>>"
#define CODEX_HOOK_END "# <<< project-mind-mcp SessionStart <<<"

static int pmm_build_augment_command(const char *binary_path, char *out, size_t out_size) {
    char quoted[CLI_BUF_8K];
    if (pmm_shell_quote_word(binary_path, quoted, sizeof(quoted)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int pmm_build_augment_dialect_command(const char *binary_path, const char *dialect,
    ...
    if (pmm_build_augment_command(binary_path, base, sizeof(base)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int pmm_build_augment_command_windows(const char *binary_path, char *out, size_t out_size) {
    char quoted[CLI_BUF_8K];
    if (pmm_powershell_quote_word(binary_path, quoted, sizeof(quoted)) != CLI_OK) {
        return CLI_ERR;
    }
    int written = snprintf(out, out_size, "& %s hook-augment", quoted);
    return written > 0 && (size_t)written < out_size ? CLI_OK : CLI_ERR;
...
static int pmm_build_dialect_hook_command(const char *binary_path, const char *dialect,
    ...
    if (!windows) {
        shell[0] = '\0';
        return pmm_build_augment_dialect_command(binary_path, dialect, command, command_size);
    }
    int shell_written = snprintf(shell, shell_size, "%s", "powershell");
    char base[CLI_BUF_8K];
    if (shell_written < 0 || (size_t)shell_written >= shell_size ||
        pmm_build_augment_command_windows(binary_path, base, sizeof(base)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int pmm_build_qwen_hook_command(const char *binary_path, bool windows, char *command,
                                       size_t command_size, char *shell, size_t shell_size) {
    return pmm_build_dialect_hook_command(binary_path, "qwen", windows, command, command_size,
                                          shell, shell_size);
}

static int pmm_build_qoder_hook_command(const char *binary_path, bool windows, char *command,
                                        size_t command_size, char *shell, size_t shell_size) {
    return pmm_build_dialect_hook_command(binary_path, "qoder", windows, command, command_size,
                                          shell, shell_size);
}

static bool pmm_optional_hook_supported(const char *agent_name, bool windows) {
...
static bool pmm_current_platform_is_windows(void) {
#ifdef _WIN32
    return true;
#else
    return false;
...
int pmm_build_qwen_hook_command_for_testing(const char *binary_path, bool windows, char *command,
                                            size_t command_size, char *shell, size_t shell_size) {
    return pmm_build_qwen_hook_command(binary_path, windows, command, command_size, shell,
                                       shell_size);
}

int pmm_build_qoder_hook_command_for_testing(const char *binary_path, bool windows, char *command,
                                             size_t command_size, char *shell, size_t shell_size) {
    return pmm_build_qoder_hook_command(binary_path, windows, command, command_size, shell,
                                        shell_size);
}
...
bool pmm_optional_hook_supported_for_testing(const char *agent_name, bool windows) {
    return pmm_optional_hook_supported(agent_name, windows);
}
#endif

static int pmm_upsert_codex_hooks_command(const char *config_path, const char *command,
    ...
    if (pmm_toml_escape_basic_string(command, escaped, sizeof(escaped)) != CLI_OK ||
        pmm_toml_escape_basic_string(command_windows, escaped_windows, sizeof(escaped_windows)) !=
            CLI_OK) {
        return CLI_ERR;
    }
    char block[CLI_BUF_8K];
    ...
    return pmm_toml_upsert_managed_block(config_path, CODEX_HOOK_BEGIN, CODEX_HOOK_END, block) == 0
               ? CLI_OK
               : CLI_ERR;
}
...
int pmm_upsert_codex_hooks(const char *config_path) {
    return pmm_upsert_codex_hooks_command(config_path, "project-mind-mcp hook-augment",
                                          "project-mind-mcp hook-augment");
}

int pmm_remove_codex_hooks(const char *config_path) {
    return config_path &&
                   pmm_toml_remove_managed_block(config_path, CODEX_HOOK_BEGIN, CODEX_HOOK_END) == 0
               ? CLI_OK
               : CLI_ERR;
}
...
int pmm_upsert_opencode_mcp(const char *binary_path, const char *config_path) {
    ...
    return pmm_upsert_json_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_LOCAL_ARRAY);
}

int pmm_remove_opencode_mcp(const char *config_path) {
    static const char *const path[] = {"mcp"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_LOCAL_ARRAY, NULL);
}

int pmm_remove_opencode_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_LOCAL_ARRAY, binary_path);
}

static int pmm_upsert_kilo_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return pmm_upsert_json_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_LOCAL_ARRAY);
}

static int pmm_remove_kilo_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_LOCAL_ARRAY, binary_path);
}

static int pmm_upsert_cline_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return pmm_upsert_json_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_CLINE);
}

static int pmm_remove_cline_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_CLINE, binary_path);
}

static int pmm_upsert_copilot_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return pmm_upsert_json_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_COPILOT);
}

static int pmm_remove_copilot_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_COPILOT, binary_path);
}

enum { COPILOT_HOOK_TIMEOUT_SEC = 5 };

static int pmm_build_copilot_hook_command(const char *binary_path, const char *event,
    ...
    char quoted[CLI_BUF_8K];
    int quote_rc = powershell ? pmm_powershell_quote_word(binary_path, quoted, sizeof(quoted))
                              : pmm_shell_quote_word(binary_path, quoted, sizeof(quoted));
    if (quote_rc != CLI_OK) {
        return CLI_ERR;
    }
...
static bool pmm_copilot_add_hook_event(yyjson_mut_doc *doc, yyjson_mut_val *hooks,
...
static char *pmm_build_copilot_hook_manifest(const char *binary_path) {
    ...
    if (pmm_build_copilot_hook_command(binary_path, "SessionStart", false, session_bash,
                                       sizeof(session_bash)) != CLI_OK ||
        pmm_build_copilot_hook_command(binary_path, "SessionStart", true, session_powershell,
                                       sizeof(session_powershell)) != CLI_OK ||
        pmm_build_copilot_hook_command(binary_path, "SubagentStart", false, subagent_bash,
                                       sizeof(subagent_bash)) != CLI_OK ||
        pmm_build_copilot_hook_command(binary_path, "SubagentStart", true, subagent_powershell,
    ...
    bool ok =
        ...
        pmm_copilot_add_hook_event(doc, hooks, "sessionStart", session_bash, session_powershell) &&
        pmm_copilot_add_hook_event(doc, hooks, "subagentStart", subagent_bash,
                                   subagent_powershell) &&
...
static int pmm_upsert_copilot_hooks(const char *binary_path, const char *manifest_path) {
    char *manifest = pmm_build_copilot_hook_manifest(binary_path);
    if (!manifest || !manifest_path) {
        free(manifest);
        return CLI_ERR;
    }
    int rc = pmm_text_ensure_owned_document(manifest_path, manifest);
    free(manifest);
    return rc == 0 ? CLI_OK : CLI_ERR;
}

static int pmm_remove_copilot_hooks(const char *manifest_path, const char *binary_path) {
    char *manifest = pmm_build_copilot_hook_manifest(binary_path);
    if (!manifest || !manifest_path) {
        free(manifest);
        return CLI_ERR;
    }
    int rc = pmm_text_remove_owned_document(manifest_path, manifest);
    free(manifest);
    return rc >= 0 ? CLI_OK : CLI_ERR;
}

static int pmm_upsert_factory_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return pmm_upsert_json_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_FACTORY);
}

static int pmm_remove_factory_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_FACTORY, binary_path);
}

static int pmm_upsert_crush_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return pmm_upsert_json_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_CRUSH);
}

static int pmm_upsert_crush_context_path(const char *config_path, const char *context_path) {
    static const char *const path[] = {"options"};
    return pmm_json_like_add_unique_string_at_path(config_path, path, 1U, "context_paths",
                                                   context_path) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int pmm_remove_crush_context_path(const char *config_path, const char *context_path) {
    static const char *const path[] = {"options"};
    return pmm_json_like_remove_string_at_path(config_path, path, 1U, "context_paths",
                                               context_path) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int pmm_remove_crush_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return pmm_remove_json_mcp(config_path, path, 1U, PMM_JSON_MCP_CRUSH, binary_path);
}

static int pmm_build_yaml_stdio_mcp_block(const char *binary_path, bool goose_schema, char *block,
                                          size_t block_size) {
    if (!binary_path || !block || block_size == 0U) {
        return CLI_ERR;
    }
    char *encoded = NULL;
    if (pmm_yaml_encode_double_quoted_scalar(binary_path, &encoded) != 0 || !encoded) {
        free(encoded);
        return CLI_ERR;
    }
...
static int pmm_upsert_yaml_stdio_mcp(const char *binary_path, const char *config_path,
                                     const char *section_key, bool goose_schema) {
    char block[CLI_BUF_8K];
    if (!config_path || !section_key ||
        pmm_build_yaml_stdio_mcp_block(binary_path, goose_schema, block, sizeof(block)) != CLI_OK) {
        return CLI_ERR;
    }
    return pmm_yaml_upsert_owned_mapping_entry(config_path, section_key, "project-mind-mcp",
                                               block) == PMM_YAML_IDENTITY_EDIT_OK
               ? CLI_OK
               : CLI_ERR;
}

static int pmm_remove_yaml_stdio_mcp(const char *binary_path, const char *config_path,
                                     const char *section_key, bool goose_schema) {
    char block[CLI_BUF_8K];
    if (!config_path || !section_key ||
        pmm_build_yaml_stdio_mcp_block(binary_path, goose_schema, block, sizeof(block)) != CLI_OK) {
    ...
    return pmm_yaml_remove_owned_mapping_entry(config_path, section_key, "project-mind-mcp", block);
}

static int pmm_upsert_hermes_mcp(const char *binary_path, const char *config_path) {
    return pmm_upsert_yaml_stdio_mcp(binary_path, config_path, "mcp_servers", false);
}

static int pmm_remove_hermes_mcp_owned(const char *binary_path, const char *config_path) {
    return pmm_remove_yaml_stdio_mcp(binary_path, config_path, "mcp_servers", false);
}

static int pmm_upsert_goose_mcp(const char *binary_path, const char *config_path) {
    return pmm_upsert_yaml_stdio_mcp(binary_path, config_path, "extensions", true);
}

static int pmm_remove_goose_mcp_owned(const char *binary_path, const char *config_path) {
    return pmm_remove_yaml_stdio_mcp(binary_path, config_path, "extensions", true);
}
...
int pmm_upsert_antigravity_mcp(const char *binary_path, const char *config_path) {
    ...
    return pmm_install_editor_mcp(binary_path, config_path);
}

int pmm_remove_antigravity_mcp(const char *config_path) {
    return pmm_remove_editor_mcp(config_path);
}

int pmm_remove_antigravity_mcp_owned(const char *binary_path, const char *config_path) {
    return pmm_remove_editor_mcp_owned(binary_path, config_path);
}

/* ── Junie MCP config (JSON, same mcpServers format) ──────────── */

static int pmm_junie_mcp_preflight(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    static const struct {
        const char *name;
        const char *argument;
        ...
        {PMM_DEFAULT_MCP_SERVER_NAME, NULL},
        {PMM_SCOUT_MCP_SERVER_NAME, PMM_SCOUT_PROFILE_ARGUMENT},
        {PMM_ANALYSIS_MCP_SERVER_NAME, PMM_ANALYSIS_PROFILE_ARGUMENT},
    };
    char *document = NULL;
    size_t document_length = 0U;
    int read_result = pmm_json_like_read_document(config_path, &document, &document_length);
    ...
    for (size_t i = 0U; i < sizeof(entries) / sizeof(entries[0]); i++) {
        int ownership = pmm_json_mcp_snapshot_ownership(
            document, document_length, path, 1U, PMM_JSON_MCP_STANDARD, entries[i].name,
...
int pmm_upsert_junie_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    if (pmm_junie_mcp_preflight(binary_path, config_path) != CLI_OK ||
        pmm_upsert_json_named_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_STANDARD,
                                  PMM_DEFAULT_MCP_SERVER_NAME, NULL) != CLI_OK ||
        pmm_upsert_json_named_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_STANDARD,
                                  PMM_SCOUT_MCP_SERVER_NAME,
                                  PMM_SCOUT_PROFILE_ARGUMENT) != CLI_OK ||
        pmm_upsert_json_named_mcp(binary_path, config_path, path, 1U, PMM_JSON_MCP_STANDARD,
                                  PMM_ANALYSIS_MCP_SERVER_NAME,
                                  PMM_ANALYSIS_PROFILE_ARGUMENT) != CLI_OK) {
        return CLI_ERR;
    }
    return CLI_OK;
}
...
int pmm_upsert_junie_mcp_with_previous_for_testing(const char *binary_path,
    ...
    const char *saved = g_previous_managed_mcp_command;
    g_previous_managed_mcp_command = previous_binary_path;
    int result = pmm_upsert_junie_mcp(binary_path, config_path);
    g_previous_managed_mcp_command = saved;
    return result;
}
#endif
...
int pmm_remove_junie_mcp(const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    int result = CLI_OK;
    if (pmm_remove_json_named_mcp(config_path, path, 1U, PMM_JSON_MCP_STANDARD,
                                  PMM_DEFAULT_MCP_SERVER_NAME, NULL, NULL) != CLI_OK) {
        result = CLI_ERR;
    }
    if (pmm_remove_json_named_mcp(config_path, path, 1U, PMM_JSON_MCP_STANDARD,
                                  PMM_SCOUT_MCP_SERVER_NAME, PMM_SCOUT_PROFILE_ARGUMENT,
...
int pmm_remove_junie_mcp_owned(const char *binary_path, const char *config_path) {
    ...
    int result = CLI_OK;
    if (pmm_remove_json_named_mcp(config_path, path, 1U, PMM_JSON_MCP_STANDARD,
                                  PMM_DEFAULT_MCP_SERVER_NAME, NULL, binary_path) != CLI_OK) {
        result = CLI_ERR;
    }
    if (pmm_remove_json_named_mcp(config_path, path, 1U, PMM_JSON_MCP_STANDARD,
                                  PMM_SCOUT_MCP_SERVER_NAME, PMM_SCOUT_PROFILE_ARGUMENT,
...
static int pmm_build_vibe_mcp_body(const char *binary_path, char *body, size_t body_size) {
...
static int pmm_upsert_vibe_mcp(const char *binary_path, const char *config_path) {
    char body[CLI_BUF_8K];
    if (!config_path || pmm_build_vibe_mcp_body(binary_path, body, sizeof(body)) != CLI_OK) {
        return CLI_ERR;
    }
    return pmm_toml_upsert_owned_named_array_table(config_path, "mcp_servers", "name",
                                                   "project-mind-mcp",
                                                   body) == PMM_TOML_OWNED_EDIT_OK
               ? CLI_OK
               : CLI_ERR;
}

static int pmm_remove_vibe_mcp_owned(const char *binary_path, const char *config_path) {
    char body[CLI_BUF_8K];
    if (!config_path || pmm_build_vibe_mcp_body(binary_path, body, sizeof(body)) != CLI_OK) {
        return CLI_ERR;
    }
    return pmm_toml_remove_owned_named_array_table(config_path, "mcp_servers", "name",
                                                   "project-mind-mcp", body);
}
 ...
 * pmm_resolve_hook_command so $CLAUDE_CONFIG_DIR is honored. */
#ifdef _WIN32
...
#define CMM_HOOK_GATE_SCRIPT "pmm-code-discovery-gate.cmd"
#else
#define CMM_HOOK_GATE_SCRIPT "pmm-code-discovery-gate"
#endif
#define CMM_HOOK_GATE_SCRIPT_LEGACY "pmm-code-discovery-gate"
...
static PMM_TLS pmm_hook_json_prewrite_test_hook_t pmm_hook_json_prewrite_test_hook = NULL;
static PMM_TLS void *pmm_hook_json_prewrite_test_context = NULL;

void pmm_set_hook_json_prewrite_hook_for_testing(pmm_hook_json_prewrite_test_hook_t hook,
                                                 void *context) {
    pmm_hook_json_prewrite_test_hook = hook;
    pmm_hook_json_prewrite_test_context = context;
}
#endif
...
static int write_hook_event_array(const char *settings_path, const char *hook_event,
...
#ifdef PMM_JSON_LIKE_ENABLE_TEST_API
    if (pmm_hook_json_prewrite_test_hook) {
        pmm_hook_json_prewrite_test_hook(settings_path, pmm_hook_json_prewrite_test_context);
    }
#endif
    if (yyjson_mut_arr_size(event_arr) == 0U) {
        return pmm_json_like_remove_entry_if_unchanged(settings_path, hooks_path, 1U, hook_event,
    ...
    int rc = pmm_json_like_upsert_entry_if_unchanged(settings_path, hooks_path, 1U, hook_event,
...
static int upsert_hooks_json(hooks_upsert_args_t args) {
    ...
    int read_result =
        pmm_json_like_read_document(settings_path, &expected_content, &expected_length);
    if (read_result < 0) {
        return CLI_ERR;
    }
...
static int pmm_upsert_qoder_context_hook(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char shell[CLI_BUF_32];
    if (pmm_build_qoder_hook_command(binary_path, pmm_current_platform_is_windows(), command,
...
static int pmm_remove_qoder_context_hook(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char shell[CLI_BUF_32];
    if (pmm_build_qoder_hook_command(binary_path, pmm_current_platform_is_windows(), command,
                                     sizeof(command), shell, sizeof(shell)) != CLI_OK) {
...
int pmm_upsert_qoder_context_hooks_for_testing(const char *settings_path, const char *binary_path) {
    return pmm_upsert_qoder_context_hook(settings_path, binary_path);
}

int pmm_remove_qoder_context_hooks_for_testing(const char *settings_path, const char *binary_path) {
    return pmm_remove_qoder_context_hook(settings_path, binary_path);
}
#endif

#define KIMI_HOOK_BEGIN "# >>> project-mind-mcp Kimi UserPromptSubmit >>>"
#define KIMI_HOOK_END "# <<< project-mind-mcp Kimi UserPromptSubmit <<<"

static int pmm_upsert_kimi_context_hook(const char *config_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char escaped[CLI_BUF_8K];
    char block[CLI_BUF_8K];
    if (pmm_build_augment_dialect_command(binary_path, "kimi", command, sizeof(command)) !=
            CLI_OK ||
        pmm_toml_escape_basic_string(command, escaped, sizeof(escaped)) != 0) {
        return CLI_ERR;
    }
    int written = snprintf(block, sizeof(block),
                           "[[hooks]]\n"
    ...
    return pmm_toml_upsert_managed_block(config_path, KIMI_HOOK_BEGIN, KIMI_HOOK_END, block) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int pmm_remove_kimi_context_hook(const char *config_path) {
    return pmm_toml_remove_managed_block(config_path, KIMI_HOOK_BEGIN, KIMI_HOOK_END) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int pmm_upsert_gitlab_session_hook(const char *hooks_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (pmm_build_augment_command(binary_path, command, sizeof(command)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int pmm_remove_gitlab_session_hook(const char *hooks_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (pmm_build_augment_command(binary_path, command, sizeof(command)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int pmm_edit_devin_context_hooks(const char *config_path, const char *binary_path,
                                        bool remove, bool include_session_start) {
    static const char *const events[] = {"SessionStart", "UserPromptSubmit", "PostCompaction"};
    char command[CLI_BUF_8K];
    if (pmm_build_augment_dialect_command(binary_path, "devin", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
    }
    int result = CLI_OK;
...
static int pmm_upsert_devin_context_hooks(const char *config_path, const char *binary_path,
                                          bool include_session_start) {
    return pmm_edit_devin_context_hooks(config_path, binary_path, false, include_session_start);
}

static int pmm_remove_devin_context_hooks(const char *config_path, const char *binary_path) {
    return pmm_edit_devin_context_hooks(config_path, binary_path, true, true);
}

static int pmm_remove_devin_session_hook(const char *config_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (pmm_build_augment_dialect_command(binary_path, "devin", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
...
#define CMM_HERMES_HOOK_ID "project-mind-mcp"

static int pmm_build_hermes_context_hook_item(const char *binary_path, char *item,
                                              size_t item_size) {
    char command[CLI_BUF_8K];
    if (pmm_build_augment_dialect_command(binary_path, "hermes", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
    }
    char *encoded_command = NULL;
    if (pmm_yaml_encode_double_quoted_scalar(command, &encoded_command) != CLI_OK) {
        return CLI_ERR;
    }
...
static int pmm_upsert_hermes_context_hook(const char *config_path, const char *binary_path) {
    ...
    if (pmm_build_hermes_context_hook_item(binary_path, item, sizeof(item)) != CLI_OK) {
        return PMM_YAML_IDENTITY_EDIT_ERROR;
    }
    return pmm_yaml_upsert_mapping_sequence_item(config_path, sequence_path,
                                                 sizeof(sequence_path) / sizeof(sequence_path[0]),
                                                 "id", identity, item);
}

static bool pmm_yaml_line_is_empty_key(const char *line, size_t line_len, size_t indent,
                                       const char *key) {
...
static bool pmm_hermes_pre_llm_sequence_is_empty(const char *document) {
    bool in_hooks = false;
    ...
    while (cursor && *cursor) {
        ...
        if (!blank) {
            ...
            if (indent == 0U) {
                in_hooks = pmm_yaml_line_is_empty_key(line, line_len, 0U, "hooks");
            } else if (in_hooks && indent == CLI_PAIR_LEN &&
                       pmm_yaml_line_is_empty_key(line, line_len, CLI_PAIR_LEN, "pre_llm_call")) {
                in_pre_llm = true;
            }
        }
...
static int pmm_remove_hermes_context_hook(const char *config_path, const char *binary_path) {
    static const char *const sequence_path[] = {"hooks", "pre_llm_call"};
    static const char identity[] = "\"" CMM_HERMES_HOOK_ID "\"";
    char item[CLI_BUF_8K];
    if (pmm_build_hermes_context_hook_item(binary_path, item, sizeof(item)) != CLI_OK) {
    ...
    int result = pmm_yaml_remove_mapping_sequence_item(
        config_path, sequence_path, sizeof(sequence_path) / sizeof(sequence_path[0]), "id",
        identity, item);
    if (result != PMM_YAML_IDENTITY_EDIT_OK || !before) {
        free(before);
        return result;
    }
    size_t after_len = 0U;
    ...
    bool remove_empty_sequence = exact_removed && pmm_hermes_pre_llm_sequence_is_empty(after);
    free(before);
    free(after);
    if (remove_empty_sequence &&
        pmm_yaml_remove_mapping_entry(config_path, "hooks", "pre_llm_call") != CLI_OK) {
        return PMM_YAML_IDENTITY_EDIT_ERROR;
    }
    return PMM_YAML_IDENTITY_EDIT_OK;
}
...
int pmm_upsert_claude_hooks(const char *settings_path) {
    ...
    if (pmm_resolve_hook_command(CMM_HOOK_GATE_SCRIPT, command, sizeof(command)) != CLI_OK ||
        pmm_resolve_previous_hook_command(CMM_HOOK_GATE_SCRIPT, previous_command,
                                          sizeof(previous_command)) != CLI_OK ||
        pmm_resolve_released_hook_command(CMM_HOOK_GATE_SCRIPT, released_command,
                                          sizeof(released_command)) != CLI_OK ||
        pmm_resolve_previous_hook_command(CMM_HOOK_GATE_SCRIPT_LEGACY, previous_legacy_command,
                                          sizeof(previous_legacy_command)) != CLI_OK ||
        pmm_resolve_released_hook_command(CMM_HOOK_GATE_SCRIPT_LEGACY, released_legacy_command,
...
static bool pmm_write_owned_hook_script_with_legacy(const char *path, const char *script,
                                                    const char *const *legacy_scripts,
                                                    size_t legacy_count) {
    return pmm_text_migrate_owned_document_mode(path, script, legacy_scripts, legacy_count,
                                                CLI_OCTAL_PERM) == CLI_OK;
}

static bool pmm_write_owned_hook_script(const char *path, const char *script) {
    return pmm_write_owned_hook_script_with_legacy(path, script, NULL, 0U);
}

#ifdef _WIN32
#define AUGMENT_SESSION_SCRIPT "project-mind-session.ps1"
#define AUGMENT_COVERAGE_SCRIPT "project-mind-coverage.ps1"
#else
#define AUGMENT_SESSION_SCRIPT "project-mind-session.sh"
#define AUGMENT_COVERAGE_SCRIPT "project-mind-coverage.sh"
#endif

static int pmm_build_augment_session_script(const char *binary_path, char *script,
                                            size_t script_size) {
    ...
    int written = snprintf(script, script_size,
                           "# SessionStart adapter installed by project-mind-mcp.\n"
                           "$bin = %s\n"
...
static bool pmm_install_augment_session_script(const char *binary_path, const char *script_path) {
    char script[CLI_BUF_8K];
    return ensure_parent_dir(script_path) == CLI_OK &&
           pmm_build_augment_session_script(binary_path, script, sizeof(script)) == CLI_OK &&
           pmm_write_owned_hook_script(script_path, script);
}

static int pmm_build_augment_coverage_script(const char *binary_path, char *script,
                                             size_t script_size) {
    if (!binary_path || !script || script_size == 0U) {
        return CLI_ERR;
    }
    ...
    int written = snprintf(script, script_size,
                           "# PostToolUse view adapter installed by project-mind-mcp.\n"
                           "$bin = %s\n"
                           "if (-not (Test-Path -LiteralPath $bin -PathType Leaf)) { exit 0 }\n"
...
static bool pmm_install_augment_coverage_script(const char *binary_path, const char *script_path) {
    char script[CLI_BUF_8K];
    return ensure_parent_dir(script_path) == CLI_OK &&
           pmm_build_augment_coverage_script(binary_path, script, sizeof(script)) == CLI_OK &&
           pmm_write_owned_hook_script(script_path, script);
}
...
static int pmm_cline_hook_path(const char *cline_root, const char *event, char *path,
                               size_t path_size) {
#ifdef _WIN32
...
static int pmm_build_cline_context_script(const char *binary_path, const char *event, char *script,
                                          size_t script_size) {
    char quoted[CLI_BUF_8K];
#ifdef _WIN32
    if (pmm_powershell_quote_word(binary_path, quoted, sizeof(quoted)) != CLI_OK) {
    ...
    int written = snprintf(script, script_size,
                           "# Cline %s context adapter installed by project-mind-mcp.\n"
                           "$bin = %s\n"
                           "if (-not (Test-Path -LiteralPath $bin -PathType Leaf)) { exit 0 }\n"
...
static const char cmm_gate_script_prefix[] =
    "#!/usr/bin/env bash\n"
    "# project-mind-mcp search augmenter (Claude Code PreToolUse).\n"
...
static const char cmm_session_script_prefix[] =
    "#!/usr/bin/env bash\n"
    "# SessionStart context adapter installed by project-mind-mcp.\n"
    "# Fail-open: it never blocks or logs hook/prompt content.\n"
    "BIN=";

static const char cmm_subagent_script_prefix[] =
    "#!/usr/bin/env bash\n"
    "# SubagentStart context adapter installed by project-mind-mcp.\n"
    "# Fail-open: it never blocks or logs hook/prompt content.\n"
    "BIN=";
...
#ifdef _WIN32
static int pmm_escape_batch_value(const char *value, char *escaped, size_t escaped_size) {
...
static int pmm_build_current_hook_script(const char *prefix, const char *binary_path, char *script,
                                         size_t script_size) {
#ifdef _WIN32
    char escaped_binary[CLI_BUF_8K];
    if (!prefix || !binary_path || !script ||
        pmm_escape_batch_value(binary_path, escaped_binary, sizeof(escaped_binary)) != CLI_OK) {
        return CLI_ERR;
    }
    ...
    int written = snprintf(script, script_size,
                           ...
                           "REM %s installed by project-mind-mcp.\r\n"
    ...
    if (!prefix || !binary_path || !script ||
        pmm_shell_quote_word(binary_path, quoted_binary, sizeof(quoted_binary)) != CLI_OK) {
        return CLI_ERR;
    }
    int written =
...
static const char cmm_released_session_script[] =
    "#!/usr/bin/env bash\n"
    "# SessionStart hook: remind agent to use project-mind-mcp tools.\n"
    "# Installed by project-mind-mcp. Fires on startup/resume/clear/compact.\n"
    "cat << 'REMINDER'\n"
    "CRITICAL - Code Discovery Protocol:\n"
    "1. ALWAYS use project-mind-mcp tools FIRST for ANY code exploration:\n"
...
static const char cmm_released_subagent_script[] =
    "#!/usr/bin/env bash\n"
    "# SubagentStart hook: tell subagents to use project-mind-mcp tools.\n"
    "# Installed by project-mind-mcp. Fires when any subagent is spawned.\n"
    "# SubagentStart injects context via JSON additionalContext, not plain stdout.\n"
    "cat << 'REMINDER'\n"
    "{\"hookSpecificOutput\":{\"hookEventName\":\"SubagentStart\","
    "\"additionalContext\":\"Code discovery: prefer project-mind-mcp tools "
...
static int pmm_build_released_gate_script(const char *binary_path, char *script,
                                          size_t script_size) {
...
static int pmm_remove_owned_hook_script(const char *path, const char *expected_current,
                                        const char *const *released_scripts,
                                        size_t released_script_count) {
    return pmm_text_remove_owned_document_any(path, expected_current, released_scripts,
 ...
 * The legacy filename `pmm-code-discovery-gate` is retained so existing
...
#ifdef _WIN32
static int pmm_remove_owned_legacy_hook_script(const char *hooks_dir, const char *legacy_name,
    ...
    int result = pmm_text_remove_owned_document_any(legacy_path, current_script, released_scripts,
                                                    released_script_count);
...
bool pmm_install_hook_gate_script(const char *home, const char *binary_path) {
    ...
    pmm_claude_config_dir(home, config_dir, sizeof(config_dir));
    if (!config_dir[0]) {
        return false;
    }
    ...
    if (!pmm_mkdir_p(hooks_dir, CLI_OCTAL_PERM)) {
        return false;
    }
    ...
    char script[CLI_BUF_8K];
    if (pmm_build_current_hook_script(cmm_gate_script_prefix, binary_path, script,
                                      sizeof(script)) != CLI_OK) {
        return false;
    }
    char released_script[CLI_BUF_8K];
    const char *const legacy[] = {released_script};
    size_t legacy_count = pmm_build_released_gate_script(binary_path, released_script,
                                                         sizeof(released_script)) == CLI_OK
                              ? 1U
                              : 0U;
#ifdef _WIN32
    if (pmm_remove_owned_legacy_hook_script(hooks_dir, CMM_HOOK_GATE_SCRIPT_LEGACY, script, legacy,
                                            legacy_count) != CLI_OK) {
        return false;
    }
#endif
    return pmm_write_owned_hook_script_with_legacy(script_path, script, legacy, legacy_count);
}

/* SessionStart hook: remind agent to use MCP tools on every context reset. */
#ifdef _WIN32
#define CMM_SESSION_REMINDER_SCRIPT "pmm-session-reminder.cmd"
#else
#define CMM_SESSION_REMINDER_SCRIPT "pmm-session-reminder"
#endif
#define CMM_SESSION_REMINDER_SCRIPT_LEGACY "pmm-session-reminder"

static bool pmm_install_session_reminder_script(const char *home, const char *binary_path) {
    if (!home || !binary_path) {
        return false;
    }
    char config_dir[CLI_BUF_1K];
    ...
    char script[CLI_BUF_8K];
    if (pmm_build_current_hook_script(cmm_session_script_prefix, binary_path, script,
                                      sizeof(script)) != CLI_OK) {
        return false;
    }
    const char *const legacy[] = {cmm_released_session_script};
#ifdef _WIN32
    if (pmm_remove_owned_legacy_hook_script(hooks_dir, CMM_SESSION_REMINDER_SCRIPT_LEGACY, script,
                                            legacy, 1U) != CLI_OK) {
        return false;
    }
#endif
    return pmm_write_owned_hook_script_with_legacy(script_path, script, legacy, 1U);
}

static int pmm_upsert_session_hooks(const char *settings_path) {
    ...
    char previous_legacy_command[CLI_BUF_8K];
    char released_legacy_command[CLI_BUF_8K];
    if (pmm_resolve_hook_command(CMM_SESSION_REMINDER_SCRIPT, command, sizeof(command)) != CLI_OK ||
        pmm_resolve_previous_hook_command(CMM_SESSION_REMINDER_SCRIPT, previous_command,
                                          sizeof(previous_command)) != CLI_OK ||
        pmm_resolve_released_hook_command(CMM_SESSION_REMINDER_SCRIPT, released_command,
                                          sizeof(released_command)) != CLI_OK ||
        pmm_resolve_previous_hook_command(CMM_SESSION_REMINDER_SCRIPT_LEGACY,
                                          previous_legacy_command,
                                          sizeof(previous_legacy_command)) != CLI_OK ||
        pmm_resolve_released_hook_command(CMM_SESSION_REMINDER_SCRIPT_LEGACY,
...
static int pmm_remove_session_hooks(const char *settings_path) {
    static const char *matchers[] = {"startup", "resume", "clear", "compact"};
    char command[CLI_BUF_8K];
    char previous_command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
...
static bool pmm_has_complete_claude_session_hooks(const char *home) {
    ...
    pmm_claude_config_dir(home, config_dir, sizeof(config_dir));
    if (!config_dir[0] || pmm_resolve_hook_command(CMM_SESSION_REMINDER_SCRIPT, expected_command,
    ...
    if (pmm_json_like_read_document(settings_path, &content, &content_length) != 0) {
        free(content);
        return false;
    }
...
#ifdef _WIN32
#define CMM_SUBAGENT_REMINDER_SCRIPT "pmm-subagent-reminder.cmd"
#else
#define CMM_SUBAGENT_REMINDER_SCRIPT "pmm-subagent-reminder"
#endif
#define CMM_SUBAGENT_REMINDER_SCRIPT_LEGACY "pmm-subagent-reminder"

static bool pmm_install_subagent_reminder_script(const char *home, const char *binary_path) {
    if (!home || !binary_path) {
        return false;
    }
    char config_dir[CLI_BUF_1K];
    ...
    char script[CLI_BUF_8K];
    if (pmm_build_current_hook_script(cmm_subagent_script_prefix, binary_path, script,
                                      sizeof(script)) != CLI_OK) {
        return false;
    }
    const char *const legacy[] = {cmm_released_subagent_script};
#ifdef _WIN32
    if (pmm_remove_owned_legacy_hook_script(hooks_dir, CMM_SUBAGENT_REMINDER_SCRIPT_LEGACY, script,
                                            legacy, 1U) != CLI_OK) {
        return false;
    }
#endif
...
int pmm_upsert_claude_subagent_hooks(const char *settings_path) {
    ...
    char previous_legacy_command[CLI_BUF_8K];
    char released_legacy_command[CLI_BUF_8K];
    if (pmm_resolve_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT, command, sizeof(command)) !=
            CLI_OK ||
        pmm_resolve_previous_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT, previous_command,
                                          sizeof(previous_command)) != CLI_OK ||
        pmm_resolve_released_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT, released_command,
                                          sizeof(released_command)) != CLI_OK ||
        pmm_resolve_previous_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT_LEGACY,
                                          previous_legacy_command,
                                          sizeof(previous_legacy_command)) != CLI_OK ||
        pmm_resolve_released_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT_LEGACY,
                                          released_legacy_command,
                                          sizeof(released_legacy_command)) != CLI_OK) {
        return CLI_ERR;
    }
...
#define GEMINI_HOOK_COMMAND                                                         \
    ...
    "project-mind-mcp search_graph, trace_path, and get_code_snippet over grep or " \
    "file search.'}}))\""
static const char *const cmm_gemini_released_hook_commands[] = {
    "echo 'Reminder: prefer project-mind-mcp search_graph/trace_path/get_code_snippet over "
    "grep/file search for code discovery.' >&2",
    "echo 'Reminder: prefer project-mind-mcp search_graph/trace_call_path/get_code_snippet "
...
#ifndef _WIN32
static int pmm_upsert_gemini_coverage_hook(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (pmm_build_augment_dialect_command(binary_path, "gemini", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
    }
    return upsert_hooks_json((hooks_upsert_args_t){
...
static int pmm_remove_gemini_coverage_hook(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (pmm_build_augment_dialect_command(binary_path, "gemini", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
...
#define GEMINI_SESSION_COMMAND                                                       \
    ...
    "project-mind-mcp search_graph, trace_path, get_code_snippet, query_graph, and " \
    "search_code; run index_repository first when needed.'}}))\""
static const char *const cmm_gemini_released_session_commands[] = {
    "echo \"Code discovery: prefer project-mind-mcp (search_graph, trace_path, "
...
static int pmm_upsert_paired_lifecycle_hooks_json(const char *settings_path, const char *command,
...
static int pmm_remove_paired_lifecycle_hooks_json(const char *settings_path,
                                                  const char *canonical_command);

static int pmm_upsert_qwen_lifecycle_hooks(const char *settings_path, const char *binary_path,
                                           bool windows) {
    char command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    char shell[CLI_BUF_32];
    if (pmm_build_qwen_hook_command(binary_path, windows, command, sizeof(command), shell,
                                    sizeof(shell)) != CLI_OK ||
        (windows ? pmm_build_augment_command_windows(binary_path, released_command,
                                                     sizeof(released_command))
                 : pmm_build_augment_command(binary_path, released_command,
                                             sizeof(released_command))) != CLI_OK) {
        return CLI_ERR;
    }
    int legacy_result = pmm_remove_paired_lifecycle_hooks_json(settings_path, released_command);
    int lifecycle_result = pmm_upsert_paired_lifecycle_hooks_json(settings_path, command, NULL,
...
int pmm_upsert_qwen_lifecycle_hooks_for_testing(const char *settings_path, const char *binary_path,
                                                bool windows) {
    return pmm_upsert_qwen_lifecycle_hooks(settings_path, binary_path, windows);
}
#endif
...
static int pmm_remove_qwen_lifecycle_hooks(const char *settings_path, const char *binary_path,
                                           bool windows) {
    char command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    char shell[CLI_BUF_32];
    ...
    int lifecycle_result = pmm_remove_paired_lifecycle_hooks_json(settings_path, command);
    int legacy_result = pmm_remove_paired_lifecycle_hooks_json(settings_path, released_command);
    int read_result = remove_hooks_json((hooks_remove_args_t){
        .settings_path = settings_path,
        .hook_event = "PostToolUse",
...
static int pmm_upsert_factory_hooks(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    if (pmm_build_augment_dialect_command(binary_path, "factory", command, sizeof(command)) !=
            CLI_OK ||
        pmm_build_augment_command(binary_path, released_command, sizeof(released_command)) !=
            CLI_OK) {
        return CLI_ERR;
    }
...
static int pmm_remove_factory_hooks(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    if (pmm_build_augment_dialect_command(binary_path, "factory", command, sizeof(command)) !=
            CLI_OK ||
...
static int pmm_upsert_augment_session_hook(const char *settings_path, const char *script_path) {
    return upsert_hooks_json((hooks_upsert_args_t){
        .settings_path = settings_path,
        .hook_event = "SessionStart",
        .matcher_str = NULL,
...
static int pmm_remove_augment_session_hook(const char *settings_path, const char *script_path) {
    return remove_hooks_json((hooks_remove_args_t){
        .settings_path = settings_path,
        .hook_event = "SessionStart",
        .matcher_str = NULL,
...
static int pmm_upsert_augment_coverage_hook(const char *settings_path, const char *script_path) {
    return upsert_hooks_json((hooks_upsert_args_t){
        .settings_path = settings_path,
        .hook_event = "PostToolUse",
        .matcher_str = "view",
...
static int pmm_remove_augment_coverage_hook(const char *settings_path, const char *script_path) {
    return remove_hooks_json((hooks_remove_args_t){
        .settings_path = settings_path,
        .hook_event = "PostToolUse",
        .matcher_str = "view",
...
int pmm_ensure_path(const char *bin_dir, const char *rc_file, bool dry_run) {
    ...
    bool is_fish = rc_len >= PMM_SZ_5 && strcmp(rc_file + rc_len - PMM_SZ_5, ".fish") == 0;
    ...
    (void)fprintf(f, "\n# Added by project-mind-mcp install\n%s\n", line);
    (void)fclose(f);
    return 0;
}
...
static int cli_windows_open_user_path_key(HKEY *environment) {
    ...
    *environment = NULL;
    ...
    DWORD needed = GetEnvironmentVariableW(L"PMM_TEST_WINDOWS_USER_PATH_RUN_ID", NULL, 0U);
    ...
    if (needed != CLI_BUF_32 + 1U ||
        GetEnvironmentVariableW(L"PMM_TEST_WINDOWS_USER_PATH_RUN_ID", run_id,
...
unsigned char *pmm_extract_binary_from_targz(const unsigned char *data, int data_len,
                                             int *out_len) {
    ...
    /* Parse tar: find entry starting with "project-mind-mcp" */
    size_t pos = 0;
...
unsigned char *pmm_extract_binary_from_zip(const unsigned char *data, int data_len, int *out_len) {
    ...
    *out_len = 0;
    ...
    while (pos + ZIP_HDR_SZ <= data_len) {
        ...
        if (strcmp(basename, "project-mind-mcp") == 0 ||
            strcmp(basename, "project-mind-mcp.exe") == 0) {
...
#endif /* PMM_CLI_ENABLE_TEST_API — tar.gz / zip extraction */

/* ── Index management ─────────────────────────────────────────── */

static const char *get_cache_dir(const char *home_dir) {
    if (!home_dir) {
        home_dir = pmm_get_home_dir();
    }
    if (!home_dir) {
        return NULL;
    }
    return pmm_resolve_cache_dir();
}
...
int pmm_list_indexes(const char *home_dir) {
    ...
    pmm_dir_t *d = pmm_opendir(cache_dir);
    if (!d) {
        return 0;
    }
...
pmm_config_t *pmm_config_open(const char *cache_dir) {
    ...
    pmm_config_t *cfg = calloc(PMM_ALLOC_ONE, sizeof(*cfg));
    if (!cfg) {
        sqlite3_close(db);
        return NULL;
    }
...
const char *pmm_config_get(pmm_config_t *cfg, const char *key, const char *default_val) {
    static PMM_TLS char result_buf[CLI_BUF_4K];
    if (!cfg || !key) {
        return default_val;
    }
    ...
    sqlite3_bind_text(stmt, SQL_PARAM_1, key, SQL_NUL_TERM, pmm_sqlite_transient);
...
bool pmm_config_get_bool(pmm_config_t *cfg, const char *key, bool default_val) {
    const char *val = pmm_config_get(cfg, key, NULL);
    if (!val) {
        return default_val;
    }
...
int pmm_config_set(pmm_config_t *cfg, const char *key, const char *value) {
    ...
    sqlite3_bind_text(stmt, SQL_PARAM_1, key, SQL_NUL_TERM, pmm_sqlite_transient);
    sqlite3_bind_text(stmt, SQL_PARAM_2, value, SQL_NUL_TERM, pmm_sqlite_transient);
...
int pmm_cmd_config(int argc, char **argv) {
    if (argc == 0 || (argv && (strcmp(argv[0], "--help") == 0 || strcmp(argv[0], "-h") == 0))) {
        printf("Usage: project-mind-mcp config <command> [args]\n\n");
        printf("Commands:\n");
    ...
    snprintf(cache_dir, sizeof(cache_dir), "%s", pmm_resolve_cache_dir());

    pmm_config_t *cfg = pmm_config_open(cache_dir);
    if (!cfg) {
    ...
    if (strcmp(argv[0], "list") == 0 || strcmp(argv[0], "ls") == 0) {
        ...
        printf("  %-25s = %-10s\n", PMM_CONFIG_AUTO_INDEX,
               pmm_config_get(cfg, PMM_CONFIG_AUTO_INDEX, "false"));
        printf("  %-25s = %-10s\n", PMM_CONFIG_AUTO_INDEX_LIMIT,
               pmm_config_get(cfg, PMM_CONFIG_AUTO_INDEX_LIMIT, "50000"));
        printf("  %-25s = %-10s\n", PMM_CONFIG_AUTO_WATCH,
               pmm_config_get(cfg, PMM_CONFIG_AUTO_WATCH, "true"));
        printf("  %-25s = %-10s\n", PMM_CONFIG_UI_LANG,
               pmm_config_get(cfg, PMM_CONFIG_UI_LANG, "auto"));
    } else if (strcmp(argv[0], "get") == 0) {
        if (argc < MIN_ARGC_GET) {
            (void)fprintf(stderr, "Usage: config get <key>\n");
            rc = CLI_TRUE;
        } else {
            printf("%s\n", pmm_config_get(cfg, argv[CLI_SKIP_ONE], ""));
        }
        ...
        if (argc < MIN_ARGC_CMD) {
            ...
            if (pmm_config_set(cfg, argv[CLI_SKIP_ONE], argv[CLI_PAIR_LEN]) == 0) {
        ...
        if (argc < MIN_ARGC_GET) {
            ...
            pmm_config_delete(cfg, argv[CLI_SKIP_ONE]);
...
void pmm_set_auto_answer_for_test(int value);
void pmm_set_auto_answer_for_test(int value) {
    g_auto_answer = value;
}
...
#define SHA256_HEX_LEN PMM_SZ_64
...
int pmm_cli_sha256_file(const char *path, char *out, size_t out_size) {
    if (out_size < SHA256_BUF_SIZE) {
        return CLI_ERR;
    }
    FILE *fp = pmm_fopen(path, "rb");
    if (!fp) {
        return CLI_ERR;
    }
    pmm_sha256_ctx ctx;
    ...
    uint8_t digest[PMM_SHA256_DIGEST_LEN];
    pmm_sha256_final(&ctx, digest);
    static const char hex[] = "0123456789abcdef";
    for (int i = 0; i < PMM_SHA256_DIGEST_LEN; i++) {
...
int pmm_cli_checksum_manifest_digest(const char *manifest_path, const char *archive_name, char *out,
                                     size_t out_size) {
    if (out && out_size > 0) {
        out[0] = '\0';
    }
    ...
    FILE *fp = pmm_fopen(manifest_path, "rb");
    if (!fp) {
        return CLI_ERR;
    }
...
static bool cli_download_is_explicit_file_override(const char *url) {
    ...
    const char *override =
        pmm_safe_getenv("PMM_DOWNLOAD_URL", override_buffer, sizeof(override_buffer), NULL);
...
static int pmm_download_to_file(const char *url, const char *dest) {
    ...
    const char *argv[] = {"curl",    "-fSL",   "--progress-bar",
                          ...
                          protocol,  "-o",     dest,
                          url,       NULL};
    return pmm_exec_no_shell(argv);
}

static int pmm_download_to_file_quiet(const char *url, const char *dest) {
    const char *protocol = cli_download_protocol(url);
    if (!protocol || !dest) {
...
#ifdef __APPLE__
static int pmm_macos_adhoc_sign(const char *binary_path) {
...
static int verify_download_checksum(const char *archive_path, const char *archive_name) {
    ...
    int checksum_path_length =
        snprintf(checksum_file, sizeof(checksum_file), "%s/pmm-checksums-XXXXXX", pmm_tmpdir());
    if (checksum_path_length <= 0 || (size_t)checksum_path_length >= sizeof(checksum_file)) {
        return CLI_ERR;
    }
    int checksum_descriptor = pmm_mkstemp(checksum_file);
    ...
    if (checksum_close_status != 0) {
        pmm_unlink(checksum_file);
        return CLI_ERR;
    }

    char dl_base_buf[CLI_BUF_512];
    const char *dl_base =
        pmm_safe_getenv("PMM_DOWNLOAD_URL", dl_base_buf, sizeof(dl_base_buf), NULL);
    ...
    if (dl_base && dl_base[0]) {
        ...
        checksum_url_length =
            snprintf(checksum_url, sizeof(checksum_url), "%s",
                     "https://github.com/usman8786/project-mind-mcp/releases/latest/"
    ...
    int rc = pmm_download_to_file_quiet(checksum_url, checksum_file);
    if (rc != 0) {
    ...
    int manifest_status =
        pmm_cli_checksum_manifest_digest(checksum_file, archive_name, expected, sizeof(expected));
    pmm_unlink(checksum_file);
    ...
    if (pmm_cli_sha256_file(archive_path, actual, sizeof(actual)) != 0) {
...
static void print_detected_agents(const pmm_detected_agents_t *a, const char *home) {
    struct {
        bool flag;
        const char *name;
    } agents[] = {
...
typedef struct {
    pmm_plan_entry_t *items;
    int count;
    int cap;
} pmm_install_plan_t;

static pmm_install_plan_t *g_install_plan = NULL;
...
static void plan_record(const char *agent, const char *kind, const char *path) {
    ...
    pmm_install_plan_t *pl = g_install_plan;
    if (pl->count >= pl->cap) {
        int ncap = pl->cap ? pl->cap * 2 : CLI_BUF_16;
        pmm_plan_entry_t *ni = realloc(pl->items, (size_t)ncap * sizeof(*ni));
        if (!ni) {
            return;
        }
        pl->items = ni;
    ...
    pmm_plan_entry_t *e = &pl->items[pl->count++];
...
static bool prepare_config_parent(const char *path) {
    ...
    *slash = '\0';
    return pmm_mkdir_p(parent, CLI_OCTAL_PERM);
}
...
typedef struct {
    ...
    pmm_graph_profile_dialect_t dialect;
    bool force_handoff;
} pmm_tiered_profile_set_t;

static void install_tiered_agent_profiles(pmm_tiered_profile_set_t profiles, bool dry_run);
static void uninstall_tiered_agent_profiles(pmm_tiered_profile_set_t profiles, bool dry_run);
...
static void install_claude_code_config(const char *home, const char *binary_path, bool force,
    ...
    pmm_claude_user_root(home, user_root, sizeof(user_root));

    char skills_dir[CLI_BUF_1K];
    char agent_path[CLI_BUF_1K];
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
    snprintf(agent_path, sizeof(agent_path), "%s/agents/project-mind.md", config_dir);

    /* Plan mode: record the planned writes and return without mutating (#388). */
    if (g_install_plan) {
        char p[CLI_BUF_1K];
        snprintf(p, sizeof(p), "%s/project-mind/SKILL.md", skills_dir);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_CLAUDE,
            },
            dry_run);
    ...
    int skill_count = pmm_install_skills(skills_dir, force, dry_run);
    ...
    if (pmm_remove_old_monolithic_skill(skills_dir, dry_run)) {
    ...
    if (!dry_run && pmm_file_exists(legacy_mcp_path) &&
        pmm_remove_editor_mcp_owned(binary_path, legacy_mcp_path) != CLI_OK) {
    ...
    if (!dry_run) {
        if (!prepare_config_parent(mcp_path2) ||
            pmm_install_editor_mcp(binary_path, mcp_path2) != CLI_OK) {
    ...
    if (!dry_run) {
        ...
        gate_ok = pmm_install_hook_gate_script(home, binary_path);
        ...
        if (!gate_ok) {
            ...
            (void)pmm_remove_claude_hooks(settings_path);
        } else if (pmm_upsert_claude_hooks(settings_path) != CLI_OK) {
            gate_ok = false;
            record_agent_config_error(false, "Claude Code", "hook_register", settings_path);
        }

        session_ok = pmm_install_session_reminder_script(home, binary_path);
        snprintf(hook_path, sizeof(hook_path), "%s/hooks/%s", config_dir,
                 CMM_SESSION_REMINDER_SCRIPT);
        if (!session_ok) {
            record_agent_config_error(false, "Claude Code", "hook_script_install", hook_path);
            (void)pmm_remove_session_hooks(settings_path);
        } else if (pmm_upsert_session_hooks(settings_path) != CLI_OK) {
            session_ok = false;
            record_agent_config_error(false, "Claude Code", "hook_register", settings_path);
        }

        subagent_ok = pmm_install_subagent_reminder_script(home, binary_path);
        snprintf(hook_path, sizeof(hook_path), "%s/hooks/%s", config_dir,
                 CMM_SUBAGENT_REMINDER_SCRIPT);
        if (!subagent_ok) {
            record_agent_config_error(false, "Claude Code", "hook_script_install", hook_path);
            (void)pmm_remove_claude_subagent_hooks(settings_path);
        } else if (pmm_upsert_claude_subagent_hooks(settings_path) != CLI_OK) {
            subagent_ok = false;
            record_agent_config_error(false, "Claude Code", "hook_register", settings_path);
        }
    }
    ...
    if (instr_path) {
        if (!dry_run) {
            if (!prepare_config_parent(instr_path) ||
                pmm_upsert_instructions(instr_path, agent_instructions_content) != CLI_OK) {
...
static void install_windsurf_config(const char *binary_path, const char *config_path,
    ...
    if (!dry_run) {
        if (!prepare_config_parent(config_path) ||
            pmm_install_editor_mcp(binary_path, config_path) != CLI_OK) {
            record_agent_config_error(false, "Windsurf", "mcp_install", config_path);
        }
        if (!prepare_config_parent(rules_path) ||
            pmm_upsert_windsurf_rules(rules_path, agent_instructions_content) != CLI_OK) {
...
static void install_agent_skill(const char *label, const char *skills_dir, bool force,
                                bool dry_run) {
    char skill_path[CLI_BUF_1K];
    int written = snprintf(skill_path, sizeof(skill_path), "%s/project-mind/SKILL.md", skills_dir);
    ...
    int installed = pmm_install_skills(skills_dir, force, dry_run);
...
static int pmm_tiered_profile_path(const char *verify_path, pmm_graph_tier_t tier, char *output,
                                   size_t output_size) {
    static const char verify_basename[] = "project-mind";
    ...
    const char *slug = pmm_graph_tier_slug(tier);
    if (!slug) {
        return CLI_ERR;
    }
...
static pmm_graph_access_t pmm_tiered_profile_access(pmm_graph_profile_dialect_t dialect) {
    return pmm_graph_dialect_direct_capable(dialect) ? PMM_GRAPH_ACCESS_DIRECT
                                                     : PMM_GRAPH_ACCESS_HANDOFF;
}

static pmm_graph_access_t pmm_tiered_profile_set_access(pmm_tiered_profile_set_t profiles) {
    return !profiles.force_handoff && pmm_graph_dialect_direct_capable(profiles.dialect)
               ? PMM_GRAPH_ACCESS_DIRECT
               : PMM_GRAPH_ACCESS_HANDOFF;
}

static void install_tiered_agent_profiles(pmm_tiered_profile_set_t profiles, bool dry_run) {
    pmm_graph_access_t access = pmm_tiered_profile_set_access(profiles);
    for (int value = 0; value < (int)PMM_GRAPH_TIER_COUNT; value++) {
        pmm_graph_tier_t tier = (pmm_graph_tier_t)value;
        char path[CLI_BUF_1K];
        if (pmm_tiered_profile_path(profiles.verify_path, tier, path, sizeof(path)) != CLI_OK) {
        ...
        char *current =
            pmm_render_graph_profile(profiles.dialect, tier, access, profiles.binary_path);
        if (!current) {
            record_agent_config_error(false, profiles.label, "agent_render", path);
            continue;
        }
        pmm_graph_access_t alternate_access =
            access == PMM_GRAPH_ACCESS_DIRECT ? PMM_GRAPH_ACCESS_HANDOFF : PMM_GRAPH_ACCESS_DIRECT;
        char *alternate = pmm_render_graph_profile(profiles.dialect, tier, alternate_access,
        ...
        if (tier == PMM_GRAPH_TIER_VERIFY && profiles.legacy_verify_content) {
            released[released_count++] = profiles.legacy_verify_content;
        }
        int result = prepare_config_parent(path)
                         ? pmm_text_migrate_owned_document(path, current, released, released_count)
                         : CLI_ERR;
        free(alternate);
        free(current);
        if (result != CLI_OK) {
...
static void uninstall_tiered_agent_profiles(pmm_tiered_profile_set_t profiles, bool dry_run) {
    ...
    for (int value = 0; value < (int)PMM_GRAPH_TIER_COUNT; value++) {
        ...
        int result = pmm_text_remove_owned_document_any(path, current, released, released_count);
        free(alternate);
        free(current);
        if (result < CLI_OK) {
...
static void install_tiered_profile_prompts(const char *label, const char *verify_path,
                                           pmm_graph_profile_dialect_t dialect,
                                           const char *legacy_verify_content, bool dry_run) {
    pmm_graph_access_t access = pmm_tiered_profile_access(dialect);
    for (int value = 0; value < (int)PMM_GRAPH_TIER_COUNT; value++) {
        pmm_graph_tier_t tier = (pmm_graph_tier_t)value;
        char path[CLI_BUF_1K];
        if (pmm_tiered_profile_path(verify_path, tier, path, sizeof(path)) != CLI_OK) {
        ...
        char *current = pmm_render_graph_prompt(tier, access);
        if (!current) {
            record_agent_config_error(false, label, "prompt_render", path);
            continue;
        }
        const char *released[] = {legacy_verify_content};
        size_t released_count = tier == PMM_GRAPH_TIER_VERIFY && legacy_verify_content ? 1U : 0U;
        int result = prepare_config_parent(path)
                         ? pmm_text_migrate_owned_document(path, current, released, released_count)
                         : CLI_ERR;
        free(current);
...
static void install_copilot_durable_context(const char *home, const char *binary_path, bool force,
    ...
    char skills_dir[CLI_BUF_1K];
    char agent_path[CLI_BUF_1K];
    pmm_copilot_config_dir(home, config_dir, sizeof(config_dir));
    snprintf(hooks_dir, sizeof(hooks_dir), "%s/hooks", config_dir);
    snprintf(hook_path, sizeof(hook_path), "%s/hooks/project-mind-mcp.json", config_dir);
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
    snprintf(agent_path, sizeof(agent_path), "%s/agents/project-mind.agent.md", config_dir);
    ...
    install_tiered_agent_profiles(
        (pmm_tiered_profile_set_t){
            ...
            .dialect = PMM_GRAPH_DIALECT_COPILOT,
        },
        dry_run);
    if (g_install_plan) {
    ...
    bool hook_ok = true;
    if (!dry_run && (!pmm_mkdir_p(hooks_dir, CLI_OCTAL_PERM) ||
                     pmm_upsert_copilot_hooks(binary_path, hook_path) != CLI_OK)) {
        hook_ok = false;
...
typedef struct {
    pmm_agent_client_resolve_options_t options;
...
} pmm_agent_registry_context_t;

static const char *pmm_agent_registry_env_path(const char *env_name, const char *home,
                                               char *resolved, size_t resolved_size) {
    char value[CLI_BUF_1K];
    resolved[0] = '\0';
    const char *configured = pmm_safe_getenv(env_name, value, sizeof(value), NULL);
    return configured && configured[0] &&
                   pmm_expand_user_path(home, configured, resolved, resolved_size)
               ? resolved
               : NULL;
}

static bool pmm_agent_registry_path_exists(const char *path, const void *context) {
    (void)context;
    struct stat state;
#ifndef _WIN32
...
static bool pmm_agent_registry_command_exists(const char *command, const void *context) {
    const pmm_agent_registry_context_t *registry = context;
    return registry && pmm_agent_cli_exists(command, registry->options.home_dir);
}

static void pmm_init_agent_registry_context(const char *home,
                                            pmm_agent_registry_context_t *registry) {
    memset(registry, 0, sizeof(*registry));
    registry->options.home_dir = home;
    registry->options.xdg_config_home = pmm_agent_registry_env_path(
        "XDG_CONFIG_HOME", home, registry->xdg_config_home, sizeof(registry->xdg_config_home));
    registry->options.appdata_dir = pmm_agent_registry_env_path(
        "APPDATA", home, registry->appdata_dir, sizeof(registry->appdata_dir));
    registry->options.glab_config_dir = pmm_agent_registry_env_path(
        "GLAB_CONFIG_DIR", home, registry->glab_config_dir, sizeof(registry->glab_config_dir));
    registry->options.kimi_code_home = pmm_agent_registry_env_path(
        "KIMI_CODE_HOME", home, registry->kimi_code_home, sizeof(registry->kimi_code_home));
    registry->options.continue_config_path = pmm_agent_registry_env_path(
        "PMM_CONTINUE_CONFIG_PATH", home, registry->continue_config_path,
        sizeof(registry->continue_config_path));
    registry->options.trae_config_path =
        pmm_agent_registry_env_path("PMM_TRAE_CONFIG_PATH", home, registry->trae_config_path,
                                    sizeof(registry->trae_config_path));
    registry->options.roo_config_path = pmm_agent_registry_env_path(
        "PMM_ROO_CONFIG_PATH", home, registry->roo_config_path, sizeof(registry->roo_config_path));
    registry->options.cody_config_path =
        pmm_agent_registry_env_path("PMM_CODY_CONFIG_PATH", home, registry->cody_config_path,
...
#endif
    registry->options.path_exists = pmm_agent_registry_path_exists;
    registry->options.command_exists = pmm_agent_registry_command_exists;
    registry->options.probe_context = registry;
}

static void print_detected_registry_agents(const char *home, bool *any) {
    pmm_agent_registry_context_t registry;
    pmm_init_agent_registry_context(home, &registry);
    for (size_t index = 0U; index < pmm_agent_client_count(); index++) {
        const pmm_agent_client_profile_t *profile = pmm_agent_client_at(index);
        if (profile && pmm_agent_client_detect(profile->id, &registry.options)) {
...
static void pmm_agent_installed_binary_path(const char *home, char *binary_path,
                                            size_t binary_path_size) {
#ifdef _WIN32
    snprintf(binary_path, binary_path_size, "%s/.local/bin/project-mind-mcp.exe", home);
#else
    snprintf(binary_path, binary_path_size, "%s/.local/bin/project-mind-mcp", home);
#endif
}
...
static void install_managed_agent_instructions(const char *label, const char *instructions_path,
    ...
    if (!dry_run &&
        (!prepare_config_parent(instructions_path) ||
         pmm_upsert_instructions(instructions_path, agent_instructions_content) != CLI_OK)) {
        installed = false;
...
static void install_qoder_durable_context(const char *home, const char *binary_path,
    ...
    snprintf(agent_path, sizeof(agent_path), "%s/.qoder/agents/project-mind.md", home);
    ...
    install_tiered_agent_profiles(
        (pmm_tiered_profile_set_t){
            ...
            .dialect = PMM_GRAPH_DIALECT_QODER,
        },
        dry_run);
    bool hook_supported = pmm_optional_hook_supported("qoder", pmm_current_platform_is_windows());
    if (g_install_plan) {
        if (hook_supported) {
    ...
    if (!dry_run && (!prepare_config_parent(settings_path) ||
                     pmm_upsert_qoder_context_hook(settings_path, binary_path) != CLI_OK)) {
        installed = false;
...
static bool pmm_gitlab_hooks_path(const pmm_agent_registry_context_t *registry, char *path,
                                  size_t path_size) {
...
static bool pmm_devin_user_dir(const pmm_agent_registry_context_t *registry, char *path,
                               size_t path_size) {
    const char *base = registry->options.home_dir;
...
static void install_gitlab_durable_context(const pmm_agent_registry_context_t *registry,
                                           const char *binary_path, bool dry_run) {
    char hooks_path[CLI_BUF_1K];
    if (!pmm_gitlab_hooks_path(registry, hooks_path, sizeof(hooks_path))) {
        record_agent_config_error(false, "GitLab Duo CLI", "hook_resolve", "hooks.json");
        return;
    }
    bool hook_supported = pmm_optional_hook_supported("gitlab", registry->options.is_windows);
    if (g_install_plan) {
        if (hook_supported) {
    ...
    if (!dry_run && (!prepare_config_parent(hooks_path) ||
                     pmm_upsert_gitlab_session_hook(hooks_path, binary_path) != CLI_OK)) {
        installed = false;
...
static void install_devin_durable_context(const pmm_agent_registry_context_t *registry,
    ...
    char skills_dir[CLI_BUF_1K];
    if (!pmm_devin_user_dir(registry, devin_dir, sizeof(devin_dir))) {
    ...
    bool hook_supported = pmm_optional_hook_supported("devin", registry->options.is_windows);
    if (g_install_plan) {
        if (hook_supported) {
    ...
    if (!dry_run && ((inherit_claude_session &&
                      pmm_remove_devin_session_hook(config_path, binary_path) != CLI_OK) ||
                     pmm_upsert_devin_context_hooks(config_path, binary_path,
...
static void install_kimi_durable_context(const pmm_agent_registry_context_t *registry,
    ...
    if (!dry_run && (!prepare_config_parent(config_path) ||
                     pmm_upsert_kimi_context_hook(config_path, binary_path) != CLI_OK)) {
        installed = false;
...
static void install_rovo_durable_context(const char *home, bool force, bool dry_run) {
    ...
    snprintf(agent_path, sizeof(agent_path), "%s/.rovodev/subagents/project-mind.md", home);
    ...
    install_tiered_agent_profiles(
        (pmm_tiered_profile_set_t){
            ...
            .dialect = PMM_GRAPH_DIALECT_ROVO,
        },
        dry_run);
}
...
static void install_codebuddy_durable_context(const char *home, bool force, bool dry_run) {
    ...
    snprintf(agent_path, sizeof(agent_path), "%s/.codebuddy/agents/project-mind.md", home);
    ...
    install_tiered_agent_profiles(
        (pmm_tiered_profile_set_t){
            ...
            .dialect = PMM_GRAPH_DIALECT_CODEBUDDY,
        },
        dry_run);
}
...
static void install_bob_durable_context(const char *home, bool ide, bool force, bool dry_run) {
    ...
    snprintf(rules_path, sizeof(rules_path), "%s/.bob/rules/project-mind.md", home);
...
static void install_pochi_durable_context(const char *home, bool force, bool dry_run) {
    ...
    snprintf(agent_path, sizeof(agent_path), "%s/.pochi/agents/project-mind.md", home);
    ...
    install_tiered_agent_profiles(
        (pmm_tiered_profile_set_t){
            ...
            .dialect = PMM_GRAPH_DIALECT_POCHI,
        },
        dry_run);
}
...
static void install_agent_client_registry(const char *home, const char *binary_path,
    ...
    for (size_t index = 0U; index < pmm_agent_client_count(); index++) {
        ...
        if ((profile->capabilities & PMM_AGENT_CAP_MCP) != 0U) {
            int resolved = pmm_agent_client_resolve_path(profile->id, &registry.options,
...
static void install_gemini_config(const char *home, const char *binary_path, bool dry_run) {
    ...
    snprintf(ap, sizeof(ap), "%s/.gemini/agents/project-mind.md", home);
    install_generic_agent_config("Gemini CLI", binary_path, cp, ip, dry_run,
                                 pmm_install_editor_mcp);
    install_tiered_agent_profiles(
        (pmm_tiered_profile_set_t){
            .label = "Gemini CLI",
            .verify_path = ap,
            ...
            .dialect = PMM_GRAPH_DIALECT_GEMINI,
        },
        dry_run);
    if (g_install_plan) {
...
static void install_cli_agent_configs(const pmm_detected_agents_t *agents, const char *home,
                                      const char *binary_path, bool force, bool dry_run) {
    if (agents->codex) {
        char config_dir[CLI_BUF_1K];
        char cp[CLI_BUF_1K];
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_CODEX,
            },
            dry_run);
        ...
        bool use_hooks_json = pmm_file_exists(hooks_json);
        ...
        if (g_install_plan) {
            ...
            if (!dry_run) {
                ...
                if (pmm_build_augment_command(binary_path, command, sizeof(command)) == CLI_OK &&
                    pmm_build_augment_command_windows(binary_path, command_windows,
                                                      sizeof(command_windows)) == CLI_OK) {
                    if (use_hooks_json) {
                        if (pmm_upsert_paired_lifecycle_hooks_json(
                                hooks_json, command, command_windows, NULL, CMM_HOOK_TIMEOUT_SEC) ==
                            CLI_OK) {
                            if (pmm_remove_codex_hooks(cp) != CLI_OK) {
                                hook_ok = false;
                        ...
                        if (pmm_upsert_codex_hooks_command(cp, command, command_windows) !=
                            CLI_OK) {
                            hook_ok = false;
    ...
    if (agents->opencode) {
        ...
        char skills_dir[CLI_BUF_1K];
        char ap[CLI_BUF_1K];
        pmm_opencode_config_path(home, cp, sizeof(cp));
        ...
        snprintf(ap, sizeof(ap), "%s/.config/opencode/agents/project-mind.md", home);
        install_generic_agent_config("OpenCode", binary_path, cp, ip, dry_run,
                                     pmm_upsert_opencode_mcp);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_OPENCODE,
            },
            dry_run);
    }
    if (agents->antigravity) {
        ...
        if (!dry_run && !g_install_plan) {
            ...
            if (pmm_file_exists(legacy_settings) &&
                pmm_remove_gemini_session_hooks(legacy_settings) != CLI_OK) {
    ...
    if (agents->aider) {
        ...
        if (g_install_plan) {
            ...
            if (!dry_run) {
                ...
                if (pmm_upsert_instructions(ip, aider_instructions_content) != CLI_OK) {
                    record_agent_config_error(false, "Aider", "instructions_install", ip);
                }
                if (pmm_yaml_upsert_string_list_item(cp, "read", ip) != CLI_OK) {
...
static void install_vscode_profile_configs(const char *code_user, const char *binary_path,
    ...
    pmm_dir_t *d = pmm_opendir(profiles_dir);
    if (!d) {
        return;
    }
    pmm_dirent_t *ent;
...
static void uninstall_vscode_profile_configs(const char *code_user, const char *binary_path,
    ...
    char profiles_dir[CLI_BUF_1K];
    snprintf(profiles_dir, sizeof(profiles_dir), "%s/profiles", code_user);
    pmm_dir_t *directory = pmm_opendir(profiles_dir);
    if (!directory) {
        return;
    }
    pmm_dirent_t *entry;
    while ((entry = pmm_readdir(directory)) != NULL) {
        ...
        if (!dry_run && pmm_remove_vscode_mcp_owned(binary_path, config_path) != CLI_OK) {
...
static void install_editor_agent_configs(const pmm_detected_agents_t *agents, const char *home,
                                         const char *binary_path, bool force, bool dry_run) {
    if (agents->zed) {
        char config_dir[CLI_BUF_1K];
        char cp[CLI_BUF_1K];
        char ip[CLI_BUF_1K];
        char skills_dir[CLI_BUF_1K];
        pmm_zed_config_dir(home, config_dir, sizeof(config_dir));
        pmm_zed_instructions_path(home, ip, sizeof(ip));
        snprintf(cp, sizeof(cp), "%s/settings.json", config_dir);
        snprintf(skills_dir, sizeof(skills_dir), "%s/.agents/skills", home);
        install_generic_agent_config("Zed", binary_path, cp, ip, dry_run, pmm_install_zed_mcp);
    ...
    if (agents->kilocode) {
        ...
        snprintf(ip, sizeof(ip), "%s/.config/kilo/rules/project-mind-mcp.md", home);
        snprintf(ap, sizeof(ap), "%s/.config/kilo/agents/project-mind.md", home);
        install_generic_agent_config("KiloCode", binary_path, cp, ip, dry_run, pmm_upsert_kilo_mcp);
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                .label = "KiloCode",
                .verify_path = ap,
                ...
                .dialect = PMM_GRAPH_DIALECT_KILO,
            },
            dry_run);
        if (!dry_run && !g_install_plan) {
            if (pmm_json_like_add_unique_string(cp, "instructions", ip) != CLI_OK) {
            ...
            snprintf(legacy_cp, sizeof(legacy_cp),
                     ...
                     home);
#endif
            snprintf(legacy_ip, sizeof(legacy_ip), "%s/.kilocode/rules/project-mind-mcp.md", home);
            if (pmm_file_exists(legacy_cp)) {
                if (pmm_remove_editor_mcp_owned(binary_path, legacy_cp) != CLI_OK) {
                    record_agent_config_error(false, "KiloCode", "legacy_mcp_cleanup", legacy_cp);
                }
            }
            if (pmm_file_exists(legacy_ip)) {
                if (pmm_remove_instructions(legacy_ip) != CLI_OK) {
...
#else
        snprintf(code_user, sizeof(code_user), "%s/Code/User", pmm_app_config_dir());
#endif
        char cp[CLI_BUF_1K];
    ...
    if (agents->cursor) {
        ...
        snprintf(ap, sizeof(ap), "%s/.cursor/agents/project-mind.md", home);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_CURSOR,
            },
            dry_run);
        ...
        if (!pmm_openclaw_config_path(home, cp, sizeof(cp))) {
            ...
            if (workspace_ok) {
                ...
                if (g_install_plan) {
                    ...
                    if (!dry_run) {
                        if (pmm_upsert_instructions(agents_path, agent_instructions_content) !=
                            CLI_OK) {
                            record_agent_config_error(false, "OpenClaw", "instructions_install",
                                                      agents_path);
                        }
                        if (pmm_upsert_instructions(tools_path, agent_instructions_content) !=
                            CLI_OK) {
                            record_agent_config_error(false, "OpenClaw", "tools_context_install",
                                                      tools_path);
                        }
                        if (pmm_upsert_openclaw_compaction(cp) != CLI_OK) {
                    ...
                    if (compaction_installed) {
                        printf("  compaction: reinjects Project Mind\n");
                    } else {
    ...
    if (agents->kiro) {
        ...
        char skills_dir[CLI_BUF_1K];
        char ap[CLI_BUF_1K];
        pmm_kiro_home_dir(home, kiro_home, sizeof(kiro_home));
        snprintf(cp, sizeof(cp), "%s/settings/mcp.json", kiro_home);
        snprintf(ip, sizeof(ip), "%s/steering/project-mind.md", kiro_home);
        snprintf(skills_dir, sizeof(skills_dir), "%s/skills", kiro_home);
        snprintf(ap, sizeof(ap), "%s/agents/project-mind.json", kiro_home);
        install_generic_agent_config("Kiro", binary_path, cp, ip, dry_run, pmm_install_editor_mcp);
        install_agent_skill("Kiro", skills_dir, force, dry_run);
        char *legacy_agent_content = pmm_build_legacy_kiro_verify_agent_content(binary_path);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_KIRO,
            },
            dry_run);
    ...
    if (agents->junie) {
        ...
        snprintf(agent_path, sizeof(agent_path), "%s/.junie/agents/project-mind.md", home);
        if (!dry_run && !g_install_plan) {
            pmm_mkdir_p(sd, CLI_OCTAL_PERM);
        }
        bool direct_profiles_ready = install_generic_agent_config("Junie", binary_path, cp, NULL,
                                                                  dry_run, pmm_upsert_junie_mcp);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_JUNIE,
...
static void install_additional_agent_configs(const pmm_detected_agents_t *agents, const char *home,
                                             const char *binary_path, bool force, bool dry_run) {
    if (agents->hermes) {
        char hermes_home[CLI_BUF_1K];
        char cp[CLI_BUF_1K];
        char skills_dir[CLI_BUF_1K];
        pmm_hermes_home_dir(home, hermes_home, sizeof(hermes_home));
        ...
        install_generic_agent_config("Hermes", binary_path, cp, NULL, dry_run,
                                     pmm_upsert_hermes_mcp);
        install_agent_skill("Hermes", skills_dir, force, dry_run);
        if (g_install_plan) {
            plan_record("Hermes", "hook", cp);
        } else {
            int hook_result = PMM_YAML_IDENTITY_EDIT_OK;
            if (!dry_run) {
                hook_result = prepare_config_parent(cp)
                                  ? pmm_upsert_hermes_context_hook(cp, binary_path)
                                  : PMM_YAML_IDENTITY_EDIT_ERROR;
            }
            if (hook_result == PMM_YAML_IDENTITY_EDIT_FOREIGN) {
                record_agent_config_error(false, "Hermes", "pre_llm_hook_foreign", cp);
            } else if (hook_result != PMM_YAML_IDENTITY_EDIT_OK) {
    ...
    if (agents->augment) {
        ...
        snprintf(ip, sizeof(ip), "%s/.augment/rules/project-mind.md", home);
        snprintf(ap, sizeof(ap), "%s/.augment/agents/project-mind.md", home);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_AUGMENT,
            },
            dry_run);
        if (g_install_plan) {
            ...
            bool hook_ok = true;
            if (!dry_run) {
                if (!pmm_install_augment_session_script(binary_path, session_hp)) {
                    hook_ok = false;
                    record_agent_config_error(false, "Augment/Auggie", "session_script_install",
                                              session_hp);
                } else if (pmm_upsert_augment_session_hook(cp, session_hp) != CLI_OK) {
                    hook_ok = false;
                    record_agent_config_error(false, "Augment/Auggie", "session_hook_install", cp);
                }
                if (!pmm_install_augment_coverage_script(binary_path, coverage_hp)) {
                    hook_ok = false;
                    record_agent_config_error(false, "Augment/Auggie", "coverage_script_install",
                                              coverage_hp);
                } else if (pmm_upsert_augment_coverage_hook(cp, coverage_hp) != CLI_OK) {
                    hook_ok = false;
    ...
    if (agents->cline) {
        ...
        char ip[CLI_BUF_1K];
        char skills_dir[CLI_BUF_1K];
        pmm_cline_root_dir(home, cline_root, sizeof(cline_root));
        pmm_cline_data_dir(home, cline_data, sizeof(cline_data));
        ...
        snprintf(ip, sizeof(ip), "%s/rules/project-mind-mcp.md", cline_root);
    ...
    if (agents->qwen) {
        ...
        char skills_dir[CLI_BUF_1K];
        char ap[CLI_BUF_1K];
        pmm_qwen_home_dir(home, qwen_home, sizeof(qwen_home));
        ...
        snprintf(ap, sizeof(ap), "%s/agents/project-mind.md", qwen_home);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_QWEN,
            },
            dry_run);
        if (g_install_plan) {
            ...
            bool windows = false;
#endif
            if (pmm_upsert_qwen_lifecycle_hooks(cp, binary_path, windows) != CLI_OK) {
    ...
    if (agents->factory_droid) {
        ...
        snprintf(ap, sizeof(ap), "%s/.factory/droids/project-mind.md", home);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_FACTORY,
            },
            dry_run);
        bool hook_supported =
            pmm_optional_hook_supported("factory", pmm_current_platform_is_windows());
        if (g_install_plan) {
            if (hook_supported) {
            ...
            bool hook_ok = true;
            if (!dry_run) {
                if (pmm_upsert_factory_hooks(hp, binary_path) != CLI_OK) {
                    hook_ok = false;
    ...
    if (agents->crush) {
        char cp[CLI_BUF_1K];
        char ip[CLI_BUF_1K];
        pmm_crush_config_path(home, cp, sizeof(cp));
        snprintf(ip, sizeof(ip), "%s/.config/crush/project-mind.md", home);
        install_generic_agent_config("Crush", binary_path, cp, NULL, dry_run, pmm_upsert_crush_mcp);
        if (g_install_plan) {
            plan_record("Crush", "instructions", ip);
        } else {
            if (!dry_run) {
                if (pmm_upsert_instructions(ip, crush_context_content) != CLI_OK) {
                    record_agent_config_error(false, "Crush", "task_context_install", ip);
                }
                if (pmm_upsert_crush_context_path(cp, ip) != CLI_OK) {
    ...
    if (agents->goose) {
        ...
        char cp[CLI_BUF_1K];
        char ip[CLI_BUF_1K];
        pmm_goose_config_dir(home, config_dir, sizeof(config_dir));
        snprintf(cp, sizeof(cp), "%s/config.yaml", config_dir);
        snprintf(ip, sizeof(ip), "%s/.config/goose/.goosehints", home);
        install_generic_agent_config("Goose", binary_path, cp, ip, dry_run, pmm_upsert_goose_mcp);
    }
    if (agents->mistral_vibe) {
        ...
        pmm_vibe_config_dir(home, config_dir, sizeof(config_dir));
        snprintf(cp, sizeof(cp), "%s/config.toml", config_dir);
        snprintf(ip, sizeof(ip), "%s/AGENTS.md", config_dir);
        snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
        snprintf(ap, sizeof(ap), "%s/agents/project-mind.toml", config_dir);
        snprintf(prompt_path, sizeof(prompt_path), "%s/prompts/project-mind.md", config_dir);
        ...
        install_tiered_agent_profiles(
            (pmm_tiered_profile_set_t){
                ...
                .dialect = PMM_GRAPH_DIALECT_VIBE,
            },
            dry_run);
        install_tiered_profile_prompts("Mistral Vibe", prompt_path, PMM_GRAPH_DIALECT_VIBE,
...
int pmm_install_agent_configs(const char *home, const char *binary_path, bool force, bool dry_run) {
    ...
    pmm_detected_agents_t agents = pmm_detect_agents(home);
    if (!g_install_plan) {
    ...
    bool inherit_claude_session =
        agents.claude_code && !dry_run && pmm_has_complete_claude_session_hooks(home);
...
static int pmm_install_agent_configs_with_previous(const char *home, const char *binary_path,
                                                   const char *previous_managed_binary_path,
                                                   bool force, bool dry_run) {
    const char *saved = g_previous_managed_mcp_command;
    g_previous_managed_mcp_command = previous_managed_binary_path;
    int result = pmm_install_agent_configs(home, binary_path, force, dry_run);
    g_previous_managed_mcp_command = saved;
    return result;
}
 ...
 */
int pmm_install_handle_existing_indexes(const char *home, bool reset, bool dry_run);
static int pmm_install_prepare_existing_indexes(const char *home, bool reset, bool dry_run,
...
int pmm_install_handle_existing_indexes(const char *home, bool reset, bool dry_run) {
    ...
    int prepare_result =
        pmm_install_prepare_existing_indexes(home, reset, dry_run, &delete_indexes);
    if (prepare_result == 1 && delete_indexes) {
        int removed = pmm_remove_indexes(home);
...
static bool pmm_detect_self_path(char *buf, size_t buf_sz, const char *home) {
    buf[0] = '\0';
    bool exact = false;
#ifdef _WIN32
    ...
    if (!buf[0]) {
#ifdef _WIN32
        snprintf(buf, buf_sz, "%s/.local/bin/project-mind-mcp.exe", home);
#else
        snprintf(buf, buf_sz, "%s/.local/bin/project-mind-mcp", home);
#endif
    }
    return exact;
}
...
static char *pmm_build_install_plan_json_options(const char *home, const char *binary_path,
                                                 bool skip_config) {
    if (!home || !binary_path) {
        return NULL;
    }
    ...
    pmm_install_plan_t plan = {0};
    if (!skip_config) {
        g_install_plan = &plan;
        pmm_install_agent_configs(home, binary_path, false, true);
        g_install_plan = NULL;
    }

    pmm_detected_agents_t det = pmm_detect_agents(home);
    struct {
        bool flag;
        const char *name;
    } names[] = {
    ...
    for (int i = 0; i < plan.count; i++) {
        pmm_plan_entry_t *e = &plan.items[i];
    ...
    yyjson_mut_obj_add_str(doc, root, "next_safe_command", "project-mind-mcp install -y");
...
char *pmm_build_install_plan_json(const char *home, const char *binary_path) {
    return pmm_build_install_plan_json_options(home, binary_path, false);
}
...
typedef struct {
    ...
    pmm_activation_transaction_t *binary_transaction;
...
static int cli_install_activate(void *opaque) {
    ...
    bool previous_managed_binary_exact = pmm_detect_self_path(
    ...
    if (!activation->dry_run && !activation->binary_transaction && activation->prepared_candidate) {
        if (!pmm_mkdir_p(activation->bin_dir, CLI_OCTAL_PERM)) {
            (void)fprintf(stderr, "error: cannot create install directory %s\n",
                          activation->bin_dir);
            return CLI_TRUE;
        }
        pmm_activation_transaction_status_t stage_status = pmm_activation_transaction_stage_file(
        ...
        if (stage_status != PMM_ACTIVATION_TRANSACTION_OK || !activation->binary_transaction ||
    ...
    if (!activation->skip_config) {
        agent_config_rc = pmm_install_agent_configs_with_previous(
    ...
    if (activation->shell_rc[0]) {
        path_rc = pmm_ensure_path(activation->bin_dir, activation->shell_rc, activation->dry_run);
        if (path_rc == 0) {
    ...
    if (!activation->dry_run && activation->delete_indexes) {
        ...
        int removed = pmm_remove_indexes(activation->home);
        printf("Removed %d index(es).\n\n", removed);
        if (removed != expected) {
            cli_activation_transaction_finalize_committed_or_fail_stop(
...
int pmm_cmd_install(int argc, char **argv) {
    ...
    pmm_normalize_path_sep(bin_dir);
    char bin_target[CLI_BUF_1K];
#ifdef _WIN32
    int target_length =
        snprintf(bin_target, sizeof(bin_target), "%s/project-mind-mcp.exe", bin_dir);
#else
    int target_length = snprintf(bin_target, sizeof(bin_target), "%s/project-mind-mcp", bin_dir);
#endif
    ...
    if (plan) {
        char *json = pmm_build_install_plan_json_options(home, bin_target, skip_config);
        if (!json) {
    ...
    printf("project-mind-mcp install %s\n\n", PMM_VERSION);

    char self_path[CLI_BUF_1K] = {0};
    (void)pmm_detect_self_path(self_path, sizeof(self_path), home);

    struct stat target_status;
    bool target_exists = (stat(bin_target, &target_status) == 0);
    bool same_binary = pmm_same_file(self_path, bin_target);
    ...
    bool delete_indexes = false;
    if (pmm_install_prepare_existing_indexes(home, reset_indexes, dry_run, &delete_indexes) == 0) {
        return CLI_TRUE;
    }
...
#endif
    pmm_activation_transaction_t *binary_transaction = NULL;
    ...
    if (!dry_run && prepare_binary) {
...
#endif
        bool target_parent_exists = pmm_is_dir(bin_dir);
        ...
        if (prepare_out_of_line) {
            int dir_length =
                snprintf(prepared_dir, sizeof(prepared_dir), "%s/pmm-install-XXXXXX", pmm_tmpdir());
            if (dir_length <= 0 || (size_t)dir_length >= sizeof(prepared_dir) ||
                !pmm_mkdtemp(prepared_dir)) {
            ...
            int candidate_length = snprintf(prepared_candidate, sizeof(prepared_candidate),
                                            "%s/project-mind-mcp.exe", prepared_dir);
#else
            int candidate_length = snprintf(prepared_candidate, sizeof(prepared_candidate),
                                            "%s/project-mind-mcp", prepared_dir);
#endif
            if (candidate_length <= 0 || (size_t)candidate_length >= sizeof(prepared_candidate)) {
                (void)pmm_rmdir(prepared_dir);
                (void)fprintf(stderr, "error: private install staging path is too long\n");
                return CLI_TRUE;
            }
            stage_target = prepared_candidate;
        }
        pmm_activation_transaction_status_t stage_status =
            pmm_activation_transaction_stage_file(stage_target, candidate, &binary_transaction);
        cli_binary_validator_t staged_validator = {{0}};
        if (stage_status != PMM_ACTIVATION_TRANSACTION_OK || !binary_transaction ||
            !cli_activation_transaction_expected_build(binary_transaction, &staged_validator)) {
            const char *stage_refusal = pmm_activation_transaction_refusal_note();
            (void)fprintf(stderr, "error: failed to stage install candidate: %s%s%s\n",
                          pmm_activation_transaction_status_message(stage_status),
        ...
        if (prepare_out_of_line) {
            if (cli_activation_transaction_commit_validated(binary_transaction, &staged_validator,
                ...
                (void)cli_activation_transaction_abort(&binary_transaction);
                (void)pmm_unlink(prepared_candidate);
                (void)pmm_rmdir(prepared_dir);
...
#ifdef __APPLE__
            if (sign_binary && pmm_macos_adhoc_sign(prepared_candidate) != 0) {
            ...
            has_binary_validator =
                pmm_daemon_build_fingerprint_file(prepared_candidate, binary_validator.fingerprint);
            if (has_binary_validator && !g_cli_activation_test_ops_set) {
                const char *candidate_argv[] = {prepared_candidate, "--version", NULL};
                has_binary_validator = pmm_exec_no_shell(candidate_argv) == CLI_OK;
            }
            ...
            if (target_parent_exists) {
                stage_status = pmm_activation_transaction_stage_file(bin_target, prepared_candidate,
                                                                     &binary_transaction);
...
#ifndef _WIN32
    snprintf(shell_rc, sizeof(shell_rc), "%s", pmm_detect_shell_rc(home));
#endif
    ...
    int activation_rc =
        dry_run ? cli_install_activate(&activation)
                : cli_activation_guard(PMM_DAEMON_RUNTIME_ACTIVATION_INSTALL, PMM_VERSION,
...
static void uninstall_claude_code(const char *home, bool dry_run) {
    ...
    pmm_agent_installed_binary_path(home, installed_binary, sizeof(installed_binary));
    char config_dir[CLI_BUF_1K];
    pmm_claude_config_dir(home, config_dir, sizeof(config_dir));
    char user_root[CLI_BUF_1K];
    pmm_claude_user_root(home, user_root, sizeof(user_root));
    ...
    char skills_dir[CLI_BUF_1K];
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
    int removed = pmm_remove_skills(skills_dir, dry_run);
    ...
    if (!dry_run && pmm_remove_editor_mcp_owned(installed_binary, mcp_path) != CLI_OK) {
    ...
    char mcp_path2[CLI_BUF_1K];
    snprintf(mcp_path2, sizeof(mcp_path2), "%s/.claude.json", user_root);
    if (!dry_run && pmm_remove_editor_mcp_owned(installed_binary, mcp_path2) != CLI_OK) {
    ...
    if (!dry_run) {
        ...
        size_t gate_legacy_count = pmm_build_released_gate_script(installed_binary, released_gate,
        ...
        struct {
            ...
            {hook_types[0].name, hook_types[0].legacy_name, current_gate, gate_legacy,
             gate_legacy_count,
             pmm_build_current_hook_script(hook_types[0].prefix, installed_binary, current_gate,
                                           sizeof(current_gate)) == CLI_OK},
            {hook_types[1].name, hook_types[1].legacy_name, current_session, session_legacy, 1U,
             pmm_build_current_hook_script(hook_types[1].prefix, installed_binary, current_session,
                                           sizeof(current_session)) == CLI_OK},
            {hook_types[2].name, hook_types[2].legacy_name, current_subagent, subagent_legacy, 1U,
             pmm_build_current_hook_script(hook_types[2].prefix, installed_binary, current_subagent,
        ...
        for (size_t i = 0; i < sizeof(owned_scripts) / sizeof(owned_scripts[0]); i++) {
            ...
            if (!script_path_valid ||
                pmm_remove_owned_hook_script(script_path, owned_scripts[i].current,
...
#ifdef _WIN32
            if (!hooks_dir_valid ||
                pmm_remove_owned_legacy_hook_script(
...
static void uninstall_agent_mcp_instr(mcp_uninstall_args_t paths, bool dry_run,
    ...
    if (!dry_run) {
        ...
        pmm_agent_installed_binary_path(pmm_get_home_dir(), binary_path, sizeof(binary_path));
    ...
    if (instr_path) {
        if (!dry_run) {
            if (pmm_remove_instructions(instr_path) != CLI_OK) {
...
static void uninstall_copilot_durable_context(const char *home, bool dry_run) {
    ...
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
    snprintf(agent_path, sizeof(agent_path), "%s/agents/project-mind.agent.md", config_dir);
    pmm_agent_installed_binary_path(home, binary_path, sizeof(binary_path));
    if (!dry_run && pmm_remove_copilot_hooks(hook_path, binary_path) != CLI_OK) {
...
static int pmm_remove_managed_instructions(const char *instructions_path) {
    if (pmm_remove_instructions(instructions_path) != CLI_OK) {
        return CLI_ERR;
    }
    struct stat state;
#ifndef _WIN32
    if (lstat(instructions_path, &state) == 0 && S_ISREG(state.st_mode) && state.st_size == 0 &&
        pmm_unlink(instructions_path) != 0) {
#else
...
static void uninstall_qoder_durable_context(const char *home, const char *binary_path,
    ...
    if (!dry_run && config_resolved &&
        pmm_remove_qoder_context_hook(settings_path, binary_path) != CLI_OK) {
        cleanup_ok = false;
...
static void uninstall_gitlab_durable_context(const pmm_agent_registry_context_t *registry,
                                             const char *binary_path, bool dry_run) {
    char hooks_path[CLI_BUF_1K];
    if (!pmm_gitlab_hooks_path(registry, hooks_path, sizeof(hooks_path))) {
    ...
    if (!dry_run && pmm_remove_gitlab_session_hook(hooks_path, binary_path) != CLI_OK) {
...
static void uninstall_devin_durable_context(const pmm_agent_registry_context_t *registry,
                                            const char *binary_path, const char *config_path,
    ...
    bool cleanup_ok = true;
    if (!dry_run && config_resolved &&
        pmm_remove_devin_context_hooks(config_path, binary_path) != CLI_OK) {
        cleanup_ok = false;
...
static void uninstall_pi_durable_context(const char *home, bool dry_run) {
    ...
    snprintf(instructions_path, sizeof(instructions_path), "%s/.pi/agent/AGENTS.md", home);
    snprintf(skills_dir, sizeof(skills_dir), "%s/.pi/agent/skills", home);
    if (!dry_run && pmm_remove_managed_instructions(instructions_path) != CLI_OK) {
...
static bool remove_cline_context_hooks(const char *cline_root, const char *binary_path,
    ...
    for (size_t i = 0U; i < sizeof(cmm_cline_context_events) / sizeof(cmm_cline_context_events[0]);
        ...
        char hook_path[CLI_BUF_1K];
        char script[CLI_BUF_8K];
        if (pmm_cline_hook_path(cline_root, cmm_cline_context_events[i], hook_path,
                                sizeof(hook_path)) != CLI_OK ||
            pmm_build_cline_context_script(binary_path, cmm_cline_context_events[i], script,
                                           sizeof(script)) != CLI_OK) {
            ok = false;
        ...
        int result = pmm_text_remove_owned_document(hook_path, script);
        if (result < 0) {
            ok = false;
...
static void uninstall_kimi_durable_context(const pmm_agent_registry_context_t *registry,
                                           bool dry_run) {
    const char *kimi_home = registry->options.kimi_code_home;
    char default_home[CLI_BUF_1K];
    if (!kimi_home || !kimi_home[0]) {
    ...
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", kimi_home);
    snprintf(config_path, sizeof(config_path), "%s/config.toml", kimi_home);
    if (!dry_run && pmm_remove_kimi_context_hook(config_path) != CLI_OK) {
...
static void uninstall_agent_client_registry(const char *home, bool dry_run) {
    ...
    for (size_t index = 0U; index < pmm_agent_client_count(); index++) {
        const pmm_agent_client_profile_t *profile = pmm_agent_client_at(index);
        if (!profile || !pmm_agent_client_cleanup_candidate(profile->id, &registry.options)) {
            continue;
        }
        printf("%s:\n", profile->display_name);
        char config_path[CLI_BUF_1K] = {0};
...
static void uninstall_cli_agents(const pmm_detected_agents_t *agents, const char *home,
                                 bool dry_run) {
    if (agents->codex) {
        char config_dir[CLI_BUF_1K];
        char cp[CLI_BUF_1K];
        ...
        if (!dry_run) {
            ...
            snprintf(hooks_json, sizeof(hooks_json), "%s/hooks.json", config_dir);
            pmm_agent_installed_binary_path(home, installed_binary, sizeof(installed_binary));
            if (pmm_file_exists(hooks_json) &&
                (pmm_build_augment_command(installed_binary, hook_command, sizeof(hook_command)) !=
                     CLI_OK ||
                 pmm_remove_paired_lifecycle_hooks_json(hooks_json, hook_command) != CLI_OK)) {
    ...
    if (agents->antigravity) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Antigravity", cp, ip}, dry_run,
                                  pmm_remove_antigravity_mcp_owned);
        if (!dry_run) {
            char sp[CLI_BUF_1K];
            snprintf(sp, sizeof(sp), "%s/.gemini/antigravity-cli/settings.json", home);
            if (pmm_remove_gemini_session_hooks(sp) != CLI_OK) {
    ...
    if (agents->aider) {
        ...
        snprintf(ip, sizeof(ip), "%s/CONVENTIONS.md", home);
        if (!dry_run) {
            if (pmm_yaml_remove_string_list_item(cp, "read", ip) != CLI_OK) {
...
static void uninstall_editor_agents(const pmm_detected_agents_t *agents, const char *home,
                                    bool dry_run) {
    char installed_binary[CLI_BUF_1K];
    pmm_agent_installed_binary_path(home, installed_binary, sizeof(installed_binary));
    if (agents->zed) {
    ...
    if (agents->kilocode) {
        ...
        snprintf(ap, sizeof(ap), "%s/.config/kilo/agents/project-mind.md", home);
        if (!dry_run) {
            if (pmm_remove_kilo_mcp_owned(installed_binary, cp) != CLI_OK) {
                record_agent_config_error(true, "KiloCode", "mcp_uninstall", cp);
            }
            if (pmm_json_like_remove_string(cp, "instructions", ip) != CLI_OK) {
            ...
            snprintf(legacy_ip, sizeof(legacy_ip), "%s/.kilocode/rules/project-mind-mcp.md", home);
            if (pmm_file_exists(legacy_cp) &&
                pmm_remove_editor_mcp_owned(installed_binary, legacy_cp) != CLI_OK) {
                record_agent_config_error(true, "KiloCode", "legacy_mcp_uninstall", legacy_cp);
            }
            if (pmm_file_exists(legacy_ip) && pmm_remove_instructions(legacy_ip) != CLI_OK) {
    ...
    if (agents->openclaw) {
        ...
        if (pmm_openclaw_config_path(home, cp, sizeof(cp))) {
            ...
            uninstall_agent_mcp_instr((mcp_uninstall_args_t){"OpenClaw", cp, NULL}, dry_run,
                                      pmm_remove_openclaw_mcp_owned);
            if (!dry_run && pmm_remove_openclaw_compaction(cp) != CLI_OK) {
            ...
            if (workspace_ok) {
                ...
                snprintf(tools_path, sizeof(tools_path), "%s/TOOLS.md", workspace);
                if (!dry_run) {
                    if (pmm_remove_instructions(agents_path) != CLI_OK) {
                        record_agent_config_error(true, "OpenClaw", "instructions_uninstall",
                                                  agents_path);
                    }
                    if (pmm_remove_instructions(tools_path) != CLI_OK) {
    ...
    if (agents->kiro) {
        ...
        char *legacy_agent_content = pmm_build_legacy_kiro_verify_agent_content(installed_binary);
        if (!legacy_agent_content) {
...
static void uninstall_additional_agents(const pmm_detected_agents_t *agents, const char *home,
                                        bool dry_run) {
    char installed_binary[CLI_BUF_1K];
    pmm_agent_installed_binary_path(home, installed_binary, sizeof(installed_binary));
    if (agents->hermes) {
        ...
        pmm_agent_installed_binary_path(home, binary_path, sizeof(binary_path));
        int hook_result =
            dry_run ? PMM_YAML_IDENTITY_EDIT_OK : pmm_remove_hermes_context_hook(cp, binary_path);
        if (hook_result == PMM_YAML_IDENTITY_EDIT_FOREIGN) {
    ...
    if (agents->openhands) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"OpenHands", cp, NULL}, dry_run,
                                  pmm_remove_editor_mcp_owned);
        printf("  removed %d skill(s)\n", pmm_remove_skills(skills_dir, dry_run));
    }
    if (agents->augment) {
        char cp[CLI_BUF_1K];
        char ip[CLI_BUF_1K];
        ...
        if (!dry_run) {
            ...
            if (pmm_build_augment_coverage_script(binary_path, coverage_script,
                                                  sizeof(coverage_script)) != CLI_OK) {
                record_agent_config_error(true, "Augment/Auggie", "coverage_script_build",
                                          coverage_hp);
            } else {
                int script_rc = pmm_text_remove_owned_document(coverage_hp, coverage_script);
                if (script_rc < 0) {
    ...
    if (agents->qwen) {
        ...
        if (!dry_run) {
            ...
            bool windows = false;
#endif
            if (pmm_remove_qwen_lifecycle_hooks(cp, installed_binary, windows) != CLI_OK) {
    ...
    if (agents->factory_droid) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Factory Droid", cp, ip}, dry_run,
                                  pmm_remove_factory_mcp_owned);
        if (!dry_run && pmm_remove_factory_hooks(hp, installed_binary) != CLI_OK) {
    ...
    if (agents->crush) {
        ...
        pmm_crush_config_path(home, cp, sizeof(cp));
        snprintf(ip, sizeof(ip), "%s/.config/crush/project-mind.md", home);
        if (!dry_run && pmm_remove_crush_context_path(cp, ip) != CLI_OK) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Crush", cp, ip}, dry_run,
                                  pmm_remove_crush_mcp_owned);
        char legacy_ip[CLI_BUF_1K];
        snprintf(legacy_ip, sizeof(legacy_ip), "%s/.config/crush/CRUSH.md", home);
        if (!dry_run && pmm_file_exists(legacy_ip) &&
            pmm_remove_instructions(legacy_ip) != CLI_OK) {
    ...
    if (agents->mistral_vibe) {
        ...
        uninstall_tiered_agent_profiles(
            ...
            dry_run);
        uninstall_tiered_profile_prompts("Mistral Vibe", prompt_path, PMM_GRAPH_DIALECT_VIBE,
                                         legacy_vibe_verify_prompt_content, dry_run);
    }
}
...
static int cli_uninstall_activate(void *opaque) {
    ...
    if (activation->delete_indexes && !activation->dry_run) {
        int expected = count_db_indexes(activation->home);
        int idx_removed = pmm_remove_indexes(activation->home);
        printf("Removed %d index(es).\n", idx_removed);
...
int pmm_cmd_uninstall(int argc, char **argv) {
    ...
    printf("project-mind-mcp uninstall\n\n");
...
#ifdef _WIN32
    snprintf(bin_path_storage, sizeof(bin_path_storage), "%s/.local/bin/project-mind-mcp.exe",
             home);
#else
    snprintf(bin_path_storage, sizeof(bin_path_storage), "%s/.local/bin/project-mind-mcp", home);
#endif
    ...
    if (!dry_run && binary_exists) {
        pmm_activation_transaction_status_t stage_status =
            pmm_activation_transaction_stage_removal(bin_path, &binary_transaction);
        if (stage_status != PMM_ACTIVATION_TRANSACTION_OK || !binary_transaction) {
    ...
    if (dry_run) {
        ...
        activation_rc = cli_activation_guard(PMM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL, NULL, NULL,
...
static int cli_update_activate_binary(void *opaque) {
     ...
     * exact-build activation window. Otherwise another PMM process can start
     * after the binary swap but before its MCP/hook entries are refreshed. */
    printf("Refreshing agent configurations...\n");
    if (pmm_install_agent_configs_with_previous(
...
static int extract_and_install_binary(extract_install_args_t args) {
    ...
    if (strcmp(ext, "tar.gz") == 0) {
        bin_data = pmm_extract_binary_from_targz(data, (int)fsize, &bin_len);
    } else {
        bin_data = pmm_extract_binary_from_zip(data, (int)fsize, &bin_len);
    }
    free(data);
    pmm_unlink(tmp_archive);
    ...
    int prepared_dir_length =
        snprintf(prepared_dir, sizeof(prepared_dir), "%s/pmm-update-sign-XXXXXX", pmm_tmpdir());
    bool prepared = prepared_dir_length > 0 && (size_t)prepared_dir_length < sizeof(prepared_dir) &&
                    pmm_mkdtemp(prepared_dir) != NULL;
    ...
    pmm_activation_transaction_t *preparation = NULL;
    stage_status = prepared ? pmm_activation_transaction_stage_bytes(prepared_candidate, bin_data,
                                                                     (size_t)bin_len, &preparation)
                            : PMM_ACTIVATION_TRANSACTION_IO;
    free(bin_data);
    cli_binary_validator_t unsigned_validator = {{0}};
    prepared = stage_status == PMM_ACTIVATION_TRANSACTION_OK && preparation &&
               ...
               pmm_macos_adhoc_sign(prepared_candidate) == CLI_OK &&
               pmm_daemon_build_fingerprint_file(prepared_candidate, validator.fingerprint);
    const char *prepared_argv[] = {prepared_candidate, "--version", NULL};
    prepared = prepared && pmm_exec_no_shell(prepared_argv) == CLI_OK;
    if (prepared) {
        stage_status = pmm_activation_transaction_stage_file(bin_dest, prepared_candidate,
                                                             &binary_transaction);
        cli_binary_validator_t staged_validator = {{0}};
        prepared =
            stage_status == PMM_ACTIVATION_TRANSACTION_OK && binary_transaction &&
            cli_activation_transaction_expected_build(binary_transaction, &staged_validator) &&
...
#else
    stage_status = pmm_activation_transaction_stage_bytes(bin_dest, bin_data, (size_t)bin_len,
                                                          &binary_transaction);
    free(bin_data);
    if (stage_status != PMM_ACTIVATION_TRANSACTION_OK || !binary_transaction ||
...
#ifndef _WIN32
    const char *staged = pmm_activation_transaction_staged_path(binary_transaction);
    const char *candidate_argv[] = {staged, "--version", NULL};
    if (!staged || pmm_exec_no_shell(candidate_argv) != 0) {
    ...
    int activation_rc =
        cli_activation_guard(PMM_DAEMON_RUNTIME_ACTIVATION_UPDATE, NULL, validator.fingerprint,
...
static void build_update_url(char *url, int url_sz, const char *os, const char *arch,
    ...
    const char *base_url =
        pmm_safe_getenv("PMM_DOWNLOAD_URL", base_url_buf, sizeof(base_url_buf), NULL);
    if (!base_url || !base_url[0]) {
        base_url = "https://github.com/usman8786/project-mind-mcp/releases/latest/download";
    }
    ...
    snprintf(url, url_sz, "%s/project-mind-mcp-%s%s-%s%s.%s", base_url, want_ui ? "ui-" : "", os,
             arch, portable, ext);
}
...
static int download_verify_install(const char *url, const char *ext, const char *os,
    ...
    int archive_path_length =
        snprintf(tmp_archive, sizeof(tmp_archive), "%s/pmm-update-XXXXXX", pmm_tmpdir());
    if (archive_path_length <= 0 || (size_t)archive_path_length >= sizeof(tmp_archive)) {
        return CLI_TRUE;
    }
    int archive_descriptor = pmm_mkstemp(tmp_archive);
    ...
    int rc = pmm_download_to_file(url, tmp_archive);
    if (rc != 0) {
    ...
    const char *portable = (strcmp(os, "linux") == 0) ? "-portable" : "";
    snprintf(archive_name, sizeof(archive_name), "project-mind-mcp-%s%s-%s%s.%s",
...
static char *fetch_latest_tag(void) {
    FILE *fp = pmm_popen(
        "curl -sfI https://github.com/usman8786/project-mind-mcp/releases/latest 2>/dev/null", "r");
    if (!fp) {
        return NULL;
    }
    char line[PMM_SZ_512];
    char *tag = NULL;
...
static bool check_already_latest(void) {
    char dl_env[PMM_SZ_256] = "";
    pmm_safe_getenv("PMM_DOWNLOAD_URL", dl_env, sizeof(dl_env), NULL);
    if (dl_env[0]) {
    ...
    int cmp = pmm_compare_versions(latest, PMM_VERSION);
    if (cmp <= 0) {
        if (cmp < 0) {
...
int pmm_cmd_update(int argc, char **argv) {
     ...
     * it IS the update -- and runs while pmm is NOT running. Print the exact
     * command instead of feigning self-update. */
#ifndef PMM_CLI_ENABLE_TEST_API
...
#ifdef PMM_CLI_ENABLE_TEST_API
    if (g_cli_activation_test_ops_set) {
        (void)fprintf(stderr, "*** pmm test seam: portable update flow engaged; the "
...
#endif
    {
...
#else
        char *last_sep = pmm_detect_self_path(self_dir, sizeof(self_dir), pmm_get_home_dir())
                             ? strrchr(self_dir, '/')
                             : NULL;
#endif
        if (last_sep) {
        ...
        printf("project-mind-mcp update (current: %s)\n\n", PMM_VERSION);
#ifdef _WIN32
...
#ifdef _WIN32
        printf("  target: %s/.local/bin/project-mind-mcp.exe\n", home);
#else
        printf("  target: %s/.local/bin/project-mind-mcp\n", home);
#endif
        printf("  variant: %s\n", variant_label);
        printf("  os/arch: %s/%s\n", os, arch);
...
#ifdef _WIN32
    snprintf(bin_dest_storage, sizeof(bin_dest_storage), "%s/.local/bin/project-mind-mcp.exe",
             home);
#else
    snprintf(bin_dest_storage, sizeof(bin_dest_storage), "%s/.local/bin/project-mind-mcp", home);
#endif
    char bin_dir[CLI_BUF_1K];
    snprintf(bin_dir, sizeof(bin_dir), "%s/.local/bin", home);
    if (!pmm_mkdir_p(bin_dir, CLI_OCTAL_PERM)) {
    ...
    printf("\nUpdate complete. Verifying:\n");
    {
        ...
        (void)pmm_exec_no_shell(ver_argv);
    }
...
char *pmm_cli_build_args_json(const char *tool_name, int argc, char **argv, char **err_out) {
    ...
    const char *schema_str = pmm_mcp_tool_input_schema(tool_name);
...
int pmm_cli_print_tool_help(const char *tool_name) {
    ...
    printf("Usage:\n");
    printf("  project-mind-mcp cli %s --flag value [--flag2 value2 ...]\n", tool_name);
    printf("  project-mind-mcp cli %s --args-file <path-to-json>\n", tool_name);
    printf("  echo '<json>' | project-mind-mcp cli %s\n", tool_name);
    printf("  project-mind-mcp cli %s '<raw-json-args>'\n", tool_name);
