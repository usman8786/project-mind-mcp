...
#ifdef PMM_TEXT_EDIT_ENABLE_TEST_API
static PMM_TLS pmm_text_precommit_test_hook_t text_precommit_test_hook = NULL;
static PMM_TLS void *text_precommit_test_context = NULL;
static PMM_TLS pmm_text_precommit_test_hook_t text_prepublish_test_hook = NULL;
static PMM_TLS void *text_prepublish_test_context = NULL;
static PMM_TLS pmm_text_precommit_test_hook_t text_temp_closed_test_hook = NULL;
static PMM_TLS void *text_temp_closed_test_context = NULL;
#endif
...
#ifdef _WIN32
    intptr_t native_handle = _get_osfhandle(pmm_fileno(file));
    return native_handle == -1
               ? TEXT_ERROR
               : text_snapshot_from_handle((HANDLE)(uintptr_t)native_handle, snapshot);
#else
    struct stat state;
    return fstat(pmm_fileno(file), &state) == 0 ? text_snapshot_from_stat(&state, snapshot)
                                                : TEXT_ERROR;
#endif
}
...
static int text_write_atomic_mode(const char *path, const char *new_data, size_t new_len,
    ...
    for (unsigned attempt = 0U; attempt < TEXT_TEMP_ATTEMPTS; attempt++) {
        ...
        int written = snprintf(temp_path, capacity, "%s.pmm-text-%ld-%u.tmp", path,
    ...
    if (!failed && snapshot->exists &&
        fchown(pmm_fileno(file), snapshot->owner, snapshot->group) != 0) {
        failed = 1;
    }
...
#endif
    if (!failed && TEXT_SYNC(pmm_fileno(file)) != 0) {
        failed = 1;
    }
...
#ifdef PMM_TEXT_EDIT_ENABLE_TEST_API
void pmm_text_set_precommit_hook_for_testing(pmm_text_precommit_test_hook_t hook, void *context) {
...
int pmm_text_upsert_managed_block(const char *file_path, const char *begin_marker,
                                  const char *end_marker, const char *owned_content) {
    return text_upsert_managed_block(file_path, begin_marker, end_marker, owned_content,
                                     TEXT_MAX_BYTES);
}

int pmm_text_upsert_managed_block_limited(const char *file_path, const char *begin_marker,
                                          const char *end_marker, const char *owned_content,
                                          size_t max_document_bytes) {
    return text_upsert_managed_block(file_path, begin_marker, end_marker, owned_content,
                                     max_document_bytes);
...
int pmm_text_remove_managed_block(const char *file_path, const char *begin_marker,
...
int pmm_text_write_owned_document(const char *file_path, const char *owned_content) {
    size_t content_len = 0U;
    if (!text_valid_path(file_path) ||
        text_bounded_strlen(owned_content, TEXT_MAX_BYTES, &content_len) != TEXT_OK ||
...
int pmm_text_create_owned_document(const char *file_path, const char *owned_content) {
    size_t content_len = 0U;
    if (!text_valid_path(file_path) ||
        text_bounded_strlen(owned_content, TEXT_MAX_BYTES, &content_len) != TEXT_OK ||
        text_validate_bytes(owned_content, content_len, 1) != TEXT_OK) {
...
int pmm_text_ensure_owned_document(const char *file_path, const char *owned_content) {
    size_t content_len = 0U;
    if (!text_valid_path(file_path) ||
        text_bounded_strlen(owned_content, TEXT_MAX_BYTES, &content_len) != TEXT_OK ||
        text_validate_bytes(owned_content, content_len, 1) != TEXT_OK) {
...
int pmm_text_migrate_owned_document(const char *file_path, const char *current_content,
                                    const char *const *released_contents, size_t released_count) {
    return text_migrate_owned_document(file_path, current_content, released_contents,
                                       released_count, 0, 0U);
}

int pmm_text_migrate_owned_document_mode(const char *file_path, const char *current_content,
...
int pmm_text_remove_owned_document(const char *file_path, const char *expected_owned_content) {
...
int pmm_text_remove_owned_document_any(const char *file_path, const char *current_content,
                                       const char *const *released_contents,
