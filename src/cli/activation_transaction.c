...
struct pmm_activation_transaction {
...
static pmm_activation_transaction_before_absent_publish_for_test_fn
...
#endif
    int written =
        snprintf(path, needed, "%s%s.pmm-%s-%lu-%" PRIu64, directory,
...
static bool activation_posix_acl_empty(int descriptor) {
    return pmm_macos_extended_acl_fd_is_empty(descriptor);
}
...
static bool activation_directory_still_valid(const pmm_activation_transaction_t *transaction) {
#ifdef _WIN32
    int ignored = 0;
...
static activation_create_status_t activation_private_file_create(
    const pmm_activation_transaction_t *transaction, const char *path, const char *name,
...
static pmm_activation_transaction_status_t activation_create_unique(
    const pmm_activation_transaction_t *transaction, const char *tag, char **path_out,
    ...
    *file_out = ACTIVATION_INVALID_FILE;
    for (unsigned int attempt = 0; attempt < 1024U; attempt++) {
        ...
        if (!candidate) {
            return PMM_ACTIVATION_TRANSACTION_NO_MEMORY;
        }
        ...
        if (created == ACTIVATION_CREATE_OK) {
            *path_out = candidate;
            *name_out = name;
            return PMM_ACTIVATION_TRANSACTION_OK;
        }
        free(candidate);
        free(name);
        if (created != ACTIVATION_CREATE_EXISTS) {
            return PMM_ACTIVATION_TRANSACTION_IO;
        }
    }
    return PMM_ACTIVATION_TRANSACTION_IO;
}
...
static bool activation_posix_entry_snapshot_with_links(
    const pmm_activation_transaction_t *transaction, const char *name, nlink_t required_links,
...
static bool activation_entry_snapshot(const pmm_activation_transaction_t *transaction,
...
static bool activation_path_matches(const pmm_activation_transaction_t *transaction,
...
static bool activation_sync_directory(const pmm_activation_transaction_t *transaction) {
#ifdef _WIN32
...
static bool activation_rename(const pmm_activation_transaction_t *transaction, const char *source,
...
static void activation_transaction_destroy(pmm_activation_transaction_t *transaction) {
    if (!transaction) {
        return;
    }
#ifndef _WIN32
...
static pmm_activation_transaction_status_t activation_discard_staged_assets(
    ...
    return ok ? PMM_ACTIVATION_TRANSACTION_OK : PMM_ACTIVATION_TRANSACTION_IO;
}

static pmm_activation_transaction_status_t activation_transaction_prepare(
    ...
    *transaction_out = NULL;
    ...
    pmm_activation_transaction_t *transaction = calloc(1, sizeof(*transaction));
    if (!transaction) {
        return PMM_ACTIVATION_TRANSACTION_NO_MEMORY;
    }
#ifndef _WIN32
    ...
    if (!activation_target_parts(target_path, &transaction->directory_path,
        ...
        activation_transaction_destroy(transaction);
        return PMM_ACTIVATION_TRANSACTION_INVALID_ARGUMENT;
    }
#ifdef _WIN32
    int ignored = 0;
    ...
    if (transaction->target_existed) {
        ...
        pmm_activation_transaction_status_t status = activation_create_unique(
...
static void activation_failed_stage_cleanup(pmm_activation_transaction_t *transaction) {
    if (transaction) {
        (void)activation_discard_staged_assets(transaction);
        activation_transaction_destroy(transaction);
    }
...
#ifdef _WIN32
static bool activation_windows_copy_target_to_backup(pmm_activation_transaction_t *transaction) {
    if (!activation_directory_still_valid(transaction)) {
        return false;
    }
...
#ifndef _WIN32
static bool activation_linked_backup_pair_valid(const pmm_activation_transaction_t *transaction) {
    activation_file_identity_t target_identity;
    activation_file_identity_t backup_identity;
    bool target_exists = false;
...
static bool activation_remove_linked_backup(pmm_activation_transaction_t *transaction) {
...
static bool activation_target_still_original(const pmm_activation_transaction_t *transaction) {
    activation_file_identity_t current;
    bool exists = false;
    if (!activation_entry_snapshot(transaction, transaction->target_path, transaction->target_name,
                                   &exists, &current) ||
...
static bool activation_absent_target_snapshot(const pmm_activation_transaction_t *transaction,
...
static bool activation_remove_absent_published_target(pmm_activation_transaction_t *transaction,
                                                      bool linked_pair) {
    if (!linked_pair) {
...
static pmm_activation_transaction_status_t activation_rollback_internal(
    pmm_activation_transaction_t *transaction) {
    if (transaction->target_existed) {
        bool backup_exists = false;
        if (!transaction->backup_contains_target ||
            ...
            return PMM_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
        }
        bool target_exists = false;
    ...
    return activation_sync_directory(transaction) ? PMM_ACTIVATION_TRANSACTION_OK
...
pmm_activation_transaction_status_t pmm_activation_transaction_commit(
    ...
    if (transaction->state != ACTIVATION_STAGED) {
        return PMM_ACTIVATION_TRANSACTION_INVALID_STATE;
    }
    ...
    if (transaction->target_existed) {
        if (transaction->action == ACTIVATION_REPLACE) {
            ...
            if (published == ACTIVATION_PUBLISH_CHANGED_ERROR) {
                transaction->state = ACTIVATION_RECOVERY_NEEDED;
                pmm_activation_transaction_status_t restored =
                    activation_rollback_internal(transaction);
                return restored == PMM_ACTIVATION_TRANSACTION_OK
                           ? PMM_ACTIVATION_TRANSACTION_IO
                           : PMM_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
            }
        } else {
    ...
    if (!activation_directory_still_valid(transaction) || !activation_sync_directory(transaction)) {
        pmm_activation_transaction_status_t restored = activation_rollback_internal(transaction);
        return restored == PMM_ACTIVATION_TRANSACTION_OK
                   ? PMM_ACTIVATION_TRANSACTION_IO
                   : PMM_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
    }
...
pmm_activation_transaction_status_t pmm_activation_transaction_finalize(
    ...
    if (transaction->backup_contains_target) {
        ...
        if (removed == ACTIVATION_UNLINK_DEFERRED) {
            ...
            return PMM_ACTIVATION_TRANSACTION_DEFERRED;
        }
        transaction->backup_exists = false;
        transaction->backup_contains_target = false;
    }
...
pmm_activation_transaction_status_t pmm_activation_transaction_close(
    ...
    pmm_activation_transaction_t *transaction = *transaction_io;
    if (transaction->state == ACTIVATION_COMMITTED ||
        transaction->state == ACTIVATION_RECOVERY_NEEDED) {
        pmm_activation_transaction_status_t status = activation_rollback_internal(transaction);
        if (status != PMM_ACTIVATION_TRANSACTION_OK) {
            return status;
        }
    }
    if (transaction->state == ACTIVATION_STAGED || transaction->state == ACTIVATION_ROLLED_BACK) {
        pmm_activation_transaction_status_t status = activation_discard_staged_assets(transaction);
        if (status != PMM_ACTIVATION_TRANSACTION_OK) {
            return status;
        }
    }
...
const char *pmm_activation_transaction_target_path(
    const pmm_activation_transaction_t *transaction) {
    return transaction ? transaction->target_path : NULL;
}

const char *pmm_activation_transaction_staged_path(
    const pmm_activation_transaction_t *transaction) {
    return transaction ? transaction->staged_path : NULL;
}

const char *pmm_activation_transaction_backup_path(
    const pmm_activation_transaction_t *transaction) {
