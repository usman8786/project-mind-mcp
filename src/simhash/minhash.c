...
static void weighted_minhash_update(pmm_minhash_t *out, const char *trigram, int len, int w) {
    for (int k = 0; k < PMM_MINHASH_K; k++) {
...
static int hash_trigrams(const char **tokens, int token_count, pmm_minhash_t *out) {
    for (int k = 0; k < PMM_MINHASH_K; k++) {
...
bool pmm_minhash_compute(TSNode func_body, const char *source, int language, pmm_minhash_t *out) {
    ...
    if (token_count < PMM_MINHASH_MIN_NODES) {
        return false;
    }
...
double pmm_minhash_jaccard(const pmm_minhash_t *a, const pmm_minhash_t *b) {
    ...
    return (double)matching / (double)PMM_MINHASH_K;
}

/* ── Hex encoding/decoding ───────────────────────────────────────── */

void pmm_minhash_to_hex(const pmm_minhash_t *fp, char *buf, int bufsize) {
    if (!fp || !buf || bufsize < PMM_MINHASH_HEX_BUF) {
        if (buf && bufsize > 0) {
            buf[0] = '\0';
        }
        return;
...
bool pmm_minhash_from_hex(const char *hex, pmm_minhash_t *out) {
    ...
    if ((int)len != PMM_MINHASH_HEX_LEN) {
        return false;
    }
    for (int i = 0; i < PMM_MINHASH_K; i++) {
...
static uint32_t band_hash(const pmm_minhash_t *fp, int band) {
    int base = band * PMM_LSH_ROWS;
    /* Combine r=2 values into a single hash */
    uint32_t combined[PMM_LSH_ROWS];
    for (int r = 0; r < PMM_LSH_ROWS; r++) {
...
pmm_lsh_index_t *pmm_lsh_new(void) {
    pmm_lsh_index_t *idx = calloc(SKIP_ONE, sizeof(pmm_lsh_index_t));
    return idx;
}
...
void pmm_lsh_insert(pmm_lsh_index_t *idx, const pmm_lsh_entry_t *entry) {
    ...
    if (idx->entry_count >= idx->entry_cap) {
        ...
        pmm_lsh_entry_t *new_entries =
            realloc(idx->entries, (size_t)new_cap * sizeof(pmm_lsh_entry_t));
        if (!new_entries) {
            return;
        }
...
static bool result_push(pmm_lsh_index_t *idx, const pmm_lsh_entry_t *candidate) {
    if (idx->result_count >= idx->result_cap) {
        int new_cap =
            idx->result_cap < RESULT_INIT_CAP ? RESULT_INIT_CAP : idx->result_cap * GROW_FACTOR;
        const pmm_lsh_entry_t **new_buf =
            realloc(idx->result_buf, (size_t)new_cap * sizeof(const pmm_lsh_entry_t *));
        if (!new_buf) {
            return false;
        }
...
void pmm_lsh_query(const pmm_lsh_index_t *idx, const pmm_minhash_t *fp,
    ...
    *count = 0;
    ...
    pmm_lsh_index_t *mut_idx = (pmm_lsh_index_t *)idx;
    ...
    for (int b = 0; b < PMM_LSH_BANDS; b++) {
        ...
        for (int i = 0; i < bucket->count; i++) {
            const pmm_lsh_entry_t *candidate = &idx->entries[bucket->items[i]];
