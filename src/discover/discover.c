...
#include "pmm.h" // CBMLanguage, PMM_LANG_COUNT, PMM_LANG_JSON
...
int pmm_gitignore_match_result(const pmm_gitignore_t *gi, const char *rel_path, bool is_dir);
...
static char *strip_matching_quotes(char *s) {
    size_t len = strlen(s);
    if (len >= PMM_QUOTE_PAIR && ((s[0] == '"' && s[len - SKIP_ONE] == '"') ||
                                  (s[0] == '\'' && s[len - SKIP_ONE] == '\''))) {
        s[len - SKIP_ONE] = '\0';
        return s + SKIP_ONE;
    }
...
static bool expand_git_path(const char *path, char *out, size_t out_sz) {
    ...
    char normalized[PMM_SZ_4K];
    snprintf(normalized, sizeof(normalized), "%s", path);
    pmm_normalize_path_sep(normalized);
...
static bool read_core_excludes_file(const char *config_path, char *out, size_t out_sz) {
    FILE *f = pmm_fopen(config_path, "r");
    if (!f) {
        return false;
    }
...
static bool resolve_xdg_git_config_dir(char *out, size_t out_sz) {
    char env[PMM_SZ_4K];
    if (pmm_safe_getenv("XDG_CONFIG_HOME", env, sizeof(env), NULL) && env[0] != '\0') {
...
bool pmm_should_skip_dir(const char *dirname, pmm_index_mode_t mode) {
    if (!dirname) {
        return false;
    }
...
bool pmm_has_ignored_suffix(const char *filename, pmm_index_mode_t mode) {
    if (!filename) {
        return false;
    }
...
bool pmm_should_skip_filename(const char *filename, pmm_index_mode_t mode) {
    if (!filename) {
        return false;
    }
...
bool pmm_matches_fast_pattern(const char *filename, pmm_index_mode_t mode) {
    if (!filename || mode == PMM_MODE_FULL) {
        return false;
    }
...
static bool file_list_should_stop(file_list_t *fl) {
    ...
    if (!fl->failed && fl->deadline_ms != 0 && pmm_now_ms() >= fl->deadline_ms) {
        fl->failed = true;
    }
...
static void file_list_add_excluded(file_list_t *fl, const char *rel_path) {
    ...
    if (fl->excluded_count >= fl->excluded_cap) {
        int new_cap = fl->excluded_cap ? fl->excluded_cap * PAIR_LEN : PMM_SZ_64;
...
static void file_list_add_ignored(file_list_t *fl, const char *rel_path, const char *reason) {
    ...
    fl->ignored_total++;
    if (fl->ignored_count >= PMM_DISCOVER_IGNORED_CAP) {
        return; /* counted above — truncation stays visible via the total */
    }
    if (fl->ignored_count >= fl->ignored_cap) {
        int new_cap = fl->ignored_cap ? fl->ignored_cap * PAIR_LEN : PMM_SZ_64;
        pmm_ignored_file_t *grown = realloc(fl->ignored, new_cap * sizeof(pmm_ignored_file_t));
        if (!grown) {
            fl->failed = true;
            return;
        }
...
static void fl_add(file_list_t *fl, const char *abs_path, const char *rel_path, CBMLanguage lang,
    ...
    if (fl->count >= fl->capacity) {
        int new_cap = fl->capacity ? fl->capacity * PAIR_LEN : PMM_SZ_256;
        pmm_file_info_t *new_files = realloc(fl->files, new_cap * sizeof(pmm_file_info_t));
        if (!new_files) {
            fl->failed = true;
            return;
        }
    ...
    pmm_file_info_t *fi = &fl->files[fl->count++];
    fi->path = path_copy;
...
/* Non-negatable safety core: built-in skip dirs that a .pmmignore negation
 * can NEVER un-skip. A repo-committed .pmmignore must not be able to defeat
...
static bool should_skip_directory(const char *entry_name, const char *rel_path,
                                  const pmm_discover_opts_t *opts, const pmm_gitignore_t *gitignore,
                                  const pmm_gitignore_t *global_gi,
                                  const pmm_gitignore_t *pmmignore, const pmm_gitignore_t *local_gi,
                                  const char *local_gi_prefix) {
    if (pmm_should_skip_dir(entry_name, opts ? opts->mode : PMM_MODE_FULL)) {
        /* #500: a .pmmignore negation (e.g. "!obj/") whose rule is the last
        ...
        bool unskipped = pmmignore && !is_safety_core_dir(entry_name) &&
                         pmm_gitignore_match_result(pmmignore, rel_path, true) < 0;
        if (!unskipped) {
            return true;
        }
    }
    if (gitignore && pmm_gitignore_matches(gitignore, rel_path, true)) {
        return true;
    }
    bool global_ignored = global_gi && pmm_gitignore_matches(global_gi, rel_path, true);
    if (local_gi) {
        const char *lrel = local_rel_path(rel_path, local_gi_prefix);
        if (pmm_gitignore_matches(local_gi, lrel, true)) {
            return true;
        }
    }
    if (pmmignore) {
        int pmm_result = pmm_gitignore_match_result(pmmignore, rel_path, true);
        if (pmm_result > 0) {
            return true;
        }
        if (pmm_result < 0 && global_ignored) {
            return false;
        }
    }
 ...
 * semantics — including the .pmmignore negation un-ignoring a global-gitignore
...
static const char *file_skip_reason(const char *entry_name, const char *rel_path,
                                    const pmm_discover_opts_t *opts,
                                    const pmm_gitignore_t *gitignore,
                                    const pmm_gitignore_t *global_gi,
                                    const pmm_gitignore_t *pmmignore,
                                    const pmm_gitignore_t *local_gi, const char *local_gi_prefix,
                                    off_t file_size) {
    pmm_index_mode_t mode = opts ? opts->mode : PMM_MODE_FULL;
    if (pmm_has_ignored_suffix(entry_name, mode)) {
        return "ignored-suffix";
    }
    if (pmm_should_skip_filename(entry_name, mode)) {
        return "skip-list";
    }
    if (pmm_matches_fast_pattern(entry_name, mode)) {
        return "fast-pattern";
    }
    if (gitignore && pmm_gitignore_matches(gitignore, rel_path, false)) {
        return "gitignore";
    }
    bool global_ignored = global_gi && pmm_gitignore_matches(global_gi, rel_path, false);
    if (local_gi) {
        const char *lrel = local_rel_path(rel_path, local_gi_prefix);
        if (pmm_gitignore_matches(local_gi, lrel, false)) {
            return "gitignore";
        }
    }
    if (pmmignore) {
        int pmm_result = pmm_gitignore_match_result(pmmignore, rel_path, false);
        if (pmm_result > 0) {
            return "pmmignore";
        }
        if (pmm_result < 0 && global_ignored) {
...
static CBMLanguage detect_file_language(const char *entry_name, const char *abs_path) {
    CBMLanguage lang = pmm_language_for_filename(entry_name);
...
#ifdef _WIN32
    wchar_t *wpath = pmm_path_to_wide(path);
    if (!wpath) {
...
#ifdef _WIN32
    wchar_t *wpath = pmm_path_to_wide(abs_path);
    if (wpath) {
...
static void walk_dir_process_file(const char *abs_path, const char *rel_path, const char *name,
    ...
    const char *skip_reason = file_skip_reason(name, rel_path, opts, gitignore, global_gi,
                                               pmmignore, local_gi, local_gi_prefix, size);
    if (skip_reason) {
...
typedef struct {
    char dir[PMM_SZ_4K];
    char prefix[PMM_SZ_4K];
    pmm_gitignore_t *local_gi;       /* nested .gitignore for this subtree */
    char local_gi_prefix[PMM_SZ_4K]; /* rel_prefix when local_gi was loaded */
} walk_frame_t;
...
static void walk_push_subdir(walk_stack_t *ws, const char *abs_path, const char *rel_path,
    ...
    int directory_length = snprintf(slot->dir, PMM_SZ_4K, "%s", abs_path);
    int prefix_length = snprintf(slot->prefix, PMM_SZ_4K, "%s", rel_path);
    if (directory_length <= 0 || directory_length >= PMM_SZ_4K || prefix_length < 0 ||
        prefix_length >= PMM_SZ_4K) {
        out->failed = true;
        return;
    }
    slot->local_gi = parent->local_gi;
    int local_prefix_length =
        snprintf(slot->local_gi_prefix, PMM_SZ_4K, "%s", parent->local_gi_prefix);
    if (local_prefix_length < 0 || local_prefix_length >= PMM_SZ_4K) {
        out->failed = true;
        return;
    }
    ws->top++;
...
static void walk_dir_process_entry(pmm_dirent_t *entry, const walk_frame_t *frame,
                                   const pmm_discover_opts_t *opts,
                                   const pmm_gitignore_t *gitignore,
                                   const pmm_gitignore_t *global_gi,
                                   const pmm_gitignore_t *pmmignore, walk_stack_t *ws,
                                   file_list_t *out) {
    char abs_path[PMM_SZ_4K];
    char rel_path[PMM_SZ_4K];
    ...
    if (S_ISDIR(st.st_mode)) {
        if (!should_skip_directory(entry->name, rel_path, opts, gitignore, global_gi, pmmignore,
        ...
        walk_dir_process_file(abs_path, rel_path, entry->name, opts, gitignore, global_gi,
                              pmmignore, frame->local_gi, frame->local_gi_prefix, st.st_size, out);
    }
}

static bool walk_owned_gitignore_append(pmm_gitignore_t ***owned, size_t *count, size_t *capacity,
                                        pmm_gitignore_t *gitignore) {
    ...
    if (*count == *capacity) {
        ...
        pmm_gitignore_t **grown = realloc(*owned, next_capacity * sizeof(*grown));
        if (!grown) {
            return false;
        }
        *owned = grown;
...
static void walk_dir(const char *dir_path, const char *rel_prefix, const pmm_discover_opts_t *opts,
                     const pmm_gitignore_t *gitignore, const pmm_gitignore_t *global_gi,
                     const pmm_gitignore_t *pmmignore, file_list_t *out) {
    walk_stack_t ws = {
    ...
    pmm_gitignore_t **owned_gis = NULL;
    size_t owned_count = 0;
    size_t owned_capacity = 0;

    int initial_directory_length = snprintf(ws.frames[0].dir, PMM_SZ_4K, "%s", dir_path);
    int initial_prefix_length = snprintf(ws.frames[0].prefix, PMM_SZ_4K, "%s", rel_prefix);
    if (initial_directory_length <= 0 || initial_directory_length >= PMM_SZ_4K ||
        initial_prefix_length < 0 || initial_prefix_length >= PMM_SZ_4K) {
        out->failed = true;
        free(ws.frames);
        return;
    }
    ...
    while (ws.top > 0 && !file_list_should_stop(out)) {
        ...
        pmm_gitignore_t *loaded = try_load_nested_gitignore(&frame);
        if (loaded) {
            int local_prefix_length =
        ...
        pmm_dir_t *d = pmm_opendir(frame.dir);
        if (!d) {
            if (out->count_only) {
                out->failed = true;
            }
        ...
        pmm_dirent_t *entry;
        while (!file_list_should_stop(out) && (entry = pmm_readdir(d)) != NULL) {
            walk_dir_process_entry(entry, &frame, opts, gitignore, global_gi, pmmignore, &ws, out);
        }
        pmm_closedir(d);
    }
    for (size_t i = 0; i < owned_count; i++) {
        pmm_gitignore_free(owned_gis[i]);
    }
    free(owned_gis);
    free(ws.frames);
}
...
static bool resolve_git_common_dir(const char *repo_path, char *common_dir, size_t cd_sz) {
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        pmm_normalize_path_sep(common_dir);
        return true;
    }
     ...
     * pmm_fopen (not raw fopen) so non-ASCII repo paths open on Windows. */
    FILE *f = pmm_fopen(dot_git, "r");
    if (!f) {
        return false;
    }
    char git_dir[PMM_SZ_4K];
        ...
        if (strncmp(gs, "gitdir:", 7) == 0) {
            ...
            if (val[0] != '\0') {
                if (discover_path_is_absolute(val)) {
                    ...
                    pmm_normalize_path_sep(git_dir);
                } else {
...
int pmm_discover(const char *repo_path, const pmm_discover_opts_t *opts, pmm_file_info_t **out,
                 int *count) {
    return pmm_discover_ex(repo_path, opts, out, count, NULL, NULL);
}

int pmm_discover_ex(const char *repo_path, const pmm_discover_opts_t *opts, pmm_file_info_t **out,
                    int *count, char ***excluded_out, int *excluded_count_out) {
    return pmm_discover_ex2(repo_path, opts, out, count, excluded_out, excluded_count_out, NULL,
                            NULL, NULL);
}

static pmm_discover_status_t discover_impl(const char *repo_path, const pmm_discover_opts_t *opts,
                                           pmm_file_info_t **out, int *count, char ***excluded_out,
                                           int *excluded_count_out,
                                           pmm_ignored_file_t **ignored_out, int *ignored_count_out,
    ...
    *count = 0;
    ...
    pmm_gitignore_t *gitignore = NULL;
    char gi_path[PMM_SZ_4K];
    struct stat gi_stat;
    ...
    if (is_git_repo) {
        ...
        if (git_exclude) {
            if (!gitignore) {
                ...
                (void)pmm_gitignore_merge(gitignore, git_exclude);
                pmm_gitignore_free(git_exclude);
            }
        }
    }

    pmm_gitignore_t *global_gi = NULL;
    if (has_git_config && resolve_global_excludes_path(gi_path, sizeof(gi_path))) {
        global_gi = pmm_gitignore_load(gi_path);
    }

    /* Load pmmignore if specified or exists at repo root */
    pmm_gitignore_t *pmmignore = NULL;
    if (opts && opts->ignore_file) {
        pmmignore = pmm_gitignore_load(opts->ignore_file);
    } else {
        snprintf(gi_path, sizeof(gi_path), "%s/.pmmignore", repo_path);
        pmmignore = pmm_gitignore_load(gi_path);
    }
    ...
    walk_dir(repo_path, "", opts, gitignore, global_gi, pmmignore, &fl);

    /* Cleanup */
    pmm_gitignore_free(gitignore);
    pmm_gitignore_free(global_gi);
    pmm_gitignore_free(pmmignore);

    if (count_only) {
        pmm_discover_free(fl.files, fl.count);
        pmm_discover_free_excluded(fl.excluded, fl.excluded_count);
        pmm_discover_free_ignored(fl.ignored, fl.ignored_count);
        *count = fl.count;
        if (fl.failed) {
            return PMM_DISCOVER_ERROR;
        }
        return fl.limit_exceeded ? PMM_DISCOVER_LIMIT_EXCEEDED : PMM_DISCOVER_OK;
    }
    if (fl.failed) {
        pmm_discover_free(fl.files, fl.count);
        pmm_discover_free_excluded(fl.excluded, fl.excluded_count);
    ...
    *count = fl.count;
    ...
    return PMM_DISCOVER_OK;
}

int pmm_discover_ex2(const char *repo_path, const pmm_discover_opts_t *opts, pmm_file_info_t **out,
                     int *count, char ***excluded_out, int *excluded_count_out,
                     pmm_ignored_file_t **ignored_out, int *ignored_count_out,
...
pmm_discover_status_t pmm_discover_count_bounded(const char *repo_path,
                                                 const pmm_discover_opts_t *opts, int max_files,
    ...
    int count = 0;
    pmm_discover_status_t status = discover_impl(repo_path, opts, &files, &count, NULL, NULL, NULL,
                                                 NULL, NULL, true, max_files, deadline_ms);
    pmm_discover_free(files, count);
    *count_out = status == PMM_DISCOVER_ERROR ? -1 : count;
    return status;
}

void pmm_discover_free(pmm_file_info_t *files, int count) {
    if (!files) {
        return;
    }
...
void pmm_discover_free_excluded(char **excluded, int count) {
    if (!excluded) {
        return;
    }
    for (int i = 0; i < count; i++) {
...
void pmm_discover_free_ignored(pmm_ignored_file_t *ignored, int count) {
    if (!ignored) {
        return;
    }
    for (int i = 0; i < count; i++) {
