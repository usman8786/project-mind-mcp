 ...
 *
 * Provides:
 *   - Language detection from filename/extension (PMM_SZ_64 languages)
 ...
 *
 * Depends on: foundation (platform.h for file ops), pmm.h (CBMLanguage enum)
 */
#ifndef PMM_DISCOVER_H
#define PMM_DISCOVER_H
 ...
 * Returns PMM_LANG_COUNT if unknown. */
 ...
 * Returns "Unknown" for PMM_LANG_COUNT or out-of-range values. */
const char *pmm_language_name(CBMLanguage lang);

/* Disambiguate .m files by reading first 4KB of content.
 * Returns PMM_LANG_OBJC, PMM_LANG_MAGMA, or PMM_LANG_MATLAB.
 * On read failure, defaults to PMM_LANG_MATLAB. */
CBMLanguage pmm_disambiguate_m(const char *path);

/* Disambiguate .cls files by reading first 4KB of content.
 * Returns PMM_LANG_OBJECTSCRIPT_UDL if a line starts with "Class <Uppercase>",
 * otherwise PMM_LANG_APEX. On read failure, defaults to PMM_LANG_APEX. */
CBMLanguage pmm_disambiguate_cls(const char *path);

/* Disambiguate .inc files by reading first 4KB of content.
 * Returns PMM_LANG_OBJECTSCRIPT_ROUTINE if it looks like an ObjectScript
 * include (a "ROUTINE <Uppercase>" header), otherwise PMM_LANG_BITBAKE.
 * On read failure, defaults to PMM_LANG_BITBAKE. */
CBMLanguage pmm_disambiguate_inc(const char *path);

/* ── Gitignore pattern matching ──────────────────────────────────── */

typedef struct pmm_gitignore pmm_gitignore_t;

/* Parse gitignore patterns from a file. Returns NULL on error (file not found, etc.).
 * Caller must call pmm_gitignore_free(). */
pmm_gitignore_t *pmm_gitignore_load(const char *path);

/* Parse gitignore patterns from a string (for testing).
 * Caller must call pmm_gitignore_free(). */
pmm_gitignore_t *pmm_gitignore_parse(const char *content);

/* Check if a relative path matches any gitignore pattern.
 * rel_path should use '/' separators. is_dir indicates if path is a directory. */
bool pmm_gitignore_matches(const pmm_gitignore_t *gi, const char *rel_path, bool is_dir);

/* Free a gitignore matcher. NULL-safe. */
void pmm_gitignore_free(pmm_gitignore_t *gi);
...
bool pmm_gitignore_merge(pmm_gitignore_t *dst, const pmm_gitignore_t *src);
...
typedef enum {
    PMM_MODE_FULL = 0,     /* parse everything supported */
    PMM_MODE_MODERATE = 1, /* aggressive filtering + similarity/semantic edges */
    PMM_MODE_FAST = 2,     /* aggressive filtering + no similarity/semantic edges */
} pmm_index_mode_t;
#endif
...
typedef struct {
    pmm_index_mode_t mode;   /* PMM_MODE_FULL or PMM_MODE_FAST */
    const char *ignore_file; /* path to .pmmignore file, or NULL */
    int64_t max_file_size;   /* 0 = no limit */
} pmm_discover_opts_t;

typedef enum {
    PMM_DISCOVER_ERROR = -1,
    PMM_DISCOVER_OK = 0,
    PMM_DISCOVER_LIMIT_EXCEEDED = 1,
} pmm_discover_status_t;
 ...
 * Caller must call pmm_discover_free() on the results. */
 ...
 * allocation. deadline_ms is an absolute pmm_now_ms() deadline; zero disables
...
/* Like pmm_discover(), but also reports the directory subtrees that were
 ...
 * owns it and must free via pmm_discover_free_excluded(). Pass NULL for
...
typedef struct {
    ...
    char *reason;   /* heap-allocated: "gitignore" | "pmmignore" |
...
enum { PMM_DISCOVER_IGNORED_CAP = 2000 };

/* Like pmm_discover_ex(), but additionally reports the individual files that
 * ignore rules dropped (#963 "purposely not indexed"). *ignored_out receives
 * a heap array (caller frees via pmm_discover_free_ignored),
 * *ignored_count_out its stored length (<= PMM_DISCOVER_IGNORED_CAP), and
...
/* Free the excluded-directory list returned by pmm_discover_ex(). NULL-safe. */
void pmm_discover_free_excluded(char **excluded, int count);

/* Free the ignored-file list returned by pmm_discover_ex2(). NULL-safe. */
void pmm_discover_free_ignored(pmm_ignored_file_t *ignored, int count);

#endif /* PMM_DISCOVER_H */
