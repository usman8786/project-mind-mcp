...
static void gi_add_pattern(pmm_gitignore_t *gi, const char *line, int len) {
...
pmm_gitignore_t *pmm_gitignore_parse(const char *content) {
    ...
    pmm_gitignore_t *gi = calloc(PMM_ALLOC_ONE, sizeof(pmm_gitignore_t));
    if (!gi) {
        return NULL;
    }
...
pmm_gitignore_t *pmm_gitignore_load(const char *path) {
    ...
    pmm_gitignore_t *gi = pmm_gitignore_parse(buf);
    free(buf);
    return gi;
}
...
bool pmm_gitignore_matches(const pmm_gitignore_t *gi, const char *rel_path, bool is_dir) {
    return pmm_gitignore_match_result(gi, rel_path, is_dir) > 0;
}
...
char *(*pmm_gitignore_merge_dup_hook_for_test)(const char *) = NULL;
...
bool pmm_gitignore_merge(pmm_gitignore_t *dst, const pmm_gitignore_t *src) {
    ...
    for (int i = 0; i < src->count; i++) {
        char *pat = pmm_gitignore_merge_dup_hook_for_test
                        ? pmm_gitignore_merge_dup_hook_for_test(src->patterns[i].pattern)
