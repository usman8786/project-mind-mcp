#include "pmm.h"
#include "arena.h" // CBMArena, pmm_arena_strndup
#include "helpers.h"
...
static const char *extract_env_key_from_call(CBMExtractCtx *ctx, TSNode node,
    ...
    char *callee = pmm_node_text(ctx->arena, func_node, ctx->source);
    if (!callee) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < ts_node_child_count(args); i++) {
        ...
        char *arg_text = pmm_node_text(ctx->arena, child, ctx->source);
...
static const char *extract_env_key_from_member(CBMExtractCtx *ctx, TSNode node,
    ...
    for (const char **pat = spec->env_access_member_patterns; *pat; pat++) {
        ...
        if (strncmp(text, *pat, plen) == 0 && text[plen] == '.') {
            const char *key = text + plen + PMM_QUOTE_OFFSET;
        ...
        if (strncmp(text, *pat, plen) == 0 && text[plen] == '[') {
            const char *inner = text + plen + PMM_QUOTE_OFFSET;
            size_t ilen = strlen(inner);
            if (ilen > 0 && inner[ilen - PMM_QUOTE_OFFSET] == ']') {
                char *bracket_content =
                    pmm_arena_strndup(ctx->arena, inner, ilen - PMM_QUOTE_OFFSET);
...
static void walk_env_accesses(CBMExtractCtx *ctx, TSNode root, const CBMLangSpec *spec) {
    ...
    while (stack.count > 0) {
        ...
        if (env_key && env_key[0] && is_env_var_name(env_key)) {
            CBMEnvAccess ea;
            ea.env_key = env_key;
            ea.enclosing_func_qn = pmm_enclosing_func_qn_cached(ctx, node);
            pmm_envaccess_push(&ctx->result->env_accesses, ctx->arena, ea);
...
void handle_env_accesses(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec,
    ...
    if (has_funcs && spec->call_node_types && pmm_kind_in_set(node, spec->call_node_types)) {
        env_key = extract_env_key_from_call(ctx, node, spec);
