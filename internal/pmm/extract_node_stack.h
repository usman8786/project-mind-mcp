 ...
 * no extra memory.
 */
#ifndef PMM_EXTRACT_NODE_STACK_H
#define PMM_EXTRACT_NODE_STACK_H
...
static inline void ts_nstack_init(TSNodeStack *s, CBMArena *arena, int initial_cap) {
    s->items = (TSNode *)pmm_arena_alloc(arena, (size_t)initial_cap * sizeof(TSNode));
    s->count = 0;
...
static inline void ts_nstack_push(TSNodeStack *s, CBMArena *arena, TSNode node) {
    if (s->count >= s->cap) {
        ...
        TSNode *new_items = (TSNode *)pmm_arena_alloc(arena, (size_t)new_cap * sizeof(TSNode));
        if (!new_items)
...
#endif /* PMM_EXTRACT_NODE_STACK_H */
