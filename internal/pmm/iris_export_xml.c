...
static void ub_init(UdlBuf *b, CBMArena *arena) {
    b->buf = (char *)pmm_arena_alloc(arena, BUF_CAP);
    b->pos = 0;
    b->cap = BUF_CAP;
    if (b->buf)
        b->buf[0] = '\0';
...
char **pmm_iris_export_to_udl(CBMArena *arena, const char *xml, int xml_len, int *class_count) {
    if (class_count)
        *class_count = 0;
    ...
    if (!count)
        return NULL;
    char **arr = (char **)pmm_arena_alloc(arena, (size_t)(count + 1) * sizeof(char *));
    if (!arr)
        return NULL;
