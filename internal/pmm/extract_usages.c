...
static bool is_inside_call(TSNode node, const CBMLangSpec *spec) {
    ...
    while (!ts_node_is_null(cur) && depth < MAX_PARENT_DEPTH) {
        if (pmm_kind_in_set(cur, spec->call_node_types)) {
            return true;
        }
...
static bool is_inside_import(TSNode node, const CBMLangSpec *spec) {
    ...
    int depth = 0;
    while (!ts_node_is_null(cur) && depth < MAX_PARENT_DEPTH) {
        if (pmm_kind_in_set(cur, spec->import_node_types)) {
            return true;
        }
        cur = ts_node_parent(cur);
        depth++;
...
static void try_emit_usage(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec) {
    ...
    if (name && name[0] && !pmm_is_keyword(name, ctx->language)) {
        CBMUsage usage;
        usage.ref_name = name;
        usage.enclosing_func_qn = pmm_enclosing_func_qn_cached(ctx, node);
        pmm_usages_push(&ctx->result->usages, ctx->arena, usage);
    }
}
