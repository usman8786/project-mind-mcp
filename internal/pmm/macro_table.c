...
void pmm_macro_table_free(CBMMacroTable *t) {
    if (!t)
        return;
    pmm_arena_destroy(&t->arena);
    free(t);
}

void pmm_macro_table_init_system(CBMMacroTable *t) {
    t->count = 0;
    for (int i = 0; SYSTEM_MACROS[i].name; i++) {
        if (t->count >= PMM_MACRO_TABLE_CAP)
            break;
        ...
        for (int p = 0; p < PMM_MACRO_MAX_PARAMS; p++)
            e->param_names[p] = NULL;
    }
}

void pmm_macro_table_add(CBMMacroTable *t, CBMArena *arena, const char *name, int param_count,
                         const char **param_names, const char *expansion,
                         const char *resolved_callee) {
    if (t->count >= PMM_MACRO_TABLE_CAP || !name)
        return;
    ...
    CBMMacroEntry *e = &t->entries[t->count++];
    e->name = pmm_arena_strdup(arena, name);
    e->param_count = param_count;
    e->expansion = expansion ? pmm_arena_strdup(arena, expansion) : NULL;
    e->resolved_callee = resolved_callee ? pmm_arena_strdup(arena, resolved_callee) : NULL;
    for (int p = 0; p < PMM_MACRO_MAX_PARAMS; p++) {
        e->param_names[p] =
            (param_names && p < param_count) ? pmm_arena_strdup(arena, param_names[p]) : NULL;
    }
}

const CBMMacroEntry *pmm_macro_table_find(const CBMMacroTable *t, const char *name) {
    if (!t || !name)
        return NULL;
    for (int i = 0; i < t->count; i++) {
        if (strcasecmp(t->entries[i].name, name) == 0)
...
void pmm_parse_inc_file(CBMMacroTable *t, CBMArena *arena, const char *content) {
    if (!content)
        return;
    ...
    while (*line) {
        ...
        if (strncmp(p, "#define", 7) == 0 && (p[7] == ' ' || p[7] == '\t')) {
            ...
            int param_count = -1;
            char param_names_buf[PMM_MACRO_MAX_PARAMS][64];
            const char *param_name_ptrs[PMM_MACRO_MAX_PARAMS] = {NULL};
            ...
            if (*p == '(') {
                ...
                while (*p && *p != ')' && p < end) {
                    ...
                    while (plen > 0 && (pn_start[plen - 1] == ' ' || pn_start[plen - 1] == '\t'))
                        plen--;
                    if (plen > 0 && param_count < PMM_MACRO_MAX_PARAMS) {
            ...
            char *expansion = NULL;
            if (explen > 0) {
                expansion = pmm_arena_strndup(arena, p, explen);
            }

            pmm_macro_table_add(t, arena, name, param_count,
...
char *pmm_macro_expand(CBMArena *arena, const CBMMacroEntry *entry, const char **args,
                       int arg_count) {
    ...
    while (*p && out < (int)sizeof(buf) - 1) {
        if (*p == '%') {
            bool matched = false;
            for (int i = 0; i < entry->param_count && i < PMM_MACRO_MAX_PARAMS; i++) {
    ...
    buf[out] = '\0';
    return pmm_arena_strdup(arena, buf);
}

char *pmm_macro_extract_callee(CBMArena *arena, const char *expansion) {
    if (!expansion)
        return NULL;
    ...
    if (p) {
        ...
        if (clen <= 0 || mlen <= 0)
            return NULL;
        return pmm_arena_sprintf(arena, "%.*s.%.*s", clen, p, mlen, method_start);
    }
    ...
    if (p && p[2] != '$') {
        ...
        if (*tag_end == '^') {
            ...
            if (tlen > 0 && rlen > 0) {
                return pmm_arena_sprintf(arena, "%.*s^%.*s", tlen, p, rlen, rtn);
            }
        }
    }
