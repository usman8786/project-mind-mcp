#include "pmm.h"
#include "arena.h" // CBMArena, pmm_arena_sprintf
#include "helpers.h"
...
#include "service_patterns.h" // pmm_service_pattern_route_method (#952)
...
enum { MAX_STRING_ARG_LEN = PMM_SZ_512 };
...
static const char *strip_quotes(CBMArena *a, const char *text) {
    ...
    if (len >= PMM_QUOTE_PAIR && (text[0] == '"' || text[0] == '\'')) {
        return pmm_arena_strndup(a, text + PMM_QUOTE_OFFSET, (size_t)(len - PMM_QUOTE_PAIR));
    }
    return text;
}
...
// language rows.
const char **pmm_string_dispatch_suffixes(CBMLanguage lang) {
 ...
 * Without this, pmm_node_text returns full text including args/parens.
...
static char *resolve_chained_selector(CBMArena *a, TSNode sel, const char *source) {
    ...
    if (ts_node_is_null(operand) || ts_node_is_null(field) ||
        ...
        return pmm_node_text(a, sel, source);
    }
    ...
    char *method = pmm_node_text(a, field, source);
    ...
    for (int depth = 0; depth < MAX_CHAIN_DEPTH; depth++) {
        ...
        char *base = pmm_node_text(a, fn, source);
        if (base && method) {
            return pmm_arena_sprintf(a, "%s.%s", base, method);
        }
        return method;
    }
...
static char *extract_constructor_callee(CBMArena *a, TSNode node, const char *source,
    ...
    for (const char **f = type_fields; *f; f++) {
        ...
        if (!ts_node_is_null(tn)) {
            ...
            char *t = strip_generic_args(pmm_node_text(a, tn, source));
            if (t && t[0]) {
                return t;
            }
        }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "identifier") == 0 ||
            ...
            char *t = strip_generic_args(pmm_node_text(a, child, source));
            if (t && t[0]) {
                return t;
            }
        }
...
static char *extract_callee_from_fields(CBMArena *a, TSNode node, const char *source) {
    ...
    if (!ts_node_is_null(func_node)) {
        ...
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "simple_identifier") == 0 ||
            ...
            return pmm_node_text(a, func_node, source);
        }
        ...
        if (strcmp(fk, "extract_operator") == 0) {
            ...
            if (!ts_node_is_null(rhs)) {
                char *rt = pmm_node_text(a, rhs, source);
                if (!ts_node_is_null(lhs)) {
                    char *lt = pmm_node_text(a, lhs, source);
                    if (lt && lt[0] && rt && rt[0]) {
                        return pmm_arena_sprintf(a, "%s.%s", lt, rt);
                    }
                }
                return rt;
            }
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (!ts_node_is_null(obj) && name) {
            char *obj_text = pmm_node_text(a, obj, source);
            if (obj_text && obj_text[0]) {
                return pmm_arena_sprintf(a, "%s.%s", obj_text, name);
            }
        }
        return name;
    }
    ...
    if (!ts_node_is_null(method_node)) {
        char *method = pmm_node_text(a, method_node, source);
        TSNode recv = ts_node_child_by_field_name(node, TS_FIELD("receiver"));
        if (!ts_node_is_null(recv) && method) {
            char *recv_text = pmm_node_text(a, recv, source);
            if (recv_text && recv_text[0]) {
                return pmm_arena_sprintf(a, "%s.%s", recv_text, method);
            }
        }
        return method;
    }
...
static char *extract_fp_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "apply") == 0 || strcmp(nk, "application_expression") == 0 ||
        ...
        if (ts_node_child_count(node) > 0) {
            ...
            if (strcmp(ck, "identifier") == 0 || strcmp(ck, "variable") == 0 ||
                ...
                return pmm_node_text(a, callee, source);
            }
    ...
    if (strcmp(nk, "infix") == 0 || strcmp(nk, "infix_expression") == 0) {
        ...
        if (!ts_node_is_null(op)) {
            return pmm_node_text(a, op, source);
        }
        enum { INFIX_MIN_CHILDREN = 3, INFIX_OP_IDX = 1 };
        if (ts_node_child_count(node) >= INFIX_MIN_CHILDREN) {
            return pmm_node_text(a, ts_node_child(node, INFIX_OP_IDX), source);
        }
    }
    return NULL;
}
...
static char *extract_wolfram_callee(CBMArena *a, TSNode node, const char *source) {
    ...
    if (ts_node_named_child_count(node) > 0) {
        ...
        if (strcmp(hk, "user_symbol") == 0 || strcmp(hk, "builtin_symbol") == 0) {
            return pmm_node_text(a, head, source);
        }
    }
    return NULL;
}
...
static char *extract_scripting_callee(CBMArena *a, TSNode node, const char *source,
    ...
    if (lang == PMM_LANG_ELIXIR && strcmp(nk, "call") == 0 && ts_node_child_count(node) > 0) {
        TSNode first = ts_node_child(node, 0);
        const char *fk = ts_node_type(first);
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "dot") == 0) {
            return pmm_node_text(a, first, source);
        }
        return NULL;
    }
    if (lang == PMM_LANG_PERL && ts_node_child_count(node) > 0) {
        ...
        char *pn = pmm_node_text(a, name_node, source);
    ...
    if (lang == PMM_LANG_PHP) {
        ...
        char *pn = ts_node_is_null(func_node) ? NULL : pmm_node_text(a, func_node, source);
        return pn;
    }
    if (lang == PMM_LANG_KOTLIN && ts_node_child_count(node) > 0) {
        return pmm_node_text(a, ts_node_child(node, 0), source);
    }
    if (lang == PMM_LANG_MATLAB && strcmp(nk, "command") == 0 && ts_node_child_count(node) > 0) {
        return pmm_node_text(a, ts_node_child(node, 0), source);
    }
    return NULL;
}
...
static char *extract_objc_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(selector) ? NULL : pmm_node_text(a, selector, source);
}
...
static char *extract_css_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(fn) ? NULL : pmm_node_text(a, fn, source);
}
...
static char *extract_powershell_callee(CBMArena *a, TSNode node, const char *source,
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ts_node_type(c), "command_name") == 0) {
            return pmm_node_text(a, c, source);
        }
    }
    return NULL;
}
...
static char *extract_dart_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "selector") == 0) {
        ...
        if (!ts_node_is_null(prev) && strcmp(ts_node_type(prev), "identifier") == 0) {
            return pmm_node_text(a, prev, source);
        }
        return NULL;
    }
...
static char *extract_scss_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "include_statement") == 0) {
        ...
        return ts_node_is_null(id) ? NULL : pmm_node_text(a, id, source);
    }
    ...
    if (strcmp(nk, "call_expression") == 0) {
        TSNode fn = pmm_find_child_by_kind(node, "function_name");
        if (!ts_node_is_null(fn)) {
            return pmm_node_text(a, fn, source);
        }
    }
    return NULL;
}
...
static char *extract_sql_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(nm) ? NULL : pmm_node_text(a, nm, source);
}
...
static char *extract_cobol_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (ts_node_is_null(x)) {
        x = pmm_find_child_by_kind(node, "string");
    }
    if (ts_node_is_null(x)) {
        return NULL;
    }
    char *text = pmm_node_text(a, x, source);
...
static char *extract_elm_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    TSNode ve = strcmp(ts_node_type(target), "value_expr") == 0
                    ? target
                    : pmm_find_child_by_kind(target, "value_expr");
    if (ts_node_is_null(ve)) {
        return NULL;
    }
    TSNode qid = ts_node_child_by_field_name(ve, TS_FIELD("name"));
    if (ts_node_is_null(qid)) {
        qid = pmm_find_child_by_kind(ve, "value_qid");
    }
    if (ts_node_is_null(qid)) {
        return NULL;
    }
    TSNode id = pmm_find_child_by_kind(qid, "lower_case_identifier");
    if (ts_node_is_null(id)) {
        // module-qualified call: emit the whole qualified id text
        return pmm_node_text(a, qid, source);
    }
    return pmm_node_text(a, id, source);
}
...
static char *extract_jsonnet_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    TSNode id = pmm_find_child_by_kind(node, "id");
    return ts_node_is_null(id) ? NULL : pmm_node_text(a, id, source);
}
...
static char *extract_nickel_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    for (int depth = 0; depth < NICKEL_APPLY_DEPTH && !ts_node_is_null(cur); depth++) {
        ...
        if (strcmp(ck, "ident") == 0) {
            return pmm_node_text(a, cur, source);
        }
...
static char *extract_typst_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(item) ? NULL : pmm_node_text(a, item, source);
}
...
static char *extract_meson_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(cmd) ? NULL : pmm_node_text(a, cmd, source);
}
...
static char *first_leaf_identifier(CBMArena *a, TSNode node, const char *source) {
    ...
    for (int depth = 0; depth < 8 && !ts_node_is_null(cur); depth++) {
        ...
        if (strcmp(k, "simple_identifier") == 0 || strcmp(k, "identifier") == 0 ||
            ...
            char *t = pmm_node_text(a, cur, source);
            return (t && t[0]) ? t : NULL;
        }
...
static char *extract_vhdl_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (strcmp(pk, "library_function") == 0 || strcmp(pk, "identifier") == 0 ||
        ...
        char *t = pmm_node_text(a, prev, source);
        return (t && t[0]) ? t : NULL;
    }
    return NULL;
}
...
static char *extract_nasm_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    char *m = pmm_node_text(a, mnem, source);
...
static char *extract_llvm_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (!t) {
        t = pmm_node_text(a, callee, source);
    }
    if (t && (t[0] == '@' || t[0] == '%')) {
        return t + 1;
    }
...
static char *extract_make_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "shell_function") == 0) {
        return pmm_arena_strndup(a, "shell", 5);
    }
...
static char *extract_just_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(name) ? NULL : pmm_node_text(a, name, source);
}
...
static char *extract_puppet_callee(CBMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "include_statement") == 0) {
        return pmm_arena_strndup(a, "include", 7);
    }
    if (strcmp(nk, "function_call") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            TSNode head = ts_node_named_child(node, 0);
...
static char *extract_callee_lang_specific(CBMArena *a, TSNode node, const char *source,
    ...
    if (lang == PMM_LANG_PYTHON && strcmp(nk, "call") == 0) {
    ...
    if (lang == PMM_LANG_ERLANG) {
        return extract_erlang_callee(a, node, source, nk);
    }
    if (lang == PMM_LANG_HASKELL || lang == PMM_LANG_OCAML || lang == PMM_LANG_PURESCRIPT) {
        return extract_fp_callee(a, node, source, nk);
    }
    if (lang == PMM_LANG_WOLFRAM && strcmp(nk, "apply") == 0) {
        return extract_wolfram_callee(a, node, source);
    }
    if (lang == PMM_LANG_SWIFT) {
    ...
    if (lang == PMM_LANG_OBJECTSCRIPT_UDL || lang == PMM_LANG_OBJECTSCRIPT_ROUTINE) {
        // ##class(Pkg.Class).Method() -> "Pkg.Class.Method"
        if (strcmp(nk, "class_method_call") == 0) {
            TSNode class_ref = pmm_find_child_by_kind(node, "class_ref");
            TSNode method_name = pmm_find_child_by_kind(node, "method_name");
            if (!ts_node_is_null(class_ref) && !ts_node_is_null(method_name)) {
                TSNode cname = pmm_find_child_by_kind(class_ref, "class_name");
                if (ts_node_is_null(cname)) {
                    return NULL;
                }
                char *cls = pmm_node_text(a, cname, source);
                if (!cls || !cls[0]) {
                    return NULL;
                }
                ...
                char *meth = pmm_node_text(a, mname_ident, source);
                if (!meth || !meth[0]) {
                    return cls;
                }
                return pmm_arena_sprintf(a, "%s.%s", cls, meth);
            }
            return NULL;
        }
        // $$label^routine extrinsic / routine tag call -> the line_ref text
        if (strcmp(nk, "routine_tag_call") == 0) {
            TSNode line_ref = pmm_find_child_by_kind(node, "line_ref");
            if (!ts_node_is_null(line_ref)) {
                return pmm_node_text(a, line_ref, source);
            }
            return NULL;
        }
        // $$$Macro(...) -> raw "$$$Name" callee (expanded later in handle_calls)
        if (strcmp(nk, "macro") == 0) {
            char *raw = pmm_node_text(a, node, source);
            ...
            return pmm_arena_sprintf(a, "$$$%s", name_start);
        }
        return NULL;
    }
...
static const char *php_chain_prefix_arg(CBMArena *a, TSNode call_node, const char *source) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ts_node_type(inner), "string") == 0 ||
            ...
            char *txt = pmm_node_text(a, inner, source);
            if (txt && txt[0]) {
...
static const char *php_group_prefix_for_call(CBMArena *a, TSNode node, const char *source) {
    ...
    for (int depth = 0; depth < PHP_GROUP_WALK_MAX && !ts_node_is_null(cur); depth++) {
        if (strcmp(ts_node_type(cur), "anonymous_function") == 0 ||
            ...
            if (!ts_node_is_null(p) && (strcmp(ts_node_type(p), "member_call_expression") == 0 ||
                ...
                char *gtxt = ts_node_is_null(gname) ? NULL : pmm_node_text(a, gname, source);
                if (gtxt && strcmp(gtxt, "group") == 0) {
                    ...
                    for (int hops = 0; hops < PHP_GROUP_WALK_MAX && !ts_node_is_null(recv);
                        ...
                        if (strcmp(rk, "member_call_expression") == 0 ||
                            ...
                            char *rtxt =
                                ts_node_is_null(rname) ? NULL : pmm_node_text(a, rname, source);
    ...
    buf[pos] = '\0';
    return pos ? pmm_arena_strndup(a, buf, pos) : NULL;
}

static char *extract_callee_name(CBMArena *a, TSNode node, const char *source, CBMLanguage lang) {
    // Lean 4: skip type-position applies
    if (lang == PMM_LANG_LEAN && strcmp(ts_node_type(node), "apply") == 0) {
    ...
    if (lang == PMM_LANG_RUBY) {
        ...
        if (!ts_node_is_null(m) && !ts_node_is_null(recv) &&
            ...
            char *mt = pmm_node_text(a, m, source);
            if (mt && strcmp(mt, "new") == 0) {
                char *rt = pmm_node_text(a, recv, source);
                if (rt && rt[0]) {
                    return rt;
                }
            }
     ...
     * not recognized. */
    if (lang == PMM_LANG_PHP && strcmp(ts_node_type(node), "scoped_call_expression") == 0) {
        ...
        if (!ts_node_is_null(scope) && !ts_node_is_null(mname)) {
            char *sc = pmm_node_text(a, scope, source);
            char *mn = pmm_node_text(a, mname, source);
            if (sc && mn && strcmp(sc, "Route") == 0) {
                char *qual = pmm_arena_sprintf(a, "%s::%s", sc, mn);
                if (qual && pmm_service_pattern_route_method(qual) != NULL) {
                    return qual;
                }
            }
        }
...
static const char *strip_and_validate_string_arg(CBMArena *a, char *text) {
    ...
    int len = (int)strlen(text);
    if (len >= PMM_QUOTE_PAIR && (text[0] == '"' || text[0] == '\'')) {
        text = pmm_arena_strndup(a, text + PMM_QUOTE_OFFSET, (size_t)(len - PMM_QUOTE_PAIR));
        len -= PMM_QUOTE_PAIR;
    }
...
static char *gotemplate_string_child(CBMArena *a, TSNode parent, const char *source) {
    TSNode s = pmm_find_child_by_kind(parent, "interpreted_string_literal");
    if (ts_node_is_null(s)) {
        return NULL;
    }
    char *text = pmm_node_text(a, s, source);
...
static char *gotemplate_callee(CBMArena *a, TSNode node, const char *source) {
    ...
    if (strcmp(k, "function_call") == 0) {
        TSNode fn = pmm_find_child_by_kind(node, "identifier");
        if (ts_node_is_null(fn)) {
            return NULL;
        }
        char *fname = pmm_node_text(a, fn, source);
        ...
        if (ts_node_is_null(args)) {
            args = pmm_find_child_by_kind(node, "argument_list");
        }
        if (ts_node_is_null(args)) {
            return NULL;
        }
...
static const char *extract_nth_string_arg(CBMExtractCtx *ctx, TSNode args, uint32_t n) {
    ...
    for (uint32_t ai = 0; ai < nc && ai < MAX_POSITIONAL_SCAN + n; ai++) {
        ...
        if (is_string_like(ak)) {
            if (found == n) {
                char *text = pmm_node_text(ctx->arena, arg, ctx->source);
...
static void process_keyword_arg(CBMExtractCtx *ctx, TSNode arg_node, CBMCallArg *ca) {
    ...
    if (!ts_node_is_null(key_n)) {
        ca->keyword = pmm_node_text(ctx->arena, key_n, ctx->source);
    }
    if (!ts_node_is_null(val_n)) {
        ca->expr = pmm_node_text(ctx->arena, val_n, ctx->source);
...
static void extract_call_args(CBMExtractCtx *ctx, TSNode args, CBMCall *call) {
    ...
    for (uint32_t ai = 0; ai < argc && call->arg_count < PMM_MAX_CALL_ARGS; ai++) {
        ...
        if (strcmp(ak, "keyword_argument") == 0 || strcmp(ak, "pair") == 0) {
            ...
            ca->expr = pmm_node_text(ctx->arena, arg_node, ctx->source);
            ca->index = positional_idx++;
...
static const char *extract_string_value(CBMExtractCtx *ctx, TSNode val_node) {
    ...
    if (is_string_like(vk)) {
        char *text = pmm_node_text(ctx->arena, val_node, ctx->source);
        if (text && text[0]) {
            return strip_quotes(ctx->arena, text);
        }
    } else if (strcmp(vk, "identifier") == 0) {
        char *const_name = pmm_node_text(ctx->arena, val_node, ctx->source);
        if (const_name) {
...
static const char *extract_composite_queue_field(CBMExtractCtx *ctx, TSNode node) {
    ...
    if (ts_node_is_null(body)) {
        body = pmm_find_child_by_kind(node, "literal_value");
    }
    if (ts_node_is_null(body)) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *key = pmm_node_text(ctx->arena, key_n, ctx->source);
        ...
        char *raw = pmm_node_text(ctx->arena, val_n, ctx->source);
        if (raw && raw[0]) {
            return raw;
        }
    }
...
static const char *extract_keyword_url(CBMExtractCtx *ctx, TSNode arg) {
    ...
    char *key = pmm_node_text(ctx->arena, key_node, ctx->source);
...
static const char *extract_positional_url(CBMExtractCtx *ctx, TSNode arg, const char *ak) {
    ...
    if (strcmp(ak, "identifier") == 0) {
        char *const_name = pmm_node_text(ctx->arena, arg, ctx->source);
        if (const_name) {
            return lookup_string_constant(ctx, const_name);
        }
    }
...
static const char *normalize_string_handler(CBMArena *a, const char *raw) {
    ...
    if (at && at[1]) {
        return pmm_arena_strdup(a, at + 1);
    }
    return unq;
}
...
static const char *extract_handler_arg(CBMExtractCtx *ctx, TSNode args) {
    ...
    for (uint32_t ai = HANDLER_START_IDX; ai < nc && ai < MAX_HANDLER_SCAN; ai++) {
        ...
        if (strcmp(ak2, "identifier") == 0 || strcmp(ak2, "member_expression") == 0 ||
            ...
            return pmm_node_text(ctx->arena, arg2, ctx->source);
        }
        if (is_string_like(ak2)) {
            const char *h =
                normalize_string_handler(ctx->arena, pmm_node_text(ctx->arena, arg2, ctx->source));
            if (h && h[0]) {
                return h;
            }
        }
...
static void extract_jsx_component_ref(CBMExtractCtx *ctx, TSNode node, const char *kind,
    ...
    char *name = pmm_node_text(ctx->arena, name_node, ctx->source);
    if (name && name[0] >= 'A' && name[0] <= 'Z') {
        CBMCall call = {0};
        call.callee_name = name;
        call.enclosing_func_qn = enclosing_func_qn;
        pmm_calls_push(&ctx->result->calls, ctx->arena, call);
    }
}
...
static void extract_kotlin_operator_call(CBMExtractCtx *ctx, TSNode node, const char *kind,
    ...
    if (pmm_memmem(between, blen, "===", 3) || pmm_memmem(between, blen, "!==", 3)) {
        return; // identity comparison: no operator method
    } else if (pmm_memmem(between, blen, "==", 2) || pmm_memmem(between, blen, "!=", 2)) {
        op_method = "equals";
    } else if (pmm_memmem(between, blen, "..<", 3)) {
        op_method = "rangeUntil";
    } else if (pmm_memmem(between, blen, "..", 2)) {
        op_method = "rangeTo";
    } else if (pmm_memmem(between, blen, "<", 1) || pmm_memmem(between, blen, ">", 1)) {
        op_method = "compareTo"; // covers <, >, <=, >=
    } else if (pmm_memmem(between, blen, "+", 1)) {
        op_method = "plus";
    } else if (pmm_memmem(between, blen, "-", 1)) {
        op_method = "minus";
    } else if (pmm_memmem(between, blen, "*", 1)) {
...
static void extract_cpp_operator_call(CBMExtractCtx *ctx, TSNode node, const char *kind,
    ...
    for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
        ...
        char *op = pmm_node_text(ctx->arena, child, ctx->source);
        if (op && op[0]) {
            CBMCall call = {0};
            call.callee_name = pmm_arena_sprintf(ctx->arena, "operator%s", op);
            call.enclosing_func_qn = enclosing_func_qn;
            call.start_line = (int)ts_node_start_point(node).row + TS_LINE_OFFSET;
            pmm_calls_push(&ctx->result->calls, ctx->arena, call);
        }
...
static void extract_cpp_implicit_calls(CBMExtractCtx *ctx, TSNode node, const char *kind,
    ...
    if (strcmp(kind, "delete_expression") == 0) {
        ...
        if (!ts_node_is_null(operand)) {
            callee = pmm_node_text(ctx->arena, operand, ctx->source);
        }
        ...
        if (!ts_node_is_null(type) && !ts_node_is_null(decl) &&
            ...
            if (!ts_node_is_null(value) && strcmp(ts_node_type(value), "identifier") == 0) {
                char *tn = pmm_node_text(ctx->arena, type, ctx->source);
                if (tn) {
    ...
    if (strcmp(kind, "property_declaration") == 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            for (uint32_t j = 0; j < vc; j++) {
                ...
                comp++;
                kt_push_implicit_call(ctx, node, pmm_arena_sprintf(ctx->arena, "component%u", comp),
                                      enclosing_func_qn);
            }
            break;
        }
...
static void extract_java_method_reference(CBMExtractCtx *ctx, TSNode node, const char *kind,
    ...
    char *mname = NULL;
    if (nc >= 2) {
        mname = pmm_node_text(ctx->arena, ts_node_named_child(node, nc - 1), ctx->source);
    }
    if (!mname || !mname[0]) {
...
static char *resolve_objectscript_instance_call(CBMArena *a, TSNode node, const char *source,
    ...
    TSNode method_name_node = pmm_find_child_by_kind(oref, "method_name");
    ...
    char *method = pmm_node_text(a, mn_ident, source);
    if (!method || !method[0]) {
        return NULL;
    }
    ...
    if (nk_first && strcmp(nk_first, "relative_dot_property") == 0) {
        TSNode prop_name = pmm_find_child_by_kind(receiver, "member_name");
        if (!ts_node_is_null(prop_name)) {
            char *pname = pmm_node_text(a, prop_name, source);
            if (pname && pname[0]) {
                var_text = pmm_arena_sprintf(a, "..%s", pname);
            }
        }
        if (!var_text) {
            var_text = pmm_node_text(a, receiver, source);
        }
    } else {
        var_text = pmm_node_text(a, receiver, source);
    }
    ...
    for (int i = 0; i < type_map->count; i++) {
        if (strcasecmp(type_map->entries[i].var_name, var_text) == 0) {
            return pmm_arena_sprintf(a, "%s.%s", type_map->entries[i].class_name, method);
        }
    }
    return NULL;
}
...
void handle_calls(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec, WalkState *state) {
    ...
    if (pmm_kind_in_set(node, spec->call_node_types)) {
        ...
        if (!callee &&
            (ctx->language == PMM_LANG_OBJECTSCRIPT_UDL ||
        ...
        if (!callee &&
            ...
            TSNode oref = pmm_find_child_by_kind(node, "oref_method");
            if (!ts_node_is_null(oref)) {
                TSNode mname_node = pmm_find_child_by_kind(oref, "method_name");
                if (!ts_node_is_null(mname_node)) {
                    ...
                    if (!ts_node_is_null(ident)) {
                        char *mname = pmm_node_text(ctx->arena, ident, ctx->source);
                        if (mname && mname[0]) {
                            callee = pmm_arena_sprintf(ctx->arena, "%s.%s",
        ...
        if (callee && callee[0] == '$' && callee[1] == '$' && callee[2] == '$' &&
            ...
            const CBMMacroEntry *entry = pmm_macro_table_find(ctx->macro_table, macro_name);
            if (entry) {
                if (entry->resolved_callee) {
                    callee = pmm_arena_strdup(ctx->arena, entry->resolved_callee);
                } else if (entry->expansion) {
                    callee = pmm_macro_extract_callee(ctx->arena, entry->expansion);
                } else {
                    callee = NULL;
                }
            }
        ...
        if (callee && callee[0] &&
            (!pmm_is_keyword(callee, ctx->language) ||
             pmm_is_resolvable_builtin(callee, ctx->language))) {
            CBMCall call = {0};
            call.callee_name = callee;
            ...
            if (ts_node_is_null(args) && (ctx->language == PMM_LANG_OBJECTSCRIPT_UDL ||
                                          ctx->language == PMM_LANG_OBJECTSCRIPT_ROUTINE)) {
                TSNode oref = pmm_find_child_by_kind(node, "oref_method");
                if (!ts_node_is_null(oref)) {
                    args = pmm_find_child_by_kind(oref, "method_args");
                }
                if (ts_node_is_null(args)) {
                    args = pmm_find_child_by_kind(node, "method_args");
                }
            }
            if (!ts_node_is_null(args)) {
                ...
                if (ctx->language == PMM_LANG_PHP && call.first_string_arg &&
                    call.first_string_arg[0] == '/' && call.callee_name &&
                    pmm_service_pattern_route_method(call.callee_name) != NULL) {
                    ...
                    if (gp && gp[0]) {
                        ...
                        call.first_string_arg =
                            rel[0] ? pmm_arena_sprintf(ctx->arena, "%s/%s", gp, rel)
                                   : pmm_arena_strndup(ctx->arena, gp, strlen(gp));
                    }
                }
                ...
                if (ctx->language == PMM_LANG_OBJECTSCRIPT_UDL ||
                    ctx->language == PMM_LANG_OBJECTSCRIPT_ROUTINE) {
                    for (uint32_t ai = 0;
                         ai < ts_node_named_child_count(args) && call.arg_count < PMM_MAX_CALL_ARGS;
                         ai++) {
                        ...
                        ca->expr = pmm_node_text(ctx->arena, achild, ctx->source);
            ...
            const char **dispatch_suffixes = pmm_string_dispatch_suffixes(ctx->language);
            if (dispatch_suffixes && !ts_node_is_null(args)) {
                ...
                for (const char **nm = dispatch_suffixes; *nm; nm++) {
                    ...
                    if (len >= nlen && strcmp(cn + len - nlen, *nm) == 0) {
                        ...
                        if (cls && mth) {
                            CBMCall xcall = {0};
                            xcall.callee_name = pmm_arena_sprintf(ctx->arena, "%s.%s", cls, mth);
                            xcall.enclosing_func_qn = call.enclosing_func_qn;
                            pmm_calls_push(&ctx->result->calls, ctx->arena, xcall);
                        }
                        break;
                    }
                }
