...
static const char *unquote_string(CBMArena *a, const char *s) {
    ...
    size_t len = strlen(s);
    if (len < PMM_QUOTE_PAIR) {
        return NULL;
    }
    char first = s[0];
    char last = s[len - PMM_QUOTE_OFFSET];
    if ((first == '"' && last == '"') || (first == '\'' && last == '\'') ||
        (first == '`' && last == '`')) {
        return pmm_arena_strndup(a, s + PMM_QUOTE_OFFSET, len - PMM_QUOTE_PAIR);
    }
    return NULL;
}
        ...
        if (strcmp(kind, "variable_declarator") == 0) {
            ...
            if (!ts_node_is_null(name_node) && !ts_node_is_null(value_node)) {
                ...
                if (strcmp(nk, "identifier") == 0 &&
                    ...
                    char *name_text = pmm_node_text(ctx->arena, name_node, ctx->source);
                    char *value_text = pmm_node_text(ctx->arena, value_node, ctx->source);
    ...
    while (stack.count > 0 && tbl->count < CHAN_CONST_CAP) {
        ...
        if (strcmp(kind, "assignment") == 0) {
            ...
            if (!ts_node_is_null(left) && !ts_node_is_null(right) &&
                ...
                char *name = pmm_node_text(ctx->arena, left, ctx->source);
...
static const char *extract_channel_name(CBMExtractCtx *ctx, TSNode args,
    ...
    if (!channel_name && consts) {
        ...
        if (strcmp(kind, "identifier") == 0) {
            char *ident = pmm_node_text(ctx->arena, first, ctx->source);
...
static void push_channel(CBMExtractCtx *ctx, const char *channel_name, const char *transport,
    ...
    pmm_channels_push(&ctx->result->channels, ctx->arena, ch);
}
...
static const char *js_classify_receiver(CBMExtractCtx *ctx, TSNode object_node) {
    char *text = pmm_node_text(ctx->arena, object_node, ctx->source);
    if (!text) {
        return NULL;
    }
...
static void js_process_call(CBMExtractCtx *ctx, TSNode call, const chan_const_table_t *consts) {
    ...
    char *method = pmm_node_text(ctx->arena, property, ctx->source);
    ...
    if (strcmp(transport, "kafka") == 0) {
        if (js_is_kafka_send(method)) {
            direction = PMM_CHANNEL_EMIT;
        } else if (js_is_kafka_listen(method)) {
            direction = PMM_CHANNEL_LISTEN;
        } else {
            return;
        }
...
static const struct {
    ...
    {"kafka", "send", PMM_CHANNEL_EMIT},
    {"kafka", "produce", PMM_CHANNEL_EMIT},
    {"kafka", "subscribe", PMM_CHANNEL_LISTEN},
    {"kafka", "poll", PMM_CHANNEL_LISTEN},
    {"django_channels", "send", PMM_CHANNEL_EMIT},
    {"django_channels", "group_send", PMM_CHANNEL_EMIT},
    {"django_channels", "receive", PMM_CHANNEL_LISTEN},
    {"django_channels", "group_add", PMM_CHANNEL_LISTEN},
    {"websocket", "send", PMM_CHANNEL_EMIT},
    {"websocket", "send_text", PMM_CHANNEL_EMIT},
    {"websocket", "send_json", PMM_CHANNEL_EMIT},
    {"websocket", "send_bytes", PMM_CHANNEL_EMIT},
    {"websocket", "receive", PMM_CHANNEL_LISTEN},
    {"websocket", "receive_text", PMM_CHANNEL_LISTEN},
    {"websocket", "receive_json", PMM_CHANNEL_LISTEN},
    {"websocket", "receive_bytes", PMM_CHANNEL_LISTEN},
    {NULL, "emit", PMM_CHANNEL_EMIT},
    {NULL, "send", PMM_CHANNEL_EMIT},
    {NULL, "on", PMM_CHANNEL_LISTEN},
...
static void py_process_call(CBMExtractCtx *ctx, TSNode call, const chan_const_table_t *consts) {
    ...
    char *method = pmm_node_text(ctx->arena, attr, ctx->source);
...
static void py_process_decorator(CBMExtractCtx *ctx, TSNode decorator,
    ...
    if (strcmp(ek, "call") == 0) {
        ...
        push_channel(ctx, channel_name, transport, PMM_CHANNEL_LISTEN, decorator);
    }
}
...
static void go_process_call(CBMExtractCtx *ctx, TSNode call) {
    ...
    char *method = pmm_node_text(ctx->arena, field, ctx->source);
    if (!method) {
        return;
    }
    ...
    char *recv = pmm_node_text(ctx->arena, operand, ctx->source);
    if (!recv) {
        return;
    }
...
static void java_process_call(CBMExtractCtx *ctx, TSNode call) {
    ...
    char *method = pmm_node_text(ctx->arena, func, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "convertAndSend") == 0 || strcmp(method, "convertAndSendToUser") == 0) {
        ...
        if (channel_name) {
            push_channel(ctx, channel_name, "spring_websocket", PMM_CHANNEL_EMIT, call);
        }
        return;
    }
    ...
    if (strcmp(method, "sendText") == 0 || strcmp(method, "sendObject") == 0 ||
        ...
        const char *func_name = enclosing_function_qn(ctx, call);
        const char *channel_name = func_name ? func_name : "(websocket)";
        push_channel(ctx, channel_name, "websocket", PMM_CHANNEL_EMIT, call);
        return;
    }
...
static void java_process_annotation(CBMExtractCtx *ctx, TSNode annotation) {
    ...
    if (strcmp(name, "OnMessage") == 0 || strcmp(name, "OnOpen") == 0 ||
        ...
        const char *channel_name = func_name ? func_name : "(websocket)";
        push_channel(ctx, channel_name, "websocket", PMM_CHANNEL_LISTEN, annotation);
        ...
        if (!ts_node_is_null(args)) {
            ...
            if (path) {
                push_channel(ctx, path, "spring_websocket", PMM_CHANNEL_LISTEN, annotation);
                return;
            }
        }
        push_channel(ctx, "(spring_ws)", "spring_websocket", PMM_CHANNEL_LISTEN, annotation);
    } else if (strcmp(name, "ServerEndpoint") == 0) {
        TSNode args = ts_node_child_by_field_name(annotation, TS_FIELD("arguments"));
        if (!ts_node_is_null(args)) {
            const char *path = extract_channel_name(ctx, args, NULL);
            if (path) {
                push_channel(ctx, path, "websocket", PMM_CHANNEL_LISTEN, annotation);
                return;
            }
        }
    }
...
static void csharp_process_call(CBMExtractCtx *ctx, TSNode call) {
    ...
    char *method = pmm_node_text(ctx->arena, name_node, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "SendAsync") == 0 || strcmp(method, "SendCoreAsync") == 0) {
        char *full = pmm_node_text(ctx->arena, func, ctx->source);
        if (full && (strstr(full, "Clients") || strstr(full, "clients"))) {
            TSNode args = ts_node_child_by_field_name(call, TS_FIELD("arguments"));
            if (!ts_node_is_null(args)) {
                const char *channel_name = extract_channel_name(ctx, args, NULL);
                if (channel_name) {
                    push_channel(ctx, channel_name, "signalr", PMM_CHANNEL_EMIT, call);
                }
            }
        }
        return;
    ...
    if (strcmp(method, "On") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "signalr", PMM_CHANNEL_LISTEN, call);
            }
        }
    }
}
...
static void ruby_process_call(CBMExtractCtx *ctx, TSNode call) {
    ...
    char *method = pmm_node_text(ctx->arena, method_node, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "broadcast") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "actioncable", PMM_CHANNEL_EMIT, call);
            }
        }
        return;
    }
    ...
    if (strcmp(method, "stream_from") == 0 || strcmp(method, "stream_for") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "actioncable", PMM_CHANNEL_LISTEN, call);
            }
        }
    }
}
...
static void elixir_process_call(CBMExtractCtx *ctx, TSNode call) {
    ...
    char *target_text = pmm_node_text(ctx->arena, target, ctx->source);
    if (!target_text) {
        return;
    }
    ...
    if (strstr(target_text, "PubSub.broadcast") || strstr(target_text, "PubSub.local_broadcast")) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_pubsub", PMM_CHANNEL_EMIT);
        return;
    }
    if (strstr(target_text, "PubSub.subscribe")) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_pubsub", PMM_CHANNEL_LISTEN);
        return;
    }
    if (strcmp(target_text, "push") == 0 || strcmp(target_text, "broadcast") == 0 ||
        strcmp(target_text, "broadcast!") == 0) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_channel", PMM_CHANNEL_EMIT);
    }
}
...
static void elixir_process_function_def(CBMExtractCtx *ctx, TSNode func_def) {
    ...
    if (val) {
        push_channel(ctx, val, "phoenix_channel", PMM_CHANNEL_LISTEN, func_def);
    }
}
...
static void rust_process_call(CBMExtractCtx *ctx, TSNode call) {
    ...
    char *recv = pmm_node_text(ctx->arena, value, ctx->source);
    if (!recv) {
        return;
    }
    const char *tail = recv;
