...
#include <process.h>
#include <windows.h>
#define pmm_writer_getpid _getpid
#else
#include <unistd.h>
#define pmm_writer_getpid getpid
#endif

#define PMM_PAGE_SIZE 65536
...
#define PMM_PENDING_BYTE (0x40000000u)
#define PMM_PENDING_BYTE_PAGE ((PMM_PENDING_BYTE / PMM_PAGE_SIZE) + 1)

/* Skip the pending byte page if allocation lands on it. */
static inline uint32_t pmm_skip_pending_byte(uint32_t pgno) {
    return pgno == PMM_PENDING_BYTE_PAGE ? pgno + SKIP_ONE : pgno;
}
...
#define HDR_OFF_PMM_PAGE_SIZE 16
...
static bool dynbuf_ensure(DynBuf *b, int needed) {
    ...
    if (!p) {
        (void)fprintf(stderr, "pmm_write_db: dynbuf realloc failed size=%d\n", newcap);
        return false;
    }
    b->data = p;
    b->cap = newcap;
...
static void pb_init(PageBuilder *pb, FILE *fp, uint32_t start_page, bool is_index) {
    ...
    pb->cell_count = 0;
    pb->content_offset = PMM_PAGE_SIZE;
    ...
    memset(pb->page, 0, PMM_PAGE_SIZE);
    pb->leaves = NULL;
    pb->leaf_count = 0;
    pb->leaf_cap = 0;
}
...
static void pb_flush_leaf(PageBuilder *pb) {
    ...
    pb->next_page = pmm_skip_pending_byte(pb->next_page);
    uint32_t page_num = pb->next_page;
    long offset = (long)(page_num - SKIP_ONE) * PMM_PAGE_SIZE;
    (void)fseek(pb->fp, offset, SEEK_SET);
    (void)fwrite(pb->page, SKIP_ONE, PMM_PAGE_SIZE, pb->fp);
...
static int write_interior_page(PageBuilder *pb, uint8_t *page, int cell_count, int content_offset,
    ...
    (void)fseek(pb->fp, (long)(pnum - SKIP_ONE) * PMM_PAGE_SIZE, SEEK_SET);
    (void)fwrite(page, SKIP_ONE, PMM_PAGE_SIZE, pb->fp);
...
static uint32_t pb_build_interior(PageBuilder *pb, bool is_index) {
    ...
    while (child_count > SKIP_ONE && children) {
        ...
        while (i < child_count) {
            ...
            int cell_count = 0;
            int content_offset = PMM_PAGE_SIZE;
...
// Each overflow page: 4-byte next-page pointer + up to (PMM_PAGE_SIZE-4) bytes of data.
static uint32_t write_overflow_pages(FILE *fp, uint32_t *next_page, const uint8_t *data,
                                     int data_len) {
    int per_page = PMM_PAGE_SIZE - BTREE_PTR_SIZE;
    ...
    while (offset < data_len) {
        ...
        long page_offset = (long)(pnum - SKIP_ONE) * PMM_PAGE_SIZE;
        prev_next_ptr_offset = page_offset;
        (void)fseek(fp, page_offset, SEEK_SET);
        (void)fwrite(page, SKIP_ONE, PMM_PAGE_SIZE, fp);
...
static uint8_t *overflowize_index_cell(FILE *fp, uint32_t *next_page, uint8_t *cell,
    ...
    int64_t per_ovfl = (int64_t)PMM_PAGE_SIZE - BTREE_PTR_SIZE;
...
static void pb_add_table_cell_with_flush(PageBuilder *pb, int64_t rowid, const uint8_t *payload,
    ...
    if (payload_len > TABLE_OVERFLOW_MAX_LOCAL) {
        ...
        int ovfl_page_data = PMM_PAGE_SIZE - BTREE_PTR_SIZE;
...
static uint32_t write_table_btree(FILE *fp, uint32_t *next_page, const uint8_t **records,
    ...
    if (count == 0) {
        ...
        *next_page = pmm_skip_pending_byte(*next_page);
        uint32_t pnum = (*next_page)++;
        uint8_t page[PMM_PAGE_SIZE];
        memset(page, 0, PMM_PAGE_SIZE);
        ...
        put_u16(page + hdr + HDR_CONTENT_OFF, (uint16_t)PMM_PAGE_SIZE); // content at end of page
        page[hdr + HDR_FRAGBYTES_OFF] = 0;                              // 0 fragmented bytes
        (void)fseek(fp, (long)(pnum - SKIP_ONE) * PMM_PAGE_SIZE, SEEK_SET);
        (void)fwrite(page, SKIP_ONE, PMM_PAGE_SIZE, fp);
        return pnum;
    }
...
static uint32_t write_empty_index_leaf(FILE *fp, uint32_t *next_page) {
    *next_page = pmm_skip_pending_byte(*next_page);
    ...
    put_u16(page + HDR_CONTENT_OFF, (uint16_t)PMM_PAGE_SIZE);
    page[HDR_FRAGBYTES_OFF] = 0;
    (void)fseek(fp, (long)(pnum - SKIP_ONE) * PMM_PAGE_SIZE, SEEK_SET);
    (void)fwrite(page, SKIP_ONE, PMM_PAGE_SIZE, fp);
    return pnum;
...
static uint32_t write_index_btree(FILE *fp, uint32_t *next_page, uint8_t **cells, int *cell_lens,
    ...
    for (int i = 0; i < count; i++) {
        if (!pb_cell_fits(&pb, cell_lens[i])) {
            ...
            if (!pb_cell_fits(&pb, cell_lens[i])) {
                (void)fprintf(stderr, "pmm_write_db: index cell oversized, skipped len=%d idx=%d\n",
                              cell_lens[i], i);
                continue;
            }
        }
...
static int *make_sorted_perm(int n, int (*cmp)(const void *, const void *)) {
    ...
    if (!perm) {
        (void)fprintf(stderr, "pmm_write_db: perm malloc failed n=%d size=%zu\n", n,
                      (size_t)n * sizeof(int));
        return NULL;
    }
    for (int i = 0; i < n; i++) {
...
/* Write context passed to sub-phases of pmm_write_db. */
typedef struct {
    FILE *fp;
    uint32_t next_page;
    char final_path[PMM_SZ_4K];
    char temp_path[PMM_SZ_4K];
    const char *project;
...
static int make_writer_temp_path(const char *path, const void *token, char *out, size_t out_size) {
    int n = snprintf(out, out_size, "%s.tmp.%ld.%p", path, (long)pmm_writer_getpid(), token);
...
static int publish_writer_output(write_db_ctx_t *w) {
    ...
    if (pmm_rename_replace(w->temp_path, w->final_path) != 0) {
        (void)pmm_unlink(w->temp_path);
        return ERR_WRITE_FAILED;
    }
    ...
    pmm_remove_db_sidecars(w->final_path);
    return 0;
}
 ...
 * PageBuilder in pmm_writer_append_nodes, so no node adapter is needed here). */
...
static void write_sqlite_file_header(uint8_t *page1, uint32_t total_pages) {
    ...
    put_u16(page1 + HDR_OFF_PMM_PAGE_SIZE,
            PMM_PAGE_SIZE == SQLITE_MAX_PAGE_SIZE ? (uint16_t)SKIP_ONE : (uint16_t)PMM_PAGE_SIZE);
...
static int write_master_page1(FILE *fp, MasterEntry *master, int master_count, uint32_t next_page) {
    ...
    uint8_t page1[PMM_PAGE_SIZE];
    memset(page1, 0, PMM_PAGE_SIZE);
    int hdr = SQLITE_HEADER_SIZE;
    page1[hdr] = BTREE_LEAF_TABLE;
    int content_off = PMM_PAGE_SIZE;
    ...
    (void)fwrite(page1, SKIP_ONE, PMM_PAGE_SIZE, fp);
...
static void pad_file_to_page_boundary(FILE *fp, uint32_t next_page) {
    ...
    long expected_size = (long)(next_page - SKIP_ONE) * PMM_PAGE_SIZE;
...
static void parallel_sort_indexes(SortJob *nsorts, int n_node, SortJob *esorts, int n_edge) {
    pmm_thread_t st[TOTAL_SORT_THREADS];
    int nt = 0;
    for (int i = 0; i < n_node; i++) {
        if (nsorts[i].count > 0) {
            pmm_thread_create(&st[nt++], 0, sort_worker, &nsorts[i]);
        }
    }
    for (int i = 0; i < n_edge; i++) {
        if (esorts[i].count > 0) {
            pmm_thread_create(&st[nt++], 0, sort_worker, &esorts[i]);
        }
    }
    for (int i = 0; i < nt; i++) {
        pmm_thread_join(&st[i]);
    }
}
...
static int write_db_after_nodes(write_db_ctx_t *w, uint32_t nodes_root) {
    ...
    PMM_PROF_END_N("write_db", "1_data_tables", t_data, node_count + edge_count);

    // Phase 2: Metadata tables (projects, file_hashes, summaries, sqlite_sequence)
    PMM_PROF_START(t_meta);
    ...
    PMM_PROF_END("write_db", "2_metadata_tables", t_meta);
    ...
    PMM_PROF_END_N("write_db", "3_parallel_sort_indexes", t_sort, node_count + edge_count);

    /* Phase 4-5: Build node + edge index B-trees */
    PMM_PROF_START(t_node_idx);
    ...
    PMM_PROF_END_N("write_db", "4_node_indexes_seq", t_node_idx, node_count * NODE_SORT_THREADS);
    if (nrc != 0) {
        return discard_writer_output(w, nrc);
    }

    PMM_PROF_START(t_edge_idx);
    ...
    PMM_PROF_END_N("write_db", "5_edge_indexes_seq", t_edge_idx, edge_count * EDGE_SORT_THREADS);
    if (erc != 0) {
...
pmm_db_writer_t *pmm_writer_open(const char *path) {
    pmm_db_writer_t *w = (pmm_db_writer_t *)calloc(PMM_ALLOC_ONE, sizeof(*w));
    if (!w) {
        return NULL;
    }
    ...
    FILE *fp = pmm_fopen(w->wc.temp_path, "wb");
    if (!fp) {
        (void)pmm_unlink(w->wc.temp_path);
        free(w);
        return NULL;
    }
    w->wc.fp = fp;
...
int pmm_writer_append_nodes(pmm_db_writer_t *w, const CBMDumpNode *nodes, int count) {
    if (!w) {
        return PMM_NOT_FOUND;
    }
    if (w->err) {
...
int pmm_writer_finalize(pmm_db_writer_t *w, const char *project, const char *root_path,
...
int pmm_write_db(const char *path, const char *project, const char *root_path,
    ...
    (void)pmm_writer_append_nodes(w, nodes,
                                  node_count); /* error recorded in w, handled by finalize */
    return pmm_writer_finalize(w, project, root_path, indexed_at, nodes, node_count, edges,
