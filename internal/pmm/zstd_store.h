#ifndef PMM_ZSTD_STORE_H
#define PMM_ZSTD_STORE_H
...
int pmm_zstd_compress(const char *src, int srcLen, char *dst, int dstCap, int level);
...
int64_t pmm_zstd_decompress(const char *src, size_t srcLen, char *dst, size_t dstCap);
...
size_t pmm_zstd_frame_content_size(const char *src, size_t srcLen);

// Maximum compressed size bound for given input size.
size_t pmm_zstd_compress_bound(int inputSize);

#endif // PMM_ZSTD_STORE_H
