...
//
// For PMM_LANG_KUSTOMIZE: walks top-level block_mapping_pair nodes whose key
// matches "resources", "bases", "patches", "components", or
// "patchesStrategicMerge", then emits one CBMImport per block_sequence item.
//
// For PMM_LANG_K8S: finds apiVersion, kind, and metadata.name scalars in the
...
static const char *get_scalar_text(CBMArena *a, TSNode node, const char *source) {
    ...
    for (int depth = 0; depth < MAX_UNWRAP; depth++) {
        ...
        if (strcmp(type, "double_quote_scalar") == 0 || strcmp(type, "single_quote_scalar") == 0) {
            const char *raw = pmm_node_text(a, node, source);
            if (!raw) {
                return NULL;
            }
            size_t len = strlen(raw);
            if (len >= PAIR_LEN) {
                return pmm_arena_strndup(a, raw + SKIP_ONE, len - PAIR_LEN);
            }
            return raw;
        }
        break;
...
static void emit_kustomize_sequence(CBMExtractCtx *ctx, TSNode seq_node, const char *key_name) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (uint32_t j = 0; j < ic; j++) {
            ...
            CBMImport imp = {
                .local_name = pmm_arena_strdup(a, key_name),
                .module_path = pmm_arena_strdup(a, scalar),
            };
...
static void emit_kustomize_kind_def(CBMExtractCtx *ctx, TSNode mapping) {
    ...
    for (uint32_t pi = 0; pi < pair_n; pi++) {
        ...
        CBMDefinition def = {0};
        def.name = pmm_arena_strdup(a, kind);
        def.qualified_name = pmm_arena_sprintf(a, "%s.%s", ctx->module_qn, kind);
        def.label = pmm_arena_strdup(a, "Resource");
...
static void extract_k8s_manifest(CBMExtractCtx *ctx) {
    ...
    for (uint32_t si = 0; si < root_n; si++) {
        ...
        CBMDefinition def = {0};
        def.name = pmm_arena_strdup(a, def_name);
        def.qualified_name = pmm_arena_sprintf(a, "%s.%s", ctx->module_qn, def_name);
...
void pmm_extract_k8s(CBMExtractCtx *ctx) {
    if (ctx->language == PMM_LANG_KUSTOMIZE) {
        extract_kustomize(ctx);
    } else if (ctx->language == PMM_LANG_K8S) {
