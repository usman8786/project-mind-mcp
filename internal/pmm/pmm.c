#include "pmm.h"
#include "arena.h" // CBMArena, pmm_arena_init/alloc/strdup/destroy
#include "helpers.h"
...
#include "foundation/compat_fs.h"  // pmm_fopen — crash-supervisor per-file marker write
...
#if defined(PMM_BIND_TS_ALLOCATOR) && PMM_BIND_TS_ALLOCATOR
...
/* Use compat.h's pmm_clock_gettime which accepts CLOCK_MONOTONIC (value
 * varies by platform: 1 on Linux/Windows, 6 on macOS). We pass the
 * platform value via the compat.h fallback. */
#if defined(CLOCK_MONOTONIC)
#define PMM_CLOCK_MONO CLOCK_MONOTONIC
#elif defined(__APPLE__)
#define PMM_CLOCK_MONO 6
#else
#define PMM_CLOCK_MONO 1
#endif

static uint64_t now_ns(void) {
    struct timespec ts;
    pmm_clock_gettime(PMM_CLOCK_MONO, &ts);
    return ((uint64_t)ts.tv_sec * NSEC_PER_SEC) + (uint64_t)ts.tv_nsec;
}

// pmm_get_profile returns accumulated parse/extract times and file count.
...
// pmm_reset_profile zeros the profiling counters.
...
#define GROW_ARRAY(arr, arena)                                                                   \
    do {                                                                                         \
        if ((arr)->count >= (arr)->cap) {                                                        \
            int new_cap = (arr)->cap == 0 ? PMM_SZ_32 : (arr)->cap * PAIR_LEN;                   \
            void *new_items = pmm_arena_alloc((arena), (size_t)new_cap * sizeof(*(arr)->items)); \
            if (!new_items)                                                                      \
                return;                                                                          \
...
static const char *pmm_string_read(void *payload, uint32_t byte, TSPoint point,
                                   uint32_t *bytes_read) {
    (void)point;
...
static bool pmm_timeout_cb(TSParseState *state) {
...
static PMM_TLS TSParser *tl_parser = NULL;
static PMM_TLS CBMLanguage tl_parser_lang = PMM_LANG_COUNT; // invalid sentinel
 ...
 *
 * Guarded to the production build (PMM_BIND_TS_ALLOCATOR=1, which CFLAGS_PROD
 ...
 * (#581). */
static void *pmm_sqlite_malloc(int n) {
    void *block = mi_malloc((size_t)n);
    return block;
}
static void pmm_sqlite_free(void *p) {
    mi_free(p);
}
static void *pmm_sqlite_realloc(void *p, int n) {
    if (p) {}
    void *grown = mi_realloc(p, (size_t)n);
    return grown;
}
static int pmm_sqlite_size(void *p) {
    return (int)mi_usable_size(p);
}
static int pmm_sqlite_roundup(int n) {
...
static void *pmm_ts_malloc(size_t n) {
    void *block = mi_malloc(n);
    return block;
}
static void *pmm_ts_calloc(size_t count, size_t size) {
    void *block = mi_calloc(count, size);
    return block;
}
static void *pmm_ts_realloc(void *p, size_t n) {
    if (p) {}
    void *grown = mi_realloc(p, n);
    return grown;
}
static void pmm_ts_free(void *p) {
    mi_free(p);
}

static int pmm_sqlite_meminit(void *appdata) {
    (void)appdata;
    return SQLITE_OK;
}
static void pmm_sqlite_memshutdown(void *appdata) {
    (void)appdata;
}
#endif /* PMM_BIND_TS_ALLOCATOR */
...
void pmm_alloc_init(void) {
    ...
    /* tree-sitter runtime (was previously bound in pmm_init; consolidated here). */
    ts_set_allocator(pmm_ts_malloc, pmm_ts_calloc, pmm_ts_realloc, pmm_ts_free);
     ...
     * silently and the binding is ignored. pmm_alloc_init() runs as the very
     * first statement of main(), before pmm_mcp_server_new → pmm_store_open*. */
    static sqlite3_mem_methods pmm_sqlite_mem = {
        pmm_sqlite_malloc,      /* xMalloc */
        pmm_sqlite_free,        /* xFree */
        pmm_sqlite_realloc,     /* xRealloc */
        pmm_sqlite_size,        /* xSize */
        pmm_sqlite_roundup,     /* xRoundup */
        pmm_sqlite_meminit,     /* xInit */
        pmm_sqlite_memshutdown, /* xShutdown */
        NULL,                   /* pAppData */
    };
    int sqlite_rc = sqlite3_config(SQLITE_CONFIG_MALLOC, &pmm_sqlite_mem);
...
int pmm_init(void) {
    ...
    enum { PMM_INIT_DONE = 1 };
    pmm_initialized = PMM_INIT_DONE;
    /* Defense-in-depth allocator binds (idempotent). main() calls pmm_alloc_init
     * first; this covers non-main entry points (pipeline passes call pmm_init).
...
void pmm_reset_thread_parser(void) {
    ...
    // Must be called BEFORE pmm_slab_reset_thread() to avoid corrupting
...
static void pmm_index_mark(const char *rel_path, char event) {
    const char *mf = getenv("PMM_INDEX_MARKER_FILE");
    if (!mf || !mf[0] || !rel_path || !rel_path[0]) {
        return;
    }
    FILE *f = pmm_fopen(mf, "ab");
    if (f) {
        (void)fprintf(f, "%c %s\n", event, rel_path);
        (void)fclose(f);
    }
 ...
 * (pmm_extract_file): whether a pass re-extracts from disk on a cache miss
 ...
 * pmm_index_is_quarantined() so the crasher surfaces in the response skipped[].
...
enum { PMM_QSET_UNINIT = 0, PMM_QSET_INITING = 1, PMM_QSET_INITED = 2 };
static atomic_int g_quarantine_state = PMM_QSET_UNINIT;

static void pmm_quarantine_load(void) {
    const char *qf = getenv("PMM_INDEX_QUARANTINE_FILE");
    if (!qf || !qf[0]) {
        return; /* normal path: empty set */
    }
    FILE *f = pmm_fopen(qf, "rb");
    if (!f) {
        return;
    }
    CBMHashTable *set = pmm_ht_create(16);
    if (!set) {
        (void)fclose(f);
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
         ...
         * The value stores the phase so pmm_index_quarantine_phase() can report
         * "crash" vs "hang"; membership (pmm_index_is_quarantined) is value != NULL. */
        char *key = pmm_strdup(line);
        char *pval = pmm_strdup(phase);
        if (key && pval) {
            pmm_ht_set(set, key, (void *)pval);
        }
    }
    (void)fclose(f);
...
bool pmm_index_is_quarantined(const char *rel_path) {
    ...
    if (state != PMM_QSET_INITED) {
        /* First caller wins the CAS and loads; racers spin until INITED.
         * Same once-init pattern as pmm_ui_log_init (http_server.c). */
        state = PMM_QSET_UNINIT;
        if (atomic_compare_exchange_strong(&g_quarantine_state, &state, PMM_QSET_INITING)) {
            pmm_quarantine_load();
            atomic_store(&g_quarantine_state, PMM_QSET_INITED);
        } else {
            while (atomic_load(&g_quarantine_state) != PMM_QSET_INITED) {
                pmm_usleep(1000); /* 1ms */
            }
        }
    }
    return g_quarantine_set && pmm_ht_has(g_quarantine_set, rel_path);
}

const char *pmm_index_quarantine_phase(const char *rel_path) {
    /* pmm_index_is_quarantined drives the lazy once-load and returns true only
    ...
    if (!pmm_index_is_quarantined(rel_path)) {
        return NULL;
    }
    return (const char *)pmm_ht_get(g_quarantine_set, rel_path);
}

static void pmm_test_fault_inject(const char *rel_path) {
    if (!rel_path || !rel_path[0]) {
        return;
    }
    const char *crash_on = getenv("PMM_TEST_CRASH_ON");
    if (crash_on && crash_on[0] && strstr(rel_path, crash_on)) {
        abort(); /* SIGABRT → WIFSIGNALED → classified as a crash */
    }
    const char *hang_on = getenv("PMM_TEST_HANG_ON");
 ...
 *
 * pmm_source_nesting_exceeds scans the raw bytes for the maximum bracket-nesting
...
#define PMM_PERL_MAX_PARSE_NESTING 128

static bool pmm_source_nesting_exceeds(const char *source, int source_len, int cap) {
    int depth = 0;
 ...
 * Bounded by PMM_MAX_ERROR_REGIONS so pathological input can't blow up the
 * output. The ranges mark where constructs were dropped; they are a detection
 * aid, never a completeness proof. */
#define PMM_MAX_ERROR_REGIONS 64
typedef struct {
    uint32_t starts[PMM_MAX_ERROR_REGIONS];
    uint32_t ends[PMM_MAX_ERROR_REGIONS];
    int count;
} pmm_error_regions_t;

static void pmm_error_regions_push(pmm_error_regions_t *acc, TSNode n) {
    if (acc->count >= PMM_MAX_ERROR_REGIONS) {
        return;
    }
...
static void pmm_collect_error_regions(TSNode n, pmm_error_regions_t *acc) {
    if (acc->count >= PMM_MAX_ERROR_REGIONS) {
        return;
    }
    uint32_t k = ts_node_child_count(n);
    for (uint32_t i = 0; i < k && acc->count < PMM_MAX_ERROR_REGIONS; i++) {
        TSNode c = ts_node_child(n, i);
        if (ts_node_is_missing(c) || strcmp(ts_node_type(c), "ERROR") == 0) {
            pmm_error_regions_push(acc, c); /* top-most region; do not descend */
        } else if (ts_node_has_error(c)) {
            pmm_collect_error_regions(c, acc);
        }
    }
}
...
static bool pmm_region_is_recovered(uint32_t rs, uint32_t re, const CBMDefArray *defs) {
...
static bool pmm_line_contains(const char *src, int src_len, uint32_t line, const char *name) {
...
static bool pmm_identifier_char(char c) {
...
static bool pmm_span_contains_callable_def(const char *src, int src_len, uint32_t start_line,
    ...
    for (int pos = span_start; pos + (int)name_len <= span_end; pos++) {
        if (strncmp(src + pos, name, name_len) != 0 ||
            (pos > 0 && pmm_identifier_char(src[pos - 1])) ||
            (pos + (int)name_len < src_len && pmm_identifier_char(src[pos + name_len]))) {
            continue;
        }
        int before = pos;
...
static bool pmm_remap_preprocessed_def(CBMDefinition *def, const CBMPreprocessedSource *pp) {
...
static void pmm_subtract_recovered_regions(pmm_error_regions_t *regs, const CBMDefArray *defs) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        if (!pmm_region_is_recovered(regs->starts[i], regs->ends[i], defs)) {
...
static bool pmm_span_is_macro_invocation(const char *src, int src_len, uint32_t start_line,
    ...
    for (int di = 0; di < defs->count; di++) {
        ...
        for (int pos = span_start; pos + nlen <= span_end; pos++) {
            if (strncmp(src + pos, d->name, (size_t)nlen) != 0 ||
                (pos > 0 && pmm_identifier_char(src[pos - 1])) ||
                (pos + nlen < src_len && pmm_identifier_char(src[pos + nlen]))) {
                continue;
            }
            int open = pos + nlen;
            while (open < span_end && isspace((unsigned char)src[open])) {
...
static bool pmm_region_inside_callable(uint32_t rs, uint32_t re, const CBMDefArray *defs) {
...
static void pmm_subtract_macro_invocation_regions(pmm_error_regions_t *regs,
                                                  const CBMDefArray *defs, const char *src,
                                                  int src_len) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        bool benign =
            pmm_span_is_macro_invocation(src, src_len, regs->starts[i], regs->ends[i], defs) &&
            pmm_region_inside_callable(regs->starts[i], regs->ends[i], defs);
        if (!benign) {
            regs->starts[kept] = regs->starts[i];
            regs->ends[kept] = regs->ends[i];
            kept++;
...
static const char *pmm_error_ranges_str(CBMArena *a, const pmm_error_regions_t *regs) {
    if (regs->count <= 0) {
        return NULL;
    }
    enum { RANGE_MAX = 24 }; /* "4294967295-4294967295," */
    char *buf = (char *)pmm_arena_alloc(a, (size_t)regs->count * RANGE_MAX);
    if (!buf) {
        return NULL;
    }
    size_t off = 0;
...
CBMFileResult *pmm_extract_file(const char *source, int source_len, CBMLanguage language,
    ...
    CBMFileResult *r =
        pmm_extract_file_ex(source, source_len, language, project, rel_path, timeout_micros,
                            extra_defines, include_paths, NULL, NULL);
    return r;
}

CBMFileResult *pmm_extract_file_ex(const char *source, int source_len, CBMLanguage language,
    ...
    pmm_index_mark_start(rel_path);
    pmm_test_fault_inject(rel_path);

    // Get language spec
    const CBMLangSpec *spec = pmm_lang_spec(language);
    if (!spec) {
        result->has_error = true;
        result->error_msg = pmm_arena_strdup(a, "unsupported language");
        pmm_index_mark_done(rel_path);
        return result;
    }

    // Get tree-sitter language
    const TSLanguage *ts_lang = pmm_ts_language(language);
    if (!ts_lang) {
        result->has_error = true;
        result->error_msg = pmm_arena_strdup(a, "no tree-sitter grammar");
        pmm_index_mark_done(rel_path);
        return result;
    }
    ...
    // pmm_source_nesting_exceeds). Scoped to Perl: its ambiguous call grammar is
    // the only one that drives that recursion to the nesting depth.
    if (language == PMM_LANG_PERL &&
        pmm_source_nesting_exceeds(source, source_len, PMM_PERL_MAX_PARSE_NESTING)) {
        result->has_error = true;
        result->error_msg = pmm_arena_strdup(a, "perl source nesting too deep; skipped");
        return result;
    }
    ...
    if (!parser) {
        result->has_error = true;
        result->error_msg = pmm_arena_strdup(a, "parser alloc failed");
        pmm_index_mark_done(rel_path);
        return result;
    }
    ...
    if (timeout_micros > 0) {
        ...
        opts.progress_callback = pmm_timeout_cb;
    }
    ...
    if (!tree) {
        result->has_error = true;
        result->error_msg =
            pmm_arena_strdup(a, timeout_micros > 0 ? "parse timeout" : "parse failed");
        pmm_index_mark_done(rel_path);
        return result;
    }
    ...
    result->module_qn = pmm_fqn_module_source_lang(a, project, rel_path, language);
    result->is_test_file = pmm_is_test_file(rel_path, language);
    ...
    pmm_extract_definitions(&ctx);
    pmm_extract_imports(&ctx);
    pmm_extract_unified(&ctx);

    // Channel detection (Socket.IO / EventEmitter) — JS/TS only.
    pmm_extract_channels(&ctx);

    // K8s / Kustomize semantic pass (additional structured extraction for YAML-based infra files).
    if (ctx.language == PMM_LANG_KUSTOMIZE || ctx.language == PMM_LANG_K8S) {
        pmm_extract_k8s(&ctx);
    }
    ...
    {
        if (language == PMM_LANG_GO) {
            pmm_run_go_lsp(a, result, source, source_len, root);
        }
        if (language == PMM_LANG_C || language == PMM_LANG_CPP || language == PMM_LANG_CUDA) {
            pmm_run_c_lsp(a, result, source, source_len, root, language != PMM_LANG_C);
        }
        if (language == PMM_LANG_PHP) {
            pmm_run_php_lsp(a, result, source, source_len, root);
        }
        if (language == PMM_LANG_PERL) {
            pmm_run_perl_lsp(a, result, source, source_len, root);
        }
        if (language == PMM_LANG_PYTHON) {
            pmm_run_py_lsp(a, result, source, source_len, root);
        }
        if (language == PMM_LANG_JAVASCRIPT || language == PMM_LANG_TYPESCRIPT ||
            language == PMM_LANG_TSX) {
            bool js_mode = (language == PMM_LANG_JAVASCRIPT);
            // jsx_mode: TSX always; .jsx in the JS bucket also enables it.
            bool jsx_mode = (language == PMM_LANG_TSX);
            if (language == PMM_LANG_JAVASCRIPT && rel_path) {
            ...
            bool dts_mode = false;
            if (language == PMM_LANG_TYPESCRIPT && rel_path) {
                size_t rl = strlen(rel_path);
                if (rl >= 5 && strcmp(rel_path + rl - 5, ".d.ts") == 0)
                    dts_mode = true;
            }
            pmm_run_ts_lsp(a, result, source, source_len, root, js_mode, jsx_mode, dts_mode);
        }
        if (language == PMM_LANG_CSHARP) {
            pmm_run_cs_lsp(a, result, source, source_len, root);
        }
    }
    if (language == PMM_LANG_JAVA) {
        pmm_run_java_lsp(a, result, source, source_len, root);
    }
    if (language == PMM_LANG_KOTLIN) {
        pmm_run_kotlin_lsp(a, result, source, source_len, root);
    }
    if (language == PMM_LANG_RUST) {
        pmm_run_rust_lsp(a, result, source, source_len, root);
    }
    ...
    if (language == PMM_LANG_C || language == PMM_LANG_CPP || language == PMM_LANG_CUDA) {
        ...
        CBMPreprocessedSource *preprocessed = pmm_preprocess_with_map(
            source, source_len, rel_path, extra_defines, include_paths, language != PMM_LANG_C);
        if (preprocessed && preprocessed->source) {
            ...
            if (pp_parser) {
                ...
                if (pp_tree) {
                    ...
                    pmm_run_c_lsp(a, result, expanded, expanded_len, pp_root,
                                  language != PMM_LANG_C);
                    ...
                    if (ts_node_has_error(root)) {
                        pmm_error_regions_t raw_regs = {{0}, {0}, 0};
                        pmm_collect_error_regions(root, &raw_regs);
                        if (raw_regs.count > 0) {
                            int defs_before = result->defs.count;
                            pmm_extract_definitions(&pp_ctx);
                            int w = defs_before;
                            for (int i = defs_before; i < result->defs.count; i++) {
                                CBMDefinition *d = &result->defs.items[i];
                                bool adopt = false;
                                if (pmm_remap_preprocessed_def(d, preprocessed)) {
                                ...
                                if (adopt && (!d->name ||
                                              !pmm_line_contains(source, source_len, d->start_line,
                                                                 d->name) ||
                                              !pmm_span_contains_callable_def(
            ...
            pmm_preprocessed_source_free(preprocessed);
    ...
    if (ts_node_has_error(root)) {
        pmm_error_regions_t regs = {{0}, {0}, 0};
        if (strcmp(ts_node_type(root), "ERROR") == 0) {
            pmm_error_regions_push(&regs, root); /* whole file unparseable */
        } else {
            pmm_collect_error_regions(root, &regs);
        }
        pmm_subtract_recovered_regions(&regs, &result->defs);
        /* #1071: don't flag a benign function-like-macro call (defined in-file)
         * that tree-sitter can't parse without the preprocessor. */
        pmm_subtract_macro_invocation_regions(&regs, &result->defs, source, source_len);
        if (regs.count > 0) {
            result->parse_incomplete = true;
            result->error_region_count = regs.count;
            result->error_ranges = pmm_error_ranges_str(a, &regs);
        }
    }
    ...
    // Retain tree for cross-file LSP reuse (caller frees via pmm_free_tree)
...
void pmm_free_result(CBMFileResult *result) {
    ...
    pmm_arena_destroy(&result->arena);
    free(result);
}
