#ifndef PMM_LSP_GO_LSP_H
#define PMM_LSP_GO_LSP_H
...
typedef struct {
    ...
    // on deeply-nested/cyclic files; see pmm_lsp_max_walk_depth). Zero via memset.
    int walk_depth;
...
// Called from pmm_extract_file() after definitions and imports are extracted.
...
// String fields are borrowed (caller owns memory until pmm_run_go_lsp_cross returns).
typedef struct {
 ...
 * value is 0, the caller can skip the pmm_run_go_lsp_cross_with_
