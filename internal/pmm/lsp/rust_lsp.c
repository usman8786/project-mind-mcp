...
void rust_lsp_init(RustLSPContext *ctx, CBMArena *arena, const char *source, int source_len,
    ...
    const char *dbg = getenv("PMM_LSP_DEBUG");
    ctx->debug = (dbg && dbg[0]);
}
...
void rust_lsp_add_use(RustLSPContext *ctx, const char *local_name, const char *module_path) {
    ...
    if (ctx->use_count % 32 == 0) {
        ...
        const char **nl =
            (const char **)pmm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(char *));
        const char **np =
            (const char **)pmm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(char *));
        if (!nl || !np) {
            return;
    ...
    ctx->use_local_names[ctx->use_count] = pmm_arena_strdup(ctx->arena, local_name);
    ctx->use_module_paths[ctx->use_count] = pmm_arena_strdup(ctx->arena, module_path);
    ctx->use_count++;
}
...
void rust_lsp_add_glob(RustLSPContext *ctx, const char *module_qn) {
    ...
    ctx->glob_module_qns[ctx->glob_count++] = pmm_arena_strdup(ctx->arena, module_qn);
}
...
static const char *convert_path_to_qn(CBMArena *arena, const char *path) {
    ...
    char *out = (char *)pmm_arena_alloc(arena, len + 1);
    if (!out) {
        return path;
    }
    size_t j = 0;
...
static const char *rust_resolve_path_expr(RustLSPContext *ctx, const char *path) {
    ...
    if (strncmp(path, "Self::", 6) == 0 && ctx->self_type_qn) {
        return pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->self_type_qn,
    ...
    if (strncmp(path, "crate::", 7) == 0 && ctx->module_qn) {
        ...
        char *crate_buf = pmm_arena_strndup(ctx->arena, ctx->module_qn, crate_len);
        return pmm_arena_sprintf(ctx->arena, "%s.%s", crate_buf,
    ...
    if (strncmp(path, "super::", 7) == 0 && ctx->module_qn) {
        ...
        if (dot) {
            char *parent =
                pmm_arena_strndup(ctx->arena, ctx->module_qn, (size_t)(dot - ctx->module_qn));
            return pmm_arena_sprintf(ctx->arena, "%s.%s", parent,
                                     convert_path_to_qn(ctx->arena, path + 7));
        }
    }
    ...
    if (!sep) {
        ...
        if (ctx->module_qn) {
            return pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, path);
        }
        return path;
    }

    char *head = pmm_arena_strndup(ctx->arena, path, (size_t)(sep - path));
    ...
    if (full) {
        ...
        return pmm_arena_sprintf(ctx->arena, "%s.%s", full_dotted, tail_dotted);
    }

    /* Prelude head: `String::from` → `alloc.string.String.from`. */
    const char *prelude = rust_lookup_prelude(head);
    if (prelude) {
        return pmm_arena_sprintf(ctx->arena, "%s.%s", prelude,
    ...
    if (ctx->cargo_manifest) {
        ...
        const CBMCargoMember *mem = pmm_cargo_find_member(m, head);
        if (mem) {
            /* Workspace member: route to `<member_name>.<tail>` so the
             * pipeline's cross-crate resolution can match it. */
            return pmm_arena_sprintf(ctx->arena, "%s.%s", head,
                                     convert_path_to_qn(ctx->arena, tail));
        }
        if (pmm_cargo_is_known_dep(m, head)) {
...
static const char *resolve_path_to_type_qn(RustLSPContext *ctx, const char *path) {
    ...
    if (pmm_registry_lookup_type(ctx->registry, qn)) {
        return qn;
    }
...
const CBMType *rust_parse_type_node(RustLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "type_identifier") == 0) {
        ...
        if (strcmp(name, "Self") == 0 && ctx->self_type_qn) {
            return pmm_type_named(ctx->arena, ctx->self_type_qn);
        }
    ...
    if (strcmp(kind, "reference_type") == 0) {
        ...
        return pmm_type_reference(ctx->arena, elem);
    }
    ...
    if (strcmp(kind, "pointer_type") == 0) {
        ...
        return pmm_type_pointer(ctx->arena, rust_parse_type_node(ctx, inner));
    }
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        return pmm_type_slice(ctx->arena, rust_parse_type_node(ctx, elem));
    }
    ...
    if (strcmp(kind, "tuple_type") == 0) {
        ...
        if (count == 0) {
            return pmm_type_builtin(ctx->arena, "()");
        }
        if (count == 1) {
            return elems[0];
        }
        return pmm_type_tuple(ctx->arena, elems, count);
    }
    ...
    if (strcmp(kind, "generic_type") == 0) {
        ...
        if (targ_count > 0) {
            return pmm_type_template(ctx->arena, head_qn, targs, targ_count);
        }
        return pmm_type_named(ctx->arena, head_qn);
    }
 ...
 * without a parser. This is the same trade-off `pmm_rust_parse_return_type_text`
 * makes for Go. */
static const CBMType *parse_type_text_with_params(CBMArena *arena, const char *text,
    ...
    if (text[0] == '&') {
        ...
        return pmm_type_reference(arena, elem);
    }
    ...
    if (text[0] == '[' && text[strlen(text) - 1] == ']') {
        ...
        char *inner = pmm_arena_strndup(arena, p, inner_len);
    ...
    if (text[0] == '(' && text[strlen(text) - 1] == ')') {
        ...
        if (count == 1)
            return elems[0];
        return pmm_type_tuple(arena, elems, count);
    }
    ...
    if (lt) {
        ...
        if (*gt == '>') {
            ...
            char *head = pmm_arena_strndup(arena, text, head_len);
            /* Recursive split of args by top-level commas. */
            const char *args = lt + 1;
            size_t args_len = (size_t)(gt - args);
            char *abuf = pmm_arena_strndup(arena, args, args_len);
            const CBMType *targs[16];
            int targ_count = 0;
            int depth = 0;
            char *start = abuf;
            ...
            if (is_rust_primitive(head)) {
                ...
                return pmm_type_builtin(arena, head);
            }
            /* Map a few well-known std type sugars. */
            return pmm_type_template(arena, head_qn, targs, targ_count);
        }
    }

    /* Bare identifier or path. */
    if (is_rust_primitive(text)) {
        return pmm_type_builtin(arena, text);
    }
    if (type_params) {
        for (int i = 0; type_params[i]; i++) {
            if (strcmp(text, type_params[i]) == 0) {
                return pmm_type_type_param(arena, text);
            }
        }
    }
    /* Self -> module-qualified placeholder caller will substitute. */
    if (strcmp(text, "Self") == 0) {
        return pmm_type_named(arena, "Self");
    }
    /* Has `::` → absolute path; treat dotted paths as already-qualified
     * QNs (cross-file callers pass module-qualified text directly). */
    if (strstr(text, "::")) {
        return pmm_type_named(arena, convert_path_to_qn(arena, text));
    }
    if (strchr(text, '.')) {
        return pmm_type_named(arena, text);
    }
    return pmm_type_named(arena, pmm_arena_sprintf(arena, "%s.%s", module_qn, text));
}
...
static const CBMType *rust_substitute_type(CBMArena *arena, const CBMType *t, const char **params,
    ...
    case PMM_TYPE_TYPE_PARAM:
        ...
        return t;
    case PMM_TYPE_REFERENCE:
        return pmm_type_reference(
            arena, rust_substitute_type(arena, t->data.reference.elem, params, args));
    case PMM_TYPE_POINTER:
        return pmm_type_pointer(arena,
                                rust_substitute_type(arena, t->data.pointer.elem, params, args));
    case PMM_TYPE_SLICE:
        return pmm_type_slice(arena, rust_substitute_type(arena, t->data.slice.elem, params, args));
    case PMM_TYPE_TEMPLATE: {
        ...
        return pmm_type_template(arena, t->data.template_type.template_name, new_args, n);
    }
    case PMM_TYPE_TUPLE: {
        ...
        return pmm_type_tuple(arena, new_elems, n);
    }
    default:
        return t;
    }
...
static void rust_unify_type(const CBMType *param_type, const CBMType *arg_type,
                            ...
                            int param_count) {
    if (!param_type || !arg_type || pmm_type_is_unknown(arg_type)) {
        return;
    }
    if (param_type->kind == PMM_TYPE_TYPE_PARAM) {
    ...
    if (param_type->kind == PMM_TYPE_TUPLE && arg_type->kind == PMM_TYPE_TUPLE) {
    ...
    /* POINTER unification. */
    if (param_type->kind == PMM_TYPE_POINTER && arg_type->kind == PMM_TYPE_POINTER) {
    ...
    if (arg_type->kind == PMM_TYPE_TYPE_PARAM) {
...
static const CBMType *rust_eval_literal_type(RustLSPContext *ctx, const char *kind, TSNode node) {
    if (strcmp(kind, "integer_literal") == 0) {
        ...
        if (text) {
            ...
            if (*p) {
                return pmm_type_builtin(ctx->arena, p);
            }
        }
        return pmm_type_builtin(ctx->arena, "i32");
    }
    if (strcmp(kind, "float_literal") == 0) {
        ...
        return pmm_type_builtin(ctx->arena, "f64");
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "raw_string_literal") == 0) {
        /* &'static str — represented as &str */
        return pmm_type_reference(ctx->arena, pmm_type_builtin(ctx->arena, "str"));
    }
...
const CBMType *rust_eval_expr_type(RustLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (full) {
            ...
            const CBMRegisteredFunc *uf = pmm_registry_lookup_func(ctx->registry, qn);
            if (uf && uf->signature) {
                return uf->signature;
            }
            const CBMRegisteredType *ut = pmm_registry_lookup_type(ctx->registry, qn);
            if (ut) {
                return pmm_type_named(ctx->arena, qn);
            }
        }
        /* Same-module type? */
        const char *type_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name);
        if (pmm_registry_lookup_type(ctx->registry, type_qn)) {
            return pmm_type_named(ctx->arena, type_qn);
        }
        return pmm_type_unknown();
    }
    ...
    if (strcmp(kind, "self") == 0 || strcmp(kind, "self_parameter") == 0) {
        return pmm_scope_lookup(ctx->current_scope, "self");
    }
    ...
    if (strcmp(kind, "scoped_identifier") == 0) {
        ...
        const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, qn);
        if (f && f->signature) {
            return f->signature;
        }
        if (pmm_registry_lookup_type(ctx->registry, qn)) {
    ...
    if (strcmp(kind, "field_expression") == 0) {
        ...
        if (strcmp(fk, "integer_literal") == 0) {
            ...
            if (recv) {
                ...
                while (base && base->kind == PMM_TYPE_REFERENCE) {
                    base = base->data.reference.elem;
                }
                if (base && base->kind == PMM_TYPE_TUPLE) {
    ...
    if (strcmp(kind, "call_expression") == 0) {
        ...
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "scoped_identifier") == 0) {
            ...
            if (path) {
                ...
                if (qn) {
                    ...
                    const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, qn);
                    if (f && f->signature && f->signature->kind == PMM_TYPE_FUNC) {
                        ...
                        if (rt_arr && rt_arr[0]) {
                            ...
                            if (f->receiver_type && pmm_type_is_unknown(ret) && f->short_name &&
                                ...
                                if (is_wrapper && !ts_node_is_null(args_node)) {
                                    ...
                                    if (anc > 0) {
                                        ...
                                        if (arg_t && !pmm_type_is_unknown(arg_t)) {
                                            return pmm_type_template(ctx->arena, f->receiver_type,
                                                                     &arg_t, 1);
                                        }
                                    }
                                }
                                return pmm_type_named(ctx->arena, f->receiver_type);
                            }
                            /* Self -> receiver_type substitution. */
                            if (f->receiver_type && ret->kind == PMM_TYPE_NAMED &&
                     ...
                     * `pmm_registry_lookup_method`. */
                    const char *dot = strrchr(qn, '.');
                    if (dot) {
                        char *head = pmm_arena_strndup(ctx->arena, qn, (size_t)(dot - qn));
                        const char *short_name = dot + 1;
                        const CBMRegisteredFunc *m =
                            pmm_registry_lookup_method_aliased(ctx->registry, head, short_name);
                        if (!m && ctx->module_qn) {
                            const char *full_head =
                                pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, head);
                            m = pmm_registry_lookup_method_aliased(ctx->registry, full_head,
                                                                   short_name);
                            if (m)
                                head = (char *)full_head;
                        }
                        if (m && m->signature && m->signature->kind == PMM_TYPE_FUNC &&
                            ...
                            if (ret->kind == PMM_TYPE_NAMED &&
                                strcmp(ret->data.named.qualified_name, "Self") == 0) {
                                return pmm_type_named(ctx->arena, head);
                            }
                            if (pmm_type_is_unknown(ret) &&
                                ...
                                if (strcmp(short_name, "new") == 0 &&
                                    ...
                                    if (anc > 0) {
                                        const CBMType *arg_t = rust_eval_expr_type(
                                            ctx, ts_node_named_child(args_node, 0));
                                        if (arg_t && !pmm_type_is_unknown(arg_t)) {
                                            return pmm_type_template(ctx->arena, head, &arg_t, 1);
                                        }
                                    }
                                }
                                return pmm_type_named(ctx->arena, head);
        ...
        if (strcmp(fk, "field_expression") == 0) {
            ...
            if (!ts_node_is_null(value) && !ts_node_is_null(field)) {
                ...
                if (mname && recv) {
                    ...
                    if (base && base->kind == PMM_TYPE_TEMPLATE) {
                        ...
                        if (strstr(tname, "Vec") || strstr(tname, "VecDeque") ||
                            ...
                            if (strcmp(mname, "iter") == 0 || strcmp(mname, "iter_mut") == 0 ||
                                ...
                                if (base->data.template_type.arg_count > 0) {
                                    return pmm_type_template(
                                        ctx->arena, "core.iter.Iterator",
                                        &base->data.template_type.template_args[0], 1);
                                }
                                return pmm_type_template(ctx->arena, "core.iter.Iterator", NULL, 0);
                            }
                            ...
                            if (strcmp(mname, "len") == 0 || strcmp(mname, "capacity") == 0) {
                                return pmm_type_builtin(ctx->arena, "usize");
                            }
                            ...
                            if (strcmp(mname, "first") == 0 || strcmp(mname, "last") == 0 ||
                                ...
                                if (base->data.template_type.arg_count > 0) {
                                    ...
                                    return pmm_type_template(ctx->arena, "core.option.Option",
                                                             opt_args, 1);
                                }
                            }
                        ...
                        if (strstr(tname, "Option")) {
                            ...
                            if (strcmp(mname, "as_ref") == 0) {
                                if (base->data.template_type.arg_count > 0) {
                                    const CBMType *arg0 = pmm_type_reference(
                        ...
                        if (strstr(tname, "Result")) {
                            ...
                            if (strcmp(mname, "ok") == 0 &&
                                ...
                                return pmm_type_template(ctx->arena, "core.option.Option", &a0, 1);
                            }
                            if (strcmp(mname, "err") == 0 &&
                                base->data.template_type.arg_count > 1) {
                                const CBMType *a1 = base->data.template_type.template_args[1];
                                return pmm_type_template(ctx->arena, "core.option.Option", &a1, 1);
                            }
                        ...
                        if (strstr(tname, "Iterator")) {
                            ...
                            if (strcmp(mname, "collect") == 0) {
                                ...
                                if (base->data.template_type.arg_count > 0) {
                                    return pmm_type_template(ctx->arena, "alloc.vec.Vec",
                            ...
                            if (strcmp(mname, "next") == 0 || strcmp(mname, "last") == 0 ||
                                ...
                                if (base->data.template_type.arg_count > 0) {
                                    const CBMType *a0 = base->data.template_type.template_args[0];
                                    return pmm_type_template(ctx->arena, "core.option.Option", &a0,
                                                             1);
                                }
                            }
                            ...
                            if (strcmp(mname, "position") == 0) {
                                const CBMType *usize = pmm_type_builtin(ctx->arena, "usize");
                                return pmm_type_template(ctx->arena, "core.option.Option", &usize,
                                                         1);
                            }
                            if (strcmp(mname, "enumerate") == 0 &&
                                ...
                                const CBMType *pair[2] = {
                                    pmm_type_builtin(ctx->arena, "usize"),
                                    base->data.template_type.template_args[0]};
                                const CBMType *tup = pmm_type_tuple(ctx->arena, pair, 2);
                                return pmm_type_template(ctx->arena, "core.iter.Iterator", &tup, 1);
                            }
                        }
                        ...
                        if (strstr(tname, "HashMap") || strstr(tname, "BTreeMap")) {
                            ...
                            if (strcmp(mname, "get") == 0 || strcmp(mname, "get_mut") == 0 ||
                                ...
                                if (base->data.template_type.arg_count > 1) {
                                    ...
                                    return pmm_type_template(ctx->arena, "core.option.Option", &v,
                                                             1);
                                }
                            }
                            if (strcmp(mname, "iter") == 0 || strcmp(mname, "iter_mut") == 0) {
                                ...
                                if (base->data.template_type.arg_count > 1) {
                                    ...
                                    const CBMType *tup = pmm_type_tuple(ctx->arena, pair, 2);
                                    return pmm_type_template(ctx->arena, "core.iter.Iterator", &tup,
                                                             1);
                                }
                            }
                            if (strcmp(mname, "keys") == 0 &&
                                base->data.template_type.arg_count > 0) {
                                return pmm_type_template(ctx->arena, "core.iter.Iterator",
                                                         &base->data.template_type.template_args[0],
                                                         1);
                            }
                    ...
                    if (t && t->kind == PMM_TYPE_FUNC && t->data.func.return_types &&
                        ...
                        if (ret->kind == PMM_TYPE_NAMED &&
                            ...
                            while (rb && rb->kind == PMM_TYPE_REFERENCE)
                                rb = rb->data.reference.elem;
                            if (rb && rb->kind == PMM_TYPE_NAMED) {
                                return pmm_type_named(ctx->arena, rb->data.named.qualified_name);
                            }
                        }
                        return ret;
                    }
        ...
        if (func_type && func_type->kind == PMM_TYPE_FUNC && func_type->data.func.return_types &&
            func_type->data.func.return_types[0]) {
            return func_type->data.func.return_types[0];
        }
        if (func_type && func_type->kind == PMM_TYPE_NAMED) {
            return func_type;
        }
        return pmm_type_unknown();
    }
        ...
        if (strcmp(name, "vec") == 0) {
            ...
            if (!ts_node_is_null(args)) {
                ...
                for (uint32_t i = 0; i < nc; i++) {
                    ...
                    if (elem && !pmm_type_is_unknown(elem)) {
                        return pmm_type_template(ctx->arena, "alloc.vec.Vec", &elem, 1);
                    }
                }
            }
            return pmm_type_template(ctx->arena, "alloc.vec.Vec", NULL, 0);
        }
        if (is_string_macro(name)) {
            return pmm_type_named(ctx->arena, "alloc.string.String");
        }
    ...
    if (strcmp(kind, "reference_expression") == 0) {
        ...
        return pmm_type_reference(ctx->arena, rust_eval_expr_type(ctx, value));
    }
    ...
    if (strcmp(kind, "unary_expression") == 0) {
        ...
        const CBMType *inner =
            ts_node_is_null(operand) ? pmm_type_unknown() : rust_eval_expr_type(ctx, operand);
        if (op && strcmp(op, "*") == 0) {
            if (inner && inner->kind == PMM_TYPE_REFERENCE) {
                return inner->data.reference.elem;
            }
            if (inner && inner->kind == PMM_TYPE_POINTER) {
    ...
    if (strcmp(kind, "index_expression") == 0) {
        ...
        while (base && base->kind == PMM_TYPE_REFERENCE)
            base = base->data.reference.elem;
        if (base && base->kind == PMM_TYPE_SLICE) {
    ...
    if (strcmp(kind, "await_expression") == 0 && ts_node_named_child_count(node) > 0) {
        const CBMType *inner = rust_eval_expr_type(ctx, ts_node_named_child(node, 0));
        if (inner && inner->kind == PMM_TYPE_TEMPLATE && inner->data.template_type.arg_count > 0) {
            return inner->data.template_type.template_args[0];
        }
        return inner;
    }
    ...
    if (strcmp(kind, "array_expression") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            ...
            return pmm_type_slice(ctx->arena, elem);
        }
        return pmm_type_unknown();
    }
    ...
    if (strcmp(kind, "range_expression") == 0) {
        ...
        if (elem_type) {
            return pmm_type_template(ctx->arena, "core.iter.Iterator", &elem_type, 1);
        }
        return pmm_type_template(ctx->arena, "core.iter.Iterator", NULL, 0);
    }
    ...
    if (strcmp(kind, "struct_expression") == 0) {
        ...
        if (!ts_node_is_null(name_node)) {
            ...
            if (path) {
                return pmm_type_named(ctx->arena, rust_resolve_path_expr(ctx, path));
            }
        }
        return pmm_type_unknown();
    }
...
const CBMType *rust_eval_expr_typed(RustLSPContext *ctx, TSNode node, const CBMType *expected) {
    ...
    if (!expected)
        return synth;
    if (!synth || pmm_type_is_unknown(synth)) {
        return expected;
    }
    /* Template with same head and missing args -> use expected. */
    if (synth->kind == PMM_TYPE_TEMPLATE && expected->kind == PMM_TYPE_TEMPLATE &&
    ...
    if (synth->kind == PMM_TYPE_NAMED && expected->kind == PMM_TYPE_TEMPLATE &&
...
static const CBMType *rust_eval_member_access(RustLSPContext *ctx, const CBMType *recv,
    ...
    const CBMType *base = recv;
    while (base && (base->kind == PMM_TYPE_REFERENCE || base->kind == PMM_TYPE_POINTER)) {
        base = (base->kind == PMM_TYPE_REFERENCE) ? base->data.reference.elem
    ...
    if (base->kind == PMM_TYPE_NAMED) {
    ...
    } else if (base->kind == PMM_TYPE_TEMPLATE) {
    ...
    } else if (base->kind == PMM_TYPE_BUILTIN) {
        ...
        if (strcmp(nm, "str") == 0)
            ...
            type_qn = nm;
    } else if (base->kind == PMM_TYPE_SLICE) {
        type_qn = "core.slice";
    } else {
        return pmm_type_unknown();
    }
    ...
    if (method && method->signature) {
        ...
        if (sig && sig->kind == PMM_TYPE_FUNC && sig->data.func.return_types &&
            sig->data.func.return_types[0]) {
            const CBMType *ret = sig->data.func.return_types[0];
            if (ret && ret->kind == PMM_TYPE_NAMED &&
                strcmp(ret->data.named.qualified_name, "Self") == 0) {
                return pmm_type_named(ctx->arena, type_qn);
            }
        }
...
static const CBMType *rust_lookup_field(RustLSPContext *ctx, const char *type_qn,
                                        const char *field_name, int depth) {
    if (!type_qn || !field_name || depth > PMM_LSP_MAX_LOOKUP_DEPTH)
        return NULL;
...
static const CBMType *rust_deref_step(RustLSPContext *ctx, const CBMType *t) {
    ...
    if (t->kind == PMM_TYPE_TEMPLATE && t->data.template_type.template_name) {
        ...
        if (rt && rt->embedded_types) {
            for (int i = 0; rt->embedded_types[i]; i++) {
                ...
                if (strncmp(e, prefix, sizeof(prefix) - 1) == 0) {
                    return pmm_type_named(ctx->arena, e + sizeof(prefix) - 1);
                }
            }
        }
    }
...
static const CBMRegisteredFunc *rust_lookup_method_depth(RustLSPContext *ctx, const char *type_qn,
    ...
    const CBMRegisteredFunc *f =
        pmm_registry_lookup_method_aliased(ctx->registry, type_qn, member_name);
    if (f)
        return f;
...
static const CBMRegisteredFunc *rust_lookup_method_in_trait(RustLSPContext *ctx,
    ...
    return pmm_registry_lookup_method(reg, trait_qn, method_name);
}
...
static const CBMRegisteredFunc *rust_resolve_trait_method(RustLSPContext *ctx,
    ...
    const CBMRegisteredFunc *inh =
        pmm_registry_lookup_method_aliased(reg, receiver_type_qn, method_name);
    if (inh) {
        if (out_impl_count)
            *out_impl_count = 1;
        return inh;
    ...
    uint64_t nm_key = 0;
    if (nm_active) {
        nm_key = pmm_negmemo_key(1, receiver_type_qn, method_name);
        if (pmm_negmemo_contains(&ctx->neg_memo, nm_key)) {
            return NULL;
        }
    }
    ...
    pmm_registry_types_by_embedded_bare(reg, rbare, &eit);
    for (int ti; impls < 3 && (ti = pmm_type_embed_iter_next(&eit)) >= 0;) {
        ...
        for (int j = 0; t->embedded_types[j]; j++) {
            if (strcmp(t->embedded_types[j], receiver_type_qn) == 0) {
                const CBMRegisteredFunc *mf =
                    pmm_registry_lookup_method(reg, t->qualified_name, method_name);
                if (mf) {
                    impls++;
                    if (impls == 1)
                        unique = mf;
    ...
    if (nm_active && !tm && impls == 0) {
        pmm_negmemo_insert(&ctx->neg_memo, ctx->arena, nm_key);
    }
    return tm;
}
...
static bool rust_method_is_trait_impl(RustLSPContext *ctx, const char *type_qn,
    ...
    if (!rt || !rt->embedded_types)
        return false;
    for (int i = 0; rt->embedded_types[i]; i++) {
        if (pmm_registry_lookup_method(ctx->registry, rt->embedded_types[i], method_name))
            return true;
    }
    return false;
}
...
static const CBMRegisteredFunc *rust_find_sole_trait_impl(RustLSPContext *ctx, const char *trait_qn,
    ...
    uint64_t nm_key = 0;
    if (nm_active) {
        nm_key = pmm_negmemo_key(2, trait_qn, method_name);
        if (pmm_negmemo_contains(&ctx->neg_memo, nm_key)) {
            return NULL;
        }
    }
    ...
    CBMTypeEmbedIter eit;
    pmm_registry_types_by_embedded_bare(reg, tbare, &eit);
    for (int ti; n < 2 && (ti = pmm_type_embed_iter_next(&eit)) >= 0;) {
        const CBMRegisteredType *t = &reg->types[ti];
...
static void rust_record_macro_rule(RustLSPContext *ctx, const char *macro_name, TSNode pattern,
    ...
    if (ctx->macro_rules_count % 16 == 0) {
        ...
        struct RustMacroRule **narr = (struct RustMacroRule **)pmm_arena_alloc(
    ...
    RustMacroRule *r = (RustMacroRule *)pmm_arena_alloc(ctx->arena, sizeof(*r));
    if (!r)
        return;
    memset(r, 0, sizeof(*r));
    r->macro_name = pmm_arena_strdup(ctx->arena, macro_name);

    /* Cache pattern text for matching at invocation time. */
    if (!ts_node_is_null(pattern)) {
        char *pt = pmm_node_text(ctx->arena, pattern, ctx->source);
        if (pt) {
            const char *inner;
            int inner_len;
            rust_macro_strip_outer(pt, (int)strlen(pt), &inner, &inner_len);
            r->pattern_text = pmm_arena_strndup(ctx->arena, inner, (size_t)inner_len);
            r->pattern_len = inner_len;
        }
    }

    char *tt = pmm_node_text(ctx->arena, transcriber, ctx->source);
    if (tt) {
        int len = (int)strlen(tt);
        const char *inner;
        int inner_len;
        rust_macro_strip_outer(tt, len, &inner, &inner_len);
        r->transcriber_text = pmm_arena_strndup(ctx->arena, inner, (size_t)inner_len);
...
static char *macro_substitute(CBMArena *arena, const char *xs, int xs_len, const MacroEnv *env) {
    ...
    char *out = (char *)pmm_arena_alloc(arena, cap + 1);
    if (!out)
        return NULL;
    int op = 0;
    int xp = 0;
...
static void rust_expand_user_macro(RustLSPContext *ctx, const char *mname, TSNode invocation) {
    ...
    if (!ts_node_is_null(invocation)) {
        ...
        if (!ts_node_is_null(args_tt)) {
            char *at = pmm_node_text(ctx->arena, args_tt, ctx->source);
            if (at) {
    ...
    uint64_t mm_key = pmm_negmemo_key(3, mname, substituted);
    if (pmm_negmemo_contains(&ctx->macro_memo, mm_key)) {
        return;
    }
    pmm_negmemo_insert(&ctx->macro_memo, ctx->arena, mm_key);

    /* Wrap and parse. */
    char *wrapped = pmm_arena_sprintf(ctx->arena, "fn __pmm_macro_expand() { %s; }\n", substituted);
    if (!wrapped)
        return;
...
static void rust_resolve_macro_arg_exprs(RustLSPContext *ctx, TSNode invocation) {
    ...
    if (inner_len <= 0)
        return;
    char *arg_text = pmm_arena_strndup(ctx->arena, inner, (size_t)inner_len);
    if (!arg_text)
        return;
    ...
    if (ctx->macro_expand_depth > 0) {
        uint64_t am_key = pmm_negmemo_key(4, arg_text, NULL);
        if (pmm_negmemo_contains(&ctx->macro_memo, am_key)) {
            return;
        }
        pmm_negmemo_insert(&ctx->macro_memo, ctx->arena, am_key);
    }
    ...
    char *wrapped =
        pmm_arena_sprintf(ctx->arena, "fn __pmm_macro_args() { let _ = (%s); }\n", arg_text);
    if (!wrapped)
        return;
...
static void rust_bind_pattern(RustLSPContext *ctx, TSNode pattern, const CBMType *type) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (name && strcmp(name, "_") != 0) {
            pmm_scope_bind(ctx->current_scope, name, type);
        }
        return;
    }
    ...
    if (strcmp(kind, "tuple_pattern") == 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            const CBMType *elem_t = pmm_type_unknown();
            if (base && base->kind == PMM_TYPE_TUPLE && (int)i < base->data.tuple.count) {
    ...
    if (strcmp(kind, "tuple_struct_pattern") == 0) {
        ...
        while (base && base->kind == PMM_TYPE_REFERENCE)
            base = base->data.reference.elem;
        const CBMType *inner = pmm_type_unknown();
        if (base && base->kind == PMM_TYPE_TEMPLATE && base->data.template_type.arg_count > 0) {
    ...
    if (strcmp(kind, "struct_pattern") == 0) {
        ...
        if (base && base->kind == PMM_TYPE_NAMED)
            type_qn = base->data.named.qualified_name;
        else if (base && base->kind == PMM_TYPE_TEMPLATE)
            type_qn = base->data.template_type.template_name;
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(fk, "field_pattern") == 0) {
                ...
                if (!fname)
                    continue;
                const CBMType *ft =
                    type_qn ? rust_lookup_field(ctx, type_qn, fname, 0) : pmm_type_unknown();
                if (!ft)
                    ft = pmm_type_unknown();
                if (!ts_node_is_null(pat_node)) {
                    rust_bind_pattern(ctx, pat_node, ft);
                } else {
                    pmm_scope_bind(ctx->current_scope, fname, ft);
                }
                ...
                if (fname && strcmp(fname, "_") != 0) {
                    const CBMType *ft =
                        type_qn ? rust_lookup_field(ctx, type_qn, fname, 0) : pmm_type_unknown();
                    pmm_scope_bind(ctx->current_scope, fname, ft ? ft : pmm_type_unknown());
                }
            }
        }
        return;
...
void rust_process_statement(RustLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "const_item") == 0 || strcmp(kind, "static_item") == 0) {
        ...
        TSNode tn = ts_node_child_by_field_name(node, "type", 4);
        const CBMType *type =
            ts_node_is_null(tn) ? pmm_type_unknown() : rust_parse_type_node(ctx, tn);
        if (!ts_node_is_null(name_node)) {
            char *name = rust_node_text(ctx, name_node);
            if (name)
                pmm_scope_bind(ctx->current_scope, name, type);
 ...
 * (pmm_pipeline_find_lsp_resolution) can pair it with the resolved_call and
...
static void rust_inject_syn_call(RustLSPContext *ctx, const char *callee_qn) {
    ...
    if (!short_name || !short_name[0])
        return;
    CBMCall call = {0};
    call.callee_name = pmm_arena_strdup(ctx->arena, short_name);
    call.enclosing_func_qn = ctx->enclosing_func_qn;
    pmm_calls_push(ctx->syn_calls, ctx->arena, call);
}
...
static void rust_resolve_call_expression(RustLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(fk, "field_expression") == 0) {
        ...
        if (base && base->kind == PMM_TYPE_NAMED)
            ...
            type_qn = base->data.template_type.template_name;
        else if (base && base->kind == PMM_TYPE_BUILTIN) {
            const char *nm = base->data.builtin.name;
            if (strcmp(nm, "str") == 0)
                type_qn = "core.str";
            else if (strcmp(nm, "String") == 0)
        ...
        } else if (base && base->kind == PMM_TYPE_SLICE) {
            /* `&[T]` / `[T]` method dispatch. */
            type_qn = "core.slice";
        }
        ...
        if (type_qn) {
            ...
            if (m) {
                ...
                float conf = PMM_RUST_CONF_METHOD;
                if (m->receiver_type && strcmp(m->receiver_type, type_qn) != 0) {
                    strategy = "lsp_trait_dispatch";
                    conf = (impl_count == 1) ? PMM_RUST_CONF_TRAIT_SOLE : PMM_RUST_CONF_TRAIT_AMB;
            ...
            for (int hop = 0; hop < 8; hop++) {
                ...
                while (next &&
                       (next->kind == PMM_TYPE_REFERENCE || next->kind == PMM_TYPE_POINTER)) {
                    next = (next->kind == PMM_TYPE_REFERENCE) ? next->data.reference.elem
                                                              : next->data.pointer.elem;
                }
                if (!next)
                    break;
                const char *next_qn = NULL;
                if (next->kind == PMM_TYPE_NAMED)
                    next_qn = next->data.named.qualified_name;
                else if (next->kind == PMM_TYPE_TEMPLATE)
                    next_qn = next->data.template_type.template_name;
                else if (next->kind == PMM_TYPE_BUILTIN) {
            ...
            if (is_prelude_trait_method(mname)) {
                rust_emit_resolved_call(ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", type_qn, mname),
                                        "lsp_prelude_trait", PMM_RUST_CONF_TRAIT_AMB);
                return;
            }
            rust_emit_unresolved_call(ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", type_qn, mname),
                                      "method_not_found");
            return;
        }
        ...
        rust_emit_unresolved_call(
            ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", recv_text ? recv_text : "?", mname),
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "scoped_identifier") == 0 ||
        ...
        if (pmm_registry_lookup_func(ctx->registry, qn)) {
            rust_emit_resolved_call(ctx, qn, "lsp_direct", PMM_RUST_CONF_DIRECT);
            return;
        }
        if (ctx->module_qn && strstr(qn, ".") == NULL) {
            const char *full = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
            if (pmm_registry_lookup_func(ctx->registry, full)) {
                rust_emit_resolved_call(ctx, full, "lsp_direct", PMM_RUST_CONF_DIRECT);
                return;
            }
        }
        ...
        if (dot) {
            ...
            const CBMRegisteredType *head_t = pmm_registry_lookup_type(ctx->registry, head);
            if (head_t && head_t->is_interface) {
                trait_qn = head;
            } else if (ctx->module_qn) {
                const char *fh = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, head);
                const CBMRegisteredType *ft = pmm_registry_lookup_type(ctx->registry, fh);
                ...
                if (tn >= 1) {
                    rust_emit_resolved_call(
                        ...
                        ti_m ? ti_m->qualified_name
                             : pmm_arena_sprintf(ctx->arena, "%s.%s", trait_qn, short_name),
                        tn == 1 ? "lsp_trait_ufcs" : "lsp_trait_ufcs_amb",
                        tn == 1 ? PMM_RUST_CONF_TRAIT_SOLE : PMM_RUST_CONF_TRAIT_AMB);
                }
                return;
            }
            const CBMRegisteredFunc *m =
            ...
            if (!m && ctx->module_qn) {
                ...
                const char *full_head =
                    pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, head);
                m = pmm_registry_lookup_method_aliased(ctx->registry, full_head, short_name);
            }
            if (m) {
            ...
            if (tm) {
                rust_emit_resolved_call(
                    ...
                    impls == 1 ? PMM_RUST_CONF_TRAIT_SOLE : PMM_RUST_CONF_TRAIT_AMB);
                return;
            }
        }
        ...
        if (ctx->cargo_manifest && tail && *tail) {
            ...
            if (head_sep && head_sep > path) {
                char *head = pmm_arena_strndup(ctx->arena, path, (size_t)(head_sep - path));
                const CBMCargoManifest *m = (const CBMCargoManifest *)ctx->cargo_manifest;
                if (head && pmm_cargo_find_member(m, head)) {
                    /* `.crate_a.` — the member directory appears as a dotted
                     * QN segment for every def inside that crate. */
                    char *needle = pmm_arena_sprintf(ctx->arena, ".%s.", head);
                    ...
                    pmm_registry_free_funcs_by_short_name(ctx->registry, tail, &ffit);
                    for (int i; mem_matches < 2 && (i = pmm_free_func_iter_next(&ffit)) >= 0;) {
        ...
        if (tail && *tail && ctx->module_qn) {
            ...
            CBMFreeFuncIter ffit;
            pmm_registry_free_funcs_by_short_name(ctx->registry, tail, &ffit);
            for (int i; matches < 2 && (i = pmm_free_func_iter_next(&ffit)) >= 0;) {
                const CBMRegisteredFunc *f = &ctx->registry->funcs[i];
                if (!f->short_name || !f->qualified_name)
                    continue;
                if (f->receiver_type)
...
static void rust_resolve_calls_in_node(RustLSPContext *ctx, TSNode node) {
        ...
        if (!ts_node_is_null(fn) && strcmp(ts_node_type(fn), "field_expression") == 0) {
            ...
            if (!ts_node_is_null(val) && !ts_node_is_null(fld)) {
                ...
                if (is_item_method) {
                    ...
                    const CBMType *base = recv;
                    while (base &&
                           (base->kind == PMM_TYPE_REFERENCE || base->kind == PMM_TYPE_POINTER)) {
                        base = (base->kind == PMM_TYPE_REFERENCE) ? base->data.reference.elem
                                                                  : base->data.pointer.elem;
                    }
                    if (base && base->kind == PMM_TYPE_TEMPLATE &&
        ...
        if (!ts_node_is_null(mname_node)) {
            ...
            if (mname) {
                ...
                if (strcmp(mname, "println") == 0 || strcmp(mname, "eprintln") == 0 ||
                    ...
                    path = pmm_arena_sprintf(ctx->arena, "std.macros.%s", mname);
                    ...
                    path = pmm_arena_sprintf(ctx->arena, "core.macros.%s", mname);
                ...
                if (path) {
                    rust_emit_resolved_call(ctx, path, "lsp_macro", PMM_RUST_CONF_MACRO_KNOWN);
                } else {
            ...
            if (base && base->kind == PMM_TYPE_SLICE) {
                ...
                if ((strstr(nm, "HashMap") || strstr(nm, "BTreeMap")) &&
                    ...
                    const CBMType *pair[2] = {base->data.template_type.template_args[0],
                                              base->data.template_type.template_args[1]};
                    elem = pmm_type_tuple(ctx->arena, pair, 2);
                }
            }
    ...
    if (strcmp(kind, "match_arm") == 0) {
         ...
         * work using pmm_type_unknown() if we cannot derive it. */
    }
    ...
    if (strcmp(kind, "closure_expression") == 0) {
        ...
        if (!ts_node_is_null(params)) {
            ...
            for (uint32_t i = 0; i < pc; i++) {
                 ...
                 * as the pattern. */
                TSNode pat = p;
                const CBMType *bound = (i == 0 && hint) ? hint : pmm_type_unknown();
...
static void rust_process_function(RustLSPContext *ctx, TSNode func_node, const char *parent_qn) {
    ...
    ctx->enclosing_func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", prefix, name);
        ...
        for (uint32_t i = 0; i < pc; i++) {
            ...
            if (strcmp(pk, "self_parameter") == 0) {
                if (ctx->self_type_qn) {
                    ...
                    const CBMType *self_t = pmm_type_named(ctx->arena, ctx->self_type_qn);
                    if (text && strchr(text, '&')) {
                        self_t = pmm_type_reference(ctx->arena, self_t);
                    }
                    pmm_scope_bind(ctx->current_scope, "self", self_t);
                }
                continue;
            }
...
static void rust_record_type_param_bound(RustLSPContext *ctx, const char *param_name,
    ...
    if (ctx->type_param_bound_count % 8 == 0) {
        ...
        void *narr = pmm_arena_alloc(ctx->arena, new_cap * sizeof(*ctx->type_param_bounds));
        if (!narr)
            return;
    ...
    ctx->type_param_bounds[ctx->type_param_bound_count].param_name =
        pmm_arena_strdup(ctx->arena, param_name);
    ctx->type_param_bounds[ctx->type_param_bound_count].trait_qn =
        pmm_arena_strdup(ctx->arena, trait_qn);
...
static void rust_collect_bounds_from_text(RustLSPContext *ctx, const char *text) {
    ...
    while (from < len) {
        ...
        char *pname =
            pmm_arena_strndup(ctx->arena, text + name_start, (size_t)(name_end - name_start));
        /* Look for `:`. */
        while (from < len && (text[from] == ' ' || text[from] == '\t'))
            from++;
        ...
        while (from < len) {
            ...
            if (c == '<' || c == '(' || c == '[')
                ...
                if (b_end > b_start) {
                    char *btext =
                        pmm_arena_strndup(ctx->arena, text + b_start, (size_t)(b_end - b_start));
...
static void rust_collect_uses(RustLSPContext *ctx, TSNode root) {
    ...
    stack_t *top = (stack_t *)pmm_arena_alloc(ctx->arena, sizeof(stack_t));
    top->node = root;
    top->prev = NULL;
    while (top) {
        TSNode n = top->node;
        ...
        if (strcmp(k, "use_declaration") == 0) {
            ...
            if (full) {
                ...
                if (flen >= 3 && strcmp(full + flen - 3, "::*") == 0) {
                    char *mod = pmm_arena_strndup(ctx->arena, full, flen - 3);
                    ...
                    if (lbr) {
                        ...
                        char *prefix = pmm_arena_strndup(ctx->arena, full, prefix_len);
                        char *body = pmm_arena_strdup(ctx->arena, lbr + 1);
                        ...
                        while (tok) {
                            ...
                            char *full_path =
                                (strcmp(tok, "self") == 0)
                                    ? pmm_arena_strdup(ctx->arena, prefix)
                                    : pmm_arena_sprintf(ctx->arena, "%s::%s", prefix, path_part);
        ...
        if (strcmp(k, "mod_item") == 0 || strcmp(k, "source_file") == 0 ||
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (ts_node_is_null(c))
                    continue;
                stack_t *nx = (stack_t *)pmm_arena_alloc(ctx->arena, sizeof(stack_t));
                nx->node = c;
                nx->prev = top;
                top = nx;
            }
...
static void rust_build_registry_from_defs(CBMArena *arena, CBMTypeRegistry *reg,
    ...
    pmm_rust_stdlib_register(reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (pmm_label_is_type_like(d->label)) {
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->param_types) {
                ...
                while (d->param_types[count])
                    count++;
                if (count > 0) {
                    param_types = (const CBMType **)pmm_arena_alloc(
                        arena, (count + 1) * sizeof(const CBMType *));
                    for (int j = 0; j < count; j++) {
                        param_types[j] =
            ...
            rf.signature = pmm_type_func(arena, d->param_names, param_types, ret_types);

            if (strcmp(d->label, "Method") == 0 && d->parent_class) {
                rf.receiver_type = d->parent_class;
                if (!pmm_registry_lookup_type(reg, rf.receiver_type)) {
                    ...
                    auto_t.short_name = dot ? pmm_arena_strdup(arena, dot + 1) : rf.receiver_type;
                    pmm_registry_add_type(reg, auto_t);
                }
            }
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(tk, "struct_item") == 0) {
                ...
                if (ts_node_is_null(name_node) || ts_node_is_null(body))
                    continue;
                char *tn = pmm_node_text(arena, name_node, source);
                if (!tn || !tn[0])
                    continue;
                const char *type_qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, tn);
                ...
                if (strcmp(ts_node_type(body), "field_declaration_list") == 0) {
                    ...
                    for (uint32_t j = 0; j < fc && fld_count < 63; j++) {
                        ...
                        char *fname = pmm_node_text(arena, fn, source);
                        if (!fname)
                            continue;
                    ...
                    if (fld_count > 0) {
                        for (int ti = 0; ti < reg->type_count; ti++) {
                            if (reg->types[ti].qualified_name &&
                                ...
                                const char **names = (const char **)pmm_arena_alloc(
                                    arena, (fld_count + 1) * sizeof(const char *));
                                const CBMType **types = (const CBMType **)pmm_arena_alloc(
            ...
            if (strcmp(tk, "trait_item") == 0) {
                ...
                if (!tn || !tn[0])
                    continue;
                const char *trait_qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, tn);
                        ...
                        if (!ts_node_is_null(body)) {
                            ...
                            for (uint32_t j = 0; j < bc && mc < 63; j++) {
                                ...
                                if (ts_node_is_null(mn))
                                    continue;
                                char *mname = pmm_node_text(arena, mn, source);
                                if (mname)
                                    methods[mc++] = mname;
                            }
                            if (mc > 0) {
    ...
    if (!ts_node_is_null(root)) {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            ...
            if (ts_node_is_null(mn) || ts_node_is_null(rtn))
                continue;
            char *fname = pmm_node_text(arena, mn, source);
            if (!fname)
                continue;
            const CBMType *ret = rust_parse_type_node(&tmp, rtn);
            const char *fn_qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, fname);
            for (int k = 0; k < reg->func_count; k++) {
                ...
                const CBMType **ret_arr =
                    (const CBMType **)pmm_arena_alloc(arena, 2 * sizeof(const CBMType *));
                ret_arr[0] = ret;
                ret_arr[1] = NULL;
                rf->signature = pmm_type_func(arena, NULL, NULL, ret_arr);
                break;
            }
        }
    }
            ...
            for (int di = 0; d->decorators[di]; di++) {
                ...
                while (q < rparen) {
                    ...
                    for (int di2 = 0; di2 < derive_count; di2++) {
                        ...
                        const char **new_arr = (const char **)pmm_arena_alloc(
                        ...
                        for (int mi = 0; mi < 4 && di_entry->methods[mi].short_name; mi++) {
                            ...
                            rf.qualified_name = pmm_arena_sprintf(arena, "%s.%s", d->qualified_name,
                                                                  dm->short_name);
                            ...
                            rf.min_params = -1;
                            const CBMType *ret_t = pmm_type_unknown();
                            if (dm->return_type) {
                                if (strcmp(dm->return_type, "bool") == 0) {
                                    ret_t = pmm_type_builtin(arena, "bool");
                                } else if (strcmp(dm->return_type, "()") == 0) {
                                    ret_t = pmm_type_builtin(arena, "()");
                                }
                            } else if (dm->is_static) {
                                /* `default()`, `parse()` return Self. */
                                ret_t = pmm_type_named(arena, d->qualified_name);
                            }
                            const CBMType **ra = (const CBMType **)pmm_arena_alloc(
                                arena, 2 * sizeof(const CBMType *));
                            ra[0] = ret_t;
                            ra[1] = NULL;
                            rf.signature = pmm_type_func(arena, NULL, NULL, ra);
                            pmm_registry_add_func(reg, rf);
                        }
                        break;
                    }
    ...
    if (!ts_node_is_null(root)) {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            ...
            if (ts_node_is_null(type_node) || ts_node_is_null(body))
                continue;
            char *type_name = pmm_node_text(arena, type_node, source);
            if (!type_name || !type_name[0])
                continue;
            const char *type_qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, type_name);
            ...
            for (uint32_t j = 0; j < bnc; j++) {
                ...
                if (ret && ret->kind == PMM_TYPE_NAMED &&
                    strcmp(ret->data.named.qualified_name, "Self") == 0) {
                    ret = pmm_type_named(arena, type_qn);
                }
    ...
    for (int i = 0; i < result->impl_traits.count; i++) {
        ...
        if (!it->struct_name || !it->trait_name)
            continue;
        const char *recv_qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, it->struct_name);
        const char *trait_qn = strstr(it->trait_name, "::")
                                   ? convert_path_to_qn(arena, it->trait_name)
                                   : pmm_arena_sprintf(arena, "%s.%s", module_qn, it->trait_name);
...
void pmm_run_rust_lsp_with_manifest(CBMArena *arena, CBMFileResult *result, const char *source,
    ...
    pmm_rust_synth_proc_macro_edges(arena, result);
}

void pmm_run_rust_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
                      TSNode root) {
    pmm_run_rust_lsp_with_manifest(arena, result, source, source_len, root, NULL);
}
 ...
 * per-file resolver (pmm_run_rust_lsp_cross_with_manifest) and the build-once shared
 * registry (pmm_rust_build_cross_registry) so both produce a byte-identical registry.
...
static void rust_populate_cross_registry(CBMTypeRegistry *reg, CBMArena *arena,
    ...
    for (int ti = 0; ti < reg->type_count; ti++) {
        ...
        if (qn) {
            pmm_idxmemo_put_if_absent(&type_idx, arena, qn, ti);
        }
    }
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (pmm_label_is_type_like(d->label)) {
            CBMRegisteredType rt;
            memset(&rt, 0, sizeof(rt));
            rt.qualified_name = pmm_arena_strdup(arena, d->qualified_name);
            rt.short_name = pmm_arena_strdup(arena, d->short_name);
            rt.is_interface = d->is_interface || strcmp(d->label, "Trait") == 0 ||
                              strcmp(d->label, "Interface") == 0;
            pmm_registry_add_type(reg, rt);
            pmm_idxmemo_put_if_absent(&type_idx, arena, rt.qualified_name, reg->type_count - 1);
        }

        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            CBMRegisteredFunc rf;
            memset(&rf, 0, sizeof(rf));
            rf.qualified_name = pmm_arena_strdup(arena, d->qualified_name);
            rf.short_name = pmm_arena_strdup(arena, d->short_name);
            rf.min_params = -1;
            ...
            if (d->return_types && d->return_types[0]) {
                ...
                ret_types =
                    (const CBMType **)pmm_arena_alloc(arena, (count + 1) * sizeof(const CBMType *));
                int idx = 0;
                char *buf = pmm_arena_strdup(arena, d->return_types);
                char *start = buf;
                for (char *p = buf;; p++) {
                    if (*p == '|' || *p == '\0') {
                        char save = *p;
            ...
            rf.signature = pmm_type_func(arena, NULL, NULL, ret_types);

            if (strcmp(d->label, "Method") == 0 && d->receiver_type && d->receiver_type[0]) {
                rf.receiver_type = pmm_arena_strdup(arena, d->receiver_type);
                if (pmm_idxmemo_get(&type_idx, rf.receiver_type) < 0) {
                    CBMRegisteredType auto_t;
                    memset(&auto_t, 0, sizeof(auto_t));
                    auto_t.qualified_name = rf.receiver_type;
                    ...
                    auto_t.short_name = dot ? pmm_arena_strdup(arena, dot + 1) : rf.receiver_type;
                    pmm_registry_add_type(reg, auto_t);
                    pmm_idxmemo_put_if_absent(&type_idx, arena, auto_t.qualified_name,
                                              reg->type_count - 1);
                }
            }
            ...
            if (rf.receiver_type && d->trait_qn && d->trait_qn[0]) {
                CBMRegisteredType *rt = NULL;
                int32_t tix = pmm_idxmemo_get(&type_idx, rf.receiver_type);
                if (tix >= 0) {
                    rt = &reg->types[tix];
                }
                if (rt) {
                    ...
                    for (int j = 0; j < existing; j++)
                        new_arr[j] = rt->embedded_types[j];
                    new_arr[existing] = pmm_arena_strdup(arena, d->trait_qn);
                    new_arr[existing + 1] = NULL;
                    rt->embedded_types = new_arr;
                }
            }
...
CBMTypeRegistry *pmm_rust_build_cross_registry(CBMArena *arena, CBMLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
    ...
    if (def_count > 0) {
        rdefs = (CBMRustLSPDef *)pmm_arena_alloc(arena, (size_t)def_count * sizeof(CBMRustLSPDef));
        if (!rdefs)
            return NULL;
        for (int i = 0; i < def_count; i++) {
 ...
 * per-file registry build; just parse + resolve. Mirrors pmm_run_c_lsp_cross_with_registry. */
void pmm_run_rust_lsp_cross_with_registry(CBMArena *arena, const char *source, int source_len,
                                          const char *module_qn, const CBMTypeRegistry *reg,
                                          const char **import_names, const char **import_qns,
...
void pmm_run_rust_lsp_cross_with_manifest(CBMArena *arena, const char *source, int source_len,
     ...
     * Mirrors the single-file path (pmm_run_rust_lsp_with_manifest). NULL
...
void pmm_run_rust_lsp_cross(CBMArena *arena, const char *source, int source_len,
                            const char *module_qn, CBMRustLSPDef *defs, int def_count,
                            const char **import_names, const char **import_qns, int import_count,
                            TSTree *cached_tree, CBMResolvedCallArray *out) {
    pmm_run_rust_lsp_cross_with_manifest(arena, source, source_len, module_qn, defs, def_count,
                                         import_names, import_qns, import_count, cached_tree, NULL,
                                         out);
}

void pmm_batch_rust_lsp_cross(CBMArena *arena, CBMBatchRustLSPFile *files, int file_count,
                              CBMResolvedCallArray *out) {
    ...
    for (int f = 0; f < file_count; f++) {
        ...
        pmm_run_rust_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
