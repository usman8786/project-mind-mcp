#ifndef PMM_LSP_PHP_LSP_H
#define PMM_LSP_PHP_LSP_H
...
typedef struct {
    ...
    enum { PMM_PHP_USE_CLASS = 0, PMM_PHP_USE_FUNCTION, PMM_PHP_USE_CONST } *use_kinds;
    int use_count;
    int use_cap;
...
/* Evaluate a PHP expression's type. May return NULL / PMM_TYPE_UNKNOWN. */
 ...
 * then run resolution. Called from pmm_extract_file(). */
 ...
 *
 * Mirrors pmm_run_py_lsp_cross / pmm_run_ts_lsp_cross. Caller supplies the
...
void pmm_batch_php_lsp_cross(
    CBMArena *arena,
    CBMBatchPHPLSPFile *files, int file_count,
    CBMResolvedCallArray *out);

#endif /* PMM_LSP_PHP_LSP_H */
