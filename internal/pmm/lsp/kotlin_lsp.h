 ...
 * call-edge and method-dispatch attribution problems PMM cares about.
 *
 ...
 */
#ifndef PMM_LSP_KOTLIN_LSP_H
#define PMM_LSP_KOTLIN_LSP_H
...
typedef enum {
    PMM_KT_USE_UNKNOWN = 0,
    PMM_KT_USE_TYPE,     /* class / interface / object / typealias */
    PMM_KT_USE_FUNCTION, /* top-level fun, extension fun */
    PMM_KT_USE_PROPERTY, /* top-level val/var */
    PMM_KT_USE_WILDCARD, /* import a.b.* — local_name is a.b prefix */
} CBMKotlinUseKind;
...
typedef struct KotlinLSPContext {
     ...
     * overflow on deeply-nested/cyclic files; see pmm_lsp_max_walk_depth).
     * Zero via memset. */
    int walk_depth;

    /* Debug mode (PMM_LSP_DEBUG env). */
    bool debug;
} KotlinLSPContext;
 ...
 * pass the package prefix as target_qn and PMM_KT_USE_WILDCARD as kind. */
...
/* Evaluate a Kotlin expression's type. Returns pmm_type_unknown() on
 * failure — never NULL. */
 ...
 * Called from pmm_extract_file() after definitions and imports have been
 * extracted. */
 ...
 * Mirrors pmm_run_java_lsp_cross. `defs` carries the graph QNs of every
...
void pmm_kotlin_stdlib_register(CBMTypeRegistry *reg, CBMArena *arena);

/* Register Kotlin default-import targets — the prefixes auto-imported
 * by every Kotlin file. Used by the LSP context init. */
const char *const *pmm_kotlin_default_import_packages(int *count_out);

#endif /* PMM_LSP_KOTLIN_LSP_H */
