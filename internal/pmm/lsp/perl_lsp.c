 ...
 * at internal/pmm/vendored/grammars/perl/parser.c (ts_symbol_names and
 ...
 *
 * QN scheme (verified against helpers.c pmm_enclosing_func_qn): Perl has no
...
#define PMM_LSP_PERL_MAX_WALK_DEPTH 512
...
static char *perl_unquote(CBMArena *a, const char *s) {
    ...
    if ((s[0] == '\'' || s[0] == '"') && n >= 2 && s[n - 1] == s[0]) {
        return pmm_arena_strndup(a, s + 1, n - 2);
    }
    return NULL;
}
...
void perl_lsp_add_use(PerlLSPContext *ctx, const char *local_name, const char *target_qn) {
    ...
    if (ctx->use_count >= ctx->use_cap) {
        ...
        const char **ln =
            (const char **)pmm_arena_alloc(ctx->arena, (size_t)newcap * sizeof(char *));
        const char **tq =
            (const char **)pmm_arena_alloc(ctx->arena, (size_t)newcap * sizeof(char *));
        if (!ln || !tq)
            return;
...
static void perl_add_isa(PerlLSPContext *ctx, const char *pkg, const char *parent) {
    ...
    ctx->isa_pkg_qns[ctx->isa_count] = pmm_arena_strdup(ctx->arena, pkg);
    ctx->isa_parent_qns[ctx->isa_count] = pmm_arena_strdup(ctx->arena, parent);
    ctx->isa_count++;
}
 ...
 * CBMRegisteredFunc or NULL. Bounded by PMM_LSP_MAX_LOOKUP_DEPTH * 2 visited.
 *
...
const CBMRegisteredFunc *perl_lookup_method(PerlLSPContext *ctx, const char *package_qn,
    ...
    enum { CAP = PMM_LSP_MAX_LOOKUP_DEPTH * 2 };
    ...
    while (frontier_count > 0 && visited_count < CAP) {
        ...
        const CBMRegisteredType *t = pmm_registry_lookup_type(ctx->registry, pkg);
        if (!t) {
            /* Even without a type entry, a stdlib receiver-keyed method may
             * exist (e.g. a curated CPAN class). */
            const CBMRegisteredFunc *direct =
                pmm_registry_lookup_method(ctx->registry, pkg, method_name);
            if (direct)
                return direct;
            continue;
        }
            ...
            for (int i = 0; t->method_names[i]; i++) {
                if (strcmp(t->method_names[i], method_name) == 0) {
                    const CBMRegisteredFunc *f =
                        pmm_registry_lookup_func(ctx->registry, t->method_qns[i]);
                    if (f)
                        return f;
                }
            }
...
static const CBMType *perl_eval_bless(PerlLSPContext *ctx, TSNode call_node) {
    ...
    if (!have_class) {
        ...
        if (pkg && pkg[0])
            return pmm_type_named(ctx->arena, pkg);
    ...
    if (perl_is_string_node(ack)) {
        ...
        if (inner && inner[0])
            return pmm_type_named(ctx->arena, perl_resolve_package_name(ctx, inner));
        ...
        if (bw && strcmp(bw, "__PACKAGE__") == 0) {
            ...
            return pmm_type_named(ctx->arena, perl_resolve_package_name(ctx, bw));
        }
    } else {
...
const CBMType *perl_eval_expr_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "scalar") == 0 || strcmp(k, "scalar_variable") == 0) {
        ...
        if (txt) {
            ...
            const CBMType *t = pmm_scope_lookup(ctx->current_scope, bare);
            if (t)
                result = t;
        }
        ...
        if (blessed && !pmm_type_is_unknown(blessed))
            result = blessed;
        else
...
static const CBMType *perl_eval_new_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (perl_is_bareword_node(ik)) {
        ...
        if (cls && cls[0])
            return pmm_type_named(ctx->arena, perl_resolve_package_name(ctx, cls));
...
static const CBMType *perl_eval_function_call_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (colons) {
        ...
        char *pkg = pmm_arena_strndup(ctx->arena, name, plen);
        const char *shortn = colons + 2;
        f = perl_lookup_method(ctx, pkg, shortn);
        if (!f)
            f = pmm_registry_lookup_symbol(ctx->registry, pkg, shortn);
    } else {
        /* Bare func() — Exporter import map, then file-local/global func. */
        const char *imp = perl_find_import(ctx, name);
        if (imp)
            f = pmm_registry_lookup_func(ctx->registry, imp);
        if (!f)
            f = pmm_registry_lookup_symbol(ctx->registry, ctx->module_qn, name);
    }
...
static void perl_process_assignment(PerlLSPContext *ctx, TSNode assign) {
    ...
    if (rt && rt->kind == PMM_TYPE_NAMED)
        pmm_scope_bind(ctx->current_scope, bare, rt);
}
 ...
 * for the Java/C++ walkers). Past PMM_LSP_PERL_MAX_WALK_DEPTH the subtree is
 * skipped — graceful degradation, never a wrong edge. */
static void perl_resolve_calls_in_node(PerlLSPContext *ctx, TSNode node) {
    if (ctx->walk_depth >= PMM_LSP_PERL_MAX_WALK_DEPTH)
        return;
    ctx->walk_depth++;
...
static void perl_infer_self_type(PerlLSPContext *ctx, TSNode body) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const char *bare = perl_strip_sigil(vtxt);
        if (bare && bare[0])
            pmm_scope_bind(ctx->current_scope, bare, pmm_type_named(ctx->arena, pkg));
        free(kids);
...
static void process_subroutine(PerlLSPContext *ctx, TSNode node) {
    ...
    if (sname && sname[0]) {
        if (ctx->module_qn)
            ctx->enclosing_func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, sname);
        else
            ctx->enclosing_func_qn = pmm_arena_strdup(ctx->arena, sname);
    }
...
static void process_package_decl(PerlLSPContext *ctx, TSNode node) {
    ...
    if (!pkg || !pkg[0])
        return;
    ctx->current_package_qn = pmm_arena_strdup(ctx->arena, pkg);
...
static void perl_collect_qw_imports(PerlLSPContext *ctx, TSNode container,
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *target = pmm_arena_sprintf(ctx->arena, "%s.%s", module_dot, fn);
...
void perl_lsp_process_file(PerlLSPContext *ctx, TSNode root) {
     ...
     * (pmm_run_perl_lsp) has already run a pre-pass to build registry types. */
    ctx->current_package_qn = "";
...
static void perl_register_packages(PerlLSPContext *ctx, CBMTypeRegistry *reg) {
    for (int i = 0; i < ctx->isa_count; i++) {
        ...
        for (int s = 0; s < 2; s++) {
            ...
            if (!pkg || !pkg[0] || pmm_registry_lookup_type(reg, pkg))
                continue;
            CBMRegisteredType rt;
            memset(&rt, 0, sizeof(rt));
            rt.qualified_name = pmm_arena_strdup(ctx->arena, pkg);
    ...
    for (int t = 0; t < reg->type_count; t++) {
        ...
        if (pc == 0)
            continue;
        const char **parents =
            (const char **)pmm_arena_alloc(ctx->arena, (size_t)(pc + 1) * sizeof(char *));
        if (!parents)
            continue;
        int w = 0;
        for (int i = 0; i < ctx->isa_count; i++) {
...
static void perl_type_set_methods(PerlLSPContext *ctx, CBMTypeRegistry *reg, const char *pkg,
    ...
    if (!rt) {
        ...
        nt.qualified_name = pmm_arena_strdup(ctx->arena, pkg);
        nt.short_name = nt.qualified_name;
        pmm_registry_add_type(reg, nt);
        if (reg->type_count == 0)
    ...
    const char **mn =
        (const char **)pmm_arena_alloc(ctx->arena, (size_t)(total + 1) * sizeof(char *));
    const char **mq =
        (const char **)pmm_arena_alloc(ctx->arena, (size_t)(total + 1) * sizeof(char *));
    if (!mn || !mq)
        return;
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(k, "package_statement") == 0) {
            ...
            if (!ts_node_is_null(name)) {
                ...
                if (p && p[0])
                    cur_pkg = pmm_arena_strdup(ctx->arena, p);
            }
            ...
            for (uint32_t bi = 0; bi < bn; bi++) {
                ...
                const char *bqn = ctx->module_qn
                                      ? pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bsn)
                                      : pmm_arena_strdup(ctx->arena, bsn);
        ...
        const char *sub_qn = ctx->module_qn
                                 ? pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, sname)
                                 : pmm_arena_strdup(ctx->arena, sname);
...
/* ── entry: pmm_run_perl_lsp ────────────────────────────────────── */

void pmm_run_perl_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
                      TSNode root) {
    ...
    pmm_perl_stdlib_register(&reg, arena);
