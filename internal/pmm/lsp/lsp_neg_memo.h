 ...
 * (reg->read_only, set at finalize; pmm_registry_add_* hard-return then) and
 ...
 * key each cascade entry with pmm_negmemo_key (site tag + query strings),
 ...
 */
#ifndef PMM_LSP_NEG_MEMO_H
#define PMM_LSP_NEG_MEMO_H
...
enum {
    PMM_NEGMEMO_INIT_CAP = 1024,
    PMM_NEGMEMO_GROW = 2,
    PMM_NEGMEMO_LOAD_NUM = 7, /* grow at 70% load */
    PMM_NEGMEMO_LOAD_DEN = 10,
};
...
static inline uint64_t pmm_negmemo_key(uint8_t site, const char *a, const char *b) {
...
static inline bool pmm_negmemo_contains(const CBMNegMemo *m, uint64_t key) {
...
static inline void pmm_negmemo_insert_raw(uint64_t *slots, int cap, uint64_t key) {
...
static inline void pmm_negmemo_insert(CBMNegMemo *m, CBMArena *arena, uint64_t key) {
    if (!arena) {
        return; /* no arena — memo silently disabled */
    }
    if (!m->slots) {
        m->slots = pmm_arena_alloc(arena, sizeof(uint64_t) * PMM_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(uint64_t) * PMM_NEGMEMO_INIT_CAP);
        m->cap = PMM_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * PMM_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * PMM_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * PMM_NEGMEMO_GROW;
        uint64_t *ns = pmm_arena_alloc(arena, sizeof(uint64_t) * (size_t)new_cap);
        if (!ns) {
        ...
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i]) {
                pmm_negmemo_insert_raw(ns, new_cap, m->slots[i]);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    }
    if (!pmm_negmemo_contains(m, key)) {
        pmm_negmemo_insert_raw(m->slots, m->cap, key);
        m->count++;
    }
}
...
static inline int32_t pmm_idxmemo_get(const CBMIdxMemo *m, const char *key) {
    if (!m->slots || !key || m->count == 0) {
        return -1;
    }
    uint64_t h = pmm_negmemo_key(0, key, NULL);
    uint64_t mask = (uint64_t)(m->cap - 1);
...
static inline void pmm_idxmemo_put_raw(struct pmm_idxmemo_slot *slots, int cap, uint64_t h,
...
static inline void pmm_idxmemo_put_if_absent(CBMIdxMemo *m, CBMArena *arena, const char *key,
                                             int32_t val) {
    if (!arena || !key) {
        return;
    }
    if (!m->slots) {
        m->slots = pmm_arena_alloc(arena,
                                   sizeof(struct pmm_idxmemo_slot) * PMM_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(struct pmm_idxmemo_slot) * PMM_NEGMEMO_INIT_CAP);
        m->cap = PMM_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * PMM_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * PMM_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * PMM_NEGMEMO_GROW;
        struct pmm_idxmemo_slot *ns =
            pmm_arena_alloc(arena, sizeof(struct pmm_idxmemo_slot) * (size_t)new_cap);
        if (!ns) {
            return;
        }
        memset(ns, 0, sizeof(struct pmm_idxmemo_slot) * (size_t)new_cap);
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i].h) {
                pmm_idxmemo_put_raw(ns, new_cap, m->slots[i].h, m->slots[i].key, m->slots[i].val);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    ...
    uint64_t h = pmm_negmemo_key(0, key, NULL);
    if (pmm_idxmemo_get(m, key) < 0) {
        pmm_idxmemo_put_raw(m->slots, m->cap, h, key, val);
        m->count++;
    }
}

#endif /* PMM_LSP_NEG_MEMO_H */
