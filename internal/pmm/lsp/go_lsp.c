 ...
 * crash. The cap is PMM_LSP_MAX_WALK_DEPTH, env-overridable via the same name.
...
static void resolve_calls_in_node(GoLSPContext* ctx, TSNode node) {
    if (ctx->walk_depth >= pmm_lsp_max_walk_depth())
        return;
    ctx->walk_depth++;
...
void go_lsp_init(GoLSPContext* ctx, CBMArena* arena, const char* source, int source_len,
    ...
    ctx->current_scope = pmm_scope_push(arena, NULL); // root scope
...
void go_lsp_add_import(GoLSPContext* ctx, const char* local_name, const char* pkg_qn) {
    ...
    if (ctx->import_count % 32 == 0) {
        ...
        const char** new_names = (const char**)pmm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char*));
        const char** new_qns = (const char**)pmm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char*));
    ...
    ctx->import_local_names[ctx->import_count] = pmm_arena_strdup(ctx->arena, local_name);
    ctx->import_package_qns[ctx->import_count] = pmm_arena_strdup(ctx->arena, pkg_qn);
    ctx->import_count++;
}
...
const CBMType* go_parse_type_node(GoLSPContext* ctx, TSNode node) {
    if (ts_node_is_null(node)) return pmm_type_unknown();
    ...
    if (strcmp(kind, "type_identifier") == 0) {
        ...
        if (!name) return pmm_type_unknown();
        ...
        return pmm_type_named(ctx->arena,
            pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->package_qn, name));
    }
    ...
    if (strcmp(kind, "qualified_type") == 0) {
        ...
        if (!ts_node_is_null(pkg_node) && !ts_node_is_null(name_node)) {
            ...
            if (pkg_qn) {
                return pmm_type_named(ctx->arena,
                    pmm_arena_sprintf(ctx->arena, "%s.%s", pkg_qn, name));
            }
        }
    ...
    if (strcmp(kind, "slice_type") == 0) {
        ...
        return pmm_type_slice(ctx->arena, go_parse_type_node(ctx, elem));
    }
    ...
    if (strcmp(kind, "channel_type") == 0) {
        ...
        return pmm_type_channel(ctx->arena, go_parse_type_node(ctx, value), dir);
    }

    // function_type: func(...)...
    if (strcmp(kind, "function_type") == 0) {
        return pmm_type_func(ctx->arena, NULL, NULL, NULL); // simplified
    }
    ...
    if (strcmp(kind, "parameter_list") == 0) {
        ...
        if (count > 1) return pmm_type_tuple(ctx->arena, elems, count);
    }
...
static void go_unify_type(const CBMType* param_type, const CBMType* arg_type,
    ...
    if (!param_type || !arg_type || pmm_type_is_unknown(arg_type)) return;
    ...
    if (param_type->kind == PMM_TYPE_MAP && arg_type->kind == PMM_TYPE_MAP) {
    ...
    if (param_type->kind == PMM_TYPE_CHANNEL && arg_type->kind == PMM_TYPE_CHANNEL) {
        go_unify_type(param_type->data.channel.elem, arg_type->data.channel.elem,
            type_param_names, inferred, param_count);
    }
    if (param_type->kind == PMM_TYPE_FUNC && arg_type->kind == PMM_TYPE_FUNC) {
        // Match param types
...
const CBMType* go_eval_expr_type(GoLSPContext* ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (!pmm_type_is_unknown(t)) return t;

        // Check if it's a package-level function
        const CBMRegisteredFunc* f = pmm_registry_lookup_symbol(ctx->registry, ctx->package_qn, name);
        ...
        if (bt) return pmm_type_named(ctx->arena, name);

        // Check if it's a registered type (for type conversions like MyType(x))
        const char* type_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->package_qn, name);
        const CBMRegisteredType* rt = pmm_registry_lookup_type(ctx->registry, type_qn);
        if (rt) return pmm_type_named(ctx->arena, type_qn);
    ...
    if (strcmp(kind, "selector_expression") == 0) {
        ...
        if (ts_node_is_null(operand) || ts_node_is_null(field)) return pmm_type_unknown();

        char* field_name = lsp_node_text(ctx, field);
        if (!field_name) return pmm_type_unknown();
        ...
        if (strcmp(ts_node_type(operand), "identifier") == 0) {
            ...
            if (pkg_name) {
                ...
                if (pkg_qn) {
                    ...
                    const CBMRegisteredFunc* f = pmm_registry_lookup_symbol(ctx->registry, pkg_qn, field_name);
                    if (f && f->signature) return f->signature;
                    // Check if it's a type
                    char* type_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", pkg_qn, field_name);
                    const CBMRegisteredType* rt = pmm_registry_lookup_type(ctx->registry, type_qn);
                    if (rt) return pmm_type_named(ctx->arena, type_qn);
                    return pmm_type_unknown();
                }
        ...
        if (pmm_type_is_unknown(recv_type)) return pmm_type_unknown();

        // Auto-deref pointers for method calls
        const CBMType* base_type = recv_type;
        if (base_type->kind == PMM_TYPE_POINTER) {
            base_type = pmm_type_deref(base_type);
        }

        if (base_type->kind == PMM_TYPE_NAMED) {
            ...
            if (field_type && !pmm_type_is_unknown(field_type)) return field_type;
        }
    ...
    if (strcmp(kind, "call_expression") == 0) {
        ...
        if (ts_node_is_null(func_node)) return pmm_type_unknown();
        ...
        if (func_type && func_type->kind == PMM_TYPE_FUNC &&
            ...
            if (!ts_node_is_null(targs_node)) {
                ...
                if (func_name) {
                    const char* func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s",
                        ctx->package_qn, func_name);
                    rfunc = pmm_registry_lookup_func(ctx->registry, func_qn);
                    ...
                    if (!rfunc && strcmp(ts_node_type(func_node), "selector_expression") == 0) {
                        ...
                        if (!ts_node_is_null(operand) && !ts_node_is_null(field)) {
                            ...
                            if (pkg && fn) {
                                const char* pkg_qn = resolve_import(ctx, pkg);
                                if (pkg_qn) {
                                    rfunc = pmm_registry_lookup_symbol(ctx->registry, pkg_qn, fn);
                                }
                            }
                        }
                    }
                ...
                if (rfunc && rfunc->type_param_names) {
                    ...
                    if (targ_count > 0 && targ_count == param_count) {
                        const CBMType** targ_arr = (const CBMType**)pmm_arena_alloc(
                        ...
                        if (ret_count == 1) {
                            return pmm_type_substitute(ctx->arena,
                        ...
                        for (int ri = 0; ri < ret_count; ri++) {
                            new_rets[ri] = pmm_type_substitute(ctx->arena,
                                func_type->data.func.return_types[ri],
                                rfunc->type_param_names, targ_arr);
                        }
                        new_rets[ret_count] = NULL;
                        return pmm_type_tuple(ctx->arena, new_rets, ret_count);
                    }
                }
            }
            ...
            if (ts_node_is_null(targs_node)) {
                ...
                if (rfunc && rfunc->type_param_names && func_type->data.func.param_types) {
                    ...
                    for (int ri = 0; ri < ret_count && !has_type_param; ri++) {
                        ...
                        if (rt->kind == PMM_TYPE_TYPE_PARAM) has_type_param = true;
                        else if (rt->kind == PMM_TYPE_SLICE && rt->data.slice.elem &&
                                 rt->data.slice.elem->kind == PMM_TYPE_TYPE_PARAM) has_type_param = true;
                        else if (rt->kind == PMM_TYPE_POINTER && rt->data.pointer.elem &&
                                 rt->data.pointer.elem->kind == PMM_TYPE_TYPE_PARAM) has_type_param = true;
                        else if (rt->kind == PMM_TYPE_MAP) {
                            if (rt->data.map.key && rt->data.map.key->kind == PMM_TYPE_TYPE_PARAM) has_type_param = true;
                            if (rt->data.map.value && rt->data.map.value->kind == PMM_TYPE_TYPE_PARAM) has_type_param = true;
                        }
                    }
            ...
            if (count > 1) {
                return pmm_type_tuple(ctx->arena, func_type->data.func.return_types, count);
            }
        }
    ...
    if (strcmp(kind, "unary_expression") == 0) {
        TSNode operand = ts_node_child_by_field_name(node, "operand", 7);
        if (ts_node_is_null(operand)) return pmm_type_unknown();
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                ...
                if (!op) continue;
                if (strcmp(op, "&") == 0) {
                    return pmm_type_pointer(ctx->arena, go_eval_expr_type(ctx, operand));
                }
                if (strcmp(op, "*") == 0) {
                    return pmm_type_deref(go_eval_expr_type(ctx, operand));
                }
                if (strcmp(op, "<-") == 0) {
                    const CBMType* ch_type = go_eval_expr_type(ctx, operand);
                    if (ch_type && ch_type->kind == PMM_TYPE_CHANNEL) {
    ...
    if (strcmp(kind, "index_expression") == 0) {
        ...
        if (!op_type) return pmm_type_unknown();
        if (op_type->kind == PMM_TYPE_MAP) return op_type->data.map.value;
        if (op_type->kind == PMM_TYPE_SLICE) return op_type->data.slice.elem;
        return pmm_type_unknown();
    }
    ...
    if (strcmp(kind, "func_literal") == 0) {
                    ...
                    for (uint32_t j = 0; j < pnc; j++) {
                        ...
                        if (!ts_node_is_null(ch) && ts_node_is_named(ch) &&
                            ...
                            if (pname && strcmp(pname, "_") != 0)
                                pmm_scope_bind(ctx->current_scope, pname, pt);
                        }
                    }
                }
            }
        ...
        return pmm_type_func(ctx->arena, NULL,
...
const CBMType* go_eval_builtin_call(GoLSPContext* ctx, const char* name, TSNode args) {
    ...
    if (strcmp(name, "new") == 0 && !ts_node_is_null(args)) {
        ...
        if (nc > 0) {
            TSNode first_arg = ts_node_named_child(args, 0);
            return pmm_type_pointer(ctx->arena, go_parse_type_node(ctx, first_arg));
        }
    }
...
static const CBMRegisteredFunc* go_lookup_field_or_method_depth(GoLSPContext* ctx,
    ...
    if (depth > PMM_LSP_MAX_LOOKUP_DEPTH) return NULL;
...
void go_process_statement(GoLSPContext* ctx, TSNode node) {
    ...
    if (strcmp(kind, "short_var_declaration") == 0) {
        ...
        if (strcmp(ts_node_type(left), "expression_list") == 0) {
            ...
            for (uint32_t i = 0; i < lhs_count; i++) {
                ...
                const CBMType* var_type = pmm_type_unknown();
                if (rhs_type) {
                    if (rhs_type->kind == PMM_TYPE_TUPLE && (int)i < rhs_type->data.tuple.count) {
            ...
            if (var_name && strcmp(var_name, "_") != 0 && rhs_type) {
                pmm_scope_bind(ctx->current_scope, var_name, rhs_type);
            }
        }
        return;
    }
    ...
    if (strcmp(kind, "const_spec") == 0) {
        ...
        const CBMType* const_type = pmm_type_unknown();
        if (!ts_node_is_null(type_node)) {
        ...
        if (strcmp(ts_node_type(name_node), "identifier") == 0) {
            ...
            if (name && strcmp(name, "_") != 0)
                pmm_scope_bind(ctx->current_scope, name, const_type);
        }
        return;
    }
    ...
    if (strcmp(kind, "range_clause") == 0) {
        ...
        const CBMType* key_type = pmm_type_unknown();
        const CBMType* val_type = pmm_type_unknown();

        if (container_type) {
            switch (container_type->kind) {
            case PMM_TYPE_SLICE:
                key_type = pmm_type_builtin(ctx->arena, "int");
            ...
            case PMM_TYPE_MAP:
                ...
                break;
            case PMM_TYPE_CHANNEL:
                val_type = container_type->data.channel.elem;
                break;
            default:
                if (container_type->kind == PMM_TYPE_BUILTIN &&
                    strcmp(container_type->data.builtin.name, "string") == 0) {
                    key_type = pmm_type_builtin(ctx->arena, "int");
                    val_type = pmm_type_builtin(ctx->arena, "rune");
                }
                break;
            }
        }
        ...
        if (!ts_node_is_null(left) && strcmp(ts_node_type(left), "expression_list") == 0) {
            ...
            for (uint32_t i = 0; i < lhs_count; i++) {
                ...
                if (!var_name || strcmp(var_name, "_") == 0) continue;
                pmm_scope_bind(ctx->current_scope, var_name, i == 0 ? key_type : val_type);
            }
        }
        return;
    }
            ...
            if (strcmp(fk, "selector_expression") == 0) {
                ...
                if (!ts_node_is_null(operand) && !ts_node_is_null(field)) {
                        ...
                        if (pkg_name) {
                            ...
                            if (pkg_qn && field_name) {
                                ...
                                emit_unresolved_call(ctx,
                                    pmm_arena_sprintf(ctx->arena, "%s.%s", pkg_name, field_name),
                    ...
                    if (field_name) {
                        ...
                        if (base && base->kind == PMM_TYPE_POINTER) base = pmm_type_deref(base);
                        ...
                        if (base && field_name) {
                            bool is_iface = (base->kind == PMM_TYPE_INTERFACE);
                            const char* iface_qn = NULL;
                            if (!is_iface && base->kind == PMM_TYPE_NAMED) {
                                const CBMRegisteredType* rt = pmm_registry_lookup_type(ctx->registry,
                            ...
                            if (is_iface) {
                                ...
                                const CBMRegisteredType* iface_rt = iface_qn ?
                                    pmm_registry_lookup_type(ctx->registry, iface_qn) : NULL;
                                if (iface_rt && iface_rt->method_names && iface_rt->method_names[0]) {
                                    ...
                                    for (int ti = 0; ti < ctx->registry->type_count && impl_count < 2; ti++) {
                                        ...
                                        for (int mi = 0; mi < iface_mcount; mi++) {
                                            if (!pmm_registry_lookup_method(ctx->registry,
                                    ...
                                    if (impl_count == 1 && sole_impl_qn) {
                                        ...
                                        const CBMRegisteredFunc* concrete_method =
                                            pmm_registry_lookup_method(ctx->registry, sole_impl_qn, field_name);
                                        if (concrete_method) {
                        ...
                        if (base && base->kind == PMM_TYPE_NAMED) {
                            emit_unresolved_call(ctx,
                                pmm_arena_sprintf(ctx->arena, "%s.%s",
                                    base->data.named.qualified_name, field_name),
                                "method_not_found");
                        } else if (pmm_type_is_unknown(recv_type)) {
...
recurse:;
    ...
    if (strcmp(kind, "type_switch_statement") == 0) {
        ...
        for (uint32_t i = 0; i < nc2; i++) {
            ...
            for (uint32_t j = 0; j < cc_count; j++) {
                ...
                if (strcmp(cc_kind, "type_identifier") == 0 ||
                    ...
                    if (switch_var) {
                        pmm_scope_bind(ctx->current_scope, switch_var,
        ...
        if (push_scope) ctx->current_scope = pmm_scope_pop(ctx->current_scope);
        return;
    }
                ...
                for (uint32_t j = 0; j < cc_count; j++) {
                    ...
                    if (strcmp(cc_kind, "receive_statement") == 0) {
                        ...
                        if (!ts_node_is_null(right)) {
                            ...
                            if (right_type && right_type->kind == PMM_TYPE_CHANNEL) {
                            ...
                            if (!ts_node_is_null(left)) {
                                if (strcmp(ts_node_type(left), "expression_list") == 0) {
                                    ...
                                    for (uint32_t k = 0; k < lhs_count; k++) {
                                        ...
                                        if (!var_name || strcmp(var_name, "_") == 0) continue;
                                        if (k == 0) {
                                            pmm_scope_bind(ctx->current_scope, var_name, recv_type);
                                        } else {
                                            // second var is the ok bool
                                            pmm_scope_bind(ctx->current_scope, var_name,
                                                pmm_type_builtin(ctx->arena, "bool"));
                                        }
                                    }
                                } else if (strcmp(ts_node_type(left), "identifier") == 0) {
                                    char* var_name = lsp_node_text(ctx, left);
...
static void process_function(GoLSPContext* ctx, TSNode func_node) {
    ...
    // call's enclosing_func_qn, so pmm_pipeline_find_lsp_resolution never
    ...
    if (recv_type_name) {
        ctx->enclosing_func_qn =
            pmm_arena_sprintf(ctx->arena, "%s.%s.%s", ctx->package_qn, recv_type_name, func_name);
    } else {
        ctx->enclosing_func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->package_qn, func_name);
    }
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (is_variadic && param_type) {
                param_type = pmm_type_slice(ctx->arena, param_type);
            }
            ...
            for (uint32_t j = 0; j < pnc; j++) {
                ...
                if (strcmp(ts_node_type(child), "identifier") == 0) {
                    ...
                    if (pname && strcmp(pname, "_") != 0) {
                        pmm_scope_bind(ctx->current_scope, pname, param_type);
                    }
                }
            }
        }
                ...
                for (uint32_t j = 0; j < rpnc; j++) {
                    ...
                    if (strcmp(ts_node_type(child), "identifier") == 0) {
                        ...
                        if (rname && strcmp(rname, "_") != 0) {
                            pmm_scope_bind(ctx->current_scope, rname, ret_type);
                        }
                    }
                }
            }
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            ...
            for (uint32_t j = 0; j < rpnc; j++) {
                ...
                if (!ts_node_is_null(rc) && ts_node_is_named(rc) &&
                    ...
                    if (rname && strcmp(rname, "_") != 0) {
                        pmm_scope_bind(ctx->current_scope, rname, recv_type);
                    }
                    break;
                }
            }
...
const CBMType* pmm_parse_return_type_text(CBMArena* a, const char* text, const char* module_qn) {
    if (!text || !text[0]) return pmm_type_unknown();

    // Pointer: *Foo
    if (text[0] == '*') {
        return pmm_type_pointer(a, pmm_parse_return_type_text(a, text + 1, module_qn));
    }

    // Slice: []Foo
    if (text[0] == '[' && text[1] == ']') {
        return pmm_type_slice(a, pmm_parse_return_type_text(a, text + 2, module_qn));
    }
    ...
    for (const char** b = builtins; *b; b++) {
        if (strcmp(text, *b) == 0) return pmm_type_builtin(a, text);
    }
...
void pmm_run_go_lsp(CBMArena* arena, CBMFileResult* result,
    ...
    pmm_go_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (pmm_label_is_type_like(d->label)) {
            ...
            if (d->base_classes) {
                ...
                if (bc_count > 0) {
                    const char** embedded = (const char**)pmm_arena_alloc(arena, (bc_count + 1) * sizeof(const char*));
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->return_types) {
                ...
                if (count > 0) {
                    ret_types = (const CBMType**)pmm_arena_alloc(arena, (count + 1) * sizeof(const CBMType*));
                    for (int j = 0; j < count; j++) {
                        ret_types[j] = pmm_parse_return_type_text(arena, d->return_types[j], module_qn);
                    }
                ...
                ret_types[0] = pmm_parse_return_type_text(arena, d->return_type, module_qn);
                ret_types[1] = NULL;
            }
            ...
            if (d->param_types) {
                ...
                if (count > 0) {
                    param_types_arr = (const CBMType**)pmm_arena_alloc(arena, (count + 1) * sizeof(const CBMType*));
                    for (int j = 0; j < count; j++) {
                        param_types_arr[j] = pmm_parse_return_type_text(arena, d->param_types[j], module_qn);
                    }
            ...
            if (strcmp(d->label, "Method") == 0 && d->receiver && d->receiver[0]) {
                ...
                if (end > r) {
                    char* type_name = pmm_arena_strndup(arena, r, end - r);
                    rf.receiver_type = pmm_arena_sprintf(arena, "%s.%s", module_qn, type_name);

                    // Also register this method under the type
                    const CBMRegisteredType* existing = pmm_registry_lookup_type(&reg, rf.receiver_type);
                    if (!existing) {
    ...
    // Phase 1b: Scan AST for struct definitions to populate embedded_types
    {
        // Collect top-level children once (O(n)); ts_node_child(root,i) is O(i) → O(n²).
        uint32_t rkn = 0;
        TSNode* rkids = pmm_lsp_collect_children(arena, root, &rkn);
            ...
            for (uint32_t j = 0; j < td_nc; j++) {
                ...
                if (strcmp(spec_kind, "type_alias") == 0) {
                    ...
                    if (!ts_node_is_null(alias_name) && !ts_node_is_null(alias_type)) {
                        char* aname = pmm_node_text(arena, alias_name, source);
                        char* atarget = pmm_node_text(arena, alias_type, source);
                        if (aname && aname[0] && atarget && atarget[0]) {
                            const char* alias_type_qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, aname);
                            const char* alias_target_qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, atarget);
                            bool found_a = false;
                            ...
                            if (!found_a) {
                                ...
                                pmm_registry_add_type(&reg, alias_rt);
                            }
                        }
                    }
                    continue;
                ...
                char* type_name = pmm_node_text(arena,name_node, source);
                ...
                if (strcmp(ts_node_type(type_node), "interface_type") == 0) {
                    ...
                    for (uint32_t k = 0; k < inl_nc && iface_method_count < 63; k++) {
                        ...
                        if (strcmp(ck, "method_spec") == 0 || strcmp(ck, "method_elem") == 0) {
                            ...
                            if (!ts_node_is_null(mname)) {
                                char* mn = pmm_node_text(arena, mname, source);
                                if (mn && mn[0]) {
                    ...
                    if (iface_method_count > 0) {
                        for (int ti = 0; ti < reg.type_count; ti++) {
                            ...
                            const char** names = (const char**)pmm_arena_alloc(arena,
                ...
                for (uint32_t k = 0; k < fl_nc; k++) {
                    ...
                    if (ts_node_is_null(fname) && !ts_node_is_null(ftype)) {
                        ...
                        if (embed_count < 15) {
                            char* embed_text = pmm_node_text(arena, ftype, source);
                            if (embed_text && embed_text[0]) {
                                const char* et = embed_text;
                                while (*et == '*') et++;
                                embeds[embed_count++] = pmm_arena_sprintf(arena, "%s.%s", module_qn, et);
                            }
                        }
                    } else if (!ts_node_is_null(fname) && !ts_node_is_null(ftype) && fld_count < 63) {
                        // Named field: name + type
                        char* fn = pmm_node_text(arena, fname, source);
                        if (fn && fn[0]) {
                ...
                for (int ti = 0; ti < reg.type_count; ti++) {
                    ...
                    if (embed_count > 0) {
                        const char** arr = (const char**)pmm_arena_alloc(arena,
                    ...
                    if (fld_count > 0) {
                        ...
                        const CBMType** types = (const CBMType**)pmm_arena_alloc(arena,
...
static const CBMType** split_pipe_types(CBMArena* a, const char* text, const char* module_qn) {
    ...
    const CBMType** arr = (const CBMType**)pmm_arena_alloc(a, (count + 1) * sizeof(const CBMType*));
    if (!arr) return NULL;

    // Split and parse each type
    char* buf = pmm_arena_strdup(a, text);
    int idx = 0;
    char* start = buf;
    for (char* p = buf; ; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            *p = '\0';
            if (start[0]) {
                arr[idx++] = pmm_parse_return_type_text(a, start, module_qn);
            }
            if (save == '\0') break;
            start = p + 1;
        }
...
static const char** split_pipe_strings(CBMArena* a, const char* text) {
    ...
    const char** arr = (const char**)pmm_arena_alloc(a, (count + 1) * sizeof(const char*));
    if (!arr) return NULL;
    ...
    for (char* p = buf; ; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            *p = '\0';
            if (start[0]) {
                arr[idx++] = pmm_arena_strdup(a, start);
            }
            if (save == '\0') break;
            start = p + 1;
        }
...
static void parse_field_defs_into_type(CBMArena* arena, CBMTypeRegistry* reg,
    ...
    const char** names = (const char**)pmm_arena_alloc(arena, (count + 1) * sizeof(const char*));
    const CBMType** types = (const CBMType**)pmm_arena_alloc(arena, (count + 1) * sizeof(const CBMType*));
    if (!names || !types) return;

    char* buf = pmm_arena_strdup(arena, field_defs);
    int idx = 0;
    char* start = buf;
    for (char* p = buf; ; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            *p = '\0';
            ...
            if (colon && idx < count) {
                *colon = '\0';
                names[idx] = pmm_arena_strdup(arena, start);
                types[idx] = pmm_parse_return_type_text(arena, colon + 1, def_module_qn);
                idx++;
            }
            if (save == '\0') break;
            start = p + 1;
...
static const CBMType* parse_type_node_with_params(CBMArena* arena, TSNode node,
    ...
    if (strcmp(kind, "type_identifier") == 0) {
        ...
        if (strcmp(name, "int") == 0 || strcmp(name, "string") == 0 ||
            ...
            return pmm_type_builtin(arena, name);
        }
        return pmm_type_named(arena, pmm_arena_sprintf(arena, "%s.%s", module_qn, name));
    }
    ...
    if (strcmp(kind, "qualified_type") == 0) {
        ...
        TSNode name_node = ts_node_child_by_field_name(node, "name", 4);
        if (!ts_node_is_null(pkg_node) && !ts_node_is_null(name_node)) {
            char* pkg = pmm_node_text(arena, pkg_node, source);
            char* name = pmm_node_text(arena, name_node, source);
            if (pkg && name) {
                return pmm_type_named(arena, pmm_arena_sprintf(arena, "%s.%s", pkg, name));
            }
        }
        return pmm_type_unknown();
    }
    ...
    if (strcmp(kind, "pointer_type") == 0) {
        uint32_t nc = ts_node_named_child_count(node);
        if (nc > 0)
            return pmm_type_pointer(arena, parse_type_node_with_params(arena,
    ...
    if (strcmp(kind, "slice_type") == 0) {
        ...
        if (ts_node_is_null(elem) && ts_node_named_child_count(node) > 0)
            elem = ts_node_named_child(node, ts_node_named_child_count(node) - 1);
        return pmm_type_slice(arena, parse_type_node_with_params(arena,
    ...
    if (strcmp(kind, "channel_type") == 0) {
        ...
        if (ts_node_is_null(value) && ts_node_named_child_count(node) > 0)
            value = ts_node_named_child(node, ts_node_named_child_count(node) - 1);
        char* text = pmm_node_text(arena, node, source);
        int dir = 0;
        if (text) {
            if (strncmp(text, "chan<-", 6) == 0 || strncmp(text, "chan <-", 7) == 0) dir = 1;
            else if (strncmp(text, "<-chan", 6) == 0 || strncmp(text, "<- chan", 7) == 0) dir = 2;
        }
        return pmm_type_channel(arena, parse_type_node_with_params(arena,
    ...
    if (strcmp(kind, "function_type") == 0) {
        ...
        return pmm_type_func(arena, NULL, pt, rt);
    }

    // interface_type
    if (strcmp(kind, "interface_type") == 0) {
        CBMType* t = (CBMType*)pmm_arena_alloc(arena, sizeof(CBMType));
        memset(t, 0, sizeof(CBMType));
        t->kind = PMM_TYPE_INTERFACE;
        return t;
    }
...
static void extract_type_params_from_ast(CBMArena* arena, CBMTypeRegistry* reg,
    ...
    for (uint32_t i = 0; i < rkn; i++) {
        ...
        char* func_name = pmm_node_text(arena, name_node, source);
        ...
        for (uint32_t j = 0; j < tp_nc && param_count < 15; j++) {
            ...
            if (!ts_node_is_null(pname)) {
                char* pn = pmm_node_text(arena, pname, source);
                if (pn && pn[0]) {
                    params[param_count++] = pmm_arena_strdup(arena, pn);
                }
            }
        }
        ...
        const char** tp_names = (const char**)pmm_arena_alloc(arena, (param_count + 1) * sizeof(const char*));
        for (int j = 0; j <= param_count; j++) tp_names[j] = params[j];

        // Find the matching registered function and set type_param_names
        const char* func_qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, func_name);
        for (int fi = 0; fi < reg->func_count; fi++) {
            if (reg->funcs[fi].qualified_name &&
                ...
                if (reg->funcs[fi].signature && reg->funcs[fi].signature->kind == PMM_TYPE_FUNC &&
                    ...
                    for (int ri = 0; ri < ret_count && !needs_reparse; ri++) {
                        ...
                        if (check->kind == PMM_TYPE_SLICE && check->data.slice.elem)
                            check = check->data.slice.elem;
                        else if (check->kind == PMM_TYPE_POINTER && check->data.pointer.elem)
                            check = check->data.pointer.elem;
                        if (check->kind == PMM_TYPE_NAMED) {
                    ...
                    if (needs_reparse) {
                        ...
                        if (!ts_node_is_null(result_node)) {
                            const CBMType** new_rets = (const CBMType**)pmm_arena_alloc(arena,
                            ...
                            CBMType* new_sig = (CBMType*)pmm_arena_alloc(arena, sizeof(CBMType));
                ...
                if (reg->funcs[fi].signature && reg->funcs[fi].signature->kind == PMM_TYPE_FUNC &&
                    ...
                    if (!ts_node_is_null(params_node)) {
                        const CBMType** new_params = (const CBMType**)pmm_arena_alloc(arena,
    ...
    CBMResolvedCallArray* out)
{
    ...
    // caller's arena which outlives this call, so pmm_arena_strdup is
    ...
    // pmm_registry_lookup_method / lookup_type. Without this, every
 ...
 *
 * pmm_go_build_cross_registry runs the registry build ONCE per project (in
 ...
 *
 * pmm_run_go_lsp_cross_with_registry is the per-file entrypoint used by
...
CBMTypeRegistry* pmm_go_build_cross_registry(
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (d->lang != PMM_LANG_GO) continue;
        /* In the pre-built path every def carries its own def_module_qn
         * (set by pmm_pxc_collect_all_defs). There is no caller module to
...
void pmm_run_go_lsp_cross_with_registry(
    CBMArena* arena,
    const char* source, int source_len,
    const char* module_qn,
    CBMTypeRegistry* reg,
...
int pmm_go_fast_resolve_qualified_calls(
    ...
    for (int i = 0; i < initial_count; i++) {
        ...
        const CBMRegisteredFunc* f = pmm_registry_lookup_func(reg, uc->callee_qn);
                    ...
                    for (int j = 0; j < import_count; j++) {
                        if (import_names[j] && import_qns[j] &&
                            ...
                            if (n > 0 && n < (int)sizeof(fq)) {
                                f = pmm_registry_lookup_func(reg, fq);
                            }
                            break;
                        }
                    }
        ...
        /* Emit a resolved entry. pmm_pipeline_find_lsp_resolution
        ...
        rc.confidence = 0.92f;
        rc.reason = NULL;
        pmm_resolvedcall_push(&result->resolved_calls, &result->arena, rc);
        newly_resolved++;
    }
    ...
    CBMResolvedCallArray* out)
{
    ...
    for (int f = 0; f < file_count; f++) {
        ...
        if (file_out.count > 0) {
            ...
            out[f].items = (CBMResolvedCall*)pmm_arena_alloc(arena,
