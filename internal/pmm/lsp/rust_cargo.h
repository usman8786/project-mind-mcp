...
#ifndef PMM_LSP_RUST_CARGO_H
#define PMM_LSP_RUST_CARGO_H

#include "../arena.h"
#include <stdbool.h>

#define PMM_CARGO_MAX_DEPS    256
#define PMM_CARGO_MAX_MEMBERS  64
...
typedef struct CBMCargoManifest {
    ...
    CBMCargoDep deps[PMM_CARGO_MAX_DEPS];
    int dep_count;

    CBMCargoMember members[PMM_CARGO_MAX_MEMBERS];
    int member_count;
} CBMCargoManifest;

/* Parse a Cargo.toml-formatted string. The output strings are
 * arena-allocated (so the caller doesn't need to keep `src` alive). */
void pmm_cargo_parse(CBMArena* arena, const char* src, int src_len,
 ...
 * paths. */
bool pmm_cargo_is_known_dep(const CBMCargoManifest* m, const char* head);

/* Find a workspace member by crate name. Returns NULL if absent. */
const CBMCargoMember* pmm_cargo_find_member(const CBMCargoManifest* m,
    const char* name);

#endif /* PMM_LSP_RUST_CARGO_H */
