...
static void reg_method(CBMTypeRegistry* reg, CBMArena* arena,
    ...
    char* qn = (char*)pmm_arena_alloc(arena, rlen + 1 + mlen + 1);
    if (!qn) return;
    ...
    rf.signature = pmm_type_func(arena, NULL, NULL,
...
void pmm_cpp_stdlib_register(CBMTypeRegistry* reg, CBMArena* arena) {
    const CBMType* t_void = pmm_type_builtin(arena, "void");
    const CBMType* t_void_ptr = pmm_type_pointer(arena, t_void);
    const CBMType* t_bool = pmm_type_builtin(arena, "bool");
    const CBMType* t_int = pmm_type_builtin(arena, "int");
    const CBMType* t_size_t = pmm_type_builtin(arena, "size_t");
    const CBMType* t_long = pmm_type_builtin(arena, "long");
    const CBMType* t_char = pmm_type_builtin(arena, "char");
    const CBMType* t_char_ptr = pmm_type_pointer(arena, pmm_type_builtin(arena, "const char"));
    ...
    const CBMType* t_string = pmm_type_named(arena, string_qn);
    const CBMType* t_string_ref = pmm_type_reference(arena, t_string);
    ...
    reg_method(reg, arena, string_qn, "front", pmm_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "back", pmm_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "at", pmm_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "operator[]", pmm_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "begin", pmm_type_named(arena, "std.string.iterator"));
    reg_method(reg, arena, string_qn, "end", pmm_type_named(arena, "std.string.iterator"));
    ...
    reg_method(reg, arena, sv_qn, "substr", pmm_type_named(arena, sv_qn));
    ...
    const CBMType* t_T = pmm_type_type_param(arena, "T");
    const CBMType* t_T_ref = pmm_type_reference(arena, t_T);
    const CBMType* t_T_ptr = pmm_type_pointer(arena, t_T);
    ...
    reg_method(reg, arena, vec_qn, "begin", pmm_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "end", pmm_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "erase", pmm_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "insert", pmm_type_named(arena, "std.vector.iterator"));
    ...
    const CBMType* t_V = pmm_type_type_param(arena, "V");
    const CBMType* t_V_ref = pmm_type_reference(arena, t_V);

    reg_method(reg, arena, map_qn, "operator[]", t_V_ref);
    reg_method(reg, arena, map_qn, "at", t_V_ref);
    reg_method(reg, arena, map_qn, "find", pmm_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "insert", pmm_type_named(arena, "std.pair"));
    reg_method(reg, arena, map_qn, "emplace", pmm_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, map_qn, "begin", pmm_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "end", pmm_type_named(arena, "std.map.iterator"));
    ...
    reg_method(reg, arena, umap_qn, "find", pmm_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "insert", pmm_type_named(arena, "std.pair"));
    reg_method(reg, arena, umap_qn, "emplace", pmm_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, umap_qn, "begin", pmm_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "end", pmm_type_named(arena, "std.unordered_map.iterator"));
    ...
    reg_method(reg, arena, set_qn, "insert", pmm_type_named(arena, "std.pair"));
    reg_method(reg, arena, set_qn, "find", pmm_type_named(arena, "std.set.iterator"));
    ...
    reg_method(reg, arena, set_qn, "begin", pmm_type_named(arena, "std.set.iterator"));
    reg_method(reg, arena, set_qn, "end", pmm_type_named(arena, "std.set.iterator"));

    const char* uset_qn = "std.unordered_set";
    reg_type(reg, uset_qn, "unordered_set");
    reg_method(reg, arena, uset_qn, "insert", pmm_type_named(arena, "std.pair"));
    reg_method(reg, arena, uset_qn, "find", pmm_type_named(arena, "std.unordered_set.iterator"));
    ...
    reg_method(reg, arena, wptr_qn, "lock", pmm_type_named(arena, sptr_qn));
    ...
    reg_func(reg, arena, "std.make_unique", "make_unique",
        pmm_type_named(arena, uptr_qn));
    reg_func(reg, arena, "std.make_shared", "make_shared",
        pmm_type_named(arena, sptr_qn));
    ...
    const char* pair_qn = "std.pair";
    {
        ...
        const CBMType** pair_field_types = (const CBMType**)pmm_arena_alloc(arena,
            3 * sizeof(const CBMType*));
        pair_field_types[0] = pmm_type_type_param(arena, "T1");
        pair_field_types[1] = pmm_type_type_param(arena, "T2");
    ...
    reg_func(reg, arena, "std.get", "get", pmm_type_unknown());
    ...
    reg_method(reg, arena, "std.function", "operator()", pmm_type_unknown());
    ...
    const CBMType* t_ostream_ref = pmm_type_reference(arena, pmm_type_named(arena, os_qn));
    ...
    const CBMType* t_istream_ref = pmm_type_reference(arena, pmm_type_named(arena, is_qn));
    ...
    reg_func(reg, arena, "std.cout", "cout", pmm_type_named(arena, os_qn));
    reg_func(reg, arena, "std.cin", "cin", pmm_type_named(arena, is_qn));
    reg_func(reg, arena, "std.cerr", "cerr", pmm_type_named(arena, os_qn));
    reg_func(reg, arena, "std.clog", "clog", pmm_type_named(arena, os_qn));
    ...
    const CBMType* t_path = pmm_type_named(arena, fspath_qn);
    ...
    reg_method(reg, arena, fspath_qn, "operator/=", pmm_type_reference(arena, t_path));
    ...
    reg_method(reg, arena, thread_qn, "get_id", pmm_type_named(arena, "std.thread.id"));
    ...
    reg_func(reg, arena, "std.chrono.system_clock.now", "now",
        pmm_type_named(arena, "std.chrono.time_point"));
    ...
    reg_func(reg, arena, "std.find", "find", pmm_type_unknown());
    reg_func(reg, arena, "std.find_if", "find_if", pmm_type_unknown());
    reg_func(reg, arena, "std.for_each", "for_each", pmm_type_unknown());
    reg_func(reg, arena, "std.transform", "transform", pmm_type_unknown());
    reg_func(reg, arena, "std.copy", "copy", pmm_type_unknown());
    reg_func(reg, arena, "std.move", "move", pmm_type_unknown());
    reg_func(reg, arena, "std.swap", "swap", t_void);
    reg_func(reg, arena, "std.min", "min", pmm_type_unknown());
    reg_func(reg, arena, "std.max", "max", pmm_type_unknown());
    reg_func(reg, arena, "std.accumulate", "accumulate", pmm_type_unknown());
    reg_func(reg, arena, "std.count", "count", t_size_t);
    reg_func(reg, arena, "std.count_if", "count_if", t_size_t);
    reg_func(reg, arena, "std.remove", "remove", pmm_type_unknown());
    reg_func(reg, arena, "std.remove_if", "remove_if", pmm_type_unknown());
    reg_func(reg, arena, "std.reverse", "reverse", t_void);
    reg_func(reg, arena, "std.unique", "unique", pmm_type_unknown());
    reg_func(reg, arena, "std.lower_bound", "lower_bound", pmm_type_unknown());
    reg_func(reg, arena, "std.upper_bound", "upper_bound", pmm_type_unknown());
    ...
    reg_func(reg, arena, "std.stof", "stof", pmm_type_builtin(arena, "float"));
    reg_func(reg, arena, "std.stod", "stod", pmm_type_builtin(arena, "double"));
    ...
    reg_method(reg, arena, "std.any", "type", pmm_type_unknown()); // type_info
    reg_method(reg, arena, "std.any", "reset", t_void);
    reg_func(reg, arena, "std.any_cast", "any_cast", t_T);
    reg_func(reg, arena, "std.make_any", "make_any", pmm_type_named(arena, "std.any"));
    ...
    reg_method(reg, arena, "std.smatch", "prefix", pmm_type_unknown());
    reg_method(reg, arena, "std.smatch", "suffix", pmm_type_unknown());
    ...
    reg_func(reg, arena, "std.partial_sum", "partial_sum", pmm_type_unknown());
    reg_func(reg, arena, "std.adjacent_difference", "adjacent_difference", pmm_type_unknown());
    ...
    reg_func(reg, arena, "std.distance", "distance", pmm_type_builtin(arena, "ptrdiff_t"));
    reg_func(reg, arena, "std.next", "next", pmm_type_unknown()); // iterator
    reg_func(reg, arena, "std.prev", "prev", pmm_type_unknown());
    reg_func(reg, arena, "std.begin", "begin", pmm_type_unknown());
    reg_func(reg, arena, "std.end", "end", pmm_type_unknown());
    reg_func(reg, arena, "std.rbegin", "rbegin", pmm_type_unknown());
    reg_func(reg, arena, "std.rend", "rend", pmm_type_unknown());
    ...
    reg_func(reg, arena, "std.min_element", "min_element", pmm_type_unknown());
    reg_func(reg, arena, "std.max_element", "max_element", pmm_type_unknown());
    ...
    reg_func(reg, arena, "std.partition", "partition", pmm_type_unknown());
    reg_func(reg, arena, "std.stable_partition", "stable_partition", pmm_type_unknown());
    ...
    reg_func(reg, arena, "std.merge", "merge", pmm_type_unknown());
    reg_func(reg, arena, "std.includes", "includes", t_bool);
    reg_func(reg, arena, "std.set_union", "set_union", pmm_type_unknown());
    reg_func(reg, arena, "std.set_intersection", "set_intersection", pmm_type_unknown());
    reg_func(reg, arena, "std.set_difference", "set_difference", pmm_type_unknown());
    reg_func(reg, arena, "std.generate", "generate", t_void);
    reg_func(reg, arena, "std.generate_n", "generate_n", pmm_type_unknown());
    ...
    reg_method(reg, arena, "std.bitset", "set", pmm_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "reset", pmm_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "flip", pmm_type_named(arena, "std.bitset"));
    ...
    reg_method(reg, arena, "std.bitset", "to_ulong", pmm_type_builtin(arena, "unsigned long"));
    ...
    reg_func(reg, arena, "std.filesystem.file_size", "file_size", pmm_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.remove_all", "remove_all", pmm_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.current_path", "current_path",
        pmm_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "path", pmm_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "file_size", pmm_type_builtin(arena, "uintmax_t"));

    // =========================================================================
    // std.chrono — additional
    // =========================================================================
    reg_type(reg, "std.chrono.time_point", "time_point");
    reg_method(reg, arena, "std.chrono.time_point", "time_since_epoch", pmm_type_unknown());
    reg_method(reg, arena, "std.chrono.system_clock", "now",
        pmm_type_named(arena, "std.chrono.time_point"));
    reg_method(reg, arena, "std.chrono.steady_clock", "now",
        pmm_type_named(arena, "std.chrono.time_point"));
    reg_func(reg, arena, "std.chrono.duration_cast", "duration_cast", pmm_type_unknown());
    ...
    reg_method(reg, arena, bfspath_qn, "parent_path", pmm_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "filename", pmm_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "extension", pmm_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "stem", pmm_type_named(arena, bfspath_qn));
    ...
    reg_method(reg, arena, absv_qn, "data", pmm_type_pointer(arena, pmm_type_builtin(arena, "char")));
    ...
    reg_method(reg, arena, absv_qn, "substr", pmm_type_named(arena, absv_qn));

    const char* abstat_qn = "absl.Status";
    reg_type(reg, abstat_qn, "Status");
    reg_method(reg, arena, abstat_qn, "ok", t_bool);
    reg_method(reg, arena, abstat_qn, "code", pmm_type_unknown());
    reg_method(reg, arena, abstat_qn, "message", pmm_type_named(arena, absv_qn));
    reg_method(reg, arena, abstat_qn, "ToString", t_string);
    reg_func(reg, arena, "absl.OkStatus", "OkStatus", pmm_type_named(arena, abstat_qn));

    const char* absor_qn = "absl.StatusOr";
    reg_type(reg, absor_qn, "StatusOr");
    reg_method(reg, arena, absor_qn, "ok", t_bool);
    reg_method(reg, arena, absor_qn, "status", pmm_type_named(arena, abstat_qn));
    ...
    reg_method(reg, arena, abfhm_qn, "find", pmm_type_unknown());
    ...
    reg_method(reg, arena, abfhm_qn, "insert", pmm_type_unknown());
    ...
    reg_method(reg, arena, abfhs_qn, "insert", pmm_type_unknown());
    ...
    reg_method(reg, arena, abspan_qn, "subspan", pmm_type_named(arena, abspan_qn));
    ...
    reg_func(reg, arena, "absl.StrSplit", "StrSplit",
        pmm_type_named(arena, "std.vector"));
    ...
    reg_method(reg, arena, grpc_status_qn, "error_code", pmm_type_unknown());
    reg_method(reg, arena, grpc_status_qn, "error_message", t_string);
    reg_func(reg, arena, "grpc.Status.OK", "OK", pmm_type_named(arena, grpc_status_qn));

    const char* grpc_sb_qn = "grpc.ServerBuilder";
    reg_type(reg, grpc_sb_qn, "ServerBuilder");
    reg_method(reg, arena, grpc_sb_qn, "AddListeningPort", pmm_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "RegisterService", pmm_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "BuildAndStart", pmm_type_unknown());

    const char* grpc_chan_qn = "grpc.Channel";
    reg_type(reg, grpc_chan_qn, "Channel");
    reg_func(reg, arena, "grpc.CreateChannel", "CreateChannel",
        pmm_type_named(arena, grpc_chan_qn));
    ...
    reg_func(reg, arena, "spdlog.get", "get",
        pmm_type_named(arena, spdlog_qn));
    ...
    reg_method(reg, arena, qobj_qn, "parent", pmm_type_pointer(arena, pmm_type_named(arena, qobj_qn)));
    reg_method(reg, arena, qobj_qn, "children", pmm_type_unknown());
    reg_method(reg, arena, qobj_qn, "objectName", pmm_type_named(arena, "QString"));
    ...
    reg_method(reg, arena, qstr_qn, "toUtf8", pmm_type_unknown());
    reg_method(reg, arena, qstr_qn, "toLatin1", pmm_type_unknown());
    ...
    reg_method(reg, arena, qstr_qn, "trimmed", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "simplified", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toLower", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toUpper", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "arg", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "split", pmm_type_unknown());
    reg_method(reg, arena, qstr_qn, "replace", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "mid", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "left", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "right", pmm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toInt", t_int);
    reg_method(reg, arena, qstr_qn, "toDouble", pmm_type_builtin(arena, "double"));
    reg_func(reg, arena, "QString.number", "number", pmm_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromStdString", "fromStdString", pmm_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromUtf8", "fromUtf8", pmm_type_named(arena, qstr_qn));
