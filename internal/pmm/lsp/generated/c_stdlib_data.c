...
#define REG_FUNC(qn, short, ret_type) do { \
    ...
    rf.short_name = (short); \
    rf.signature = pmm_type_func(arena, NULL, NULL, (const CBMType*[]){(ret_type), NULL}); \
...
void pmm_c_stdlib_register(CBMTypeRegistry* reg, CBMArena* arena) {
    CBMRegisteredFunc rf;
    CBMRegisteredType rt;

    const CBMType* t_int = pmm_type_builtin(arena, "int");
    const CBMType* t_size_t = pmm_type_builtin(arena, "size_t");
    const CBMType* t_double = pmm_type_builtin(arena, "double");
    const CBMType* t_void = pmm_type_builtin(arena, "void");
    const CBMType* t_char_ptr = pmm_type_pointer(arena, pmm_type_builtin(arena, "char"));
    const CBMType* t_void_ptr = pmm_type_pointer(arena, t_void);

    // FILE type
    REG_TYPE("FILE", "FILE");
    const CBMType* t_file_ptr = pmm_type_pointer(arena, pmm_type_named(arena, "FILE"));
    ...
    REG_FUNC("ftell", "ftell", pmm_type_builtin(arena, "long"));
    ...
    REG_FUNC("atol", "atol", pmm_type_builtin(arena, "long"));
    REG_FUNC("strtol", "strtol", pmm_type_builtin(arena, "long"));
    ...
    REG_FUNC("time", "time", pmm_type_builtin(arena, "time_t"));
    REG_FUNC("clock", "clock", pmm_type_builtin(arena, "clock_t"));
