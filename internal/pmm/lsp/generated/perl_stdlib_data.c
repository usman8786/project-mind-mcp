 ...
 *
 * Return types are left UNKNOWN (pmm_type_unknown) for v1: real signature
...
#define REG_FUNC(module_dot_, name_, ret_type_)                                                 \
    do {                                                                                        \
        memset(&rf, 0, sizeof(rf));                                                             \
        rf.min_params = -1;                                                                     \
        rf.qualified_name = pmm_arena_sprintf(arena, "%s.%s", (module_dot_), (name_));          \
        rf.short_name = (name_);                                                                \
        {                                                                                       \
            const CBMType **rets = (const CBMType **)pmm_arena_alloc(arena, 2 * sizeof(*rets)); \
            rets[0] = (ret_type_);                                                              \
...
void pmm_perl_stdlib_register(CBMTypeRegistry *reg, CBMArena *arena) {
    ...
    REG_BUILTIN("sprintf", pmm_type_builtin(arena, "string"));
    REG_BUILTIN("open", MIXED);
    REG_BUILTIN("close", MIXED);
    REG_BUILTIN("push", pmm_type_builtin(arena, "int"));
    REG_BUILTIN("pop", MIXED);
    REG_BUILTIN("shift", MIXED);
    REG_BUILTIN("unshift", pmm_type_builtin(arena, "int"));
    REG_BUILTIN("map", MIXED);
    REG_BUILTIN("grep", MIXED);
    REG_BUILTIN("sort", MIXED);
    REG_BUILTIN("join", pmm_type_builtin(arena, "string"));
    REG_BUILTIN("split", MIXED);
    REG_BUILTIN("length", pmm_type_builtin(arena, "int"));
    REG_BUILTIN("substr", pmm_type_builtin(arena, "string"));
    REG_BUILTIN("chomp", MIXED);
    REG_BUILTIN("chop", MIXED);
    REG_BUILTIN("die", MIXED);
    REG_BUILTIN("warn", MIXED);
    REG_BUILTIN("ref", pmm_type_builtin(arena, "string"));
    REG_BUILTIN("bless", MIXED);
    REG_BUILTIN("defined", pmm_type_builtin(arena, "bool"));
    REG_BUILTIN("exists", pmm_type_builtin(arena, "bool"));
    ...
    REG_FUNC("Scalar.Util", "reftype", pmm_type_builtin(arena, "string"));
    ...
    REG_FUNC("POSIX", "strftime", pmm_type_builtin(arena, "string"));
    REG_FUNC("POSIX", "INT_MAX", pmm_type_builtin(arena, "int"));

    /* ── Storable ───────────────────────────────────────────────────
     * Source: RESEARCH.md L367. Module QN "Storable". */
    REG_FUNC("Storable", "dclone", MIXED);
    REG_FUNC("Storable", "freeze", pmm_type_builtin(arena, "string"));
    ...
    REG_FUNC("Data.Dumper", "Dumper", pmm_type_builtin(arena, "string"));
}
