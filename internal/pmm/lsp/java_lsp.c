 ...
 *     compatibility (pmm_registry_lookup_method_by_types). Ambiguity ⇒
 ...
 *
 * Confidence policy (consumed by lsp_resolve.h with PMM_LSP_CONFIDENCE_FLOOR
 * = 0.6):
...
static const char *strip_generics(CBMArena *a, const char *type_text) {
    ...
    if (!lt)
        return type_text;
    return pmm_arena_strndup(a, type_text, (size_t)(lt - type_text));
}
...
static const char *unwrap_array_text(CBMArena *a, const char *type_text, int *out_dim) {
    ...
    if (dim == 0)
        return type_text;
    return pmm_arena_strndup(a, type_text, n);
}
...
static const CBMType *box_primitive(CBMArena *a, const char *prim) {
    ...
    for (size_t i = 0; i < sizeof(map) / sizeof(map[0]); i++) {
        if (strcmp(prim, map[i][0]) == 0)
            return pmm_type_named(a, map[i][1]);
    }
    return pmm_type_unknown();
}
...
void java_lsp_add_import(JavaLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->import_count >= ctx->import_cap) {
        ...
        const char **new_names =
            (const char **)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_names));
        const char **new_qns =
            (const char **)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_qns));
        int *new_kinds = (int *)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_kinds));
    ...
    ctx->import_target_qns[ctx->import_count] = pmm_arena_strdup(ctx->arena, target_qn);
...
static void push_enclosing_class(JavaLSPContext *ctx, const char *class_qn) {
    if (ctx->enclosing_class_depth >= ctx->enclosing_class_cap) {
        ...
        const char **arr =
            (const char **)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*arr));
        if (!arr)
            return;
...
const CBMType *java_parse_type_node(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "scoped_type_identifier") == 0) {
        ...
        if (pmm_registry_lookup_type(ctx->registry, full)) {
            return pmm_type_named(ctx->arena, full);
        }
        /* Otherwise resolve the head segment and append the rest. */
        char *dot = strchr(full, '.');
        if (dot) {
            char *head = pmm_arena_strndup(ctx->arena, full, (size_t)(dot - full));
            const char *head_qn = java_resolve_type_name(ctx, head);
            if (head_qn) {
                return pmm_type_named(ctx->arena,
                                      pmm_arena_sprintf(ctx->arena, "%s%s", head_qn, dot));
            }
        }
        return pmm_type_named(ctx->arena, full);
    }
    ...
    if (strcmp(kind, "generic_type") == 0) {
        ...
        if (arg_count == 0) {
            arg_buf[arg_count++] = pmm_type_unknown();
        }
        const CBMType **args =
            (const CBMType **)pmm_arena_alloc(ctx->arena, (size_t)(arg_count + 1) * sizeof(*args));
        for (int i = 0; i < arg_count; i++)
            args[i] = arg_buf[i];
        args[arg_count] = NULL;
        return pmm_type_template(ctx->arena, base_qn, args, arg_count);
    }
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        return pmm_type_slice(ctx->arena, java_parse_type_node(ctx, elem));
    }
    ...
    if (!txt)
        return pmm_type_unknown();
    return pmm_type_named(ctx->arena, txt);
}
...
const char *java_resolve_type_name(JavaLSPContext *ctx, const char *name) {
    ...
    for (int i = ctx->enclosing_class_depth - 1; i >= 0; i--) {
        ...
        if (n > 0 && (size_t)n < sizeof(buf)) {
            if (pmm_registry_lookup_type(ctx->registry, buf)) {
                return pmm_arena_strdup(ctx->arena, buf);
            }
        }
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != PMM_JAVA_IMPORT_TYPE)
            continue;
    ...
    /* 4. On-demand imports. */
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != PMM_JAVA_IMPORT_ON_DEMAND)
            continue;
        char buf[JAVA_LSP_BUF];
    ...
    if (pmm_registry_lookup_type(ctx->registry, name)) {
        return pmm_arena_strdup(ctx->arena, name);
    }
    ...
    if (ctx->registry && ctx->registry->types) {
        ...
        if (matches == 1 && only_qn) {
            return pmm_arena_strdup(ctx->arena, only_qn);
        }
    }
...
const CBMType *java_eval_expr_type(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "decimal_integer_literal") == 0 || strcmp(kind, "hex_integer_literal") == 0 ||
        ...
        if (txt) {
            ...
            if (len > 0 && (txt[len - 1] == 'l' || txt[len - 1] == 'L')) {
                result = pmm_type_builtin(ctx->arena, "long");
                goto out;
            }
        }
    ...
    if (strcmp(kind, "decimal_floating_point_literal") == 0 ||
        ...
        result = pmm_type_builtin(ctx->arena, "double");
        goto out;
    }
    ...
    if (strcmp(kind, "character_literal") == 0) {
        result = pmm_type_builtin(ctx->arena, "char");
        goto out;
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "string_fragment") == 0) {
        result = pmm_type_named(ctx->arena, "java.lang.String");
        goto out;
    }
    ...
    if (strcmp(kind, "super") == 0) {
        if (ctx->enclosing_super_qn) {
            result = pmm_type_named(ctx->arena, ctx->enclosing_super_qn);
        } else {
            result = pmm_type_named(ctx->arena, "java.lang.Object");
        }
        goto out;
    }
    ...
    if (strcmp(kind, "class_literal") == 0) {
        result = pmm_type_named(ctx->arena, "java.lang.Class");
        goto out;
    }
    ...
    if (strcmp(kind, "array_creation_expression") == 0) {
        ...
        result = pmm_type_slice(ctx->arena, elem);
        goto out;
    }
...
static const CBMType *resolve_identifier_type(JavaLSPContext *ctx, const char *name) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != PMM_JAVA_IMPORT_STATIC)
            continue;
        if (strcmp(ctx->import_local_names[i], name) == 0) {
            ...
            if (last_dot) {
                char *cls = pmm_arena_strndup(ctx->arena, target, (size_t)(last_dot - target));
...
static const CBMType *eval_field_access(JavaLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(obj) && strcmp(ts_node_type(obj), "identifier") == 0) {
        ...
        if (oname && strcmp(oname, "System") == 0) {
            if (strcmp(fname, "out") == 0 || strcmp(fname, "err") == 0) {
                return pmm_type_named(ctx->arena, "java.io.PrintStream");
            }
            if (strcmp(fname, "in") == 0) {
                return pmm_type_named(ctx->arena, "java.io.InputStream");
            }
        }
    }

    /* Special-case: `length` on array types is `int`. */
    const CBMType *recv = ts_node_is_null(obj) ? pmm_type_unknown() : java_eval_expr_type(ctx, obj);
    if (recv && recv->kind == PMM_TYPE_SLICE && strcmp(fname, "length") == 0) {
    ...
    if (!ts_node_is_null(obj) && strcmp(ts_node_type(obj), "this") == 0) {
        const CBMType *scope_t = pmm_scope_lookup(ctx->current_scope, fname);
        if (scope_t && !pmm_type_is_unknown(scope_t))
            return scope_t;
    }

    const CBMType *res = resolve_member_type(ctx, recv, fname);
    if (res && !pmm_type_is_unknown(res))
        return res;
...
const CBMType *java_lookup_field_type(JavaLSPContext *ctx, const char *class_qn,
    ...
    for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur; hops++) {
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, cur);
        if (!rt)
            break;
...
const CBMRegisteredFunc *java_lookup_method(JavaLSPContext *ctx, const char *class_qn,
    ...
    for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur; hops++) {
        ...
        const CBMRegisteredFunc *m =
            pmm_registry_lookup_method_by_args(ctx->registry, cur, method_name, arg_count);
        if (m)
            return m;
        /* Otherwise capture any name match as fallback. */
        if (!fallback) {
            fallback = pmm_registry_lookup_method(ctx->registry, cur, method_name);
        }
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, cur);
        if (!rt)
            break;
...
static const CBMType *substitute_generic_return(JavaLSPContext *ctx, const char *recv_qn,
    ...
    if (fallback->kind != PMM_TYPE_NAMED)
        return fallback;
    ...
    if (strcmp(ts_node_type(obj), "identifier") == 0) {
        ...
        if (oname) {
            ...
            if (cls_qn) {
                ...
                if (pmm_type_is_unknown(pmm_scope_lookup(ctx->current_scope, oname))) {
...
static const CBMType *eval_binary(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(op, "+") == 0 && is_string_concat(ctx, node)) {
        return pmm_type_named(ctx->arena, "java.lang.String");
    }
...
void java_process_statement(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "local_variable_declaration") == 0) {
        ...
        if (rn) {
            const CBMType *rt = pmm_type_unknown();
            bool is_var = false;
            ...
            if ((is_var || pmm_type_is_unknown(rt)) && !ts_node_is_null(rvalue)) {
                rt = java_eval_expr_type(ctx, rvalue);
            }
            pmm_scope_bind(ctx->current_scope, rn, rt ? rt : pmm_type_unknown());
        }
        ...
        if (!ts_node_is_null(formal)) {
            ...
            if (pn) {
                ...
                pmm_scope_bind(ctx->current_scope, pn, pt);
            }
        }
    }
...
static void process_local_var_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    const CBMType *static_type =
        ts_node_is_null(type_node) ? pmm_type_unknown() : java_parse_type_node(ctx, type_node);
    bool is_var =
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if ((is_var || pmm_type_is_unknown(bind_type)) && !ts_node_is_null(value_node)) {
            bind_type = java_eval_expr_type(ctx, value_node);
        }
        pmm_scope_bind(ctx->current_scope, vname, bind_type ? bind_type : pmm_type_unknown());
    }
}
...
static void process_enhanced_for(JavaLSPContext *ctx, TSNode node) {
    ...
    if (pmm_type_is_unknown(t) && !ts_node_is_null(value_node)) {
        const CBMType *iter_t = java_eval_expr_type(ctx, value_node);
        if (iter_t && iter_t->kind == PMM_TYPE_SLICE) {
            t = iter_t->data.slice.elem;
        } else if (iter_t && iter_t->kind == PMM_TYPE_TEMPLATE &&
    ...
    pmm_scope_bind(ctx->current_scope, vname, t ? t : pmm_type_unknown());
}
...
static void process_field_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!fname)
            continue;
        pmm_scope_bind(ctx->current_scope, fname, static_type);
    }
}
...
static void process_method_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
    ...
    if (!mname)
        return;
    char *method_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mname);
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            if (!pn)
                continue;
            const CBMType *pt =
                ts_node_is_null(ptype) ? pmm_type_unknown() : java_parse_type_node(ctx, ptype);
            if (strcmp(pk, "spread_parameter") == 0 && pt) {
                pt = pmm_type_slice(ctx->arena, pt);
            }
            pmm_scope_bind(ctx->current_scope, pn, pt);
        }
    }
...
static void process_constructor_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
    ...
    char *ctor_qn;
    if (cname) {
        ctor_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, cname);
    } else {
        ctor_qn = pmm_arena_sprintf(ctx->arena, "%s.<init>", class_qn);
    }
...
static void java_process_class_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    char *class_qn;
    if (ctx->enclosing_class_qn) {
        class_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, cname);
    } else if (ctx->module_qn && ctx->module_qn[0]) {
        class_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
        class_qn = pmm_arena_strdup(ctx->arena, cname);
    }
...
void java_lsp_process_file(JavaLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(k, "package_declaration") == 0) {
            ...
            if (path) {
                if (is_on_demand) {
                    int kind = is_static ? PMM_JAVA_IMPORT_STATIC_OD : PMM_JAVA_IMPORT_ON_DEMAND;
                    ...
                    java_lsp_add_import(ctx, local, path, PMM_JAVA_IMPORT_STATIC);
                } else {
                    const char *last_dot = strrchr(path, '.');
                    const char *local = last_dot ? last_dot + 1 : path;
                    java_lsp_add_import(ctx, local, path, PMM_JAVA_IMPORT_TYPE);
                }
            }
        }
    }
...
static const char *java_find_sole_impl(JavaLSPContext *ctx, const char *iface_qn, const char *mname,
    ...
    for (int ti = 0; ti < ctx->registry->type_count && distinct < 2; ti++) {
         ...
         * name array. */
        bool has = pmm_registry_lookup_method(ctx->registry, cand->qualified_name, mname) != NULL;
        ...
        for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur && !subtype; hops++) {
            const CBMRegisteredType *ct = pmm_registry_lookup_type(ctx->registry, cur);
...
static bool java_emit_interface_resolution(JavaLSPContext *ctx, const char *iface_qn,
    ...
    if (impl_count == 1 && sole_impl) {
        const CBMRegisteredFunc *cf = pmm_registry_lookup_method(ctx->registry, sole_impl, mname);
        const char *target =
            cf ? cf->qualified_name : pmm_arena_sprintf(ctx->arena, "%s.%s", sole_impl, mname);
        java_emit_resolved(ctx, target, "lsp_interface_resolve", 0.85f);
        return true;
    }
    if (impl_count >= 2) {
        java_emit_resolved(ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", iface_qn, mname),
...
static void resolve_method_call(JavaLSPContext *ctx, TSNode call) {
    ...
    if (strcmp(ts_node_type(obj), "identifier") == 0) {
        char *oname = java_node_text(ctx, obj);
        if (oname && pmm_type_is_unknown(pmm_scope_lookup(ctx->current_scope, oname))) {
            const char *cls_qn = java_resolve_type_name(ctx, oname);
            if (cls_qn) {
                const CBMRegisteredFunc *f = java_lookup_method(ctx, cls_qn, mname, arity);
                if (f) {
    ...
    if (recv_qn) {
        ...
        if (f) {
            ...
            const CBMRegisteredType *rt0 = pmm_registry_lookup_type(ctx->registry, recv_qn);
        ...
        if (rt && rt->is_interface) {
            ...
            java_emit_resolved(ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", recv_qn, mname),
                               "lsp_interface_dispatch", 0.80f);
            return;
        }
        java_emit_unresolved(ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", recv_qn, mname),
                             "no_method_match");
        return;
    }
 ...
 * `fi_qn` parameterized with `targs`. Returns pmm_type_unknown() when the
...
static const CBMType *sam_param_type(CBMArena *a, const char *fi_qn, int param_idx,
    ...
    for (int i = 0; single_t[i]; i++) {
        if (strcmp(single_t[i], fi_qn) == 0 && param_idx == 0 && targ_count >= 1) {
            return targs[0] ? targs[0] : pmm_type_unknown();
        }
    }
    ...
    if (strcmp(fi_qn, "java.util.function.BiFunction") == 0 ||
        ...
        if (param_idx <= 1 && targ_count > param_idx) {
            return targs[param_idx] ? targs[param_idx] : pmm_type_unknown();
        }
    }
...
static const CBMType *propagate_template(CBMArena *a, const char *recv_qn, const char *method_name,
                                         const CBMType *const *recv_targs, int recv_targ_count,
                                         const CBMType *return_t) {
    if (!return_t || return_t->kind != PMM_TYPE_NAMED)
        return return_t;
    ...
    if (is_t0) {
        const CBMType **args = (const CBMType **)pmm_arena_alloc(a, 2 * sizeof(*args));
        if (!args)
            return return_t;
        args[0] = recv_targs[0];
        args[1] = NULL;
        return pmm_type_template(a, ret_qn, args, 1);
    }
    ...
    if (is_map_like(recv_qn) && recv_targ_count >= 2) {
        ...
        if (strcmp(method_name, "keySet") == 0) {
            args[0] = recv_targs[0];
            args[1] = NULL;
            return pmm_type_template(a, "java.util.Set", args, 1);
        }
        if (strcmp(method_name, "values") == 0) {
            args[0] = recv_targs[1];
            args[1] = NULL;
            return pmm_type_template(a, "java.util.Collection", args, 1);
        }
    }
    return return_t;
}
...
static uint32_t bind_lambda_args(JavaLSPContext *ctx, TSNode call_node,
    ...
    if (resolved && resolved->signature && resolved->signature->kind == PMM_TYPE_FUNC) {
    ...
    } else if (recv_type && recv_type->kind == PMM_TYPE_NAMED) {
    ...
    for (uint32_t i = 0; i < n && i < 32; i++) {
                ...
                if (!ts_node_is_null(body_node)) {
                    ...
                    if (!ts_node_is_null(params_node)) {
                        ...
                        if (strcmp(pn_kind, "identifier") == 0) {
                            ...
                            if (pname)
                                pmm_scope_bind(ctx->current_scope, pname, h_p0);
        ...
        int fi_targ_count = 0;
        if (expected->kind == PMM_TYPE_NAMED) {
            fi_qn = expected->data.named.qualified_name;
        } else if (expected->kind == PMM_TYPE_TEMPLATE) {
        ...
        if (!ts_node_is_null(params_node)) {
            ...
            if (strcmp(pn_kind, "identifier") == 0) {
                ...
                for (uint32_t pi = 0; pi < pc; pi++) {
                    ...
                    if (pname) {
                        pmm_scope_bind(ctx->current_scope, pname, pt ? pt : pmm_type_unknown());
                        idx++;
                    }
                }
            }
...
static void resolve_method_reference(JavaLSPContext *ctx, TSNode mref,
    ...
    if (outer_resolved && outer_resolved->signature &&
        outer_resolved->signature->kind == PMM_TYPE_FUNC &&
    ...
    if (lhs_is_type) {
        ...
        if (lhs_t && lhs_t->kind == PMM_TYPE_NAMED)
            type_qn = lhs_t->data.named.qualified_name;
        else if (lhs_t && lhs_t->kind == PMM_TYPE_TEMPLATE)
    ...
    if (strcmp(mname, "new") == 0) {
        ...
        const CBMRegisteredFunc *cf =
            pmm_registry_lookup_method(ctx->registry, type_qn, short_name);
    ...
    if (m) {
        ...
        java_emit_unresolved(ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", type_qn, mname),
...
static const CBMRegisteredFunc *lookup_method_for_call(JavaLSPContext *ctx, TSNode call,
    ...
    if (recv && recv->kind == PMM_TYPE_NAMED)
        recv_qn = recv->data.named.qualified_name;
    else if (recv && recv->kind == PMM_TYPE_TEMPLATE)
        recv_qn = recv->data.template_type.template_name;
    if (recv_qn)
...
static void java_resolve_calls_in_node_inner(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "method_invocation") == 0) {
        ...
        if (!ts_node_is_null(type_node)) {
            ...
            else if (t && t->kind == PMM_TYPE_TEMPLATE)
            ...
            if (qn) {
                ...
                const CBMRegisteredFunc *cf =
                    pmm_registry_lookup_method_by_args(ctx->registry, qn, short_name, arity);
                if (!cf)
                    cf = pmm_registry_lookup_method(ctx->registry, qn, short_name);
                if (cf) {
...
static const char **split_generic_args(CBMArena *a, const char *inside, int *out_count) {
    *out_count = 0;
    ...
    for (const char *p = inside; *p; p++) {
        if (*p == '<')
            ...
            if (count < 16 && seg_end > seg_start) {
                args[count++] = pmm_arena_strndup(a, seg_start, (size_t)(seg_end - seg_start));
            }
            seg_start = p + 1;
        }
    }
    ...
    if (count == 0)
        return NULL;
    const char **result = (const char **)pmm_arena_alloc(a, (size_t)(count + 1) * sizeof(*result));
...
static const CBMType *parse_param_text_full(CBMArena *a, const char *text, const char *parent_class,
    ...
    if (no_arr) {
        ...
        if (lt) {
            ...
            if (gt && gt > lt + 1) {
                gen_args_inner = pmm_arena_strndup(a, lt + 1, (size_t)(gt - lt - 1));
            }
        }
    }
    ...
    if (no_gen && is_java_primitive(no_gen)) {
        base = pmm_type_builtin(a, no_gen);
    } else if (no_gen) {
        if (strchr(no_gen, '.')) {
            base = pmm_type_named(a, no_gen);
        } else {
            /* Try java.lang well-knowns. */
            for (int i = 0; JAVA_LANG_TYPES[i]; i++) {
                if (strcmp(JAVA_LANG_TYPES[i], no_gen) == 0) {
                    base = pmm_type_named(a, pmm_arena_sprintf(a, "java.lang.%s", no_gen));
                    break;
                }
            }
            ...
            if (!base && parent_class && reg) {
                ...
                while (cur) {
                    const char *cand = pmm_arena_sprintf(a, "%s.%s", cur, no_gen);
                    if (pmm_registry_lookup_type(reg, cand)) {
                        base = pmm_type_named(a, cand);
                        break;
                    }
                    const char *last_dot = strrchr(cur, '.');
                    if (!last_dot)
                        break;
                    cur = pmm_arena_strndup(a, cur, (size_t)(last_dot - cur));
                }
            }
            ...
            if (!base && reg) {
                for (int i = 0; JAVA_FALLBACK_PACKAGES[i]; i++) {
                    const char *cand =
                        pmm_arena_sprintf(a, "%s.%s", JAVA_FALLBACK_PACKAGES[i], no_gen);
                    if (pmm_registry_lookup_type(reg, cand)) {
                        base = pmm_type_named(a, cand);
                        break;
                    }
            ...
            if (!base) {
                if (module_qn && module_qn[0]) {
                    base = pmm_type_named(a, pmm_arena_sprintf(a, "%s.%s", module_qn, no_gen));
                } else {
                    base = pmm_type_named(a, no_gen);
                }
            }
...
static void register_local_func_or_type_from_file(JavaLSPContext *ctx, CBMTypeRegistry *reg,
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (pmm_registry_lookup_type(reg, d->qualified_name))
            continue;
        ...
        pmm_registry_add_type(reg, stub);
    }
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        const CBMRegisteredType *existing = pmm_registry_lookup_type(reg, d->qualified_name);
        if (!existing)
            continue;
        CBMRegisteredType rt = *existing; /* Will be re-added; lookup_type returns the
                                             registry's stored copy. We instead build a
                                             local copy and let pmm_registry_add_type's
        ...
        if (d->base_classes) {
            ...
            if (bc_count > 0) {
                const char **emb =
                    (const char **)pmm_arena_alloc(a, (size_t)(bc_count + 1) * sizeof(*emb));
                    ...
                    if (!strchr(no_gen, '.')) {
                        ...
                        for (int k = 0; JAVA_LANG_TYPES[k]; k++) {
                            if (strcmp(JAVA_LANG_TYPES[k], no_gen) == 0) {
                                resolved = pmm_arena_sprintf(a, "java.lang.%s", no_gen);
                                break;
                            }
                        }
                        ...
                        if (!parent_qn && d->qualified_name) {
                            ...
                            if (last_dot_qn) {
                                parent_qn =
                                    pmm_arena_strndup(a, d->qualified_name,
                        ...
                        if (!resolved && module_qn && module_qn[0]) {
                            resolved = pmm_arena_sprintf(a, "%s.%s", module_qn, no_gen);
                        }
                        emb[j] = resolved ? resolved : pmm_arena_strdup(a, no_gen);
                    } else {
                        emb[j] = pmm_arena_strdup(a, no_gen);
                    }
                }
                emb[bc_count] = NULL;
        ...
        if (!slot->embedded_types && !slot->is_interface) {
            const char **emb = (const char **)pmm_arena_alloc(a, 2 * sizeof(*emb));
            if (emb) {
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->param_types) {
            ...
            while (d->param_types[pc])
                pc++;
            if (pc > 0) {
                ptypes = (const CBMType **)pmm_arena_alloc(a, (size_t)(pc + 1) * sizeof(*ptypes));
                for (int j = 0; j < pc; j++) {
        ...
        if (d->return_type && d->return_type[0]) {
            rtypes = (const CBMType **)pmm_arena_alloc(a, 2 * sizeof(*rtypes));
        ...
        rf.signature = pmm_type_func(a, d->param_names, ptypes, rtypes);
        if (d->parent_class) {
...
static void patch_one_method(JavaLSPContext *ctx, CBMTypeRegistry *reg, TSNode method_node,
    ...
    const CBMType **ptypes =
        (const CBMType **)pmm_arena_alloc(ctx->arena, (size_t)(pn + 1) * sizeof(*ptypes));
    if (!ptypes)
        return;
    int idx = 0;
    ...
    if (!is_constructor) {
        ...
        if (!ts_node_is_null(rtype)) {
            rtypes = (const CBMType **)pmm_arena_alloc(ctx->arena, 2 * sizeof(*rtypes));
    ...
    if (is_constructor) {
        rtypes = (const CBMType **)pmm_arena_alloc(ctx->arena, 2 * sizeof(*rtypes));
        rtypes[0] = pmm_type_named(ctx->arena, class_qn);
        rtypes[1] = NULL;
    }

    /* Rebuild the FUNC type with both param-types and return-types. */
    slot->signature = pmm_type_func(ctx->arena, NULL, ptypes, rtypes);
}
...
static void patch_method_signatures_from_ast(JavaLSPContext *ctx, CBMTypeRegistry *reg,
    ...
    if (enclosing_class_qn) {
        class_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", enclosing_class_qn, cname);
    } else if (ctx->module_qn && ctx->module_qn[0]) {
        class_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
        class_qn = pmm_arena_strdup(ctx->arena, cname);
...
static void append_field_to_class(CBMTypeRegistry *reg, CBMArena *a, const char *class_qn,
    ...
    const char **new_names =
        (const char **)pmm_arena_alloc(a, (size_t)(existing + 2) * sizeof(*new_names));
    const CBMType **new_types =
        (const CBMType **)pmm_arena_alloc(a, (size_t)(existing + 2) * sizeof(*new_types));
    ...
    new_names[existing] = pmm_arena_strdup(a, field_name);
    new_types[existing] = ftype ? ftype : pmm_type_unknown();
...
static void populate_class_fields_from_ast(JavaLSPContext *ctx, CBMTypeRegistry *reg,
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ck, "field_declaration") == 0) {
            ...
            const CBMType *ftype =
                ts_node_is_null(tn) ? pmm_type_unknown() : java_parse_type_node(ctx, tn);
...
/* ── Top-level entry: pmm_run_java_lsp ────────────────────────────── */

extern const TSLanguage *tree_sitter_java(void);

void pmm_run_java_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
                      TSNode root) {
    ...
    pmm_java_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->imports.count; i++) {
        ...
        if (!imp->local_name || !imp->module_path)
            continue;
        java_lsp_add_import(&ctx, imp->local_name, imp->module_path, PMM_JAVA_IMPORT_TYPE);
    }
...
void pmm_run_java_lsp_cross(CBMArena *arena, const char *source, int source_len,
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0 ||
            ...
            if (d->embedded_types && d->embedded_types[0]) {
                ...
                for (const char *p = d->embedded_types; *p; p++)
                    if (*p == '|')
                        n++;
                const char **emb =
                    (const char **)pmm_arena_alloc(arena, (size_t)(n + 1) * sizeof(*emb));
                int idx = 0;
                ...
                while (*start) {
                    ...
                    if (end > start) {
                        emb[idx++] = pmm_arena_strndup(arena, start, (size_t)(end - start));
                    }
                    if (!*end)
                        break;
                    start = end + 1;
            ...
            if (d->return_types && d->return_types[0]) {
                ...
                const char *first;
                if (bar) {
                    first = pmm_arena_strndup(arena, no_pipe, (size_t)(bar - no_pipe));
                } else {
                    first = no_pipe;
                }
            ...
            const CBMType **rtypes = NULL;
            if (ret) {
                rtypes = (const CBMType **)pmm_arena_alloc(arena, 2 * sizeof(*rtypes));
                rtypes[0] = ret;
                rtypes[1] = NULL;
            }
            rf.signature = pmm_type_func(arena, NULL, NULL, rtypes);
            pmm_registry_add_func(&reg, rf);
        }
    }
    ...
    for (int i = 0; i < import_count; i++) {
        if (!import_names[i] || !import_qns[i])
            continue;
        java_lsp_add_import(&ctx, import_names[i], import_qns[i], PMM_JAVA_IMPORT_TYPE);
    }
...
void pmm_batch_java_lsp_cross(CBMArena *arena, CBMBatchJavaLSPFile *files, int file_count,
                              CBMResolvedCallArray *out) {
    if (!files || file_count <= 0)
        return;
    for (int i = 0; i < file_count; i++) {
        pmm_run_java_lsp_cross(arena, files[i].source, files[i].source_len, files[i].module_qn,
