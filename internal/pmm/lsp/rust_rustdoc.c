...
static const char* join_path(CBMArena* arena, yyjson_val* path_arr,
    ...
    return pmm_arena_strndup(arena, buf, (size_t)off);
}
...
static const CBMType* parse_rustdoc_type(yyjson_val* t, CBMArena* arena,
    const char* crate_qn) {
    if (!t || !yyjson_is_obj(t)) return pmm_type_unknown();

    yyjson_val* prim = yyjson_obj_get(t, "primitive");
    if (prim && yyjson_is_str(prim)) {
        return pmm_type_builtin(arena, yyjson_get_str(prim));
    }
    ...
    if (resolved && yyjson_is_obj(resolved)) {
        ...
        if (name && yyjson_is_str(name)) {
            qn = pmm_arena_strdup(arena, yyjson_get_str(name));
        }
        if (!qn) return pmm_type_unknown();
        ...
        if (args && yyjson_is_obj(args)) {
            ...
            if (angle && yyjson_is_obj(angle)) {
                ...
                if (arr && yyjson_is_arr(arr) && yyjson_arr_size(arr) > 0) {
                    ...
                    if (ti > 0) {
                        return pmm_type_template(arena, qn, targs, ti);
                    }
                }
            }
        }
    ...
    if (borrowed && yyjson_is_obj(borrowed)) {
        ...
        return pmm_type_reference(arena,
    ...
    if (tuple && yyjson_is_arr(tuple)) {
        ...
        if (n == 0) return pmm_type_builtin(arena, "()");
        ...
        return pmm_type_tuple(arena, elems, ei);
    }
...
static bool register_function(CBMTypeRegistry* reg, CBMArena* arena,
     ...
     * (implicit unit). */
    const CBMType* ret = pmm_type_builtin(arena, "()");
    ...
    const CBMType** ra = (const CBMType**)pmm_arena_alloc(arena,
...
int pmm_rust_rustdoc_ingest(CBMTypeRegistry* reg, CBMArena* arena,
    ...
    while ((key = yyjson_obj_iter_next(&it))) {
        ...
        if (kind && (strcmp(kind, "struct") == 0 ||
            ...
            if (register_function(reg, arena, pmm_arena_strdup(arena, short_name),
                                  qn, NULL, fn, crate_qn)) {
                count++;
            }
        }
