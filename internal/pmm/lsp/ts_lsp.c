...
long pmm_ts_full_registry_builds(void) {
    return atomic_load(&g_ts_full_reg_builds);
}
void pmm_ts_full_registry_builds_reset(void) {
 ...
 * enough that no real-world file ever hits it. PMM_TS_TYPE_BUDGET (absolute
...
static void ts_type_budget_reset(size_t source_len) {
    const char *e = getenv("PMM_TS_TYPE_BUDGET");
    if (e && e[0]) {
        long v = atol(e);
...
static TSNode *collect_children(CBMArena *arena, TSNode node, uint32_t *out_n) {
    ...
    *out_n = 0;
    if (nc == 0)
        return NULL;
    TSNode *kids = (TSNode *)pmm_arena_alloc(arena, (size_t)nc * sizeof(TSNode));
    if (!kids)
        return NULL;
    uint32_t kn = 0;
...
// `pmm_node_text` on a `type_annotation` AST node), and trailing whitespace/`;`.
static const CBMType *parse_ts_type_text(CBMArena *arena, const char *text, const char *module_qn) {
    if (!text || !text[0])
    ...
    if (text[0] == '(') {
        ...
        if (after + 2 <= len && text[after] == '=' && text[after + 1] == '>') {
            ...
            if (params_len > 0) {
                ...
                for (size_t i = 0; i <= params_len && pc < 15; i++) {
                    ...
                    if (c == '(' || c == '<' || c == '[')
                        ...
                        if (i > start) {
                            char *part = pmm_arena_strndup(arena, params_text + start, i - start);
                            ...
                            if (colon) {
                                *colon = '\0';
                                ...
                                while (nl > 0 && (nm[nl - 1] == ' ' || nm[nl - 1] == '?'))
                                    nl--;
                                fn_param_names[pc] = pmm_arena_strndup(arena, nm, nl);
                                fn_param_types[pc] =
                                    parse_ts_type_text(arena, colon + 1, module_qn);
                            } else {
                                fn_param_names[pc] = pmm_arena_sprintf(arena, "_%d", pc);
            ...
            return pmm_type_func(arena, fn_param_names, fn_param_types, rets);
        }
    ...
    if (len == 4 && memcmp(text, "this", 4) == 0) {
        return pmm_type_type_param(arena, "this");
    }
    ...
    // keywords at depth 0, in order.
    {
        ...
        if (ext_pos != (size_t)-1 && q_pos != (size_t)-1 && colon_pos != (size_t)-1) {
            char *check_text = pmm_arena_strndup(arena, text, ext_pos);
            char *extends_text =
                pmm_arena_strndup(arena, text + ext_pos + 9, q_pos - (ext_pos + 9));
            char *true_text = pmm_arena_strndup(arena, text + q_pos + 3, colon_pos - (q_pos + 3));
            char *false_text =
                pmm_arena_strndup(arena, text + colon_pos + 3, len - (colon_pos + 3));
            ...
            return pmm_type_conditional(arena, check, extends, true_branch, false_branch);
        }
    }

    // `infer X` — emit PMM_TYPE_INFER. Only meaningful inside conditional `extends`
    ...
    if (len > 6 && memcmp(text, "infer ", 6) == 0) {
        ...
        char *name = pmm_arena_strndup(arena, rest, (size_t)(end - rest));
        return pmm_type_infer(arena, name);
    }

    // `keyof T` — emit PMM_TYPE_KEYOF for downstream evaluation.
    if (len > 6 && memcmp(text, "keyof ", 6) == 0) {
        const char *rest = text + 6;
        while (*rest == ' ' || *rest == '\t')
            rest++;
        char *rest_term = pmm_arena_strdup(arena, rest);
        const CBMType *operand = parse_ts_type_text(arena, rest_term, module_qn);
        return pmm_type_keyof(arena, operand);
    }

    // `typeof X` in type position — emit PMM_TYPE_TYPEOF_QUERY.
    if (len > 7 && memcmp(text, "typeof ", 7) == 0) {
        const char *rest = text + 7;
        while (*rest == ' ' || *rest == '\t')
            rest++;
        return pmm_type_typeof_query(arena, rest);
    }
    ...
    if (text[0] == '[' && len >= 2 && text[len - 1] == ']') {
        ...
        for (size_t i = 0; i <= inner_len && ec < 15; i++) {
            ...
            if (c == '<' || c == '[')
                ...
                char *part = pmm_arena_strndup(arena, inner_text + start, i - start);
                elems[ec++] = parse_ts_type_text(arena, part, module_qn);
                start = i + 1;
            }
        }
        elems[ec] = NULL;
        return pmm_type_tuple(arena, elems, ec);
    }
    ...
    // Top-level UNION ` | ` or INTERSECTION ` & ` — split at depth 0 (outside <>, []).
    {
        ...
        for (size_t i = 0; i < len; i++) {
            ...
            if (c == '<' || c == '[')
                ...
                depth--;
                     ...
                     (text[i - 1] == ' ' || text[i - 1] == '\t')) {
                if (mc < 15) {
                    char *part = pmm_arena_strndup(arena, text + start, i - start);
        ...
        if (mc > 0 && (is_union || is_inter)) {
            ...
            if (mc < 15) {
                char *part = pmm_arena_strndup(arena, text + start, len - start);
                members[mc++] = parse_ts_type_text(arena, part, module_qn);
            }
            members[mc] = NULL;
            return is_union ? pmm_type_union(arena, members, mc)
                            : pmm_type_intersection(arena, members, mc);
        }
    }
    ...
    if (len > 2 && text[len - 1] == '>') {
        ...
        if (open != (size_t)-1 && open > 0) {
            char *base = pmm_arena_strndup(arena, text, open);
            ...
            for (size_t i = 0; i <= args_len && arg_count < 15; i++) {
                ...
                if (c == '<')
                    ...
                    char *part = pmm_arena_strndup(arena, args_text + start, i - start);
            ...
            if (!is_stdlib && module_qn && strchr(base, '.') == NULL && base[0] >= 'A' &&
                base[0] <= 'Z') {
                qualified_base = pmm_arena_sprintf(arena, "%s.%s", module_qn, base);
            }
            return pmm_type_template(arena, qualified_base, args, arg_count);
        }
    }
    ...
    if (module_qn && strchr(text, '.') == NULL) {
        ...
        char *terminated = pmm_arena_strndup(arena, text, len);
        if (!terminated)
            return pmm_type_unknown();
        for (int i = 0; bare_stdlib_names[i]; i++) {
            if (strcmp(terminated, bare_stdlib_names[i]) == 0) {
                return pmm_type_named(arena, terminated);
            }
        }
        ...
        if ((terminated[0] >= 'A' && terminated[0] <= 'Z')) {
            const char *qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, terminated);
            return pmm_type_named(arena, qn);
        }
        return pmm_type_named(arena, terminated);
    }
    return pmm_type_named(arena, pmm_arena_strndup(arena, text, len));
}
...
static const CBMType **parse_param_types_array(CBMArena *arena, const char **texts,
    ...
    if (count == 0)
        return NULL;
    const CBMType **arr =
        (const CBMType **)pmm_arena_alloc(arena, (size_t)(count + 1) * sizeof(const CBMType *));
    if (!arr)
        return NULL;
...
static const CBMType *ts_relater_unwrap(const CBMType *t) {
    ...
    if (t->kind == PMM_TYPE_ALIAS)
        return pmm_type_resolve_alias(t);
    return t;
}
...
static bool ts_literal_promotes_to_builtin(const CBMType *lit, const CBMType *bi) {
    if (!lit || lit->kind != PMM_TYPE_TS_LITERAL || !lit->data.literal_ts.tag)
        return false;
    if (!bi || bi->kind != PMM_TYPE_BUILTIN || !bi->data.builtin.name)
        return false;
...
static bool ts_class_extends(TSLSPContext *ctx, const char *sub_qn, const char *super_qn,
    ...
    if (strcmp(sub_qn, super_qn) == 0)
        return true;
    const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, sub_qn);
...
static int ts_is_assignable_inner(TSLSPContext *ctx, const CBMType *a, const CBMType *b, int depth,
    ...
    if (b->kind == PMM_TYPE_BUILTIN && b->data.builtin.name) {
        if (strcmp(b->data.builtin.name, "any") == 0 ||
            strcmp(b->data.builtin.name, "unknown") == 0)
            return 1;
    }
    if (a->kind == PMM_TYPE_BUILTIN && a->data.builtin.name) {
    ...
    if (a->kind == PMM_TYPE_UNION && a->data.union_type.members) {
        result = 1;
    ...
    if (b->kind == PMM_TYPE_UNION && b->data.union_type.members) {
    ...
    if (a->kind == PMM_TYPE_INTERSECTION && a->data.union_type.members) {
        for (int i = 0; i < a->data.union_type.count; i++) {
            if (ts_is_assignable_inner(ctx, a->data.union_type.members[i], b, depth + 1, cache)) {
                result = 1;
                break;
    ...
    if (b->kind == PMM_TYPE_INTERSECTION && b->data.union_type.members) {
        result = 1;
        for (int i = 0; i < b->data.union_type.count; i++) {
            if (!ts_is_assignable_inner(ctx, a, b->data.union_type.members[i], depth + 1, cache)) {
                result = 0;
...
// may contain PMM_TYPE_INFER nodes) and records bindings into a small fixed-size
...
static bool match_with_infer(TSLSPContext *ctx, const CBMType *source, const CBMType *pattern,
    ...
    if (pattern->kind == PMM_TYPE_INFER) {
...
static const CBMType *subst_infer_bindings(TSLSPContext *ctx, const CBMType *t,
    ...
    if (t->kind == PMM_TYPE_TYPE_PARAM && t->data.type_param.name) {
        for (int i = 0; i < bind_count; i++) {
    ...
    if (t->kind == PMM_TYPE_NAMED && t->data.named.qualified_name) {
    ...
    if (t->kind == PMM_TYPE_TEMPLATE && t->data.template_type.template_args &&
        t->data.template_type.arg_count > 0) {
        int ac = t->data.template_type.arg_count;
        const CBMType **new_args = (const CBMType **)pmm_arena_alloc(
        ...
        new_args[ac] = NULL;
        return pmm_type_template(ctx->arena, t->data.template_type.template_name, new_args, ac);
    }
    if (t->kind == PMM_TYPE_TUPLE && t->data.tuple.count > 0) {
        int n = t->data.tuple.count;
        const CBMType **new_elems = (const CBMType **)pmm_arena_alloc(
        ...
        new_elems[n] = NULL;
        return pmm_type_tuple(ctx->arena, new_elems, n);
    }
    return t;
}

// Walk a type tree to detect any PMM_TYPE_INFER nodes.
static bool contains_infer(const CBMType *t, int depth) {
    if (!t || depth > 32)
        return false;
    if (t->kind == PMM_TYPE_INFER)
        return true;
    if (t->kind == PMM_TYPE_TEMPLATE && t->data.template_type.template_args) {
...
// Evaluate a PMM_TYPE_CONDITIONAL `T extends U ? X : Y`. With distribution: when T is
...
static const CBMType *eval_conditional(TSLSPContext *ctx, const CBMType *t, int depth) {
    if (!t || t->kind != PMM_TYPE_CONDITIONAL || depth > 16)
        return t;
    ...
    if (check->kind == PMM_TYPE_UNION && check->data.union_type.members &&
        check->data.union_type.count > 0) {
        const CBMType **results = (const CBMType **)pmm_arena_alloc(
        ...
        for (int i = 0; i < check->data.union_type.count; i++) {
            ...
            const CBMType *sub = pmm_type_conditional(ctx->arena, member, extends, tb, fb);
            results[n++] = eval_conditional(ctx, sub, depth + 1);
        }
        results[n] = NULL;
        if (n == 1)
            return results[0];
        return pmm_type_union(ctx->arena, results, n);
    }
...
static const CBMType *simplify_type(TSLSPContext *ctx, const CBMType *t) {
    if (!t || !ctx || !ctx->registry)
        return t;
    if (t->kind == PMM_TYPE_CONDITIONAL) {
    ...
    if (t->kind != PMM_TYPE_NAMED)
        return t;
    for (int i = 0; i < 16; i++) {
        if (!t || t->kind != PMM_TYPE_NAMED)
            return t;
        const CBMRegisteredType *rt =
            pmm_registry_lookup_type(ctx->registry, t->data.named.qualified_name);
        if (!rt || !rt->alias_of)
            return t;
        t = pmm_type_named(ctx->arena, rt->alias_of);
    }
    return t;
}
...
static const CBMType *type_of_identifier(TSLSPContext *ctx, const char *name) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        ...
        if (mqn_len > name_len + 1 && mqn[mqn_len - name_len - 1] == '.' &&
            ...
            full_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", mqn, name);
        }

        // Try the symbol lookup precedence: registered func, then registered type.
        const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, full_qn);
        if (f && f->signature)
            return f->signature;
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, full_qn);
        if (rt)
            return pmm_type_named(ctx->arena, full_qn);

        // No specific match — fall back to NAMED(module_qn), which is correct for
        // namespace-style imports (`import * as foo from './foo'`).
        return pmm_type_named(ctx->arena, mqn);
    }

    // Module-local function or class.
    if (ctx->module_qn) {
        const char *candidate = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name);
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, candidate);
        if (rt)
            return pmm_type_named(ctx->arena, candidate);
        const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, candidate);
        if (f && f->signature)
            return f->signature;
    }
    ...
    {
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, name);
        if (rt)
...
static const CBMType *lookup_member_type_inner(TSLSPContext *ctx, const CBMType *recv,
    ...
    if (base->kind == PMM_TYPE_BUILTIN) {
        const char *wrap = builtin_wrapper_class(base->data.builtin.name);
        if (wrap) {
            const CBMType *wrapped = pmm_type_named(ctx->arena, wrap);
            return lookup_member_type(ctx, wrapped, name);
        }
        return pmm_type_unknown();
    }

    if (base->kind == PMM_TYPE_TEMPLATE) {
        // Look up on the template name (e.g., "Array", "Promise"); generic params are
        // applied via pmm_type_substitute when we know the registered type's params.
        const CBMRegisteredType *rt =
            pmm_registry_lookup_type(ctx->registry, base->data.template_type.template_name);
        if (rt) {
            // Field?
            if (rt->field_names && rt->field_types) {
                for (int i = 0; rt->field_names[i]; i++) {
                    if (strcmp(rt->field_names[i], name) == 0) {
                        return pmm_type_substitute(ctx->arena, rt->field_types[i],
                                                   rt->type_param_names,
                ...
                for (int i = 0; rt->method_names[i]; i++) {
                    if (strcmp(rt->method_names[i], name) == 0) {
                        const CBMRegisteredFunc *f =
                            pmm_registry_lookup_func(ctx->registry, rt->method_qns[i]);
                        if (f && f->signature) {
                            return pmm_type_substitute(ctx->arena, f->signature,
                                                       rt->type_param_names,
                                                       base->data.template_type.template_args);
                        }
                    }
    ...
    if (base->kind == PMM_TYPE_OBJECT_LIT) {
    ...
    if (base->kind == PMM_TYPE_INTERSECTION) {
        if (base->data.union_type.members) {
            for (int i = 0; i < base->data.union_type.count; i++) {
                const CBMType *m = lookup_member_type(ctx, base->data.union_type.members[i], name);
                if (!pmm_type_is_unknown(m))
    ...
    if (base->kind != PMM_TYPE_NAMED)
        return pmm_type_unknown();

    const char *recv_qn = base->data.named.qualified_name;
    const CBMRegisteredType *rt = pmm_registry_resolve_alias(ctx->registry, recv_qn);
    if (!rt) {
        rt = pmm_registry_lookup_type(ctx->registry, recv_qn);
        if (!rt)
            return pmm_type_unknown();
    }
    ...
    if (rt->embedded_types) {
        for (int i = 0; rt->embedded_types[i]; i++) {
            const CBMType *parent = pmm_type_named(ctx->arena, rt->embedded_types[i]);
...
static const CBMType *unwrap_passthrough_template(const CBMType *t) {
    ...
    if (strcmp(name, "ReturnType") == 0) {
        ...
        if (f && f->kind == PMM_TYPE_FUNC && f->data.func.return_types &&
...
static const CBMType *eval_keyof(TSLSPContext *ctx, const CBMType *operand) {
    ...
    if (!base || base->kind != PMM_TYPE_NAMED)
        return pmm_type_unknown();
    const CBMRegisteredType *rt =
        pmm_registry_lookup_type(ctx->registry, base->data.named.qualified_name);
    ...
    for (int i = 0; i < fc && mc < 63; i++) {
        members[mc++] = pmm_type_ts_literal(ctx->arena, "string", rt->field_names[i]);
    }
    members[mc] = NULL;
    if (mc == 0)
        return pmm_type_unknown();
    if (mc == 1)
        return members[0];
    return pmm_type_union(ctx->arena, members, mc);
}
...
static const CBMType *eval_indexed_access(TSLSPContext *ctx, const CBMType *obj,
    ...
    if (!ctx || !obj || !key)
        return pmm_type_unknown();
    if (key->kind == PMM_TYPE_TS_LITERAL && key->data.literal_ts.value &&
...
static const CBMRegisteredFunc *lookup_method(TSLSPContext *ctx, const CBMType *recv,
    ...
    if (ctx->debug) {
        const char *base_qn = (base->kind == PMM_TYPE_NAMED) ? base->data.named.qualified_name
        ...
        if (rt && rt->method_qns && rt->method_names) {
            for (int i = 0; rt->method_names[i]; i++) {
                if (strcmp(rt->method_names[i], method_name) == 0) {
                    return pmm_registry_lookup_func(ctx->registry, rt->method_qns[i]);
                }
            }
        }
    }

    if (base->kind == PMM_TYPE_UNION || base->kind == PMM_TYPE_INTERSECTION) {
        if (base->data.union_type.members) {
            for (int i = 0; i < base->data.union_type.count; i++) {
                const CBMRegisteredFunc *f =
    ...
    const CBMRegisteredFunc *f =
        pmm_registry_lookup_method_aliased(ctx->registry, recv_qn, method_name);
    if (f)
        return f;

    // Walk extends/implements via the registered type.
    const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, recv_qn);
    if (rt && rt->embedded_types) {
        for (int i = 0; rt->embedded_types[i]; i++) {
            const CBMType *parent = pmm_type_named(ctx->arena, rt->embedded_types[i]);
...
static const CBMType *return_type_of(CBMArena *arena, const CBMType *sig) {
    ...
    // crashed in pmm_arena_alloc when the cross-resolve path evaluated a
    // multi-return signature (only reachable once the per-file LSP O(n²) was
    // fixed and the resolve phase actually ran).
    return pmm_type_tuple(arena, sig->data.func.return_types, count);
}
...
const CBMType *ts_eval_expr_type(TSLSPContext *ctx, TSNode node) {
    ...
    const CBMType *result = pmm_type_unknown();
    ...
    if (strcmp(kind, "identifier") == 0 || strcmp(kind, "shorthand_property_identifier") == 0) {
        ...
        if (ctx->enclosing_class_qn) {
            result = pmm_type_named(ctx->arena, ctx->enclosing_class_qn);
        }
    } else if (strcmp(kind, "super") == 0) {
        if (ctx->enclosing_class_qn) {
            const CBMRegisteredType *rt =
                pmm_registry_lookup_type(ctx->registry, ctx->enclosing_class_qn);
            if (rt && rt->embedded_types && rt->embedded_types[0]) {
                result = pmm_type_named(ctx->arena, rt->embedded_types[0]);
            }
        }
        ...
        if (!ts_node_is_null(ctor)) {
            ...
            if (cname) {
                ...
                if (strchr(cname, '.') == NULL && ctx->module_qn) {
                    const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
                    result = pmm_type_named(ctx->arena, qn);
                } else {
                    result = pmm_type_named(ctx->arena, cname);
                }
            }
        }
        ...
        if (!ts_node_is_null(fn)) {
            ...
            if (fn_type && fn_type->kind == PMM_TYPE_FUNC) {
                ...
                if (result && result->kind == PMM_TYPE_TYPE_PARAM && result->data.type_param.name &&
                    ...
                    if (!ts_node_is_null(obj)) {
                        const CBMType *recv = ts_eval_expr_type(ctx, obj);
                        if (recv && !pmm_type_is_unknown(recv))
                            result = recv;
                    }
                }
                ...
                if (result && result->kind == PMM_TYPE_TYPE_PARAM &&
                    ...
                    if (!ts_node_is_null(args)) {
                        ...
                        for (uint32_t i = 0; i < pc && i < argc && ic < 7; i++) {
                            ...
                            if (!pt || pt->kind != PMM_TYPE_TYPE_PARAM)
                                continue;
                            TSNode arg = ts_node_named_child(args, i);
                            if (ts_node_is_null(arg))
                                continue;
                            const CBMType *at = ts_eval_expr_type(ctx, arg);
                            if (pmm_type_is_unknown(at))
                                continue;
                        ...
                        if (ic > 0) {
                            ...
                            inf_args[ic] = NULL;
                            const CBMType *sub =
                                pmm_type_substitute(ctx->arena, result, inf_names, inf_args);
                            if (sub && !pmm_type_is_unknown(sub))
                                result = sub;
                        }
                    }
                }
                ...
                if (ctx->debug) {
                    const char *res_qn = (result && result->kind == PMM_TYPE_NAMED)
        ...
        // Promise<T> → T.
        if (t && t->kind == PMM_TYPE_TEMPLATE && t->data.template_type.template_name &&
        ...
        result = pmm_type_builtin(ctx->arena, "string");
    } else if (strcmp(kind, "number") == 0) {
        result = pmm_type_builtin(ctx->arena, "number");
    } else if (strcmp(kind, "true") == 0 || strcmp(kind, "false") == 0) {
        result = pmm_type_builtin(ctx->arena, "boolean");
    } else if (strcmp(kind, "null") == 0) {
        result = pmm_type_builtin(ctx->arena, "null");
    } else if (strcmp(kind, "undefined") == 0) {
        result = pmm_type_builtin(ctx->arena, "undefined");
    } else if (strcmp(kind, "array") == 0) {
        // Use first element's type as Array<T> argument.
        const CBMType *elem = pmm_type_unknown();
        uint32_t nc = ts_node_named_child_count(node);
        for (uint32_t i = 0; i < nc; i++) {
            TSNode c = ts_node_named_child(node, i);
            if (!ts_node_is_null(c)) {
                elem = ts_eval_expr_type(ctx, c);
                if (!pmm_type_is_unknown(elem))
                    break;
            }
        }
        const CBMType *args[2] = {elem, NULL};
        result = pmm_type_template(ctx->arena, "Array", args, 1);
        ...
        names[count] = NULL;
        types[count] = NULL;
        result = pmm_type_object_lit(ctx->arena, names, types, NULL, NULL);
        ...
        if (pmm_type_is_unknown(a))
            result = b;
        else if (pmm_type_is_unknown(b))
            result = a;
        else if (a == b)
            result = a;
        else {
            const CBMType *members[3] = {a, b, NULL};
            result = pmm_type_union(ctx->arena, members, 2);
        }
        ...
        if (!ts_node_is_null(left) && !ts_node_is_null(right)) {
            ...
            if ((l && l->kind == PMM_TYPE_BUILTIN && l->data.builtin.name &&
                 strcmp(l->data.builtin.name, "string") == 0) ||
                (r && r->kind == PMM_TYPE_BUILTIN && r->data.builtin.name &&
...
static const CBMType *resolve_type_with_imports(TSLSPContext *ctx, const CBMType *t) {
    ...
    if (t->kind == PMM_TYPE_NAMED && t->data.named.qualified_name) {
        ...
        if (!dot && ctx->module_qn) {
            const char *local_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare);
            const CBMRegisteredType *local_rt = pmm_registry_lookup_type(ctx->registry, local_qn);
            if (local_rt)
                return pmm_type_named(ctx->arena, local_qn);
        }
        ...
        for (int i = 0; i < ctx->import_count; i++) {
            ...
            return pmm_type_named(ctx->arena, pmm_arena_sprintf(ctx->arena, "%s.%s", mqn, bare));
        }
        return t;
    }
    if (t->kind == PMM_TYPE_TEMPLATE && t->data.template_type.template_args &&
    ...
    if (t->kind == PMM_TYPE_UNION || t->kind == PMM_TYPE_INTERSECTION) {
        if (!t->data.union_type.members || t->data.union_type.count == 0)
            return t;
        int mc = t->data.union_type.count;
        const CBMType **new_members = (const CBMType **)pmm_arena_alloc(
        ...
        new_members[mc] = NULL;
        return t->kind == PMM_TYPE_UNION ? pmm_type_union(ctx->arena, new_members, mc)
                                         : pmm_type_intersection(ctx->arena, new_members, mc);
    }
    return t;
}
...
static void bind_variable_declarator(TSLSPContext *ctx, TSNode decl) {
    ...
    const CBMType *inferred =
        ts_node_is_null(value) ? pmm_type_unknown() : ts_eval_expr_type(ctx, value);
    const CBMType *bound = (declared && !pmm_type_is_unknown(declared)) ? declared : inferred;

    if (node_kind_is(name, "identifier")) {
        char *nm = node_text(ctx, name);
        if (nm)
            pmm_scope_bind(ctx->current_scope, nm, bound);
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(pk, "shorthand_property_identifier_pattern") == 0) {
                ...
                if (pnm)
                    pmm_scope_bind(ctx->current_scope, pnm, lookup_member_type(ctx, bound, pnm));
                ...
                if (!ts_node_is_null(key) && !ts_node_is_null(val) &&
                    ...
                    if (knm && vnm) {
                        pmm_scope_bind(ctx->current_scope, vnm,
...
static void bind_parameter(TSLSPContext *ctx, TSNode param) {
    ...
    const CBMType *t = pmm_type_unknown();
    if (!ts_node_is_null(tann)) {
            ...
            if (strcmp(tk, "object_type") == 0) {
                ...
                for (uint32_t oi = 0; oi < omc && pcount < 31; oi++) {
                    ...
                    if (strcmp(mk, "property_signature") == 0) {
                        ...
                        if (!pnm_text)
                            continue;
                        const CBMType *pt = pmm_type_unknown();
                ...
                p_names[pcount] = NULL;
                p_types[pcount] = NULL;
                t = pmm_type_object_lit(ctx->arena, p_names, p_types, NULL, NULL);
            } else {
    ...
    if (node_kind_is(pattern, "identifier")) {
        ...
        if (nm)
            pmm_scope_bind(ctx->current_scope, nm, t);
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (node_kind_is(p, "shorthand_property_identifier_pattern")) {
                ...
                if (pnm)
                    pmm_scope_bind(ctx->current_scope, pnm, lookup_member_type(ctx, t, pnm));
                ...
                if (!ts_node_is_null(key) && !ts_node_is_null(val) &&
                    ...
                    char *vnm = node_text(ctx, val);
                    if (knm && vnm) {
                        pmm_scope_bind(ctx->current_scope, vnm, lookup_member_type(ctx, t, knm));
                    }
                }
            }
        }
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (!pnm)
                continue;
            const CBMType *et = pmm_type_unknown();
            if (t && t->kind == PMM_TYPE_TEMPLATE && t->data.template_type.template_args &&
...
void ts_process_statement(TSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "lexical_declaration") == 0 || strcmp(kind, "variable_declaration") == 0) {
        ...
        if (!ts_node_is_null(left) && !ts_node_is_null(right)) {
            ...
            const CBMType *elem = pmm_type_unknown();
            if (iter && iter->kind == PMM_TYPE_TEMPLATE && iter->data.template_type.template_args &&
            ...
            if (node_kind_is(left, "lexical_declaration") ||
                ...
                if (node_kind_is(v, "variable_declarator")) {
                    ...
                    if (node_kind_is(nm, "identifier")) {
                        ...
                        if (s)
                            pmm_scope_bind(ctx->current_scope, s, elem);
                    }
                }
...
static void resolve_call_at(TSLSPContext *ctx, TSNode call_node) {
    ...
    if (strcmp(fk, "member_expression") == 0) {
        ...
        if (!ts_node_is_null(obj) && !ts_node_is_null(prop)) {
            ...
            if (mname) {
                    ...
                    if (on) {
                        for (int i = 0; i < ctx->import_count; i++) {
                            ...
                            if (lname && mqn && strcmp(lname, on) == 0) {
                                const CBMRegisteredFunc *f = pmm_registry_lookup_symbol_by_args(
                                ...
                                ts_emit_unresolved_call(
                                    ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", on, mname),
                ...
                // arg's type and let pmm_registry_lookup_method_by_types score them.
                // Falls back to the first match if scoring fails.
                if (f && recv) {
                    const CBMType *base = simplify_type(ctx, recv);
                    base = base ? unwrap_passthrough_template(base) : NULL;
                    if (base && base->kind == PMM_TYPE_NAMED && base->data.named.qualified_name &&
                        arg_count > 0) {
                        ...
                        const CBMRegisteredFunc *better = pmm_registry_lookup_method_by_types(
    ...
    if (strcmp(fk, "identifier") == 0) {
        ...
        if (ctx->module_qn) {
            ...
            if (typed_arg_n > 0) {
                f = pmm_registry_lookup_symbol_by_types(ctx->registry, ctx->module_qn, name,
                                                        arg_types, typed_arg_n);
            }
            if (!f) {
                f = pmm_registry_lookup_symbol_by_args(ctx->registry, ctx->module_qn, name,
                                                       arg_count);
            }
            if (f) {
        ...
        for (int i = 0; i < ctx->import_count; i++) {
            ...
            if (lname && mqn && strcmp(lname, name) == 0) {
                const CBMRegisteredFunc *f =
                    pmm_registry_lookup_symbol_by_args(ctx->registry, mqn, name, arg_count);
                if (f) {
                    ts_emit_resolved_call(ctx, f->qualified_name, "lsp_ts_import", 0.95f);
                    return;
                }
                // Fall through to qualified_name fallback.
                const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", mqn, name);
...
static void process_callback_arrow(TSLSPContext *ctx, TSNode arrow, const CBMType *expected) {
    if (ts_node_is_null(arrow) || !expected || expected->kind != PMM_TYPE_FUNC)
        return;

    CBMScope *saved = ctx->current_scope;
    ctx->current_scope = pmm_scope_push(ctx->arena, saved);
    ...
    if (!ts_node_is_null(bare_param)) {
        ...
        if (strcmp(pk, "identifier") == 0) {
            ...
            const CBMType *pt =
                (expected->data.func.param_types && expected->data.func.param_types[0])
                    ...
                    : pmm_type_unknown();
            if (nm)
                pmm_scope_bind(ctx->current_scope, nm, pt);
        }
        ...
        for (uint32_t i = 0; i < pc; i++) {
            ...
            if (strcmp(pk, "identifier") == 0) {
                ...
                if (!ts_node_is_null(tann)) {
                    ...
                    if (!ts_node_is_null(tch)) {
                        ...
                        if (!pmm_type_is_unknown(annotated)) {
            ...
            if (pname) {
                pmm_scope_bind(ctx->current_scope, pname,
                               expected_pt ? expected_pt : pmm_type_unknown());
            }
            idx++;
        }
    }
                ...
                if (opt2 && strcmp(opt2, "typeof") == 0 && !ts_node_is_null(arg) &&
                    ...
                    *out_var = node_text(ctx, arg);
                    ...
                    if (lit) {
                        ...
                        *out_type = pmm_type_builtin(ctx->arena, lit);
                    }
...
static const CBMType *narrow_discriminated_union(TSLSPContext *ctx, TSNode condition,
    ...
    *out_var = NULL;
    ...
    const CBMType *var_type = pmm_scope_lookup(ctx->current_scope, var_name);
    if (!var_type || var_type->kind != PMM_TYPE_UNION)
        return NULL;
    ...
    for (int i = 0; i < var_type->data.union_type.count; i++) {
        ...
        if (!m || m->kind != PMM_TYPE_NAMED)
            continue;
        const CBMRegisteredType *rt =
            pmm_registry_lookup_type(ctx->registry, m->data.named.qualified_name);
        ...
        for (int j = 0; rt->field_names[j]; j++) {
            ...
            if (ft->kind == PMM_TYPE_TS_LITERAL && ft->data.literal_ts.value &&
            ...
            if (ft->kind == PMM_TYPE_NAMED && ft->data.named.qualified_name) {
...
static void resolve_jsx_element(TSLSPContext *ctx, TSNode element_node) {
    ...
    if (ctx->module_qn) {
        const CBMRegisteredFunc *f =
            pmm_registry_lookup_symbol(ctx->registry, ctx->module_qn, tag_name);
        if (f) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        ...
        if (lname && mqn && strcmp(lname, tag_name) == 0) {
            ...
            const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", mqn, tag_name);
...
static void process_node(TSLSPContext *ctx, TSNode node) {
        ...
        do {
            ...
            for (uint32_t i = 0; i < argc; i++) {
                ...
                if ((strcmp(ak, "arrow_function") == 0 || strcmp(ak, "function_expression") == 0) &&
                    expected && expected->kind == PMM_TYPE_FUNC) {
        ...
        if (!ts_node_is_null(value)) {
            ...
            if (!ts_node_is_null(v) && strcmp(ts_node_type(v), "member_expression") == 0) {
                ...
                if (!ts_node_is_null(obj) && !ts_node_is_null(prop) &&
                    ...
                    if (disc_var) {
                        const CBMType *t = pmm_scope_lookup(ctx->current_scope, disc_var);
                        if (t && t->kind == PMM_TYPE_UNION)
                            var_union = t;
                    }
                }
            }
        ...
        if (!ts_node_is_null(body)) {
            ...
            for (uint32_t i = 0; i < bnc; i++) {
                    ...
                    if (!ts_node_is_null(case_value) &&
                        ...
                        if (lit) {
                            ...
                            for (int mi = 0; mi < var_union->data.union_type.count; mi++) {
                                ...
                                if (!m || m->kind != PMM_TYPE_NAMED)
                                    continue;
                                const CBMRegisteredType *rt = pmm_registry_lookup_type(
                                ...
                                for (int fj = 0; rt->field_names[fj]; fj++) {
                                    ...
                                    if (!ft)
                                        continue;
                                    if (ft->kind == PMM_TYPE_TS_LITERAL &&
                                        ft->data.literal_ts.value &&
                                        strcmp(ft->data.literal_ts.value, lit) == 0) {
                                        narrowed = m;
                                        break;
                                    }
                                    if (ft->kind == PMM_TYPE_NAMED &&
                ...
                if (narrowed && disc_var) {
                    ctx->current_scope = pmm_scope_push(ctx->arena, saved);
                    pmm_scope_bind(ctx->current_scope, disc_var, narrowed);
                }
    ...
    if (strcmp(kind, "if_statement") == 0) {
        ...
        if (!ts_node_is_null(consequence)) {
            if (ok && nv && nt && !inverted) {
                CBMScope *saved = ctx->current_scope;
                ctx->current_scope = pmm_scope_push(ctx->arena, saved);
                pmm_scope_bind(ctx->current_scope, nv, nt);
    ...
    if (strcmp(kind, "class_declaration") == 0) {
        ...
        if (!ts_node_is_null(name) && ctx->module_qn) {
            ...
            if (cn)
                class_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cn);
        }
        TSNode body = ts_node_child_by_field_name(node, "body", TS_LSP_FIELD_LEN("body"));
        if (!ts_node_is_null(body) && class_qn) {
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                const char *mqn = strcmp(mn, "constructor") == 0
                                      ? pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mn)
                                      : pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mn);
...
static void process_function_body(TSLSPContext *ctx, TSNode func_node, const char *func_qn,
    ...
    ctx->current_scope = pmm_scope_push(ctx->arena, saved_scope);
    ...
    if (class_qn) {
        ...
        pmm_scope_bind(ctx->current_scope, "this", pmm_type_named(ctx->arena, class_qn));
    }
    ...
    if (!ts_node_is_null(params)) {
        ...
        if (func_qn) {
            const CBMRegisteredFunc *rf = pmm_registry_lookup_func(ctx->registry, func_qn);
            if (rf && rf->signature && rf->signature->kind == PMM_TYPE_FUNC &&
                ...
                for (int i = 0; rf->signature->data.func.param_names[i] &&
                    ...
                    if (pmm_type_is_unknown(ptype))
                        continue;
                    const CBMType *existing = pmm_scope_lookup(ctx->current_scope, pname);
                    if (existing && !pmm_type_is_unknown(existing))
                        continue;
                    pmm_scope_bind(ctx->current_scope, pname, ptype);
                }
            }
        }
    }
...
void ts_lsp_process_file(TSLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (strcmp(kind, "function_declaration") == 0) {
            ...
            if (!fn)
                continue;
            const char *fqn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fn);
...
void ts_lsp_init(TSLSPContext *ctx, CBMArena *arena, const char *source, int source_len,
    ...
    ctx->current_scope = arena ? pmm_scope_push(arena, NULL) : NULL;
...
void ts_lsp_add_import(TSLSPContext *ctx, const char *local_name, const char *module_qn) {
    ...
    const char **names =
        (const char **)pmm_arena_alloc(ctx->arena, (size_t)new_count * sizeof(const char *));
    const char **qns =
        (const char **)pmm_arena_alloc(ctx->arena, (size_t)new_count * sizeof(const char *));
    if (!names || !qns)
        return;
    ...
    names[ctx->import_count] = pmm_arena_strdup(ctx->arena, local_name);
    qns[ctx->import_count] = pmm_arena_strdup(ctx->arena, module_qn);
...
static const CBMType *tt_builtin(CBMArena *a, const char *n) {
    return pmm_type_builtin(a, n);
}
static const CBMType *tt_param(CBMArena *a, const char *n) {
    return pmm_type_type_param(a, n);
}
...
static void ts_reg_method(CBMTypeRegistry *reg, CBMArena *arena, const char *recv_qn,
    ...
    rf.qualified_name = pmm_arena_sprintf(arena, "%s.%s", recv_qn, name);
    rf.short_name = pmm_arena_strdup(arena, name);
    rf.receiver_type = pmm_arena_strdup(arena, recv_qn);
    rf.min_params = -1;
    const CBMType *rets[2] = {ret_type ? ret_type : pmm_type_unknown(), NULL};
...
static void reg_type_with_methods(CBMTypeRegistry *reg, CBMArena *arena, const char *qn,
    ...
    rt.qualified_name = pmm_arena_strdup(arena, qn);
    rt.short_name = pmm_arena_strdup(arena, short_name);
    ...
    if (method_names) {
        ...
        while (method_names[count])
            count++;
        const char **mqns =
            (const char **)pmm_arena_alloc(arena, (size_t)(count + 1) * sizeof(const char *));
        const char **mnames_copy =
            (const char **)pmm_arena_alloc(arena, (size_t)(count + 1) * sizeof(const char *));
        if (mqns && mnames_copy) {
            for (int i = 0; i < count; i++) {
                mnames_copy[i] = pmm_arena_strdup(arena, method_names[i]);
                mqns[i] = pmm_arena_sprintf(arena, "%s.%s", qn, method_names[i]);
            }
...
void pmm_ts_stdlib_register(CBMTypeRegistry *reg, CBMArena *arena) {
    if (!reg || !arena)
        return;
    ...
    // Array<T>: minimal method surface for v1.
    {
        ...
        const CBMType *t_arr_T = pmm_type_template(arena, "Array", t_arr_T_args, 1);
        ...
        returns[11] =
            pmm_type_template(arena, "Array", (const CBMType *[]){tparam_U, NULL}, 1); // map<U>
        ...
        for (int ci = 0; cb_specs[ci].method; ci++) {
            ...
            const CBMType *cb_func =
                pmm_type_func(arena, cb_param_names, cb_param_types, cb_returns);
            ...
            const CBMType *new_sig =
                pmm_type_func(arena, p_names, outer_param_types, outer_returns);

            // Find Array.<method> in the registry and patch its signature.
            char *qn = pmm_arena_sprintf(arena, "Array.%s", cb_specs[ci].method);
    ...
    // Promise<T>
    {
        ...
        const CBMType *t_promise_T = pmm_type_template(arena, "Promise", t_promise_T_args, 1);
    ...
    // Map<K,V>
    {
        const char **tparams = (const char **)pmm_arena_alloc(arena, 3 * sizeof(const char *));
        tparams[0] = "K";
        tparams[1] = "V";
        tparams[2] = NULL;
    ...
    // Set<T>
    {
        ...
        const CBMType *returns[6] = {
            pmm_type_template(arena, "Set", (const CBMType *[]){tt_param(arena, "T"), NULL}, 1),
            t_boolean,
            t_boolean,
            t_void,
            t_void,
    ...
    // Iterator<T> / Iterable<T>
    {
        ...
        const CBMType *iter_t =
            pmm_type_template(arena, "Iterator", (const CBMType *[]){tparam_T, NULL}, 1);
    ...
    // AsyncIterator<T> / AsyncIterable<T> — methods return Promise<T>
    {
        ...
        const CBMType *prom_T =
            pmm_type_template(arena, "Promise", (const CBMType *[]){tparam_T, NULL}, 1);
    ...
    // String prototype methods (most common).
    {
        ...
        const CBMType **returns =
            (const CBMType **)pmm_arena_alloc(arena, (size_t)(mc + 1) * sizeof(const CBMType *));
        for (int i = 0; i < mc; i++) {
            ...
            if (strcmp(m, "includes") == 0 || strcmp(m, "startsWith") == 0 ||
                ...
                returns[i] =
                    pmm_type_template(arena, "Array", (const CBMType *[]){t_string, NULL}, 1);
            } else {
                returns[i] = t_string;
            }
        }
    ...
    // Object — constructor namespace methods that are commonly chained.
    {
        ...
        const CBMType *returns[9] = {
            pmm_type_template(arena, "Array", (const CBMType *[]){t_string, NULL}, 1),
            pmm_type_template(arena, "Array", (const CBMType *[]){t_unknown, NULL}, 1),
            pmm_type_template(arena, "Array", (const CBMType *[]){t_unknown, NULL}, 1),
            t_unknown,
            t_unknown,
            t_boolean,
    ...
    // Array.of(...items): Array<T>
    {
        ...
        const CBMType *t_T = tparam_T;
        const CBMType *prom_T =
            pmm_type_template(arena, "Promise", (const CBMType *[]){t_T, NULL}, 1);
        const CBMType *arr_T = pmm_type_template(arena, "Array", (const CBMType *[]){t_T, NULL}, 1);
        const CBMType *prom_arr_T =
            pmm_type_template(arena, "Promise", (const CBMType *[]){arr_T, NULL}, 1);
    ...
    // be Phase 5 (generator).
    {
        ...
        const CBMType *node_t = pmm_type_named(arena, "Node");
        ...
        const CBMType *el_t = pmm_type_named(arena, "Element");
        const CBMType *nodelist_t =
            pmm_type_template(arena, "NodeList", (const CBMType *[]){el_t, NULL}, 1);
        ...
        const CBMType *html_t = pmm_type_named(arena, "HTMLElement");
        const CBMType *html_returns[12] = {
            t_string, t_void,
            html_t,   pmm_type_template(arena, "NodeList", (const CBMType *[]){html_t, NULL}, 1),
            html_t,   t_void,
            t_void,   t_void,
            t_void,   t_void,
            t_void,   NULL,
        ...
        const CBMType *doc_returns[11] = {
            html_t,
            html_t,
            pmm_type_template(arena, "NodeList", (const CBMType *[]){html_t, NULL}, 1),
            pmm_type_template(arena, "HTMLCollection", (const CBMType *[]){html_t, NULL}, 1),
            pmm_type_template(arena, "HTMLCollection", (const CBMType *[]){html_t, NULL}, 1),
            html_t,
            node_t,
            node_t,
        ...
        const CBMType *event_returns[5] = {
            ...
            pmm_type_template(
                arena, "Array",
                (const CBMType *[]){
                    et_methods[0] ? pmm_type_named(arena, "EventTarget") : t_unknown, NULL},
                1),
            NULL,
        };
        ...
        const CBMType *resp_returns[7] = {
            pmm_type_template(arena, "Promise", (const CBMType *[]){t_unknown, NULL}, 1),
            pmm_type_template(arena, "Promise", (const CBMType *[]){t_string, NULL}, 1),
            pmm_type_template(arena, "Promise", (const CBMType *[]){t_unknown, NULL}, 1),
            pmm_type_template(arena, "Promise", (const CBMType *[]){t_unknown, NULL}, 1),
            pmm_type_template(arena, "Promise", (const CBMType *[]){t_unknown, NULL}, 1),
            pmm_type_named(arena, "Response"),
            NULL,
        };
        ...
        const CBMType *win_returns[13] = {
            ...
            t_boolean,
            t_string,
            pmm_type_template(arena, "Promise",
                              (const CBMType *[]){pmm_type_named(arena, "Response"), NULL}, 1),
            t_number,
            t_void,
            t_number,
            t_void,
...
static void register_file_defs(CBMArena *arena, CBMTypeRegistry *reg, CBMFileResult *result,
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0) {
            ...
            if (d->base_classes) {
                ...
                while (d->base_classes[bc])
                    bc++;
                if (bc > 0) {
                    const char **emb = (const char **)pmm_arena_alloc(
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (b) {
                                ...
                                char *base_name = pmm_arena_strndup(arena, b, bl);
                                if (strchr(base_name, '.') == NULL) {
                                    emb[j] =
                                        pmm_arena_sprintf(arena, "%s.%s", module_qn, base_name);
                                } else {
                                    emb[j] = base_name;
                                }
                            } else {
            ...
            if (d->return_type && d->return_type[0]) {
                rets = (const CBMType **)pmm_arena_alloc(arena, 2 * sizeof(const CBMType *));
                if (rets) {
                ...
                while (d->return_types[n])
                    n++;
                if (n > 0) {
                    rets = (const CBMType **)pmm_arena_alloc(arena, (size_t)(n + 1) *
            ...
            rf.signature = pmm_type_func(arena, d->param_names, params_t, rets);
            ...
            if (strcmp(d->label, "Method") == 0) {
                ...
                if (dot && dot != d->qualified_name) {
                    char *recv = pmm_arena_strndup(arena, d->qualified_name,
                                                   (size_t)(dot - d->qualified_name));
                    rf.receiver_type = recv;

                    // Ensure the receiver type is registered (auto-create if missing).
                    const CBMRegisteredType *existing = pmm_registry_lookup_type(reg, recv);
                    if (!existing) {
                        ...
                        pmm_registry_add_type(reg, auto_rt);
                    }
                }
            }
    ...
    for (int ti = 0; ti < reg->type_count; ti++) {
        ...
        const char **mnames =
            (const char **)pmm_arena_alloc(arena, (size_t)(mcount + 1) * sizeof(const char *));
        const char **mqns =
            (const char **)pmm_arena_alloc(arena, (size_t)(mcount + 1) * sizeof(const char *));
        if (!mnames || !mqns)
            continue;
...
static void rebuild_signatures_from_ast(TSLSPContext *ctx, TSNode root, CBMTypeRegistry *reg) {
    ...
    while (top > 0) {
        ...
        const char *fqn = NULL;
        if (is_func) {
            fqn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
        } else {
            ...
            if (!cname)
                continue;
            fqn = pmm_arena_sprintf(ctx->arena, "%s.%s.%s", ctx->module_qn, cname, fname);
        }
        ...
        const CBMType *ret = pmm_type_unknown();
        TSNode rt_node =
        ...
        const CBMType *rets[2] = {ret, NULL};
        const CBMType *new_sig = pmm_type_func(ctx->arena, p_names, p_types, rets);
        for (int fi = 0; fi < reg->func_count; fi++) {
            if (!reg->funcs[fi].qualified_name)
                continue;
            ...
            if (pmm_type_is_unknown(ret) && reg->funcs[fi].signature &&
                reg->funcs[fi].signature->kind == PMM_TYPE_FUNC &&
                reg->funcs[fi].signature->data.func.return_types &&
                reg->funcs[fi].signature->data.func.return_types[0] &&
                !pmm_type_is_unknown(reg->funcs[fi].signature->data.func.return_types[0])) {
                const CBMType *preserved[2] = {reg->funcs[fi].signature->data.func.return_types[0],
                                               NULL};
                new_sig = pmm_type_func(ctx->arena, p_names, p_types, preserved);
            }
            reg->funcs[fi].signature = new_sig;
            break;
        }
    ...
    while (top > 0) {
        ...
        for (uint32_t i = 0; i < tnc && tpc < 15; i++) {
            ...
            if (!nm)
                continue;
            names[tpc] = nm;
            args[tpc] = pmm_type_type_param(ctx->arena, nm);
            tpc++;
        }
        names[tpc] = NULL;
        args[tpc] = NULL;
        ...
        for (int fi = 0; fi < reg->func_count; fi++) {
            ...
            if (!old_sig)
                break;
            const CBMType *new_sig = pmm_type_substitute(ctx->arena, old_sig, names, args);
            if (new_sig)
                reg->funcs[fi].signature = new_sig;
            // Also store the type_param_names for later overload-by-types use.
            const char **stored = (const char **)pmm_arena_alloc(
...
static void parse_jsdoc_block(TSLSPContext *ctx, const char *text, size_t len,
    ...
    *out_param_count = 0;
    ...
    while (p < end) {
        ...
        char *type_text = pmm_arena_strndup(ctx->arena, type_start, type_len);
        ...
        if (*out_param_count < max_params) {
            char *nm = pmm_arena_strndup(ctx->arena, name_start, name_len);
...
static void apply_jsdoc_signatures(TSLSPContext *ctx, TSNode root, CBMTypeRegistry *reg) {
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (!fname)
            continue;
        const char *fqn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
        ...
        for (int fi = 0; fi < reg->func_count; fi++) {
            ...
            const CBMType *old_sig = reg->funcs[fi].signature;
            const CBMType **rets = NULL;
            if (ret && !pmm_type_is_unknown(ret)) {
                rets = (const CBMType **)pmm_arena_alloc(ctx->arena, 2 * sizeof(const CBMType *));
                if (rets) {
                    rets[0] = ret;
                    rets[1] = NULL;
                }
            } else if (old_sig && old_sig->kind == PMM_TYPE_FUNC) {
                rets = old_sig->data.func.return_types;
            }
            const CBMType **params_arr = NULL;
            const char **param_names_arr = NULL;
            if (pcount > 0) {
                params_arr = (const CBMType **)pmm_arena_alloc(
                    ctx->arena, (size_t)(pcount + 1) * sizeof(const CBMType *));
                param_names_arr = (const char **)pmm_arena_alloc(
            ...
            reg->funcs[fi].signature = pmm_type_func(ctx->arena, param_names_arr, params_arr, rets);
            break;
        }
    }
}
...
static void infer_implicit_returns(TSLSPContext *ctx, TSNode root, CBMTypeRegistry *reg) {
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (!fname)
            continue;
        const char *fqn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
        const CBMRegisteredFunc *existing = pmm_registry_lookup_func(reg, fqn);
        if (!existing)
            continue;
        // Only infer when the existing signature lacks an explicit return type.
        if (existing->signature && existing->signature->kind == PMM_TYPE_FUNC &&
            existing->signature->data.func.return_types &&
            existing->signature->data.func.return_types[0] &&
            !pmm_type_is_unknown(existing->signature->data.func.return_types[0])) {
            continue;
        }

        const CBMType *inferred = infer_return_type_from_body(ctx, body);
        if (pmm_type_is_unknown(inferred))
            continue;
        ...
        const CBMType *new_sig = pmm_type_func(ctx->arena, param_names, param_types, rets);
...
static void ast_sweep_shapes(TSLSPContext *ctx, TSNode root, CBMTypeRegistry *reg) {
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (strcmp(k, "type_alias_declaration") == 0) {
            ...
            if (!nm)
                continue;
            const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, nm);
            ...
            if (val_text && val_text[0] && strchr(val_text, ' ') == NULL &&
                ...
                if (strchr(val_text, '.') == NULL && val_text[0] >= 'A' && val_text[0] <= 'Z') {
                    alias_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, val_text);
                } else {
                    alias_qn = pmm_arena_strdup(ctx->arena, val_text);
                }
            }
            // Find or insert.
            bool found = false;
        ...
        if (!cname)
            continue;
        const char *class_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
        ...
        for (uint32_t bi = 0; bi < bnc && field_count < 63; bi++) {
            ...
            if (strcmp(mk, "public_field_definition") == 0 ||
                ...
                if (!fnm)
                    continue;
                const CBMType *ft = pmm_type_unknown();
            ...
            if (is_interface && (strcmp(mk, "method_signature") == 0)) {
                ...
                const CBMType *rets[2] = {ret, NULL};
                const CBMType *sig = pmm_type_func(ctx->arena, NULL, NULL, rets);

                const char *mqn = pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mnm);
                iface_method_names[iface_method_count] = mnm;
                iface_method_qns[iface_method_count] = mqn;
                iface_method_sigs[iface_method_count] = sig;
                iface_method_count++;

                // Also register a CBMRegisteredFunc so pmm_registry_lookup_method picks it up.
                CBMRegisteredFunc rf;
                memset(&rf, 0, sizeof(rf));
        ...
        if (field_count > 0) {
            const char **fn_arr = (const char **)pmm_arena_alloc(
                ctx->arena, (size_t)(field_count + 1) * sizeof(const char *));
            const CBMType **ft_arr = (const CBMType **)pmm_arena_alloc(
        ...
        if (iface_method_count > 0) {
            const char **mn_arr = (const char **)pmm_arena_alloc(
                ctx->arena, (size_t)(iface_method_count + 1) * sizeof(const char *));
            const char **mq_arr = (const char **)pmm_arena_alloc(
                ctx->arena, (size_t)(iface_method_count + 1) * sizeof(const char *));
            if (mn_arr && mq_arr) {
            ...
            (void)iface_method_sigs; // sigs already attached via pmm_registry_add_func
        }
    }
}

void pmm_run_ts_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
    ...
    // Diagnostic / benchmarking knob: setting `PMM_LSP_DISABLED=1` skips the resolver.
    ...
    const char *disabled = getenv("PMM_LSP_DISABLED");
    if (disabled && disabled[0] && disabled[0] != '0')
        return;

    CBMTypeRegistry reg;
    pmm_registry_init(&reg, arena);
    pmm_ts_stdlib_register(&reg, arena);
 ...
 * loop in pmm_run_ts_lsp_cross — KEEP IN SYNC. Shared by the Tier 2
            ...
            if (d->embedded_types && d->embedded_types[0]) {
                ...
                for (const char *s = d->embedded_types; *s; s++)
                    if (*s == '|')
                        n++;
                const char **arr =
                    (const char **)pmm_arena_alloc(arena, (size_t)(n + 1) * sizeof(const char *));
                if (arr) {
                    int idx = 0;
                    const char *start = d->embedded_types;
                    for (const char *s = d->embedded_types;; s++) {
                        if (*s == '|' || *s == '\0') {
                            arr[idx++] = pmm_arena_strndup(arena, start, (size_t)(s - start));
                            if (*s == '\0')
                                break;
                            start = s + 1;
                        }
            ...
            if (d->field_defs && d->field_defs[0]) {
                ...
                if (fn_arr && ft_arr) {
                    ...
                    for (const char *s = d->field_defs;; s++) {
                        if (*s == '|' || *s == '\0') {
                            char *pair = pmm_arena_strndup(arena, start, (size_t)(s - start));
            ...
            if (d->method_names_str && d->method_names_str[0]) {
                ...
                if (mn_arr && mqn_arr) {
                    ...
                    for (const char *s = d->method_names_str;; s++) {
                        if (*s == '|' || *s == '\0') {
                            char *m = pmm_arena_strndup(arena, start, (size_t)(s - start));
                            mn_arr[idx] = m;
                            mqn_arr[idx] = pmm_arena_sprintf(arena, "%s.%s", d->qualified_name, m);
                            idx++;
                            if (*s == '\0')
                                break;
                            start = s + 1;
            ...
            if (d->return_types && d->return_types[0]) {
                ...
                if (rets) {
                    ...
                    for (const char *s = d->return_types;; s++) {
                        if (*s == '|' || *s == '\0') {
                            char *part = pmm_arena_strndup(arena, start, (size_t)(s - start));
 ...
 * Per-file overlays (built by pmm_run_ts_lsp_cross_with_registry)
 * chain to this via the registry fallback pointer. */
CBMTypeRegistry *pmm_ts_build_cross_registry(CBMArena *arena, CBMLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
    ...
    for (int i = 0; i < def_count; i++) {
        CBMLSPDef *d = &defs[i];
        if (d->lang != PMM_LANG_JAVASCRIPT && d->lang != PMM_LANG_TYPESCRIPT &&
            d->lang != PMM_LANG_TSX) {
            continue;
        }
...
void pmm_run_ts_lsp_cross_with_registry(CBMArena *arena, const char *source, int source_len,
    ...
    pmm_registry_init(&overlay, arena);
...
void pmm_run_ts_lsp_cross(CBMArena *arena, const char *source, int source_len,
...
void pmm_batch_ts_lsp_cross(CBMArena *arena, CBMBatchTSLSPFile *files, int file_count,
                            CBMResolvedCallArray *out) {
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        pmm_run_ts_lsp_cross(arena, f->source, f->source_len, f->module_qn, f->js_mode, f->jsx_mode,
