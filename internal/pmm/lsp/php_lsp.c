...
static char *php_ns_to_dot(CBMArena *a, const char *ns) {
    if (!ns)
        return NULL;
    size_t n = strlen(ns);
    char *out = (char *)pmm_arena_alloc(a, n + 1);
    if (!out)
        return NULL;
...
void php_lsp_add_use(PHPLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->use_count >= ctx->use_cap) {
        ...
        const char **nl = (const char **)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nl));
        const char **nq = (const char **)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nq));
        int *nk = (int *)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(int));
        if (!nl || !nq || !nk)
            return;
    ...
    ctx->use_target_qns[ctx->use_count] = pmm_arena_strdup(ctx->arena, target_qn);
...
const char *php_resolve_class_name(PHPLSPContext *ctx, const char *name) {
    ...
    char *first;
    if (*sep) {
        first = pmm_arena_strndup(ctx->arena, name, (size_t)(sep - name));
    } else {
        first = pmm_arena_strdup(ctx->arena, name);
    }

    const char *use_target = find_use(ctx, first, PMM_PHP_USE_CLASS);
    if (use_target) {
        if (*sep) {
            /* use App\\Models as M;  M\\User -> App.Models.User */
            return pmm_arena_sprintf(ctx->arena, "%s%s", use_target,
    ...
    if (ctx->module_qn && ctx->module_qn[0]) {
        return pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
...
static const CBMRegisteredType *lookup_type_with_project(PHPLSPContext *ctx, const char *qn) {
    if (!qn)
        return NULL;
    const CBMRegisteredType *t = pmm_registry_lookup_type(ctx->registry, qn);
    if (t)
        return t;

    if (ctx->module_qn && ctx->module_qn[0]) {
        const char *combo = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
        t = pmm_registry_lookup_type(ctx->registry, combo);
        if (t)
            return t;
        ...
        while (last_dot) {
            char *prefix = pmm_arena_strndup(ctx->arena, m, (size_t)(last_dot - m));
            const char *try_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", prefix, qn);
            t = pmm_registry_lookup_type(ctx->registry, try_qn);
            if (t)
                return t;
...
const CBMRegisteredFunc *php_lookup_method(PHPLSPContext *ctx, const char *class_qn,
    ...
    const CBMRegisteredFunc *f = pmm_registry_lookup_method(ctx->registry, class_qn, method_name);
    if (f)
        return f;

    /* If the QN we have isn't the registry's canonical key, resolve the
     * type to its registered identity and retry. */
    const CBMRegisteredType *t = pmm_registry_lookup_type(ctx->registry, class_qn);
    if (!t)
        t = lookup_type_with_project(ctx, class_qn);
    if (!t)
        return NULL;
    if (strcmp(t->qualified_name, class_qn) != 0) {
        f = pmm_registry_lookup_method(ctx->registry, t->qualified_name, method_name);
        if (f)
            return f;
    }
    ...
    while (frontier_count > 0 && visited_count < 32) {
        ...
        f = pmm_registry_lookup_method(ctx->registry, parent, method_name);
        if (f)
            return f;

        const CBMRegisteredType *next = pmm_registry_lookup_type(ctx->registry, parent);
        if (!next)
        ...
        if (strcmp(next->qualified_name, parent) != 0) {
            f = pmm_registry_lookup_method(ctx->registry, next->qualified_name, method_name);
            if (f)
                return f;
        }
            ...
            for (int i = 0; next->method_names[i]; i++) {
                if (strcmp(next->method_names[i], method_name) == 0) {
                    const CBMRegisteredFunc *cand =
                        pmm_registry_lookup_func(ctx->registry, next->method_qns[i]);
                    if (cand)
                        return cand;
                }
            }
...
const CBMType *php_parse_type_node(PHPLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "primitive_type") == 0) {
        ...
        return pmm_type_builtin(ctx->arena, t ? t : "mixed");
    }
    if (strcmp(kind, "named_type") == 0 || strcmp(kind, "qualified_name") == 0 ||
        ...
        if (!resolved)
            return pmm_type_unknown();
        return pmm_type_named(ctx->arena, resolved);
    }
    ...
    if (strcmp(kind, "union_type") == 0 || strcmp(kind, "intersection_type") == 0 ||
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (t && t->kind != PMM_TYPE_UNKNOWN) {
                if (t->kind == PMM_TYPE_BUILTIN && strcmp(t->data.builtin.name, "null") == 0) {
                    continue;
                }
                return t;
            }
    ...
    if (php_is_builtin_type_name(t))
        return pmm_type_builtin(ctx->arena, t);
    const char *resolved = php_resolve_class_name(ctx, t);
    if (!resolved)
        return pmm_type_unknown();
    return pmm_type_named(ctx->arena, resolved);
...
static int parse_phpdoc_template_args(PHPLSPContext *ctx, const char *p, const CBMType ***out_args,
    ...
    int count = 0;
    const CBMType **buf =
        (const CBMType **)pmm_arena_alloc(ctx->arena, (size_t)(cap + 1) * sizeof(*buf));
    if (!buf) {
        *out_args = NULL;
        *out_close = p;
        return 0;
    ...
    while (*p && depth > 0) {
        ...
        if (arg_len > 0) {
            char *arg_text = pmm_arena_strndup(ctx->arena, arg_start, arg_len);
            const CBMType *arg_type = resolve_phpdoc_type(ctx, arg_text);
            if (count >= cap) {
                int new_cap = cap * 2;
                const CBMType **new_buf = (const CBMType **)pmm_arena_alloc(
...
static void php_add_phpstan_alias(PHPLSPContext *ctx, const char *name, const CBMType *type) {
    ...
    if (ctx->phpstan_alias_count >= ctx->phpstan_alias_cap) {
        ...
        const char **nn =
            (const char **)pmm_arena_alloc(ctx->arena, (size_t)(new_cap + 1) * sizeof(*nn));
        const CBMType **nt =
            (const CBMType **)pmm_arena_alloc(ctx->arena, (size_t)(new_cap + 1) * sizeof(*nt));
        if (!nn || !nt)
            return;
        for (int i = 0; i < ctx->phpstan_alias_count; i++) {
    ...
    ctx->phpstan_alias_names[ctx->phpstan_alias_count] = pmm_arena_strdup(ctx->arena, name);
...
static const CBMType *resolve_phpdoc_type(PHPLSPContext *ctx, const char *type_text) {
    ...
    if (end == 0)
        return pmm_type_unknown();
    char *first = pmm_arena_strndup(ctx->arena, type_text, end);
    if (!first)
        return pmm_type_unknown();
    ...
    if (has_array_shape)
        return pmm_type_builtin(ctx->arena, "array");
    ...
    if (php_is_builtin_type_name(first)) {
        if (has_generic) {
            ...
            return pmm_type_template(ctx->arena, first, args, n);
        }
        return pmm_type_builtin(ctx->arena, first);
    }
    ...
    if (has_generic) {
        ...
        const char *after = NULL;
        int n = parse_phpdoc_template_args(ctx, type_text + end + 1, &args, &after);
        return pmm_type_template(ctx->arena, resolved, args, n);
    }
    return pmm_type_named(ctx->arena, resolved);
}
...
static void parse_phpdoc_for_params(PHPLSPContext *ctx, const char *docstring, TSNode params) {
    ...
    while ((p = strstr(p, "@param")) != NULL) {
        ...
        if (p == type_start)
            continue;
        char *type_text = pmm_arena_strndup(ctx->arena, type_start, (size_t)(p - type_start));
        while (*p == ' ' || *p == '\t')
            p++;
        if (*p != '$')
            continue;
        ...
        if (p == name_start)
            continue;
        char *vname = pmm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        const CBMType *t = resolve_phpdoc_type(ctx, type_text);
        if (vname && t && t->kind != PMM_TYPE_UNKNOWN) {
            pmm_scope_bind(ctx->current_scope, vname, t);
        }
    }
}
...
const CBMType *php_eval_expr_type(PHPLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "variable_name") == 0) {
        ...
        if (t) {
            ...
            if (strcmp(name, "this") == 0) {
                ...
                const CBMType *bound = pmm_scope_lookup(ctx->current_scope, name);
                if (bound)
                    result = bound;
            }
        }
        ...
        if (!ts_node_is_null(lhs) && !ts_node_is_null(rhs)) {
            ...
            if (cls_text && member) {
                ...
                if (cls_qn) {
                    const CBMRegisteredType *t = pmm_registry_lookup_type(ctx->registry, cls_qn);
                    if (!t)
        ...
        if (!ts_node_is_null(obj)) {
            ...
            if (recv && recv->kind == PMM_TYPE_TEMPLATE) {
            ...
            } else if (recv && recv->kind == PMM_TYPE_NAMED) {
                ...
                if (og && og->signature && og->signature->kind == PMM_TYPE_FUNC &&
    ...
    match_done:;
        ...
        if ((!result || result->kind == PMM_TYPE_UNKNOWN) && !ts_node_is_null(else_n)) {
        ...
        result = pmm_type_builtin(ctx->arena, "int");
    } else if (strcmp(kind, "float") == 0) {
        result = pmm_type_builtin(ctx->arena, "float");
    } else if (strcmp(kind, "boolean") == 0 || strcmp(kind, "true") == 0 ||
               strcmp(kind, "false") == 0) {
        result = pmm_type_builtin(ctx->arena, "bool");
        ...
        if (!ts_node_is_null(obj) && !ts_node_is_null(field)) {
            const CBMType *recv = php_eval_expr_type(ctx, obj);
            if (recv && recv->kind == PMM_TYPE_NAMED) {
                char *fname = php_node_text(ctx, field);
                const CBMRegisteredType *t =
                    pmm_registry_lookup_type(ctx->registry, recv->data.named.qualified_name);
                if (!t)
...
static const CBMType *eval_function_call_type(PHPLSPContext *ctx, TSNode call_node) {
    ...
    const char *target = find_use(ctx, name, PMM_PHP_USE_FUNCTION);
    const char *qn = NULL;
    if (target) {
        qn = target;
    } else if (ctx->current_namespace_qn && ctx->current_namespace_qn[0]) {
        qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace_qn, name);
    } else {
        qn = pmm_arena_strdup(ctx->arena, name);
    }
    const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, qn);
    if (!f && ctx->module_qn) {
        f = pmm_registry_lookup_func(ctx->registry,
    ...
    if (sig->kind == PMM_TYPE_FUNC && sig->data.func.return_types &&
...
static const CBMType *eval_member_call_type(PHPLSPContext *ctx, TSNode call_node) {
    ...
    if (is_static) {
        ...
        const CBMType *recv_type =
            ts_node_is_null(recv_node) ? pmm_type_unknown() : php_eval_expr_type(ctx, recv_node);
        if (recv_type && recv_type->kind == PMM_TYPE_NAMED) {
            class_qn = recv_type->data.named.qualified_name;
        } else if (recv_type && recv_type->kind == PMM_TYPE_TEMPLATE) {
            class_qn = recv_type->data.template_type.template_name;
        }
        if (recv_type && recv_type->kind == PMM_TYPE_TEMPLATE) {
            ...
            if (sig->kind == PMM_TYPE_FUNC && sig->data.func.return_types &&
                sig->data.func.return_types[0]) {
                const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, class_qn);
                if (!rt)
...
static void process_assignment(PHPLSPContext *ctx, TSNode node) {
    ...
    if (rhs_type && rhs_type->kind != PMM_TYPE_UNKNOWN) {
        /* Don't override a more-specific (NAMED/TEMPLATE) binding with
         * a trivial null literal — preserves `@var T $x; $x = null;`. */
        if (rhs_type->kind == PMM_TYPE_BUILTIN && rhs_type->data.builtin.name &&
            strcmp(rhs_type->data.builtin.name, "null") == 0) {
            const CBMType *existing = pmm_scope_lookup(ctx->current_scope, name);
            if (existing &&
                (existing->kind == PMM_TYPE_NAMED || existing->kind == PMM_TYPE_TEMPLATE)) {
                return;
            }
        }
        pmm_scope_bind(ctx->current_scope, name, rhs_type);
    }
}
...
static void process_foreach(PHPLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(iterable)) {
        ...
        if (it_type && it_type->kind == PMM_TYPE_TEMPLATE) {
    ...
    for (int i = 0; i < loop_var_count; i++) {
        ...
        if (!t)
            continue;
        const char *name = (t[0] == '$') ? t + 1 : t;
        const CBMType *bind_type = (i == loop_var_count - 1) ? elem_type : pmm_type_unknown();
        pmm_scope_bind(ctx->current_scope, name, bind_type);
    }
}
...
static void process_catch(PHPLSPContext *ctx, TSNode node) {
    ...
    const CBMType *exc_type = pmm_type_unknown();
    ...
    pmm_scope_bind(ctx->current_scope, vname, exc_type);
}
...
static void resolve_function_call(PHPLSPContext *ctx, TSNode call) {
    ...
    const char *target = find_use(ctx, name, PMM_PHP_USE_FUNCTION);
    if (target) {
        const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, target);
        if (f) {
    ...
    if (ctx->current_namespace_qn && ctx->current_namespace_qn[0]) {
        const char *ns_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace_qn, name);
        const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, ns_qn);
        if (f) {
            emit_resolved(ctx, f->qualified_name, "php_function_namespaced", 0.95f);
            return;
        }
...
static void resolve_member_call(PHPLSPContext *ctx, TSNode call) {
    ...
    if (!recv)
        return;
    const char *class_qn = NULL;
    if (recv->kind == PMM_TYPE_NAMED) {
        class_qn = recv->data.named.qualified_name;
    } else if (recv->kind == PMM_TYPE_TEMPLATE) {
    ...
    if (class_has_magic_call(ctx, class_qn, false)) {
        emit_resolved_reason(ctx, pmm_arena_sprintf(ctx->arena, "%s.__call", class_qn),
    ...
    emit_resolved(ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, method_name),
...
static int parse_narrowing_one(PHPLSPContext *ctx, TSNode cond, php_narrowing_t *out) {
    ...
    if (strcmp(kind, "binary_expression") == 0) {
        ...
        if (opt && strcmp(opt, "instanceof") == 0) {
            ...
            if (!resolved)
                return false;
            out->var_name = pmm_arena_strdup(ctx->arena, vname);
            out->type = pmm_type_named(ctx->arena, resolved);
            return true;
        }
    ...
    if (strcmp(kind, "function_call_expression") == 0) {
        ...
        for (uint32_t i = 0; i < anc; i++) {
            ...
            const char *vname = (vt[0] == '$') ? vt + 1 : vt;
            out->var_name = pmm_arena_strdup(ctx->arena, vname);
            out->type = pmm_type_builtin(ctx->arena, builtin);
            return true;
        }
    }
    return false;
...
static void apply_assert_narrowing(PHPLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < anc; i++) {
        ...
        if (parse_narrowing(ctx, predicate, &nw) && nw.var_name && nw.type) {
            pmm_scope_bind(ctx->current_scope, nw.var_name, nw.type);
        }
        return;
    }
}
...
static void walk_with_narrowings(PHPLSPContext *ctx, TSNode body, const php_narrowing_t *nws,
    ...
    for (int i = 0; i < nw_count; i++) {
        if (nws[i].var_name && nws[i].type) {
            pmm_scope_bind(ctx->current_scope, nws[i].var_name, nws[i].type);
        }
    }
    php_resolve_calls_in_node(ctx, body);
...
static void process_if_statement(PHPLSPContext *ctx, TSNode node) {
    ...
    if (body_is_terminator && has_neg_nw && neg_nw.var_name && neg_nw.type) {
        pmm_scope_bind(ctx->current_scope, neg_nw.var_name, neg_nw.type);
    }
}
...
static void bind_typed_parameters(PHPLSPContext *ctx, TSNode params) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const CBMType *ptype = pmm_type_unknown();
        if (!ts_node_is_null(tnode))
        ...
        if (strcmp(pk, "variadic_parameter") == 0) {
            ptype = pmm_type_builtin(ctx->arena, "array");
        }
        pmm_scope_bind(ctx->current_scope, name, ptype);
    }
}
            ...
            if (fname) {
                if (is_method && ctx->enclosing_class_qn) {
                    ctx->enclosing_func_qn =
                        pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, fname);
                } else if (ctx->module_qn) {
                    ctx->enclosing_func_qn =
                        pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
                } else {
                    ctx->enclosing_func_qn = pmm_arena_strdup(ctx->arena, fname);
                }
            }
        }
    }
    ...
    if (strcmp(kind, "anonymous_function") == 0) {
        ...
        for (uint32_t i = 0; i < fnc; i++) {
            ...
            for (uint32_t j = 0; j < unc; j++) {
                ...
                const CBMType *captured = pmm_scope_lookup(saved_scope, vname);
                if (captured) {
                    pmm_scope_bind(ctx->current_scope, vname, captured);
                }
            }
        }
    }
...
static void process_class_decl(PHPLSPContext *ctx, TSNode node) {
    ...
    if (ctx->module_qn) {
        ...
        ctx->enclosing_class_qn = pmm_arena_strdup(ctx->arena, cname);
    }
...
static void collect_use_declaration(PHPLSPContext *ctx, TSNode use_decl) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(k, "function") == 0)
            kind = PMM_PHP_USE_FUNCTION;
        if (strcmp(k, "const") == 0)
            kind = PMM_PHP_USE_CONST;
    }
    for (uint32_t i = 0; i < nc; i++) {
        TSNode c = ts_node_child(use_decl, i);
        if (ts_node_is_null(c) || !ts_node_is_named(c))
...
static const CBMType *parse_return_type_text(CBMArena *arena, const char *text,
    ...
    char *first;
    if (cut) {
        first = pmm_arena_strndup(arena, text, (size_t)(cut - text));
    } else {
        first = pmm_arena_strdup(arena, text);
    }
    if (!first)
        return pmm_type_unknown();
    /* trim. */
    ...
    *end = '\0';
    if (!*first || strcmp(first, "null") == 0)
        return pmm_type_unknown();
    if (php_is_builtin_type_name(first))
        return pmm_type_builtin(arena, first);

    /* Resolve via use map. */
    if (first[0] == '\\') {
        return pmm_type_named(arena, php_ns_to_dot(arena, first + 1));
    }
    /* Find first segment for use lookup. */
    const char *bs = first;
    while (*bs && *bs != '\\')
        bs++;
    char *first_seg = (*bs) ? pmm_arena_strndup(arena, first, (size_t)(bs - first))
                            : pmm_arena_strdup(arena, first);
    for (int i = 0; i < use_count; i++) {
        if (strcmp(use_local_names[i], first_seg) == 0) {
            const char *target = use_target_qns[i];
            if (*bs) {
                return pmm_type_named(
                    arena, pmm_arena_sprintf(arena, "%s%s", target, php_ns_to_dot(arena, bs)));
            }
            return pmm_type_named(arena, target);
        }
    }
    /* Fallback: current namespace prefix. */
    if (current_namespace_qn && *current_namespace_qn) {
        return pmm_type_named(arena, pmm_arena_sprintf(arena, "%s.%s", current_namespace_qn,
                                                       php_ns_to_dot(arena, first)));
    }
    if (module_qn) {
        return pmm_type_named(
            arena, pmm_arena_sprintf(arena, "%s.%s", module_qn, php_ns_to_dot(arena, first)));
    }
    return pmm_type_named(arena, php_ns_to_dot(arena, first));
}
...
static php_class_fields_t *fields_for(php_class_field_table_t *tab, const char *class_qn,
    ...
    if (tab->count >= tab->cap) {
        ...
        php_class_fields_t *next =
            (php_class_fields_t *)pmm_arena_alloc(arena, (size_t)new_cap * sizeof(*next));
        if (!next)
            return NULL;
        for (int i = 0; i < tab->count; i++)
            next[i] = tab->items[i];
    ...
    memset(f, 0, sizeof(*f));
    f->class_qn = pmm_arena_strdup(arena, class_qn);
    return f;
}
...
static void add_field(php_class_fields_t *f, CBMArena *arena, const char *name,
    ...
    if (f->count >= f->cap) {
        ...
        const char **nn =
            (const char **)pmm_arena_alloc(arena, (size_t)(new_cap + 1) * sizeof(*nn));
        const CBMType **nt =
            (const CBMType **)pmm_arena_alloc(arena, (size_t)(new_cap + 1) * sizeof(*nt));
        if (!nn || !nt)
            return;
        for (int i = 0; i < f->count; i++) {
    ...
    f->field_names[f->count] = pmm_arena_strdup(arena, name);
...
static const char *class_qn_for_node(PHPLSPContext *ctx, TSNode class_node) {
    ...
    if (!cname)
        return NULL;
    if (ctx->module_qn) {
        return pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    }
    return pmm_arena_strdup(ctx->arena, cname);
}
...
static void extract_property_decl(PHPLSPContext *ctx, TSNode prop_decl, php_class_fields_t *out) {
    ...
    if (!ts_node_is_null(tnode))
        ptype = php_parse_type_node(ctx, tnode);
    if (!ptype || ptype->kind == PMM_TYPE_UNKNOWN) {
...
static void extract_ctor_assignments(PHPLSPContext *ctx, TSNode ctor_method,
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < nc && pc < 32; i++) {
            ...
            if (!vt)
                continue;
            names[pc] = pmm_arena_strdup(ctx->arena, (vt[0] == '$') ? vt + 1 : vt);
            types[pc] = ptype;
            pc++;
        }
    }
...
static const char **parse_phpdoc_template_params(PHPLSPContext *ctx, const char *docstring) {
    ...
    while ((p = strstr(p, "@template")) != NULL) {
        ...
        if (p == name_start)
            continue;
        if (n < 16) {
            names[n++] = pmm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        }
    }
    if (n == 0)
        return NULL;
    const char **out = (const char **)pmm_arena_alloc(ctx->arena, (size_t)(n + 1) * sizeof(*out));
    if (!out)
        return NULL;
    for (int i = 0; i < n; i++)
        out[i] = names[i];
...
static const CBMType *php_substitute_template(CBMArena *arena, const CBMType *t,
    ...
    if (t->kind == PMM_TYPE_TEMPLATE) {
        ...
        const CBMType **new_args =
            (const CBMType **)pmm_arena_alloc(arena, (size_t)(n + 1) * sizeof(*new_args));
        if (!new_args)
            return t;
        for (int i = 0; i < n; i++) {
...
static void apply_phpdoc_class_tags(PHPLSPContext *ctx, CBMTypeRegistry *reg, const char *class_qn,
        ...
        for (int pi = 0; prefixes[pi]; pi++) {
            ...
            while ((p = strstr(p, prefixes[pi])) != NULL) {
                ...
                if (p == name_start)
                    continue;
                char *alias_name =
                    pmm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
                while (*p == ' ' || *p == '\t' || *p == '=')
                    p++;
                ...
                if (p == type_start)
                    continue;
                char *type_text =
                    pmm_arena_strndup(ctx->arena, type_start, (size_t)(p - type_start));
                ...
                if (aliased && aliased->kind != PMM_TYPE_UNKNOWN) {
     ...
     * inherited methods at least resolve (untyped). */
    {
        ...
        while ((p = strstr(p, "@extends")) != NULL) {
            ...
            if (p == parent_start)
                continue;
            char *parent_text =
                pmm_arena_strndup(ctx->arena, parent_start, (size_t)(p - parent_start));
            const CBMType *parent_type = resolve_phpdoc_type(ctx, parent_text);
            const char *parent_qn = NULL;
            if (parent_type && parent_type->kind == PMM_TYPE_NAMED) {
                parent_qn = parent_type->data.named.qualified_name;
            } else if (parent_type && parent_type->kind == PMM_TYPE_TEMPLATE) {
            ...
            for (int t = 0; t < reg->type_count; t++) {
                ...
                if (found)
                    break;
                const char **expanded = (const char **)pmm_arena_alloc(
    ...
    while ((p = strstr(p, "@property")) != NULL) {
        ...
        if (p == name_start)
            continue;
        char *fname = pmm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        const CBMType *t = resolve_phpdoc_type(ctx, type_text);
        if (out_fields && fname && t && t->kind != PMM_TYPE_UNKNOWN) {
    ...
    while ((p = strstr(p, "@method")) != NULL) {
        ...
        if (p == name_start)
            continue;
        char *mname = pmm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));

        /* Register as a virtual function whose receiver_type is class_qn.
         * Generates a synthetic QN class_qn + "." + mname. */
        CBMRegisteredFunc rf;
        memset(&rf, 0, sizeof(rf));
        rf.qualified_name = pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mname);
        rf.short_name = mname;
        rf.receiver_type = class_qn;
        rf.min_params = -1;
        const CBMType *ret = resolve_phpdoc_type(ctx, type_text);
        const CBMType **rets = (const CBMType **)pmm_arena_alloc(ctx->arena, 2 * sizeof(*rets));
        if (rets) {
            rets[0] = ret;
            rets[1] = NULL;
        }
...
static void flatten_trait_into_class(PHPLSPContext *ctx, CBMTypeRegistry *reg, const char *class_qn,
    ...
    const CBMRegisteredType *t = pmm_registry_lookup_type(reg, trait_qn);
    if (!t)
     ...
     * then re-matches the loop's filter while pmm_registry_add_func() keeps
     ...
     *
     * Snapshot the count before iterating: pmm_registry_add_func() below
    ...
    for (int i = 0; i < func_count_before; i++) {
        ...
        CBMRegisteredFunc rf;
        memset(&rf, 0, sizeof(rf));
        rf.qualified_name = pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, new_short);
        rf.short_name = pmm_arena_strdup(ctx->arena, new_short);
        rf.receiver_type = class_qn;
        ...
        if (src->signature && src->signature->kind == PMM_TYPE_FUNC &&
            src->signature->data.func.return_types) {
            const CBMType *ret0 = src->signature->data.func.return_types[0];
            if (ret0 && ret0->kind == PMM_TYPE_NAMED && ret0->data.named.qualified_name &&
                strcmp(ret0->data.named.qualified_name, canonical_trait_qn) == 0) {
                const CBMType **rets =
                    (const CBMType **)pmm_arena_alloc(ctx->arena, 2 * sizeof(*rets));
                if (rets) {
                    rets[0] = pmm_type_named(ctx->arena, class_qn);
                    rets[1] = NULL;
                    new_sig = pmm_type_func(ctx->arena, NULL, NULL, rets);
                }
            }
        }
        ...
        if (strcmp(k, "property_declaration") == 0) {
            ...
            if (!ts_node_is_null(case_name)) {
                ...
                if (cn) {
                     ...
                     * handles below. */
                    add_field(f, ctx->arena, cn, pmm_type_named(ctx->arena, cqn));
                }
            }
            ...
            if (!ts_node_is_null(rt_node) && mn) {
                ...
                ctx->enclosing_class_qn = saved_class;
                ctx->enclosing_parent_qn = saved_parent;
                if (parsed && parsed->kind != PMM_TYPE_UNKNOWN) {
                ...
                if (p) {
                    ...
                    if (p > type_start) {
                        ...
                        if (strcmp(type_text, "$this") == 0 || strcmp(type_text, "static") == 0 ||
                            ...
                            ret_t = pmm_type_named(ctx->arena, cqn);
                        }
                            ...
                            for (int t = 0; t < reg->type_count; t++) {
                                ...
                                for (int j = 0; reg->types[t].type_param_names[j]; j++) {
                                    if (strcmp(reg->types[t].type_param_names[j], type_text) == 0) {
                                        ret_t = pmm_type_named(ctx->arena, type_text);
                                        break;
                                    }
                                }
                                break;
                        ...
                        if (ret_t && ret_t->kind != PMM_TYPE_UNKNOWN) {
...
/* ── entry: pmm_run_php_lsp ─────────────────────────────────────── */

void pmm_run_php_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
                     TSNode root) {
    ...
    pmm_php_stdlib_register(&reg, arena);
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (base[0] != '\\' && !strchr(base, '.')) {
                                qualified = pmm_arena_sprintf(arena, "%s.%s", module_qn, base);
                            } else if (base[0] == '\\') {
     ...
     * for $this->prop and $obj->prop sees the field types. */
    {
        ...
        /* Collect top-level children once (O(n)); ts_node_child(root,i) is O(i) → O(n²). */
        uint32_t pkn = 0;
        TSNode *pkids = pmm_lsp_collect_children(ctx.arena, root, &pkn);
...
void pmm_run_php_lsp_cross(CBMArena *arena, const char *source, int source_len,
    ...
    for (int i = 0; i < import_count; i++) {
        if (import_names && import_qns && import_names[i] && import_qns[i]) {
            php_lsp_add_use(&ctx, import_names[i], import_qns[i], PMM_PHP_USE_CLASS);
        }
    }

    /* Class field collection — same flow as pmm_run_php_lsp. Walks the AST
...
void pmm_batch_php_lsp_cross(CBMArena *arena, CBMBatchPHPLSPFile *files, int file_count,
                             CBMResolvedCallArray *out) {
    ...
    for (int f = 0; f < file_count; f++) {
        ...
        pmm_run_php_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
