...
static char *c_node_text(CLSPContext *ctx, TSNode node) {
    return pmm_node_text(ctx->arena, node, ctx->source);
}
...
void c_lsp_init(CLSPContext *ctx, CBMArena *arena, const char *source, int source_len,
    ...
    ctx->current_scope = pmm_scope_push(arena, NULL);

    const char *debug_env = getenv("PMM_LSP_DEBUG");
...
void c_lsp_add_include(CLSPContext *ctx, const char *header_path, const char *ns_qn) {
    if (ctx->include_count % 32 == 0) {
        ...
        const char **new_paths =
            (const char **)pmm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char *));
        const char **new_qns =
            (const char **)pmm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char *));
    ...
    ctx->include_paths[ctx->include_count] = pmm_arena_strdup(ctx->arena, header_path);
    ctx->include_ns_qns[ctx->include_count] = pmm_arena_strdup(ctx->arena, ns_qn);
    ctx->include_count++;
}
...
static void c_add_using_namespace(CLSPContext *ctx, const char *ns_qn) {
    if (ctx->using_ns_count >= ctx->using_ns_cap) {
        ...
        const char **new_arr =
            (const char **)pmm_arena_alloc(ctx->arena, new_cap * sizeof(const char *));
        if (!new_arr)
            return;
    ...
    ctx->using_namespaces[ctx->using_ns_count++] = pmm_arena_strdup(ctx->arena, ns_qn);
}
...
static void c_add_using_decl(CLSPContext *ctx, const char *short_name, const char *full_qn) {
    ...
    ctx->using_decl_names[ctx->using_decl_count] = pmm_arena_strdup(ctx->arena, short_name);
    ctx->using_decl_qns[ctx->using_decl_count] = pmm_arena_strdup(ctx->arena, full_qn);
...
static void c_add_ns_alias(CLSPContext *ctx, const char *alias, const char *full_qn) {
    ...
    ctx->ns_alias_names[ctx->ns_alias_count] = pmm_arena_strdup(ctx->arena, alias);
    ctx->ns_alias_qns[ctx->ns_alias_count] = pmm_arena_strdup(ctx->arena, full_qn);
    ctx->ns_alias_count++;
}
...
static void c_add_fp_target(CLSPContext *ctx, const char *var_name, const char *target_qn) {
    ...
    ctx->fp_var_names[ctx->fp_count] = pmm_arena_strdup(ctx->arena, var_name);
    ctx->fp_target_qns[ctx->fp_count] = pmm_arena_strdup(ctx->arena, target_qn);
    ctx->fp_count++;
}
    ...
    *out_has_cast = false;
    ...
    for (uint32_t i = 0; i < anc; i++) {
        ...
        if (len < 2 || text[0] != '"' || text[len - 1] != '"')
            continue;
        char *name = pmm_arena_strndup(ctx->arena, text + 1, len - 2);
        if (!name || !name[0])
            continue;
...
static void c_add_pending_template_call(CLSPContext *ctx, const char *func_qn,
    ...
    if (ctx->pending_tc_count >= ctx->pending_tc_cap) {
        ...
        void *new_arr = pmm_arena_alloc(ctx->arena, sz);
        if (!new_arr)
            return;
    ...
    const char *func_qn_copy = pmm_arena_strdup(ctx->arena, func_qn);
    const char *type_param_copy = pmm_arena_strdup(ctx->arena, type_param);
    const char *method_name_copy = pmm_arena_strdup(ctx->arena, method_name);
...
static void c_resolve_pending_template_calls(CLSPContext *ctx, const CBMRegisteredFunc *callee,
    ...
    if (callee->signature && callee->signature->kind == PMM_TYPE_FUNC &&
        ...
        for (int i = 0; i < limit; i++) {
            ...
            while (formal &&
                   (formal->kind == PMM_TYPE_REFERENCE || formal->kind == PMM_TYPE_RVALUE_REF ||
                    formal->kind == PMM_TYPE_POINTER)) {
                formal = (formal->kind == PMM_TYPE_POINTER) ? formal->data.pointer.elem
                                                            : formal->data.reference.elem;
            }
            if (formal && formal->kind == PMM_TYPE_TYPE_PARAM) {
...
static const CBMType **c_extract_call_arg_types(CLSPContext *ctx, TSNode call_node,
    ...
    const CBMType **types =
        (const CBMType **)pmm_arena_alloc(ctx->arena, (nc + 1) * sizeof(const CBMType *));
    if (!types) {
        *out_count = 0;
        return NULL;
    }
...
static void c_parse_template_params(CLSPContext *ctx, TSNode template_decl) {
    ...
    const char **names =
        (const char **)pmm_arena_alloc(ctx->arena, (nc + 1) * sizeof(const char *));
    ...
    for (uint32_t i = 0; i < nc && idx < 16; i++) {
        ...
        if (strcmp(pk, "type_parameter_declaration") == 0 ||
            ...
            if (!pname)
                pname = pmm_arena_sprintf(ctx->arena, "T%d", idx);
...
static const char *c_build_qn(CLSPContext *ctx, const char *text) {
    ...
    size_t len = strlen(text);
    char *buf = (char *)pmm_arena_alloc(ctx->arena, len + 1);
    if (!buf)
        return text;
    size_t j = 0;
...
static const char *c_resolve_name(CLSPContext *ctx, const char *name) {
    ...
    // 1. Scope lookup
    const CBMType *t = pmm_scope_lookup(ctx->current_scope, name);
    if (!pmm_type_is_unknown(t))
    ...
    if (ctx->enclosing_class_qn) {
        const CBMRegisteredFunc *f =
            pmm_registry_lookup_method(ctx->registry, ctx->enclosing_class_qn, name);
        if (f)
    ...
    for (int i = 0; i < ctx->using_ns_count; i++) {
        const CBMRegisteredFunc *f =
            pmm_registry_lookup_symbol(ctx->registry, ctx->using_namespaces[i], name);
        if (f)
            return f->qualified_name;
    }
    ...
    if (ctx->current_namespace) {
        ...
        while (ns && ns[0]) {
            const CBMRegisteredFunc *f = pmm_registry_lookup_symbol(ctx->registry, ns, name);
            if (f)
                return f->qualified_name;
            // Walk up: "a.b.c" -> "a.b" -> "a"
            ...
            if (!dot)
                break;
            char *parent = (char *)pmm_arena_alloc(ctx->arena, (size_t)(dot - ns) + 1);
            if (!parent)
                break;
    ...
    if (ctx->module_qn) {
        const CBMRegisteredFunc *f =
            pmm_registry_lookup_symbol(ctx->registry, ctx->module_qn, name);
        if (f)
            return f->qualified_name;
    }

    // 7. Global scope (no prefix — C stdlib functions)
    const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, name);
    if (f)
        return f->qualified_name;
...
static const char *extract_namespace_from_qn(CBMArena *arena, const char *qn) {
    ...
    char *ns = (char *)pmm_arena_alloc(arena, len + 1);
    if (!ns)
        return NULL;
    memcpy(ns, qn, len);
    ns[len] = '\0';
...
static const char *c_adl_resolve(CLSPContext *ctx, const char *name, TSNode call_node) {
    ...
    for (int i = 0; i < ns_count; i++) {
        const CBMRegisteredFunc *f = pmm_registry_lookup_symbol(ctx->registry, namespaces[i], name);
        if (!f && ctx->module_qn) {
            const char *prefixed =
                pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, namespaces[i]);
            f = pmm_registry_lookup_symbol(ctx->registry, prefixed, name);
        }
        if (f)
            return f->qualified_name;
    }
...
static const char *c_resolve_name_to_func_qn(CLSPContext *ctx, const char *name) {
    ...
    // Global scope
    const CBMRegisteredFunc *gf = pmm_registry_lookup_func(ctx->registry, name);
    if (gf)
...
static const CBMType *c_resolve_name_to_type(CLSPContext *ctx, const char *name) {
    ...
    if (is_c_builtin_type(name))
        return pmm_type_builtin(ctx->arena, name);

    // Scope lookup: typedef/using aliases are bound here.
    // If the name resolves to an alias, follow it to the underlying type.
    {
        const CBMType *scoped = pmm_scope_lookup(ctx->current_scope, name);
        if (scoped && scoped->kind == PMM_TYPE_ALIAS) {
            const CBMType *resolved = pmm_type_resolve_alias(scoped);
            if (resolved && !pmm_type_is_unknown(resolved))
                return resolved;
        }
    }
    ...
    if (ctx->in_template && ctx->template_param_names) {
        for (int i = 0; i < ctx->template_param_count; i++) {
            if (ctx->template_param_names[i] && strcmp(ctx->template_param_names[i], name) == 0) {
                ...
                return pmm_type_type_param(ctx->arena, name);
            }
        }
    }
    ...
    for (int i = 0; i < ctx->using_decl_count; i++) {
        if (strcmp(ctx->using_decl_names[i], name) == 0) {
            const CBMRegisteredType *rt =
                pmm_registry_lookup_type(ctx->registry, ctx->using_decl_qns[i]);
            if (rt)
                return pmm_type_named(ctx->arena, rt->qualified_name);
        }
    }

    // Enclosing class (nested types: Factory::Product)
    if (ctx->enclosing_class_qn) {
        const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, name);
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return pmm_type_named(ctx->arena, qn);
    }

    // Current namespace
    if (ctx->current_namespace) {
        const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, name);
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return pmm_type_named(ctx->arena, qn);
    }
    ...
    // Module scope
    if (ctx->module_qn) {
        const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name);
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return pmm_type_named(ctx->arena, qn);
    }
    ...
    // Using namespaces
    for (int i = 0; i < ctx->using_ns_count; i++) {
        const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->using_namespaces[i], name);
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return pmm_type_named(ctx->arena, qn);
    }
    ...
    if (ctx->cpp_mode) {
        const char *std_qn = pmm_arena_sprintf(ctx->arena, "std.%s", name);
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, std_qn);
        if (rt)
            return pmm_type_named(ctx->arena, std_qn);
    }

    // Unresolved — return named with module prefix
    if (ctx->module_qn) {
        return pmm_type_named(ctx->arena,
                              pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name));
    }
    return pmm_type_named(ctx->arena, name);
}
...
const CBMType *c_parse_type_node(CLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "primitive_type") == 0) {
        ...
        return name ? pmm_type_builtin(ctx->arena, name) : pmm_type_unknown();
    }
    ...
    if (strcmp(kind, "qualified_identifier") == 0 || strcmp(kind, "scoped_type_identifier") == 0) {
        ...
        if (!ts_node_is_null(name_child) &&
            ...
            if (scope_text) {
                const char *scoped =
                    pmm_arena_sprintf(ctx->arena, "%s::%s", scope_text, tmpl_name_text);
            ...
            const CBMRegisteredType *trt = pmm_registry_lookup_type(ctx->registry, template_qn);
            if (!trt && ctx->module_qn) {
                const char *mod_qn =
                    pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, template_qn);
                trt = pmm_registry_lookup_type(ctx->registry, mod_qn);
                if (trt)
                    template_qn = mod_qn;
            }
            ...
            const CBMType *targs[16] = {
                NULL}; /* zero-fill: pmm_type_substitute requires NULL-terminated args
            ...
            if (targ_count > 0)
                return pmm_type_template(ctx->arena, template_qn, targs, targ_count);
            return pmm_type_named(ctx->arena, template_qn);
        }
            ...
            if (scope_text) {
                ...
                if (alias_qn) {
                    if (!ts_node_is_null(name_child)) {
                        ...
                        if (name)
                            text = (char *)pmm_arena_sprintf(ctx->arena, "%s::%s", alias_qn, name);
                    }
                }
            }
        }
    ...
    if (strcmp(kind, "template_type") == 0) {
        ...
        if (!trt && ctx->current_namespace) {
            ...
            // "std.shared_ptr")
            const char *ns_qn =
                pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, template_qn);
            trt = pmm_registry_lookup_type(ctx->registry, ns_qn);
            if (trt)
                template_qn = ns_qn;
            // Also try module + namespace
            if (!trt && ctx->module_qn) {
                const char *full_qn = pmm_arena_sprintf(ctx->arena, "%s.%s.%s", ctx->module_qn,
                                                        ctx->current_namespace, template_qn);
                trt = pmm_registry_lookup_type(ctx->registry, full_qn);
                if (trt)
                    template_qn = full_qn;
            }
        }
        if (!trt) {
            // Try with std:: prefix
            const char *std_qn = pmm_arena_sprintf(ctx->arena, "std.%s", template_qn);
            trt = pmm_registry_lookup_type(ctx->registry, std_qn);
            if (trt)
                template_qn = std_qn;
        }
        ...
        // Parse template arguments
        const CBMType *targs[16] = {
            NULL}; /* zero-fill: pmm_type_substitute requires NULL-terminated args (uninitialized
    ...
    if (strcmp(kind, "pointer_declarator") == 0 || strcmp(kind, "pointer_type") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            return pmm_type_pointer(ctx->arena,
                                    c_parse_type_node(ctx, ts_node_named_child(node, 0)));
        }
        return pmm_type_pointer(ctx->arena, pmm_type_unknown());
    }
    ...
    if (strcmp(kind, "reference_declarator") == 0 ||
        ...
        if (ts_node_named_child_count(node) > 0) {
            return pmm_type_reference(ctx->arena,
                                      c_parse_type_node(ctx, ts_node_named_child(node, 0)));
        }
        return pmm_type_reference(ctx->arena, pmm_type_unknown());
    }

    // auto keyword
    if (strcmp(kind, "auto") == 0 || strcmp(kind, "placeholder_type_specifier") == 0) {
        return pmm_type_unknown(); // will be resolved from initializer
    }
    ...
    if (strcmp(kind, "type_qualifier") == 0) {
        return pmm_type_unknown(); // qualifier itself has no type
    }

    // array_declarator: int[10]
    if (strcmp(kind, "array_declarator") == 0 || strcmp(kind, "array_type") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            return pmm_type_slice(ctx->arena, c_parse_type_node(ctx, ts_node_named_child(node, 0)));
        }
        return pmm_type_unknown();
    }
    ...
    if (strcmp(kind, "function_declarator") == 0 ||
        ...
        return pmm_type_func(ctx->arena, NULL, NULL, NULL);
    }
    ...
    if (strcmp(kind, "struct_specifier") == 0 || strcmp(kind, "class_specifier") == 0) {
        ...
        // Anonymous struct
        CBMType *t = (CBMType *)pmm_arena_alloc(ctx->arena, sizeof(CBMType));
        memset(t, 0, sizeof(CBMType));
        t->kind = PMM_TYPE_STRUCT;
        return t;
    }
    ...
    if (strcmp(kind, "enum_specifier") == 0) {
        ...
        return pmm_type_builtin(ctx->arena, "int");
    }
    ...
    if (strcmp(kind, "dependent_type") == 0 || strcmp(kind, "dependent_name") == 0) {
        char *text = c_node_text(ctx, node);
        if (text)
            return pmm_type_named(ctx->arena, c_build_qn(ctx, text));
        return pmm_type_unknown();
    }
...
const CBMType *c_simplify_type(CLSPContext *ctx, const CBMType *t, bool unwrap_pointer) {
    for (int i = 0; i < 64 && t; i++) {
        ...
        if (t->kind == PMM_TYPE_TYPE_PARAM && ctx->in_template) {
        ...
        // Unwrap references
        if (t->kind == PMM_TYPE_REFERENCE || t->kind == PMM_TYPE_RVALUE_REF) {
        ...
        // Unwrap pointer
        if (t->kind == PMM_TYPE_POINTER) {
            t = t->data.pointer.elem;
            break;
        }

        // Smart pointer: extract first template arg
        if (t->kind == PMM_TYPE_TEMPLATE && is_smart_ptr(t->data.template_type.template_name)) {
        ...
        if (t->kind == PMM_TYPE_NAMED) {
            const CBMRegisteredFunc *op = pmm_registry_lookup_method(
                ctx->registry, t->data.named.qualified_name, "operator->");
            if (op && op->signature && op->signature->kind == PMM_TYPE_FUNC &&
        ...
        if (t->kind == PMM_TYPE_TEMPLATE) {
            ...
            if (op && op->signature && op->signature->kind == PMM_TYPE_FUNC &&
                ...
                if (ret->kind == PMM_TYPE_TYPE_PARAM || ret->kind == PMM_TYPE_POINTER ||
                    ret->kind == PMM_TYPE_REFERENCE) {
                    const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, tqn);
                    if (!rt && ctx->module_qn) {
                        rt = pmm_registry_lookup_type(
                            ctx->registry,
                            pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, tqn));
                    }
                    if (rt && rt->type_param_names) {
                        ret = pmm_type_substitute(ctx->arena, ret, rt->type_param_names,
                        ...
                        ret = pmm_type_substitute(ctx->arena, ret, fb, fa);
                    }
                }
                t = ret;
                if (t->kind == PMM_TYPE_POINTER)
...
const CBMType *c_eval_expr_type(CLSPContext *ctx, TSNode node) {
    ...
    ctx->eval_depth--;
    return result ? result : pmm_type_unknown();
}
...
static const CBMType *c_eval_expr_type_inner(CLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (fqn) {
            const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, fqn);
            if (f && f->signature)
                return f->signature;
        }
        ...
        if (type && type->kind == PMM_TYPE_NAMED) {
    ...
    if (strcmp(kind, "this") == 0) {
        if (ctx->enclosing_class_qn) {
            return pmm_type_pointer(ctx->arena,
                                    pmm_type_named(ctx->arena, ctx->enclosing_class_qn));
        }
        return pmm_type_unknown();
    }
    ...
    if (strcmp(kind, "field_expression") == 0) {
        ...
        if (pmm_type_is_unknown(obj_type))
            return pmm_type_unknown();
        ...
        if (field_type && !pmm_type_is_unknown(field_type)) {
            ...
            bool is_tparam = (field_type->kind == PMM_TYPE_TYPE_PARAM);
            ...
            if (!is_tparam && field_type->kind == PMM_TYPE_NAMED &&
                base->kind == PMM_TYPE_TEMPLATE) {
                const CBMRegisteredType *trt = pmm_registry_lookup_type(ctx->registry, type_qn);
                if (!trt && ctx->module_qn) {
                    trt = pmm_registry_lookup_type(
                        ctx->registry,
                        pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn));
                }
                if (trt && trt->type_param_names) {
                    ...
                    for (int tpi = 0; trt->type_param_names[tpi]; tpi++) {
                        if (strcmp(trt->type_param_names[tpi], short_fn) == 0) {
                            ...
                            field_type = pmm_type_type_param(ctx->arena, short_fn);
                            break;
                        }
                    }
                }
            }
            if (is_tparam && base->kind == PMM_TYPE_TEMPLATE &&
                base->data.template_type.template_args) {
                const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, type_qn);
                if (!rt && ctx->module_qn) {
                    rt = pmm_registry_lookup_type(
                        ctx->registry,
                        pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn));
                ...
                if (rt && rt->type_param_names) {
                    field_type = pmm_type_substitute(ctx->arena, field_type, rt->type_param_names,
                                                     base->data.template_type.template_args);
                } else {
                    ...
                    field_type = pmm_type_substitute(ctx->arena, field_type, params, args);
                }
            }
            return field_type;
        }
    ...
    if (strcmp(kind, "qualified_identifier") == 0 || strcmp(kind, "scoped_identifier") == 0) {
        ...
        if (!ts_node_is_null(qi_name_child) &&
            ...
            if (bare_name) {
                text = scope_text
                           ? (char *)pmm_arena_sprintf(ctx->arena, "%s::%s", scope_text, bare_name)
                           : bare_name;
            }
        }
        ...
        if (ctx->module_qn) {
            f = pmm_registry_lookup_func(
                ctx->registry, pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn));
        }
        if (!f)
            f = pmm_registry_lookup_func(ctx->registry, qn);
        ...
        if (!f) {
            ...
            if (ns_dot && ctx->module_qn) {
                ...
                f = pmm_registry_lookup_func(
                    ctx->registry, pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare));
                if (!f)
                    f = pmm_registry_lookup_func(ctx->registry, bare);
            }
        }
        if (f && f->signature) {
            // If template args present, substitute into return type
            if (qi_has_tmpl_args && !ts_node_is_null(qi_tmpl_args) &&
                f->signature->kind == PMM_TYPE_FUNC && f->signature->data.func.return_types &&
                ...
                if (targ_count > 0) {
                    ...
                    if (tpn) {
                        base_ret = pmm_type_substitute(ctx->arena, base_ret, tpn, targs);
                    } else {
                        const char *fallback[] = {"T", "U", "V", "W", NULL};
                        base_ret = pmm_type_substitute(ctx->arena, base_ret, fallback, targs);
                    }
                    ...
                    if (base_ret->kind == PMM_TYPE_NAMED) {
                        const CBMType **final_targs = (const CBMType **)pmm_arena_alloc(
                            ctx->arena, (targ_count + 1) * sizeof(const CBMType *));
                        for (int i = 0; i < targ_count; i++)
                            final_targs[i] = targs[i];
                        final_targs[targ_count] = NULL;
                        base_ret =
                            pmm_type_template(ctx->arena, base_ret->data.named.qualified_name,
                                              final_targs, targ_count);
                    }
                }
                const CBMType **rets =
                    (const CBMType **)pmm_arena_alloc(ctx->arena, 2 * sizeof(const CBMType *));
                rets[0] = base_ret;
                rets[1] = NULL;
                return pmm_type_func(ctx->arena, NULL, NULL, rets);
            }
            return f->signature;
        }
        ...
        if (rt)
            return pmm_type_named(ctx->arena, qn);
        if (!rt && ctx->module_qn) {
            const char *mod_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
            rt = pmm_registry_lookup_type(ctx->registry, mod_qn);
            if (rt)
                return pmm_type_named(ctx->arena, mod_qn);
        }
        ...
        if (dot) {
            ...
            char *enum_qn = (char *)pmm_arena_alloc(ctx->arena, prefix_len + 1);
            if (enum_qn) {
                memcpy(enum_qn, qn, prefix_len);
                enum_qn[prefix_len] = '\0';
                const CBMRegisteredType *enum_rt = pmm_registry_lookup_type(ctx->registry, enum_qn);
                if (enum_rt) {
                    return pmm_type_named(ctx->arena, enum_qn);
                }
                // Try with module prefix
                if (ctx->module_qn) {
                    const char *fqn_enum =
                        pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, enum_qn);
                    enum_rt = pmm_registry_lookup_type(ctx->registry, fqn_enum);
                    if (enum_rt)
                        return pmm_type_named(ctx->arena, fqn_enum);
                }
            }
            ...
            if (ctx->in_template && dot) {
                ...
                char *scope_name = (char *)pmm_arena_alloc(ctx->arena, prefix_len2 + 1);
                if (scope_name) {
                    ...
                    if (default_type) {
                        ...
                        if (default_qn) {
                            ...
                            const CBMRegisteredFunc *mf =
                                pmm_registry_lookup_method(ctx->registry, default_qn, member);
                            if (mf && mf->signature)
                                return mf->signature;
                            // Try as static function
                            const char *dep_qn =
                                pmm_arena_sprintf(ctx->arena, "%s.%s", default_qn, member);
                            const CBMRegisteredFunc *sf =
                                pmm_registry_lookup_func(ctx->registry, dep_qn);
                            if (sf && sf->signature)
                                return sf->signature;
                        }
                    }
    ...
    if (strcmp(kind, "template_function") == 0) {
        ...
        if (strcmp(nk, "qualified_identifier") == 0 || strcmp(nk, "scoped_identifier") == 0) {
            ...
            if (!f && ctx->module_qn) {
                const char *mod_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
                f = pmm_registry_lookup_func(ctx->registry, mod_qn);
            }
        } else {
            const char *fqn = c_resolve_name(ctx, name);
            if (fqn)
                f = pmm_registry_lookup_func(ctx->registry, fqn);
        }
        if (!f || !f->signature)
            return pmm_type_unknown();
        ...
        if (f->signature->kind == PMM_TYPE_FUNC && f->signature->data.func.return_types &&
            f->signature->data.func.return_types[0]) {
        ...
        if (targ_count > 0) {
            ...
            if (tpn) {
                ...
                base_ret = pmm_type_substitute(ctx->arena, base_ret, fallback_names, targs);
            }
        }

        // Wrap return type in FUNC signature
        const CBMType **rets = pmm_arena_alloc(ctx->arena, 2 * sizeof(CBMType *));
        rets[0] = base_ret;
        rets[1] = NULL;
        return pmm_type_func(ctx->arena, NULL, NULL, rets);
    }
    ...
    if (strcmp(kind, "call_expression") == 0) {
            ...
            if (fname && (strcmp(fname, "std::move") == 0 || strcmp(fname, "move") == 0 ||
                ...
                if (!ts_node_is_null(call_args) && ts_node_named_child_count(call_args) > 0) {
                    ...
                    if (!ts_node_is_null(first_arg)) {
                        ...
                        if (!pmm_type_is_unknown(arg_type))
                            return arg_type;
                    }
                }
            }
        ...
        if (func_type->kind == PMM_TYPE_FUNC && func_type->data.func.return_types &&
            ...
            bool needs_subst = (ret->kind == PMM_TYPE_TYPE_PARAM || ret->kind == PMM_TYPE_NAMED ||
                                ret->kind == PMM_TYPE_POINTER || ret->kind == PMM_TYPE_REFERENCE ||
                                ret->kind == PMM_TYPE_RVALUE_REF ||
                                ret->kind == PMM_TYPE_TEMPLATE || ret->kind == PMM_TYPE_SLICE);
            ...
            if (needs_subst) {
                ...
                if (strcmp(ts_node_type(func_node), "field_expression") == 0) {
                    ...
                    if (!ts_node_is_null(arg_node)) {
                        ...
                        if (obj_type && obj_type->kind == PMM_TYPE_TEMPLATE &&
                            ...
                            if (!tpn && recv_qn) {
                                const CBMRegisteredType *rrt =
                                    pmm_registry_lookup_type(ctx->registry, recv_qn);
                                if (!rrt && ctx->module_qn) {
                                    rrt = pmm_registry_lookup_type(
                                        ctx->registry, pmm_arena_sprintf(ctx->arena, "%s.%s",
                            ...
                            if (tpn) {
                                ...
                                ret =
                                    pmm_type_substitute(ctx->arena, ret, tpn,
                                                        obj_type->data.template_type.template_args);
                                substituted = true;
                            } else {
                                ...
                                ret = pmm_type_substitute(ctx->arena, ret, params, pargs);
                                substituted = true;
                            }
                        }
                    }
                ...
                if (!substituted) {
                    ...
                    if (strcmp(fn_type, "identifier") == 0) {
                        ...
                        if (fname) {
                            ...
                            if (fqn)
                                rf = pmm_registry_lookup_func(ctx->registry, fqn);
                        }
                        ...
                        if (fname) {
                            ...
                            rf = pmm_registry_lookup_func(ctx->registry, qn);
                        }
                    ...
                    if (rf && rf->type_param_names && rf->signature &&
                        rf->signature->kind == PMM_TYPE_FUNC &&
                        ...
                        if (tp_count > 0 && tp_count <= 8) {
                            ...
                            if (!ts_node_is_null(call_args)) {
                                ...
                                for (uint32_t ai = 0;
                                    ...
                                    while (param_t && (param_t->kind == PMM_TYPE_REFERENCE ||
                                                       param_t->kind == PMM_TYPE_RVALUE_REF ||
                                                       param_t->kind == PMM_TYPE_POINTER)) {
                                        if (param_t->kind == PMM_TYPE_POINTER)
                                            param_t = param_t->data.pointer.elem;
                                        else
                                            param_t = param_t->data.reference.elem;
                                    }
                                    // If param type is TYPE_PARAM, bind it
                                    if (param_t && param_t->kind == PMM_TYPE_TYPE_PARAM) {
                                        for (int ti = 0; ti < tp_count; ti++) {
                                            if (strcmp(rf->type_param_names[ti],
                                                ...
                                                while (unwrapped &&
                                                       (unwrapped->kind == PMM_TYPE_REFERENCE ||
                                                        unwrapped->kind == PMM_TYPE_RVALUE_REF ||
                                                        unwrapped->kind == PMM_TYPE_POINTER)) {
                                                    if (unwrapped->kind == PMM_TYPE_POINTER)
                            ...
                            if (any_deduced) {
                                const CBMType *substituted_ret = pmm_type_substitute(
            ...
            if (!ret)
                return pmm_type_unknown();
            if (ret->kind == PMM_TYPE_REFERENCE || ret->kind == PMM_TYPE_RVALUE_REF)
                ret = ret->data.reference.elem;
            return ret;
        }

        // Constructor call: Type(args) — if func_node resolves to a named type
        if (func_type->kind == PMM_TYPE_NAMED) {
            return func_type;
        }
    ...
    if (strcmp(kind, "new_expression") == 0) {
        ...
        if (!ts_node_is_null(type_node)) {
            return pmm_type_pointer(ctx->arena, c_parse_type_node(ctx, type_node));
        }
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "template_type") == 0 ||
                ...
                return pmm_type_pointer(ctx->arena, c_parse_type_node(ctx, child));
            }
        }
        return pmm_type_pointer(ctx->arena, pmm_type_unknown());
    }
    ...
    if (strcmp(kind, "subscript_expression") == 0) {
        ...
        if (arr_type->kind == PMM_TYPE_SLICE)
            return arr_type->data.slice.elem;
        if (arr_type->kind == PMM_TYPE_POINTER)
        ...
        if (qn) {
            const CBMRegisteredFunc *op =
                pmm_registry_lookup_method(ctx->registry, qn, "operator[]");
            if (op && op->signature && op->signature->kind == PMM_TYPE_FUNC &&
                op->signature->data.func.return_types && op->signature->data.func.return_types[0]) {
                const CBMType *ret = op->signature->data.func.return_types[0];
                if (ret->kind == PMM_TYPE_REFERENCE)
                    ret = ret->data.reference.elem;

                // Substitute template params
                if (ret->kind == PMM_TYPE_TYPE_PARAM && arr_type->kind == PMM_TYPE_TEMPLATE) {
                    ...
                    ret = pmm_type_substitute(ctx->arena, ret, params, args);
                }
                return ret;
            }
        }
                ...
                if (strcmp(op, "*") == 0) {
                    ...
                    if (inner && inner->kind == PMM_TYPE_TEMPLATE) {
                        ...
                        if (opf && opf->signature && opf->signature->kind == PMM_TYPE_FUNC &&
                            ...
                            const CBMRegisteredType *rt =
                                pmm_registry_lookup_type(ctx->registry, tqn);
                            if (!rt && ctx->module_qn) {
                                rt = pmm_registry_lookup_type(
                                    ctx->registry,
                                    pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, tqn));
                    ...
                    return pmm_type_deref(inner);
                }
                if (strcmp(op, "&") == 0) {
                    return pmm_type_pointer(ctx->arena, c_eval_expr_type(ctx, operand));
                }
    ...
    if (strcmp(kind, "pointer_expression") == 0) {
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                ...
                if (op && strcmp(op, "&") == 0)
                    return pmm_type_pointer(ctx->arena, c_eval_expr_type(ctx, arg));
                break;
            }
        }
        return pmm_type_unknown();
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                ...
                if (op && (strcmp(op, "==") == 0 || strcmp(op, "!=") == 0 || strcmp(op, "<") == 0 ||
                    ...
                    return pmm_type_builtin(ctx->arena, "bool");
                }
                break;
            }
        }
    ...
    if (strcmp(kind, "sizeof_expression") == 0 || strcmp(kind, "alignof_expression") == 0) {
        return pmm_type_builtin(ctx->arena, "size_t");
    }
    ...
    if (strcmp(kind, "lambda_expression") == 0) {
        ...
        if (!ret_type || pmm_type_is_unknown(ret_type)) {
        ...
        if (ret_type && !pmm_type_is_unknown(ret_type)) {
            ...
            if (ret_types) {
                ...
                ret_types[1] = NULL;
                return pmm_type_func(ctx->arena, NULL, NULL, ret_types);
            }
        }
        return pmm_type_func(ctx->arena, NULL, NULL, NULL);
    }
    ...
    if (strcmp(kind, "co_await_expression") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            ...
            if (op_type && op_type->kind == PMM_TYPE_TEMPLATE &&
            ...
            if (op_qn) {
                ...
                if (ar && ar->signature && ar->signature->kind == PMM_TYPE_FUNC &&
        ...
        if (text) {
            ...
            for (const char *p = text; *p; p++) {
                if (*p == '.' || *p == 'f' || *p == 'F' || *p == 'e' || *p == 'E')
                    return pmm_type_builtin(ctx->arena, "double");
            }
        }
        return pmm_type_builtin(ctx->arena, "int");
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "concatenated_string") == 0 ||
        ...
        return pmm_type_pointer(ctx->arena, pmm_type_builtin(ctx->arena, "char"));
    }
    if (strcmp(kind, "char_literal") == 0) {
        return pmm_type_builtin(ctx->arena, "char");
    }
    ...
    if (strcmp(kind, "initializer_list") == 0) {
        return pmm_type_unknown(); // context-dependent
    }
...
// pmm_arena_sprintf). The buffer is only read by the immediate registry lookup — a
...
static const CBMRegisteredFunc *c_lookup_member_depth(CLSPContext *ctx, const char *type_qn,
    ...
    if (!type_qn || !member_name)
        return NULL;
    if (depth > PMM_LSP_MAX_LOOKUP_DEPTH)
        return NULL;
    ...
    const CBMRegisteredFunc *f = pmm_registry_lookup_method(ctx->registry, type_qn, member_name);
    if (f)
        return f;
    ...
    if (!neg_memo_hit && ctx->module_qn) {
        ...
        if (!prefixed)
            prefixed = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn);
        f = pmm_registry_lookup_method(ctx->registry, prefixed, member_name);
        if (f)
            return f;
    }
    ...
    {
        const CBMType *scoped = pmm_scope_lookup(ctx->current_scope, type_qn);
        if (scoped && scoped->kind == PMM_TYPE_ALIAS) {
            const CBMType *underlying = pmm_type_resolve_alias(scoped);
            if (underlying && !pmm_type_is_unknown(underlying)) {
    ...
    if (!rt && ctx->module_qn) {
        ...
        if (!prefixed)
            prefixed = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn);
        rt = pmm_registry_lookup_type(ctx->registry, prefixed);
    }
    if (rt) {
        // Alias chain
        if (rt->alias_of) {
...
static bool c_base_declares_member(CLSPContext *ctx, const char *type_qn, const char *member_name) {
    ...
    if (!rt && ctx->module_qn) {
        rt = pmm_registry_lookup_type(
            ctx->registry, pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn));
    }
...
void c_process_statement(CLSPContext *ctx, TSNode node) {
            ...
            if (strcmp(ck, "init_declarator") == 0) {
                ...
                if (!ts_node_is_null(decl)) {
                    ...
                    if (strcmp(dk, "identifier") == 0) {
                        ...
                        if (!has_auto) {
                            for (int d = 0; d < ptr_depth; d++)
                                var_type = pmm_type_pointer(ctx->arena, var_type);
                        }
                        ...
                        if (ts_node_named_child_count(decl) > 0) {
                            ...
                            if (strcmp(ik, "identifier") == 0) {
                                ...
                                const CBMType *rhs_type = pmm_type_unknown();
                                ...
                                for (uint32_t bi = 0; bi < bnc; bi++) {
                                    ...
                                    if (strcmp(ts_node_type(bn), "identifier") == 0) {
                                        ...
                                        if (!bname)
                                            continue;
                                        const CBMType *elem_type = pmm_type_unknown();
                                        if (rhs_type && rhs_type->kind == PMM_TYPE_TEMPLATE &&
                        ...
                        if (!has_auto)
                            var_type = pmm_type_reference(ctx->arena, var_type);
                        ...
                        var_type = pmm_type_slice(ctx->arena, var_type);
                        ...
                        for (uint32_t bi = 0; bi < bnc; bi++) {
                            ...
                            if (strcmp(ts_node_type(bn), "identifier") == 0) {
                                ...
                                if (pmm_type_is_unknown(elem_type) && rhs_type) {
                                    const char *rhs_qn = type_to_qn(rhs_type);
                                    const CBMRegisteredType *rt =
                                        rhs_qn ? pmm_registry_lookup_type(ctx->registry, rhs_qn)
                                               : NULL;
                                    if (!rt && rhs_qn && ctx->module_qn)
                                        rt = pmm_registry_lookup_type(
                                            ctx->registry,
                                            pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
                                                              rhs_qn));
                    ...
                    if (var_name && var_name[0] && strcmp(var_name, "_") != 0) {
                        pmm_scope_bind(ctx->current_scope, var_name, var_type);

                        // Track function pointer targets: fp = &foo or fp = foo
                        // Accept pointer, func, or named types (typedef function pointers
                        // like fn_t appear as PMM_TYPE_NAMED, not FUNC/POINTER).
                        if (var_type && (var_type->kind == PMM_TYPE_FUNC ||
                                         var_type->kind == PMM_TYPE_POINTER ||
                                         var_type->kind == PMM_TYPE_NAMED)) {
                            if (!ts_node_is_null(value)) {
                        ...
                        if (!c_lookup_fp_target(ctx, var_name) && !ts_node_is_null(value)) {
                            ...
                            if (dll_func) {
                                bool is_fp_type = var_type && (var_type->kind == PMM_TYPE_FUNC ||
                                                               var_type->kind == PMM_TYPE_POINTER);
                                if (is_fp_type || has_cast) {
                                    const char *target_qn =
                                        pmm_arena_sprintf(ctx->arena, "external.%s", dll_func);
                                    c_add_fp_target(ctx, var_name, target_qn);
                                }
                            }
                        }
                ...
                if (var_name && var_name[0]) {
                    pmm_scope_bind(ctx->current_scope, var_name, base_type);
                }
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    ...
                    if (var_name) {
                        ...
                        for (int d = 0; d < ptr_depth; d++)
                            vt = pmm_type_pointer(ctx->arena, vt);
                        pmm_scope_bind(ctx->current_scope, var_name, vt);
                    }
                }
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    char *var_name = c_node_text(ctx, inner);
                    if (var_name) {
                        pmm_scope_bind(ctx->current_scope, var_name,
                                       pmm_type_slice(ctx->arena, base_type));
                    }
                }
            }
        }
                ...
                if (strcmp(ck, "identifier") == 0 || strcmp(ck, "qualified_identifier") == 0 ||
                    ...
                    if (enum_text) {
                        ...
                        const CBMRegisteredType *et =
                            pmm_registry_lookup_type(ctx->registry, enum_qn);
                        if (!et && ctx->module_qn) {
                            enum_qn =
                                pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, enum_qn);
                            et = pmm_registry_lookup_type(ctx->registry, enum_qn);
                        }
                        if (!et && ctx->current_namespace) {
                            enum_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace,
                                                        enum_text);
                            et = pmm_registry_lookup_type(ctx->registry, enum_qn);
                        }
            ...
            if (alias_name && target) {
                ...
                const char *alias_qn =
                    ctx->current_namespace
                        ? pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, alias_name)
                        : pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, alias_name);
                pmm_scope_bind(ctx->current_scope, alias_name,
                               pmm_type_alias(ctx->arena, alias_qn, target));
            }
        }
        return;
    }
    ...
    if (strcmp(kind, "type_definition") == 0) {
        ...
        const CBMType *target = pmm_type_unknown();
        uint32_t nc = ts_node_named_child_count(node);
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (!found_type && strcmp(ck, "type_identifier") != 0 &&
                ...
                if (alias_name) {
                    const char *alias_qn =
                        ctx->current_namespace
                            ? pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace,
                                                alias_name)
                            : pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, alias_name);
                    pmm_scope_bind(ctx->current_scope, alias_name,
                                   pmm_type_alias(ctx->arena, alias_qn, target));
    ...
    if (strcmp(kind, "for_range_loop") == 0) {
        ...
        if (!ts_node_is_null(right)) {
            ...
            if (container_type) {
                if (container_type->kind == PMM_TYPE_SLICE) {
                    elem_type = container_type->data.slice.elem;
                } else if (container_type->kind == PMM_TYPE_TEMPLATE &&
                    ...
                    if (tname && container_type->data.template_type.arg_count >= 2 &&
                        ...
                        pair_args[2] = NULL;
                        elem_type = pmm_type_template(ctx->arena, "std.pair", pair_args, 2);
                    } else {
                        elem_type = container_type->data.template_type.template_args[0];
                    }
                }
                // Iterator protocol fallback: begin() -> iter -> operator*() -> elem
                if (pmm_type_is_unknown(elem_type) && container_type->kind == PMM_TYPE_NAMED) {
                    ...
                    if (begin_fn && begin_fn->signature &&
                        begin_fn->signature->kind == PMM_TYPE_FUNC &&
                        ...
                        if (iter_qn) {
                            ...
                            if (deref && deref->signature &&
                                deref->signature->kind == PMM_TYPE_FUNC &&
        ...
        if (!ts_node_is_null(decl)) {
            ...
            if (strcmp(dk, "identifier") == 0) {
                ...
                if (var_name)
                    pmm_scope_bind(ctx->current_scope, var_name, elem_type);
            } else if (strcmp(dk, "structured_binding_declarator") == 0) {
                ...
                for (uint32_t bi = 0; bi < bnc; bi++) {
                    ...
                    if (strcmp(ts_node_type(bn), "identifier") == 0) {
                        ...
                        if (!bname)
                            continue;
                        const CBMType *bt = pmm_type_unknown();
                        // TEMPLATE decomposition (e.g., pair<K,V>)
                        if (elem_type && elem_type->kind == PMM_TYPE_TEMPLATE &&
                        ...
                        // SLICE decomposition
                        if (pmm_type_is_unknown(bt) && elem_type &&
                            elem_type->kind == PMM_TYPE_SLICE) {
                        ...
                        if (pmm_type_is_unknown(bt) && elem_type) {
                            ...
                            const CBMRegisteredType *rt =
                                eq ? pmm_registry_lookup_type(ctx->registry, eq) : NULL;
                            if (!rt && eq && ctx->module_qn)
                                rt = pmm_registry_lookup_type(
                                    ctx->registry,
                                    pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, eq));
                            if (rt && rt->field_types && rt->field_types[binding_idx])
                                bt = rt->field_types[binding_idx];
                        }
                        pmm_scope_bind(ctx->current_scope, bname, bt);
                        binding_idx++;
                    }
                }
            }
    ...
    if (strcmp(kind, "parameter_declaration") == 0) {
        ...
        if (!ts_node_is_null(decl)) {
            ...
            if (strcmp(dk, "identifier") == 0) {
                ...
                if (name)
                    pmm_scope_bind(ctx->current_scope, name, param_type);
            } else if (strcmp(dk, "pointer_declarator") == 0) {
                int ptr_depth = 0;
                TSNode inner = decl;
                while (!ts_node_is_null(inner) &&
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    ...
                    if (name) {
                        ...
                        for (int d = 0; d < ptr_depth; d++)
                            vt = pmm_type_pointer(ctx->arena, vt);
                        pmm_scope_bind(ctx->current_scope, name, vt);
                    }
                }
            } else if (strcmp(dk, "reference_declarator") == 0) {
                if (ts_node_named_child_count(decl) > 0) {
                    ...
                    if (strcmp(ts_node_type(inner), "identifier") == 0) {
                        char *name = c_node_text(ctx, inner);
                        if (name)
                            pmm_scope_bind(ctx->current_scope, name,
                                           pmm_type_reference(ctx->arena, param_type));
                    }
                }
            }
        }
...
static void c_emit_resolved_call_orig(CLSPContext *ctx, const char *callee_qn, const char *orig,
    ...
    rc.reason = orig;
    pmm_resolvedcall_push(ctx->resolved_calls, ctx->arena, rc);
}
 ...
 * pmm_type_substitute under hundreds of recursive c_resolve_calls_in_node
...
static void c_resolve_calls_in_node_inner(CLSPContext *ctx, TSNode node) {
        ...
        if (!ts_node_is_null(func_node)) {
                ...
                if (!ts_node_is_null(arg_node) && !ts_node_is_null(field_node)) {
                    ...
                    if (field_name) {
                        ...
                        if (type_qn) {
                            ...
                            const CBMRegisteredFunc *method = pmm_registry_lookup_method_by_types(
                            ...
                            if (method) {
                                ...
                                if (method->receiver_type &&
                                    ...
                                    const CBMRegisteredFunc *override_m =
                                        pmm_registry_lookup_method(ctx->registry, type_qn,
                                                                   field_name);
                                    if (override_m) {
                                        method = override_m;
                                ...
                                if (is_arrow && obj_type->kind == PMM_TYPE_TEMPLATE &&
                        ...
                        if (base && base->kind == PMM_TYPE_TYPE_PARAM && ctx->enclosing_func_qn &&
                            ctx->in_template) {
                            int ac = 0;
                        ...
                        if (pmm_type_is_unknown(obj_type)) {
                            ...
                            c_emit_unresolved_call(
                                ctx, pmm_arena_sprintf(ctx->arena, "%s.%s", type_qn, field_name),
                                "method_not_found");
                        }
                    }
                }
                ...
                if (text) {
                    ...
                    if (angle) {
                        ...
                        if (!dot_after) {
                            ...
                            char *stripped = (char *)pmm_arena_alloc(ctx->arena, strip_len + 1);
                            ...
                            if (close && close < dot_after) {
                                ...
                                char *stripped = (char *)pmm_arena_alloc(
                    ...
                    if (ctx->module_qn) {
                        f = pmm_registry_lookup_func(
                            ctx->registry,
                            pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn));
                    }
                    if (!f)
                        f = pmm_registry_lookup_func(ctx->registry, qn);
                    if (f) {
                    ...
                    if (dot) {
                        ...
                        char *class_qn = (char *)pmm_arena_alloc(ctx->arena, prefix_len + 1);
                        memcpy(class_qn, qn, prefix_len);
                        class_qn[prefix_len] = '\0';
                        const CBMRegisteredFunc *m = pmm_registry_lookup_method_by_types(
                            ctx->registry, class_qn, dot + 1, arg_types, arg_count);
                        // Try with module prefix
                        if (!m && ctx->module_qn) {
                            const char *mod_class =
                                pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, class_qn);
                            m = pmm_registry_lookup_method_by_types(ctx->registry, mod_class,
                    ...
                    if (dot && ctx->module_qn) {
                        ...
                        const CBMRegisteredFunc *nf =
                            pmm_registry_lookup_func(ctx->registry, bare_name);
                        if (!nf) {
                            nf = pmm_registry_lookup_func(
                                ctx->registry,
                                pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare_name));
                        }
                        if (nf) {
                    ...
                    if (name) {
                        ...
                        if (strcmp(nk, "qualified_identifier") == 0 ||
                            ...
                            if (!f) {
                                ...
                                if (dot) {
                                    size_t plen = dot - qn;
                                    char *cls = (char *)pmm_arena_alloc(ctx->arena, plen + 1);
                                    memcpy(cls, qn, plen);
                                    cls[plen] = '\0';
                                    const CBMRegisteredFunc *m =
                                        pmm_registry_lookup_method(ctx->registry, cls, dot + 1);
                                    if (!m && ctx->module_qn) {
                                        const char *mc = pmm_arena_sprintf(ctx->arena, "%s.%s",
                                                                           ctx->module_qn, cls);
                                        m = pmm_registry_lookup_method(ctx->registry, mc, dot + 1);
                                    }
                                    if (m)
                                        f = m;
                                }
            ...
            if (strcmp(fk, "identifier") == 0) {
                ...
                if (name && !is_c_builtin_func(name)) {
                    ...
                    const CBMType *var_type = pmm_scope_lookup(ctx->current_scope, name);
                    if (!pmm_type_is_unknown(var_type)) {
                        if (var_type->kind == PMM_TYPE_FUNC) {
                    ...
                    if (type && type->kind == PMM_TYPE_NAMED) {
                        ...
                        const CBMRegisteredType *rt =
                            pmm_registry_lookup_type(ctx->registry, type_qn_str);
                        if (rt && !rt->is_interface) {
                            // Constructor call
                            const char *ctor_qn =
                                pmm_arena_sprintf(ctx->arena, "%s.%s", type_qn_str, name);
                    ...
                    if (fqn) {
                        ...
                        if (ctx->enclosing_class_qn) {
                            const CBMRegisteredFunc *m = pmm_registry_lookup_method(
                        ...
                        if (ctx->pending_tc_count > 0) {
                            const CBMRegisteredFunc *called =
                                pmm_registry_lookup_func(ctx->registry, fqn);
    ...
    if (strcmp(kind, "declaration") == 0 && ctx->cpp_mode) {
        ...
        uint32_t nc = ts_node_named_child_count(node);
        const CBMType *decl_type = pmm_type_unknown();
        bool has_type = false;
        for (uint32_t i = 0; i < nc; i++) {
            TSNode child = ts_node_named_child(node, i);
            const char *ck = ts_node_type(child);
                ...
                if (!ts_node_is_null(value)) {
                    ...
                    if (strcmp(vk, "argument_list") == 0 || strcmp(vk, "initializer_list") == 0) {
                        ...
                        if (type_qn) {
                            ...
                            const char *ctor_qn =
                                pmm_arena_sprintf(ctx->arena, "%s.%s", type_qn, short_name);
    ...
    if (strcmp(kind, "new_expression") == 0) {
        ...
        if (!ts_node_is_null(type_node)) {
            ...
            if (type_qn) {
                const char *short_name = strrchr(type_qn, '.');
                short_name = short_name ? short_name + 1 : type_qn;
                const char *ctor_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", type_qn, short_name);
                c_emit_resolved_call(ctx, ctor_qn, "lsp_constructor", 0.95f);
            }
        }
    }
    ...
    if (strcmp(kind, "delete_expression") == 0 && ctx->cpp_mode) {
        ...
        if (!ts_node_is_null(operand)) {
            ...
            if (type_qn) {
                const char *short_name = strrchr(type_qn, '.');
                short_name = short_name ? short_name + 1 : type_qn;
                const char *dtor_qn = pmm_arena_sprintf(ctx->arena, "%s.~%s", type_qn, short_name);
    ...
    if (ctx->cpp_mode && strcmp(kind, "binary_expression") == 0) {
        ...
        if (!ts_node_is_null(left)) {
            ...
            if (base && base->kind != PMM_TYPE_BUILTIN && !pmm_type_is_unknown(base)) {
                const char *type_qn = type_to_qn(base);
                if (type_qn) {
                    // Extract operator
                    for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
                        ...
                        if (!ts_node_is_named(child)) {
                            ...
                            if (op) {
                                const char *op_name =
                                    pmm_arena_sprintf(ctx->arena, "operator%s", op);
    ...
    if (ctx->cpp_mode && strcmp(kind, "subscript_expression") == 0) {
        ...
        if (!ts_node_is_null(arg_node)) {
            ...
            if (base && base->kind != PMM_TYPE_BUILTIN && !pmm_type_is_unknown(base) &&
                base->kind != PMM_TYPE_POINTER && base->kind != PMM_TYPE_SLICE) {
                const char *type_qn = type_to_qn(base);
                if (type_qn) {
    ...
    if (ctx->cpp_mode && strcmp(kind, "declaration") == 0) {
        ...
        if (decl_type && decl_type->kind == PMM_TYPE_NAMED) {
...
recurse:;
    ...
    if (push_scope) {
        ctx->current_scope = pmm_scope_push(ctx->arena, ctx->current_scope);
    }
                ...
                for (uint32_t ci = 0; ci < cnc; ci++) {
                    ...
                    if (strcmp(ck, "lambda_capture_initializer") == 0) {
                        ...
                        if (!ts_node_is_null(name_node) && !ts_node_is_null(val_node)) {
                            ...
                            if (cap_name && !pmm_type_is_unknown(cap_type)) {
                                pmm_scope_bind(ctx->current_scope, cap_name, cap_type);
                            }
                        }
                    }
                }
    ...
    if (push_scope) {
        ctx->current_scope = pmm_scope_pop(ctx->current_scope);
    }
}
...
static void c_process_function(CLSPContext *ctx, TSNode func_node) {
        ...
        if (strcmp(dk, "qualified_identifier") == 0 || strcmp(dk, "scoped_identifier") == 0) {
            ...
            if (!ts_node_is_null(scope_node)) {
                ...
                if (scope_text) {
                    ...
                    const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, scope_qn);
                    if (rt) {
                        ctx->enclosing_class_qn = scope_qn;
                    } else if (ctx->module_qn) {
                        const char *fqn =
                            pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, scope_qn);
                        rt = pmm_registry_lookup_type(ctx->registry, fqn);
                        if (rt)
    ...
    // pmm_pipeline_find_lsp_resolution never joined them — every in-method call
    ...
    if (ctx->enclosing_class_qn && saved_class_qn == ctx->enclosing_class_qn &&
        !strchr(func_qn, '.')) {
        func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, func_qn);
        ...
        func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, dot + 1);
        ...
        if (scope) {
            func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", scope, func_qn);
        }
    }
    ...
    if (ctx->in_template && ctx->template_param_count > 0 && !ctx->registry_shared) {
        ...
        for (int ri = 0; ri < ((CBMTypeRegistry *)ctx->registry)->func_count; ri++) {
            ...
            if (strcmp(rf->qualified_name, func_qn) == 0 && !rf->type_param_names) {
                const char **tpn = (const char **)pmm_arena_alloc(
    ...
    if (ctx->enclosing_class_qn) {
        pmm_scope_bind(
            ...
            pmm_type_pointer(ctx->arena, pmm_type_named(ctx->arena, ctx->enclosing_class_qn)));
    }
                        ...
                        if (fname && fname[0]) {
                            ...
                            for (uint32_t ri = 0; ri < dnc; ri++) {
                                ...
                                if (strcmp(rk, "function_declarator") != 0 &&
                                    ...
                                    if (!pmm_type_is_unknown(ret_type))
                                        break;
                                }
                            }
                            if (ret_type && !pmm_type_is_unknown(ret_type)) {
                                if (is_ref_ret)
                                    ret_type = pmm_type_reference(ctx->arena, ret_type);
                                if (is_ptr_ret)
                                    ret_type = pmm_type_pointer(ctx->arena, ret_type);
                                const char *func_qn = c_build_qn(ctx, fname);
                                if (ctx->module_qn && !strchr(func_qn, '.'))
                                    func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
                                                                func_qn);
                                // Use namespace if available
                                if (ctx->current_namespace &&
                                    !strstr(func_qn, ctx->current_namespace))
                                    func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s",
                                ...
                                if (!ctx->registry_shared &&
                                    !pmm_registry_lookup_func(ctx->registry, func_qn)) {
                                    const CBMType **rets = (const CBMType **)pmm_arena_alloc(
                                    ...
                                    rf.short_name = fname;
                                    rf.signature = pmm_type_func(ctx->arena, NULL, NULL, rets);
                                    ...
                                    pmm_registry_add_func((CBMTypeRegistry *)ctx->registry, rf);
                                }
                            }
                        }
                    }
...
static void c_process_namespace(CLSPContext *ctx, TSNode ns_node) {
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (ns_name) {
            if (saved_ns) {
                ctx->current_namespace = pmm_arena_sprintf(ctx->arena, "%s.%s", saved_ns, ns_name);
            } else if (ctx->module_qn) {
                ctx->current_namespace =
                    pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, ns_name);
            } else {
...
static void c_process_class(CLSPContext *ctx, TSNode class_node) {
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (class_name) {
            ...
            if (ctx->current_namespace) {
                class_qn =
                    pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, class_name);
            } else if (ctx->module_qn) {
                class_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, class_name);
            } else {
                class_qn = class_name;
            }
    ...
    if (!ts_node_is_null(body)) {
        ...
        if (ctx->enclosing_class_qn) {
            uint32_t bkn = 0;
            TSNode *bkids = pmm_lsp_collect_children(ctx->arena, body, &bkn);
                    ...
                    for (int depth = 0; depth < 5 && !ts_node_is_null(cur); depth++) {
                        ...
                        if (strcmp(dk, "function_declarator") == 0) {
                            ...
                            if (!method_name || !method_name[0])
                                break;
                            const char *method_qn = pmm_arena_sprintf(
                            ...
                            const CBMRegisteredFunc *existing = pmm_registry_lookup_method(
                            ...
                            if (!existing && ctx->module_qn) {
                                ...
                                if (strncmp(class_qn, ctx->module_qn, mod_len) == 0 &&
                                    class_qn[mod_len] == '.') {
                                    existing = pmm_registry_lookup_method(
                            ...
                            if (existing) {
                                ...
                                if (existing->signature &&
                                    existing->signature->kind == PMM_TYPE_FUNC &&
                                    ...
                                    int nk2 = actual_ret ? actual_ret->kind : PMM_TYPE_UNKNOWN;
                                    // Upgrade if existing is NAMED and new is more specific
                                    if (ek == PMM_TYPE_NAMED &&
                                        (nk2 == PMM_TYPE_TEMPLATE || nk2 == PMM_TYPE_TYPE_PARAM ||
                                         nk2 == PMM_TYPE_POINTER || nk2 == PMM_TYPE_REFERENCE))
                                        should_upgrade = true;
                                }
                                if (should_upgrade) {
                                    // Update existing entry's signature return type
                                    const CBMType **new_rets = (const CBMType **)pmm_arena_alloc(
                                        ctx->arena, 2 * sizeof(const CBMType *));
                                    new_rets[0] = actual_ret;
                                    new_rets[1] = NULL;
                                    CBMRegisteredFunc *mut = (CBMRegisteredFunc *)existing;
                                    mut->signature =
                                        pmm_type_func(ctx->arena, NULL, NULL, new_rets);
                                }
                                break;
                            }
                            ...
                            actual_ret = pmm_type_reference(ctx->arena, actual_ret);
                            cur = ts_node_named_child_count(cur) > 0 ? ts_node_named_child(cur, 0)
                                                                     : (TSNode){0};
                        } else if (strcmp(dk, "pointer_declarator") == 0) {
                            actual_ret = pmm_type_pointer(ctx->arena, actual_ret);
                            cur = ts_node_named_child_count(cur) > 0 ? ts_node_named_child(cur, 0)
                                                                     : (TSNode){0};
                        } else {
                            break;
...
__attribute__((no_sanitize("address"))) void c_lsp_process_file(CLSPContext *ctx, TSNode root) {
    ...
    uint32_t kn = 0;
    TSNode *kids = pmm_lsp_collect_children(ctx->arena, root, &kn);
...
static const CBMType *c_parse_return_type_text(CBMArena *a, const char *text,
    ...
    size_t len = strlen(text);
    if (len > 0 && text[len - 1] == '*') {
        char *inner = pmm_arena_strndup(a, text, len - 1);
        // Trim trailing space
        size_t ilen = strlen(inner);
        while (ilen > 0 && inner[ilen - 1] == ' ')
            inner[--ilen] = '\0';
        return pmm_type_pointer(a, c_parse_return_type_text(a, inner, module_qn));
    }
    ...
    if (len > 0 && text[len - 1] == '&') {
        ...
        if (ilen > 0 && inner[ilen - 1] == '&') {
            ...
            while (ilen > 1 && inner[ilen - 2] == ' ')
                inner[--ilen - 1] = '\0';
            return pmm_type_rvalue_ref(a, c_parse_return_type_text(a, inner, module_qn));
        }
        return pmm_type_reference(a, c_parse_return_type_text(a, inner, module_qn));
    }

    // decltype(expr) — extract the type from the expression
    if (strncmp(text, "decltype(", 9) == 0 && len > 10 && text[len - 1] == ')') {
        char *inner = pmm_arena_strndup(a, text + 9, len - 10);
        size_t ilen = strlen(inner);
    ...
    if (is_c_builtin_type(text))
        return pmm_type_builtin(a, text);
    ...
    if (len == 1 &&
        ...
        return pmm_type_type_param(a, text);
    }
    ...
    // Template type: Name<Arg1, Arg2, ...>
    {
        ...
        if (angle && len > 0 && text[len - 1] == '>') {
            ...
            char *tmpl_name = pmm_arena_strndup(a, text, name_len);
            ...
            char *args_text = pmm_arena_strndup(a, args_start, args_len);
            ...
            for (const char *p = args_text;; p++) {
                ...
                if ((*p == ',' && depth == 0) || *p == '\0') {
                    if (arg_count < 16) {
                        ...
                        char *arg = pmm_arena_strndup(a, arg_begin, alen);
                        // Trim whitespace
                        while (*arg == ' ')
                            arg++;
            ...
            if (!strchr(tmpl_name, '.') && module_qn && module_qn[0]) {
                qname = pmm_arena_sprintf(a, "%s.%s", module_qn, tmpl_name);
            }
            return pmm_type_template(a, qname, arg_types, arg_count);
        }
    }
    ...
    if (strstr(text, "::")) {
        // Replace :: with .
        char *qn = pmm_arena_strdup(a, text);
        for (char *p = qn; *p; p++) {
            if (p[0] == ':' && p[1] == ':') {
                p[0] = '.';
                memmove(p + 1, p + 2, strlen(p + 2) + 1);
    ...
    if (module_qn && module_qn[0]) {
        return pmm_type_named(a, pmm_arena_sprintf(a, "%s.%s", module_qn, text));
    }
    return pmm_type_named(a, text);
}
...
void pmm_run_c_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
                   TSNode root, bool cpp_mode) {

    CBMTypeRegistry reg;
    pmm_registry_init(&reg, arena);

    // Register stdlib
    pmm_c_stdlib_register(&reg, arena);
    if (cpp_mode)
        pmm_cpp_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->label && (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Type") == 0)) {
            ...
            if (d->base_classes) {
                ...
                while (d->base_classes[bc_count])
                    bc_count++;
                if (bc_count > 0) {
                    const char **embedded = (const char **)pmm_arena_alloc(
                    ...
                    for (int j = 0; j < bc_count; j++) {
                        ...
                        if (strchr(bc, '.') || strstr(bc, "::")) {
                            ...
                            embedded[j] = pmm_arena_sprintf(arena, "%s.%s", module_qn, bc);
                        }
                    }
        ...
        if (d->label && strcmp(d->label, "Field") == 0 && d->parent_class && d->return_type) {
            ...
            if (parent_rt) {
                ...
                const char **new_fnames =
                    (const char **)pmm_arena_alloc(arena, (existing + 2) * sizeof(const char *));
                const CBMType **new_ftypes = (const CBMType **)pmm_arena_alloc(
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->return_type && d->return_type[0]) {
                ret_types = (const CBMType **)pmm_arena_alloc(arena, 2 * sizeof(const CBMType *));
                ...
                while (d->return_types[count])
                    count++;
                if (count > 0) {
                    ret_types = (const CBMType **)pmm_arena_alloc(
            ...
            if (d->param_types) {
                ...
                while (d->param_types[count])
                    count++;
                if (count > 0) {
                    param_types_arr = (const CBMType **)pmm_arena_alloc(
                        arena, (count + 1) * sizeof(const CBMType *));
                    for (int j = 0; j < count; j++)
                        param_types_arr[j] =
            ...
            rf.signature = pmm_type_func(arena, d->param_names, param_types_arr, ret_types);

            // Method receiver
            if (strcmp(d->label, "Method") == 0 && d->parent_class) {
                rf.receiver_type = d->parent_class;
                // Auto-create type if needed
                if (!pmm_registry_lookup_type(&reg, d->parent_class)) {
                    ...
                    pmm_registry_add_type(&reg, auto_type);
                }
            }
        ...
        if (d->label && (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Type") == 0 ||
            ...
            if (d->embedded_types) {
                ...
                while (*src && embed_count < 31) {
                    ...
                    if (sep) {
                        embeds[embed_count++] = pmm_arena_strndup(arena, src, sep - src);
                        src = sep + 1;
                    } else {
                        embeds[embed_count++] = pmm_arena_strdup(arena, src);
                        break;
                    }
                }
                if (embed_count > 0) {
                    const char **arr = (const char **)pmm_arena_alloc(
            ...
            if (d->field_defs) {
                ...
                while (*fsrc && fcount < 63) {
                    ...
                    char *pair = pmm_arena_strndup(arena, fsrc, end - fsrc);
                ...
                if (fcount > 0) {
                    const char **fnarr =
                        (const char **)pmm_arena_alloc(arena, (fcount + 1) * sizeof(const char *));
                    const CBMType **ftarr = (const CBMType **)pmm_arena_alloc(
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->return_types) {
                ...
                while (*rsrc && rcount < 15) {
                    ...
                    char *rt_text = pmm_arena_strndup(arena, rsrc, end - rsrc);
                ...
                if (rcount > 0) {
                    const CBMType **rarr = (const CBMType **)pmm_arena_alloc(
                        arena, (rcount + 1) * sizeof(const CBMType *));
                    for (int j = 0; j < rcount; j++)
                        rarr[j] = rets[j];
                    rarr[rcount] = NULL;
                    rf.signature = pmm_type_func(arena, NULL, NULL, rarr);
                }
            }
            if (!rf.signature)
                rf.signature = pmm_type_func(arena, NULL, NULL, NULL);
...
CBMTypeRegistry *pmm_c_build_cross_registry(CBMArena *arena, CBMLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
    CBMTypeRegistry *reg = (CBMTypeRegistry *)pmm_arena_alloc(arena, sizeof(*reg));
    if (!reg)
        return NULL;
    pmm_registry_init(reg, arena);
    pmm_c_stdlib_register(reg, arena);
    ...
    for (int i = 0; i < def_count; i++) {
        CBMLSPDef *d = &defs[i];
        if (d->lang != PMM_LANG_C && d->lang != PMM_LANG_CPP && d->lang != PMM_LANG_CUDA) {
            continue;
        }
...
void pmm_run_c_lsp_cross_with_registry(CBMArena *arena, const char *source, int source_len,
...
void pmm_run_c_lsp_cross(CBMArena *arena, const char *source, int source_len, const char *module_qn,
...
void pmm_batch_c_lsp_cross(CBMArena *arena, CBMBatchCLSPFile *files, int file_count,
                           CBMResolvedCallArray *out) {
    ...
    for (int f = 0; f < file_count; f++) {
        ...
        CBMArena file_arena;
        pmm_arena_init(&file_arena);

        CBMResolvedCallArray file_out;
        memset(&file_out, 0, sizeof(file_out));

        // Delegate to existing per-file function
        pmm_run_c_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
        ...
        if (file_out.count > 0) {
            ...
            out[f].items =
                (CBMResolvedCall *)pmm_arena_alloc(arena, file_out.count * sizeof(CBMResolvedCall));
            for (int j = 0; j < file_out.count; j++) {
                CBMResolvedCall *src = &file_out.items[j];
                CBMResolvedCall *dst = &out[f].items[j];
                dst->caller_qn = src->caller_qn ? pmm_arena_strdup(arena, src->caller_qn) : NULL;
                dst->callee_qn = src->callee_qn ? pmm_arena_strdup(arena, src->callee_qn) : NULL;
                dst->strategy = src->strategy ? pmm_arena_strdup(arena, src->strategy) : NULL;
                dst->confidence = src->confidence;
                dst->reason = src->reason ? pmm_arena_strdup(arena, src->reason) : NULL;
            }
        }

        pmm_arena_destroy(&file_arena);
    }
}
