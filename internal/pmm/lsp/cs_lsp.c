...
static char *cs_strip_generic_args(CBMArena *a, const char *name) {
    ...
    if (!lt) return pmm_arena_strdup(a, name);
    size_t len = (size_t)(lt - name);
    return pmm_arena_strndup(a, name, len);
}
...
void cs_lsp_init(CSLSPContext *ctx, CBMArena *arena, const char *source, int source_len,
    ...
    cs_lsp_add_using(ctx, PMM_CS_USING_NAMESPACE, "", "System", false);
...
void cs_lsp_add_using(CSLSPContext *ctx, CBMCSUsingKind kind, const char *local_name,
    ...
    if (ctx->using_count >= ctx->using_cap) {
        ...
        CBMCSUsing *nu = (CBMCSUsing *)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nu));
        if (!nu) return;
    ...
    slot->kind = kind;
    slot->local_name = local_name ? pmm_arena_strdup(ctx->arena, local_name) : "";
    slot->target_qn = pmm_arena_strdup(ctx->arena, target_qn);
...
static void cs_namespace_push(CSLSPContext *ctx, const char *ns_name) {
    ...
    if (ctx->namespace_count >= ctx->namespace_cap) {
        ...
        const char **arr = (const char **)pmm_arena_alloc(ctx->arena,
    ...
    ctx->namespace_stack[ctx->namespace_count++] = pmm_arena_strdup(ctx->arena, ns_name);
}
...
static const char *cs_namespace_qn(CSLSPContext *ctx) {
    ...
    char *out = (char *)pmm_arena_alloc(ctx->arena, total + 1);
    if (!out) return "";
    size_t w = 0;
    for (int i = 0; i < ctx->namespace_count; i++) {
        if (i > 0) out[w++] = '.';
...
static const CBMRegisteredType *cs_lookup_type_qn(CSLSPContext *ctx, const char *qn) {
    ...
    return pmm_registry_lookup_type(ctx->registry, qn);
}
...
const char *cs_resolve_type_name(CSLSPContext *ctx, const char *raw) {
    ...
    for (int i = 0; i < ctx->type_param_count; i++) {
        if (strcmp(ctx->type_param_names[i], bare) == 0) {
            ...
            if (arg && arg->kind == PMM_TYPE_NAMED) return arg->data.named.qualified_name;
    ...
    if (ctx->enclosing_class_qn) {
        const char *try_qn =
            pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, bare);
    ...
    if (ctx->namespace_count > 0) {
        ...
        for (;;) {
            if (!ns || !*ns) break;
            const char *try_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ns, bare);
            if (cs_lookup_type_qn(ctx, try_qn)) return try_qn;
            const char *dot = strrchr(ns, '.');
            if (!dot) break;
            char *trim = pmm_arena_strndup(ctx->arena, ns, (size_t)(dot - ns));
            ns = trim;
        }
    }
    ...
    if (ctx->module_qn && ctx->module_qn[0]) {
        const char *try_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn)) return try_qn;
    }
    ...
    for (int i = 0; i < ctx->using_count; i++) {
        ...
        if (u->kind != PMM_CS_USING_NAMESPACE) continue;
        const char *try_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", u->target_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn)) return try_qn;
    }
    ...
    for (int i = 0; i < ctx->using_count; i++) {
        const CBMCSUsing *u = &ctx->usings[i];
        if (u->kind != PMM_CS_USING_ALIAS) continue;
        ...
        if (strncmp(bare, u->local_name, alias_len) == 0) {
            if (bare[alias_len] == '\0') {
                ...
                if (target_bare && cs_lookup_type_qn(ctx, target_bare))
                    return target_bare;
                return pmm_arena_strdup(ctx->arena, u->target_qn);
            }
            if (bare[alias_len] == '.') {
                /* `Alias.Sub` -> `Target.Sub` */
                char *target_bare = cs_strip_generic_args(ctx->arena, u->target_qn);
                const char *base = target_bare ? target_bare : u->target_qn;
                const char *try_qn = pmm_arena_sprintf(ctx->arena, "%s%s", base,
                                                        bare + alias_len);
                if (cs_lookup_type_qn(ctx, try_qn)) return try_qn;
            }
        }
...
const CBMRegisteredFunc *cs_lookup_method(CSLSPContext *ctx, const char *type_qn,
    ...
    const CBMRegisteredFunc *f =
        pmm_registry_lookup_method(ctx->registry, type_qn, method_name);
    if (f) return f;
    ...
    if (strcmp(type_qn, "System.Object") != 0) {
        f = pmm_registry_lookup_method(ctx->registry, "System.Object", method_name);
        if (f) return f;
    }
    return NULL;
}
...
static const CBMRegisteredFunc *cs_lookup_extension(CSLSPContext *ctx,
    ...
    for (int i = 0; i < ctx->registry->func_count; i++) {
        ...
        if (!cand->signature || cand->signature->kind != PMM_TYPE_FUNC) continue;
        ...
        bool match = false;
        if (first->kind == PMM_TYPE_NAMED && first->data.named.qualified_name) {
        ...
        } else if (first->kind == PMM_TYPE_TEMPLATE) {
...
const CBMType *cs_parse_type_node(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "predefined_type") == 0) {
        ...
        if (!t) return pmm_type_unknown();
        const char *aliased = cs_predefined_alias(t);
        if (aliased) return pmm_type_named(ctx->arena, aliased);
    ...
    if (strcmp(kind, "nullable_type") == 0) {
        ...
        if (ts_node_is_null(inner)) return pmm_type_unknown();
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        return pmm_type_template(ctx->arena, "System.Array",
    ...
    if (strcmp(kind, "tuple_type") == 0) {
        ...
        if (!elems) return pmm_type_unknown();
        int count = 0;
        for (uint32_t i = 0; i < nc; i++) {
    ...
    if (strcmp(kind, "generic_name") == 0) {
        ...
        if (ts_node_is_null(name_node)) return pmm_type_unknown();
        char *raw = cs_node_text(ctx, name_node);
        if (!raw) return pmm_type_unknown();
        const char *resolved = cs_resolve_type_name(ctx, raw);
        if (!resolved) return pmm_type_unknown();
        ...
        const CBMType **args = (const CBMType **)pmm_arena_alloc(
            ctx->arena, (size_t)(nc + 1) * sizeof(*args));
        if (!args) return pmm_type_named(ctx->arena, resolved);
        int count = 0;
        for (uint32_t i = 0; i < nc; i++) {
        ...
        args[count] = NULL;
        return pmm_type_template(ctx->arena, resolved, args, count);
    }
    if (strcmp(kind, "qualified_name") == 0 || strcmp(kind, "identifier") == 0 ||
        ...
        if (pre) return pmm_type_named(ctx->arena, pre);
        return pmm_type_named(ctx->arena, resolved);
    }
    if (strcmp(kind, "ref_type") == 0) {
        ...
        return pmm_type_reference(ctx->arena, t);
    }
...
const CBMType *cs_eval_expr_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (ctx->enclosing_base_qn) {
            result = pmm_type_named(ctx->arena, ctx->enclosing_base_qn);
        }
        ...
        result = pmm_type_named(ctx->arena, "System.String");
    } else if (strcmp(kind, "character_literal") == 0) {
        result = pmm_type_named(ctx->arena, "System.Char");
    } else if (strcmp(kind, "integer_literal") == 0) {
        result = pmm_type_named(ctx->arena, "System.Int32");
    } else if (strcmp(kind, "real_literal") == 0) {
        result = pmm_type_named(ctx->arena, "System.Double");
    } else if (strcmp(kind, "boolean_literal") == 0) {
        result = pmm_type_named(ctx->arena, "System.Boolean");
        ...
        uint32_t nc = ts_node_child_count(node);
        const CBMType **elems =
            (const CBMType **)pmm_arena_alloc(ctx->arena, (size_t)(nc + 1) * sizeof(*elems));
        if (!elems) {
            ctx->eval_depth--;
            return pmm_type_unknown();
        }
        ...
        elems[count] = NULL;
        result = pmm_type_tuple(ctx->arena, elems, count);
        ...
        result = pmm_type_named(ctx->arena, "System.Type");
        ...
        if (!ts_node_is_null(tnode)) {
            ...
            result = pmm_type_template(ctx->arena, "System.Array",
                                       (const CBMType *[]){elem, NULL}, 1);
        }
    } else if (strcmp(kind, "implicit_array_creation_expression") == 0) {
        result = pmm_type_template(ctx->arena, "System.Array",
                                   (const CBMType *[]){pmm_type_unknown(), NULL}, 1);
    }
...
static const CBMType *cs_eval_identifier_type(CSLSPContext *ctx, TSNode node) {
    ...
    /* Local / parameter scope. pmm_scope_lookup returns pmm_type_unknown()
    ...
    if (bound && bound->kind != PMM_TYPE_UNKNOWN) return bound;
    ...
    if (resolved) {
        ...
        if (rt) return pmm_type_named(ctx->arena, rt->qualified_name);
    }
...
static const CBMType *cs_eval_invocation_type(CSLSPContext *ctx, TSNode call) {
    ...
    if (ts_node_is_null(fn)) return pmm_type_unknown();
    ...
    if (strcmp(fk, "member_access_expression") == 0 ||
        ...
        if (ts_node_is_null(name)) return pmm_type_unknown();
        char *mname = cs_node_text(ctx, name);
        if (!mname) return pmm_type_unknown();
        char *bare_name = cs_strip_generic_args(ctx->arena, mname);

        const CBMType *recv_type = pmm_type_unknown();
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "generic_name") == 0) {
        ...
        if (!fname) return pmm_type_unknown();
        ...
        if (strcmp(bare, "nameof") == 0) {
            return pmm_type_named(ctx->arena, "System.String");
        }
        if (strcmp(bare, "typeof") == 0) {
            return pmm_type_named(ctx->arena, "System.Type");
        }
        ...
        const char *ns = cs_namespace_qn(ctx);
        if (ns && *ns) {
            const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ns, bare);
        ...
        for (int i = 0; i < ctx->using_count; i++) {
            const CBMCSUsing *u = &ctx->usings[i];
            if (u->kind != PMM_CS_USING_STATIC) continue;
            const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", u->target_qn, bare);
            const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, qn);
            if (f && f->signature && f->signature->data.func.return_types &&
                f->signature->data.func.return_types[0]) {
                return f->signature->data.func.return_types[0];
...
static const CBMType *cs_eval_member_access_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(obj) || ts_node_is_null(name)) return pmm_type_unknown();
    ...
    if (!recv) return pmm_type_unknown();
        ...
        for (int i = 0; rt->field_names[i]; i++) {
            if (strcmp(rt->field_names[i], fname) == 0) {
                if (rt->field_types && rt->field_types[i]) {
                    ...
                    if (recv->kind == PMM_TYPE_TEMPLATE && rt->type_param_names &&
    ...
    if (f && f->signature && f->signature->kind == PMM_TYPE_FUNC &&
        ...
        const CBMType *ret = f->signature->data.func.return_types[0];
        if (recv->kind == PMM_TYPE_TEMPLATE && rt && rt->type_param_names &&
            recv->data.template_type.template_args) {
...
static const CBMType *cs_eval_object_creation_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(tnode)) return pmm_type_unknown();
...
static void cs_process_local_decl(CSLSPContext *ctx, TSNode node) {
    ...
    const CBMType *declared_type = ts_node_is_null(tnode) ? pmm_type_unknown()
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (!rhs_t || rhs_t->kind == PMM_TYPE_UNKNOWN) {
            ...
            for (uint32_t j = 0; j < cc; j++) {
                ...
                if (strcmp(ck, "equals_value_clause") == 0) {
                    ...
                    if (!ts_node_is_null(rhs)) {
                        ...
                        if (t && t->kind != PMM_TYPE_UNKNOWN) rhs_t = t;
                    }
                    break;
                }
                if (seen_named == 0) {
        ...
        if (!bind) bind = pmm_type_unknown();
        pmm_scope_bind(ctx->current_scope, vname, bind);
    }
}
...
static void cs_process_foreach(CSLSPContext *ctx, TSNode node) {
    ...
    const CBMType *element_t = pmm_type_unknown();
    if (!ts_node_is_null(iter)) {
        const CBMType *iter_t = cs_eval_expr_type(ctx, iter);
        if (iter_t && iter_t->kind == PMM_TYPE_TEMPLATE) {
    ...
    if (!ts_node_is_null(tnode)) {
        ...
        if (!is_var && declared && declared->kind != PMM_TYPE_UNKNOWN) element_t = declared;
    }
    pmm_scope_bind(ctx->current_scope, vname, element_t);
}
...
static void cs_process_assignment(CSLSPContext *ctx, TSNode node) {
    ...
    if (!vname) return;
    const CBMType *t = cs_eval_expr_type(ctx, rhs);
    if (!t || t->kind == PMM_TYPE_UNKNOWN) return;
    /* Don't override more-specific bindings with weaker ones. */
    const CBMType *existing = pmm_scope_lookup(ctx->current_scope, vname);
    if (existing && existing->kind == PMM_TYPE_NAMED && t->kind == PMM_TYPE_UNKNOWN) return;
        ...
        if (recv_text) {
            ...
            if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
                ...
                cs_emit_resolved(ctx,
                                  pmm_arena_sprintf(ctx->arena, "%s.%s", type_qn, bare),
...
static void cs_resolve_object_creation(CSLSPContext *ctx, TSNode call) {
    ...
    const char *tqn = NULL;
    if (t && t->kind == PMM_TYPE_NAMED) tqn = t->data.named.qualified_name;
    else if (t && t->kind == PMM_TYPE_TEMPLATE) tqn = t->data.template_type.template_name;
    if (!tqn) return;
...
static void cs_resolve_calls_in_node(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "block") == 0) {
        ...
        ctx->current_scope = pmm_scope_push(ctx->arena, prev);
...
static void cs_collect_type_params(CSLSPContext *ctx, TSNode node, const char ***out_names,
    ...
    *out_count = 0;
    ...
    int cap = 4;
    const char **arr = (const char **)pmm_arena_alloc(ctx->arena, (size_t)cap * sizeof(*arr));
    if (!arr) return;
    int n = 0;
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (n + 1 >= cap) {
            int new_cap = cap * 2;
            const char **ne =
                (const char **)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*ne));
            if (!ne) break;
...
static void cs_process_function_like(CSLSPContext *ctx, TSNode node) {
    ...
    if (short_name) {
        if (ctx->enclosing_class_qn) {
            ctx->enclosing_func_qn =
                pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, short_name);
        } else if (ctx->module_qn) {
            ctx->enclosing_func_qn =
                pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, short_name);
        } else {
            ctx->enclosing_func_qn = pmm_arena_strdup(ctx->arena, short_name);
        }
    }
    ...
    if (is_method) {
        ...
        if (tp_count > 0) {
            const CBMType **args =
                (const CBMType **)pmm_arena_alloc(ctx->arena,
                                                  (size_t)(tp_count + 1) * sizeof(*args));
            if (args) {
                for (int i = 0; i < tp_count; i++) {
                    args[i] = pmm_type_type_param(ctx->arena, tp_names[i]);
                }
                args[tp_count] = NULL;
...
static void cs_process_type_decl(CSLSPContext *ctx, TSNode node) {
    ...
    if (ctx->module_qn) {
        ctx->enclosing_class_qn =
            pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
    ...
    /* Base list -> enclosing_base_qn + enclosing_iface_qns. */
    {
        ...
        if (!ts_node_is_null(bl)) {
            ...
            int icap = 4, icount = 0;
            const char **ifs =
                (const char **)pmm_arena_alloc(ctx->arena, (size_t)icap * sizeof(*ifs));
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (rt && rt->is_interface) {
                    if (icount + 1 >= icap) {
                        int nc2 = icap * 2;
                        const char **ne = (const char **)pmm_arena_alloc(
    ...
    if (!is_enum) {
        ...
        if (!ts_node_is_null(pctor)) {
            ...
            ctx->enclosing_func_qn =
                pmm_arena_sprintf(ctx->arena, "%s..ctor", ctx->enclosing_class_qn);
...
static void cs_collect_imports(CSLSPContext *ctx, TSNode root) {
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "using_directive") == 0) {
            ...
            if (target) {
                if (is_alias) {
                    cs_lsp_add_using(ctx, PMM_CS_USING_ALIAS, alias_name ? alias_name : "",
                                      target, is_global);
                } else if (is_static) {
                    cs_lsp_add_using(ctx, PMM_CS_USING_STATIC, "", target, is_global);
                } else {
                    cs_lsp_add_using(ctx, PMM_CS_USING_NAMESPACE, "", target, is_global);
                }
            }
        }
...
static void cs_parse_signature(CBMArena *arena, const char *signature,
    ...
    *out_types = NULL;
    ...
    int cap = 8;
    int count = 0;
    const char **names = (const char **)pmm_arena_alloc(arena, (size_t)cap * sizeof(*names));
    const CBMType **types = (const CBMType **)pmm_arena_alloc(arena, (size_t)cap * sizeof(*types));
    ...
    while (*p && *p != ')') {
        ...
        if (!last_space) {
            ...
            char *type_text = pmm_arena_strndup(arena, type_start, (size_t)(name_end - type_start));
            const CBMType *t = pmm_type_unknown();
            if (ctx) t = cs_resolve_type_name(ctx, type_text)
                          ? pmm_type_named(ctx->arena, cs_resolve_type_name(ctx, type_text))
                          : pmm_type_unknown();
            (void)t;
            if (count + 1 >= cap) break;
            names[count] = pmm_arena_sprintf(arena, "_arg%d", count);
            types[count] = t;
            count++;
        } else {
            char *type_text = pmm_arena_strndup(arena, type_start, (size_t)(last_space - type_start));
            char *pname = pmm_arena_strndup(arena, last_space + 1, (size_t)(name_end - last_space - 1));
            const CBMType *t = pmm_type_unknown();
            if (ctx) {
                const char *resolved = cs_resolve_type_name(ctx, type_text);
                if (resolved) t = pmm_type_named(ctx->arena, resolved);
            }
            if (count + 1 >= cap) break;
            names[count] = pname;
            types[count] = t;
...
static cs_fields_t *cs_fields_get(CBMArena *arena, cs_fields_table_t *tab,
    ...
    if (tab->count + 1 >= tab->cap) {
        ...
        cs_fields_t *ne =
            (cs_fields_t *)pmm_arena_alloc(arena, (size_t)new_cap * sizeof(*ne));
        if (!ne) return NULL;
    ...
    slot->cap = 8;
    slot->field_names =
        (const char **)pmm_arena_alloc(arena, (size_t)slot->cap * sizeof(*slot->field_names));
    slot->field_types =
        (const CBMType **)pmm_arena_alloc(arena, (size_t)slot->cap * sizeof(*slot->field_types));
    return slot;
}
...
static void cs_fields_add_debug(CBMArena *arena, cs_fields_t *f, const char *name,
    ...
    if (f->count + 2 >= f->cap) {
        int new_cap = f->cap * 2;
        const char **nn = (const char **)pmm_arena_alloc(arena, (size_t)new_cap * sizeof(*nn));
        const CBMType **nt = (const CBMType **)pmm_arena_alloc(arena, (size_t)new_cap * sizeof(*nt));
        if (!nn || !nt) return;
        for (int i = 0; i < f->count; i++) {
    ...
    f->field_types[f->count] = type ? type : pmm_type_unknown();
    f->count++;
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "field_declaration") == 0 || strcmp(k, "property_declaration") == 0 ||
            ...
            if (cls_short) {
                const char *cls_qn =
                    ctx->module_qn
                        ? pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cls_short)
                        : pmm_arena_strdup(ctx->arena, cls_short);
...
static void cs_method_rt_add(CBMArena *arena, cs_method_rt_table_t *tab,
    ...
    if (tab->count + 1 >= tab->cap) {
        ...
        cs_method_rt_entry_t *ne = (cs_method_rt_entry_t *)pmm_arena_alloc(
            ...
            if (is_indexer) {
                ...
                if (!ts_node_is_null(tnode)) {
                    ...
                    cs_method_rt_add(ctx->arena, tab,
                                      pmm_arena_sprintf(ctx->arena, "%s.%s", cls_qn,
                                                         short_name),
                                      rt);
                }
...
static const CBMType *cs_parse_return_type_text(CSLSPContext *ctx, const char *text) {
    if (!text || !*text) return pmm_type_unknown();
    const char *p = text;
    while (*p == ' ' || *p == ':') p++;
    if (!*p) return pmm_type_unknown();
    /* Strip trailing whitespace + '?' nullability. */
    size_t n = strlen(p);
    while (n > 0 && (p[n - 1] == ' ' || p[n - 1] == '?')) n--;
    if (n == 0) return pmm_type_unknown();
    char *trimmed = pmm_arena_strndup(ctx->arena, p, n);
...
/* ── single-file entry: pmm_run_cs_lsp ──────────────────────────── */

void pmm_run_cs_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
                    TSNode root) {
    ...
    /* Phase A: stdlib. */
    pmm_csharp_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (base[0] != '.' && !strchr(base, '.') && module_qn) {
                                q = pmm_arena_sprintf(arena, "%s.%s", module_qn, base);
                            }
                            emb[j] = q;
                        }
                        emb[bc] = NULL;
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            const CBMType *rt =
                d->return_type ? pmm_type_unknown() /* will refine below with ctx */
                               : pmm_type_unknown();
            const CBMType **rets =
        ...
        for (int j = 0; j < reg.func_count; j++) {
            if (strcmp(reg.funcs[j].qualified_name, d->qualified_name) == 0) {
                ...
                if (rets) {
                    rets[0] = rt;
                    rets[1] = NULL;
                    reg.funcs[j].signature = pmm_type_func(arena, NULL, NULL, rets);
                }
                break;
            }
        }
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0 ||
            ...
            if (d->embedded_types && *d->embedded_types) {
                ...
                const char **arr =
                    (const char **)pmm_arena_alloc(arena, (size_t)(n + 1) * sizeof(*arr));
                if (arr) {
                    int idx = 0;
                    ...
                    for (const char *p = d->embedded_types; ; p++) {
                        if (*p == '|' || *p == '\0') {
                            ...
                            if (len > 0) {
                                arr[idx++] = pmm_arena_strndup(arena, start, len);
                            }
                            if (*p == '\0') break;
                            start = p + 1;
                        }
...
CBMTypeRegistry *pmm_cs_build_cross_registry(CBMArena *arena, CBMLSPDef *defs, int def_count) {
    ...
    pmm_csharp_stdlib_register(reg, arena);
    for (int i = 0; i < def_count; i++) {
        if (defs[i].lang != PMM_LANG_CSHARP) continue;
...
void pmm_run_cs_lsp_cross_with_registry(CBMArena *arena, const char *source, int source_len,
    ...
    for (int i = 0; i < using_count; i++) {
        if (using_targets[i]) {
            cs_lsp_add_using(&ctx, PMM_CS_USING_NAMESPACE, "", using_targets[i], false);
        }
    }
    cs_lsp_process_file(&ctx, root);
...
void pmm_run_cs_lsp_cross(CBMArena *arena, const char *source, int source_len,
...
void pmm_batch_cs_lsp_cross(CBMArena *arena, CBMBatchCSLSPFile *files, int file_count,
                             CBMResolvedCallArray *out) {
    if (!arena || !files) return;
    for (int i = 0; i < file_count; i++) {
        CBMBatchCSLSPFile *f = &files[i];
        pmm_run_cs_lsp_cross(arena, f->source, f->source_len, f->module_qn, f->defs, f->def_count,
