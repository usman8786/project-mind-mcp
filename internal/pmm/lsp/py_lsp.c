 ...
 * wrappers — never call pmm_scope_bind(ctx->current_scope, ...) or assign
...
void py_lsp_add_import(PyLSPContext *ctx, const char *local_name, const char *module_qn) {
    ...
    const char **names =
        (const char **)pmm_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    const char **qns =
        (const char **)pmm_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    if (!names || !qns)
        return;
...
static void py_bind_dotted_prefixes(PyLSPContext *ctx, const char *qn) {
    ...
    for (;;) {
        ...
        char *prefix = (char *)pmm_arena_alloc(ctx->arena, prefix_len + 1);
        if (!prefix)
            return;
        ...
        if (pmm_type_is_unknown(pmm_scope_lookup(ctx->current_scope, bind_short))) {
            py_scope_bind(ctx, bind_short, pmm_type_module(ctx->arena, prefix));
        }
        p = dot + 1;
    }
}
...
void py_lsp_bind_imports(PyLSPContext *ctx) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        ...
        if (import_is_from_style(local, qn)) {
            ...
            t = pmm_type_module(ctx->arena, qn);
        }
...
const CBMType *py_lsp_lookup_in_scope(const PyLSPContext *ctx, const char *name) {
    ...
    return pmm_scope_lookup(ctx->current_scope, name);
}
...
static char *py_node_text(PyLSPContext *ctx, TSNode node) {
    ...
    char *out = (char *)pmm_arena_alloc(ctx->arena, len + 1);
    if (!out)
        return NULL;
...
static void py_register_lambda(PyLSPContext *ctx, const char *name, TSNode lambda_node) {
    ...
    if (ctx->lambda_count >= ctx->lambda_cap) {
        ...
        CBMLambdaEntry *grown =
            (CBMLambdaEntry *)pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(CBMLambdaEntry));
        if (!grown)
            return;
    ...
    ctx->lambdas[ctx->lambda_count].name = pmm_arena_strdup(ctx->arena, name);
...
static void py_register_dict_literal(PyLSPContext *ctx, const char *var, TSNode dict_node) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const CBMRegisteredFunc *f =
            pmm_registry_lookup_symbol(ctx->registry, ctx->module_qn, val_text);
        if (!f) {
            f = pmm_registry_lookup_symbol(ctx->registry, "builtins", val_text);
        }
        if (!f)
            continue;
        ...
        if (ctx->dict_literal_count >= ctx->dict_literal_cap) {
            ...
            CBMDictLiteralEntry *grown = (CBMDictLiteralEntry *)pmm_arena_alloc(
        ...
        ctx->dict_literals[ctx->dict_literal_count].var_name = pmm_arena_strdup(ctx->arena, var);
        ctx->dict_literals[ctx->dict_literal_count].literal_key =
            pmm_arena_strdup(ctx->arena, key_text);
        ctx->dict_literals[ctx->dict_literal_count].target_qn =
            pmm_arena_strdup(ctx->arena, f->qualified_name);
...
static void py_emit_resolved_call_reason(PyLSPContext *ctx, const char *callee_qn,
    ...
    rc.callee_qn = pmm_arena_strdup(ctx->arena, callee_qn);
    rc.strategy = strategy;
    rc.confidence = confidence;
    rc.reason = reason ? pmm_arena_strdup(ctx->arena, reason) : NULL;
...
static void py_overlay_register_field(PyLSPContext *ctx, const char *class_qn,
    ...
    if (ctx->field_overlay_count >= ctx->field_overlay_cap) {
        ...
        void *na = pmm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*ctx->field_overlay));
        if (!na)
    ...
    ctx->field_overlay[ctx->field_overlay_count].class_qn = pmm_arena_strdup(ctx->arena, class_qn);
    ctx->field_overlay[ctx->field_overlay_count].field_name =
        pmm_arena_strdup(ctx->arena, field_name);
...
static void py_register_instance_field(PyLSPContext *ctx, const char *class_qn,
    ...
    // Find the type entry. pmm_registry_lookup_type returns a const pointer;
    ...
    const char **new_names =
        (const char **)pmm_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    const CBMType **new_types = (const CBMType **)pmm_arena_alloc(
    ...
    new_names[existing] = pmm_arena_strdup(ctx->arena, field_name);
...
static const CBMType *py_literal_type(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "list_comprehension") == 0)
        return pmm_type_builtin(ctx->arena, "list");
    if (strcmp(k, "dictionary_comprehension") == 0)
        return pmm_type_builtin(ctx->arena, "dict");
    if (strcmp(k, "set_comprehension") == 0)
        return pmm_type_builtin(ctx->arena, "set");
    if (strcmp(k, "generator_expression") == 0)
        return pmm_type_builtin(ctx->arena, "generator");
    return NULL;
}
...
static const CBMType *py_substitute_self(PyLSPContext *ctx, const CBMType *t,
    ...
    if (t->kind == PMM_TYPE_NAMED && t->data.named.qualified_name) {
        ...
        if (strcmp(qn, "Self") == 0 || strcmp(qn, "typing.Self") == 0 ||
            ...
            return pmm_type_named(ctx->arena, receiver_qn);
        }
    }
    if (t->kind == PMM_TYPE_UNION) {
        int n = t->data.union_type.count;
        const CBMType **members = (const CBMType **)pmm_arena_alloc(
        ...
        return pmm_type_union(ctx->arena, members, n);
    }
    return t;
}
...
static const CBMType *py_func_return_type_recv(PyLSPContext *ctx, const char *func_qn,
    ...
    if (!ctx || !func_qn)
        return pmm_type_unknown();
    const CBMRegisteredFunc *f = pmm_registry_lookup_func(ctx->registry, func_qn);
    if (!f || !f->signature)
        return pmm_type_unknown();
    if (f->signature->kind != PMM_TYPE_FUNC)
        return pmm_type_unknown();
    ...
    if (!rets[1]) {
        ...
        while (rets[count])
            count++;
        base = pmm_type_tuple(ctx->arena, rets, count);
    }
    if (receiver_qn) {
...
static void py_bind_for_target(PyLSPContext *ctx, TSNode left, const CBMType *elem_type) {
    ...
    if (strcmp(lk, "pattern_list") == 0 || strcmp(lk, "tuple_pattern") == 0 ||
        ...
        int count = 0;
        if (elem_type) {
            if (elem_type->kind == PMM_TYPE_TUPLE) {
                elems = elem_type->data.tuple.elems;
                count = elem_type->data.tuple.count;
            } else if (elem_type->kind == PMM_TYPE_TEMPLATE &&
...
static const CBMType *py_iterable_element_type(PyLSPContext *ctx, const CBMType *iter_type) {
    if (!iter_type)
        return pmm_type_unknown();
    if (iter_type->kind == PMM_TYPE_TEMPLATE) {
        ...
        if (strcmp(name, "ItemsView") == 0 || strcmp(name, "dict_items") == 0) {
            ...
            if (n >= 2) {
                ...
                return pmm_type_tuple(ctx->arena, elems, 2);
            }
            return pmm_type_unknown();
        }
        ...
        if (strcmp(name, "tuple") == 0) {
            if (n == 1)
                return args[0];
            return pmm_type_union(ctx->arena, args, n);
        }
...
static const CBMType *py_eval_expr_type_uncached(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "tuple") == 0) {
        ...
        if (cn == 0)
            return pmm_type_builtin(ctx->arena, "tuple");
        const CBMType **elems = (const CBMType **)pmm_arena_alloc(
        ...
        elems[cn] = NULL;
        return pmm_type_tuple(ctx->arena, elems, (int)cn);
    }
    if (strcmp(k, "list") == 0) {
        uint32_t cn = ts_node_named_child_count(node);
        if (cn == 0)
        ...
        if (!first || pmm_type_is_unknown(first))
            return pmm_type_builtin(ctx->arena, "list");
        return pmm_type_template(ctx->arena, "list", &first, 1);
    }
    if (strcmp(k, "set") == 0) {
        uint32_t cn = ts_node_named_child_count(node);
        if (cn == 0)
        ...
        if (!first || pmm_type_is_unknown(first))
            return pmm_type_builtin(ctx->arena, "set");
        return pmm_type_template(ctx->arena, "set", &first, 1);
    }
    if (strcmp(k, "dictionary") == 0) {
        ...
        for (uint32_t i = 0; i < cn; i++) {
            ...
            return pmm_type_template(ctx->arena, "dict", args, 2);
        }
        return pmm_type_builtin(ctx->arena, "dict");
    }
    ...
    if (strcmp(k, "identifier") == 0) {
        ...
        f = pmm_registry_lookup_symbol(ctx->registry, "builtins", name);
        if (f)
            return py_func_return_type(ctx, f->qualified_name);
        const CBMRegisteredType *rt = pmm_registry_lookup_type(
            ctx->registry, pmm_arena_sprintf(ctx->arena, "builtins.%s", name));
        if (rt)
    ...
    if (strcmp(k, "attribute") == 0) {
        ...
        if (obj_type)
            obj_type = pmm_type_resolve_alias(obj_type);
        char *attr_name = py_node_text(ctx, attr);
        if (!attr_name || !obj_type)
            return pmm_type_unknown();

        if (obj_type->kind == PMM_TYPE_MODULE) {
            // module.attr — return type of the registered symbol if known.
            const char *mod = obj_type->data.module.module_qn;
            const CBMRegisteredFunc *f = pmm_registry_lookup_symbol(ctx->registry, mod, attr_name);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Could also be a class — return NAMED("mod.attr")
            const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", mod, attr_name);
            ...
            const char *prefix = pmm_arena_sprintf(ctx->arena, "%s.", qn);
            ...
            if (is_submodule)
                return pmm_type_module(ctx->arena, qn);
            return pmm_type_unknown();
        }
        if (obj_type->kind == PMM_TYPE_NAMED) {
            const CBMRegisteredFunc *f =
        ...
        if (obj_type->kind == PMM_TYPE_BUILTIN && obj_type->data.builtin.name) {
            // Builtins map to typeshed stdlib via "builtins.<typename>".
            const char *recv_qn =
                pmm_arena_sprintf(ctx->arena, "builtins.%s", obj_type->data.builtin.name);
        ...
        if (obj_type->kind == PMM_TYPE_TEMPLATE && obj_type->data.template_type.template_name) {
            ...
            const char *recv_qn = pmm_arena_sprintf(ctx->arena, "builtins.%s", tname);
            const CBMRegisteredFunc *f = py_lookup_attribute(ctx, recv_qn, attr_name);
            if (!f) {
        ...
        if (obj_type->kind == PMM_TYPE_UNION) {
            int matches = 0;
    ...
    if (strcmp(k, "call") == 0) {
        ...
        if (strcmp(fk, "attribute") == 0) {
            ...
            if (!ts_node_is_null(obj) && !ts_node_is_null(attr)) {
                ...
                if (mname && obj_type && obj_type->kind == PMM_TYPE_TEMPLATE) {
                    const char *tname = obj_type->data.template_type.template_name;
                    ...
                    if (is_dict_like && args && n >= 2) {
                        if (strcmp(mname, "items") == 0) {
                            ...
                            return pmm_type_template(ctx->arena, "ItemsView", pair, 2);
                        }
                        if (strcmp(mname, "keys") == 0) {
                            const CBMType *k1[2] = {args[0], NULL};
                            return pmm_type_template(ctx->arena, "KeysView", k1, 1);
                        }
                        if (strcmp(mname, "values") == 0) {
                            const CBMType *v1[2] = {args[1], NULL};
                            return pmm_type_template(ctx->arena, "ValuesView", v1, 1);
                        }
                        if (strcmp(mname, "get") == 0) {
                            // dict.get(k) -> Optional[V]
                            return pmm_type_optional(ctx->arena, args[1]);
                        }
        ...
        if (strcmp(fk, "identifier") == 0) {
            ...
            const CBMType *in_scope = pmm_scope_lookup(ctx->current_scope, fname);
            if (!pmm_type_is_unknown(in_scope)) {
                if (in_scope->kind == PMM_TYPE_NAMED) {
                    const CBMRegisteredType *rt = pmm_registry_lookup_type(
                ...
                if (in_scope->kind == PMM_TYPE_CALLABLE) {
                ...
                if (in_scope->kind == PMM_TYPE_TEMPLATE &&
            ...
            const CBMRegisteredFunc *f =
                pmm_registry_lookup_symbol(ctx->registry, ctx->module_qn, fname);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Builtins fallback: range / len / list / str / int / etc.
            f = pmm_registry_lookup_symbol(ctx->registry, "builtins", fname);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Builtin class constructor: list() / dict() / set() / str() / ...
            const CBMRegisteredType *rt = pmm_registry_lookup_type(
                ctx->registry, pmm_arena_sprintf(ctx->arena, "builtins.%s", fname));
            if (rt)
                return pmm_type_builtin(ctx->arena, fname);
        }
        ...
        if (strcmp(fk, "call") == 0 || strcmp(fk, "parenthesized_expression") == 0 ||
            ...
            if (fn_type && fn_type->kind == PMM_TYPE_CALLABLE) {
                return fn_type->data.callable.return_type ? fn_type->data.callable.return_type
                                                          : pmm_type_unknown();
            }
            if (fn_type && fn_type->kind == PMM_TYPE_TEMPLATE &&
        ...
        if (strcmp(fk, "attribute") == 0) {
            ...
            if (obj_type->kind == PMM_TYPE_MODULE) {
                const char *mod = obj_type->data.module.module_qn;
                const CBMRegisteredFunc *f =
                    pmm_registry_lookup_symbol(ctx->registry, mod, attr_name);
                if (f)
                    return py_func_return_type(ctx, f->qualified_name);
            }
            if (obj_type->kind == PMM_TYPE_NAMED) {
    ...
    if (strcmp(k, "binary_operator") == 0) {
        ...
        if (lt && lt->kind == PMM_TYPE_NAMED && !ts_node_is_null(op)) {
    ...
    if (strcmp(k, "conditional_expression") == 0) {
        ...
        if (nc >= 2) {
            ...
            return pmm_type_union(ctx->arena, members, 2);
        }
    }
    ...
    if (strcmp(k, "subscript") == 0) {
        ...
        if (container && container->kind == PMM_TYPE_NAMED) {
        ...
        if (!container || container->kind != PMM_TYPE_TEMPLATE) {
            return pmm_type_unknown();
        }
 ...
 * store NULL (the wrapper normalizes to pmm_type_unknown() first), so NULL
...
static bool py_type_cache_grow(PyLSPContext *ctx) {
    ...
    CBMPyTypeCacheEntry *new_entries = (CBMPyTypeCacheEntry *)pmm_arena_alloc(
...
static void py_process_statement(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "assignment") == 0) {
        ...
        const CBMType *rhs_type =
            ts_node_is_null(right) ? pmm_type_unknown() : py_eval_expr_type(ctx, right);
        ...
        if (strcmp(lk, "pattern_list") == 0 || strcmp(lk, "tuple_pattern") == 0 ||
            ...
            int rhs_count = 0;
            if (rhs_type) {
                if (rhs_type->kind == PMM_TYPE_TUPLE) {
                    rhs_elems = rhs_type->data.tuple.elems;
                    rhs_count = rhs_type->data.tuple.count;
                } else if (rhs_type->kind == PMM_TYPE_TEMPLATE &&
            ...
            for (uint32_t i = 0; i < lc; i++) {
                ...
                const CBMType *bind_type;
                if (is_rest) {
                    bind_type = elem_type ? pmm_type_template(ctx->arena, "list", &elem_type, 1)
                                          : pmm_type_unknown();
                } else if (rhs_elems && (int)i < rhs_count && rhs_elems[i]) {
                    bind_type = rhs_elems[i];
                } else {
                    bind_type = elem_type ? elem_type : pmm_type_unknown();
                }
                py_scope_bind(ctx, nm, bind_type);
            }
            return;
    ...
    if (strcmp(k, "for_statement") == 0) {
        ...
        if (!ts_node_is_null(right)) {
            ...
            if (pmm_type_is_unknown(elem_type) && iter_type && iter_type->kind == PMM_TYPE_NAMED) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            for (uint32_t j = 0; j < cn; j++) {
                ...
                const CBMType *cm_type =
                    ts_node_is_null(value) ? pmm_type_unknown() : py_eval_expr_type(ctx, value);
                const CBMType *bind_type = cm_type;
                if (cm_type && cm_type->kind == PMM_TYPE_NAMED) {
...
static void py_emit_call_for(PyLSPContext *ctx, TSNode call_node) {
    ...
    if (strcmp(fk, "identifier") == 0) {
        ...
        if (!ts_node_is_null(lambda_node)) {
            ...
            ctx->enclosing_func_qn = pmm_arena_sprintf(ctx->arena, "%s.<lambda>",
        ...
        const CBMType *in_scope = pmm_scope_lookup(ctx->current_scope, fname);
        if (!pmm_type_is_unknown(in_scope) && in_scope->kind == PMM_TYPE_NAMED) {
    ...
    if (strcmp(fk, "attribute") == 0) {
                        ...
                        for (int i = 0; enclosing->embedded_types[i]; i++) {
                            ...
                            if (strcmp(attr_name, "__init__") == 0) {
                                ...
                                const char *init_qn =
                                       ...
                                       : pmm_arena_sprintf(ctx->arena, "%s.__init__",
            ...
            if (mod && ctx->module_qn) {
                ...
                if (last_dot && last_dot > ctx->module_qn) {
                    ...
                    if (!(strncmp(mod, ctx->module_qn, root_len) == 0 && mod[root_len] == '.')) {
                        char *qual_mod = (char *)pmm_arena_alloc(ctx->arena, root_len + 1 +
                                                                                strlen(mod) + 1);
                        if (qual_mod) {
                            ...
                            const CBMRegisteredFunc *qf =
                                pmm_registry_lookup_symbol(ctx->registry, qual_mod, attr_name);
                            if (qf) {
        ...
        if (obj_type->kind == PMM_TYPE_UNION) {
            ...
            for (int i = 0; i < obj_type->data.union_type.count; i++) {
                ...
                if (!m)
                    continue;
                if (m->kind == PMM_TYPE_BUILTIN && m->data.builtin.name &&
                    strcmp(m->data.builtin.name, "None") == 0)
                    continue;
                if (m->kind != PMM_TYPE_NAMED)
                    continue;
                const CBMRegisteredFunc *f =
                    py_lookup_attribute(ctx, m->data.named.qualified_name, attr_name);
                if (f) {
...
static const CBMType *py_strip_none(PyLSPContext *ctx, const CBMType *t) {
    ...
    int retained = 0;
    const CBMType **kept =
        (const CBMType **)pmm_arena_alloc(ctx->arena, (size_t)(n + 1) * sizeof(const CBMType *));
    if (!kept)
        return t;
    for (int i = 0; i < n; i++) {
        const CBMType *m = t->data.union_type.members[i];
        if (m && m->kind == PMM_TYPE_BUILTIN && m->data.builtin.name &&
            strcmp(m->data.builtin.name, "None") == 0)
            continue;
        kept[retained++] = m;
    }
    ...
    if (retained == 1)
        return kept[0];
    return pmm_type_union(ctx->arena, kept, retained);
}
...
static void py_walk_if_statement(PyLSPContext *ctx, TSNode if_node) {
    ...
    if (!ts_node_is_null(body)) {
        ...
        if (none_kind == 1 && none_var) {
            const CBMType *current = pmm_scope_lookup(ctx->current_scope, none_var);
            if (current && !pmm_type_is_unknown(current)) {
    ...
    if (!ts_node_is_null(body) && py_block_terminates(body)) {
        ...
        if (neg_none_kind == -1 && neg_none_var) {
            ...
            const CBMType *current = pmm_scope_lookup(ctx->current_scope, neg_none_var);
            if (current && !pmm_type_is_unknown(current)) {
                const CBMType *narrowed = py_strip_none(ctx, current);
...
static void py_emit_dunder_call(PyLSPContext *ctx, const CBMType *recv, const char *dunder) {
    if (!recv || recv->kind != PMM_TYPE_NAMED || !dunder)
        return;
    ...
    if (f && f->qualified_name) {
        ...
        if (ctx->syn_calls && ctx->arena && ctx->enclosing_func_qn) {
            ...
            call.callee_name = pmm_arena_strdup(ctx->arena, dunder);
        ...
        if (!ts_node_is_null(body)) {
            ...
            for (uint32_t i = 0; i < bcc; i++) {
                    ...
                    for (uint32_t j = 1; j < ts_node_named_child_count(actual_pattern); j++) {
                        ...
                        if (strcmp(ts_node_type(sub), "capture_pattern") == 0) {
                            if (ts_node_named_child_count(sub) > 0) {
                                ...
                                if (nm)
                                    py_scope_bind(ctx, nm, pmm_type_unknown());
                            }
                        }
                    }
                }
                ...
                if (subject_name && !ts_node_is_null(actual_pattern) &&
                    ...
                    const CBMType *subj_t = pmm_scope_lookup(ctx->current_scope, subject_name);
                    ...
                    for (uint32_t j = 0; j < pn; j++) {
                        ...
                        if (strcmp(ek, "splat_pattern") == 0 ||
                            ...
                            if (!ts_node_is_null(id)) {
                                ...
                                if (nm && elem_t) {
                                    const CBMType *lst =
                                        pmm_type_template(ctx->arena, "list", &elem_t, 1);
...
static const char **py_split_subscript_args(CBMArena *arena, const char *s, int *out_n) {
    ...
    int cap = 8;
    const char **out =
        (const char **)pmm_arena_alloc(arena, (size_t)(cap + 1) * sizeof(const char *));
    if (!out) {
        *out_n = 0;
        return NULL;
    }
    ...
    for (size_t i = 0; i <= len; i++) {
        ...
        if (c == '[' || c == '(' || c == '{')
            ...
            if (part && part[0]) {
                if (n >= cap) {
                    int new_cap = cap * 2;
                    const char **grown = (const char **)pmm_arena_alloc(
...
static const CBMType *py_parse_type_text_qn(CBMArena *arena, const char *ann,
    ...
    if (len >= 2 && (ann[0] == '"' || ann[0] == '\'') && ann[len - 1] == ann[0]) {
        char *unquoted = (char *)pmm_arena_alloc(arena, len - 1);
        if (unquoted) {
    ...
    if (lb && len > 0 && ann[len - 1] == ']') {
        ...
        char *base = (char *)pmm_arena_alloc(arena, base_len + 1);
        if (base) {
            ...
            char *args_text = (char *)pmm_arena_alloc(arena, inner_len + 1);
            if (args_text) {
                ...
                if (arg_n > 0 && arg_strs) {
                    arg_types = (const CBMType **)pmm_arena_alloc(
                ...
                if (strcmp(btrim, "Optional") == 0 || strcmp(btrim, "typing.Optional") == 0) {
                    if (arg_types && arg_n >= 1 && arg_types[0]) {
                        return pmm_type_optional(arena, arg_types[0]);
                    }
                }
                if (strcmp(btrim, "Union") == 0 || strcmp(btrim, "typing.Union") == 0) {
                    if (arg_types && arg_n > 0) {
                        return pmm_type_union(arena, arg_types, arg_n);
                    }
                }
                ...
                if (arg_types && arg_n > 0) {
                    return pmm_type_template(arena, btrim, arg_types, arg_n);
                }
        ...
        if (out) {
            ...
            if (seen_pipe && onum >= 2) {
                ...
                if (members) {
                    ...
                    return pmm_type_union(arena, members, onum);
                }
            }
        }
    }
    ...
    for (int i = 0; builtins[i]; i++) {
        if (strcmp(ann, builtins[i]) == 0) {
            return pmm_type_builtin(arena, ann);
        }
    }
    ...
    if (module_qn && !strchr(ann, '.')) {
        ...
        if (!is_typing) {
            const char *qn = pmm_arena_sprintf(arena, "%s.%s", module_qn, ann);
            return pmm_type_named(arena, qn);
        }
    }
    return pmm_type_named(arena, ann);
}
...
static const CBMType *py_resolve_annotation(PyLSPContext *ctx, const char *ann) {
    ...
    size_t len = strlen(ann);
    if (len >= 2 && (ann[0] == '"' || ann[0] == '\'') && ann[len - 1] == ann[0]) {
        char *unquoted = (char *)pmm_arena_alloc(ctx->arena, len - 1);
        if (unquoted) {
            memcpy(unquoted, ann + 1, len - 2);
            unquoted[len - 2] = '\0';
    ...
    if (lb && len > 0 && ann[len - 1] == ']') {
        size_t base_len = (size_t)(lb - ann);
        char *base = (char *)pmm_arena_alloc(ctx->arena, base_len + 1);
        if (base) {
            memcpy(base, ann, base_len);
            base[base_len] = '\0';
            ...
            size_t inner_start = (size_t)(lb - ann) + 1;
            size_t inner_len = len - inner_start - 1;
            char *args_text = (char *)pmm_arena_alloc(ctx->arena, inner_len + 1);
            if (args_text) {
                memcpy(args_text, ann + inner_start, inner_len);
                args_text[inner_len] = '\0';
                int arg_n = 0;
                ...
                if (strcmp(btrim, "Optional") == 0 || strcmp(btrim, "typing.Optional") == 0) {
                    if (arg_types && arg_n >= 1 && arg_types[0]) {
                        return pmm_type_optional(ctx->arena, arg_types[0]);
                    }
                }
                // Union[A, B, ...] -> UNION
                if (strcmp(btrim, "Union") == 0 || strcmp(btrim, "typing.Union") == 0) {
                    if (arg_types && arg_n > 0) {
                        return pmm_type_union(ctx->arena, arg_types, arg_n);
                    }
                }
                ...
                if (arg_types && arg_n > 0) {
                    return pmm_type_template(ctx->arena, btrim, arg_types, arg_n);
                }
    ...
    // X | Y top-level union (PEP 604).
    {
        ...
        for (size_t i = 0; i < len; i++) {
            ...
            if (c == '[' || c == '(' || c == '{')
                ...
                int cap = 4;
                const char **out = (const char **)pmm_arena_alloc(
                ...
                if (onum >= 2) {
                    ...
                    return pmm_type_union(ctx->arena, members, onum);
                }
                break;
            }
        }
    ...
    const CBMType *t = pmm_scope_lookup(ctx->current_scope, ann);
    if (!pmm_type_is_unknown(t))
        return t;
    if (ctx->module_qn) {
        const char *qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, ann);
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return pmm_type_named(ctx->arena, qn);
    }
    ...
    for (int i = 0; builtins[i]; i++) {
        if (strcmp(ann, builtins[i]) == 0) {
            return pmm_type_builtin(ctx->arena, ann);
        }
    }
    return pmm_type_named(ctx->arena, ann);
}
...
static void py_bind_parameters(PyLSPContext *ctx, TSNode params) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (is_splat && t && !pmm_type_is_unknown(t)) {
            t = pmm_type_template(ctx->arena, "tuple", &t, 1);
        }
        if (is_kwargs && t && !pmm_type_is_unknown(t)) {
            const CBMType *str_t = pmm_type_builtin(ctx->arena, "str");
            const CBMType *args[3] = {str_t, t, NULL};
            t = pmm_type_template(ctx->arena, "dict", args, 2);
        }
        py_scope_bind(ctx, name, t);
    }
}
...
static void py_process_function(PyLSPContext *ctx, TSNode func_node, const char *container_qn) {
    ...
    ctx->enclosing_func_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", base_qn, fname);
...
static void py_process_class(PyLSPContext *ctx, TSNode class_node) {
    ...
    ctx->enclosing_class_qn = pmm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
...
static void py_bind_module_classes(PyLSPContext *ctx) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (qn[prefix_len] != '.')
            continue;
        py_scope_bind(ctx, sname, pmm_type_named(ctx->arena, qn));
    }
}
...
void py_lsp_process_file(PyLSPContext *ctx, TSNode root) {
    ...
    const char *prev_func = ctx->enclosing_func_qn;
    ctx->enclosing_func_qn = pmm_arena_sprintf(ctx->arena, "%s.__module__", ctx->module_qn);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode c = ts_node_named_child(root, i);
        const char *ck = ts_node_type(c);
        if (strcmp(ck, "function_definition") == 0) {
...
static bool py_register_def(CBMArena *arena, CBMTypeRegistry *reg, CBMDefinition *d,
                ...
                for (size_t k = lo; k <= hi; k++) {
                    ...
                    if (c == '[' || c == '(')
                        ...
                        if (eq >= name_end && name_end > s) {
                            ...
                            char *bname = (char *)pmm_arena_alloc(arena, blen + 1);
                            if (bname) {
                                ...
                                if (n < cap) {
                                    embedded[n++] =
                                        strchr(bname, '.')
                                            ? bname
                                            : pmm_arena_sprintf(arena, "%s.%s", module_qn, bname);
                                }
                            }
                        }
                        start = k + 1;
    ...
    if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            if (dec_count > 0) {
                ...
                for (int j = 0; j < dec_count; j++) {
                    ...
                    if (strcmp(short_dec, "property") == 0)
                        rf.flags |= PMM_FUNC_FLAG_PROPERTY;
                    else if (strcmp(short_dec, "classmethod") == 0)
                        rf.flags |= PMM_FUNC_FLAG_CLASSMETHOD;
                    else if (strcmp(short_dec, "staticmethod") == 0)
                        rf.flags |= PMM_FUNC_FLAG_STATICMETHOD;
                    else if (strcmp(short_dec, "abstractmethod") == 0)
                        rf.flags |= PMM_FUNC_FLAG_ABSTRACTMETHOD;
                    else if (strcmp(short_dec, "overload") == 0)
                        rf.flags |= PMM_FUNC_FLAG_OVERLOAD;
                    else if (strcmp(short_dec, "final") == 0)
                        rf.flags |= PMM_FUNC_FLAG_FINAL;
                }
        ...
        // is the pmm_node_text of the whole annotation node, which is the
        ...
        if (prefer_full) {
            ...
            while (d->return_types[count])
                count++;
            ret_types = (const CBMType **)pmm_arena_alloc(arena, (size_t)(count + 1) *
        ...
        rf.signature = pmm_type_func(arena, d->param_names, NULL, ret_types);
        ...
        if (strcmp(d->label, "Method") == 0) {
            ...
            if (last_dot && last_dot != qn) {
                ...
                rf.receiver_type = pmm_arena_strndup(arena, qn, prefix_len);
            }
        }
...
/* ── pmm_run_py_lsp: single-file entry point ──────────────────── */

void pmm_run_py_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
                    TSNode root) {
    if (!arena || !result)
        return;
     ...
     * graph node. We run inside pmm_extract_file, before the pipeline mints
    ...
    pmm_python_stdlib_register(&reg, arena);
...
static const char **py_split_pipe(CBMArena *arena, const char *text) {
    ...
    for (const char *p = text;; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            char *s = (char *)pmm_arena_alloc(arena, n + 1);
            if (!s)
                return NULL;
            memcpy(s, start, n);
            s[n] = '\0';
...
static void py_register_lsp_defs(CBMArena *arena, CBMArena *idx_arena, CBMTypeRegistry *reg,
     ...
     * registry arena. */
    if (idx_arena) {
        pmm_registry_finalize_into(reg, idx_arena);
    }
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            if (ret_strs) {
                ...
                if (n > 0) {
                    ...
                    for (int j = 0; j < n; j++) {
                        ret_types[j] = pmm_type_named(arena, ret_strs[j]);
                    }
                    ret_types[n] = NULL;
                }
            }
...
void pmm_run_py_lsp_cross(CBMArena *arena, const char *source, int source_len,
...
CBMTypeRegistry *pmm_py_build_cross_registry(CBMArena *arena, CBMLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
...
void pmm_run_py_lsp_cross_with_registry(CBMArena *arena, const char *source, int source_len,
...
void pmm_batch_py_lsp_cross(CBMArena *arena, CBMBatchPyLSPFile *files, int file_count,
                            CBMResolvedCallArray *out) {
    ...
    for (int f = 0; f < file_count; f++) {
        ...
        pmm_run_py_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
        ...
        if (file_out.count > 0) {
            ...
            out[f].items = (CBMResolvedCall *)pmm_arena_alloc(arena, (size_t)file_out.count *
            ...
            if (out[f].items) {
                for (int j = 0; j < file_out.count; j++) {
                    ...
                    dst->caller_qn =
                        src->caller_qn ? pmm_arena_strdup(arena, src->caller_qn) : NULL;
                    dst->callee_qn =
                        src->callee_qn ? pmm_arena_strdup(arena, src->callee_qn) : NULL;
...
#ifndef PMM_PYTHON_STDLIB_GENERATED
void pmm_python_stdlib_register(CBMTypeRegistry *reg, CBMArena *arena) {
    (void)reg;
    (void)arena;
}
#endif
