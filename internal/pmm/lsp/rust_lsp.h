 ...
 *
 * Public entry points:
 *   - `pmm_run_rust_lsp` — single-file resolution invoked from
 *     `pmm_extract_file()`.
 *   - `pmm_run_rust_lsp_cross` — cross-file resolution given a list of
 *     `CBMLSPDef`s gathered by the pipeline.
 *   - `pmm_batch_rust_lsp_cross` — batch wrapper that processes several
 *     files in one CGo call (per-file arenas + result copy).
 */
#ifndef PMM_LSP_RUST_LSP_H
#define PMM_LSP_RUST_LSP_H
 ...
 * scoring >= PMM_LSP_CONFIDENCE_FLOOR (0.6). Numbers here mirror the Go
 * LSP so the call-edge mix from a polyglot project stays comparable. */
#define PMM_RUST_CONF_DIRECT 0.95f      /* path::to::func or alias hit  */
#define PMM_RUST_CONF_METHOD 0.95f      /* inherent method dispatch     */
#define PMM_RUST_CONF_TRAIT_SOLE 0.92f  /* trait method, single impl    */
#define PMM_RUST_CONF_TRAIT_AMB 0.85f   /* trait method, many impls     */
#define PMM_RUST_CONF_UFCS 0.93f        /* T::method() / Self::new()    */
#define PMM_RUST_CONF_PROMOTED 0.90f    /* through Deref / blanket impl */
#define PMM_RUST_CONF_MACRO_KNOWN 0.85f /* known std macro mapped to fn */
#define PMM_RUST_CONF_OPERATOR 0.88f    /* a+b → T::add (operator trait) */

/* Rust-flavoured LSP context: one per file, lifetime tied to a single
 * `pmm_extract_file()` invocation (or the cross-file caller's arena). */
typedef struct {
    CBMArena *arena;
    const char *source;
    int source_len;
    ...
    /* PMM_LSP_DEBUG=1 in env enables verbose stderr trace. */
    bool debug;
 ...
 * `pmm_type_unknown()` for anything we cannot type. */
...
/* Entry point — called from `pmm_extract_file()` after the unified
 ...
 *
 * Called automatically from `pmm_run_rust_lsp_with_manifest`. */
void pmm_rust_synth_proc_macro_edges(CBMArena *arena, CBMFileResult *result);

/* Same as `pmm_run_rust_lsp`, but accepts an optional Cargo manifest
...
/* Same as `pmm_run_rust_lsp_cross`, plus an optional parsed Cargo manifest
 ...
 * not synthesise proc-macro edges). Mirrors pmm_run_c_lsp_cross_with_registry. */
