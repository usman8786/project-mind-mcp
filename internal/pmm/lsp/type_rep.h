#ifndef PMM_LSP_TYPE_REP_H
#define PMM_LSP_TYPE_REP_H
...
typedef enum {
    PMM_TYPE_UNKNOWN = 0,
    PMM_TYPE_NAMED,       // named type: "Database", "http.Request"
    PMM_TYPE_POINTER,     // *T
    PMM_TYPE_SLICE,       // []T
    PMM_TYPE_MAP,         // map[K]V
    PMM_TYPE_CHANNEL,     // chan T
    PMM_TYPE_FUNC,        // func(params) returns
    PMM_TYPE_INTERFACE,   // interface{...}
    PMM_TYPE_STRUCT,      // struct{...}
    PMM_TYPE_BUILTIN,     // int, string, bool, error, etc.
    PMM_TYPE_TUPLE,       // multi-return (T1, T2) / TS tuple [T,U]
    PMM_TYPE_TYPE_PARAM,  // generic type parameter: T, K, V
    PMM_TYPE_REFERENCE,   // T& (C++ lvalue reference)
    PMM_TYPE_RVALUE_REF,  // T&& (C++ rvalue reference)
    PMM_TYPE_TEMPLATE,    // Parameterized type: vector<T> — stores template name + args
    PMM_TYPE_ALIAS,       // Type alias: using/typedef — stores alias name + underlying type
    PMM_TYPE_UNION,       // Python: A | B; TS: A | B | C — sorted-canonical list (shared)
    PMM_TYPE_LITERAL,     // Python: Literal["foo", 3] — wraps a base type + literal value text
    PMM_TYPE_PROTOCOL,    // Python: typing.Protocol — like INTERFACE but matched structurally
    PMM_TYPE_MODULE,      // Python: import os; os is a module-typed binding
    PMM_TYPE_CALLABLE,    // Python: Callable[[A, B], R] — untyped-named callable variant of FUNC

    // --- TS-specific kinds (added in TS LSP integration) ---
    PMM_TYPE_INTERSECTION,  // TS: A & B — intersection type
    PMM_TYPE_TS_LITERAL,    // TS: "foo" / 42 / true literal types (tag+value layout, distinct
                            // from Python's PMM_TYPE_LITERAL which uses base+literal_text)
    PMM_TYPE_INDEXED,       // TS: T[K] — indexed access type
    PMM_TYPE_KEYOF,         // TS: keyof T
    PMM_TYPE_TYPEOF_QUERY,  // TS: typeof x in type position
    PMM_TYPE_CONDITIONAL,   // TS: T extends U ? X : Y
    PMM_TYPE_OBJECT_LIT,    // TS: { a: T1; b: T2 } anonymous object type
    PMM_TYPE_INFER,         // TS: `infer X` placeholder inside conditional
    PMM_TYPE_MAPPED,        // TS: {[K in keyof T]: ...} — v1 stub, members may be NULL
} CBMTypeKind;
...
struct CBMType {
    ...
    union {
        ...
        struct {
            ...
            const CBMType* return_type;    // single return; for tuples wrap in PMM_TYPE_TUPLE
...
const CBMType* pmm_type_func(CBMArena* a, const char** param_names, const CBMType** param_types, const CBMType** return_types);
...
const CBMType* pmm_type_template(CBMArena* a, const char* name, const CBMType** args, int arg_count);
...
const CBMType* pmm_type_optional(CBMArena* a, const CBMType* t);  // Optional[T] == Union[T, None]
const CBMType* pmm_type_literal(CBMArena* a, const CBMType* base, const char* literal_text);
const CBMType* pmm_type_protocol(CBMArena* a, const char* qualified_name,
...
// Distinct from pmm_type_literal (Python) which uses base+literal_text.
...
const CBMType* pmm_type_conditional(CBMArena* a,
...
const CBMType* pmm_type_object_lit(CBMArena* a,
...
const CBMType* pmm_type_mapped(CBMArena* a,
    const char* key_name, const CBMType* key_constraint, const CBMType* value);

// Operations
const CBMType* pmm_type_deref(const CBMType* t);         // remove one pointer level
const CBMType* pmm_type_elem(const CBMType* t);           // get element type (slice/chan/pointer)
...
const CBMType* pmm_type_substitute(CBMArena* a, const CBMType* t,
    const char** type_params, const CBMType** type_args);

#endif // PMM_LSP_TYPE_REP_H
