#ifndef PMM_LSP_TYPE_REGISTRY_H
#define PMM_LSP_TYPE_REGISTRY_H
...
typedef enum {
    PMM_FUNC_FLAG_NONE = 0,
    PMM_FUNC_FLAG_PROPERTY = 1 << 0,       // @property -> obj.attr returns getter return
    PMM_FUNC_FLAG_CLASSMETHOD = 1 << 1,    // @classmethod -> first arg is cls (the class)
    PMM_FUNC_FLAG_STATICMETHOD = 1 << 2,   // @staticmethod -> no implicit self/cls
    PMM_FUNC_FLAG_ABSTRACTMETHOD = 1 << 3, // @abstractmethod -> still callable for resolution
    PMM_FUNC_FLAG_OVERLOAD = 1 << 4,       // @overload entry — non-implementation stub
    PMM_FUNC_FLAG_ASYNC = 1 << 5,          // async def — return is Coroutine[..., T]
    PMM_FUNC_FLAG_GENERATOR = 1 << 6,      // contains yield — return is Generator[T, ...]
    PMM_FUNC_FLAG_FINAL = 1 << 7,          // @final — overrides not allowed
} CBMFuncFlags;
...
typedef struct {
    ...
    int flags;                     // PMM_FUNC_FLAG_* bitfield (Python decorator info; 0 elsewhere)
...
typedef struct CBMTypeRegistry {
    ...
    // Hash indexes (built lazily by pmm_registry_finalize, NULL until then).
    ...
    /* Sealed / read-only. Set true by the pmm_X_build_cross_registry builders
     ...
     * pmm_registry_add_func/_type no-op on a sealed registry, so a per-file
...
// Like pmm_registry_finalize, but the hash-index allocations (buckets/entries)
...
//   CBMTypeEmbedIter it; pmm_registry_types_by_embedded_bare(reg, bare, &it);
//   int ti; while ((ti = pmm_type_embed_iter_next(&it)) >= 0) { ... reg->types[ti] ... }
typedef struct {
