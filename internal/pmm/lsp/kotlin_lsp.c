...
static char *kt_join_dot(CBMArena *a, const char *prefix, const char *name) {
    ...
    if (!prefix || !*prefix) {
        return pmm_arena_strdup(a, name);
    }
    size_t pl = strlen(prefix);
    size_t nl = strlen(name);
    char *out = (char *)pmm_arena_alloc(a, pl + 1 + nl + 1);
    if (!out) {
        return NULL;
    }
...
static void kt_emit_resolved(KotlinLSPContext *ctx, const char *callee_qn, const char *strategy,
    ...
    if (arr->count >= arr->cap) {
        ...
        CBMResolvedCall *new_items = (CBMResolvedCall *)pmm_arena_alloc(
...
/* Build a PMM_TYPE_FUNC signature carrying just the return type, so that
 * pmm_registry_lookup_method's caller can read rf->signature->return_types
...
static const CBMType *kt_build_func_sig_with_return(KotlinLSPContext *ctx,
    ...
    if (!return_type || pmm_type_is_unknown(return_type)) {
        return NULL;
    }
    const char **empty_pn = (const char **)pmm_arena_alloc(ctx->arena, sizeof(const char *));
    if (!empty_pn) {
        return NULL;
    }
    empty_pn[0] = NULL;
    const CBMType **empty_pt =
        (const CBMType **)pmm_arena_alloc(ctx->arena, sizeof(const CBMType *));
    if (!empty_pt) {
        return NULL;
    }
    empty_pt[0] = NULL;
    const CBMType **rt = (const CBMType **)pmm_arena_alloc(ctx->arena, 2 * sizeof(const CBMType *));
    if (!rt) {
        return NULL;
    }
    rt[0] = return_type;
    rt[1] = NULL;
    return pmm_type_func(ctx->arena, empty_pn, empty_pt, rt);
}
...
void kotlin_lsp_init(KotlinLSPContext *ctx, CBMArena *arena, const char *source, int source_len,
    ...
    ctx->package_qn = package_qn ? pmm_arena_strdup(arena, package_qn) : "";
    ctx->module_qn = module_qn ? pmm_arena_strdup(arena, module_qn) : "";
    ctx->project_name = project_name ? pmm_arena_strdup(arena, project_name) : "";
    ctx->rel_path = rel_path ? pmm_arena_strdup(arena, rel_path) : "";
    ctx->resolved_calls = out;
    ctx->import_cap = KT_IMPORT_INITIAL_CAP;
    ctx->import_locals =
        (const char **)pmm_arena_alloc(arena, sizeof(const char *) * (size_t)ctx->import_cap);
    ctx->import_targets =
        (const char **)pmm_arena_alloc(arena, sizeof(const char *) * (size_t)ctx->import_cap);
    ctx->import_kinds = (CBMKotlinUseKind *)pmm_arena_alloc(arena, sizeof(CBMKotlinUseKind) *
                                                                       (size_t)ctx->import_cap);
    ctx->import_count = 0;
    ctx->debug = (getenv("PMM_LSP_DEBUG") != NULL);
    ...
    if (rel_path && *rel_path) {
        ...
        char *fn = (char *)pmm_arena_alloc(arena, name_len + 3);
        if (fn) {
...
void kotlin_lsp_add_import(KotlinLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->import_count >= ctx->import_cap) {
        ...
        const char **new_locals =
            (const char **)pmm_arena_alloc(ctx->arena, sizeof(const char *) * (size_t)new_cap);
        const char **new_targets =
            (const char **)pmm_arena_alloc(ctx->arena, sizeof(const char *) * (size_t)new_cap);
        CBMKotlinUseKind *new_kinds = (CBMKotlinUseKind *)pmm_arena_alloc(
    ...
    ctx->import_locals[ctx->import_count] =
        local_name ? pmm_arena_strdup(ctx->arena, local_name) : NULL;
    ctx->import_targets[ctx->import_count] = pmm_arena_strdup(ctx->arena, target_qn);
...
const char *kotlin_resolve_class_name(KotlinLSPContext *ctx, const char *name) {
    ...
    if (ctx->package_qn && *ctx->package_qn) {
        ...
        if (ctx->registry && pmm_registry_lookup_type(ctx->registry, cand)) {
            return cand;
        }
    ...
    if (ctx->registry && pmm_registry_lookup_type(ctx->registry, name)) {
        return pmm_arena_strdup(ctx->arena, name);
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        ...
        if (local && strcmp(local, name) == 0 &&
            (ctx->import_kinds[i] == PMM_KT_USE_TYPE ||
             ctx->import_kinds[i] == PMM_KT_USE_UNKNOWN)) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] == PMM_KT_USE_WILDCARD && ctx->import_targets[i]) {
    ...
    int n = 0;
    const char *const *defs = pmm_kotlin_default_import_packages(&n);
    for (int i = 0; i < n; i++) {
...
const char *kotlin_resolve_function_name(KotlinLSPContext *ctx, const char *name) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        const char *local = ctx->import_locals[i];
        if (local && strcmp(local, name) == 0 &&
            (ctx->import_kinds[i] == PMM_KT_USE_FUNCTION ||
             ctx->import_kinds[i] == PMM_KT_USE_UNKNOWN ||
             ctx->import_kinds[i] == PMM_KT_USE_TYPE)) {
            return ctx->import_targets[i];
        }
    ...
    if (ctx->package_qn && *ctx->package_qn) {
        char *cand = kt_join_dot(ctx->arena, ctx->package_qn, name);
        if (ctx->registry && pmm_registry_lookup_func(ctx->registry, cand)) {
            return cand;
        }
    }

    /* Bare-name lookup for files without a package declaration. */
    if (ctx->registry && pmm_registry_lookup_func(ctx->registry, name)) {
        return pmm_arena_strdup(ctx->arena, name);
    }
    ...
    /* Default imports */
    const char *via_default = kt_resolve_in_default_imports(ctx, name, PMM_KT_USE_FUNCTION);
    if (via_default) {
        return via_default;
    }
...
static const char *kt_resolve_in_default_imports(KotlinLSPContext *ctx, const char *name,
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (kind == PMM_KT_USE_FUNCTION || kind == PMM_KT_USE_UNKNOWN) {
            if (pmm_registry_lookup_func(ctx->registry, cand)) {
                return cand;
            }
        }
        if (kind == PMM_KT_USE_TYPE || kind == PMM_KT_USE_UNKNOWN) {
            if (pmm_registry_lookup_type(ctx->registry, cand)) {
                return cand;
            }
        }
    }
...
static const CBMRegisteredFunc *kt_synth_method(KotlinLSPContext *ctx, const char *class_qn,
    ...
    CBMRegisteredFunc *rf =
        (CBMRegisteredFunc *)pmm_arena_alloc(ctx->arena, sizeof(CBMRegisteredFunc));
    if (!rf) {
        return NULL;
    }
    memset(rf, 0, sizeof(CBMRegisteredFunc));
    rf->qualified_name = kt_join_dot(ctx->arena, class_qn, method_name);
    rf->receiver_type = pmm_arena_strdup(ctx->arena, class_qn);
    rf->short_name = pmm_arena_strdup(ctx->arena, method_name);
    rf->min_params = 0;
    return rf;
}
...
static const CBMType *kt_stdlib_method_return_type(KotlinLSPContext *ctx, const char *class_qn,
    ...
    if (kt_method_returns_self(class_qn, method)) {
        return pmm_type_named(ctx->arena, class_qn);
    }
    if (strcmp(method, "toString") == 0 || strcmp(method, "toRegex") == 0 ||
        ...
        return pmm_type_named(ctx->arena, "kotlin.String");
    }
    if (strcmp(method, "size") == 0 || strcmp(method, "length") == 0 ||
        ...
        return pmm_type_named(ctx->arena, "kotlin.Int");
    }
    if (strcmp(method, "isEmpty") == 0 || strcmp(method, "isNotEmpty") == 0 ||
        ...
        return pmm_type_named(ctx->arena, "kotlin.Boolean");
    }
...
const CBMRegisteredFunc *kotlin_lookup_method(KotlinLSPContext *ctx, const char *class_qn,
    ...
    while (qhead < qtail) {
         ...
         * via pmm_registry_lookup_method_aliased). */
        const CBMRegisteredFunc *rf =
            pmm_registry_lookup_method_aliased(ctx->registry, cur_qn, method_name);
        if (rf) {
            return rf;
        }

        /* method_names fallback */
        const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, cur_qn);
...
static const char *kt_parse_package_header(KotlinLSPContext *ctx, TSNode header) {
    ...
    char *out = (char *)pmm_arena_alloc(ctx->arena, strlen(txt) + 1);
    if (!out) {
        return "";
    }
    int oi = 0;
...
static void kt_parse_import_directive(KotlinLSPContext *ctx, TSNode imp) {
    ...
    /* Compress whitespace */
    char *cleaned = (char *)pmm_arena_alloc(ctx->arena, strlen(qid_text) + 1);
    int oi = 0;
    ...
    if (full_text) {
        ...
        if (as_kw) {
            ...
            char *al = (char *)pmm_arena_alloc(ctx->arena, 128);
            if (al) {
                int ai = 0;
    ...
    if (wildcard) {
        kotlin_lsp_add_import(ctx, NULL, cleaned, PMM_KT_USE_WILDCARD);
        return;
    }

    /* Default: kind = UNKNOWN — we'll match it both ways at lookup time. */
    const char *local = alias ? alias : kt_short(cleaned);
    kotlin_lsp_add_import(ctx, local, cleaned, PMM_KT_USE_UNKNOWN);
}
...
static void kt_collect_top_level_decls(KotlinLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(kind, "class_declaration") == 0) {
             ...
             * pmm_registry_resolve_alias / lookup_method_aliased follow
            ...
            if (!ts_node_is_null(rhs)) {
                ...
                if (underlying && underlying->kind == PMM_TYPE_NAMED &&
                    underlying->data.named.qualified_name) {
                    rt.alias_of = underlying->data.named.qualified_name;
                }
            }
            pmm_registry_add_type((CBMTypeRegistry *)ctx->registry, rt);
        }
    }
}
...
static void kt_process_class_decl(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (field_count > 0) {
        const char **fn = (const char **)pmm_arena_alloc(ctx->arena, sizeof(const char *) *
                                                                         (size_t)(field_count + 1));
        const CBMType **ft = (const CBMType **)pmm_arena_alloc(
             ...
             * site and propagate it as the lambda's `this`. */
            {
                ...
                if (!ts_node_is_null(params)) {
                    ...
                    for (uint32_t pi = 0; pi < pnc; pi++) {
                        ...
                        const char **dq =
                            (const char **)pmm_arena_alloc(ctx->arena, 2 * sizeof(const char *));
                        if (dq) {
                            char *tag = (char *)pmm_arena_alloc(
...
static void kt_process_property_decl(KotlinLSPContext *ctx, TSNode node) {
    ...
    pmm_scope_bind(ctx->current_scope, pmm_arena_strdup(ctx->arena, pname), pt);
}
...
static const CBMType *kt_eval_user_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    /* Strip whitespace */
    char *clean = (char *)pmm_arena_alloc(ctx->arena, strlen(txt) + 1);
    int oi = 0;
    for (int i = 0; txt[i]; i++) {
        if (txt[i] != ' ' && txt[i] != '\t' && txt[i] != '\n') {
            clean[oi++] = txt[i];
    ...
    if (!resolved) {
        return pmm_type_named(ctx->arena, clean);
    }
    return pmm_type_named(ctx->arena, resolved);
}
...
static const CBMType *kt_eval_literal_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "character_literal") == 0) {
        return pmm_type_named(ctx->arena, "kotlin.Char");
    }
    if (strcmp(kind, "number_literal") == 0) {
        ...
        if (tail == 'l') {
            return pmm_type_named(ctx->arena, "kotlin.Long");
        }
        if (tail == 'f') {
            return pmm_type_named(ctx->arena, "kotlin.Float");
        }
        if (strchr(txt, '.') || strchr(txt, 'e') || strchr(txt, 'E')) {
            return pmm_type_named(ctx->arena, "kotlin.Double");
        }
        return pmm_type_named(ctx->arena, "kotlin.Int");
    }
    ...
    if (strcmp(kind, "null_literal") == 0 ||
        ...
        return pmm_type_named(ctx->arena, "kotlin.Nothing");
    }
    return pmm_type_unknown();
}
...
const CBMType *kotlin_eval_expr_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0 || strcmp(kind, "simple_identifier") == 0) {
        ...
        if (strcmp(name, "true") == 0 || strcmp(name, "false") == 0) {
            result = pmm_type_named(ctx->arena, "kotlin.Boolean");
            goto out;
        }
        if (strcmp(name, "null") == 0) {
            result = pmm_type_named(ctx->arena, "kotlin.Nothing");
            goto out;
        }
        ...
        if (cls_qn) {
            const CBMRegisteredType *rt = pmm_registry_lookup_type(ctx->registry, cls_qn);
            if (rt) {
                result = pmm_type_named(ctx->arena, cls_qn);
                goto out;
            }
        }
    ...
    if (strcmp(kind, "this_expression") == 0) {
        result = ctx->this_type ? ctx->this_type : pmm_type_unknown();
        goto out;
    }
    if (strcmp(kind, "super_expression") == 0) {
        result = ctx->super_type ? ctx->super_type : pmm_type_unknown();
        goto out;
    }
    ...
    if (strcmp(kind, "binary_expression") == 0 || strcmp(kind, "additive_expression") == 0 ||
        ...
        if (full) {
            ...
            if (between_len > 0 && lhs_end > node_start) {
                ...
                if (pmm_memmem(between, (size_t)between_len, "===", 3)) {
                    op_method = NULL; /* identity, no method */
                } else if (pmm_memmem(between, (size_t)between_len, "!==", 3)) {
                    op_method = NULL;
                } else if (pmm_memmem(between, (size_t)between_len, "==", 2)) {
                    op_method = "equals";
                } else if (pmm_memmem(between, (size_t)between_len, "!=", 2)) {
                    op_method = "equals";
                } else if (pmm_memmem(between, (size_t)between_len, "..<", 3)) {
                    ...
                    op_method = "remAssign";
                           ...
                           pmm_memmem(between, (size_t)between_len, ">=", 2) ||
                           pmm_memmem(between, (size_t)between_len, "<", 1) ||
                           pmm_memmem(between, (size_t)between_len, ">", 1)) {
                    op_method = "compareTo";
                } else if (pmm_memmem(between, (size_t)between_len, "+", 1)) {
                    op_method = "plus";
                } else if (pmm_memmem(between, (size_t)between_len, "-", 1)) {
                    op_method = "minus";
                } else if (pmm_memmem(between, (size_t)between_len, "*", 1)) {
        ...
        if (op_method && lhs_t && !pmm_type_is_unknown(lhs_t)) {
            ...
            if (lhs_qn) {
                ...
                if (rf && rf->qualified_name) {
                    ...
                    if (rf->signature && rf->signature->kind == PMM_TYPE_FUNC &&
        ...
        if (nc > 0) {
            ...
            if (full) {
                ...
                if (op_end > node_start) {
                    ...
                    if (pmm_memmem(prefix, (size_t)prefix_len, "++", 2)) {
                        op_method = "inc";
                    } else if (pmm_memmem(prefix, (size_t)prefix_len, "--", 2)) {
                        op_method = "dec";
                    } else if (pmm_memmem(prefix, (size_t)prefix_len, "!", 1)) {
                        op_method = "not";
                    } else if (pmm_memmem(prefix, (size_t)prefix_len, "-", 1)) {
            ...
            if (op_method && t && !pmm_type_is_unknown(t)) {
    ...
    if (strcmp(kind, "in_expression") == 0 || strcmp(kind, "check_expression") == 0) {
        ...
        bool is_membership =
            ntext && (pmm_memmem(ntext, strlen(ntext), " in ", 4) || strstr(ntext, "!in") != NULL);
        uint32_t nc = ts_node_named_child_count(node);
                ...
                if (nc >= 2) {
                    ...
                    if (recv_qn) {
                        if (strcmp(member, "class") == 0) {
                            ...
                            result = pmm_type_named(ctx->arena, "kotlin.reflect.KClass");
                            goto out;
                        }
    ...
    if (strcmp(kind, "object_literal") == 0) {
        ...
        result = pmm_type_named(ctx->arena, "kotlin.Any");
        goto out;
    }
    if (strcmp(kind, "collection_literal") == 0) {
        /* [a, b, c] — typically a List */
        result = pmm_type_named(ctx->arena, "kotlin.collections.List");
        goto out;
    }
    if (strcmp(kind, "range_expression") == 0) {
        result = pmm_type_named(ctx->arena, "kotlin.ranges.IntRange");
        goto out;
    }
...
static const CBMType *kt_eval_constructor_or_func_call(KotlinLSPContext *ctx, TSNode call_node,
    ...
    if (ctx->this_type && !pmm_type_is_unknown(ctx->this_type)) {
    ...
    if (cls_qn && ctx->registry) {
        ...
        if (rt) {
            ...
            return pmm_type_named(ctx->arena, cls_qn);
        }
    }
    /* 2. Top-level fun */
    ...
    if (fn_qn) {
        ...
        if (ctx->registry) {
            const CBMRegisteredFunc *rf = pmm_registry_lookup_func(ctx->registry, fn_qn);
        ...
        for (int i = 0; stdlib_returns[i].qn; i++) {
            if (strcmp(stdlib_returns[i].qn, fn_qn) == 0) {
                return pmm_type_named(ctx->arena, stdlib_returns[i].ret);
            }
        }
    }
    (void)call_node;
...
static const CBMType *kt_eval_navigation_expression_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (recv_qn && ctx->registry) {
        ...
        rf = pmm_registry_lookup_method(ctx->registry, recv_qn, member_text);
        if (rf && rf->qualified_name) {
...
static void kt_bind_property_to_scope(KotlinLSPContext *ctx, TSNode prop) {
        ...
        if (init_t && !pmm_type_is_unknown(init_t)) {
            ...
            for (uint32_t i = 0; i < mnc; i++) {
                ...
                if (!ts_node_is_null(vid)) {
                    ...
                    if (vname) {
                        pmm_scope_bind(ctx->current_scope, pmm_arena_strdup(ctx->arena, vname),
                                       pmm_type_unknown());
                    }
                }
            }
    ...
    pmm_scope_bind(ctx->current_scope, pmm_arena_strdup(ctx->arena, name), t);
}
...
static void kt_apply_smart_cast(KotlinLSPContext *ctx, TSNode condition_expr, bool then_branch) {
    ...
    if (!is_check && kt_node_is(condition_expr, "check_expression")) {
        ...
        if (ctext &&
            (pmm_memmem(ctext, strlen(ctext), " is ", 4) != NULL || strstr(ctext, "!is") != NULL)) {
            is_check = true;
        }
    }
    if (!is_check) {
        ...
        for (uint32_t e = 0; e < en; e++) {
            ...
            if (kt_node_is(ec, "_when_condition") || kt_node_is(ec, "when_condition") ||
                ...
                if (!ts_node_is_null(tt) && subject_name) {
                    ...
                    if (!pmm_type_is_unknown(narrow)) {
                        pmm_scope_bind(ctx->current_scope,
                                       pmm_arena_strdup(ctx->arena, subject_name), narrow);
                    }
                }
            }
        }
...
static void kt_process_for_statement(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (iter_t && !pmm_type_is_unknown(iter_t)) {
    ...
    if (!ts_node_is_null(loop_var)) {
        ...
        if (!ts_node_is_null(id)) {
            char *name = kt_node_text(ctx, id);
            if (name) {
                pmm_scope_bind(ctx->current_scope, pmm_arena_strdup(ctx->arena, name),
                               pmm_type_unknown());
            }
        }
    }
...
static void kt_process_lambda(KotlinLSPContext *ctx, TSNode lambda, const CBMType *receiver_type) {
    ...
    if (receiver_type && !pmm_type_is_unknown(receiver_type)) {
        ...
        for (uint32_t i = 0; i < pnc; i++) {
            ...
            if (!ts_node_is_null(id)) {
                ...
                if (name) {
                    ...
                    const CBMType *t =
                        ts_node_is_null(tn) ? pmm_type_unknown() : kotlin_parse_type_node(ctx, tn);
                    pmm_scope_bind(ctx->current_scope, pmm_arena_strdup(ctx->arena, name), t);
                }
            }
        }
                ...
                if (rf && rf->decorator_qns) {
                    for (int i = 0; rf->decorator_qns[i]; i++) {
                        ...
                        if (strncmp(d, "lambda_receiver:", 16) == 0) {
                            recv_t = pmm_type_named(ctx->arena, d + 16);
                            break;
                        }
                    }
                }
...
static void kt_process_statement(KotlinLSPContext *ctx, TSNode stmt) {
    ...
    if (strcmp(kind, "function_declaration") == 0) {
        ...
        const char *new_qn = kt_join_dot(ctx->arena, prev_func ? prev_func : ctx->module_qn, fname);
        {
            ...
            lrf.min_params = 0;
            pmm_registry_add_func((CBMTypeRegistry *)ctx->registry, lrf);
            ...
            if (ctx->package_qn && *ctx->package_qn) {
                ...
                pmm_registry_add_func((CBMTypeRegistry *)ctx->registry, alias);
            } else {
                /* No package — register at root. */
                CBMRegisteredFunc alias = lrf;
                alias.qualified_name = pmm_arena_strdup(ctx->arena, fname);
                pmm_registry_add_func((CBMTypeRegistry *)ctx->registry, alias);
            }
        }
...
static void kt_bind_function_params(KotlinLSPContext *ctx, TSNode func_node) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const CBMType *t = ts_node_is_null(type_node) ? pmm_type_unknown()
                                                      : kotlin_parse_type_node(ctx, type_node);
        pmm_scope_bind(ctx->current_scope, pmm_arena_strdup(ctx->arena, pname), t);
    }

    /* For extension fun and method: bind `this` */
    if (ctx->this_type) {
        pmm_scope_bind(ctx->current_scope, "this", ctx->this_type);
    }
}
...
static void kt_process_function_body(KotlinLSPContext *ctx, TSNode func_node, const char *func_qn,
    ...
    if (receiver_class_qn) {
        ctx->this_type = pmm_type_named(ctx->arena, receiver_class_qn);
        receiver_rt = pmm_registry_lookup_type(ctx->registry, receiver_class_qn);
        if (receiver_rt && receiver_rt->embedded_types && receiver_rt->embedded_types[0]) {
            ...
            ctx->super_type = pmm_type_named(ctx->arena, receiver_rt->embedded_types[0]);
        }
    }
    ...
    if (receiver_rt && receiver_rt->field_names && receiver_rt->field_types) {
        for (int i = 0; receiver_rt->field_names[i]; i++) {
            pmm_scope_bind(ctx->current_scope,
                           pmm_arena_strdup(ctx->arena, receiver_rt->field_names[i]),
...
/* Debug AST dumper — only active when PMM_LSP_KOTLIN_AST=1 in env. */
...
/* ── public entry point used by pmm.c dispatcher ──────────────────── */
...
static char *kt_repair_bodyless_interface_methods(CBMArena *arena, const char *src, int src_len,
    ...
    char *out = (char *)pmm_arena_alloc(arena, (size_t)new_len + 1);
    if (!out) {
        return NULL;
    }
    int oi = 0;
...
void pmm_run_kotlin_lsp(CBMArena *arena, CBMFileResult *result, const char *source, int source_len,
                        TSNode root) {
    ...
    bool debug = (getenv("PMM_LSP_DEBUG") != NULL);
    ...
    pmm_registry_init(&registry, arena);

    /* Curated stdlib */
    pmm_kotlin_stdlib_register(&registry, arena);
    ...
    if (first_dot) {
        ...
        char *pn = (char *)pmm_arena_alloc(arena, pl + 1);
        if (pn) {
...
static const CBMType *kt_cross_return_type(CBMArena *arena, const CBMLSPDef *d) {
    ...
    const char *first = bar ? pmm_arena_strndup(arena, text, (size_t)(bar - text)) : text;
    if (!first || !first[0]) {
        return NULL;
    }
    if (!strchr(first, '.')) {
    ...
    return pmm_type_named(arena, first);
}
...
static const CBMType *kt_cross_func_sig_with_return(CBMArena *arena, const CBMLSPDef *d) {
    ...
    const char **empty_pn = (const char **)pmm_arena_alloc(arena, sizeof(*empty_pn));
    const CBMType **empty_pt = (const CBMType **)pmm_arena_alloc(arena, sizeof(*empty_pt));
    ...
    rets[0] = ret;
    rets[1] = NULL;
    return pmm_type_func(arena, empty_pn, empty_pt, rets);
}
...
void pmm_run_kotlin_lsp_cross(CBMArena *arena, const char *source, int source_len,
    ...
    pmm_kotlin_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < import_count; i++) {
        ...
        kotlin_lsp_add_import(&ctx, import_names[i], import_qns[i], PMM_KT_USE_UNKNOWN);
    }
