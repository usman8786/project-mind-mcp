...
static void build_qn_index(CBMTypeRegistry *reg, CBMArena *idx_arena, bool for_funcs) {
    ...
    int *buckets = (int *)pmm_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    CBMRegistryHashEntry *entries = (CBMRegistryHashEntry *)pmm_arena_alloc(
...
void pmm_registry_types_by_embedded_bare(const CBMTypeRegistry *reg, const char *bare,
...
int pmm_type_embed_iter_next(CBMTypeEmbedIter *it) {
...
void pmm_registry_free_funcs_by_short_name(const CBMTypeRegistry *reg, const char *short_name,
                                           CBMFreeFuncIter *out) {
    out->reg = reg;
...
int pmm_free_func_iter_next(CBMFreeFuncIter *it) {
    const CBMTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
...
void pmm_registry_finalize_into(CBMTypeRegistry *reg, CBMArena *idx_arena) {
    if (!reg || !idx_arena)
        return;
...
void pmm_registry_finalize(CBMTypeRegistry *reg) {
    if (!reg)
        return;
    pmm_registry_finalize_into(reg, reg->arena);
}

void pmm_registry_init(CBMTypeRegistry *reg, CBMArena *arena) {
    memset(reg, 0, sizeof(CBMTypeRegistry));
    reg->arena = arena;
}

void pmm_registry_add_func(CBMTypeRegistry *reg, CBMRegisteredFunc func) {
    if (reg->read_only) {
    ...
    if (reg->func_count >= reg->func_cap) {
        ...
        CBMRegisteredFunc *new_items = (CBMRegisteredFunc *)pmm_arena_alloc(
...
void pmm_registry_add_type(CBMTypeRegistry *reg, CBMRegisteredType type) {
    if (reg->read_only) {
        return; /* sealed Tier-2 shared registry: refuse post-finalize mutation (O(n^2)+race guard)
                 */
    }
    if (reg->type_count >= reg->type_cap) {
        ...
        CBMRegisteredType *new_items = (CBMRegisteredType *)pmm_arena_alloc(
...
const CBMRegisteredFunc *pmm_registry_lookup_method(const CBMTypeRegistry *reg,
                                                    const char *receiver_qn,
                                                    const char *method_name) {
    ...
    if (!r && reg->fallback) {
        return pmm_registry_lookup_method(reg->fallback, receiver_qn, method_name);
    }
    return r;
}
...
const CBMRegisteredType *pmm_registry_lookup_type(const CBMTypeRegistry *reg,
                                                  const char *qualified_name) {
    if (!reg || !qualified_name)
        return NULL;
    const CBMRegisteredType *r = lookup_type_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return pmm_registry_lookup_type(reg->fallback, qualified_name);
    }
    return r;
}
...
const CBMRegisteredFunc *pmm_registry_lookup_func(const CBMTypeRegistry *reg,
                                                  const char *qualified_name) {
    if (!reg || !qualified_name)
        return NULL;
    const CBMRegisteredFunc *r = lookup_func_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return pmm_registry_lookup_func(reg->fallback, qualified_name);
    }
    return r;
}

const CBMRegisteredType *pmm_registry_resolve_alias(const CBMTypeRegistry *reg,
                                                    const char *type_qn) {
    if (!reg || !type_qn)
        return NULL;
    const CBMRegisteredType *rt = pmm_registry_lookup_type(reg, type_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        const CBMRegisteredType *next = pmm_registry_lookup_type(reg, rt->alias_of);
        if (!next)
            return rt;
        rt = next;
    }
...
const CBMRegisteredFunc *pmm_registry_lookup_method_aliased(const CBMTypeRegistry *reg,
                                                            const char *receiver_qn,
                                                            const char *method_name) {
    if (!reg || !receiver_qn || !method_name)
        return NULL;
    ...
    // Direct lookup first
    const CBMRegisteredFunc *f = pmm_registry_lookup_method(reg, receiver_qn, method_name);
    if (f)
        return f;

    // Follow alias chain
    const CBMRegisteredType *rt = pmm_registry_lookup_type(reg, receiver_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        f = pmm_registry_lookup_method(reg, rt->alias_of, method_name);
        if (f)
            return f;
        rt = pmm_registry_lookup_type(reg, rt->alias_of);
    }
    return NULL;
}

const CBMRegisteredFunc *pmm_registry_lookup_symbol(const CBMTypeRegistry *reg,
    ...
    return pmm_registry_lookup_func(reg, buf);
}
...
const CBMRegisteredFunc *pmm_registry_lookup_method_by_args(const CBMTypeRegistry *reg,
                                                            const char *receiver_qn,
                                                            const char *method_name,
                                                            int arg_count) {
    if (!reg || !receiver_qn || !method_name)
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        if (reg->fallback) {
            return pmm_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                      arg_count);
        }
        return NULL;
    }
...
static int score_overload_match(const CBMRegisteredFunc *f, const CBMType **arg_types,
    ...
    for (int i = 0; i < arg_count; i++) {
        ...
        if (!expected || !actual || pmm_type_is_unknown(actual))
            continue; // neutral
...
const CBMRegisteredFunc *pmm_registry_lookup_method_by_types(const CBMTypeRegistry *reg,
                                                             const char *receiver_qn,
                                                             const char *method_name,
    ...
    if (!arg_types)
        return pmm_registry_lookup_method_by_args(reg, receiver_qn, method_name, arg_count);
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return pmm_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                       arg_types, arg_count);
        }
        return NULL;
    }
...
const CBMRegisteredFunc *pmm_registry_lookup_symbol_by_types(const CBMTypeRegistry *reg,
    ...
    if (!reg || !package_qn || !name)
        return NULL;
    if (!arg_types)
        return pmm_registry_lookup_symbol_by_args(reg, package_qn, name, arg_count);
    ...
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return pmm_registry_lookup_symbol_by_types(reg->fallback, package_qn, name, arg_types,
                                                       arg_count);
        }
        return NULL;
    }
...
const CBMRegisteredFunc *pmm_registry_lookup_symbol_by_args(const CBMTypeRegistry *reg,
                                                            const char *package_qn,
    ...
    // pmm_registry_lookup_symbol_by_types). O(overloads) not O(func_count).
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        int exact_pi = -1, range_pi = -1, first_pi = -1;
        uint64_t h = fnv1a(buf);
        int slot = (int)(h & (uint64_t)(reg->func_qn_bucket_count - 1));
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return pmm_registry_lookup_symbol_by_args(reg->fallback, package_qn, name, arg_count);
        }
        return NULL;
    }
...
const CBMRegisteredFunc *pmm_registry_lookup_callable(const CBMTypeRegistry *reg, CBMArena *arena,
                                                      const char *type_qn) {
    ...
    char *qn = (char *)pmm_arena_alloc(arena, total);
    if (!qn)
        return NULL;
    memcpy(qn, type_qn, qn_len);
    memcpy(qn + qn_len, suffix, sizeof(suffix));

    CBMRegisteredFunc *f = (CBMRegisteredFunc *)pmm_arena_alloc(arena, sizeof(CBMRegisteredFunc));
    if (!f)
        return NULL;
    memset(f, 0, sizeof(*f));
    f->qualified_name = qn;
...
const CBMType *pmm_registry_lookup_index_signature(const CBMTypeRegistry *reg, const char *type_qn,
    ...
    if (rt->index_key_type->kind == PMM_TYPE_BUILTIN && key_type->kind == PMM_TYPE_BUILTIN) {
