#include "pmm.h"
#include "arena.h" // CBMArena, pmm_arena_strdup/strndup/sprintf
#include "helpers.h"
#include "lang_specs.h"      // CBMLangSpec, CBMEmbeddedLangSpec, pmm_lang_spec, pmm_ts_language
...
static char *strip_quotes(CBMArena *a, const char *s) {
    ...
    size_t len = strlen(s);
    if (len >= PMM_QUOTE_PAIR && (s[0] == '"' || s[0] == '\'') && s[len - SKIP_ONE] == s[0]) {
        return pmm_arena_strndup(a, s + SKIP_ONE, len - PAIR_LEN);
    }
    return pmm_arena_strdup(a, s);
}
...
static const char *path_last(CBMArena *a, const char *path) {
    ...
    if (last_sep) {
        return pmm_arena_strdup(a, last_sep + SKIP_ONE);
    }
    return path;
}
...
static void parse_go_import_spec(CBMExtractCtx *ctx, TSNode spec) {
    ...
    char *path = strip_quotes(a, pmm_node_text(a, path_node, ctx->source));
    if (!path || !path[0]) {
        return;
    }
    ...
    const char *local_name =
        !ts_node_is_null(name_node) ? pmm_node_text(a, name_node, ctx->source) : path_last(a, path);

    CBMImport imp = {.local_name = local_name, .module_path = path};
    pmm_imports_push(&ctx->result->imports, a, imp);
}
...
static void emit_py_aliased_import(CBMExtractCtx *ctx, TSNode child, const char *mod_prefix) {
    ...
    char *name = pmm_node_text(a, mod_node, ctx->source);
    if (!name || !name[0]) {
        return;
    }
    const char *local = !ts_node_is_null(alias_node) ? pmm_node_text(a, alias_node, ctx->source)
                                                     : path_last(a, name);
    const char *full = mod_prefix ? pmm_arena_sprintf(a, "%s.%s", mod_prefix, name) : name;
    ...
    if (ts_node_is_null(name_node)) {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ck, "dotted_name") == 0 || strcmp(ck, "identifier") == 0) {
                char *mod = pmm_node_text(a, child, ctx->source);
                if (mod && mod[0]) {
        ...
        char *mod = pmm_node_text(a, name_node, ctx->source);
        if (mod && mod[0]) {
            CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
            pmm_imports_push(&ctx->result->imports, a, imp);
        }
...
static void emit_py_import_from_name(CBMExtractCtx *ctx, TSNode child, const char *mod_path) {
    CBMArena *a = ctx->arena;
    char *name = pmm_node_text(a, child, ctx->source);
    if (name && name[0]) {
        const char *full = mod_path ? pmm_arena_sprintf(a, "%s.%s", mod_path, name) : name;
...
static void process_py_import_from(CBMExtractCtx *ctx, TSNode node) {
    ...
    if (strcmp(ts_node_type(node), "future_import_statement") == 0) {
        CBMImport imp = {.local_name = pmm_arena_strdup(a, "__future__"),
                         .module_path = pmm_arena_strdup(a, "__future__")};
        pmm_imports_push(&ctx->result->imports, a, imp);
        return;
    }
    TSNode module_node = resolve_py_module_node(node);
    char *mod_path =
        ts_node_is_null(module_node) ? NULL : pmm_node_text(a, module_node, ctx->source);
...
static bool process_named_imports(CBMExtractCtx *ctx, TSNode sub, const char *path) {
    ...
    for (uint32_t m = 0; m < nc2; m++) {
        ...
        if (!ts_node_is_null(orig)) {
            char *local_name = !ts_node_is_null(local) ? pmm_node_text(a, local, ctx->source)
                                                       : pmm_node_text(a, orig, ctx->source);
            CBMImport imp = {.local_name = local_name, .module_path = path};
            pmm_imports_push(&ctx->result->imports, a, imp);
            found = true;
        }
...
static bool process_import_clause(CBMExtractCtx *ctx, TSNode clause, const char *path) {
    ...
    for (uint32_t k = 0; k < cc; k++) {
        ...
        if (strcmp(sk, "identifier") == 0) {
            char *name = pmm_node_text(a, sub, ctx->source);
            ...
            if (!ts_node_is_null(as_name)) {
                char *name = pmm_node_text(a, as_name, ctx->source);
                CBMImport imp = {.local_name = name, .module_path = path};
                pmm_imports_push(&ctx->result->imports, a, imp);
                found = true;
            }
...
static bool process_es_import_statement(CBMExtractCtx *ctx, TSNode node) {
    ...
    char *path = strip_quotes(a, pmm_node_text(a, source_node, ctx->source));
    if (!path || !path[0]) {
        return false;
    }
    uint32_t nc = ts_node_child_count(node);
...
static bool process_commonjs_require(CBMExtractCtx *ctx, TSNode call) {
    ...
    char *fn_name = pmm_node_text(a, fn, ctx->source);
    ...
    for (uint32_t i = 0; i < argc; i++) {
        ...
        if (strcmp(ak, "string") == 0 || strcmp(ak, "string_literal") == 0 ||
            ...
            path = strip_quotes(a, pmm_node_text(a, arg, ctx->source));
            break;
        }
    }
    if (!path || !path[0]) {
    ...
    if (!ts_node_is_null(parent) && strcmp(ts_node_type(parent), "variable_declarator") == 0) {
        ...
        if (!ts_node_is_null(name_node) && strcmp(ts_node_type(name_node), "identifier") == 0) {
            local_name = pmm_node_text(a, name_node, ctx->source);
        }
    }
    if (!local_name) {
...
static void walk_es_imports(CBMExtractCtx *ctx, TSNode root) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, PMM_SZ_512);
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(kind, "import_statement") == 0) {
            ...
            if (!ts_node_is_null(src)) {
                char *path = strip_quotes(ctx->arena, pmm_node_text(ctx->arena, src, ctx->source));
                if (path && path[0]) {
                    CBMImport imp = {.local_name = path_last(ctx->arena, path),
                                     .module_path = path};
                    pmm_imports_push(&ctx->result->imports, ctx->arena, imp);
                }
            }
    ...
    do {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ck, "scoped_identifier") == 0 || strcmp(ck, "identifier") == 0) {
                char *path = pmm_node_text(a, child, ctx->source);
                if (path && path[0]) {
                    CBMImport imp = {.local_name = path_last(a, path), .module_path = path};
                    pmm_imports_push(&ctx->result->imports, a, imp);
                }
...
static void parse_rust_imports(CBMExtractCtx *ctx) {
    ...
    do {
        ...
        char *full = pmm_node_text(a, node, ctx->source);
        if (!full) {
            continue;
        }
...
static char *strip_angle_brackets(CBMArena *a, char *path) {
    if (path && path[0] == '<') {
        ...
        if (len > SKIP_ONE && path[len - SKIP_ONE] == '>') {
            return pmm_arena_strndup(a, path + SKIP_ONE, len - PAIR_LEN);
        }
    }
    return path;
}
...
static const char *ruby_require_method(CBMArena *a, TSNode node, const char *source) {
    ...
    char *name = pmm_node_text(a, method, source);
...
static char *extract_ruby_require_arg(CBMArena *a, TSNode node, const char *source) {
    ...
    for (uint32_t j = 0; j < ac; j++) {
        ...
        const char *ck = ts_node_type(c);
        if (strcmp(ck, "string") == 0 || strcmp(ck, "string_literal") == 0) {
            return strip_quotes(a, pmm_node_text(a, c, source));
        }
    }
    return strip_quotes(a, pmm_node_text(a, args, source));
}
...
static void parse_lua_imports(CBMExtractCtx *ctx) {
    ...
    do {
        ...
        char *text = pmm_node_text(a, node, ctx->source);
        if (!text) {
            continue;
        }
        ...
        char *mod = pmm_arena_strndup(a, start, (size_t)(end - start));
        CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
        pmm_imports_push(&ctx->result->imports, a, imp);
    } while (ts_tree_cursor_goto_next_sibling(&cursor));
    ts_tree_cursor_delete(&cursor);
    ...
    if (strcmp(ts_node_type(node), "call") == 0) {
        ...
        if (!ts_node_is_null(fn) && !ts_node_is_null(args)) {
            ...
            if (strcmp(ft, "namespace_operator") == 0) {
                ...
                char *lt =
                    ts_node_is_null(lhs) ? NULL : pmm_node_text(ctx->arena, lhs, ctx->source);
                char *rt =
                    ts_node_is_null(rhs) ? NULL : pmm_node_text(ctx->arena, rhs, ctx->source);
                if (lt && rt && strcmp(lt, "box") == 0 && strcmp(rt, "use") == 0) {
                    ...
                    for (uint32_t i = 0; i < na; i++) {
                        ...
                        if (!ts_node_is_null(val)) {
                            r_push_import(ctx, pmm_node_text(ctx->arena, val, ctx->source));
                        }
                    }
                }
                ...
                char *fname = pmm_node_text(ctx->arena, fn, ctx->source);
                if (fname &&
...
static void parse_dart_imports(CBMExtractCtx *ctx) {
    ...
    do {
        ...
        if (find_first_descendant_of(node, "string_literal", &uri)) {
            char *path = strip_quotes(a, pmm_node_text(a, uri, ctx->source));
            if (path && path[0]) {
                CBMImport imp = {.local_name = path_last(a, path), .module_path = path};
                pmm_imports_push(&ctx->result->imports, a, imp);
            }
        ...
        if (strcmp(ts_node_type(node), "builtin_function") == 0) {
            ...
            if (bf && (strncmp(bf, "@import", sizeof("@import") - 1) == 0 ||
                ...
                if (find_first_descendant_of(node, "string", &str)) {
                    char *path = strip_quotes(a, pmm_node_text(a, str, ctx->source));
                    if (path && path[0]) {
                        CBMImport imp = {.local_name = path_last(a, path), .module_path = path};
                        pmm_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void process_wolfram_get_top(CBMExtractCtx *ctx, TSNode node) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "string") == 0 || strcmp(ck, "user_symbol") == 0) {
            char *text = pmm_node_text(a, child, ctx->source);
            if (text && text[0]) {
...
static void process_wolfram_needs(CBMExtractCtx *ctx, TSNode node) {
    ...
    char *name = pmm_node_text(a, head, ctx->source);
    if (!name || strcmp(name, "Needs") != 0) {
        return;
    }
    TSNode arg = ts_node_named_child(node, SECOND_IDX);
    char *text = pmm_node_text(a, arg, ctx->source);
    if (text && text[0]) {
        char *path = strip_quotes(a, text);
        CBMImport imp = {.local_name = path_last(a, path), .module_path = path};
        pmm_imports_push(&ctx->result->imports, a, imp);
...
static void emit_php_use_clause(CBMExtractCtx *ctx, TSNode clause, const char *group_prefix) {
    ...
    char *path = pmm_node_text(a, path_node, ctx->source);
    if (!path || !path[0]) {
        return;
    }
    if (group_prefix && group_prefix[0]) {
        path = pmm_arena_sprintf(a, "%s\\%s", group_prefix, path);
    }
    ...
    const char *local =
        !ts_node_is_null(alias) ? pmm_node_text(a, alias, ctx->source) : path_last(a, path);
    ...
    if (find_first_descendant_of(decl, "namespace_use_group", &group)) {
        ...
        for (uint32_t i = 0; i < dc; i++) {
            ...
            if (strcmp(ck, "qualified_name") == 0 || strcmp(ck, "namespace_name") == 0 ||
                strcmp(ck, "name") == 0) {
                prefix = pmm_node_text(a, c, ctx->source);
                break;
            }
        }
...
static void parse_csharp_imports(CBMExtractCtx *ctx) {
    ...
    do {
        ...
        char *path = found ? pmm_node_text(a, path_node, ctx->source) : NULL;
        if (!path || !path[0]) {
...
// resolved through pmm_ts_language(spec->embedded_language), the same hook
...
static void parse_embedded_imports(CBMExtractCtx *ctx) {
    ...
    for (const CBMEmbeddedLangSpec *e = spec->embedded_imports; e->script_node_type != NULL; e++) {
        const TSLanguage *embedded_lang = pmm_ts_language(e->embedded_language);
        if (!embedded_lang) {
    ...
    do {
        ...
        for (const char **nk = name_kinds; *nk; nk++) {
            ...
            if (find_first_descendant_of(node, *nk, &nn)) {
                char *ns = pmm_node_text(a, nn, ctx->source);
                if (ns && ns[0]) {
        ...
        if (strcmp(ts_node_type(node), "use_statement") == 0) {
            ...
            for (uint32_t j = 0; j < nc; j++) {
                ...
                if (strcmp(ck, "identifier") == 0 || strcmp(ck, "scoped_identifier") == 0) {
                    char *mod = pmm_node_text(a, c, ctx->source);
                    if (mod && mod[0]) {
                        CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        pmm_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void parse_powershell_imports(CBMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "command") == 0) {
            ...
            char *nm = ts_node_is_null(name) ? NULL : pmm_node_text(a, name, ctx->source);
            if (nm && strcmp(nm, "using") == 0) {
                ...
                TSNodeStack inner;
                ts_nstack_init(&inner, a, PMM_SZ_512);
                ...
                while (inner.count > 0) {
                    ...
                    if (strcmp(ts_node_type(c), "generic_token") == 0) {
                        char *t = pmm_node_text(a, c, ctx->source);
...
static void lisp_push_module(CBMExtractCtx *ctx, TSNode mod_node) {
    ...
    if (strcmp(mk, "symbol") == 0 || strcmp(mk, "string") == 0 || strcmp(mk, "sym_lit") == 0 ||
        ...
        mod = pmm_node_text(a, mod_node, ctx->source);
        if (mod) {
            size_t len = strlen(mod);
            if (len >= 2 && mod[0] == '(' && mod[len - 1] == ')') {
                mod = pmm_arena_strndup(a, mod + 1, len - 2);
            }
        }
    }
    if (mod && mod[0]) {
...
static void lisp_set_namespace(CBMExtractCtx *ctx, TSNode name_node) {
    CBMArena *a = ctx->arena;
    char *ns = lisp_strip_kw(a, pmm_node_text(a, name_node, ctx->source));
...
static char *lisp_head_text(CBMExtractCtx *ctx, TSNode list, TSNode *out_head) {
    ...
    return pmm_node_text(ctx->arena, head, ctx->source);
}
        ...
        for (uint32_t j = 2; j < nc; j++) {
            ...
            if (chn && (strcmp(chn, ":use") == 0 || strstr(chn, "import-from"))) {
                ...
                if (strcmp(c0k, "kwd_lit") == 0) {
                    char *kw = pmm_node_text(ctx->arena, ch0, ctx->source);
...
static void parse_starlark_imports(CBMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "call") == 0) {
            TSNode fn = ts_node_child_by_field_name(node, TS_FIELD("function"));
            char *fname = ts_node_is_null(fn) ? NULL : pmm_node_text(a, fn, ctx->source);
                    ...
                    if (find_first_descendant_of(args, "string", &str)) {
                        ...
                        if (path && path[0] && (path[0] == '"' || path[0] == '\'')) {
                            ...
                            if (find_first_descendant_of(str, "string_content", &content)) {
                                path = pmm_node_text(a, content, ctx->source);
                            }
                        }
                        if (path && path[0]) {
                            CBMImport imp = {.local_name = path_last(a, path), .module_path = path};
            ...
            if (nm && strcmp(nm, "source") == 0) {
                ...
                if (!ts_node_is_null(args)) {
                    ...
                    for (uint32_t j = 0; j < nc; j++) {
                        ...
                        char *mod = strip_quotes(a, pmm_node_text(a, c, ctx->source));
                        if (mod && mod[0]) {
                            CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                            pmm_imports_push(&ctx->result->imports, a, imp);
                            break;
...
static void parse_teal_imports(CBMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "function_call") == 0) {
            ...
            char *cn = ts_node_is_null(callee) ? NULL : pmm_node_text(a, callee, ctx->source);
            if (cn && strcmp(cn, "require") == 0) {
                ...
                if (!ts_node_is_null(args) && find_first_descendant_of(args, "string", &str)) {
                    char *mod = strip_quotes(a, pmm_node_text(a, str, ctx->source));
                    if (mod && mod[0] && (mod[0] == '"' || mod[0] == '\'')) {
                        TSNode content = str;
                        if (find_first_descendant_of(str, "string_content", &content)) {
                            mod = pmm_node_text(a, content, ctx->source);
                        }
                    }
                    if (mod && mod[0]) {
                        CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "command") == 0) {
            ...
            if (nm && (strcmp(nm, "source") == 0 || strcmp(nm, ".") == 0)) {
                ...
                if (!ts_node_is_null(arg)) {
                    char *mod = strip_quotes(a, pmm_node_text(a, arg, ctx->source));
                    if (mod && mod[0]) {
                        CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        pmm_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void css_push_import_from_stmt(CBMExtractCtx *ctx, TSNode stmt) {
    ...
    if (find_first_descendant_of(sv, "string_content", &content)) {
        ...
        path = strip_quotes(a, pmm_node_text(a, sv, ctx->source));
    }
    if (!path || !path[0]) {
        return;
    }
...
static void html_extract_tag_src(CBMExtractCtx *ctx, TSNode tag) {
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        char *an = pmm_node_text(a, name, ctx->source);
        ...
        if (find_first_descendant_of(attr, "attribute_value", &val)) {
            path = pmm_node_text(a, val, ctx->source);
        }
        if (path && path[0]) {
            CBMImport imp = {.local_name = path_last(a, path), .module_path = path};
            pmm_imports_push(&ctx->result->imports, a, imp);
...
static void push_path_import(CBMExtractCtx *ctx, TSNode node) {
    CBMArena *a = ctx->arena;
    char *path = strip_quotes(a, pmm_node_text(a, node, ctx->source));
    if (!path || !path[0]) {
        return;
    }
...
static void parse_cmake_imports(CBMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "normal_command") == 0) {
            ...
            char *cmd = pmm_node_text(a, id, ctx->source);
            if (cmd && (strcmp(cmd, "include") == 0 || strcmp(cmd, "add_subdirectory") == 0)) {
                TSNode args = pmm_find_child_by_kind(node, "argument_list");
                if (!ts_node_is_null(args)) {
                    uint32_t nc = ts_node_named_child_count(args);
                    for (uint32_t j = 0; j < nc; j++) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(k, "normal_command") == 0) {
            ...
            if (nc >= 1) {
                ...
                char *fname = pmm_node_text(a, fn, ctx->source);
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "import_declaration") == 0) {
            ...
            if (find_first_descendant_of(node, "module_fqn", &fqn)) {
                char *mod = pmm_node_text(a, fqn, ctx->source);
                if (mod && mod[0]) {
                    CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    pmm_imports_push(&ctx->result->imports, a, imp);
                }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(k, "import_decl") == 0 || strcmp(k, "open_expression") == 0) {
            ...
            if (find_first_descendant_of(node, "long_identifier", &id) ||
                ...
                char *mod = pmm_node_text(a, id, ctx->source);
                if (mod && mod[0]) {
                    CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    pmm_imports_push(&ctx->result->imports, a, imp);
                }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "import_clause") == 0) {
            ...
            if (find_first_descendant_of(node, "upper_case_qid", &name)) {
                char *mod = pmm_node_text(a, name, ctx->source);
                if (mod && mod[0]) {
...
static void parse_move_imports(CBMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "use_declaration") == 0) {
            ...
            char *mod = NULL;
            if (!ts_node_is_null(arg)) {
                mod = pmm_node_text(a, arg, ctx->source);
...
static char *smali_demangle_descriptor(CBMArena *a, const char *desc) {
    ...
    char *out = pmm_arena_strndup(a, start, len);
    if (out) {
        for (char *p = out; *p; p++) {
            if (*p == '/') {
                *p = '.';
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(k, "super_directive") == 0 || strcmp(k, "implements_directive") == 0) {
            ...
            if (find_first_descendant_of(node, "class_identifier", &cid)) {
                char *mod = smali_demangle_descriptor(a, pmm_node_text(a, cid, ctx->source));
                if (mod && mod[0]) {
                    CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    pmm_imports_push(&ctx->result->imports, a, imp);
                }
        ...
        if (strcmp(ts_node_type(node), "use_clause") == 0) {
            ...
            if (find_first_descendant_of(node, "selected_name", &sel)) {
                ...
                if (!ts_node_is_null(pkg)) {
                    mod = pmm_node_text(a, pkg, ctx->source);
                }
                if (mod && mod[0]) {
                    CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    pmm_imports_push(&ctx->result->imports, a, imp);
        ...
        if (strcmp(ts_node_type(node), "use_item") == 0) {
            ...
            if (find_first_descendant_of(node, "use_path", &path)) {
                ...
                if (find_first_descendant_of(path, "id", &idn)) {
                    char *mod = pmm_node_text(a, idn, ctx->source);
                    if (mod && mod[0]) {
                        CBMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        pmm_imports_push(&ctx->result->imports, a, imp);
                    }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "use_statement") == 0) {
            ...
            if (find_first_descendant_of(node, "absolute_root_shape_id", &sid) ||
                ...
                char *mod = pmm_node_text(a, sid, ctx->source);
                if (mod && mod[0]) {
...
static void parse_hyprlang_imports(CBMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "source") == 0) {
            ...
            char *path = NULL;
            if (find_first_descendant_of(node, "string", &str)) {
                path = strip_quotes(a, pmm_node_text(a, str, ctx->source));
            } else {
                path = strip_quotes(a, pmm_node_text(a, node, ctx->source));
            }
            if (path && path[0]) {
                CBMImport imp = {.local_name = path_last(a, path), .module_path = path};
                pmm_imports_push(&ctx->result->imports, a, imp);
...
void pmm_extract_imports(CBMExtractCtx *ctx) {
    ...
    case PMM_LANG_MAGMA:
        ...
        break;
    case PMM_LANG_WOLFRAM:
        parse_wolfram_imports(ctx);
        break;
    case PMM_LANG_HARE:
        parse_hare_imports(ctx);
        break;
    case PMM_LANG_PASCAL:
        parse_pascal_imports(ctx);
        break;
    case PMM_LANG_POWERSHELL:
        parse_powershell_imports(ctx);
        break;
    case PMM_LANG_SCHEME:
    case PMM_LANG_RACKET:
    case PMM_LANG_EMACSLISP:
    case PMM_LANG_FENNEL:
    case PMM_LANG_COMMONLISP:
    case PMM_LANG_CLOJURE:
        parse_lisp_imports(ctx);
        break;
    case PMM_LANG_STARLARK:
    ...
    case PMM_LANG_CMAKE:
        ...
        break;
    case PMM_LANG_BITBAKE:
        parse_bitbake_imports(ctx);
        break;
    case PMM_LANG_KCONFIG:
    ...
    case PMM_LANG_DLANG:
        ...
        break;
    case PMM_LANG_TABLEGEN:
        parse_tablegen_imports(ctx);
        break;
    case PMM_LANG_CRYSTAL:
        parse_crystal_imports(ctx);
        break;
    case PMM_LANG_FSHARP:
    ...
    case PMM_LANG_WIT:
        ...
        break;
    case PMM_LANG_SMITHY:
        parse_smithy_imports(ctx);
        break;
    case PMM_LANG_HYPRLANG:
