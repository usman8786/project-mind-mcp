...
#define PMM_AC_MAX_BITMASK 64

#define PMM_AC_BYTE_RANGE 256
#define PMM_AC_NO_STATE (-1)
#define PMM_AC_ROOT_STATES 1
#define PMM_AC_ALLOC_ONE 1
#define PMM_AC_PATTERN_BIT(p) (1ULL << (p))
#define PMM_AC_CLEAR_LOW_BIT(b) ((b) & ((b) - 1ULL))
...
struct CBMAutomaton {
    ...
    uint8_t alpha_map[PMM_AC_BYTE_RANGE]; // byte → mapped index (identity if alpha_size==256)
...
static int ac_build_trie(CBMAutomaton *ac, const char **patterns, const int *lengths, int count) {
    ...
    int num_states = PMM_AC_ROOT_STATES; // state 0 = root
    ...
    for (int p = 0; p < count; p++) {
        ...
        for (int j = 0; j < lengths[p]; j++) {
            ...
            if (ac->go_table[idx] == PMM_AC_NO_STATE) {
                ac->go_table[idx] = num_states++;
            }
            state = ac->go_table[idx];
        }
        if (p < PMM_AC_MAX_BITMASK) {
            ac->output[state] |= PMM_AC_PATTERN_BIT(p);
        }
    ...
    for (int c = 0; c < alpha_size; c++) {
        if (ac->go_table[c] == PMM_AC_NO_STATE) {
            ac->go_table[c] = 0;
        }
    }
    return num_states;
...
static void ac_build_failure(CBMAutomaton *ac, int num_states) {
    ...
    while (!queue_empty(&q)) {
        ...
        for (int c = 0; c < alpha_size; c++) {
            ...
            if (s != PMM_AC_NO_STATE) {
                fail[s] = ac->go_table[(fail[r] * alpha_size) + c];
                ac->output[s] |= ac->output[fail[s]];
                if (ac->output_next[s] == PMM_AC_NO_STATE &&
                    ac->output_list[fail[s]] != PMM_AC_NO_STATE) {
...
// pmm_ac_build constructs an Aho-Corasick automaton from a set of patterns.
//
// Parameters:
...
//
// Returns a heap-allocated automaton. Caller must call pmm_ac_free().
CBMAutomaton *pmm_ac_build(const char **patterns, const int *lengths, int count,
    ...
    if (alpha_size <= 0) {
        alpha_size = PMM_AC_BYTE_RANGE;
    }

    int max_states = PMM_AC_ROOT_STATES;
    for (int i = 0; i < count; i++) {
        max_states += lengths[i];
    }

    CBMAutomaton *ac = (CBMAutomaton *)calloc(PMM_AC_ALLOC_ONE, sizeof(CBMAutomaton));
    ac->alpha_size = alpha_size;
    ac->num_patterns = count;

    if (alpha_map) {
        memcpy(ac->alpha_map, alpha_map, PMM_AC_BYTE_RANGE);
    } else {
        for (int i = 0; i < PMM_AC_BYTE_RANGE; i++) {
            ac->alpha_map[i] = (uint8_t)i;
        }
    }

    ac->go_table = (int *)malloc((size_t)max_states * alpha_size * sizeof(int));
    memset(ac->go_table, PMM_AC_NO_STATE, (size_t)max_states * alpha_size * sizeof(int));
    ...
    for (int i = 0; i < max_states; i++) {
        ac->output_list[i] = PMM_AC_NO_STATE;
        ac->output_next[i] = PMM_AC_NO_STATE;
    }
...
// pmm_ac_free releases all memory for an automaton.
...
// pmm_ac_scan_bitmask scans text through the automaton and returns a bitmask
...
// Each goroutine gets its own OS thread (via CGo), so PMM_TLS is safe.
static PMM_TLS char *tls_decomp_buf = NULL;
static PMM_TLS int tls_decomp_cap = 0;
...
// pmm_ac_scan_lz4_bitmask decompresses LZ4 data into a thread-local buffer
...
uint64_t pmm_ac_scan_lz4_bitmask(const CBMAutomaton *ac, const char *compressed, int compressed_len,
                                 int original_len) {
    ...
    int decompressed = pmm_lz4_decompress(compressed, compressed_len, buf, original_len);
    if (decompressed < 0) {
        return 0;
    }

    return pmm_ac_scan_bitmask(ac, buf, decompressed);
}
...
// pmm_ac_scan_lz4_batch decompresses and scans multiple files in one call.
// Returns the number of matching files written to out_matches.
// Uses a single reusable decompression buffer across all files.
int pmm_ac_scan_lz4_batch(const CBMAutomaton *ac, const CBMLz4Entry *entries, int num_entries,
    ...
    for (int i = 0; i < num_entries && total < max_matches; i++) {
        ...
        int decompressed = pmm_lz4_decompress(entries[i].data, entries[i].compressed_len, buf,
...
// pmm_ac_scan_batch scans multiple NUL-separated names through the automaton.
        ...
        for (int i = 0; i < text_len; i++) {
            ...
            while (s > 0 && total < max_matches) {
                ...
                while (bits && total < max_matches) {
                    ...
                    total++;
                    seen |= PMM_AC_PATTERN_BIT(pid);
                    bits = PMM_AC_CLEAR_LOW_BIT(bits);
                }

                // Follow output_next for patterns beyond bitmask range.
                int next_state = ac->output_next[s];
                if (next_state == PMM_AC_NO_STATE || next_state == s) {
                    break;
                }
                s = next_state;
            }
...
// pmm_ac_table_bytes returns the approximate memory used by the goto table.
