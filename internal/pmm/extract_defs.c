#include "pmm.h"
#include "arena.h" // CBMArena, pmm_arena_alloc/strdup/sprintf
#include "helpers.h"
#include "lang_specs.h"
#include "foundation/constants.h"
#include "foundation/platform.h" // safe_realloc (frees old on failure)
#include "foundation/log.h"      // pmm_log_warn
...
enum {
    ...
    DECLARATOR_DEPTH_LIMIT = PMM_DECLARATOR_DEPTH_LIMIT, // shared define in helpers.h
...
static char *extract_body_ident_tokens(CBMExtractCtx *ctx, TSNode body) {
    ...
    while (bt_top > 0 && bt_count < BT_MAX_IDENTS) {
        ...
        if (nc == 0) {
            ...
            if (strcmp(k, "identifier") == 0 || strcmp(k, "field_identifier") == 0 ||
                ...
                if (len > 0 && len < PMM_SZ_64 && s < (uint32_t)ctx->source_len) {
    ...
    if (bt_pos > 0) {
        bt_buf[bt_pos] = '\0';
        return pmm_arena_strdup(ctx->arena, bt_buf);
    }
    return NULL;
}
...
static void compute_fingerprint(CBMExtractCtx *ctx, CBMDefinition *def, TSNode func_node) {
    ...
    pmm_minhash_t result;
    if (!pmm_minhash_compute(body, ctx->source, (int)ctx->language, &result)) {
        return; /* Too short or empty — no fingerprint */
    }
    /* Arena-allocate the fingerprint array */
    uint32_t *fp = pmm_arena_alloc(ctx->arena, PMM_MINHASH_K * sizeof(uint32_t));
    if (!fp) {
        return;
    }
    memcpy(fp, result.values, PMM_MINHASH_K * sizeof(uint32_t));
    def->fingerprint = fp;
    def->fingerprint_k = PMM_MINHASH_K;

    /* AST structural profile (signals 8, 9, 11) — rides the same body node */
    pmm_ast_profile_t profile;
    int pc = 0;
    if (def->param_names) {
    ...
    if (pmm_ast_profile_compute(body, ctx->source, def->param_names, pc, &profile)) {
        profile.body_lines = (uint16_t)def->lines;
        char sp_buf[PMM_AST_PROFILE_BUF];
        pmm_ast_profile_to_str(&profile, sp_buf, sizeof(sp_buf));
        def->structural_profile = pmm_arena_strdup(ctx->arena, sp_buf);
    }
}
...
static TSNode func_name_node(TSNode node) {
    ...
    if (ts_node_is_null(name)) {
        ...
        name = pmm_find_child_by_kind(node, "rpc_name");
    }
    return name;
}
...
static TSNode resolve_ocaml_func_name(TSNode node) {
    TSNode binding = pmm_find_child_by_kind(node, "let_binding");
...
static TSNode resolve_sql_func_name(TSNode node) {
    TSNode obj_ref = pmm_find_child_by_kind(node, "object_reference");
    if (!ts_node_is_null(obj_ref)) {
        TSNode id = pmm_find_child_by_kind(obj_ref, "identifier");
        if (!ts_node_is_null(id)) {
            return id;
        }
    }
    return pmm_find_child_by_kind(node, "identifier");
}

// Zig: resolve test_declaration name from string→string_content.
static TSNode resolve_zig_test_name(TSNode node) {
    TSNode str_node = pmm_find_child_by_kind(node, "string");
    if (!ts_node_is_null(str_node)) {
        TSNode content = pmm_find_child_by_kind(str_node, "string_content");
...
static TSNode resolve_vimscript_func_name(TSNode node) {
    TSNode decl = pmm_find_child_by_kind(node, "function_declaration");
...
static TSNode resolve_func_name_scripting(TSNode node, CBMLanguage lang, const char *kind) {
    if (lang == PMM_LANG_LUA && strcmp(kind, "function_definition") == 0) {
        return resolve_lua_func_name(node);
    }
    if (lang == PMM_LANG_OCAML && strcmp(kind, "value_definition") == 0) {
        return resolve_ocaml_func_name(node);
    }
    if (lang == PMM_LANG_SQL && strcmp(kind, "create_function") == 0) {
        return resolve_sql_func_name(node);
    }
    if (lang == PMM_LANG_ZIG && strcmp(kind, "test_declaration") == 0) {
        return resolve_zig_test_name(node);
    }
    if (lang == PMM_LANG_VIMSCRIPT && strcmp(kind, "function_definition") == 0) {
        return resolve_vimscript_func_name(node);
    }
    if (lang == PMM_LANG_JULIA && strcmp(kind, "function_definition") == 0) {
    ...
    if (lang == PMM_LANG_JULIA && strcmp(kind, "assignment") == 0) {
        if (ts_node_named_child_count(node) > 0) {
...
static TSNode resolve_lean_func_name(TSNode node, TSNode name) {
    ...
    if (!ts_node_is_null(decl_id)) {
        TSNode id = pmm_find_child_by_kind(decl_id, "ident");
        if (!ts_node_is_null(id)) {
            return id;
        }
    ...
    return pmm_find_child_by_kind(node, "ident");
}
...
static TSNode resolve_commonlisp_func_name(TSNode node) {
    ...
    TSNode header = pmm_find_child_by_kind(node, "defun_header");
    if (!ts_node_is_null(header)) {
        return pmm_find_child_by_kind(header, "sym_lit");
    }
    TSNode null_node = {0};
    return null_node;
}
...
static TSNode resolve_elm_func_name(TSNode node) {
    ...
    if (ts_node_is_null(fdl)) {
        fdl = pmm_find_child_by_kind(node, "function_declaration_left");
    }
...
static TSNode resolve_func_name_fp(TSNode node, CBMLanguage lang, const char *kind, TSNode name) {
    if (lang == PMM_LANG_COMMONLISP && strcmp(kind, "defun") == 0) {
        return resolve_commonlisp_func_name(node);
    }

    if (lang == PMM_LANG_MAKEFILE && strcmp(kind, "rule") == 0) {
        return resolve_makefile_func_name(node);
    }

    if (lang == PMM_LANG_HASKELL && strcmp(kind, "function") == 0) {
        return resolve_haskell_func_name(node);
    }

    if (lang == PMM_LANG_ELM && strcmp(kind, "value_declaration") == 0) {
        return resolve_elm_func_name(node);
    }

    if (lang == PMM_LANG_MATLAB && strcmp(kind, "function_definition") == 0) {
        if (!ts_node_is_null(name)) {
            return name;
        }
        return pmm_find_child_by_kind(node, "identifier");
    ...
    if (lang == PMM_LANG_LEAN) {
        return resolve_lean_func_name(node, name);
    }

    if (lang == PMM_LANG_WOLFRAM &&
...
char *pmm_cpp_out_of_line_parent_class(CBMArena *a, TSNode node, const char *source) {
    ...
    char *text = pmm_node_text(a, scope, source);
...
// like port lists).
enum { PMM_DESCENDANT_MAX_DEPTH = 6 };
...
// unified/calls extractor shares this one resolver — see pmm_resolve_func_name.
TSNode pmm_resolve_func_name(TSNode node, CBMLanguage lang);
...
static TSNode find_cpp_template_inner_node(TSNode node, CBMLanguage lang) {
    if ((lang != PMM_LANG_CPP && lang != PMM_LANG_CUDA) ||
...
static TSNode resolve_func_name_c_family(TSNode *node_ptr, CBMLanguage lang, const char *kind) {
    ...
    if ((lang == PMM_LANG_C || lang == PMM_LANG_CPP || lang == PMM_LANG_CUDA ||
         lang == PMM_LANG_GLSL || lang == PMM_LANG_HLSL || lang == PMM_LANG_ISPC ||
         lang == PMM_LANG_SLANG || lang == PMM_LANG_OBJC) &&
        ...
        return pmm_resolve_c_declarator_name_node(*node_ptr);
    }
    TSNode null_node = {0};
    return null_node;
}
...
TSNode pmm_resolve_func_name(TSNode node, CBMLanguage lang) {
    ...
    for (int tmpl_depth = 0; tmpl_depth < MAX_TEMPLATE_DEPTH; tmpl_depth++) {
        ...
        if (lang == PMM_LANG_HASKELL && strcmp(kind, "signature") == 0) {
            TSNode null_node = {0};
            return null_node;
        }

        // ObjectScript routine tag is its own name node.
        if (lang == PMM_LANG_OBJECTSCRIPT_ROUTINE && strcmp(kind, "tag") == 0) {
            return node;
        }
        // ObjectScript method/classmethod: name lives under method_definition ->
        // method_name -> first named child.
        if (lang == PMM_LANG_OBJECTSCRIPT_UDL &&
            (strcmp(kind, "method") == 0 || strcmp(kind, "classmethod") == 0)) {
            TSNode mdef = pmm_find_child_by_kind(node, "method_definition");
            if (!ts_node_is_null(mdef)) {
                TSNode mname = pmm_find_child_by_kind(mdef, "method_name");
        ...
        if (lang == PMM_LANG_R && strcmp(kind, "function_definition") == 0) {
        ...
        if ((lang == PMM_LANG_SWIFT || lang == PMM_LANG_KOTLIN) &&
            strcmp(kind, "function_declaration") == 0) {
            TSNode si = pmm_find_child_by_kind(node, "simple_identifier");
        ...
        if (lang == PMM_LANG_POWERSHELL && strcmp(kind, "function_statement") == 0) {
            TSNode fn = pmm_find_child_by_kind(node, "function_name");
            if (!ts_node_is_null(fn)) {
                return fn;
            }
        }
        ...
        if ((lang == PMM_LANG_CAIRO || lang == PMM_LANG_DLANG || lang == PMM_LANG_ODIN ||
             lang == PMM_LANG_SQUIRREL) &&
            (strcmp(kind, "function_definition") == 0 || strcmp(kind, "function_signature") == 0 ||
             strcmp(kind, "function_declaration") == 0 ||
             strcmp(kind, "procedure_declaration") == 0)) {
            TSNode id = pmm_find_child_by_kind(node, "identifier");
            if (!ts_node_is_null(id)) {
                return id;
            }
        }
        ...
        if (lang == PMM_LANG_ADA &&
            (strcmp(kind, "subprogram_body") == 0 || strcmp(kind, "subprogram_declaration") == 0)) {
            TSNode spec = pmm_find_child_by_kind(node, "procedure_specification");
            if (ts_node_is_null(spec)) {
                spec = pmm_find_child_by_kind(node, "function_specification");
            }
            if (!ts_node_is_null(spec)) {
        ...
        if (lang == PMM_LANG_PASCAL && strcmp(kind, "defProc") == 0) {
        ...
        if (lang == PMM_LANG_JUST && strcmp(kind, "recipe") == 0) {
            TSNode hdr = pmm_find_child_by_kind(node, "recipe_header");
            if (!ts_node_is_null(hdr)) {
                TSNode nm = ts_node_child_by_field_name(hdr, TS_FIELD("name"));
                if (!ts_node_is_null(nm)) {
                    return nm;
        ...
        if (lang == PMM_LANG_RESCRIPT && strcmp(kind, "function") == 0) {
            TSNode parent = ts_node_parent(node);
        ...
        if (lang == PMM_LANG_NICKEL && strcmp(kind, "fun_expr") == 0) {
            TSNode parent = ts_node_parent(node);
        ...
        if (lang == PMM_LANG_NIX && strcmp(kind, "function_expression") == 0) {
            TSNode parent = ts_node_parent(node);
        ...
        if (lang == PMM_LANG_FORTRAN &&
            (strcmp(kind, "subroutine") == 0 || strcmp(kind, "function") == 0)) {
            TSNode stmt = pmm_find_child_by_kind(node, "subroutine_statement");
            if (ts_node_is_null(stmt)) {
                stmt = pmm_find_child_by_kind(node, "function_statement");
            }
            if (!ts_node_is_null(stmt)) {
        ...
        if (lang == PMM_LANG_FSHARP && strcmp(kind, "function_or_value_defn") == 0) {
            TSNode lhs = pmm_find_child_by_kind(node, "function_declaration_left");
            if (ts_node_is_null(lhs)) {
                lhs = pmm_find_child_by_kind(node, "value_declaration_left");
            }
            if (!ts_node_is_null(lhs)) {
                TSNode nm = pmm_find_child_by_kind(lhs, "identifier");
                if (ts_node_is_null(nm)) {
                    nm = pmm_find_child_by_kind(lhs, "long_identifier");
                }
                if (!ts_node_is_null(nm)) {
                    return nm;
                }
        ...
        if (lang == PMM_LANG_GROOVY && strcmp(kind, "function_definition") == 0) {
            TSNode fn = ts_node_child_by_field_name(node, TS_FIELD("function"));
            if (ts_node_is_null(fn)) {
                fn = pmm_find_child_by_kind(node, "identifier");
            }
            if (!ts_node_is_null(fn)) {
                return fn;
            }
        ...
        if (lang == PMM_LANG_AGDA && strcmp(kind, "function") == 0) {
            TSNode lhs = pmm_find_child_by_kind(node, "lhs");
            if (!ts_node_is_null(lhs)) {
                TSNode fn = pmm_find_child_by_kind(lhs, "function_name");
                if (!ts_node_is_null(fn)) {
                    return fn;
                }
            }
        ...
        if (lang == PMM_LANG_COBOL && strcmp(kind, "program_definition") == 0) {
            TSNode iddiv = pmm_find_child_by_kind(node, "identification_division");
            if (!ts_node_is_null(iddiv)) {
                TSNode pname = pmm_find_child_by_kind(iddiv, "program_name");
        ...
        if (lang == PMM_LANG_SCSS &&
            (strcmp(kind, "function_statement") == 0 || strcmp(kind, "mixin_statement") == 0)) {
            TSNode nm = pmm_find_child_by_kind(node, "name");
            if (!ts_node_is_null(nm)) {
                return nm;
            }
        }
        ...
        if (lang == PMM_LANG_JSONNET && strcmp(kind, "bind") == 0) {
        ...
        if (lang == PMM_LANG_TYPST && strcmp(kind, "let") == 0) {
        ...
        if (lang == PMM_LANG_SQL && strcmp(kind, "create_function") == 0) {
            TSNode oref = pmm_find_child_by_kind(node, "object_reference");
        ...
        if (lang == PMM_LANG_ELM && strcmp(kind, "value_declaration") == 0) {
            ...
            if (ts_node_is_null(lhs)) {
                lhs = pmm_find_child_by_kind(node, "function_declaration_left");
            }
            if (!ts_node_is_null(lhs)) {
                TSNode nm = pmm_find_child_by_kind(lhs, "lower_case_identifier");
                if (!ts_node_is_null(nm)) {
                    return nm;
                }
            }
        ...
        if (lang == PMM_LANG_PINE && strcmp(kind, "function_declaration_statement") == 0) {
            TSNode nm = ts_node_child_by_field_name(node, TS_FIELD("function"));
            if (ts_node_is_null(nm)) {
        ...
        if (lang == PMM_LANG_SMALI && strcmp(kind, "method_definition") == 0) {
            TSNode sig = pmm_find_child_by_kind(node, "method_signature");
            if (!ts_node_is_null(sig)) {
                TSNode mid = pmm_find_child_by_kind(sig, "method_identifier");
        ...
        if ((lang == PMM_LANG_VERILOG || lang == PMM_LANG_SYSTEMVERILOG) &&
            (strcmp(kind, "function_declaration") == 0 || strcmp(kind, "task_declaration") == 0)) {
            TSNode si =
                find_first_descendant_by_kind(node, "simple_identifier", PMM_DESCENDANT_MAX_DEPTH);
            if (!ts_node_is_null(si)) {
                return si;
            }
        }
        ...
        if (lang == PMM_LANG_VHDL && (strcmp(kind, "subprogram_declaration") == 0 ||
                                      strcmp(kind, "subprogram_definition") == 0)) {
            TSNode spec = pmm_find_child_by_kind(node, "function_specification");
            if (ts_node_is_null(spec)) {
                spec = pmm_find_child_by_kind(node, "procedure_specification");
            }
            if (!ts_node_is_null(spec)) {
        ...
        if (lang == PMM_LANG_THRIFT || lang == PMM_LANG_SMITHY) {
            TSNode id = pmm_find_child_by_kind(node, "identifier");
            if (!ts_node_is_null(id)) {
                return id;
            }
        ...
        if (lang == PMM_LANG_CAPNP) {
            ...
            if (name_kind) {
                TSNode id = pmm_find_child_by_kind(node, name_kind);
                if (!ts_node_is_null(id)) {
                    return id;
                }
            }
        ...
        if (lang == PMM_LANG_CMAKE &&
            (strcmp(kind, "function_def") == 0 || strcmp(kind, "macro_def") == 0)) {
            const char *cmd_kind =
                strcmp(kind, "function_def") == 0 ? "function_command" : "macro_command";
            TSNode cmd = pmm_find_child_by_kind(node, cmd_kind);
            if (!ts_node_is_null(cmd)) {
                TSNode alist = pmm_find_child_by_kind(cmd, "argument_list");
                if (!ts_node_is_null(alist)) {
                    TSNode arg = pmm_find_child_by_kind(alist, "argument");
                    if (!ts_node_is_null(arg)) {
                        TSNode uq = pmm_find_child_by_kind(arg, "unquoted_argument");
         ...
         * child (no `name` field). */
        if (lang == PMM_LANG_PUPPET &&
            (strcmp(kind, "function_declaration") == 0 || strcmp(kind, "lambda") == 0)) {
            TSNode id = pmm_find_child_by_kind(node, "identifier");
            if (ts_node_is_null(id)) {
                id = pmm_find_child_by_kind(node, "class_identifier");
            }
            if (!ts_node_is_null(id)) {
                return id;
            }
        ...
        if (lang == PMM_LANG_ASSEMBLY && strcmp(kind, "label") == 0) {
            TSNode id = pmm_find_child_by_kind(node, "ident");
            if (!ts_node_is_null(id)) {
                return id;
            }
        }
         ...
         * field). */
        if (lang == PMM_LANG_BITBAKE && (strcmp(kind, "function_definition") == 0 ||
         ...
         * child. */
        if (lang == PMM_LANG_PKL &&
            (strcmp(kind, "classMethod") == 0 || strcmp(kind, "objectMethod") == 0)) {
            TSNode hdr = pmm_find_child_by_kind(node, "methodHeader");
            if (!ts_node_is_null(hdr)) {
                TSNode id = pmm_find_child_by_kind(hdr, "identifier");
                if (!ts_node_is_null(id)) {
                    return id;
                }
            }
...
static bool is_js_exported(TSNode node) {
    return pmm_has_ancestor_kind(node, "export_statement", EXPORT_ANCESTOR_DEPTH);
}
...
static char *extract_comment_text(CBMArena *a, TSNode node, const char *source) {
    char *text = pmm_node_text(a, node, source);
...
static const char *route_path_from_string_node(CBMArena *a, TSNode node, const char *source) {
    ...
    char *path = pmm_node_text(a, node, source);
    if (!path) {
        return NULL;
    }
    int plen = (int)strlen(path);
    if (plen >= PAIR_CHARS && (path[0] == '"' || path[0] == '\'')) {
        path = pmm_arena_strndup(a, path + SKIP_CHAR, (size_t)(plen - PAIR_CHARS));
    }
...
static const char *extract_route_path_from_args(CBMArena *a, TSNode args, const char *source) {
    ...
    for (uint32_t ai = 0; ai < nc && ai < DECORATOR_SCAN_LIMIT; ai++) {
        ...
        const char *path = find_route_path_literal(a, arg, source, PMM_DESCENDANT_MAX_DEPTH);
        if (path) {
            return path;
        }
    }
...
static TSNode find_drf_kwarg_in_args(CBMArena *a, TSNode args, const char *kwarg_name,
    ...
    for (uint32_t ai = 0; ai < nc; ai++) {
        ...
        if (ts_node_is_null(name_node))
            continue;
        char *name = pmm_node_text(a, name_node, source);
...
static bool try_drf_action_decorator(CBMArena *a, TSNode dchild, const char *source,
    ...
    char *fn_text = pmm_node_text(a, fn, source);
    ...
    if (!ts_node_is_null(methods_val) && strcmp(ts_node_type(methods_val), "list") == 0) {
        ...
        for (uint32_t mi = 0; mi < mc && !method; mi++) {
            ...
            if (strcmp(ts_node_type(item), "string") != 0)
                continue;
            char *text = pmm_node_text(a, item, source);
            if (!text)
                continue;
            int tlen = (int)strlen(text);
            if (tlen < PAIR_CHARS || (text[0] != '"' && text[0] != '\''))
                continue;
            char inner[PMM_SZ_16];
            ...
            method = pmm_arena_strdup(a, inner);
        }
    }
    if (!method) {
        method = "GET";
    ...
    if (!ts_node_is_null(url_path_val) && strcmp(ts_node_type(url_path_val), "string") == 0) {
        char *text = pmm_node_text(a, url_path_val, source);
        if (text) {
            int tlen = (int)strlen(text);
            if (tlen >= PAIR_CHARS && (text[0] == '"' || text[0] == '\'')) {
                segment = pmm_arena_strndup(a, text + SKIP_CHAR, (size_t)(tlen - PAIR_CHARS));
            }
        }
    }
    if (!segment) {
        ...
        if (!ts_node_is_null(name_node)) {
            segment = pmm_node_text(a, name_node, source);
        }
    }
    if (!segment) {
        return false;
    ...
    if (detail) {
        *out_path = pmm_arena_sprintf(a, "/{pk}/%s", segment);
    } else {
        *out_path = pmm_arena_sprintf(a, "/%s", segment);
    }
    *out_method = method;
    return true;
}
...
static TSNode annotation_name_node(TSNode annotation) {
    ...
    TSNode ut = pmm_find_child_by_kind(annotation, "user_type");
    if (ts_node_is_null(ut)) {
        TSNode ci = pmm_find_child_by_kind(annotation, "constructor_invocation");
        if (!ts_node_is_null(ci)) {
            ut = pmm_find_child_by_kind(ci, "user_type");
        }
    }
    if (!ts_node_is_null(ut)) {
        TSNode ti = pmm_find_child_by_kind(ut, "type_identifier");
        if (ts_node_is_null(ti)) {
            ti = pmm_find_child_by_kind(ut, "simple_identifier");
        }
        return ti;
    }
    TSNode null_node = {0};
...
static TSNode annotation_args_node(TSNode annotation) {
    ...
    TSNode ci = pmm_find_child_by_kind(annotation, "constructor_invocation");
    if (!ts_node_is_null(ci)) {
        return pmm_find_child_by_kind(ci, "value_arguments");
    }
    return args;
}
    ...
    *out_jax_path = NULL;
    ...
    for (int w = 0; w < wn; w++) {
        ...
        for (uint32_t ci = 0; ci < cc; ci++) {
            ...
            if (!pmm_kind_in_set(child, spec->decorator_node_types)) {
                continue;
            }
...
static void extract_route_from_decorators(CBMArena *a, TSNode func_node, const char *source,
    ...
    *out_method = NULL;
    ...
    while (!ts_node_is_null(prev)) {
        if (!pmm_kind_in_set(prev, spec->decorator_node_types)) {
            break;
        }
...
static const char *join_route_paths(CBMArena *a, const char *prefix, const char *path) {
    ...
    if (prefix_slash && path_slash) {
        return pmm_arena_sprintf(a, "%s%s", prefix, path + SKIP_CHAR);
    }
    if (!prefix_slash && !path_slash) {
        return pmm_arena_sprintf(a, "%s/%s", prefix, path);
    }
    return pmm_arena_sprintf(a, "%s%s", prefix, path);
}
...
static int count_modifier_annotations(TSNode modifiers, const CBMLangSpec *spec) {
    ...
    for (uint32_t mi = 0; mi < mc; mi++) {
        ...
        if (pmm_kind_in_set(mchild, spec->decorator_node_types)) {
            count++;
        }
    }
    return count;
...
static TSNode find_jvm_modifiers(TSNode node, CBMLanguage lang) {
    ...
    if (ts_node_is_null(w)) {
        w = pmm_find_child_by_kind(node, wrapper);
    }
    return w;
}
...
static int collect_child_decorators(CBMArena *a, TSNode node, const char *source,
    ...
    for (uint32_t ci = 0; ci < cc && idx < max; ci++) {
        TSNode child = ts_node_child(node, ci);
        if (pmm_kind_in_set(child, spec->decorator_node_types)) {
            result[idx++] = pmm_node_text(a, child, source);
        }
    }
    return idx;
}
...
static int collect_modifier_decorators(CBMArena *a, TSNode modifiers, const char *source,
    ...
    for (uint32_t mi = 0; mi < mc && idx < max; mi++) {
        TSNode mchild = ts_node_child(modifiers, mi);
        if (pmm_kind_in_set(mchild, spec->decorator_node_types)) {
            result[idx++] = pmm_node_text(a, mchild, source);
        }
    }
    return idx;
}
...
static const char **extract_decorators(CBMArena *a, TSNode node, const char *source,
    ...
    const char **result =
        (const char **)pmm_arena_alloc(a, sizeof(const char *) * (total + NULL_TERM));
    if (!result) {
        return NULL;
    }
    ...
    while (!ts_node_is_null(prev) && idx < count) {
        if (pmm_kind_in_set(prev, spec->decorator_node_types)) {
            result[idx++] = pmm_node_text(a, prev, source);
        } else if (ts_node_is_named(prev)) {
            break;
        }
        prev = ts_node_prev_sibling(prev);
 ...
 * pmm's test detection is otherwise file-path-based (pmm_is_test_file:
...
static const char *rust_cfg_qualified_name(CBMArena *a, const char *base_qn,
    ...
    for (int i = 0; decorators[i]; i++) {
        ...
        buf[bi] = '\0';
        return pmm_arena_sprintf(a, "%s#%s", base_qn, buf);
    }
    return base_qn;
}
...
static char *extract_cpp_base_text(CBMArena *a, TSNode bc, const char *source) {
    ...
    if (strcmp(bk, "type_identifier") == 0 || strcmp(bk, "qualified_identifier") == 0 ||
        ...
        char *t = pmm_node_text(a, bc, source);
        if (t) {
            char *angle = strchr(t, '<');
            if (angle) {
                *angle = '\0';
    ...
    if (strcmp(bk, "template_type") == 0) {
        ...
        if (!ts_node_is_null(tname)) {
            return pmm_node_text(a, tname, source);
        }
    }
    return NULL;
}
...
static const char **extract_cpp_base_classes(CBMArena *a, TSNode clause, const char *source) {
    ...
    if (base_count > 0) {
        const char **result =
            (const char **)pmm_arena_alloc(a, (base_count + NULL_TERM) * sizeof(const char *));
        if (result) {
...
static const char **make_single_base(CBMArena *a, const char *text) {
    ...
    const char **result = (const char **)pmm_arena_alloc(a, sizeof(const char *) * RT_PAIR_SIZE);
    if (result) {
        result[0] = text;
...
static const char **find_base_from_children(CBMArena *a, TSNode node, const char *source,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        for (const char **t = base_types; *t; t++) {
            if (strcmp(ck, *t) == 0) {
                const char **r = make_single_base(a, pmm_node_text(a, child, source));
                if (r) {
                    return r;
                }
            }
...
static const char *extract_csharp_base_child_text(CBMArena *a, TSNode bc, const char *source) {
    ...
    if (strcmp(bk, "identifier") == 0 || strcmp(bk, "generic_name") == 0 ||
        ...
        text = pmm_node_text(a, bc, source);
    } else {
        TSNode inner = ts_node_named_child(bc, 0);
        if (!ts_node_is_null(inner)) {
            text = pmm_node_text(a, inner, source);
        }
    }
    if (text && text[0]) {
...
static void push_base_text(CBMArena *a, TSNode n, const char *source, const char **out, int out_cap,
    ...
    char *t = pmm_node_text(a, n, source);
    if (!t) {
        return;
    }
    char *angle = strchr(t, '<');
...
static const char **extract_ts_bases(CBMArena *a, TSNode node, const char *source) {
    ...
    const char **result =
        (const char **)pmm_arena_alloc(a, (size_t)(count + NULL_TERM) * sizeof(const char *));
    if (!result) {
        return NULL;
    }
...
static int collect_bases_from_field(CBMArena *a, TSNode field_node, const char *source,
    ...
    if (strcmp(fk, "type_identifier") == 0 || strcmp(fk, "generic_type") == 0 ||
        ...
        char *t = pmm_node_text(a, field_node, source);
        if (t) {
            char *angle = strchr(t, '<');
            if (angle) {
                *angle = '\0';
    ...
    for (uint32_t i = 0; i < nc && count < out_cap; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "generic_type") == 0 ||
            ...
            char *t = pmm_node_text(a, child, source);
            if (t) {
                char *angle = strchr(t, '<');
                if (angle) {
                    *angle = '\0';
            ...
            if (!ts_node_is_null(val)) {
                char *t = pmm_node_text(a, val, source);
                if (t && t[0]) {
                    out[count++] = t;
                }
            }
            ...
            for (uint32_t ti = 0; ti < tlnc && count < out_cap; ti++) {
                ...
                if (strcmp(tlk, "type_identifier") == 0 || strcmp(tlk, "generic_type") == 0 ||
                    ...
                    char *t = pmm_node_text(a, tl_child, source);
                    if (t) {
                        char *angle = strchr(t, '<');
                        if (angle) {
                            *angle = '\0';
...
static const char **extract_julia_base_classes(CBMArena *a, TSNode node, const char *source) {
    TSNode th = pmm_find_child_by_kind(node, "type_head");
    ...
    char *bname = pmm_node_text(a, base, source);
    if (!bname || !bname[0]) {
        return NULL;
    }
    const char **result = (const char **)pmm_arena_alloc(a, 2 * sizeof(const char *));
    if (!result) {
        return NULL;
    }
    result[0] = bname;
...
static const char **extract_base_classes(CBMArena *a, TSNode node, const char *source,
    ...
    if (lang == PMM_LANG_OBJECTSCRIPT_UDL) {
        TSNode ext = pmm_find_child_by_kind(node, "class_extends");
        if (!ts_node_is_null(ext)) {
            ...
            for (uint32_t i = 0; i < nc && base_count < MAX_BASES_MINUS_1; i++) {
                ...
                if (strcmp(ts_node_type(ch), "class_name") == 0) {
                    char *base = pmm_node_text(a, ch, source);
                    if (base && base[0]) {
            ...
            if (base_count > 0) {
                const char **result =
                    (const char **)pmm_arena_alloc(a, (base_count + 1) * sizeof(const char *));
                if (result) {
    ...
    if (lang == PMM_LANG_SQUIRREL) {
        ...
        for (uint32_t i = 0; i < sc; i++) {
            ...
            if (seen_extends && strcmp(ck, "identifier") == 0) {
                char *base = pmm_node_text(a, child, source);
                if (base && base[0]) {
                    const char **result =
                        (const char **)pmm_arena_alloc(a, 2 * sizeof(const char *));
                    if (result) {
                        result[0] = base;
                        result[1] = NULL;
                        return result;
    ...
    if (lang == PMM_LANG_FSHARP) {
        TSNode inh =
            find_first_descendant_by_kind(node, "class_inherits_decl", PMM_DESCENDANT_MAX_DEPTH);
        if (!ts_node_is_null(inh)) {
            TSNode st = pmm_find_child_by_kind(inh, "simple_type");
            char *bn = ts_node_is_null(st) ? NULL : pmm_node_text(a, st, source);
            if (bn && bn[0]) {
                const char **result = (const char **)pmm_arena_alloc(a, 2 * sizeof(const char *));
                if (result) {
                    result[0] = bn;
    ...
    if (lang == PMM_LANG_DLANG) {
        ...
        for (uint32_t i = 0; i < nc && pc < MAX_BASES_MINUS_1; i++) {
            ...
            char *bn = pmm_node_text(a, c, source);
            if (bn && bn[0]) {
                pbases[pc++] = bn;
            }
        }
        if (pc > 0) {
            const char **result =
                (const char **)pmm_arena_alloc(a, (pc + NULL_TERM) * sizeof(const char *));
            if (result) {
                for (int i = 0; i < pc; i++) {
                    result[i] = pbases[i];
                }
    ...
    if (lang == PMM_LANG_POWERSHELL && strcmp(ts_node_type(node), "class_statement") == 0) {
        const char *pbases[MAX_BASES];
        int pc = 0;
    ...
    if (lang == PMM_LANG_PASCAL && strcmp(ts_node_type(node), "declClass") == 0) {
        const char *pbases[MAX_BASES];
        int pc = 0;
        uint32_t nc = ts_node_child_count(node);
        for (uint32_t i = 0; i < nc && pc < MAX_BASES_MINUS_1; i++) {
            ...
            char *bn = pmm_node_text(a, pn, source);
            if (bn && bn[0]) {
                pbases[pc++] = bn;
            }
        }
...
static char *clean_type_name(CBMArena *a, const char *raw) {
    ...
    char *result = pmm_arena_alloc(a, len + NULL_TERM);
    memcpy(result, s, len);
    result[len] = '\0';
    return result;
}
...
static char *resolve_param_name(CBMArena *a, TSNode param, const char *source) {
    ...
    if (strcmp(pk, "parameter_declaration") == 0) {
        ...
        if (!ts_node_is_null(nm)) {
            return pmm_node_text(a, nm, source);
        }
        return NULL;
    }
    if (strcmp(pk, "identifier") == 0) {
        return pmm_node_text(a, param, source);
    }
...
static const char **extract_param_names(CBMArena *a, TSNode params, const char *source,
    ...
    const char **result =
        (const char **)pmm_arena_alloc(a, (count + NULL_TERM) * sizeof(const char *));
    for (int i = 0; i < count; i++) {
        result[i] = names[i];
    }
    result[count] = NULL;
    ...
    for (uint32_t i = 0; i < nc && *count < MAX_RETURN_TYPES_MINUS_1; i++) {
        ...
        if (strcmp(ts_node_type(child), "parameter_declaration") == 0) {
            ...
            if (!ts_node_is_null(tn)) {
                add_cleaned_type(a, types, count, pmm_node_text(a, tn, source));
            }
        } else {
            add_cleaned_type(a, types, count, pmm_node_text(a, child, source));
        }
    }
}
...
static const char **extract_return_types(CBMArena *a, TSNode rt_node, const char *source,
    ...
    if (strcmp(ts_node_type(rt_node), "parameter_list") == 0) {
        ...
        add_cleaned_type(a, types, &count, pmm_node_text(a, rt_node, source));
    }
...
static char *extract_ts_param_type(CBMArena *a, TSNode param, const char *source) {
    TSNode ta = pmm_find_child_by_kind(param, "type_annotation");
    ...
    for (uint32_t ti = 0; ti < tanc; ti++) {
        ...
        if (strcmp(tk, "type_identifier") == 0 || strcmp(tk, "generic_type") == 0 ||
            ...
            return pmm_node_text(a, tch, source);
        }
    }
    return NULL;
}
...
static char *resolve_jvm_param_type(CBMArena *a, TSNode param, const char *pk, const char *source,
    ...
    if (lang == PMM_LANG_KOTLIN) {
        ...
        if (!ts_node_is_null(tn)) {
            return pmm_node_text(a, tn, source);
        }
        TSNode ut = pmm_find_child_by_kind(param, "user_type");
        return ts_node_is_null(ut) ? NULL : pmm_node_text(a, ut, source);
    }
    if (lang == PMM_LANG_SCALA || lang == PMM_LANG_DART) {
        TSNode tid = pmm_find_child_by_kind(param, "type_identifier");
        return ts_node_is_null(tid) ? NULL : pmm_node_text(a, tid, source);
    }
    if (lang == PMM_LANG_GROOVY) {
        TSNode tn = ts_node_child_by_field_name(param, TS_FIELD("type"));
        return ts_node_is_null(tn) ? NULL : pmm_node_text(a, tn, source);
    }
    if (lang == PMM_LANG_OCAML) {
        TSNode tp = pmm_find_child_by_kind(param, "typed_pattern");
...
static char *resolve_param_type_text(CBMArena *a, TSNode param, const char *source,
    ...
    if (lang == PMM_LANG_KOTLIN || lang == PMM_LANG_SCALA || lang == PMM_LANG_DART ||
        lang == PMM_LANG_GROOVY || lang == PMM_LANG_OCAML) {
...
static void resolve_cpp_trailing_return(CBMArena *a, TSNode func_node, const char *source,
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ts_node_type(ch), "trailing_return_type") == 0) {
            ...
            def->return_type = pmm_node_text(a, type_desc, source);
            if (def->return_type && def->return_type[0]) {
                const char **rt =
                    (const char **)pmm_arena_alloc(a, RT_PAIR_SIZE * sizeof(const char *));
                if (rt) {
...
static void set_def_complexity(CBMDefinition *def, TSNode body, const CBMLangSpec *spec) {
    pmm_complexity_t cx;
    pmm_compute_complexity(body, spec->branching_node_types, &cx);
...
static void extract_func_def(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec) {
    ...
    TSNode name_node = pmm_resolve_func_name(node, ctx->language);
    if (ts_node_is_null(name_node)) {
        return;
    }

    char *name = pmm_func_name_node_text(a, name_node, ctx->source);
    if (!name || !name[0] || strcmp(name, "function") == 0) {
        return;
    }

    // Makefile special targets (.PHONY, .DEFAULT, .SUFFIXES, …) are directives,
    // not build-rule defs. Their leading '.' would also make pmm_fqn_compute
    // emit a "..PHONY" segment (a "double dot") and thus a malformed QN. Skip
    // any dot-prefixed Make target.
    if (ctx->language == PMM_LANG_MAKEFILE && name[0] == '.') {
        return;
    }
    ...
    def.qualified_name =
        pmm_fqn_compute_source_lang(a, ctx->project, ctx->rel_path, name, ctx->language);
    ...
    if (ctx->enclosing_class_qn &&
        (ctx->language == PMM_LANG_CPP || ctx->language == PMM_LANG_CUDA)) {
        def.qualified_name = pmm_arena_sprintf(a, "%s.%s", ctx->enclosing_class_qn, name);
    }
    def.label = "Function";
    ...
    def.is_exported = pmm_is_exported(name, ctx->language);
    ...
    if (ts_node_is_null(params) &&
        (ctx->language == PMM_LANG_C || ctx->language == PMM_LANG_CPP ||
         ctx->language == PMM_LANG_CUDA || ctx->language == PMM_LANG_GLSL)) {
        params = find_c_params(func_node);
    }
    if (!ts_node_is_null(params)) {
        def.signature = pmm_node_text(a, params, ctx->source);
    ...
    for (const char **f = rt_fields; *f; f++) {
        ...
        if (!ts_node_is_null(rt)) {
            def.return_type = pmm_node_text(a, rt, ctx->source);
    ...
    if (!ts_node_is_null(recv)) {
        def.receiver = pmm_node_text(a, recv, ctx->source);
        def.label = "Method";
         ...
         * is computed the same way (pmm_fqn_compute on the type name). */
        ...
        if (recv_type && recv_type[0]) {
            ...
            def.parent_class = pmm_fqn_compute_source_lang(a, ctx->project, ctx->rel_path,
    ...
    if ((ctx->language == PMM_LANG_CPP || ctx->language == PMM_LANG_CUDA) &&
        strcmp(ts_node_type(node), "function_definition") == 0) {
        char *scope_name = pmm_cpp_out_of_line_parent_class(a, node, ctx->source);
        if (scope_name && scope_name[0]) {
            const char *class_qn = pmm_fqn_compute(a, ctx->project, ctx->rel_path, scope_name);
            def.qualified_name = pmm_arena_sprintf(a, "%s.%s", class_qn, name);
            def.label = "Method";
    ...
    if (ctx->language == PMM_LANG_PONY && def.label && strcmp(def.label, "Function") == 0 &&
        spec->class_node_types) {
        for (TSNode cur = ts_node_parent(node); !ts_node_is_null(cur); cur = ts_node_parent(cur)) {
            if (pmm_kind_in_set(cur, spec->class_node_types)) {
                def.label = "Method";
                TSNode cn = pmm_find_child_by_kind(cur, "identifier");
                if (!ts_node_is_null(cn)) {
                    char *cname = pmm_node_text(a, cn, ctx->source);
                    if (cname && cname[0]) {
                        def.parent_class = pmm_fqn_compute(a, ctx->project, ctx->rel_path, cname);
                    }
                }
                break;
            }
    ...
    if (ctx->language == PMM_LANG_JAVASCRIPT || ctx->language == PMM_LANG_TYPESCRIPT ||
        ctx->language == PMM_LANG_TSX) {
    ...
    pmm_defs_push(&ctx->result->defs, a, def);
}
...
// well-formed QN segment. Markdown headings (e.g. "Project Mind") legitimately
...
static const char *qn_safe_segment(CBMArena *a, const char *name) {
    ...
    char *out = pmm_arena_strdup(a, name);
    if (!out) {
        return name;
    }
    char *w = out;
...
static void push_simple_class_def(CBMExtractCtx *ctx, TSNode node, char *name, const char *label) {
    ...
    memset(&def, 0, sizeof(def));
    def.name = name;
    def.qualified_name = pmm_fqn_compute(a, ctx->project, ctx->rel_path, qn_safe_segment(a, name));
    def.label = label;
    def.file_path = ctx->rel_path;
    def.start_line = ts_node_start_point(node).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(node).row + TS_LINE_OFFSET;
...
static char *find_toml_key_name(CBMArena *a, TSNode node, const char *source) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "bare_key") == 0 || strcmp(ck, "dotted_key") == 0 ||
            ...
            return pmm_node_text(a, child, source);
        }
    }
    return NULL;
}
        ...
        if (strcmp(ck, "start_tag") == 0 || strcmp(ck, "self_closing_tag") == 0 ||
            ...
            for (uint32_t j = 0; j < tnc; j++) {
                ...
                if (strcmp(tk, "tag_name") == 0 || strcmp(tk, "Name") == 0) {
                    return pmm_node_text(a, tag, source);
                }
            }
        }
    }
    // Fallback: try "Name" field directly for some XML grammars
    TSNode name_child = pmm_find_child_by_kind(node, "Name");
    if (!ts_node_is_null(name_child)) {
        return pmm_node_text(a, name_child, source);
    }
    return NULL;
}
...
static char *extract_atx_heading_text(CBMArena *a, TSNode node, const char *source) {
    ...
    char *full = pmm_node_text(a, node, source);
    if (full) {
        char *p = full;
        while (*p == '#') {
            p++;
...
static char *extract_markdown_heading_name(CBMArena *a, TSNode node, const char *kind,
    ...
    if (strcmp(kind, "atx_heading") == 0) {
        ...
        if (ts_node_child_count(node) > 0) {
            name = pmm_node_text(a, ts_node_child(node, 0), source);
        }
    }
...
static char *find_ini_section_name(CBMArena *a, TSNode node, const char *source) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        TSNode text = pmm_find_child_by_kind(child, "text");
        if (!ts_node_is_null(text)) {
            return pmm_node_text(a, text, source);
        }
        return pmm_node_text(a, child, source);
    }
    return NULL;
...
static char *find_hcl_block_name(CBMArena *a, TSNode node, const char *source) {
    ...
    char *name = pmm_node_text(a, id, source);
    if (!name || !name[0]) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        TSNode lit = pmm_find_child_by_kind(ch, "template_literal");
        if (ts_node_is_null(lit)) {
            continue;
        }
        char *label = pmm_node_text(a, lit, source);
        if (!label || !label[0]) {
            continue;
        }
        char *joined = pmm_arena_sprintf(a, "%s.%s", name, label);
        if (joined) {
            name = joined;
        }
    }
...
static bool extract_config_class_def(CBMExtractCtx *ctx, TSNode node, const char *kind) {
    ...
    if (ctx->language == PMM_LANG_TOML &&
        ...
        label = "Section";
    } else if (ctx->language == PMM_LANG_HCL && strcmp(kind, "block") == 0) {
...
static void extract_class_def(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec) {
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_OBJC) {
        name_node = pmm_find_child_by_kind(node, "identifier");
    }
    // ObjectScript UDL: class name is a `class_name` child (no "name" field).
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_OBJECTSCRIPT_UDL) {
        name_node = pmm_find_child_by_kind(node, "class_name");
    }
    // Swift and newer tree-sitter-kotlin: class/object name is a type_identifier
    // child (no "name" field).
    if (ts_node_is_null(name_node) &&
        (ctx->language == PMM_LANG_SWIFT || ctx->language == PMM_LANG_KOTLIN)) {
        name_node = pmm_find_child_by_kind(node, "type_identifier");
    }
    // Protobuf: service_name / message_name / enum_name children
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_PROTOBUF) {
        name_node = pmm_find_child_by_kind(node, "service_name");
        if (ts_node_is_null(name_node)) {
            name_node = pmm_find_child_by_kind(node, "message_name");
        }
        if (ts_node_is_null(name_node)) {
            name_node = pmm_find_child_by_kind(node, "enum_name");
        }
    }
    ...
    if (ts_node_is_null(name_node) &&
        (ctx->language == PMM_LANG_THRIFT || ctx->language == PMM_LANG_SMITHY ||
         ctx->language == PMM_LANG_PONY || ctx->language == PMM_LANG_PKL)) {
        name_node = pmm_find_child_by_kind(node, "identifier");
    }
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_FSHARP) {
        TSNode tn = find_first_descendant_by_kind(node, "type_name", PMM_DESCENDANT_MAX_DEPTH);
        if (!ts_node_is_null(tn)) {
            TSNode id = ts_node_child_by_field_name(tn, "type_name", 9);
            if (ts_node_is_null(id)) {
                id = pmm_find_child_by_kind(tn, "identifier");
            }
            if (!ts_node_is_null(id)) {
                name_node = id;
            }
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_DLANG) {
        name_node = pmm_find_child_by_kind(node, "identifier");
    }
    // PowerShell: class_statement / enum_statement have no `name` field; the
    // name is the FIRST `simple_name` child (`class Dog : Animal { ... }`).
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_POWERSHELL) {
        name_node = pmm_find_child_by_kind(node, "simple_name");
    }
    ...
    // (`TFoo = class ... end`).
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_PASCAL) {
        TSNode parent = ts_node_parent(node);
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_JULIA) {
        TSNode th = pmm_find_child_by_kind(node, "type_head");
        if (!ts_node_is_null(th)) {
            TSNode inner = ts_node_named_child_count(th) > 0 ? ts_node_named_child(th, 0) : th;
            if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "binary_expression") == 0 &&
                ...
                name_node = pmm_find_child_by_kind(th, "identifier");
            }
        }
    }
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_CAPNP) {
        if (strcmp(kind, "enum") == 0) {
            name_node = pmm_find_child_by_kind(node, "enum_identifier");
        } else {
            name_node = pmm_find_child_by_kind(node, "type_identifier");
        }
    }
    // Agda (FIELD_COUNT 0): data > data_name, record > record_name (direct child).
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_AGDA) {
        if (strcmp(kind, "data") == 0) {
            name_node = pmm_find_child_by_kind(node, "data_name");
        } else if (strcmp(kind, "record") == 0) {
            name_node = pmm_find_child_by_kind(node, "record_name");
        }
    }
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_GRAPHQL) {
        name_node = pmm_find_child_by_kind(node, "name");
    }
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_PRISMA) {
        name_node = pmm_find_child_by_kind(node, "identifier");
    }
    // Puppet: class_definition / type_declaration name is a plain identifier or
    // class_identifier child (no `name` field).
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_PUPPET) {
        name_node = pmm_find_child_by_kind(node, "identifier");
        if (ts_node_is_null(name_node)) {
            name_node = pmm_find_child_by_kind(node, "class_identifier");
        }
    }
    // Smali (no `name` field): class_definition > class_directive > class_identifier.
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_SMALI) {
        TSNode dir = pmm_find_child_by_kind(node, "class_directive");
        if (!ts_node_is_null(dir)) {
            name_node = pmm_find_child_by_kind(dir, "class_identifier");
        }
    }
    // VHDL: name lives on a declaration-keyword-named field whose value is an
    // `identifier`. Map node kind -> field name.
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_VHDL) {
    ...
    if (ts_node_is_null(name_node) &&
        (ctx->language == PMM_LANG_VERILOG || ctx->language == PMM_LANG_SYSTEMVERILOG)) {
        if (strcmp(kind, "type_declaration") == 0) {
            name_node = pmm_find_child_by_kind(node, "simple_identifier");
    ...
    if (ts_node_is_null(name_node)) {
        switch (ctx->language) {
        case PMM_LANG_SQUIRREL: // class_declaration > identifier
        case PMM_LANG_DLANG:    // class/struct/interface_declaration > identifier
        case PMM_LANG_HARE:     // type_declaration > identifier
        case PMM_LANG_ODIN:     // struct_declaration > identifier
        case PMM_LANG_BICEP:    // resource/module_declaration > identifier
            name_node = pmm_find_child_by_kind(node, "identifier");
            break;
        case PMM_LANG_POWERSHELL: // class_statement > simple_name
            name_node = pmm_find_child_by_kind(node, "simple_name");
            break;
        case PMM_LANG_SWAY: // impl_item: name is the implemented type (field `type`)
            if (strcmp(kind, "impl_item") == 0) {
                name_node = ts_node_child_by_field_name(node, TS_FIELD("type"));
            }
            break;
        case PMM_LANG_GLEAM: // type_definition > type_name > type_identifier
            name_node =
                find_first_descendant_by_kind(node, "type_identifier", PMM_DESCENDANT_MAX_DEPTH);
            break;
        case PMM_LANG_RESCRIPT: { // type_declaration > type_binding(name=type_identifier)
            TSNode binding = pmm_find_child_by_kind(node, "type_binding");
            if (!ts_node_is_null(binding)) {
                name_node = ts_node_child_by_field_name(binding, TS_FIELD("name"));
                if (ts_node_is_null(name_node)) {
                    name_node = pmm_find_child_by_kind(binding, "type_identifier");
                }
            }
            break;
        }
        case PMM_LANG_FSHARP: { // type_definition > *_type_defn > type_name > identifier
            TSNode tn = find_first_descendant_by_kind(node, "type_name", PMM_DESCENDANT_MAX_DEPTH);
            if (!ts_node_is_null(tn)) {
                name_node = ts_node_child_by_field_name(tn, TS_FIELD("type_name"));
                if (ts_node_is_null(name_node)) {
                    name_node = pmm_find_child_by_kind(tn, "identifier");
                }
            }
            break;
        }
        case PMM_LANG_JULIA: { // struct/abstract_definition > type_head > identifier
            TSNode head = pmm_find_child_by_kind(node, "type_head");
            if (!ts_node_is_null(head)) {
                name_node =
                    find_first_descendant_by_kind(head, "identifier", PMM_DESCENDANT_MAX_DEPTH);
            }
            break;
        }
        case PMM_LANG_TCL: { // namespace > word_list > simple_word ("eval" then name)
            TSNode wl = pmm_find_child_by_kind(node, "word_list");
        ...
        case PMM_LANG_PASCAL: { // declClass is nested; name lives on the parent declType
            TSNode parent = ts_node_parent(node);
            if (!ts_node_is_null(parent) && strcmp(ts_node_type(parent), "declType") == 0) {
                name_node = ts_node_child_by_field_name(parent, TS_FIELD("name"));
            }
        ...
        case PMM_LANG_ZIG: { // `const Foo = struct {...}`: struct/enum/union_declaration
            ...
            if (!ts_node_is_null(parent) &&
                ...
                name_node = pmm_find_child_by_kind(parent, "identifier");
            }
            break;
        }
        default:
    ...
    char *name = pmm_node_text(a, name_node, ctx->source);
    if (!name || !name[0]) {
        return;
    }
    ...
    const char *class_qn;
    if (ctx->enclosing_class_qn) {
        class_qn = pmm_arena_sprintf(a, "%s.%s", ctx->enclosing_class_qn, name);
    } else {
        class_qn = pmm_fqn_compute_source_lang(a, ctx->project, ctx->rel_path, name, ctx->language);
    }
    ...
    if (ctx->language == PMM_LANG_SWAY || ctx->language == PMM_LANG_WGSL) {
    ...
    // through pmm_label_is_type_like(), so a struct still resolves as a type for
    // its methods, fields, inheritance and impls.
    if (ctx->language == PMM_LANG_RUST || ctx->language == PMM_LANG_SWIFT ||
        ctx->language == PMM_LANG_DLANG) {
        if (strcmp(kind, "struct_item") == 0 || strcmp(kind, "struct_declaration") == 0) {
            label = "Struct";
        }
    }
    ...
    if (ctx->language == PMM_LANG_SWIFT && strcmp(kind, "class_declaration") == 0) {
        TSNode dk = ts_node_child_by_field_name(node, TS_FIELD("declaration_kind"));
        if (!ts_node_is_null(dk)) {
            char *dk_text = pmm_node_text(a, dk, ctx->source);
    ...
    if (ctx->language == PMM_LANG_FSHARP && strcmp(label, "Type") == 0) {
        if (!ts_node_is_null(find_first_descendant_by_kind(node, "primary_constr_args",
                                                           PMM_DESCENDANT_MAX_DEPTH)) ||
    ...
    // its methods/fields/embedding resolve through pmm_label_is_type_like(), and
    // pmm_pipeline_implements_go() collects Struct nodes too).
    ...
    if (ctx->language == PMM_LANG_CSHARP) {
        ...
        if (!ts_node_is_null(primary_params)) {
            ...
            for (uint32_t k = 0; k < pcount; k++) {
                ...
                pdef.name = pname;
                pdef.qualified_name = pmm_arena_sprintf(a, "%s.%s", class_qn, pname);
                pdef.label = "Field";
                ...
                pmm_defs_push(&ctx->result->defs, a, pdef);
            }
        }
    }
}
...
static TSNode find_class_body(TSNode class_node, CBMLanguage lang) {
    ...
    if (lang == PMM_LANG_SMALI) {
        return class_node;
    }
    // GraphQL: object/interface fields live in a fields_definition child.
    if (lang == PMM_LANG_GRAPHQL) {
        TSNode b = pmm_find_child_by_kind(class_node, "fields_definition");
    ...
    if (lang == PMM_LANG_PRISMA) {
        TSNode b = pmm_find_child_by_kind(class_node, "statement_block");
        if (!ts_node_is_null(b)) {
            return b;
        }
    }
...
static TSNode resolve_dart_method_name(TSNode child, const char *ck) {
    if (strcmp(ck, "method_signature") == 0) {
        TSNode func_sig = pmm_find_child_by_kind(child, "function_signature");
        if (!ts_node_is_null(func_sig)) {
            TSNode name_node = func_name_node(func_sig);
            if (!ts_node_is_null(name_node)) {
                return name_node;
            }
            return pmm_find_child_by_kind(func_sig, "identifier");
        }
    }
    if (strcmp(ck, "function_signature") == 0) {
        return pmm_find_child_by_kind(child, "identifier");
    }
    TSNode null_node = {0};
    return null_node;
}
...
static TSNode resolve_method_name(TSNode child, CBMLanguage lang) {
    ...
    if ((lang == PMM_LANG_C || lang == PMM_LANG_CPP || lang == PMM_LANG_CUDA ||
        ...
        return pmm_resolve_func_name(child, lang);
    }

    if (lang == PMM_LANG_GROOVY && strcmp(ck, "function_definition") == 0) {
    ...
    if (lang == PMM_LANG_DART) {
        return resolve_dart_method_name(child, ck);
    }

    if (lang == PMM_LANG_OBJC && strcmp(ck, "method_definition") == 0) {
        return pmm_find_child_by_kind(child, "identifier");
    }
    ...
    // (mirrors the free-function case in pmm_resolve_func_name).
    if (lang == PMM_LANG_PONY && (strcmp(ck, "method") == 0 || strcmp(ck, "constructor") == 0 ||
    ...
    if ((lang == PMM_LANG_SWIFT || lang == PMM_LANG_KOTLIN) &&
        ...
        return pmm_find_child_by_kind(child, "simple_identifier");
    }

    // Squirrel: function_declaration's name is a plain `identifier` child.
    if (lang == PMM_LANG_SQUIRREL && strcmp(ck, "function_declaration") == 0) {
        return pmm_find_child_by_kind(child, "identifier");
    }
    ...
    if (lang == PMM_LANG_OBJECTSCRIPT_UDL &&
        ...
        TSNode mdef = pmm_find_child_by_kind(child, "method_definition");
        if (!ts_node_is_null(mdef)) {
            TSNode mname = pmm_find_child_by_kind(mdef, "method_name");
            if (!ts_node_is_null(mname) && ts_node_named_child_count(mname) > 0) {
                return ts_node_named_child(mname, 0);
    ...
    if (lang == PMM_LANG_OBJECTSCRIPT_UDL && strcmp(ck, "query") == 0) {
        return pmm_find_child_by_kind(child, "query_name");
    }
...
static void push_method_def(CBMExtractCtx *ctx, TSNode child, TSNode class_node,
    ...
    const char *method_qn = pmm_arena_sprintf(a, "%s.%s", class_qn, name);
    ...
    if (ts_node_is_null(params) && (ctx->language == PMM_LANG_OBJECTSCRIPT_UDL ||
                                    ctx->language == PMM_LANG_OBJECTSCRIPT_ROUTINE)) {
    ...
    if (!def.return_type && (ctx->language == PMM_LANG_OBJECTSCRIPT_UDL ||
                             ctx->language == PMM_LANG_OBJECTSCRIPT_ROUTINE)) {
        TSNode mdef = pmm_find_child_by_kind(child, "method_definition");
        if (ts_node_is_null(mdef)) {
            mdef = child;
        }
        TSNode rt_node = pmm_find_child_by_kind(mdef, "return_type");
        if (!ts_node_is_null(rt_node)) {
            TSNode tname = pmm_find_child_by_kind(rt_node, "typename");
            if (!ts_node_is_null(tname)) {
                def.return_type = pmm_node_text(a, tname, ctx->source);
            }
        }
    }
    ...
    if (def.route_path && (ctx->language == PMM_LANG_JAVA || ctx->language == PMM_LANG_KOTLIN)) {
...
static void extract_objc_impl_methods(CBMExtractCtx *ctx, TSNode impl_node, const char *class_qn,
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        if (pmm_kind_in_set(inner, spec->function_node_types)) {
...
static void extract_class_methods(CBMExtractCtx *ctx, TSNode class_node, const char *class_qn,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (ctx->language == PMM_LANG_SQUIRREL &&
            strcmp(ts_node_type(child), "member_declaration") == 0) {
            TSNode inner = pmm_find_child_by_kind(child, "function_declaration");
            if (!ts_node_is_null(inner)) {
                child = inner;
            }
        }
        ...
        if (strcmp(ts_node_type(child), "decorated_definition") == 0) {
            ...
            if (ts_node_is_null(def) || !pmm_kind_in_set(def, spec->function_node_types)) {
                continue;
            }
            method_node = def;
        }
        ...
        if (strcmp(ts_node_type(child), "public_field_definition") == 0) {
            ...
            if (ts_node_is_null(value) || !pmm_kind_in_set(value, spec->function_node_types)) {
                continue;
            }
        ...
        if (ctx->language == PMM_LANG_OBJECTSCRIPT_UDL &&
        ...
        if (!pmm_kind_in_set(method_node, spec->function_node_types)) {
            continue;
        }
...
static void extract_rust_impl(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec) {
    ...
    char *type_name = pmm_node_text(a, type_node, ctx->source);
    ...
    if (!ts_node_is_null(trait_node)) {
        char *trait_name = pmm_node_text(a, trait_node, ctx->source);
        ...
        if (trait_name && trait_name[0]) {
            ...
            pmm_impltrait_push(&ctx->result->impl_traits, a, it);
        }
    }

    const char *type_qn = pmm_fqn_compute(a, ctx->project, ctx->rel_path, type_name);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!pmm_kind_in_set(child, spec->function_node_types)) {
            continue;
        }
        ...
        const char *method_qn = pmm_arena_sprintf(a, "%s.%s", type_qn, name);
...
static void extract_elixir_func_def(CBMExtractCtx *ctx, TSNode node, const char *macro) {
    ...
    if (strcmp(fk, "call") == 0 && ts_node_child_count(first_arg) > 0) {
        name = pmm_node_text(a, ts_node_child(first_arg, 0), ctx->source);
    } else if (strcmp(fk, "identifier") == 0) {
        name = pmm_node_text(a, first_arg, ctx->source);
    }
    if (!name || !name[0]) {
        return;
    }
    ...
    memset(&def, 0, sizeof(def));
    def.name = name;
    def.qualified_name = pmm_fqn_compute(a, ctx->project, ctx->rel_path, name);
    def.label = "Function";
    def.file_path = ctx->rel_path;
    def.start_line = ts_node_start_point(node).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(node).row + TS_LINE_OFFSET;
...
static TSNode emit_elixir_module_class(CBMExtractCtx *ctx, TSNode cur) {
    ...
    def.is_exported = true;
    pmm_defs_push(&ctx->result->defs, a, def);
    return pmm_find_child_by_kind(cur, "do_block");
}
...
static void extract_elixir_call(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec) {
    (void)spec;
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, PMM_SZ_64);
    ...
    while (stack.count > 0) {
        ...
        char *macro = pmm_node_text(a, callee, ctx->source);
        if (!macro) {
            continue;
        }
...
static const char *extract_c_declarator_name(CBMArena *a, TSNode decl, const char *source) {
    ...
    if (strcmp(dk, "identifier") == 0) {
        return pmm_node_text(a, declarator, source);
    }
    return NULL;
}
...
static const char *extract_java_field_name(CBMArena *a, TSNode field, const char *source) {
    ...
    if (!ts_node_is_null(name)) {
        return pmm_node_text(a, name, source);
    }
    return NULL;
}
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ts_node_type(decl), "variable_declarator") == 0) {
                ...
                if (ts_node_is_null(id)) {
                    id = pmm_find_child_by_kind(decl, "identifier");
                }
                if (!ts_node_is_null(id)) {
                    push_var_def(ctx, pmm_node_text(a, id, ctx->source), decl);
                }
            }
        }
    }
...
static void extract_enum_members(CBMExtractCtx *ctx, TSNode node, const char *class_qn) {
    ...
    for (uint32_t mi = 0; mi < mc; mi++) {
        ...
        if (ts_node_is_null(mname)) {
            mname = pmm_find_child_by_kind(member, "identifier");
        }
        if (ts_node_is_null(mname)) {
            continue;
        }
        char *member_name = pmm_node_text(a, mname, ctx->source);
        ...
        mdef.qualified_name = pmm_arena_sprintf(a, "%s.%s", class_qn, member_name);
        mdef.label = "Variable";
        ...
        pmm_defs_push(&ctx->result->defs, a, mdef);
    }
}
...
static void extract_destructured_vars(CBMExtractCtx *ctx, TSNode pattern, TSNode decl,
    ...
    for (uint32_t pi = 0; pi < pc; pi++) {
        ...
        char *id_text = pmm_node_text(a, ident, ctx->source);
...
static bool is_require_import_call(TSNode value, const char *source, CBMArena *a) {
    ...
    char *fn_name = pmm_node_text(a, fn, source);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!ts_node_is_null(vname)) {
            ...
            if (strcmp(nk, "object_pattern") == 0 || strcmp(nk, "array_pattern") == 0) {
                ...
                push_var_def(ctx, pmm_node_text(a, vname, ctx->source), child);
            }
        }
    }
}
...
static void extract_vars_mainstream(CBMExtractCtx *ctx, TSNode node, CBMArena *a,
    ...
    case PMM_LANG_PYTHON: {
        ...
        if (strcmp(lt, "identifier") == 0) {
            push_var_def(ctx, pmm_node_text(a, left, ctx->source), node);
            ...
            for (uint32_t li = 0; li < ln; li++) {
                ...
                if (strcmp(ts_node_type(part), "identifier") == 0) {
                    push_var_def(ctx, pmm_node_text(a, part, ctx->source), node);
                }
            }
        }
        break;
    ...
    case PMM_LANG_JAVASCRIPT:
    case PMM_LANG_TYPESCRIPT:
    case PMM_LANG_TSX:
    ...
    case PMM_LANG_RUST: {
        TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
        if (!ts_node_is_null(name_node)) {
            push_var_def(ctx, pmm_node_text(a, name_node, ctx->source), node);
        }
        break;
    }
    default:
...
static void extract_lua_vars(CBMExtractCtx *ctx, TSNode node, CBMArena *a) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        TSNode expr_list = pmm_find_child_by_kind(child, "expression_list");
...
static void extract_perl_vars(CBMExtractCtx *ctx, TSNode node, CBMArena *a) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (is_perl_var_type(ck)) {
            push_var_def(ctx, strip_perl_sigil(pmm_node_text(a, child, ctx->source)), node);
            return;
        }
        ...
        push_var_def(ctx, strip_perl_sigil(pmm_node_text(a, left, ctx->source)), node);
        return;
    }
}
...
static void extract_php_vars(CBMExtractCtx *ctx, TSNode node, CBMArena *a, const char *kind) {
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        if (strcmp(ts_node_type(inner), "assignment_expression") == 0) {
            ...
            if (!ts_node_is_null(left)) {
                char *name = pmm_node_text(a, left, ctx->source);
                if (name && name[0] == '$') {
                    name++;
                }
...
static TSNode resolve_kotlin_var_name(TSNode node) {
    ...
    TSNode var_decl = pmm_find_child_by_kind(node, "variable_declaration");
    if (!ts_node_is_null(var_decl)) {
        name_node = pmm_find_child_by_kind(var_decl, "simple_identifier");
        if (!ts_node_is_null(name_node)) {
            return name_node;
        }
        return pmm_find_child_by_kind(var_decl, "identifier");
    }
    TSNode null_node = {0};
    return null_node;
}
...
static void extract_vars_jvm(CBMExtractCtx *ctx, TSNode node, CBMArena *a) {
    ...
    case PMM_LANG_SCALA: {
        ...
        if (!ts_node_is_null(pattern)) {
            push_var_def(ctx, pmm_node_text(a, pattern, ctx->source), node);
        } else {
            TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
            if (!ts_node_is_null(name_node)) {
                push_var_def(ctx, pmm_node_text(a, name_node, ctx->source), node);
...
static void extract_ini_vars(CBMExtractCtx *ctx, TSNode node, CBMArena *a) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "setting_name") == 0 || strcmp(ck, "name") == 0) {
            push_var_def(ctx, trim_whitespace(pmm_node_text(a, child, ctx->source)), node);
            return;
        }
    }
    if (nc > 0) {
        ...
        if (!found_name) {
            push_var_def(
                ctx, trim_whitespace(pmm_node_text(a, ts_node_child(node, 0), ctx->source)), node);
        }
    }
}
...
static void push_first_matching_child(CBMExtractCtx *ctx, TSNode node, CBMArena *a,
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (const char **t = match_types; *t; t++) {
            if (strcmp(ck, *t) == 0) {
                push_var_def(ctx, pmm_node_text(a, child, ctx->source), node);
                return;
            }
        }
    }
...
static void extract_json_var(CBMExtractCtx *ctx, TSNode node, CBMArena *a) {
    ...
    char *raw = pmm_node_text(a, key_node, ctx->source);
    if (raw) {
...
static void extract_scss_var(CBMExtractCtx *ctx, TSNode node, CBMArena *a) {
    ...
    if (ts_node_is_null(prop)) {
        prop = pmm_find_child_by_kind(node, "property_name");
    }
    if (ts_node_is_null(prop)) {
        prop = pmm_find_child_by_kind(node, "variable_name");
    }
    if (!ts_node_is_null(prop)) {
        push_var_def(ctx, pmm_node_text(a, prop, ctx->source), node);
    }
}
...
static void extract_vars_config(CBMExtractCtx *ctx, TSNode node, CBMArena *a, const char *kind) {
    ...
    case PMM_LANG_YAML: {
        ...
        if (!ts_node_is_null(key)) {
            push_var_def(ctx, pmm_node_text(a, key, ctx->source), node);
        }
        break;
    }
    case PMM_LANG_TOML: {
...
static void extract_var_names(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec) {
    ...
    case PMM_LANG_DOCKERFILE:
        if (strcmp(kind, "env_instruction") == 0) {
            ...
            for (uint32_t i = 0; i < ec; i++) {
                ...
                if (!ts_node_is_null(nm)) {
                    push_var_def(ctx, pmm_node_text(a, nm, ctx->source), pair);
                }
            }
        } else if (strcmp(kind, "arg_instruction") == 0) {
            TSNode nm = ts_node_child_by_field_name(node, TS_FIELD("name"));
            if (ts_node_is_null(nm)) {
                nm = pmm_find_child_by_kind(node, "unquoted_string");
            }
            if (!ts_node_is_null(nm)) {
                push_var_def(ctx, pmm_node_text(a, nm, ctx->source), node);
            }
        }
        return;
    ...
    case PMM_LANG_PROPERTIES:
        if (strcmp(kind, "property") == 0) {
            TSNode key = pmm_find_child_by_kind(node, "key");
            if (!ts_node_is_null(key)) {
                push_var_def(ctx, pmm_node_text(a, key, ctx->source), node);
            }
        }
    ...
    case PMM_LANG_GOMOD:
        if (strcmp(kind, "require_directive") == 0 || strcmp(kind, "replace_directive") == 0) {
            ...
            for (uint32_t i = 0; i < rc; i++) {
                ...
                TSNode mp = pmm_find_child_by_kind(req_spec, "module_path");
                if (!ts_node_is_null(mp)) {
                    push_var_def(ctx, pmm_node_text(a, mp, ctx->source), req_spec);
                }
            }
        }
        return;
...
static void walk_variables_iter(CBMExtractCtx *ctx, TSNode root, const CBMLangSpec *spec) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, PMM_SZ_256);
...
static void extract_yaml_toplevel_keys(CBMExtractCtx *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!ts_node_is_null(key)) {
            push_var_def(ctx, pmm_node_text(a, key, ctx->source), pair);
        }
    }
}
...
static void extract_variables(CBMExtractCtx *ctx, TSNode root, const CBMLangSpec *spec) {
    ...
    if (ctx->language == PMM_LANG_YAML && is_helm_values_file(ctx->rel_path)) {
        extract_yaml_toplevel_keys(ctx, root);
        return;
    }

    // Config languages with nested structure: use recursive walk
    if (ctx->language == PMM_LANG_YAML || ctx->language == PMM_LANG_TOML ||
        ctx->language == PMM_LANG_INI || ctx->language == PMM_LANG_JSON) {
    ...
    if (!pmm_is_module_level_p(root, ctx->language)) {
        return;
    }
        ...
        do {
            ...
            if (strcmp(ck, "expression_statement") == 0 || strcmp(ck, "export_statement") == 0 ||
                ...
                for (uint32_t j = 0; j < nc; j++) {
                    ...
                    if (pmm_kind_in_set(inner, spec->variable_node_types)) {
...
static bool extract_schema_field(CBMExtractCtx *ctx, TSNode child, const char *class_qn) {
    ...
    if (ctx->language == PMM_LANG_GRAPHQL) {
        name_node = ts_node_child_by_field_name(child, TS_FIELD("name"));
        if (ts_node_is_null(name_node)) {
            name_node = pmm_find_child_by_kind(child, "name");
        }
        type_node = ts_node_child_by_field_name(child, TS_FIELD("type"));
    } else if (ctx->language == PMM_LANG_PRISMA) {
        name_node = pmm_find_child_by_kind(child, "identifier");
        type_node = pmm_find_child_by_kind(child, "column_type");
    } else if (ctx->language == PMM_LANG_SMALI) {
        name_node = pmm_find_child_by_kind(child, "field_identifier");
        type_node = pmm_find_child_by_kind(child, "field_type");
    } else {
        return false;
    }
    ...
    def.parent_class = class_qn;
    if (!ts_node_is_null(type_node)) {
        def.return_type = pmm_node_text(a, type_node, ctx->source);
    }
    def.start_line = ts_node_start_point(child).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(child).row + TS_LINE_OFFSET;
    def.is_exported = pmm_is_exported(name, ctx->language);
...
static void extract_class_fields(CBMExtractCtx *ctx, TSNode class_node, const char *class_qn,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!pmm_kind_in_set(child, spec->field_node_types)) {
            continue;
        }
        ...
        if (ctx->language == PMM_LANG_OBJECTSCRIPT_UDL) {
            if (strcmp(ts_node_type(child), "property") == 0 ||
                ...
                TSNode pname = pmm_find_child_by_kind(child, "property_name");
                if (ts_node_is_null(pname)) {
                    pname = pmm_find_child_by_kind(child, "parameter_name");
                }
                if (!ts_node_is_null(pname) && ts_node_named_child_count(pname) > 0) {
                    TSNode ident = ts_node_named_child(pname, 0);
                    char *pn = pmm_node_text(a, ident, ctx->source);
                    if (pn && pn[0]) {
                        CBMDefinition pdef;
                        memset(&pdef, 0, sizeof(pdef));
                        pdef.name = pn;
                        pdef.qualified_name = pmm_arena_sprintf(a, "%s.%s", class_qn, pn);
                        pdef.label = "Variable";
                        pdef.file_path = ctx->rel_path;
                        pdef.parent_class = class_qn;
            ...
            if (name_child_kind) {
                TSNode nname = pmm_find_child_by_kind(child, name_child_kind);
                if (!ts_node_is_null(nname)) {
                    char *mn = pmm_node_text(a, nname, ctx->source);
                    if (mn && mn[0]) {
                        CBMDefinition mdef;
                        memset(&mdef, 0, sizeof(mdef));
                        mdef.name = mn;
                        mdef.qualified_name = pmm_arena_sprintf(a, "%s.%s", class_qn, mn);
                        ...
                        if (strcmp(member_label, "Storage") == 0) {
                            TSNode sbody = pmm_find_child_by_kind(child, "storage_body");
                            if (!ts_node_is_null(sbody)) {
                                char *xml = pmm_node_text(a, sbody, ctx->source);
                                if (xml) {
                                    char props[PMM_SZ_2K];
                                    ...
                                    if (!first) {
                                        mdef.docstring = pmm_arena_strdup(a, props);
                                    }
                                }
                            }
                        }
                        ...
                        if (strcmp(member_label, "Trigger") == 0) {
                            TSNode tbody = pmm_find_child_by_kind(child, "core_trigger");
                            if (ts_node_is_null(tbody)) {
                                tbody = pmm_find_child_by_kind(child, "external_trigger");
                            }
                            if (!ts_node_is_null(tbody)) {
                                mdef.body_tokens = extract_body_ident_tokens(ctx, tbody);
                                char *raw = pmm_node_text(a, tbody, ctx->source);
                                if (raw && raw[0]) {
                                    char esc[PMM_SZ_512];
                                    int ei = 0;
        ...
        char *type_text = pmm_node_text(a, type_node, ctx->source);
        if (!type_text || !type_text[0]) {
            continue;
        }
        ...
        const char *field_qn = pmm_arena_sprintf(a, "%s.%s", class_qn, name);
...
// Generous safety ceiling (frames), env-overridable via PMM_WALK_DEFS_MAX.
...
static int wd_stack_max(void) {
    const char *e = getenv("PMM_WALK_DEFS_MAX");
    if (e) {
        int v = atoi(e);
        if (v > 0) {
            return v;
    ...
    if (s->top >= s->cap) {
        ...
        if (ncap > wd_stack_max()) {
            if (!s->warned) {
                ...
                pmm_log_warn("extract.walk_defs_capped", "limit", lim, "path",
                             s->path ? s->path : "");
                s->warned = true;
            }
...
static void push_nested_class_nodes(TSNode body, const CBMLangSpec *spec, wd_stack_t *s,
    ...
    while (nc_stack.count > 0) {
        ...
        for (int i = (int)nc - SKIP_CHAR; i >= 0; i--) {
            ...
            if (pmm_kind_in_set(child, spec->class_node_types)) {
...
static const char *compute_class_qn(CBMExtractCtx *ctx, TSNode node, const char *saved_enclosing) {
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_SWIFT) {
        name_node = pmm_find_child_by_kind(node, "type_identifier");
    }
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_OBJECTSCRIPT_UDL) {
        name_node = pmm_find_child_by_kind(node, "class_name");
    ...
    if (!ts_node_is_null(name_node)) {
        char *cname = pmm_node_text(ctx->arena, name_node, ctx->source);
        if (cname && cname[0]) {
            if (saved_enclosing) {
                return pmm_arena_sprintf(ctx->arena, "%s.%s", saved_enclosing, cname);
            }
            /* Top-level: language-aware module so Java/Go don't double the
             * filename stem (matches extract_class_def above). */
            return pmm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, cname,
                                               ctx->language);
        }
    }
...
static void push_class_body_children(TSNode node, const CBMLangSpec *spec, wd_stack_t *s,
    ...
    for (uint32_t ci = 0; ci < nc; ci++) {
        ...
        if (strcmp(ck, "field_declaration_list") == 0 || strcmp(ck, "class_body") == 0 ||
            ...
            (strcmp(ck, "closure") == 0 && spec->language == PMM_LANG_GROOVY)) {
...
static void extract_cfml_function_tag(CBMExtractCtx *ctx, TSNode node) {
    ...
    for (uint32_t i = 0; i < cc && !name; i++) {
        ...
        TSNode an = pmm_find_child_by_kind(ch, "cf_attribute_name");
        if (ts_node_is_null(an)) {
            continue;
        }
        char *aname = pmm_node_text(a, an, ctx->source);
        if (!ascii_ci_equals(aname, "name")) {
            continue;
        }
        TSNode val = pmm_find_child_by_kind(ch, "quoted_cf_attribute_value");
        if (ts_node_is_null(val)) {
            val = pmm_find_child_by_kind(ch, "cf_attribute_value");
        }
        if (ts_node_is_null(val)) {
            continue;
        }
        TSNode inner = pmm_find_child_by_kind(val, "attribute_value");
        name = pmm_node_text(a, ts_node_is_null(inner) ? val : inner, ctx->source);
    }
    if (!name || !name[0]) {
        return;
    }
...
static void extract_gotemplate_define(CBMExtractCtx *ctx, TSNode node) {
    CBMArena *a = ctx->arena;
    TSNode s = pmm_find_child_by_kind(node, "interpreted_string_literal");
    if (ts_node_is_null(s)) {
        return;
    }
    char *raw = pmm_node_text(a, s, ctx->source);
    if (!raw) {
        return;
    }
    size_t len = strlen(raw);
    if (len >= 2 && (raw[0] == '"' || raw[0] == '`')) {
        raw = pmm_arena_strndup(a, raw + 1, len - 2); // strip surrounding quotes
    }
    if (!raw || !raw[0]) {
        return;
    }
    ...
    memset(&def, 0, sizeof(def));
    def.name = raw;
    def.qualified_name = pmm_fqn_compute(a, ctx->project, ctx->rel_path, raw);
    def.label = "Function";
    def.file_path = ctx->rel_path;
    def.start_line = ts_node_start_point(node).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(node).row + TS_LINE_OFFSET;
...
static void extract_janet_def(CBMExtractCtx *ctx, TSNode node) {
    ...
    char *head_text = pmm_node_text(a, head, ctx->source);
...
static bool is_c_preprocessor_lang(CBMLanguage lang) {
    return lang == PMM_LANG_C || lang == PMM_LANG_CPP || lang == PMM_LANG_CUDA ||
           lang == PMM_LANG_GLSL || lang == PMM_LANG_OBJC || lang == PMM_LANG_ISPC;
}
...
static void extract_lisp_def(CBMExtractCtx *ctx, TSNode node) {
    ...
    char *head = pmm_node_text(a, ts_node_named_child(node, 0), ctx->source);
...
static void recover_kotlin_error_classes(CBMExtractCtx *ctx, TSNode err_node) {
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        if (ctx->enclosing_class_qn) {
            ...
            class_qn = pmm_fqn_compute(a, ctx->project, ctx->rel_path, name);
        }
        ...
        def.is_exported = pmm_is_exported(name, ctx->language);
        if (bcount > 0) {
            const char **result = (const char **)pmm_arena_alloc(a, (size_t)(bcount + NULL_TERM) *
                                                                        sizeof(const char *));
            if (result) {
...
static void walk_defs(CBMExtractCtx *ctx, TSNode root, const CBMLangSpec *spec, int depth_unused) {
    ...
    while (s.top > 0) {
        ...
        if (ctx->language == PMM_LANG_KOTLIN && strcmp(kind, "ERROR") == 0) {
            recover_kotlin_error_classes(ctx, node);
        }

        if (ctx->language == PMM_LANG_ELIXIR && strcmp(kind, "call") == 0) {
        ...
        if (ctx->language == PMM_LANG_GOTEMPLATE && strcmp(kind, "define_action") == 0) {
        ...
        if ((ctx->language == PMM_LANG_CLOJURE || ctx->language == PMM_LANG_RACKET ||
             ctx->language == PMM_LANG_SCHEME) &&
        ...
        if (ctx->language == PMM_LANG_JANET && strcmp(kind, "par_tup_lit") == 0) {
        ...
        if (ctx->language == PMM_LANG_WIT &&
            (strcmp(kind, "export_item") == 0 || strcmp(kind, "import_item") == 0) &&
            ts_node_is_null(
                find_first_descendant_by_kind(node, "func_type", PMM_DESCENDANT_MAX_DEPTH))) {
            wd_push_children_reverse(&s, node, frame.enclosing_class_qn);
            continue;
        }

        if (pmm_kind_in_set(node, spec->function_node_types)) {
            if (!is_template_class_node(node, ctx->language)) {
                ...
                bool descend_into_func =
                    (ctx->language == PMM_LANG_WOLFRAM || ctx->language == PMM_LANG_TYPESCRIPT ||
                     ctx->language == PMM_LANG_JAVASCRIPT || ctx->language == PMM_LANG_TSX ||
                     ctx->language == PMM_LANG_ADA);
        ...
        if (ctx->language == PMM_LANG_RUST && strcmp(kind, "impl_item") == 0) {
        ...
        if (pmm_kind_in_set(node, spec->class_node_types)) {
...
void pmm_extract_definitions(CBMExtractCtx *ctx) {
    const CBMLangSpec *spec = pmm_lang_spec(ctx->language);
    if (!spec) {
        return;
    }
    ...
    pmm_defs_push(&ctx->result->defs, a, mod);
