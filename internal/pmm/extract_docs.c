...
static int doc_paths_push(char ***paths, int *count, int *cap, const char *path) {
    if (*count >= *cap) {
        int nc = *cap ? *cap * 2 : 16;
        char **grown = realloc(*paths, (size_t)nc * sizeof(char *));
        if (!grown) {
            return -1;
        }
        *paths = grown;
...
static int doc_collect_walk(const char *dir, char ***paths, int *count, int *cap, int depth) {
    if (depth > 8 || *count >= 500) {
        return 0;
    }
    DIR *d = opendir(dir);
    if (!d) {
    ...
    while ((ent = readdir(d)) != NULL) {
        ...
        if (S_ISDIR(st.st_mode)) {
            if (doc_collect_walk(full, paths, count, cap, depth + 1) != 0) {
                closedir(d);
                return -1;
            }
        } else if (S_ISREG(st.st_mode) && pmm_doc_path_is_document(full)) {
            if (doc_paths_push(paths, count, cap, full) != 0) {
                closedir(d);
                return -1;
            }
        }
...
int pmm_doc_collect_paths(const char *root_path, char ***out_paths, int *out_count) {
    if (!root_path || !out_paths || !out_count) {
        return -1;
    }
    *out_paths = NULL;
    *out_count = 0;
    ...
    if (S_ISREG(st.st_mode)) {
        if (!pmm_doc_path_is_document(root_path)) {
            return -1;
        }
        if (doc_paths_push(&paths, &count, &cap, root_path) != 0) {
            pmm_doc_paths_free(paths, count);
            return -1;
        }
    } else if (S_ISDIR(st.st_mode)) {
        if (doc_collect_walk(root_path, &paths, &count, &cap, 0) != 0) {
            pmm_doc_paths_free(paths, count);
            return -1;
        }
    } else {
...
enum { DOC_WALK_MAX_FILES = 500, DOC_WALK_MAX_DEPTH = 8 };
...
static void doc_walk_dir(const char *dir, int depth, char ***paths, int *count, int *cap) {
    if (depth > DOC_WALK_MAX_DEPTH || *count >= DOC_WALK_MAX_FILES) {
        return;
    }
    DIR *d = opendir(dir);
    if (!d) {
    ...
    struct dirent *ent;
    while ((ent = readdir(d)) != NULL && *count < DOC_WALK_MAX_FILES) {
        if (ent->d_name[0] == '.') {
            continue;
        }
        char full[2048];
        ...
        if (S_ISDIR(st.st_mode)) {
            doc_walk_dir(full, depth + 1, paths, count, cap);
        } else if (S_ISREG(st.st_mode) && pmm_doc_path_is_document(full)) {
            if (doc_paths_push(paths, count, cap, full) != 0) {
                break;
            }
...
int pmm_doc_collect_paths(const char *root_path, char ***out_paths, int *out_count) {
    ...
    *out_count = 0;
    ...
    if (S_ISREG(st.st_mode)) {
        ...
        doc_walk_dir(root_path, 0, &paths, &count, &cap);
        if (count == 0) {
            free(paths);
            return -1;
        }
