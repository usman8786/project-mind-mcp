#ifndef PMM_HELPERS_H
#define PMM_HELPERS_H
...
// pmm_is_keyword: it only covers builtins with a target node, so un-filtering
// them cannot produce a node-less / Module-sourced edge. The Python set MUST
// stay in sync with kPyBuiltinNodes in internal/pmm/lsp/py_builtins.c.
bool pmm_is_resolvable_builtin(const char *name, CBMLanguage lang);

// Classify a string literal as URL, config, or neither.
// Returns PMM_STRREF_URL (0), PMM_STRREF_CONFIG (1), or -1 for neither.
...
#define PMM_DECLARATOR_DEPTH_LIMIT 8
...
// Free the calling thread's pmm_kind_in_set bitset cache (call at thread/process
...
// pmm_fqn_module returns (no behavior change).
...
// language this is exactly pmm_fqn_compute (no behavior change).
