...
#include "arena.h" // pmm_arena_sprintf
#include "pmm.h"   // CBMExtractCtx
#include "helpers.h"
#include "lang_specs.h"      // CBMLangSpec, pmm_lang_spec, PMM_LANG_*
    ...
    if (ts_node_named_child_count(node) > 0) {
        ...
        if (strcmp(ts_node_type(lhs), "apply") == 0 && ts_node_named_child_count(lhs) > 0) {
            ...
            if (strcmp(ts_node_type(head), "user_symbol") == 0) {
                char *name = pmm_node_text(ctx->arena, head, ctx->source);
                if (name && name[0]) {
                    return pmm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
                }
            }
        }
    }
...
static const char *compute_lisp_func_qn(CBMExtractCtx *ctx, TSNode node) {
    ...
    char *head = pmm_node_text(ctx->arena, ts_node_named_child(node, 0), ctx->source);
...
static const char *compute_elixir_func_qn(CBMExtractCtx *ctx, TSNode node) {
    ...
    char *macro = pmm_node_text(ctx->arena, ts_node_child(node, 0), ctx->source);
    ...
    if (strcmp(fk, "call") == 0 && ts_node_child_count(first_arg) > 0) {
        name = pmm_node_text(ctx->arena, ts_node_child(first_arg, 0), ctx->source);
    } else if (strcmp(fk, "identifier") == 0) {
        name = pmm_node_text(ctx->arena, first_arg, ctx->source);
    }
    if (!name || !name[0]) {
        return NULL;
    }
...
static const char *compute_cfml_func_qn(CBMExtractCtx *ctx, TSNode node) {
    ...
    for (uint32_t i = 0; i < cc && !name; i++) {
        ...
        char *aname = pmm_node_text(ctx->arena, an, ctx->source);
        ...
        name = pmm_node_text(ctx->arena, ts_node_is_null(inner) ? val : inner, ctx->source);
    }
    if (!name || !name[0]) {
        return NULL;
    }
...
static const char *compute_gotemplate_func_qn(CBMExtractCtx *ctx, TSNode node) {
    ...
    char *raw = pmm_node_text(ctx->arena, s, ctx->source);
    if (!raw) {
        return NULL;
    }
    size_t len = strlen(raw);
    if (len >= 2 && (raw[0] == '"' || raw[0] == '`')) {
        raw = pmm_arena_strndup(ctx->arena, raw + 1, len - 2); // strip surrounding quotes
    }
    if (!raw || !raw[0]) {
        return NULL;
    }
    return pmm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, raw);
}
...
static TSNode find_class_method_call(TSNode root, const char *end) {
    ...
    for (const char **c = containers; *c; c++) {
        TSNode inner = pmm_find_child_by_kind(root, *c);
        if (!ts_node_is_null(inner)) {
            TSNode hit = pmm_find_child_by_kind(inner, "class_method_call");
            if (!ts_node_is_null(hit)) {
                return hit;
            }
            TSNode inner2 = pmm_find_child_by_kind(inner, "expr_atom");
            if (!ts_node_is_null(inner2)) {
                hit = pmm_find_child_by_kind(inner2, "class_method_call");
                if (!ts_node_is_null(hit)) {
                    return hit;
                }
            }
    ...
    return pmm_find_child_by_kind(root, "class_method_call");
}
...
static void handle_objectscript_type_map(CBMExtractCtx *ctx, TSNode node, WalkState *state) {
    ...
    if (strcmp(nk, "command_set") == 0) {
        for (uint32_t i = 0; i < ts_node_named_child_count(node); i++) {
            ...
            TSNode method_name_node = pmm_find_child_by_kind(cm_call, "method_name");
            ...
            char *method_text = pmm_node_text(ctx->arena, mn_ident, ctx->source);
            if (!method_text) {
                continue;
            }

            TSNode class_ref = pmm_find_child_by_kind(cm_call, "class_ref");
            ...
            char *cls = pmm_node_text(ctx->arena, cname, ctx->source);
            if (!cls || !cls[0]) {
                continue;
            }
            ...
            TSNode var_node = lhs;
            TSNode inner = pmm_find_child_by_kind(lhs, "glvn");
            if (!ts_node_is_null(inner)) {
                var_node = inner;
            }
            inner = pmm_find_child_by_kind(var_node, "lvn");
            if (!ts_node_is_null(inner)) {
                var_node = inner;
            }
            char *var = pmm_node_text(ctx->arena, var_node, ctx->source);
            if (!var || !var[0]) {
                continue;
            }
    ...
    if (strcmp(nk, "property") == 0 || strcmp(nk, "relationship") == 0) {
        TSNode prop_name_node = pmm_find_child_by_kind(node, "property_name");
        if (ts_node_is_null(prop_name_node)) {
            prop_name_node = pmm_find_child_by_kind(node, "relationship_name");
        }
        TSNode ret_type = pmm_find_child_by_kind(node, "return_type");
        if (!ts_node_is_null(prop_name_node) && !ts_node_is_null(ret_type)) {
            TSNode tname = pmm_find_child_by_kind(ret_type, "typename");
            if (!ts_node_is_null(tname)) {
                char *pname = pmm_node_text(ctx->arena, prop_name_node, ctx->source);
                char *ptype = pmm_node_text(ctx->arena, tname, ctx->source);
                if (pname && pname[0] && ptype && ptype[0]) {
                    char *dot_name = pmm_arena_sprintf(ctx->arena, "..%s", pname);
...
static const char *objectscript_get_class_name(CBMExtractCtx *ctx, TSNode node) {
    for (uint32_t i = 0; i < ts_node_named_child_count(node); i++) {
        ...
        if (strcmp(ts_node_type(child), "class_name") == 0) {
            char *name = pmm_node_text(ctx->arena, child, ctx->source);
            if (name && name[0]) {
                return pmm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
            }
        }
            ...
            for (uint32_t j = 0; j < ts_node_named_child_count(child); j++) {
                ...
                if (strcmp(ts_node_type(mchild), "method_name") == 0) {
                    if (ts_node_named_child_count(mchild) > 0) {
                        ...
                        char *name = pmm_node_text(ctx->arena, ident, ctx->source);
                        if (name && name[0]) {
                            if (enclosing_class_qn) {
                                return pmm_arena_sprintf(ctx->arena, "%s.%s", enclosing_class_qn,
                                                         name);
                            }
                            return pmm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
                        }
...
static const char *compute_func_qn(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec,
     ...
     * resolver below. */
    if (ctx->language == PMM_LANG_CFML && strcmp(ts_node_type(node), "cf_function_tag") == 0) {
    ...
    if (ctx->language == PMM_LANG_GOTEMPLATE) {
    ...
    if (ctx->language == PMM_LANG_CLOJURE || ctx->language == PMM_LANG_SCHEME ||
        ctx->language == PMM_LANG_RACKET) {
    ...
    if (ctx->language == PMM_LANG_OBJC && strcmp(ts_node_type(node), "method_definition") == 0) {
        TSNode id = pmm_find_child_by_kind(node, "identifier");
        if (!ts_node_is_null(id)) {
            char *mname = pmm_node_text(ctx->arena, id, ctx->source);
            if (mname && mname[0]) {
                if (state->enclosing_class_qn) {
                    return pmm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, mname);
                }
                return pmm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, mname,
                                                   ctx->language);
            }
        }
    }
    ...
    if (ctx->language == PMM_LANG_DART && (strcmp(ts_node_type(node), "function_signature") == 0 ||
                                           strcmp(ts_node_type(node), "method_signature") == 0)) {
        TSNode sig = node;
        if (strcmp(ts_node_type(node), "method_signature") == 0) {
            TSNode fs = pmm_find_child_by_kind(node, "function_signature");
            if (!ts_node_is_null(fs)) {
                sig = fs;
            }
        }
        TSNode id = pmm_find_child_by_kind(sig, "identifier");
        if (!ts_node_is_null(id)) {
            char *nm = pmm_node_text(ctx->arena, id, ctx->source);
            if (nm && nm[0]) {
                if (state->enclosing_class_qn) {
                    return pmm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, nm);
                }
                return pmm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, nm,
                                                   ctx->language);
            }
        }
    }
    ...
    if (ctx->language == PMM_LANG_AGDA && strcmp(ts_node_type(node), "function") == 0) {
        TSNode lhs = pmm_find_child_by_kind(node, "lhs");
        if (!ts_node_is_null(lhs)) {
            TSNode nm = pmm_find_child_by_kind(lhs, "function_name");
            ...
            if (!ts_node_is_null(nm)) {
                char *name = pmm_node_text(ctx->arena, nm, ctx->source);
                if (name && name[0]) {
                    return pmm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path,
                                                       name, ctx->language);
                }
            }
        }
    ...
    char *name = pmm_func_name_node_text(ctx->arena, name_node, ctx->source);
    if (!name || !name[0]) {
        return NULL;
    }
    ...
    if ((ctx->language == PMM_LANG_CPP || ctx->language == PMM_LANG_CUDA) &&
        ...
        char *scope_name = pmm_cpp_out_of_line_parent_class(ctx->arena, node, ctx->source);
        if (scope_name && scope_name[0]) {
            const char *class_qn =
                pmm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, scope_name);
            return pmm_arena_sprintf(ctx->arena, "%s.%s", class_qn, name);
        }
    }

    if (state->enclosing_class_qn) {
        return pmm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, name);
    }
    ...
    return pmm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, name,
                                       ctx->language);
}
...
static const char *compute_class_qn(CBMExtractCtx *ctx, TSNode node, const WalkState *state) {
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_KOTLIN) {
    ...
    if (ts_node_is_null(name_node) && ctx->language == PMM_LANG_RUST &&
...
static void handle_string_constants(CBMExtractCtx *ctx, TSNode node, const WalkState *state) {
    ...
    char *name = pmm_node_text(ctx->arena, name_node, ctx->source);
    char *value = pmm_node_text(ctx->arena, value_node, ctx->source);
    ...
    if (vlen >= PMM_QUOTE_PAIR && (value[0] == '"' || value[0] == '\'')) {
        value = pmm_arena_strndup(ctx->arena, value + SKIP_ONE, (size_t)(vlen - PAIR_LEN));
        if (!value) {
            return;
        }
    }
    ...
    if (map->count < PMM_MAX_STRING_CONSTANTS) {
...
static void handle_string_refs(CBMExtractCtx *ctx, TSNode node, const WalkState *state) {
    ...
    char *text = pmm_node_text(ctx->arena, node, ctx->source);
    if (!text || !text[0]) {
        return;
    }
    ...
    /* Classify */
    int kind_val = pmm_classify_string(content, len);
    if (kind_val < 0) {
        return;
    }

    /* Build null-terminated content string in arena */
    char *val = pmm_arena_strndup(ctx->arena, content, (size_t)len);
    if (!val) {
        return;
    }
    ...
    pmm_stringref_push(&ctx->result->string_refs, ctx->arena, ref);
}
...
static void emit_yaml_leaf_value(CBMExtractCtx *ctx, TSNode val, const char *path) {
    char *val_text = pmm_node_text(ctx->arena, val, ctx->source);
    ...
    if (vlen >= PMM_QUOTE_PAIR && (val_text[0] == '"' || val_text[0] == '\'')) {
    ...
    int kind_val = pmm_classify_string(content, vlen);
    if (kind_val < 0) {
        return;
    }

    char *stored = pmm_arena_strndup(ctx->arena, content, (size_t)vlen);
    if (!stored) {
        return;
    }
...
#define YAML_WALK_STACK_CAP PMM_SZ_256
...
static void walk_yaml_mapping(CBMExtractCtx *ctx, TSNode root, const char *root_prefix) {
    ...
    while (top > 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            char *key_text = pmm_node_text(ctx->arena, key, ctx->source);
            if (!key_text || !key_text[0]) {
                continue;
            }
            const char *path =
                prefix ? pmm_arena_sprintf(ctx->arena, "%s.%s", prefix, key_text) : key_text;
...
static char *strip_yaml_quotes(CBMArena *a, char *v) {
    ...
    if (vlen >= PMM_QUOTE_PAIR && (v[0] == '"' || v[0] == '\'')) {
        return pmm_arena_strndup(a, v + SKIP_ONE, (size_t)(vlen - PAIR_LEN));
    }
    return v;
}
...
static void scan_nested_mapping_targets(CBMExtractCtx *ctx, TSNode val, const char **targets,
    ...
    for (uint32_t vi = 0; vi < vnc; vi++) {
        ...
        for (uint32_t mi = 0; mi < mnc; mi++) {
            ...
            char *mktext = pmm_node_text(ctx->arena, mk, ctx->source);
            if (mktext && is_target_key(mktext) && *n_targets < MAX_INFRA_BINDINGS) {
                char *mvtext =
                    strip_yaml_quotes(ctx->arena, pmm_node_text(ctx->arena, mv, ctx->source));
...
static void emit_infra_bindings(CBMExtractCtx *ctx, const char **sources, const char **source_keys,
    ...
    for (int si = 0; si < n_sources; si++) {
        for (int ti = 0; ti < n_targets; ti++) {
            ...
            pmm_infrabinding_push(&ctx->result->infra_bindings, ctx->arena, ib);
        }
    }
}
...
static void scan_mapping_for_bindings(CBMExtractCtx *ctx, TSNode mapping) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *k = pmm_node_text(ctx->arena, key, ctx->source);
        if (!k) {
            continue;
        }
        ...
        if (strcmp(vtype, "block_node") != 0 && strcmp(vtype, "block_mapping") != 0) {
            char *v = strip_yaml_quotes(ctx->arena, pmm_node_text(ctx->arena, val, ctx->source));
...
#define INFRA_SCAN_STACK_CAP PMM_SZ_512
...
static char *extract_hcl_string_val(CBMArena *a, TSNode val_node, const char *source) {
    ...
    for (int depth = 0; depth < HCL_MAX_UNWRAP; depth++) {
        ...
        if (strcmp(vk, "quoted_template") == 0 || strcmp(vk, "template_literal") == 0 ||
            ...
            char *val = pmm_node_text(a, val_node, source);
...
static TSNode hcl_block_body(TSNode block) {
    TSNode body = pmm_find_child_by_kind(block, "body");
...
static void scan_hcl_nested_block_targets(CBMExtractCtx *ctx, TSNode block, const char **targets,
    ...
    for (uint32_t bi = 0; bi < bnc; bi++) {
        ...
        char *bk = pmm_node_text(ctx->arena, bkey, ctx->source);
...
static const char *hcl_scheduler_source(CBMExtractCtx *ctx, TSNode block, const char **broker_out) {
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        char *label = pmm_node_text(ctx->arena, lit, ctx->source);
        if (!label || !label[0]) {
            continue;
        }
        if (!first_label) {
...
static void scan_infra_bindings(CBMExtractCtx *ctx, TSNode node) {
    if (ctx->language == PMM_LANG_YAML || ctx->language == PMM_LANG_JSON) {
        const char *nk = ts_node_type(node);
    ...
    } else if (ctx->language == PMM_LANG_HCL) {
...
static void push_boundary_scopes(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec,
    ...
    if (spec->function_node_types && pmm_kind_in_set(node, spec->function_node_types)) {
        ...
        if (ctx->language == PMM_LANG_OCAML) {
            for (int i = 0; i < state->scope_top; i++) {
        ...
        if (!skip_nested) {
            ...
            if (fqn) {
                ...
                if (ctx->language == PMM_LANG_OBJECTSCRIPT_UDL ||
                    ...
                    TSNode args_node = pmm_find_child_by_kind(mdef, "arguments");
                        ...
                        for (uint32_t ai = 0; ai < ts_node_named_child_count(args_node); ai++) {
                            ...
                            for (uint32_t pi = 0; pi < ts_node_named_child_count(arg); pi++) {
                                ...
                                if (strcmp(pk, "method_arg") == 0) {
                                    ...
                                    type_node = pmm_find_child_by_kind(pchild, "typename");
                                }
                            }
                            if (!ts_node_is_null(param_name_node) && !ts_node_is_null(type_node)) {
                                TSNode lvn = pmm_find_child_by_kind(param_name_node, "expr_atom");
                                if (ts_node_is_null(lvn)) {
                                    lvn = param_name_node;
                                }
                                char *pname = pmm_node_text(ctx->arena, lvn, ctx->source);
                                char *ptype = pmm_node_text(ctx->arena, type_node, ctx->source);
                                if (pname && pname[0] && ptype && ptype[0]) {
    ...
    } else if (spec->class_node_types && pmm_kind_in_set(node, spec->class_node_types)) {
    ...
    } else if (ctx->language == PMM_LANG_RUST && strcmp(ts_node_type(node), "impl_item") == 0) {
        TSNode type_node = ts_node_child_by_field_name(node, TS_FIELD("type"));
        if (!ts_node_is_null(type_node)) {
            char *type_name = pmm_node_text(ctx->arena, type_node, ctx->source);
            if (type_name && type_name[0]) {
                const char *tqn =
                    pmm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, type_name);
                push_scope(state, SCOPE_CLASS, depth, tqn);
            }
        }
    } else if (ctx->language == PMM_LANG_DART && strcmp(ts_node_type(node), "function_body") == 0) {
    ...
    if (spec->call_node_types && pmm_kind_in_set(node, spec->call_node_types)) {
        push_scope(state, SCOPE_CALL, depth, NULL);
    }
    if (spec->import_node_types && pmm_kind_in_set(node, spec->import_node_types) &&
    ...
    if (ts_node_is_named(node) && pmm_is_loop_node_type(ts_node_type(node))) {
        push_scope(state, SCOPE_LOOP, depth, NULL);
    } else if (spec->branching_node_types && pmm_kind_in_set(node, spec->branching_node_types)) {
