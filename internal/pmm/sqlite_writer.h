#ifndef PMM_SQLITE_WRITER_H
#define PMM_SQLITE_WRITER_H
...
// indexes, sqlite_master) is written at finalize. pmm_write_db() above is a
// one-shot wrapper over this API (open -> append all nodes -> finalize) and
// produces byte-identical output.
//
// Usage: w = pmm_writer_open(path);
//        pmm_writer_append_nodes(w, batch, n) x N  (ascending, contiguous ids);
//        pmm_writer_finalize(w, ...);   // consumes + frees w, closes the file.
typedef struct pmm_db_writer pmm_db_writer_t;
