#ifndef PMM_PREPROCESSOR_H
#define PMM_PREPROCESSOR_H
...
// The returned string must be freed with pmm_preprocess_free().
char *pmm_preprocess(const char *source, int source_len, const char *filename,
...
// Free with pmm_preprocessed_source_free().
CBMPreprocessedSource *pmm_preprocess_with_map(const char *source, int source_len,
                                               const char *filename, const char **extra_defines,
                                               const char **include_paths, int cpp_mode);

// Free preprocessed source returned by pmm_preprocess.
void pmm_preprocess_free(char *expanded);

// Free preprocessed source and line maps returned by pmm_preprocess_with_map.
void pmm_preprocessed_source_free(CBMPreprocessedSource *pp);

#ifdef __cplusplus
}
#endif

#endif // PMM_PREPROCESSOR_H
