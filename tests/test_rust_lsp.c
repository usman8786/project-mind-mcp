 ...
 *   - Cross-file: pmm_run_rust_lsp_cross resolves project + stdlib defs
 *
...
static CBMFileResult *extract_rust(const char *source) {
    return pmm_extract_file(source, (int)strlen(source), PMM_LANG_RUST,
...
TEST(rustlsp_crossfile_method_dispatch) {
    ...
    pmm_run_rust_lsp_cross(&a, caller, (int)strlen(caller),
                           "test.caller",
                           defs, 2,
                           imp_names, imp_qns, 1,
                           NULL, &out);
...
/* F4 Tier-2: the build-once shared registry path (pmm_rust_build_cross_registry +
 * pmm_run_rust_lsp_cross_with_registry) must resolve IDENTICALLY to the per-file
 * pmm_run_rust_lsp_cross — same fixture as rustlsp_crossfile_method_dispatch. Also
...
TEST(rustlsp_shared_registry_resolves_like_per_file) {
    ...
    CBMTypeRegistry *reg = pmm_rust_build_cross_registry(&a, defs, 2);
    ASSERT_NOT_NULL(reg);
    ASSERT_TRUE(reg->read_only);

    CBMResolvedCallArray out;
    memset(&out, 0, sizeof(out));
    pmm_run_rust_lsp_cross_with_registry(&a, caller, (int)strlen(caller), "test.caller", reg,
...
TEST(rustlsp_crossfile_free_function) {
    ...
    pmm_run_rust_lsp_cross(&a, caller, (int)strlen(caller), "test.main",
...
TEST(rustlsp_cov_xf_two_methods) {
    ...
    pmm_run_rust_lsp_cross(&a, src, (int)strlen(src), "p.caller", defs, 3, imp_n, imp_q, 1, NULL, &out);
    ASSERT_GTE(find_confident(&out, "run", "Thing.alpha"), 0);
    ASSERT_GTE(find_confident(&out, "run", "Thing.beta"), 0);
    pmm_arena_destroy(&a); PASS();
}
...
TEST(rustlsp_cov_xf_free_function_chain) {
    ...
    CBMResolvedCallArray out; memset(&out, 0, sizeof(out));
    pmm_run_rust_lsp_cross(&a, src, (int)strlen(src), "p.caller", defs, 1, imp_n, imp_q, 1, NULL, &out);
...
TEST(rustlsp_cov_xf_batch) {
    ...
    pmm_batch_rust_lsp_cross(&a, files, 2, out);
    pmm_arena_destroy(&a); PASS();
}
TEST(rustlsp_cov_xf_empty_defs) {
    ...
    CBMArena a; pmm_arena_init(&a);
    CBMResolvedCallArray out; memset(&out, 0, sizeof(out));
    pmm_run_rust_lsp_cross(&a, src, (int)strlen(src), "p.caller", NULL, 0, NULL, NULL, 0, NULL, &out);
    pmm_arena_destroy(&a); PASS();
}
TEST(rustlsp_cov_xf_nested_modules) {
    ...
    CBMResolvedCallArray out; memset(&out, 0, sizeof(out));
    pmm_run_rust_lsp_cross(&a, src, (int)strlen(src), "p.caller", defs, 1, NULL, NULL, 0, NULL, &out);
    pmm_arena_destroy(&a); PASS();
}
...
TEST(rustlsp_partial_cargo_parses_simple) {
    ...
    const char *toml =
        ...
        "local = { path = \"../local\" }\n";
    CBMCargoManifest m;
    pmm_cargo_parse(&a, toml, (int)strlen(toml), &m);
    ...
    ASSERT_EQ(true, pmm_cargo_is_known_dep(&m, "serde"));
    ASSERT_EQ(true, pmm_cargo_is_known_dep(&m, "tokio"));
    ASSERT_EQ(true, pmm_cargo_is_known_dep(&m, "local"));
    ASSERT_EQ(false, pmm_cargo_is_known_dep(&m, "unknown_crate"));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(rustlsp_partial_cargo_parses_workspace) {
    ...
    const CBMCargoMember *a_mem = pmm_cargo_find_member(&m, "a");
    ASSERT_NOT_NULL(a_mem);
    ASSERT_EQ(0, strcmp(a_mem->member_path, "crates/a"));
    const CBMCargoMember *shared = pmm_cargo_find_member(&m, "shared");
...
TEST(rustlsp_partial_cargo_handles_comments_and_quirks) {
    ...
    ASSERT_GTE(m.dep_count, 3);
    ASSERT_EQ(true, pmm_cargo_is_known_dep(&m, "regex"));
    ASSERT_EQ(true, pmm_cargo_is_known_dep(&m, "log"));
    ASSERT_EQ(true, pmm_cargo_is_known_dep(&m, "anyhow"));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(rustlsp_extra_cargo_wires_workspace_member) {
    ...
    CBMFileResult* r = pmm_extract_file(src, (int)strlen(src),
        PMM_LANG_RUST, "test", "src/main.rs", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);

    /* Manually re-run with manifest. */
    CBMArena scratch; pmm_arena_init(&scratch);
    /* Discard prior resolved calls. */
    memset(&r->resolved_calls, 0, sizeof(r->resolved_calls));
    pmm_run_rust_lsp_with_manifest(&scratch, r, src, (int)strlen(src),
...
TEST(rustlsp_extra_rustdoc_ingest_basic_function) {
    ...
    const char* json =
        ...
        "  }\n"
        "}\n";
    int n = pmm_rust_rustdoc_ingest(&reg, &a, json, (int)strlen(json), "my_crate");
    ASSERT_GTE(n, 1);
    /* hello should be findable. */
    const CBMRegisteredFunc* f = pmm_registry_lookup_func(&reg, "my_crate.hello");
    ASSERT_NOT_NULL(f);
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(rustlsp_extra_rustdoc_ingest_struct_and_trait) {
    ...
    int n = pmm_rust_rustdoc_ingest(&reg, &a, json, (int)strlen(json), "my_crate");
    ASSERT_GTE(n, 2);
    const CBMRegisteredType* foo = pmm_registry_lookup_type(&reg, "my_crate.Foo");
    ASSERT_NOT_NULL(foo);
    const CBMRegisteredType* bar = pmm_registry_lookup_type(&reg, "my_crate.Bar");
    ASSERT_NOT_NULL(bar);
...
TEST(rustlsp_extra_rustdoc_handles_malformed) {
    ...
    int n = pmm_rust_rustdoc_ingest(&reg, &a, "{}", 2, "x");
    ASSERT_EQ(0, n);
    /* Truly broken — also should not crash. */
    int n2 = pmm_rust_rustdoc_ingest(&reg, &a, "not json", 8, "x");
    ASSERT_EQ(0, n2);
    pmm_arena_destroy(&a);
    PASS();
}
