...
static CBMFileResult *extract_kotlin(const char *source) {
    return pmm_extract_file(source, (int)strlen(source), PMM_LANG_KOTLIN, "test", "Main.kt", 0,
                            NULL, NULL);
}

static CBMFileResult *extract_kotlin_path(const char *source, const char *rel_path) {
    return pmm_extract_file(source, (int)strlen(source), PMM_LANG_KOTLIN, "test", rel_path, 0,
                            NULL, NULL);
}
