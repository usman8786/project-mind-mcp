...
FIXTURE="$(mktemp -d "${TMPDIR:-/tmp}/pmm-vm-manifest-test.XXXXXX")"
manifest="$(mktemp "${TMPDIR:-/tmp}/pmm-vm-manifest-output.XXXXXX")"
...
pmm_vm_write_untracked_manifest "$FIXTURE" "$manifest"
...
for entrypoint in "$driver" "$provisioner"; do
    ...
    if ! grep -Fq 'pmm_vm_require_safe_branch "$BRANCH"' "$entrypoint"; then
...
if ! grep -Fq 'pmm_vm_sync_windows_clock()' \
...
clock_helper=$(sed -n '/^pmm_vm_sync_windows_clock() {$/,/^}$/p' \
...
for entrypoint in "$driver" "$provisioner"; do
    if ! grep -Fq 'pmm_vm_sync_windows_clock "${SSH[@]}"' "$entrypoint"; then
        echo "FAIL: Windows VM entry point must repair suspended-guest clock drift: $entrypoint" >&2
        exit 1
    fi
done
if ! grep -Fq 'pmm_vm_run_powershell()' \
    "$ROOT/test-infrastructure/vm/ssh-common.sh" ||
...
if ! grep -Fq 'scripts/build.sh --with-ui CC=clang CXX=clang++ SANITIZE= BUILD_DIR=build/guards' \
    ...
    <<<"$guards_block" ||
    ! grep -Fq -- '-GuardsOnly -Binary build\\guards\\project-mind-mcp.exe' <<<"$guards_block"; then
...
if ! grep -Fq 'PMM_VM_BRANCH' "$ROOT/test-infrastructure/vm/README.md"; then
