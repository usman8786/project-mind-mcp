...
static pmm_store_t *pb_open_indexed(ProbeLangProj *lp) {
...
static pmm_store_t *pb_index_files(ProbeLangProj *lp, const ProbeLangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_pb_XXXXXX");
...
static pmm_store_t *pb_index(ProbeLangProj *lp, const char *filename, const char *content) {
    ProbeLangFile f = {filename, content};
    return pb_index_files(lp, &f, 1);
}

static void pb_cleanup(ProbeLangProj *lp, pmm_store_t *store) {
...
static int pb_count_label(pmm_store_t *store, const char *project, const char *label) {
...
static int pb_callable_nodes(pmm_store_t *store, const char *project) {
...
static int pb_type_nodes(pmm_store_t *store, const char *project) {
...
TEST(glsl_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "shader.glsl",
...
TEST(glsl_struct_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "light.glsl",
        "struct Light {\n"
        "    vec3 position;\n"
        "    vec3 color;\n"
        "    float intensity;\n"
...
TEST(glsl_vertex_shader_functions) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "vert.glsl",
...
TEST(hare_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "math.ha",
...
TEST(hare_struct_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "point.ha",
        "type point = struct {\n"
        "    x: i64,\n"
        "    y: i64,\n"
        "};\n\n"
 ...
 * fixture cannot trigger — so we probe at extraction level via pmm_extract_file. */
TEST(hare_use_import_extracted) {
    static const char *src =
        ...
        "    fmt::println(name);\n"
        "};\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_HARE, "lc", "greet.ha", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
...
TEST(hlsl_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "pixel.hlsl",
...
TEST(hlsl_struct_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "types.hlsl",
        "struct VSInput {\n"
...
TEST(hlsl_compute_shader_function) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "compute.hlsl",
...
TEST(ispc_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "kernel.ispc",
...
TEST(ispc_struct_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "types.ispc",
        "struct Vec3 {\n"
        "    float x, y, z;\n"
        "};\n\n"
...
TEST(ispc_task_function) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "tasks.ispc",
...
TEST(julia_function_and_struct_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "geom.jl",
        "struct Point\n"
        "    x::Float64\n"
        "    y::Float64\n"
        "end\n\n"
...
TEST(julia_using_import_extracted) {
    static const char *src =
        ...
        "end\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_JULIA, "lc", "vec.jl", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int n = r->imports.count;
    pmm_free_result(r);
...
TEST(julia_import_extracted) {
    ...
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_JULIA, "lc", "myvec.jl", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int n = r->imports.count;
    pmm_free_result(r);
...
TEST(julia_abstract_subtype_extracted) {
    static const char *src =
        ...
        "end\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_JULIA, "lc", "shapes.jl", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
...
TEST(julia_inherits_edge) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "animals.jl",
        ...
        "    return \"Woof: \" * d.name\n"
        "end\n");
    int inherits = store ? pmm_store_count_edges_by_type(store, lp.project, "INHERITS") : -1;
    pb_cleanup(&lp, store);
...
TEST(luau_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "utils.luau",
...
TEST(luau_type_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "types.luau",
        "type Vector2 = {\n"
        "    x: number,\n"
        "    y: number,\n"
        "}\n\n"
...
TEST(luau_class_style_functions) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "player.luau",
        "local Player = {}\n"
...
TEST(matlab_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "stats.m",
...
TEST(matlab_multiple_functions) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "linalg.m",
...
TEST(odin_proc_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "math.odin",
        "package math\n\n"
...
TEST(odin_struct_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "point.odin",
        "package point\n\n"
        "Vec2 :: struct {\n"
        "    x, y: f64,\n"
        "}\n\n"
...
TEST(odin_import_extracted) {
    static const char *src =
        ...
        "}\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_ODIN, "lc", "main.odin", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int n = r->imports.count;
    pmm_free_result(r);
...
TEST(pascal_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "arith.pas",
        "unit Arith;\n\n"
        "interface\n\n"
...
TEST(pascal_record_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "geom.pas",
        "unit Geom;\n\n"
        "interface\n\n"
        "type\n"
        "  TPoint = record\n"
...
TEST(pascal_uses_import_extracted) {
    static const char *src =
        ...
        "end.\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_PASCAL, "lc", "myunit.pas", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int n = r->imports.count;
    pmm_free_result(r);
...
TEST(pascal_class_inheritance_extracted) {
    static const char *src =
        ...
        "end.\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_PASCAL, "lc", "zoo.pas", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int found_base = 0;
    for (int i = 0; i < r->defs.count; i++) {
        if (r->defs.items[i].base_classes && r->defs.items[i].base_classes[0] != NULL) {
...
TEST(pascal_inherits_edge) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "vehicles.pas",
        "unit Vehicles;\n\n"
        "interface\n\n"
        "type\n"
        "  TVehicle = class\n"
...
TEST(powershell_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "utils.ps1",
        "function Get-Square {\n"
        "    param([double]$x)\n"
        "    return $x * $x\n"
        "}\n\n"
...
TEST(powershell_class_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "shapes.ps1",
        "class Shape {\n"
        "    [string]$Name\n"
...
TEST(powershell_using_import_extracted) {
    static const char *src =
        ...
        "}\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_POWERSHELL, "lc", "task.ps1", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int n = r->imports.count;
    pmm_free_result(r);
...
TEST(powershell_class_inheritance_extracted) {
    static const char *src =
        ...
        "}\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_POWERSHELL, "lc", "animals.ps1", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int found_base = 0;
    for (int i = 0; i < r->defs.count; i++) {
        if (r->defs.items[i].base_classes && r->defs.items[i].base_classes[0] != NULL) {
...
TEST(powershell_inherits_edge) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "exceptions.ps1",
        "class AppException {\n"
        "    [string]$Message\n"
...
TEST(racket_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "math.rkt",
        "#lang racket\n\n"
        "(define (square x) (* x x))\n\n"
...
TEST(racket_struct_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "point.rkt",
        "#lang racket\n\n"
...
TEST(racket_require_import_extracted) {
    static const char *src =
        ...
        "  (string-join words \" \"))\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_RACKET, "lc", "utils.rkt", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int n = r->imports.count;
    pmm_free_result(r);
...
TEST(racket_imports_edge) {
    ...
    ProbeLangProj lp;
    pmm_store_t *store = pb_index_files(&lp, files, 2);
    int imports = store ? pmm_store_count_edges_by_type(store, lp.project, "IMPORTS") : -1;
    pb_cleanup(&lp, store);
...
TEST(rescript_function_nodes) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "math.res",
...
TEST(rescript_type_node) {
    ProbeLangProj lp;
    pmm_store_t *store = pb_index(&lp, "types.res",
...
TEST(rescript_open_import_extracted) {
    ...
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src),
                                        PMM_LANG_RESCRIPT, "lc", "utils.res", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    int n = r->imports.count;
    pmm_free_result(r);
