...
for relative, source in (
    ...
    for variable in (
        ...
        "GLAB_CONFIG_DIR",
        "KIMI_CODE_HOME",
        "PMM_CONTINUE_CONFIG_PATH",
        "PMM_TRAE_CONFIG_PATH",
        "PMM_ROO_CONFIG_PATH",
        "PMM_CODY_CONFIG_PATH",
        "PMM_TEST_WINDOWS_USER_PATH_RUN_ID",
    ):
        require(
...
require(
    "project-mind-mcp${SUFFIX}-${OS}-${ARCH}.tar.gz" in smoke_local
    and "project-mind-mcp-${OS}-${ARCH}.tar.gz" in smoke_local,
    "smoke-local.sh must create the selected variant and standard alias",
)
require(
    "project-mind-mcp${SUFFIX}-${OS}-${ARCH}-portable.tar.gz" in smoke_local
    and "project-mind-mcp-${OS}-${ARCH}-portable.tar.gz" in smoke_local,
    "smoke-local.sh must create Linux portable selected-variant and standard aliases",
)
require(
    'PMM_CACHE_DIR="$WORK_DIR/cache"' in smoke_local
...
require(
    "project-mind-mcp.payload.exe" not in vm_smoke,
...
require(
    ...
    and 'PMM_CACHE_DIR="$(cygpath -m "$SMOKE_DIR/cache")"' in vm_smoke,
...
require(
    'PMM_TEST_WINDOWS_USER_PATH_RUN_ID="$PATH_RUN_ID"' in vm_smoke
...
require(
    "PMM_TEST_WINDOWS_USER_PATH_RUN_ID" in cli_source
...
for service in ("smoke-windows:",):
    ...
    section = match.group("body") if match else ""
    require(
        "project-mind-mcp-launcher" not in section
        and "project-mind-mcp.payload.exe" not in section.replace(
            "test ! -e build/win-cross/project-mind-mcp.payload.exe", ""
        ),
        f"docker-compose {service[:-1]} must build ONE Windows binary, not a launcher/payload pair",
    )
    require(
        "test ! -e build/win-cross/project-mind-mcp.payload.exe" in section,
        f"docker-compose {service[:-1]} must assert no payload sibling is produced",
    )
require(
    "wine64 ./build/win-cross/project-mind-mcp.exe --version" in compose
    and "wine64 cmd /c build/win-cross/project-mind-mcp.exe --version" in compose,
...
require(
    'UPDATE_DRIVER="$UPDATE_HOME/.local/bin/project-mind-mcp"' in smoke_test
...
if helper.is_file():
    with tempfile.TemporaryDirectory(prefix="pmm-fixture-contract-") as temp:
...
if helper.is_file():
    with tempfile.TemporaryDirectory(prefix="pmm-fixture-dns-") as dns_temp:
