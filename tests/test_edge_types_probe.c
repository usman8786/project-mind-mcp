...
static pmm_store_t *et_index_files(EtProj *lp, const EtFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_et_XXXXXX");
...
static void et_cleanup(EtProj *lp, pmm_store_t *store) {
...
static int et_edge_present(const EtFile *files, int nfiles, const char *edge, int floor) {
    EtProj lp;
    pmm_store_t *store = et_index_files(&lp, files, nfiles);
...
static int et_routes_exact(const EtFile *files, int nfiles, const char **routes) {
    ...
    if (!store || pmm_store_find_nodes_by_label(store, lp.project, "Route", &nodes, &node_count) !=
                      PMM_STORE_OK) {
        ok = 0;
    } else {
...
static pmm_store_t *et_index_parallel(EtProj *lp, const EtFile *meaningful, int n_mean) {
...
static int et_calls_to_name_parallel(const EtFile *meaningful, int n_mean, const char *name) {
    EtProj lp;
    pmm_store_t *store = et_index_parallel(&lp, meaningful, n_mean);
    int hits = 0;
    if (store) {
        pmm_edge_t *edges = NULL;
        int n = 0;
        if (pmm_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &n) == PMM_STORE_OK) {
            for (int i = 0; i < n; i++) {
                pmm_node_t tgt;
 ...
 *  resolver (pass_calls.c → pmm_service_pattern_match → PMM_SVC_ROUTE_REG) fires.
 ...
 * fix, GREEN with it). Direction-agnostic count via pmm_store_count_edges_by_type. */
 ...
 * sequential resolver classifies the call as PMM_SVC_ROUTE_REG and emits
 ...
 * pmm_service_pattern_match as ROUTE_REG or the handler-arg (an identifier
 ...
 * REAL BUG: internal/pmm/extract_calls.c:extract_handler_arg only accepts an
 ...
 * routed through pmm_service_pattern_match for HTTP classification (the C# lsp
 ...
 * "Client.get" — it drops the namespace/path, so pmm_service_pattern_match never
 ...
 * resolve a `::`-qualified cross-file path (pmm_registry_resolve splits on '.',
 ...
 *     ("aws-sdk-go/service/sqs"), but pmm_fqn_compute (internal/pmm/helpers.c)
...
TEST(throws_java) {
    ...
    int throws = store ? pmm_store_count_edges_by_type(store, lp.project, "THROWS") : -1;
    if (throws < 1) {
 ...
 * internal/pmm/extract_semantic.c:resolve_exception_name does not recover the
...
TEST(raises_python) {
    ...
    pmm_store_t *store =
        et_index_parallel(&lp, meaningful, (int)(sizeof(meaningful) / sizeof(meaningful[0])));
    int raises = store ? pmm_store_count_edges_by_type(store, lp.project, "RAISES") : -1;
    if (raises < 1) {
 ...
 * internal/pmm/extract_semantic.c:resolve_exception_name fails to extract the
 ...
 *    Variable nodes ARE created (pmm_gbuf_upsert_node, "Variable" label) but are
 *    NEVER added to the resolver registry — both register_and_link_def()
 *    (src/pipeline/pass_parallel.c:781) and process_def()
 *    (src/pipeline/pass_definitions.c:262) only pmm_registry_add() the labels
 *    Function/Method/Class/Interface.  resolve_file_rw() resolves the written
 *    var via pmm_registry_resolve(var_name) → always empty → no target node →
...
TEST(writes_python) {
    ...
    pmm_store_t *store =
        et_index_parallel(&lp, meaningful, (int)(sizeof(meaningful) / sizeof(meaningful[0])));
    int writes = store ? pmm_store_count_edges_by_type(store, lp.project, "WRITES") : -1;
    if (writes < 1) {
 ...
 * (internal/pmm/extract_defs.c:2042-2047) but def.parent_class is NEVER derived
 ...
 *  Produced by pass_semantic.c:pmm_pipeline_implements_go().
 *  Parallel-path only (> 50 files).
 *
 *  REAL BUG (same root cause as defines_method_go): pmm_pipeline_implements_go
...
TEST(override_go_interface) {
    ...
    pmm_store_t *store =
        et_index_parallel(&lp, meaningful, (int)(sizeof(meaningful) / sizeof(meaningful[0])));
    int override_edges = store ? pmm_store_count_edges_by_type(store, lp.project, "OVERRIDE") : -1;
    int implements     = store ? pmm_store_count_edges_by_type(store, lp.project, "IMPLEMENTS") : -1;
