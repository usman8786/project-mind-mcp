 ...
 *
 * Covers the in-memory resolver (pmm_path_alias_resolve), the
 * directory-scoped collection lookup (pmm_path_alias_find_for_file),
...
static pmm_path_alias_map_t *make_map(const char *base_url, int count, ...) {
    pmm_path_alias_map_t *map = calloc(1, sizeof(*map));
    map->base_url = base_url ? strdup(base_url) : NULL;
    map->entries = calloc((size_t)count, sizeof(pmm_path_alias_t));
    map->count = count;
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (tstar) {
            map->entries[i].target_prefix =
                pmm_strndup(target_pattern, (size_t)(tstar - target_pattern));
    ...
    for (int i = 0; i < count - 1; i++) {
        for (int j = i + 1; j < count; j++) {
            ...
            if (lj > li) {
                pmm_path_alias_t tmp = map->entries[i];
...
static void free_map(pmm_path_alias_map_t *map) {
    if (!map) {
        return;
    }
...
TEST(path_alias_at_wildcard) {
    pmm_path_alias_map_t *m = make_map(NULL, 1, "@/*", "src/*");
    char *r = pmm_path_alias_resolve(m, "@/lib/auth");
    ASSERT_NOT_NULL(r);
...
TEST(path_alias_at_nested) {
    pmm_path_alias_map_t *m = make_map(NULL, 1, "@/*", "src/*");
    char *r = pmm_path_alias_resolve(m, "@/components/Button");
    ASSERT_NOT_NULL(r);
...
TEST(path_alias_specificity_longest_first) {
    ...
    char *r2 = pmm_path_alias_resolve(m, "@/components/X");
    ASSERT_NOT_NULL(r2);
...
TEST(path_alias_exact_match) {
    pmm_path_alias_map_t *m = make_map(NULL, 1, "@app/config", "src/config/index");
    char *r = pmm_path_alias_resolve(m, "@app/config");
    ASSERT_NOT_NULL(r);
    ASSERT_STR_EQ(r, "src/config/index");
    free(r);
    /* Anything else under @app/ should not match. */
    char *miss = pmm_path_alias_resolve(m, "@app/other");
    ASSERT_NULL(miss);
    free_map(m);
    PASS();
}
...
TEST(path_alias_strips_ext) {
    pmm_path_alias_map_t *m = make_map(NULL, 1, "@/*", "src/*.ts");
    char *r = pmm_path_alias_resolve(m, "@/lib/auth");
    ASSERT_NOT_NULL(r);
    ASSERT_STR_EQ(r, "src/lib/auth");
    free(r);
...
TEST(path_alias_baseurl_fallback) {
    pmm_path_alias_map_t *m = make_map("src", 0);
    ...
    char *miss = pmm_path_alias_resolve(m, "react");
    ASSERT_NULL(miss);
    free_map(m);
    PASS();
}
...
TEST(path_alias_null_safety) {
    ASSERT_NULL(pmm_path_alias_resolve(NULL, "anything"));
    pmm_path_alias_map_t *m = make_map(NULL, 1, "@/*", "src/*");
    ASSERT_NULL(pmm_path_alias_resolve(m, NULL));
    free_map(m);
    /* Free of NULL collection is a no-op, not a crash. */
    pmm_path_alias_collection_free(NULL);
    PASS();
}
...
TEST(path_alias_find_for_file_nearest_ancestor) {
    ...
    coll->scopes = calloc(2, sizeof(pmm_path_alias_scope_t));
    coll->count = 2;
    ...
    const pmm_path_alias_map_t *m1 =
        pmm_path_alias_find_for_file(coll, "apps/manager/src/lib/auth.ts");
    ASSERT_NOT_NULL(m1);
    ASSERT_EQ(m1->count, 1);
    ASSERT_STR_EQ(m1->entries[0].alias_prefix, "@/");

    const pmm_path_alias_map_t *m2 =
        pmm_path_alias_find_for_file(coll, "packages/utils/index.ts");
    ASSERT_NOT_NULL(m2);
    ASSERT_EQ(m2->count, 1);
    ASSERT_STR_EQ(m2->entries[0].alias_prefix, "@root/");

    pmm_path_alias_collection_free(coll);
    PASS();
}
...
TEST(path_alias_loader_monorepo) {
    char tmpl[256];
    snprintf(tmpl, sizeof(tmpl), "/tmp/pmm_palias_XXXXXX");
    char *root = pmm_mkdtemp(tmpl);
    ...
    pmm_path_alias_collection_t *coll = pmm_load_path_aliases(root);
    ASSERT_NOT_NULL(coll);
    ASSERT_EQ(coll->count, 2);

    /* sub-package file picks up its own tsconfig. */
    const pmm_path_alias_map_t *m =
        pmm_path_alias_find_for_file(coll, "apps/manager/src/feature/x.ts");
    ASSERT_NOT_NULL(m);
    char *r = pmm_path_alias_resolve(m, "@/lib/auth");
    ASSERT_NOT_NULL(r);
    ...
    const pmm_path_alias_map_t *m2 = pmm_path_alias_find_for_file(coll, "scripts/build.ts");
    ASSERT_NOT_NULL(m2);
    char *r2 = pmm_path_alias_resolve(m2, "@root/utils");
    ASSERT_NOT_NULL(r2);
...
TEST(path_alias_loader_monorepo_dotdot_climb) {
    char tmpl[256];
    snprintf(tmpl, sizeof(tmpl), "/tmp/pmm_palias_climb_XXXXXX");
    char *root = pmm_mkdtemp(tmpl);
    ASSERT_NOT_NULL(root);
    ...
    const pmm_path_alias_map_t *m =
        pmm_path_alias_find_for_file(coll, "apps/web/src/feature/x.ts");
    ASSERT_NOT_NULL(m);
    char *r = pmm_path_alias_resolve(m, "@shared/utils");
    ASSERT_NOT_NULL(r);
...
TEST(path_alias_loader_honors_discovery_exclusions) {
    char tmpl[256];
    snprintf(tmpl, sizeof(tmpl), "/tmp/pmm_palias_excl_XXXXXX");
    char *root = pmm_mkdtemp(tmpl);
    ASSERT_NOT_NULL(root);
    ...
    coll = pmm_load_path_aliases_excluded(root, excluded, 1);
    ASSERT_NOT_NULL(coll);
    ASSERT_EQ(coll->count, 1);
    const pmm_path_alias_map_t *m = pmm_path_alias_find_for_file(coll, "src/x.ts");
    ASSERT_NOT_NULL(m);
    char *r = pmm_path_alias_resolve(m, "@root/utils");
    ASSERT_NOT_NULL(r);
...
TEST(path_alias_loader_no_configs) {
    char tmpl[256];
    snprintf(tmpl, sizeof(tmpl), "/tmp/pmm_palias_empty_XXXXXX");
    char *root = pmm_mkdtemp(tmpl);
    ASSERT_NOT_NULL(root);
