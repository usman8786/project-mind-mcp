 ...
 * pmm_extract_file_impl never sets them.
 *
 ...
 *   GREEN (fixed):  pmm_extract_file sets parse_incomplete=true iff the tree
...
static CBMFileResult *do_extract(const char *src, CBMLanguage lang, const char *path) {
    return pmm_extract_file(src, (int)strlen(src), lang, "covproj", path, 0, NULL, NULL);
}
...
TEST(c_ifdef_split_brace_sets_parse_incomplete) {
    CBMFileResult *r = do_extract(C_IFDEF_SPLIT, PMM_LANG_C, "split.c");
    ASSERT_NOT_NULL(r);
...
TEST(c_clean_file_not_flagged) {
    ...
    CBMFileResult *r = do_extract(C_CLEAN, PMM_LANG_C, "clean.c");
    ASSERT_NOT_NULL(r);
...
TEST(py_unrecovered_garbage_sets_parse_incomplete) {
    CBMFileResult *r = do_extract(PY_GARBAGE, PMM_LANG_PYTHON, "garbage.py");
    ASSERT_NOT_NULL(r);
    ASSERT_TRUE(r->parse_incomplete);
    ASSERT_GTE(r->error_region_count, 1);
    ASSERT_NOT_NULL(r->error_ranges);
...
TEST(py_recovered_def_not_flagged) {
    ...
    CBMFileResult *r = do_extract(PY_BROKEN_RECOVERED, PMM_LANG_PYTHON, "broken.py");
    ASSERT_NOT_NULL(r);
...
TEST(py_clean_file_not_flagged) {
    CBMFileResult *r = do_extract(PY_CLEAN, PMM_LANG_PYTHON, "clean.py");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->parse_incomplete);
    ASSERT_EQ(r->error_region_count, 0);
    ASSERT_NULL(r->error_ranges);
...
TEST(error_region_cap_is_honored) {
     ...
     * 64-region cap (matches PMM_MAX_ERROR_REGIONS in pmm.c) — pathological
    ...
    CBMFileResult *r = do_extract(src, PMM_LANG_PYTHON, "many_errors.py");
    free(src);
    ASSERT_NOT_NULL(r);
    ASSERT_TRUE(r->parse_incomplete);
    ASSERT_GTE(r->error_region_count, 1);
...
TEST(c_trailing_recovered_defs_keep_flag) {
    ...
    CBMFileResult *r = do_extract(src, PMM_LANG_C, "probe.c");
    ASSERT_NOT_NULL(r);
