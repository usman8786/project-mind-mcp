...
# a LINEAR scan over the whole project's registered funcs (pmm_registry_lookup_
...
#   PMM_HANG_TIMEOUT=60 PMM_HANG_CLASSES=6000 bash tests/test_cpp_index_hang.sh

set -uo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BIN="$ROOT/build/c/project-mind-mcp"
...
WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"; rm -f "$HOME/.cache/project-mind-mcp/$PROJ.db"* 2>/dev/null' EXIT
...
# type go through pmm_registry_lookup_method_by_types — the O(project_funcs) scan.
{
    echo '#pragma once'
...
rm -f "$HOME/.cache/project-mind-mcp/$PROJ.db"* 2>/dev/null
