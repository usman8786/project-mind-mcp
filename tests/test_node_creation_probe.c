 ...
 *
 * How languages were confirmed supported: each uses a PMM_LANG_* constant
 * from pmm.h (the canonical enum); the label-golden table in
...
static pmm_store_t *ncp_open_indexed(NcpLangProj *lp) {
...
static pmm_store_t *ncp_index_files(NcpLangProj *lp, const NcpLangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_ncp_XXXXXX");
...
static void ncp_cleanup(NcpLangProj *lp, pmm_store_t *store) {
    if (store)
...
static int ncp_count_label(pmm_store_t *store, const char *project, const char *label) {
...
static int ncp_type_nodes(pmm_store_t *store, const char *project) {
...
static int ncp_callables_with_outbound(pmm_store_t *store, const char *project) {
...
static NcpMetrics ncp_metrics_files(const NcpLangFile *files, int nfiles) {
    NcpLangProj lp;
    pmm_store_t *store = ncp_index_files(&lp, files, nfiles);
    NcpMetrics m = {0};
    if (store) {
        m.ok = 1;
