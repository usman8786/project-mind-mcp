...
static pmm_store_t *mkc_open_indexed(MKC_Proj *lp) {
...
static pmm_store_t *mkc_index(MKC_Proj *lp, const MKC_File *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_mkc_XXXXXX");
...
static void mkc_cleanup(MKC_Proj *lp, pmm_store_t *store) {
    if (store)
...
static void mkc_diag(pmm_store_t *store, const char *project, const char *label) {
    if (!store) {
    ...
    for (int i = 0; MKC_ALL_EDGE_TYPES[i]; i++) {
        int c = pmm_store_count_edges_by_type(store, project, MKC_ALL_EDGE_TYPES[i]);
...
static int mkc_edge(const MKC_File *files, int nfiles, const char *edge_type, int floor,
    ...
    MKC_Proj lp;
    pmm_store_t *store = mkc_index(&lp, files, nfiles);
 ...
 * no pmm_run_swift_lsp_cross), and the generic resolver does not model
 ...
 * due to the `::` split bug in pmm_registry_resolve.  Here we test a plain
...
TEST(mkc_c1_rust_new_samefile) {
     ...
     * Root cause: registry.c pmm_registry_resolve doesn't handle Rust `::` paths.
