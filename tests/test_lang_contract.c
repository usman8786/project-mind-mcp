...
/* Normalize backslashes to forward slashes in place. pmm_mkdtemp on Windows
...
static pmm_store_t *lang_open_indexed(LangProj *lp) {
    lp->project = pmm_project_name_from_path(lp->tmpdir);
    if (!lp->project) {
        return NULL;
    }
    char cache_dir[512];
    const char *configured_cache = getenv("PMM_CACHE_DIR");
    if (configured_cache && configured_cache[0]) {
        ...
        snprintf(cache_dir, sizeof(cache_dir), "%s/.cache/project-mind-mcp", home);
    }
    pmm_mkdir_p(cache_dir, 0755);
    snprintf(lp->dbpath, sizeof(lp->dbpath), "%s/%s.db", cache_dir, lp->project);
    unlink(lp->dbpath);
    lp->srv = pmm_mcp_server_new(NULL);
    if (!lp->srv) {
        return NULL;
    }
    char args[700];
    snprintf(args, sizeof(args), "{\"repo_path\":\"%s\"}", lp->tmpdir);
    char *resp = pmm_mcp_handle_tool(lp->srv, "index_repository", args);
    if (resp) {
        free(resp);
    }
    return pmm_store_open_path(lp->dbpath);
}
...
static pmm_store_t *lang_index_files(LangProj *lp, const LangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_lc_XXXXXX");
    if (!pmm_mkdtemp(lp->tmpdir)) {
        return NULL;
    }
...
static pmm_store_t *lang_index(LangProj *lp, const char *filename, const char *content) {
    LangFile f = {filename, content};
    return lang_index_files(lp, &f, 1);
}

static void lang_cleanup(LangProj *lp, pmm_store_t *store) {
    if (store) {
        pmm_store_close(store);
    }
    if (lp->srv) {
        pmm_mcp_server_free(lp->srv);
        lp->srv = NULL;
    }
    free(lp->project);
    lp->project = NULL;
...
static int label_with_outbound(pmm_store_t *store, const char *project, const char *label) {
    pmm_search_params_t p = {0};
    p.project = project;
    p.label = label;
    p.min_degree = 1;
    p.max_degree = -1;
    ...
    int n = -1;
    if (pmm_store_search(store, &p, &out) == PMM_STORE_OK) {
        n = out.count;
    }
...
static int callables_with_outbound(pmm_store_t *store, const char *project) {
...
static int count_label(pmm_store_t *store, const char *project, const char *label) {
    pmm_node_t *nodes = NULL;
    int count = 0;
    if (pmm_store_find_nodes_by_label(store, project, label, &nodes, &count) != PMM_STORE_OK) {
        return -1;
    }
...
static int type_like_nodes(pmm_store_t *store, const char *project) {
...
/* Run pmm_extract_file in a forked child; return true if the child died from a
...
#if defined(_WIN32)
    CBMFileResult *r =
        pmm_extract_file(content, (int)strlen(content), lang, "lc", relpath, 0, NULL, NULL);
    if (r) {
        pmm_free_result(r);
    }
    return false;
 ...
 * This probe asserts pmm_extract_file captures them; it is the precise, reliable
...
TEST(contract_kotlin_imports_extracted) {
    CBMFileResult *r =
        pmm_extract_file(KT_SRC, (int)strlen(KT_SRC), PMM_LANG_KOTLIN, "lc", "a.kt", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
...
TEST(contract_c_calls_attributed_to_function) {
    LangProj lp;
    pmm_store_t *store = lang_index(&lp, "a.c", C_SRC);
    int calls = store ? pmm_store_count_edges_by_type(store, lp.project, "CALLS") : -1;
...
TEST(contract_java_extract_no_crash) {
    bool crashed = extract_crashes(JAVA_SRC, PMM_LANG_JAVA, "Tracker.java");
...
static LangMetrics lang_metrics(const LangFile *files, int nfiles) {
    LangProj lp;
    pmm_store_t *store = lang_index_files(&lp, files, nfiles);
    LangMetrics m = {0};
    if (store) {
        m.ok = 1;
        m.calls = pmm_store_count_edges_by_type(store, lp.project, "CALLS");
        m.callers = callables_with_outbound(store, lp.project);
        m.types = type_like_nodes(store, lp.project);
        m.imports = pmm_store_count_edges_by_type(store, lp.project, "IMPORTS");
    }
    lang_cleanup(&lp, store);
    return m;
}
...
static int def_nodes_for_file(pmm_store_t *store, const char *project, const char *rel) {
    pmm_node_t *nodes = NULL;
    int count = 0;
    if (pmm_store_find_nodes_by_file(store, project, rel, &nodes, &count) != PMM_STORE_OK) {
        return -1;
    }
    int defs = 0;
...
static void breadth_diag(pmm_store_t *store, const char *project, const char *rel,
                         const GrammarCase *c) {
    CBMFileResult *dr =
        pmm_extract_file(c->src, (int)strlen(c->src), c->lang, "lc", c->path, 0, NULL, NULL);
...
TEST(contract_all_grammars_in_graph) {
    int n = (int)PMM_GRAMMAR_CASES_COUNT;
    ...
    for (int i = 0; i < n; i++) {
        snprintf(names[i], sizeof(names[i]), "g%03d/%s", i, PMM_GRAMMAR_CASES[i].path);
        files[i].name = names[i];
        files[i].content = PMM_GRAMMAR_CASES[i].src;
    }

    LangProj lp;
    pmm_store_t *store = lang_index_files(&lp, files, n);
    int total_nodes = store ? pmm_store_count_nodes(store, lp.project) : -1;
...
static void dump_edge_histogram(pmm_store_t *store, const char *project) {
    if (!store) {
    ...
    for (int i = 0; ALL_EDGE_TYPES[i]; i++) {
        int c = pmm_store_count_edges_by_type(store, project, ALL_EDGE_TYPES[i]);
...
static int edge_present(const LangFile *files, int nfiles, const char *edge, int floor) {
    LangProj lp;
    pmm_store_t *store = lang_index_files(&lp, files, nfiles);
    int got = store ? pmm_store_count_edges_by_type(store, lp.project, edge) : -1;
    if (got < floor) {
...
TEST(contract_edge_imports_alias_no_phantom_folder_edge_issue767) {
    ...
    pmm_store_t *store = lang_index_files(&lp, f, 3);
    int got = store ? pmm_store_count_edges_by_type(store, lp.project, "IMPORTS") : -1;
    if (got != 0) {
...
static int calls_edge_between(pmm_store_t *store, const char *project, const char *src_suffix,
                              const char *tgt_suffix) {
    pmm_edge_t *edges = NULL;
    int n = 0;
    if (pmm_store_find_edges_by_type(store, project, "CALLS", &edges, &n) != PMM_STORE_OK)
        return 0;
    int found = 0;
    ...
    for (int i = 0; i < n && !found; i++) {
        pmm_node_t s, t;
        if (pmm_store_find_node_by_id(store, edges[i].source_id, &s) != PMM_STORE_OK)
            continue;
        if (pmm_store_find_node_by_id(store, edges[i].target_id, &t) != PMM_STORE_OK) {
            pmm_node_free_fields(&s);
            continue;
        }
 ...
 * pallets/flask). */
static int count_infra_routes_matching(pmm_store_t *store, const char *project,
                                       const char *substr) {
    pmm_node_t *nodes = NULL;
    int count = 0;
    if (pmm_store_find_nodes_by_label(store, project, "Route", &nodes, &count) != PMM_STORE_OK)
        return -1;
    int hits = 0;
    for (int i = 0; i < count; i++) {
...
TEST(contract_edge_infra_routes_from_deploy_configs_still_minted) {
    ...
    pmm_store_t *store = lang_index_files(&lp, f, 1);
    ASSERT_NOT_NULL(store);
...
static int calls_edge_targets(pmm_store_t *store, const char *project, const char *label,
    ...
    for (int i = 0; i < n && !found; i++) {
        pmm_node_t tgt;
        if (pmm_store_find_node_by_id(store, edges[i].target_id, &tgt) != PMM_STORE_OK)
            continue;
...
TEST(contract_edge_python_aliased_import_call_resolves_issue988) {
    ...
    static const LangFile f[] = {
        ...
        {"caller_dotalias.py", "import pkg.dm as dz\n"
                               ...
                               "def use_dotalias(x):\n"
                               "    return dz.h(x)\n"}};
    pmm_store_t *store = lang_index_files(&lp, f, 7);
...
TEST(contract_edge_commonjs_require_call_resolves_issue871) {
    ...
    pmm_store_t *store = lang_index_files(&lp, f, 2);
    ASSERT_TRUE(store != NULL);
...
static pmm_store_t *index_parallel_fixture(LangProj *lp, const LangFile *meaningful, int n_mean) {
...
TEST(contract_edge_parallel_service_edges) {
    ...
    int graphql = store ? pmm_store_count_edges_by_type(store, lp.project, "GRAPHQL_CALLS") : -1;
    int grpc = store ? pmm_store_count_edges_by_type(store, lp.project, "GRPC_CALLS") : -1;
    int trpc = store ? pmm_store_count_edges_by_type(store, lp.project, "TRPC_CALLS") : -1;
    int infra = store ? pmm_store_count_edges_by_type(store, lp.project, "INFRA_MAPS") : -1;
...
TEST(contract_edge_file_changes_with) {
    LangProj lp;
    memset(&lp, 0, sizeof(lp));
    snprintf(lp.tmpdir, sizeof(lp.tmpdir), "/tmp/pmm_fcw_XXXXXX");
    ASSERT_NOT_NULL(pmm_mkdtemp(lp.tmpdir));
    lc_to_fwd_slashes(lp.tmpdir);
    ...
    pmm_store_t *store = lang_open_indexed(&lp);
    int fcw = store ? pmm_store_count_edges_by_type(store, lp.project, "FILE_CHANGES_WITH") : -1;
    if (fcw < 1) {
        dump_edge_histogram(store, lp.project);
    }
    lang_cleanup(&lp, store);
