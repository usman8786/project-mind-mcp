...
typedef struct {
    ...
    pmm_version_cohort_quiesce_result_t quiesce_result;
    pmm_version_cohort_lease_t *lease;
} version_cohort_mutation_wait_t;

static pmm_daemon_build_identity_t version_cohort_identity(const char *version, const char *build) {
...
static bool version_cohort_fixture_start(version_cohort_fixture_t *fixture, const char *tag) {
    ...
    int written = snprintf(fixture->parent, sizeof(fixture->parent),
                           "%s/pmm-version-cohort-%s-XXXXXX", pmm_tmpdir(), tag);
    if (written <= 0 || written >= (int)sizeof(fixture->parent) || !pmm_mkdtemp(fixture->parent)) {
        return false;
    }
    fixture->endpoint = pmm_daemon_ipc_endpoint_new("0123456789abcdef", fixture->parent);
    return fixture->endpoint != NULL;
}

static void version_cohort_release(pmm_version_cohort_lease_t **lease) {
    while (lease && *lease && pmm_version_cohort_lease_release(lease) != PMM_PRIVATE_FILE_LOCK_OK) {
        pmm_usleep(1000);
    }
}

static void version_cohort_manager_close(pmm_version_cohort_manager_t **manager) {
    while (manager && *manager &&
           pmm_version_cohort_manager_free(manager) != PMM_PRIVATE_FILE_LOCK_OK) {
        pmm_usleep(1000);
    }
}
...
static void version_cohort_mutation_wait_init(version_cohort_mutation_wait_t *wait,
    ...
    wait->status = PMM_VERSION_COHORT_IO;
    wait->quiesce_result = PMM_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    atomic_init(&wait->callback_count, 0);
    atomic_init(&wait->callback_seen, false);
    atomic_init(&wait->finished, false);
}

static pmm_version_cohort_quiesce_result_t version_cohort_test_request_quiesce(void *context) {
...
static void *version_cohort_mutation_wait_thread(void *context) {
    version_cohort_mutation_wait_t *wait = context;
    wait->status = pmm_version_cohort_reserve_for_mutation(
...
static bool version_cohort_wait_for_atomic(atomic_bool *value, uint64_t deadline_ms) {
    while (!atomic_load_explicit(value, memory_order_acquire) && pmm_now_ms() < deadline_ms) {
        pmm_usleep(1000);
    }
...
TEST(version_cohort_shares_exact_build_rejects_conflict_and_turns_over) {
    ...
    pmm_version_cohort_manager_t *first = pmm_version_cohort_manager_new(fixture.endpoint);
    pmm_version_cohort_manager_t *second = pmm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(first);
    ASSERT_NOT_NULL(second);

    pmm_daemon_build_identity_t build_a = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    pmm_daemon_build_identity_t build_b = version_cohort_identity("2.5.0", VERSION_COHORT_BUILD_B);
    pmm_version_cohort_lease_t *a_first = NULL;
    pmm_version_cohort_lease_t *a_second = NULL;
    pmm_version_cohort_lease_t *b_lease = NULL;
    pmm_daemon_conflict_t conflict;

    ASSERT_EQ(pmm_version_cohort_acquire(first, &build_a, UINT64_MAX, &a_first, &conflict),
              PMM_VERSION_COHORT_OK);
    ASSERT_NOT_NULL(a_first);
    ASSERT_EQ(pmm_version_cohort_acquire(second, &build_a, UINT64_MAX, &a_second, &conflict),
              PMM_VERSION_COHORT_OK);
    ASSERT_NOT_NULL(a_second);
    ASSERT_EQ(pmm_version_cohort_acquire(second, &build_b, pmm_now_ms(), &b_lease, &conflict),
              PMM_VERSION_COHORT_CONFLICT);
    ASSERT_NULL(b_lease);
    ASSERT_EQ(conflict.status, PMM_DAEMON_HELLO_VERSION_CONFLICT);
    ...
    ASSERT_EQ(pmm_version_cohort_acquire(second, &build_b, UINT64_MAX, &b_lease, &conflict),
              PMM_VERSION_COHORT_OK);
...
TEST(version_cohort_rejects_same_hash_with_different_abi) {
    ...
    ASSERT_NOT_NULL(first);
    ASSERT_NOT_NULL(second);
    pmm_daemon_build_identity_t active = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    pmm_daemon_build_identity_t requested = active;
    requested.feature_abi++;
    pmm_version_cohort_lease_t *active_lease = NULL;
    pmm_version_cohort_lease_t *requested_lease = NULL;
    pmm_daemon_conflict_t conflict;

    ASSERT_EQ(pmm_version_cohort_acquire(first, &active, UINT64_MAX, &active_lease, &conflict),
              PMM_VERSION_COHORT_OK);
    ASSERT_EQ(
        pmm_version_cohort_acquire(second, &requested, pmm_now_ms(), &requested_lease, &conflict),
        PMM_VERSION_COHORT_CONFLICT);
    ASSERT_NULL(requested_lease);
    ASSERT_EQ(conflict.status, PMM_DAEMON_HELLO_FEATURE_ABI_CONFLICT);
...
TEST(version_cohort_rejects_missing_cache_fingerprint) {
    ...
    pmm_version_cohort_manager_t *manager = pmm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(manager);
    pmm_daemon_build_identity_t identity = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    identity.cache_fingerprint = NULL;
    pmm_version_cohort_lease_t *lease = NULL;
    pmm_daemon_conflict_t conflict;

    ASSERT_EQ(pmm_version_cohort_acquire(manager, &identity, pmm_now_ms(), &lease, &conflict),
              PMM_VERSION_COHORT_UNSAFE);
    ASSERT_NULL(lease);
...
TEST(version_cohort_rejects_exact_build_with_different_cache_root) {
    ...
    ASSERT_EQ(
        ...
        PMM_VERSION_COHORT_CONFLICT);
    ASSERT_NULL(requested_lease);
    ASSERT_EQ(conflict.status, PMM_DAEMON_HELLO_CACHE_CONFLICT);
    ...
    ASSERT_TRUE(pmm_daemon_conflict_format(&conflict, message, sizeof(message)));
...
TEST(version_cohort_exclusive_activation_blocks_and_is_blocked_by_participants) {
    ...
    pmm_version_cohort_manager_t *participant_manager =
        pmm_version_cohort_manager_new(fixture.endpoint);
    pmm_version_cohort_manager_t *activation_manager =
        pmm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(participant_manager);
    ASSERT_NOT_NULL(activation_manager);
    pmm_daemon_build_identity_t build_a = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    pmm_version_cohort_lease_t *participant = NULL;
    pmm_version_cohort_lease_t *activation = NULL;
    pmm_daemon_conflict_t conflict;

    ASSERT_EQ(pmm_version_cohort_acquire(participant_manager, &build_a, UINT64_MAX, &participant,
                                         &conflict),
              PMM_VERSION_COHORT_OK);
    ASSERT_EQ(pmm_version_cohort_reserve_exclusive(activation_manager, pmm_now_ms(), &activation),
              PMM_VERSION_COHORT_BUSY);
    ASSERT_NULL(activation);
    version_cohort_release(&participant);

    ASSERT_EQ(pmm_version_cohort_reserve_exclusive(activation_manager, UINT64_MAX, &activation),
              PMM_VERSION_COHORT_OK);
    ASSERT_NOT_NULL(activation);
    ASSERT_EQ(pmm_version_cohort_acquire(participant_manager, &build_a, pmm_now_ms(), &participant,
                                         &conflict),
              PMM_VERSION_COHORT_BUSY);
...
TEST(version_cohort_mutation_intent_fails_new_admission_and_spans_lease) {
    ...
    pmm_version_cohort_manager_t *participant_manager =
        pmm_version_cohort_manager_new(fixture.endpoint);
    pmm_version_cohort_manager_t *mutation_manager =
        pmm_version_cohort_manager_new(fixture.endpoint);
    pmm_version_cohort_manager_t *contender_manager =
        pmm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(participant_manager);
    ...
    pmm_daemon_conflict_t conflict;
    pmm_version_cohort_lease_t *participant = NULL;
    pmm_version_cohort_lease_t *contender = NULL;
    ASSERT_EQ(pmm_version_cohort_acquire(participant_manager, &build_a, UINT64_MAX, &participant,
                                         &conflict),
              PMM_VERSION_COHORT_OK);
    pmm_version_cohort_maintenance_presence_t before =
        pmm_version_cohort_maintenance_presence(contender_manager);

    version_cohort_mutation_wait_t wait;
    version_cohort_mutation_wait_init(&wait, mutation_manager, pmm_now_ms() + 5000U);
    pmm_thread_t thread;
    bool started = pmm_thread_create(&thread, 0, version_cohort_mutation_wait_thread, &wait) == 0;
    bool callback_seen =
        started && version_cohort_wait_for_atomic(&wait.callback_seen, pmm_now_ms() + 2000U);
    pmm_version_cohort_maintenance_presence_t during =
        callback_seen ? pmm_version_cohort_maintenance_presence(contender_manager)
                      : PMM_VERSION_COHORT_MAINTENANCE_IO;
    pmm_version_cohort_status_t racing_status =
        callback_seen ? pmm_version_cohort_acquire(contender_manager, &build_a, UINT64_MAX,
                                                   &contender, &conflict)
    ...
    version_cohort_release(&participant);
    bool finished = started && version_cohort_wait_for_atomic(&wait.finished, pmm_now_ms() + 5500U);
    bool joined = started && pmm_thread_join(&thread) == 0;
    pmm_version_cohort_maintenance_presence_t retained =
        finished && wait.lease ? pmm_version_cohort_maintenance_presence(contender_manager)
                               : PMM_VERSION_COHORT_MAINTENANCE_IO;
    pmm_version_cohort_status_t retained_admission_status =
        finished && wait.lease ? pmm_version_cohort_acquire(contender_manager, &build_a, UINT64_MAX,
                                                            &contender, &conflict)
                               : PMM_VERSION_COHORT_IO;
    bool retained_admission_absent = contender == NULL;
    version_cohort_release(&contender);
    version_cohort_release(&wait.lease);
    pmm_version_cohort_maintenance_presence_t after =
        pmm_version_cohort_maintenance_presence(contender_manager);
    pmm_version_cohort_status_t post_status =
        pmm_version_cohort_acquire(contender_manager, &build_a, UINT64_MAX, &contender, &conflict);
    ...
    ASSERT_EQ(before, PMM_VERSION_COHORT_MAINTENANCE_ABSENT);
    ASSERT_TRUE(started);
    ASSERT_TRUE(callback_seen);
    ASSERT_EQ(during, PMM_VERSION_COHORT_MAINTENANCE_REQUESTED);
    ASSERT_EQ(racing_status, PMM_VERSION_COHORT_BUSY);
    ASSERT_TRUE(racing_lease_absent);
    ASSERT_TRUE(still_draining);
    ASSERT_TRUE(finished);
    ASSERT_TRUE(joined);
    ASSERT_EQ(wait.status, PMM_VERSION_COHORT_OK);
    ASSERT_EQ(wait.quiesce_result, PMM_VERSION_COHORT_QUIESCE_REQUESTED);
    ASSERT_EQ(atomic_load_explicit(&wait.callback_count, memory_order_relaxed), 1);
    ASSERT_EQ(retained, PMM_VERSION_COHORT_MAINTENANCE_REQUESTED);
    ASSERT_EQ(retained_admission_status, PMM_VERSION_COHORT_BUSY);
    ASSERT_TRUE(retained_admission_absent);
    ASSERT_EQ(after, PMM_VERSION_COHORT_MAINTENANCE_ABSENT);
    ASSERT_EQ(post_status, PMM_VERSION_COHORT_OK);
    PASS();
}
...
TEST(version_cohort_mutation_waits_for_every_lifetime_participant) {
    ...
    pmm_version_cohort_manager_t *first_manager = pmm_version_cohort_manager_new(fixture.endpoint);
    pmm_version_cohort_manager_t *second_manager = pmm_version_cohort_manager_new(fixture.endpoint);
    pmm_version_cohort_manager_t *mutation_manager =
        pmm_version_cohort_manager_new(fixture.endpoint);
    ...
    pmm_daemon_build_identity_t build_a = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    pmm_daemon_conflict_t conflict;
    pmm_version_cohort_lease_t *first = NULL;
    pmm_version_cohort_lease_t *second = NULL;
    ASSERT_EQ(pmm_version_cohort_acquire(first_manager, &build_a, UINT64_MAX, &first, &conflict),
              PMM_VERSION_COHORT_OK);
    ASSERT_EQ(pmm_version_cohort_acquire(second_manager, &build_a, UINT64_MAX, &second, &conflict),
              PMM_VERSION_COHORT_OK);
...
TEST(version_cohort_mutation_timeout_releases_all_guards) {
    ...
    pmm_version_cohort_manager_t *mutation_manager =
        pmm_version_cohort_manager_new(fixture.endpoint);
    pmm_version_cohort_manager_t *probe_manager = pmm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(participant_manager);
    ASSERT_NOT_NULL(mutation_manager);
    ...
    pmm_daemon_conflict_t conflict;
    pmm_version_cohort_lease_t *participant = NULL;
    pmm_version_cohort_lease_t *mutation = NULL;
    pmm_version_cohort_lease_t *probe = NULL;
    ASSERT_EQ(pmm_version_cohort_acquire(participant_manager, &build_a, UINT64_MAX, &participant,
                                         &conflict),
              PMM_VERSION_COHORT_OK);

    version_cohort_mutation_wait_t callback;
    version_cohort_mutation_wait_init(&callback, mutation_manager, pmm_now_ms() + 25U);
    pmm_version_cohort_quiesce_result_t quiesce_result = PMM_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    pmm_version_cohort_status_t timeout_status = pmm_version_cohort_reserve_for_mutation(
    ...
    pmm_version_cohort_maintenance_presence_t after_timeout =
        pmm_version_cohort_maintenance_presence(probe_manager);
    pmm_version_cohort_status_t admission_status =
        pmm_version_cohort_acquire(probe_manager, &build_a, UINT64_MAX, &probe, &conflict);

    version_cohort_release(&probe);
    version_cohort_release(&participant);
    pmm_version_cohort_quiesce_result_t invalid_result = PMM_VERSION_COHORT_QUIESCE_REQUESTED;
    pmm_version_cohort_status_t unbounded_status = pmm_version_cohort_reserve_for_mutation(
    ...
    pmm_version_cohort_quiesce_result_t retry_result = PMM_VERSION_COHORT_QUIESCE_REQUESTED;
    pmm_version_cohort_status_t retry_status = pmm_version_cohort_reserve_for_mutation(
        mutation_manager, pmm_now_ms() + 250U, version_cohort_test_request_quiesce, &callback,
        &retry_result, &mutation);
    pmm_version_cohort_maintenance_presence_t during_retry =
        mutation ? pmm_version_cohort_maintenance_presence(probe_manager)
                 : PMM_VERSION_COHORT_MAINTENANCE_IO;
    ...
    pmm_version_cohort_maintenance_presence_t after_retry =
        pmm_version_cohort_maintenance_presence(probe_manager);
    ...
    ASSERT_EQ(timeout_status, PMM_VERSION_COHORT_BUSY);
    ASSERT_TRUE(no_mutation_authority);
    ASSERT_EQ(quiesce_result, PMM_VERSION_COHORT_QUIESCE_REQUESTED);
    ASSERT_EQ(atomic_load_explicit(&callback.callback_count, memory_order_relaxed), 1);
    ASSERT_EQ(after_timeout, PMM_VERSION_COHORT_MAINTENANCE_ABSENT);
    ASSERT_EQ(admission_status, PMM_VERSION_COHORT_OK);
    ASSERT_EQ(unbounded_status, PMM_VERSION_COHORT_UNSAFE);
    ASSERT_EQ(invalid_result, PMM_VERSION_COHORT_QUIESCE_NOT_NEEDED);
    ASSERT_TRUE(unbounded_lease_absent);
    ASSERT_EQ(retry_status, PMM_VERSION_COHORT_OK);
    ASSERT_EQ(retry_result, PMM_VERSION_COHORT_QUIESCE_NOT_NEEDED);
    ASSERT_EQ(during_retry, PMM_VERSION_COHORT_MAINTENANCE_REQUESTED);
    ASSERT_EQ(callback_count_after_retry, 1);
    ASSERT_EQ(after_retry, PMM_VERSION_COHORT_MAINTENANCE_ABSENT);
    PASS();
}
...
TEST(version_cohort_does_not_repurpose_daemon_startup_lock_for_lifetime) {
    ...
    ASSERT_EQ(pmm_daemon_ipc_startup_lock_try_acquire(fixture.endpoint, &startup), 1);
    ...
    pmm_daemon_conflict_t conflict;
    ASSERT_NOT_NULL(manager);
    ASSERT_EQ(pmm_version_cohort_acquire(manager, &build_a, UINT64_MAX, &lease, &conflict),
              PMM_VERSION_COHORT_OK);
...
TEST(version_cohort_distinguishes_coordinated_daemon_without_connecting) {
    ...
    pmm_version_cohort_manager_t *manager = pmm_version_cohort_manager_new(fixture.endpoint);
    pmm_version_cohort_daemon_claim_t *claim = NULL;
    pmm_daemon_ipc_lifetime_reservation_t *lifetime = NULL;
    pmm_daemon_ipc_startup_lock_t *startup = NULL;
    ASSERT_NOT_NULL(manager);

    ASSERT_EQ(pmm_version_cohort_daemon_presence(manager, fixture.endpoint),
              PMM_VERSION_COHORT_DAEMON_ABSENT);
    ...
    ASSERT_NOT_NULL(startup);
    ASSERT_EQ(pmm_version_cohort_daemon_presence(manager, fixture.endpoint),
              PMM_VERSION_COHORT_DAEMON_UNCOORDINATED);
    pmm_daemon_ipc_startup_lock_release(&startup);
    startup = NULL;
    ASSERT_EQ(pmm_version_cohort_daemon_presence(manager, fixture.endpoint),
              PMM_VERSION_COHORT_DAEMON_ABSENT);
    ...
    ASSERT_EQ(pmm_daemon_ipc_lifetime_reservation_try_acquire(fixture.endpoint, &lifetime), 1);
    ASSERT_NOT_NULL(lifetime);
    ASSERT_EQ(pmm_version_cohort_daemon_presence(manager, fixture.endpoint),
              PMM_VERSION_COHORT_DAEMON_UNCOORDINATED);

    ASSERT_EQ(pmm_version_cohort_daemon_claim_acquire(manager, &claim), PMM_VERSION_COHORT_OK);
    ASSERT_NOT_NULL(claim);
    ASSERT_EQ(pmm_version_cohort_daemon_presence(manager, fixture.endpoint),
              PMM_VERSION_COHORT_DAEMON_COORDINATED);
    ...
    ASSERT_EQ(pmm_version_cohort_daemon_claim_release(&claim), PMM_PRIVATE_FILE_LOCK_OK);
    ASSERT_NULL(claim);
    ASSERT_EQ(pmm_version_cohort_daemon_presence(manager, fixture.endpoint),
              PMM_VERSION_COHORT_DAEMON_ABSENT);
...
TEST(version_cohort_transition_presence_is_authoritative_and_marker_checked) {
    ...
    ASSERT_EQ(pmm_daemon_ipc_local_transition_try_acquire(fixture.endpoint, &transition), 1);
    ASSERT_NOT_NULL(transition);
    ASSERT_EQ(
        pmm_version_cohort_daemon_presence_under_transition(manager, fixture.endpoint, transition),
        PMM_VERSION_COHORT_DAEMON_UNSAFE);
    ASSERT_EQ(pmm_daemon_ipc_local_transition_seal_legacy(transition), 1);
    ASSERT_EQ(
        pmm_version_cohort_daemon_presence_under_transition(manager, fixture.endpoint, transition),
        PMM_VERSION_COHORT_DAEMON_ABSENT);
    ASSERT_TRUE(pmm_daemon_ipc_local_transition_release(&transition));
...
TEST(version_cohort_presence_recovers_current_posix_listener_crash) {
    ...
    if (child == 0) {
        ...
        pmm_daemon_ipc_listener_t *listener = pmm_daemon_ipc_listen(fixture.endpoint);
    ...
    ASSERT_EQ(pmm_daemon_ipc_lifetime_reservation_probe(fixture.endpoint), 0);
    ASSERT_EQ(pmm_daemon_ipc_endpoint_probe(fixture.endpoint, 1), 1);
    ...
    ASSERT_EQ(
        pmm_version_cohort_daemon_presence_under_transition(manager, fixture.endpoint, transition),
        PMM_VERSION_COHORT_DAEMON_ABSENT);
    ASSERT_EQ(pmm_daemon_ipc_endpoint_probe(fixture.endpoint, 1), 0);
...
TEST(version_cohort_crash_releases_process_lifetime_lease) {
    ...
    const char *const argv[] = {
        self, "__pmm_version_cohort_crash_holder", "0123456789abcdef", fixture.parent, ready_path,
        NULL,
    };
    ...
    pmm_subprocess_t *child = NULL;
    int spawn_status = launch_ready ? pmm_subprocess_spawn(&options, &child) : -1;
    bool ready = false;
    bool terminal = false;
    pmm_proc_result_t process_result = {0};
    uint64_t ready_deadline = pmm_now_ms() + 5000U;
    while (child && pmm_now_ms() < ready_deadline) {
        pmm_proc_poll_t poll = pmm_subprocess_poll(child, &process_result);
        if (poll == PMM_PROC_POLL_TERMINAL) {
            terminal = true;
            break;
        }
        if (poll == PMM_PROC_POLL_ERROR) {
            break;
        }
        FILE *marker = pmm_fopen(ready_path, "rb");
        if (marker) {
    ...
    pmm_version_cohort_manager_t *manager =
        ready ? pmm_version_cohort_manager_new(fixture.endpoint) : NULL;
    pmm_daemon_build_identity_t build_b = version_cohort_identity("2.5.0", VERSION_COHORT_BUILD_B);
    pmm_version_cohort_lease_t *lease = NULL;
    pmm_daemon_conflict_t conflict;
    pmm_version_cohort_status_t conflict_status =
        manager ? pmm_version_cohort_acquire(manager, &build_b, pmm_now_ms(), &lease, &conflict)
                : PMM_VERSION_COHORT_IO;
    bool conflict_lease_absent = lease == NULL;
    version_cohort_release(&lease);
    bool cancel_requested = child && !terminal && pmm_subprocess_request_cancel(child);
    uint64_t terminal_deadline = pmm_now_ms() + 5000U;
    while (child && !terminal && pmm_now_ms() < terminal_deadline) {
        pmm_proc_poll_t poll = pmm_subprocess_poll(child, &process_result);
        if (poll == PMM_PROC_POLL_TERMINAL) {
            terminal = true;
            break;
    ...
    pmm_daemon_conflict_t turnover_conflict;
    pmm_version_cohort_status_t turnover_status =
        manager && terminal
            ? pmm_version_cohort_acquire(manager, &build_b, UINT64_MAX, &lease, &turnover_conflict)
            : PMM_VERSION_COHORT_IO;

    version_cohort_release(&lease);
    version_cohort_manager_close(&manager);
    if (child && terminal) {
        pmm_subprocess_destroy(child);
        child = NULL;
    }
    (void)pmm_unlink(ready_path);
    version_cohort_fixture_finish(&fixture);

    ASSERT_TRUE(launch_ready);
    ASSERT_EQ(spawn_status, 0);
    ASSERT_TRUE(ready);
    ASSERT_EQ(conflict_status, PMM_VERSION_COHORT_CONFLICT);
    ...
    ASSERT_EQ(process_result.outcome, PMM_PROC_KILLED);
    ASSERT_EQ(turnover_status, PMM_VERSION_COHORT_OK);
    PASS();
#else
    version_cohort_fixture_t fixture;
    ...
    if (child == 0) {
        ...
        pmm_version_cohort_manager_t *manager = pmm_version_cohort_manager_new(fixture.endpoint);
        pmm_daemon_build_identity_t build_a =
            version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
        pmm_version_cohort_lease_t *lease = NULL;
        pmm_daemon_conflict_t conflict;
        bool acquired = manager && pmm_version_cohort_acquire(manager, &build_a, UINT64_MAX, &lease,
    ...
    pmm_daemon_conflict_t conflict;
    ASSERT_NOT_NULL(manager);
    ASSERT_EQ(pmm_version_cohort_acquire(manager, &build_b, pmm_now_ms(), &lease, &conflict),
              PMM_VERSION_COHORT_CONFLICT);
    ASSERT_NULL(lease);
    ASSERT_EQ(conflict.status, PMM_DAEMON_HELLO_VERSION_CONFLICT);
    ...
    ASSERT_EQ(pmm_version_cohort_acquire(manager, &build_b, UINT64_MAX, &lease, &conflict),
              PMM_VERSION_COHORT_OK);
    version_cohort_release(&lease);
    version_cohort_manager_close(&manager);
    version_cohort_fixture_finish(&fixture);
