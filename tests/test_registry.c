...
TEST(fqn_simple) {
    char *qn = pmm_pipeline_fqn_compute("myproj", "cmd/server/main.go", "HandleRequest");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_no_name) {
    char *qn = pmm_pipeline_fqn_compute("myproj", "pkg/service.go", NULL);
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_python_init) {
    ...
    char *qn = pmm_pipeline_fqn_compute("myproj", "pkg/__init__.py", "Foo");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_js_index) {
    ...
    char *qn = pmm_pipeline_fqn_compute("myproj", "src/index.ts", "App");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_module) {
    char *qn = pmm_pipeline_fqn_module("myproj", "cmd/server/main.go");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_folder) {
    char *qn = pmm_pipeline_fqn_folder("myproj", "cmd/server");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_root_file) {
    char *qn = pmm_pipeline_fqn_compute("proj", "main.go", "main");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_init_module_distinct_from_folder) {
    ...
    char *mod_qn = pmm_pipeline_fqn_module("proj", "pkg/__init__.py");
    char *folder_qn = pmm_pipeline_fqn_folder("proj", "pkg");
...
TEST(fqn_init_nested_module_distinct) {
    ...
    char *mod_qn =
        pmm_pipeline_fqn_module("proj", "docker-images/cloud-runs/bq-sync-api/__init__.py");
    char *folder_qn = pmm_pipeline_fqn_folder("proj", "docker-images/cloud-runs/bq-sync-api");
    ASSERT_NOT_NULL(mod_qn);
    ASSERT_NOT_NULL(folder_qn);
    ASSERT_STR_NEQ(mod_qn, folder_qn);
    free(mod_qn);
...
TEST(fqn_index_ts_module_distinct_from_folder) {
    ...
    char *mod_qn = pmm_pipeline_fqn_module("proj", "src/components/index.ts");
    char *folder_qn = pmm_pipeline_fqn_folder("proj", "src/components");
    ASSERT_NOT_NULL(mod_qn);
    ASSERT_NOT_NULL(folder_qn);
    ASSERT_STR_NEQ(mod_qn, folder_qn);
    free(mod_qn);
...
TEST(fqn_init_symbols_get_clean_package_qn) {
    ...
    char *sym_qn = pmm_pipeline_fqn_compute("proj", "pkg/__init__.py", "Foo");
...
TEST(fqn_index_symbols_get_clean_qn) {
    ...
    char *sym_qn = pmm_pipeline_fqn_compute("proj", "src/index.ts", "App");
    ASSERT_NOT_NULL(sym_qn);
...
TEST(fqn_init_file_node_distinct) {
    ...
    char *file_qn = pmm_pipeline_fqn_compute("proj", "pkg/__init__.py", "__file__");
    char *folder_qn = pmm_pipeline_fqn_folder("proj", "pkg");
...
TEST(fqn_regular_module_unchanged) {
    ...
    char *qn = pmm_pipeline_fqn_module("proj", "pkg/utils.py");
    ASSERT_NOT_NULL(qn);
...
TEST(project_name_from_path) {
    char *name = pmm_project_name_from_path("/Users/dev/project");
...
TEST(project_name_from_root) {
    char *name = pmm_project_name_from_path("/");
    ASSERT_NOT_NULL(name);
...
TEST(registry_create_free) {
    pmm_registry_t *r = pmm_registry_new();
    ASSERT_NOT_NULL(r);
    ASSERT_EQ(pmm_registry_size(r), 0);
    pmm_registry_free(r);
    PASS();
}

TEST(registry_free_null) {
    pmm_registry_free(NULL); /* should not crash */
    PASS();
}

TEST(registry_add_and_exists) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "main", "proj.cmd.main", "Function");
    ASSERT_EQ(pmm_registry_size(r), 1);
    ASSERT_TRUE(pmm_registry_exists(r, "proj.cmd.main"));
    ASSERT_FALSE(pmm_registry_exists(r, "proj.cmd.other"));
    pmm_registry_free(r);
    PASS();
}
...
TEST(registry_label_of) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Foo", "proj.pkg.Foo", "Class");
    pmm_registry_add(r, "bar", "proj.pkg.bar", "Function");

    ASSERT_STR_EQ(pmm_registry_label_of(r, "proj.pkg.Foo"), "Class");
    ASSERT_STR_EQ(pmm_registry_label_of(r, "proj.pkg.bar"), "Function");
    ASSERT_NULL(pmm_registry_label_of(r, "proj.pkg.baz"));
...
TEST(registry_find_by_name) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "main", "proj.cmd.main", "Function");
    pmm_registry_add(r, "main", "proj.srv.main", "Function");
    pmm_registry_add(r, "helper", "proj.util.helper", "Function");

    const char **out = NULL;
    int count = 0;
    pmm_registry_find_by_name(r, "main", &out, &count);
    ASSERT_EQ(count, 2);

    pmm_registry_find_by_name(r, "helper", &out, &count);
    ASSERT_EQ(count, 1);

    pmm_registry_find_by_name(r, "nonexistent", &out, &count);
    ASSERT_EQ(count, 0);
...
TEST(registry_no_duplicates) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "main", "proj.cmd.main", "Function");
    pmm_registry_add(r, "main", "proj.cmd.main", "Function"); /* duplicate */
    ASSERT_EQ(pmm_registry_size(r), 1);
...
TEST(resolve_same_module) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "helper", "proj.pkg.service.helper", "Function");

    /* Call "helper" from the same module → should resolve */
    pmm_resolution_t res = pmm_registry_resolve(r, "helper", "proj.pkg.service", NULL, NULL, 0);
...
TEST(resolve_qualified_disambiguates_same_name) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "save", "proj.lib.App.Alpha.save", "Function");
    pmm_registry_add(r, "save", "proj.lib.App.Beta.save", "Function");
    pmm_registry_add(r, "save", "proj.lib.App.Gamma.save", "Function");

    /* Each fully-qualified call routes to its own package. */
    pmm_resolution_t a =
        pmm_registry_resolve(r, "App::Alpha::save", "proj.lib.App.Caller", NULL, NULL, 0);
    ASSERT_STR_EQ(a.qualified_name, "proj.lib.App.Alpha.save");
    ASSERT_STR_EQ(a.strategy, "qualified_suffix");

    pmm_resolution_t b =
        pmm_registry_resolve(r, "App::Beta::save", "proj.lib.App.Caller", NULL, NULL, 0);
    ASSERT_STR_EQ(b.qualified_name, "proj.lib.App.Beta.save");

    pmm_resolution_t g =
        pmm_registry_resolve(r, "App::Gamma::save", "proj.lib.App.Caller", NULL, NULL, 0);
    ASSERT_STR_EQ(g.qualified_name, "proj.lib.App.Gamma.save");

    /* The dotted callee form (Go/Python/C#) disambiguates identically. */
    pmm_resolution_t dotted =
        pmm_registry_resolve(r, "App.Beta.save", "proj.lib.App.Caller", NULL, NULL, 0);
    ...
    pmm_resolution_t nomatch =
        pmm_registry_resolve(r, "Other::Pkg::save", "proj.lib.App.Caller", NULL, NULL, 0);
    ...
    pmm_resolution_t bare =
        pmm_registry_resolve(r, "save", "proj.lib.App.Caller", NULL, NULL, 0);
...
TEST(resolve_qualified_ambiguous_tail_falls_through) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "run", "proj.svcA.Foo.Bar.run", "Function");
    pmm_registry_add(r, "run", "proj.svcB.Foo.Bar.run", "Function");

    /* "Foo::Bar::run" tail matches BOTH candidates → not unique → fall through. */
    pmm_resolution_t res =
        pmm_registry_resolve(r, "Foo::Bar::run", "proj.svcA.Caller", NULL, NULL, 0);
...
TEST(resolve_import_map) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Process", "proj.pkg.worker.Process", "Function");
    ...
    pmm_resolution_t res =
        pmm_registry_resolve(r, "worker.Process", "proj.cmd.main", keys, vals, 1);
...
TEST(resolve_import_map_bare_function) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "requireAdmin", "proj.lib.authorization.requireAdmin", "Function");
    /* Same name in another module — without the fix this is what gets picked. */
    pmm_registry_add(r, "requireAdmin", "proj.lib.users.requireAdmin", "Function");

    const char *keys[] = {"requireAdmin"};
    const char *vals[] = {"proj.lib.authorization"};

    pmm_resolution_t res =
        pmm_registry_resolve(r, "requireAdmin", "proj.actions.settings", keys, vals, 1);
...
TEST(resolve_import_map_bare_alias) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "scan_bash", "proj.security_scan.scan_bash", "Function");
    ...
    pmm_resolution_t res =
        pmm_registry_resolve(r, "_scan_bash", "proj.hooks.pre_tool", keys, vals, 1);
...
TEST(resolve_unique_name) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "UniqueFunc", "proj.deep.path.UniqueFunc", "Function");

    /* Call "UniqueFunc" — only one candidate project-wide */
    pmm_resolution_t res =
        pmm_registry_resolve(r, "UniqueFunc", "proj.other.module", NULL, NULL, 0);
...
TEST(resolve_unresolved) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "foo", "proj.pkg.foo", "Function");

    /* Call "nonexistent" — not in registry */
    pmm_resolution_t res = pmm_registry_resolve(r, "nonexistent", "proj.other", NULL, NULL, 0);
...
TEST(resolve_many_nodes) {
    ...
    for (int i = 0; i < 500; i++) {
        ...
        pmm_registry_add(r, name, qn, "Function");
    }
    ASSERT_EQ(pmm_registry_size(r), 500);

    /* Resolve one */
    pmm_resolution_t res = pmm_registry_resolve(r, "func_250", "proj.pkg", NULL, NULL, 0);
...
TEST(resolve_suffix_match) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Process", "proj.svcA.Process", "Function");
    pmm_registry_add(r, "Process", "proj.svcB.Process", "Function");

    /* Caller in svcA — should prefer svcA via import distance */
    pmm_resolution_t res = pmm_registry_resolve(r, "Process", "proj.svcA.caller", NULL, NULL, 0);
...
TEST(resolve_caps_unresolvably_ambiguous_names) {
    ...
    for (int i = 0; i < 300; i++) {
        ...
        pmm_registry_add(r, "flags", qn, "Variable");
    }
    pmm_resolution_t res = pmm_registry_resolve(r, "flags", "proj.other.caller", NULL, NULL, 0);
    ASSERT_TRUE(res.qualified_name == NULL || res.qualified_name[0] == '\0');

    /* Same-module resolution still wins regardless of candidate count. */
    res = pmm_registry_resolve(r, "flags", "proj.mod7", NULL, NULL, 0);
...
TEST(resolve_import_map_suffix) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Foo", "proj.other.sub.Foo", "Function");
    ...
    pmm_resolution_t res = pmm_registry_resolve(r, "other.Foo", "proj.pkg", keys, vals, 1);
...
TEST(resolve_is_import_reachable) {
    ...
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Helper", "proj.shared.utils.Helper", "Function");

    /* With import covering the module → full confidence */
    const char *keys1[] = {"utils"};
    const char *vals1[] = {"proj.shared.utils"};
    pmm_resolution_t res = pmm_registry_resolve(r, "Helper", "proj.caller", keys1, vals1, 1);
    ASSERT_STR_EQ(res.strategy, "unique_name");
    ...
    res = pmm_registry_resolve(r, "Helper", "proj.caller", keys2, vals2, 1);
    ASSERT_STR_EQ(res.strategy, "unique_name");
...
TEST(resolve_import_reachable_prefix) {
    ...
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Process", "proj.handler.sub.Process", "Function");

    const char *keys[] = {"handler"};
    const char *vals[] = {"proj.handler"};
    pmm_resolution_t res = pmm_registry_resolve(r, "Process", "proj.caller", keys, vals, 1);
    ASSERT_STR_EQ(res.strategy, "unique_name");
...
TEST(negative_import_rejects_unimported) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Process", "proj.billing.Process", "Function");
    pmm_registry_add(r, "Process", "proj.handler.Process", "Function");
...
TEST(fuzzy_resolve_single_candidate) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "CreateOrder", "svcA.handlers.CreateOrder", "Function");
    pmm_registry_add(r, "ValidateOrder", "svcB.validators.ValidateOrder", "Function");

    /* FuzzyResolve should find by simple name even with unknown prefix */
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "unknownPkg.CreateOrder", "svcC.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_resolve_nonexistent) {
    ...
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "NonExistent", "svcC.caller", NULL, NULL, 0);
    ASSERT_FALSE(fr.ok);
...
TEST(fuzzy_resolve_multiple_best_by_distance) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Process", "svcA.handlers.Process", "Function");
    pmm_registry_add(r, "Process", "svcB.handlers.Process", "Function");

    /* Caller in svcA — should prefer svcA */
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "unknown.Process", "svcA.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT_STR_EQ(fr.result.qualified_name, "svcA.handlers.Process");

    /* Caller in svcB — should prefer svcB */
    fr = pmm_registry_fuzzy_resolve(r, "unknown.Process", "svcB.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_resolve_deep_name_extraction) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "DoWork", "myproject.utils.DoWork", "Function");

    /* Deeply qualified callee — should extract "DoWork" */
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "some.deep.module.DoWork", "myproject.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_resolve_empty_registry) {
    ...
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "SomeFunc", "myproject.caller", NULL, NULL, 0);
    ASSERT_FALSE(fr.ok);
...
TEST(fuzzy_resolve_confidence_single) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Handler", "proj.svc.Handler", "Function");

    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "unknownPkg.Handler", "proj.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_resolve_confidence_distance) {
    ...
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "unknownPkg.Process", "proj.svcA.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_penalty_unreachable_import) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "Handler", "proj.billing.Handler", "Function");

    /* Import for different module → confidence halved */
    const char *keys[] = {"other"};
    const char *vals[] = {"proj.other"};
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "unknown.Handler", "proj.caller", keys, vals, 1);
    ASSERT_TRUE(fr.ok);
    /* 0.40 * 0.5 = 0.20 */
...
TEST(fuzzy_no_import_map_passthrough) {
    ...
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(r, "unknown.Handler", "proj.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT_TRUE(fr.result.confidence >= 0.35 && fr.result.confidence <= 0.45);
...
TEST(perl_builtin_set_recognizes_core_builtins) {
    ...
    ASSERT_TRUE(pmm_perl_is_builtin("push"));
    ASSERT_TRUE(pmm_perl_is_builtin("shift"));
    ASSERT_TRUE(pmm_perl_is_builtin("keys"));
    ASSERT_TRUE(pmm_perl_is_builtin("sprintf"));
    ASSERT_TRUE(pmm_perl_is_builtin("abs"));   /* first element */
    ASSERT_TRUE(pmm_perl_is_builtin("write")); /* last element */
    ASSERT_TRUE(pmm_perl_is_builtin("wantarray"));
    PASS();
}

TEST(perl_builtin_set_rejects_project_subs) {
    /* Genuine project sub names and edge inputs must NOT be flagged. */
    ASSERT_FALSE(pmm_perl_is_builtin("helper"));
    ASSERT_FALSE(pmm_perl_is_builtin("process_request"));
    ASSERT_FALSE(pmm_perl_is_builtin("Push")); /* case-sensitive */
    ASSERT_FALSE(pmm_perl_is_builtin(""));
    ASSERT_FALSE(pmm_perl_is_builtin(NULL));
    PASS();
}
...
TEST(perl_suppress_drops_weak_builtin_and_method_matches) {
    ...
    ASSERT_TRUE(pmm_perl_suppress_generic_match(true, false, "push", "suffix_match"));
    ASSERT_TRUE(pmm_perl_suppress_generic_match(true, false, "keys", "unique_name"));
    ASSERT_TRUE(pmm_perl_suppress_generic_match(true, true, "commit", "suffix_match"));
    ASSERT_TRUE(pmm_perl_suppress_generic_match(true, true, "log", "unique_name"));
    PASS();
}
...
TEST(perl_suppress_keeps_high_confidence_and_genuine_calls) {
    ...
    ASSERT_FALSE(pmm_perl_suppress_generic_match(true, false, "log", "same_module"));
    ASSERT_FALSE(pmm_perl_suppress_generic_match(true, false, "open", "import_map"));
    ...
    ASSERT_FALSE(pmm_perl_suppress_generic_match(true, true, "Foo::Bar::m", "import_map_suffix"));
    ASSERT_FALSE(pmm_perl_suppress_generic_match(true, true, "commit", "same_module"));
    /* A genuine non-builtin function call is never suppressed (edge survives). */
    ASSERT_FALSE(pmm_perl_suppress_generic_match(true, false, "helper", "suffix_match"));
    /* Non-Perl languages are never affected. */
    ASSERT_FALSE(pmm_perl_suppress_generic_match(false, false, "push", "suffix_match"));
    ASSERT_FALSE(pmm_perl_suppress_generic_match(false, true, "commit", "suffix_match"));
    /* No match (NULL/empty strategy) → nothing to suppress. */
    ASSERT_FALSE(pmm_perl_suppress_generic_match(true, false, "push", NULL));
    ASSERT_FALSE(pmm_perl_suppress_generic_match(true, true, "commit", ""));
    PASS();
}
...
TEST(tsjs_suppress_drops_weak_method_matches) {
     ...
     * covered defensively (pmm_registry_fuzzy_resolve is not wired into the
     * resolvers today) so a future wiring cannot silently reintroduce it. */
    ASSERT_TRUE(pmm_tsjs_suppress_weak_method_match(true, true, "suffix_match"));
    ASSERT_TRUE(pmm_tsjs_suppress_weak_method_match(true, true, "unique_name"));
    ASSERT_TRUE(pmm_tsjs_suppress_weak_method_match(true, true, "field_type_hint"));
    ASSERT_TRUE(pmm_tsjs_suppress_weak_method_match(true, true, "fuzzy"));
    PASS();
}
...
TEST(tsjs_suppress_keeps_high_confidence_and_non_methods) {
    ...
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "same_module"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "import_map"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "import_map_suffix"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "qualified_suffix"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "callee_suffix"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "service_pattern"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "lsp_ts_method"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "lsp_cross"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, "lsp_ts_local"));
    /* A bare call (is_method=false) is a free-function call → never suppressed. */
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, false, "unique_name"));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, false, "suffix_match"));
    /* Non-TS/JS languages are never affected. */
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(false, true, "suffix_match"));
    /* No match (NULL/empty strategy) → nothing to suppress. */
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, NULL));
    ASSERT_FALSE(pmm_tsjs_suppress_weak_method_match(true, true, ""));
    PASS();
}
...
TEST(resolve_import_map_alias_with_suffix_hits_method) {
    pmm_registry_t *r = pmm_registry_new();
    pmm_registry_add(r, "user_logged_in", "proj.auth.signals.user_logged_in", "Variable");
    pmm_registry_add(r, "send", "proj.auth.signals.user_logged_in.send", "Method");
    const char *keys[] = {"user_logged_in"};
    const char *vals[] = {"proj.auth.signals.user_logged_in"};
    pmm_resolution_t res =
        pmm_registry_resolve(r, "user_logged_in.send", "proj.auth.views", keys, vals, 1);
