...
TEST(config_load_defaults) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_test_config_XXXXXX");
...
TEST(config_save_and_reload) {
    ...
    /* Save */
    pmm_ui_config_t cfg = {.ui_enabled = true, .ui_port = 8080};
    ASSERT_TRUE(pmm_ui_config_save(&cfg));

    /* Reload */
    pmm_ui_config_t loaded;
    pmm_ui_config_load(&loaded);
...
TEST(config_save_atomically_replaces_a_complete_generation) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_test_config_atomic_XXXXXX");
    char *td = pmm_mkdtemp(tmpdir);
    ASSERT_NOT_NULL(td);

    char *old_cache = getenv("PMM_CACHE_DIR") ? strdup(getenv("PMM_CACHE_DIR")) : NULL;
    ASSERT_EQ(pmm_setenv("PMM_CACHE_DIR", td, 1), 0);

    pmm_ui_config_t old_generation = {
        .ui_enabled = false,
        .ui_port = 11111,
    };
    ASSERT_TRUE(pmm_ui_config_save(&old_generation));

    char path[1024];
    pmm_ui_config_path(path, (int)sizeof(path));
#ifdef _WIN32
...
#else
    FILE *old_handle = pmm_fopen(path, "rb");
    ASSERT_NOT_NULL(old_handle);
#endif

    pmm_ui_config_t new_generation = {
        .ui_enabled = true,
        .ui_port = 22222,
    };
    /* Capture everything, clean up, and only then assert: an assert firing
     * before the env restore leaks PMM_CACHE_DIR into every later test in the
     * process, turning one regression into a cascade. */
    bool saved = pmm_ui_config_save(&new_generation);
...
TEST(config_overwrite) {
    ...
    pmm_ui_config_t cfg1 = {.ui_enabled = true, .ui_port = 9749};
    ASSERT_TRUE(pmm_ui_config_save(&cfg1));

    /* Overwrite with ui_enabled=false */
    pmm_ui_config_t cfg2 = {.ui_enabled = false, .ui_port = 9749};
    ASSERT_TRUE(pmm_ui_config_save(&cfg2));
...
TEST(config_corrupt_file) {
    ...
    char dir[1024];
    snprintf(dir, sizeof(dir), "%s/.cache/project-mind-mcp", td);
...
TEST(embedded_lookup_not_found) {
    ...
    const pmm_embedded_file_t *f = pmm_embedded_lookup("/nonexistent");
    ASSERT_NULL(f);
    PASS();
}
...
TEST(embedded_stub_count) {
    ...
    ASSERT_EQ(PMM_EMBEDDED_FILE_COUNT, 0);
    PASS();
}
...
TEST(layout_empty_graph) {
    ...
    pmm_layout_result_t *r =
        pmm_layout_compute(store, "test-project", PMM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NOT_NULL(r);
...
TEST(layout_single_node) {
    ...
    pmm_store_upsert_project(store, "test", "/tmp/test");
    pmm_node_t node = {
        .project = "test",
        .label = "Function",
        .name = "main",
    ...
    int64_t id = pmm_store_upsert_node(store, &node);
    ASSERT_GT(id, 0);

    pmm_layout_result_t *r = pmm_layout_compute(store, "test", PMM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NOT_NULL(r);
...
TEST(layout_two_connected) {
    ...
    pmm_node_t n2 = {.project = "test",
                     ...
                     .start_line = 1,
                     .end_line = 5};
    int64_t id1 = pmm_store_upsert_node(store, &n1);
    int64_t id2 = pmm_store_upsert_node(store, &n2);

    pmm_edge_t edge = {.project = "test", .source_id = id1, .target_id = id2, .type = "CALLS"};
    pmm_store_insert_edge(store, &edge);
...
TEST(layout_respects_max_nodes) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        pmm_node_t n = {.project = "test",
                        ...
                        .start_line = i,
                        .end_line = i + 1};
        pmm_store_upsert_node(store, &n);
    }

    /* max_nodes=5 should return at most 5 */
    pmm_layout_result_t *r = pmm_layout_compute(store, "test", PMM_LAYOUT_OVERVIEW, NULL, 0, 5);
    ASSERT_NOT_NULL(r);
...
TEST(layout_clamps_render_cap_from_env) {
    ...
    const char *old_raw = getenv("PMM_UI_MAX_RENDER_NODES");
    char *old_cap = old_raw ? strdup(old_raw) : NULL;
    pmm_setenv("PMM_UI_MAX_RENDER_NODES", "25", 1);
    ...
    pmm_layout_result_t *r = pmm_layout_compute(store, "test", PMM_LAYOUT_OVERVIEW, NULL, 0, 50000);
    ASSERT_NOT_NULL(r);
    ...
    pmm_store_close(store);
    if (old_cap) {
        pmm_setenv("PMM_UI_MAX_RENDER_NODES", old_cap, 1);
        free(old_cap);
    } else {
        pmm_unsetenv("PMM_UI_MAX_RENDER_NODES");
    }
    PASS();
}
...
TEST(layout_honors_budget_above_default) {
    ...
    pmm_layout_result_t *r =
        pmm_layout_compute(store, "test", PMM_LAYOUT_OVERVIEW, NULL, 0, BUDGET_NODES);
    ASSERT_NOT_NULL(r);
...
TEST(layout_deterministic) {
    ...
    pmm_node_t n2 = {.project = "test",
                     ...
                     .start_line = 1,
                     .end_line = 5};
    pmm_store_upsert_node(store, &n1);
    pmm_store_upsert_node(store, &n2);

    /* Run twice, check positions match */
    pmm_layout_result_t *r1 = pmm_layout_compute(store, "test", PMM_LAYOUT_OVERVIEW, NULL, 0, 100);
    pmm_layout_result_t *r2 = pmm_layout_compute(store, "test", PMM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NOT_NULL(r1);
    ASSERT_NOT_NULL(r2);
...
TEST(layout_to_json) {
    ...
    char *json = pmm_layout_to_json(r);
...
TEST(layout_null_inputs) {
    ...
    pmm_layout_result_t *r = pmm_layout_compute(NULL, "test", PMM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NULL(r);

    /* NULL project → NULL result */
    pmm_store_t *store = pmm_store_open_memory();
    r = pmm_layout_compute(store, NULL, PMM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NULL(r);

    /* pmm_layout_free(NULL) should not crash */
    pmm_layout_free(NULL);

    /* pmm_layout_to_json(NULL) should return NULL */
    char *json = pmm_layout_to_json(NULL);
    ASSERT_NULL(json);
...
static const pmm_layout_node_t *find_layout_node(const pmm_layout_result_t *r, const char *name) {
...
TEST(layout_dead_code_classification) {
    pmm_store_t *store = pmm_store_open_memory();
    ASSERT_NOT_NULL(store);
    ASSERT_EQ(pmm_store_upsert_project(store, "dc", "/tmp/dc"), PMM_STORE_OK);

    /* Candidates (Function, non-test path unless noted). */
    pmm_node_t dead = {.project = "dc",
                       .label = "Function",
                       .name = "deadfn",
    ...
    int64_t id_dead = pmm_store_upsert_node(store, &dead);
    pmm_store_upsert_node(store, &entry);
    pmm_store_upsert_node(store, &tst);
    pmm_store_upsert_node(store, &tstpath);
    pmm_store_upsert_node(store, &exp);
    int64_t id_single = pmm_store_upsert_node(store, &single);
    int64_t id_norm = pmm_store_upsert_node(store, &norm);
    int64_t id_caller = pmm_store_upsert_node(store, &caller);
    pmm_store_upsert_node(store, &cls);
    ASSERT_GT(id_dead, 0);
    ...
    pmm_edge_t e3 = {.project = "dc", .source_id = id_dead, .target_id = id_norm, .type = "CALLS"};
    pmm_store_insert_edge(store, &e1);
    pmm_store_insert_edge(store, &e2);
    pmm_store_insert_edge(store, &e3);

    pmm_layout_result_t *r = pmm_layout_compute(store, "dc", PMM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NOT_NULL(r);

    const pmm_layout_node_t *ln;
...
static void layout_octree_guard_child(void) {
    ...
    if (!store)
        _exit(2);
    if (pmm_store_upsert_project(store, "test", "/tmp/test") != PMM_STORE_OK)
        _exit(2);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        pmm_node_t n = {.project = "test",
                        ...
                        .start_line = i + 1,
                        .end_line = i + 2};
        if (pmm_store_upsert_node(store, &n) <= 0)
            _exit(2);
    }
