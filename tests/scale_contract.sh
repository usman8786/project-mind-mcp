...
#
# This tier runs the COMPILED binary on the pmm-bench-validate repos and asserts
...
#   PMM_SCALE_INCLUDE_C=1 bash tests/scale_contract.sh c   # add the slow C tier
#   PMM_BENCH_DIR=/path bash tests/scale_contract.sh       # override repo root
...
BENCH="${PMM_BENCH_DIR:-$HOME/pmm-bench-validate}"
PY="${PYTHON:-python3.9}"
FAILURES=0
RAN=0
...
# project name == pmm_project_name_from_path(): strip leading '/', map every
...
assert_lang() {
    ...
    rm -f "$HOME/.cache/project-mind-mcp/$proj.db"*
...
if [ "${#LANGS[@]}" -eq 0 ]; then
    ...
    [ "${PMM_SCALE_INCLUDE_C:-0}" = "1" ] && LANGS+=(c)
fi
