...
static int setup_parallel_repo(void) {
    snprintf(g_par_tmpdir, sizeof(g_par_tmpdir), "/tmp/pmm_par_XXXXXX");
    if (!pmm_mkdtemp(g_par_tmpdir))
        return -1;
...
static void harness_ctx_free_tables(pmm_pipeline_ctx_t *ctx) {
...
static pmm_gbuf_t *run_sequential(const char *project, const char *repo_path,
                                  pmm_file_info_t *files, int file_count) {
    pmm_gbuf_t *gbuf = pmm_gbuf_new(project, repo_path);
    ...
    pmm_init();
    pmm_pipeline_pass_definitions(&ctx, files, file_count);
    pmm_pipeline_pass_calls(&ctx, files, file_count);
    pmm_pipeline_pass_usages(&ctx, files, file_count);
    pmm_pipeline_pass_semantic(&ctx, files, file_count);
...
static pmm_gbuf_t *run_parallel_with_extract_opts(const char *project, const char *repo_path,
                                                  pmm_file_info_t *files, int file_count,
                                                  int worker_count,
                                                  const pmm_parallel_extract_opts_t *extract_opts) {
    pmm_gbuf_t *gbuf = pmm_gbuf_new(project, repo_path);
    pmm_registry_t *reg = pmm_registry_new();
    atomic_int cancelled;
    atomic_init(&cancelled, 0);
    ...
    int64_t gbuf_next = pmm_gbuf_next_id(gbuf);
    ...
    pmm_init();
    if (extract_opts) {
        pmm_parallel_extract_ex(&ctx, files, file_count, result_cache, &shared_ids, worker_count,
                                extract_opts);
    } else {
        pmm_parallel_extract(&ctx, files, file_count, result_cache, &shared_ids, worker_count);
    }
    pmm_gbuf_set_next_id(gbuf, atomic_load(&shared_ids));

    pmm_build_registry_from_cache(&ctx, files, file_count, result_cache);
     ...
     * pmm_parallel_resolve where the fused resolve_worker invokes
     * pmm_pxc_run_one(_ts) per file BEFORE materializing CALLS edges. */
    char **def_modules = (char **)calloc((size_t)file_count, sizeof(char *));
    int def_count = 0;
    CBMLSPDef *all_defs = def_modules
                              ? pmm_pxc_collect_all_defs(result_cache, files, file_count,
    ...
    pmm_parallel_resolve(&ctx, files, file_count, result_cache, &shared_ids, worker_count, all_defs,
    ...
    for (int i = 0; i < file_count; i++)
        if (result_cache[i])
            pmm_free_result(result_cache[i]);
    free(result_cache);
...
static pmm_gbuf_t *run_parallel(const char *project, const char *repo_path, pmm_file_info_t *files,
...
static pmm_gbuf_t *g_seq_gbuf = NULL;
static pmm_gbuf_t *g_par_gbuf = NULL;
...
static int ensure_parity_setup(void) {
    ...
    /* Discover files */
    pmm_discover_opts_t opts = {.mode = PMM_MODE_FULL};
    pmm_file_info_t *files = NULL;
    int file_count = 0;
    if (pmm_discover(g_par_tmpdir, &opts, &files, &file_count) != 0)
        return -1;
...
static void parity_teardown(void) {
    if (g_seq_gbuf) {
        pmm_gbuf_free(g_seq_gbuf);
        g_seq_gbuf = NULL;
    }
    if (g_par_gbuf) {
        pmm_gbuf_free(g_par_gbuf);
        g_par_gbuf = NULL;
    }
...
TEST(parallel_node_count) {
    if (ensure_parity_setup() != 0)
        FAIL("setup failed");
    int seq = pmm_gbuf_node_count(g_seq_gbuf);
    int par = pmm_gbuf_node_count(g_par_gbuf);
    ASSERT_GT(seq, 0);
    ASSERT_EQ(seq, par);
    PASS();
}
...
static int assert_edge_type_parity(const char *type) {
    if (ensure_parity_setup() != 0)
        return -1;
    int seq = pmm_gbuf_edge_count_by_type(g_seq_gbuf, type);
    int par = pmm_gbuf_edge_count_by_type(g_par_gbuf, type);
    if (seq != par) {
...
TEST(parallel_semantic_fixture_expected_counts) {
    ...
    ASSERT_EQ(pmm_gbuf_edge_count_by_type(g_seq_gbuf, "IMPLEMENTS"), 1);
    ASSERT_EQ(pmm_gbuf_edge_count_by_type(g_par_gbuf, "IMPLEMENTS"), 1);
    ASSERT_EQ(pmm_gbuf_edge_count_by_type(g_seq_gbuf, "INHERITS"), 1);
    ASSERT_EQ(pmm_gbuf_edge_count_by_type(g_par_gbuf, "INHERITS"), 1);
    /* Circle.area overrides Shape.area; Base.area overrides Circle.area. */
    ASSERT_EQ(pmm_gbuf_edge_count_by_type(g_seq_gbuf, "OVERRIDE"), 2);
    ASSERT_EQ(pmm_gbuf_edge_count_by_type(g_par_gbuf, "OVERRIDE"), 2);
    PASS();
}
...
TEST(parallel_total_edges) {
    if (ensure_parity_setup() != 0)
        FAIL("setup failed");
    int seq = pmm_gbuf_edge_count(g_seq_gbuf);
    int par = pmm_gbuf_edge_count(g_par_gbuf);
    ASSERT_GT(seq, 0);
    ASSERT_EQ(seq, par);
    PASS();
}
...
TEST(parallel_empty_files) {
    pmm_gbuf_t *gbuf = pmm_gbuf_new("empty-proj", "/tmp");
    pmm_registry_t *reg = pmm_registry_new();
    atomic_int cancelled;
    atomic_init(&cancelled, 0);
    ...
    int rc = pmm_parallel_extract(&ctx, NULL, 0, cache, &shared_ids, 2);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(pmm_gbuf_node_count(gbuf), 0);
 ...
 * position exceed the fixed PMM_SZ_2K `props` stack buffer in
...
TEST(parallel_args_json_no_overflow) {
    char dir[256];
    snprintf(dir, sizeof(dir), "/tmp/pmm_argov_XXXXXX");
    ASSERT_TRUE(pmm_mkdtemp(dir) != NULL);
    ...
    pmm_file_info_t *files = NULL;
    int file_count = 0;
    ASSERT_EQ(pmm_discover(dir, &opts, &files, &file_count), 0);
    ASSERT_GT(file_count, 0);

    pmm_gbuf_t *gbuf = run_parallel("argov-test", dir, files, file_count, 4);
    ASSERT_TRUE(gbuf != NULL);
    ASSERT_GT(pmm_gbuf_edge_count(gbuf), 0);
...
TEST(gbuf_shared_ids_unique) {
    ...
    pmm_gbuf_t *ga = pmm_gbuf_new_shared_ids("proj", "/", &shared);
    pmm_gbuf_t *gb = pmm_gbuf_new_shared_ids("proj", "/", &shared);

    int64_t id1 = pmm_gbuf_upsert_node(ga, "Function", "foo", "proj.foo", "a.go", 1, 5, "{}");
    int64_t id2 = pmm_gbuf_upsert_node(gb, "Function", "bar", "proj.bar", "b.go", 1, 3, "{}");
    ASSERT_GT(id1, 0);
    ASSERT_GT(id2, 0);
    ASSERT_NEQ(id1, id2);
...
TEST(gbuf_merge_nodes) {
    _Atomic int64_t shared = 1;
    pmm_gbuf_t *dst = pmm_gbuf_new_shared_ids("proj", "/", &shared);
    pmm_gbuf_t *src = pmm_gbuf_new_shared_ids("proj", "/", &shared);

    pmm_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    pmm_gbuf_upsert_node(dst, "Function", "b", "proj.b", "a.go", 6, 10, "{}");
    pmm_gbuf_upsert_node(src, "Function", "c", "proj.c", "b.go", 1, 5, "{}");
    pmm_gbuf_upsert_node(src, "Function", "d", "proj.d", "b.go", 6, 10, "{}");

    ASSERT_EQ(pmm_gbuf_node_count(dst), 2);
    pmm_gbuf_merge(dst, src);
    ASSERT_EQ(pmm_gbuf_node_count(dst), 4);

    ASSERT_NOT_NULL(pmm_gbuf_find_by_qn(dst, "proj.c"));
    ASSERT_NOT_NULL(pmm_gbuf_find_by_qn(dst, "proj.d"));
    /* dst originals still there */
    ASSERT_NOT_NULL(pmm_gbuf_find_by_qn(dst, "proj.a"));
    ASSERT_NOT_NULL(pmm_gbuf_find_by_qn(dst, "proj.b"));
...
TEST(gbuf_merge_edges) {
    ...
    int64_t a = pmm_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    int64_t b = pmm_gbuf_upsert_node(dst, "Function", "b", "proj.b", "a.go", 6, 10, "{}");
    /* Put an edge in src that references dst nodes (by ID) */
    pmm_gbuf_insert_edge(src, a, b, "CALLS", "{}");

    pmm_gbuf_merge(dst, src);
    ASSERT_GT(pmm_gbuf_edge_count(dst), 0);

    const pmm_gbuf_edge_t **edges = NULL;
    int count = 0;
    pmm_gbuf_find_edges_by_source_type(dst, a, "CALLS", &edges, &count);
    ASSERT_EQ(count, 1);
...
TEST(gbuf_merge_empty_src) {
    ...
    pmm_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    int before = pmm_gbuf_node_count(dst);
    pmm_gbuf_merge(dst, src);
    ASSERT_EQ(pmm_gbuf_node_count(dst), before);
...
TEST(gbuf_merge_src_free_safe) {
    ...
    pmm_gbuf_upsert_node(src, "Function", "x", "proj.x", "x.go", 1, 5, "{}");
    pmm_gbuf_merge(dst, src);
    pmm_gbuf_free(src); /* must not crash */

    /* dst node still accessible */
    ASSERT_NOT_NULL(pmm_gbuf_find_by_qn(dst, "proj.x"));
    pmm_gbuf_free(dst);
    PASS();
}

TEST(gbuf_next_id_accessors) {
    pmm_gbuf_t *gb = pmm_gbuf_new("proj", "/");
    ASSERT_EQ(pmm_gbuf_next_id(gb), 1);

    pmm_gbuf_upsert_node(gb, "Function", "foo", "proj.foo", "f.go", 1, 5, "{}");
    ASSERT_GT(pmm_gbuf_next_id(gb), 1);

    pmm_gbuf_set_next_id(gb, 100);
    int64_t id = pmm_gbuf_upsert_node(gb, "Function", "bar", "proj.bar", "f.go", 6, 10, "{}");
    ASSERT_GTE(id, 100);
 ...
 * pass_parallel.c (parallel) on pmm_pipeline_find_lsp_resolution +
 * PMM_LSP_CONFIDENCE_FLOOR (lsp_resolve.h). Before the unification, the
...
static void count_lsp_call_edges(const pmm_gbuf_edge_t *edge, void *ud) {
...
static const pmm_gbuf_node_t *find_unique_callable_node_by_tail(const pmm_gbuf_t *gbuf,
                                                                const char *tail) {
    ...
    int count = 0;
    if (pmm_gbuf_find_by_name(gbuf, method, &nodes, &count) != 0) {
        return NULL;
    }
    const pmm_gbuf_node_t *match = NULL;
    for (int i = 0; i < count; i++) {
        const pmm_gbuf_node_t *node = nodes[i];
...
static const pmm_gbuf_edge_t *find_calls_edge_by_tails(const pmm_gbuf_t *gbuf,
                                                       const char *source_tail,
                                                       const char *target_tail) {
    const pmm_gbuf_node_t *source = find_unique_callable_node_by_tail(gbuf, source_tail);
    const pmm_gbuf_node_t *target = find_unique_callable_node_by_tail(gbuf, target_tail);
    if (!source || !target) {
        return NULL;
    }
    ...
    const pmm_gbuf_edge_t **edges = NULL;
    int count = 0;
    if (pmm_gbuf_find_edges_by_source_type(gbuf, source->id, "CALLS", &edges, &count) != 0) {
        return NULL;
    }
    for (int i = 0; i < count; i++) {
...
TEST(parallel_java_kotlin_lsp_override_cross_file_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_par_jvm_XXXXXX");
 ...
 * pmm_pipeline_lsp_allow_tail_match must keep the fallbacks OFF there.
 *
...
TEST(parallel_lsp_tail_match_fallbacks_gated_to_jvm) {
    ...
    ASSERT_TRUE(pmm_pipeline_lsp_allow_tail_match(PMM_LANG_JAVA));
    ASSERT_TRUE(pmm_pipeline_lsp_allow_tail_match(PMM_LANG_KOTLIN));
    ASSERT_TRUE(!pmm_pipeline_lsp_allow_tail_match(PMM_LANG_PYTHON));
    ASSERT_TRUE(!pmm_pipeline_lsp_allow_tail_match(PMM_LANG_GO));
    ASSERT_TRUE(!pmm_pipeline_lsp_allow_tail_match(PMM_LANG_TYPESCRIPT));
    ASSERT_TRUE(!pmm_pipeline_lsp_allow_tail_match(PMM_LANG_CPP));
    ...
    ASSERT_TRUE(pmm_pipeline_find_lsp_resolution(&arr, &call, false) == NULL);
    ASSERT_TRUE(pmm_pipeline_find_lsp_resolution(&arr, &call, true) == &rc_item);
    ...
    pmm_gbuf_t *tgbuf = pmm_gbuf_new("proj", "/tmp");
    ASSERT_NOT_NULL(tgbuf);
    int64_t nid = pmm_gbuf_upsert_node(tgbuf, "Method", "run", "proj.zeta.Helper.run",
                                       "zeta/helper.py", 1, 3, NULL);
    ASSERT_TRUE(nid != 0);
    ASSERT_TRUE(pmm_pipeline_lsp_target_node(tgbuf, "proj", "com.other.Helper.run", false) == NULL);
    const pmm_gbuf_node_t *jvm_hit =
        pmm_pipeline_lsp_target_node(tgbuf, "proj", "com.other.Helper.run", true);
...
TEST(parallel_python_lsp_override_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_par_pylsp_XXXXXX");
    if (!pmm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    pmm_file_info_t files[1] = {0};
    files[0].path = fpath0;
    files[0].rel_path = (char *)"app.py";
    files[0].language = PMM_LANG_PYTHON;

    pmm_gbuf_t *gbuf = run_parallel("pmm_par_pylsp", tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(gbuf);

    lsp_edge_count_ctx_t c = {0};
    pmm_gbuf_foreach_edge(gbuf, count_lsp_call_edges, &c);
     ...
     * CALLS edge. This proves the unified pmm_pipeline_find_lsp_resolution
 ...
 * Before pmm_pipeline_lsp_target_node added the project-prefix fallback,
 * the LSP match succeeded (lsp_overrides counter incremented) but the
 * downstream pmm_gbuf_find_by_qn lookup missed silently, dropping the
...
TEST(parallel_python_lsp_override_cross_file_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_par_pylsp_xf_XXXXXX");
    if (!pmm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    files[1].language = PMM_LANG_PYTHON;

    pmm_gbuf_t *gbuf = run_parallel("pmm_par_pylsp_xf", tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(gbuf);
     ...
     * pmm_pipeline_lsp_target_node this assertion would fail because the
...
TEST(parallel_cross_file_reread_preserves_unretained_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_par_xf_reread_XXXXXX");
    if (!pmm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    const pmm_parallel_extract_opts_t no_retain = {
        .retain_sources = false,
        .retain_sources_set = true,
    };
    const pmm_parallel_extract_opts_t over_cap = {
        .retain_sources = true,
        .retain_sources_set = true,
        .retain_per_file_max_bytes = 1, /* 1 byte → every file dropped by the size cap */
    };
    const pmm_parallel_extract_opts_t *scenarios[3] = {NULL, &no_retain, &over_cap};

    for (int s = 0; s < 3; s++) {
        pmm_gbuf_t *gbuf = run_parallel_with_extract_opts("com", tmpdir, files, 2, 2, scenarios[s]);
        ASSERT_NOT_NULL(gbuf);
...
/* ── Shared "::" normalization in pmm_pipeline_find_lsp_resolution (QA F3) ─
 *
 ...
 * against pmm_pipeline_find_lsp_resolution: (1) a qualified static call still
...
TEST(lsp_resolve_qualified_static_call_normalizes_colons) {
    ...
    const CBMResolvedCall *hit = pmm_pipeline_find_lsp_resolution(&arr, &call, false);
    ASSERT(hit != NULL);
    ...
    const CBMResolvedCall *bare_hit = pmm_pipeline_find_lsp_resolution(&arr, &bare, false);
...
TEST(lsp_resolve_misattribution_is_bounded) {
    ...
    const CBMResolvedCall *other_hit = pmm_pipeline_find_lsp_resolution(&arr, &other, false);
    ...
    ASSERT(pmm_pipeline_find_lsp_resolution(&arr, &absent, false) == NULL);
    PASS();
}
