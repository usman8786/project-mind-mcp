 ...
 * ──────────────────────
 * The hybrid LSP TYPE-RESOLUTION PASS (pass_lsp_cross.c / pmm_run_X_lsp_cross)
 * is wired for: Go, C, C++, CUDA, Python, JavaScript, TypeScript, TSX, PHP.
 * It is NOT wired (pmm_pxc_has_cross_lsp returns false) for:
 *   - Rust   — pmm_run_rust_lsp_cross EXISTS in rust_lsp.c but is never called
 *   - C#     — pmm_run_cs_lsp_cross EXISTS in cs_lsp.c but is never called
 *   - Java   — pmm_run_java_lsp_cross EXISTS in java_lsp.h but is never called
 *   - Kotlin — NO cross-file LSP function; uses per-file pmm_run_kotlin_lsp only
 *
...
static pmm_store_t *lrp_open_indexed(LRP_Proj *lp) {
...
static pmm_store_t *lrp_index(LRP_Proj *lp, const LRP_File *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_lrp_XXXXXX");
    if (!pmm_mkdtemp(lp->tmpdir)) return NULL;
...
static void lrp_cleanup(LRP_Proj *lp, pmm_store_t *store) {
    if (store) pmm_store_close(store);
    if (lp->srv) { pmm_mcp_server_free(lp->srv); lp->srv = NULL; }
...
static int lrp_usage(LRP_Proj *lp, const LRP_File *files, int nfiles) {
    pmm_store_t *store = lrp_index(lp, files, nfiles);
    int n = store ? pmm_store_count_edges_by_type(store, lp->project, "USAGE") : -1;
...
static void lrp_diag(pmm_store_t *store, const char *project, const char *scenario) {
    if (!store) { fprintf(stderr, "    [LRP] %s: no graph DB\n", scenario); return; }
    char line[512] = {0};
    for (int i = 0; LRP_ALL_EDGE_TYPES[i]; i++) {
        int c = pmm_store_count_edges_by_type(store, project, LRP_ALL_EDGE_TYPES[i]);
...
static int lrp_assert_calls(const LRP_File *files, int nfiles, int min_calls,
    ...
    LRP_Proj lp;
    pmm_store_t *store = lrp_index(&lp, files, nfiles);
    int got = store ? pmm_store_count_edges_by_type(store, lp.project, "CALLS") : -1;
    if (got < min_calls) {
 ...
 * ══════════════════════════════════════════════════════════════════════
 * C has lsp_cross via pmm_run_c_lsp_cross.  Function-pointer dispatch,
...
TEST(lrp_c_s4_static_local) {
    ...
    LRP_Proj lp;
    pmm_store_t *store = lrp_index(&lp, f, 2);
...
TEST(lrp_cpp_s4_static_method) {
     ...
     * internal/pmm/lsp (pmm_run_c_lsp_cross, cpp path). */
...
TEST(lrp_cpp_s6_virtual_inherited) {
     ...
     * scope-qualified calls (internal/pmm/lsp, pmm_run_c_lsp_cross cpp path). */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * pmm_run_rust_lsp_cross EXISTS in rust_lsp.c but pmm_pxc_has_cross_lsp
 * returns false for PMM_LANG_RUST — the pass is NEVER called by the
 ...
 * outcome; they FAIL until pmm_pxc_has_cross_lsp is updated to include Rust).
 *
 * ROOT CAUSE: pass_lsp_cross.c pmm_pxc_has_cross_lsp() switch missing
 *   PMM_LANG_RUST case.
 ...
 * NOT resolve → calls=0.  pmm_registry_resolve (src/pipeline/registry.c:638)
 ...
 * lsp_cross is not wired (pass_lsp_cross.c:pmm_pxc_has_cross_lsp lacks
 * PMM_LANG_RUST).  Fix either the '::' split or wire Rust lsp_cross. */
...
TEST(lrp_rust_s2_method_dispatch) {
     ...
     * Root cause: pmm_pxc_has_cross_lsp returns false for PMM_LANG_RUST.
...
TEST(lrp_rust_s7_generic) {
     ...
     * `algo::max_of(...)` is not resolved → calls=0.  pmm_registry_resolve
     ...
     * (pass_lsp_cross.c:pmm_pxc_has_cross_lsp lacks PMM_LANG_RUST). */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * Python has pmm_run_py_lsp_cross.  Type inference is limited by duck
 ...
 * ══════════════════════════════════════════════════════════════════════
 * TypeScript has pmm_run_ts_lsp_cross.  Static types make S2–S8 more
...
TEST(lrp_ts_s3_constructor) {
     ...
     * Python constructor path in internal/pmm/lsp vs the TS new_expression path. */
...
TEST(lrp_ts_s6_inherited_method) {
    ...
    int calls = pmm_store_count_edges_by_type(store, lp.project, "CALLS");
    lrp_cleanup(&lp, store);
    ASSERT_EQ(calls, 0);
    PASS();
}
 ...
 * ══════════════════════════════════════════════════════════════════════
 * pmm_run_java_lsp_cross EXISTS (java_lsp.h) but pmm_pxc_has_cross_lsp
 * returns false for PMM_LANG_JAVA.
 * S1 (plain call via Util.method()): GREEN via name-based resolver.
 * S2–S8: RED until pmm_pxc_has_cross_lsp is updated.
 *
 * ROOT CAUSE: pass_lsp_cross.c pmm_pxc_has_cross_lsp() missing PMM_LANG_JAVA.
 * FIX LOCATION: src/pipeline/pass_lsp_cross.c line ~280.
 */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * Kotlin has pmm_run_kotlin_lsp (per-file) but NO pmm_run_kotlin_lsp_cross.
 ...
 * pmm_run_kotlin_lsp is never invoked with cross-file project-wide defs.
 * FIX LOCATION: internal/pmm/lsp/kotlin_lsp.c — add pmm_run_kotlin_lsp_cross,
 ...
 * Kotlin has no pmm_run_kotlin_lsp_cross and the generic name resolver does not
 * link this Kotlin cross-file call.  Fix = add Kotlin cross-file LSP
 * (internal/pmm/lsp/kotlin_lsp.c) and wire it in pass_lsp_cross.c. */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * pmm_run_cs_lsp_cross EXISTS (cs_lsp.c) and the CBMCrossLspRegistries
 * struct even has a `cs` field — but pmm_pxc_has_cross_lsp returns false
 * for PMM_LANG_CSHARP.  The cross-LSP pass is never invoked for C# files.
 *
 * ROOT CAUSE: pass_lsp_cross.c pmm_pxc_has_cross_lsp() missing PMM_LANG_CSHARP.
 * FIX LOCATION: src/pipeline/pass_lsp_cross.c line ~280.
 * NOTE: CBMCrossLspRegistries.cs is already defined and pmm_pxc_registry_for_lang
 * handles CSHARP — so the fix is a 1-line addition to pmm_pxc_has_cross_lsp().
 */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * PHP has pmm_run_php_lsp_cross.  Key scenarios: $this->method(),
...
SUITE(lsp_resolution_probe) {
    ...
    /* ── C# (lsp_cross NOT WIRED — but pmm_run_cs_lsp_cross EXISTS) ── */
