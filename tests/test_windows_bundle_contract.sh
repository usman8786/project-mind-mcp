...
# permanent launcher (project-mind-mcp.exe) plus the real product binary
# (project-mind-mcp.payload.exe). The launcher existed for exactly one
...
# install.ps1, which runs while PMM is NOT running. So this file asserts the
...
binary = "project-mind-mcp.exe"
payload = "project-mind-mcp.payload.exe"
...
shipped_surfaces = (
    ...
    "pkg/npm/install.js",
    "pkg/npm/bin.js",
    "pkg/pypi/src/project_mind_mcp/_cli.py",
...
makefile = read("Makefile.pmm")
require(
    "project-mind-mcp-launcher" not in makefile and "WINDOWS_LAUNCHER" not in makefile,
    "Makefile.pmm must not expose a Windows launcher target or variable",
)
...
for archive, call, ui in (
    ("project-mind-mcp-windows-amd64.zip", "scripts/package-release.sh windows amd64", False),
    ("project-mind-mcp-ui-windows-amd64.zip",
     "scripts/package-release.sh windows amd64 --variant ui", True),
    ("project-mind-mcp-windows-arm64.zip", "scripts/package-release.sh windows arm64", False),
    ("project-mind-mcp-ui-windows-arm64.zip",
...
# install.ps1 runs while PMM is not running, renames the installed binary to
...
single_binary_contracts = {
    "pkg/npm/install.js": (
        r"const\s+WINDOWS_BINARY_NAME\s*=\s*['\"]project-mind-mcp\.exe['\"]",
        r"installWindowsBinaryAtomically\(",
        r"windowsBinaryReady\(",
    ),
    "pkg/npm/bin.js": (
        r"binName\s*=\s*isWindows\s*\?\s*['\"]project-mind-mcp\.exe['\"]",
        r"const\s+executionPath\s*=\s*binPath",
    ),
    "pkg/pypi/src/project_mind_mcp/_cli.py": (
        r"_WINDOWS_BINARY_NAME\s*=\s*['\"]project-mind-mcp\.exe['\"]",
...
for relative, patterns in single_binary_contracts.items():
    ...
    require(
        ".pmm/generations" not in source and "current-v1" not in source,
...
guidance_contracts = {
    "pkg/npm/bin.js": (
        "npm install project-mind-mcp@latest",
        "npm uninstall project-mind-mcp",
        "project-mind-mcp install --yes",
    ),
    "pkg/pypi/src/project_mind_mcp/_cli.py": (
        "python -m pip install --upgrade project-mind-mcp",
        "python -m pip uninstall project-mind-mcp",
        "install --yes",
    ),
}
...
require(
    all(
        ...
        for needle in (
            ...
            '("pmm-windows-guards-root-" + [guid]::NewGuid().ToString("N"))',
            '$env:TEMP = $guardRoot',
            '$env:TMP = $guardRoot',
...
for relative, source in (
    ...
    require(
        "PMM_VM_HOST_KEY_SHA256" in source,
...
require(
    ...
    and "PMM_WIN_VM_SSH" not in read("test-infrastructure/run.sh"),
...
require(
    ...
    and "pmm_vm_write_untracked_manifest" in sync_case.group("body")
...
require(
    sync_case is not None
    and 'remote_head="$(vm clangarm64 "cd /c/pmm && git rev-parse --verify HEAD")"'
...
require(
    'scripts/smoke-test.sh "$SMOKE_DIR/project-mind-mcp.exe"' in vm_smoke,
    f"Windows smoke wrapper must execute the canonical {binary}",
)
require(
    "bash test-infrastructure/vm/vm-smoke.sh" in windows_smoke
    and 'PMM_SMOKE_ARTIFACT_DIR="$(cygpath -u "$RUNNER_TEMP")/pmm-artifact"' in windows_smoke,
...
require(
    all(
        ...
        for needle in (
            ...
            'SMOKE_DIR="$(mktemp -d "$PROFILE_ROOT/pmm-vm-smoke.XXXXXX")"',
            'cp "$BINARY_SRC" "$SMOKE_DIR/project-mind-mcp.exe"',
            'PMM_CACHE_DIR="$(cygpath -m "$SMOKE_DIR/cache")"',
...
windows_release_version_blocks = [
    ...
    if 'LAUNCH_DIR="$(mktemp -d "$PROFILE_ROOT/pmm-release-version.XXXXXX")"' in block
]
require(
    ...
    and all(
        ...
        for needle in (
            'PROFILE_ROOT="$(cygpath -u "$USERPROFILE")"',
            'cp "$ARTIFACT_DIR/project-mind-mcp.exe" "$LAUNCH_DIR/"',
            '"$LAUNCH_DIR/project-mind-mcp.exe" --version',
        )
    ),
...
runner_temp_allowed = (
    re.compile(r'ARTIFACT_DIR="\$\(cygpath -u "\$RUNNER_TEMP"\)/pmm-artifact"'),
    re.compile(r'Join-Path \$env:RUNNER_TEMP "pmm-artifact"'),
)
...
windows_release_security_blocks = [
    re.sub(r"\s+", " ", re.sub(r"\\\s*\n\s*", " ", block)).strip()
    for block in smoke_blocks
    if 'scripts/security-install.sh "$SECURITY_DIR/project-mind-mcp.exe"' in block
]
require(
    ...
    and all(
        ...
        for needle in (
            'PROFILE_ROOT="$(cygpath -u "$USERPROFILE")"',
            'SECURITY_DIR="$(mktemp -d "$PROFILE_ROOT/pmm-release-security.XXXXXX")"',
            'cp "$ARTIFACT_DIR/project-mind-mcp.exe" "$SECURITY_DIR/"',
            'TMPDIR="$SECURITY_DIR" '
            'scripts/security-install.sh "$SECURITY_DIR/project-mind-mcp.exe"',
        )
    ),
...
require(
    'copy_smoke_binary "$FAKE_HOME/.local/bin/project-mind-mcp.exe"' not in smoke_script
    and 'copy_smoke_binary "$UPDATE_HOME/.local/bin/project-mind-mcp.exe"' not in smoke_script,
...
require(
    ...
    and 'UPDATE_DOWNLOAD_URL="file:///$UPDATE_FIXTURE_DIR"' in smoke_script
    and 'PMM_DOWNLOAD_URL="$UPDATE_DOWNLOAD_URL"' in smoke_script,
...
require(
    'PMM_DOWNLOAD_URL="$SMOKE_DOWNLOAD_URL"' in smoke_script
...
require(
    ...
    and smoke_workflow.count("PMM_SMOKE_ARTIFACT_DIR") >= 2,
    "Unix and Windows release smoke must identify their local update fixture via the "
    "canonical wrappers",
)

cli_source = read("src/cli/cli.c")
probe_start = cli_source.find("pmm_json_mcp_probe_windows_command_path(")
...
require(
    re.search(r'pmm_safe_getenv\s*\(\s*"PMM_DOWNLOAD_URL"', file_override) is not None
...
download_helpers = cli_source[
    cli_source.find("static int pmm_download_to_file(") : cli_source.find("/* ── macOS ad-hoc signing")
]
require(
...
update_start = cli_source.find("int pmm_cmd_update(int argc, char **argv) {")
...
require(
    ...
    "pmm_cmd_update must print the install.ps1 command on Windows",
)
require(
    "pmm_windows_launcher" not in cli_source
