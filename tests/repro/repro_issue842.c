 ...
 *
 * Both called pmm_gbuf_find_by_qn(enclosing_func_qn) directly, with no guard
...
static int collect_edge_sources(const RFile *files, int nfiles, const char *node_name,
    ...
    int ncand = 0;
    int rc = pmm_store_find_nodes_by_name(store, lp.project, node_name, &candidates, &ncand);
    ...
    int nedges = 0;
    rc = pmm_store_find_edges_by_target_type(store, target_id, edge_type, &edges, &nedges);
    if (rc != PMM_STORE_OK) {
        rh_cleanup(&lp, store);
        return -1;
    }
 ...
 * direct pmm_gbuf_find_by_qn(enclosing_func_qn) with no dir-container guard,
 ...
 * pmm_gbuf_find_by_qn(enclosing_func_qn) with no dir-container guard and no
