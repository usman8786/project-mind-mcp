 ...
 *   Change the initializer in pmm_return_clause_t to use -1, update the
...
static pmm_store_t *setup_limit_store(void) {
    pmm_store_t *s = pmm_store_open_memory();
    if (!s) return NULL;
    pmm_store_upsert_project(s, "test", "/tmp/test");

    pmm_node_t n1 = {.project = "test", .label = "Function", .name = "HandleOrder",
                     .qualified_name = "test.HandleOrder", .file_path = "handler.go"};
    pmm_node_t n2 = {.project = "test", .label = "Function", .name = "ValidateOrder",
                     .qualified_name = "test.ValidateOrder", .file_path = "validate.go"};
    pmm_node_t n3 = {.project = "test", .label = "Function", .name = "SubmitOrder",
                     .qualified_name = "test.SubmitOrder", .file_path = "submit.go"};
    pmm_node_t n4 = {.project = "test", .label = "Function", .name = "LogError",
...
TEST(repro_new_cypher_limit_zero_returns_no_rows) {
    pmm_store_t *s = setup_limit_store();
    ASSERT_NOT_NULL(s);
    ...
    memset(&r, 0, sizeof(r));
    rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.name", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ...
    memset(&r, 0, sizeof(r));
    rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 0", "test", 0, &r);
    ASSERT_EQ(rc, 0);
