 ...
 *
 * We then call pmm_discover() in PMM_MODE_FULL (NULL opts) so FAST_SKIP_DIRS
...
/* Helper: create fixture, run pmm_discover, check canary. */
/* Returns:  0  canary NOT discovered (correct — directory skipped)            */
/*          >0  canary WAS discovered (bug — directory NOT in skip-list)       */
/*          -1  setup error                                                     */
static int check_dir_skipped(const char *dir_name, pmm_index_mode_t mode) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/pmm_disc_XXXXXX", pmm_tmpdir());
    ...
    int count = 0;
    int rc = pmm_discover(tmpdir, (mode == PMM_MODE_FULL) ? NULL : &opts, &files, &count);
    if (rc != 0) {
        th_rmtree(tmpdir);
        return -1;
    }
...
TEST(invariant_discovery_always_skip_dirs) {
    /*
     * Table of directory names that MUST be skipped in PMM_MODE_FULL.
    ...
    struct { const char *name; int expected_green; } cases[] = {
         ...
         * compound name was absent from ALWAYS_SKIP_DIRS, so pmm_discover()
    ...
    for (int i = 0; i < n; i++) {
        int result = check_dir_skipped(cases[i].name, PMM_MODE_FULL);
...
/* ── PART A TEST — FAST_SKIP_DIRS table (mode != PMM_MODE_FULL) ────────────
 *
 * FAST_SKIP_DIRS entries are only skipped when mode != PMM_MODE_FULL.
 * We test them in PMM_MODE_MODERATE to confirm they are guarded.
...
TEST(invariant_discovery_fast_skip_dirs) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int moderate = check_dir_skipped(fast_cases[i].name, PMM_MODE_MODERATE);
        if (moderate < 0) {
        ...
        int full = check_dir_skipped(fast_cases[i].name, PMM_MODE_FULL);
        if (full < 0) {
...
TEST(invariant_discovery_control_always_found) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/pmm_ctrl_XXXXXX", pmm_tmpdir());
    ...
    pmm_file_info_t *files = NULL;
    int count = 0;
    int rc = pmm_discover(tmpdir, NULL, &files, &count);
    ASSERT_EQ(0, rc);
 ...
 *   pmm_pipeline_fqn_compute() calls strip_file_extension() which removes
 *   everything from the last '.' in the basename.  pmm_fqn_compute() in
 ...
 *
 * Assertion for RED cases: after indexing, pmm_store_find_nodes_by_name()
...
static int count_nodes_by_name(pmm_store_t *store, const char *project,
                               const char *sym_name) {
    pmm_node_t *nodes = NULL;
    int node_count = 0;
    int rc = pmm_store_find_nodes_by_name(store, project, sym_name,
                                          &nodes, &node_count);
    if (rc != PMM_STORE_OK) {
        return -1;
    }
...
static int count_distinct_qns(pmm_store_t *store, const char *project,
                               const char *sym_name) {
    pmm_node_t *nodes = NULL;
    int node_count = 0;
    int rc = pmm_store_find_nodes_by_name(store, project, sym_name,
...
TEST(invariant_fqn_api_h_api_c) {
    /* PARKED for release: api.h and api.c share a module QN because pmm_fqn
    ...
    RProj lp;
    pmm_store_t *store = rh_index_files(&lp, files, nfiles);
...
TEST(invariant_fqn_ts_vs_dts) {
    ...
    pmm_node_t *nodes = NULL;
    int node_count = 0;
    int rc = pmm_store_find_nodes_by_name(store, lp.project, "widget_fn",
                                          &nodes, &node_count);
    int distinct = 0;
    if (rc == PMM_STORE_OK && node_count > 1) {
