 ...
 * Python hybrid LSPs (internal/pmm/lsp/go_lsp.c, internal/pmm/lsp/py_lsp.c).
 *
 ...
 *   these can silently fail: pmm_pipeline_find_lsp_resolution joins each
 ...
 *   from the source (grep '"lsp_' internal/pmm/lsp/go_lsp.c), with its keying
 *   site:
 ...
 *   (grep '"lsp_' internal/pmm/lsp/py_lsp.c), with its keying site:
 ...
 * func_node is a bare identifier resolved via pmm_registry_lookup_symbol on the
 * package QN). */
 ...
 * resolves via pmm_registry_lookup_symbol on the module QN). */
 ...
 * (py_lsp.c:1620-1624: pmm_scope_lookup yields a NAMED type → emit constructor
 ...
 * MODULE and pmm_registry_lookup_symbol(module_qn, attr) hits → lsp_module_attr).
 ...
 * pmm_registry_lookup_symbol("builtins", fname) hits). EXPECTED RED in a
 ...
 * constructor (py_lsp.c:1640-1643: pmm_registry_lookup_type("builtins.str")
...
TEST(repro_lsp_go_strategy_cross_file) {
     ...
     * cross-file pass (pmm_go_fast_resolve_qualified_calls), which runs only when
