 ...
 *
 *     if (repo_path && pmm_artifact_exists(repo_path)) {
 *         pmm_artifact_export(db_path, repo_path, project, PMM_ARTIFACT_FAST);
 *     }
 *
 ...
 *   but pmm_pipeline_run_incremental() calls the local dump_and_persist()
 *   which only checks pmm_artifact_exists(), not the pipeline flag.
 *
 ...
 *   prior artifact MUST create .project-mind-mcp/graph.db.zst after the run.
 *   pmm_artifact_exists(repo_path) MUST return true after the first
 ...
 *   written. pmm_artifact_exists() returns false. Only a SECOND call (when
 ...
 *   repo (no prior artifact). We then assert pmm_artifact_exists() returns
 *   true. On buggy code dump_and_persist() skips the export because
 *   pmm_artifact_exists() was false at the time of the check, so the
 *   assertion fires RED.
 *
 ...
 *   call pmm_artifact_export() when persistence=true, regardless of whether
 *   an artifact already exists. The existing auto-update branch should be
 *   merged with a new persistence-flag branch so that:
 *     if (repo_path && (persistence || pmm_artifact_exists(repo_path))) {
 *         pmm_artifact_export(...);
 *     }
...
TEST(repro_issue434_persistence_honored_on_first_create) {
     ...
     * pmm_mcp_get_bool_arg -> pmm_pipeline_set_persistence -> the pipeline. */
    RProj lp;
    memset(&lp, 0, sizeof(lp));

    /* Create a fresh temp directory for the fixture repo */
    snprintf(lp.tmpdir, sizeof(lp.tmpdir), "/tmp/pmm_repro434_XXXXXX");
    ...
    ASSERT_FALSE(pmm_artifact_exists(lp.tmpdir));
    ...
    if (!lp.srv) {
        th_rmtree(lp.tmpdir);
        FAIL("pmm_mcp_server_new failed");
    }
     ...
     * the artifact MUST exist in .project-mind-mcp/graph.db.zst.
     *
     * On buggy code (pipeline_incremental.c dump_and_persist only checks
     * pmm_artifact_exists() not p->persistence) the artifact is NOT written
     * on the first run, so pmm_artifact_exists() returns false here and this
    ...
    char art_dir[600];
    snprintf(art_dir, sizeof(art_dir), "%s/.project-mind-mcp", lp.tmpdir);
    th_rmtree(art_dir);
    th_rmtree(lp.tmpdir);
    ...
    if (proj) {
        ...
        if (!home) home = "/tmp";
        char dbpath[600];
        snprintf(dbpath, sizeof(dbpath), "%s/.cache/project-mind-mcp/%s.db",
                 home, proj);
        unlink(dbpath);
        free(proj);
    }
