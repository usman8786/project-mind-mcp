 ...
 *     - sequential: pmm_gbuf_upsert_node updated name/file_path/range in
 ...
 *   1. graph_buffer.c pmm_gbuf_upsert_node: a Module def colliding with a
 ...
 *      pmm_pipeline_node_is_dir_container) is treated as a miss, falling
 ...
 *   - sequential: pmm_gbuf_upsert_node clobbered the Folder node in place;
...
static int collect_usage_sources_n(const RFile *files, int nfiles, char **out, int cap) {
    ...
    pmm_node_t *candidates = NULL;
    int ncand = 0;
    int rc = pmm_store_find_nodes_by_name(store, lp.project, "IRepository",
                                         &candidates, &ncand);
    if (rc != PMM_STORE_OK || ncand == 0) {
        pmm_store_free_nodes(candidates, ncand);
        rh_cleanup(&lp, store);
        return -1;
    }
    ...
    int nedges = 0;
    rc = pmm_store_find_edges_by_target_type(store, target_id, "USAGE", &edges, &nedges);
    if (rc != PMM_STORE_OK) {
        rh_cleanup(&lp, store);
        return -1;
    }
    ...
    for (int i = 0; i < nedges && found < cap; i++) {
        pmm_node_t src_node;
        if (pmm_store_find_node_by_id(store, edges[i].source_id, &src_node) != PMM_STORE_OK) {
            continue;
        }
