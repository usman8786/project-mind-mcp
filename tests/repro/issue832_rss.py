...
the PROD binary, which links mimalloc as the global allocator (Makefile.pmm:
MI_OVERRIDE=1). The C test-runner and the C repro-runner are built CRT+ASan
(MI_OVERRIDE=0), so mimalloc is inert there and pmm_mem_rss() falls back to
os_rss() -- a C test would be VACUOUS. Hence this drives the real
`build/c/project-mind-mcp` server over stdio and samples its RSS from `ps`.

WHAT IT SHOWS
-------------
A long-lived MCP server is driven through K index_repository cycles of the same
fixture. The in-process pipeline (PMM_INDEX_SUPERVISOR=0) is the pre-#832-fix
...
BINARY = os.path.join(ROOT, "build", "c", "project-mind-mcp")
CYCLES = 10
...
def run_series(repo, cache, supervised):
    env = dict(os.environ)
    env["PMM_CACHE_DIR"] = cache
    if supervised:
        env.pop("PMM_INDEX_SUPERVISOR", None)
    else:
        env["PMM_INDEX_SUPERVISOR"] = "0"  # in-process (pre-fix background behaviour)
    env["PMM_INDEX_WORKER_TIMEOUT_S"] = "120"
...
def main():
    if not os.path.exists(BINARY):
        print(f"missing prod binary: {BINARY}\n  build it: make -f Makefile.pmm pmm")
        return 2
    base = tempfile.mkdtemp(prefix="pmm-832-")
