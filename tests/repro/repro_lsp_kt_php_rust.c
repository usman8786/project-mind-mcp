 ...
 *   (internal/pmm/lsp/kotlin_lsp.c, php_lsp.c, rust_lsp.c).
 *
 ...
 *   has NO PMM_LANG_RUST case in either pmm_pxc_has_cross_lsp (lines 282-298)
 *   or the pmm_pxc_run_one dispatch (lines 372-407). Go/C/C++/Python/PHP/Java/
 ...
 * pmm_registry_lookup_method finds a func whose receiver_type == recv type and
...
TEST(repro_lsp_rust_macro) {
    ...
    RProj lp;
    pmm_store_t *store = rh_index(&lp, "main.rs", kRustMacro);
    if (!store) {
