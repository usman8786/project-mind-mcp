 ...
 *
 * Root cause (as filed):
 *   pmm_install_hook_gate_script wrote the legacy blocking shell gate
 ...
 *
 * Expected (correct) behaviour after pmm_upsert_claude_hooks +
 * pmm_install_hook_gate_script:
 *   1. The gate script written to
 *      <home>/.claude/hooks/pmm-code-discovery-gate
 ...
 *      "pmm-code-discovery-gate" (the shim), not an inline blocking script.
 *
 ...
 *   Then running both pmm_upsert_claude_hooks + pmm_install_hook_gate_script
 *   (the actual install/update code path) and asserting the CORRECT result.
 *
 *   This is the critical gap: existing tests call pmm_install_hook_gate_script
 ...
 *
 * After calling pmm_upsert_claude_hooks + pmm_install_hook_gate_script
 ...
 *   2. settings.json PreToolUse command references "pmm-code-discovery-gate"
 ...
 *   - settings.json does NOT contain "pmm-code-discovery-gate" (wrong command)
 *
 * Oracle used: pmm_upsert_claude_hooks(settings_path) +
 *              pmm_install_hook_gate_script(home, binary_path)
...
TEST(repro_issue409_install_wires_hook_augment_not_blocking_gate) {
     ...
     * overwritten (truncated) by pmm_install_hook_gate_script. */
    char script_path[512];
    snprintf(script_path, sizeof(script_path),
             "%s/pmm-code-discovery-gate", hooks_dir);
    rp409_write_file(script_path,
        "#!/bin/bash\n"
        "# Gate hook: nudges Claude toward project-mind-mcp for code discovery.\n"
        "# First Grep/Glob/Read per session -> block. Subsequent -> allow.\n"
        "# PPID = Claude Code process PID, unique per session.\n"
        "GATE=/tmp/pmm-code-discovery-gate-$PPID\n"
        "if [ -f \"$GATE\" ]; then exit 0; fi\n"
        "touch \"$GATE\"\n"
        "echo 'BLOCKED: use project-mind-mcp' >&2\n"
        "exit 2\n");
     ...
     * /tmp/pmm-code-discovery-gate-$PPID as a per-invocation state file.
    ...
    ASSERT(strstr(settings_data, "pmm-code-discovery-gate") != NULL);
