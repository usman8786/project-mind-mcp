 ...
 *   checks:
 *     pmm_kind_in_set(method_node, spec->function_node_types)
 ...
 *   A. pmm_extract_file must emit a Method def with name="handleClick"
 ...
 *      line ~3620 guards on pmm_kind_in_set(method_node, spec->function_node_types);
...
static CBMFileResult *rx_ts(const char *src) {
    return pmm_extract_file(src, (int)strlen(src), PMM_LANG_TYPESCRIPT, "proj", "ts.ts",
                            0, NULL, NULL);
}
...
TEST(repro_new_ts_class_field_arrow_method_def_dropped) {
     ...
     * WHY RED: extract_class_methods bails out at the pmm_kind_in_set check
