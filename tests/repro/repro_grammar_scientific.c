 ...
 *
 * Languages covered (15) and the PMM_LANG_* enum each uses (all verified present
 * in internal/pmm/pmm.h -- none missing, none skipped):
 *   GLSL     -> PMM_LANG_GLSL      (shader; reuses C node types)
 *   HLSL     -> PMM_LANG_HLSL      (shader; C++-family node types)
 *   WGSL     -> PMM_LANG_WGSL      (shader; own grammar)
 *   ISPC     -> PMM_LANG_ISPC      (shader/SIMD; C-family node types)
 *   Slang    -> PMM_LANG_SLANG     (shader; C++-family node types)
 *   Cairo    -> PMM_LANG_CAIRO     (smart-contract; Rust-like)
 *   Sway     -> PMM_LANG_SWAY      (smart-contract; Rust-like)
 *   FunC     -> PMM_LANG_FUNC      (smart-contract; TON)
 *   Wolfram  -> PMM_LANG_WOLFRAM   (CAS; assignment-as-definition)
 *   MATLAB   -> PMM_LANG_MATLAB    (numeric)
 *   Magma    -> PMM_LANG_MAGMA     (CAS)
 *   FORM     -> PMM_LANG_FORM      (symbolic; procedure_definition / call_statement)
 *   TLA+     -> PMM_LANG_TLAPLUS   (formal spec; operator_definition)
 *   Agda     -> PMM_LANG_AGDA      (dependently-typed)
 *   Apex     -> PMM_LANG_APEX      (Salesforce; Java-like, methods only)
 *
 ...
 * via the same pmm_enclosing_func_qn -> Module fallback documented in
...
TEST(repro_grammar_scientific_glsl) {
    static const char src[] =
        ...
        "    gl_FragColor = vec4(v);\n"
        "}\n";
    if (single_file_battery("GLSL", src, PMM_LANG_GLSL, "shader.frag",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("GLSL", "void main() { float v = scale(",
                         PMM_LANG_GLSL, "shader.frag") != 0)
        return 1;
...
TEST(repro_grammar_scientific_hlsl) {
    static const char src[] =
        ...
        "}\n";
    if (single_file_battery("HLSL", src, PMM_LANG_HLSL, "shader.hlsl",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("HLSL", "float4 PSMain( { return scale(",
                         PMM_LANG_HLSL, "shader.hlsl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_wgsl) {
    static const char src[] =
        ...
        "}\n";
    if (single_file_battery("WGSL", src, PMM_LANG_WGSL, "shader.wgsl",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("WGSL", "fn fs_main() -> { let v = scale(",
                         PMM_LANG_WGSL, "shader.wgsl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_ispc) {
    static const char src[] =
        ...
        "    }\n"
        "}\n";
    if (single_file_battery("ISPC", src, PMM_LANG_ISPC, "kernel.ispc",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("ISPC", "export void run( { out[0] = scale(",
                         PMM_LANG_ISPC, "kernel.ispc") != 0)
        return 1;
...
TEST(repro_grammar_scientific_slang) {
    static const char src[] =
        ...
        "    outBuf[tid.x] = v;\n"
        "}\n";
    if (single_file_battery("Slang", src, PMM_LANG_SLANG, "shader.slang",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("Slang", "void csMain( { float v = scale(",
                         PMM_LANG_SLANG, "shader.slang") != 0)
        return 1;
...
TEST(repro_grammar_scientific_cairo) {
    static const char src[] =
        ...
        "    add(x, 1)\n"
        "}\n";
    if (single_file_battery("Cairo", src, PMM_LANG_CAIRO, "lib.cairo",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Cairo", "fn compute(x: felt252) -> { add(",
                         PMM_LANG_CAIRO, "lib.cairo") != 0)
        return 1;
...
TEST(repro_grammar_scientific_sway) {
    static const char src[] =
        ...
        "    add(x, 1)\n"
        "}\n";
    if (single_file_battery("Sway", src, PMM_LANG_SWAY, "main.sw",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Sway", "fn compute(x: u64) -> { add(",
                         PMM_LANG_SWAY, "main.sw") != 0)
        return 1;
...
TEST(repro_grammar_scientific_func) {
    static const char src[] =
        ...
        "    return add(x, 1);\n"
        "}\n";
    if (single_file_battery("FunC", src, PMM_LANG_FUNC, "contract.fc",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("FunC", "int compute(int x) { return add(",
                         PMM_LANG_FUNC, "contract.fc") != 0)
        return 1;
...
TEST(repro_grammar_scientific_wolfram) {
    static const char src[] =
        ...
        "\n"
        "compute[x_] := add[x, 1]\n";
    if (single_file_battery("Wolfram", src, PMM_LANG_WOLFRAM, "calc.wl",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Wolfram", "compute[x_] := add[x,",
                         PMM_LANG_WOLFRAM, "calc.wl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_matlab) {
    static const char src[] =
        ...
        "    s = a + b;\n"
        "end\n";
    if (single_file_battery("MATLAB", src, PMM_LANG_MATLAB, "calc.m",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("MATLAB", "function r = compute(x)\n  r = add(",
                         PMM_LANG_MATLAB, "calc.m") != 0)
        return 1;
...
TEST(repro_grammar_scientific_magma) {
    static const char src[] =
        ...
        "    return Add(x, 1);\n"
        "end function;\n";
    if (single_file_battery("Magma", src, PMM_LANG_MAGMA, "calc.magma",
                            "Function", NULL, "Add") != 0)
        return 1;
    if (robustness_probe("Magma", "function Compute(x)\n  return Add(",
                         PMM_LANG_MAGMA, "calc.magma") != 0)
        return 1;
...
TEST(repro_grammar_scientific_form) {
    static const char src[] =
        ...
        "    #call add(`y')\n"
        "#endprocedure\n";
    if (single_file_battery("FORM", src, PMM_LANG_FORM, "calc.frm",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("FORM", "#procedure compute(y)\n  #call add(",
                         PMM_LANG_FORM, "calc.frm") != 0)
        return 1;
...
TEST(repro_grammar_scientific_tlaplus) {
    static const char src[] =
        ...
        "Compute(x) == Add(x, 1)\n"
        "====\n";
    if (single_file_battery("TLA+", src, PMM_LANG_TLAPLUS, "Calc.tla",
                            "Function", NULL, "Add") != 0)
        return 1;
    if (robustness_probe("TLA+", "---- MODULE Calc ----\nCompute(x) == Add(",
                         PMM_LANG_TLAPLUS, "Calc.tla") != 0)
        return 1;
...
TEST(repro_grammar_scientific_agda) {
    static const char src[] =
        ...
        "compute : Nat -> Nat\n"
        "compute x = add x 1\n";
    if (single_file_battery("Agda", src, PMM_LANG_AGDA, "Calc.agda",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Agda", "module Calc where\ncompute x = add x",
                         PMM_LANG_AGDA, "Calc.agda") != 0)
        return 1;
...
TEST(repro_grammar_scientific_apex) {
    static const char src[] =
        ...
        "    }\n"
        "}\n";
    if (single_file_battery("Apex", src, PMM_LANG_APEX, "Calculator.cls",
                            "Method", "Class", "add") != 0)
        return 1;
    if (robustness_probe("Apex", "public class Calculator { Integer compute() { return add(",
                         PMM_LANG_APEX, "Calculator.cls") != 0)
        return 1;
