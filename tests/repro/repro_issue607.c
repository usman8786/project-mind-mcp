 ...
 *   src/cli/cli.c  pmm_cmd_install()  printed
 *     "Found %d existing index(es) that must be rebuilt:\n"
 *   then called pmm_remove_indexes(home) which unlinked every .db and NEVER
 ...
 *
 *     int pmm_install_handle_existing_indexes(const char *home,
 ...
 *        pmm_install_handle_existing_indexes(home, reset=false, dry_run=false)
 ...
 *        pmm_install_handle_existing_indexes(home, reset=true, dry_run=false)
 ...
 *      PMM_ASSUME_YES so the test is non-interactive.
 *
...
#define REPRO607_PROJECT "pmm-repro607-test"
...
static int repro607_make_index(const char *tmp_cache, char *db_path, size_t db_path_sz) {
    ...
    int upsert_rc =
        pmm_store_upsert_project(setup_store, REPRO607_PROJECT, "/home/user/my-project");
    pmm_store_close(setup_store);
    return (upsert_rc == PMM_STORE_OK) ? 1 : 0;
}
...
TEST(repro_issue607_reinstall_preserves_index) {
    /* Redirect PMM_CACHE_DIR to a fresh temp dir so the real user cache is
     * never touched and count_db_indexes()/pmm_list_indexes() see only the
     * DB we create here. */
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/pmm_repro607_XXXXXX");
     ...
     * call pmm_remove_indexes(). The fix must NOT delete here regardless.
     */
    int proceed =
        pmm_install_handle_existing_indexes(tmp_cache /* fake home */, false /* reset */,
                                            false /* dry_run */);
 ...
 * --reset-indexes flag. Auto-answers the delete prompt via PMM_ASSUME_YES so
 * the test stays non-interactive.
 * ─────────────────────────────────────────────────────────────────── */
TEST(repro_issue607_reset_indexes_deletes) {
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/pmm_repro607r_XXXXXX");
    if (!pmm_mkdtemp(tmp_cache)) {
    ...
    pmm_set_auto_answer_for_test(1 /* AUTO_YES */);

    /* Opt-in destructive path: reset=true must delete the index. */
    int proceed =
        pmm_install_handle_existing_indexes(tmp_cache /* fake home */, true /* reset */,
                                            false /* dry_run */);
    int proceeded = (proceed == 1);
    ...
    pmm_set_auto_answer_for_test(0 /* prompt */);
