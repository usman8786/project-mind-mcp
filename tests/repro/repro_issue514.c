 ...
 *   qualified_name, hop, risk, is_test) from pmm_node_hop_t.  The edge that
 *   carried the arg expressions is NOT propagated by pmm_store_bfs() into the
 *   pmm_traverse_result_t (pmm_edge_info_t carries only from_name, to_name,
 ...
 *   pmm_store_bfs() in src/store/store.c must propagate edge properties_json
 *   into the pmm_traverse_result_t (extend pmm_edge_info_t or pmm_node_hop_t).
...
TEST(repro_issue514_data_flow_surfaces_arg_expr) {
     ...
     *
     * WHY RED on current code:
     *   bfs_to_json_array() (mcp.c ~line 2283) only emits pmm_node_hop_t fields
     *   (name, qualified_name, hop).  pmm_edge_info_t (store.h ~line 146) does
