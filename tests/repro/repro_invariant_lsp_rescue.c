 ...
 *   pmm_pipeline_find_lsp_resolution  (src/pipeline/lsp_resolve.h:48)
 ...
 *
 *   When tree-sitter's enclosing-func walk FAILS, pmm_enclosing_func_qn
 *   falls back to the MODULE QN, so call->enclosing_func_qn is the module
 *   QN. The C/C++ LSP cross resolver (internal/pmm/lsp/c_lsp.c) builds its
 ...
 *   PMM_LSP_CONFIDENCE_FLOOR (0.6f, lsp_resolve.h:36).
 *
 ...
 *   out-of-line method body, tree-sitter's pmm_find_enclosing_func cannot
 ...
 *   class-qualified method QN, so pmm_enclosing_func_qn falls back to the
 *   module QN (issue #554 / extract_defs.c + c_lsp.c dominate the
 *   QUALITY_ANALYSIS Module-sourced-CALLS top-file list). C/C++ has a
 *   cross-file LSP wired up (pmm_pxc_has_cross_lsp, pass_lsp_cross.c:281),
 ...
 */
static int find_call_edge_to_helper(pmm_store_t *store, const char *project,
                                    pmm_node_t *out_src, char *out_props,
                                    size_t props_cap) {
    pmm_edge_t *edges = NULL;
    int nedges = 0;
    if (pmm_store_find_edges_by_type(store, project, "CALLS", &edges, &nedges)
            != PMM_STORE_OK) {
        return 0;
    }
    ...
    for (int i = 0; i < nedges; i++) {
        pmm_node_t tgt;
        if (pmm_store_find_node_by_id(store, edges[i].target_id, &tgt)
                != PMM_STORE_OK) {
            continue;
        }
        ...
        if (pmm_store_find_node_by_id(store, edges[i].source_id, out_src)
                == PMM_STORE_OK) {
 ...
 *
 * Today the join in pmm_pipeline_find_lsp_resolution (lsp_resolve.h:65)
...
TEST(repro_invariant_lsp_rescue_source) {
    RProj lp;
    pmm_store_t *store = rh_index(&lp, "main.cpp", kCppOutOfLine);
