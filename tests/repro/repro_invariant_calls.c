 ...
 *   Graph quality audit over the real project-mind-mcp repo showed only
 *   3.69% of CALLS edges are callable-sourced (207/5607). The dominant
 *   failure mode is pmm_enclosing_func_qn returning the module QN when
 *   pmm_find_enclosing_func cannot walk the TSNode ancestry back to a
 ...
 *   type matches the list, pmm_find_enclosing_func returns a null node and
 *   pmm_enclosing_func_qn falls back to the module QN. The LSP rescue path
 ...
 *                  Rust LSP is hybrid but pmm_pxc_has_cross_lsp returns false
 *                  for PMM_LANG_RUST (pass_lsp_cross.c:281). The enclosing-
 ...
 *
 * ASSERTION (per edge):
 *   For every pmm_edge_t e where e.type == "CALLS":
 *     pmm_store_find_node_by_id(store, e.source_id, &src) == PMM_STORE_OK
...
static int assert_calls_callable_sourced(const char *lang_tag,
    ...
    for (int i = 0; i < nedges; i++) {
        pmm_node_t src;
        if (pmm_store_find_node_by_id(store, edges[i].source_id, &src)
                != PMM_STORE_OK) {
 ...
 * pmm_enclosing_func_qn path does not correctly resolve the caller QN and
 ...
 * drop the class qualifier (issue #554 / helpers.c pmm_enclosing_func_qn).
 ...
 * pmm_pxc_has_cross_lsp returns false for PMM_LANG_RUST (pass_lsp_cross.c:281)
