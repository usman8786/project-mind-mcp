 ...
 *   pmm_install_vscode_mcp() itself takes a single config_path argument and
 ...
 *   Concretely: after pmm_build_install_plan_json() (the dry-run oracle for
 ...
 *   pmm_build_install_plan_json() runs the real install logic in dry-run mode.
 ...
 *   the plan) for each discovered profile path, using pmm_install_vscode_mcp.
 */
 ...
 * same on both platforms.  We use the portable pmm_app_config_dir() path on
 * non-Apple builds and the Library path on Apple builds so the detection in
 * pmm_detect_agents() actually fires, which is required for the plan to
 ...
 *
 * After pmm_build_install_plan_json(tmpdir, BIN) the plan JSON must contain:
...
TEST(repro_issue431_vscode_profile_inherits_mcp_json) {
    ...
    char tmpdir[512];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_repro431_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("pmm_mkdtemp failed");

    /* Create the VSCode User dir so pmm_detect_agents() marks vscode=true.
...
#else
    /* Linux: detection uses pmm_app_config_dir() which is XDG-derived.
     * pmm_detect_agents() resolves that internally; we emulate it with
    ...
    if (!agents.vscode) {
         ...
         * pmm_detect_agents() cannot see the fixture home on some platform — in
    ...
    char *plan_json =
        pmm_build_install_plan_json(tmpdir, "/usr/local/bin/project-mind-mcp");
