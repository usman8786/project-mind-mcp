 ...
 *   https://github.com/usman8786/project-mind-mcp/issues/570
 *
 * TWO FILES WRONGLY WRITTEN (Codex SessionStart hook):
 *   ~/.codex/config.toml   -- always written by pmm_upsert_codex_hooks()
 ...
 *   target to pmm_upsert_codex_hooks():
 *
 *     snprintf(cp, sizeof(cp), "%s/.codex/config.toml", home);
 *     ...
 *     pmm_upsert_codex_hooks(cp);
 *
 ...
 *
 *   So pmm_build_install_plan_json() always lists config.toml as the Codex
 *   hook target, even when hooks.json is already in use.
 *
 * EXPECTED vs ACTUAL (oracle: pmm_build_install_plan_json plan JSON):
 ...
 *   pmm_detect_agents() sets agents.codex = dir_exists("~/.codex").
 ...
 *     - If it does, pass that path to pmm_upsert_codex_session_hooks_json()
 ...
 *
 * Oracle: pmm_build_install_plan_json(home, binary) -- dry-run plan, no writes.
 *
...
TEST(repro_issue570_no_dual_hook_write) {
    char home[256];
    snprintf(home, sizeof(home), "/tmp/pmm-repro570-XXXXXX");
    if (!pmm_mkdtemp(home))
    ...
    char *json = pmm_build_install_plan_json(home, "/usr/local/bin/project-mind-mcp");
