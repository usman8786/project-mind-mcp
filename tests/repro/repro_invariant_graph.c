 ...
 *   /Users/martinvogel/project_dir/pmm-quality-contracts/QUALITY_ANALYSIS.md
 *
...
TEST(invariant_discovery_hygiene) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/pmm_inv_disc_XXXXXX", pmm_tmpdir());
 ...
 *
 * WHY RED today:
 *   pmm_fqn_compute() in internal/pmm/helpers.c calls strip_ext_len() on the
...
TEST(invariant_fqn_same_stem_distinct) {
    ...
    int node_count = 0;
    int rc = pmm_store_find_nodes_by_name(store, lp.project, "api_init",
                                          &nodes, &node_count);
 ...
 * resolve to an existing node via pmm_store_find_node_by_id.
 *
 ...
 * ──────────────────────────────────────────────────────────────────────── */
static int count_dangling_edges(pmm_store_t *store, const char *project,
                                const char *edge_type) {
    pmm_edge_t *edges = NULL;
    int edge_count = 0;
    int rc = pmm_store_find_edges_by_type(store, project, edge_type,
                                          &edges, &edge_count);
    if (rc != PMM_STORE_OK) {
        return -1;
    }
    ...
    for (int i = 0; i < edge_count; i++) {
        pmm_node_t src_node;
        pmm_node_t tgt_node;
        if (pmm_store_find_node_by_id(store, edges[i].source_id,
                                      &src_node) != PMM_STORE_OK) {
            dangling++;
        }
        if (pmm_store_find_node_by_id(store, edges[i].target_id,
                                      &tgt_node) != PMM_STORE_OK) {
            dangling++;
        }
    }
 ...
 *
 * QUALITY_ANALYSIS.md gap #3: pmm_find_enclosing_func() in helpers.c uses a
 ...
 *   helpers.c     func_kinds_for_lang(PMM_LANG_PERL) falls through to default
 ...
 * pmm_find_enclosing_func() can NEVER find an enclosing function for Perl
 * call nodes, and pmm_enclosing_func_qn() always returns the module QN.
 ...
 *
 * WHY RED today:
 *   helpers.c func_kinds_for_lang has no PMM_LANG_PERL case.  The Perl
 ...
 *
 * Fix location:
 *   internal/pmm/helpers.c, function func_kinds_for_lang():
 *   Add a PMM_LANG_PERL case returning {"subroutine_declaration_statement", NULL}.
 * ──────────────────────────────────────────────────────────────────────── */
TEST(invariant_enclosing_func_perl_parity) {
    ...
    RProj lp;
    pmm_store_t *store = rh_index(&lp, "main.pl", perl_src);
    ASSERT_NOT_NULL(store);

    /* Retrieve all CALLS edges for this project */
    pmm_edge_t *edges = NULL;
    int edge_count = 0;
    int rc = pmm_store_find_edges_by_type(store, lp.project, "CALLS",
                                          &edges, &edge_count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    /*
     * RED: callable_sourced == 0 because helpers.c has no PMM_LANG_PERL case.
     ...
     *
     * GREEN when helpers.c adds PMM_LANG_PERL -> {"subroutine_declaration_statement"}.
     */
