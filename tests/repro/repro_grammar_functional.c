 ...
 *
 * Languages covered (13) and the PMM_LANG_* enum each uses:
 ...
 *   Scheme       -> PMM_LANG_SCHEME
 *   Racket       -> PMM_LANG_RACKET
 *   Common Lisp  -> PMM_LANG_COMMONLISP
 *   Emacs Lisp   -> PMM_LANG_EMACSLISP   (note: not ELISP)
 *   Lean 4       -> PMM_LANG_LEAN
 ...
 * code. pmm_enclosing_func_qn falls back to the module QN when
 * pmm_find_enclosing_func cannot match tree-sitter node types to
...
TEST(repro_grammar_functional_haskell) {
    static const char src[] =
        ...
        "compute :: Int -> Int\n"
        "compute x = add x 1\n";
    if (single_file_battery("Haskell", src, PMM_LANG_HASKELL, "Calc.hs",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_ocaml) {
    ...
    if (single_file_battery("OCaml", src, PMM_LANG_OCAML, "calc.ml",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_fsharp) {
    static const char src[] =
        ...
        "\n"
        "let compute x = add x 1\n";
    if (single_file_battery("F#", src, PMM_LANG_FSHARP, "Calc.fs",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_elixir) {
    static const char src[] =
        ...
        "  end\n"
        "end\n";
    if (single_file_battery("Elixir", src, PMM_LANG_ELIXIR, "calc.ex",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_erlang) {
    static const char src[] =
        ...
        "compute(X) ->\n"
        "    add(X, 1).\n";
    if (single_file_battery("Erlang", src, PMM_LANG_ERLANG, "calc.erl",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_elm) {
    static const char src[] =
        ...
        "compute x =\n"
        "    add x 1\n";
    if (single_file_battery("Elm", src, PMM_LANG_ELM, "Calc.elm",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_clojure) {
    static const char src[] =
        ...
        "(defn compute [x]\n"
        "  (add x 1))\n";
    if (single_file_battery("Clojure", src, PMM_LANG_CLOJURE, "calc.clj",
                            "Function", "add") != 0)
        return 1;
 ...
 * NOTE: PMM_LANG_SCHEME has func_types = empty_types, so extract_func_def is
...
TEST(repro_grammar_functional_scheme) {
    static const char src[] =
        ...
        "(define (compute x)\n"
        "  (add x 1))\n";
    if (single_file_battery("Scheme", src, PMM_LANG_SCHEME, "calc.scm",
                            "Function", "add") != 0)
        return 1;
 ...
 * NOTE: PMM_LANG_RACKET has func_types = empty_types, so definitions only
...
TEST(repro_grammar_functional_racket) {
    static const char src[] =
        ...
        "(define (compute x)\n"
        "  (add x 1))\n";
    if (single_file_battery("Racket", src, PMM_LANG_RACKET, "calc.rkt",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_commonlisp) {
    static const char src[] =
        ...
        "(defun compute (x)\n"
        "  (add x 1))\n";
    if (single_file_battery("Common Lisp", src, PMM_LANG_COMMONLISP, "calc.lisp",
                            "Function", "add") != 0)
        return 1;
 ...
 * Note: the enum is PMM_LANG_EMACSLISP (not ELISP).
...
TEST(repro_grammar_functional_emacslisp) {
    static const char src[] =
        ...
        "(defun compute (x)\n"
        "  (add x 1))\n";
    if (single_file_battery("Emacs Lisp", src, PMM_LANG_EMACSLISP, "calc.el",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_lean) {
    static const char src[] =
        ...
        "  add x 1\n";
    if (single_file_battery("Lean", src, PMM_LANG_LEAN, "Calc.lean",
                            "Function", "add") != 0)
        return 1;
...
TEST(repro_grammar_functional_gleam) {
    static const char src[] =
        ...
        "  add(x, 1)\n"
        "}\n";
    if (single_file_battery("Gleam", src, PMM_LANG_GLEAM, "calc.gleam",
                            "Function", "add") != 0)
        return 1;
