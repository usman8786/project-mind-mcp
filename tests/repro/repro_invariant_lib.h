 ...
 *
 * Two harness tiers:
 *   - single-file extraction:  inv_rx() / the inv_extract_* checks (pmm_extract_file)
...
#include "repro_harness.h" /* RProj/RFile, rh_index*, pmm_store, <store/store.h> */
#include "pmm.h"
#include <string.h>
...
static inline CBMFileResult *inv_rx(const char *src, CBMLanguage lang, const char *file) {
    return pmm_extract_file(src, (int)strlen(src), lang, "t", file, 0, NULL, NULL);
}
...
static inline void inv_count_calls_by_source(pmm_store_t *store, const char *project,
...
static inline int inv_count_dangling_edges(pmm_store_t *store, const char *project,
                                           const char *type) {
    pmm_edge_t *edges = NULL;
    int n = 0;
    if (pmm_store_find_edges_by_type(store, project, type, &edges, &n) != PMM_STORE_OK)
        return -1;
    int dangling = 0;
    for (int i = 0; i < n; i++) {
        pmm_node_t a, b;
        if (pmm_store_find_node_by_id(store, edges[i].source_id, &a) != PMM_STORE_OK)
            dangling++;
        else if (pmm_store_find_node_by_id(store, edges[i].target_id, &b) != PMM_STORE_OK)
            dangling++;
    }
    pmm_store_free_edges(edges, n);
    return dangling;
...
static inline int inv_edge_has_strategy(pmm_store_t *store, const char *project,
                                        const char *strategy) {
    pmm_edge_t *edges = NULL;
    int n = 0;
    if (pmm_store_find_edges_by_type(store, project, "CALLS", &edges, &n) != PMM_STORE_OK)
...
static inline int inv_no_calls_edge_to_qn(pmm_store_t *store, const char *project,
...
static inline int inv_calls_target_qn_suffix(pmm_store_t *store, const char *project,
                                             const char *suffix) {
    pmm_edge_t *edges = NULL;
    int n = 0;
    if (pmm_store_find_edges_by_type(store, project, "CALLS", &edges, &n) != PMM_STORE_OK)
