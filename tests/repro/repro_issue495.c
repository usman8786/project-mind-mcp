 ...
 *     def.qualified_name = pmm_fqn_compute(project, rel_path, name)
 ...
 * Caller must pmm_free_result() the returned pointer. */
static CBMFileResult *rx(const char *src, const char *proj, const char *path) {
    return pmm_extract_file(src, (int)strlen(src), PMM_LANG_RUST, proj, path, 0, NULL, NULL);
}
