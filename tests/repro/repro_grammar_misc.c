 ...
 * completes the all-159-grammar reproduce-first coverage: every PMM_LANG_* now
 ...
 * actually models (verified against internal/pmm/lang_specs.c and the
 * *_func_types / *_class_types / *_call_types arrays):
 *
 *   CALLABLE family (func_types AND call_types both non-empty) -> FULL battery
 *   (dims 1-8) + robustness:
 *     VERILOG       -> PMM_LANG_VERILOG       (func: function_declaration/task;
 *                                              call: system_tf_call/subroutine_call)
 *     SYSTEMVERILOG -> PMM_LANG_SYSTEMVERILOG (func: function_declaration/task;
 *                                              call: function_subroutine_call)
 *     VHDL          -> PMM_LANG_VHDL          (func: subprogram_declaration/def;
 *                                              call: function_call/procedure_call)
 *     CFML          -> PMM_LANG_CFML          (func: function_declaration;
 *                                              call: call_expression)
 *     CFSCRIPT      -> PMM_LANG_CFSCRIPT      (func: function_declaration; call:
 *                                              js_call_types = call_expression)
 *     RESCRIPT      -> PMM_LANG_RESCRIPT      (func: function; call: call_expression)
 *     SQUIRREL      -> PMM_LANG_SQUIRREL      (func: function_declaration; call:
 *                                              call_expression)
 *     PINE          -> PMM_LANG_PINE          (func: function_declaration_statement;
 *                                              call: call)
 *     TEMPL         -> PMM_LANG_TEMPL         (func: function_declaration/method;
 *                                              call: call_expression)
 *     SQL           -> PMM_LANG_SQL           (func: create_function; call:
 ...
 *     ASSEMBLY      -> PMM_LANG_ASSEMBLY      (func_types = {"label"}; defs are
 *                                              labels routed through the func-def
 *                                              path -> "Function"). defs-present
 *                                              asserts "Function".
 *     LINKERSCRIPT  -> PMM_LANG_LINKERSCRIPT  (only module_types + call_types; no
 *                                              func/class/var defs in spec). NO
 *                                              defs-present assertion -- dims 1-4
 *                                              + robustness only.
 *     TABLEGEN      -> PMM_LANG_TABLEGEN      (func: def/multiclass/defm ->
 *                                              "Function"; class: class -> "Class").
 *                                              defs-present asserts "Function" and
 *                                              "Class". No call_types -> no call dim.
 *     BEANCOUNT     -> PMM_LANG_BEANCOUNT     (only module_types + import_types; no
 *                                              func/class/var/call defs in spec).
 *                                              NO defs-present -- dims 1-4 +
 *                                              robustness only.
 *     BICEP         -> PMM_LANG_BICEP         (func: user_defined_function ->
 ...
 *      deliberately truncated/broken version of the fixture. pmm_extract_file may
 ...
 * tree-sitter enclosing-func walk (pmm_find_enclosing_func + func_kinds_for_lang in
...
TEST(repro_grammar_misc_assembly) {
    ...
    if (misc_single_file_battery("ASSEMBLY", src, PMM_LANG_ASSEMBLY, "f.s",
                                 "Function", NULL, NULL) != 0)
        return 1;
    return misc_robustness("ASSEMBLY", bad, PMM_LANG_ASSEMBLY, "f.s");
}
...
TEST(repro_grammar_misc_beancount) {
    ...
    if (misc_single_file_battery("BEANCOUNT", src, PMM_LANG_BEANCOUNT,
                                 "main.beancount", NULL, NULL, NULL) != 0)
        return 1;
    return misc_robustness("BEANCOUNT", bad, PMM_LANG_BEANCOUNT,
                           "main.beancount");
}
...
TEST(repro_grammar_misc_bicep) {
    ...
    if (misc_single_file_battery("BICEP", src, PMM_LANG_BICEP, "main.bicep",
                                 "Class", NULL, NULL) != 0)
        return 1;
    return misc_robustness("BICEP", bad, PMM_LANG_BICEP, "main.bicep");
}
...
TEST(repro_grammar_misc_cfml) {
    ...
    if (misc_single_file_battery("CFML", src, PMM_LANG_CFML, "calc.cfm",
                                 "Function", NULL, "add") != 0)
        return 1;
    if (misc_robustness("CFML", bad, PMM_LANG_CFML, "calc.cfm") != 0)
        return 1;
...
TEST(repro_grammar_misc_cfscript) {
    ...
    if (misc_single_file_battery("CFSCRIPT", src, PMM_LANG_CFSCRIPT, "Calc.cfc",
                                 "Function", NULL, "add") != 0)
        return 1;
    if (misc_robustness("CFSCRIPT", bad, PMM_LANG_CFSCRIPT, "Calc.cfc") != 0)
        return 1;
...
TEST(repro_grammar_misc_linkerscript) {
    ...
    if (misc_single_file_battery("LINKERSCRIPT", src, PMM_LANG_LINKERSCRIPT,
                                 "link.ld", NULL, NULL, NULL) != 0)
        return 1;
    return misc_robustness("LINKERSCRIPT", bad, PMM_LANG_LINKERSCRIPT, "link.ld");
}
...
TEST(repro_grammar_misc_pine) {
    static const char src[] =
        "//@version=5\n"
        "indicator(\"PMM EMA\", overlay=true)\n"
        "\n"
        "ema2(src, len) =>\n"
        "    a = src + len\n"
        "    a\n"
    ...
    if (misc_single_file_battery("PINE", src, PMM_LANG_PINE, "ind.pine",
                                 "Function", NULL, "ema2") != 0)
        return 1;
    if (misc_robustness("PINE", bad, PMM_LANG_PINE, "ind.pine") != 0)
        return 1;
...
TEST(repro_grammar_misc_rescript) {
    ...
    if (misc_single_file_battery("RESCRIPT", src, PMM_LANG_RESCRIPT, "Calc.res",
                                 "Function", NULL, "add") != 0)
        return 1;
    if (misc_robustness("RESCRIPT", bad, PMM_LANG_RESCRIPT, "Calc.res") != 0)
        return 1;
...
TEST(repro_grammar_misc_sql) {
    ...
    if (misc_single_file_battery("SQL", src, PMM_LANG_SQL, "fn.sql",
                                 "Function", NULL, "add") != 0)
        return 1;
    if (misc_robustness("SQL", bad, PMM_LANG_SQL, "fn.sql") != 0)
        return 1;
...
TEST(repro_grammar_misc_squirrel) {
    ...
    if (misc_single_file_battery("SQUIRREL", src, PMM_LANG_SQUIRREL, "calc.nut",
                                 "Function", NULL, "add") != 0)
        return 1;
    if (misc_robustness("SQUIRREL", bad, PMM_LANG_SQUIRREL, "calc.nut") != 0)
        return 1;
...
TEST(repro_grammar_misc_systemverilog) {
    ...
    if (misc_single_file_battery("SYSTEMVERILOG", src, PMM_LANG_SYSTEMVERILOG,
                                 "calc.sv", "Function", NULL, "add") != 0)
        return 1;
    if (misc_robustness("SYSTEMVERILOG", bad, PMM_LANG_SYSTEMVERILOG,
                        "calc.sv") != 0)
        return 1;
...
TEST(repro_grammar_misc_tablegen) {
    ...
    if (misc_single_file_battery("TABLEGEN", src, PMM_LANG_TABLEGEN, "instr.td",
                                 "Function", "Class", NULL) != 0)
        return 1;
    return misc_robustness("TABLEGEN", bad, PMM_LANG_TABLEGEN, "instr.td");
}
...
TEST(repro_grammar_misc_templ) {
    ...
    if (misc_single_file_battery("TEMPL", src, PMM_LANG_TEMPL, "page.templ",
                                 "Function", NULL, "greeting") != 0)
        return 1;
    if (misc_robustness("TEMPL", bad, PMM_LANG_TEMPL, "page.templ") != 0)
        return 1;
...
TEST(repro_grammar_misc_verilog) {
    ...
    if (misc_single_file_battery("VERILOG", src, PMM_LANG_VERILOG, "calc.v",
                                 "Function", NULL, "add") != 0)
        return 1;
    if (misc_robustness("VERILOG", bad, PMM_LANG_VERILOG, "calc.v") != 0)
        return 1;
...
TEST(repro_grammar_misc_vhdl) {
    ...
    if (misc_single_file_battery("VHDL", src, PMM_LANG_VHDL, "calc.vhd",
                                 "Function", NULL, "add") != 0)
        return 1;
    if (misc_robustness("VHDL", bad, PMM_LANG_VHDL, "calc.vhd") != 0)
        return 1;
