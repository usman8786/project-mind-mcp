 ...
 *   `pmm_registry_resolve` resolves the callee name to a candidate QN
 *   containing "requests", but `pmm_gbuf_find_by_qn(ctx->gbuf, res.qualified_name)`
 ...
 *   pass_cross_repo.c finds nothing to iterate over, and `pmm_cross_repo_match`
 ...
 *   Subsequently, `pmm_cross_repo_match` must produce at least one
 ...
 *   pmm_cross_repo_match returns http_edges == 0.  The assertion below is RED.
 *
 ...
 *   pmm_registry_resolve("requests.get", …) returns a candidate QN that
 *   contains "requests" (service-pattern match → PMM_SVC_HTTP), BUT
 *   pmm_gbuf_find_by_qn returns NULL for that QN because no `requests` node
 ...
 *   via pmm_route_canon_path).  A HANDLES edge links the Route to `get_order`.
 */
 ...
 *   3. Run pmm_cross_repo_match(client_project, [server_project], 1).
...
TEST(repro_issue523_crossrepo_http_calls_edge) {
    ...
    RProj client;
    pmm_store_t *client_store = rh_index_files(&client, client_files, N_CLIENT_FILES);
    ...
    pmm_store_close(client_store);
    client_store = NULL; /* re-opened inside pmm_cross_repo_match via cache dir */

    /* ── Index server service ─────────────────────────────────── */
    RProj server;
    pmm_store_t *server_store = rh_index_files(&server, server_files, N_SERVER_FILES);
    ...
    pmm_store_close(server_store);
    server_store = NULL; /* re-opened bidirectionally inside pmm_cross_repo_match */

    /* ── Cross-repo match ─────────────────────────────────────── */
    /*
     * pmm_cross_repo_match opens both project DBs from the cache directory
     * (the same $HOME/.cache/project-mind-mcp/<project>.db paths that
    ...
    pmm_cross_repo_result_t result = pmm_cross_repo_match(client.project, &server_project, 1);
     ...
     *   pmm_gbuf_find_by_qn returns NULL for the `requests` QN (not indexed).
     ...
     *   pmm_cross_repo_match returns http_edges = 0.
     *
     ...
     * (i.e., skip the pmm_gbuf_find_by_qn guard for PMM_SVC_HTTP / PMM_SVC_ASYNC
 ...
 *      from the call's first string arg. pmm_route_canon_path never strips
 ...
 *      → reverse-direction run in pmm_cross_repo_match()
 ...
 * pipeline. Closes both stores again — pmm_cross_repo_match re-opens the DBs
...
static bool cr536_setup(RProj *client, const char *client_py, RProj *server,
    ...
    pmm_store_t *cs = rh_index(client, "client/orders.py", client_py);
    if (!cs) {
        return false;
    }
    ...
    pmm_store_t *ss = rh_index(server, "server/app.py", server_py);
    if (!ss) {
        return false;
    }
 ...
 * because pmm_cross_repo_match wrote through its own store handles). */
static int cr536_count_cross(RProj *proj) {
    pmm_store_t *st = pmm_store_open_path(proj->dbpath);
    if (!st) {
        return -1;
    }
...
TEST(repro_issue523_scheme_stripped_url_match) {
    ...
    pmm_cross_repo_result_t r = pmm_cross_repo_match(client.project, &tgt, 1);
...
TEST(repro_issue523_reverse_direction_match) {
    ...
    pmm_cross_repo_result_t r = pmm_cross_repo_match(server.project, &tgt, 1);
...
TEST(repro_issue523_idempotent_cross_edges) {
    ...
    pmm_cross_repo_result_t r1 = pmm_cross_repo_match(client.project, &tgt_srv, 1);
    ...
    pmm_cross_repo_result_t r2 = pmm_cross_repo_match(server.project, &tgt_cli, 1);
    ...
    pmm_cross_repo_result_t r3 = pmm_cross_repo_match(client.project, &tgt_srv, 1);
