 ...
 *   pmm_pipeline_fqn_compute (src/pipeline/fqn.c) strips the file extension,
...
static int r964_file_node_exists(pmm_store_t *store, const char *project, const char *name) {
    pmm_node_t *nodes = NULL;
    int count = 0;
    if (pmm_store_find_nodes_by_label(store, project, "File", &nodes, &count) != PMM_STORE_OK)
        return 0;
    int found = 0;
...
static int r964_file_inbound(pmm_store_t *store, const char *project, const char *name) {
    ...
    for (int t = 0; types[t]; t++) {
        ...
        int n = 0;
        if (pmm_store_find_edges_by_type(store, project, types[t], &edges, &n) != PMM_STORE_OK)
            continue;
        for (int i = 0; i < n; i++) {
            pmm_node_t tgt;
...
TEST(repro_issue964_header_has_node_and_is_connected) {
    ...
    RProj lp;
    pmm_store_t *store = rh_index_files(&lp, files, 3);
