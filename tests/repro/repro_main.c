 ...
 *   2. add it to TEST_REPRO_SRCS in Makefile.pmm
 ...
 * have reds). When PMM_REPRO_ONLY is set (comma/space list of suite-name
 * substrings), only matching suites run — for fast targeted validation of a
 * single fix without rebuilding intent. */
static int pmm_suite_enabled(const char *name) {
    const char *only = getenv("PMM_REPRO_ONLY");
    if (!only || !*only)
        return 1;
...
int main(void) {
    /* #845 belt-and-suspenders: this binary EMBEDS pmm_mcp_handle_tool and its
