 ...
 *
 * Issue: #363 — "Linux: pmm_system_info / pmm_default_worker_count don't
 ...
 *     pmm_detect_cgroup_cpus() reads /sys/fs/cgroup/cpu.max (v2) or
 *     .../cpu/cpu.cfs_quota_us + .../cpu/cpu.cfs_period_us (v1); the result
 *     feeds detect_system_linux(). pmm_default_worker_count() also honours the
 *     PMM_WORKERS env override (commit d952238). Both tested in test_platform.c.
 *
 *   Memory axis — FIXED.
 *     pmm_detect_cgroup_mem() reads /sys/fs/cgroup/memory.max (v2) or
 *     .../memory/memory.limit_in_bytes (v1); detect_system_linux() applies
 *     min(cgroup, host). The missing env knob — the "EXACT OPEN GAP" this
 *     repro was filed for — now exists: pmm_mem_init() reads PMM_MEM_BUDGET_MB
 *     and routes it through the pure pmm_mem_resolve_budget() (explicit
 *     override > implicit detection, mirroring PMM_WORKERS).
 *
 * IMPORTANT — clamp semantics:
 *   pmm_mem_resolve_budget() clamps the override to effective (cgroup/host) RAM
 ...
 *
 * NOTE on pmm_mem_init() caching:
 *   g_budget is initialised once via atomic_compare_exchange_strong, so this
 *   guard relies on running before any suite that calls pmm_mem_init(); the
 ...
 *
 * Precondition: PMM_MEM_BUDGET_MB is set before the process's first
 * pmm_mem_init(). The budget must equal the override (honoured exactly, since
 * it is below any host's effective RAM), proving the env knob is wired.
 */
TEST(repro_issue363_mem_budget_env_override) {
    pmm_setenv("PMM_MEM_BUDGET_MB", "128", 1);

    pmm_mem_init(0.5);

    size_t budget = pmm_mem_budget();

    pmm_unsetenv("PMM_MEM_BUDGET_MB");
