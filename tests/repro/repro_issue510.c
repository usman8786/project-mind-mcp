 ...
 *   pmm_discover_ex() loads the root .gitignore ONLY when a .git directory is
 ...
 *   When pmm_discover() is called on a directory that is NOT a git repo root
 ...
 *   pmm_discover() returns the excluded file as a normal discovered file
 ...
 *   and writes secret.py + keep.py.  After pmm_discover(), the loop below
 ...
 *   Additionally, pmm_discover_ex() should attempt to load a root .gitignore
 ...
 *   pmm_gitignore_matches(gi, "secret.py", false) == true.
 ...
 *   After pmm_discover(), "secret.py" must NOT appear in the file list.
 *
...
TEST(repro_issue510_nested_gitignore_honored) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/pmm_repro510_XXXXXX", pmm_tmpdir());
    ...
    pmm_gitignore_t *gi = pmm_gitignore_parse("secret.py\n");
    ASSERT_NOT_NULL(gi);
    /* If this assertion fails, the bug is in the gitignore matcher, not
     * in discovery — a different bug, not #510. */
    ASSERT_TRUE(pmm_gitignore_matches(gi, "secret.py", false));
