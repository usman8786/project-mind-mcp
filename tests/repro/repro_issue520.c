...
TEST(repro_issue520_detect_changes_includes_new_untracked_file) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_r520_XXXXXX");
    ...
    if (!srv) {
        th_rmtree(tmpdir);
        FAIL("pmm_mcp_server_new returned NULL");
    }
    ...
    char *dc_project = pmm_project_name_from_path(tmpdir);
    if (!dc_project) {
        pmm_mcp_server_free(srv);
        th_rmtree(tmpdir);
        FAIL("pmm_project_name_from_path failed");
    }
    char dc_args[640];
    snprintf(dc_args, sizeof(dc_args),
             "{\"base_branch\":\"main\",\"project\":\"%s\"}", dc_project);
    free(dc_project);
    char *dc_resp = pmm_mcp_handle_tool(srv, "detect_changes", dc_args);
