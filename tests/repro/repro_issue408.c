 ...
 *   is supposed to:
 *     1. Walk the repo filesystem for package.json manifests (pmm_pkgmap_scan_repo).
 *     2. Parse each sibling package.json, mapping its `"name"` field to its
 *        entry-point QN (parse_package_json → pkg_entries_push).
 *     3. On import resolution (pmm_pipeline_resolve_module), perform an exact
 ...
 *   maintainer confirmed: on macOS/Linux pmm_pkgmap_scan_repo may resolve
 *   workspace names at the manifest-parse level (pmm_pkgmap_try_parse), but
 ...
 *   in pmm_pipeline_resolve_module (step 3) silently falls through to
...
TEST(repro_issue408_workspace_crosspkg_import) {
    /*
     * Fixture layout mirrors the reporter's /tmp/pmm-issue408-repro tree
    ...
    RProj lp;
    pmm_store_t *store = rh_index_files(&lp, files, 5);
     ...
     * does not match any graph node, so pmm_pipeline_resolve_import_node
