 ...
 *
 * Issue: #557 -- "pmm v0.8.1 silently deletes project DBs on 'corrupt'
 ...
 *
 *   The sequence is:
 *     1. resolve_store() opens the project DB with pmm_store_open_path_query().
 *     2. It calls pmm_store_check_integrity() (src/store/store.c:664).
 ...
 *     3. On false, resolve_store() calls pmm_unlink(path) at mcp.c:803,
 *        then pmm_unlink(wal_path) and pmm_unlink(shm_path) -- with NO rename,
 ...
 *   After pmm_store_check_integrity() returns false and resolve_store()
 ...
 *   pmm_unlink(path) at mcp.c:803 destroys the DB file.  After resolve_store()
 ...
 *   On buggy code pmm_unlink() runs with no rename, so both conditions are
 ...
 *
 *   1. Set PMM_CACHE_DIR to a temp directory so the DB lands in a controlled
 *      location and does not pollute the real cache.
 *   2. Create the DB via pmm_store_open_path() (creates schema + tables).
 ...
 *      pmm_store_check_integrity() because '8' is not '/', 'A'-'Z', or 'a'-'z'.
 *   4. Close the store, verify the DB file exists (precondition).
 *   5. Call pmm_mcp_handle_tool(srv, "search_graph", ...) with the project
 ...
 *      and executes the destroying pmm_unlink() at mcp.c:803.
 ...
 *   dependent on kill timing or WAL state.  pmm_store_check_integrity() is
 ...
 *   Replace pmm_unlink(path) with a rename to a timestamped .corrupt path,
...
/* Project name used throughout: must pass pmm_validate_project_name().
 * Kept short and slug-safe so it is valid on every platform. */
#define REPRO557_PROJECT "pmm-repro557-test"
...
static int unique_corrupt_main_exists(const char *cache_dir) {
    pmm_dir_t *dir = pmm_opendir(cache_dir);
    if (!dir) {
        return 0;
    }
...
static int is_backup_artifact_name(const char *name) {
    char corrupt_prefix[PMM_DIRENT_NAME_MAX];
    char bak_prefix[PMM_DIRENT_NAME_MAX];
...
TEST(repro_issue557_corrupt_db_not_silently_deleted) {
    /* ── Step 1: redirect PMM_CACHE_DIR to a temp dir ─────────────────
     *
     * pmm_resolve_cache_dir() checks the PMM_CACHE_DIR env var first.
     ...
     *
     * The static buffer in pmm_resolve_cache_dir() is updated on the
     * next call because it re-reads PMM_CACHE_DIR each time.  We must
     * also call pmm_mkdir on the directory before opening the store.
     */
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/pmm_repro557_XXXXXX");
    if (!pmm_mkdtemp(tmp_cache)) {
        /* mkdtemp failed -- cannot run the test */
        ASSERT_NOT_NULL(NULL); /* marks setup failure clearly */
    }

    /* Set the env var so all subsequent pmm_resolve_cache_dir() calls
     * return tmp_cache.  setenv is POSIX; Windows uses _putenv_s. */
#if defined(_WIN32)
    char ev[600];
    snprintf(ev, sizeof(ev), "PMM_CACHE_DIR=%s", tmp_cache);
    _putenv(ev);
#else
    setenv("PMM_CACHE_DIR", tmp_cache, 1 /* overwrite */);
#endif
    ...
    /* ── Step 3: create the DB via pmm_store_open_path() ──────────────
     *
     * pmm_store_open_path() calls store_open_internal() with
     ...
     */
    pmm_store_t *setup_store = pmm_store_open_path(db_path);
     ...
     * SQL in pmm_store_check_integrity():
     *
     ...
     * the query returns the row and pmm_store_check_integrity() returns
     * false -- which is the exact trigger for the destroying path.
     *
     * pmm_store_upsert_project() is the store's own public API for
     * writing project rows (used by the pipeline on every full index).
     */
    int rc = pmm_store_upsert_project(setup_store, REPRO557_PROJECT, "826");
    ASSERT_EQ(rc, PMM_STORE_OK); /* precondition: row must be written */

    pmm_store_close(setup_store);
    setup_store = NULL;
    ...
    /* ── Step 6: drive resolve_store() via pmm_mcp_handle_tool ────────
     *
     * search_graph is the lightest query tool that reaches resolve_store().
     * The tool handler calls resolve_store(srv, project) which:
     *   1. Calls pmm_store_open_path_query(path) -- opens read-write/no-create.
     *      The DB was created in step 3 so SQLITE_OPEN_READWRITE succeeds.
     *   2. Calls pmm_store_check_integrity() -- returns false (root_path="826").
     *   3. Closes the store and calls pmm_unlink(path) at mcp.c:803.
     *      Then pmm_unlink(wal_path) and pmm_unlink(shm_path).
     ...
     *   pmm_unlink(path) at mcp.c:803 removes the file.
...
#if defined(_WIN32)
    _putenv("PMM_CACHE_DIR=");
#else
    unsetenv("PMM_CACHE_DIR");
#endif
     ...
     *
     * On buggy code: both are 0 because pmm_unlink() ran with no backup.
