 ...
 *   extract_decorators() in internal/pmm/extract_defs.c first scans
...
static CBMFileResult *rx(const char *src, CBMLanguage lang,
    ...
    return pmm_extract_file(src, (int)strlen(src), lang, proj, path,
                            0, NULL, NULL);
}
 ...
 * pmm_extract_file carry their Java annotations in .decorators:
 *
 *   @Entity
 *   @RestController
 *   public class User {
