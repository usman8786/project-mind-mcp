 ...
 * https://github.com/usman8786/project-mind-mcp/issues/403
 *
 ...
 *   pmm_should_skip_dir() (line 339) tests only the BARE directory name
 ...
 *   appears in either list.  Therefore pmm_discover() walks straight into the
 ...
 *     - The pmmignore path (opts->ignore_file or .pmmignore at root) is
 ...
 *   When pmm_discover() is called on a directory that contains an
 ...
 *   pmm_discover() returns files under Antigravity/ as normal discovered
 ...
 *   pmm_discover() is called on the temp dir.  The loop below asserts that
 ...
 * .pmmignore file at the repo root (loaded unconditionally, unlike .gitignore
 ...
 *
 * pmm_discover() is called on <tmpdir> with no opts (NULL) so all default
...
TEST(repro_issue403_install_dir_excluded) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/pmm_repro403_XXXXXX", pmm_tmpdir());
    ...
    /* --- Run discovery (default opts: no .git, no .pmmignore, no opts) --- */
     ...
     * pmm_should_skip_dir("Antigravity", ...) returns false and the walk
     ...
     * equivalent install-path exclusion applied) -- pmm_should_skip_dir
