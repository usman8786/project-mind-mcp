...
//   https://github.com/usman8786/project-mind-mcp/issues/581
//
// OBSERVED BEHAVIOUR:
//   project-mind-mcp in stdio MCP server mode grows from ~12 MB working
...
//
//   2. mimalloc page retention: pmm_mem_collect() is called after
...
//
//   (a) RSS MEASUREMENT: we use pmm_mem_rss() (src/foundation/mem.c) which
...
//
//   (d) LINUX-ONLY ALTERNATIVE: if pmm_mem_rss() returns 0 (e.g. MI_OVERRIDE=0
//       without the OS fallback compiled), the test falls back to reading
//       /proc/self/statm directly below.  On macOS and Windows pmm_mem_rss()
...
//   1. src/mcp/mcp.c, pmm_mcp_server_run event loop (or after each tool call
//      in pmm_mcp_handle_tool): periodically call
//        sqlite3_wal_checkpoint_v2(..., SQLITE_CHECKPOINT_TRUNCATE, ...)
//      and pmm_mem_collect() after query bursts (e.g. every N=50 calls or
//      after exceeding a RSS threshold via pmm_mem_over_budget()).
//   2. src/mcp/mcp.c, pmm_mcp_server_evict_idle: on idle eviction, call
//      pmm_mem_collect() so mimalloc returns pages to the OS, matching the
...
//
// GREEN after fix:
//   pmm_mem_collect() and/or TRUNCATE checkpoint called periodically by the
...
TEST(repro_issue581_query_rss_stable) {
    ...
    for (int i = 0; i < ITER_TOTAL; i++) {
        char *resp = pmm_mcp_handle_tool(lp.srv, "search_graph", args);
    ...
    //   (b) the fix — periodic SQLITE_CHECKPOINT_TRUNCATE + pmm_mem_collect() in
