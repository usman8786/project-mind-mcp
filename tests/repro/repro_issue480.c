 ...
 *   handle_trace_call_path() calls pmm_store_find_nodes_by_name() to locate
 ...
 *   pmm_store_find_node_by_qn() also returns nothing, so the handler exits
 ...
 *   graph, so pmm_store_bfs() returns visited_count == 0 and the "callers"
 ...
 *   the resolved node id is passed to pmm_store_bfs(), NOT to edge creation.
 *
 ...
 *   FAILS because pmm_store_bfs() finds no hops from the resolved start node.
 *
 ...
 *   pmm_store_find_nodes_by_name() or pmm_store_bfs() in
...
TEST(repro_issue480_trace_path_nonempty_with_calls) {
    RProj lp;
    pmm_store_t *store = rh_index_files(&lp, k_files,
     ...
     *   project        — lp.project (derived from tmpdir by pmm_project_name_from_path)
    ...
    char *resp = pmm_mcp_handle_tool(lp.srv, "trace_path", args);
     ...
     * pmm_store_bfs() returning 0 hops serialises \"callers\":[] (no caller
