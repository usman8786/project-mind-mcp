 ...
 * resolution — internal/pmm/lsp/ts_lsp.c resolve_member_call/lookup_method
...
static int node_with_qn_suffix(pmm_store_t *store, const char *project,
                               const char *label, const char *suffix) {
    pmm_node_t *nodes = NULL;
    int count = 0;
    if (pmm_store_find_nodes_by_label(store, project, label, &nodes, &count) !=
        PMM_STORE_OK)
        return 0;
    int found = 0;
 ...
 */
static int lsp_resolved_edge_exists(pmm_store_t *store, const char *project,
                                    const char *caller_substr,
                                    const char *callee_suffix,
                                    const char *strategy_substr, int dump) {
    pmm_edge_t *edges = NULL;
    int n = 0;
    if (pmm_store_find_edges_by_type(store, project, "CALLS", &edges, &n) !=
        PMM_STORE_OK)
        return 0;
    int found = 0;
...
TEST(repro_ts_inherited_extraction_preconditions) {
    ...
    ASSERT_TRUE(inv_extract_clean(kTsInherited[0].content, PMM_LANG_TYPESCRIPT,
                                  "base.ts"));
    CBMFileResult *rb =
        inv_rx(kTsInherited[0].content, PMM_LANG_TYPESCRIPT, "base.ts");
    ASSERT_NOT_NULL(rb);
    int greet_methods = 0;
    ...
    ASSERT_TRUE(inv_extract_clean(kTsInherited[1].content, PMM_LANG_TYPESCRIPT,
                                  "derived.ts"));
    CBMFileResult *rd =
        inv_rx(kTsInherited[1].content, PMM_LANG_TYPESCRIPT, "derived.ts");
    ASSERT_NOT_NULL(rd);
...
static int run_ts_inherited_pipeline(void) {
    RProj lp;
    pmm_store_t *store = rh_index_files(
