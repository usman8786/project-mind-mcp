 ...
 *
 * Single-file extraction bugs do NOT need this — use pmm_extract_file directly
...
#include <pipeline/pipeline.h> /* pmm_project_name_from_path */
...
static inline pmm_store_t *rh_open_indexed(RProj *lp) {
...
static inline pmm_store_t *rh_index_files(RProj *lp, const RFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_repro_XXXXXX");
...
static inline pmm_store_t *rh_index(RProj *lp, const char *filename, const char *content) {
    RFile f = {filename, content};
    return rh_index_files(lp, &f, 1);
}

static inline void rh_cleanup(RProj *lp, pmm_store_t *store) {
    if (store)
...
static inline int rh_count_edges(pmm_store_t *store, const char *project, const char *edge) {
    return store ? pmm_store_count_edges_by_type(store, project, edge) : -1;
}

/* Count nodes carrying `label`. Returns -1 on query error. */
static inline int rh_count_label(pmm_store_t *store, const char *project, const char *label) {
...
/* TIER B: returns true if pmm_extract_file CRASHES (signal) on `content`.
...
#if defined(_WIN32)
    CBMFileResult *r =
        pmm_extract_file(content, (int)strlen(content), lang, "repro", relpath, 0, NULL, NULL);
    if (r)
        pmm_free_result(r);
    return false;
#else
