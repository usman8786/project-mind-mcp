 ...
 * read directly from internal/pmm/lang_specs.c (the func/class/field/call type
 * arrays per PMM_LANG_*). The dimensions applied per language are documented in
 * the per-TEST comment.
 *
 * Languages covered (19) and the PMM_LANG_* enum each uses (all verified present
 * in internal/pmm/pmm.h):
 *   BASH       -> PMM_LANG_BASH        (callable: func + call)
 *   ZSH        -> PMM_LANG_ZSH         (callable: func + call)
 *   FISH       -> PMM_LANG_FISH        (callable: func + call)
 *   POWERSHELL -> PMM_LANG_POWERSHELL  (callable: func + class + call)
 *   TCL        -> PMM_LANG_TCL         (callable: func + class + call)
 *   AWK        -> PMM_LANG_AWK         (callable: func + call)
 *   VIMSCRIPT  -> PMM_LANG_VIMSCRIPT   (callable: func + call)
 *   FENNEL     -> PMM_LANG_FENNEL      (callable: func + call, lisp)
 *   NIX        -> PMM_LANG_NIX         (callable: func + call)
 *   GDSCRIPT   -> PMM_LANG_GDSCRIPT    (callable: func + class + call)
 *   LUAU       -> PMM_LANG_LUAU        (callable: func + class + call)
 *   TEAL       -> PMM_LANG_TEAL        (callable: func + class + call)
 *   LLVM_IR    -> PMM_LANG_LLVM_IR     (callable: func + call)
 *   NASM       -> PMM_LANG_NASM        (callable: func(label) + call)
 *   JANET      -> PMM_LANG_JANET       (STRUCTURAL ONLY: spec has only module_types)
 *   SMALI      -> PMM_LANG_SMALI       (structural-with-defs: func/class/field, NO calls)
 *   DEVICETREE -> PMM_LANG_DEVICETREE  (structural: call_types but NO func anchor)
 *   KCONFIG    -> PMM_LANG_KCONFIG     (structural-with-defs: class_types, NO calls)
 *   HYPRLANG   -> PMM_LANG_HYPRLANG    (pure structural: only module_types)
 *
 * No language in this set was skipped; every PMM_LANG_* above is defined in pmm.h.
 *
 * SPEC-DRIVEN CLASSIFICATION (from internal/pmm/lang_specs.c)
 * ----------------------------------------------------------
 ...
 *   R. extract-on-malformed: pmm_extract_file on a truncated/broken fixture must
 ...
 * The enclosing-func walk pmm_find_enclosing_func() uses func_kinds_for_lang()
 * in internal/pmm/helpers.c. In that switch ONLY PMM_LANG_BASH has a dedicated
...
static int sh_robustness(const char *lang_tag, const char *bad_src, CBMLanguage lang,
    ...
    CBMFileResult *r =
        pmm_extract_file(bad_src, (int)strlen(bad_src), lang, "t", file, 0, NULL, NULL);
    if (!r) {
...
TEST(repro_grammar_shells_bash) {
    ...
    if (sh_callable_battery("BASH", src, PMM_LANG_BASH, "run.sh",
                            "Function", NULL, "compute_inner") != 0)
        return 1;
    if (sh_robustness("BASH", bad, PMM_LANG_BASH, "run.sh") != 0)
        return 1;
...
TEST(repro_grammar_shells_zsh) {
    ...
    if (sh_callable_battery("ZSH", src, PMM_LANG_ZSH, "run.zsh",
                            "Function", NULL, "inner_fn") != 0)
        return 1;
    if (sh_robustness("ZSH", bad, PMM_LANG_ZSH, "run.zsh") != 0)
        return 1;
...
TEST(repro_grammar_shells_fish) {
    ...
    if (sh_callable_battery("FISH", src, PMM_LANG_FISH, "run.fish",
                            "Function", NULL, "inner_fn") != 0)
        return 1;
    if (sh_robustness("FISH", bad, PMM_LANG_FISH, "run.fish") != 0)
        return 1;
...
TEST(repro_grammar_shells_powershell) {
    ...
    if (sh_callable_battery("PowerShell", src, PMM_LANG_POWERSHELL, "run.ps1",
                            "Function", NULL, "Get-Inner") != 0)
        return 1;
    if (sh_robustness("PowerShell", bad, PMM_LANG_POWERSHELL, "run.ps1") != 0)
        return 1;
...
TEST(repro_grammar_shells_tcl) {
    ...
    if (sh_callable_battery("TCL", src, PMM_LANG_TCL, "run.tcl",
                            "Function", NULL, "inner_proc") != 0)
        return 1;
    if (sh_robustness("TCL", bad, PMM_LANG_TCL, "run.tcl") != 0)
        return 1;
...
TEST(repro_grammar_shells_awk) {
    ...
    if (sh_callable_battery("AWK", src, PMM_LANG_AWK, "prog.awk",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("AWK", bad, PMM_LANG_AWK, "prog.awk") != 0)
        return 1;
...
TEST(repro_grammar_shells_vimscript) {
    ...
    if (sh_callable_battery("VimScript", src, PMM_LANG_VIMSCRIPT, "plugin.vim",
                            "Function", NULL, "Inner") != 0)
        return 1;
    if (sh_robustness("VimScript", bad, PMM_LANG_VIMSCRIPT, "plugin.vim") != 0)
        return 1;
...
TEST(repro_grammar_shells_fennel) {
    ...
    if (sh_callable_battery("Fennel", src, PMM_LANG_FENNEL, "init.fnl",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("Fennel", bad, PMM_LANG_FENNEL, "init.fnl") != 0)
        return 1;
...
TEST(repro_grammar_shells_nix) {
    ...
    if (sh_callable_battery("Nix", src, PMM_LANG_NIX, "default.nix",
                            "Function", NULL, "addOne") != 0)
        return 1;
    if (sh_robustness("Nix", bad, PMM_LANG_NIX, "default.nix") != 0)
        return 1;
...
TEST(repro_grammar_shells_gdscript) {
    ...
    if (sh_callable_battery("GDScript", src, PMM_LANG_GDSCRIPT, "calc.gd",
                            "Function", NULL, "_inner") != 0)
        return 1;
    if (sh_robustness("GDScript", bad, PMM_LANG_GDSCRIPT, "calc.gd") != 0)
        return 1;
...
TEST(repro_grammar_shells_luau) {
    ...
    if (sh_callable_battery("Luau", src, PMM_LANG_LUAU, "mod.luau",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("Luau", bad, PMM_LANG_LUAU, "mod.luau") != 0)
        return 1;
...
TEST(repro_grammar_shells_teal) {
    ...
    static const char src[] =
        ...
        "end\n";
    static const char bad[] = "local function outer(x: number): number\n    return inner(";
    if (sh_callable_battery("Teal", src, PMM_LANG_TEAL, "mod.tl",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("Teal", bad, PMM_LANG_TEAL, "mod.tl") != 0)
        return 1;
...
TEST(repro_grammar_shells_llvm_ir) {
    ...
    if (sh_callable_battery("LLVM-IR", src, PMM_LANG_LLVM_IR, "mod.ll",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("LLVM-IR", bad, PMM_LANG_LLVM_IR, "mod.ll") != 0)
        return 1;
...
TEST(repro_grammar_shells_nasm) {
    ...
    if (sh_callable_battery("NASM", src, PMM_LANG_NASM, "prog.asm",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("NASM", bad, PMM_LANG_NASM, "prog.asm") != 0)
        return 1;
    return sh_pipeline_battery("NASM", "prog.asm", src);
}

/* ── JANET (structural only) ──────────────────────────────────────────────────
 * Idiomatic Janet with a defn and a call. spec entry PMM_LANG_JANET maps ONLY
...
TEST(repro_grammar_shells_janet) {
    ...
    if (sh_base_battery("Janet", src, PMM_LANG_JANET, "init.janet") != 0)
        return 1;
    return sh_robustness("Janet", bad, PMM_LANG_JANET, "init.janet");
}
...
TEST(repro_grammar_shells_smali) {
    ...
    if (sh_struct_battery("Smali", src, PMM_LANG_SMALI, "Calculator.smali",
                          "Class", "Function", "Field") != 0)
        return 1;
    return sh_robustness("Smali", bad, PMM_LANG_SMALI, "Calculator.smali");
}
...
TEST(repro_grammar_shells_devicetree) {
    ...
    if (sh_base_battery("DeviceTree", src, PMM_LANG_DEVICETREE, "board.dts") != 0)
        return 1;
    return sh_robustness("DeviceTree", bad, PMM_LANG_DEVICETREE, "board.dts");
}
...
TEST(repro_grammar_shells_kconfig) {
    ...
    if (sh_struct_battery("Kconfig", src, PMM_LANG_KCONFIG, "Kconfig",
                          "Class", NULL, NULL) != 0)
        return 1;
    return sh_robustness("Kconfig", bad, PMM_LANG_KCONFIG, "Kconfig");
}

/* ── HYPRLANG (pure structural) ───────────────────────────────────────────────
 * Idiomatic Hyprland config with sections and key=value assignments. spec entry
 * PMM_LANG_HYPRLANG maps ONLY module_types=source_file; every other type array
...
TEST(repro_grammar_shells_hyprlang) {
    ...
    if (sh_base_battery("Hyprlang", src, PMM_LANG_HYPRLANG, "hyprland.conf") != 0)
        return 1;
    return sh_robustness("Hyprlang", bad, PMM_LANG_HYPRLANG, "hyprland.conf");
}
