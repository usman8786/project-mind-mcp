 ...
 *
 *   DEFECT A (line ~411) — missing pmm_normalize_path_sep() before pmm_is_dir():
 *     The raw "path" query parameter (which may carry Windows backslash
 *     separators, e.g. "D:\projects\demo") is passed directly to pmm_is_dir()
 *     without first normalizing backslashes to forward slashes via
 *     pmm_normalize_path_sep().  On POSIX pmm_is_dir() never matches a path
 ...
 *     it.  pmm_normalize_path_sep() is already called on the repo_path in the
 *     MCP handler (mcp.c:2806) and in pmm_project_name_from_path() (fqn.c:332),
 ...
 *     browse request calls pmm_is_dir("D:") which on Windows resolves to the
 ...
 *     1. Normalized to forward slashes before reaching pmm_is_dir().
 ...
 *     because pmm_is_dir() sees the un-normalized backslash string.
 *   DEFECT B: browse for "D:/" returns parent="D:" instead of "D:/", stranding
 *     the user at the drive root because the next pmm_is_dir("D:") fails or
 *     resolves to the wrong directory.
 *
 * WHY RED on current code:
 *   test_repro_issue548_pmm_is_dir_rejects_backslash_path:
 ...
 *   DEFECT A: add pmm_normalize_path_sep(path) after pmm_http_query_param()
 ...
 *   pmm_is_dir() on the un-normalized path — it proves the gate that
...
/* ── TEST A: pmm_is_dir rejects a backslash path (the gate handle_browse uses) */

/*
 * repro_issue548_pmm_is_dir_rejects_backslash_path
 *
 * WHY RED on current code (DEFECT A):
 *   handle_browse() (src/ui/http_server.c:411) calls pmm_is_dir(path) before
 *   calling pmm_normalize_path_sep(path).  When the query param carries
 *   Windows backslashes (e.g. "D:\projects\demo"), the raw backslash string
 *   reaches pmm_is_dir() un-normalized.
 *
 *   On POSIX (macOS/Linux CI), pmm_is_dir() wraps stat(2).  The OS treats
 *   backslash as a valid filename character — not a path separator — so the
 *   path "tmp\pmm_repro548_abc123" (with backslashes) is a single component
 *   that does not exist in the filesystem.  stat() returns ENOENT →
 *   pmm_is_dir returns false.  The handler then returns 400 "not a directory".
 *
 *   This test creates a real tmpdir so that pmm_is_dir() WOULD return true if
 ...
 *   pmm_is_dir() returns true for that backslash form.  On current code it
 *   returns false → ASSERT fails → RED.
 *
 *   The test does not need a live server.  It calls pmm_is_dir() directly,
 *   which is exactly the function handle_browse() calls at the bug site.
 *
 *   Fix: add pmm_normalize_path_sep(path) in handle_browse() before pmm_is_dir().
 *   After the fix, handle_browse() converts backslashes first, so pmm_is_dir()
 *   sees forward-slash paths and succeeds → handler returns 200 → test GREEN.
 */
TEST(repro_issue548_pmm_is_dir_rejects_backslash_path) {
    /*
     * Create a real tmpdir on POSIX so pmm_is_dir() would succeed on the
     * forward-slash path.  The test then converts it to backslash form to
     * reproduce what handle_browse() passes to pmm_is_dir() on current code.
     */
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_repro548_XXXXXX");
    if (!pmm_mkdtemp(tmpdir)) {
        FAIL("pmm_mkdtemp failed — cannot create fixture tmpdir");
    }
     ...
     */
    if (!pmm_is_dir(tmpdir)) {
        FAIL("sanity: pmm_is_dir on fresh tmpdir returned false — fixture broken");
    }

    /*
     * Convert every '/' in tmpdir to '\\' to produce the backslash form that
     * the Windows browser UI sends (URL-decoded, e.g. \tmp\pmm_repro548_abc).
     * handle_browse() receives exactly this string from pmm_http_query_param()
     * before the missing pmm_normalize_path_sep() call.
     */
     ...
     * param: pmm_normalize_path_sep(path) THEN pmm_is_dir(path).  This pins the
     * fix at the missing normalize call-site:
     *   - BEFORE the fix, handle_browse() skipped pmm_normalize_path_sep(), so
     *     the raw backslash string reached pmm_is_dir() and the directory was
     *     rejected (the user could never open a D:/ path).
     *   - AFTER the fix (src/ui/http_server.c, normalize-before-is_dir), the
     *     backslash form is converted to forward slashes first and pmm_is_dir()
     *     sees the real tmpdir path → returns true.
     * pmm_normalize_path_sep() itself is verified correct by TEST C; here it
     * stands in for the call handle_browse() makes before the gate.
     */
    pmm_normalize_path_sep(backslash_path);
    int result = pmm_is_dir(backslash_path) ? 1 : 0;
    ASSERT_EQ(result, 1);
...
TEST(repro_issue548_drive_root_parent_correct) {
     ...
     * pmm_normalize_path_sep() has converted "D:\" to "D:/".
     *
...
/* ── TEST C: pmm_normalize_path_sep handles D:\ backslash form ──────────── */

/*
 * repro_issue548_normalize_backslash_drive_path
 *
 * Documents that pmm_normalize_path_sep() itself correctly converts
 ...
 * simply never calls it before the pmm_is_dir() gate.
 *
 ...
 * pmm_normalize_path_sep(path) to handle_browse(), all three tests in this
 * suite will be GREEN.
 *
...
SUITE(repro_issue548) {
    /*
     * RED: pmm_is_dir() returns false for a backslash path, reproducing the
     * effect of handle_browse() missing pmm_normalize_path_sep() before
     * pmm_is_dir().  A real tmpdir exists on disk; the forward-slash form
     * would pass the gate, but handle_browse() passes the raw backslash form.
     */
    RUN_TEST(repro_issue548_pmm_is_dir_rejects_backslash_path);
    ...
    /*
     * GREEN (intentional): pmm_normalize_path_sep() itself is correct.
