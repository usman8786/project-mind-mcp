 ...
 *      YAML file.  Any value containing "://" passes pmm_classify_string()
 *      as PMM_STRREF_URL, landing in CBMFileResult.string_refs.
 *
 *   2. pipeline.c:pmm_pipeline_extract_infra_routes() iterates files that
 *      match is_infra_file() — which includes ".yaml" / ".yml" — and calls
 *      try_upsert_infra_route() for every PMM_STRREF_URL entry whose value
 *      contains "://".
 *
 *   3. try_upsert_infra_route() unconditionally mints a "Route" node:
 *         pmm_gbuf_upsert_node(gbuf, "Route", sr->value, route_qn, ...)
 ...
 * prevents minting Route nodes from arbitrary PMM_STRREF_URL values in
...
static pmm_store_t *r521_index_files(R521Proj *lp, const R521File *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_r521_XXXXXX");
...
static void r521_cleanup(R521Proj *lp, pmm_store_t *store) {
...
static int r521_count_routes(pmm_store_t *store, const char *project) {
 ...
 * pass pmm_classify_string() as PMM_STRREF_URL.  On buggy code,
...
TEST(repro_issue521_no_route_from_config_url) {
    ...
    R521Proj lp;
    pmm_store_t *store = r521_index_files(&lp, files, 3);
     ...
     *   pipeline.c:try_upsert_infra_route() calls pmm_gbuf_upsert_node(…,"Route",…)
     *   for every PMM_STRREF_URL string_ref extracted from files matching
