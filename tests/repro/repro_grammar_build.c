 ...
 *
 * Languages covered (15) and the PMM_LANG_* enum each uses (all verified in
 * internal/pmm/pmm.h; none missing, none skipped):
 ...
 *   Just          -> PMM_LANG_JUST
 *   K8s           -> PMM_LANG_K8S
 *   Kustomize     -> PMM_LANG_KUSTOMIZE
 *   GoMod         -> PMM_LANG_GOMOD
 *   Requirements  -> PMM_LANG_REQUIREMENTS
 *   Gitignore     -> PMM_LANG_GITIGNORE
 *   Gitattributes -> PMM_LANG_GITATTRIBUTES
 *   SSHConfig     -> PMM_LANG_SSHCONFIG
 *   BitBake       -> PMM_LANG_BITBAKE
 *   Puppet        -> PMM_LANG_PUPPET
 *
 * Langs NOT in PMM_LANG_* (skipped, noted):
 *   none -- all 15 target languages are present in the enum.
 *
 * BATTERY DIMENSIONS
 * ------------------
 * SINGLE-FILE (pmm_extract_file, via inv_rx + inv_count_* helpers):
 ...
 *
 * FULL-PIPELINE (rh_index_files -> pmm_store_t*, via inv_count_* store helpers):
 ...
 *      Implemented inline at the end of each TEST via pmm_extract_file directly.
 *
 ...
 *     K8S            -- semantic extractor (pmm_extract_k8s); extracts kind -> "Resource".
 *     KUSTOMIZE      -- semantic extractor (pmm_extract_k8s); extracts kind -> "Resource".
 *
...
static int build_pipeline_battery(const char *lang_tag, const char *filename,
    ...
    RProj lp;
    pmm_store_t *store = rh_index_files(&lp, files, 1);
    if (!store) {
 ...
 *
 * A truncated version of the fixture is passed through pmm_extract_file.
...
static int build_robustness(const char *lang_tag, const char *bad_src,
    ...
    CBMFileResult *r = pmm_extract_file(bad_src, (int)strlen(bad_src),
...
TEST(repro_grammar_build_dockerfile) {
    static const char src[] =
        ...
        "ARG VERSION=0.8.1\n"
        "COPY . .\n"
        "RUN go build -o /pmm-server ./cmd/server\n"
        "\n"
        "FROM debian:bookworm-slim\n"
        "ENV PORT=8080\n"
        "ENV LOG_LEVEL=info\n"
        "COPY --from=builder /pmm-server /usr/local/bin/pmm-server\n"
        "EXPOSE 8080\n"
        "ENTRYPOINT [\"/usr/local/bin/pmm-server\"]\n";
    static const char bad[] = "FROM golang:1.22 AS\n";
    if (build_struct_battery("Dockerfile", src, PMM_LANG_DOCKERFILE,
                             "Dockerfile", "Variable", NULL) != 0)
        return 1;
    return build_robustness("Dockerfile", bad, PMM_LANG_DOCKERFILE, "Dockerfile");
}
...
TEST(repro_grammar_build_makefile) {
    ...
    if (build_callable_battery("Makefile", src, PMM_LANG_MAKEFILE, "Makefile",
                               "Function", "shell") != 0)
        return 1;
    return build_robustness("Makefile", bad, PMM_LANG_MAKEFILE, "Makefile");
}
 ...
 *
 * Dims asserted: 1-6 + R.
 * Dim 5 expected GREEN: "Function" def for the function_def "pmm_setup_target".
...
TEST(repro_grammar_build_cmake) {
    static const char src[] =
        ...
        "project(pmm VERSION 0.8.1 LANGUAGES C)\n"
        "\n"
        "set(CMAKE_C_STANDARD 11)\n"
        "\n"
        "function(pmm_setup_target target)\n"
        "    target_include_directories(${target} PRIVATE include)\n"
        "    target_compile_options(${target} PRIVATE -Wall -Wextra)\n"
        "endfunction()\n"
        "\n"
        "add_executable(pmm-server src/main.c src/server.c)\n"
        "pmm_setup_target(pmm-server)\n"
        "target_link_libraries(pmm-server PRIVATE sqlite3)\n";
    static const char bad[] = "cmake_minimum_required(VERSION 3.20\n";
    if (build_callable_battery("CMake", src, PMM_LANG_CMAKE, "CMakeLists.txt",
                               "Function", "add_executable") != 0)
        return 1;
    return build_robustness("CMake", bad, PMM_LANG_CMAKE, "CMakeLists.txt");
}
 ...
 *   "pmm_flags". RED would indicate function_expression->Function extraction or
...
TEST(repro_grammar_build_meson) {
     ...
     * `pmm_flags = func (target) ... endfunc` user-function syntax is not parsed
     ...
     * a pmm bug. Original assertions below are preserved (unreachable). */
    ...
    static const char src[] =
        ...
        "\n"
        "pmm_flags = func (target)\n"
        "    return ['-DVERSION=\"' + target + '\"']\n"
        "endfunc\n"
        "\n"
        "sqlite = dependency('sqlite3')\n"
        "executable('pmm-server',\n"
        "    sources: ['src/main.c', 'src/server.c'],\n"
        "    dependencies: [sqlite],\n"
        "    install: true)\n";
    static const char bad[] = "project('pmm', 'c',\n    version: '0.8.1'";
    if (build_callable_battery("Meson", src, PMM_LANG_MESON, "meson.build",
                               "Function", "executable") != 0)
        return 1;
    return build_robustness("Meson", bad, PMM_LANG_MESON, "meson.build");
}
...
TEST(repro_grammar_build_gn) {
    static const char src[] =
        ...
        "}\n";
    static const char bad[] = "executable(\"pmm-server\") {\n  sources = [";
    if (build_callable_battery("GN", src, PMM_LANG_GN, "BUILD.gn",
                               NULL, "executable") != 0)
        return 1;
    return build_robustness("GN", bad, PMM_LANG_GN, "BUILD.gn");
}
...
TEST(repro_grammar_build_just) {
    static const char src[] =
        ...
        "    just test\n";
    static const char bad[] = "build:\n    go build -o ";
    if (build_callable_battery("Just", src, PMM_LANG_JUST, "justfile",
                               "Function", "build") != 0)
        return 1;
    if (build_robustness("Just", bad, PMM_LANG_JUST, "justfile") != 0)
        return 1;
 ...
 * kind: Deployment). The K8s/Kustomize semantic extractor pmm_extract_k8s()
 * is called for PMM_LANG_K8S; it reads the kind field from the YAML tree and
 ...
 * Dim 5 expected GREEN: "Resource" def extracted by pmm_extract_k8s for the kind.
...
TEST(repro_grammar_build_k8s) {
    ...
    if (build_struct_battery("K8s", src, PMM_LANG_K8S, "deployment.yaml",
                             "Resource", NULL) != 0)
        return 1;
    return build_robustness("K8s", bad, PMM_LANG_K8S, "deployment.yaml");
}

/* ── Kustomize ────────────────────────────────────────────────────────────────
 * Idiomatic kustomization.yaml: the Kustomize overlay tool's root file
 * (kind: Kustomization). pmm_extract_k8s() is called for PMM_LANG_KUSTOMIZE
 * just as for PMM_LANG_K8S; it should mint a "Resource" def for the
 * "Kustomization" kind, which is the canonical Kustomize resource kind.
 *
 * Dims asserted: 1-5 + R ("Resource" for the Kustomization kind).
 * Dim 5 expected GREEN: "Resource" def for "Kustomization" from pmm_extract_k8s.
...
TEST(repro_grammar_build_kustomize) {
    ...
    if (build_struct_battery("Kustomize", src, PMM_LANG_KUSTOMIZE,
                             "kustomization.yaml", "Resource", NULL) != 0)
        return 1;
    return build_robustness("Kustomize", bad, PMM_LANG_KUSTOMIZE,
                            "kustomization.yaml");
}
...
TEST(repro_grammar_build_gomod) {
    static const char src[] =
        "module github.com/usman8786/project-mind-mcp\n"
        "\n"
        "go 1.22\n"
        "\n"
        "require (\n"
        ...
        ")\n";
    static const char bad[] = "module github.com/usman8786/project-mind-mcp\nrequire (";
    if (build_struct_battery("GoMod", src, PMM_LANG_GOMOD, "go.mod",
                             "Variable", NULL) != 0)
        return 1;
    return build_robustness("GoMod", bad, PMM_LANG_GOMOD, "go.mod");
}
...
TEST(repro_grammar_build_requirements) {
    static const char src[] =
        ...
        "\n"
        "# URL requirement\n"
        "pmm-client @ git+https://github.com/DeusData/pmm-client.git@v0.8.1\n";
    static const char bad[] = "requests==2.31.0\nbroken>=";
    if (build_base_battery("Requirements", src, PMM_LANG_REQUIREMENTS,
                           "requirements.txt") != 0)
        return 1;
    return build_robustness("Requirements", bad, PMM_LANG_REQUIREMENTS,
                            "requirements.txt");
}
...
TEST(repro_grammar_build_gitignore) {
    static const char src[] =
        ...
        "coverage.out\n"
        "*.prof\n";
    static const char bad[] = "pmm-server\n[invalid";
    if (build_base_battery("Gitignore", src, PMM_LANG_GITIGNORE, ".gitignore") != 0)
        return 1;
    return build_robustness("Gitignore", bad, PMM_LANG_GITIGNORE, ".gitignore");
}
...
TEST(repro_grammar_build_gitattributes) {
    static const char src[] =
        ...
        "internal/pmm/vendored/** linguist-vendored\n";
    static const char bad[] = "* text=auto eol=lf\n*.go [broken";
    if (build_base_battery("Gitattributes", src, PMM_LANG_GITATTRIBUTES,
                           ".gitattributes") != 0)
        return 1;
    return build_robustness("Gitattributes", bad, PMM_LANG_GITATTRIBUTES,
                            ".gitattributes");
}
...
TEST(repro_grammar_build_sshconfig) {
    ...
    if (build_base_battery("SSHConfig", src, PMM_LANG_SSHCONFIG, "config") != 0)
        return 1;
    return build_robustness("SSHConfig", bad, PMM_LANG_SSHCONFIG, "config");
}
...
TEST(repro_grammar_build_bitbake) {
    ...
    return -1; /* skip — not counted as pass or fail */
    static const char src[] =
        "DESCRIPTION = \"PMM MCP server component\"\n"
        "HOMEPAGE    = \"https://github.com/usman8786/project-mind-mcp\"\n"
        "LICENSE     = \"MIT\"\n"
        "PV          = \"0.8.1\"\n"
        "\n"
        "SRC_URI = \"git://github.com/usman8786/project-mind-mcp.git;protocol=https\"\n"
        "\n"
        "do_fetch() {\n"
        "    git clone ${SRC_URI} ${S}\n"
        "}\n"
        ...
        "\n"
        "do_compile() {\n"
        "    go build -o ${B}/pmm-server ./cmd/server\n"
        "}\n"
        "\n"
        "do_install() {\n"
        "    install -d ${D}${bindir}\n"
        "    install -m 0755 ${B}/pmm-server ${D}${bindir}/\n"
        "}\n";
    static const char bad[] = "DESCRIPTION = \"PMM\"\ndo_fetch() {\n    git clone ";
    if (build_callable_battery("BitBake", src, PMM_LANG_BITBAKE,
                               "pmm-server_0.8.1.bb", "Function", "subprocess") != 0)
        return 1;
    if (build_robustness("BitBake", bad, PMM_LANG_BITBAKE,
                         "pmm-server_0.8.1.bb") != 0)
        return 1;
    return build_pipeline_battery("BitBake", "pmm-server_0.8.1.bb", src);
}
 ...
 *
 * Dims asserted: 1-8 (full battery).
 * Dim 5 expected GREEN: "Function" def for the function_declaration "pmm_validate"
 *   AND "Class" def for the class_definition "pmm". RED for either label
 ...
 * Dim 7 expected GREEN for calls inside function_declaration "pmm_validate"
...
TEST(repro_grammar_build_puppet) {
    ...
    static const char src[] =
        ...
        "}\n"
        "\n"
        "define pmm::port_config (\n"
        "  Integer $port,\n"
        ") {\n"
        "  file { '/etc/pmm/port.conf':\n"
        "    content => \"port=${port}\\n\",\n"
        "  }\n"
        "}\n"
        "\n"
        "function pmm_validate(String $version) >> Boolean {\n"
        "  $parts = split($version, /\\./ )\n"
        "  length($parts) == 3\n"
        "}\n";
    static const char bad[] = "class pmm (\n  String $version = '0.8.1',\n) {\n  include";
    if (build_callable_battery("Puppet", src, PMM_LANG_PUPPET, "pmm.pp",
                               "Function", "include") != 0)
        return 1;
    if (build_robustness("Puppet", bad, PMM_LANG_PUPPET, "pmm.pp") != 0)
        return 1;
    return build_pipeline_battery("Puppet", "pmm.pp", src);
}
