 ...
 *   helpers.c  pmm_find_enclosing_func() (helpers.c:700) walks a call node's
 ...
 *   generic strings, pmm_find_enclosing_func() never matches, returns the null
 *   node, and pmm_enclosing_func_qn() falls back to the MODULE qn. Every call
 ...
 *   language's spec->func_types (lang_specs.c) so pmm_find_enclosing_func uses
 ...
 *   pmm_store_find_node_by_id(store, e.source_id, &src) == PMM_STORE_OK AND
...
static int run_parity_case(const parity_case_t *c) {
    ...
    int         nedges = 0;
    int rc = pmm_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &nedges);
    if (rc != PMM_STORE_OK) {
        printf("  %sFAIL%s  [%s] pmm_store_find_edges_by_type rc=%d\n", RED, RST, c->name, rc);
        rh_cleanup(&lp, store);
        return 1;
    }
    ...
    if (nedges == 0) {
        ...
        printf("  %sFAIL%s  [%s] no CALLS edges (callee-resolution gap; gap #3 fix "
               ...
               RED, RST, c->name);
        pmm_store_free_edges(edges, nedges);
        rh_cleanup(&lp, store);
        return 1;
    }
    ...
    for (int i = 0; i < nedges; i++) {
        pmm_node_t src;
        if (pmm_store_find_node_by_id(store, edges[i].source_id, &src) != PMM_STORE_OK) {
...
static const parity_case_t case_fortran = {
    PMM_LANG_FORTRAN, "Fortran", "a.f90",
 ...
 * PMM_LANG_VERILOG via EXT_TABLE.)
 */
static const parity_case_t case_verilog = {
    PMM_LANG_VERILOG, "Verilog", "a.sv",
    "module m;\n"
...
static const parity_case_t case_commonlisp = {
    PMM_LANG_COMMONLISP, "CommonLisp", "a.lisp",
    "(defun helper (x)\n"
    "  (* x 2))\n"
    "\n"
    "(defun run ()\n"
...
static const parity_case_t case_emacslisp = {
    PMM_LANG_EMACSLISP, "EmacsLisp", "a.el",
    "(defmacro run (n)\n"
 ...
 * CALLS edge ever forms. Verified to fail identically under PMM_LANG_SYSTEMVERILOG
 * (function_subroutine_call). This is a tree-sitter grammar defect, not a pmm
