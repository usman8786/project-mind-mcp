 ...
 * JavaScript / JSX hybrid LSP (internal/pmm/lsp/ts_lsp.c).
 *
 ...
 *   the source (grep '"lsp_' internal/pmm/lsp/ts_lsp.c), with its keying site:
 ...
 *   production indexer does: ".ts" → PMM_LANG_TYPESCRIPT, ".tsx" → PMM_LANG_TSX.
 *   jsx_mode (required by resolve_jsx_element, ts_lsp.c:2620) is enabled ONLY for
 *   PMM_LANG_TSX (pmm.c:619, pass_lsp_cross.c:267), so the two JSX fixtures use
...
static int assert_no_resolvable_edge(const char *filename, const char *src,
    ...
    RProj lp;
    pmm_store_t *store = rh_index(&lp, filename, src);
    if (!store) {
 ...
 * pmm_registry_lookup_symbol_by_args hits on the module QN). */
 ...
 * pmm_registry_lookup_symbol on the module QN. App() renders <Widget/> defined
 * in the same file. */
