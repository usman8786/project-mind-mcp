 ...
 * comment.
 *
 * Languages covered (16) and the PMM_LANG_* enum each uses (all verified in
 * internal/pmm/pmm.h):
 *   JSON       -> PMM_LANG_JSON
 ...
 *   DOTENV     -> PMM_LANG_DOTENV
 *   KDL        -> PMM_LANG_KDL
 *   RON        -> PMM_LANG_RON
 *   PKL        -> PMM_LANG_PKL
 *   NICKEL     -> PMM_LANG_NICKEL
 *   JSONNET    -> PMM_LANG_JSONNET
 *   STARLARK   -> PMM_LANG_STARLARK
 *
 * BATTERY DIMENSIONS
 * ------------------
...
TEST(repro_grammar_config_json) {
    ...
    if (config_base_battery("JSON", src, PMM_LANG_JSON, "config.json") != 0)
        return 1;
    return config_robustness("JSON", bad, PMM_LANG_JSON, "config.json");
}
...
TEST(repro_grammar_config_json5) {
    static const char src[] =
        ...
        "{\n"
        "  name: 'pmm',       // unquoted keys + single-quoted values\n"
        "  version: '0.8.1',\n"
        "  features: [\n"
        "    'graph',\n"
        "    'lsp',\n"
    ...
    if (config_base_battery("JSON5", src, PMM_LANG_JSON5, "config.json5") != 0)
        return 1;
    return config_robustness("JSON5", bad, PMM_LANG_JSON5, "config.json5");
}
...
TEST(repro_grammar_config_yaml) {
    static const char src[] =
        ...
        "  - python\n"
        "  - typescript\n";
    static const char bad[] = "name: pmm\n  - broken: [";
    if (config_base_battery("YAML", src, PMM_LANG_YAML, "config.yaml") != 0)
        return 1;
    return config_robustness("YAML", bad, PMM_LANG_YAML, "config.yaml");
}
...
TEST(repro_grammar_config_toml) {
    static const char src[] =
        ...
        "enabled = true\n";
    static const char bad[] = "name = \"pmm\"\n[[language\n";
    if (config_struct_battery("TOML", src, PMM_LANG_TOML, "config.toml",
                              "Class", NULL) != 0)
        return 1;
    return config_robustness("TOML", bad, PMM_LANG_TOML, "config.toml");
}
...
TEST(repro_grammar_config_ini) {
    ...
    if (config_struct_battery("INI", src, PMM_LANG_INI, "config.ini",
                              "Class", NULL) != 0)
        return 1;
    return config_robustness("INI", bad, PMM_LANG_INI, "config.ini");
}
...
TEST(repro_grammar_config_hcl) {
    ...
    if (config_callable_battery("HCL", src, PMM_LANG_HCL, "main.tf",
                                NULL, "tomap") != 0)
        return 1;
    return config_robustness("HCL", bad, PMM_LANG_HCL, "main.tf");
}
...
TEST(repro_grammar_config_xml) {
    static const char src[] =
        ...
        "  </server>\n"
        "  <database>\n"
        "    <url>postgres://localhost/pmm</url>\n"
        "    <maxConns>10</maxConns>\n"
        "  </database>\n"
        "</config>\n";
    static const char bad[] = "<config>\n  <server>\n    <host>";
    if (config_struct_battery("XML", src, PMM_LANG_XML, "config.xml",
                              "Class", NULL) != 0)
        return 1;
    return config_robustness("XML", bad, PMM_LANG_XML, "config.xml");
}
...
TEST(repro_grammar_config_csv) {
    static const char src[] =
        ...
        "1,pmm-go,go,true\n"
        "2,pmm-py,python,true\n"
        "3,pmm-ts,typescript,false\n";
    static const char bad[] = "id,name\n1,\"unclosed";
    if (config_base_battery("CSV", src, PMM_LANG_CSV, "data.csv") != 0)
        return 1;
    return config_robustness("CSV", bad, PMM_LANG_CSV, "data.csv");
}
...
TEST(repro_grammar_config_properties) {
    static const char src[] =
        ...
        "server.port=8080\n"
        "db.url=jdbc:postgresql://localhost/pmm\n"
        "db.pool.size=10\n";
    static const char bad[] = "app.name=pmm\nbroken";
    if (config_struct_battery("PROPERTIES", src, PMM_LANG_PROPERTIES,
                              "app.properties", "Variable", NULL) != 0)
        return 1;
    return config_robustness("PROPERTIES", bad, PMM_LANG_PROPERTIES,
                             "app.properties");
}
...
TEST(repro_grammar_config_dotenv) {
    static const char src[] =
        "# Database\n"
        "DATABASE_URL=postgres://localhost:5432/pmm\n"
    ...
    if (config_base_battery("DOTENV", src, PMM_LANG_DOTENV, ".env") != 0)
        return 1;
    return config_robustness("DOTENV", bad, PMM_LANG_DOTENV, ".env");
}
...
TEST(repro_grammar_config_kdl) {
    ...
    if (config_base_battery("KDL", src, PMM_LANG_KDL, "config.kdl") != 0)
        return 1;
    return config_robustness("KDL", bad, PMM_LANG_KDL, "config.kdl");
}
...
TEST(repro_grammar_config_ron) {
    static const char src[] =
        ...
        ")\n";
    static const char bad[] = "Config(\n  name: \"pmm\",\n  broken: [";
    if (config_base_battery("RON", src, PMM_LANG_RON, "config.ron") != 0)
        return 1;
    return config_robustness("RON", bad, PMM_LANG_RON, "config.ron");
}
...
TEST(repro_grammar_config_pkl) {
    static const char src[] =
        ...
        "  port = 9000\n"
        "}\n";
    static const char bad[] = "module pmm.Config\nclass Server {\n  host:";
    if (config_struct_battery("PKL", src, PMM_LANG_PKL, "config.pkl",
                              "Class", "Function") != 0)
        return 1;
    return config_robustness("PKL", bad, PMM_LANG_PKL, "config.pkl");
}
...
TEST(repro_grammar_config_nickel) {
    ...
    if (config_callable_battery("Nickel", src, PMM_LANG_NICKEL, "config.ncl",
                                "Function", "addPort") != 0)
        return 1;
    if (config_robustness("Nickel", bad, PMM_LANG_NICKEL, "config.ncl") != 0)
        return 1;
...
TEST(repro_grammar_config_jsonnet) {
    ...
    if (config_callable_battery("Jsonnet", src, PMM_LANG_JSONNET, "config.jsonnet",
                                "Function", "makeServer") != 0)
        return 1;
    if (config_robustness("Jsonnet", bad, PMM_LANG_JSONNET, "config.jsonnet") != 0)
        return 1;
...
TEST(repro_grammar_config_starlark) {
    ...
    if (config_callable_battery("Starlark", src, PMM_LANG_STARLARK, "BUILD",
                                "Function", "go_binary") != 0)
        return 1;
    if (config_robustness("Starlark", bad, PMM_LANG_STARLARK, "BUILD") != 0)
        return 1;
