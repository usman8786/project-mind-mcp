 ...
 *   the bare-name probe never matched and pmm_find_cli("opencode", ...) always
 ...
 *             pmm_find_cli("opencode", ...) resolves correctly -- the baseline.
 ...
 *
 * Verify that pmm_find_cli("opencode", ...) resolves the opencode executable
 * (or its Windows .cmd shim) when the containing directory is on PATH.
 *
 * CORRECT BEHAVIOUR (post-fix):
 *   pmm_find_cli returns a non-empty string whose basename starts with
 *   "opencode" -- meaning find_in_path found the file.
 *
 * BUGGY BEHAVIOUR (pre-fix, Windows only):
 *   pmm_find_cli returns "" because find_in_path only probed the bare name
...
TEST(repro_issue221_opencode_pathext_lookup) {
    ...
    /*
     * The function under test: pmm_find_cli is the public API that calls
     ...
     *   and returned false.  pmm_find_cli returned "".
     * Post-fix (Windows): find_in_path also probes "<tmpdir>/opencode.cmd"
     *   (present), finds it, and pmm_find_cli returns the full path.
     * POSIX (before and after): bare-name probe succeeds immediately.
     */
    const char *result = pmm_find_cli("opencode", "/nonexistent-home-dir");
     ...
     *
     * pmm_find_cli MUST return a non-empty path that contains "opencode".
     *
