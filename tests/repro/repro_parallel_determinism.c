...
static char *rpd_edge_fingerprint(pmm_store_t *s, const char *project) {
    struct sqlite3 *db = pmm_store_get_db(s);
    if (!db)
        return NULL;
 ...
 * fingerprint. Honors PMM_INDEX_SINGLE_THREAD from the environment. */
static char *rpd_index_and_fingerprint(const char *repo, const char *dbpath) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(repo, dbpath, PMM_MODE_FULL);
    if (!p)
        return NULL;
...
TEST(repro_parallel_edge_determinism) {
    ...
    char dbpath[512];
    snprintf(dbpath, sizeof(dbpath), "%s/pmm_rpd_par_det.db", pmm_tmpdir());
...
TEST(repro_seq_parallel_equivalence) {
    ...
    char dbpath[512];
    snprintf(dbpath, sizeof(dbpath), "%s/pmm_rpd_seq_par.db", pmm_tmpdir());

    setenv("PMM_INDEX_SINGLE_THREAD", "1", 1);
    char *fp_st = rpd_index_and_fingerprint(RPD_CORPUS, dbpath);
    unsetenv("PMM_INDEX_SINGLE_THREAD");
