 ...
 *   pmm_pipeline → graph store) persists a cross-crate CALLS edge for a
 ...
 *   the LSP layer (pmm_run_rust_lsp_with_manifest called with a parsed
 ...
 *   (pmm_extract_file → pmm_run_rust_lsp) does NOT receive a pre-parsed
 *   workspace manifest — it only gets the individual file's content.
 *   Additionally, pmm_pxc_has_cross_lsp() returns false for PMM_LANG_RUST
 ...
 *   1. pmm_run_rust_lsp is called with NULL manifest (pmm.c:645), so no
 *      workspace metadata is available at extraction time.
 *   2. pmm_pxc_has_cross_lsp returns false for PMM_LANG_RUST
...
TEST(repro_issue56_cross_crate_calls) {
    ...
    int edge_count = 0;
    int rc = pmm_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &edge_count);
    ASSERT_EQ(rc, PMM_STORE_OK);

    int found_cross_crate = 0;
    for (int i = 0; i < edge_count && !found_cross_crate; i++) {
        pmm_node_t target_node;
        if (pmm_store_find_node_by_id(store, edges[i].target_id, &target_node) == PMM_STORE_OK) {
