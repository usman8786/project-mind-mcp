...
typedef struct {
    ...
    pmm_daemon_bootstrap_probe_status_t forced_probe;
    pmm_version_cohort_status_t forced_cohort;
    char diagnostic[PMM_DAEMON_CONFLICT_MESSAGE_SIZE];
} bootstrap_fake_ops_t;

typedef struct {
    const pmm_daemon_bootstrap_config_t *config;
    const pmm_daemon_bootstrap_ops_t *ops;
    atomic_int *ready;
    atomic_bool *go;
    pmm_daemon_bootstrap_result_t result;
    pmm_daemon_bootstrap_status_t status;
} bootstrap_thread_call_t;

static pmm_daemon_build_identity_t bootstrap_identity(const char *version, const char *build) {
...
static bool bootstrap_endpoint_fixture_start(bootstrap_endpoint_fixture_t *fixture,
    ...
    int written = snprintf(fixture->parent, sizeof(fixture->parent), "%s/pmm-bootstrap-%s-XXXXXX",
                           pmm_tmpdir(), tag);
    if (written <= 0 || written >= (int)sizeof(fixture->parent) || !pmm_mkdtemp(fixture->parent)) {
        return false;
    }
    fixture->endpoint = pmm_daemon_bootstrap_endpoint_new(fixture->parent);
    const char *runtime_dir =
        fixture->endpoint ? pmm_daemon_ipc_endpoint_runtime_dir(fixture->endpoint) : NULL;
    if (!runtime_dir) {
        return false;
    }
...
static pmm_daemon_bootstrap_probe_status_t bootstrap_fake_probe(
    void *opaque, const pmm_daemon_ipc_endpoint_t *endpoint,
    const pmm_daemon_build_identity_t *identity, uint32_t timeout_ms,
    pmm_daemon_runtime_client_t **client_out, pmm_daemon_runtime_connect_result_t *result_out) {
    (void)endpoint;
    (void)identity;
    (void)timeout_ms;
    ...
    *client_out = NULL;
    ...
    if (reserved_remaining > 0) {
        ...
        return PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED;
    }
    ...
    if (terminal_remaining > 0) {
        return PMM_DAEMON_BOOTSTRAP_PROBE_TERMINAL;
    }
    if (fake->forced_probe == PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED) {
        return fake->forced_probe;
    }
    if (fake->forced_probe == PMM_DAEMON_BOOTSTRAP_PROBE_CONFLICT) {
        result_out->status = PMM_DAEMON_RUNTIME_CONNECT_CONFLICT;
        result_out->hello_status = PMM_DAEMON_HELLO_BUILD_CONFLICT;
        snprintf(result_out->message, sizeof(result_out->message),
                 "PMM daemon could not start: conflicting versions");
        return fake->forced_probe;
    }
    if (fake->forced_probe == PMM_DAEMON_BOOTSTRAP_PROBE_TERMINAL) {
        return fake->forced_probe;
    }
    ...
    if (atomic_load(&fake->available)) {
        result_out->status = PMM_DAEMON_RUNTIME_CONNECT_ACCEPTED;
        result_out->hello_status = PMM_DAEMON_HELLO_COMPATIBLE;
        result_out->client_id = 1;
        *client_out = (pmm_daemon_runtime_client_t *)(uintptr_t)1;
        return PMM_DAEMON_BOOTSTRAP_PROBE_CONNECTED;
    }
    return PMM_DAEMON_BOOTSTRAP_PROBE_UNAVAILABLE;
}

static pmm_version_cohort_status_t bootstrap_fake_cohort_acquire(
    void *opaque, const pmm_daemon_ipc_endpoint_t *endpoint,
    const pmm_daemon_build_identity_t *identity, uint64_t deadline_ms,
    pmm_daemon_bootstrap_cohort_t *cohort_out, pmm_daemon_conflict_t *conflict_out) {
    (void)endpoint;
    (void)deadline_ms;
    bootstrap_fake_ops_t *fake = opaque;
    ...
    *cohort_out = NULL;
    ...
    if (fake->forced_cohort == PMM_VERSION_COHORT_CONFLICT) {
        pmm_daemon_build_identity_t active = bootstrap_identity("2.3.0", BOOTSTRAP_BUILD_A);
        (void)pmm_daemon_hello_compare(&active, identity, conflict_out);
        return fake->forced_cohort;
    }
    if (fake->forced_cohort != PMM_VERSION_COHORT_OK) {
        return fake->forced_cohort;
    }
    *cohort_out = fake;
...
static void bootstrap_fake_cohort_release(void *opaque, pmm_daemon_bootstrap_cohort_t cohort) {
    bootstrap_fake_ops_t *fake = opaque;
    if (cohort == fake) {
...
static int bootstrap_fake_lock(void *opaque, const pmm_daemon_ipc_endpoint_t *endpoint,
...
static bool bootstrap_fake_unlock(void *opaque, pmm_daemon_bootstrap_lock_t *lock_io) {
    bootstrap_fake_ops_t *fake = opaque;
...
static bool bootstrap_fake_handoff(void *opaque, pmm_daemon_bootstrap_lock_t lock) {
    bootstrap_fake_ops_t *fake = opaque;
...
static bool bootstrap_fake_spawn(void *opaque, const pmm_daemon_bootstrap_launch_spec_t *spec) {
    bootstrap_fake_ops_t *fake = opaque;
    ...
    bool exact = spec && spec->argc == 2U && spec->argv[0] &&
                 ...
                 strcmp(spec->argv[1], PMM_DAEMON_INTERNAL_ARG) == 0 && spec->detached &&
...
static pmm_daemon_bootstrap_ops_t bootstrap_fake_callbacks(bootstrap_fake_ops_t *fake) {
    pmm_daemon_bootstrap_ops_t ops = {
        .context = fake,
...
static void *bootstrap_thread_execute(void *opaque) {
    ...
    call->status = pmm_daemon_bootstrap_execute_with_ops(call->config, call->ops, &call->result);
    return NULL;
}

static pmm_daemon_process_role_t classify(int argc, char **argv) {
    return pmm_daemon_process_role(argc, argv);
}

TEST(daemon_bootstrap_classifies_default_and_ui_as_mcp_clients) {
    char *plain[] = {"project-mind-mcp", NULL};
    char *ui[] = {"project-mind-mcp", "--ui=true", "--port=9750", NULL};
    ASSERT_EQ(classify(1, plain), PMM_DAEMON_PROCESS_MCP_CLIENT);
    ASSERT_EQ(classify(3, ui), PMM_DAEMON_PROCESS_MCP_CLIENT);
    ASSERT_TRUE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_MCP_CLIENT));
    PASS();
}

TEST(daemon_bootstrap_classifies_stateless_commands_without_client) {
    char *version[] = {"project-mind-mcp", "--version", NULL};
    char *help[] = {"project-mind-mcp", "--profile", "--help", NULL};
    char *install[] = {"project-mind-mcp", "install", "--dry-run", NULL};
    char *uninstall[] = {"project-mind-mcp", "uninstall", NULL};
    char *update[] = {"project-mind-mcp", "update", "-n", NULL};
    ASSERT_EQ(classify(2, version), PMM_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(3, help), PMM_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(3, install), PMM_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(2, uninstall), PMM_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(3, update), PMM_DAEMON_PROCESS_STATELESS);
    ASSERT_FALSE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_STATELESS));
    PASS();
}

TEST(daemon_bootstrap_classifies_config_as_coordinated_local_cli) {
    char *list[] = {"project-mind-mcp", "config", "list", NULL};
    char *set[] = {"project-mind-mcp", "config", "set", "auto_watch", "false", NULL};
    char *help[] = {"project-mind-mcp", "config", "--help", NULL};
    ASSERT_EQ(classify(3, list), PMM_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_EQ(classify(5, set), PMM_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_EQ(classify(3, help), PMM_DAEMON_PROCESS_STATELESS);
    ASSERT_FALSE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_LOCAL_CLI));
    PASS();
}

TEST(daemon_bootstrap_cli_help_is_stateless_but_tool_calls_are_local) {
    char *tool_help[] = {"project-mind-mcp", "cli", "search_graph", "--help", NULL};
    char *tool_call[] = {"project-mind-mcp", "cli", "search_graph", "{}", NULL};
    ASSERT_EQ(classify(4, tool_help), PMM_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(4, tool_call), PMM_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_FALSE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_LOCAL_CLI));
    PASS();
}
...
TEST(daemon_bootstrap_cli_arguments_cannot_reclassify_the_process) {
    char *install_value[] = {
        "project-mind-mcp", "cli", "search_code", "--query", "install", NULL};
    char *version_value[] = {"project-mind-mcp", "cli", "search_code", "--query",
                             "--version",           NULL};
    ASSERT_EQ(classify(5, install_value), PMM_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_EQ(classify(5, version_value), PMM_DAEMON_PROCESS_LOCAL_CLI);
    PASS();
}
...
TEST(daemon_bootstrap_internal_roles_never_take_client_leases) {
    ...
    char *daemon[] = {"project-mind-mcp", PMM_DAEMON_INTERNAL_ARG, NULL};
    char *worker[] = {"project-mind-mcp", "cli", "--index-worker", "--index-worker-build", build,
                      "index_repository",    "{}",  "--response-out", "/tmp/response",        NULL};
    char *malformed_worker[] = {"project-mind-mcp", "cli", "--index-worker",
                                "index_repository",    "{}",  NULL};
    char *reserved_user_value[] = {"project-mind-mcp", "cli", "search_code", "--query",
                                   "--index-worker",      NULL};
    char *hook[] = {"project-mind-mcp", "hook-augment", NULL};
    ASSERT_EQ(classify(2, daemon), PMM_DAEMON_PROCESS_DAEMON);
    ASSERT_EQ(classify(9, worker), PMM_DAEMON_PROCESS_WORKER);
    ASSERT_EQ(classify(5, malformed_worker), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(5, reserved_user_value), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(2, hook), PMM_DAEMON_PROCESS_HOOK_CLIENT);
    ASSERT_FALSE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_DAEMON));
    ASSERT_FALSE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_WORKER));
    ASSERT_TRUE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_HOOK_CLIENT));
    PASS();
}

TEST(daemon_bootstrap_rejects_ambiguous_internal_daemon_argv) {
    char *missing[] = {NULL};
    char *mixed[] = {"project-mind-mcp", PMM_DAEMON_INTERNAL_ARG, "cli", NULL};
    ASSERT_EQ(classify(0, missing), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(3, mixed), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_FALSE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_INVALID));
    PASS();
}
...
TEST(daemon_bootstrap_uses_one_stable_per_account_endpoint) {
    ...
    pmm_daemon_ipc_endpoint_t *second = pmm_daemon_bootstrap_endpoint_new(fixture.parent);
    ASSERT_NOT_NULL(second);
    ASSERT_STR_EQ(pmm_daemon_ipc_endpoint_address(fixture.endpoint),
                  pmm_daemon_ipc_endpoint_address(second));
    pmm_daemon_ipc_endpoint_free(second);
    bootstrap_endpoint_fixture_finish(&fixture);
    PASS();
}

TEST(daemon_bootstrap_launches_only_exact_detached_hidden_role) {
    pmm_daemon_bootstrap_launch_spec_t spec;
    ASSERT_TRUE(pmm_daemon_bootstrap_launch_spec_init("/tmp/pmm exact", &spec));
    ASSERT_EQ(spec.argc, 2U);
    ASSERT_STR_EQ(spec.executable_path, "/tmp/pmm exact");
    ASSERT_STR_EQ(spec.argv[0], "/tmp/pmm exact");
    ASSERT_STR_EQ(spec.argv[1], PMM_DAEMON_INTERNAL_ARG);
...
TEST(daemon_bootstrap_permanent_daemon_argv_is_byte_exact) {
    char *permanent[] = {"project-mind-mcp", PMM_DAEMON_INTERNAL_ARG, PMM_DAEMON_PERMANENT_ARG,
                         NULL};
    char *reordered[] = {"project-mind-mcp", PMM_DAEMON_PERMANENT_ARG, PMM_DAEMON_INTERNAL_ARG,
                         NULL};
    char *repeated[] = {"project-mind-mcp", PMM_DAEMON_INTERNAL_ARG, PMM_DAEMON_INTERNAL_ARG,
                        NULL};
    char *extended[] = {"project-mind-mcp", PMM_DAEMON_INTERNAL_ARG, PMM_DAEMON_PERMANENT_ARG,
                        "extra", NULL};
    char *wrong_flag[] = {"project-mind-mcp", PMM_DAEMON_INTERNAL_ARG, "--permanent", NULL};
    ASSERT_EQ(classify(3, permanent), PMM_DAEMON_PROCESS_DAEMON);
    ASSERT_EQ(classify(3, reordered), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(3, repeated), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(4, extended), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(3, wrong_flag), PMM_DAEMON_PROCESS_INVALID);
    PASS();
}

TEST(daemon_bootstrap_daemon_ctl_token_routes_after_cli) {
    char *start[] = {"project-mind-mcp", "daemon", "start", NULL};
    char *stop[] = {"project-mind-mcp", "daemon", "stop", NULL};
    char *status[] = {"project-mind-mcp", "daemon", "status", NULL};
    char *help[] = {"project-mind-mcp", "daemon", "--help", NULL};
    /* `daemon` after `cli` is opaque tool input, never a control command. */
    char *opaque[] = {"project-mind-mcp", "cli", "search_code", "daemon", "start", NULL};
    ASSERT_EQ(classify(3, start), PMM_DAEMON_PROCESS_DAEMON_CTL);
    ASSERT_EQ(classify(3, stop), PMM_DAEMON_PROCESS_DAEMON_CTL);
    ASSERT_EQ(classify(3, status), PMM_DAEMON_PROCESS_DAEMON_CTL);
    ASSERT_EQ(classify(3, help), PMM_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(5, opaque), PMM_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_FALSE(pmm_daemon_process_role_requires_client(PMM_DAEMON_PROCESS_DAEMON_CTL));
    PASS();
}

TEST(daemon_bootstrap_permanent_launch_spec_is_exact) {
    pmm_daemon_bootstrap_launch_spec_t spec;
    ASSERT_TRUE(pmm_daemon_bootstrap_launch_spec_init_permanent("/tmp/pmm exact", &spec));
    ASSERT_EQ(spec.argc, 3U);
    ASSERT_STR_EQ(spec.argv[0], "/tmp/pmm exact");
    ASSERT_STR_EQ(spec.argv[1], PMM_DAEMON_INTERNAL_ARG);
    ASSERT_STR_EQ(spec.argv[2], PMM_DAEMON_PERMANENT_ARG);
...
TEST(daemon_bootstrap_stateless_roles_bypass_every_daemon_operation) {
    ...
    pmm_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    pmm_daemon_bootstrap_config_t config = {
        .role = PMM_DAEMON_PROCESS_STATELESS,
    };
    pmm_daemon_bootstrap_result_t result;
    ASSERT_EQ(pmm_daemon_bootstrap_execute_with_ops(&config, &ops, &result),
              PMM_DAEMON_BOOTSTRAP_BYPASSED);
    ASSERT_EQ(result.status, PMM_DAEMON_BOOTSTRAP_BYPASSED);
...
TEST(daemon_bootstrap_cohort_conflict_is_visible_before_probe_or_spawn) {
    ...
    bootstrap_fake_ops_t fake = {0};
    fake.forced_cohort = PMM_VERSION_COHORT_CONFLICT;
    pmm_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    pmm_daemon_build_identity_t identity = bootstrap_identity("2.4.0", BOOTSTRAP_BUILD_B);
    pmm_daemon_bootstrap_config_t config = {
        .role = PMM_DAEMON_PROCESS_MCP_CLIENT,
        .endpoint = fixture.endpoint,
        .identity = &identity,
        .executable_path = "/tmp/pmm",
    ...
    ASSERT_EQ(result.status, PMM_DAEMON_BOOTSTRAP_CONFLICT);
    ...
    ASSERT_NOT_NULL(strstr(fake.diagnostic, "conflicting PMM process is active"));
    ASSERT_NOT_NULL(strstr(fake.diagnostic, "Close all PMM sessions and commands"));
    bootstrap_endpoint_fixture_finish(&fixture);
    PASS();
}
...
TEST(daemon_bootstrap_existing_exact_daemon_connects_without_spawn) {
    ...
    atomic_store(&fake.available, true);
    pmm_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    pmm_daemon_build_identity_t identity = bootstrap_identity("2.4.0", BOOTSTRAP_BUILD_A);
    pmm_daemon_bootstrap_config_t config = {
        .role = PMM_DAEMON_PROCESS_MCP_CLIENT,
        .endpoint = fixture.endpoint,
        .identity = &identity,
    ...
    ASSERT_EQ(result.status, PMM_DAEMON_BOOTSTRAP_CONNECTED);
...
TEST(daemon_bootstrap_conflict_is_visible_and_never_spawns) {
    ...
    bootstrap_fake_ops_t fake = {0};
    fake.forced_probe = PMM_DAEMON_BOOTSTRAP_PROBE_CONFLICT;
    pmm_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    pmm_daemon_build_identity_t identity = bootstrap_identity("2.4.0", BOOTSTRAP_BUILD_B);
    pmm_daemon_bootstrap_config_t config = {
        .role = PMM_DAEMON_PROCESS_MCP_CLIENT,
...
TEST(daemon_bootstrap_terminal_generation_that_never_exits_is_not_replaced) {
    ...
    bootstrap_fake_ops_t fake = {0};
    fake.forced_probe = PMM_DAEMON_BOOTSTRAP_PROBE_TERMINAL;
    pmm_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    pmm_daemon_build_identity_t identity = bootstrap_identity("2.4.0", BOOTSTRAP_BUILD_A);
    pmm_daemon_bootstrap_config_t config = {
        .role = PMM_DAEMON_PROCESS_HOOK_CLIENT,
        .endpoint = fixture.endpoint,
        .identity = &identity,
        .executable_path = "/tmp/pmm",
...
TEST(daemon_bootstrap_rejected_connect_is_reserved_and_never_unavailable) {
    pmm_daemon_runtime_connect_result_t capacity = {0};
    capacity.status = PMM_DAEMON_RUNTIME_CONNECT_REJECTED;
    snprintf(capacity.message, sizeof(capacity.message), "PMM daemon connection capacity reached");
    ASSERT_EQ(pmm_daemon_bootstrap_classify_failed_connect(&capacity, 1),
              PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ASSERT_EQ(pmm_daemon_bootstrap_classify_failed_connect(&capacity, 0),
              PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ASSERT_EQ(pmm_daemon_bootstrap_classify_failed_connect(&capacity, -1),
              PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED);

    pmm_daemon_runtime_connect_result_t stopping = capacity;
    snprintf(stopping.message, sizeof(stopping.message), "PMM daemon is stopping");
    ASSERT_EQ(pmm_daemon_bootstrap_classify_failed_connect(&stopping, 1),
              PMM_DAEMON_BOOTSTRAP_PROBE_TERMINAL);

    pmm_daemon_runtime_connect_result_t absent = {0};
    absent.status = PMM_DAEMON_RUNTIME_CONNECT_ERROR;
    ASSERT_EQ(pmm_daemon_bootstrap_classify_failed_connect(&absent, 1),
              PMM_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ASSERT_EQ(pmm_daemon_bootstrap_classify_failed_connect(&absent, 0),
              PMM_DAEMON_BOOTSTRAP_PROBE_UNAVAILABLE);
    ASSERT_EQ(pmm_daemon_bootstrap_classify_failed_connect(&absent, -1),
              PMM_DAEMON_BOOTSTRAP_PROBE_ERROR);
    PASS();
}
...
TEST(daemon_bootstrap_concurrent_first_clients_spawn_one_daemon) {
    ...
    ASSERT_EQ(pmm_thread_create(&threads[0], 0, bootstrap_thread_execute, &calls[0]), 0);
    ASSERT_EQ(pmm_thread_create(&threads[1], 0, bootstrap_thread_execute, &calls[1]), 0);
    uint64_t ready_deadline = pmm_now_ms() + BOOTSTRAP_TEST_TIMEOUT_MS;
    while (atomic_load(&ready) != 2 && pmm_now_ms() < ready_deadline) {
        const struct timespec pause = {0, 1000000L};
        (void)pmm_nanosleep(&pause, NULL);
    }
    atomic_store(&go, true);
    ASSERT_EQ(pmm_thread_join(&threads[0]), 0);
    ASSERT_EQ(pmm_thread_join(&threads[1]), 0);
    ASSERT_EQ(calls[0].status, PMM_DAEMON_BOOTSTRAP_CONNECTED);
    ASSERT_EQ(calls[1].status, PMM_DAEMON_BOOTSTRAP_CONNECTED);
    ASSERT_EQ(atomic_load(&fake.spawn_count), 1);
...
TEST(daemon_bootstrap_darwin_launch_failure_is_synchronous) {
    ...
    int written = snprintf(missing, sizeof(missing), "%s/definitely-missing-pmm", fixture.parent);
    ...
    pmm_daemon_bootstrap_result_t result;
    uint64_t started = pmm_now_ms();
    ASSERT_EQ(pmm_daemon_bootstrap_execute(&config, &result), PMM_DAEMON_BOOTSTRAP_FAILED);
    uint64_t elapsed = pmm_now_ms() - started;
    ASSERT_FALSE(result.daemon_spawned);
