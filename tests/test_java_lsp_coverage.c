...
static CBMFileResult *cov_extract(const char *source) {
    return pmm_extract_file(source, (int)strlen(source), PMM_LANG_JAVA, "test", "Main.java", 0,
                            NULL, NULL);
}
...
TEST(cov_cross_basic) {
    ...
    pmm_run_java_lsp_cross(&arena, src, (int)strlen(src), "test.com.example", defs, 2, imp_names,
                           imp_qns, 1, NULL, &out);
    int found = 0;
...
TEST(cov_cross_no_imports) {
    ...
    CBMResolvedCallArray out;
    memset(&out, 0, sizeof(out));
    pmm_run_java_lsp_cross(&arena, src, (int)strlen(src), "test.A", NULL, 0, NULL, NULL, 0, NULL, &out);
...
TEST(cov_cross_batch) {
    ...
    pmm_batch_java_lsp_cross(&arena, files, 2, outs);
    pmm_arena_destroy(&arena);
    PASS();
}
TEST(cov_cross_inheritance) {
    ...
    defs[1].return_types = "java.lang.String";
    pmm_run_java_lsp_cross(&arena, src, (int)strlen(src), "test.B", defs, 2, NULL, NULL, 0, NULL, &out);
    int found = 0;
    for (int i = 0; i < out.count; i++) {
        if (out.items[i].confidence < 0.5f) continue;
