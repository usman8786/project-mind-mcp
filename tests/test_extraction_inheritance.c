...
static int run_inherit_case(const inherit_case_t *tc) {
    CBMFileResult *r =
        pmm_extract_file(tc->src, (int)strlen(tc->src), tc->lang, "t", tc->path, 0, NULL, NULL);
    if (!r) {
...
static int run_rust_impl(const rust_impl_case_t *tc) {
    CBMFileResult *r = pmm_extract_file(tc->src, (int)strlen(tc->src), PMM_LANG_RUST, "t", "lib.rs",
                                        0, NULL, NULL);
    if (!r) {
