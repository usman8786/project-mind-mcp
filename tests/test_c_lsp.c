 ...
 *
 * AUTO-GENERATED from internal/pmm/lsp_c_test.go by scripts/port_clsp_tests.py
...
static CBMFileResult *extract_c(const char *src) {
    return pmm_extract_file(src, (int)strlen(src), PMM_LANG_C, "test", "main.c", 0, NULL, NULL);
}

static CBMFileResult *extract_cpp(const char *src) {
    return pmm_extract_file(src, (int)strlen(src), PMM_LANG_CPP, "test", "main.cpp", 0, NULL, NULL);
}
...
TEST(clsp_nocrash_issue355_xxhash_header) {
    ...
    buf[rd] = '\0';
    CBMFileResult *r =
        pmm_extract_file(buf, (int)rd, PMM_LANG_C, "test", "xxhash.h", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    pmm_free_result(r);
    free(buf);
    PASS();
...
TEST(clsp_nocrash_empty_source) {
    CBMFileResult *r = pmm_extract_file("", 0, PMM_LANG_CPP, "test", "main.cpp", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    pmm_free_result(r);
    PASS();
}
 ...
 * (pmm_c_build_cross_registry), FINALIZES it (O(1) hash lookups), then shares it
 * READ-ONLY across the parallel resolve workers (ctx.registry_shared = true).
 * pmm_run_c_lsp_cross_with_registry must therefore NOT mutate it.
 *
...
TEST(clsp_tier2_shared_registry_readonly_c) {
    ...
    CBMTypeRegistry *reg = pmm_c_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    ...
    pmm_run_c_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod",
...
TEST(registry_short_name_indexes) {
    ...
    CBMTypeEmbedIter it;
    pmm_registry_types_by_embedded_bare(&reg, "Trait", &it);
    ASSERT_EQ(pmm_type_embed_iter_next(&it), 1);
    ASSERT_EQ(pmm_type_embed_iter_next(&it), 2);
    ASSERT_EQ(pmm_type_embed_iter_next(&it), -1);

    /* Bare "Other": only type 3 (C). */
    pmm_registry_types_by_embedded_bare(&reg, "Other", &it);
    ASSERT_EQ(pmm_type_embed_iter_next(&it), 3);
    ASSERT_EQ(pmm_type_embed_iter_next(&it), -1);

    /* Bare with no implementers: empty. */
    pmm_registry_types_by_embedded_bare(&reg, "Nope", &it);
    ASSERT_EQ(pmm_type_embed_iter_next(&it), -1);

    /* Free-func index for "helper": func 0 then 1 (ascending), NOT the method (2). */
    CBMFreeFuncIter fit;
    pmm_registry_free_funcs_by_short_name(&reg, "helper", &fit);
    ASSERT_EQ(pmm_free_func_iter_next(&fit), 0);
    ASSERT_EQ(pmm_free_func_iter_next(&fit), 1);
    ASSERT_EQ(pmm_free_func_iter_next(&fit), -1);

    /* Free-func "other": only func 3. */
    pmm_registry_free_funcs_by_short_name(&reg, "other", &fit);
    ASSERT_EQ(pmm_free_func_iter_next(&fit), 3);
    ASSERT_EQ(pmm_free_func_iter_next(&fit), -1);
...
TEST(seal_py_shared_registry_readonly) {
    CBMArena arena;
    pmm_arena_init(&arena);
    CBMTypeRegistry *reg = pmm_py_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "def helper(x):\n    return x + 1\n\ndef caller():\n    return helper(1)\n";
    CBMResolvedCallArray out = {0};
    pmm_run_py_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, NULL,
                                       0, NULL, &out);
 ...
 * pmm_registry_add_func/_type (which the read_only seal no-ops). But
 ...
 * read_only registry (pipeline.c:901 → pmm_py_build_cross_registry), so this is a
...
TEST(seal_py_shared_registry_readonly_fields) {
    ...
    defs[0].lang = PMM_LANG_PYTHON;
    defs[1].qualified_name = "test.mod.Foo.m";
    defs[1].short_name = "m";
    defs[1].label = "Method";
    defs[1].receiver_type = "test.mod.Foo";
    defs[1].def_module_qn = "test.mod";
    defs[1].lang = PMM_LANG_PYTHON;

    CBMTypeRegistry *reg = pmm_py_build_cross_registry(&arena, defs, 2);
    ASSERT_NOT_NULL(reg);
    ...
    const CBMRegisteredType *rt = pmm_registry_lookup_type(reg, "test.mod.Foo");
    ASSERT_NOT_NULL(rt);
    ...
    const char *src = "class Foo:\n"
                      ...
                      "        self.x = 1\n";
    CBMResolvedCallArray out = {0};
    pmm_run_py_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, NULL, 0,
                                       NULL, &out);
...
TEST(seal_cs_shared_registry_readonly) {
    CBMArena arena;
    pmm_arena_init(&arena);
    CBMTypeRegistry *reg = pmm_cs_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "namespace N { class Box { int Unwrap() { return 0; }\n"
                      "  int Caller() { return Unwrap(); } } }\n";
    CBMResolvedCallArray out = {0};
    pmm_run_cs_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, 0,
                                       NULL, &out);
    ASSERT_EQ(reg->func_count, fb);
    ASSERT_EQ(reg->type_count, tb);
    pmm_arena_destroy(&arena);
...
TEST(seal_ts_shared_registry_readonly) {
    CBMArena arena;
    pmm_arena_init(&arena);
    CBMTypeRegistry *reg = pmm_ts_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "class Box { unwrap(): number { return 0; } }\n"
                      "function caller(b: Box): number { return b.unwrap(); }\n";
    CBMResolvedCallArray out = {0};
    pmm_run_ts_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", false, false,
...
TEST(seal_go_shared_registry_readonly) {
    CBMArena arena;
    pmm_arena_init(&arena);
    CBMTypeRegistry *reg = pmm_go_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "package main\nfunc helper(x int) int { return x + 1 }\n"
                      "func caller() int { return helper(1) }\n";
    CBMResolvedCallArray out = {0};
    pmm_run_go_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, NULL,
                                       0, NULL, &out);
    ASSERT_EQ(reg->func_count, fb);
    ASSERT_EQ(reg->type_count, tb);
    pmm_arena_destroy(&arena);
