...
# test-only injector (PMM_TEST_HANG_ON) busy-spins on one file, so the worker
...
BINARY="${PMM_TEST_BINARY:-${ROOT}/build/c/project-mind-mcp}"
...
# never ship — see src/main.c and TEST_SEAMS in Makefile.pmm). Against a
...
if ! LC_ALL=C grep -a -q -F 'PMM_TEST_WORKER_DESCENDANT_PID_FILE' "${BINARY}"; then
  echo "binary lacks the crash-orphan test seam: ${BINARY}" >&2
  echo "  rebuild it with:  make -f Makefile.pmm pmm TEST_SEAMS=1" >&2
...
PMM_BINARY="${BINARY}" BUILD_FINGERPRINT="${BUILD_FINGERPRINT}" TMPDIR_PATH="${tmpdir}" \
  ARGS_JSON="{\"repo_path\":\"${tmpdir}/repo\"}" \
  PMM_TEST_HANG_ON=hang_me \
  PMM_TEST_WORKER_DESCENDANT_PID_FILE="${tmpdir}/descendant.pid" \
  "${tmpdir}/wrapper.sh" &
wrapper_pid=$!
