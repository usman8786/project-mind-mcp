...
static pmm_store_t *gpg_open_indexed(GpgProj *lp) {
...
static pmm_store_t *gpg_index_files(GpgProj *lp, const GpgFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_gpg_XXXXXX");
...
static void gpg_cleanup(GpgProj *lp, pmm_store_t *store) {
    if (store)
...
static int gpg_count_label(pmm_store_t *store, const char *project, const char *label) {
...
static GpgMetrics gpg_metrics_files(const GpgFile *files, int nfiles) {
    GpgProj lp;
    pmm_store_t *store = gpg_index_files(&lp, files, nfiles);
    GpgMetrics m = {0};
    if (store) {
        m.ok = 1;
        ...
        m.depends = pmm_store_count_edges_by_type(store, lp.project, "DEPENDS_ON");
    }
...
TEST(probe_json_more_variables) {
     ...
     * detect_file_language() returns PMM_LANG_COUNT and the file is never
...
/* INI: .conf extension also maps to PMM_LANG_INI; must produce same structure. */
...
TEST(probe_dotenv_module_only) {
     ...
     * language.c, so file-based index_repository's pmm_language_for_filename
     * returns PMM_LANG_COUNT and the file is never indexed → 0 Module nodes.
...
TEST(probe_dotenv_local_suffix) {
     ...
     * routes it to PMM_LANG_DOTENV → 0 Module nodes.  Same routing gap as
 ...
 * Extension: pmm_path_ext(".gitignore") → "gitignore" (strips the leading dot).
 * ══════════════════════════════════════════════════════════════════ */
...
TEST(probe_gitignore_module_only) {
     ...
     * NOT indexed as a source file — the pmm_discover walker consumes it as
     * ignore-rule input (see test_discover.c discover_with_gitignore /
     * discover_gitignore_dir_excluded_issue234 / discover_pmmignore_stacks,
     ...
     * to PMM_LANG_GITIGNORE would regress those three established tests. */
...
TEST(probe_properties_module_only) {
     ...
     * property line (extract_defs.c PMM_LANG_PROPERTIES → push_var_def). The
 ...
 * Extension: ".re" maps to PMM_LANG_REGEX in grammar_cases.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * one definition.  Extension: .frm (PMM_LANG_FORM in EXT_TABLE).
 * ══════════════════════════════════════════════════════════════════ */
