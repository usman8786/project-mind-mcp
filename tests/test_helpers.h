 ...
 * leaf made by pmm_mkdtemp() has a private DACL. Keep these security-sensitive
...
#ifdef _WIN32
    const char *base = pmm_app_local_dir();
#else
    const char *base = pmm_tmpdir();
#endif
    if (!base || !base[0]) {
        out[0] = '\0';
        return false;
    }
    int written = snprintf(out, out_cap, "%s/pmm-runtime-%s-XXXXXX", base, tag);
    if (written <= 0 || (size_t)written >= out_cap) {
        out[0] = '\0';
        return false;
    }
    return pmm_mkdtemp(out) != NULL;
}
