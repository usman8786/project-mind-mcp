...
#include "foundation/platform.h" // pmm_normalize_path_sep (drive-canonicalization regression)
#include "test_framework.h"
#include "test_helpers.h"
#include "foundation/mem.h" // pmm_mem_init/budget (back-pressure futile-nap test)
...
static int setup_test_repo(void) {
    snprintf(g_tmpdir, sizeof(g_tmpdir), "/tmp/pmm_test_XXXXXX");
    if (!pmm_mkdtemp(g_tmpdir))
        return -1;
...
TEST(pipeline_create_free) {
    pmm_pipeline_t *p = pmm_pipeline_new("/some/path", NULL, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_STR_EQ(pmm_pipeline_project_name(p), "some-path");
    pmm_pipeline_free(p);
    PASS();
}

TEST(pipeline_null_repo) {
    pmm_pipeline_t *p = pmm_pipeline_new(NULL, NULL, PMM_MODE_FULL);
    ASSERT_NULL(p);
    PASS();
}

TEST(pipeline_free_null) {
    pmm_pipeline_free(NULL); /* should not crash */
    PASS();
}
...
TEST(pipeline_cancel_null) {
    pmm_pipeline_cancel(NULL); /* should not crash */
    PASS();
}

TEST(pipeline_run_null) {
    int rc = pmm_pipeline_run(NULL);
    ASSERT_EQ(rc, -1);
    PASS();
}
...
TEST(store_file_persistence) {
    ...
    /* Write data */
    pmm_store_t *s1 = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s1);
    pmm_store_upsert_project(s1, "proj", "/tmp");
    pmm_node_t n = {.project = "proj",
                    .label = "Function",
                    .name = "foo",
                    .qualified_name = "proj.foo",
                    .file_path = "f.go"};
    int64_t id = pmm_store_upsert_node(s1, &n);
    ASSERT_GT(id, 0);
    int cnt1 = pmm_store_count_nodes(s1, "proj");
    ASSERT_EQ(cnt1, 1);
    pmm_store_checkpoint(s1);
    pmm_store_close(s1);

    /* Reopen and verify */
    pmm_store_t *s2 = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s2);
    int cnt2 = pmm_store_count_nodes(s2, "proj");
    ASSERT_EQ(cnt2, 1);
    pmm_store_close(s2);
...
TEST(store_bulk_persistence) {
    ...
    pmm_store_t *s = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s);

    pmm_store_upsert_project(s, "proj", "/tmp");
    ...
    ASSERT_GT(id, 0);
    ASSERT_EQ(pmm_store_commit(s), 0);
    pmm_store_end_bulk(s);
    pmm_store_checkpoint(s);
    pmm_store_close(s);

    /* Reopen and verify data survived */
    pmm_store_t *s2 = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s2);
    ASSERT_EQ(pmm_store_count_nodes(s2, "proj"), 1);
    pmm_store_close(s2);
...
TEST(pipeline_structure_nodes) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ...
    const char *project = pmm_pipeline_project_name(p);

    /* Should have nodes: 1 Project + 2 Folders + 3 Files + N Functions */
    int node_count = pmm_store_count_nodes(s, project);
    ASSERT_GTE(node_count, 9); /* 6 structure + at least 3 definitions */

    /* Verify project node exists */
    pmm_node_t proj_node = {0};
    rc = pmm_store_find_node_by_qn(s, project, project, &proj_node);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(proj_node.label, "Project");
    pmm_node_free_fields(&proj_node);

    /* Verify folder nodes */
    pmm_node_t *folders = NULL;
    int folder_count = 0;
    rc = pmm_store_find_nodes_by_label(s, project, "Folder", &folders, &folder_count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_GTE(folder_count, 2); /* pkg, pkg/util */
    pmm_store_free_nodes(folders, folder_count);

    /* Verify file nodes */
    pmm_node_t *file_nodes = NULL;
    int file_count = 0;
    rc = pmm_store_find_nodes_by_label(s, project, "File", &file_nodes, &file_count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_GTE(file_count, 3); /* main.go, service.go, helper.go */
    pmm_store_free_nodes(file_nodes, file_count);

    /* Verify edges exist */
    int edge_count = pmm_store_count_edges(s, project);
...
TEST(pipeline_adr_survives_full_reindex) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_adr_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
        FAIL("failed to create temp dir");
    }
    ...
    pmm_pipeline_t *p1 = pmm_pipeline_new(tmp, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p1);
    ASSERT_EQ(pmm_pipeline_run(p1), 0);
    const char *project = pmm_pipeline_project_name(p1);
    char project_copy[256];
    snprintf(project_copy, sizeof(project_copy), "%s", project);
    pmm_pipeline_free(p1);

    /* Store an ADR. */
    const char *adr_text = "# Decision\nWe chose X over Y.";
    pmm_store_t *s1 = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s1);
    ASSERT_EQ(pmm_store_adr_store(s1, project_copy, adr_text), PMM_STORE_OK);
    pmm_store_close(s1);
    ...
    pmm_pipeline_t *p2 = pmm_pipeline_new(tmp, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p2);
    ASSERT_EQ(pmm_pipeline_run(p2), 0);
    pmm_pipeline_free(p2);

    /* The ADR must still be present and unchanged. */
    pmm_store_t *s2 = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s2);
    pmm_adr_t adr = {0};
    int rc = pmm_store_adr_get(s2, project_copy, &adr);
    ASSERT_EQ(rc, PMM_STORE_OK);
...
TEST(pipeline_structure_edges) {
    ...
    int cf_count = pmm_store_count_edges_by_type(s, project, "CONTAINS_FILE");
    /* Check CONTAINS_FOLDER edges */
    int cd_count = pmm_store_count_edges_by_type(s, project, "CONTAINS_FOLDER");
...
TEST(pipeline_branch_root_structure) {
    ...
    pmm_node_t project_node = {0};
    pmm_node_t branch_node = {0};
    pmm_node_t root_file_node = {0};
    pmm_node_t root_folder_node = {0};
    rc = pmm_store_find_node_by_qn(s, project, project, &project_node);
    ASSERT_EQ(rc, PMM_STORE_OK);
    rc = pmm_store_find_node_by_qn(s, project, branch_qn, &branch_node);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    char *root_folder_qn = pmm_pipeline_fqn_folder(project, "pkg");
    char *root_file_qn = pmm_pipeline_fqn_compute(project, "main.go", "__file__");
    ASSERT_NOT_NULL(root_folder_qn);
    ASSERT_NOT_NULL(root_file_qn);
    rc = pmm_store_find_node_by_qn(s, project, root_folder_qn, &root_folder_node);
    ASSERT_EQ(rc, PMM_STORE_OK);
    rc = pmm_store_find_node_by_qn(s, project, root_file_qn, &root_file_node);
    ASSERT_EQ(rc, PMM_STORE_OK);

    pmm_edge_t *has_branch = NULL;
    int has_branch_count = 0;
    rc = pmm_store_find_edges_by_source_type(s, project_node.id, "HAS_BRANCH", &has_branch,
                                             &has_branch_count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(has_branch_count, 1);
    ASSERT_EQ(has_branch[0].target_id, branch_node.id);

    pmm_edge_t *project_files = NULL;
    int project_file_count = 0;
    rc = pmm_store_find_edges_by_source_type(s, project_node.id, "CONTAINS_FILE", &project_files,
                                             &project_file_count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(project_file_count, 0);

    pmm_edge_t *branch_files = NULL;
    int branch_file_count = 0;
    rc = pmm_store_find_edges_by_source_type(s, branch_node.id, "CONTAINS_FILE", &branch_files,
                                             &branch_file_count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(branch_file_count, 1);
    ASSERT_EQ(branch_files[0].target_id, root_file_node.id);

    pmm_edge_t *branch_folders = NULL;
    int branch_folder_count = 0;
    rc = pmm_store_find_edges_by_source_type(s, branch_node.id, "CONTAINS_FOLDER", &branch_folders,
                                             &branch_folder_count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(branch_folder_count, 1);
    ASSERT_EQ(branch_folders[0].target_id, root_folder_node.id);

    pmm_store_free_edges(has_branch, has_branch_count);
    pmm_store_free_edges(project_files, project_file_count);
    pmm_store_free_edges(branch_files, branch_file_count);
    pmm_store_free_edges(branch_folders, branch_folder_count);
    pmm_node_free_fields(&project_node);
    pmm_node_free_fields(&branch_node);
    pmm_node_free_fields(&root_file_node);
    pmm_node_free_fields(&root_folder_node);
    free(root_folder_qn);
    free(root_file_qn);
    pmm_store_close(s);
    pmm_pipeline_free(p);
...
TEST(pipeline_project_name_derived) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_tmpdir, NULL, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);

    /* Project name should be derived from tmpdir path */
    const char *name = pmm_pipeline_project_name(p);
...
TEST(pipeline_fast_mode) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_tmpdir, db_path, PMM_MODE_FAST);
    ASSERT_NOT_NULL(p);
...
TEST(pipeline_definitions_function_nodes) {
    ...
    pmm_node_t *funcs = NULL;
    int func_count = 0;
    rc = pmm_store_find_nodes_by_label(s, project, "Function", &funcs, &func_count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    pmm_store_free_nodes(funcs, func_count);
    pmm_store_close(s);
    pmm_pipeline_free(p);
    teardown_test_repo();
    PASS();
...
TEST(pipeline_definitions_defines_edges) {
    ...
    int defines_count = pmm_store_count_edges_by_type(s, project, "DEFINES");
...
TEST(pipeline_def_props_valid_json_when_oversized) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);
    ...
    pmm_node_t *funcs = NULL;
    int func_count = 0;
    ASSERT_EQ(pmm_store_find_nodes_by_label(s, project, "Function", &funcs, &func_count),
              PMM_STORE_OK);
    int checked = 0;
    for (int i = 0; i < func_count; i++) {
...
TEST(pipeline_edge_props_valid_json) {
    ...
    int edge_count = 0;
    ASSERT_EQ(pmm_store_find_edges_by_type(s, project, "DECORATES", &edges, &edge_count),
              PMM_STORE_OK);
    ...
    pmm_store_free_edges(edges, edge_count);
    pmm_store_close(s);
    pmm_pipeline_free(p);
    teardown_test_repo();
    PASS();
...
TEST(pipeline_calls_resolution) {
    ...
    int calls_count = pmm_store_count_edges_by_type(s, project, "CALLS");
...
static bool cross_file_call_exists(pmm_store_t *s, const char *project, const char *src_name,
                                   const char *tgt_name) {
    pmm_node_t *srcs = NULL;
    pmm_node_t *tgts = NULL;
    int sc = 0;
    int tc = 0;
    pmm_store_find_nodes_by_name(s, project, src_name, &srcs, &sc);
    pmm_store_find_nodes_by_name(s, project, tgt_name, &tgts, &tc);
    bool found = false;
    for (int i = 0; i < sc && !found; i++) {
        pmm_edge_t *edges = NULL;
        int ec = 0;
        pmm_store_find_edges_by_source_type(s, srcs[i].id, "CALLS", &edges, &ec);
        ...
        if (edges) {
            pmm_store_free_edges(edges, ec);
        }
    }
    if (srcs) {
        pmm_store_free_nodes(srcs, sc);
    }
    if (tgts) {
        pmm_store_free_nodes(tgts, tc);
    }
    return found;
}
...
TEST(pipeline_incremental_preserves_cross_file_calls) {
    ...
    /* 1. Full index. */
    pmm_pipeline_t *p1 = pmm_pipeline_new(g_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p1);
    ASSERT_EQ(pmm_pipeline_run(p1), 0);
    const char *project = pmm_pipeline_project_name(p1);
    ...
    pmm_store_t *s1 = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s1);
    int calls_before = pmm_store_count_edges_by_type(s1, project, "CALLS");
    ...
    pmm_pipeline_t *p2 = pmm_pipeline_new(g_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p2);
    ASSERT_EQ(pmm_pipeline_run(p2), 0);
    ...
    const char *project2 = pmm_pipeline_project_name(p2);
    pmm_store_t *s2 = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s2);
    int calls_after = pmm_store_count_edges_by_type(s2, project2, "CALLS");
...
TEST(pipeline_tsjs_receiver_suppresses_weak_method_edge) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_tsjs_recv_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
        FAIL("tmpdir");
    }
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(tmp, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);
    const char *project = pmm_pipeline_project_name(p);
    ...
    ASSERT_GTE(pmm_store_count_edges_by_type(s, project, "CALLS"), 2);
...
static int count_nodes_named(pmm_store_t *s, const char *project, const char *name) {
    pmm_node_t *ns = NULL;
    int n = 0;
    pmm_store_find_nodes_by_name(s, project, name, &ns, &n);
    if (ns) {
        pmm_store_free_nodes(ns, n);
    }
    return n;
}
 ...
 * The regex false edge must stay suppressed in parallel too. PMM_WORKERS forces
...
TEST(pipeline_tsjs_receiver_parallel_keeps_service_edges) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_tsjs_par_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
        FAIL("tmpdir");
    }
    ...
    char *old_workers = getenv("PMM_WORKERS");
    char *saved = old_workers ? strdup(old_workers) : NULL;
    pmm_setenv("PMM_WORKERS", "4", 1); /* force parallel regardless of host cores */
    ...
    ASSERT_GTE(pmm_store_count_edges_by_type(s, project, "HTTP_CALLS"), 3);
    ...
    pmm_pipeline_free(p);
    if (saved) {
        pmm_setenv("PMM_WORKERS", saved, 1);
        free(saved);
    } else {
        pmm_unsetenv("PMM_WORKERS");
    }
    th_rmtree(tmp);
    PASS();
}
...
TEST(pipeline_native_fetch_classified_as_http_calls) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_fetch_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
        FAIL("tmpdir");
    }
    ...
    ASSERT_GTE(pmm_store_count_edges_by_type(s, project, "HTTP_CALLS"), 1);
    /* Exactly the bare call, not the method call too. */
    ASSERT_EQ(pmm_store_count_edges_by_type(s, project, "HTTP_CALLS"), 1);
 ...
 * which would otherwise re-derive PMM_SVC_NONE for "fetch" and silently fall
 * through to a plain CALLS edge). */
TEST(pipeline_native_fetch_parallel_classified_as_http_calls) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_fetch_par_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
        FAIL("tmpdir");
    }
...
TEST(pipeline_local_fetch_shadow_not_classified_as_http) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_fetch_shadow_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
        FAIL("tmpdir");
    }
    ...
    ASSERT_EQ(pmm_store_count_edges_by_type(s, project, "HTTP_CALLS"), 0);
...
TEST(githistory_is_trackable) {
    ...
    ASSERT_TRUE(pmm_is_trackable_file("main.go"));
    ASSERT_TRUE(pmm_is_trackable_file("src/app.py"));
    ASSERT_TRUE(pmm_is_trackable_file("README.md"));

    /* Non-trackable: skip prefixes */
    ASSERT_FALSE(pmm_is_trackable_file("node_modules/foo/bar.js"));
    ASSERT_FALSE(pmm_is_trackable_file("vendor/lib/dep.go"));
    ASSERT_FALSE(pmm_is_trackable_file(".git/config"));
    ASSERT_FALSE(pmm_is_trackable_file("__pycache__/mod.pyc"));

    /* Non-trackable: lock files */
    ASSERT_FALSE(pmm_is_trackable_file("package-lock.json"));
    ASSERT_FALSE(pmm_is_trackable_file("go.sum"));

    /* Non-trackable: binary/minified extensions */
    ASSERT_FALSE(pmm_is_trackable_file("image.png"));
    ASSERT_FALSE(pmm_is_trackable_file("src/style.min.css"));
...
TEST(githistory_compute_coupling) {
    ...
    pmm_commit_files_t commits[] = {
        {files_0, 3, 0}, {files_1, 2, 0}, {files_2, 2, 0}, {files_3, 2, 0}, {files_4, 2, 0},
    };

    pmm_change_coupling_t results[100];
    int n = pmm_compute_change_coupling(commits, 5, results, 100);
...
TEST(githistory_coupling_carries_last_co_change) {
    ...
    pmm_change_coupling_t results[16];
    int n = pmm_compute_change_coupling(commits, 4, results, 16);
    ASSERT_GTE(n, 1);
...
TEST(githistory_skip_large_commits) {
    ...
    pmm_commit_files_t commits[] = {{files, 25, 0}};

    pmm_change_coupling_t results[100];
    int n = pmm_compute_change_coupling(commits, 1, results, 100);
    ASSERT_EQ(n, 0);
...
TEST(githistory_limits_to_max) {
    ...
    pmm_commit_files_t *commits = calloc(ncommits, sizeof(pmm_commit_files_t));
    ...
    pmm_change_coupling_t results[100];
    int n = pmm_compute_change_coupling(commits, ncommits, results, 100);
    ASSERT_TRUE(n <= 100);
...
TEST(testdetect_is_test_file) {
    ...
    ASSERT_TRUE(pmm_is_test_path("foo_test.go"));
    ASSERT_TRUE(pmm_is_test_path("test_handler.py"));
    ASSERT_TRUE(pmm_is_test_path("handler.test.js"));
    ASSERT_TRUE(pmm_is_test_path("handler.spec.ts"));
    ASSERT_TRUE(pmm_is_test_path("Component.test.tsx"));
    ASSERT_TRUE(pmm_is_test_path("OrderTest.java"));
    ASSERT_TRUE(pmm_is_test_path("handler_test.rs"));
    ASSERT_TRUE(pmm_is_test_path("handler_test.cpp"));
    ASSERT_TRUE(pmm_is_test_path("OrderTest.cs"));
    ASSERT_TRUE(pmm_is_test_path("OrderTest.php"));
    ASSERT_TRUE(pmm_is_test_path("OrderSpec.scala"));
    ASSERT_TRUE(pmm_is_test_path("OrderTest.kt"));
    ASSERT_TRUE(pmm_is_test_path("handler_test.lua"));

    /* Non-test file patterns */
    ASSERT_FALSE(pmm_is_test_path("foo.go"));
    ASSERT_FALSE(pmm_is_test_path("handler.py"));
    ASSERT_FALSE(pmm_is_test_path("handler.js"));
    ASSERT_FALSE(pmm_is_test_path("handler.ts"));
    ASSERT_FALSE(pmm_is_test_path("Component.tsx"));
    ASSERT_FALSE(pmm_is_test_path("Order.java"));
    ASSERT_FALSE(pmm_is_test_path("handler.rs"));
    ASSERT_FALSE(pmm_is_test_path("handler.cpp"));
    ASSERT_FALSE(pmm_is_test_path("Order.cs"));
    ASSERT_FALSE(pmm_is_test_path("Order.php"));
    ASSERT_FALSE(pmm_is_test_path("Order.scala"));
    ASSERT_FALSE(pmm_is_test_path("Order.kt"));
    ASSERT_FALSE(pmm_is_test_path("handler.lua"));
...
TEST(testdetect_is_test_function) {
    ...
    ASSERT_TRUE(pmm_is_test_func_name("TestCreate"));  /* Go */
    ASSERT_TRUE(pmm_is_test_func_name("test_create")); /* Python/Rust/Lua */
    ASSERT_TRUE(pmm_is_test_func_name("test"));        /* JS/TS */
    ASSERT_TRUE(pmm_is_test_func_name("describe"));    /* JS/TS */
    ASSERT_TRUE(pmm_is_test_func_name("it"));          /* JS/TS */
    ASSERT_TRUE(pmm_is_test_func_name("testCreate"));  /* Java/PHP/Scala/Kotlin */
    ASSERT_TRUE(pmm_is_test_func_name("TestCreate"));  /* C++/C# */

    /* Non-test function patterns */
    ASSERT_FALSE(pmm_is_test_func_name("create"));
    ASSERT_FALSE(pmm_is_test_func_name("handleRequest"));
    ASSERT_FALSE(pmm_is_test_func_name("process"));
...
TEST(implements_creates_override) {
    ...
    pmm_gbuf_t *gb = pmm_gbuf_new("test-proj", "/tmp/test");
    ASSERT_NOT_NULL(gb);

    /* Interface "Reader" with methods "Read" and "Close" */
    int64_t iface_id =
        pmm_gbuf_upsert_node(gb, "Interface", "Reader", "pkg.Reader", "pkg/reader.go", 1, 5, "{}");
    ASSERT_GT(iface_id, 0);

    int64_t read_method_id =
        pmm_gbuf_upsert_node(gb, "Method", "Read", "pkg.Reader.Read", "pkg/reader.go", 2, 2, "{}");
    int64_t close_method_id = pmm_gbuf_upsert_node(gb, "Method", "Close", "pkg.Reader.Close",
                                                   "pkg/reader.go", 3, 3, "{}");
    ASSERT_GT(read_method_id, 0);
    ASSERT_GT(close_method_id, 0);

    /* DEFINES_METHOD edges */
    pmm_gbuf_insert_edge(gb, iface_id, read_method_id, "DEFINES_METHOD", "{}");
    pmm_gbuf_insert_edge(gb, iface_id, close_method_id, "DEFINES_METHOD", "{}");

    /* Class "FileReader" with matching methods */
    int64_t class_id = pmm_gbuf_upsert_node(gb, "Class", "FileReader", "pkg.FileReader",
                                            "pkg/filereader.go", 1, 10, "{}");
    ASSERT_GT(class_id, 0);

    int64_t fr_read_id =
        pmm_gbuf_upsert_node(gb, "Method", "Read", "pkg.FileReader.Read", "pkg/filereader.go", 2, 4,
                             "{\"receiver\":\"(f *FileReader)\"}");
    int64_t fr_close_id =
        pmm_gbuf_upsert_node(gb, "Method", "Close", "pkg.FileReader.Close", "pkg/filereader.go", 5,
    ...
    int edges_created = pmm_pipeline_implements_go(&ctx);
    ASSERT_GT(edges_created, 0);

    /* Verify IMPLEMENTS edge: FileReader → Reader */
    const pmm_gbuf_edge_t **impl_edges = NULL;
    int impl_count = 0;
    int rc =
        pmm_gbuf_find_edges_by_source_type(gb, class_id, "IMPLEMENTS", &impl_edges, &impl_count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(impl_count, 1);
    ASSERT_EQ(impl_edges[0]->target_id, iface_id);

    /* Verify OVERRIDE edges: FileReader.Read → Reader.Read */
    const pmm_gbuf_edge_t **read_overrides = NULL;
    int read_override_count = 0;
    rc = pmm_gbuf_find_edges_by_source_type(gb, fr_read_id, "OVERRIDE", &read_overrides,
                                            &read_override_count);
    ASSERT_EQ(rc, 0);
    ...
    const pmm_gbuf_edge_t **close_overrides = NULL;
    int close_override_count = 0;
    rc = pmm_gbuf_find_edges_by_source_type(gb, fr_close_id, "OVERRIDE", &close_overrides,
...
TEST(implements_no_match) {
    ...
    int64_t iface_id = pmm_gbuf_upsert_node(gb, "Interface", "ReadWriter", "pkg.ReadWriter",
                                            "pkg/rw.go", 1, 5, "{}");
    int64_t read_id =
        pmm_gbuf_upsert_node(gb, "Method", "Read", "pkg.ReadWriter.Read", "pkg/rw.go", 2, 2, "{}");
    int64_t write_id = pmm_gbuf_upsert_node(gb, "Method", "Write", "pkg.ReadWriter.Write",
                                            "pkg/rw.go", 3, 3, "{}");
    pmm_gbuf_insert_edge(gb, iface_id, read_id, "DEFINES_METHOD", "{}");
    pmm_gbuf_insert_edge(gb, iface_id, write_id, "DEFINES_METHOD", "{}");

    /* Struct "OnlyReader" with only "Read" (missing "Write") */
    int64_t class_id = pmm_gbuf_upsert_node(gb, "Class", "OnlyReader", "pkg.OnlyReader",
                                            "pkg/onlyreader.go", 1, 10, "{}");
    int64_t or_read_id =
        pmm_gbuf_upsert_node(gb, "Method", "Read", "pkg.OnlyReader.Read", "pkg/onlyreader.go", 2, 4,
    ...
    const pmm_gbuf_edge_t **override_edges = NULL;
    int override_count = 0;
    pmm_gbuf_find_edges_by_source_type(gb, or_read_id, "OVERRIDE", &override_edges,
                                       &override_count);
...
static int setup_usages_repo(const char *filename, const char *content, const char *extra_file,
    ...
    snprintf(g_usages_tmpdir, sizeof(g_usages_tmpdir), "/tmp/pmm_usage_XXXXXX");
    if (!pmm_mkdtemp(g_usages_tmpdir))
        return -1;
...
TEST(usages_creates_edges) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_usages_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    int rc = pmm_pipeline_run(p);
    ASSERT_EQ(rc, 0);
    ...
    pmm_edge_t *edges = NULL;
    int edge_count = 0;
    rc = pmm_store_find_edges_by_type(s, project, "USAGE", &edges, &edge_count);

    bool found_usage = false;
    for (int i = 0; i < edge_count; i++) {
        pmm_node_t src = {0}, tgt = {0};
        if (pmm_store_find_node_by_id(s, edges[i].source_id, &src) == PMM_STORE_OK &&
            pmm_store_find_node_by_id(s, edges[i].target_id, &tgt) == PMM_STORE_OK) {
            if (strcmp(src.name, "Register") == 0 && strcmp(tgt.name, "Process") == 0) {
                found_usage = true;
            }
        }
        pmm_node_free_fields(&src);
        pmm_node_free_fields(&tgt);
    }
    if (edges)
        pmm_store_free_edges(edges, edge_count);
...
TEST(usages_no_duplicate_calls) {
    ...
    pmm_edge_t *call_edges = NULL;
    int call_count = 0;
    pmm_store_find_edges_by_type(s, project, "CALLS", &call_edges, &call_count);

    bool found_call = false;
    for (int i = 0; i < call_count; i++) {
        pmm_node_t src = {0}, tgt = {0};
        if (pmm_store_find_node_by_id(s, call_edges[i].source_id, &src) == PMM_STORE_OK &&
            pmm_store_find_node_by_id(s, call_edges[i].target_id, &tgt) == PMM_STORE_OK) {
    ...
    if (call_edges)
        pmm_store_free_edges(call_edges, call_count);
    ASSERT_TRUE(found_call);

    /* Should NOT have USAGE edge from Main to Helper */
    pmm_edge_t *usage_edges = NULL;
    int usage_count = 0;
    pmm_store_find_edges_by_type(s, project, "USAGE", &usage_edges, &usage_count);

    for (int i = 0; i < usage_count; i++) {
        pmm_node_t src = {0}, tgt = {0};
        if (pmm_store_find_node_by_id(s, usage_edges[i].source_id, &src) == PMM_STORE_OK &&
            pmm_store_find_node_by_id(s, usage_edges[i].target_id, &tgt) == PMM_STORE_OK) {
    ...
    if (usage_edges)
        pmm_store_free_edges(usage_edges, usage_count);
...
static int setup_lang_repo(const char **filenames, const char **contents, int count) {
    snprintf(g_lang_tmpdir, sizeof(g_lang_tmpdir), "/tmp/pmm_lang_XXXXXX");
    if (!pmm_mkdtemp(g_lang_tmpdir))
        return -1;
...
TEST(pipeline_python_project) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_lang_tmpdir, db, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);

    pmm_store_t *s = pmm_store_open_path(db);
    ASSERT_NOT_NULL(s);
    const char *proj = pmm_pipeline_project_name(p);

    pmm_node_t *funcs = NULL;
    int fc = 0;
    pmm_store_find_nodes_by_label(s, proj, "Function", &funcs, &fc);
    ASSERT_GTE(fc, 3); /* greet, process, fetch_data */
    pmm_store_free_nodes(funcs, fc);

    pmm_node_t *cls = NULL;
    int cc = 0;
    pmm_store_find_nodes_by_label(s, proj, "Class", &cls, &cc);
    ASSERT_GTE(cc, 1); /* DataProcessor */
    pmm_store_free_nodes(cls, cc);

    pmm_node_t *methods = NULL;
    int mc = 0;
    pmm_store_find_nodes_by_label(s, proj, "Method", &methods, &mc);
    ASSERT_GTE(mc, 1); /* transform */
    pmm_store_free_nodes(methods, mc);
...
TEST(pipeline_imports_multi_symbol_edges) {
    ...
    pmm_edge_t *edges = NULL;
    int count = 0;
    ASSERT_EQ(pmm_store_find_edges_by_type(s, proj, "IMPORTS", &edges, &count), PMM_STORE_OK);
    ASSERT_EQ(count, 2);
...
TEST(pipeline_go_cross_package_call) {
    ...
    pmm_node_t *targets = NULL;
    int tc = 0;
    pmm_store_find_nodes_by_name(s, proj, "ProcessOrder", &targets, &tc);
    ASSERT_GT(tc, 0);

    /* Verify run() exists */
    pmm_node_t *callers = NULL;
    int clc = 0;
    pmm_store_find_nodes_by_name(s, proj, "run", &callers, &clc);
    ASSERT_GT(clc, 0);

    /* Check CALLS edge from run to ProcessOrder */
    pmm_edge_t *edges = NULL;
    int ec = 0;
    pmm_store_find_edges_by_source_type(s, callers[0].id, "CALLS", &edges, &ec);
    bool found = false;
    for (int i = 0; i < ec; i++) {
    ...
    if (edges)
        pmm_store_free_edges(edges, ec);
    pmm_store_free_nodes(targets, tc);
    pmm_store_free_nodes(callers, clc);
    pmm_store_close(s);
    pmm_pipeline_free(p);
    teardown_lang_repo();
    PASS();
...
TEST(pipeline_python_cross_module_call) {
    ...
    pmm_node_t *targets = NULL;
    int tc = 0;
    pmm_store_find_nodes_by_name(s, proj, "fetch_data", &targets, &tc);
    ASSERT_GT(tc, 0);

    pmm_node_t *callers = NULL;
    int clc = 0;
    pmm_store_find_nodes_by_name(s, proj, "process", &callers, &clc);
    ASSERT_GT(clc, 0);
...
TEST(pipeline_go_type_classification) {
    ...
    pmm_node_t *ifaces = NULL;
    int ic = 0;
    pmm_store_find_nodes_by_label(s, proj, "Interface", &ifaces, &ic);
    ASSERT_EQ(ic, 2);
    pmm_store_free_nodes(ifaces, ic);

    /* Should have 1 Struct node (Config struct) */
    pmm_node_t *cls = NULL;
    int cc = 0;
    pmm_store_find_nodes_by_label(s, proj, "Struct", &cls, &cc);
    ASSERT_EQ(cc, 1);
    ASSERT_STR_EQ(cls[0].name, "Config");
    pmm_store_free_nodes(cls, cc);

    /* Should have 1 Type node (ID alias) */
    pmm_node_t *types = NULL;
    int tc = 0;
    pmm_store_find_nodes_by_label(s, proj, "Type", &types, &tc);
    ASSERT_EQ(tc, 1);
    ASSERT_STR_EQ(types[0].name, "ID");
    pmm_store_free_nodes(types, tc);
...
TEST(pipeline_kotlin_project) {
    ...
    int edge_count = pmm_store_count_edges(s, proj);
...
TEST(pipeline_lua_anonymous_functions) {
    ...
    for (int e = 0; e < 3; e++) {
        pmm_node_t *found = NULL;
        int fnc = 0;
        pmm_store_find_nodes_by_name(s, proj, expected[e], &found, &fnc);
        ASSERT_GT(fnc, 0);
        pmm_store_free_nodes(found, fnc);
    }
...
TEST(pipeline_csharp_modern) {
    ...
    pmm_node_t *modules = NULL;
    int modc = 0;
    pmm_store_find_nodes_by_label(s, proj, "Module", &modules, &modc);
    ASSERT_GTE(modc, 2);
    pmm_store_free_nodes(modules, modc);
...
TEST(pipeline_bom_stripping) {
    ...
    snprintf(g_lang_tmpdir, sizeof(g_lang_tmpdir), "/tmp/pmm_bom_XXXXXX");
    if (!pmm_mkdtemp(g_lang_tmpdir))
        FAIL("tmpdir");
    ...
    pmm_node_t *found = NULL;
    int fc = 0;
    pmm_store_find_nodes_by_name(s, proj, "BOMFunc", &found, &fc);
    ASSERT_GT(fc, 0); /* BOMFunc should be found despite BOM */
    pmm_store_free_nodes(found, fc);
...
TEST(pipeline_form_call_resolution) {
    ...
    pmm_edge_t *edges = NULL;
    int ec = 0;
    pmm_store_find_edges_by_type(s, proj, "CALLS", &edges, &ec);
    bool found = false;
    for (int i = 0; i < ec; i++) {
        pmm_node_t tgt = {0};
        if (pmm_store_find_node_by_id(s, edges[i].target_id, &tgt) == PMM_STORE_OK &&
...
TEST(pipeline_python_type_inference) {
    ...
    pmm_node_t *methods = NULL;
    int mc = 0;
    pmm_store_find_nodes_by_name(s, proj, "transform", &methods, &mc);
    ASSERT_GT(mc, 0);
...
TEST(pipeline_docstring_go_function) {
    ...
    int nc = 0;
    pmm_store_find_nodes_by_name(s, proj, "Compute", &nodes, &nc);
    ASSERT_GT(nc, 0);
...
TEST(pipeline_docstring_go_class) {
    ...
    pmm_node_t *nodes = NULL;
    int nc = 0;
    pmm_store_find_nodes_by_name(s, proj, "MyStruct", &nodes, &nc);
    ASSERT_GT(nc, 0);
...
TEST(project_name_from_path) {
    ...
    for (int i = 0; i < 4; i++) {
        char *got = pmm_project_name_from_path(cases[i].path);
        ASSERT_NOT_NULL(got);
...
TEST(project_name_drive_letter_case_insensitive_issue394) {
    char *lower = pmm_project_name_from_path("c:/WEBDEV/Cardio-Cloud");
    char *upper = pmm_project_name_from_path("C:/WEBDEV/Cardio-Cloud");
...
TEST(git_context_non_git_path) {
    char *tmp = th_mktempdir("pmm_gitctx_nongit");
    ASSERT_NOT_NULL(tmp);

    pmm_git_context_t ctx = {0};
    ASSERT_EQ(pmm_git_context_resolve(tmp, &ctx), 0);
    ASSERT_FALSE(ctx.is_git);
    ASSERT_TRUE(ctx.root_exists);

    char *qn = pmm_git_context_branch_qn("proj", &ctx);
    ASSERT_NOT_NULL(qn);
    ASSERT_STR_EQ(qn, "proj.__branch__.working-tree");
    free(qn);

    char json[1024];
    ASSERT_GT(pmm_git_context_props_json(&ctx, json, sizeof(json)), 0);
    ...
    pmm_git_context_t long_ctx = {
        .root_exists = true,
        .canonical_root = long_value,
    };
    char small_json[64];
    ASSERT_EQ(pmm_git_context_props_json(&long_ctx, small_json, sizeof(small_json)), 0);
...
TEST(git_context_linked_worktree) {
    ...
    char *tmp = th_mktempdir("pmm_gitctx_repo");
    ASSERT_NOT_NULL(tmp);
    ...
    snprintf(cmd, sizeof(cmd),
             "git -C \"%s\" -c user.name=\"PMM Test\" -c user.email=\"pmm@example.invalid\" "
    ...
    pmm_git_context_t main_ctx = {0};
    pmm_git_context_t wt_ctx = {0};
    ASSERT_EQ(pmm_git_context_resolve(repo, &main_ctx), 0);
    ASSERT_EQ(pmm_git_context_resolve(wt, &wt_ctx), 0);
    ...
    char *qn = pmm_git_context_branch_qn("proj", &wt_ctx);
    ASSERT_NOT_NULL(qn);
    ASSERT_STR_EQ(qn, "proj.__branch__.feature-git-context");
    free(qn);

    char json[2048];
    ASSERT_GT(pmm_git_context_props_json(&wt_ctx, json, sizeof(json)), 0);
    ...
    pmm_git_context_free(&main_ctx);
    pmm_git_context_free(&wt_ctx);

    snprintf(cmd, sizeof(cmd), "git -C \"%s\" checkout --detach HEAD >%s 2>&1", repo, null_dev);
    ASSERT_EQ(run_cmd(cmd), 0);
    pmm_git_context_t detached_ctx = {0};
    ASSERT_EQ(pmm_git_context_resolve(repo, &detached_ctx), 0);
    ASSERT_TRUE(detached_ctx.is_detached);
    ASSERT_STR_EQ(detached_ctx.branch_slug, "detached");
    pmm_git_context_free(&detached_ctx);
...
TEST(project_name_uniqueness) {
    ...
    char *a = pmm_project_name_from_path("/tmp/bench/zig/lib/std");
    char *b = pmm_project_name_from_path("/tmp/bench/erlang/lib/stdlib/src");
    ASSERT_NOT_NULL(a);
    ASSERT_NOT_NULL(b);
...
TEST(gitdiff_parse_range_with_count) {
    int start, count;
    pmm_parse_range("10,5", &start, &count);
    ASSERT_EQ(start, 10);
    ASSERT_EQ(count, 5);
    PASS();
}
...
TEST(gitdiff_parse_range_no_count) {
    int start, count;
    pmm_parse_range("10", &start, &count);
    ASSERT_EQ(start, 10);
    ASSERT_EQ(count, 1);
    PASS();
}
...
TEST(gitdiff_parse_range_zero_count) {
    int start, count;
    pmm_parse_range("1,0", &start, &count);
    ASSERT_EQ(start, 1);
    ASSERT_EQ(count, 0);
    PASS();
}
...
TEST(gitdiff_parse_range_large) {
    int start, count;
    pmm_parse_range("52,2", &start, &count);
    ASSERT_EQ(start, 52);
    ASSERT_EQ(count, 2);
    PASS();
}
...
TEST(gitdiff_parse_name_status) {
    ...
    pmm_changed_file_t files[16];
    int n = pmm_parse_name_status(input, files, 16);
...
TEST(gitdiff_parse_hunks) {
    ...
    pmm_changed_hunk_t hunks[16];
    int n = pmm_parse_hunks(input, hunks, 16);
...
TEST(gitdiff_parse_hunks_no_newline_marker) {
    ...
    pmm_changed_hunk_t hunks[4];
    int n = pmm_parse_hunks(input, hunks, 4);
...
TEST(configures_is_env_var_name) {
    ASSERT(pmm_is_env_var_name("DATABASE_URL"));
    ASSERT(pmm_is_env_var_name("API_KEY"));
    ASSERT(pmm_is_env_var_name("PORT"));
    ASSERT(!pmm_is_env_var_name("A"));      /* too short */
    ASSERT(!pmm_is_env_var_name("port"));   /* lowercase */
    ASSERT(!pmm_is_env_var_name("apiKey")); /* camelCase */
    ASSERT(pmm_is_env_var_name("DB_2"));    /* with digit */
    ASSERT(!pmm_is_env_var_name("__"));     /* no uppercase */
    ASSERT(!pmm_is_env_var_name(""));       /* empty */
    PASS();
}
...
TEST(configures_normalize_config_key) {
    ...
    tokens = pmm_normalize_config_key("max_connections", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "max_connections");
    ASSERT_EQ(tokens, 2);

    tokens = pmm_normalize_config_key("maxConnections", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "max_connections");
    ASSERT_EQ(tokens, 2);

    tokens = pmm_normalize_config_key("DATABASE_HOST", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "database_host");
    ASSERT_EQ(tokens, 2);

    tokens = pmm_normalize_config_key("database.host", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "database_host");
    ASSERT_EQ(tokens, 2);

    tokens = pmm_normalize_config_key("port", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "port");
    ASSERT_EQ(tokens, 1);

    tokens = pmm_normalize_config_key("maxRetryCount", norm, sizeof(norm));
...
TEST(configures_has_config_extension) {
    ASSERT(pmm_has_config_extension("config.toml"));
    ASSERT(pmm_has_config_extension("settings.yaml"));
    ASSERT(pmm_has_config_extension("config.yml"));
    ASSERT(pmm_has_config_extension(".env"));
    ASSERT(pmm_has_config_extension("config.ini"));
    ASSERT(pmm_has_config_extension("data.json"));
    ASSERT(pmm_has_config_extension("pom.xml"));
    ASSERT(!pmm_has_config_extension("main.go"));
    ASSERT(!pmm_has_config_extension("app.py"));
    ASSERT(!pmm_has_config_extension("data.csv"));
    PASS();
}
...
TEST(configures_env_var_in_config) {
    ...
    pmm_edge_t *edges = NULL;
    int ec = 0;
    pmm_store_find_edges_by_type(s, proj, "CONFIGURES", &edges, &ec);
...
TEST(configures_full_pipeline_integration) {
    ...
    pmm_node_t *classes = NULL;
    int cc = 0;
    pmm_store_find_nodes_by_label(s, proj, "Class", &classes, &cc);
    ASSERT_GT(cc, 0);
    if (classes)
        pmm_store_free_nodes(classes, cc);

    /* Should have Variable nodes from config files */
    pmm_node_t *vars = NULL;
    int vc = 0;
    pmm_store_find_nodes_by_label(s, proj, "Variable", &vars, &vc);
    ASSERT_GT(vc, 0);
    if (vars)
        pmm_store_free_nodes(vars, vc);
...
TEST(enrichment_split_camel_case) {
    ...
    n = pmm_split_camel_case("GetMapping", parts, 8);
    ASSERT_EQ(n, 2);
    ...
    n = pmm_split_camel_case("getMessage", parts, 8);
    ASSERT_EQ(n, 2);
    ASSERT_STR_EQ(parts[0], "get");
    ...
    n = pmm_split_camel_case("cache", parts, 8);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(parts[0], "cache");
    for (int i = 0; i < n; i++)
        free(parts[i]);

    n = pmm_split_camel_case("HTMLParser", parts, 8);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(parts[0], "HTMLParser");
    for (int i = 0; i < n; i++)
        free(parts[i]);

    n = pmm_split_camel_case("", parts, 8);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(enrichment_tokenize_decorator) {
    ...
    n = pmm_tokenize_decorator("@Override", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "override");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = pmm_tokenize_decorator("@Deprecated", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "deprecated");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = pmm_tokenize_decorator("@Test", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "test");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = pmm_tokenize_decorator("@login_required", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    n = pmm_tokenize_decorator("@cache", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "cache");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = pmm_tokenize_decorator("@pytest.fixture", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    n = pmm_tokenize_decorator("@GetMapping(\"/api\")", tokens, 16);
    ASSERT_EQ(n, 1);
    ...
    n = pmm_tokenize_decorator("@PostMapping(\"/api\")", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    n = pmm_tokenize_decorator("@Transactional", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "transactional");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = pmm_tokenize_decorator("@MessageMapping(\"/chat\")", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    /* #[derive(Debug)] */
    n = pmm_tokenize_decorator("#[derive(Debug)]", tokens, 16);
    ASSERT_EQ(n, 1);
    ...
    n = pmm_tokenize_decorator("@app.get(\"/api\")", tokens, 16);
    ASSERT_EQ(n, 0);

    /* "router" is stopword, "post" passes */
    n = pmm_tokenize_decorator("@router.post(\"/api\")", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "post");
    for (int i = 0; i < n; i++)
        free(tokens[i]);
    ...
    n = pmm_tokenize_decorator("@x", tokens, 16);
    ASSERT_EQ(n, 0);

    /* Empty */
    n = pmm_tokenize_decorator("", tokens, 16);
    ASSERT_EQ(n, 0);

    n = pmm_tokenize_decorator("@click.command", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    n = pmm_tokenize_decorator("@celery.task", tokens, 16);
    ASSERT_EQ(n, 2);
...
TEST(compile_commands_split_command) {
    ...
    n = pmm_split_command("gcc -c main.c", args, 16);
    ASSERT_EQ(n, 3);
    ...
    n = pmm_split_command("gcc -DFOO=\"bar baz\" -c main.c", args, 16);
    ASSERT_EQ(n, 4);
    for (int i = 0; i < n; i++)
        free(args[i]);

    n = pmm_split_command("g++ -I/usr/include -std=c++17 -o out -c in.cpp", args, 16);
    ASSERT_EQ(n, 7);
    for (int i = 0; i < n; i++)
        free(args[i]);
    PASS();
...
TEST(compile_commands_parse_json) {
    ...
    char **paths = NULL;
    pmm_compile_flags_t **flags = NULL;
    int n = pmm_parse_compile_commands(json, "/home/user/project", &paths, &flags);
    ...
    for (int i = 0; i < n; i++) {
        free(paths[i]);
        pmm_compile_flags_free(flags[i]);
    }
    free(paths);
    free(flags);
    PASS();
...
TEST(compile_commands_parse_empty) {
    char **paths = NULL;
    pmm_compile_flags_t **flags = NULL;
    int n = pmm_parse_compile_commands("[]", "/repo", &paths, &flags);
    ASSERT_EQ(n, 0);
    free(paths);
    free(flags);
    PASS();
...
TEST(compile_commands_parse_invalid) {
    char **paths = NULL;
    pmm_compile_flags_t **flags = NULL;
    int n = pmm_parse_compile_commands("not json", "/repo", &paths, &flags);
    ASSERT(n < 0);
    PASS();
}
...
TEST(infra_is_compose_file) {
    ...
    ASSERT(pmm_is_compose_file("docker-compose.yml"));
    ASSERT(pmm_is_compose_file("docker-compose.yaml"));
    ASSERT(pmm_is_compose_file("docker-compose.prod.yml"));
    ASSERT(pmm_is_compose_file("compose.yml"));
    ASSERT(pmm_is_compose_file("compose.yaml"));
    ASSERT(!pmm_is_compose_file("mycompose.yml"));
    ASSERT(!pmm_is_compose_file("docker-compose.txt"));
    ASSERT(!pmm_is_compose_file("Dockerfile"));
    PASS();
}

TEST(infra_is_cloudbuild_file) {
    /* Port of TestIsCloudbuildFile (5 cases) */
    ASSERT(pmm_is_cloudbuild_file("cloudbuild.yaml"));
    ASSERT(pmm_is_cloudbuild_file("cloudbuild.yml"));
    ASSERT(pmm_is_cloudbuild_file("cloudbuild-prod.yaml"));
    ASSERT(pmm_is_cloudbuild_file("Cloudbuild.yml"));
    ASSERT(!pmm_is_cloudbuild_file("build.yaml"));
    PASS();
}

TEST(infra_is_shell_script) {
    /* Port of TestIsShellScript (5 cases) */
    ASSERT(pmm_is_shell_script("run.sh", ".sh"));
    ASSERT(pmm_is_shell_script("deploy.bash", ".bash"));
    ASSERT(pmm_is_shell_script("init.zsh", ".zsh"));
    ASSERT(!pmm_is_shell_script("main.py", ".py"));
    ASSERT(!pmm_is_shell_script("Dockerfile", ""));
    PASS();
}

TEST(infra_is_dockerfile) {
    ASSERT(pmm_is_dockerfile("Dockerfile"));
    ASSERT(pmm_is_dockerfile("dockerfile"));
    ASSERT(pmm_is_dockerfile("Dockerfile.prod"));
    ASSERT(pmm_is_dockerfile("app.dockerfile"));
    ASSERT(!pmm_is_dockerfile("docker-compose.yml"));
    ASSERT(!pmm_is_dockerfile("main.go"));
    PASS();
}

TEST(infra_is_kustomize_file) {
    ASSERT(pmm_is_kustomize_file("kustomization.yaml"));
    ASSERT(pmm_is_kustomize_file("kustomization.yml"));
    ASSERT(pmm_is_kustomize_file("KUSTOMIZATION.YAML")); /* case-insensitive */
    ASSERT(!pmm_is_kustomize_file("deployment.yaml"));
    ASSERT(!pmm_is_kustomize_file("kustomize.yaml"));
    ASSERT(!pmm_is_kustomize_file(NULL));
    PASS();
}
...
TEST(infra_is_k8s_manifest) {
    ...
    ASSERT(pmm_is_k8s_manifest("deployment.yaml", deploy));
    ASSERT(!pmm_is_k8s_manifest("deployment.yaml", plain));
    /* kustomize file should return false even if it has apiVersion */
    ASSERT(!pmm_is_k8s_manifest("kustomization.yaml", kust));
    ASSERT(!pmm_is_k8s_manifest(NULL, deploy));
    ASSERT(!pmm_is_k8s_manifest("deployment.yaml", NULL));
    PASS();
}

TEST(infra_is_env_file) {
    ASSERT(pmm_is_env_file(".env"));
    ASSERT(pmm_is_env_file(".env.local"));
    ASSERT(pmm_is_env_file("prod.env"));
    ASSERT(!pmm_is_env_file("main.go"));
    ASSERT(!pmm_is_env_file("env.txt"));
    PASS();
}
...
TEST(infra_clean_json_brackets) {
    ...
    pmm_clean_json_brackets("[\"./server\"]", out, sizeof(out));
    ASSERT_STR_EQ(out, "./server");

    pmm_clean_json_brackets("[\"python\", \"main.py\"]", out, sizeof(out));
    ASSERT_STR_EQ(out, "python main.py");

    pmm_clean_json_brackets("./server", out, sizeof(out));
    ASSERT_STR_EQ(out, "./server");

    pmm_clean_json_brackets("[\"./app\", \"--flag\", \"value\"]", out, sizeof(out));
...
TEST(infra_secret_detection) {
    /* Key-based detection */
    ASSERT(pmm_is_secret_binding("JWT_SECRET", "anything"));
    ASSERT(pmm_is_secret_binding("API_KEY", "anything"));
    ASSERT(pmm_is_secret_binding("my_password", "anything"));
    ASSERT(pmm_is_secret_binding("AUTH_TOKEN", "anything"));
    ASSERT(!pmm_is_secret_binding("DATABASE_URL", "https://db.example.com"));

    /* Value-based detection */
    ASSERT(pmm_is_secret_value("sk-1234567890abcdef12345"));
    ASSERT(pmm_is_secret_value("-----BEGIN RSA PRIVATE KEY-----"));
    ASSERT(!pmm_is_secret_value("https://db.example.com"));
    ASSERT(!pmm_is_secret_value("hello world"));
    ASSERT(!pmm_is_secret_value("8080"));

    /* isSecretBinding checks both */
    ASSERT(pmm_is_secret_binding("ANYTHING", "sk-1234567890abcdef12345"));
    ASSERT(!pmm_is_secret_binding("PORT", "8080"));
...
static const char *find_env_var(const pmm_env_kv_t *vars, int count, const char *key) {
    for (int i = 0; i < count; i++) {
...
TEST(infra_parse_dockerfile_multistage) {
    ...
    pmm_dockerfile_result_t r;
    ASSERT_EQ(pmm_parse_dockerfile_source(src, &r), 0);
...
TEST(infra_parse_dockerfile_empty) {
    ...
    pmm_dockerfile_result_t r;
    ASSERT_EQ(pmm_parse_dockerfile_source("# just a comment\n", &r), -1);
    PASS();
}
...
TEST(infra_parse_dotenv) {
    ...
    pmm_dotenv_result_t r;
    ASSERT_EQ(pmm_parse_dotenv_source(src, &r), 0);
...
TEST(infra_parse_shell) {
    ...
    pmm_shell_result_t r;
    ASSERT_EQ(pmm_parse_shell_source(src, &r), 0);
...
TEST(infra_parse_shell_truly_empty) {
    ...
    pmm_shell_result_t r;
    ASSERT_EQ(pmm_parse_shell_source("# no shebang, just comments\n", &r), -1);
    PASS();
}
...
TEST(infra_parse_terraform_full) {
    ...
    pmm_terraform_result_t r;
    ASSERT_EQ(pmm_parse_terraform_source(src, &r), 0);
...
TEST(infra_parse_terraform_empty) {
    ...
    pmm_terraform_result_t r;
    ASSERT_EQ(pmm_parse_terraform_source("# just comments\n", &r), -1);
    PASS();
}
...
TEST(helm_parse_chart_dependencies_issue338) {
    ...
    pmm_helm_chart_t hc;
    ASSERT_EQ(pmm_parse_helm_chart(src, &hc), 0);
...
TEST(helm_parse_chart_no_deps_issue338) {
    pmm_helm_chart_t hc;
    ASSERT_EQ(pmm_parse_helm_chart("name: solo\nversion: 0.1.0\n", &hc), 0);
...
TEST(registry_resolve_single_candidate) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "CreateOrder", "svcA.handlers.CreateOrder", "Function");
    pmm_registry_add(reg, "ValidateOrder", "svcB.validators.ValidateOrder", "Function");

    /* Normal resolve unique name */
    pmm_resolution_t r = pmm_registry_resolve(reg, "CreateOrder", "svcC.caller", NULL, NULL, 0);
    ASSERT_STR_EQ(r.qualified_name, "svcA.handlers.CreateOrder");

    /* Fuzzy resolve with unknown prefix */
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(reg, "unknownPkg.CreateOrder", "svcC.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT_STR_EQ(fr.result.qualified_name, "svcA.handlers.CreateOrder");

    pmm_registry_free(reg);
    PASS();
}
...
TEST(registry_fuzzy_nonexistent) {
    ...
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(reg, "NonExistent", "svcC.caller", NULL, NULL, 0);
    ASSERT_FALSE(fr.ok);
...
TEST(registry_fuzzy_multiple_best_by_distance) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Process", "svcA.handlers.Process", "Function");
    pmm_registry_add(reg, "Process", "svcB.handlers.Process", "Function");

    /* Caller in svcA → prefer svcA */
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(reg, "unknown.Process", "svcA.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT_STR_EQ(fr.result.qualified_name, "svcA.handlers.Process");

    /* Caller in svcB → prefer svcB */
    fr = pmm_registry_fuzzy_resolve(reg, "unknown.Process", "svcB.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(registry_fuzzy_simple_name_extraction) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "DoWork", "myproject.utils.DoWork", "Function");

    /* Deeply qualified name → extract "DoWork" */
    pmm_fuzzy_result_t fr = pmm_registry_fuzzy_resolve(reg, "some.deep.module.DoWork",
...
TEST(registry_fuzzy_empty) {
    ...
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(reg, "SomeFunc", "myproject.caller", NULL, NULL, 0);
    ASSERT_FALSE(fr.ok);
...
TEST(registry_exists) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Foo", "pkg.module.Foo", "Function");
    pmm_registry_add(reg, "Bar", "pkg.module.Bar", "Method");

    ASSERT_TRUE(pmm_registry_exists(reg, "pkg.module.Foo"));
    ASSERT_TRUE(pmm_registry_exists(reg, "pkg.module.Bar"));
    ASSERT_FALSE(pmm_registry_exists(reg, "pkg.module.Missing"));
    ASSERT_FALSE(pmm_registry_exists(reg, ""));
...
TEST(registry_confidence_import_map) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Foo", "proj.other.Foo", "Function");

    const char *keys[] = {"other"};
    const char *vals[] = {"proj.other"};
    pmm_resolution_t r = pmm_registry_resolve(reg, "other.Foo", "proj.pkg", keys, vals, 1);
...
TEST(registry_confidence_import_map_suffix) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Foo", "proj.other.sub.Foo", "Function");
...
TEST(registry_confidence_same_module) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Foo", "proj.pkg.Foo", "Function");

    pmm_resolution_t r = pmm_registry_resolve(reg, "Foo", "proj.pkg", NULL, NULL, 0);
...
TEST(registry_confidence_unique_name) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Bar", "proj.pkg.Bar", "Function");

    pmm_resolution_t r = pmm_registry_resolve(reg, "Bar", "proj.unrelated", NULL, NULL, 0);
...
TEST(registry_confidence_suffix_match) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Process", "proj.svcA.Process", "Function");
    pmm_registry_add(reg, "Process", "proj.svcB.Process", "Function");

    pmm_resolution_t r = pmm_registry_resolve(reg, "Process", "proj.svcA.caller", NULL, NULL, 0);
...
TEST(registry_fuzzy_confidence_single) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Handler", "proj.svc.Handler", "Function");

    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(reg, "unknownPkg.Handler", "proj.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(registry_fuzzy_confidence_distance) {
    ...
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(reg, "unknownPkg.Process", "proj.svcA.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(registry_negative_import_rejects) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Process", "proj.billing.Process", "Function");
    pmm_registry_add(reg, "Process", "proj.handler.Process", "Function");

    /* Import only handler's module → should prefer handler */
    const char *keys[] = {"handler"};
    const char *vals[] = {"proj.handler"};
    pmm_resolution_t r = pmm_registry_resolve(reg, "Process", "proj.caller", keys, vals, 1);
...
TEST(registry_fuzzy_import_penalty) {
    pmm_registry_t *reg = pmm_registry_new();
    pmm_registry_add(reg, "Handler", "proj.billing.Handler", "Function");

    /* Has imports but billing not imported → confidence halved */
    const char *keys[] = {"other"};
    const char *vals[] = {"proj.other"};
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(reg, "unknown.Handler", "proj.caller", keys, vals, 1);
    ASSERT_TRUE(fr.ok);
    /* 0.40 * 0.5 = 0.20 */
...
TEST(registry_fuzzy_no_import_map_passthrough) {
    ...
    pmm_fuzzy_result_t fr =
        pmm_registry_fuzzy_resolve(reg, "unknown.Handler", "proj.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT(fr.result.confidence > 0.35 && fr.result.confidence <= 0.45);
...
TEST(registry_find_by_name) {
    ...
    pmm_registry_add(reg, "Bar", "proj.pkg.Bar", "Function");
    pmm_registry_add(reg, "Foo", "proj.other.Foo", "Function");
    pmm_registry_add(reg, "transform", "proj.utils.DataProcessor.transform", "Method");

    /* FindByName returns all entries for "Foo" */
    const char **foos = NULL;
    int foos_count = 0;
    pmm_registry_find_by_name(reg, "Foo", &foos, &foos_count);
    ASSERT_EQ(foos_count, 2);

    /* FindByName for unique "Bar" */
    const char **bars = NULL;
    int bars_count = 0;
    pmm_registry_find_by_name(reg, "Bar", &bars, &bars_count);
    ...
    int trans_count = 0;
    pmm_registry_find_by_name(reg, "transform", &transforms, &trans_count);
    ASSERT_EQ(trans_count, 1);
    ASSERT_STR_EQ(transforms[0], "proj.utils.DataProcessor.transform");

    /* label_of */
    ASSERT_STR_EQ(pmm_registry_label_of(reg, "proj.utils.DataProcessor.transform"), "Method");
    ASSERT_STR_EQ(pmm_registry_label_of(reg, "proj.pkg.Foo"), "Function");

    /* Total size */
    ASSERT_EQ(pmm_registry_size(reg), 4);
    ...
    const char *keys[] = {"other"};
    const char *vals[] = {"proj.other"};
    r = pmm_registry_resolve(reg, "other.Foo", "proj.pkg", keys, vals, 1);
    ASSERT_STR_EQ(r.qualified_name, "proj.other.Foo");

    /* Resolve unique name */
    r = pmm_registry_resolve(reg, "Bar", "proj.unrelated", NULL, NULL, 0);
    ASSERT_STR_EQ(r.qualified_name, "proj.pkg.Bar");
...
TEST(registry_confidence_band) {
    ASSERT_STR_EQ(pmm_confidence_band(0.95), "high");
    ASSERT_STR_EQ(pmm_confidence_band(0.70), "high");
    ASSERT_STR_EQ(pmm_confidence_band(0.55), "medium");
    ASSERT_STR_EQ(pmm_confidence_band(0.45), "medium");
    ASSERT_STR_EQ(pmm_confidence_band(0.40), "speculative");
    ASSERT_STR_EQ(pmm_confidence_band(0.25), "speculative");
    ASSERT_STR_EQ(pmm_confidence_band(0.20), "");
    ASSERT_STR_EQ(pmm_confidence_band(0.0), "");
    PASS();
}
...
TEST(infra_qn_helper) {
    ...
    char *qn = pmm_infra_qn("myproject", "docker-images/service/Dockerfile", "dockerfile", NULL);
    ASSERT_NOT_NULL(qn);
    ASSERT(strstr(qn, ".__infra__") != NULL);
    free(qn);

    /* Compose service → ::service_name suffix */
    qn = pmm_infra_qn("myproject", "docker-compose.yml", "compose-service", "web");
    ASSERT_NOT_NULL(qn);
...
TEST(infra_pipeline_integration) {
    ...
    /* Parse Dockerfile */
    pmm_dockerfile_result_t dr;
    ASSERT_EQ(pmm_parse_dockerfile_source("FROM alpine:3.19\nEXPOSE 8080\n", &dr), 0);
    ASSERT_STR_EQ(dr.base_image, "alpine:3.19");
    ASSERT_GTE(dr.port_count, 1);

    /* Parse .env */
    pmm_dotenv_result_t er;
    ASSERT_EQ(pmm_parse_dotenv_source("APP_PORT=8080\nDEBUG=true\n", &er), 0);
...
TEST(infra_pipeline_idempotent) {
    ...
    pmm_dockerfile_result_t r1, r2;
    ASSERT_EQ(pmm_parse_dockerfile_source(src, &r1), 0);
    ASSERT_EQ(pmm_parse_dockerfile_source(src, &r2), 0);
...
TEST(k8s_extract_kustomize) {
    const char *src = "apiVersion: kustomize.config.k8s.io/v1beta1\n"
                      ...
                      "  - deployment.yaml\n"
                      "  - service.yaml\n";
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src), PMM_LANG_KUSTOMIZE, "myproj",
...
TEST(k8s_extract_manifest) {
    ...
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src), PMM_LANG_K8S, "myproj",
...
static const pmm_env_binding_t *find_binding_by_key(const pmm_env_binding_t *bindings, int count,
                                                    const char *key) {
    for (int i = 0; i < count; i++) {
...
static int has_binding_value(const pmm_env_binding_t *bindings, int count, const char *value) {
    for (int i = 0; i < count; i++) {
...
TEST(envscan_dockerfile_env_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_dock_XXXXXX");
    ...
    pmm_env_binding_t bindings[32];
    int count = pmm_scan_project_env_urls(tmpdir, bindings, 32);
...
TEST(envscan_shell_env_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_sh_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_env_file_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_env_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_toml_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_toml_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_yaml_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_yaml_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_terraform_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_tf_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_properties_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_prop_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_secret_key_exclusion) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_skey_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_secret_value_exclusion) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_sval_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_secret_file_exclusion) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_sfile_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_skips_ignored_dirs) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_ign_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(envscan_non_url_values_skipped) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_nurl_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
...
TEST(pipeline_relpath_excluded_boundary) {
    ...
    ASSERT_TRUE(pmm_pipeline_relpath_is_excluded("vendor_big", excluded, 2));
    ASSERT_TRUE(pmm_pipeline_relpath_is_excluded("vendor_big/lib/package.json", excluded, 2));
    ASSERT_TRUE(pmm_pipeline_relpath_is_excluded("packages/big", excluded, 2));
    ASSERT_TRUE(pmm_pipeline_relpath_is_excluded("packages/big/src/x.ts", excluded, 2));

    /* Sibling names sharing the prefix are NOT excluded ('/'-boundary). */
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded("vendor_bigger", excluded, 2));
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded("vendor", excluded, 2));
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded("packages/bigger/x.ts", excluded, 2));

    /* Exclusions are root-anchored prefixes, not substring matches. */
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded("src/vendor_big/x.c", excluded, 2));

    /* NULL / empty safety. */
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded(NULL, excluded, 2));
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded("", excluded, 2));
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded("vendor_big", NULL, 0));
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded("vendor_big", excluded, 0));
    char *with_empty[] = {(char *)""};
    ASSERT_FALSE(pmm_pipeline_relpath_is_excluded("vendor_big", with_empty, 1));
    PASS();
}

/* Helper: does the entries array contain a package with this name? */
static int pkg_entries_has_name(const pmm_pkg_entries_t *e, const char *name) {
...
TEST(pkgmap_scan_repo_honors_discovery_exclusions) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_pkgmap_excl_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
    ...
    pmm_pkg_entries_t control;
    pmm_pkg_entries_init(&control);
    pmm_pkgmap_scan_repo(tmpdir, &control, NULL, 0);
    ASSERT_TRUE(pkg_entries_has_name(&control, "@org/app"));
    ASSERT_TRUE(pkg_entries_has_name(&control, "@org/vendored"));
    pmm_pkg_entries_free(&control);
    ...
    pmm_pkg_entries_t entries;
    pmm_pkg_entries_init(&entries);
    pmm_pkgmap_scan_repo(tmpdir, &entries, excluded, 1);
    ASSERT_TRUE(pkg_entries_has_name(&entries, "@org/app"));
    ASSERT_FALSE(pkg_entries_has_name(&entries, "@org/vendored"));
    pmm_pkg_entries_free(&entries);
...
TEST(envscan_walk_honors_discovery_exclusions) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_envscan_excl_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
    ...
    count = pmm_scan_project_env_urls_excluded(tmpdir, bindings, 32, excluded, 1);
    ASSERT_NOT_NULL(find_binding_by_key(bindings, count, "CONTROL_URL"));
...
TEST(githistory_compute_change_coupling) {
    ...
    pmm_commit_files_t commits[5] = {
        {files_aaa, 3, 0}, {files_bbb, 2, 0}, {files_ccc, 2, 0},
        {files_ddd, 2, 0}, {files_eee, 2, 0},
    };

    pmm_change_coupling_t out[100];
    int count = pmm_compute_change_coupling(commits, 5, out, 100);
...
TEST(githistory_coupling_skips_large_commits) {
    ...
    pmm_commit_files_t commits[1] = {{files, 25, 0}};

    pmm_change_coupling_t out[100];
    int count = pmm_compute_change_coupling(commits, 1, out, 100);
    ASSERT_EQ(count, 0);
    PASS();
}
...
TEST(githistory_coupling_limits_output) {
    ...
    pmm_commit_files_t commits[3675];
    int ci = 0;
    ...
    pmm_change_coupling_t out[200];
    int count = pmm_compute_change_coupling(commits, ci, out, 100);
    ASSERT(count <= 100);
    PASS();
}
...
TEST(registry_is_import_reachable) {
    ...
    ASSERT_TRUE(pmm_registry_is_import_reachable("proj.handler.Process", import_vals, 2));
    /* Sub-package: proj.handler.sub.Process → true (handler contains handler) */
    ASSERT_TRUE(pmm_registry_is_import_reachable("proj.handler.sub.Process", import_vals, 2));
    /* Nested match: proj.shared.utils.Helper → true */
    ASSERT_TRUE(pmm_registry_is_import_reachable("proj.shared.utils.Helper", import_vals, 2));
    /* Unrelated: proj.billing.Process → false */
    ASSERT_FALSE(pmm_registry_is_import_reachable("proj.billing.Process", import_vals, 2));
    /* Completely unrelated: unrelated.pkg.Func → false */
    ASSERT_FALSE(pmm_registry_is_import_reachable("unrelated.pkg.Func", import_vals, 2));
    PASS();
}
...
TEST(registry_find_ending_with) {
    ...
    int count = pmm_registry_find_ending_with(reg, "DataProcessor.transform", &matches);
    ASSERT_EQ(count, 1);
    ...
    matches = NULL;
    count = pmm_registry_find_ending_with(reg, "Foo", &matches);
    ASSERT_EQ(count, 2);
    ...
    matches = NULL;
    count = pmm_registry_find_ending_with(reg, "Nonexistent", &matches);
    ASSERT_EQ(count, 0);
...
static int setup_incremental_repo(void) {
    snprintf(g_incr_tmpdir, sizeof(g_incr_tmpdir), "/tmp/pmm_incr_XXXXXX");
    if (!pmm_mkdtemp(g_incr_tmpdir)) {
        return -1;
    }
...
extern void pmm_pipeline_set_before_publish_hook_for_tests(
...
typedef struct {
    char staging_path[PMM_SZ_4K];
    int calls;
    bool staging_was_valid;
} publish_observe_ctx_t;

static void observe_publish_boundary(pmm_pipeline_t *p, const char *staging_path, void *arg) {
    (void)p;
    ...
    pmm_store_t *staging = pmm_store_open_path_existing(staging_path);
    if (staging) {
        ctx->staging_was_valid = pmm_store_check_integrity(staging);
        pmm_store_close(staging);
    }
}
...
static int fail_publish_rename(const char *staging_path, const char *final_path, void *arg) {
    ...
    ctx->calls++;
    return staging_path && final_path ? PMM_NOT_FOUND : 0;
}
...
static void cancel_at_publish_boundary(pmm_pipeline_t *p, const char *staging_path, void *arg) {
    ...
    if (staging_path && staging_path[0]) {
        ...
        if (ctx->staging_existed) {
            pmm_store_t *staging = pmm_store_open_path(staging_path);
            if (staging) {
                ctx->staging_was_valid = pmm_store_check_integrity(staging);
...
TEST(pipeline_fastapi_depends_edges) {
    ...
    pmm_edge_t *edges = NULL;
    int edge_count = 0;
    pmm_store_find_edges_by_type(s, proj, "CALLS", &edges, &edge_count);
...
TEST(incremental_full_then_noop) {
    ...
    /* First: full index */
    pmm_pipeline_t *p = pmm_pipeline_new(g_incr_tmpdir, g_incr_dbpath, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);
    char *project = strdup(pmm_pipeline_project_name(p));
    pmm_pipeline_free(p);

    /* Verify nodes exist */
    pmm_store_t *s = pmm_store_open_path(g_incr_dbpath);
    ASSERT_NOT_NULL(s);
    int nodes_before = pmm_store_count_nodes(s, project);
    ASSERT_GT(nodes_before, 0);
    pmm_store_close(s);

    /* Second: incremental — nothing changed → should be no-op */
    p = pmm_pipeline_new(g_incr_tmpdir, g_incr_dbpath, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);
    pmm_pipeline_free(p);

    s = pmm_store_open_path(g_incr_dbpath);
    ASSERT_NOT_NULL(s);
    int nodes_after = pmm_store_count_nodes(s, project);
...
TEST(incremental_aborts_when_previous_coverage_is_unreadable) {
    ...
    ASSERT_EQ(
        pmm_store_exec(s, "ALTER TABLE index_coverage RENAME COLUMN detail TO broken_detail;"),
        PMM_STORE_OK);
    pmm_store_close(s);
    ...
    p = pmm_pipeline_new(g_incr_tmpdir, g_incr_dbpath, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_TRUE(pmm_pipeline_run(p) != 0);
    pmm_pipeline_free(p);
    ...
    s = pmm_store_open_path(g_incr_dbpath);
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_count_nodes(s, project), nodes_before);
    pmm_store_close(s);
    free(project);
...
TEST(cancelled_full_reindex_preserves_committed_db) {
    ...
    pmm_store_t *live = pmm_store_open_path(g_incr_dbpath);
    ASSERT_NOT_NULL(live);
    ASSERT_TRUE(pmm_store_check_integrity(live));
    int baseline_nodes = pmm_store_count_nodes(live, project);
    ...
    p = pmm_pipeline_new(g_incr_tmpdir, g_incr_dbpath, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    pmm_pipeline_set_before_publish_hook_for_tests(p, cancel_at_publish_boundary, &hook);
    int rc = pmm_pipeline_run(p);
    pmm_pipeline_free(p);

    live = pmm_store_open_path(g_incr_dbpath);
    ASSERT_NOT_NULL(live);
    bool live_valid = pmm_store_check_integrity(live);
    int nodes_after = pmm_store_count_nodes(live, project);
...
TEST(backup_failed_publish_failure_preserves_final_sidecars) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_incr_tmpdir, final_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    pmm_pipeline_set_before_publish_hook_for_tests(p, observe_publish_boundary, &hook);
    int rc = pmm_pipeline_run(p);
    pmm_pipeline_free(p);
    ...
    (void)pmm_unlink(final_path);
    (void)pmm_unlink(wal_path);
    (void)pmm_unlink(shm_path);
    (void)pmm_unlink(journal_path);
    cleanup_incremental_repo();
...
TEST(backup_failed_rename_failure_preserves_corrupt_main) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_incr_tmpdir, final_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    pmm_pipeline_set_before_publish_hook_for_tests(p, observe_publish_boundary, &observe);
    pmm_pipeline_set_rename_hook_for_tests(p, fail_publish_rename, &rename_fail);
    int rc = pmm_pipeline_run(p);
    pmm_pipeline_free(p);
...
static void cleanup_long_db_fixture(char *deep_dir, const char *root, const char *db_path,
    ...
    (void)pmm_remove_db_sidecars(db_path);
    ...
    if (staging_path && strlen(staging_path) >= PMM_SZ_1K) {
        char truncated[PMM_SZ_1K];
        memcpy(truncated, staging_path, sizeof(truncated) - 1);
        truncated[sizeof(truncated) - 1] = '\0';
        (void)pmm_unlink(truncated);
        (void)pmm_remove_db_sidecars(truncated);
    }

    size_t root_len = strlen(root);
    while (strlen(deep_dir) > root_len) {
        (void)pmm_rmdir(deep_dir);
...
TEST(full_reindex_preserves_exact_long_db_path) {
    ...
    char *created_root = th_mktempdir("pmm_long_db");
    ...
    ASSERT_TRUE(pmm_mkdir_p(deep_dir, 0755));

    char db_path[1600];
    int db_n = snprintf(db_path, sizeof(db_path), "%s/graph.db", deep_dir);
    ASSERT_TRUE(db_n > PMM_SZ_1K && (size_t)db_n < sizeof(db_path));

    publish_observe_ctx_t hook = {0};
    pmm_pipeline_t *p = pmm_pipeline_new(g_incr_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    char *project = strdup(pmm_pipeline_project_name(p));
    ASSERT_NOT_NULL(project);
    pmm_pipeline_set_before_publish_hook_for_tests(p, observe_publish_boundary, &hook);
    ...
    int node_count = -1;
    pmm_store_t *published = rc == 0 ? pmm_store_open_path_existing(db_path) : NULL;
    if (published) {
        node_count = pmm_store_count_nodes(published, project);
        pmm_store_close(published);
    }
    bool stray_truncated_db = false;
    if (strlen(hook.staging_path) >= PMM_SZ_1K) {
        char truncated[PMM_SZ_1K];
    ...
    ASSERT_EQ(hook.calls, 1);
    ASSERT_TRUE(strlen(hook.staging_path) >= PMM_SZ_1K);
    ASSERT_EQ(rc, 0);
    ASSERT_GT(node_count, 0);
...
TEST(incremental_fast_preserves_mode_skipped_tools_dir) {
     ...
     * from disk. */
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_modeskip_XXXXXX");
    if (!pmm_mkdtemp(tmpdir)) {
        FAIL("tmpdir");
    }
    char dbpath[512];
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(tmpdir, dbpath, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);
    char *project = strdup(pmm_pipeline_project_name(p));
    pmm_pipeline_free(p);

    pmm_store_t *s = pmm_store_open_path(dbpath);
    ASSERT_NOT_NULL(s);
    pmm_node_t *tools_nodes_before = NULL;
    int tools_count_before = 0;
    pmm_store_find_nodes_by_file(s, project, "tools/util.go", &tools_nodes_before,
                                 &tools_count_before);
    ASSERT_GT(tools_count_before, 0); /* full mode must see tools/util.go */
    pmm_store_free_nodes(tools_nodes_before, tools_count_before);
    int total_before = pmm_store_count_nodes(s, project);
    pmm_store_close(s);

    /* Step 2: fast-mode reindex — tools/util.go MUST survive (additive semantics) */
    p = pmm_pipeline_new(tmpdir, dbpath, PMM_MODE_FAST);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);
    pmm_pipeline_free(p);

    s = pmm_store_open_path(dbpath);
    ASSERT_NOT_NULL(s);
    pmm_node_t *tools_nodes_after = NULL;
    int tools_count_after = 0;
    pmm_store_find_nodes_by_file(s, project, "tools/util.go", &tools_nodes_after,
                                 &tools_count_after);
    ...
    pmm_store_free_nodes(tools_nodes_after, tools_count_after);

    /* Sanity: total node count should not have collapsed by ~the size of tools/ */
    int total_after = pmm_store_count_nodes(s, project);
    ...
    s = pmm_store_open_path(dbpath);
    ASSERT_NOT_NULL(s);
    pmm_node_t *tools_nodes_run3 = NULL;
    int tools_count_run3 = 0;
    pmm_store_find_nodes_by_file(s, project, "tools/util.go", &tools_nodes_run3, &tools_count_run3);
    ...
    pmm_store_free_nodes(tools_nodes_run3, tools_count_run3);
    pmm_store_close(s);
    ...
    p = pmm_pipeline_new(tmpdir, dbpath, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);
    pmm_pipeline_free(p);
    ...
    s = pmm_store_open_path(dbpath);
    ASSERT_NOT_NULL(s);
    pmm_node_t *tools_nodes_deleted = NULL;
    int tools_count_deleted = 0;
    pmm_store_find_nodes_by_file(s, project, "tools/util.go", &tools_nodes_deleted,
                                 &tools_count_deleted);
    ASSERT_EQ(tools_count_deleted, 0); /* truly deleted → purged */
    pmm_store_free_nodes(tools_nodes_deleted, tools_count_deleted);
    pmm_store_close(s);
...
TEST(incremental_k8s_manifest_indexed) {
     ...
     * Verifies that pmm_pipeline_pass_k8s() runs during incremental re-index. */
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_k8s_incr_XXXXXX");
    if (!pmm_mkdtemp(tmpdir)) {
        FAIL("tmpdir");
    }
    char dbpath[512];
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    pmm_store_find_nodes_by_label(s, project, "Resource", &nodes, &count);
    ASSERT_GT(count, 0);
    pmm_store_free_nodes(nodes, count);
    pmm_store_close(s);
...
TEST(incremental_kustomize_module_indexed) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_kust_incr_XXXXXX");
    if (!pmm_mkdtemp(tmpdir)) {
        FAIL("tmpdir");
    }
    char dbpath[512];
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    pmm_store_find_nodes_by_label(s, project, "Module", &nodes, &count);
...
TEST(pipeline_lock_try_acquire) {
    ...
    ASSERT_TRUE(pmm_pipeline_try_lock());
    /* Second try-lock should fail (already held) */
    ASSERT_FALSE(pmm_pipeline_try_lock());
...
TEST(pipeline_lock_contention) {
    ...
    pmm_thread_t tid;
    int rc = pmm_thread_create(&tid, 0, try_lock_thread, NULL);
    ASSERT_EQ(rc, 0);
...
TEST(pipeline_empty_path) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new("", NULL, PMM_MODE_FULL);
...
TEST(pipeline_project_name_content) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new("/home/user/my-project", NULL, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    const char *name = pmm_pipeline_project_name(p);
    ASSERT_NOT_NULL(name);
    ASSERT_TRUE(strlen(name) > 0);
...
TEST(pipeline_cancel_sets_flag) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new("/tmp/nonexistent", NULL, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    /* Cancel before run */
    pmm_pipeline_cancel(p);
...
TEST(trackable_source_files) {
    ...
    ASSERT_TRUE(pmm_is_trackable_file("main.go"));
    ASSERT_TRUE(pmm_is_trackable_file("src/handler.py"));
    ASSERT_TRUE(pmm_is_trackable_file("lib/server.js"));
    ASSERT_TRUE(pmm_is_trackable_file("components/App.tsx"));
    ASSERT_TRUE(pmm_is_trackable_file("pkg/svc.rs"));
    ASSERT_TRUE(pmm_is_trackable_file("config.yaml"));
    PASS();
}

TEST(trackable_rejects_images) {
    /* Image and binary files are not trackable */
    ASSERT_FALSE(pmm_is_trackable_file("logo.png"));
    ASSERT_FALSE(pmm_is_trackable_file("photo.jpg"));
    ASSERT_FALSE(pmm_is_trackable_file("icon.gif"));
    ASSERT_FALSE(pmm_is_trackable_file("diagram.svg"));
    PASS();
}

TEST(trackable_rejects_minified) {
    /* Minified files are not trackable */
    ASSERT_FALSE(pmm_is_trackable_file("bundle.min.js"));
    ASSERT_FALSE(pmm_is_trackable_file("style.min.css"));
    PASS();
}

TEST(trackable_rejects_vendor_dirs) {
    /* Files in vendor/node_modules/__pycache__ are not trackable */
    ASSERT_FALSE(pmm_is_trackable_file("vendor/github.com/lib/dep.go"));
    ASSERT_FALSE(pmm_is_trackable_file("node_modules/lodash/index.js"));
    ASSERT_FALSE(pmm_is_trackable_file("__pycache__/module.pyc"));
    ASSERT_FALSE(pmm_is_trackable_file(".git/objects/pack/data"));
    PASS();
}
...
TEST(trackable_rejects_lock_files) {
    ...
    ASSERT_FALSE(pmm_is_trackable_file("package-lock.json"));
    ASSERT_FALSE(pmm_is_trackable_file("go.sum"));
    ASSERT_FALSE(pmm_is_trackable_file("yarn.lock"));
    PASS();
}

TEST(test_path_directory_patterns) {
    /* Test directory patterns: __tests__, test/, tests/, spec/ */
    ASSERT_TRUE(pmm_is_test_path("__tests__/Component.js"));
    ASSERT_TRUE(pmm_is_test_path("tests/test_main.py"));
    ASSERT_TRUE(pmm_is_test_path("spec/handler_spec.rb"));
    PASS();
}
...
TEST(test_path_suffix_patterns) {
    ...
    ASSERT_TRUE(pmm_is_test_path("handler.spec.ts"));
    ASSERT_TRUE(pmm_is_test_path("handler.test.tsx"));
    ASSERT_TRUE(pmm_is_test_path("handler_test.go"));
    ASSERT_TRUE(pmm_is_test_path("test_handler.py"));
    /* Non-test files */
    ASSERT_FALSE(pmm_is_test_path("handler.ts"));
    ASSERT_FALSE(pmm_is_test_path("contest.go"));
    ASSERT_FALSE(pmm_is_test_path("latest.py"));
    PASS();
}

TEST(test_func_name_go_patterns) {
    /* Go test function names: Test + uppercase char */
    ASSERT_TRUE(pmm_is_test_func_name("TestFoo"));
    ASSERT_TRUE(pmm_is_test_func_name("TestHTTPHandler"));
    /* Non-test: "Test" alone or Test + lowercase */
    ASSERT_FALSE(pmm_is_test_func_name("Testable")); /* lowercase 'a' after Test */
    PASS();
}

TEST(test_func_name_js_helpers) {
    /* JS/TS test helper function names */
    ASSERT_TRUE(pmm_is_test_func_name("it"));
    ASSERT_TRUE(pmm_is_test_func_name("describe"));
    ASSERT_TRUE(pmm_is_test_func_name("test"));
    ASSERT_TRUE(pmm_is_test_func_name("beforeEach"));
    ASSERT_TRUE(pmm_is_test_func_name("afterEach"));
    PASS();
}

TEST(env_var_name_valid) {
    /* Valid env var names: uppercase + underscores, at least 2 chars with uppercase */
    ASSERT_TRUE(pmm_is_env_var_name("DATABASE_URL"));
    ASSERT_TRUE(pmm_is_env_var_name("API_KEY"));
    ASSERT_TRUE(pmm_is_env_var_name("PORT"));
    ASSERT_TRUE(pmm_is_env_var_name("DB_2"));
    ASSERT_TRUE(pmm_is_env_var_name("MY_VAR_123"));
    PASS();
}

TEST(env_var_name_invalid) {
    /* Invalid: too short, lowercase, mixed case, empty */
    ASSERT_FALSE(pmm_is_env_var_name("A"));      /* single char */
    ASSERT_FALSE(pmm_is_env_var_name("port"));   /* lowercase */
    ASSERT_FALSE(pmm_is_env_var_name("apiKey")); /* camelCase */
    ASSERT_FALSE(pmm_is_env_var_name("__"));     /* no uppercase */
    ASSERT_FALSE(pmm_is_env_var_name(""));       /* empty */
    ASSERT_FALSE(pmm_is_env_var_name("123"));    /* digits only */
    PASS();
}

TEST(config_ext_positive) {
    /* Config file extensions recognized */
    ASSERT_TRUE(pmm_has_config_extension(".env"));
    ASSERT_TRUE(pmm_has_config_extension("config.yaml"));
    ASSERT_TRUE(pmm_has_config_extension("config.yml"));
    ASSERT_TRUE(pmm_has_config_extension("settings.toml"));
    ASSERT_TRUE(pmm_has_config_extension("app.ini"));
    ASSERT_TRUE(pmm_has_config_extension("data.json"));
    ASSERT_TRUE(pmm_has_config_extension("app.properties"));
    PASS();
}

TEST(config_ext_negative) {
    /* Non-config extensions rejected */
    ASSERT_FALSE(pmm_has_config_extension("main.go"));
    ASSERT_FALSE(pmm_has_config_extension("app.py"));
    ASSERT_FALSE(pmm_has_config_extension("handler.rs"));
    ASSERT_FALSE(pmm_has_config_extension("data.csv"));
    ASSERT_FALSE(pmm_has_config_extension("README.md"));
    PASS();
}
...
TEST(split_camel_basic) {
    ...
    n = pmm_split_camel_case("getCamelCase", parts, 16);
    ASSERT_EQ(n, 3);
    ASSERT_STR_EQ(parts[0], "get");
...
TEST(split_camel_single_word) {
    ...
    char *parts[16];
    int n = pmm_split_camel_case("hello", parts, 16);
    ASSERT_EQ(n, 1);
...
TEST(split_camel_empty) {
    ...
    char *parts[16];
    int n = pmm_split_camel_case("", parts, 16);
    for (int i = 0; i < n; i++)
        free(parts[i]);
...
TEST(tokenize_decorator_login_required) {
    ...
    char *tokens[16];
    int n = pmm_tokenize_decorator("@login_required", tokens, 16);
    ASSERT_EQ(n, 2);
    ASSERT_STR_EQ(tokens[0], "login");
    ASSERT_STR_EQ(tokens[1], "required");
    for (int i = 0; i < n; i++)
...
TEST(tokenize_decorator_single) {
    /* @Override → ["override"] */
    char *tokens[16];
    int n = pmm_tokenize_decorator("@Override", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "override");
    for (int i = 0; i < n; i++)
        free(tokens[i]);
...
TEST(split_command_basic) {
    ...
    char *args[16];
    int n = pmm_split_command("gcc -c main.c -o main.o", args, 16);
    ASSERT_EQ(n, 5);
    ASSERT_STR_EQ(args[0], "gcc");
    ASSERT_STR_EQ(args[1], "-c");
    ASSERT_STR_EQ(args[2], "main.c");
...
TEST(split_command_quoted) {
    ...
    char *args[16];
    int n = pmm_split_command("gcc -DFOO=\"bar baz\" main.c", args, 16);
    ASSERT_GTE(n, 3);
    ASSERT_STR_EQ(args[0], "gcc");
    for (int i = 0; i < n; i++)
        free(args[i]);
...
TEST(split_command_empty) {
    ...
    char *args[16];
    int n = pmm_split_command("", args, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(parse_range_single) {
    /* "42" → start=42, count=1 */
    int start, count;
    pmm_parse_range("42", &start, &count);
    ASSERT_EQ(start, 42);
    ASSERT_EQ(count, 1);
    PASS();
}
...
TEST(parse_range_zero_count) {
    ...
    int start, count;
    pmm_parse_range("100,0", &start, &count);
    ASSERT_EQ(start, 100);
    ASSERT_EQ(count, 0);
    PASS();
}
...
TEST(parse_name_status_basic) {
    ...
    pmm_changed_file_t files[16];
    int n = pmm_parse_name_status(input, files, 16);
    /* Exact count depends on which files pass pmm_is_trackable_file */
    ASSERT_GTE(n, 0);
    ASSERT_LTE(n, 3);
    PASS();
}
...
TEST(parse_name_status_empty) {
    /* Empty input */
    pmm_changed_file_t files[16];
    int n = pmm_parse_name_status("", files, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(parse_hunks_empty) {
    /* Empty input */
    pmm_changed_hunk_t hunks[16];
    int n = pmm_parse_hunks("", hunks, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(coupling_empty_commits) {
    ...
    pmm_change_coupling_t results[16];
    int n = pmm_compute_change_coupling(NULL, 0, results, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(coupling_single_file_commit) {
    ...
    char *f1[] = {"a.go"};
    char *f2[] = {"b.go"};
    pmm_commit_files_t commits[] = {{f1, 1, 0}, {f2, 1, 0}};
    pmm_change_coupling_t results[16];
    int n = pmm_compute_change_coupling(commits, 2, results, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(clean_json_brackets_array) {
    ...
    char out[256];
    pmm_clean_json_brackets("[\"./app\", \"--flag\"]", out, sizeof(out));
...
TEST(clean_json_brackets_empty) {
    ...
    char out[256];
    pmm_clean_json_brackets("", out, sizeof(out));
    ASSERT_STR_EQ(out, "");
    PASS();
}
...
TEST(fqn_compute_basic) {
    ...
    char *fqn = pmm_pipeline_fqn_compute("proj", "pkg/handler.go", "Serve");
    ASSERT_NOT_NULL(fqn);
...
TEST(fqn_compute_strips_ext) {
    ...
    char *fqn = pmm_pipeline_fqn_compute("proj", "main.go", "main");
    ASSERT_NOT_NULL(fqn);
...
TEST(fqn_module_basic) {
    ...
    char *mod = pmm_pipeline_fqn_module("proj", "pkg/util/helper.go");
    ASSERT_NOT_NULL(mod);
...
TEST(fqn_folder_basic) {
    ...
    char *folder = pmm_pipeline_fqn_folder("proj", "pkg/util");
...
TEST(project_name_special_chars) {
    ...
    char *name = pmm_project_name_from_path("/home/user/my:project");
    ASSERT_NOT_NULL(name);
...
TEST(project_name_trailing_slash) {
    ...
    char *a = pmm_project_name_from_path("/home/user/project");
    ASSERT_NOT_NULL(a);
    free(a);
    PASS();
}
...
static const pmm_node_t *find_node_named(pmm_node_t *funcs, int count, const char *name) {
    for (int i = 0; i < count; i++) {
...
TEST(pipeline_complexity_transitive_loop_depth) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_cx_XXXXXX");
    if (!pmm_mkdtemp(tmpdir))
        FAIL("tmpdir");
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pmm_pipeline_run(p), 0);
    ...
    const pmm_node_t *inner = find_node_named(funcs, func_count, "inner");
    ...
    const pmm_node_t *outer = find_node_named(funcs, func_count, "outer");
    ...
    const pmm_node_t *recur = find_node_named(funcs, func_count, "recur");
 ...
 * pmm_gbuf_dump_to_sqlite frees the gbuf node index — otherwise it reads 0 and
...
TEST(pipeline_committed_counts_match_persisted) {
    ...
    pmm_pipeline_get_committed_counts(p, &committed_nodes, &committed_edges);

    pmm_store_t *s = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s);
    const char *project = pmm_pipeline_project_name(p);
    int persisted_nodes = pmm_store_count_nodes(s, project);
    pmm_store_close(s);
    ...
    ASSERT_FALSE(pmm_dump_verify_is_degraded(committed_nodes, persisted_nodes,
                                             PMM_DUMP_VERIFY_DEFAULT_RATIO,
                                             PMM_DUMP_VERIFY_MIN_FLOOR));
 ...
 * full nap cycle has failed to reclaim under budget. With PMM_MEM_BUDGET_MB=1
 ...
 * count (64) > workers+2.
 * The counter (pmm_pp_bp_nap_cycles) makes this deterministic — no timing.
 *
...
TEST(pipeline_backpressure_futile_nap_disengages) {
     ...
     * Set via the test hook, NOT setenv + pmm_mem_init: the init-once guard
    ...
    size_t saved_budget = pmm_mem_budget();
    pmm_mem_set_budget_for_tests((size_t)1024 * 1024);
    ASSERT_TRUE(pmm_mem_budget() > 0);
    ASSERT_TRUE(pmm_mem_over_budget());

    pmm_pp_bp_nap_cycles_reset();
    char db_path[512];
     ...
     * bound host-independent. pmm_pipeline_run joins its workers before return,
     * so restoring the process environment after freeing p cannot race them. */
    enum { TEST_WORKERS = 4 };
    const char *old_workers = getenv("PMM_WORKERS");
    char *saved_workers = old_workers ? strdup(old_workers) : NULL;
    if (old_workers && !saved_workers) {
        pmm_mem_set_budget_for_tests(saved_budget);
        pmm_pipeline_free(p);
        teardown_test_repo();
        FAIL("failed to save PMM_WORKERS");
    }
    if (pmm_setenv("PMM_WORKERS", "4", 1) != 0) {
        free(saved_workers);
        pmm_mem_set_budget_for_tests(saved_budget);
        pmm_pipeline_free(p);
        teardown_test_repo();
        FAIL("failed to set PMM_WORKERS");
    }

    int rc = pmm_pipeline_run(p);
    long cycles = pmm_pp_bp_nap_cycles();

    /* Restore the caller-visible budget BEFORE asserting. */
    pmm_mem_set_budget_for_tests(saved_budget);
    pmm_pipeline_free(p);
    int restore_workers_rc =
        saved_workers ? pmm_setenv("PMM_WORKERS", saved_workers, 1) : pmm_unsetenv("PMM_WORKERS");
    free(saved_workers);
    teardown_test_repo();
...
extern long pmm_ts_full_registry_builds(void);
extern void pmm_ts_full_registry_builds_reset(void);
 ...
 * registry, never a full per-file build. pmm_run_ts_lsp_cross registers
...
TEST(pipeline_seq_ts_cross_uses_shared_registry) {
    ...
    pmm_ts_full_registry_builds_reset();
    pmm_pipeline_t *p = pmm_pipeline_new(g_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    int rc = pmm_pipeline_run(p);
    long builds = pmm_ts_full_registry_builds();
    ...
    if (s) {
        linked =
            cross_file_call_exists(s, pmm_pipeline_project_name(p), "caller00", "sharedHelper");
        pmm_store_close(s);
    }
    pmm_pipeline_free(p);
    teardown_test_repo();
