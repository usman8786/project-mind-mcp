...
static git_env_snapshot_t save_git_env(void) {
    ...
    snapshot.home = home ? pmm_strdup(home) : NULL;
    snapshot.xdg_config_home = xdg ? pmm_strdup(xdg) : NULL;
    return snapshot;
}

static void restore_git_env(git_env_snapshot_t *snapshot) {
    if (snapshot->home) {
        pmm_setenv("HOME", snapshot->home, 1);
        free(snapshot->home);
    } else {
        pmm_unsetenv("HOME");
    }
    ...
    if (snapshot->xdg_config_home) {
        pmm_setenv("XDG_CONFIG_HOME", snapshot->xdg_config_home, 1);
...
static bool discover_has_rel_path(const pmm_file_info_t *files, int count, const char *rel_path) {
...
TEST(skip_git) {
    ASSERT_TRUE(pmm_should_skip_dir(".git", PMM_MODE_FULL));
    PASS();
}
TEST(skip_node_modules) {
    ASSERT_TRUE(pmm_should_skip_dir("node_modules", PMM_MODE_FULL));
    PASS();
}
TEST(skip_pycache) {
    ASSERT_TRUE(pmm_should_skip_dir("__pycache__", PMM_MODE_FULL));
    PASS();
}
TEST(skip_venv) {
    ASSERT_TRUE(pmm_should_skip_dir("venv", PMM_MODE_FULL));
    PASS();
}
TEST(skip_dist) {
    ASSERT_TRUE(pmm_should_skip_dir("dist", PMM_MODE_FULL));
    PASS();
}
TEST(skip_target) {
    ASSERT_TRUE(pmm_should_skip_dir("target", PMM_MODE_FULL));
    PASS();
}
TEST(skip_vendor) {
    ASSERT_TRUE(pmm_should_skip_dir("vendor", PMM_MODE_FULL));
    PASS();
}
TEST(skip_vendored) {
    ASSERT_TRUE(pmm_should_skip_dir("vendored", PMM_MODE_FULL));
    PASS();
}
TEST(skip_terraform) {
    ASSERT_TRUE(pmm_should_skip_dir(".terraform", PMM_MODE_FULL));
    PASS();
}
TEST(skip_coverage) {
    ASSERT_TRUE(pmm_should_skip_dir("coverage", PMM_MODE_FULL));
    PASS();
}
TEST(skip_idea) {
    ASSERT_TRUE(pmm_should_skip_dir(".idea", PMM_MODE_FULL));
    PASS();
}
TEST(skip_claude) {
    ASSERT_TRUE(pmm_should_skip_dir(".claude", PMM_MODE_FULL));
    PASS();
}

/* Not skipped in full mode */
TEST(no_skip_src) {
    ASSERT_FALSE(pmm_should_skip_dir("src", PMM_MODE_FULL));
    PASS();
}
TEST(no_skip_lib) {
    ASSERT_FALSE(pmm_should_skip_dir("lib", PMM_MODE_FULL));
    PASS();
}
TEST(no_skip_docs_full) {
    ASSERT_FALSE(pmm_should_skip_dir("docs", PMM_MODE_FULL));
    PASS();
}
TEST(no_skip_test_full) {
    ASSERT_FALSE(pmm_should_skip_dir("__tests__", PMM_MODE_FULL));
    PASS();
}
...
TEST(skip_fast_examples) {
    ASSERT_TRUE(pmm_should_skip_dir("examples", PMM_MODE_FAST));
    PASS();
}
TEST(skip_fast_tests) {
    ASSERT_TRUE(pmm_should_skip_dir("__tests__", PMM_MODE_FAST));
    PASS();
}
TEST(skip_fast_fixtures) {
    ASSERT_TRUE(pmm_should_skip_dir("fixtures", PMM_MODE_FAST));
    PASS();
}
TEST(skip_fast_testdata) {
    ASSERT_TRUE(pmm_should_skip_dir("testdata", PMM_MODE_FAST));
    PASS();
}
TEST(skip_fast_generated) {
    ASSERT_TRUE(pmm_should_skip_dir("generated", PMM_MODE_FAST));
    PASS();
}
TEST(skip_fast_assets) {
    ASSERT_TRUE(pmm_should_skip_dir("assets", PMM_MODE_FAST));
    PASS();
}
TEST(skip_fast_3rdparty) {
    ASSERT_TRUE(pmm_should_skip_dir("third_party", PMM_MODE_FAST));
    PASS();
}
TEST(skip_fast_e2e) {
    ASSERT_TRUE(pmm_should_skip_dir("e2e", PMM_MODE_FAST));
    PASS();
}
...
TEST(suffix_pyc) {
    ASSERT_TRUE(pmm_has_ignored_suffix("module.pyc", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_o) {
    ASSERT_TRUE(pmm_has_ignored_suffix("main.o", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_so) {
    ASSERT_TRUE(pmm_has_ignored_suffix("lib.so", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_png) {
    ASSERT_TRUE(pmm_has_ignored_suffix("icon.png", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_jpg) {
    ASSERT_TRUE(pmm_has_ignored_suffix("photo.jpg", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_wasm) {
    ASSERT_TRUE(pmm_has_ignored_suffix("app.wasm", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_db) {
    ASSERT_TRUE(pmm_has_ignored_suffix("data.db", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_sqlite) {
    ASSERT_TRUE(pmm_has_ignored_suffix("store.sqlite3", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_tmp) {
    ASSERT_TRUE(pmm_has_ignored_suffix("file.tmp", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_tilde) {
    ASSERT_TRUE(pmm_has_ignored_suffix("file~", PMM_MODE_FULL));
    PASS();
}

/* Not ignored */
TEST(suffix_go) {
    ASSERT_FALSE(pmm_has_ignored_suffix("main.go", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_py) {
    ASSERT_FALSE(pmm_has_ignored_suffix("app.py", PMM_MODE_FULL));
    PASS();
}
TEST(suffix_c) {
    ASSERT_FALSE(pmm_has_ignored_suffix("lib.c", PMM_MODE_FULL));
    PASS();
}

/* Fast mode additional suffixes */
TEST(suffix_fast_zip) {
    ASSERT_TRUE(pmm_has_ignored_suffix("archive.zip", PMM_MODE_FAST));
    PASS();
}
TEST(suffix_fast_pdf) {
    ASSERT_TRUE(pmm_has_ignored_suffix("manual.pdf", PMM_MODE_FAST));
    PASS();
}
TEST(suffix_fast_mp3) {
    ASSERT_TRUE(pmm_has_ignored_suffix("sound.mp3", PMM_MODE_FAST));
    PASS();
}
TEST(suffix_fast_pem) {
    ASSERT_TRUE(pmm_has_ignored_suffix("cert.pem", PMM_MODE_FAST));
    PASS();
}
...
TEST(fn_skip_license) {
    ASSERT_TRUE(pmm_should_skip_filename("LICENSE", PMM_MODE_FAST));
    PASS();
}
TEST(fn_skip_changelog) {
    ASSERT_TRUE(pmm_should_skip_filename("CHANGELOG.md", PMM_MODE_FAST));
    PASS();
}
TEST(fn_skip_gosum) {
    ASSERT_TRUE(pmm_should_skip_filename("go.sum", PMM_MODE_FAST));
    PASS();
}
TEST(fn_skip_yarnlock) {
    ASSERT_TRUE(pmm_should_skip_filename("yarn.lock", PMM_MODE_FAST));
    PASS();
}
TEST(fn_skip_pkglock) {
    ASSERT_TRUE(pmm_should_skip_filename("package-lock.json", PMM_MODE_FAST));
    PASS();
}

/* Not skipped in full mode */
TEST(fn_no_skip_license_full) {
    ASSERT_FALSE(pmm_should_skip_filename("LICENSE", PMM_MODE_FULL));
    PASS();
}
...
TEST(pattern_dts) {
    ASSERT_TRUE(pmm_matches_fast_pattern("types.d.ts", PMM_MODE_FAST));
    PASS();
}
TEST(pattern_pbgo) {
    ASSERT_TRUE(pmm_matches_fast_pattern("service.pb.go", PMM_MODE_FAST));
    PASS();
}
TEST(pattern_pb2py) {
    ASSERT_TRUE(pmm_matches_fast_pattern("api_pb2.py", PMM_MODE_FAST));
    PASS();
}
TEST(pattern_mock) {
    ASSERT_TRUE(pmm_matches_fast_pattern("mock_service.go", PMM_MODE_FAST));
    PASS();
}
TEST(pattern_test_dot) {
    ASSERT_TRUE(pmm_matches_fast_pattern("App.test.js", PMM_MODE_FAST));
    PASS();
}
TEST(pattern_spec) {
    ASSERT_TRUE(pmm_matches_fast_pattern("App.spec.ts", PMM_MODE_FAST));
    PASS();
}
TEST(pattern_stories) {
    ASSERT_TRUE(pmm_matches_fast_pattern("Button.stories.tsx", PMM_MODE_FAST));
    PASS();
}

/* Not matched in full mode */
TEST(pattern_dts_full) {
    ASSERT_FALSE(pmm_matches_fast_pattern("types.d.ts", PMM_MODE_FULL));
    PASS();
}
...
TEST(discover_simple) {
    char *base = th_mktempdir("pmm_disc_simple");
    ASSERT(base != NULL);
    ...
    pmm_discover_opts_t opts = {0};
    pmm_file_info_t *files = NULL;
    int count = 0;

    int rc = pmm_discover(base, &opts, &files, &count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 2); /* main.go + app.py, not icon.png */

    bool found_go = false, found_py = false;
    for (int i = 0; i < count; i++) {
        if (files[i].language == PMM_LANG_GO)
            found_go = true;
        if (files[i].language == PMM_LANG_PYTHON)
            found_py = true;
    }
...
TEST(discover_wide_sibling_fanout_exceeds_initial_walk_stack) {
    char *base = th_mktempdir("pmm_disc_wide");
    ASSERT(base != NULL);
...
TEST(discover_bounded_count_is_allocation_free_and_limit_exact) {
    char *base = th_mktempdir("pmm_disc_bounded");
    ASSERT(base != NULL);
    ...
    int limited_count = -1;
    pmm_discover_status_t limited =
        pmm_discover_count_bounded(base, &opts, 1, pmm_now_ms() + 2000, &limited_count);
    int exact_count = -1;
    pmm_discover_status_t exact =
        pmm_discover_count_bounded(base, &opts, 2, pmm_now_ms() + 2000, &exact_count);

    th_cleanup(base);
    ASSERT_EQ(limited, PMM_DISCOVER_LIMIT_EXCEEDED);
    ASSERT_EQ(limited_count, 1);
    ASSERT_EQ(exact, PMM_DISCOVER_OK);
    ASSERT_EQ(exact_count, 2);
    PASS();
}

TEST(discover_bounded_count_fails_closed_after_deadline) {
    char *base = th_mktempdir("pmm_disc_deadline");
    ASSERT(base != NULL);
    th_write_file(TH_PATH(base, "source.c"), "int source;\n");

    pmm_discover_opts_t opts = {.mode = PMM_MODE_FULL};
    int count = 99;
    pmm_discover_status_t status = pmm_discover_count_bounded(base, &opts, 100, 1, &count);

    th_cleanup(base);
    ASSERT_EQ(status, PMM_DISCOVER_ERROR);
    ASSERT_EQ(count, -1);
    PASS();
}

TEST(discover_skips_git_dir) {
    char *base = th_mktempdir("pmm_disc_git");
    ASSERT(base != NULL);
...
TEST(discover_with_gitignore) {
    char *base = th_mktempdir("pmm_disc_gi");
    ASSERT(base != NULL);
    ...
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 1);
    ASSERT_EQ(files[0].language, PMM_LANG_GO);
...
TEST(discover_with_global_xdg_ignore) {
    ...
    char *tmp = th_mktempdir("pmm_disc_global_xdg");
    ASSERT(tmp != NULL);
    ...
    pmm_setenv("XDG_CONFIG_HOME", xdg_env, 1);
    ...
    int rc = pmm_discover(repo, &opts, &files, &count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 1);
...
TEST(discover_global_excludesfile_from_gitconfig_tilde) {
    git_env_snapshot_t env = save_git_env();
    char *tmp = th_mktempdir("pmm_disc_global_cfg");
    ASSERT(tmp != NULL);
...
TEST(discover_repo_local_excludesfile_is_ignored) {
    git_env_snapshot_t env = save_git_env();
    char *tmp = th_mktempdir("pmm_disc_repo_cfg_ignored");
    ASSERT(tmp != NULL);
...
TEST(discover_missing_global_excludes_is_noop) {
    git_env_snapshot_t env = save_git_env();
    char *tmp = th_mktempdir("pmm_disc_global_missing");
    ASSERT(tmp != NULL);
...
TEST(discover_pmmignore_negates_global_ignore) {
    git_env_snapshot_t env = save_git_env();
    char *tmp = th_mktempdir("pmm_disc_global_neg");
    ASSERT(tmp != NULL);
    ...
    th_write_file(TH_PATH(repo, ".pmmignore"), "!rescued.go\n");
    th_write_file(TH_PATH(repo, "main.go"), "package main\n");
...
TEST(discover_gitignore_dir_excluded_issue234) {
    char *base = th_mktempdir("pmm_disc_gi234");
    ASSERT(base != NULL);
...
TEST(discover_max_file_size) {
    char *base = th_mktempdir("pmm_disc_size");
    ASSERT(base != NULL);
...
TEST(discover_null_path) {
    pmm_file_info_t *files = NULL;
    int count = 0;
    int rc = pmm_discover(NULL, NULL, &files, &count);
    ASSERT_EQ(rc, -1);
    PASS();
}

TEST(discover_nonexistent_path) {
    char *base = th_mktempdir("pmm_disc_noexist");
    char fake[512];
    snprintf(fake, sizeof(fake), "%s/nonexistent_12345", base ? base : "/tmp");
    pmm_file_info_t *files = NULL;
    int count = 0;
    int rc = pmm_discover(fake, NULL, &files, &count);
    ASSERT_EQ(rc, -1);
    th_cleanup(base);
    PASS();
}
...
TEST(discover_free_null) {
    pmm_discover_free(NULL, 0);
    PASS();
}

TEST(discover_skips_worktrees) {
    char *base = th_mktempdir("pmm_disc_wt");
    ASSERT(base != NULL);
...
TEST(discover_pmmignore) {
    char *base = th_mktempdir("pmm_disc_cbmi");
    ASSERT(base != NULL);

    th_mkdir_p(TH_PATH(base, ".git"));
    th_write_file(TH_PATH(base, ".pmmignore"), "generated/\n*.pb.go\n");
...
TEST(discover_pmmignore_stacks) {
    char *base = th_mktempdir("pmm_disc_stack");
    ASSERT(base != NULL);
...
#else
    char *base = th_mktempdir("pmm_disc_sym");
    ASSERT(base != NULL);
...
TEST(discover_new_ignore_patterns) {
    char *base = th_mktempdir("pmm_disc_newign");
    ASSERT(base != NULL);
...
TEST(discover_generic_dirs_full_mode) {
    char *base = th_mktempdir("pmm_disc_genfull");
    ASSERT(base != NULL);
...
TEST(discover_generic_dirs_fast_mode) {
    char *base = th_mktempdir("pmm_disc_genfast");
    ASSERT(base != NULL);
...
TEST(discover_deploy_excluded_full_mode) {
    char *base = th_mktempdir("pmm_disc_deploy");
    ASSERT(base != NULL);
...
TEST(discover_pmmignore_no_git) {
    char *base = th_mktempdir("pmm_disc_nogit");
    ASSERT(base != NULL);

    th_write_file(TH_PATH(base, ".pmmignore"), "scratch/\n");
    th_write_file(TH_PATH(base, "main.go"), "package main\n");
...
/* ── .pmmignore negation vs built-in skip dirs (issue #500) ────── */
...
/* A "!obj/" negation in .pmmignore must un-skip the built-in ALWAYS_SKIP
...
TEST(discover_pmmignore_negates_always_skip_dir) {
    char *base = th_mktempdir("pmm_disc_cbmi_neg_obj");
    ASSERT(base != NULL);

    th_write_file(TH_PATH(base, ".pmmignore"), "!obj/\n");
    th_write_file(TH_PATH(base, "main.go"), "package main\n");
    ...
    int rc = pmm_discover_ex(base, &opts, &files, &count, &excluded, &excluded_count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 2);
    ASSERT_TRUE(discover_has_rel_path(files, count, "main.go"));
    ...
    pmm_discover_free_excluded(excluded, excluded_count);
    pmm_discover_free(files, count);
    th_cleanup(base);
    PASS();
}
...
TEST(discover_pmmignore_negates_only_nested_skip_dir) {
    char *base = th_mktempdir("pmm_disc_cbmi_neg_nested");
    ASSERT(base != NULL);

    th_write_file(TH_PATH(base, ".pmmignore"), "!src/target/\n");
...
TEST(discover_pmmignore_negation_last_match_wins) {
    char *base = th_mktempdir("pmm_disc_cbmi_order1");
    ASSERT(base != NULL);
    ...
    base = th_mktempdir("pmm_disc_cbmi_order2");
    ASSERT(base != NULL);
    ...
    rc = pmm_discover(base, &opts, &files, &count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 1);
    ASSERT_TRUE(discover_has_rel_path(files, count, "main.go"));
 ...
 * .pmmignore negation. Green by construction; guards the policy. */
TEST(discover_pmmignore_negation_cannot_unskip_safety_core) {
    char *base = th_mktempdir("pmm_disc_cbmi_safety");
    ASSERT(base != NULL);

    th_write_file(TH_PATH(base, ".pmmignore"),
                  "!.git/\n!node_modules/\n!.worktrees/\n!.claude-worktrees/\n");
    th_write_file(TH_PATH(base, "main.go"), "package main\n");
...
TEST(discover_git_info_exclude) {
    char *base = th_mktempdir("pmm_disc_exc");
    ASSERT(base != NULL);
...
TEST(discover_git_info_exclude_stacks_with_gitignore) {
    char *base = th_mktempdir("pmm_disc_exc_stack");
    ASSERT(base != NULL);
...
TEST(discover_worktree_info_exclude) {
    char *base = th_mktempdir("pmm_disc_wt_exc");
    ASSERT(base != NULL);
...
TEST(discover_worktree_committed_gitignore) {
    char *base = th_mktempdir("pmm_disc_wt_gi");
    ASSERT(base != NULL);
...
TEST(discover_nested_gitignore) {
    char *base = th_mktempdir("pmm_disc_ngi");
    ASSERT(base != NULL);
...
TEST(discover_nested_gitignore_stacks_with_root) {
    char *base = th_mktempdir("pmm_disc_ngi_stack");
    ASSERT(base != NULL);
...
TEST(discover_many_nested_gitignores_do_not_exhaust_matcher_ownership) {
    ...
    char *base = th_mktempdir("pmm_disc_many_ngi");
    ASSERT(base != NULL);
    ...
    pmm_file_info_t *files = NULL;
    int count = 0;
    int discover_rc = fixture_ready ? pmm_discover(base, &opts, &files, &count) : -1;
    int bounded_count = -1;
    pmm_discover_status_t bounded_status =
        fixture_ready
            ? pmm_discover_count_bounded(base, &opts, PACKAGE_COUNT + 1, 0, &bounded_count)
            : PMM_DISCOVER_ERROR;
    ...
    ASSERT_EQ(bounded_status, PMM_DISCOVER_OK);
...
SUITE(discover) {
    ...
    RUN_TEST(discover_pmmignore_negates_global_ignore);
    ...
    RUN_TEST(discover_pmmignore);
    RUN_TEST(discover_pmmignore_stacks);
    ...
    RUN_TEST(discover_pmmignore_no_git);

    /* .pmmignore negation vs built-in skip dirs (issue #500) */
    RUN_TEST(discover_pmmignore_negates_always_skip_dir);
    RUN_TEST(discover_pmmignore_negates_only_nested_skip_dir);
    RUN_TEST(discover_pmmignore_negates_fast_skip_dir);
    RUN_TEST(discover_pmmignore_negation_last_match_wins);
    RUN_TEST(discover_pmmignore_negation_cannot_unskip_safety_core);
