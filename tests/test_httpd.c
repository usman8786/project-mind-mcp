...
TEST(httpd_parse_simple_get) {
    ...
    int rc = pmm_http_parse_head(raw, strlen(raw), &req, &body_off, &clen);
    ASSERT_EQ(rc, 0);
...
TEST(httpd_parse_security_headers_and_rejects_duplicates) {
    ...
    ASSERT_EQ(pmm_http_parse_head(raw, strlen(raw), &req, &body_off, &content_length), 0);
    ...
    for (size_t i = 0; i < sizeof(duplicates) / sizeof(duplicates[0]); i++) {
        ASSERT_EQ(pmm_http_parse_head(duplicates[i], strlen(duplicates[i]), &req, &body_off,
                                      &content_length),
                  400);
    }
    PASS();
...
TEST(httpd_parse_origin_case_insensitive) {
    ...
    pmm_http_req_t req;
    size_t body_off = 0, clen = 0;
    ASSERT_EQ(pmm_http_parse_head(raw, strlen(raw), &req, &body_off, &clen), 0);
...
TEST(httpd_parse_rejects_bare_lf) {
    ...
    pmm_http_req_t req;
    size_t body_off = 0, clen = 0;
    ASSERT_EQ(pmm_http_parse_head(raw, strlen(raw), &req, &body_off, &clen), 400);
    PASS();
}
...
TEST(httpd_parse_rejects_chunked) {
    ...
    pmm_http_req_t req;
    size_t body_off = 0, clen = 0;
    ASSERT_EQ(pmm_http_parse_head(raw, strlen(raw), &req, &body_off, &clen), 411);
    PASS();
}
...
TEST(httpd_parse_rejects_oversized_content_length) {
    ...
    pmm_http_req_t req;
    size_t body_off = 0, clen = 0;
    ASSERT_EQ(pmm_http_parse_head(raw, strlen(raw), &req, &body_off, &clen), 413);
    PASS();
}
...
TEST(httpd_parse_rejects_garbage_content_length) {
    ...
    ASSERT_EQ(pmm_http_parse_head(neg, strlen(neg), &req, &body_off, &clen), 400);
    PASS();
}
...
TEST(httpd_parse_rejects_percent00_in_target) {
    ...
    ASSERT_EQ(pmm_http_parse_head(q, strlen(q), &req, &body_off, &clen), 400);
    PASS();
}
...
TEST(httpd_parse_rejects_raw_nul_in_head) {
    ...
    pmm_http_req_t req;
    size_t body_off = 0, clen = 0;
    ASSERT_EQ(pmm_http_parse_head(raw, len, &req, &body_off, &clen), 400);
    PASS();
}
...
TEST(httpd_parse_incomplete_head_needs_more) {
    ...
    pmm_http_req_t req;
    size_t body_off = 0, clen = 0;
    ASSERT_EQ(pmm_http_parse_head(raw, strlen(raw), &req, &body_off, &clen), PMM_HTTP_NEED_MORE);
    PASS();
}
...
TEST(httpd_parse_rejects_missing_version) {
    ...
    ASSERT_EQ(pmm_http_parse_head(v2, strlen(v2), &req, &body_off, &clen), 400);
    PASS();
}

TEST(httpd_parse_rejects_oversized_head) {
    /* A head that exceeds PMM_HTTP_MAX_HEAD without terminating → 431 */
    size_t big = PMM_HTTP_MAX_HEAD + 1024;
    char *raw = malloc(big);
    ASSERT_NOT_NULL(raw);
    ...
    pmm_http_req_t req;
    size_t body_off = 0, clen = 0;
    int rc = pmm_http_parse_head(raw, big, &req, &body_off, &clen);
    free(raw);
    ASSERT_EQ(rc, 431);
    PASS();
}
...
TEST(httpd_query_param_decode) {
    char buf[64];
    ASSERT_TRUE(pmm_http_query_param("a=hello+world&b=%2Ffoo%2F", "a", buf, (int)sizeof(buf)));
    ASSERT_STR_EQ(buf, "hello world");
    ASSERT_TRUE(pmm_http_query_param("a=hello+world&b=%2Ffoo%2F", "b", buf, (int)sizeof(buf)));
    ASSERT_STR_EQ(buf, "/foo/");
    /* uppercase + lowercase hex */
    ASSERT_TRUE(pmm_http_query_param("p=%2fTmp%2F", "p", buf, (int)sizeof(buf)));
...
TEST(httpd_query_param_edge_cases) {
    char buf[8];
    /* missing param */
    ASSERT_FALSE(pmm_http_query_param("a=1", "b", buf, (int)sizeof(buf)));
    /* empty value (current server treats it as absent) */
    ASSERT_FALSE(pmm_http_query_param("a=&b=2", "a", buf, (int)sizeof(buf)));
    /* value too large for buf */
    ASSERT_FALSE(pmm_http_query_param("a=123456789", "a", buf, (int)sizeof(buf)));
    /* decoded NUL rejected */
    char big[32];
    ASSERT_FALSE(pmm_http_query_param("a=x%00y", "a", big, (int)sizeof(big)));
    /* name is a prefix of another name — must not match */
    ASSERT_FALSE(pmm_http_query_param("abc=1", "ab", buf, (int)sizeof(buf)));
    /* truncated percent escape */
    ASSERT_FALSE(pmm_http_query_param("a=%2", "a", buf, (int)sizeof(buf)));
    PASS();
}

TEST(httpd_path_match_matrix) {
    /* exact */
    ASSERT_TRUE(pmm_http_path_match("/", "/"));
    ASSERT_FALSE(pmm_http_path_match("/x", "/"));
    ASSERT_TRUE(pmm_http_path_match("/rpc", "/rpc"));
    ASSERT_FALSE(pmm_http_path_match("/rpc2", "/rpc"));
    /* trailing-* prefix */
    ASSERT_TRUE(pmm_http_path_match("/api/layout", "/api/layout*"));
    ASSERT_TRUE(pmm_http_path_match("/assets/index-abc.js", "/assets/*"));
    ASSERT_FALSE(pmm_http_path_match("/api/browse", "/api/layout*"));
    /* raw path is matched — percent-encoded slash must NOT route */
    ASSERT_FALSE(pmm_http_path_match("/api%2Fbrowse", "/api/browse*"));
    ASSERT_FALSE(pmm_http_path_match("/api%2fbrowse", "/api/browse*"));
    /* CORS origin patterns */
    ASSERT_TRUE(pmm_http_path_match("http://localhost:5173", "http://localhost:*"));
    ASSERT_TRUE(pmm_http_path_match("http://127.0.0.1:9749", "http://127.0.0.1:*"));
    ASSERT_FALSE(pmm_http_path_match("http://evil.com", "http://localhost:*"));
    ASSERT_FALSE(pmm_http_path_match("https://localhost:5173", "http://localhost:*"));
    ASSERT_FALSE(pmm_http_path_match("http://localhost.evil.com:80", "http://localhost:*"));
    PASS();
}
...
TEST(httpd_resolves_bare_binary_path_from_path) {
...
#else
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_httpd_bin_XXXXXX");
    char *td = pmm_mkdtemp(tmpdir);
    ASSERT_NOT_NULL(td);

    char exe[512];
    snprintf(exe, sizeof(exe), "%s/project-mind-mcp", td);
    FILE *f = fopen(exe, "w");
    ASSERT_NOT_NULL(f);
    fputs("#!/bin/sh\nexit 0\n", f);
    fclose(f);
    ...
    pmm_setenv("PATH", td, 1);

    char resolved[1024];
    ASSERT_TRUE(
        pmm_http_server_resolve_binary_path("project-mind-mcp", resolved, sizeof(resolved)));
...
TEST(httpd_listen_ephemeral_port) {
    pmm_httpd_t *d = pmm_httpd_listen(0);
    ASSERT_NOT_NULL(d);
    int port = pmm_httpd_port(d);
    ASSERT_GT(port, 0);
    /* accept with a short timeout and no client → NULL, promptly */
    pmm_http_conn_t *c = pmm_httpd_accept(d, 50);
    ASSERT_NULL(c);
    ASSERT_TRUE(pmm_httpd_close(d));
    PASS();
}

TEST(httpd_listen_port_collision_returns_null) {
    pmm_httpd_t *d1 = pmm_httpd_listen(0);
    ASSERT_NOT_NULL(d1);
    pmm_httpd_t *d2 = pmm_httpd_listen(pmm_httpd_port(d1));
    ASSERT_NULL(d2);
    ASSERT_TRUE(pmm_httpd_close(d1));
    PASS();
}

TEST(httpd_close_refuses_while_connection_owns_listener) {
    pmm_httpd_t *listener = pmm_httpd_listen(0);
    ASSERT_NOT_NULL(listener);
    th_sock_t client = th_connect(pmm_httpd_port(listener));
    ASSERT_TRUE(client != TH_SOCK_BAD);
    pmm_http_conn_t *connection = pmm_httpd_accept(listener, 1000);
    ASSERT_NOT_NULL(connection);

    ASSERT_FALSE(pmm_httpd_close(listener));
    pmm_httpd_conn_close(connection);
    th_sock_close(client);
    ASSERT_TRUE(pmm_httpd_close(listener));
    PASS();
}
...
typedef struct {
    pmm_http_server_t *srv;
    pmm_thread_t tid;
} th_server_t;

static void *th_server_thread(void *arg) {
    pmm_http_server_run((pmm_http_server_t *)arg);
    return NULL;
}

static int th_server_thread_start(pmm_thread_t *thread, pmm_http_server_t *server) {
    if (!pmm_http_server_schedule_run(server))
        return -1;
    int rc = pmm_thread_create(thread, 0, th_server_thread, server);
...
static int th_server_start(th_server_t *ts) {
    ts->srv = pmm_http_server_new(0);
    if (!ts->srv)
        return -1;
    if (th_server_thread_start(&ts->tid, ts->srv) != 0) {
        (void)pmm_http_server_free(ts->srv);
        return -1;
    }
    return 0;
}

static int th_server_start_with_watcher(th_server_t *ts, pmm_watcher_t *watcher) {
    ts->srv = pmm_http_server_new(0);
    if (!ts->srv)
        return -1;
    pmm_http_server_set_watcher(ts->srv, watcher);
    if (th_server_thread_start(&ts->tid, ts->srv) != 0) {
        (void)pmm_http_server_free(ts->srv);
        return -1;
    }
...
static bool th_wait_http_server_activity(pmm_http_server_t *server, pmm_httpd_activity_t expected,
                                         uint32_t timeout_ms) {
    uint64_t deadline = pmm_now_ms() + timeout_ms;
    while (pmm_now_ms() < deadline) {
        if (pmm_http_server_activity_for_test(server) == expected)
            return true;
        pmm_usleep(1000);
    }
    return pmm_http_server_activity_for_test(server) == expected;
}

static bool th_wait_httpd_activity(pmm_httpd_t *listener, pmm_httpd_activity_t expected,
                                   uint32_t timeout_ms) {
    uint64_t deadline = pmm_now_ms() + timeout_ms;
    while (pmm_now_ms() < deadline) {
        if (pmm_httpd_activity_for_test(listener) == expected)
            return true;
        pmm_usleep(1000);
    }
    return pmm_httpd_activity_for_test(listener) == expected;
}
...
static int th_server_start_with_mutation_guard(th_server_t *ts, pmm_watcher_t *watcher,
    ...
    if (watcher)
        pmm_http_server_set_watcher(ts->srv, watcher);
    pmm_http_server_set_project_mutation_guard(ts->srv, th_ui_mutation_begin, th_ui_mutation_end,
                                               guard);
    if (th_server_thread_start(&ts->tid, ts->srv) != 0) {
        (void)pmm_http_server_free(ts->srv);
        return -1;
...
static void th_server_stop(th_server_t *ts) {
    pmm_http_server_stop(ts->srv);
    (void)pmm_thread_join(&ts->tid);
    (void)pmm_http_server_free(ts->srv);
}
...
static int ui_delete_fixture_init(ui_delete_fixture_t *fx) {
    ...
    char *tmp = th_mktempdir("pmm_httpd_delete");
    if (!tmp)
        return -1;
    ...
    pmm_setenv("PMM_CACHE_DIR", fx->cache_dir, 1);

    fx->store = pmm_store_open_memory();
    fx->watcher = pmm_watcher_new(fx->store, NULL, NULL);
    return fx->store && fx->watcher ? 0 : -1;
}

static void ui_delete_fixture_cleanup(ui_delete_fixture_t *fx) {
    if (fx->watcher)
        pmm_watcher_free(fx->watcher);
    if (fx->store)
        pmm_store_close(fx->store);
    if (fx->saved_cache_dir) {
        pmm_setenv("PMM_CACHE_DIR", fx->saved_cache_dir, 1);
...
static int ui_delete_request(th_server_t *ts, const char *target, char *resp, size_t respsz) {
    ...
    return th_http(pmm_http_server_port(ts->srv), req, resp, respsz);
}
...
static int ui_adr_seed(const ui_delete_fixture_t *fx, const char *project, const char *content) {
    ...
    int rc = pmm_store_adr_store(store, project, content);
...
static bool ui_adr_equals(const ui_delete_fixture_t *fx, const char *project,
    ...
    pmm_adr_t adr = {0};
    int rc = pmm_store_adr_get(store, project, &adr);
    bool equal = rc == PMM_STORE_OK && adr.content && strcmp(adr.content, expected) == 0;
    if (rc == PMM_STORE_OK)
...
TEST(ui_server_unknown_path_404) {
    ...
    int port = pmm_http_server_port(ts.srv);
...
TEST(ui_server_routes_indexing_through_joinable_daemon_executor) {
    char *root = th_mktempdir("pmm_httpd_daemon_index");
    ...
    pmm_http_server_set_index_executor(ts.srv, th_ui_index_executor, &executor);
    ...
    char response[4096];
    int response_length =
        th_http(pmm_http_server_port(ts.srv), request, response, sizeof(response));
...
TEST(ui_server_free_never_joins_active_index_worker) {
    char *root = th_mktempdir("pmm_httpd_active_index");
    ASSERT_NOT_NULL(root);
    ...
    ts.srv = pmm_http_server_new(0);
    ASSERT_NOT_NULL(ts.srv);
    pmm_http_server_set_index_executor(ts.srv, th_ui_blocking_index_executor, &executor);
    ASSERT_EQ(th_server_thread_start(&ts.tid, ts.srv), 0);
    ...
    snprintf(request, sizeof(request),
             ...
             strlen(body), body);
    char response[4096];
    ASSERT_GT(th_http(pmm_http_server_port(ts.srv), request, response, sizeof(response)), 0);
    ASSERT_EQ(th_status(response), 202);
    ASSERT_TRUE(th_wait_atomic_int(&executor.calls, 1, 2000));

    pmm_http_server_stop(ts.srv);
    ASSERT_EQ(pmm_thread_join(&ts.tid), 0);
    uint64_t started = pmm_now_ms();
    ASSERT_FALSE(pmm_http_server_free(ts.srv));
    ASSERT_LT(pmm_now_ms() - started, 1000);

    atomic_store(&executor.release, 1);
    bool freed = false;
    uint64_t deadline = pmm_now_ms() + 2000;
    while (!freed && pmm_now_ms() < deadline) {
        freed = pmm_http_server_free(ts.srv);
        if (!freed)
            pmm_usleep(1000);
    }
    ASSERT_TRUE(freed);
...
TEST(ui_server_root_serves_stub_404) {
    ...
    ASSERT_EQ(th_server_start(&ts), 0);
    char resp[4096];
    int n = th_http(pmm_http_server_port(ts.srv), "GET / HTTP/1.1\r\n\r\n", resp, sizeof(resp));
    ASSERT_GT(n, 0);
    ASSERT_EQ(th_status(resp), 404);
...
TEST(ui_server_rpc_allows_only_ui_read_tools) {
    ...
    char resp[8192];
    int n = th_http(pmm_http_server_port(ts.srv), req, resp, sizeof(resp));
    ASSERT_GT(n, 0);
    ASSERT_EQ(th_status(resp), 200);
    ...
    for (size_t i = 0; i < sizeof(blocked_tools) / sizeof(blocked_tools[0]); i++) {
        ...
        n = th_http(pmm_http_server_port(ts.srv), req, resp, sizeof(resp));
        ASSERT_GT(n, 0);
        ASSERT_EQ(th_status(resp), 403);
    }
...
TEST(ui_server_encoded_slash_not_routed) {
    ...
    ASSERT_EQ(th_server_start(&ts), 0);
    char resp[4096];
    int n = th_http(pmm_http_server_port(ts.srv), "GET /api%2Fbrowse?path=/tmp HTTP/1.1\r\n\r\n",
                    resp, sizeof(resp));
    ASSERT_GT(n, 0);
...
TEST(ui_server_nul_in_target_rejected) {
    ...
    char resp[4096];
    int n =
        th_http(pmm_http_server_port(ts.srv), "GET /a%00b HTTP/1.1\r\n\r\n", resp, sizeof(resp));
    ASSERT_GT(n, 0);
...
TEST(ui_server_browse_traversal_probe) {
    ...
    ASSERT_EQ(th_server_start(&ts), 0);
    char resp[65536];
    int n = th_http(pmm_http_server_port(ts.srv),
...
TEST(ui_server_adr_mutation_guard_busy_preserves_existing_adr) {
    ...
    ASSERT_EQ(ui_adr_seed(&fx, project, original), PMM_STORE_OK);
...
TEST(ui_server_delete_mutation_guard_busy_preserves_project) {
    ...
    pmm_watcher_watch(fx.watcher, project, fx.root_dir);
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 1);
    ...
    ASSERT_TRUE(pmm_file_exists(shm_path));
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 1);
...
TEST(ui_server_delete_mutation_guard_balances_success) {
    ...
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 0);
...
TEST(ui_server_delete_project_unwatches_after_delete) {
    ...
    pmm_watcher_watch(fx.watcher, "ui-delete-watch", fx.root_dir);
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 1);
    ...
    ASSERT_FALSE(pmm_file_exists(db));
    ASSERT_FALSE(pmm_file_exists(wal));
    ASSERT_FALSE(pmm_file_exists(shm));
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 0);
...
TEST(ui_server_delete_project_unwatches_missing_db) {
    ui_delete_fixture_t fx;
    ASSERT_EQ(ui_delete_fixture_init(&fx), 0);
    pmm_watcher_watch(fx.watcher, "ui-delete-missing", fx.root_dir);
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 1);
...
TEST(ui_server_delete_project_missing_name_keeps_watch) {
    ui_delete_fixture_t fx;
    ASSERT_EQ(ui_delete_fixture_init(&fx), 0);
    pmm_watcher_watch(fx.watcher, "ui-delete-still-watched", fx.root_dir);
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 1);
...
TEST(ui_server_delete_project_invalid_name_keeps_watch) {
    ui_delete_fixture_t fx;
    ASSERT_EQ(ui_delete_fixture_init(&fx), 0);
    pmm_watcher_watch(fx.watcher, "bad/name", fx.root_dir);
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 1);
...
TEST(ui_server_delete_project_unlink_failure_keeps_watch) {
    ...
    pmm_watcher_watch(fx.watcher, "ui-delete-unlink-fails", fx.root_dir);
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 1);
    ...
    ASSERT_TRUE(pmm_file_exists(db));
    ASSERT_EQ(pmm_watcher_watch_count(fx.watcher), 1);
...
TEST(ui_server_ui_config_prefers_config_lang) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/pmm_httpd_cfg_XXXXXX");
    char *td = pmm_mkdtemp(tmpdir);
    ASSERT_NOT_NULL(td);

    char *old_home = getenv("HOME") ? strdup(getenv("HOME")) : NULL;
    pmm_setenv("HOME", td, 1);

    char cache_dir[1024];
    snprintf(cache_dir, sizeof(cache_dir), "%s", pmm_resolve_cache_dir());
    pmm_config_t *cfg = pmm_config_open(cache_dir);
    ASSERT_NOT_NULL(cfg);
    ASSERT_EQ(pmm_config_set(cfg, PMM_CONFIG_UI_LANG, "zh"), 0);
...
TEST(ui_server_slow_request_hits_deadline) {
    ...
    pmm_http_server_set_recv_deadline_ms(ts.srv, 300);
    int port = pmm_http_server_port(ts.srv);
...
TEST(ui_server_access_log_redacts_query) {
    ...
    pmm_log_set_sink_ex(httpd_capture_log, PMM_LOG_SINK_REPLACE);
...
TEST(ui_server_free_refuses_active_loop) {
    ...
    char response[512];
    ASSERT_GT(
        th_http(pmm_http_server_port(server), "GET / HTTP/1.1\r\n\r\n", response, sizeof(response)),
        0);
    ASSERT_FALSE(pmm_http_server_free(server));
    pmm_http_server_stop(server);
    ASSERT_EQ(pmm_thread_join(&thread), 0);
    ASSERT_TRUE(pmm_http_server_free(server));
    PASS();
}
...
TEST(ui_server_free_refuses_scheduled_run_before_child_starts) {
    ...
    ASSERT_TRUE(pmm_http_server_schedule_run(server));
    ASSERT_FALSE(pmm_http_server_free(server));
    ASSERT_TRUE(pmm_http_server_cancel_scheduled_run(server));
    ASSERT_TRUE(pmm_http_server_free(server));
    PASS();
}

TEST(daemon_host_http_thread_create_failure_cancels_scheduled_run) {
    ASSERT_TRUE(pmm_daemon_host_http_thread_create_failure_lifecycle_for_test());
    PASS();
}
...
static void *th_httpd_large_reply(void *opaque) {
    ...
    pmm_http_conn_t *connection = pmm_httpd_accept(reply->listener, 3000);
    if (!connection) {
    ...
    if (response) {
        ...
        pmm_http_reply_buf(connection, 200, "Content-Type: application/octet-stream\r\n", response,
                           response_size);
        free(response);
    }
    pmm_httpd_conn_close(connection);
...
TEST(httpd_interrupt_unblocks_nonreading_large_response_within_one_second) {
    pmm_httpd_t *listener = pmm_httpd_listen(0);
    ASSERT_NOT_NULL(listener);
    pmm_httpd_set_send_buffer_for_test(listener, 64 * 1024);
    ...
    pmm_httpd_set_send_deadline_for_test(listener, 60 * 1000);
    th_httpd_large_reply_t reply = {.listener = listener};
    atomic_init(&reply.accepted, 0);
    atomic_init(&reply.finished, 0);

    th_sock_t socket = th_connect_with_recv_buffer(pmm_httpd_port(listener), 1024);
    ASSERT_TRUE(socket != TH_SOCK_BAD);
    pmm_thread_t reply_thread;
    ASSERT_EQ(pmm_thread_create(&reply_thread, 0, th_httpd_large_reply, &reply), 0);

    ASSERT_TRUE(th_wait_httpd_activity(listener, PMM_HTTPD_ACTIVITY_RESPONDING, 1000));
    ...
    pmm_thread_t watchdog_thread;
    ASSERT_EQ(pmm_thread_create(&watchdog_thread, 0, th_http_stop_watchdog, &watchdog), 0);

    pmm_httpd_interrupt(listener);
    ASSERT_EQ(pmm_thread_join(&reply_thread), 0);
    atomic_store(&watchdog.stop_finished, 1);
    ASSERT_EQ(pmm_thread_join(&watchdog_thread), 0);
...
TEST(ui_server_stop_interrupts_partial_request_within_one_second) {
    th_server_t ts;
    ASSERT_EQ(th_server_start(&ts), 0);
    pmm_http_server_set_recv_deadline_ms(ts.srv, 3000);
    int port = pmm_http_server_port(ts.srv);
    ...
    ASSERT_TRUE(th_wait_http_server_activity(ts.srv, PMM_HTTPD_ACTIVITY_READING_REQUEST, 1000));
    ...
    (void)th_sock_shutdown(socket);
    th_sock_close(socket);
    ASSERT_TRUE(pmm_http_server_free(ts.srv));
...
TEST(repo_info_web_base_normalizes_to_https) {
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        char *got = pmm_ui_git_web_base(cases[i].in);
        ASSERT_NOT_NULL(got);
    ...
    ASSERT_NULL(pmm_ui_git_web_base(""));
    ASSERT_NULL(pmm_ui_git_web_base("not-a-url"));
    PASS();
}

/* The remote_url field echoed to the client must have any user:pass@ stripped. */
TEST(repo_info_strips_credentials_from_remote) {
    char *safe = pmm_ui_git_strip_credentials("https://alice:s3cr3t@github.com/org/repo.git");
...
TEST(ui_server_list_projects_responds_under_watchdog) {
    ...
    snprintf(req, sizeof(req),
             ...
             (int)strlen(body), body);
    char resp[8192];
    int n = th_http_deadline(pmm_http_server_port(ts.srv), req, resp, sizeof(resp), 15000);
    th_server_stop(&ts);
...
static DWORD WINAPI th_gitctx_probe_thread(LPVOID arg) {
    ...
    pmm_git_context_t ctx;
    memset(&ctx, 0, sizeof(ctx));
    int rc = pmm_git_context_resolve(p->path, &ctx);
 ...
 * holds LIVE listening/AFD socket handles in this process, pmm_git_context_resolve
 * — the exact path list_projects runs (add_git_context_json → resolve →
 * pmm_popen(git)) — must not hang. Under a raw-_popen regression git inherits
...
TEST(git_context_resolve_no_hang_under_live_ui_sockets) {
...
#else
    char *tmp = th_mktempdir("pmm_798repro");
    if (!tmp)
    ...
    if (w != WAIT_OBJECT_0) {
        ...
        th_server_stop(&ts);
        FAIL("pmm_git_context_resolve hung under live UI sockets (#798 regression)");
    }
    CloseHandle(h);
    th_server_stop(&ts);
...
#else
    char *dir = th_mktempdir("pmm_browse");
    if (!dir) {
        FAIL("mktempdir");
    }
    char longname[240];
