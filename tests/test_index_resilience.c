 ...
 * error-surfacing wiring (has_error / read / oversized → pmm_file_error_t →
...
static pmm_store_t *ri_index_capture(RProj *lp, char **out_resp) {
...
TEST(index_oversized_file_reported) {
    ...
    snprintf(lp.tmpdir, sizeof(lp.tmpdir), "/tmp/pmm_resil_XXXXXX");
    ...
    pmm_setenv("PMM_MAX_FILE_BYTES", "1048576", 1); /* 1 MiB cap */
    pmm_setenv("PMM_INDEX_LOG", logpath, 1);        /* deterministic logfile path */

    char *resp = NULL;
    pmm_store_t *store = ri_index_capture(&lp, &resp);

    /* Unset env IMMEDIATELY (before any assert can bail) so a low cap never
     * leaks into other tests in this process — pmm_max_file_bytes() reads env
     * on every file. */
    pmm_unsetenv("PMM_MAX_FILE_BYTES");
    pmm_unsetenv("PMM_INDEX_LOG");
...
TEST(index_parse_partial_reported) {
    ...
    pmm_setenv("PMM_INDEX_LOG", logpath, 1);
    ...
    int cov_count = 0;
    ASSERT_EQ(pmm_store_coverage_get(store, lp.project, &rows, &cov_count), PMM_STORE_OK);
    int marked = 0;
    ...
    pmm_store_free_coverage(rows, cov_count);
    ASSERT_TRUE(marked);

    char qargs[900];
    snprintf(qargs, sizeof(qargs), "{\"project\":\"%s\"}", lp.project);
    char *qresp = pmm_mcp_handle_tool(lp.srv, "index_status", qargs);
    ...
    snprintf(qargs, sizeof(qargs), "{\"project\":\"%s\",\"qualified_name\":\"ok_before\"}",
             lp.project);
    char *sresp = pmm_mcp_handle_tool(lp.srv, "get_code_snippet", qargs);
    ...
    char *cresp2 = pmm_mcp_handle_tool(lp.srv, "get_code_snippet", qargs);
    ...
    snprintf(qargs, sizeof(qargs),
             ...
             lp.project);
    char *gresp = pmm_mcp_handle_tool(lp.srv, "query_graph", qargs);
    ...
    snprintf(qargs, sizeof(qargs),
             ...
             lp.project);
    char *cresp = pmm_mcp_handle_tool(lp.srv, "query_graph", qargs);
...
TEST(index_parse_partial_clears_on_fix) {
    ...
    char qargs[900];
    snprintf(qargs, sizeof(qargs), "{\"project\":\"%s\"}", lp.project);
    char *cov1 = pmm_mcp_handle_tool(lp.srv, "index_status", qargs);
    ...
    char *resp2 = pmm_mcp_handle_tool(lp.srv, "index_repository", iargs);
    ASSERT_NOT_NULL(resp2);
    free(resp2);

    char *cov2 = pmm_mcp_handle_tool(lp.srv, "index_status", qargs);
    ASSERT_NOT_NULL(cov2);
    ASSERT_NULL(strstr(cov2, "flaky.c"));
    free(cov2);

    store = pmm_store_open_path(lp.dbpath);
    if (!store) {
...
TEST(index_not_indexed_by_design_reported) {
    ...
    char qargs[900];
    snprintf(qargs, sizeof(qargs), "{\"project\":\"%s\"}", lp.project);
    char *sresp = pmm_mcp_handle_tool(lp.srv, "index_status", qargs);
    ASSERT_NOT_NULL(sresp);
    ...
    ASSERT_NOT_NULL(resp2);
    free(resp2);
    char *sresp2 = pmm_mcp_handle_tool(lp.srv, "index_status", qargs);
    ...
    pmm_node_t *hits = NULL;
    int hit_count = 0;
    ASSERT_EQ(pmm_store_find_nodes_by_name(store, lp.project, "hidden", &hits, &hit_count),
              PMM_STORE_OK);
    ASSERT_EQ(hit_count, 0);
    pmm_store_free_nodes(hits, hit_count);
    hits = NULL;
    hit_count = 0;
    ASSERT_EQ(pmm_store_find_nodes_by_name(store, lp.project, "alpha", &hits, &hit_count),
              PMM_STORE_OK);
...
TEST(index_relative_repo_path_canonicalized) {
    ...
    lp.project = pmm_project_name_from_path(canon);
    if (!lp.project) {
    ...
    char *resp = pmm_mcp_handle_tool(lp.srv, "index_repository", "{\"repo_path\":\".\"}");
    ...
    pmm_store_t *store = pmm_store_open_path(lp.dbpath);
    if (!store) {
    ...
    pmm_project_t info = {0};
    ASSERT_EQ(pmm_store_get_project(store, lp.project, &info), PMM_STORE_OK);
