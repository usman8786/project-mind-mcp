...
static pmm_store_t *mn_open_indexed(MN_LangProj *lp) {
...
static pmm_store_t *mn_index_files(MN_LangProj *lp, const MN_LangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_mn_XXXXXX");
...
static void mn_cleanup(MN_LangProj *lp, pmm_store_t *store) {
...
static int mn_count_label(pmm_store_t *store, const char *project, const char *label) {
...
static int mn_type_nodes(pmm_store_t *store, const char *project) {
...
static int mn_callables_with_outbound(pmm_store_t *store, const char *project) {
...
static MN_Metrics mn_metrics_files(const MN_LangFile *files, int nfiles) {
    MN_LangProj lp;
    pmm_store_t *store = mn_index_files(&lp, files, nfiles);
    MN_Metrics m = {0};
    if (store) {
        m.ok             = 1;
        ...
        m.implements     = pmm_store_count_edges_by_type(store, lp.project, "IMPLEMENTS");
        m.defines_method = pmm_store_count_edges_by_type(store, lp.project, "DEFINES_METHOD");
    }
    mn_cleanup(&lp, store);
    return m;
}
...
TEST(mn_module_python_package) {
    static const MN_LangFile f[] = {
        ...
        {"main.py",
         "from .pkg import add\n\n\ndef run():\n    return add(1, 2)\n"}};
    MN_LangProj lp;
    pmm_store_t *store = mn_index_files(&lp, f, 3);
    int functions = store ? mn_count_label(store, lp.project, "Function") : 0;
    int imports   = store ? pmm_store_count_edges_by_type(store, lp.project, "IMPORTS") : 0;
    int ok        = (store != NULL);
    mn_cleanup(&lp, store);
    ASSERT_TRUE(ok);
