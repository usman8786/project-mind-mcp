...
TEST(cypher_lex_simple_match) {
    pmm_lex_result_t r = {0};
    int rc = pmm_lex("MATCH (n:Function)", &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
...
TEST(cypher_lex_string_overflow) {
    ...
    pmm_lex_result_t r = {0};
    int rc = pmm_lex(query, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_GTE(r.count, 1);
    ASSERT_EQ(r.tokens[0].type, TOK_STRING);
    /* The string should be truncated to PMM_SZ_4K - 1 (4095) characters. */
...
TEST(cypher_lex_keywords_case_insensitive) {
    pmm_lex_result_t r = {0};
    int rc = pmm_lex("match WHERE Return limit", &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.tokens[0].type, TOK_MATCH);
...
TEST(cypher_lex_pipe_and_star) {
    pmm_lex_result_t r = {0};
    int rc = pmm_lex("[:TYPE1|TYPE2*1..3]", &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_simple_node) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(err);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(pmm_query_pattern(q).node_count, 1);
    ASSERT_EQ(pmm_query_pattern(q).rel_count, 0);
    ASSERT_STR_EQ(pmm_query_pattern(q).nodes[0].variable, "f");
    ASSERT_STR_EQ(pmm_query_pattern(q).nodes[0].label, "Function");
...
TEST(cypher_parse_relationship_outbound) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function)-[:CALLS]->(g:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(pmm_query_pattern(q).node_count, 2);
    ASSERT_EQ(pmm_query_pattern(q).rel_count, 1);
    ASSERT_STR_EQ(pmm_query_pattern(q).rels[0].types[0], "CALLS");
    ASSERT_STR_EQ(pmm_query_pattern(q).rels[0].direction, "outbound");
    ASSERT_EQ(pmm_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(pmm_query_pattern(q).rels[0].max_hops, 1);
...
TEST(cypher_parse_relationship_inbound) {
    ...
    ASSERT_STR_EQ(pmm_query_pattern(q).rels[0].direction, "inbound");
...
TEST(cypher_parse_relationship_any) {
    ...
    ASSERT_STR_EQ(pmm_query_pattern(q).rels[0].direction, "any");
...
TEST(cypher_parse_variable_length) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function)-[:CALLS*1..3]->(g:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(pmm_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(pmm_query_pattern(q).rels[0].max_hops, 3);
...
TEST(cypher_parse_variable_length_unbounded) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f)-[:CALLS*]->(g)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(pmm_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(pmm_query_pattern(q).rels[0].max_hops, 0); /* 0 = unbounded */
...
TEST(cypher_parse_multiple_edge_types) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f)-[:CALLS|HTTP_CALLS]->(g)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(pmm_query_pattern(q).rels[0].type_count, 2);
    ASSERT_STR_EQ(pmm_query_pattern(q).rels[0].types[0], "CALLS");
    ASSERT_STR_EQ(pmm_query_pattern(q).rels[0].types[1], "HTTP_CALLS");
...
TEST(cypher_parse_where_clause) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function) WHERE f.name = \"Foo\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
...
TEST(cypher_parse_where_regex) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function) WHERE f.name =~ \".*Order.*\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
    ASSERT_STR_EQ(q->where->root->cond.op, "=~");
...
TEST(cypher_parse_where_and) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function) WHERE f.name = \"A\" AND f.label = \"Function\"",
                              &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_return_simple) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function) RETURN f.name, f.qualified_name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_return_count) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f)-[:CALLS]->(g) RETURN f.name, COUNT(g) AS cnt", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_return_order_limit) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("MATCH (f:Function) RETURN f.name ORDER BY f.name DESC LIMIT 5", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_return_distinct) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function) RETURN DISTINCT f.label", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_inline_props) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function {name: \"Foo\"})", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(pmm_query_pattern(q).nodes[0].prop_count, 1);
    ASSERT_STR_EQ(pmm_query_pattern(q).nodes[0].props[0].key, "name");
    ASSERT_STR_EQ(pmm_query_pattern(q).nodes[0].props[0].value, "Foo");
...
TEST(cypher_parse_error) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("INVALID QUERY", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
    free(err);
    PASS();
 ...
 */
static pmm_store_t *setup_cypher_store(void) {
    ...
    int64_t id3 = pmm_store_upsert_node(s, &n3);
    int64_t id4 = pmm_store_upsert_node(s, &n4);
    int64_t id5 = pmm_store_upsert_node(s, &n5);

    pmm_edge_t e1 = {.project = "test", .source_id = id1, .target_id = id2, .type = "CALLS"};
    pmm_edge_t e2 = {.project = "test", .source_id = id2, .target_id = id3, .type = "CALLS"};
    pmm_edge_t e3 = {.project = "test", .source_id = id1, .target_id = id5, .type = "CALLS"};
    pmm_edge_t e4 = {.project = "test", .source_id = id4, .target_id = id1, .type = "DEFINES"};
...
TEST(cypher_exec_match_all_functions) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};

    int rc = pmm_cypher_execute(s, "MATCH (f:Function)", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 4); /* HandleOrder, ValidateOrder, SubmitOrder, LogError */
    ASSERT_GT(r.col_count, 0);

    pmm_cypher_result_free(&r);
    pmm_store_close(s);
    PASS();
}
...
TEST(cypher_exec_where_eq) {
    ...
    int rc =
        pmm_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"HandleOrder\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_varlength_path_semantics_issue797) {
    ...
    pmm_node_t leaf = {.project = "test",
                       ...
                       .start_line = 5,
                       .end_line = 6};
    int64_t id_loopy = pmm_store_upsert_node(s, &loopy);
    int64_t id_mid = pmm_store_upsert_node(s, &mid);
    int64_t id_leaf = pmm_store_upsert_node(s, &leaf);
    ASSERT_GT(id_loopy, 0);
    pmm_edge_t self_loop = {
        .project = "test", .source_id = id_loopy, .target_id = id_loopy, .type = "CALLS"};
    pmm_edge_t e1 = {
        .project = "test", .source_id = id_loopy, .target_id = id_mid, .type = "CALLS"};
    pmm_edge_t e2 = {.project = "test", .source_id = id_mid, .target_id = id_leaf, .type = "CALLS"};
    pmm_store_insert_edge(s, &self_loop);
    pmm_store_insert_edge(s, &e1);
    pmm_store_insert_edge(s, &e2);

    /* Bug 1: repeated variable must unify — only the self-loop matches. */
    pmm_cypher_result_t r1 = {0};
    ASSERT_EQ(pmm_cypher_execute(s, "MATCH (a)-[:CALLS]->(a) RETURN a.name", "test", 0, &r1), 0);
    ASSERT_EQ(r1.row_count, 1);
    pmm_cypher_result_free(&r1);

    /* Bug 2: *2..2 from loopy — only the REAL 2-chain (leaf); the self-loop
     * must not be reused to pad paths (relationship uniqueness). */
    pmm_cypher_result_t r2 = {0};
    ASSERT_EQ(pmm_cypher_execute(s,
                                 "MATCH (a {name: \"loopy\"})-[:CALLS*2..2]->(b) "
                                 "RETURN DISTINCT b.name",
                                 "test", 0, &r2),
              0);
    ASSERT_EQ(r2.row_count, 1); /* leaf only */
    pmm_cypher_result_free(&r2);

    /* Bug 2 amplifier: no directed path of length 5 exists at all. */
    pmm_cypher_result_t r3 = {0};
    ASSERT_EQ(pmm_cypher_execute(s,
...
TEST(cypher_exec_where_coalesce_issue874) {
    ...
    pmm_node_t c = {.project = "test",
                    ...
                    .start_line = 5,
                    .end_line = 6};
    ASSERT_GT(pmm_store_upsert_node(s, &a), 0);
    ASSERT_GT(pmm_store_upsert_node(s, &b), 0);
    ASSERT_GT(pmm_store_upsert_node(s, &c), 0);

    /* Default FAILS the predicate: only the node with depth 3 matches. */
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s,
...
TEST(cypher_exec_where_regex) {
    ...
    int rc =
        pmm_cypher_execute(s, "MATCH (f:Function) WHERE f.name =~ \".*Order.*\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_contains) {
    ...
    int rc =
        pmm_cypher_execute(s, "MATCH (f:Function) WHERE f.name CONTAINS \"Order\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_starts_with) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) WHERE f.name STARTS WITH \"Handle\"", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_func_id) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"HandleOrder\" RETURN id(f)",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_issue874_where_unsupported_func_error) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("MATCH (f:Function) WHERE foo(f.name) = \"x\" RETURN f.name", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_exec_limit) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 2", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 2);
...
TEST(cypher_exec_order_by) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.name ORDER BY f.name ASC", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 4);
 ...
 * capped at the engine ceiling (pmm_cypher_max_depth(), default 10). On
 ...
 * to pmm_store_bfs — an unbounded traversal (a DoS on cyclic graphs). RED before
...
TEST(cypher_exec_var_length_explicit_bound_capped) {
    ...
    for (int i = 0; i < 13; i++) {
        ...
        pmm_node_t n = {.project = "test",
                        ...
                        .qualified_name = qn,
                        .file_path = "chain.go"};
        ids[i] = pmm_store_upsert_node(s, &n);
    }
    for (int i = 0; i < 12; i++) {
        pmm_edge_t e = {
...
TEST(cypher_exec_no_results) {
    ...
    int rc =
        pmm_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"NonExistent\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_distinct) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN DISTINCT f.label", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_with_distinct_issue238) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) WITH DISTINCT f.label AS lbl RETURN lbl",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
    pmm_cypher_result_free(&r);
    ...
    pmm_cypher_result_t r2 = {0};
    rc = pmm_cypher_execute(s, "MATCH (f:Function) WITH f.label AS lbl RETURN lbl", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_label_test_issue241) {
    ...
    pmm_cypher_result_t r = {0};
    int rc =
        pmm_cypher_execute(s, "MATCH (f:Function) WHERE f:Function RETURN f.name", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 4);
    pmm_cypher_result_free(&r);
    ...
    pmm_cypher_result_t r2 = {0};
    rc = pmm_cypher_execute(s, "MATCH (f:Function) WHERE f:Class RETURN f.name", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ...
    pmm_cypher_result_t r3 = {0};
    rc =
        pmm_cypher_execute(s, "MATCH (f:Function) WHERE NOT f:Class RETURN f.name", "test", 0, &r3);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_count_distinct_issue239) {
    ...
    pmm_cypher_result_t r = {0};
    int rc =
        pmm_cypher_execute(s, "MATCH (f:Function) RETURN count(DISTINCT f.label)", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
    ...
    pmm_cypher_result_t r2 = {0};
    rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN count(f.label)", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ...
    pmm_cypher_result_t r3 = {0};
    rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN count(DISTINCT f.name)", "test", 0, &r3);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_unknown_func_return_errors) {
    ...
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN nosuchfunc(f.name)", "test", 0, &r);
    ASSERT_TRUE(rc != 0);
    ASSERT_NOT_NULL(r.error);
...
TEST(cypher_exec_label_alternation_issue242) {
    ...
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (n:Function|Module) RETURN n.name", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ...
    pmm_cypher_result_t r2 = {0};
    rc = pmm_cypher_execute(s, "MATCH (n:Function|Class) RETURN n.name", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r2.row_count, 4);
    pmm_cypher_result_free(&r2);
...
TEST(cypher_parse_where_starts_with) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("MATCH (f:Function) WHERE f.name STARTS WITH \"Send\" RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_where_contains) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("MATCH (f:Function) WHERE f.name CONTAINS \"Handler\" RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_where_numeric) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function) WHERE f.start_line > 10 RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
static pmm_store_t *setup_cypher_http_store(void) {
    pmm_store_t *s = pmm_store_open_memory();
    pmm_store_upsert_project(s, "test", "/tmp/test");
    ...
    pmm_store_insert_edge(s, &http);

    return s;
}

/* Helper: set up store with TWO HTTP_CALLS edges for filtering tests. */
static pmm_store_t *setup_cypher_multi_edge_store(void) {
    pmm_store_t *s = pmm_store_open_memory();
    pmm_store_upsert_project(s, "testproj", "/tmp/test");

    pmm_node_t n1 = {.project = "testproj",
                     .label = "Function",
                     .name = "SendOrder",
                     .qualified_name = "testproj.caller.SendOrder",
                     .file_path = "caller/client.go"};
    pmm_node_t n2 = {.project = "testproj",
                     .label = "Function",
                     .name = "HandleOrder",
                     .qualified_name = "testproj.handler.HandleOrder",
                     .file_path = "handler/routes.go"};
    pmm_node_t n3 = {.project = "testproj",
                     .label = "Function",
                     .name = "HandleHealth",
    ...
    pmm_edge_t e1 = {.project = "testproj",
                     .source_id = id1,
                     .target_id = id2,
                     .type = "HTTP_CALLS",
                     .properties_json =
                         "{\"url_path\":\"/api/orders\",\"confidence\":0.85,\"method\":\"POST\"}"};
    pmm_edge_t e2 = {.project = "testproj",
                     .source_id = id1,
                     .target_id = id3,
                     .type = "HTTP_CALLS",
...
static const char *cypher_get_col(const pmm_cypher_result_t *r, int row, const char *col) {
...
static bool cypher_has_row_with(const pmm_cypher_result_t *r, const char *col, const char *val) {
    int ci = -1;
    for (int c = 0; c < r->col_count; c++) {
        if (strcmp(r->columns[c], col) == 0) {
            ci = c;
...
TEST(cypher_edge_prop_access) {
    pmm_store_t *s = setup_cypher_http_store();
    pmm_cypher_result_t r = {0};
...
static void *cypher_edge_props_concurrently(void *opaque) {
    ...
    pmm_store_t *store = setup_cypher_http_store();
    if (!store) {
        return NULL;
    }
    ...
    pmm_node_t source = {.project = "test",
                         .label = "Function",
                         .name = "HandleOrder",
                         .qualified_name = "test.main.HandleOrder",
                         .file_path = "main.go",
                         .start_line = 10,
                         .end_line = 30};
    int64_t source_id = pmm_store_upsert_node(store, &source);
    for (int i = 0; i < 256; i++) {
        char name[64];
        ...
        pmm_node_t target = {.project = "test",
                             .label = "Function",
                             .name = name,
                             .qualified_name = qualified_name,
                             .file_path = "concurrent.go"};
        int64_t target_id = pmm_store_upsert_node(store, &target);
        pmm_edge_t edge = {
            .project = "test",
            .source_id = source_id,
            .target_id = target_id,
            ...
            .properties_json =
                "{\"url_path\":\"/api/orders\",\"confidence\":0.85,\"method\":\"POST\"}"};
        if (source_id < 0 || target_id < 0 || pmm_store_insert_edge(store, &edge) < 0) {
    ...
    for (int i = 0; i < 128; i++) {
        ...
        int rc = pmm_cypher_execute(store,
                                    "MATCH (a:Function)-[r:HTTP_CALLS]->(b:Function) "
...
TEST(cypher_edge_prop_storage_is_per_thread) {
    ...
    bool started0 = pmm_thread_create(&threads[0], 0, cypher_edge_props_concurrently, &ctx[0]) == 0;
    bool started1 = pmm_thread_create(&threads[1], 0, cypher_edge_props_concurrently, &ctx[1]) == 0;
...
TEST(cypher_edge_type_prop) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (a)-[r:HTTP_CALLS]->(b) RETURN r.type", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_edge_filter_contains) {
    pmm_store_t *s = setup_cypher_multi_edge_store();
    pmm_cypher_result_t r = {0};
...
TEST(cypher_bare_edge_return_exposes_properties_json) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (a)-[r:HTTP_CALLS]->(b) WHERE r.method = 'POST' RETURN r",
                                "testproj", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_apply_limit) {
    ...
    /* LIMIT 5 → 5 rows */
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 5", "lim", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 5);
    pmm_cypher_result_free(&r);
    ...
    memset(&r, 0, sizeof(r));
    rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.name", "lim", 10, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 10);
    pmm_cypher_result_free(&r);
    ...
    memset(&r, 0, sizeof(r));
    rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 30", "lim", 10, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_not_in) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f) WHERE NOT f.label IN [\"Module\"] RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_is_null) {
    ...
    pmm_store_upsert_node(s, &n2);
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) WHERE f.file_path IS NULL RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_is_not_null) {
    ...
    pmm_store_upsert_node(s, &n2);
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) WHERE f.file_path IS NOT NULL RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_return_star) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN * LIMIT 3", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 3);
...
TEST(cypher_parse_neq) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f:Function) WHERE f.name <> \"X\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
...
TEST(cypher_parse_in) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f) WHERE f.label IN [\"Function\", \"Module\"]", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
...
TEST(cypher_parse_is_null) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f) WHERE f.file_path IS NULL", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_expr_tree_and_or) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("MATCH (f) WHERE f.a = \"1\" AND f.b = \"2\" OR f.c = \"3\"", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_expr_tree_nested) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("MATCH (f) WHERE (f.a = \"1\" OR f.b = \"2\") AND f.c = \"3\"", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_error_create) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("CREATE (n:Node {name: \"X\"})", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_delete) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("DELETE n", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_set) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("SET n.name = \"X\"", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_merge) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MERGE (n:Node)", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_call) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("CALL db.labels()", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_exec_skip) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.name ORDER BY f.name ASC SKIP 2",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_sum) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN SUM(f.start_line) AS total", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_avg) {
    ...
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN AVG(f.start_line) AS avg_line",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_min) {
    ...
    int rc =
        pmm_cypher_execute(s, "MATCH (f:Function) RETURN MIN(f.start_line) AS mn", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_max) {
    ...
    pmm_cypher_result_t r = {0};
    int rc =
        pmm_cypher_execute(s, "MATCH (f:Function) RETURN MAX(f.start_line) AS mx", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_count_star) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN COUNT(*) AS n", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_parse_skip) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f) RETURN f.name SKIP 5 LIMIT 10", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->ret);
...
TEST(cypher_parse_sum_avg) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f) RETURN SUM(f.x) AS s, AVG(f.y) AS a", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_collect) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f)-[:CALLS]->(g) RETURN f.name, COLLECT(g.name) AS names", &q,
                              &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_tolower) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f) RETURN toLower(f.name) AS n", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_with_where) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("MATCH (f)-[:CALLS]->(g) WITH f.name AS caller, COUNT(g) AS cnt "
...
TEST(cypher_parse_multi_match) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("MATCH (a:Module) MATCH (b:Function) RETURN a.name, b.name", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->pattern_count, 2);
    ASSERT(!q->pattern_optional[0]);
    ASSERT(!q->pattern_optional[1]);
...
TEST(cypher_parse_union) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("MATCH (f) RETURN f.name UNION ALL MATCH (g) RETURN g.name", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT(q->union_all);
...
TEST(cypher_parse_unwind) {
    ...
    char *err = NULL;
    int rc =
        pmm_cypher_parse("UNWIND [\"a\", \"b\", \"c\"] AS x MATCH (f) RETURN f.name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_unwind_var) {
    pmm_query_t *q = NULL;
    char *err = NULL;
    int rc = pmm_cypher_parse("UNWIND items AS item MATCH (f) RETURN f.name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_issue240_labels_function) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (n:Module) RETURN labels(n) AS lbl", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 1);
    pmm_cypher_result_free(&r);
...
TEST(cypher_issue873_distinct_limit_dedupes_before_limit) {
    ...
    pmm_cypher_result_t r = {0};
    int rc =
        pmm_cypher_execute(s, "MATCH (n) RETURN DISTINCT n.label AS label LIMIT 2", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 2);
    pmm_cypher_result_free(&r);
...
TEST(cypher_issue252_tointeger) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN toInteger(f.start_line) AS ln",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    pmm_cypher_result_free(&r);
...
TEST(cypher_issue305_count_star_alias) {
    pmm_store_t *s = setup_cypher_store();
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (n) RETURN count(*) AS total", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 1);
    pmm_cypher_result_free(&r);
 ...
 * (vals[PMM_SZ_32] / func_bufs[PMM_SZ_32][…] in execute_return_simple and its
...
TEST(cypher_exec_prop_array_with_internal_commas) {
    ...
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (m:Method) RETURN m.decorators, m.lines", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_prop_string_with_escaped_quote) {
    ...
    pmm_cypher_result_t r = {0};
    int rc = pmm_cypher_execute(s, "MATCH (f:Function) RETURN f.signature", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_wide_return_projection_bounded) {
    ...
    for (int i = 0; i < 48; i++) { /* 48 > PMM_SZ_32 (32) fixed columns */
    ...
    if (pid == 0) {
        pmm_store_t *s = setup_cypher_store();
        pmm_cypher_result_t r = {0};
        (void)pmm_cypher_execute(s, query, "test", 0, &r);
        pmm_cypher_result_free(&r);
        pmm_store_close(s);
...
TEST(cypher_exec_deadline_aborts_runaway_query_issue601) {
    ...
    pmm_cypher_test_set_deadline_ms(0); /* trip on the first hot-loop check */
    int rc = pmm_cypher_execute(
        s, "MATCH (a) OPTIONAL MATCH (a)-[:CALLS]->(b) RETURN a.qualified_name, count(b)", "test",
        0, &r);
    pmm_cypher_test_set_deadline_ms(-1); /* restore default before asserting (thread-local) */

    ASSERT_TRUE(rc != 0); /* PMM_NOT_FOUND (-1) — query aborted, not success */
    ASSERT_NOT_NULL(r.error);
