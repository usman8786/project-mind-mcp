...
static int count_similar_to_edges(const pmm_gbuf_t *gb) {
    int count = 0;
    ...
    for (int i = 0; i < func_count; i++) {
        ...
        int edge_count = 0;
        pmm_gbuf_find_edges_by_source_type(gb, funcs[i]->id, "SIMILAR_TO", &edges, &edge_count);
        count += edge_count;
    }
    const pmm_gbuf_node_t **methods = NULL;
    int method_count = 0;
    pmm_gbuf_find_by_label(gb, "Method", &methods, &method_count);
    for (int i = 0; i < method_count; i++) {
        const pmm_gbuf_edge_t **edges = NULL;
        int edge_count = 0;
        pmm_gbuf_find_edges_by_source_type(gb, methods[i]->id, "SIMILAR_TO", &edges, &edge_count);
        count += edge_count;
    }
    return count;
}
...
TEST(minhash_identical_source_same_fingerprint) {
    ...
    CBMFileResult *r1 = extract_one(src, PMM_LANG_GO, "test", "a.go");
    CBMFileResult *r2 = extract_one(src, PMM_LANG_GO, "test", "b.go");
    ASSERT_NOT_NULL(r1);
    ASSERT_NOT_NULL(r2);
...
TEST(minhash_renamed_vars_same_fingerprint) {
    ...
    CBMFileResult *ra = extract_one(src_a, PMM_LANG_GO, "test", "a.go");
    CBMFileResult *rb = extract_one(src_b, PMM_LANG_GO, "test", "b.go");
    ASSERT_NOT_NULL(ra);
    ASSERT_NOT_NULL(rb);
    ...
    double j = pmm_minhash_jaccard(
        (const pmm_minhash_t *)da->fingerprint,
        (const pmm_minhash_t *)db->fingerprint);
...
TEST(minhash_empty_body_skipped) {
    ...
    CBMFileResult *r = extract_one(src, PMM_LANG_GO, "test", "tiny.go");
    ASSERT_NOT_NULL(r);
...
TEST(jaccard_identical) {
    pmm_minhash_t fp;
    for (int i = 0; i < PMM_MINHASH_K; i++) {
        fp.values[i] = (uint32_t)(i * 17 + 42);
    }
    double j = pmm_minhash_jaccard(&fp, &fp);
...
TEST(jaccard_disjoint) {
    ...
    double j = pmm_minhash_jaccard(&a, &b);
...
TEST(minhash_hex_roundtrip) {
    pmm_minhash_t original;
    for (int i = 0; i < PMM_MINHASH_K; i++) {
        original.values[i] = (uint32_t)(0xDEAD0000 + i);
    }
    char hex[PMM_MINHASH_HEX_LEN + 1];
    pmm_minhash_to_hex(&original, hex, sizeof(hex));
    ASSERT_EQ((int)strlen(hex), PMM_MINHASH_HEX_LEN);

    pmm_minhash_t decoded;
    bool ok = pmm_minhash_from_hex(hex, &decoded);
    ASSERT_TRUE(ok);
    ASSERT_MEM_EQ(&original, &decoded, sizeof(pmm_minhash_t));
    PASS();
}
...
TEST(lsh_same_bucket_similar) {
    ...
    pmm_lsh_index_t *idx = pmm_lsh_new();
    pmm_lsh_entry_t ea = {.node_id = 1, .fingerprint = &a, .file_path = "a.go", .file_ext = ".go"};
    pmm_lsh_insert(idx, &ea);

    const pmm_lsh_entry_t **candidates = NULL;
    int count = 0;
    pmm_lsh_query(idx, &b, &candidates, &count);
...
TEST(lsh_index_build_and_query) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        pmm_lsh_insert(idx, &e);
    }

    /* Query with fps[0] — should find fps[50] as candidate */
    const pmm_lsh_entry_t **candidates = NULL;
    int count = 0;
    pmm_lsh_query(idx, &fps[0], &candidates, &count);
...
static void make_fp_props(char *buf, int bufsize, const pmm_minhash_t *fp) {
    char hex[PMM_MINHASH_HEX_LEN + 1];
    pmm_minhash_to_hex(fp, hex, sizeof(hex));
...
TEST(pass_similarity_creates_edges) {
    ...
    pmm_gbuf_upsert_node(gb, "Function", "foo", "test.a.foo", "a.go", 1, 10, props);
    pmm_gbuf_upsert_node(gb, "Function", "bar", "test.b.bar", "b.go", 1, 10, props);
    ...
    int rc = pmm_pipeline_pass_similarity(&ctx);
    ASSERT_EQ(rc, 0);
...
TEST(pass_similarity_no_edges_different) {
    ...
    pmm_gbuf_t *gb = pmm_gbuf_new("test", "/tmp");
    pmm_gbuf_upsert_node(gb, "Function", "foo", "test.a.foo", "a.go", 1, 10, props_a);
    pmm_gbuf_upsert_node(gb, "Function", "bar", "test.b.bar", "b.go", 1, 10, props_b);
...
TEST(pass_similarity_same_file_tagged) {
    ...
    pmm_gbuf_t *gb = pmm_gbuf_new("test", "/tmp");
    pmm_gbuf_upsert_node(gb, "Function", "foo", "test.a.foo", "same.go", 1, 10, props);
    int64_t id_b =
        pmm_gbuf_upsert_node(gb, "Function", "bar", "test.a.bar", "same.go", 11, 20, props);
    ...
    const pmm_gbuf_edge_t **edges = NULL;
    int edge_count = 0;
    pmm_gbuf_find_edges_by_source_type(gb, id_b, "SIMILAR_TO", &edges, &edge_count);
...
TEST(pass_similarity_cross_language_skip) {
    ...
    pmm_gbuf_t *gb = pmm_gbuf_new("test", "/tmp");
    pmm_gbuf_upsert_node(gb, "Function", "foo", "test.a.foo", "a.go", 1, 10, props);
    pmm_gbuf_upsert_node(gb, "Function", "bar", "test.b.bar", "b.py", 1, 10, props);
...
TEST(pass_similarity_edge_properties) {
    ...
    pmm_gbuf_t *gb = pmm_gbuf_new("test", "/tmp");
    int64_t id_a = pmm_gbuf_upsert_node(gb, "Function", "foo", "test.a.foo", "a.go", 1, 10, props_a);
    pmm_gbuf_upsert_node(gb, "Function", "bar", "test.b.bar", "b.go", 1, 10, props_b);
    ...
    const pmm_gbuf_edge_t **edges = NULL;
    int edge_count = 0;
    pmm_gbuf_find_edges_by_source_type(gb, id_a, "SIMILAR_TO", &edges, &edge_count);
    ASSERT_EQ(edge_count, 1);
...
TEST(pass_similarity_max_edges_cap) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        pmm_gbuf_upsert_node(gb, "Function", name, qn, path, 1, 10, props);
    }
    ...
    /* Each function should have at most PMM_MINHASH_MAX_EDGES_PER_NODE edges */
    const pmm_gbuf_node_t **funcs = NULL;
    int func_count = 0;
    pmm_gbuf_find_by_label(gb, "Function", &funcs, &func_count);
    for (int i = 0; i < func_count; i++) {
        ...
        int edge_count = 0;
        pmm_gbuf_find_edges_by_source_type(gb, funcs[i]->id, "SIMILAR_TO", &edges, &edge_count);
        ASSERT_LTE(edge_count, PMM_MINHASH_MAX_EDGES_PER_NODE);
    }
...
TEST(pass_similarity_short_funcs_skipped) {
    ...
    pmm_gbuf_t *gb = pmm_gbuf_new("test", "/tmp");
    pmm_gbuf_upsert_node(gb, "Function", "tiny1", "test.t.tiny1", "t.go", 1, 3, "{}");
    pmm_gbuf_upsert_node(gb, "Function", "tiny2", "test.t.tiny2", "t.go", 4, 6, "{}");
...
static int setup_sim_test_repo(void) {
    snprintf(g_sim_tmpdir, sizeof(g_sim_tmpdir), "/tmp/pmm_sim_test_XXXXXX");
    if (!pmm_mkdtemp(g_sim_tmpdir)) {
        return -1;
    }
...
TEST(pipeline_minhash_end_to_end) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_sim_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ...
    int edge_count = 0;
    int find_rc = pmm_store_find_edges_by_type(s, project, "SIMILAR_TO", &edges, &edge_count);
    ASSERT_EQ(find_rc, PMM_STORE_OK);
...
TEST(pipeline_minhash_no_false_positives) {
    snprintf(g_sim_tmpdir, sizeof(g_sim_tmpdir), "/tmp/pmm_sim_nofp_XXXXXX");
    if (!pmm_mkdtemp(g_sim_tmpdir)) {
        FAIL("failed to create temp dir");
    }
    ...
    pmm_edge_t *edges = NULL;
    int edge_count = 0;
    pmm_store_find_edges_by_type(s, pmm_pipeline_project_name(p), "SIMILAR_TO", &edges, &edge_count);
...
TEST(pipeline_minhash_incremental) {
    ...
    pmm_pipeline_t *p1 = pmm_pipeline_new(g_sim_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p1);
    int rc = pmm_pipeline_run(p1);
    ASSERT_EQ(rc, 0);
    const char *project = pmm_pipeline_project_name(p1);

    /* Verify edge exists */
    pmm_store_t *s1 = pmm_store_open_path(db_path);
    pmm_edge_t *edges1 = NULL;
    int count1 = 0;
    pmm_store_find_edges_by_type(s1, project, "SIMILAR_TO", &edges1, &count1);
    ASSERT_GTE(count1, 1);
    if (edges1) {
        pmm_store_free_edges(edges1, count1);
    }
    pmm_store_close(s1);
    ...
    pmm_pipeline_t *p2 = pmm_pipeline_new(g_sim_tmpdir, db_path, PMM_MODE_FULL);
    ASSERT_NOT_NULL(p2);
    rc = pmm_pipeline_run(p2);
    ASSERT_EQ(rc, 0);

    /* Step 4: Verify old SIMILAR_TO edge is gone */
    pmm_store_t *s2 = pmm_store_open_path(db_path);
    pmm_edge_t *edges2 = NULL;
    int count2 = 0;
    pmm_store_find_edges_by_type(s2, pmm_pipeline_project_name(p2), "SIMILAR_TO", &edges2, &count2);
    ASSERT_EQ(count2, 0); /* Functions no longer similar */
    if (edges2) {
        pmm_store_free_edges(edges2, count2);
    }
    pmm_store_close(s2);
...
TEST(pipeline_minhash_incremental_new_clone) {
    ...
    pmm_edge_t *edges1 = NULL;
    int count1 = 0;
    pmm_store_find_edges_by_type(s1, pmm_pipeline_project_name(p1), "SIMILAR_TO", &edges1, &count1);
