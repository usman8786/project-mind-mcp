...
static pmm_agent_client_resolve_options_t agent_options(agent_probe_t *probe) {
...
static char *agent_fixture(const char *initial, char **dir_out) {
    char *dir = th_mktempdir("pmm_agent_clients");
    if (!dir) {
        return NULL;
    }
...
static char *agent_deep_same_name_json(size_t depth) {
    ...
    static const char leaf[] = "{\"project-mind-mcp\":{\"command\":\"foreign\",\"args\":[]}}";
...
TEST(agent_clients_registry_is_stable_and_callback_driven) {
    ...
    static const uint32_t expected_capabilities[] = {
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_AGENT | PMM_AGENT_CAP_HOOK,
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_HOOK,
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_HOOK,
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_AGENT,
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL,
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_HOOK,
        PMM_AGENT_CAP_MCP,
        PMM_AGENT_CAP_MCP,
        PMM_AGENT_CAP_MCP,
        ...
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_AGENT,
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL,
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS,
        PMM_AGENT_CAP_MCP | PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL | PMM_AGENT_CAP_AGENT,
        PMM_AGENT_CAP_INSTRUCTIONS | PMM_AGENT_CAP_SKILL,
        PMM_AGENT_CAP_MCP,
    };
    ASSERT_EQ(pmm_agent_client_count(), PMM_AGENT_CLIENT_COUNT);
    ASSERT_EQ(PMM_AGENT_CLIENT_COUNT, sizeof(expected) / sizeof(expected[0]));
    ASSERT_EQ(PMM_AGENT_CLIENT_COUNT,
              sizeof(expected_capabilities) / sizeof(expected_capabilities[0]));
    for (size_t i = 0U; i < PMM_AGENT_CLIENT_COUNT; i++) {
        const pmm_agent_client_profile_t *profile = pmm_agent_client_at(i);
        ASSERT_NOT_NULL(profile);
        ASSERT_EQ(profile->id, (pmm_agent_client_id_t)i);
        ...
        if (profile->id == PMM_AGENT_CLIENT_PI) {
            ASSERT(!(profile->capabilities & PMM_AGENT_CAP_MCP));
        ...
        ASSERT(pmm_agent_client_by_id(profile->id) == profile);
        ASSERT(pmm_agent_client_by_stable_id(profile->stable_id) == profile);
    }
    ASSERT_NULL(pmm_agent_client_by_id(PMM_AGENT_CLIENT_COUNT));
    ASSERT_NULL(pmm_agent_client_by_stable_id("glab"));
    ASSERT_EQ(pmm_agent_client_by_id(PMM_AGENT_CLIENT_TRAE)->stability, PMM_AGENT_CONDITIONAL);
    PASS();
}
...
TEST(agent_clients_visual_studio_cleanup_survives_missing_command) {
    ...
    pmm_agent_client_resolve_options_t options = agent_options(&probe);
    options.is_windows = true;
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_VISUAL_STUDIO, &options));
    ASSERT(pmm_agent_client_cleanup_candidate(PMM_AGENT_CLIENT_VISUAL_STUDIO, &options));

    const char *foreign = "{\"servers\":{\"project-mind-mcp\":{\"type\":\"stdio\","
                          "\"command\":\"C:/User/tool.exe\",\"args\":[]}}}\n";
    char *dir = NULL;
    char *path = agent_fixture(foreign, &dir);
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_remove_mcp(PMM_AGENT_CLIENT_VISUAL_STUDIO, path,
                                          "C:/Tools/project-mind-mcp.exe"),
              PMM_AGENT_EDIT_FOREIGN);
...
TEST(agent_clients_next_wave_metadata_matches_supported_surfaces) {
    const pmm_agent_client_profile_t *continue_profile =
        pmm_agent_client_by_id(PMM_AGENT_CLIENT_CONTINUE);
    const pmm_agent_client_profile_t *visual_studio =
        pmm_agent_client_by_id(PMM_AGENT_CLIENT_VISUAL_STUDIO);
    const pmm_agent_client_profile_t *rovo = pmm_agent_client_by_id(PMM_AGENT_CLIENT_ROVO_DEV);
    const pmm_agent_client_profile_t *gitlab = pmm_agent_client_by_id(PMM_AGENT_CLIENT_GITLAB_DUO);
    const pmm_agent_client_profile_t *devin = pmm_agent_client_by_id(PMM_AGENT_CLIENT_DEVIN);
    const pmm_agent_client_profile_t *roo = pmm_agent_client_by_id(PMM_AGENT_CLIENT_ROO_CODE);
    const pmm_agent_client_profile_t *amazon_q = pmm_agent_client_by_id(PMM_AGENT_CLIENT_AMAZON_Q);
    const pmm_agent_client_profile_t *codebuddy =
        pmm_agent_client_by_id(PMM_AGENT_CLIENT_CODEBUDDY);
    const pmm_agent_client_profile_t *ibm_bob_ide =
        pmm_agent_client_by_id(PMM_AGENT_CLIENT_IBM_BOB_IDE);
    const pmm_agent_client_profile_t *ibm_bob_shell =
        pmm_agent_client_by_id(PMM_AGENT_CLIENT_IBM_BOB_SHELL);
    const pmm_agent_client_profile_t *pochi = pmm_agent_client_by_id(PMM_AGENT_CLIENT_POCHI);
    const pmm_agent_client_profile_t *pi = pmm_agent_client_by_id(PMM_AGENT_CLIENT_PI);
    const pmm_agent_client_profile_t *cody =
        pmm_agent_client_by_id(PMM_AGENT_CLIENT_SOURCEGRAPH_CODY);
    ...
    ASSERT_EQ(continue_profile->stability, PMM_AGENT_CONDITIONAL);
    ASSERT_EQ(visual_studio->stability, PMM_AGENT_CONDITIONAL);
    ASSERT(rovo->capabilities & PMM_AGENT_CAP_SKILL);
    ASSERT(rovo->capabilities & PMM_AGENT_CAP_AGENT);
    ASSERT(gitlab->capabilities & PMM_AGENT_CAP_HOOK);
    ASSERT(!(devin->capabilities & PMM_AGENT_CAP_AGENT));
    ASSERT_EQ(roo->stability, PMM_AGENT_CONDITIONAL);
    ASSERT(roo->capabilities & PMM_AGENT_CAP_MCP);
    ASSERT(amazon_q->capabilities & PMM_AGENT_CAP_MCP);
    ASSERT_STR_EQ(amazon_q->display_name, "Amazon Q Developer IDE");
    ASSERT_EQ(codebuddy->stability, PMM_AGENT_STABLE);
    ASSERT(codebuddy->capabilities & PMM_AGENT_CAP_AGENT);
    ASSERT(codebuddy->capabilities & PMM_AGENT_CAP_SKILL);
    ASSERT_STR_EQ(codebuddy->display_name, "CodeBuddy Code CLI");
    ASSERT_STR_EQ(codebuddy->detection_command, "codebuddy");
    ASSERT_EQ(ibm_bob_ide->stability, PMM_AGENT_CONDITIONAL);
    ASSERT_STR_EQ(ibm_bob_ide->display_name, "IBM Bob IDE");
    ASSERT_NULL(ibm_bob_ide->detection_command);
    ASSERT_EQ(ibm_bob_shell->stability, PMM_AGENT_STABLE);
    ASSERT_STR_EQ(ibm_bob_shell->display_name, "IBM Bob Shell");
    ASSERT_STR_EQ(ibm_bob_shell->detection_command, "bob");
    ASSERT_EQ(pochi->stability, PMM_AGENT_STABLE);
    ASSERT(pochi->capabilities & PMM_AGENT_CAP_AGENT);
    ASSERT_STR_EQ(pochi->detection_command, "pochi");
    ASSERT(!(pi->capabilities & PMM_AGENT_CAP_MCP));
    ASSERT(pi->capabilities & PMM_AGENT_CAP_INSTRUCTIONS);
    ASSERT(pi->capabilities & PMM_AGENT_CAP_SKILL);
    ASSERT_NULL(pi->install_mcp);
    ASSERT_NULL(pi->remove_mcp);
    ASSERT_EQ(cody->stability, PMM_AGENT_OPT_IN);
    ASSERT_EQ(cody->capabilities, PMM_AGENT_CAP_MCP);
...
TEST(agent_clients_resolve_documented_paths_and_precedence) {
    ...
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_QODER, &options, path, sizeof(path)),
              0);
    ASSERT_STR_EQ(path, "/home/tester/.qoder/settings.json");
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_KIMI, &options, path, sizeof(path)),
              0);
    ASSERT_STR_EQ(path, "/home/tester/.kimi-code/mcp.json");
    ASSERT_EQ(
        pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_GITLAB_DUO, &options, path, sizeof(path)),
        0);
    ...
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_AMP, &options, path, sizeof(path)), 0);
    ASSERT_STR_EQ(path, "/home/tester/.config/agents/skills/project-mind/mcp.json");
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_DEVIN, &options, path, sizeof(path)),
              0);
...
TEST(agent_clients_resolve_rovo_override_and_conditional_paths) {
    char *dir = th_mktempdir("pmm_agent_clients_rovo");
    ASSERT_NOT_NULL(dir);
    char config[1024];
    ...
    char path[1024];
    ASSERT_EQ(
        pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_ROVO_DEV, &options, path, sizeof(path)), 0);
    ASSERT_STR_EQ(path, safe_override);

    ASSERT_EQ(
        pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_CONTINUE, &options, path, sizeof(path)), 1);
    options.continue_config_path = config;
    ASSERT_EQ(
        pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_CONTINUE, &options, path, sizeof(path)), 0);
    ASSERT_STR_EQ(path, config);
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_TRAE, &options, path, sizeof(path)),
              1);
...
TEST(agent_clients_rovo_override_rejects_relative_and_traversal_paths) {
    ...
    char *dir = th_mktempdir("pmm_agent_clients_rovo_relative");
    ASSERT_NOT_NULL(dir);
    char config[1024];
    ASSERT(snprintf(config, sizeof(config), "%s/.rovodev/config.yml", dir) > 0);
    agent_probe_t probe = {.paths = {config}, .path_count = 1U};
    ...
    for (size_t i = 0U; i < sizeof(invalid_paths) / sizeof(invalid_paths[0]); i++) {
        ...
        ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_ROVO_DEV, &options, resolved,
                                                sizeof(resolved)),
                  -1);
...
TEST(agent_clients_rovo_override_rejects_absolute_paths_outside_user_root) {
    char *dir = th_mktempdir("pmm_agent_clients_rovo_absolute");
    ASSERT_NOT_NULL(dir);
    char config[1024];
    char outside[1024];
...
TEST(agent_clients_detection_avoids_generic_binary_false_positives) {
    ...
    pmm_agent_client_resolve_options_t options = agent_options(&probe);
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_GITLAB_DUO, &options));
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_ROVO_DEV, &options));
    ...
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_CONTINUE, &options));
...
TEST(agent_clients_detect_installed_client_directories_before_mcp_exists) {
    ...
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_QODER, &options));
    ...
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_KIMI, &options));
    ...
    probe.path_count = 0U;
    options.glab_config_dir = NULL;
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_AMP, &options));
    ...
    probe.path_count = 0U;
    options.xdg_config_home = "/xdg";
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_DEVIN, &options));
    ...
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_TABNINE, &options));
    probe.paths[0] = "/home/tester/.tabnine";
    probe.path_count = 1U;
    ASSERT(pmm_agent_client_detect(PMM_AGENT_CLIENT_TABNINE, &options));

    probe.path_count = 0U;
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_AMAZON_Q, &options));
    probe.paths[0] = "/home/tester/.aws/amazonq";
    probe.path_count = 1U;
    ASSERT(pmm_agent_client_detect(PMM_AGENT_CLIENT_AMAZON_Q, &options));

    probe.path_count = 0U;
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_PI, &options));
...
TEST(agent_clients_marker_detection_remains_fail_closed_for_conditional_clients) {
    ...
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_AMP, &options));
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_CONTINUE, &options));
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_ROO_CODE, &options));
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_TRAE, &options));
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_SOURCEGRAPH_CODY, &options));
...
TEST(agent_clients_roo_code_requires_an_explicit_existing_config) {
    ...
    pmm_agent_client_resolve_options_t options = agent_options(&probe);
    char resolved[512];
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_ROO_CODE, &options, resolved,
                                            sizeof(resolved)),
              1);
    ...
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_ROO_CODE, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_OK);
...
TEST(agent_clients_amazon_q_prefers_current_then_existing_legacy_config) {
    ...
    pmm_agent_client_resolve_options_t options = agent_options(&probe);
    char resolved[512];
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_AMAZON_Q, &options, resolved,
                                            sizeof(resolved)),
              0);
    ...
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_AMAZON_Q, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_OK);
    char *installed = agent_read(path);
    ASSERT_NOT_NULL(installed);
    ASSERT_NOT_NULL(strstr(installed, "\"mcpServers\""));
...
TEST(agent_clients_codebuddy_bob_and_pochi_use_documented_global_paths) {
    ...
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_CODEBUDDY, &options, resolved,
                                            sizeof(resolved)),
              0);
    ...
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_CODEBUDDY, &options, resolved,
              ...
              0);
    ASSERT_STR_EQ(resolved, "/home/tester/.codebuddy/.mcp.json");
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_IBM_BOB_IDE, &options, resolved,
                                            sizeof(resolved)),
              0);
    ASSERT_STR_EQ(resolved, "/home/tester/.bob/mcp.json");
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_IBM_BOB_SHELL, &options, resolved,
                                            sizeof(resolved)),
              0);
    ASSERT_STR_EQ(resolved, "/home/tester/.bob/mcp_settings.json");
    ASSERT_EQ(
        pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_POCHI, &options, resolved, sizeof(resolved)),
        0);
    ASSERT_STR_EQ(resolved, "/home/tester/.pochi/config.jsonc");

    probe.path_count = 0U;
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_CODEBUDDY, &options));
    ...
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_IBM_BOB_IDE, &options));
    probe.paths[probe.path_count++] = "/home/tester/.bob";
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_IBM_BOB_IDE, &options));
    probe.paths[probe.path_count++] = "/home/tester/.bob/mcp.json";
    ASSERT(pmm_agent_client_detect(PMM_AGENT_CLIENT_IBM_BOB_IDE, &options));
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_IBM_BOB_SHELL, &options));
    probe.commands[probe.command_count++] = "bob";
    ASSERT(pmm_agent_client_detect(PMM_AGENT_CLIENT_IBM_BOB_SHELL, &options));
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_POCHI, &options));
...
TEST(agent_clients_pi_has_no_mcp_path_or_mutation) {
    ...
    char resolved[512];
    ASSERT_EQ(
        pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_PI, &options, resolved, sizeof(resolved)),
        1);
    ASSERT(pmm_agent_client_detect(PMM_AGENT_CLIENT_PI, &options));
    ...
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_PI, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_NOT_APPLICABLE);
    char *after = agent_read(path);
...
TEST(agent_clients_cody_is_opt_in_and_requires_explicit_existing_settings) {
    ...
    pmm_agent_client_resolve_options_t options = agent_options(&probe);
    char resolved[512];
    ASSERT_EQ(pmm_agent_client_resolve_path(PMM_AGENT_CLIENT_SOURCEGRAPH_CODY, &options, resolved,
                                            sizeof(resolved)),
              1);
    ASSERT(!pmm_agent_client_detect(PMM_AGENT_CLIENT_SOURCEGRAPH_CODY, &options));
...
TEST(agent_clients_cody_uses_literal_dotted_key_without_feature_or_permission_edits) {
    ...
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_SOURCEGRAPH_CODY, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_OK);
    char *installed = agent_read(path);
    ASSERT_NOT_NULL(installed);
    ...
    const char *foreign = "{\"cody.mcpServers\":{\"project-mind-mcp\":{\"command\":\"foreign\","
...
TEST(agent_clients_json_schemas_are_exact_and_policy_neutral) {
    static const struct {
        ...
        {PMM_AGENT_CLIENT_QODER, "\"mcpServers\""},
        {PMM_AGENT_CLIENT_KIMI, "\"mcpServers\""},
        {PMM_AGENT_CLIENT_GITLAB_DUO, "\"type\": \"stdio\""},
        {PMM_AGENT_CLIENT_ROVO_DEV, "\"transport\": \"stdio\""},
        {PMM_AGENT_CLIENT_AMP, "\"project-mind-mcp\""},
        {PMM_AGENT_CLIENT_DEVIN, "\"mcpServers\""},
        {PMM_AGENT_CLIENT_TABNINE, "\"mcpServers\""},
        {PMM_AGENT_CLIENT_VISUAL_STUDIO, "\"servers\""},
        {PMM_AGENT_CLIENT_TRAE, "\"mcpServers\""},
        {PMM_AGENT_CLIENT_CODEBUDDY, "\"mcpServers\""},
        {PMM_AGENT_CLIENT_IBM_BOB_IDE, "\"mcpServers\""},
        {PMM_AGENT_CLIENT_IBM_BOB_SHELL, "\"mcpServers\""},
        {PMM_AGENT_CLIENT_POCHI, "\"mcp\""},
    };
    const char *binary = "/opt/Project Mind/bin/pmm\\\"special";
    for (size_t i = 0U; i < sizeof(cases) / sizeof(cases[0]); i++) {
        char *dir = NULL;
        char *path = agent_fixture("{\n  // user-owned\n  \"keep\": true,\n}\n", &dir);
        ASSERT_NOT_NULL(path);
        ASSERT_EQ(pmm_agent_client_install_mcp(cases[i].id, path, binary), PMM_AGENT_EDIT_OK);
...
TEST(agent_clients_new_standard_json_profiles_preserve_foreign_entries) {
    static const pmm_agent_client_id_t clients[] = {
    ...
    for (size_t i = 0U; i < sizeof(clients) / sizeof(clients[0]); i++) {
        const char *foreign =
            clients[i] == PMM_AGENT_CLIENT_POCHI
                ? "{\"mcp\":{\"project-mind-mcp\":{\"command\":\"foreign\","
                  "\"args\":[]}}}\n"
                : "{\"mcpServers\":{\"project-mind-mcp\":{\"command\":\"foreign\","
                  "\"args\":[]}}}\n";
        char *dir = NULL;
        char *path = agent_fixture(foreign, &dir);
        ASSERT_NOT_NULL(path);
        ASSERT_EQ(pmm_agent_client_install_mcp(clients[i], path, "/usr/bin/pmm"),
                  PMM_AGENT_EDIT_FOREIGN);
        char *after = agent_read(path);
        ASSERT_NOT_NULL(after);
        ASSERT_STR_EQ(after, foreign);
...
TEST(agent_clients_refuse_foreign_and_preserve_modified_entries) {
    const char *foreign =
        "{\"mcpServers\":{\"project-mind-mcp\":{\"command\":\"foreign\",\"args\":[]}}}\n";
    char *dir = NULL;
    char *path = agent_fixture(foreign, &dir);
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_QODER, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_FOREIGN);
    char *after = agent_read(path);
    ASSERT_STR_EQ(after, foreign);
    free(after);
    ...
    char *command = strstr(canonical, "/usr/bin/pmm");
    ASSERT_NOT_NULL(command);
    command[0] = 'X';
    ASSERT_EQ(th_write_file(path, canonical), 0);
    ASSERT_EQ(pmm_agent_client_remove_mcp(PMM_AGENT_CLIENT_QODER, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_FOREIGN);
    after = agent_read(path);
...
TEST(agent_clients_remove_only_canonical_and_missing_is_noop) {
    ...
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_remove_mcp(PMM_AGENT_CLIENT_GITLAB_DUO, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_OK);
    ASSERT_EQ(pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_GITLAB_DUO, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_OK);
    ASSERT_EQ(pmm_agent_client_remove_mcp(PMM_AGENT_CLIENT_GITLAB_DUO, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_OK);
    char *after = agent_read(path);
    ASSERT_NOT_NULL(after);
    ASSERT_NULL(strstr(after, "project-mind-mcp"));
...
TEST(agent_clients_registry_callbacks_apply_the_selected_schema) {
    ...
    char *path = agent_fixture("{}\n", &dir);
    ASSERT_NOT_NULL(path);
    const pmm_agent_client_profile_t *profile =
        pmm_agent_client_by_id(PMM_AGENT_CLIENT_VISUAL_STUDIO);
    ASSERT_NOT_NULL(profile);
    ASSERT_EQ(profile->install_mcp(profile->id, path, "C:/Tools/pmm.exe"), PMM_AGENT_EDIT_OK);
    char *installed = agent_read(path);
    ASSERT_NOT_NULL(installed);
    ASSERT_NOT_NULL(strstr(installed, "\"servers\""));
    ASSERT_NOT_NULL(strstr(installed, "\"type\": \"stdio\""));
    free(installed);
    ASSERT_EQ(profile->remove_mcp(profile->id, path, "C:/Tools/pmm.exe"), PMM_AGENT_EDIT_OK);
    free(path);
    th_cleanup(dir);
    PASS();
}
...
TEST(agent_clients_malformed_json_fails_byte_identically) {
    ...
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_TABNINE, path, "/usr/bin/pmm"),
...
TEST(agent_clients_continue_uses_owned_yaml_sequence_item) {
    ...
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(
        pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_CONTINUE, path, "/opt/pmm path/#quoted"),
        PMM_AGENT_EDIT_OK);
    char *first = agent_read(path);
    ASSERT_NOT_NULL(first);
    ASSERT_NOT_NULL(strstr(first, "  - name: project-mind-mcp\n"));
    ASSERT_NOT_NULL(strstr(first, "    command: \"/opt/pmm path/#quoted\"\n"));
    ASSERT_NOT_NULL(strstr(first, "  - name: other\n"));
    ASSERT_EQ(agent_occurrences(first, "name: project-mind-mcp"), 1U);
    ASSERT_EQ(
        pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_CONTINUE, path, "/opt/pmm path/#quoted"),
        PMM_AGENT_EDIT_OK);
    char *second = agent_read(path);
    ASSERT_STR_EQ(first, second);
    ASSERT_EQ(pmm_agent_client_remove_mcp(PMM_AGENT_CLIENT_CONTINUE, path, "/opt/pmm path/#quoted"),
              PMM_AGENT_EDIT_OK);
    char *removed = agent_read(path);
    ASSERT_NULL(strstr(removed, "name: project-mind-mcp"));
...
TEST(agent_clients_continue_refuses_foreign_same_name_and_nonsequence_section) {
    ...
    char *path = agent_fixture(foreign, &dir);
    ASSERT_NOT_NULL(path);
    ASSERT_EQ(pmm_agent_client_install_mcp(PMM_AGENT_CLIENT_CONTINUE, path, "/usr/bin/pmm"),
              PMM_AGENT_EDIT_FOREIGN);
    char *after = agent_read(path);
    ASSERT_STR_EQ(after, foreign);
    free(after);
