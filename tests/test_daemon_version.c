...
static pmm_daemon_build_identity_t version_test_identity(const char *version, const char *build) {
...
static bool version_test_temp_dir(char out[VERSION_TEST_PATH_CAP], const char *tag) {
    int written =
        snprintf(out, VERSION_TEST_PATH_CAP, "%s/pmm-daemon-%s-XXXXXX", pmm_tmpdir(), tag);
    return written > 0 && written < VERSION_TEST_PATH_CAP && pmm_mkdtemp(out) != NULL;
}
...
static void version_test_cleanup(const char *dir, const char *path, const char *rotated,
    ...
    if (rotated) {
        (void)pmm_unlink(rotated);
    }
    if (victim) {
        (void)pmm_unlink(victim);
    }
    if (dir) {
        (void)pmm_rmdir(dir);
    }
...
typedef struct {
    const char *path;
    const pmm_daemon_conflict_t *conflict;
    size_t cap_bytes;
    bool result;
...
static void version_test_rotation_race_hook(void *opaque,
    ...
    if (stage == PMM_DAEMON_CONFLICT_LOG_BEFORE_SERIALIZATION_LOCK) {
        (void)atomic_fetch_add_explicit(&race->lock_attempts, 1, memory_order_acq_rel);
        return;
    }
    if (stage != PMM_DAEMON_CONFLICT_LOG_AFTER_SERIALIZATION_LOCK ||
...
static void *version_test_append_thread(void *opaque) {
    ...
    call->result = pmm_daemon_conflict_log_append(call->path, call->conflict, call->cap_bytes);
    return NULL;
}
#endif

static pmm_daemon_hello_status_t version_test_compare(const pmm_daemon_build_identity_t *active,
                                                      const pmm_daemon_build_identity_t *requested,
                                                      pmm_daemon_conflict_t *conflict) {
    memset(conflict, 0, sizeof(*conflict));
    return pmm_daemon_hello_compare(active, requested, conflict);
}

static bool version_test_conflict_identity_matches(const pmm_daemon_conflict_t *conflict,
...
TEST(daemon_rendezvous_key_is_stable_and_version_independent) {
    char first[PMM_DAEMON_KEY_SIZE];
    char after_upgrade[PMM_DAEMON_KEY_SIZE];
    ...
    ASSERT_TRUE(pmm_daemon_rendezvous_key(first));
    ASSERT_TRUE(pmm_daemon_rendezvous_key(after_upgrade));
...
TEST(daemon_build_fingerprint_hashes_exact_executable_bytes) {
    ...
    char first[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];
    char repeated[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];
    char second[PMM_DAEMON_BUILD_FINGERPRINT_SIZE];
    ...
    ASSERT_TRUE(pmm_daemon_build_fingerprint_file(first_path, first));
    ASSERT_TRUE(pmm_daemon_build_fingerprint_file(first_path, repeated));
    ASSERT_TRUE(pmm_daemon_build_fingerprint_file(second_path, second));
...
TEST(daemon_hello_accepts_only_the_exact_active_build_identity) {
    pmm_daemon_build_identity_t active = version_test_identity("2.4.0", BUILD_A);
    pmm_daemon_build_identity_t exact = version_test_identity("2.4.0", BUILD_A);
    pmm_daemon_conflict_t conflict;

    ASSERT_EQ(version_test_compare(&active, &exact, &conflict), PMM_DAEMON_HELLO_COMPATIBLE);

    pmm_daemon_build_identity_t rebuilt = version_test_identity("2.4.0", BUILD_B);
    ASSERT_EQ(version_test_compare(&active, &rebuilt, &conflict), PMM_DAEMON_HELLO_BUILD_CONFLICT);
    ASSERT_TRUE(
...
TEST(daemon_hello_version_conflict_exposes_active_and_requested_builds) {
    pmm_daemon_build_identity_t active = version_test_identity("2.3.1", BUILD_A);
    pmm_daemon_build_identity_t requested = version_test_identity("2.4.0", BUILD_B);
    pmm_daemon_conflict_t conflict;
    char visible[PMM_DAEMON_CONFLICT_MESSAGE_SIZE];
    ...
    ASSERT_TRUE(pmm_daemon_conflict_format(&conflict, visible, sizeof(visible)));
...
TEST(daemon_hello_fails_closed_without_an_exact_build_fingerprint) {
    pmm_daemon_build_identity_t active = version_test_identity("2.4.0", BUILD_A);
    pmm_daemon_build_identity_t missing = version_test_identity("2.4.0", NULL);
    pmm_daemon_build_identity_t malformed = version_test_identity("2.4.0", "not-a-sha256");
    pmm_daemon_conflict_t conflict;

    ASSERT_EQ(version_test_compare(&active, &missing, &conflict), PMM_DAEMON_HELLO_INVALID);
    ASSERT_EQ(version_test_compare(&active, &malformed, &conflict), PMM_DAEMON_HELLO_INVALID);
    PASS();
}
...
TEST(daemon_conflict_log_is_durable_private_and_rotates) {
    ...
    pmm_daemon_build_identity_t active = version_test_identity("2.3.1", BUILD_A);
    pmm_daemon_build_identity_t first_requested = version_test_identity("2.4.0", BUILD_B);
    pmm_daemon_build_identity_t second_requested = version_test_identity("2.5.0", BUILD_C);
    pmm_daemon_conflict_t first_conflict;
    pmm_daemon_conflict_t second_conflict;
    ...
    ASSERT_EQ(version_test_compare(&active, &first_requested, &first_conflict),
              PMM_DAEMON_HELLO_VERSION_CONFLICT);
    ASSERT_TRUE(pmm_daemon_conflict_log_append(path, &first_conflict, 4096));
    ...
    ASSERT_EQ(version_test_compare(&active, &second_requested, &second_conflict),
              PMM_DAEMON_HELLO_VERSION_CONFLICT);
    ASSERT_TRUE(pmm_daemon_conflict_log_append(path, &second_conflict, (size_t)first_size + 1));
...
TEST(daemon_conflict_log_rotation_serializes_on_stable_sidecar) {
    ...
    pmm_daemon_build_identity_t active = version_test_identity("2.3.1", BUILD_A);
    pmm_daemon_build_identity_t seed_requested = version_test_identity("2.4.0", BUILD_B);
    pmm_daemon_build_identity_t first_requested = version_test_identity("2.5.0", BUILD_B);
    pmm_daemon_build_identity_t second_requested = version_test_identity("2.6.0", BUILD_C);
    pmm_daemon_conflict_t seed_conflict;
    pmm_daemon_conflict_t first_conflict;
    pmm_daemon_conflict_t second_conflict;
    version_test_rotation_race_t race = {0};
    version_test_append_call_t first_call = {0};
    version_test_append_call_t second_call = {0};
    pmm_thread_t first_thread;
    pmm_thread_t second_thread;
    ...
    bool setup_ok = version_test_temp_dir(dir, "log-rotation-race") &&
                    ...
                    version_test_compare(&active, &second_requested, &second_conflict) ==
                        PMM_DAEMON_HELLO_VERSION_CONFLICT &&
                    pmm_daemon_conflict_log_append(path, &seed_conflict, 8192);
    ...
    pmm_daemon_conflict_log_set_test_hook(version_test_rotation_race_hook, &race);
    first_started =
        pmm_thread_create(&first_thread, 0, version_test_append_thread, &first_call) == 0;
    bool first_locked = first_started && version_test_wait_atomic_bool(&race.first_lock_acquired);
    if (first_locked) {
        second_started =
            pmm_thread_create(&second_thread, 0, version_test_append_thread, &second_call) == 0;
    }
    if (first_started) {
        (void)pmm_thread_join(&first_thread);
    }
    if (second_started) {
        (void)pmm_thread_join(&second_thread);
    }
    pmm_daemon_conflict_log_set_test_hook(NULL, NULL);
...
TEST(daemon_conflict_log_rejects_symlink_destination) {
    ...
    ASSERT_EQ(version_test_compare(&active, &requested, &conflict),
              PMM_DAEMON_HELLO_VERSION_CONFLICT);
    ASSERT_FALSE(pmm_daemon_conflict_log_append(path, &conflict, 4096));
...
static void *version_test_windows_append_thread(void *opaque) {
    ...
    call->result = pmm_daemon_conflict_log_append(call->path, call->conflict, 64U * 1024U);
    return NULL;
}
...
TEST(daemon_conflict_log_windows_concurrent_appends_are_not_dropped) {
    ...
    pmm_daemon_build_identity_t requested = version_test_identity("2.4.0", BUILD_B);
    pmm_daemon_conflict_t conflict;
    pmm_thread_t threads[APPEND_COUNT];
    ...
    bool setup_ok =
        ...
        version_test_child_path(path, dir, "daemon.log") &&
        version_test_child_path(rotated, dir, "daemon.log.1") &&
        version_test_compare(&active, &requested, &conflict) == PMM_DAEMON_HELLO_VERSION_CONFLICT;
    if (!setup_ok) {
        version_test_cleanup(dir, path, rotated, NULL);
    ...
    for (; started < APPEND_COUNT; started++) {
        ...
        if (pmm_thread_create(&threads[started], 0, version_test_windows_append_thread,
                              &calls[started]) != 0) {
            break;
        }
    }
