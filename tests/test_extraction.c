 ...
 *
 * Port of internal/pmm/regression_test.go (1282 LOC, ~80 test cases).
 * Exercises pmm_extract_file() on code snippets across 30+ languages,
 * verifying definitions, calls, and imports are correctly extracted.
 */
#include "test_framework.h"
#include "pmm.h"
#include "../src/foundation/compat.h" /* pmm_clock_gettime (wide-flat scaling guard) */
...
static CBMFileResult *extract(const char *src, CBMLanguage lang, const char *proj,
                              const char *path) {
    CBMFileResult *r = pmm_extract_file(src, (int)strlen(src), lang, proj, path, 0, NULL, NULL);
    return r;
}
...
static CBMFileResult *extract_with_macros(const char *src, CBMLanguage lang, const char *proj,
    ...
    CBMFileResult *r =
        pmm_extract_file_ex(src, (int)strlen(src), lang, proj, path, 0, NULL, NULL, mt, NULL);
    return r;
}
...
TEST(extract_ts_factory_object_methods_issue341) {
    CBMFileResult *r = extract("export function createItemActions(set, get) {\n"
                               ...
                               "  };\n"
                               "}\n",
                               PMM_LANG_TYPESCRIPT, "t", "item-actions.ts");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
 ...
 * node — it parses `char` in expression position — which pmm_collect_error_regions
...
TEST(extract_gdscript_issue186) {
    CBMFileResult *r = extract("extends Node\n"
                               ...
                               "    func helper():\n"
                               "        pass\n",
                               PMM_LANG_GDSCRIPT, "game", "player.gd");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(extract_powershell_issue35) {
    CBMFileResult *r = extract("function Get-Greeting {\n"
                               ...
                               "}\n",
                               PMM_LANG_POWERSHELL, "ops", "greet.ps1");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(extract_cfscript_issue38) {
    CBMFileResult *r = extract("component {\n"
                               ...
                               "    }\n"
                               "}\n",
                               PMM_LANG_CFSCRIPT, "app", "User.cfc");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT(count_defs_with_label(r, "Function") >= 2);
...
TEST(extract_helm_templates_issue338) {
    CBMFileResult *r = extract("{{- define \"chart.fullname\" -}}\n"
                               ...
                               "{{- end -}}\n",
                               PMM_LANG_GOTEMPLATE, "chart", "templates/_helpers.tpl");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(extract_helm_values_toplevel_issue338) {
    CBMFileResult *r = extract("image:\n"
                               ...
                               "service:\n"
                               "  port: 80\n",
                               PMM_LANG_YAML, "chart", "values.yaml");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(java_class_extends_and_implements) {
    CBMFileResult *r = extract("public class DefaultLinkTool extends DefaultDiagramTool implements "
                               "ILinkTool, Closeable { }",
                               PMM_LANG_JAVA, "t", "DefaultLinkTool.java");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
 ...
 * (internal/pmm/extract_defs.c) matches only type_identifier / generic_type /
...
TEST(swift_class) {
    CBMFileResult *r = extract("class Vehicle {\n    var speed: Int = 0\n    func accelerate() { "
                               "speed += 10 }\n    func stop() { speed = 0 }\n}\n",
                               PMM_LANG_SWIFT, "t", "Vehicle.swift");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(groovy_class) {
    CBMFileResult *r =
        extract("class Greeter {\n    String name\n    String greet() { \"Hello, $name\" }\n    "
                ...
                PMM_LANG_GROOVY, "t", "Greeter.groovy");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(js_class) {
    CBMFileResult *r =
        extract("class Counter {\n  constructor() { this.count = 0; }\n  increment() { "
                ...
                PMM_LANG_JAVASCRIPT, "t", "counter.js");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(ts_class) {
    CBMFileResult *r =
        extract("class Service {\n  private name: string;\n  constructor(name: string) { this.name "
                "= name; }\n  getName(): string { return this.name; }\n}\n",
                PMM_LANG_TYPESCRIPT, "t", "service.ts");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT(has_def(r, "Class", "Service"));
    pmm_free_result(r);
...
TEST(body_tokens_type_identifier) {
    CBMFileResult *r = extract("function serialize(obj: MyModel): SerializedResult {\n"
                               ...
                               "  return result;\n"
                               "}\n",
                               PMM_LANG_TYPESCRIPT, "t", "serial.ts");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    for (int i = 0; i < r->defs.count; i++) {
...
TEST(dockerfile_stages) {
    CBMFileResult *r = extract(
        ...
        PMM_LANG_DOCKERFILE, "t", "Dockerfile");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    pmm_free_result(r);
    PASS();
...
TEST(cmake_function) {
    CBMFileResult *r = extract(
        ...
        PMM_LANG_CMAKE, "t", "CMakeLists.txt");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_GTE(r->defs.count, 1);
    pmm_free_result(r);
...
TEST(protobuf_message) {
    CBMFileResult *r = extract(
        ...
        PMM_LANG_PROTOBUF, "t", "user.proto");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_GTE(r->defs.count, 1);
    pmm_free_result(r);
...
TEST(graphql_type) {
    CBMFileResult *r = extract("type User {\n  id: ID!\n  name: String!\n  email: String!\n}\ntype "
                               "Query {\n  user(id: ID!): User\n  users: [User!]!\n}\n",
                               PMM_LANG_GRAPHQL, "t", "schema.graphql");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_GTE(r->defs.count, 1);
    pmm_free_result(r);
...
TEST(vimscript_function) {
    CBMFileResult *r = extract("function! SayHello()\n  echo 'Hello'\nendfunction\n",
                               PMM_LANG_VIMSCRIPT, "t", "plugin.vim");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(matlab_parse) {
    CBMFileResult *r = extract("x = 1;\ny = x + 2;\n", PMM_LANG_MATLAB, "t", "simple.matlab");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    pmm_free_result(r);
    PASS();
...
/* ═══════════════════════════════════════════════════════════════════
 * Group I: pmm_test.go ports
 * ═══════════════════════════════════════════════════════════════════ */

TEST(python_docstring) {
    CBMFileResult *r = extract(
        "def compute(x, y):\n    \"\"\"Compute the sum of x and y.\"\"\"\n    return x + y\n",
        PMM_LANG_PYTHON, "test", "test.py");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT(has_def(r, "Function", "compute"));
...
TEST(js_arrow_function) {
    CBMFileResult *r = extract("const greet = (name) => {\n  return \"Hello \" + "
                               ...
                               PMM_LANG_JAVASCRIPT, "test", "app.js");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_GTE(r->defs.count, 1);
    pmm_free_result(r);
...
TEST(commonlisp_defun) {
    CBMFileResult *r =
        extract("(defun hello () \"world\")\n", PMM_LANG_COMMONLISP, "test", "hello.lisp");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(commonlisp_multiple_functions) {
    CBMFileResult *r = extract("(defun add (a b) (+ a b))\n(defun mul (a b) (* a b))\n",
                               PMM_LANG_COMMONLISP, "test", "math.lisp");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    pmm_free_result(r);
    PASS();
...
TEST(commonlisp_defmacro) {
    CBMFileResult *r =
        extract("(defmacro when2 (condition &body body)\n  `(if ,condition (progn ,@body)))\n",
                PMM_LANG_COMMONLISP, "test", "macros.lisp");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    pmm_free_result(r);
    PASS();
...
TEST(makefile_rule_as_function) {
    CBMFileResult *r = extract("all:\n\t@echo hello\n", PMM_LANG_MAKEFILE, "test", "Makefile");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(vimscript_function_extraction) {
    CBMFileResult *r = extract("function! SayHello()\n  echo 'Hello'\nendfunction\n",
                               PMM_LANG_VIMSCRIPT, "test", "plugin.vim");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(import_stress_go) {
    ...
    CBMFileResult *r = extract(src, PMM_LANG_GO, "t", "stress.go");
    free(src);
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(svelte_imports_no_script) {
    ...
    CBMFileResult *r = extract("<h1>Static page</h1>\n"
                               "<p>No script here.</p>\n",
                               PMM_LANG_SVELTE, "t", "Static.svelte");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(json_basic_pair) {
    CBMFileResult *r =
        extract("{\"host\": \"localhost\", \"port\": 5432}", PMM_LANG_JSON, "t", "config.json");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_GTE(count_defs_with_label(r, "Variable"), 2);
    ASSERT(has_def(r, "Variable", "host"));
...
TEST(json_empty_object) {
    CBMFileResult *r = extract("{}", PMM_LANG_JSON, "t", "config.json");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_EQ(count_defs_with_label(r, "Variable"), 0);
    pmm_free_result(r);
...
TEST(xml_self_closing_tag) {
    CBMFileResult *r =
        extract("<?xml version=\"1.0\"?><config><feature enabled=\"true\"/></config>", PMM_LANG_XML,
                "t", "config.xml");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_GTE(count_defs_with_label(r, "Class"), 2);
...
TEST(xml_empty_document) {
    CBMFileResult *r = extract("<?xml version=\"1.0\"?><root/>", PMM_LANG_XML, "t", "config.xml");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_GTE(count_defs_with_label(r, "Class"), 1);
    pmm_free_result(r);
...
TEST(python_init_module_qn_not_collide_with_folder) {
    ...
    CBMFileResult *r = extract("class Config:\n    DEBUG = True\n\ndef setup():\n    pass\n",
                               PMM_LANG_PYTHON, "proj", "mypackage/__init__.py");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(python_regular_module_qn_unchanged) {
    ...
    CBMFileResult *r =
        extract("def helper():\n    pass\n", PMM_LANG_PYTHON, "proj", "mypackage/utils.py");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT_NOT_NULL(r->module_qn);
...
TEST(extract_java_jaxrs_path_composition_issue1005) {
    CBMFileResult *r = extract("import jakarta.ws.rs.GET;\n"
                               ...
                               "}\n",
                               PMM_LANG_JAVA, "t", "WidgetResource.java");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(extract_go_no_filename_in_module_qn) {
    CBMFileResult *r = extract("package db\n\n"
                               "type Conn struct{}\n\n"
                               "func (c *Conn) Query() {}\n",
                               PMM_LANG_GO, "proj", "myapp/db/conn.go");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(extract_large_ts_has_functions_issue213) {
    ...
    CBMFileResult *r =
        pmm_extract_file(src, (int)off, PMM_LANG_TYPESCRIPT, "t", "big.ts", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
 ...
 *
 * pmm_compute_complexity stamps each Function/Method with cyclomatic,
...
TEST(complexity_super_only_not_recursive) {
    CBMFileResult *r = extract("class B(A):\n"
                               "    def save(self):\n"
                               "        super().save()\n",
                               PMM_LANG_PYTHON, "t", "super_only.py");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(complexity_same_name_other_receiver_not_recursive) {
    CBMFileResult *r = extract("function get(url) {\n"
                               ...
                               "}\n",
                               PMM_LANG_JAVASCRIPT, "t", "axios_get.js");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(complexity_self_receiver_still_recursive) {
    ...
    CBMFileResult *r = extract("class C:\n"
                               ...
                               "        if n > 0:\n"
                               "            self.recur(n - 1)\n",
                               PMM_LANG_PYTHON, "t", "self_recur.py");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ...
    r = extract("class C {\n"
                ...
                "    }\n"
                "}\n",
                PMM_LANG_JAVASCRIPT, "t", "this_step.js");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    d = find_def(r, "step");
    ASSERT_NOT_NULL(d);
...
TEST(complexity_chained_receiver_not_self) {
    CBMFileResult *r = extract("class C:\n"
                               "    def recur(self, n):\n"
                               "        self.obj.recur(n)\n",
                               PMM_LANG_PYTHON, "t", "chained_recur.py");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    const CBMDefinition *d = find_def(r, "recur");
    ASSERT_NOT_NULL(d);
 ...
 * (pmm_path_within_root, handle_process_kill). The preprocessed second
...
TEST(extract_cpp_preproc_signature_gap_issue946) {
    ...
    for (int enabled = 0; enabled < 2; enabled++) {
        CBMFileResult *r = pmm_extract_file(
            CPP_PREPROC_SIGNATURE_GAP_SRC, (int)strlen(CPP_PREPROC_SIGNATURE_GAP_SRC), PMM_LANG_CPP,
...
TEST(extract_cpp_preproc_macro_generated_callable_skipped_issue949) {
    ...
    CBMFileResult *r =
        pmm_extract_file(src, (int)strlen(src), PMM_LANG_CPP, "t", "macro.cpp", 0, defines, NULL);
    ASSERT_NOT_NULL(r);
...
TEST(extract_c_ifdef_split_brace_after_include_remapped_issue949) {
    char tmpdir[512];
    snprintf(tmpdir, sizeof(tmpdir), "%s/pmm_line_map_XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(tmpdir));

    char header_path[512];
    snprintf(header_path, sizeof(header_path), "%s/padding.h", tmpdir);
    FILE *header = pmm_fopen(header_path, "wb");
    ...
    CBMFileResult *r =
        pmm_extract_file(src, (int)strlen(src), PMM_LANG_C, "t", "shifted.c", 0, NULL, includes);
    pmm_unlink(header_path);
    pmm_rmdir(tmpdir);
...
TEST(walk_defs_no_truncation_over_4096_issue668) {
    ...
    CBMFileResult *r = extract(src, PMM_LANG_PYTHON, "t", "big.py");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
 ...
 * otherwise file-path-based (pmm_is_test_file), so test fns in a regular .rs
 * file leak. (#855) */
...
TEST(docstring_utf8_truncation_boundary_issue1017) {
    ...
    CBMFileResult *r = extract(src, PMM_LANG_GO, "test", "main.go");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT(has_def(r, "Function", "Compute"));
...
static long extract_wide_flat_ms(int n, int *out_defs) {
    ...
    struct timespec a;
    struct timespec b;
    pmm_clock_gettime(CLOCK_MONOTONIC, &a);
    CBMFileResult *r =
        pmm_extract_file(src, (int)off, PMM_LANG_JAVASCRIPT, "proj", "wide.js", 0, NULL, NULL);
    pmm_clock_gettime(CLOCK_MONOTONIC, &b);
    free(src);
    if (!r) {
        return -1;
    }
...
TEST(objectscript_udl_method_return_type) {
    CBMFileResult *r = extract("Class MyApp.Factory Extends %RegisteredObject\n"
                               ...
                               "}\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Factory.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    bool found_rt = false;
    for (int i = 0; i < r->defs.count; i++) {
...
TEST(objectscript_udl_scalar_return_type_not_resolved) {
    CBMFileResult *r = extract("Class MyApp.Counter Extends %RegisteredObject\n"
                               ...
                               "}\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Counter.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    for (int i = 0; i < r->defs.count; i++) {
...
TEST(objectscript_udl_class) {
    CBMFileResult *r = extract("Class MyApp.Patient Extends %Persistent\n{\n}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Patient.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_methods_after_goto_label) {
    CBMFileResult *r = extract("Class Graph.KG.Test Extends %RegisteredObject\n"
                               ...
                               "}\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Test.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_methods) {
    CBMFileResult *r = extract("Class MyApp.Utils Extends %RegisteredObject\n"
                               ...
                               "}\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Utils.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_multiple_bases) {
    CBMFileResult *r = extract("Class MyApp.Dual Extends (MyApp.Base, %RegisteredObject)\n"
                               "{\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Dual.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    int found = 0;
    for (int i = 0; i < r->defs.count; i++) {
...
TEST(objectscript_routine_tags) {
    CBMFileResult *r = extract("UTILS\n"
                               ...
                               "    Write msg,!\n"
                               "    Quit\n",
                               PMM_LANG_OBJECTSCRIPT_ROUTINE, "t", "Utils.mac");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_query_member) {
    CBMFileResult *r =
        extract("Class MyApp.Repo Extends %Persistent\n"
                ...
                "}\n",
                PMM_LANG_OBJECTSCRIPT_UDL, "t", "Repo.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_xdata_member) {
    CBMFileResult *r = extract("Class MyApp.Service Extends %CSP.REST\n"
                               ...
                               "XData UrlMap { <Routes/> }\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Service.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_trigger_member) {
    CBMFileResult *r = extract("Class MyApp.Log Extends %Persistent\n"
                               ...
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Log.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_trigger_body_tokens) {
    CBMFileResult *r = extract("Class MyApp.Order Extends %Persistent\n"
                               ...
                               "}\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Order.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT(has_def(r, "Trigger", "AfterInsert"));
    for (int i = 0; i < r->defs.count; i++) {
...
TEST(objectscript_udl_self_call_relative_dot_method) {
    CBMFileResult *r =
        extract("Class HS.Flash.UpdateManager Extends Ens.BusinessProcess\n"
                ...
                "}\n"
                "}\n",
                PMM_LANG_OBJECTSCRIPT_UDL, "t", "UpdateManager.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_calls_typed_new) {
    CBMFileResult *r = extract("Class MyApp.Caller Extends %RegisteredObject\n"
                               ...
                               "}\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Caller.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_ensemble_production_def_parses_items) {
    CBMFileResult *r =
        extract("Class Sample.Production Extends Ens.Production\n"
                ...
                "}\n"
                "}\n",
                PMM_LANG_OBJECTSCRIPT_UDL, "t", "Production.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_udl_ensemble_production_def_hs_settings) {
    CBMFileResult *r = extract(
        ...
        "}\n"
        "}\n",
        PMM_LANG_OBJECTSCRIPT_UDL, "t", "HSProduction.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT(has_def(r, "XData", "ProductionDefinition"));
    pmm_free_result(r);
...
TEST(objectscript_udl_ensemble_production_def_absent_no_error) {
    CBMFileResult *r = extract("Class Sample.NonProduction Extends %Persistent\n"
                               ...
                               "}\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "NonProduction.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    ASSERT(!has_def(r, "XData", "ProductionDefinition"));
    pmm_free_result(r);
...
TEST(objectscript_udl_calls_typed_param) {
    CBMFileResult *r = extract("Class MyApp.Handler Extends %RegisteredObject\n"
                               ...
                               "}\n"
                               "}\n",
                               PMM_LANG_OBJECTSCRIPT_UDL, "t", "Handler.cls");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(objectscript_macro_expand_system) {
    ...
    CBMFileResult *r = extract_with_macros("Class MyApp.Caller Extends %RegisteredObject\n"
                                           ...
                                           "}\n"
                                           "}\n",
                                           PMM_LANG_OBJECTSCRIPT_UDL, "t", "Caller.cls", &mt);
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(iris_export_xml_simple_class) {
    ...
    pmm_arena_init(&arena);
    int count = 0;
    char **udl = pmm_iris_export_to_udl(&arena, SIMPLE_EXPORT, (int)strlen(SIMPLE_EXPORT), &count);
    ASSERT_NOT_NULL(udl);
    ASSERT(count == 1);
...
TEST(iris_export_xml_classmethod) {
    ...
    int count = 0;
    char **udl =
        pmm_iris_export_to_udl(&arena, CLASSMETHOD_EXPORT, (int)strlen(CLASSMETHOD_EXPORT), &count);
    ASSERT_NOT_NULL(udl);
    ASSERT(count == 1);
...
TEST(iris_export_xml_property_parameter_index) {
    ...
    pmm_arena_init(&arena);
    int count = 0;
    char **udl = pmm_iris_export_to_udl(&arena, MEMBER_EXPORT, (int)strlen(MEMBER_EXPORT), &count);
    ASSERT_NOT_NULL(udl);
    ASSERT(count == 1);
...
TEST(iris_export_xml_calls_extracted) {
    ...
    pmm_arena_init(&arena);
    int count = 0;
    char **udl = pmm_iris_export_to_udl(&arena, CALLS_EXPORT, (int)strlen(CALLS_EXPORT), &count);
    ASSERT_NOT_NULL(udl);
    ASSERT(count == 1);
    CBMFileResult *r = pmm_extract_file(udl[0], (int)strlen(udl[0]), PMM_LANG_OBJECTSCRIPT_UDL, "t",
...
TEST(iris_export_xml_multi_class) {
    ...
    pmm_arena_init(&arena);
    int count = 0;
    char **udl = pmm_iris_export_to_udl(&arena, MULTI_EXPORT, (int)strlen(MULTI_EXPORT), &count);
    ASSERT_NOT_NULL(udl);
    ASSERT(count == 2);
