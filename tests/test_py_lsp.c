 ...
 * pmm_extract_file with PMM_LANG_PYTHON, then assertions search the
...
static CBMFileResult *extract_py(const char *source) {
    return pmm_extract_file(source, (int)strlen(source), PMM_LANG_PYTHON,
...
TEST(pylsp_import_aliased) {
    ...
    ASSERT(pmm_type_is_unknown(miss));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(pylsp_import_from) {
    ...
    ASSERT_EQ(t->kind, PMM_TYPE_NAMED);
...
TEST(pylsp_import_star_best_effort) {
    ...
    ASSERT(pmm_type_is_unknown(star_miss));
...
TEST(pylsp_crossfile_method_dispatch) {
    ...
    pmm_run_py_lsp_cross(&arena, source, (int)strlen(source), "test.main",
...
TEST(pylsp_fused_self_attr_chain_via_overlay) {
    ...
    defs[2].lang = PMM_LANG_PYTHON;

    CBMTypeRegistry *reg = pmm_py_build_cross_registry(&arena, defs, 3);
    ASSERT_NOT_NULL(reg);
    ...
    const CBMRegisteredType *foo = pmm_registry_lookup_type(reg, "test.mod.Foo");
    ASSERT_NOT_NULL(foo);
...
TEST(pylsp_crossfile_classmethod_on_class_issue228) {
    ...
    pmm_run_py_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 2, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(pylsp_batch_two_files) {
    ...
    pmm_batch_py_lsp_cross(&arena, files, 2, outs);
