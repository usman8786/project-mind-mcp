...
TEST(typerep_union_two_distinct) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *i = pmm_type_builtin(&a, "int");
    const CBMType *s = pmm_type_builtin(&a, "str");
    const CBMType *m[2] = {i, s};
    const CBMType *u = pmm_type_union(&a, m, 2);
    ASSERT(pmm_type_is_union(u));
...
TEST(typerep_union_dedupes_duplicates) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *i1 = pmm_type_builtin(&a, "int");
    const CBMType *i2 = pmm_type_builtin(&a, "int");
    const CBMType *m[2] = {i1, i2};
    const CBMType *u = pmm_type_union(&a, m, 2);
    /* dedup collapses to a single int type, not a union */
    ASSERT(!pmm_type_is_union(u));
    ASSERT_EQ(u->kind, PMM_TYPE_BUILTIN);
...
TEST(typerep_union_flattens_nested) {
    ...
    const CBMType *i = pmm_type_builtin(&a, "int");
    const CBMType *s = pmm_type_builtin(&a, "str");
    const CBMType *b = pmm_type_builtin(&a, "bytes");
    const CBMType *inner_pair[2] = {i, s};
    const CBMType *inner = pmm_type_union(&a, inner_pair, 2);
    const CBMType *outer_pair[2] = {inner, b};
    const CBMType *u = pmm_type_union(&a, outer_pair, 2);
...
TEST(typerep_union_single_member_collapses) {
    ...
    const CBMType *i = pmm_type_builtin(&a, "int");
    const CBMType *m[1] = {i};
    const CBMType *u = pmm_type_union(&a, m, 1);
    ASSERT(!pmm_type_is_union(u));
    ASSERT_EQ(u->kind, PMM_TYPE_BUILTIN);
    pmm_arena_destroy(&a);
    PASS();
...
TEST(typerep_union_empty_is_unknown) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *u = pmm_type_union(&a, NULL, 0);
    ASSERT(pmm_type_is_unknown(u));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(typerep_optional_is_union_with_none) {
    ...
    pmm_arena_init(&a);
    const CBMType *i = pmm_type_builtin(&a, "int");
    const CBMType *opt = pmm_type_optional(&a, i);
    ASSERT(pmm_type_is_union(opt));
    ...
    for (int k = 0; k < opt->data.union_type.count; k++) {
        ...
        if (m->kind == PMM_TYPE_BUILTIN && strcmp(m->data.builtin.name, "int") == 0)
            has_int = true;
        if (m->kind == PMM_TYPE_BUILTIN && strcmp(m->data.builtin.name, "None") == 0)
            has_none = true;
    }
    ASSERT(has_int);
    ASSERT(has_none);
...
TEST(typerep_literal_int_3) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *base = pmm_type_builtin(&a, "int");
    const CBMType *lit = pmm_type_literal(&a, base, "3");
    ASSERT_EQ(lit->kind, PMM_TYPE_LITERAL);
    ASSERT(pmm_type_equal(lit->data.literal.base, base));
...
TEST(typerep_literal_equality_distinguishes_text) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *base = pmm_type_builtin(&a, "str");
    const CBMType *foo = pmm_type_literal(&a, base, "\"foo\"");
    const CBMType *bar = pmm_type_literal(&a, base, "\"bar\"");
    const CBMType *foo2 = pmm_type_literal(&a, base, "\"foo\"");
    ASSERT(!pmm_type_equal(foo, bar));
    ASSERT(pmm_type_equal(foo, foo2));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(typerep_protocol_method_set) {
    ...
    const CBMType *proto = pmm_type_protocol(&a, "typing.IO", methods, NULL);
    ASSERT(pmm_type_is_protocol(proto));
...
TEST(typerep_protocol_satisfied_by_protocol_with_superset) {
    ...
    const CBMType *proto = pmm_type_protocol(&a, "P1", needed, NULL);
    const CBMType *cand = pmm_type_protocol(&a, "P2", have, NULL);
    ASSERT(pmm_type_protocol_satisfied_by(proto, cand));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(typerep_module_carries_qn) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *m = pmm_type_module(&a, "os.path");
    ASSERT(pmm_type_is_module(m));
...
TEST(typerep_module_equality_by_qn) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *a1 = pmm_type_module(&a, "os");
    const CBMType *a2 = pmm_type_module(&a, "os");
    const CBMType *b = pmm_type_module(&a, "sys");
    ASSERT(pmm_type_equal(a1, a2));
    ASSERT(!pmm_type_equal(a1, b));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(typerep_callable_with_args_and_return) {
    ...
    const CBMType *c = pmm_type_callable(&a, params, 2, i);
    ASSERT_EQ(c->kind, PMM_TYPE_CALLABLE);
    ASSERT_EQ(c->data.callable.param_count, 2);
    ASSERT(pmm_type_equal(c->data.callable.return_type, i));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(typerep_callable_elliptic_arity_minus_one) {
    ...
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *r = pmm_type_builtin(&a, "int");
    const CBMType *c = pmm_type_callable(&a, NULL, -1, r);
...
TEST(typerep_callable_equality) {
    ...
    const CBMType *c1 = pmm_type_callable(&a, p1, 1, s);
    const CBMType *c2 = pmm_type_callable(&a, p1, 1, s);
    const CBMType *c3 = pmm_type_callable(&a, p1, 1, i); /* different return */
    ASSERT(pmm_type_equal(c1, c2));
    ASSERT(!pmm_type_equal(c1, c3));
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(typerep_substitute_unbound_param_preserved) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *t = pmm_type_type_param(&a, "T");
    const char *params[] = {"T", NULL};
    const CBMType *args[] = {NULL, NULL};
    const CBMType *sub = pmm_type_substitute(&a, t, params, args);
    ASSERT(sub == t);
    pmm_arena_destroy(&a);
    PASS();
}
...
TEST(typerep_substitute_short_args_no_oob_issue427) {
    CBMArena a;
    pmm_arena_init(&a);
    const CBMType *t = pmm_type_type_param(&a, "V");                   /* the 3rd declared param */
    const char *params[] = {"T", "U", "V", NULL};                      /* 3 declared params */
    const CBMType *args[] = {pmm_type_type_param(&a, "Widget"), NULL}; /* 1 arg */
    const CBMType *sub = pmm_type_substitute(&a, t, params, args);
