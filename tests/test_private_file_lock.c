...
static bool private_lock_fixture_start(private_lock_fixture_t *fixture) {
...
#ifdef _WIN32
    int written = snprintf(fixture->parent, sizeof(fixture->parent), "%s/pmm-private-lock-XXXXXX",
                           pmm_tmpdir());
    ...
    if (written <= 0 || written >= (int)sizeof(fixture->root) || pmm_mkdir(fixture->root) != 0) {
        return false;
    }
    FILE *seed = pmm_daemon_ipc_private_log_open(fixture->root, "seed", 64);
    ...
    wchar_t *wide_seed =
        written > 0 && written < (int)sizeof(seed_path) ? pmm_utf8_to_wide(seed_path) : NULL;
    if (wide_seed) {
        (void)DeleteFileW(wide_seed);
    }
    free(wide_seed);
    wchar_t *wide_root = seeded ? pmm_utf8_to_wide(fixture->root) : NULL;
    HANDLE handle =
    ...
    pmm_private_file_lock_status_t status =
        handle != INVALID_HANDLE_VALUE
            ? pmm_private_lock_directory_adopt_windows(handle, fixture->root, &fixture->directory)
            : PMM_PRIVATE_FILE_LOCK_IO;
    if (status != PMM_PRIVATE_FILE_LOCK_OK && handle != INVALID_HANDLE_VALUE) {
        (void)CloseHandle(handle);
    }
    return status == PMM_PRIVATE_FILE_LOCK_OK;
#else
    int written = snprintf(fixture->parent, sizeof(fixture->parent), "%s/pmm-private-lock-XXXXXX",
                           pmm_tmpdir());
    if (written <= 0 || written >= (int)sizeof(fixture->parent) || !pmm_mkdtemp(fixture->parent)) {
    ...
    pmm_private_file_lock_status_t status =
        pmm_private_lock_directory_adopt_posix(fd, fixture->root, &fixture->directory);
...
TEST(private_file_lock_payload_requires_exclusive_write_and_survives_reopen) {
    ...
    pmm_private_file_lock_t *writer = NULL;
    pmm_private_file_lock_t *reader = NULL;
    pmm_private_file_lock_status_t writer_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t write_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t reader_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t read_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t shared_write_status = PMM_PRIVATE_FILE_LOCK_IO;
    if (started) {
        writer_status = pmm_private_file_lock_try_acquire(fixture.directory, "payload.lock",
                                                          PMM_PRIVATE_FILE_LOCK_EX, &writer);
    }
    if (writer_status == PMM_PRIVATE_FILE_LOCK_OK) {
        write_status = pmm_private_file_lock_payload_write(writer, payload, sizeof(payload));
        (void)pmm_private_file_lock_release(&writer);
        reader_status = pmm_private_file_lock_try_acquire(fixture.directory, "payload.lock",
                                                          PMM_PRIVATE_FILE_LOCK_SH, &reader);
    }
    if (reader_status == PMM_PRIVATE_FILE_LOCK_OK) {
        read_status = pmm_private_file_lock_payload_read(reader, readback, sizeof(readback),
                                                         &readback_length);
        shared_write_status = pmm_private_file_lock_payload_write(reader, payload, sizeof(payload));
    }
    if (reader) {
        (void)pmm_private_file_lock_release(&reader);
    }
    if (writer) {
        (void)pmm_private_file_lock_release(&writer);
    }
...
TEST(private_file_lock_shared_and_exclusive_matrix) {
    private_lock_fixture_t fixture;
    bool started = private_lock_fixture_start(&fixture);
    pmm_private_file_lock_t *first_reader = NULL;
    pmm_private_file_lock_t *second_reader = NULL;
    pmm_private_file_lock_t *writer = NULL;
    pmm_private_file_lock_status_t first_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t second_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t blocked_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t one_reader_blocked_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t writer_status = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t reader_blocked_status = PMM_PRIVATE_FILE_LOCK_IO;
    bool stable_file = false;
#ifdef _WIN32
    bool noninheritable = false;
    pmm_private_file_lock_status_t device_name_status = PMM_PRIVATE_FILE_LOCK_IO;
#endif
    if (started) {
        first_status = pmm_private_file_lock_try_acquire(fixture.directory, "matrix.lock",
                                                         PMM_PRIVATE_FILE_LOCK_SH, &first_reader);
#ifdef _WIN32
        noninheritable = pmm_private_file_lock_is_cloexec_for_test(first_reader);
        pmm_private_file_lock_t *device_lock = NULL;
        device_name_status = pmm_private_file_lock_try_acquire(
            fixture.directory, "nul.lock", PMM_PRIVATE_FILE_LOCK_EX, &device_lock);
        if (device_lock) {
            (void)pmm_private_file_lock_release(&device_lock);
        }
#endif
        second_status = pmm_private_file_lock_try_acquire(fixture.directory, "matrix.lock",
                                                          PMM_PRIVATE_FILE_LOCK_SH, &second_reader);
        blocked_status = pmm_private_file_lock_try_acquire(fixture.directory, "matrix.lock",
                                                           PMM_PRIVATE_FILE_LOCK_EX, &writer);
    }
    if (second_reader) {
        (void)pmm_private_file_lock_release(&second_reader);
    }
    if (started) {
        one_reader_blocked_status = pmm_private_file_lock_try_acquire(
            fixture.directory, "matrix.lock", PMM_PRIVATE_FILE_LOCK_EX, &writer);
    }
    if (first_reader) {
        (void)pmm_private_file_lock_release(&first_reader);
    }
    if (started) {
        writer_status = pmm_private_file_lock_try_acquire(fixture.directory, "matrix.lock",
                                                          PMM_PRIVATE_FILE_LOCK_EX, &writer);
        reader_blocked_status = pmm_private_file_lock_try_acquire(
            fixture.directory, "matrix.lock", PMM_PRIVATE_FILE_LOCK_SH, &first_reader);
    }
    if (first_reader) {
        (void)pmm_private_file_lock_release(&first_reader);
    }
    ...
    wchar_t *wide_matrix = matrix_path_ok ? pmm_utf8_to_wide(matrix_path) : NULL;
    ...
    ASSERT_TRUE(started);
    ASSERT_EQ(first_status, PMM_PRIVATE_FILE_LOCK_OK);
    ASSERT_EQ(second_status, PMM_PRIVATE_FILE_LOCK_OK);
    ASSERT_EQ(blocked_status, PMM_PRIVATE_FILE_LOCK_BUSY);
    ASSERT_EQ(one_reader_blocked_status, PMM_PRIVATE_FILE_LOCK_BUSY);
    ASSERT_EQ(writer_status, PMM_PRIVATE_FILE_LOCK_OK);
    ASSERT_EQ(reader_blocked_status, PMM_PRIVATE_FILE_LOCK_BUSY);
    ASSERT_TRUE(stable_file);
#ifdef _WIN32
    ASSERT_TRUE(noninheritable);
    ASSERT_EQ(device_name_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
#endif
    PASS();
}
...
TEST(private_file_lock_unlock_failure_retains_retryable_lock) {
    ...
    pmm_private_file_lock_status_t acquired =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "release-unlock.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &lock)
                : PMM_PRIVATE_FILE_LOCK_IO;
    bool fault_set = pmm_private_file_lock_fail_next_release_step_for_test(
        lock, PMM_PRIVATE_FILE_LOCK_RELEASE_UNLOCK);
    pmm_private_file_lock_status_t first_release =
        fault_set ? pmm_private_file_lock_release(&lock) : PMM_PRIVATE_FILE_LOCK_IO;
    bool retained = lock != NULL;

    pmm_private_file_lock_t *contender = NULL;
    pmm_private_file_lock_status_t while_failed =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "release-unlock.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &contender)
                : PMM_PRIVATE_FILE_LOCK_IO;
    if (contender) {
        (void)pmm_private_file_lock_release(&contender);
    }
    pmm_private_file_lock_status_t retry =
        lock ? pmm_private_file_lock_release(&lock) : PMM_PRIVATE_FILE_LOCK_IO;
    if (lock) {
        (void)pmm_private_file_lock_release(&lock);
    }
    private_lock_fixture_finish(&fixture);
    ...
    ASSERT_EQ(while_failed, PMM_PRIVATE_FILE_LOCK_BUSY);
...
TEST(private_file_lock_close_failure_retries_without_duplicate_unlock) {
    ...
    pmm_private_file_lock_t *lock = NULL;
    pmm_private_file_lock_status_t acquired =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "release-close.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &lock)
                : PMM_PRIVATE_FILE_LOCK_IO;
    bool close_fault_set = pmm_private_file_lock_fail_next_release_step_for_test(
        lock, PMM_PRIVATE_FILE_LOCK_RELEASE_CLOSE);
    pmm_private_file_lock_status_t first_release =
        close_fault_set ? pmm_private_file_lock_release(&lock) : PMM_PRIVATE_FILE_LOCK_IO;
    bool retained = lock != NULL;
    unsigned int unlock_attempts = pmm_private_file_lock_release_step_attempts_for_test(
        lock, PMM_PRIVATE_FILE_LOCK_RELEASE_UNLOCK);
    unsigned int close_attempts = pmm_private_file_lock_release_step_attempts_for_test(
        lock, PMM_PRIVATE_FILE_LOCK_RELEASE_CLOSE);

    pmm_private_file_lock_t *contender = NULL;
    pmm_private_file_lock_status_t after_unlock =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "release-close.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &contender)
                : PMM_PRIVATE_FILE_LOCK_IO;
    if (contender) {
    ...
    bool duplicate_unlock_fault_set = pmm_private_file_lock_fail_next_release_step_for_test(
        lock, PMM_PRIVATE_FILE_LOCK_RELEASE_UNLOCK);
    pmm_private_file_lock_status_t retry =
        lock ? pmm_private_file_lock_release(&lock) : PMM_PRIVATE_FILE_LOCK_IO;
    if (lock) {
    ...
    ASSERT_EQ(after_unlock, PMM_PRIVATE_FILE_LOCK_OK);
...
TEST(private_file_lock_consumed_close_error_never_retries_recycled_fd) {
    ...
    pmm_private_file_lock_t *lock = NULL;
    pmm_private_file_lock_status_t acquired =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "consumed-close.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &lock)
                : PMM_PRIVATE_FILE_LOCK_IO;
    int consumed_fd = pmm_private_file_lock_native_fd_for_test(lock);
    bool fault_set = pmm_private_file_lock_fail_close_after_consuming_for_test(lock);
    pmm_private_file_lock_status_t first_release =
        fault_set ? pmm_private_file_lock_release(&lock) : PMM_PRIVATE_FILE_LOCK_IO;
...
TEST(private_file_lock_post_acquire_failure_returns_cleanup_owner) {
    private_lock_fixture_t fixture;
    bool started = private_lock_fixture_start(&fixture);
    bool fault_set = started && pmm_private_lock_directory_fail_post_acquire_cleanup_for_test(
                                    fixture.directory, true, true);
    pmm_private_file_lock_t *cleanup = NULL;
    pmm_private_file_lock_status_t acquire_status =
        fault_set
            ? pmm_private_file_lock_try_acquire(fixture.directory, "post-acquire-cleanup.lock",
                                                PMM_PRIVATE_FILE_LOCK_EX, &cleanup)
            : PMM_PRIVATE_FILE_LOCK_IO;
    bool cleanup_retained = cleanup != NULL;

    pmm_private_file_lock_t *contender = NULL;
    pmm_private_file_lock_status_t while_unlock_pending =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "post-acquire-cleanup.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &contender)
                : PMM_PRIVATE_FILE_LOCK_IO;
    if (contender) {
        (void)pmm_private_file_lock_release(&contender);
    }
    pmm_private_file_lock_status_t first_cleanup =
        cleanup ? pmm_private_file_lock_release(&cleanup) : PMM_PRIVATE_FILE_LOCK_IO;
    bool retained_close_pending = cleanup != NULL;
    pmm_private_file_lock_status_t second_cleanup =
        cleanup ? pmm_private_file_lock_release(&cleanup) : PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_t *after = NULL;
    pmm_private_file_lock_status_t after_status =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "post-acquire-cleanup.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &after)
                : PMM_PRIVATE_FILE_LOCK_IO;
    if (after) {
        (void)pmm_private_file_lock_release(&after);
    }
    if (cleanup) {
        (void)pmm_private_file_lock_release(&cleanup);
    }
    private_lock_fixture_finish(&fixture);

    ASSERT_TRUE(started);
    ASSERT_TRUE(fault_set);
    ASSERT_EQ(acquire_status, PMM_PRIVATE_FILE_LOCK_IO);
    ASSERT_TRUE(cleanup_retained);
    ASSERT_EQ(while_unlock_pending, PMM_PRIVATE_FILE_LOCK_BUSY);
    ASSERT_EQ(first_cleanup, PMM_PRIVATE_FILE_LOCK_IO);
    ASSERT_TRUE(retained_close_pending);
    ASSERT_EQ(second_cleanup, PMM_PRIVATE_FILE_LOCK_OK);
    ASSERT_NULL(cleanup);
...
TEST(private_file_lock_windows_lock_attempt_failure_returns_cleanup_owner) {
    ...
    bool started = private_lock_fixture_start(&fixture);
    bool fault_set =
        started && pmm_private_lock_directory_fail_lock_attempt_cleanup_for_test(fixture.directory);
    pmm_private_file_lock_t *cleanup = NULL;
    pmm_private_file_lock_status_t acquire_status =
        fault_set
            ? pmm_private_file_lock_try_acquire(fixture.directory, "lock-attempt-cleanup.lock",
                                                PMM_PRIVATE_FILE_LOCK_EX, &cleanup)
            : PMM_PRIVATE_FILE_LOCK_IO;
    ...
    pmm_private_file_lock_t *after = NULL;
    pmm_private_file_lock_status_t after_status =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "lock-attempt-cleanup.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &after)
                : PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t after_release =
        after ? pmm_private_file_lock_release(&after) : PMM_PRIVATE_FILE_LOCK_IO;
    private_lock_fixture_finish(&fixture);
...
TEST(private_file_lock_rejects_unsafe_entries_and_replaced_root) {
    ...
    pmm_private_file_lock_t *lock = NULL;
    pmm_private_file_lock_status_t symlink_status =
        fixtures_ok ? pmm_private_file_lock_try_acquire(fixture.directory, "symlink.lock",
                                                        PMM_PRIVATE_FILE_LOCK_EX, &lock)
                    : PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t hardlink_status =
        fixtures_ok ? pmm_private_file_lock_try_acquire(fixture.directory, "hardlink.lock",
                                                        PMM_PRIVATE_FILE_LOCK_EX, &lock)
                    : PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t mode_status =
        fixtures_ok ? pmm_private_file_lock_try_acquire(fixture.directory, "mode.lock",
                                                        PMM_PRIVATE_FILE_LOCK_EX, &lock)
                    : PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t special_mode_status =
        fixtures_ok ? pmm_private_file_lock_try_acquire(fixture.directory, "special-mode.lock",
                                                        PMM_PRIVATE_FILE_LOCK_EX, &lock)
                    : PMM_PRIVATE_FILE_LOCK_IO;
    if (lock) {
        (void)pmm_private_file_lock_release(&lock);
    }
    pmm_private_file_lock_status_t fifo_status =
        fixtures_ok ? pmm_private_file_lock_try_acquire(fixture.directory, "fifo.lock",
                                                        PMM_PRIVATE_FILE_LOCK_EX, &lock)
                    : PMM_PRIVATE_FILE_LOCK_IO;

    bool special_root_set = fixtures_ok && chmod(fixture.root, 01700) == 0;
    pmm_private_file_lock_status_t special_root_status =
        special_root_set ? pmm_private_file_lock_try_acquire(fixture.directory, "special-root.lock",
                                                             PMM_PRIVATE_FILE_LOCK_EX, &lock)
                         : PMM_PRIVATE_FILE_LOCK_IO;
    if (lock) {
        (void)pmm_private_file_lock_release(&lock);
    ...
    pmm_private_file_lock_status_t replaced_status =
        replaced ? pmm_private_file_lock_try_acquire(fixture.directory, "replaced.lock",
                                                     PMM_PRIVATE_FILE_LOCK_EX, &lock)
                 : PMM_PRIVATE_FILE_LOCK_IO;
    ...
    ASSERT_EQ(symlink_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    ASSERT_EQ(hardlink_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    ASSERT_EQ(mode_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    ASSERT_EQ(special_mode_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    ASSERT_EQ(fifo_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    ASSERT_TRUE(special_root_set);
    ASSERT_EQ(special_root_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    ASSERT_TRUE(special_root_restored);
    ASSERT_TRUE(replaced);
    ASSERT_EQ(replaced_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    PASS();
#endif
}
...
TEST(private_file_lock_macos_rejects_directory_acl_added_after_adoption) {
    ...
    pmm_private_file_lock_status_t acquire_status =
        mode_stayed_private
            ? pmm_private_file_lock_try_acquire(fixture.directory, "acl-directory.lock",
                                                PMM_PRIVATE_FILE_LOCK_EX, &lock)
            : PMM_PRIVATE_FILE_LOCK_IO;
    ...
    ASSERT_EQ(acquire_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    ASSERT_NULL(lock);
...
TEST(private_file_lock_macos_rejects_file_acl_added_after_acquisition) {
    ...
    pmm_private_file_lock_t *lock = NULL;
    pmm_private_file_lock_status_t acquire_status =
        path_ok ? pmm_private_file_lock_try_acquire(fixture.directory, "acl-file.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &lock)
                : PMM_PRIVATE_FILE_LOCK_IO;
    int acl_fixture = acquire_status == PMM_PRIVATE_FILE_LOCK_OK
    ...
    pmm_private_file_lock_status_t payload_status =
        mode_stayed_private ? pmm_private_file_lock_payload_write(lock, payload, sizeof(payload))
                            : PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t release_status =
        lock ? pmm_private_file_lock_release(&lock) : PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_t *reopened = NULL;
    pmm_private_file_lock_status_t reopen_status = pmm_private_file_lock_try_acquire(
        fixture.directory, "acl-file.lock", PMM_PRIVATE_FILE_LOCK_EX, &reopened);
    if (reopened) {
        (void)pmm_private_file_lock_release(&reopened);
    }
    private_lock_fixture_finish(&fixture);

    ASSERT_TRUE(started);
    ASSERT_TRUE(path_ok);
    ASSERT_EQ(acquire_status, PMM_PRIVATE_FILE_LOCK_OK);
    ASSERT_EQ(acl_fixture, 1);
    ASSERT_TRUE(mode_stayed_private);
    ASSERT_EQ(payload_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
    ASSERT_EQ(release_status, PMM_PRIVATE_FILE_LOCK_OK);
    ASSERT_NULL(lock);
    ASSERT_EQ(reopen_status, PMM_PRIVATE_FILE_LOCK_UNSAFE);
...
TEST(private_file_lock_fork_child_cannot_unlock_or_retain_parent_lock) {
    ...
    private_lock_fixture_t fixture;
    bool started = private_lock_fixture_start(&fixture);
    pmm_private_file_lock_t *parent_lock = NULL;
    pmm_private_file_lock_status_t acquired =
        started ? pmm_private_file_lock_try_acquire(fixture.directory, "fork.lock",
                                                    PMM_PRIVATE_FILE_LOCK_EX, &parent_lock)
                : PMM_PRIVATE_FILE_LOCK_IO;
    bool cloexec = pmm_private_file_lock_is_cloexec_for_test(parent_lock);
    int command_pipe[2] = {-1, -1};
    int result_pipe[2] = {-1, -1};
    bool pipes_ok =
        acquired == PMM_PRIVATE_FILE_LOCK_OK && pipe(command_pipe) == 0 && pipe(result_pipe) == 0;
    pid_t child = pipes_ok ? fork() : -1;
    if (child == 0) {
        (void)close(command_pipe[1]);
        (void)close(result_pipe[0]);
        pmm_private_file_lock_status_t child_release = pmm_private_file_lock_release(&parent_lock);
        char report = child_release == PMM_PRIVATE_FILE_LOCK_OK ? 'R' : 'E';
    ...
    pmm_private_file_lock_t *contender = NULL;
    pmm_private_file_lock_status_t while_parent = PMM_PRIVATE_FILE_LOCK_IO;
    pmm_private_file_lock_status_t after_parent = PMM_PRIVATE_FILE_LOCK_IO;
    ...
    if (child > 0) {
        ...
        while_parent = pmm_private_file_lock_try_acquire(fixture.directory, "fork.lock",
                                                         PMM_PRIVATE_FILE_LOCK_EX, &contender);
        parent_released = pmm_private_file_lock_release(&parent_lock) == PMM_PRIVATE_FILE_LOCK_OK;
        after_parent = pmm_private_file_lock_try_acquire(fixture.directory, "fork.lock",
                                                         PMM_PRIVATE_FILE_LOCK_EX, &contender);
        if (contender) {
            (void)pmm_private_file_lock_release(&contender);
        }
    ...
    if (parent_lock) {
        (void)pmm_private_file_lock_release(&parent_lock);
    }
    ...
    ASSERT_EQ(while_parent, PMM_PRIVATE_FILE_LOCK_BUSY);
    ASSERT_TRUE(parent_released);
    ASSERT_EQ(after_parent, PMM_PRIVATE_FILE_LOCK_OK);
