...
TEST(store_close_null) {
    pmm_store_close(NULL); /* should not crash */
    PASS();
}

TEST(store_open_memory_twice) {
    pmm_store_t *s1 = pmm_store_open_memory();
    pmm_store_t *s2 = pmm_store_open_memory();
    ASSERT_NOT_NULL(s1);
    ASSERT_NOT_NULL(s2);
...
TEST(store_project_crud) {
    ...
    /* Create */
    int rc = pmm_store_upsert_project(s, "myproject", "/home/user/myproject");
    ASSERT_EQ(rc, PMM_STORE_OK);

    /* Get */
    pmm_project_t p = {0};
    rc = pmm_store_get_project(s, "myproject", &p);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(p.name, "myproject");
    ASSERT_STR_EQ(p.root_path, "/home/user/myproject");
    ASSERT_NOT_NULL(p.indexed_at);
    pmm_project_free_fields(&p);

    /* List */
    pmm_project_t *projects = NULL;
    int count = 0;
    rc = pmm_store_list_projects(s, &projects, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(projects[0].name, "myproject");
    pmm_store_free_projects(projects, count);

    /* Get non-existent */
    pmm_project_t p2 = {0};
    rc = pmm_store_get_project(s, "nonexistent", &p2);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
...
TEST(store_project_update) {
    pmm_store_t *s = pmm_store_open_memory();
    pmm_store_upsert_project(s, "test", "/old/path");

    /* Update root path */
    pmm_store_upsert_project(s, "test", "/new/path");

    pmm_project_t p = {0};
    pmm_store_get_project(s, "test", &p);
    ...
    pmm_project_t *projects = NULL;
    int count = 0;
    pmm_store_list_projects(s, &projects, &count);
    ASSERT_EQ(count, 1);
    pmm_store_free_projects(projects, count);
...
TEST(store_project_delete) {
    ...
    int rc = pmm_store_delete_project(s, "test");
    ASSERT_EQ(rc, PMM_STORE_OK);

    pmm_project_t p = {0};
    rc = pmm_store_get_project(s, "test", &p);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
...
TEST(store_node_crud) {
    ...
    /* Find by QN */
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_qn(s, "test", "test.main.Foo", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    /* Find by ID */
    pmm_node_t found2 = {0};
    rc = pmm_store_find_node_by_id(s, id, &found2);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(found2.qualified_name, "test.main.Foo");
    pmm_node_free_fields(&found2);

    /* Find by name */
    pmm_node_t *nodes = NULL;
    int count = 0;
    rc = pmm_store_find_nodes_by_name(s, "test", "Foo", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(store_node_dedup) {
    ...
    pmm_node_t found = {0};
    pmm_store_find_node_by_qn(s, "test", "test.main.Foo", &found);
    ASSERT_NOT_NULL(found.properties_json);
...
TEST(store_node_find_by_label) {
    ...
    pmm_node_t n2 = {.project = "test", .label = "Class", .name = "B", .qualified_name = "test.B"};
    pmm_node_t n3 = {
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_label(s, "test", "Function", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 2);
    pmm_store_free_nodes(nodes, count);

    rc = pmm_store_find_nodes_by_label(s, "test", "Class", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(store_node_find_by_file) {
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_file(s, "test", "main.go", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 2);
    pmm_store_free_nodes(nodes, count);
...
TEST(store_node_find_not_found) {
    ...
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_qn(s, "test", "nonexistent", &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);

    rc = pmm_store_find_node_by_id(s, 99999, &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
...
TEST(store_node_delete_by_file) {
    ...
    pmm_store_delete_nodes_by_file(s, "test", "main.go");
    int cnt = pmm_store_count_nodes(s, "test");
    ASSERT_EQ(cnt, 1);
...
TEST(store_node_delete_by_label) {
    ...
    pmm_store_delete_nodes_by_label(s, "test", "Function");
    int cnt = pmm_store_count_nodes(s, "test");
    ASSERT_EQ(cnt, 1);
...
TEST(store_node_batch_upsert) {
    ...
    int rc = pmm_store_upsert_node_batch(s, nodes, 150, ids);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    int64_t ids2[150];
    rc = pmm_store_upsert_node_batch(s, nodes, 150, ids2);
    ASSERT_EQ(rc, PMM_STORE_OK);
    cnt = pmm_store_count_nodes(s, "test");
    ASSERT_EQ(cnt, 150);
...
TEST(store_node_batch_empty) {
    pmm_store_t *s = pmm_store_open_memory();
    int rc = pmm_store_upsert_node_batch(s, NULL, 0, NULL);
    ASSERT_EQ(rc, PMM_STORE_OK);
    pmm_store_close(s);
    PASS();
}
...
TEST(store_cascade_delete) {
    ...
    int64_t id1 = pmm_store_upsert_node(s, &n1);
    int64_t id2 = pmm_store_upsert_node(s, &n2);

    pmm_edge_t e = {.project = "test", .source_id = id1, .target_id = id2, .type = "CALLS"};
    pmm_store_insert_edge(s, &e);

    /* Delete project — should cascade */
    pmm_store_delete_project(s, "test");

    int ncnt = pmm_store_count_nodes(s, "test");
...
TEST(store_file_hash_crud) {
    ...
    /* Upsert */
    int rc = pmm_store_upsert_file_hash(s, "test", "main.go", "abc123", 1000000, 512);
    ASSERT_EQ(rc, PMM_STORE_OK);

    /* Get */
    pmm_file_hash_t *hashes = NULL;
    int count = 0;
    rc = pmm_store_get_file_hashes(s, "test", &hashes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    pmm_store_free_file_hashes(hashes, count);

    /* Update */
    rc = pmm_store_upsert_file_hash(s, "test", "main.go", "def456", 2000000, 1024);
    ASSERT_EQ(rc, PMM_STORE_OK);
    rc = pmm_store_get_file_hashes(s, "test", &hashes, &count);
    ASSERT_EQ(count, 1);
    ...
    /* Delete single */
    rc = pmm_store_delete_file_hash(s, "test", "main.go");
    ASSERT_EQ(rc, PMM_STORE_OK);
    rc = pmm_store_get_file_hashes(s, "test", &hashes, &count);
    ASSERT_EQ(count, 0);
    pmm_store_free_file_hashes(hashes, count);
...
TEST(store_file_hash_upsert_rejects_null_required_fields) {
    /* Pins the API contract that `pmm_store_upsert_file_hash` returns
     * PMM_STORE_ERR (not silent OK) when a NOT NULL column would receive
    ...
    rc = pmm_store_upsert_file_hash(s, "test", "other.go", NULL, 2000000, 1024);
    ASSERT_EQ(rc, PMM_STORE_ERR);

    /* NULL rel_path violates NOT NULL on file_hashes.rel_path → must return ERR. */
    rc = pmm_store_upsert_file_hash(s, "test", NULL, "deadbeef", 3000000, 2048);
    ASSERT_EQ(rc, PMM_STORE_ERR);

    /* NULL project violates NOT NULL on file_hashes.project → must return ERR. */
    rc = pmm_store_upsert_file_hash(s, NULL, "third.go", "cafebabe", 4000000, 4096);
    ASSERT_EQ(rc, PMM_STORE_ERR);
    ...
    pmm_file_hash_t *hashes = NULL;
    int count = 0;
    pmm_store_get_file_hashes(s, "test", &hashes, &count);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(hashes[0].rel_path, "main.go");
    pmm_store_free_file_hashes(hashes, count);
...
TEST(store_node_properties_json) {
    ...
    pmm_node_t found = {0};
    pmm_store_find_node_by_qn(s, "test", "test.Bar", &found);
    ASSERT_NOT_NULL(found.properties_json);
...
TEST(store_node_null_properties) {
    ...
    pmm_node_t found = {0};
    pmm_store_find_node_by_qn(s, "test", "test.Baz", &found);
    ASSERT_NOT_NULL(found.properties_json);
...
TEST(store_find_by_file_overlap) {
    ...
    pmm_node_t nm = {.project = "test",
                     ...
                     .start_line = 1,
                     .end_line = 100};
    pmm_store_upsert_node(s, &na);
    pmm_store_upsert_node(s, &nb);
    pmm_store_upsert_node(s, &nc);
    pmm_store_upsert_node(s, &nm);

    /* Overlap with funcA (lines 5-8) */
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_file_overlap(s, "test", "main.go", 5, 8, &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    rc = pmm_store_find_nodes_by_file_overlap(s, "test", "main.go", 8, 15, &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 2);
    pmm_store_free_nodes(nodes, count);

    /* No overlap (lines 26-30) */
    rc = pmm_store_find_nodes_by_file_overlap(s, "test", "main.go", 26, 30, &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 0);
    pmm_store_free_nodes(nodes, count);

    /* Different file */
    rc = pmm_store_find_nodes_by_file_overlap(s, "test", "other.go", 1, 50, &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(store_find_by_qn_suffix_single) {
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_qn_suffix(s, "test", "main.HandleRequest", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(store_find_by_qn_suffix_no_match) {
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_qn_suffix(s, "test", "main.Bar", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 0);
    pmm_store_free_nodes(nodes, count);
...
TEST(store_find_by_qn_suffix_multiple) {
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_qn_suffix(s, "test", "Run", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 2);
    pmm_store_free_nodes(nodes, count);
...
TEST(store_find_by_qn_suffix_dot_boundary) {
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_qn_suffix(s, "test", "HandleRequest", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(nodes[0].name, "HandleRequest");
    pmm_store_free_nodes(nodes, count);
...
TEST(store_node_degree) {
    ...
    pmm_store_node_degree(s, idA, &inA, &outA);
    ASSERT_EQ(inA, 0);
    ASSERT_EQ(outA, 2);

    pmm_store_node_degree(s, idB, &inB, &outB);
    ASSERT_EQ(inB, 1);
    ASSERT_EQ(outB, 1);

    pmm_store_node_degree(s, idC, &inC, &outC);
    ASSERT_EQ(inC, 2);
    ASSERT_EQ(outC, 0);
...
TEST(store_file_hash_batch) {
    ...
    int rc = pmm_store_upsert_file_hash_batch(s, hashes, 3);
    ASSERT_EQ(rc, PMM_STORE_OK);

    pmm_file_hash_t *stored = NULL;
    int count = 0;
    rc = pmm_store_get_file_hashes(s, "test", &stored, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 3);
    pmm_store_free_file_hashes(stored, count);

    /* Update hashes (should not duplicate) */
    hashes[0].sha256 = "updated";
    hashes[0].mtime_ns = 9000;
    rc = pmm_store_upsert_file_hash_batch(s, hashes, 3);
    ASSERT_EQ(rc, PMM_STORE_OK);
 ...
 * per-file pmm_store_upsert_file_hash loop to a single pmm_store_upsert_file_hash_batch:
...
TEST(store_file_hash_batch_equals_loop) {
    ...
    pmm_store_t *a = pmm_store_open_memory();
    pmm_store_upsert_project(a, "p", "/tmp/p");
    for (int i = 0; i < 4; i++) {
        ASSERT_EQ(pmm_store_upsert_file_hash(a, rows[i].project, rows[i].rel_path, rows[i].sha256,
                                             rows[i].mtime_ns, rows[i].size),
                  PMM_STORE_OK);
    }

    /* Store B: the batched path. */
    pmm_store_t *b = pmm_store_open_memory();
    pmm_store_upsert_project(b, "p", "/tmp/p");
    ASSERT_EQ(pmm_store_upsert_file_hash_batch(b, rows, 4), PMM_STORE_OK);

    pmm_file_hash_t *ha = NULL, *hb = NULL;
    int ca = 0, cb = 0;
    ASSERT_EQ(pmm_store_get_file_hashes(a, "p", &ha, &ca), PMM_STORE_OK);
    ASSERT_EQ(pmm_store_get_file_hashes(b, "p", &hb, &cb), PMM_STORE_OK);
    ASSERT_EQ(ca, 4);
    ASSERT_EQ(cb, 4);
    ...
    pmm_store_free_file_hashes(ha, ca);
    pmm_store_free_file_hashes(hb, cb);
    pmm_store_close(a);
    pmm_store_close(b);
    PASS();
}
...
TEST(store_find_edges_by_url_path) {
    ...
    pmm_node_t nt = {.project = "test",
                     .label = "Function",
                     .name = "handler",
                     .qualified_name = "test.handler"};
    int64_t srcID = pmm_store_upsert_node(s, &ns);
    int64_t tgtID = pmm_store_upsert_node(s, &nt);
    ...
    int count = 0;
    int rc = pmm_store_find_edges_by_url_path(s, "test", "orders", &edges, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    rc = pmm_store_find_edges_by_url_path(s, "test", "users", &edges, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 0);
    pmm_store_free_edges(edges, count);
...
TEST(store_restore_from) {
    ...
    pmm_store_t *src = pmm_store_open_memory();
    pmm_store_upsert_project(src, "test", "/tmp/test");
    for (int i = 0; i < 10; i++) {
        char name[32], qn[64];
        ...
        pmm_node_t n = {.project = "test",
                        ...
                        .start_line = i * 10,
                        .end_line = i * 10 + 5};
        pmm_store_upsert_node(src, &n);
    }

    /* Create destination store */
    pmm_store_t *dst = pmm_store_open_memory();

    /* Restore: copy from src → dst */
    int rc = pmm_store_restore_from(dst, src);
    ASSERT_EQ(rc, PMM_STORE_OK);

    /* Verify data survived */
    pmm_node_t found = {0};
    rc = pmm_store_find_node_by_qn(dst, "test", "test.main.Func5", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(found.name, "Func5");
    pmm_node_free_fields(&found);

    int cnt = pmm_store_count_nodes(dst, "test");
    ASSERT_EQ(cnt, 10);
...
TEST(store_find_node_ids_by_qns) {
    ...
    pmm_node_t nb = {
        .project = "test", .label = "Function", .name = "B", .qualified_name = "test.B"};
    int64_t id1 = pmm_store_upsert_node(s, &na);
    int64_t id2 = pmm_store_upsert_node(s, &nb);
    ASSERT_TRUE(id1 > 0);
    ASSERT_TRUE(id2 > 0);
    ...
    int64_t ids[3];
    int found = pmm_store_find_node_ids_by_qns(s, "test", qns, 3, ids);
    ASSERT_EQ(found, 2);
    ASSERT_EQ(ids[0], id1);
    ASSERT_EQ(ids[1], id2);
    ...
    /* Empty batch */
    int found2 = pmm_store_find_node_ids_by_qns(s, "test", NULL, 0, ids);
    ASSERT_EQ(found2, 0);
...
TEST(store_integrity_clean) {
    ...
    pmm_store_t *s = pmm_store_open_memory();
    ASSERT_NOT_NULL(s);
    pmm_store_upsert_project(s, "test-proj", "/tmp/test");
    ASSERT_TRUE(pmm_store_check_integrity(s));
    pmm_store_close(s);
    PASS();
}
...
TEST(store_integrity_windows_lowercase_drive_issue367) {
    ...
    pmm_store_t *s = pmm_store_open_memory();
    ASSERT_NOT_NULL(s);
    pmm_store_upsert_project(s, "lc-drive", "c:/Users/dev/repo");
    ASSERT_TRUE(pmm_store_check_integrity(s));
    pmm_store_close(s);
    PASS();
}
...
TEST(store_integrity_null_check) {
    ...
    ASSERT_FALSE(pmm_store_check_integrity(NULL));
    PASS();
}
...
TEST(store_node_null_qn) {
    ...
    pmm_node_t n = {.project = "test", .label = "Function", .name = "Bar", .qualified_name = NULL};
    int64_t id = pmm_store_upsert_node(s, &n);
...
TEST(store_node_empty_strings) {
    ...
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_qn(s, "test", "", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
...
TEST(store_find_by_id_not_found) {
    ...
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_id(s, 999999, &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);

    /* Negative ID should also be not found */
    rc = pmm_store_find_node_by_id(s, -1, &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
...
TEST(store_find_by_qn_not_found) {
    ...
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_qn(s, "test", "test.DoesNotExist", &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);

    /* Wrong project */
    rc = pmm_store_find_node_by_qn(s, "other-project", "test.Exists", &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
...
TEST(store_find_by_qn_any_cross_project) {
    pmm_store_t *s = pmm_store_open_memory();
    pmm_store_upsert_project(s, "proj-a", "/tmp/a");
    pmm_store_upsert_project(s, "proj-b", "/tmp/b");

    pmm_node_t na = {.project = "proj-a",
                     .label = "Function",
                     .name = "SharedFunc",
                     .qualified_name = "proj-a.main.SharedFunc"};
    pmm_node_t nb = {.project = "proj-b",
                     .label = "Class",
                     .name = "Widget",
    ...
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_qn_any(s, "proj-a.main.SharedFunc", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(found.name, "SharedFunc");
    ASSERT_STR_EQ(found.project, "proj-a");
    pmm_node_free_fields(&found);

    rc = pmm_store_find_node_by_qn_any(s, "proj-b.pkg.Widget", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    /* Non-existent QN */
    rc = pmm_store_find_node_by_qn_any(s, "nonexistent.Nope", &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
...
TEST(store_find_by_name_any_cross_project) {
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_name_any(s, "Init", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 2);
    pmm_store_free_nodes(nodes, count);

    /* Name that doesn't exist */
    rc = pmm_store_find_nodes_by_name_any(s, "Nonexistent", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 0);
    pmm_store_free_nodes(nodes, count);
...
TEST(store_find_by_file_no_match) {
    ...
    pmm_node_t *nodes = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_file(s, "test", "nonexistent.go", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(count, 0);
    pmm_store_free_nodes(nodes, count);
...
TEST(store_node_batch_upsert_100) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        nodes[i] = (pmm_node_t){.project = "test",
                                .label = "Function",
                                .name = names[i],
                                .qualified_name = qns[i],
                                .file_path = "stress.go",
    ...
    int rc = pmm_store_upsert_node_batch(s, nodes, 100, ids);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    pmm_node_t found = {0};
    rc = pmm_store_find_node_by_qn(s, "test", "test.stress.stress_0", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(found.name, "stress_0");
    pmm_node_free_fields(&found);

    rc = pmm_store_find_node_by_qn(s, "test", "test.stress.stress_99", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
...
TEST(store_delete_by_label_verify_remaining) {
    ...
    int rc = pmm_store_delete_nodes_by_label(s, "test", "Function");
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    pmm_node_t found = {0};
    rc = pmm_store_find_node_by_qn(s, "test", "test.ClassB", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(found.label, "Class");
    pmm_node_free_fields(&found);

    rc = pmm_store_find_node_by_qn(s, "test", "test.MethodD", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(found.label, "Method");
    pmm_node_free_fields(&found);

    /* Deleted ones should be gone */
    rc = pmm_store_find_node_by_qn(s, "test", "test.FuncA", &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
    rc = pmm_store_find_node_by_qn(s, "test", "test.FuncC", &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
...
TEST(store_delete_by_file_verify_remaining) {
    ...
    int rc = pmm_store_delete_nodes_by_file(s, "test", "delete_me.go");
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    pmm_node_t found = {0};
    rc = pmm_store_find_node_by_qn(s, "test", "test.B", &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(found.file_path, "keep_me.go");
    pmm_node_free_fields(&found);

    /* Deleted nodes should be gone */
    rc = pmm_store_find_node_by_qn(s, "test", "test.A", &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
    rc = pmm_store_find_node_by_qn(s, "test", "test.C", &found);
    ASSERT_EQ(rc, PMM_STORE_NOT_FOUND);
...
TEST(store_node_upsert_updates_fields) {
    ...
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_id(s, id1, &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_STR_EQ(found.label, "Method");
...
TEST(store_node_long_qn) {
    ...
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_qn(s, "test", long_qn, &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
...
TEST(store_node_properties_special_chars) {
    ...
    pmm_node_t found = {0};
    int rc = pmm_store_find_node_by_id(s, id, &found);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_NOT_NULL(found.properties_json);
...
TEST(store_delete_nodes_nonexistent) {
    ...
    int rc = pmm_store_delete_nodes_by_file(s, "test", "ghost.go");
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(pmm_store_count_nodes(s, "test"), 1);

    /* Delete by non-existent label */
    rc = pmm_store_delete_nodes_by_label(s, "test", "Interface");
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(pmm_store_count_nodes(s, "test"), 1);

    /* Delete by non-existent project */
    rc = pmm_store_delete_nodes_by_project(s, "no-such-project");
    ASSERT_EQ(rc, PMM_STORE_OK);
    ASSERT_EQ(pmm_store_count_nodes(s, "test"), 1);
...
TEST(store_count_nodes_unknown_project) {
    ...
    int cnt = pmm_store_count_nodes(s, "ghost-project");
    ASSERT_EQ(cnt, 0);
    pmm_store_close(s);
    PASS();
}
...
TEST(store_coverage_roundtrip_prune_shadow) {
    pmm_store_t *s = pmm_store_open_memory();
    pmm_store_upsert_project(s, "test", "/tmp/test");
    pmm_store_upsert_file_hash(s, "test", "src/a.py", "", 1, 10);
    ...
    ASSERT_EQ(pmm_store_coverage_replace(s, "test", rows, 4), PMM_STORE_OK);

    pmm_coverage_row_t *got = NULL;
    int n = 0;
    ASSERT_EQ(pmm_store_coverage_get(s, "test", &got, &n), PMM_STORE_OK);
    ...
    pmm_node_t *nodes = NULL;
    int nc = 0;
    ASSERT_EQ(pmm_store_find_nodes_by_label(s, "test::missed", "File", &nodes, &nc), PMM_STORE_OK);
    ASSERT_EQ(nc, 1);
    ...
    nodes = NULL;
    nc = 0;
    ASSERT_EQ(pmm_store_find_nodes_by_label(s, "test::missed", "Folder", &nodes, &nc),
              PMM_STORE_OK);
    ASSERT_EQ(nc, 1);
    ...
    ASSERT_EQ(pmm_store_coverage_replace(s, "test", NULL, 0), PMM_STORE_OK);
    got = NULL;
    n = 0;
    ASSERT_EQ(pmm_store_coverage_get(s, "test", &got, &n), PMM_STORE_OK);
    ASSERT_EQ(n, 0);
...
TEST(store_coverage_targeted_path_and_scope_lookup) {
    pmm_store_t *s = pmm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_upsert_project(s, "coverage-targeted", "/tmp/coverage-targeted"),
              PMM_STORE_OK);
    ...
    ASSERT_EQ(pmm_store_upsert_file_hash(s, "coverage-targeted", "src/partial.c", "", 10, 20),
              PMM_STORE_OK);
    ASSERT_EQ(pmm_store_upsert_file_hash(s, "coverage-targeted", "src/skipped.c", "", 11, 21),
              PMM_STORE_OK);
    ASSERT_EQ(pmm_store_upsert_file_hash(s, "coverage-targeted", "src2/other.c", "", 12, 22),
              PMM_STORE_OK);
    ...
    ASSERT_EQ(pmm_store_coverage_replace(s, "coverage-targeted", rows, 5), PMM_STORE_OK);

    pmm_coverage_row_t *got = NULL;
    int count = 0;
    ASSERT_EQ(pmm_store_coverage_get_path(s, "coverage-targeted", "src/partial.c", &got, &count),
              PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(got[0].rel_path, "src/partial.c");
    ASSERT_STR_EQ(got[0].kind, "parse_partial");
    ASSERT_STR_EQ(got[0].detail, "7-9");
    pmm_store_free_coverage(got, count);

    /* A file below an excluded directory inherits that directory row. */
    got = NULL;
    count = 0;
    ASSERT_EQ(pmm_store_coverage_get_path(s, "coverage-targeted", "generated/nested/file.c", &got,
                                          &count),
              PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    count = 0;
    ASSERT_EQ(
        pmm_store_coverage_get_path(s, "coverage-targeted", "generated2/file.c", &got, &count),
        PMM_STORE_OK);
    ASSERT_EQ(count, 0);
    pmm_store_free_coverage(got, count);
    ...
    got = NULL;
    count = 0;
    ASSERT_EQ(pmm_store_coverage_get_scope(s, "coverage-targeted", "src", &got, &count),
              PMM_STORE_OK);
    ASSERT_EQ(count, 2);
    ASSERT_STR_EQ(got[0].rel_path, "src/partial.c");
    ...
    count = 0;
    ASSERT_EQ(
        pmm_store_coverage_get_scope(s, "coverage-targeted", "generated/nested", &got, &count),
        PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(got[0].rel_path, "generated");
    pmm_store_free_coverage(got, count);
    ...
    ASSERT_EQ(pmm_store_get_file_hash(s, "coverage-targeted", "src/partial.c", &hash),
              PMM_STORE_OK);
    ...
    ASSERT_EQ(pmm_store_get_file_hash(s, "coverage-targeted", "missing.c", &hash),
              PMM_STORE_NOT_FOUND);
    pmm_store_clear_file_hash(&hash);
...
TEST(store_coverage_meta_zero_row_truncation_and_delete) {
    pmm_store_t *s = pmm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_upsert_project(s, "coverage-meta", "/tmp/coverage-meta"), PMM_STORE_OK);

    pmm_coverage_meta_t write_meta = {
    ...
    ASSERT_EQ(pmm_store_coverage_replace_ex(s, "coverage-meta", NULL, 0, &write_meta),
              PMM_STORE_OK);

    pmm_coverage_row_t *rows = NULL;
    int count = 0;
    ASSERT_EQ(pmm_store_coverage_get(s, "coverage-meta", &rows, &count), PMM_STORE_OK);
    ASSERT_EQ(count, 0);
    pmm_store_free_coverage(rows, count);

    pmm_coverage_meta_t got = {0};
    ASSERT_EQ(pmm_store_coverage_meta_get(s, "coverage-meta", &got), PMM_STORE_OK);
    ...
    pmm_store_coverage_meta_clear(&got);
    ...
    ASSERT_EQ(pmm_store_coverage_replace(s, "coverage-meta", NULL, 0), PMM_STORE_OK);
    ASSERT_EQ(pmm_store_coverage_meta_get(s, "coverage-meta", &got), PMM_STORE_NOT_FOUND);
    pmm_store_coverage_meta_clear(&got);
    ...
    ASSERT_EQ(pmm_store_upsert_file_hash(s, "coverage-meta", "bad.c", "", 1, 1), PMM_STORE_OK);
    pmm_coverage_row_t failure = {.rel_path = "bad.c", .kind = "parse_partial", .detail = "1-2"};
    ASSERT_EQ(pmm_store_coverage_replace_ex(s, "coverage-meta", &failure, 1, &write_meta),
              PMM_STORE_OK);
    ASSERT_EQ(pmm_store_delete_project(s, "coverage-meta"), PMM_STORE_OK);
    ...
    ASSERT_EQ(pmm_store_coverage_meta_get(s, "coverage-meta", &got), PMM_STORE_NOT_FOUND);
    pmm_store_coverage_meta_clear(&got);
    pmm_project_t shadow = {0};
    ASSERT_EQ(pmm_store_get_project(s, "coverage-meta::missed", &shadow), PMM_STORE_NOT_FOUND);
    pmm_project_free_fields(&shadow);
...
TEST(store_coverage_replace_rejects_invalid_row_arguments) {
    pmm_store_t *s = pmm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_upsert_project(s, "coverage-invalid", "/tmp/coverage-invalid"),
              PMM_STORE_OK);
    ASSERT_EQ(pmm_store_upsert_file_hash(s, "coverage-invalid", "kept.c", "", 1, 1), PMM_STORE_OK);
    pmm_coverage_row_t kept = {.rel_path = "kept.c", .kind = "parse_partial", .detail = "3-4"};
    pmm_coverage_meta_t original_meta = {
    ...
    ASSERT_EQ(pmm_store_coverage_replace_ex(s, "coverage-invalid", &kept, 1, &original_meta),
              PMM_STORE_OK);

    pmm_coverage_meta_t replacement_meta = original_meta;
    replacement_meta.generation = "must-not-commit";
    ASSERT_EQ(pmm_store_coverage_replace_ex(s, "coverage-invalid", &kept, -1, &replacement_meta),
              PMM_STORE_ERR);
    ASSERT_EQ(pmm_store_coverage_replace_ex(s, "coverage-invalid", NULL, 1, &replacement_meta),
              PMM_STORE_ERR);

    pmm_coverage_row_t *rows = NULL;
    int count = 0;
    ASSERT_EQ(pmm_store_coverage_get(s, "coverage-invalid", &rows, &count), PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(rows[0].rel_path, "kept.c");
    ASSERT_STR_EQ(rows[0].detail, "3-4");
    pmm_store_free_coverage(rows, count);
    pmm_coverage_meta_t got = {0};
    ASSERT_EQ(pmm_store_coverage_meta_get(s, "coverage-invalid", &got), PMM_STORE_OK);
...
TEST(store_coverage_replace_rolls_back_when_shadow_rebuild_fails) {
    pmm_store_t *s = pmm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_upsert_project(s, "coverage-shadow", "/tmp/coverage-shadow"), PMM_STORE_OK);
    ASSERT_EQ(pmm_store_upsert_file_hash(s, "coverage-shadow", "old.c", "", 1, 1), PMM_STORE_OK);
    ASSERT_EQ(pmm_store_upsert_file_hash(s, "coverage-shadow", "new.c", "", 2, 2), PMM_STORE_OK);

    pmm_coverage_row_t old_row = {
        .rel_path = "old.c", .kind = "parse_partial", .detail = "old-detail"};
    pmm_coverage_meta_t old_meta = {
    ...
    ASSERT_EQ(pmm_store_coverage_replace_ex(s, "coverage-shadow", &old_row, 1, &old_meta),
              PMM_STORE_OK);
    int old_shadow_nodes = pmm_store_count_nodes(s, "coverage-shadow::missed");
    ...
    ASSERT_EQ(pmm_store_exec(s, "CREATE TRIGGER fail_missed_insert BEFORE INSERT ON nodes "
                                "WHEN NEW.project = 'coverage-shadow::missed' "
                                "BEGIN SELECT RAISE(ABORT, 'forced missed shadow failure'); END;"),
              PMM_STORE_OK);

    pmm_coverage_row_t new_row = {
        .rel_path = "new.c", .kind = "parse_partial", .detail = "new-detail"};
    pmm_coverage_meta_t new_meta = old_meta;
    new_meta.generation = "new-generation";
    ASSERT_EQ(pmm_store_coverage_replace_ex(s, "coverage-shadow", &new_row, 1, &new_meta),
              PMM_STORE_ERR);

    pmm_coverage_row_t *rows = NULL;
    int count = 0;
    ASSERT_EQ(pmm_store_coverage_get(s, "coverage-shadow", &rows, &count), PMM_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    pmm_coverage_meta_t got = {0};
    ASSERT_EQ(pmm_store_coverage_meta_get(s, "coverage-shadow", &got), PMM_STORE_OK);
    ASSERT_STR_EQ(got.generation, "old-generation");
    pmm_store_coverage_meta_clear(&got);
    ASSERT_EQ(pmm_store_count_nodes(s, "coverage-shadow::missed"), old_shadow_nodes);
