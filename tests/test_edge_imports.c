 ...
 *   pmm_store_count_edges_by_type(store, project, "IMPORTS") >= N
 *
...
static pmm_store_t *ei_index_files(EILangProj *lp, const EILangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_ei_XXXXXX");
...
static pmm_store_t *ei_index_fixture_files(EILangProj *lp, const char *fixture_root,
...
static int64_t ei_node_id_for_file_label(pmm_store_t *store, const char *project,
                                         const char *file_path, const char *label) {
    pmm_node_t *nodes = NULL;
    int count = 0;
    if (pmm_store_find_nodes_by_file(store, project, file_path, &nodes, &count) != PMM_STORE_OK) {
        return 0;
    }
    int64_t id = 0;
...
static void ei_cleanup(EILangProj *lp, pmm_store_t *store) {
...
static int ei_edge_present(const EILangFile *files, int nfiles, const char *edge_type, int floor) {
    EILangProj lp;
    pmm_store_t *store = ei_index_files(&lp, files, nfiles);
    int got = store ? pmm_store_count_edges_by_type(store, lp.project, edge_type) : -1;
    if (got < floor) {
...
TEST(ei_cpp_header_include_targets_header_file) {
    ...
    EILangProj lp;
    pmm_store_t *store = ei_index_fixture_files(&lp, "tests/fixtures/cpp_include",
                                                fixture_files,
    ...
    int edge_count = 0;
    int rc = pmm_store_find_edges_by_source_type(store, main_id, "IMPORTS", &edges, &edge_count);
    ...
    for (int i = 0; i < edge_count; i++) {
        pmm_node_t *target = (pmm_node_t *)calloc(1, sizeof(pmm_node_t));
        
        /* Pass target directly (no &) because it is already a pointer */
        ASSERT_EQ(pmm_store_find_node_by_id(store, edges[i].target_id, target), PMM_STORE_OK);
