...
#define CTE_BEGIN "<!-- project-mind-mcp:start -->"
#define CTE_END "<!-- project-mind-mcp:end -->"
...
void pmm_text_set_prepublish_hook_for_testing(pmm_text_precommit_test_hook_t hook, void *context);
void pmm_text_set_temp_closed_hook_for_testing(pmm_text_precommit_test_hook_t hook, void *context);
int pmm_text_write_owned_document_if_unchanged(const char *file_path, const char *owned_content,
                                               const char *expected_content,
                                               size_t expected_length);

static int cte_fixture(char *dir, size_t dir_size, char *path, size_t path_size) {
    char *created = th_mktempdir("pmm_text_edit");
    if (!created) {
        return -1;
    }
...
static size_t cte_temp_count(const char *dir) {
    pmm_dir_t *directory = pmm_opendir(dir);
    if (!directory) {
        return SIZE_MAX;
    }
    size_t count = 0U;
    ...
    while ((entry = pmm_readdir(directory)) != NULL) {
        if (strstr(entry->name, ".pmm-text-") != NULL) {
            count++;
        }
    }
...
static void cte_replace_closed_temp(const char *path, void *context) {
    cte_precommit_change_t *change = (cte_precommit_change_t *)context;
    if (!change->backup_path || pmm_rename_replace(path, change->backup_path) != 0) {
        change->result = -1;
        return;
    }
    change->result = th_write_file(path, change->content);
...
TEST(config_text_managed_insert_preserves_bom_comments_and_is_idempotent) {
    ...
    ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "Use search_graph first.\n"),
              0);
...
TEST(config_text_managed_replace_preserves_crlf_surroundings) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(cte_write_bytes(path, original, sizeof(original) - 1U), 0);
    ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "new one\nnew two"), 0);
    ASSERT(cte_assert_bytes(path, expected, sizeof(expected) - 1U));
    th_cleanup(dir);
    PASS();
}
...
TEST(config_text_managed_no_final_newline_round_trip) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(cte_write_bytes(path, original, sizeof(original) - 1U), 0);
    ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned"), 0);
    ASSERT(cte_assert_bytes(path, installed, sizeof(installed) - 1U));
    ASSERT_EQ(pmm_text_remove_managed_block(path, CTE_BEGIN, CTE_END), 0);
...
TEST(config_text_managed_malformed_markers_fail_closed) {
    ...
    for (size_t i = 0U; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "new"), -1);
        ASSERT(cte_assert_bytes(path, cases[i], len));
        ASSERT_EQ(pmm_text_remove_managed_block(path, CTE_BEGIN, CTE_END), -1);
        ASSERT(cte_assert_bytes(path, cases[i], len));
    }
    th_cleanup(dir);
    PASS();
...
TEST(config_text_managed_rejects_unsafe_markers_and_owned_content) {
    ...
    ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "nested " CTE_BEGIN), -1);
    ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "nested " CTE_END), -1);
    ASSERT_EQ(pmm_text_upsert_managed_block(path, "same", "same", "owned"), -1);
    ASSERT_EQ(pmm_text_upsert_managed_block(path, "bad\nmarker", CTE_END, "owned"), -1);
    ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "bad\x01text"), -1);
...
TEST(config_text_owned_document_write_remove_exact_only) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(pmm_text_write_owned_document(path, owned), 0);
    ASSERT_EQ(pmm_text_create_owned_document(path, owned), -1);
    ASSERT_EQ(pmm_text_ensure_owned_document(path, owned), 0);
    ASSERT(cte_assert_bytes(path, owned, sizeof(owned) - 1U));
    ASSERT_EQ(pmm_text_write_owned_document(path, owned), 0);
    ASSERT_EQ(th_write_file(path, "# User changed this\n"), 0);
    ASSERT_EQ(pmm_text_ensure_owned_document(path, owned), -1);
    ASSERT_EQ(pmm_text_remove_owned_document(path, owned), 1);
    ASSERT(cte_assert_bytes(path, "# User changed this\n", strlen("# User changed this\n")));
    ASSERT_EQ(pmm_text_write_owned_document(path, owned), 0);
    ASSERT_EQ(pmm_text_remove_owned_document(path, owned), 0);
    ASSERT(!cte_path_exists(path));
    ASSERT_EQ(pmm_text_ensure_owned_document(path, owned), 0);
    ASSERT(cte_assert_bytes(path, owned, sizeof(owned) - 1U));
    ASSERT_EQ(pmm_text_remove_owned_document(path, owned), 0);
    ASSERT_EQ(pmm_text_create_owned_document(path, owned), 0);
    ASSERT(cte_assert_bytes(path, owned, sizeof(owned) - 1U));
    ASSERT_EQ(pmm_text_remove_owned_document(path, owned), 0);
    ASSERT_EQ(pmm_text_remove_owned_document(path, owned), 0);
    th_cleanup(dir);
...
TEST(config_text_owned_document_migrates_exact_releases_only) {
    char dir[CTE_PATH_CAP];
    char path[CTE_PATH_CAP];
    const char *current = "name: project-mind\ntier: verify\n";
    const char *released[] = {"name: project-mind\n", "name: project-mind\nlegacy: 2\n"};
    const char *modified = "name: project-mind\nuser-note: keep\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);

    ASSERT_EQ(pmm_text_migrate_owned_document(path, current, released, 2U), 0);
    ...
    ASSERT_EQ(pmm_text_migrate_owned_document(path, current, released, 2U), 1);
    ASSERT(cte_assert_bytes(path, modified, strlen(modified)));
    ASSERT_EQ(pmm_text_migrate_owned_document_mode(path, current, released, 2U, 04755U), -1);
    ASSERT(cte_assert_bytes(path, modified, strlen(modified)));
    ASSERT_EQ(pmm_text_migrate_owned_document_mode(path, current, released, 2U, 0200U), -1);
    ASSERT(cte_assert_bytes(path, modified, strlen(modified)));

    ASSERT_EQ(pmm_text_remove_owned_document_any(path, current, released, 2U), 1);
    ASSERT(cte_assert_bytes(path, modified, strlen(modified)));
    ASSERT_EQ(th_write_file(path, released[0]), 0);
    ASSERT_EQ(pmm_text_remove_owned_document_any(path, current, released, 2U), 0);
...
TEST(config_text_rejects_invalid_utf8_and_controls) {
    ...
    ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned"), -1);
    ...
    ASSERT_EQ(pmm_text_write_owned_document(path, "replacement\n"), -1);
    ...
    ASSERT_EQ(pmm_text_write_owned_document(path, "bad\x7ftext"), -1);
    th_cleanup(dir);
    PASS();
}
...
TEST(config_text_rejects_non_regular_paths) {
    ...
    char path[CTE_PATH_CAP];
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(pmm_mkdir(path), 0);
    ASSERT_EQ(pmm_text_write_owned_document(path, "owned\n"), -1);
    ASSERT_EQ(pmm_rmdir(path), 0);
#ifndef _WIN32
    ASSERT_EQ(mkfifo(path, 0600), 0);
    ASSERT_EQ(pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned"), -1);
    ASSERT_EQ(pmm_unlink(path), 0);
#endif
    ASSERT_EQ(cte_temp_count(dir), 0U);
    th_cleanup(dir);
    PASS();
...
TEST(config_text_rejects_links_privileged_mode_and_preserves_metadata) {
    ...
    ASSERT_EQ(pmm_text_write_owned_document(path, "owned\n"), -1);
    ASSERT_EQ(pmm_text_migrate_owned_document_mode(path, "owned\n", NULL, 0U, 0755U), -1);
    ASSERT_EQ(pmm_unlink(path), 0);
...
TEST(config_text_owned_document_mode_publishes_exact_bytes_atomically) {
    ...
    ASSERT_EQ(pmm_text_migrate_owned_document_mode(path, current, released, 1U, 0755U), 0);
    struct stat state;
    ASSERT_EQ(stat(path, &state), 0);
    ...
    ASSERT_EQ(chmod(path, 0640), 0);
    ASSERT_EQ(pmm_text_migrate_owned_document_mode(path, current, released, 1U, 0755U), 1);
    ASSERT_EQ(stat(path, &state), 0);
...
TEST(config_text_owned_document_mode_rejects_prepublish_replacement) {
    ...
    pmm_text_set_prepublish_hook_for_testing(cte_change_before_commit, &race);
    int result = pmm_text_migrate_owned_document_mode(path, current, NULL, 0U, 0755U);
    pmm_text_set_prepublish_hook_for_testing(NULL, NULL);
...
TEST(config_text_owned_document_mode_rejects_closed_temp_replacement) {
    ...
    pmm_text_set_temp_closed_hook_for_testing(cte_replace_closed_temp, &race);
    int result = pmm_text_migrate_owned_document_mode(path, current, NULL, 0U, 0755U);
    pmm_text_set_temp_closed_hook_for_testing(NULL, NULL);
...
TEST(config_text_rejects_stale_content_and_identity) {
    ...
    pmm_text_set_precommit_hook_for_testing(cte_change_before_commit, &content_change);
    int result = pmm_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned");
    pmm_text_set_precommit_hook_for_testing(NULL, NULL);
    ...
    pmm_text_set_precommit_hook_for_testing(cte_change_before_commit, &identity_change);
    result = pmm_text_write_owned_document(path, "owned\n");
    pmm_text_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_text_missing_target_race_does_not_replace_winner) {
    ...
    cte_precommit_change_t race = {
        .content = "winner\n", .backup_path = NULL, .replace_identity = 0, .result = -1};
    pmm_text_set_precommit_hook_for_testing(cte_change_before_commit, &race);
    int result = pmm_text_write_owned_document(path, "owned\n");
    pmm_text_set_precommit_hook_for_testing(NULL, NULL);
    ASSERT_EQ(race.result, 0);
    ASSERT_EQ(result, -1);
...
TEST(config_text_exact_remove_rechecks_stale_snapshot) {
    ...
    cte_precommit_change_t change = {
        .content = "winner\n", .backup_path = NULL, .replace_identity = 0, .result = -1};
    pmm_text_set_precommit_hook_for_testing(cte_change_before_commit, &change);
    int result = pmm_text_remove_owned_document(path, owned);
    pmm_text_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_text_write_owned_document_if_unchanged_rejects_stale_snapshot) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(th_write_file(path, original), 0);
    ASSERT_EQ(pmm_text_write_owned_document_if_unchanged(path, updated, original, strlen(original)),
              0);
    ...
    ASSERT_EQ(pmm_text_write_owned_document_if_unchanged(path, updated, NULL, 0U), -1);
    ASSERT(cte_assert_bytes(path, winner, strlen(winner)));
    ASSERT_EQ(cte_temp_count(dir), 0U);
    th_cleanup(dir);
    PASS();
