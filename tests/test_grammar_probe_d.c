 ...
 *
 * SKIPPED (with reason):
 *   systemverilog — PMM_LANG_SYSTEMVERILOG has no extension entry in the
 *     EXT_TABLE (language.c); the only SV extension ".sv" maps to
 *     PMM_LANG_VERILOG.  File-based index_repository cannot route to
 *     PMM_LANG_SYSTEMVERILOG via extension alone.  It is already exercised
...
static pmm_store_t *gpd_open_indexed(GpdProj *lp) {
...
static pmm_store_t *gpd_index_files(GpdProj *lp, const GpdFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_gpd_XXXXXX");
...
static void gpd_cleanup(GpdProj *lp, pmm_store_t *store) {
...
static int gpd_count_label(pmm_store_t *store, const char *project, const char *label) {
...
static int gpd_type_nodes(pmm_store_t *store, const char *project) {
...
static GpdMetrics gpd_metrics_files(const GpdFile *files, int nfiles) {
    GpdProj lp;
    pmm_store_t *store = gpd_index_files(&lp, files, nfiles);
    GpdMetrics m = {0};
    if (store) {
        m.ok          = 1;
        m.total_nodes = pmm_store_count_nodes(store, lp.project);
        ...
        m.inherits    = pmm_store_count_edges_by_type(store, lp.project, "INHERITS");
    }
 ...
 * Extension: .bicep → PMM_LANG_BICEP.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .cfc → PMM_LANG_CFSCRIPT.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Spec: PMM_LANG_JANET uses empty_types for all definition types — no function
 *       extraction is configured; the grammar emits only a source/Module node.
 * Extension: .janet → PMM_LANG_JANET.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .m → disambiguated to PMM_LANG_OBJC when @interface/@implementation
 *             markers are present (via pmm_disambiguate_m).
 * ══════════════════════════════════════════════════════════════════ */
...
TEST(probe_objc_method_nodes) {
     ...
     *        sets def.label = "Method" for PMM_LANG_OBJC method_definition),
 ...
 * Extension: .purs → PMM_LANG_PURESCRIPT.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .smali → PMM_LANG_SMALI.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .td → PMM_LANG_TABLEGEN.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .tla → PMM_LANG_TLAPLUS.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .v → PMM_LANG_VERILOG.
 * Note: .sv also maps to PMM_LANG_VERILOG (not PMM_LANG_SYSTEMVERILOG).
 * ══════════════════════════════════════════════════════════════════ */
...
/* Verilog: SystemVerilog-style file (.sv) — lands on PMM_LANG_VERILOG via EXT_TABLE.
 * No separate PMM_LANG_SYSTEMVERILOG routing is available through file extension. */
TEST(probe_verilog_sv_extension) {
    GpdMetrics m = gpd_metrics("adder.sv",
        ...
        "endmodule\n");
    ASSERT_TRUE(m.ok);
    /* GREEN: .sv → PMM_LANG_VERILOG; module_declaration must produce type-like node. */
    ASSERT_TRUE(m.types >= 1);
    PASS();
}
 ...
 * Extension: .vhd / .vhdl → PMM_LANG_VHDL.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .wl / .wls → PMM_LANG_WOLFRAM.
 * ══════════════════════════════════════════════════════════════════ */
...
SUITE(grammar_probe_d) {
    ...
    /* Verilog (.v — also covers .sv routed as PMM_LANG_VERILOG) */
