/*
 * test_route_canon.c — Unit tests for pmm_route_canon_path().
 *
...
TEST(route_canon_static_unchanged) {
    char b[128];
    ASSERT_STR_EQ(pmm_route_canon_path("/products/categories", b, sizeof(b)),
                  "/products/categories");
    PASS();
}
...
TEST(route_canon_colon_param) {
    char b[128];
    ASSERT_STR_EQ(pmm_route_canon_path("/players/:id", b, sizeof(b)), "/players/{}");
    PASS();
}
...
TEST(route_canon_colon_and_brace_converge) {
    char a[128];
    char c[128];
    pmm_route_canon_path("/clients/{id}/authorized-users", a, sizeof(a));
    pmm_route_canon_path("/clients/:clientId/authorized-users", c, sizeof(c));
    ASSERT_STR_EQ(a, c);
...
TEST(route_canon_param_name_agnostic) {
    char a[128];
    char c[128];
    pmm_route_canon_path("/link-requests/{id}/status", a, sizeof(a));
    pmm_route_canon_path("/link-requests/:requestId/status", c, sizeof(c));
    ASSERT_STR_EQ(a, c);
    PASS();
}
...
TEST(route_canon_angle_param) {
    char b[128];
    ASSERT_STR_EQ(pmm_route_canon_path("/users/<int:id>", b, sizeof(b)), "/users/{}");
    PASS();
}

TEST(route_canon_template_interpolation) {
    char b[128];
    ASSERT_STR_EQ(pmm_route_canon_path("/players/${playerId}", b, sizeof(b)), "/players/{}");
    PASS();
}

TEST(route_canon_multiple_params) {
    char b[128];
    ASSERT_STR_EQ(pmm_route_canon_path("/orders/{id}/items/{itemIndex}", b, sizeof(b)),
                  "/orders/{}/items/{}");
    PASS();
}
...
TEST(route_canon_colon_mid_segment_is_literal) {
    char b[128];
    ASSERT_STR_EQ(pmm_route_canon_path("/a/b:c", b, sizeof(b)), "/a/b:c");
    PASS();
}

TEST(route_canon_null_and_empty) {
    char b[8];
    ASSERT_STR_EQ(pmm_route_canon_path("", b, sizeof(b)), "");
    ASSERT_STR_EQ(pmm_route_canon_path(NULL, b, sizeof(b)), "");
    PASS();
}
...
TEST(route_canon_truncation_safe) {
    char b[6];
    const char *r = pmm_route_canon_path("/players/:id", b, sizeof(b));
