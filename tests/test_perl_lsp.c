 ...
 * QN is `test.main` (from the pmm_extract_file "test"/"main.pl" args), so every
...
static CBMFileResult *extract_perl(const char *source) {
    return pmm_extract_file(source, (int)strlen(source), PMM_LANG_PERL, "test", "main.pl", 0, NULL,
                            NULL);
}
...
TEST(perllsp_cpan_exported_function) {
     ...
     * pmm_perl_stdlib_register as "Scalar.Util.blessed". The bare call must
