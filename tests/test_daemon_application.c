...
typedef struct {
    ...
    pmm_project_lock_manager_t *external_locks;
    pmm_project_lock_manager_t *daemon_locks;
    pmm_project_lock_manager_t *worker_locks;
    pmm_project_lock_manager_t *probe_locks;
...
static bool app_project_lock_fixture_cleanup(app_project_lock_fixture_t *fixture) {
    ...
    if (fixture->daemon_locks &&
        pmm_project_lock_manager_free(&fixture->daemon_locks) != PMM_PRIVATE_FILE_LOCK_OK) {
        clean = false;
    }
    if (fixture->external_locks &&
        pmm_project_lock_manager_free(&fixture->external_locks) != PMM_PRIVATE_FILE_LOCK_OK) {
        clean = false;
    }
    if (fixture->worker_locks &&
        pmm_project_lock_manager_free(&fixture->worker_locks) != PMM_PRIVATE_FILE_LOCK_OK) {
        clean = false;
    }
    if (fixture->probe_locks &&
        pmm_project_lock_manager_free(&fixture->probe_locks) != PMM_PRIVATE_FILE_LOCK_OK) {
        clean = false;
    }
    if (!fixture->daemon_locks && !fixture->external_locks && !fixture->worker_locks &&
        !fixture->probe_locks) {
        pmm_daemon_ipc_endpoint_free(fixture->endpoint);
...
static bool app_project_lock_fixture_init(app_project_lock_fixture_t *fixture) {
    ...
    fixture->endpoint = pmm_daemon_ipc_endpoint_new("fedcba9876543210", fixture->runtime_parent);
    if (fixture->endpoint) {
        fixture->external_locks = pmm_project_lock_manager_new(fixture->endpoint);
        fixture->daemon_locks = pmm_project_lock_manager_new(fixture->endpoint);
        fixture->worker_locks = pmm_project_lock_manager_new(fixture->endpoint);
        fixture->probe_locks = pmm_project_lock_manager_new(fixture->endpoint);
    }
...
static bool app_project_lock_release(pmm_project_lock_lease_t **lease) {
    if (!lease) {
        return false;
    }
    uint64_t deadline = pmm_now_ms() + APP_TEST_TIMEOUT_MS;
    while (*lease && pmm_now_ms() < deadline) {
...
static pmm_private_file_lock_status_t app_project_lock_try_acquire_until_released(
    pmm_project_lock_manager_t *manager, const char *project,
    pmm_project_lock_lease_t **lease_out) {
    if (!lease_out) {
        return PMM_PRIVATE_FILE_LOCK_UNSAFE;
    }
    *lease_out = NULL;
    uint64_t deadline = pmm_now_ms() + APP_TEST_TIMEOUT_MS;
    pmm_private_file_lock_status_t status = PMM_PRIVATE_FILE_LOCK_BUSY;
    do {
        status = pmm_project_lock_try_acquire(manager, project, lease_out);
...
static pmm_daemon_runtime_application_session_t *app_test_open(
    const pmm_daemon_runtime_application_callbacks_t *callbacks, pmm_daemon_client_id_t client_id) {
...
static pmm_daemon_runtime_application_status_t app_test_request_tagged(
    const pmm_daemon_runtime_application_callbacks_t *callbacks,
    pmm_daemon_runtime_application_session_t *session,
...
static pmm_daemon_runtime_application_status_t app_test_request(
    const pmm_daemon_runtime_application_callbacks_t *callbacks,
    pmm_daemon_runtime_application_session_t *session, const uint8_t *request,
    uint32_t request_length, uint8_t **response_out, uint32_t *response_length_out) {
...
static bool app_test_context_request_options(const char *root, const char *allowed,
    ...
    request[0] = PMM_DAEMON_APPLICATION_REQUEST_SET_CONTEXT;
...
static bool app_test_context_request(const char *root, const char *allowed, uint8_t **request_out,
                                     uint32_t *length_out) {
    return app_test_context_request_options(root, allowed, PMM_MCP_TOOL_PROFILE_ALL, NULL, NULL,
                                            request_out, length_out);
}

static bool app_test_text_request(pmm_daemon_application_request_kind_t kind, const char *text,
                                  uint8_t **request_out, uint32_t *length_out) {
...
static bool app_test_tool_request(const char *tool, const char *args, uint8_t **request_out,
    ...
    request[0] = PMM_DAEMON_APPLICATION_REQUEST_TOOL;
...
static pmm_daemon_runtime_application_status_t app_test_ui_config_request(
    const pmm_daemon_runtime_application_callbacks_t *callbacks,
    pmm_daemon_runtime_application_session_t *session, uint8_t mask, uint8_t enabled, uint32_t port,
    uint32_t request_length) {
    uint8_t request[7] = {
        PMM_DAEMON_APPLICATION_REQUEST_SET_UI_CONFIG,
        mask,
        enabled,
        (uint8_t)(port >> 24),
        (uint8_t)(port >> 16),
    ...
    pmm_daemon_runtime_application_status_t status =
        app_test_request(callbacks, session, request, request_length, &response, &response_length);
    if (response || response_length != 0) {
        free(response);
        return PMM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
    }
    return status;
}

TEST(daemon_application_new_session_does_not_retain_initial_store) {
    pmm_daemon_application_t *application = pmm_daemon_application_new(NULL);
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 300);

    bool retains_store = session && pmm_daemon_application_session_retains_store_for_test(session);

    if (session) {
        callbacks.session_close(callbacks.context, session);
    }
    bool stopped = application && pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
...
TEST(daemon_application_request_cancel_is_scoped_to_exact_token) {
    ...
    (void)snprintf(root, sizeof(root), "%s/pmm-app-request-cancel-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    pmm_daemon_application_t *application = pmm_daemon_application_new(NULL);
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 301);
    uint8_t *request = NULL;
    uint32_t request_length = 0;
    bool request_created =
        root_ok && app_test_context_request(root, root, &request, &request_length);
    pmm_daemon_runtime_application_token_t cancelled_token = UINT64_C(7001);
    pmm_daemon_runtime_application_token_t next_token = UINT64_C(7002);
    uint8_t *cancelled_response = NULL;
    uint32_t cancelled_response_length = 0;
    pmm_daemon_runtime_application_status_t cancelled_status =
        PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    uint8_t *next_response = NULL;
    uint32_t next_response_length = 0;
    pmm_daemon_runtime_application_status_t next_status =
        PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    ...
    ASSERT_EQ(cancelled_status, PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED);
    ASSERT_EQ(cancelled_response_length, 0);
    ASSERT_EQ(next_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_ui_config_updates_are_masked_and_serialized) {
    ...
    (void)snprintf(cache, sizeof(cache), "%s/pmm-app-ui-config-XXXXXX", pmm_tmpdir());
    bool cache_ok = pmm_mkdtemp(cache) != NULL;
    char *old_cache = getenv("PMM_CACHE_DIR") ? strdup(getenv("PMM_CACHE_DIR")) : NULL;
    bool env_ok = cache_ok && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    pmm_ui_config_t initial = {.ui_enabled = false, .ui_port = 9749};
    bool initial_saved = env_ok && pmm_ui_config_save(&initial);

    pmm_daemon_application_t *application = pmm_daemon_application_new(NULL);
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *enabled_session = app_test_open(&callbacks, 301);
    pmm_daemon_runtime_application_session_t *port_session = app_test_open(&callbacks, 302);
    ...
    bool contexts_set =
        ...
        app_test_request(&callbacks, enabled_session, context, context_length, &context_response,
                         &context_response_length) == PMM_DAEMON_RUNTIME_APPLICATION_OK;
    ...
    pmm_daemon_runtime_application_status_t enabled_status = app_test_ui_config_request(
        &callbacks, enabled_session, PMM_DAEMON_APPLICATION_UI_CONFIG_ENABLED, 1, 0, 7);
    pmm_daemon_runtime_application_status_t port_status = app_test_ui_config_request(
        &callbacks, port_session, PMM_DAEMON_APPLICATION_UI_CONFIG_PORT, 0, 18432, 7);

    pmm_ui_config_t updated = {0};
    pmm_ui_config_load(&updated);
    ...
    free(context);
    if (old_cache) {
        (void)pmm_setenv("PMM_CACHE_DIR", old_cache, 1);
    } else {
        (void)pmm_unsetenv("PMM_CACHE_DIR");
    }
    free(old_cache);
    ...
    ASSERT_EQ(enabled_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_EQ(port_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_ui_config_rejects_noncanonical_frames) {
    char cache[APP_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/pmm-app-ui-frame-XXXXXX", pmm_tmpdir());
    bool cache_ok = pmm_mkdtemp(cache) != NULL;
    char *old_cache = getenv("PMM_CACHE_DIR") ? strdup(getenv("PMM_CACHE_DIR")) : NULL;
    bool env_ok = cache_ok && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    pmm_ui_config_t initial = {.ui_enabled = false, .ui_port = 9749};
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 303);
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    uint8_t *context_response = NULL;
    uint32_t context_response_length = 0;
    ...
    pmm_daemon_runtime_application_status_t short_frame = app_test_ui_config_request(
        &callbacks, session, PMM_DAEMON_APPLICATION_UI_CONFIG_ENABLED, 1, 0, 6);
    pmm_daemon_runtime_application_status_t empty_mask =
        app_test_ui_config_request(&callbacks, session, 0, 0, 0, 7);
    pmm_daemon_runtime_application_status_t unknown_mask =
        app_test_ui_config_request(&callbacks, session, 0x80, 0, 0, 7);
    pmm_daemon_runtime_application_status_t invalid_boolean = app_test_ui_config_request(
        &callbacks, session, PMM_DAEMON_APPLICATION_UI_CONFIG_ENABLED, 2, 0, 7);
    pmm_daemon_runtime_application_status_t invalid_port = app_test_ui_config_request(
        &callbacks, session, PMM_DAEMON_APPLICATION_UI_CONFIG_PORT, 0, 65536, 7);
    pmm_daemon_runtime_application_status_t nonzero_unused = app_test_ui_config_request(
        &callbacks, session, PMM_DAEMON_APPLICATION_UI_CONFIG_ENABLED, 1, 9749, 7);

    pmm_ui_config_t unchanged = {0};
    pmm_ui_config_load(&unchanged);
    if (session) {
        callbacks.session_close(callbacks.context, session);
    }
    bool stopped = application && pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    ...
    ASSERT_EQ(short_frame, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
    ASSERT_EQ(empty_mask, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
    ASSERT_EQ(unknown_mask, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
    ASSERT_EQ(invalid_boolean, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
    ASSERT_EQ(invalid_port, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
    ASSERT_EQ(nonzero_unused, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
...
TEST(daemon_application_requires_immutable_explicit_context) {
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 1);
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-context-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    uint8_t *mcp = NULL;
    ...
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    bool mcp_ok = app_test_text_request(PMM_DAEMON_APPLICATION_REQUEST_MCP,
                                        "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"ping\"}", &mcp,
                                        &mcp_length);
    pmm_daemon_runtime_application_status_t before =
    ...
    pmm_daemon_runtime_application_status_t first =
        context_ok ? app_test_request(&callbacks, session, context, context_length, &response,
                                      &response_length)
                   : PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    free(response);
    response = NULL;
    pmm_daemon_runtime_application_status_t repeated =
        context_ok ? app_test_request(&callbacks, session, context, context_length, &response,
                                      &response_length)
                   : PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    free(response);
    response = NULL;
    pmm_daemon_runtime_application_status_t after =
        app_test_request(&callbacks, session, mcp, mcp_length, &response, &response_length);
    bool ping_response =
        response && response_length > 0 && strstr((const char *)response, "\"result\":{}") != NULL;

    callbacks.session_close(callbacks.context, session);
    (void)pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    pmm_daemon_application_free(application);
    free(mcp);
    free(context);
    free(response);
    ...
    ASSERT_TRUE(mcp_ok);
    ASSERT_EQ(before, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
    ASSERT_EQ(first, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_EQ(repeated, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
    ASSERT_EQ(after, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_mcp_notification_has_no_response) {
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 2);
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-notify-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    ...
    bool encoded =
        ...
        app_test_text_request(PMM_DAEMON_APPLICATION_REQUEST_MCP,
                              "{\"jsonrpc\":\"2.0\",\"method\":\"notifications/initialized\"}",
                              &notification, &notification_length);
    pmm_daemon_runtime_application_status_t context_status =
    ...
    pmm_daemon_runtime_application_status_t notification_status =
    ...
    ASSERT_TRUE(encoded);
    ASSERT_EQ(context_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_EQ(notification_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
static bool app_test_create_empty_file(const char *path) {
    FILE *file = path ? pmm_fopen(path, "wb") : NULL;
...
static int app_test_git(const char *root, const char *operation, const char *argument_one,
    ...
    pmm_proc_opts_t options = {0};
    pmm_proc_result_t result = {0};
    options.bin = git;
    options.argv = argv;
    options.quiet_timeout_ms = 10000;
    return pmm_subprocess_run(&options, &result) == 0 && result.outcome == PMM_PROC_CLEAN ? 0 : -1;
}
...
TEST(daemon_application_restricted_profile_owns_no_background_surfaces) {
    const char *old_cache = getenv("PMM_CACHE_DIR");
    bool had_cache = old_cache != NULL;
    char *saved_cache = old_cache ? pmm_strdup(old_cache) : NULL;
    char root[APP_TEST_PATH_CAP];
    char cache[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-profile-root-XXXXXX", pmm_tmpdir());
    (void)snprintf(cache, sizeof(cache), "%s/pmm-app-profile-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(root) != NULL && pmm_mkdtemp(cache) != NULL;
    bool env_ok =
        dirs_ok && (!had_cache || saved_cache) && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    pmm_ui_config_t initial_ui = {.ui_enabled = false, .ui_port = 9749};
    bool ui_ready = env_ok && pmm_ui_config_save(&initial_ui);
    char *project = dirs_ok ? pmm_project_name_from_path(root) : NULL;
    ...
    pmm_watcher_t *watcher = pmm_watcher_new(store, app_test_index_noop, NULL);
    pmm_daemon_application_config_t config = {.watcher = watcher};
    pmm_daemon_application_t *application = pmm_daemon_application_new(&config);
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 304);
    ...
    bool encoded =
        db_ok && session &&
        app_test_context_request_options(root, root, PMM_MCP_TOOL_PROFILE_SCOUT, NULL, NULL,
                                         &context, &context_length) &&
        app_test_text_request(
            PMM_DAEMON_APPLICATION_REQUEST_MCP,
    ...
    free(response);
    response = NULL;
    pmm_daemon_runtime_application_status_t initialize_status =
        context_status == PMM_DAEMON_RUNTIME_APPLICATION_OK
    ...
    free(response);
    response = NULL;
    pmm_daemon_runtime_application_status_t index_status =
        initialize_status == PMM_DAEMON_RUNTIME_APPLICATION_OK
    ...
    free(response);
    response = NULL;
    pmm_daemon_runtime_application_status_t ui_status = app_test_ui_config_request(
        &callbacks, session, PMM_DAEMON_APPLICATION_UI_CONFIG_ENABLED, 1, 0, 7);
    int watch_count = watcher ? pmm_watcher_watch_count(watcher) : -1;
    size_t active_jobs = application ? pmm_daemon_application_active_jobs(application) : SIZE_MAX;
    pmm_ui_config_t final_ui = {0};
    pmm_ui_config_load(&final_ui);
    ...
    (void)pmm_rmdir(root);
    if (had_cache) {
        (void)pmm_setenv("PMM_CACHE_DIR", saved_cache, 1);
    } else {
        (void)pmm_unsetenv("PMM_CACHE_DIR");
    }
    free(saved_cache);
    ...
    ASSERT_TRUE(encoded);
    ASSERT_EQ(context_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_EQ(initialize_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_TRUE(scout_surface);
    ASSERT_EQ(index_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_TRUE(index_refused);
    ASSERT_EQ(ui_status, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
...
TEST(daemon_application_hook_context_preserves_event_and_dialect) {
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-hook-context-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    pmm_daemon_application_t *application = pmm_daemon_application_new(NULL);
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 305);
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    ...
    bool encoded =
        root_ok && session &&
        app_test_context_request_options(root, root, PMM_MCP_TOOL_PROFILE_ALL, "SessionStart",
                                         "copilot", &context, &context_length) &&
        app_test_text_request(PMM_DAEMON_APPLICATION_REQUEST_HOOK_AUGMENT, input, &hook,
                              &hook_length);
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    pmm_daemon_runtime_application_status_t context_status =
    ...
    free(response);
    response = NULL;
    pmm_daemon_runtime_application_status_t hook_status =
        context_status == PMM_DAEMON_RUNTIME_APPLICATION_OK
    ...
    ASSERT_TRUE(encoded);
    ASSERT_EQ(context_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_EQ(hook_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_reference_counts_one_shared_watch) {
    ...
    char root[APP_TEST_PATH_CAP];
    char cache[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-watch-root-XXXXXX", pmm_tmpdir());
    snprintf(cache, sizeof(cache), "%s/pmm-app-watch-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(root) != NULL && pmm_mkdtemp(cache) != NULL;
    bool env_ok = saved_cache_ok && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    char *project = dirs_ok ? pmm_project_name_from_path(root) : NULL;
    char db_path[APP_TEST_PATH_CAP] = {0};
    FILE *db = NULL;
    if (project) {
        snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project);
        db = pmm_fopen(db_path, "wb");
    }
    bool db_ok = db && fclose(db) == 0;
    pmm_store_t *store = pmm_store_open_memory();
    pmm_watcher_t *watcher = pmm_watcher_new(store, app_test_index_noop, NULL);
    pmm_daemon_application_config_t config = {.watcher = watcher, .config = NULL};
    pmm_daemon_application_t *application = pmm_daemon_application_new(&config);
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *first_session = app_test_open(&callbacks, 10);
    pmm_daemon_runtime_application_session_t *second_session = app_test_open(&callbacks, 11);
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    uint8_t *ping = NULL;
...
TEST(daemon_application_free_releases_live_watch_once) {
    ...
    bool had_cache = old_cache != NULL;
    char *saved_cache = old_cache ? pmm_strdup(old_cache) : NULL;
    const char *old_grace = getenv("PMM_WATCHER_PRUNE_GRACE_S");
    bool had_grace = old_grace != NULL;
    char *saved_grace = old_grace ? pmm_strdup(old_grace) : NULL;
    char root[APP_TEST_PATH_CAP];
    char survivor_root[APP_TEST_PATH_CAP];
    char cache[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-free-watch-root-XXXXXX", pmm_tmpdir());
    snprintf(survivor_root, sizeof(survivor_root), "%s/pmm-app-free-survivor-root-XXXXXX",
             pmm_tmpdir());
    snprintf(cache, sizeof(cache), "%s/pmm-app-free-watch-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(root) != NULL && pmm_mkdtemp(survivor_root) != NULL &&
                   pmm_mkdtemp(cache) != NULL;
    bool env_ok = (!had_cache || saved_cache) && (!had_grace || saved_grace) &&
                  pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0 &&
                  pmm_setenv("PMM_WATCHER_PRUNE_GRACE_S", "0", 1) == 0;
    char *project = dirs_ok ? pmm_project_name_from_path(root) : NULL;
    char *survivor_project = dirs_ok ? pmm_project_name_from_path(survivor_root) : NULL;
    char db_path[APP_TEST_PATH_CAP] = {0};
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 12);
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    uint8_t *ping = NULL;
    uint32_t ping_length = 0;
    ...
    requested =
        requested && app_test_request(&callbacks, session, ping, ping_length, &response,
                                      &response_length) == PMM_DAEMON_RUNTIME_APPLICATION_OK;
    bool watch_live = requested && pmm_watcher_watch_count(watcher) == 1;
    ...
    if (watch_live) {
        pmm_watcher_watch(watcher, survivor_project, survivor_root);
    }
    bool survivor_live = watch_live && pmm_watcher_watch_count(watcher) == 2;
    bool survivor_removed = survivor_live && pmm_rmdir(survivor_root) == 0;
    ...
    pmm_daemon_application_free(application);
    bool survivor_still_watched = pmm_watcher_watch_count(watcher) == 1;
    for (int miss = 0; survivor_removed && miss < 3; miss++) {
        pmm_watcher_touch(watcher, survivor_project);
        (void)pmm_watcher_poll_once(watcher);
    }
    bool post_free_poll_safe = survivor_removed && pmm_watcher_watch_count(watcher) == 0 &&
                               !pmm_file_exists(survivor_db_path);
    pmm_watcher_stop(watcher);
    pmm_watcher_free(watcher);
    pmm_store_close(store);
    free(context);
    ...
    (void)pmm_unlink(db_path);
    (void)pmm_unlink(survivor_db_path);
    (void)pmm_rmdir(root);
    (void)pmm_rmdir(survivor_root);
    (void)pmm_rmdir(cache);
    if (saved_cache) {
        (void)pmm_setenv("PMM_CACHE_DIR", saved_cache, 1);
    } else if (!had_cache) {
    ...
    if (saved_grace) {
        (void)pmm_setenv("PMM_WATCHER_PRUNE_GRACE_S", saved_grace, 1);
    } else if (!had_grace) {
        (void)pmm_unsetenv("PMM_WATCHER_PRUNE_GRACE_S");
    }
    free(saved_cache);
    free(saved_grace);
...
TEST(daemon_application_prune_clears_logical_watch_for_reregistration) {
    ...
    char root[APP_TEST_PATH_CAP];
    char cache[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-prune-watch-root-XXXXXX", pmm_tmpdir());
    snprintf(cache, sizeof(cache), "%s/pmm-app-prune-watch-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(root) != NULL && pmm_mkdtemp(cache) != NULL;
    bool env_ok = (!had_cache || saved_cache) && (!had_grace || saved_grace) &&
                  pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0 &&
                  pmm_setenv("PMM_WATCHER_PRUNE_GRACE_S", "0", 1) == 0;
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *first = app_test_open(&callbacks, 13);
    pmm_daemon_runtime_application_session_t *second = NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    uint8_t *ping = NULL;
    uint32_t ping_length = 0;
    ...
    first_registered = first_registered &&
                       app_test_request(&callbacks, first, ping, ping_length, &response,
                                        &response_length) == PMM_DAEMON_RUNTIME_APPLICATION_OK &&
                       pmm_watcher_watch_count(watcher) == 1;
    free(response);
    response = NULL;

    bool root_removed = first_registered && pmm_rmdir(root) == 0;
    for (int miss = 0; root_removed && miss < 3; miss++) {
        pmm_watcher_touch(watcher, project);
        (void)pmm_watcher_poll_once(watcher);
    }
    bool pruned = root_removed && pmm_watcher_watch_count(watcher) == 0 &&
                  !pmm_file_exists(db_path) && !pmm_file_exists(wal_path) &&
                  !pmm_file_exists(shm_path);

    bool restored = pruned && pmm_mkdir_p(root, 0755) && app_test_create_empty_file(db_path);
    if (restored) {
    ...
    bool first_close_kept_reregistered_watch =
        second_registered && pmm_watcher_watch_count(watcher) == 1;
    if (second) {
        callbacks.session_close(callbacks.context, second);
        second = NULL;
    }
    bool final_close_released_watch = pmm_watcher_watch_count(watcher) == 0;
...
typedef struct {
    ...
    pmm_proc_outcome_t outcomes[APP_FAKE_MAX_ATTEMPTS];
...
typedef struct {
    ...
    char *project_key;
    pmm_project_lock_lease_t *project_lock_lease;
...
static int app_fake_worker_start(void *opaque, const char *args_json, size_t memory_budget_bytes,
    ...
    if (context->project_locks) {
        char *repo_path = pmm_mcp_get_string_arg(args_json, "repo_path");
        char *name_override = pmm_mcp_get_string_arg(args_json, "name");
        worker->project_key = pmm_project_name_from_path(
    ...
    if (worker->attempt < APP_FAKE_MAX_ATTEMPTS) {
        ...
        if (marker_file && payload) {
            FILE *marker = pmm_fopen(marker_file, "ab");
            if (marker) {
...
static pmm_index_worker_poll_t app_fake_worker_poll(void *opaque,
    ...
    *result_out = NULL;
    ...
    if (worker->context->project_locks && !worker->project_lock_lease) {
        ...
        pmm_private_file_lock_status_t status = pmm_project_lock_try_acquire(
        ...
        if (status == PMM_PRIVATE_FILE_LOCK_OK && lease) {
            ...
            if (status == PMM_PRIVATE_FILE_LOCK_BUSY &&
                ...
                return PMM_INDEX_WORKER_POLL_RUNNING;
            }
            return PMM_INDEX_WORKER_POLL_ERROR;
        }
    }
    if (atomic_load(&worker->context->scripted)) {
        pmm_proc_outcome_t outcome = worker->attempt < APP_FAKE_MAX_ATTEMPTS
                                         ? worker->context->outcomes[worker->attempt]
                                         : PMM_PROC_SPAWN_FAILED;
        worker->result.outcome = outcome;
        worker->result.exit_code = outcome == PMM_PROC_CLEAN ? 0 : -1;
        worker->result.tree_quiesced = true;
        const char *response =
            worker->attempt < APP_FAKE_MAX_ATTEMPTS && worker->context->responses[worker->attempt]
                ...
                : "{\"content\":[{\"type\":\"text\",\"text\":\"{"
                  "\\\"status\\\":\\\"indexed\\\"}\"}]}";
        worker->result.response = outcome == PMM_PROC_CLEAN ? pmm_strdup(response) : NULL;
        *result_out = &worker->result;
        return PMM_INDEX_WORKER_POLL_TERMINAL;
    }
    if (atomic_load(&worker->context->allow_completion)) {
        worker->result.outcome = PMM_PROC_CLEAN;
        ...
        worker->result.response = pmm_strdup(
...
static bool app_fake_worker_cancel(void *opaque, pmm_daemon_application_worker_t handle) {
    app_fake_worker_context_t *context = opaque;
    app_fake_worker_t *worker = handle;
...
static const char *app_fake_worker_log_path(void *opaque, pmm_daemon_application_worker_t handle) {
    (void)opaque;
    (void)handle;
    return "/tmp/pmm-fake-worker.log";
}

static void app_fake_worker_destroy(void *opaque, pmm_daemon_application_worker_t handle) {
    app_fake_worker_context_t *context = opaque;
    app_fake_worker_t *worker = handle;
    ...
    pmm_index_worker_result_free(&worker->result);
...
static void *app_request_thread(void *opaque) {
    ...
    request->status =
        request->request_token == PMM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID
...
static bool app_wait_for_subscribers(pmm_daemon_application_t *application, const char *project,
                                     size_t expected) {
    uint64_t deadline = pmm_now_ms() + APP_TEST_TIMEOUT_MS;
    while (pmm_now_ms() < deadline) {
        if (pmm_daemon_application_job_subscribers(application, project) == expected) {
            return true;
        }
        pmm_usleep(1000);
    }
...
static bool app_wait_for_background_initializes(int minimum) {
    ...
    uint64_t deadline = pmm_now_ms() + APP_TEST_TIMEOUT_MS;
    while (pmm_now_ms() < deadline) {
        if (pmm_daemon_application_background_initializes_for_test() >= minimum) {
            return true;
        }
        pmm_usleep(1000);
    }
...
static bool app_wait_for_active_jobs(pmm_daemon_application_t *application, size_t expected) {
    uint64_t deadline = pmm_now_ms() + APP_TEST_TIMEOUT_MS;
    while (pmm_now_ms() < deadline) {
        if (pmm_daemon_application_active_jobs(application) == expected) {
            return true;
        }
        pmm_usleep(1000);
    }
...
static void *app_index_thread(void *opaque) {
    ...
    request->result =
        pmm_daemon_application_index(request->application, request->project, request->root);
    return NULL;
}
...
static bool app_watch_race_fixture_init(app_watch_race_fixture_t *fixture,
    ...
    fixture->saved_cache = old_cache ? pmm_strdup(old_cache) : NULL;
    bool saved_cache_ok = !fixture->had_cache || fixture->saved_cache;
    (void)snprintf(fixture->root, sizeof(fixture->root), "%s/pmm-app-watch-race-root-XXXXXX",
                   pmm_tmpdir());
    (void)snprintf(fixture->cache, sizeof(fixture->cache), "%s/pmm-app-watch-race-cache-XXXXXX",
                   pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(fixture->root) != NULL && pmm_mkdtemp(fixture->cache) != NULL;
    bool env_ok = saved_cache_ok && pmm_setenv("PMM_CACHE_DIR", fixture->cache, 1) == 0;
    fixture->project = dirs_ok ? pmm_project_name_from_path(fixture->root) : NULL;
    if (fixture->project) {
    ...
    pmm_daemon_application_worker_ops_t worker_ops = {
    ...
    fixture->store = pmm_store_open_memory();
    fixture->watcher = pmm_watcher_new(fixture->store, app_test_index_noop, NULL);
    pmm_daemon_application_config_t config = {
        .watcher = fixture->watcher,
        .worker_ops = &worker_ops,
    };
    fixture->application = pmm_daemon_application_new(&config);
    fixture->callbacks = pmm_daemon_application_runtime_callbacks(fixture->application);
    ...
    free(context);
    free(ping);
    return initialized && pmm_watcher_watch_count(fixture->watcher) == 1;
}
...
static bool app_watch_race_fixture_finish(app_watch_race_fixture_t *fixture) {
    ...
    bool stopped = !fixture->application ||
                   pmm_daemon_application_shutdown(fixture->application, APP_TEST_TIMEOUT_MS);
    if (stopped) {
        pmm_daemon_application_free(fixture->application);
        fixture->application = NULL;
        pmm_watcher_stop(fixture->watcher);
        pmm_watcher_free(fixture->watcher);
        fixture->watcher = NULL;
        pmm_store_close(fixture->store);
        fixture->store = NULL;
    }
    ...
    bool restored = fixture->saved_cache
                        ? pmm_setenv("PMM_CACHE_DIR", fixture->saved_cache, 1) == 0
                        : (!fixture->had_cache && pmm_unsetenv("PMM_CACHE_DIR") == 0);
...
static void *app_watcher_index_thread(void *opaque) {
    ...
    request->result =
        pmm_daemon_application_watcher_index(request->project, request->root, request->application);
    atomic_store(&request->done, true);
    return NULL;
}
...
static bool app_env_backup_capture(app_env_backup_t *backup, const char *name) {
    ...
    backup->value = value ? pmm_strdup(value) : NULL;
...
static bool app_env_backup_restore(app_env_backup_t *backup) {
    ...
    int status =
        backup->had_value ? pmm_setenv(backup->name, backup->value, 1) : pmm_unsetenv(backup->name);
    free(backup->value);
    backup->value = NULL;
    backup->name = NULL;
    return status == 0;
...
static int app_fake_update_start(void *opaque, pmm_daemon_application_update_worker_t *worker_out) {
...
static pmm_daemon_application_update_poll_t app_fake_update_poll(
    void *opaque, pmm_daemon_application_update_worker_t handle, const char **latest_version_out) {
    (void)opaque;
    app_fake_update_worker_t *worker = handle;
    *latest_version_out = NULL;
    if (atomic_load(&worker->cancelled)) {
        return PMM_DAEMON_APPLICATION_UPDATE_POLL_TERMINAL;
    }
    if (!atomic_load(&worker->context->allow_completion)) {
        return PMM_DAEMON_APPLICATION_UPDATE_POLL_RUNNING;
    }
    *latest_version_out = worker->context->latest_version;
    return PMM_DAEMON_APPLICATION_UPDATE_POLL_TERMINAL;
}

static bool app_fake_update_cancel(void *opaque, pmm_daemon_application_update_worker_t handle) {
    app_fake_update_context_t *context = opaque;
    app_fake_update_worker_t *worker = handle;
...
static void app_fake_update_destroy(void *opaque, pmm_daemon_application_update_worker_t handle) {
    app_fake_update_context_t *context = opaque;
    atomic_fetch_add(&context->destroys, 1);
    free(handle);
}

static pmm_daemon_application_update_ops_t app_fake_update_ops(app_fake_update_context_t *context) {
    return (pmm_daemon_application_update_ops_t){
        .context = context,
...
static bool app_test_initialize_profile(const pmm_daemon_runtime_application_callbacks_t *callbacks,
                                        pmm_daemon_runtime_application_session_t *session,
                                        const char *root, pmm_mcp_tool_profile_t profile,
                                        const char *hook_event, const char *hook_dialect) {
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    uint8_t *initialize = NULL;
    ...
    pmm_daemon_runtime_application_status_t initialize_status =
        context_status == PMM_DAEMON_RUNTIME_APPLICATION_OK
            ? app_test_request(callbacks, session, initialize, initialize_length, &response,
                               &response_length)
            : PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    bool initialized = initialize_status == PMM_DAEMON_RUNTIME_APPLICATION_OK && response &&
...
static pmm_daemon_runtime_application_status_t app_test_list_projects(
    const pmm_daemon_runtime_application_callbacks_t *callbacks,
    pmm_daemon_runtime_application_session_t *session, uint64_t id, uint8_t **response_out,
    ...
    uint8_t *request = NULL;
    uint32_t request_length = 0;
    if (!app_test_text_request(PMM_DAEMON_APPLICATION_REQUEST_MCP, message, &request,
                               &request_length)) {
        return PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    }
    pmm_daemon_runtime_application_status_t status = app_test_request(
        callbacks, session, request, request_length, response_out, response_length_out);
    free(request);
    return status;
}

static bool app_wait_for_update_notice(const pmm_daemon_runtime_application_callbacks_t *callbacks,
                                       pmm_daemon_runtime_application_session_t *session,
    ...
    while (pmm_now_ms() < deadline) {
        ...
        if (status == PMM_DAEMON_RUNTIME_APPLICATION_OK && response &&
...
TEST(daemon_application_initialize_coalesces_auto_index_for_full_sessions) {
    ...
    bool cache_saved = app_env_backup_capture(&cache_environment, "PMM_CACHE_DIR");
    app_fake_update_context_t update;
    app_fake_update_context_init(&update, false);
    pmm_daemon_application_update_ops_t update_ops = app_fake_update_ops(&update);
    char root[APP_TEST_PATH_CAP];
    char cache[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-auto-index-root-XXXXXX", pmm_tmpdir());
    (void)snprintf(cache, sizeof(cache), "%s/pmm-app-auto-index-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(root) != NULL && pmm_mkdtemp(cache) != NULL;
    bool cache_set = dirs_ok && cache_saved && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    pmm_config_t *stored_config = cache_set ? pmm_config_open(cache) : NULL;
    bool config_ready = stored_config &&
                        pmm_config_set(stored_config, PMM_CONFIG_AUTO_INDEX, "true") == 0 &&
                        pmm_config_set(stored_config, PMM_CONFIG_AUTO_WATCH, "false") == 0;
    char canonical_root[APP_TEST_PATH_CAP] = {0};
    bool canonical = dirs_ok && pmm_canonical_path(root, canonical_root, sizeof(canonical_root));
    char *project = canonical ? pmm_project_name_from_path(canonical_root) : NULL;
    ...
    pmm_daemon_application_t *application =
        config_ready ? pmm_daemon_application_new(&config) : NULL;
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *sessions[4] = {NULL, NULL, NULL, NULL};
    for (size_t i = 0; application && i < 4; i++) {
        sessions[i] = app_test_open(&callbacks, (pmm_daemon_client_id_t)(4110U + i));
    }

    int bg_baseline = pmm_daemon_application_background_initializes_for_test();
    bool scout_initialized = app_test_initialize_profile(&callbacks, sessions[2], root,
                                                         PMM_MCP_TOOL_PROFILE_SCOUT, NULL, NULL);
    bool hook_initialized = app_test_initialize_profile(
        &callbacks, sessions[3], root, PMM_MCP_TOOL_PROFILE_ALL, "SessionStart", "copilot");
    ...
    bool coalesced = first_owned && second_initialized && project &&
                     ...
                     pmm_daemon_application_active_jobs(application) == 1 &&
    ...
    bool restricted_disconnect_kept_job =
        coalesced && project && pmm_daemon_application_job_subscribers(application, project) == 2 &&
    ...
    bool stopped = application && pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    pmm_daemon_application_free(application);
    pmm_config_close(stored_config);
    free(project);
    (void)th_rmtree(root);
    (void)th_rmtree(cache);
...
TEST(daemon_application_auto_index_honors_tracked_file_limit) {
    ...
    (void)snprintf(root, sizeof(root), "%s/pmm-app-auto-limit-root-XXXXXX", pmm_tmpdir());
    (void)snprintf(cache, sizeof(cache), "%s/pmm-app-auto-limit-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(root) != NULL && pmm_mkdtemp(cache) != NULL;
    bool cache_set = dirs_ok && cache_saved && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    ...
    bool config_ready = stored_config &&
                        pmm_config_set(stored_config, PMM_CONFIG_AUTO_INDEX, "true") == 0 &&
                        pmm_config_set(stored_config, PMM_CONFIG_AUTO_INDEX_LIMIT, "0") == 0 &&
                        pmm_config_set(stored_config, PMM_CONFIG_AUTO_WATCH, "false") == 0;
    ...
    pmm_daemon_application_t *application =
        config_ready && git_ready ? pmm_daemon_application_new(&config) : NULL;
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session =
    ...
    bool limit_prevented_admission = initialized && admission_evaluated &&
                                     ...
                                     pmm_daemon_application_active_jobs(application) == 0;
...
TEST(daemon_application_auto_index_file_count_handles_literal_metacharacter_path) {
    ...
    char tracked[APP_TEST_PATH_CAP];
    (void)snprintf(original, sizeof(original), "%s/pmm-app-auto-count-XXXXXX", pmm_tmpdir());
    bool root_ready = pmm_mkdtemp(original) != NULL;
    ...
    int file_count = -1;
    bool within_limit = renamed && pmm_mcp_auto_index_within_file_limit(literal, 1, &file_count);
...
TEST(daemon_application_auto_index_file_count_supports_non_git_roots) {
    ...
    (void)snprintf(root, sizeof(root), "%s/pmm-app-auto-count-non-git-XXXXXX", pmm_tmpdir());
    bool root_ready = pmm_mkdtemp(root) != NULL;
    ...
    bool rejected = source_ready && pmm_mcp_auto_index_within_file_limit(root, 0, &rejected_count);
    int admitted_count = -1;
    bool admitted = source_ready && pmm_mcp_auto_index_within_file_limit(root, 1, &admitted_count);
...
TEST(daemon_application_auto_index_retries_transient_busy_admission) {
    ...
    char cache[APP_TEST_PATH_CAP];
    (void)snprintf(occupied_root, sizeof(occupied_root), "%s/pmm-app-auto-busy-first-XXXXXX",
                   pmm_tmpdir());
    (void)snprintf(auto_root, sizeof(auto_root), "%s/pmm-app-auto-busy-second-XXXXXX",
                   pmm_tmpdir());
    (void)snprintf(cache, sizeof(cache), "%s/pmm-app-auto-busy-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(occupied_root) != NULL && pmm_mkdtemp(auto_root) != NULL &&
                   pmm_mkdtemp(cache) != NULL;
    bool cache_set = dirs_ok && cache_saved && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    pmm_config_t *stored_config = cache_set ? pmm_config_open(cache) : NULL;
    bool config_ready = stored_config &&
    ...
    pmm_thread_t occupied_thread;
    bool occupied_started =
        application && dirs_ok &&
        pmm_thread_create(&occupied_thread, 0, app_index_thread, &occupied) == 0;
    ...
    atomic_store(&fake.allow_completion, true);
    if (occupied_started) {
        (void)pmm_thread_join(&occupied_thread);
    }
    ...
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    pmm_daemon_runtime_application_status_t retry_request =
        initialized
            ? app_test_list_projects(&callbacks, session, 41910, &response, &response_length)
            : PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    free(response);
    bool retry_admitted =
        retried_without_request && retry_request == PMM_DAEMON_RUNTIME_APPLICATION_OK;
...
TEST(daemon_application_update_generation_notifies_initial_and_late_sessions_once) {
    ...
    app_fake_update_context_init(&update, true);
    pmm_daemon_application_update_ops_t update_ops = app_fake_update_ops(&update);
    pmm_daemon_application_config_t config = {.update_ops = &update_ops};
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-update-root-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    pmm_daemon_application_t *application = root_ok ? pmm_daemon_application_new(&config) : NULL;
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *initial =
    ...
    pmm_daemon_runtime_application_status_t initial_second_status =
        initial_notified ? app_test_list_projects(&callbacks, initial, request_id++,
                                                  &initial_second, &initial_second_length)
                         : PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    bool initial_once = initial_second_status == PMM_DAEMON_RUNTIME_APPLICATION_OK &&
                        initial_second && !strstr((char *)initial_second, "Update available:");

    pmm_daemon_runtime_application_session_t *late =
        application ? app_test_open(&callbacks, 4212) : NULL;
    bool late_initialized =
        initial_notified &&
        app_test_initialize_profile(&callbacks, late, root, PMM_MCP_TOOL_PROFILE_ALL, NULL, NULL);
    ...
    pmm_daemon_runtime_application_status_t late_second_status =
        late_notified ? app_test_list_projects(&callbacks, late, request_id++, &late_second,
                                               &late_second_length)
                      : PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    bool late_once = late_second_status == PMM_DAEMON_RUNTIME_APPLICATION_OK && late_second &&
...
TEST(daemon_application_update_generation_retries_worker_start_failure) {
    ...
    pmm_daemon_application_config_t config = {.update_ops = &update_ops};
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-update-retry-start-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    pmm_daemon_application_t *application = root_ok ? pmm_daemon_application_new(&config) : NULL;
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
...
TEST(daemon_application_update_generation_retries_cancelled_check) {
    ...
    pmm_daemon_application_config_t config = {.update_ops = &update_ops};
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-update-retry-cancel-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    pmm_daemon_application_t *application = root_ok ? pmm_daemon_application_new(&config) : NULL;
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *first =
        application ? app_test_open(&callbacks, 4271) : NULL;
    pmm_daemon_runtime_application_session_t *scout =
    ...
    bool first_initialized =
        scout_initialized &&
        app_test_initialize_profile(&callbacks, first, root, PMM_MCP_TOOL_PROFILE_ALL, NULL, NULL);
    ...
    pmm_daemon_runtime_application_session_t *retry =
        application ? app_test_open(&callbacks, 4273) : NULL;
    bool retry_initialized =
        first_cancelled &&
        app_test_initialize_profile(&callbacks, retry, root, PMM_MCP_TOOL_PROFILE_ALL, NULL, NULL);
...
static void *app_final_disconnect_thread(void *opaque) {
    ...
    disconnect->shutdown_ok =
        pmm_daemon_application_shutdown(disconnect->application, APP_TEST_TIMEOUT_MS);
...
TEST(daemon_application_final_disconnect_cancels_and_joins_update_generation) {
    ...
    pmm_daemon_application_config_t config = {.update_ops = &update_ops};
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-update-cancel-root-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    pmm_daemon_application_t *application = root_ok ? pmm_daemon_application_new(&config) : NULL;
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    ...
    pmm_thread_t disconnect_thread;
    bool disconnect_started =
        generation_started &&
        pmm_thread_create(&disconnect_thread, 0, app_final_disconnect_thread, &disconnect) == 0;
    bool joined_before_return =
        disconnect_started && app_wait_for_atomic_bool(&disconnect.done, true);
    if (disconnect_started) {
        (void)pmm_thread_join(&disconnect_thread);
        session = NULL;
    } else if (session) {
        callbacks.session_cancel(callbacks.context, session);
        callbacks.session_close(callbacks.context, session);
        session = NULL;
        disconnect.shutdown_ok = pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    }
    pmm_daemon_application_free(application);
    (void)th_rmtree(root);
...
TEST(daemon_application_coalesces_semantically_identical_index_requests) {
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *sessions[2] = {app_test_open(&callbacks, 21),
                                                             app_test_open(&callbacks, 22)};
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-coalesce-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    char *project = root_ok ? pmm_project_name_from_path(root) : NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    ...
    for (size_t i = 0; contexts_ok && i < 2; i++) {
        contexts_ok = app_test_request(&callbacks, sessions[i], context, context_length, &empty,
                                       &empty_length) == PMM_DAEMON_RUNTIME_APPLICATION_OK;
        free(empty);
        empty = NULL;
    }
    ...
    ASSERT_EQ(requests[0].status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_EQ(requests[1].status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_fresh_request_does_not_reuse_terminal_subscribed_job) {
    ...
    fake.outcomes[0] = PMM_PROC_CLEAN;
    fake.outcomes[1] = PMM_PROC_CLEAN;
    ...
    pmm_daemon_application_config_t config = {.worker_ops = &worker_ops};
    pmm_daemon_application_t *application = pmm_daemon_application_new(&config);
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *sessions[PRIOR_SUBSCRIBERS + 1] = {0};
    bool sessions_ok = application != NULL;
    for (size_t i = 0; sessions_ok && i < PRIOR_SUBSCRIBERS + 1U; i++) {
        sessions[i] = app_test_open(&callbacks, (pmm_daemon_client_id_t)(51U + i));
        sessions_ok = sessions[i] != NULL;
    }

    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-terminal-fresh-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    char *project = root_ok ? pmm_project_name_from_path(root) : NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    ...
    pmm_thread_t threads[PRIOR_SUBSCRIBERS];
    ...
    for (size_t i = 0; all_subscribed && i < PRIOR_SUBSCRIBERS; i++) {
        ...
        started[i] = pmm_thread_create(&threads[i], 0, app_request_thread, &prior[i]) == 0;
    ...
    pmm_daemon_runtime_application_status_t fresh_status =
        job_terminal ? app_test_request(&callbacks, sessions[PRIOR_SUBSCRIBERS], tool, tool_length,
                                        &fresh, &fresh_length)
                     : PMM_DAEMON_RUNTIME_APPLICATION_REJECTED;
    ...
    ASSERT_EQ(fresh_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ...
    ASSERT_TRUE(stopped);
    for (size_t i = 0; i < PRIOR_SUBSCRIBERS; i++) {
        ASSERT_EQ(prior[i].status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_request_cancel_detaches_only_one_coalesced_subscriber) {
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *sessions[2] = {app_test_open(&callbacks, 3011),
                                                             app_test_open(&callbacks, 3012)};
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-request-cancel-coalesce-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    char *project = root_ok ? pmm_project_name_from_path(root) : NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    ...
    if (setup) {
        started[0] = pmm_thread_create(&threads[0], 0, app_request_thread, &requests[0]) == 0;
    }
    bool first_joined = started[0] && app_wait_for_subscribers(application, project, 1);
    if (first_joined) {
        started[1] = pmm_thread_create(&threads[1], 0, app_request_thread, &requests[1]) == 0;
    }
    ...
    for (size_t i = 0; i < 2; i++) {
        if (started[i]) {
            thread_joined[i] = pmm_thread_join(&threads[i]) == 0;
        }
    }
    for (size_t i = 0; i < 2; i++) {
        if (sessions[i]) {
    ...
    ASSERT_EQ(requests[0].status, PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED);
...
TEST(daemon_application_cancels_physical_job_only_after_final_session) {
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *sessions[2] = {app_test_open(&callbacks, 31),
                                                             app_test_open(&callbacks, 32)};
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-cancel-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    char *project = root_ok ? pmm_project_name_from_path(root) : NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    ...
    bool started[2] = {false, false};
    for (size_t i = 0; setup && i < 2; i++) {
        started[i] = pmm_thread_create(&threads[i], 0, app_request_thread, &requests[i]) == 0;
    }
    ...
    ASSERT_EQ(requests[0].status, PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED);
    ASSERT_NULL(requests[0].response);
    ASSERT_EQ(requests[1].status, PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED);
...
TEST(daemon_application_disconnect_before_request_callback_is_sticky) {
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *cancelled_session = app_test_open(&callbacks, 37);
    pmm_daemon_runtime_application_session_t *healthy_session = app_test_open(&callbacks, 38);
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-pre-callback-cancel-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    char args[APP_TEST_PATH_CAP + 32];
    ...
    pmm_daemon_runtime_application_status_t cancelled_status =
        setup ? app_test_request(&callbacks, cancelled_session, tool, tool_length, &response,
                                 &response_length)
              : PMM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
    ...
    pmm_daemon_runtime_application_status_t healthy_status =
    ...
    ASSERT_TRUE(setup);
    ASSERT_EQ(cancelled_status, PMM_DAEMON_RUNTIME_APPLICATION_REJECTED);
    ASSERT_NULL(cancelled_response);
    ASSERT_EQ(cancelled_response_length, 0);
    ASSERT_EQ(starts_after_cancelled_callback, 0);
    ASSERT_EQ(healthy_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_request_cancel_preserves_persistent_watch_and_session) {
    ...
    app_request_thread_t request = {
        ...
        .request = tool,
        .request_length = tool_length,
        .status = PMM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR,
    };
    atomic_init(&request.done, false);
    pmm_thread_t request_thread;
    bool thread_started =
        encoded && pmm_thread_create(&request_thread, 0, app_request_thread, &request) == 0;
    bool subscribed =
    ...
    bool thread_joined = thread_started && pmm_thread_join(&request_thread) == 0;
    int watch_count_after_cancel = fixture.watcher ? pmm_watcher_watch_count(fixture.watcher) : -1;

    uint8_t *ping_response = NULL;
    uint32_t ping_response_length = 0;
    pmm_daemon_runtime_application_status_t ping_status =
        PMM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    ...
    int watch_count_after_ping = fixture.watcher ? pmm_watcher_watch_count(fixture.watcher) : -1;
    ...
    ASSERT_EQ(request.status, PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED);
    ASSERT_NULL(request.response);
    ASSERT_EQ(request.response_length, 0);
    ASSERT_EQ(watch_count_after_cancel, 1);
    ASSERT_EQ(ping_status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_cancel_before_worker_start_skips_worker) {
    ...
    pmm_daemon_application_hold_job_before_start_for_test(true);
    app_request_thread_t request = {
        .callbacks = fixture.callbacks,
        .session = fixture.session,
    ...
    pmm_daemon_application_hold_job_before_start_for_test(false);
    bool thread_joined = thread_started && pmm_thread_join(&request_thread) == 0;
    int watch_count_after_cancel = fixture.watcher ? pmm_watcher_watch_count(fixture.watcher) : -1;
    bool cleaned = app_watch_race_fixture_finish(&fixture);
    int worker_starts = atomic_load(&fixture.fake.starts);
...
TEST(daemon_application_cancel_drops_watch_before_inflight_request_returns) {
    ...
    char root[APP_TEST_PATH_CAP];
    char cache[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-cancel-watch-root-XXXXXX", pmm_tmpdir());
    (void)snprintf(cache, sizeof(cache), "%s/pmm-app-cancel-watch-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(root) != NULL && pmm_mkdtemp(cache) != NULL;
    bool env_ok = saved_cache_ok && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    char *project = dirs_ok ? pmm_project_name_from_path(root) : NULL;
    char db_path[APP_TEST_PATH_CAP] = {0};
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 39);
    ...
    bool watch_registered = setup && pmm_watcher_watch_count(watcher) == 1;
    ...
    bool thread_started = watch_registered &&
                          pmm_thread_create(&request_thread, 0, app_request_thread, &request) == 0;
    ...
    int watch_count_at_cancel = pmm_watcher_watch_count(watcher);
    bool thread_joined = thread_started && pmm_thread_join(&request_thread) == 0;
    int watch_count_after_request = pmm_watcher_watch_count(watcher);
    bool response_cancelled = request.status == PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED &&
...
TEST(daemon_application_stale_watcher_callback_is_rejected_at_job_admission) {
    ...
    pmm_thread_t thread;
    bool thread_started =
        fixture_ready && pmm_thread_create(&thread, 0, app_watcher_index_thread, &request) == 0;
    ...
    int watches_after_cancel = pmm_watcher_watch_count(fixture.watcher);
    ...
    bool thread_joined = request_done && pmm_thread_join(&thread) == 0;
    int worker_starts = atomic_load(&fixture.fake.starts);
    size_t active_jobs = pmm_daemon_application_active_jobs(fixture.application);
    bool cleaned = app_watch_race_fixture_finish(&fixture);
...
TEST(daemon_application_final_cancel_drains_admitted_watcher_job) {
    ...
    bool job_active =
        worker_started && pmm_daemon_application_active_jobs(fixture.application) == 1;
    ...
    bool request_done = thread_started && app_wait_for_atomic_bool(&request.done, true);
    bool thread_joined = request_done && pmm_thread_join(&thread) == 0;
    size_t active_jobs_after_join = pmm_daemon_application_active_jobs(fixture.application);
    int worker_cancels = atomic_load(&fixture.fake.cancels);
    int worker_destroys = atomic_load(&fixture.fake.destroys);
    bool cleaned = app_watch_race_fixture_finish(&fixture);
...
TEST(daemon_application_watcher_job_follows_exact_live_watch_owners) {
    ...
    pmm_daemon_runtime_application_session_t *second =
    ...
    pmm_daemon_runtime_application_session_t *unrelated =
    ...
    second_watching = second_watching &&
                      app_test_request(&fixture.callbacks, second, ping, ping_length, &response,
                                       &response_length) == PMM_DAEMON_RUNTIME_APPLICATION_OK &&
                      pmm_watcher_watch_count(fixture.watcher) == 1;
    free(response);
    response = NULL;
    ...
    pmm_thread_t thread;
    bool thread_started =
        second_watching && pmm_thread_create(&thread, 0, app_watcher_index_thread, &request) == 0;
    bool worker_started = thread_started && app_wait_for_atomic_int(&fixture.fake.starts, 1);
    size_t initial_subscribers =
        worker_started
            ? pmm_daemon_application_job_subscribers(fixture.application, fixture.project)
            : 0;
    ...
    int watches_after_first = pmm_watcher_watch_count(fixture.watcher);
    ...
    int watches_after_second = pmm_watcher_watch_count(fixture.watcher);
...
TEST(daemon_application_late_watcher_session_owns_active_watcher_job) {
    ...
    bool thread_started = fixture_ready && unrelated &&
                          pmm_thread_create(&thread, 0, app_watcher_index_thread, &request) == 0;
    bool worker_started = thread_started && app_wait_for_atomic_int(&fixture.fake.starts, 1);
    size_t initial_subscribers =
        worker_started
            ? pmm_daemon_application_job_subscribers(fixture.application, fixture.project)
    ...
    size_t subscribers_with_late =
        late_watching ? pmm_daemon_application_job_subscribers(fixture.application, fixture.project)
                      : 0;
...
TEST(daemon_application_serializes_adr_mutation_with_index_job) {
    ...
    char root[APP_TEST_PATH_CAP];
    char cache[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-mutation-root-XXXXXX", pmm_tmpdir());
    snprintf(cache, sizeof(cache), "%s/pmm-app-mutation-cache-XXXXXX", pmm_tmpdir());
    bool dirs_ok = pmm_mkdtemp(root) != NULL && pmm_mkdtemp(cache) != NULL;
    bool env_ok =
        dirs_ok && (!had_cache || saved_cache) && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;
    char *project = env_ok ? pmm_project_name_from_path(root) : NULL;
    char db_path[APP_TEST_PATH_CAP] = {0};
    pmm_store_t *seed = NULL;
    bool seeded = false;
    if (project) {
        snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project);
        seed = pmm_store_open_path(db_path);
        seeded = seed && pmm_store_upsert_project(seed, project, root) == PMM_STORE_OK;
        pmm_store_close(seed);
    }

    pmm_daemon_application_t *application = seeded ? pmm_daemon_application_new(&config) : NULL;
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *index_session =
        application ? app_test_open(&callbacks, 33) : NULL;
    pmm_daemon_runtime_application_session_t *adr_session =
    ...
    pmm_thread_t index_thread;
    pmm_thread_t adr_thread;
    bool index_started =
        setup && pmm_thread_create(&index_thread, 0, app_request_thread, &index_request) == 0;
    bool index_active = index_started && project &&
                        app_wait_for_subscribers(application, project, 1) &&
                        app_wait_for_atomic_int(&fake.starts, 1);
    bool adr_started =
        index_active && pmm_thread_create(&adr_thread, 0, app_request_thread, &adr_request) == 0;
    uint64_t early_deadline = pmm_now_ms() + 100;
    while (adr_started && !atomic_load(&adr_request.done) && pmm_now_ms() < early_deadline) {
        pmm_usleep(1000);
    }
    ...
    atomic_store(&fake.allow_completion, true);
    if (index_started) {
        (void)pmm_thread_join(&index_thread);
    }
    if (adr_started) {
        (void)pmm_thread_join(&adr_thread);
    }

    pmm_store_t *check = pmm_store_open_path_query(db_path);
    pmm_adr_t adr = {0};
    bool adr_persisted = check && pmm_store_adr_get(check, project, &adr) == PMM_STORE_OK &&
    ...
    ASSERT_EQ(index_request.status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
    ASSERT_EQ(adr_request.status, PMM_DAEMON_RUNTIME_APPLICATION_OK);
...
TEST(daemon_application_reserved_mutation_delays_worker_start) {
    ...
    pmm_daemon_application_t *application = pmm_daemon_application_new(&config);
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-mutation-first-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    const char *project = "mutation-first";
    bool lease_held =
        application && pmm_daemon_application_project_mutation_try_begin(application, project);
    ...
    pmm_thread_t thread;
    bool thread_started =
        root_ok && lease_held && pmm_thread_create(&thread, 0, app_index_thread, &request) == 0;
    bool reserved = thread_started && app_wait_for_subscribers(application, project, 1);
    uint64_t early_deadline = pmm_now_ms() + 100;
    while (atomic_load(&fake.starts) == 0 && pmm_now_ms() < early_deadline) {
        pmm_usleep(1000);
    }
    bool worker_started_while_mutating = atomic_load(&fake.starts) != 0;
    if (lease_held) {
        pmm_daemon_application_project_mutation_end(application, project);
    }
    bool worker_started = thread_started && app_wait_for_atomic_int(&fake.starts, 1);
    atomic_store(&fake.allow_completion, true);
    if (thread_started) {
...
TEST(daemon_application_mutation_does_not_block_other_project) {
    ...
    pmm_daemon_application_t *application = pmm_daemon_application_new(&config);
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-mutation-other-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    bool lease_held =
        application && pmm_daemon_application_project_mutation_try_begin(application, "project-a");
    app_index_thread_t request = {
        .application = application,
        .project = "project-b",
        .root = root,
    ...
    if (lease_held) {
        pmm_daemon_application_project_mutation_end(application, "project-a");
    }
    bool stopped = application && pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    pmm_daemon_application_free(application);
    (void)pmm_rmdir(root);
...
TEST(daemon_application_mutation_still_owns_os_project_lock) {
    ...
    pmm_daemon_application_t *application =
        locks_ready ? pmm_daemon_application_new(&config) : NULL;
    const char *project = "daemon-mutation-native-lock";
    bool mutation_started =
        application && pmm_daemon_application_project_mutation_try_begin(application, project);

    pmm_project_lock_lease_t *active_probe = NULL;
    pmm_private_file_lock_status_t active_status =
        mutation_started ? pmm_project_lock_try_acquire(locks.probe_locks, project, &active_probe)
                         : PMM_PRIVATE_FILE_LOCK_IO;
    bool mutation_holds_lock = active_status == PMM_PRIVATE_FILE_LOCK_BUSY && !active_probe;
    bool active_probe_released = app_project_lock_release(&active_probe);
    if (mutation_started) {
        pmm_daemon_application_project_mutation_end(application, project);
    }

    pmm_project_lock_lease_t *terminal_probe = NULL;
    pmm_private_file_lock_status_t terminal_status =
        mutation_started ? pmm_project_lock_try_acquire(locks.probe_locks, project, &terminal_probe)
                         : PMM_PRIVATE_FILE_LOCK_IO;
    bool mutation_released_lock = terminal_status == PMM_PRIVATE_FILE_LOCK_OK && terminal_probe;
...
TEST(daemon_application_worker_boundary_is_sole_os_project_lock_owner) {
    ...
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-worker-lock-owner-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    ...
    pmm_thread_t thread;
    bool thread_started =
        root_ok && application && pmm_thread_create(&thread, 0, app_index_thread, &request) == 0;
    ...
    pmm_project_lock_lease_t *active_probe = NULL;
    pmm_private_file_lock_status_t active_probe_status =
        worker_acquired ? pmm_project_lock_try_acquire(locks.probe_locks, project, &active_probe)
                        : PMM_PRIVATE_FILE_LOCK_IO;
    bool worker_holds_lock = active_probe_status == PMM_PRIVATE_FILE_LOCK_BUSY && !active_probe;
    bool active_probe_released = app_project_lock_release(&active_probe);
    atomic_store(&fake.allow_completion, true);
    if (thread_started) {
        (void)pmm_thread_join(&thread);
    ...
    pmm_project_lock_lease_t *terminal_probe = NULL;
    pmm_private_file_lock_status_t terminal_probe_status =
        thread_started ? pmm_project_lock_try_acquire(locks.probe_locks, project, &terminal_probe)
                       : PMM_PRIVATE_FILE_LOCK_IO;
    bool worker_released_lock = terminal_probe_status == PMM_PRIVATE_FILE_LOCK_OK && terminal_probe;
    bool terminal_probe_released = app_project_lock_release(&terminal_probe);
...
TEST(daemon_application_worker_lock_serializes_external_mutation) {
    ...
    (void)snprintf(blocked_root, sizeof(blocked_root), "%s/pmm-app-external-blocked-XXXXXX",
                   pmm_tmpdir());
    (void)snprintf(other_root, sizeof(other_root), "%s/pmm-app-external-other-XXXXXX",
                   pmm_tmpdir());
    bool blocked_root_ok = pmm_mkdtemp(blocked_root) != NULL;
    bool other_root_ok = pmm_mkdtemp(other_root) != NULL;
    const char *blocked_project = "external-blocked";
    const char *other_project = "external-other";

    pmm_project_lock_lease_t *external_lease = NULL;
    bool external_lease_held =
        application && blocked_root_ok && other_root_ok &&
        pmm_project_lock_acquire(locks.external_locks, blocked_project, UINT64_MAX, NULL,
                                 &external_lease) == PMM_PRIVATE_FILE_LOCK_OK &&
        external_lease;
    ...
    pmm_thread_t blocked_thread;
    bool blocked_thread_started =
        external_lease_held &&
        pmm_thread_create(&blocked_thread, 0, app_index_thread, &blocked_request) == 0;
    ...
    pmm_thread_t other_thread;
    bool other_thread_started =
        blocked_waiting &&
        pmm_thread_create(&other_thread, 0, app_index_thread, &other_request) == 0;
    ...
    pmm_project_lock_lease_t *other_probe = NULL;
    pmm_private_file_lock_status_t active_other_status =
        other_worker_acquired
            ? pmm_project_lock_try_acquire(locks.probe_locks, other_project, &other_probe)
            : PMM_PRIVATE_FILE_LOCK_IO;
    bool other_lock_held_while_worker_active =
        active_other_status == PMM_PRIVATE_FILE_LOCK_BUSY && !other_probe;
    ...
    if (other_thread_started) {
        (void)pmm_thread_join(&other_thread);
    }
    pmm_project_lock_lease_t *terminal_other_probe = NULL;
    pmm_private_file_lock_status_t terminal_other_status =
    ...
    bool other_lock_released_at_terminal =
        terminal_other_status == PMM_PRIVATE_FILE_LOCK_OK && terminal_other_probe;
    ...
    if (blocked_thread_started) {
        (void)pmm_thread_join(&blocked_thread);
    }
    pmm_project_lock_lease_t *blocked_probe = NULL;
    pmm_private_file_lock_status_t terminal_blocked_status =
    ...
    bool blocked_lock_released_at_terminal =
        terminal_blocked_status == PMM_PRIVATE_FILE_LOCK_OK && blocked_probe;
...
TEST(daemon_application_final_unsubscribe_cancels_external_lock_wait) {
    ...
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-external-cancel-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    char *project = root_ok ? pmm_project_name_from_path(root) : NULL;
    pmm_project_lock_lease_t *external_lease = NULL;
    bool external_lease_held =
        session && project &&
        pmm_project_lock_acquire(locks.external_locks, project, UINT64_MAX, NULL,
                                 &external_lease) == PMM_PRIVATE_FILE_LOCK_OK &&
        external_lease;
    ...
    bool request_started =
        setup && pmm_thread_create(&request_thread, 0, app_request_thread, &request) == 0;
    ...
    uint64_t request_deadline = pmm_now_ms() + APP_TEST_TIMEOUT_MS;
    while (request_started && !atomic_load(&request.done) && pmm_now_ms() < request_deadline) {
        pmm_usleep(1000);
    }
    ...
    if (request_started) {
        (void)pmm_thread_join(&request_thread);
    }
    bool cancellation_reported = request.status == PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED &&
                                 request.response == NULL && request.response_length == 0;
    bool worker_never_acquired = atomic_load(&fake.project_lock_acquisitions) == 0;

    bool external_lease_released = app_project_lock_release(&external_lease);
    pmm_project_lock_lease_t *leak_probe = NULL;
    bool can_probe_for_leak = job_terminal && external_lease_held && external_lease_released &&
                              locks.probe_locks && project;
    pmm_private_file_lock_status_t leak_probe_status =
        can_probe_for_leak ? pmm_project_lock_try_acquire(locks.probe_locks, project, &leak_probe)
                           : PMM_PRIVATE_FILE_LOCK_IO;
    bool no_daemon_lease_leaked = leak_probe_status == PMM_PRIVATE_FILE_LOCK_OK && leak_probe;
...
TEST(daemon_application_cancels_mutation_wait_without_cancelling_shared_job) {
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *index_session = app_test_open(&callbacks, 35);
    pmm_daemon_runtime_application_session_t *mutation_session = app_test_open(&callbacks, 36);
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-mutation-cancel-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    char *project = root_ok ? pmm_project_name_from_path(root) : NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    ...
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    pmm_daemon_runtime_application_session_t *sessions[2] = {index_session, mutation_session};
    for (size_t i = 0; setup && i < 2; i++) {
    ...
    bool mutation_started =
        index_active &&
        pmm_thread_create(&mutation_thread, 0, app_request_thread, &mutation_request) == 0;
    uint64_t blocked_deadline = pmm_now_ms() + 100;
    while (mutation_started && !atomic_load(&mutation_request.done) &&
           pmm_now_ms() < blocked_deadline) {
        pmm_usleep(1000);
    }
    ...
    uint64_t cancel_deadline = pmm_now_ms() + APP_TEST_TIMEOUT_MS;
    while (mutation_started && !atomic_load(&mutation_request.done) &&
           pmm_now_ms() < cancel_deadline) {
        pmm_usleep(1000);
    }
    ...
    atomic_store(&fake.allow_completion, true);
    if (mutation_started) {
        (void)pmm_thread_join(&mutation_thread);
    }
    if (index_started) {
        (void)pmm_thread_join(&index_thread);
    }
    bool cancellation_reported =
        mutation_request.status == PMM_DAEMON_RUNTIME_APPLICATION_CANCELLED &&
...
TEST(daemon_application_recovers_with_unique_per_job_quarantine_files) {
    const char *old_restarts = getenv("PMM_INDEX_MAX_RESTARTS");
    char *saved_restarts = old_restarts ? pmm_strdup(old_restarts) : NULL;
    ...
    for (int base = 0; base <= 4; base += 4) {
        fake.outcomes[base] = PMM_PROC_CRASH;
        fake.outcomes[base + 1] = PMM_PROC_CRASH;
        fake.outcomes[base + 2] = PMM_PROC_CRASH;
        fake.outcomes[base + 3] = PMM_PROC_CLEAN;
    ...
    char root[APP_TEST_PATH_CAP];
    char cache[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-recovery-XXXXXX", pmm_tmpdir());
    snprintf(cache, sizeof(cache), "%s/pmm-app-recovery-cache-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    bool cache_ok = pmm_mkdtemp(cache) != NULL;
    bool env_ok = saved_ok && saved_cache_ok && cache_ok &&
                  pmm_setenv("PMM_INDEX_MAX_RESTARTS", "3", 1) == 0 &&
                  pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;

    int first = application && root_ok && env_ok
                    ? pmm_daemon_application_index(application, "recovery-one", root)
                    : -1;
    int second = application && root_ok && env_ok
                     ? pmm_daemon_application_index(application, "recovery-two", root)
                     : -1;
    bool stopped = application && pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    bool first_marker_removed = !pmm_file_exists(fake.marker_paths[1]);
    bool first_quarantine_removed = !pmm_file_exists(fake.quarantine_paths[1]);
    bool second_marker_removed = !pmm_file_exists(fake.marker_paths[5]);
    bool second_quarantine_removed = !pmm_file_exists(fake.quarantine_paths[5]);
    pmm_daemon_application_free(application);
    if (saved_restarts) {
        (void)pmm_setenv("PMM_INDEX_MAX_RESTARTS", saved_restarts, 1);
    } else if (!old_restarts) {
        (void)pmm_unsetenv("PMM_INDEX_MAX_RESTARTS");
    }
    free(saved_restarts);
    if (saved_cache) {
        (void)pmm_setenv("PMM_CACHE_DIR", saved_cache, 1);
...
TEST(daemon_application_cancellation_between_recovery_attempts_stops_retry) {
    ...
    atomic_store(&fake.scripted, true);
    atomic_store(&fake.hold_destroy_attempt, 0);
    fake.outcomes[0] = PMM_PROC_CRASH;
    pmm_daemon_application_worker_ops_t worker_ops = {
        .context = &fake,
        .start = app_fake_worker_start,
        .poll = app_fake_worker_poll,
    ...
    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 41);
    char root[APP_TEST_PATH_CAP];
    snprintf(root, sizeof(root), "%s/pmm-app-retry-cancel-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;
    uint8_t *context = NULL;
    uint32_t context_length = 0;
    char args[APP_TEST_PATH_CAP + 32];
    ...
    pmm_thread_t thread;
    bool started = setup && pmm_thread_create(&thread, 0, app_request_thread, &request) == 0;
...
TEST(daemon_application_thread_start_failure_rolls_back_job_reservation) {
    ...
    pmm_daemon_application_t *application = pmm_daemon_application_new(&config);
    char root[APP_TEST_PATH_CAP];
    (void)snprintf(root, sizeof(root), "%s/pmm-app-thread-start-XXXXXX", pmm_tmpdir());
    bool root_ok = pmm_mkdtemp(root) != NULL;

    int failed = 0;
    if (application && root_ok) {
        pmm_daemon_application_fail_next_job_thread_start_for_test();
        failed = pmm_daemon_application_index(application, "thread-start", root);
    }
    size_t active_after_failure = application ? pmm_daemon_application_active_jobs(application) : 1;
    size_t subscribers_after_failure =
        application ? pmm_daemon_application_job_subscribers(application, "thread-start") : 1;
    int retried = application && root_ok
                      ? pmm_daemon_application_index(application, "thread-start", root)
                      : -1;
    bool stopped = application && pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    pmm_daemon_application_free(application);
    (void)pmm_rmdir(root);
...
TEST(daemon_application_queues_explicit_index_behind_physical_job_limit) {
    ...
    snprintf(first_root, sizeof(first_root), "%s/pmm-app-cap-first-XXXXXX", pmm_tmpdir());
    snprintf(second_root, sizeof(second_root), "%s/pmm-app-cap-second-XXXXXX", pmm_tmpdir());
    bool roots_ok = pmm_mkdtemp(first_root) != NULL && pmm_mkdtemp(second_root) != NULL;
    ...
    pmm_thread_t thread;
    bool started =
        application && roots_ok && pmm_thread_create(&thread, 0, app_index_thread, &first) == 0;
    ...
    int busy = admitted ? pmm_daemon_application_index(application, "cap-second", second_root) : -1;

    pmm_daemon_runtime_application_callbacks_t callbacks =
        pmm_daemon_application_runtime_callbacks(application);
    pmm_daemon_runtime_application_session_t *session = app_test_open(&callbacks, 51);
    ...
    int queue_waits_baseline = pmm_daemon_application_busy_queue_waits_for_test();
    ...
    bool request_started =
        context_ok &&
        pmm_thread_create(&request_thread, 0, app_session_request_thread, &queued) == 0;
    ...
    if (request_started) {
        uint64_t deadline = pmm_now_ms() + APP_TEST_TIMEOUT_MS;
        while (pmm_now_ms() < deadline) {
            if (pmm_daemon_application_busy_queue_waits_for_test() > queue_waits_baseline) {
                queued_parked = true;
                break;
            }
            pmm_usleep(1000);
    ...
    atomic_store(&fake.allow_completion, true);
    bool request_joined = request_started && pmm_thread_join(&request_thread) == 0;
    bool queued_succeeded = request_joined && queued.status == PMM_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    (void)pmm_rmdir(first_root);
    (void)pmm_rmdir(second_root);
    PASS();
}
...
TEST(daemon_application_default_limit_admits_four_and_rejects_fifth) {
    ...
    for (int i = 0; i < DEFAULT_CAP_TOTAL; i++) {
        (void)snprintf(roots[i], sizeof(roots[i]), "%s/pmm-app-default-cap-%d-XXXXXX", pmm_tmpdir(),
                       i);
        (void)snprintf(projects[i], sizeof(projects[i]), "default-cap-%d", i);
        roots_ok = roots_ok && pmm_mkdtemp(roots[i]) != NULL;
    }

    app_index_thread_t requests[DEFAULT_CAP_RUNNING];
    pmm_thread_t threads[DEFAULT_CAP_RUNNING];
    ...
    for (int i = 0; application && roots_ok && i < DEFAULT_CAP_RUNNING; i++) {
        ...
        started[i] = pmm_thread_create(&threads[i], 0, app_index_thread, &requests[i]) == 0;
    }
    ...
    int fifth =
        four_admitted ? pmm_daemon_application_index(application, projects[4], roots[4]) : -1;
    bool stopped = application && pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    for (int i = 0; i < DEFAULT_CAP_RUNNING; i++) {
        if (started[i]) {
            (void)pmm_thread_join(&threads[i]);
...
TEST(daemon_application_free_reports_retained_live_ownership) {
    ...
    bool mutation_held = application && pmm_daemon_application_project_mutation_try_begin(
                                            application, "free-contract");
    bool freed_while_busy =
        application && mutation_held && pmm_daemon_application_free_with_timeout(application, 0);
    bool retained_usable = application && mutation_held && !freed_while_busy &&
                           pmm_daemon_application_active_jobs(application) == 0;
    if (retained_usable) {
        pmm_daemon_application_project_mutation_end(application, "free-contract");
    }
    bool freed_after_release = retained_usable && pmm_daemon_application_free_with_timeout(
...
TEST(daemon_application_rejects_clean_exit_when_process_tree_is_not_contained) {
    ...
    int result = application ? pmm_daemon_application_index(application, "unsafe-clean-project",
                                                            pmm_tmpdir())
                             : -1;
    bool stopped = application && pmm_daemon_application_shutdown(application, APP_TEST_TIMEOUT_MS);
    pmm_daemon_application_free(application);
