 ...
 * pmm_extract_file() populates the right fields.  Here we run the FULL
 * production pipeline (index_repository via MCP) and assert the resulting
 * graph DB contains the expected edges via pmm_store_count_edges_by_type.
 *
 ...
 *   Go implicit interface satisfaction: pass_semantic.c pmm_pipeline_implements_go()
...
static pmm_store_t *es_lang_open_indexed(ES_LangProj *lp) {
...
static pmm_store_t *es_lang_index_files(ES_LangProj *lp, const ES_LangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_es_XXXXXX");
...
static void es_lang_cleanup(ES_LangProj *lp, pmm_store_t *store) {
    if (store) {
...
static void es_dump_edge_histogram(pmm_store_t *store, const char *project) {
    if (!store) {
    ...
    for (int i = 0; ES_ALL_EDGE_TYPES[i]; i++) {
        int c = pmm_store_count_edges_by_type(store, project, ES_ALL_EDGE_TYPES[i]);
...
static int es_edge_present(const ES_LangFile *files, int nfiles, const char *edge, int floor) {
    ES_LangProj lp;
    pmm_store_t *store = es_lang_index_files(&lp, files, nfiles);
...
TEST(es_inherits_crossfile_python_red) {
    ...
    ES_LangProj lp;
    pmm_store_t *store = es_lang_index_files(&lp, f, 2);
    int got = store ? pmm_store_count_edges_by_type(store, lp.project, "INHERITS") : -1;
    if (got >= 1) {
 ...
 * pmm_pipeline_implements_go() checks method-set coverage. */
TEST(es_implements_go_implicit) {
    ...
    /* GREEN: pmm_pipeline_implements_go() finds MyType satisfies Stringer
...
TEST(es_override_go_implicit) {
    ...
    /* GREEN: pmm_pipeline_implements_go() emits OVERRIDE for Entity.Name -> Namer.Name. */
...
TEST(es_decorates_annotation_java) {
    ...
    ES_LangProj lp;
    pmm_store_t *store = es_lang_index_files(&lp, f, 1);
    int got = store ? pmm_store_count_edges_by_type(store, lp.project, "DECORATES") : -1;
...
SUITE(edge_structural) {
    ...
    /* Expected GREEN: pmm_pipeline_implements_go() emits OVERRIDE. */
