...
static void setup_artifact_test(void) {
    snprintf(g_tmpdir, sizeof(g_tmpdir), "%s/pmm_test_artifact_XXXXXX", pmm_tmpdir());
    pmm_mkdtemp(g_tmpdir);

    snprintf(g_repo, sizeof(g_repo), "%s/repo", g_tmpdir);
    pmm_mkdir_p(g_repo, 0755);
...
static void create_test_db(const char *path) {
    pmm_store_t *s = pmm_store_open_path(path);
    if (!s) {
        return;
    }

    pmm_store_exec(s, "INSERT OR IGNORE INTO projects(name, indexed_at, root_path) "
                      "VALUES('test-proj', '2026-01-01', '/tmp/test');");

    pmm_store_exec(s, "INSERT INTO nodes(project, label, name, qualified_name, file_path) "
                      "VALUES('test-proj', 'Function', 'foo', 'test-proj.foo', 'main.c');");
    pmm_store_exec(s, "INSERT INTO nodes(project, label, name, qualified_name, file_path) "
                      "VALUES('test-proj', 'Function', 'bar', 'test-proj.bar', 'main.c');");

    pmm_store_exec(s, "INSERT INTO edges(project, source_id, target_id, type) "
...
static void capture_logs_start(void) {
    g_log_capture[0] = '\0';
    g_prev_log_level = pmm_log_get_level();
    pmm_log_set_level(PMM_LOG_DEBUG);
    pmm_log_set_sink(capture_log_sink);
}

static const char *capture_logs_end(void) {
    pmm_log_set_sink(NULL);
    pmm_log_set_level(g_prev_log_level);
...
TEST(artifact_import_rejects_size_mismatch) {
    setup_artifact_test();
    create_test_db(g_db);
    ASSERT_EQ(pmm_artifact_export(g_db, g_repo, "test-proj", PMM_ARTIFACT_FAST), 0);

    char meta[1024];
    snprintf(meta, sizeof(meta), "%s/.project-mind-mcp/artifact.json", g_repo);
    ASSERT_TRUE(
    ...
    int rc = pmm_artifact_import(g_repo, import_db);
...
TEST(artifact_export_fast_roundtrip) {
    ...
    int rc = pmm_artifact_export(g_db, g_repo, "test-proj", PMM_ARTIFACT_FAST);
    ASSERT_EQ(rc, 0);

    /* Verify artifact files exist */
    char zst[1024];
    snprintf(zst, sizeof(zst), "%s/.project-mind-mcp/graph.db.zst", g_repo);
    struct stat st;
    ...
    char import_db[1024];
    snprintf(import_db, sizeof(import_db), "%s/imported.db", g_tmpdir);
    rc = pmm_artifact_import(g_repo, import_db);
    ASSERT_EQ(rc, 0);

    /* Verify imported DB has correct data */
    pmm_store_t *s = pmm_store_open_path(import_db);
    ASSERT_NOT_NULL(s);
    int nodes = pmm_store_count_nodes(s, "test-proj");
    int edges = pmm_store_count_edges(s, "test-proj");
    ASSERT_EQ(nodes, 2);
    ASSERT_EQ(edges, 1);
    pmm_store_close(s);
...
TEST(artifact_export_best_roundtrip) {
    ...
    int rc = pmm_artifact_export(g_db, g_repo, "test-proj", PMM_ARTIFACT_BEST);
    ASSERT_EQ(rc, 0);

    /* Source DB should be untouched (VACUUM INTO doesn't modify source) */
    pmm_store_t *src = pmm_store_open_path(g_db);
    ASSERT_NOT_NULL(src);
    ASSERT_EQ(pmm_store_count_nodes(src, "test-proj"), 2);
    pmm_store_close(src);
    ...
    pmm_store_t *s = pmm_store_open_path(import_db);
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_count_nodes(s, "test-proj"), 2);
    ASSERT_EQ(pmm_store_count_edges(s, "test-proj"), 1);
    pmm_store_close(s);
...
TEST(artifact_exists_check) {
    ...
    /* No artifact yet */
    ASSERT_FALSE(pmm_artifact_exists(g_repo));

    /* Export creates the artifact */
    pmm_artifact_export(g_db, g_repo, "test-proj", PMM_ARTIFACT_FAST);
    ASSERT_TRUE(pmm_artifact_exists(g_repo));
...
TEST(artifact_commit_hash) {
    ...
    char *commit = pmm_artifact_commit(g_repo);
...
TEST(artifact_gitattributes_created) {
    ...
    char ga[1024];
    snprintf(ga, sizeof(ga), "%s/.project-mind-mcp/.gitattributes", g_repo);
    struct stat st;
    ...
    (void)fclose(gaf);
    ASSERT_TRUE(rd > 0);
    ASSERT_NOT_NULL(strstr(content, PMM_ARTIFACT_FILENAME " binary merge=ours"));
...
TEST(artifact_export_rename_failure_logs_specific_error) {
    ...
    char art_dir[1024];
    snprintf(art_dir, sizeof(art_dir), "%s/.project-mind-mcp", g_repo);
    pmm_mkdir_p(art_dir, 0755);

    char zst[1024];
    snprintf(zst, sizeof(zst), "%s/graph.db.zst", art_dir);
    pmm_mkdir_p(zst, 0755);
    ...
    ASSERT_NEQ(rc, 0);
    ASSERT_FALSE(pmm_artifact_exists(g_repo));
    ASSERT_NOT_NULL(pmm_artifact_export_last_error());
    ASSERT(strstr(pmm_artifact_export_last_error(), "write_artifact") != NULL);
    ASSERT(strstr(pmm_artifact_export_last_error(), "rename_temp") != NULL);
...
TEST(pipeline_persistence_export_failure_returns_error) {
    ...
    pmm_pipeline_t *p = pmm_pipeline_new(g_repo, g_db, PMM_MODE_FAST);
    ASSERT_NOT_NULL(p);
    pmm_pipeline_set_persistence(p, true);
...
TEST(artifact_null_safety) {
    ASSERT_NEQ(pmm_artifact_export(NULL, "/tmp", "p", 0), 0);
    ASSERT_NEQ(pmm_artifact_export("/tmp/x.db", NULL, "p", 0), 0);
    ASSERT_NEQ(pmm_artifact_import(NULL, "/tmp/x.db"), 0);
    ASSERT_NEQ(pmm_artifact_import("/tmp", NULL), 0);
    ASSERT_FALSE(pmm_artifact_exists(NULL));
    ASSERT_NULL(pmm_artifact_commit(NULL));
    PASS();
}

/* ── git shell-out path safety ────────────────────────────────────────────────
 *
 * artifact.c shells out to git via pmm_popen with the repo path interpolated into
 ...
 * quotes; pmm_artifact_repo_path_is_shell_safe() is the guard. Rejecting quotes and
...
TEST(artifact_repo_path_shell_safe_accepts_plain_and_spaced) {
    ASSERT_TRUE(pmm_artifact_repo_path_is_shell_safe("/home/user/repo"));
    ASSERT_TRUE(pmm_artifact_repo_path_is_shell_safe("C:/Users/me/repo"));
    ASSERT_TRUE(pmm_artifact_repo_path_is_shell_safe("/home/user/my repo")); /* space OK */
    PASS();
}

TEST(artifact_repo_path_shell_safe_rejects_injection) {
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe(NULL));
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("it's"));        /* single quote */
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("a\"b"));        /* double quote */
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("x; rm -rf /")); /* command sep */
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("$(whoami)"));   /* substitution */
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("a`id`b"));      /* backtick */
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("a|b"));         /* pipe */
    PASS();
}
...
TEST(artifact_repo_path_shell_safe_rejects_cmd_metachars_on_windows) {
    ...
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("C:/a%USERPROFILE%b"));
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("C:/a!b"));
    ASSERT_FALSE(pmm_artifact_repo_path_is_shell_safe("C:/a^b"));
#else
    /* POSIX shells treat % ! ^ literally inside double quotes — allowed. */
    ASSERT_TRUE(pmm_artifact_repo_path_is_shell_safe("/a%b"));
    ASSERT_TRUE(pmm_artifact_repo_path_is_shell_safe("/a^b"));
#endif
    PASS();
}
...
TEST(artifact_fast_export_snapshots_live_wal_store) {
    ...
    pmm_store_t *s = pmm_store_open_path(g_db);
    ASSERT_NOT_NULL(s);
    ...
    for (int i = 0; i < WAL_NODES; i++) {
        ...
        pmm_node_t n = {.project = "test-proj",
                        .label = "Function",
                        .name = name,
                        .qualified_name = qn,
                        .file_path = "mod.py",
                        .start_line = i + 1,
                        .end_line = i + 2};
        ASSERT_TRUE(pmm_store_upsert_node(s, &n) > 0);
    }
    ...
    ASSERT_EQ(pmm_artifact_import(g_repo, import_db), 0);

    pmm_store_t *imp = pmm_store_open_path(import_db);
    ASSERT_NOT_NULL(imp);
    /* Torn snapshot = the WAL-resident rows are missing. */
    ASSERT_EQ(pmm_store_count_nodes(imp, "test-proj"), WAL_NODES);
    pmm_store_close(imp);
    PASS();
}
...
TEST(store_deep_integrity_detects_page_corruption) {
    ...
    pmm_store_t *s = pmm_store_open_path(db2);
    ASSERT_NOT_NULL(s);
    pmm_store_upsert_project(s, "deep", "/tmp/deep");
    ...
    pmm_store_t *ok = pmm_store_open_path(db2);
    ASSERT_NOT_NULL(ok);
    ASSERT_TRUE(pmm_store_check_integrity_deep(ok));
    pmm_store_close(ok);
    ...
    pmm_store_t *bad = pmm_store_open_path(db2);
    ASSERT_NOT_NULL(bad);
    ASSERT_FALSE(pmm_store_check_integrity_deep(bad));
    pmm_store_close(bad);
    PASS();
}
