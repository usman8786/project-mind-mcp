 ...
 *
 * Ports from internal/pmm/sqlite_writer_test.go:
 *   TestWriteDB_MinimalData, TestWriteDB_ScaleAndIndexes, TestWriteDB_Empty
 *
 * The page writer (pmm_write_db) constructs B-tree pages directly,
 * bypassing the SQL parser entirely. These tests verify integrity.
 */
#include "../src/foundation/compat.h"
#include "foundation/compat_fs.h"
#include "test_framework.h"
/* sqlite_writer.h is at internal/pmm/ — Makefile adds -Iinternal/pmm */
#include "sqlite_writer.h" /* CBMDumpNode, CBMDumpEdge, pmm_write_db */
...
static int make_temp_db(char *path, size_t pathsz) {
    snprintf(path, pathsz, "/tmp/pmm_sw_test_XXXXXX");
...
TEST(sw_minimal_data) {
    ...
    int rc = pmm_write_db(path, "test", "/tmp/test", "2026-03-14T00:00:00Z", nodes, 2, edges, 1,
                          NULL, 0, NULL, 0);
    ASSERT_EQ(rc, 0);
...
TEST(sw_imports_local_name_unique) {
    ...
    int rc = pmm_write_db(path, "test", "/tmp/test", "2026-03-14T00:00:00Z", nodes, 2, edges, 2,
                          NULL, 0, NULL, 0);
    ASSERT_EQ(rc, 0);
...
TEST(sw_scale_and_indexes) {
    ...
    int rc = pmm_write_db(path, "proj", "/repo", "2026-03-14T12:00:00Z", nodes, 100, edges,
...
TEST(sw_long_index_keys_overflow) {
    ...
    int rc = pmm_write_db(path, "test", "/tmp/test", "2026-03-14T00:00:00Z", nodes, 3, NULL, 0,
                          NULL, 0, NULL, 0);
    ASSERT_EQ(rc, 0);
...
TEST(sw_empty) {
    ...
    int rc = pmm_write_db(path, "test", "/tmp/test", "2026-03-14T00:00:00Z", NULL, 0, NULL, 0, NULL,
                          0, NULL, 0);
    ASSERT_EQ(rc, 0);
...
TEST(sw_multi_page) {
    ...
    int rc =
        pmm_write_db(path, "p", "/r", "2026-01-01T00:00:00Z", nodes, N, NULL, 0, NULL, 0, NULL, 0);
    ASSERT_EQ(rc, 0);
...
TEST(sw_oversized_node) {
    ...
    int rc = pmm_write_db(path, "test", "/tmp/test", "2026-03-28T00:00:00Z", nodes, 1, NULL, 0,
                          NULL, 0, NULL, 0);
    free(big_props);
    ASSERT_EQ(rc, 0);
...
TEST(sw_stream_open_does_not_truncate_destination) {
    ...
    int rc = pmm_writer_finalize(w, "test", "/tmp/test", "2026-07-07T00:00:00Z", NULL, 0, NULL, 0,
                                 NULL, 0, NULL, 0);
    ASSERT_EQ(rc, 0);
...
TEST(sw_publish_removes_destination_sidecars) {
    ...
    int rc = pmm_write_db(path, "test", "/tmp/test", "2026-07-07T00:00:00Z", NULL, 0, NULL, 0, NULL,
                          0, NULL, 0);
    ASSERT_EQ(rc, 0);
...
TEST(sw_publish_failure_preserves_destination_sidecars) {
    char path[256];
    snprintf(path, sizeof(path), "/tmp/pmm_sw_dir_XXXXXX");
    ASSERT(pmm_mkdtemp(path) != NULL);
...
TEST(sw_publish_supports_non_ascii_path) {
    char dir[256];
    snprintf(dir, sizeof(dir), "/tmp/pmm_sw_utf8_XXXXXX");
    ASSERT(pmm_mkdtemp(dir) != NULL);
...
TEST(sw_publish_preserves_live_reader) {
    ...
    int rc = pmm_write_db(path, "test", "/tmp/test", "2026-07-07T00:00:00Z", old_nodes, 1, NULL, 0,
                          NULL, 0, NULL, 0);
    ASSERT_EQ(rc, 0);
    ...
    rc = pmm_write_db(path, "test", "/tmp/test", "2026-07-07T00:00:01Z", new_nodes, 1, NULL, 0,
                      NULL, 0, NULL, 0);
    ASSERT_EQ(rc, 0);
