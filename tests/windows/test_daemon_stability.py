...
each other and from any interactive PMM use on the host.

Exit code: 0 == all sections green, 1 == regression, 2 == setup error.

Usage:
    python test_daemon_stability.py <path-to-project-mind-mcp[.exe]>
"""
import json
import os
import re
...
def section_hook_fail_open(binary, work):
    ...
    if first.returncode != 0:
        ...
        return False
    if "systemMessage" not in first_out or "no PMM daemon" not in first_err:
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="pmm_daemon_stab_")
    sections = [
        section_params,
