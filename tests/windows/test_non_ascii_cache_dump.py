...
succeed and `pipeline.err phase=dump`: pmm_writer_open used a raw ANSI-CP
fopen for the hand-rolled SQLite writer (internal/pmm/sqlite_writer.c), the
one file-creating call on the dump chain without UTF-8→wide conversion.
Fixed by routing it through pmm_fopen (same pattern as #700/#973).

This guard indexes an ASCII repo into a NON-ASCII cache dir (PMM_CACHE_DIR is
...
Usage:
    python test_non_ascii_cache_dump.py <path-to-project-mind-mcp[.exe]>
"""
import json
import os
import shutil
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="pmm_996_")
    try:
