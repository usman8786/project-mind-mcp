...
The documented entrypoint `project-mind-mcp cli index_repository '<json>'`
...
Usage:
    python test_cli_non_ascii_arg.py <path-to-project-mind-mcp[.exe]>
"""
import json
import os
import shutil
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="pmm_win_cliarg_")
    try:
        ...
        env = dict(os.environ)
        env["PMM_CACHE_DIR"] = os.path.join(work, "cache_ascii")
        ctrl = subprocess.run(
        ...
        # PMM_INDEX_SUPERVISOR=0 for determinism across the other guards; forcing it OFF
        ...
        env2.pop("PMM_INDEX_SUPERVISOR", None)
        ...
        # paths on CJK-codepage systems) before the pmm_canonical_path fix.
        cjk_repo = os.path.join(work, "\u96f7\u9054\u6e2c\u8a66")
        make_fixture(cjk_repo)
        env3 = dict(os.environ)
        env3["PMM_CACHE_DIR"] = os.path.join(work, "cache_cjk")
        env3.pop("PMM_INDEX_SUPERVISOR", None)
