...
Requires a UI build (`make -f Makefile.pmm pmm-with-ui`) because the HTTP server
...
Usage:
    python test_ui_drive_listing.py <path-to-project-mind-mcp-ui[.exe]> [port]
"""
import json
import os
import shutil
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="pmm_win_uidrv_")
    port = int(sys.argv[2]) if len(sys.argv) > 2 else free_port()
    env = dict(os.environ)
    env["PMM_CACHE_DIR"] = os.path.join(work, "cache")
    os.makedirs(env["PMM_CACHE_DIR"], exist_ok=True)
    ...
    try:
        if not wait_for_server(port, timeout=25):
            print("PRECONDITION: HTTP server did not start on port %d. Is this a "
                  "UI build (make -f Makefile.pmm pmm-with-ui)?" % port)
            return 2
