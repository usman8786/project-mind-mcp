...
Windows ships ONE binary, exactly like Linux and macOS: ``project-mind-mcp.exe``.
...
process into install.ps1, which runs while PMM is NOT running.
...
* ``update`` refuses to reach the network first: it hands off before it
  consults PMM_DOWNLOAD_URL, so it stays fast even when that URL is a black
  hole.
...
Usage:
    python test_windows_update_handoff.py <project-mind-mcp.exe>
"""
...
def copy_binary(source, directory):
    ...
    binary = directory / "project-mind-mcp.exe"
...
def assert_update_hands_off_to_install_script(source, env, work):
    ...
    command_env["PMM_DOWNLOAD_URL"] = "https://127.0.0.1:1"
    ...
    require(
        not (binary.parent / "project-mind-mcp.payload.exe").exists(),
...
def main():
    if os.name != "nt":
        ...
        return 2
    if len(sys.argv) != 2:
        print("usage: python test_windows_update_handoff.py <project-mind-mcp.exe>")
        return 2
    ...
    work = pathlib.Path(tempfile.mkdtemp(prefix="pmm_win_update_"))
    try:
