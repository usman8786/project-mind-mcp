...
surface (real project-mind-mcp process, real SQLite DB, real stdio). Two
...
nothing. #700 routed the per-pass reads through pmm_fopen (→ _wfopen with a wide
...
Usage:
    python test_non_ascii_path.py <path-to-project-mind-mcp[.exe]>
"""
import json
import os
import shutil
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="pmm_win_nonascii_")
    failures = []
    try:
    ...
    if failures:
        ...
        print("Invariant violated: byte-identical fixtures under non-ASCII paths "
              ...
              "#700 — has the pmm_fopen routing in the pass readers regressed?).")
        return 1
