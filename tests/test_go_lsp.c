 ...
 *
 * Ports from internal/pmm/lsp_test.go (49 tests).
 * Categories:
...
/* Extract a Go file using pmm_extract_file (runs single-file LSP internally) */
static CBMFileResult *extract_go(const char *source) {
    return pmm_extract_file(source, (int)strlen(source), PMM_LANG_GO, "test", "main.go", 0, NULL,
                            NULL);
}
...
/* ── Cross-file tests (use pmm_run_go_lsp_cross directly) ──────── */
...
TEST(golsp_crossfile_method_dispatch) {
    ...
    pmm_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 4, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(golsp_crossfile_interface_field_chain) {
    ...
    pmm_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 6, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(golsp_crossfile_map_index) {
    ...
    pmm_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 3, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(golsp_crossfile_stdlib_interface) {
    ...
    pmm_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 1, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(golsp_crossfile_local_interface_single_impl) {
    ...
    pmm_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 5, imp_names,
                         imp_qns, 1, NULL, &out);
