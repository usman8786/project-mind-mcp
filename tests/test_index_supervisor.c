...
static bool index_supervisor_test_poll_terminal(pmm_index_worker_handle_t *handle,
                                                uint32_t timeout_ms,
                                                const pmm_index_worker_result_t **result_out) {
    uint64_t deadline = pmm_now_ms() + timeout_ms;
    do {
        pmm_index_worker_poll_t state = pmm_index_worker_poll(handle, result_out);
    ...
    return pmm_index_worker_poll(handle, result_out) == PMM_INDEX_WORKER_POLL_TERMINAL;
}

static bool index_supervisor_test_wait_file(pmm_index_worker_handle_t *handle, const char *path,
                                            char *out, size_t out_size) {
    uint64_t deadline = pmm_now_ms() + INDEX_SUPERVISOR_TEST_READY_MS;
    do {
        ...
        const pmm_index_worker_result_t *unexpected = NULL;
        if (pmm_index_worker_poll(handle, &unexpected) != PMM_INDEX_WORKER_POLL_RUNNING) {
            return false;
        }
        index_supervisor_test_pause();
    } while (pmm_now_ms() < deadline);
...
static void index_supervisor_test_dump(const char *label, const char *path) {
    ...
    FILE *file = path && path[0] ? pmm_fopen(path, "rb") : NULL;
    if (!file) {
...
static void index_supervisor_test_cleanup_handle(pmm_index_worker_handle_t *handle) {
    if (!handle) {
        return;
    }
    (void)pmm_index_worker_request_cancel(handle);
    const pmm_index_worker_result_t *result = NULL;
...
static void index_supervisor_test_capture_log(const char *line, void *context) {
    ...
    if (capture->render_progress) {
        pmm_progress_sink_fn(line);
    }
}

TEST(index_supervisor_worker_argv_requires_exact_build_bound_grammar) {
    const char *captured = pmm_index_supervisor_build_fingerprint();
    ASSERT_NOT_NULL(captured);
    ASSERT_EQ(strlen(captured), PMM_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH);

    char mismatched[PMM_INDEX_WORKER_BUILD_FINGERPRINT_SIZE];
    ...
    char *valid[] = {
        ...
        "cli",
        "--index-worker",
        PMM_INDEX_WORKER_BUILD_ARG,
        (char *)captured,
        "index_repository",
        "{\"__pmm_test_worker\":\"hang-tree\"}",
        "--response-out",
        "/tmp/r",
        PMM_INDEX_WORKER_MEMORY_BUDGET_ARG,
        "1024",
        PMM_INDEX_WORKER_SINGLE_THREAD_ARG,
        PMM_INDEX_WORKER_MARKER_ARG,
        "/tmp/m",
        PMM_INDEX_WORKER_QUARANTINE_ARG,
        "/tmp/q",
        NULL,
    };
    pmm_index_worker_invocation_t invocation;
    ASSERT_EQ(pmm_index_worker_parse_process_argv(16, valid, &invocation),
              PMM_INDEX_WORKER_ARGV_VALID);
    ASSERT_EQ(pmm_daemon_process_role(16, valid), PMM_DAEMON_PROCESS_WORKER);
    ...
    ASSERT_EQ(pmm_index_worker_parse_process_argv(7, missing_build, &invocation),
              PMM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(pmm_daemon_process_role(7, missing_build), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(pmm_index_worker_parse_process_argv(9, wrong_build, &invocation),
              PMM_INDEX_WORKER_ARGV_BUILD_MISMATCH);
    ASSERT_EQ(pmm_index_worker_parse_process_argv(13, reordered, &invocation),
              PMM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(pmm_index_worker_parse_process_argv(10, trailing, &invocation),
              PMM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(pmm_index_worker_parse_process_argv(11, zero_budget, &invocation),
              PMM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(pmm_daemon_process_role(11, zero_budget), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(pmm_index_worker_parse_process_argv(11, overflow_budget, &invocation),
              PMM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(pmm_daemon_process_role(11, overflow_budget), PMM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(pmm_index_worker_parse_process_argv(5, user_value, &invocation),
              PMM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(pmm_daemon_process_role(5, user_value), PMM_DAEMON_PROCESS_INVALID);
    PASS();
}
...
TEST(index_supervisor_async_jobs_are_isolated_cancellable_and_terminal_cached) {
    ...
    ASSERT_EQ(pmm_daemon_process_role(14, worker_argv), PMM_DAEMON_PROCESS_WORKER);

    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/pmm-index-async-XXXXXX", pmm_tmpdir());
    ...
    const char *old_single = getenv("PMM_INDEX_SINGLE_THREAD");
    const char *old_marker = getenv("PMM_INDEX_MARKER_FILE");
    const char *old_quarantine = getenv("PMM_INDEX_QUARANTINE_FILE");
    char *saved_cache = old_cache ? pmm_strdup(old_cache) : NULL;
    char *saved_single = old_single ? pmm_strdup(old_single) : NULL;
    char *saved_marker = old_marker ? pmm_strdup(old_marker) : NULL;
    char *saved_quarantine = old_quarantine ? pmm_strdup(old_quarantine) : NULL;
    (void)pmm_setenv("PMM_CACHE_DIR", cache, 1);
    (void)pmm_setenv("PMM_INDEX_SINGLE_THREAD", "parent-single", 1);
    (void)pmm_setenv("PMM_INDEX_MARKER_FILE", "parent-marker", 1);
    (void)pmm_setenv("PMM_INDEX_QUARANTINE_FILE", "parent-quarantine", 1);

    const char args[] = "{\"__pmm_test_worker\":\"hang-tree\"}";
    pmm_index_worker_handle_t *first = NULL;
    pmm_index_worker_handle_t *second = NULL;
    index_supervisor_log_capture_t first_capture = {0};
    index_supervisor_log_capture_t second_capture = {0};
    uint64_t start_before = pmm_now_ms();
    int first_rc =
        pmm_index_worker_start_with_log(args, 4096, true, marker_a, quarantine_a,
                                        index_supervisor_test_capture_log, &first_capture, &first);
    int second_rc = pmm_index_worker_start_with_log(args, 8192, true, marker_b, quarantine_b,
                                                    index_supervisor_test_capture_log,
                                                    &second_capture, &second);
    uint64_t start_elapsed = pmm_now_ms() - start_before;
    ...
    if (first) {
        (void)snprintf(response_a, sizeof(response_a), "%s", pmm_index_worker_response_path(first));
        (void)snprintf(log_a, sizeof(log_a), "%s", pmm_index_worker_log_path(first));
    }
    if (second) {
        (void)snprintf(response_b, sizeof(response_b), "%s",
                       pmm_index_worker_response_path(second));
        (void)snprintf(log_b, sizeof(log_b), "%s", pmm_index_worker_log_path(second));
    }
    ...
    bool parent_unchanged = strcmp(getenv("PMM_INDEX_SINGLE_THREAD"), "parent-single") == 0 &&
                            strcmp(getenv("PMM_INDEX_MARKER_FILE"), "parent-marker") == 0 &&
                            strcmp(getenv("PMM_INDEX_QUARANTINE_FILE"), "parent-quarantine") == 0;
    ...
    const pmm_index_worker_result_t *premature = (const pmm_index_worker_result_t *)1;
    uint64_t poll_before = pmm_now_ms();
    pmm_index_worker_poll_t running =
        first ? pmm_index_worker_poll(first, &premature) : PMM_INDEX_WORKER_POLL_ERROR;
    uint64_t poll_elapsed = pmm_now_ms() - poll_before;
    uint64_t cancel_before = pmm_now_ms();
    bool first_cancel = first && pmm_index_worker_request_cancel(first);
    bool second_cancel = second && pmm_index_worker_request_cancel(second);
    uint64_t cancel_elapsed = pmm_now_ms() - cancel_before;

    const pmm_index_worker_result_t *first_result = NULL;
    const pmm_index_worker_result_t *second_result = NULL;
    ...
    bool cached_result = first_terminal &&
                         pmm_index_worker_poll(first, &cached) == PMM_INDEX_WORKER_POLL_TERMINAL &&
                         cached == first_result;
    ...
    bool terminal_cancel_rejected = first && !pmm_index_worker_request_cancel(first);
    bool response_cleaned = pmm_file_size(response_a) < 0 && pmm_file_size(response_b) < 0;
    bool failure_logs_kept = pmm_file_size(log_a) >= 0 && pmm_file_size(log_b) >= 0;

    if (first_terminal) {
        pmm_index_worker_destroy(first);
    } else {
        index_supervisor_test_cleanup_handle(first);
    }
    if (second_terminal) {
        pmm_index_worker_destroy(second);
    } else {
    ...
    (void)pmm_unlink(log_a);
    (void)pmm_unlink(log_b);
    (void)pmm_unlink(marker_a);
    (void)pmm_unlink(marker_b);
    index_supervisor_test_restore_env("PMM_CACHE_DIR", saved_cache);
    index_supervisor_test_restore_env("PMM_INDEX_SINGLE_THREAD", saved_single);
    index_supervisor_test_restore_env("PMM_INDEX_MARKER_FILE", saved_marker);
    index_supervisor_test_restore_env("PMM_INDEX_QUARANTINE_FILE", saved_quarantine);
    ...
    ASSERT_EQ(running, PMM_INDEX_WORKER_POLL_RUNNING);
...
TEST(index_supervisor_sync_wrapper_forwards_cancel_and_drains_tree) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/pmm-index-sync-cancel-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    const char *old_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache = old_cache ? pmm_strdup(old_cache) : NULL;
    (void)pmm_setenv("PMM_CACHE_DIR", cache, 1);
    ...
    pmm_index_worker_result_t result = {0};
    int run_status =
        pmm_index_spawn_worker_with_log_cancel("{\"__pmm_test_worker\":\"hang-tree\"}", false, NULL,
    ...
    pmm_index_worker_result_free(&result);
    index_supervisor_test_restore_env("PMM_CACHE_DIR", saved_cache);
    (void)th_rmtree(cache);
...
static bool index_supervisor_test_run_probe(const char *mode, bool profiling,
                                            pmm_proc_outcome_t *outcome_out, bool *has_response_out,
                                            bool *log_exists_out, bool *response_path_exists_out) {
    char args[128];
    (void)snprintf(args, sizeof(args), "{\"__pmm_test_worker\":\"%s\"}", mode);
    pmm_profile_active = profiling;
    pmm_index_worker_handle_t *handle = NULL;
    if (pmm_index_worker_start(args, 0, false, NULL, NULL, &handle) != 0 || !handle) {
        return false;
    }
    char log_path[INDEX_SUPERVISOR_TEST_PATH_CAP];
    char response_path[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(log_path, sizeof(log_path), "%s", pmm_index_worker_log_path(handle));
    (void)snprintf(response_path, sizeof(response_path), "%s",
                   pmm_index_worker_response_path(handle));
    const pmm_index_worker_result_t *result = NULL;
    bool terminal =
    ...
    if (terminal && result) {
        ...
        *log_exists_out = pmm_file_size(log_path) >= 0;
        *response_path_exists_out = pmm_file_size(response_path) >= 0;
        pmm_index_worker_destroy(handle);
    } else {
    ...
    if (pmm_file_size(log_path) >= 0) {
        (void)pmm_unlink(log_path);
    }
    return terminal;
}
...
TEST(index_supervisor_terminal_log_lifecycle_matches_outcome_and_profiling) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/pmm-index-logs-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    const char *old_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache = old_cache ? pmm_strdup(old_cache) : NULL;
    bool saved_profile = pmm_profile_active;
    (void)pmm_setenv("PMM_CACHE_DIR", cache, 1);

    pmm_proc_outcome_t clean_outcome = PMM_PROC_SPAWN_FAILED;
    pmm_proc_outcome_t profile_outcome = PMM_PROC_SPAWN_FAILED;
    pmm_proc_outcome_t crash_outcome = PMM_PROC_SPAWN_FAILED;
...
#ifdef _WIN32
    bool crash_classified_failure = crash_outcome != PMM_PROC_CLEAN;
#else
    bool crash_classified_failure = crash_outcome == PMM_PROC_CRASH;
#endif

    pmm_profile_active = saved_profile;
    index_supervisor_test_restore_env("PMM_CACHE_DIR", saved_cache);
    (void)th_rmtree(cache);

    ASSERT_TRUE(clean_terminal);
    ASSERT_EQ(clean_outcome, PMM_PROC_CLEAN);
    ASSERT_TRUE(clean_response);
    ASSERT_FALSE(clean_log);
    ASSERT_FALSE(clean_response_file);
    ASSERT_TRUE(profile_terminal);
    ASSERT_EQ(profile_outcome, PMM_PROC_CLEAN);
...
TEST(index_supervisor_drains_terminal_backlog_into_request_progress_callback) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/pmm-index-relay-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    const char *old_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache = old_cache ? pmm_strdup(old_cache) : NULL;
    (void)pmm_setenv("PMM_CACHE_DIR", cache, 1);
    ...
    pmm_progress_sink_init(progress);
    pmm_index_worker_handle_t *handle = NULL;
    int start_rc =
        pmm_index_worker_start_with_log("{\"__pmm_test_worker\":\"clean\"}", 0, false, NULL, NULL,
    ...
    pmm_proc_outcome_t outcome = result ? result->outcome : PMM_PROC_SPAWN_FAILED;
    int callbacks_at_terminal = capture.delivered;
    const pmm_index_worker_result_t *cached = NULL;
    bool terminal_cached =
        terminal && pmm_index_worker_poll(handle, &cached) == PMM_INDEX_WORKER_POLL_TERMINAL &&
    ...
    ASSERT_EQ(outcome, PMM_PROC_CLEAN);
...
TEST(index_supervisor_oversized_response_is_contained_and_log_is_retained) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/pmm-index-oversize-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    const char *old_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache = old_cache ? pmm_strdup(old_cache) : NULL;
    (void)pmm_setenv("PMM_CACHE_DIR", cache, 1);
    ...
    pmm_index_worker_handle_t *handle = NULL;
    int start_rc = pmm_index_worker_start("{\"__pmm_test_worker\":\"oversize\"}", 0, false, NULL,
                                          NULL, &handle);
    char log_path[INDEX_SUPERVISOR_TEST_PATH_CAP] = {0};
    ...
    bool rejected = terminal && result && result->response_rejected && !result->response &&
                    result->outcome == PMM_PROC_EXIT_NONZERO && result->tree_quiesced &&
                    !result->supervision_failed;
    bool log_retained = log_path[0] && pmm_file_size(log_path) >= 0;
    bool response_removed = response_path[0] && pmm_file_size(response_path) < 0;
    if (terminal) {
        pmm_index_worker_destroy(handle);
    } else {
