 ...
 *
 * Drives pmm_dump_verify_is_degraded against a real on-disk SQLite store and
 * the actual pmm_store_count_nodes / pmm_store_open_path_query path used by
...
static void dv_temp_path(char *buf, size_t n) {
    snprintf(buf, n, "%s/cmm_dump_verify_%d.db", pmm_tmpdir(), (int)getpid());
}
...
static int dv_write_nodes(const char *path, int count) {
    ...
    if (!s)
        return -1;
    if (pmm_store_upsert_project(s, DV_PROJECT, "/tmp/io_dump_verify") != PMM_STORE_OK) {
        pmm_store_close(s);
        return -1;
    }
    pmm_store_begin(s);
    for (int i = 0; i < count; i++) {
        ...
        pmm_node_t node = {.project = DV_PROJECT,
                           .label = "Function",
                           .name = "n",
                           .qualified_name = qn,
                           .file_path = "io.c",
                           .start_line = i,
                           .end_line = i};
        if (pmm_store_upsert_node(s, &node) < 0) {
            pmm_store_close(s);
            return -1;
        }
    }
...
static int dv_persisted_nodes(const char *path) {
    pmm_store_t *s = pmm_store_open_path_query(path);
    if (!s)
        return PMM_STORE_ERR;
    int n = pmm_store_count_nodes(s, DV_PROJECT);
    pmm_store_close(s);
    return n;
}
...
TEST(dump_verify_io_full_persist_ok) {
    ...
    ASSERT_FALSE(pmm_dump_verify_is_degraded(200, persisted, PMM_DUMP_VERIFY_DEFAULT_RATIO,
...
TEST(dump_verify_io_shortfall_after_delete) {
    ...
    ASSERT_FALSE(pmm_dump_verify_is_degraded(1000, dv_persisted_nodes(path),
    ...
    pmm_store_t *s = pmm_store_open_path(path);
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_exec(s, "DELETE FROM nodes WHERE start_line >= 100"), PMM_STORE_OK);
    pmm_store_close(s);

    int persisted = dv_persisted_nodes(path);
    ASSERT_EQ(persisted, 100);
    ASSERT_TRUE(pmm_dump_verify_is_degraded(1000, persisted, PMM_DUMP_VERIFY_DEFAULT_RATIO,
                                            PMM_DUMP_VERIFY_MIN_FLOOR));
...
TEST(dump_verify_io_fork_crash_uncommitted_discarded) {
    ...
    ASSERT_TRUE(pmm_dump_verify_is_degraded(5060, persisted, PMM_DUMP_VERIFY_DEFAULT_RATIO,
                                            PMM_DUMP_VERIFY_MIN_FLOOR));
