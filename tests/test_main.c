...
#include "foundation/compat.h"     /* pmm_setenv — #845 supervisor kill switch */
#include "foundation/compat_fs.h"  /* pmm_fopen — worker response file */
#include "foundation/mem.h"        /* pmm_mem_init — worker budget */
#include "foundation/platform.h"   /* pmm_file_exists — blocking-git marker */
...
#include "mcp/index_supervisor.h"  /* pmm_index_set_worker_role */
#include "mcp/mcp.h"               /* pmm_mcp_handle_tool — act as a real worker */
#include <sqlite3.h>
#include <errno.h>
#include <stdbool.h>
#include <stdint.h>
...
#define TF_BLOCKING_GIT_MARKER_ENV "PMM_TEST_RUNTIME_BLOCKING_GIT_PID_FILE"
...
#endif
    FILE *marker = pmm_fopen(marker_path, "wbx");
    if (!marker) {
        ...
        bool marker_exists = pmm_file_exists(marker_path);
...
#else
        return pmm_file_exists(marker_path) ? 0 : 30;
#endif
    }
#ifdef _WIN32
...
static bool tf_setup_cache_sentinel(void) {
    snprintf(tf_home_sentinel, sizeof(tf_home_sentinel), "/tmp/pmm-test-home-XXXXXX");
    if (!pmm_mkdtemp(tf_home_sentinel)) {
        return false;
    }
    /* Legacy integration fixtures derive DB paths from HOME, while production
     * cache_dir() prefers PMM_CACHE_DIR. A private HOME plus no inherited cache
     * override keeps both conventions pointed at the same isolated tree. */
    pmm_setenv("HOME", tf_home_sentinel, 1);
...
static void tf_index_worker_probe(const char *args_json, const char *response_out) {
    if (!args_json || !strstr(args_json, "\"__pmm_test_worker\"")) {
        return;
    }
    if (strstr(args_json, "\"clean\"")) {
        FILE *response = response_out ? pmm_fopen(response_out, "wb") : NULL;
        if (response) {
    ...
    if (strstr(args_json, "\"oversize\"")) {
        ...
        if (response) {
            written =
                fseek(response, (long)PMM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX, SEEK_SET) == 0 &&
...
#endif
        const char *marker = getenv("PMM_INDEX_MARKER_FILE");
        FILE *ready = marker ? pmm_fopen(marker, "wb") : NULL;
        if (ready) {
            (void)fprintf(
                ready, "single=%s\nmarker=%s\nquarantine=%s\nbudget=%zu\ndescendant=%ld\n",
                getenv("PMM_INDEX_SINGLE_THREAD") ? getenv("PMM_INDEX_SINGLE_THREAD") : "", marker,
                getenv("PMM_INDEX_QUARANTINE_FILE") ? getenv("PMM_INDEX_QUARANTINE_FILE") : "",
                pmm_mem_budget(), descendant);
            (void)fclose(ready);
        }
 ...
 * exact build-bound worker grammar produced by pmm_index_worker_start(), act
...
static int tf_maybe_run_index_worker(int argc, char **argv) {
    ...
    pmm_index_worker_argv_status_t status =
        pmm_index_worker_parse_process_argv(argc, argv, &invocation);
    if (status == PMM_INDEX_WORKER_ARGV_NOT_WORKER) {
        return -1;
    }
    if (status != PMM_INDEX_WORKER_ARGV_VALID) {
        (void)fprintf(stderr, "PMM test index worker could not start: %s\n",
                      pmm_index_worker_argv_status_message(status));
        return 1;
    }
    ...
    pmm_mem_init_with_cap(0.5, invocation.memory_budget_bytes);
    tf_index_worker_probe(invocation.args_json, invocation.response_out);
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    if (!srv) {
        return 1;
    }
    char *result = pmm_mcp_handle_tool(srv, "index_repository", invocation.args_json);
    if (result) {
 ...
 * pmm_popen — the same cmd.exe-grandchild path git takes — passing the numeric
 * value of an inheritable listening-socket handle. If pmm_popen correctly
...
#ifdef _WIN32
    if (argc < 3 || strcmp(argv[1], "__pmm_sockprobe") != 0) {
        return -1;
    }
    WSADATA wsa;
...
static int tf_maybe_run_daemon_ipc_lock_probe(int argc, char **argv) {
#ifdef _WIN32
    if (argc != 5 || strcmp(argv[1], "__pmm_daemon_ipc_lock_probe") != 0) {
        return -1;
    }
    pmm_daemon_ipc_endpoint_t *endpoint = pmm_daemon_ipc_endpoint_new(argv[3], argv[4]);
    if (!endpoint) {
        return 21;
    }
    int result = -1;
    if (strcmp(argv[2], "startup") == 0) {
        ...
        result = pmm_daemon_ipc_startup_lock_try_acquire(endpoint, &lock);
        if (!pmm_daemon_ipc_startup_lock_release(&lock)) {
            result = -1;
        }
    } else if (strcmp(argv[2], "lifetime") == 0) {
        pmm_daemon_ipc_lifetime_reservation_t *reservation = NULL;
        result = pmm_daemon_ipc_lifetime_reservation_try_acquire(endpoint, &reservation);
...
static int tf_maybe_run_version_cohort_crash_holder(int argc, char **argv) {
#ifdef _WIN32
    if (argc != 5 || strcmp(argv[1], "__pmm_version_cohort_crash_holder") != 0) {
        return -1;
    }
    pmm_daemon_ipc_endpoint_t *endpoint = pmm_daemon_ipc_endpoint_new(argv[2], argv[3]);
    ...
    pmm_version_cohort_status_t status =
        manager ? pmm_version_cohort_acquire(manager, &identity, UINT64_MAX, &lease, &conflict)
                : PMM_VERSION_COHORT_IO;
    FILE *ready = status == PMM_VERSION_COHORT_OK ? pmm_fopen(argv[4], "wb") : NULL;
...
#ifdef _WIN32
    if (argc != 3 || strcmp(argv[1], "__pmm_runtime_image_holder") != 0) {
        return -1;
    }
...
static int tf_maybe_run_runtime_hello_client(int argc, char **argv) {
    if (argc != 6 || strcmp(argv[1], "__pmm_runtime_hello_client") != 0) {
        return -1;
    }
    pmm_daemon_ipc_endpoint_t *endpoint = pmm_daemon_ipc_endpoint_new(argv[3], argv[2]);
    pmm_daemon_build_identity_t identity = {
    ...
    pmm_daemon_runtime_client_t *client =
        endpoint ? pmm_daemon_runtime_client_connect(endpoint, &identity,
                                                     TF_RUNTIME_IMAGE_EXCHANGE_TIMEOUT_MS, &result)
                 : NULL;
    bool accepted = client && result.status == PMM_DAEMON_RUNTIME_CONNECT_ACCEPTED &&
                    result.hello_status == PMM_DAEMON_HELLO_COMPATIBLE;
    bool closed =
        !client || pmm_daemon_runtime_client_close(client, TF_RUNTIME_IMAGE_EXCHANGE_TIMEOUT_MS);
    pmm_daemon_ipc_endpoint_free(endpoint);
    if (!accepted) {
        return 26;
    }
...
static int tf_maybe_run_runtime_activation_client(int argc, char **argv) {
    if (argc != 7 || strcmp(argv[1], "__pmm_runtime_activation_client") != 0) {
        return -1;
    }
    ...
    bool action_valid = action_end != argv[6] && *action_end == '\0' &&
                        action_value >= (unsigned long)PMM_DAEMON_RUNTIME_ACTIVATION_INSTALL &&
                        action_value <= (unsigned long)PMM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL;
    pmm_daemon_ipc_endpoint_t *endpoint =
        action_valid ? pmm_daemon_ipc_endpoint_new(argv[3], argv[2]) : NULL;
    pmm_daemon_build_identity_t identity = {
        .semantic_version = argv[4],
        .build_fingerprint = argv[5],
    };
    pmm_daemon_runtime_activation_result_t result;
    memset(&result, 0, sizeof(result));
    bool exchanged =
        endpoint && pmm_daemon_runtime_request_activation_shutdown(
                        endpoint, &identity, (pmm_daemon_runtime_activation_action_t)action_value,
                        TF_RUNTIME_IMAGE_EXCHANGE_TIMEOUT_MS, &result);
    pmm_daemon_ipc_endpoint_free(endpoint);
...
static int tf_maybe_run_runtime_mapped_hello_client(int argc, char **argv) {
#ifdef __APPLE__
    if (argc != 7 || strcmp(argv[1], "__pmm_runtime_mapped_hello_client") != 0) {
        return -1;
    }
    ...
    char *hello_argv[] = {argv[0], "__pmm_runtime_hello_client", argv[3], argv[4], argv[5],
                          argv[6]};
...
#ifndef _WIN32
    if (argc != 4 || strcmp(argv[1], "__pmm_mcp_idxfailclosed_probe") != 0) {
        return -1;
    }
...
/* PMM_SKIP_PERF=1 excludes throughput/scale/perf-metric suites from the run.
 * The fidelity pass (PMM_LOCAL_CI_CPUS=4) and CI PRs set it: those suites
 ...
 * suites run at full power in the perf/release legs (PMM_SKIP_PERF unset). The
...
extern void suite_extract_docs(void);
...
extern void pmm_kind_in_set_free_cache(void);
...
int main(int argc, char **argv) {
     ...
     * only under PMM_CLI_ENABLE_TEST_API — never in a production binary. */
    if (!getenv("PMM_TEST_BUILD_FINGERPRINT")) {
        (void)pmm_setenv("PMM_TEST_BUILD_FINGERPRINT",
    ...
    if (argc == 2 && strcmp(argv[1], "--version") == 0) {
        (void)puts("project-mind-mcp test-runner");
        return 0;
    }
    ...
    (void)pmm_index_supervisor_capture_build_fingerprint();
    ...
    /* #845 belt-and-suspenders: this binary EMBEDS pmm_mcp_handle_tool. The
    ...
    const char *skip_perf_env = getenv("PMM_SKIP_PERF");
    ...
    RUN_SELECTED_SUITE(extract_docs);
