...
static bool activation_test_fixture(char out[ACTIVATION_TEST_PATH_CAP]) {
    int written = snprintf(out, ACTIVATION_TEST_PATH_CAP, "%s/pmm-activation-transaction-XXXXXX",
                           pmm_tmpdir());
    return written > 0 && written < ACTIVATION_TEST_PATH_CAP && pmm_mkdtemp(out) != NULL;
}
...
TEST(activation_transaction_stages_same_directory_private_executable_and_aborts) {
    ...
    ASSERT_TRUE(activation_test_path(target, directory, "pmm"));

    pmm_activation_transaction_t *transaction = NULL;
    ASSERT_EQ(pmm_activation_transaction_stage_bytes(target, "candidate", strlen("candidate"),
                                                     &transaction),
    ...
    ASSERT_EQ(pmm_activation_transaction_close(&transaction), PMM_ACTIVATION_TRANSACTION_OK);
...
TEST(activation_transaction_commit_keeps_backup_until_finalize) {
    ...
    pmm_activation_transaction_t *transaction = NULL;
    ASSERT_EQ(pmm_activation_transaction_stage_bytes(target, "new", strlen("new"), &transaction),
              PMM_ACTIVATION_TRANSACTION_OK);
    const char *backup = pmm_activation_transaction_backup_path(transaction);
    ...
    ASSERT_EQ(pmm_activation_transaction_commit(transaction, activation_test_validate, &validation),
              PMM_ACTIVATION_TRANSACTION_OK);
    char contents[ACTIVATION_TEST_CONTENT_CAP];
    ...
    ASSERT_EQ(pmm_activation_transaction_finalize(transaction), PMM_ACTIVATION_TRANSACTION_OK);
...
TEST(activation_transaction_validation_failure_restores_previous_target) {
    ...
    pmm_activation_transaction_t *transaction = NULL;
    ASSERT_EQ(pmm_activation_transaction_stage_bytes(target, "bad", strlen("bad"), &transaction),
              PMM_ACTIVATION_TRANSACTION_OK);
    const char *backup = pmm_activation_transaction_backup_path(transaction);
    ASSERT_NOT_NULL(backup);
    char backup_copy[ACTIVATION_TEST_PATH_CAP];
    (void)snprintf(backup_copy, sizeof(backup_copy), "%s", backup);
    ASSERT_EQ(pmm_activation_transaction_commit(transaction, activation_test_reject, NULL),
              PMM_ACTIVATION_TRANSACTION_VALIDATION_FAILED);
...
TEST(activation_transaction_explicit_rollback_restores_previous_target) {
    ...
    ASSERT_EQ(pmm_activation_transaction_commit(transaction, activation_test_validate, &validation),
              PMM_ACTIVATION_TRANSACTION_OK);
    ASSERT_EQ(pmm_activation_transaction_rollback(transaction), PMM_ACTIVATION_TRANSACTION_OK);
    char contents[ACTIVATION_TEST_CONTENT_CAP];
    ASSERT_TRUE(activation_test_read(target, contents));
    ASSERT_STR_EQ(contents, "old");
    ASSERT_EQ(pmm_activation_transaction_close(&transaction), PMM_ACTIVATION_TRANSACTION_OK);
...
TEST(activation_transaction_stage_file_installs_new_target) {
    ...
    char target[ACTIVATION_TEST_PATH_CAP];
    ASSERT_TRUE(activation_test_fixture(directory));
    ASSERT_TRUE(activation_test_path(source, directory, "downloaded-pmm"));
    ASSERT_TRUE(activation_test_path(target, directory, "pmm"));
    ASSERT_TRUE(activation_test_write(source, "downloaded"));

    pmm_activation_transaction_t *transaction = NULL;
    ASSERT_EQ(pmm_activation_transaction_stage_file(target, source, &transaction),
              PMM_ACTIVATION_TRANSACTION_OK);
    activation_test_validation_t validation = {
        .expect_absent = false,
...
TEST(activation_transaction_stage_file_survives_long_target_path) {
    ...
    char source[ACTIVATION_TEST_PATH_CAP];
    char target[ACTIVATION_TEST_PATH_CAP];
    ASSERT_TRUE(activation_test_path(source, deep, "downloaded-pmm"));
    ASSERT_TRUE(activation_test_path(target, deep, "pmm"));
    ASSERT_TRUE(activation_test_write(source, "downloaded"));
...
TEST(activation_transaction_removal_can_rollback_or_finalize) {
    ...
    pmm_activation_transaction_t *transaction = NULL;
    ASSERT_EQ(pmm_activation_transaction_stage_removal(target, &transaction),
              PMM_ACTIVATION_TRANSACTION_OK);
    ASSERT_EQ(pmm_activation_transaction_commit(transaction, activation_test_validate, &absent),
              PMM_ACTIVATION_TRANSACTION_OK);
    ASSERT_FALSE(activation_test_exists(target));
    ASSERT_EQ(pmm_activation_transaction_rollback(transaction), PMM_ACTIVATION_TRANSACTION_OK);
...
TEST(activation_transaction_removal_survives_transient_rename_locks) {
    ...
    pmm_activation_transaction_rename_failures_set_for_test(3U);
    ASSERT_EQ(pmm_activation_transaction_commit(transaction, activation_test_validate, &absent),
              PMM_ACTIVATION_TRANSACTION_OK);
    pmm_activation_transaction_rename_failures_set_for_test(0U);
    ASSERT_FALSE(activation_test_exists(target));
    ASSERT_EQ(pmm_activation_transaction_finalize(transaction), PMM_ACTIVATION_TRANSACTION_OK);
    ASSERT_EQ(pmm_activation_transaction_close(&transaction), PMM_ACTIVATION_TRANSACTION_OK);
    ...
    ASSERT_EQ(pmm_activation_transaction_stage_removal(target, &transaction),
              PMM_ACTIVATION_TRANSACTION_OK);
    pmm_activation_transaction_rename_failures_set_for_test(64U);
    ASSERT_EQ(pmm_activation_transaction_commit(transaction, activation_test_validate, &absent),
...
TEST(activation_transaction_rejects_windows_callback_allow_directory_ace) {
    ...
    pmm_activation_transaction_t *transaction = NULL;
    pmm_activation_transaction_status_t stage_status = pmm_activation_transaction_stage_bytes(
        target, "candidate", strlen("candidate"), &transaction);
    bool transaction_was_created = transaction != NULL;
    pmm_activation_transaction_status_t close_status =
        transaction ? pmm_activation_transaction_close(&transaction)
                    : PMM_ACTIVATION_TRANSACTION_OK;
    bool acl_restored = activation_test_windows_set_directory_acl(directory, false);
    int cleanup_status = th_rmtree(directory);

    ASSERT_EQ(stage_status, PMM_ACTIVATION_TRANSACTION_IO);
    ASSERT_FALSE(transaction_was_created);
    ASSERT_EQ(close_status, PMM_ACTIVATION_TRANSACTION_OK);
    ASSERT_NULL(transaction);
...
TEST(activation_transaction_rejects_symlink_candidate_target_and_parent) {
    ...
    ASSERT_TRUE(activation_test_path(linked_parent_target, parent_link, "pmm"));
    ASSERT_TRUE(activation_test_path(linked_nested_target, parent_link, "nested/pmm"));
    ...
    ASSERT_TRUE(pmm_mkdir_p(real_parent, 0700));
    ASSERT_TRUE(pmm_mkdir_p(real_nested, 0700));
    ASSERT_TRUE(activation_test_write(real_nested_candidate, "candidate"));
    ASSERT_EQ(symlink(real_parent, parent_link), 0);

    pmm_activation_transaction_t *transaction = NULL;
    ASSERT_EQ(pmm_activation_transaction_stage_file(target, candidate_link, &transaction),
              PMM_ACTIVATION_TRANSACTION_IO);
    ASSERT_NULL(transaction);
    ASSERT_EQ(pmm_activation_transaction_stage_file(target, linked_nested_candidate, &transaction),
              PMM_ACTIVATION_TRANSACTION_IO);
    ASSERT_NULL(transaction);
    ASSERT_EQ(pmm_activation_transaction_stage_bytes(target_link, "replacement",
                                                     strlen("replacement"), &transaction),
              PMM_ACTIVATION_TRANSACTION_IO);
    ASSERT_NULL(transaction);
    ASSERT_EQ(pmm_activation_transaction_stage_bytes(linked_parent_target, "replacement",
                                                     strlen("replacement"), &transaction),
              PMM_ACTIVATION_TRANSACTION_IO);
    ASSERT_NULL(transaction);
    ASSERT_EQ(pmm_activation_transaction_stage_bytes(linked_nested_target, "replacement",
                                                     strlen("replacement"), &transaction),
              PMM_ACTIVATION_TRANSACTION_IO);
    ASSERT_NULL(transaction);
...
TEST(activation_transaction_fails_closed_if_target_directory_is_replaced) {
    ...
    ASSERT_TRUE(pmm_mkdir_p(active, 0700));
    ASSERT_TRUE(activation_test_path(target, active, "pmm"));
    ...
    ASSERT_TRUE(pmm_mkdir_p(active, 0700));
    ASSERT_EQ(pmm_activation_transaction_commit(transaction, NULL, NULL),
              PMM_ACTIVATION_TRANSACTION_IO);
    ASSERT_FALSE(activation_test_exists(target));
    ASSERT_EQ(pmm_rmdir(active), 0);
...
TEST(activation_transaction_does_not_replace_target_created_at_publish_boundary) {
    ...
    pmm_activation_transaction_set_before_absent_publish_for_test(
        activation_test_create_competing_target, &competing);
    pmm_activation_transaction_status_t commit_status =
        pmm_activation_transaction_commit(transaction, NULL, NULL);
    pmm_activation_transaction_set_before_absent_publish_for_test(NULL, NULL);
    ...
    pmm_activation_transaction_status_t close_status =
        pmm_activation_transaction_close(&transaction);
    ...
    ASSERT_EQ(commit_status, PMM_ACTIVATION_TRANSACTION_IO);
...
TEST(activation_transaction_rejects_macos_existing_target_mutating_acl) {
    ...
    if (transaction) {
        const char *staged_path = pmm_activation_transaction_staged_path(transaction);
        const char *backup_path = pmm_activation_transaction_backup_path(transaction);
        if (staged_path) {
    ...
    pmm_activation_transaction_status_t commit_status = PMM_ACTIVATION_TRANSACTION_INVALID_STATE;
    if (stage_status == PMM_ACTIVATION_TRANSACTION_OK && transaction) {
        commit_status = pmm_activation_transaction_commit(transaction, NULL, NULL);
    }
    ...
    ASSERT_TRUE(stage_status == PMM_ACTIVATION_TRANSACTION_IO ||
                (stage_status == PMM_ACTIVATION_TRANSACTION_OK &&
                 commit_status == PMM_ACTIVATION_TRANSACTION_IO));
