...
static int integration_setup(void) {
    ...
    g_project = pmm_project_name_from_path(g_tmpdir);
    if (!g_project)
        return -1;
    ...
    char *resp = pmm_mcp_handle_tool(g_srv, "index_repository", args);
    if (!resp)
        return -1;
...
static char *call_tool(const char *tool, const char *args) {
    if (!g_srv)
        return NULL;
    return pmm_mcp_handle_tool(g_srv, tool, args);
}

TEST(integ_index_has_nodes) {
    /* Open the indexed db directly and check node counts */
    pmm_store_t *store = pmm_store_open_path_existing(g_dbpath);
    ASSERT_NOT_NULL(store);

    int nodes = pmm_store_count_nodes(store, g_project);
...
TEST(integ_index_has_edges) {
    ...
    int edges = pmm_store_count_edges(store, g_project);
...
TEST(integ_index_has_functions) {
    ...
    int count = 0;
    int rc = pmm_store_find_nodes_by_label(store, g_project, "Function", &funcs, &count);
    ...
    pmm_store_free_nodes(funcs, count);
    pmm_store_close(store);
    PASS();
}
...
TEST(integ_index_has_files) {
    ...
    pmm_node_t *files = NULL;
    int count = 0;
    int rc = pmm_store_find_nodes_by_label(store, g_project, "File", &files, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    pmm_store_free_nodes(files, count);
    pmm_store_close(store);
    PASS();
}
...
TEST(integ_index_has_calls) {
    ...
    int call_count = pmm_store_count_edges_by_type(store, g_project, "CALLS");
...
TEST(integ_mcp_trace_path_cross_service) {
    ...
    int src_count = 0;
    int dst_count = 0;
    pmm_store_find_nodes_by_name(store, g_project, "greet", &src, &src_count);
    pmm_store_find_nodes_by_name(store, g_project, "farewell", &dst, &dst_count);
    ASSERT_TRUE(src_count > 0 && dst_count > 0);

    pmm_edge_t edge = {.project = g_project,
                       .source_id = src[0].id,
                       .target_id = dst[0].id,
                       .type = "CROSS_HTTP_CALLS"};
    ASSERT_TRUE(pmm_store_insert_edge(store, &edge) > 0);

    pmm_store_free_nodes(src, src_count);
    pmm_store_free_nodes(dst, dst_count);
    pmm_store_close(store);
    ...
    snprintf(args, sizeof(args),
             ...
             g_project);
    char *resp = pmm_mcp_handle_tool(srv, "trace_path", args);
    ASSERT_NOT_NULL(resp);
    ...
    snprintf(args, sizeof(args),
             ...
             g_project);
    resp = pmm_mcp_handle_tool(srv, "trace_path", args);
    ASSERT_NOT_NULL(resp);
...
TEST(integ_pipeline_fqn_compute) {
    char *fqn = pmm_pipeline_fqn_compute("myproject", "src/utils.go", "Add");
    ASSERT_NOT_NULL(fqn);
...
TEST(integ_pipeline_fqn_module) {
    char *fqn = pmm_pipeline_fqn_module("myproject", "src/utils.go");
    ASSERT_NOT_NULL(fqn);
...
TEST(integ_store_search_by_degree) {
    ...
    int rc = pmm_store_search(store, &params, &out);
    ASSERT_EQ(rc, PMM_STORE_OK);
...
TEST(integ_store_find_by_file) {
    ...
    int count = 0;
    int rc = pmm_store_find_nodes_by_file(store, g_project, "main.py", &nodes, &count);
    ASSERT_EQ(rc, PMM_STORE_OK);
...
TEST(integ_store_bfs_traversal) {
    ...
    pmm_node_t *results = NULL;
    int count = 0;
    pmm_store_find_nodes_by_name(store, g_project, "Multiply", &results, &count);

    if (count > 0) {
        /* BFS outbound from Multiply */
        pmm_traverse_result_t trav = {0};
        int rc = pmm_store_bfs(store, results[0].id, "outbound", NULL, 0, 3, 20, &trav);
        ASSERT_EQ(rc, PMM_STORE_OK);
        /* Should visit at least Add */
        ASSERT_TRUE(trav.visited_count >= 0); /* might be 0 if no edges */
        pmm_store_traverse_free(&trav);
    }

    pmm_store_free_nodes(results, count);
    pmm_store_close(store);
    PASS();
}
...
TEST(store_bfs_edges_survive_large_visited_set) {
    ...
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_upsert_project(s, "star", "/tmp/star"), PMM_STORE_OK);
    ...
    hub.file_path = "hub.c";
    int64_t hub_id = pmm_store_upsert_node(s, &hub);
    ASSERT_GT(hub_id, 0);
    ...
    for (int i = 0; i < SPOKES; i++) {
        ...
        int64_t sid = pmm_store_upsert_node(s, &sp);
        ASSERT_GT(sid, 0);
        pmm_edge_t e = {0};
        e.project = "star";
        e.source_id = sid;
        e.target_id = hub_id;
        e.type = "CALLS";
        ASSERT_GT(pmm_store_insert_edge(s, &e), 0); /* returns the edge id */
    }

    pmm_traverse_result_t tr = {0};
    ASSERT_EQ(pmm_store_bfs(s, hub_id, "inbound", NULL, 0, 1, SPOKES + 10, &tr), PMM_STORE_OK);
...
TEST(store_bfs_multi_excludes_seeds_and_takes_min_hop) {
    pmm_store_t *s = pmm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_upsert_project(s, "impact", "/tmp/impact"), PMM_STORE_OK);
    ...
    for (size_t i = 0; i < sizeof(edges) / sizeof(edges[0]); i++) {
        pmm_edge_t e = {.project = "impact",
                        .source_id = edges[i].from,
                        .target_id = edges[i].to,
                        .type = "CALLS"};
        ASSERT_GT(pmm_store_insert_edge(s, &e), 0);
    }

    int64_t seeds[2] = {A, B};
    pmm_traverse_result_t tr = {0};
    bool truncated = true;
    ASSERT_EQ(pmm_store_bfs_multi(s, seeds, 2, "outbound", NULL, 0, 5, 100, &tr, &truncated),
              PMM_STORE_OK);
...
TEST(store_bfs_multi_reports_truncation_at_ceiling) {
    pmm_store_t *s = pmm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(pmm_store_upsert_project(s, "cap", "/tmp/cap"), PMM_STORE_OK);
    pmm_node_t hub = {.project = "cap",
                      .label = "Function",
                      .name = "hub",
    ...
    for (int i = 0; i < N; i++) {
        ...
        pmm_node_t n = {.project = "cap",
                        ...
                        .start_line = 1,
                        .end_line = 2};
        int64_t nid = pmm_store_upsert_node(s, &n);
        ASSERT_GT(nid, 0);
        pmm_edge_t e = {.project = "cap", .source_id = hub_id, .target_id = nid, .type = "CALLS"};
        ASSERT_GT(pmm_store_insert_edge(s, &e), 0);
    }
    pmm_traverse_result_t tr = {0};
    bool truncated = false;
    ASSERT_EQ(pmm_store_bfs_multi(s, &hub_id, 1, "outbound", NULL, 0, 5, CEIL, &tr, &truncated),
              PMM_STORE_OK);
...
TEST(index_reports_excluded_subtrees_issue411) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_excl_XXXXXX");
