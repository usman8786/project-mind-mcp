...
static pmm_store_t *gp_open_indexed(GP_Proj *lp) {
...
static pmm_store_t *gp_index_files(GP_Proj *lp, const GP_File *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_gpc_XXXXXX");
...
static pmm_store_t *gp_index(GP_Proj *lp, const char *filename, const char *content) {
    GP_File f = {filename, content};
    return gp_index_files(lp, &f, 1);
}

static void gp_cleanup(GP_Proj *lp, pmm_store_t *store) {
...
static int gp_count_label(pmm_store_t *store, const char *project, const char *label) {
...
static int gp_type_like(pmm_store_t *store, const char *project) {
...
/* Extraction-level: count imports via pmm_extract_file (bypasses graph). */
static int gp_extract_imports(const char *src, CBMLanguage lang, const char *path) {
    CBMFileResult *r =
        pmm_extract_file(src, (int)strlen(src), lang, "gpc", path, 0, NULL, NULL);
    if (!r) return -1;
...
TEST(scheme_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "a.scm",
        "(define (compute x)\n"
        "  (* x x))\n"
        "\n"
...
TEST(scheme_module_node) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "calc.scm",
        "(define (add a b) (+ a b))\n");
...
TEST(slang_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "a.slang",
...
TEST(slang_struct_node) {
    int cls = gp_extract_label(
        ...
        "    float4 color;\n"
        "};\n",
        PMM_LANG_SLANG, "types.slang", "Class");
...
TEST(slang_shader_entry_point) {
    int fns = gp_extract_label(
        ...
        "}\n",
        PMM_LANG_SLANG, "shader.slang", "Function");
...
TEST(slang_include_import) {
    int n = gp_extract_imports(
        ...
        PMM_LANG_SLANG, "main.slang");
    /* REAL BUG (class 2): SLANG absent from pmm_extract_imports() dispatch switch
     * in extract_imports.c — slang_import_types is configured but never consumed;
     * imports always 0. Fix: add PMM_LANG_SLANG case to the dispatch. */
    ASSERT_TRUE(n >= 2);
    PASS();
}
...
TEST(solidity_contract_node) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "token.sol",
...
TEST(solidity_interface_node) {
    int cls = gp_extract_label(
        ...
        "}\n",
        PMM_LANG_SOLIDITY, "IERC20.sol", "Interface");
...
TEST(solidity_method_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "vault.sol",
        "// SPDX-License-Identifier: MIT\n"
        "pragma solidity ^0.8.0;\n"
        "\n"
        "contract Vault {\n"
...
TEST(solidity_import_extracted) {
    int n = gp_extract_imports(
        ...
        "contract MyToken {\n"
        "}\n",
        PMM_LANG_SOLIDITY, "a.sol");
    /* REAL BUG (class 2): SOLIDITY absent from pmm_extract_imports() dispatch in
...
TEST(squirrel_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "util.nut",
        "function greet(name) {\n"
        "    return \"Hello, \" + name\n"
        "}\n"
        "\n"
...
TEST(squirrel_class_node) {
    int cls = gp_extract_label(
        ...
        "}\n",
        PMM_LANG_SQUIRREL, "animal.nut", "Class");
...
TEST(squirrel_class_methods) {
    int methods = gp_extract_label(
        ...
        "}\n",
        PMM_LANG_SQUIRREL, "calc.nut", "Method");
...
TEST(starlark_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "rules.bzl",
...
TEST(starlark_star_extension_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "macros.star",
    ...
    int fns = store ? gp_count_label(store, lp.project, "Function") : -1;
    gp_cleanup(&lp, store);
    ASSERT_TRUE(fns >= 1); /* green: .star → PMM_LANG_STARLARK → Function node */
    PASS();
}
...
TEST(sway_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "contract.sw",
...
TEST(sway_use_import_extracted) {
    int n = gp_extract_imports(
        ...
        "fn run() {}\n",
        PMM_LANG_SWAY, "a.sw");
    /* REAL BUG (class 2): SWAY absent from pmm_extract_imports() dispatch in
...
TEST(sway_abi_node) {
    int types = gp_extract_label(
        ...
        "}\n",
        PMM_LANG_SWAY, "abi.sw", "Interface");
...
TEST(sway_impl_node) {
    int types = gp_extract_label(
        ...
        "}\n",
        PMM_LANG_SWAY, "counter.sw", "Class");
...
TEST(tcl_proc_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "utils.tcl",
        "proc greet {name} {\n"
        "    return \"Hello, $name\"\n"
        "}\n"
        "\n"
...
TEST(teal_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "math.tl",
...
TEST(teal_interface_node) {
    int ifaces = gp_extract_label(
        ...
        "end\n",
        PMM_LANG_TEAL, "drawable.tl", "Interface");
...
TEST(vimscript_function_bang_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "plugin.vim",
...
TEST(vimscript_function_no_bang) {
    int fns = gp_extract_label(
        ...
        "  call MyPlugin#Init()\n"
        "endfunction\n",
        PMM_LANG_VIMSCRIPT, "myplugin.vim", "Function");
...
TEST(vimscript_autoload_function) {
    int fns = gp_extract_label(
        ...
        "  let s:parts = []\n"
        "endfunction\n",
        PMM_LANG_VIMSCRIPT, "statusline.vim", "Function");
...
TEST(wgsl_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "compute.wgsl",
...
TEST(wgsl_entry_point_functions) {
    int fns = gp_extract_label(
        ...
        PMM_LANG_WGSL, "pipeline.wgsl", "Function");
...
TEST(wgsl_struct_node) {
    int types = gp_extract_label(
        ...
        "  intensity: f32,\n"
        "}\n",
        PMM_LANG_WGSL, "uniforms.wgsl", "Struct");
...
TEST(wgsl_enable_directive_extracted) {
    int n = gp_extract_imports(
        ...
        PMM_LANG_WGSL, "ext.wgsl");
    /* REAL BUG (class 2): WGSL absent from pmm_extract_imports() dispatch in
...
TEST(zsh_function_nodes) {
    GP_Proj lp;
    pmm_store_t *store = gp_index(&lp, "deploy.zsh",
        "function check_deps {\n"
...
TEST(zsh_function_shorthand) {
    int fns = gp_extract_label(
        ...
        "  echo \"tearing down\"\n"
        "}\n",
        PMM_LANG_ZSH, "test_hooks.zsh", "Function");
