...
static pmm_store_t *gpf_open_indexed(GpfProj *lp) {
...
static pmm_store_t *gpf_index_files(GpfProj *lp, const GpfFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_gpf_XXXXXX");
...
static void gpf_cleanup(GpfProj *lp, pmm_store_t *store) {
...
static int gpf_count_label(pmm_store_t *store, const char *project, const char *label) {
...
static GpfMetrics gpf_metrics_files(const GpfFile *files, int nfiles) {
    GpfProj lp;
    pmm_store_t *store = gpf_index_files(&lp, files, nfiles);
    GpfMetrics m = {0};
    if (store) {
        m.ok          = 1;
        ...
        m.depends_on  = pmm_store_count_edges_by_type(store, lp.project, "DEPENDS_ON");
        m.infra_maps  = pmm_store_count_edges_by_type(store, lp.project, "INFRA_MAPS");
    }
 ...
 * by pass_k8s.c (handle_k8s_manifest → pmm_extract_file PMM_LANG_K8S).
 * ══════════════════════════════════════════════════════════════════ */
