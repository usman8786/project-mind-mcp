 ...
 *
 * Verifies that pmm_store_begin_bulk / pmm_store_end_bulk never switch away
 ...
 * independent of any state held inside the pmm_store_t under test. */
...
static void make_temp_path(char *buf, size_t n) {
    snprintf(buf, n, "%s/cmm_bulk_test_%d.db", pmm_tmpdir(), (int)getpid());
}
...
TEST(bulk_crash_recovery) {
    ...
    pmm_store_t *s = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s);
    int rc = pmm_store_upsert_project(s, "baseline", "/tmp/baseline");
    ASSERT_EQ(rc, PMM_STORE_OK);
    pmm_store_close(s);
    ...
    pid_t pid = fork();
    if (pid == 0) {
        pmm_store_t *cs = pmm_store_open_path(db_path);
        if (!cs)
            _exit(1);
        pmm_store_begin_bulk(cs);
        pmm_store_begin(cs); /* explicit open transaction */
        pmm_store_upsert_project(cs, "crashed", "/tmp/crashed");
    ...
    pmm_store_t *recovered = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(recovered); /* NULL would indicate corruption */

    /* Baseline commit must survive. */
    pmm_project_t p = {0};
    rc = pmm_store_get_project(recovered, "baseline", &p);
    ASSERT_EQ(rc, PMM_STORE_OK);
    ...
    pmm_project_t p2 = {0};
    int rc2 = pmm_store_get_project(recovered, "crashed", &p2);
    ASSERT_NEQ(rc2, PMM_STORE_OK); /* row must be absent */

    pmm_store_close(recovered);
    cleanup_db(db_path);
    PASS();
}
#endif /* _WIN32 */
