#!/usr/bin/env python3
"""Real-binary smoke test for the mandatory per-account PMM daemon.
...
def run_successful_activation(
    ...
    check(
        "Stopping active PMM sessions and operations for {}...".format(action)
        in stdout,
    ...
    records = [
        ...
        if record.get("event") == "pmm.activation"
...
def create_local_update_release(release_dir, candidate):
    ...
    asset_name = "project-mind-mcp-{}-{}{}.tar.gz".format(
        os_name, arch, portable
    )
    archive = release_dir / asset_name
    with tarfile.open(archive, "w:gz") as stream:
        stream.add(str(candidate), arcname="project-mind-mcp", recursive=False)
...
def assert_no_activation_artifacts(directory, label):
    artifacts = (
        sorted(path.name for path in directory.iterdir() if path.name.startswith(".pmm-"))
        if directory.exists()
        else []
    )
...
def main():
    if os.name == "nt" or sys.platform.startswith(("cygwin", "msys")):
        if os.environ.get("PMM_DAEMON_SMOKE_REQUIRE_RUN") == "1":
            raise SmokeFailure(
    ...
    binary = Path(sys.argv[1] if len(sys.argv) > 1 else root / "build/c/project-mind-mcp")
    ...
    runtime_dir = runtime_parent / ("pmm-daemon-" + str(os.geteuid()))
    socket_path = runtime_dir / ("pmm-" + RENDEZVOUS_KEY + ".sock")
    startup_lock = runtime_dir / ("pmm-" + RENDEZVOUS_KEY + ".lock")
    lifetime_lock = runtime_dir / ("pmm-" + RENDEZVOUS_KEY + ".lifetime.lock")
    cohort_admission_lock = runtime_dir / "pmm-version-cohort-admission-v1.lock"
    cohort_lifetime_lock = runtime_dir / "pmm-version-cohort-lifetime-v1.lock"
    cohort_maintenance_lock = runtime_dir / "pmm-version-cohort-maintenance-v1.lock"
    cohort_daemon_lock = runtime_dir / "pmm-version-cohort-daemon-v1.lock"
    coordination_locks = (
        (startup_lock, False),
        (lifetime_lock, True),
    ...
    if any(status == "held" for _, status in coordination_before):
        if os.environ.get("PMM_DAEMON_SMOKE_REQUIRE_RUN") == "1":
            raise SmokeFailure(
                "another PMM process or activation is active or starting; "
                "required smoke needs a clean account-wide rendezvous"
            )
        print("SKIP: another PMM process or activation is active or starting")
        return 0
    for path, status in coordination_before:
        check(status == "free", "unsafe coordination lock {}: {}".format(path, status))

    clients = []
    with tempfile.TemporaryDirectory(prefix="pmm-daemon-smoke-") as raw_tmpdir:
        ...
        target_binary = target_dir / "project-mind-mcp"
        ...
        daemon_log = cache / "logs/pmm-daemon.log"
        ...
        env.update(
            {
                "HOME": str(home),
                "PMM_CACHE_DIR": str(cache_alias),
                "PMM_LOG_LEVEL": "info",
                "PMM_LOG_FORMAT": "json",
                "PMM_TEST_HANG_ON": "hang_me",
                "PMM_TEST_WORKER_DESCENDANT_PID_FILE": str(descendant_pid_path),
            }
        )
        ...
        try:
            ...
            version_prefix = "project-mind-mcp "
            ...
            check(
                "Preparing one-shot local PMM command" in local_cli.stderr,
            ...
            initialize_params = {
                ...
                "capabilities": {},
                "clientInfo": {"name": "pmm-daemon-smoke", "version": "1"},
            }
            c1.send(
            ...
            mismatched_env["PMM_CACHE_DIR"] = str(mismatched_cache)
            ...
            check(
                ...
                "mismatched PMM_CACHE_DIR unexpectedly joined the account daemon",
            )
            check(
                "active account daemon uses a different cache directory"
                in mismatched_result.stderr
                and "PMM_CACHE_DIR" in mismatched_result.stderr,
            ...
            # even if the original PMM_CACHE_DIR symlink is retargeted later.
            ...
            conflict_binary = tmpdir / "project-mind-mcp-conflict"
            ...
            if sys.platform == "darwin":
                ...
                signed = subprocess.run(
                    [
                        ...
                        "-",
                        "--identifier",
                        "com.deusdata.pmm.daemon-smoke-conflict",
                        str(conflict_binary),
                    ],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
            ...
            check(
                "project-mind-mcp: " + expected_conflict
            ...
            cli_first_env["PMM_TEST_WORKER_DESCENDANT_PID_FILE"] = str(
                cli_first_pid_path
            )
            cli_first_env["PMM_TEST_WORKER_PROJECT_LOCK_PID_FILE"] = str(
                cli_first_lock_owner_path
            )
            ...
            def competitor_waiting_or_exited():
                try:
                    ...
                    text = ""
                return (
                    "Waiting for another PMM mutation of smoke-cli-first" in text
            ...
            check(
                ...
                and "Waiting for another PMM mutation of smoke-cli-first"
            ...
            install_target = install_dir / "project-mind-mcp"
            ...
            activation_local_env["PMM_TEST_WORKER_DESCENDANT_PID_FILE"] = str(
                install_pid_path
            )
            activation_local_env["PMM_TEST_WORKER_PROJECT_LOCK_PID_FILE"] = str(
            ...
            check(
                ...
                == "project-mind-mcp " + semantic_version,
            ...
            # the next daemon. PMM_DOWNLOAD_URL points at this file:// fixture,
            ...
            update_env = env.copy()
            update_env["PMM_DOWNLOAD_URL"] = create_local_update_release(
