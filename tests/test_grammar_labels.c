 ...
 * histogram that pmm_extract_file() produces for the shared fixture (e.g.
...
TEST(grammar_label_goldens) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        CBMFileResult *r =
            pmm_extract_file(c->src, (int)strlen(c->src), c->lang, "lbl", c->path, 0, NULL, NULL);
        if (!r) {
...
TEST(grammar_code_extracts_defs) {
    ...
    for (int k = 0; MUST_EXTRACT_DEFS[k]; k++) {
        ...
        for (int i = 0; i < (int)PMM_GRAMMAR_CASES_COUNT; i++) {
            if (strcmp(PMM_GRAMMAR_CASES[i].name, name) == 0) {
                c = &PMM_GRAMMAR_CASES[i];
                break;
            }
        }
        if (!c) {
            fprintf(stderr, "  [CODE-DEFS] FAIL %-14s — no fixture in PMM_GRAMMAR_CASES\n", name);
            failures++;
            continue;
        }
        CBMFileResult *r =
            pmm_extract_file(c->src, (int)strlen(c->src), c->lang, "cd", c->path, 0, NULL, NULL);
        if (!r) {
