...
static pmm_ast_profile_t make_profile(void) {
    pmm_ast_profile_t p;
    memset(&p, 0, sizeof(p));
    p.if_count = 5;
    p.for_count = 3;
...
TEST(ast_profile_roundtrip) {
    pmm_ast_profile_t original = make_profile();
    char buf[200];
    pmm_ast_profile_to_str(&original, buf, sizeof(buf));

    pmm_ast_profile_t decoded;
    ASSERT_TRUE(pmm_ast_profile_from_str(buf, &decoded));
...
TEST(ast_profile_to_str_null) {
    char buf[200];
    pmm_ast_profile_to_str(NULL, buf, sizeof(buf)); /* should not crash */
    PASS();
}

TEST(ast_profile_to_str_small_buf) {
    pmm_ast_profile_t p = make_profile();
    /* buf too small: should write empty string */
    char buf[1] = {'X'};
    pmm_ast_profile_to_str(&p, buf, 0);
    /* 0-length buffer: function should handle gracefully */
    PASS();
}

TEST(ast_profile_from_str_null) {
    ASSERT_FALSE(pmm_ast_profile_from_str(NULL, NULL));
    PASS();
}

TEST(ast_profile_from_str_invalid) {
    pmm_ast_profile_t out;
    ASSERT_FALSE(pmm_ast_profile_from_str("not,a,valid,string", &out));
    ASSERT_FALSE(pmm_ast_profile_from_str("", &out));
    PASS();
}
...
TEST(ast_profile_from_str_too_few_fields) {
    ...
    ASSERT_FALSE(pmm_ast_profile_from_str("1,2,3,4,5", &out));
    PASS();
}
...
TEST(ast_profile_to_vector_range) {
    pmm_ast_profile_t p = make_profile();
    float vec[25];
    pmm_ast_profile_to_vector(&p, vec);
...
TEST(ast_profile_to_vector_null) {
    ...
    pmm_ast_profile_to_vector(NULL, vec); /* should not crash or write */
    PASS();
}
