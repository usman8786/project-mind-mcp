...
static CBMFileResult *extract_with(const char *source, CBMLanguage lang, const char *fname) {
    return pmm_extract_file(source, (int)strlen(source), lang, "test", fname, 0, NULL, NULL);
}

static CBMFileResult *extract_ts(const char *source) {
    return extract_with(source, PMM_LANG_TYPESCRIPT, "main.ts");
}

static CBMFileResult *extract_tsx(const char *source) {
    return extract_with(source, PMM_LANG_TSX, "main.tsx");
}

static CBMFileResult *extract_js(const char *source) {
    return extract_with(source, PMM_LANG_JAVASCRIPT, "main.js");
}

static CBMFileResult *extract_dts(const char *source) {
    return extract_with(source, PMM_LANG_TYPESCRIPT, "main.d.ts");
}
...
/* ── Cross-file resolution (uses pmm_run_ts_lsp_cross directly) ─────────────── */
...
TEST(tslsp_crossfile_method_dispatch) {
    ...
    pmm_run_ts_lsp_cross(&arena, source, (int)strlen(source), "test.main",
...
TEST(tslsp_scale_many_defs_no_crash_issue344) {
    ...
    CBMLSPDef *defs = (CBMLSPDef *)pmm_arena_alloc(&arena, (size_t)SCALE_N * sizeof(CBMLSPDef));
    ASSERT_NOT_NULL(defs);
    memset(defs, 0, (size_t)SCALE_N * sizeof(CBMLSPDef));
    for (int i = 0; i < SCALE_N; i++) {
        defs[i].qualified_name = pmm_arena_sprintf(&arena, "test.mod%d.fn%d", i, i);
        defs[i].short_name = pmm_arena_sprintf(&arena, "fn%d", i);
        defs[i].label = "Function";
        defs[i].def_module_qn = pmm_arena_sprintf(&arena, "test.mod%d", i);
...
TEST(tslsp_crossfile_function_call) {
    ...
    pmm_run_ts_lsp_cross(&arena, source, (int)strlen(source), "test.main", false, false, false,
...
TEST(tslsp_baseline_vs_lsp_simple) {
    ...
    pmm_setenv("PMM_LSP_DISABLED", "1", 1);
    CBMFileResult *base = extract_ts(source);
    int br = 0, bu = 0, bt = 0;
    count_calls(base, &br, &bu, &bt);
    pmm_free_result(base);

    pmm_unsetenv("PMM_LSP_DISABLED");
...
TEST(tslsp_hash_registry_basic) {
    ...
    ASSERT_NOT_NULL(pmm_registry_lookup_type(&reg, "test.A"));
    ASSERT_NOT_NULL(pmm_registry_lookup_func(&reg, "test.A.foo"));
    ASSERT_NOT_NULL(pmm_registry_lookup_method(&reg, "test.A", "foo"));
    ...
    ASSERT(pmm_registry_lookup_type(&reg, "test.B") == NULL);
    ASSERT(pmm_registry_lookup_method(&reg, "test.A", "bar") == NULL);

    pmm_arena_destroy(&arena);
    PASS();
}

/* Entries added AFTER pmm_registry_finalize must remain findable. The hashed
...
TEST(tslsp_hash_registry_post_finalize_adds) {
    ...
    f2.min_params = -1;
    pmm_registry_add_func(&reg, f2);

    ASSERT_NOT_NULL(pmm_registry_lookup_type(&reg, "test.A"));
    ASSERT_NOT_NULL(pmm_registry_lookup_type(&reg, "test.LateStub"));
    ASSERT_NOT_NULL(pmm_registry_lookup_func(&reg, "test.LateStub.run"));
...
TEST(tslsp_hash_registry_stress_10k) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        CBMRegisteredType t;
        memset(&t, 0, sizeof(t));
        t.qualified_name = pmm_arena_strdup(&arena, buf);
        t.short_name = "X";
        pmm_registry_add_type(&reg, t);
        ...
        CBMRegisteredFunc f;
        memset(&f, 0, sizeof(f));
        f.qualified_name = pmm_arena_strdup(&arena, buf);
        f.short_name = "go";
        f.min_params = -1;
        char recv[64];
        snprintf(recv, sizeof(recv), "p.T%d", i);
        f.receiver_type = pmm_arena_strdup(&arena, recv);
        pmm_registry_add_func(&reg, f);
    }
    pmm_registry_finalize(&reg);
    ...
    for (int i = 0; i < N; i += 100) {
        snprintf(buf, sizeof(buf), "p.T%d", i);
        if (pmm_registry_lookup_type(&reg, buf))
            hits++;
        snprintf(buf, sizeof(buf), "p.T%d", i);
        if (pmm_registry_lookup_method(&reg, buf, "go"))
            hits++;
    }
...
SUITE(ts_lsp) {
    ...
    /* LSP vs baseline comparison (requires PMM_LSP_DISABLED knob in resolver) */
