...
static bool diagnostics_capture_paths(diagnostics_paths_t *paths) {
    return pmm_diag_test_copy_paths(paths->directory, sizeof(paths->directory), paths->snapshot,
...
static void diagnostics_cleanup_outputs(const diagnostics_paths_t *paths) {
    ...
    if (snprintf(temporary, sizeof(temporary), "%s.tmp", paths->snapshot) > 0) {
        (void)pmm_unlink(temporary);
    }
    if (snprintf(rotated, sizeof(rotated), "%s.1", paths->trajectory) > 0) {
        (void)pmm_unlink(rotated);
    }
    (void)pmm_unlink(paths->snapshot);
    (void)pmm_unlink(paths->trajectory);
    (void)pmm_rmdir(paths->directory);
}
...
TEST(diagnostics_rejects_predictable_tmp_symlinks) {
    ...
    ASSERT_LT(
        snprintf(snapshot_link, sizeof(snapshot_link), "/tmp/pmm-diagnostics-%d.json.tmp", pid),
        (int)sizeof(snapshot_link));
    ASSERT_LT(
        snprintf(trajectory_link, sizeof(trajectory_link), "/tmp/pmm-diagnostics-%d.ndjson", pid),
        (int)sizeof(trajectory_link));
    ASSERT_LT(snprintf(snapshot_target, sizeof(snapshot_target),
                       "/tmp/pmm-diagnostics-snapshot-sentinel-%d", pid),
              (int)sizeof(snapshot_target));
    ASSERT_LT(snprintf(trajectory_target, sizeof(trajectory_target),
                       "/tmp/pmm-diagnostics-trajectory-sentinel-%d", pid),
              (int)sizeof(trajectory_target));

    (void)pmm_unlink(snapshot_link);
    (void)pmm_unlink(trajectory_link);
    (void)pmm_unlink(snapshot_target);
    (void)pmm_unlink(trajectory_target);

    FILE *snapshot = pmm_fopen(snapshot_target, "wb");
    ASSERT_NOT_NULL(snapshot);
    ASSERT_GTE(fputs(snapshot_seed, snapshot), 0);
    ASSERT_EQ(fclose(snapshot), 0);
    FILE *trajectory = pmm_fopen(trajectory_target, "wb");
    ...
    ASSERT_EQ(pmm_setenv("PMM_DIAGNOSTICS", "1", 1), 0);
    ASSERT_TRUE(pmm_diag_start());
    ...
    pmm_diag_stop();
    ASSERT_EQ(pmm_unsetenv("PMM_DIAGNOSTICS"), 0);
...
TEST(diagnostics_stop_interrupts_periodic_wait) {
    ...
    int pid = (int)getpid();
    ASSERT_LT(snprintf(legacy_snapshot, sizeof(legacy_snapshot), "%s/pmm-diagnostics-%d.json",
                       pmm_tmpdir(), pid),
              (int)sizeof(legacy_snapshot));
    ASSERT_LT(snprintf(legacy_trajectory, sizeof(legacy_trajectory), "%s/pmm-diagnostics-%d.ndjson",
                       pmm_tmpdir(), pid),
              (int)sizeof(legacy_trajectory));
    (void)pmm_unlink(legacy_snapshot);
    (void)pmm_unlink(legacy_trajectory);
    ...
    bool emitted = captured && diagnostics_wait_for_file(outputs.snapshot, 1000);
    uint64_t started_ms = pmm_now_ms();
    pmm_diag_stop();
    uint64_t elapsed_ms = pmm_now_ms() - started_ms;
    ASSERT_EQ(pmm_unsetenv("PMM_DIAGNOSTICS"), 0);
    (void)pmm_unlink(legacy_snapshot);
    (void)pmm_unlink(legacy_trajectory);
    if (captured) {
...
TEST(diagnostics_stalled_writer_cannot_retain_shutdown) {
    pmm_diag_test_hold_writer(true);
    ASSERT_EQ(pmm_setenv("PMM_DIAGNOSTICS", "1", 1), 0);
    if (!pmm_diag_start()) {
        pmm_diag_test_hold_writer(false);
        (void)pmm_unsetenv("PMM_DIAGNOSTICS");
        FAIL("failed to start held diagnostics writer");
    }
    diagnostics_paths_t outputs;
    bool captured = diagnostics_capture_paths(&outputs);
    uint64_t reach_deadline = pmm_now_ms() + 1000;
    while (!pmm_diag_test_writer_reached() && pmm_now_ms() < reach_deadline) {
        diagnostics_wait_ms(10);
    }
    bool reached = pmm_diag_test_writer_reached();
    if (!captured || !reached) {
        pmm_diag_test_hold_writer(false);
        pmm_diag_stop();
        (void)pmm_unsetenv("PMM_DIAGNOSTICS");
    ...
    pmm_diag_stop();
    uint64_t elapsed_ms = pmm_now_ms() - started_ms;
    bool abandoned = pmm_diag_test_abandoned();

    pmm_diag_test_hold_writer(false);
    bool reset = false;
    uint64_t reset_deadline = pmm_now_ms() + 1000;
    while (!reset && pmm_now_ms() < reset_deadline) {
        reset = pmm_diag_test_reset_abandoned();
        if (!reset) {
            diagnostics_wait_ms(10);
        }
    }
...
TEST(diagnostics_soak_discovers_daemon_emitted_paths) {
    ...
    bool isolates_daemon = strstr(script, "PMM_CACHE_DIR=\"$SOAK_CACHE_DIR_VALUE\"") != NULL;
    bool legacy_predictable_path = strstr(script, "/tmp/pmm-diagnostics-${SERVER_PID}") != NULL;
    free(script);
...
TEST(diagnostics_discovery_record_survives_suppressed_log_level) {
    ...
    pmm_log_set_level(PMM_LOG_NONE);
    pmm_log_set_sink_ex(diagnostics_discovery_sink, PMM_LOG_SINK_REPLACE);
    ASSERT_EQ(pmm_setenv("PMM_DIAGNOSTICS", "1", 1), 0);
    bool started = pmm_diag_start();
    diagnostics_paths_t outputs;
    ...
    ASSERT_TRUE(started);
    ASSERT_TRUE(captured);
    /* Exactly one discovery record, delivered despite PMM_LOG_LEVEL=none, and
...
TEST(diagnostics_placement_honors_tmpdir_with_spaces) {
    ...
    char base[256];
    ASSERT_LT(snprintf(base, sizeof(base), "/tmp/pmm diag spaces %d", (int)getpid()),
              (int)sizeof(base));
    (void)pmm_rmdir(base);
    ASSERT_EQ(mkdir(base, 0700), 0);
    ASSERT_EQ(pmm_setenv("TMPDIR", base, 1), 0);
    diagnostics_discovery_records = 0;
    diagnostics_discovery_line[0] = '\0';
    pmm_log_set_sink_ex(diagnostics_discovery_sink, PMM_LOG_SINK_REPLACE);
    ASSERT_EQ(pmm_setenv("PMM_DIAGNOSTICS", "1", 1), 0);
    ...
    pmm_log_set_sink(NULL);
    if (had_tmpdir) {
        (void)pmm_setenv("TMPDIR", saved_tmpdir, 1);
    } else {
        (void)pmm_unsetenv("TMPDIR");
    }
