...
cat > "$ENV_PROBE" <<'EOF'
#!/usr/bin/env bash
printf '%s\t%s\n' "${HOME-}" "${PMM_CACHE_DIR-}" >> "$PMM_FUZZ_ENV_PROBE"
...
if ! HOME="$CALLER_HOME" \
    PMM_CACHE_DIR="$CALLER_CACHE" \
    PMM_FUZZ_ENV_PROBE="$ENV_LOG" \
...
while IFS=$'\t' read -r child_home_raw child_cache_raw; do
    ...
    if [[ -z "$child_cache" || "$child_cache" == "$CALLER_CACHE_NORMALIZED" ]]; then
        echo "FAIL: security-fuzz exposed the caller PMM_CACHE_DIR to a fuzz target"
        exit 1
    fi
