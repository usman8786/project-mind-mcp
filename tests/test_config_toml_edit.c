 ...
 */
#define PMM_TOML_EDIT_ENABLE_TEST_API 1
...
static const char *CTE_BEGIN = "# BEGIN project-mind-mcp";
static const char *CTE_END = "# END project-mind-mcp";
static const char *CTE_TABLE = "mcp_servers";
static const char *CTE_KEY = "name";
static const char *CTE_IDENTITY = "project-mind-mcp";
static const char *CTE_BODY = "name = \"project-mind-mcp\"\n"
                              "command = \"project-mind-mcp\"\n";

static int cte_fixture(char *dir, size_t dir_size, char *path, size_t path_size) {
    char *created = th_mktempdir("pmm_toml_edit");
    if (!created) {
        return -1;
    }
...
static int cte_assert_unchanged_after_managed_upsert(const char *path, const char *original,
    ...
    return th_write_file(path, original) == 0 &&
                   pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, block) == -1 &&
...
static int cte_assert_unchanged_after_vibe_upsert(const char *path, const char *original,
    ...
    char actual[CTE_FILE_CAP];
    return th_write_file(path, original) == 0 &&
                   pmm_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY,
                                                     body) == -1 &&
                   cte_read(path, actual, sizeof(actual)) == 0 && strcmp(actual, original) == 0
               ? 1
               : 0;
...
TEST(config_toml_rejects_stale_content_and_identity) {
    ...
    pmm_toml_set_precommit_hook_for_testing(cte_change_before_commit, &content_change);
    int result = pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n");
    pmm_toml_set_precommit_hook_for_testing(NULL, NULL);
    ...
    pmm_toml_set_precommit_hook_for_testing(cte_change_before_commit, &identity_change);
    result = pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n");
    pmm_toml_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_toml_missing_target_race_does_not_replace_winner) {
    ...
    pmm_toml_set_prepublish_hook_for_testing(cte_change_before_commit, &race);
    int result = pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n");
    pmm_toml_set_prepublish_hook_for_testing(NULL, NULL);
...
TEST(config_toml_rejects_non_regular_path) {
    ...
    ASSERT_EQ(pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n"), -1);
    ASSERT_EQ(cte_temp_count(dir), 0U);
...
TEST(config_toml_managed_markers_ignore_multiline_strings) {
    ...
    const char *original = "basic = \"\"\"\n"
                           "# BEGIN project-mind-mcp\n"
                           "# END project-mind-mcp\n"
                           "\"\"\"\n"
                           "literal = '''\n"
                           "# BEGIN project-mind-mcp\n"
                           "# END project-mind-mcp\n"
    ...
    ASSERT_EQ(pmm_toml_remove_managed_block(path, CTE_BEGIN, CTE_END), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_managed_rejects_marker_in_block_and_unclosed_multiline) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(th_write_file(path, "keep = true\n"), 0);
    ASSERT_EQ(pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END,
                                            "value = true\n# END project-mind-mcp\n"),
              -1);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_STR_EQ(actual, "keep = true\n");
...
TEST(config_toml_codex_semantic_conflicts_fail_closed) {
    char dir[CTE_PATH_CAP];
    char path[CTE_PATH_CAP];
    const char *codex_block = "[mcp_servers.project-mind-mcp]\ncommand = \"new\"\n";
    static const char *conflicts[] = {
        "[mcp_servers.\"project-mind-mcp\"]\ncommand = \"old\"\n",
        "[\"mcp_servers\".'project-mind-mcp']\ncommand = \"old\"\n",
        "mcp_servers.\"project-mind-mcp\".command = \"old\"\n",
        "[mcp_servers]\n\"project-mind-mcp\".command = \"old\"\n",
        ("[mcp_servers.project-mind-mcp]\ncommand = \"one\"\n"
         "[mcp_servers.\"project-mind-mcp\"]\ncommand = \"two\"\n"),
    };
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
...
TEST(config_toml_target_table_rejects_significant_nonassignments_byte_identically) {
    ...
    for (size_t i = 0U; i < sizeof(malformed) / sizeof(malformed[0]); ++i) {
        ...
        ASSERT_EQ(
            pmm_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, CTE_BODY),
            -1);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, malformed[i]);

        ASSERT_EQ(pmm_toml_remove_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY), -1);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, malformed[i]);
    }

    const char *malformed_regular = "[mcp_servers.project-mind-mcp]\n"
                                    "command = \"winner\"\n"
                                    "this is not an assignment\n"
                                    "[unrelated]\n"
                                    "keep = true\n";
    ...
    ASSERT_EQ(
        pmm_toml_remove_legacy_table(path, "mcp_servers.project-mind-mcp", CTE_BEGIN, CTE_END),
        -1);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_remove_includes_descendant_tables) {
    ...
    ASSERT_EQ(th_write_file(path, "[[mcp_servers]]\n"
                                  ...
                                  "command = \"keep\"\n"),
              0);
    ASSERT_EQ(pmm_toml_remove_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_reinstall_preserves_user_fields_and_descendants) {
    ...
    char path[CTE_PATH_CAP];
    char actual[CTE_FILE_CAP];
    const char *desired = "name = \"project-mind-mcp\"\n"
                          "transport = \"stdio\"\n"
                          "command = \"new\"\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(th_write_file(path, "[[mcp_servers]]\n"
                                  "name = \"project-mind-mcp\" # identity comment\n"
                                  "command = \"old\"\n"
                                  ...
                                  "KEEP = \"yes\"\n"),
              0);
    ASSERT_EQ(pmm_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, desired),
              0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_preserves_bom_crlf_and_handles_no_final_newline) {
    ...
    ASSERT_NOT_NULL(strstr(actual, "# BEGIN project-mind-mcp\r\nowned = true\r\n"));
    ASSERT_EQ(pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n"), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ...
    ASSERT_EQ(th_write_file(path, "\xEF\xBB\xBF# BEGIN project-mind-mcp\r\n"
                                  "old = true\r\n"
                                  "# END project-mind-mcp\r\n"),
              0);
    ASSERT_EQ(pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "new = true\n"), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ...
    ASSERT_EQ(th_write_file(path, "\xEF\xBB\xBF[[\"mcp_servers\"]]\r\n"
                                  ...
                                  "command = \"old\""),
              0);
    ASSERT_EQ(pmm_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, CTE_BODY),
              0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT(memcmp(actual, "\xEF\xBB\xBF", 3U) == 0);
    ASSERT_NOT_NULL(strstr(actual, "command = \"project-mind-mcp\"\r\n"));
...
TEST(config_toml_rejects_oversized_input_file) {
    ...
    ASSERT_EQ(pmm_toml_remove_managed_block(path, CTE_BEGIN, CTE_END), -1);
    th_cleanup(dir);
    PASS();
}
...
TEST(config_toml_escape_windows_path_quotes) {
    char escaped[256];
    ASSERT_EQ(
        pmm_toml_escape_basic_string("C:\\Users\\Jane\\tool \"quoted\"", escaped, sizeof(escaped)),
        0);
...
TEST(config_toml_escape_newlines_controls) {
    char escaped[256];
    ASSERT_EQ(pmm_toml_escape_basic_string("line1\nline2\t\r\b\f\x01", escaped, sizeof(escaped)),
              0);
    ASSERT_STR_EQ(escaped, "line1\\nline2\\t\\r\\b\\f\\u0001");

    char too_small[4] = "bad";
    ASSERT_EQ(pmm_toml_escape_basic_string("overflow", too_small, sizeof(too_small)), -1);
    ASSERT_STR_EQ(too_small, "");
    ASSERT_EQ(pmm_toml_escape_basic_string(NULL, escaped, sizeof(escaped)), -1);
    ASSERT_EQ(pmm_toml_escape_basic_string("value", NULL, 0), -1);
    char invalid_utf8[] = {(char)0xff, '\0'};
    ASSERT_EQ(pmm_toml_escape_basic_string(invalid_utf8, escaped, sizeof(escaped)), -1);
...
TEST(config_toml_managed_fresh_missing_file) {
    ...
    ASSERT_EQ(pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "enabled = true\n"), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_STR_EQ(actual, "# BEGIN project-mind-mcp\n"
                          "enabled = true\n"
                          "# END project-mind-mcp\n");
    th_cleanup(dir);
    PASS();
...
TEST(config_toml_managed_replace_preserves_unrelated) {
    ...
    ASSERT_EQ(pmm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "new = \"value\""), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_managed_unbalanced_duplicate_fail_closed) {
    ...
    const char *duplicate = "# BEGIN project-mind-mcp\n"
                            "one = true\n"
                            "# END project-mind-mcp\n"
                            "# BEGIN project-mind-mcp\n"
                            "two = true\n"
...
TEST(config_toml_vibe_replace_target_preserves_comments_tables) {
    ...
    char path[CTE_PATH_CAP];
    char actual[CTE_FILE_CAP];
    const char *replacement = "name = \"project-mind-mcp\"\n"
                              "command = \"/new/path\"\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ...
    ASSERT_EQ(
        pmm_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, replacement), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_owned_table_installs_idempotently_and_removes_exact_state) {
    ...
    char path[CTE_PATH_CAP];
    char actual[CTE_FILE_CAP];
    const char *canonical = "name = \"project-mind-mcp\"\n"
                            "transport = \"stdio\"\n"
    ...
    ASSERT_EQ(
        pmm_toml_upsert_owned_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, canonical),
        0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ...
    ASSERT_EQ(
        pmm_toml_remove_owned_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, canonical),
        0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_owned_table_preserves_foreign_same_name_state) {
    ...
    for (size_t i = 0U; i < sizeof(foreign_cases) / sizeof(foreign_cases[0]); i++) {
        ...
        ASSERT_EQ(pmm_toml_upsert_owned_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY,
                                                          canonical),
                  PMM_TOML_OWNED_EDIT_FOREIGN);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, foreign_cases[i]);

        ASSERT_EQ(pmm_toml_remove_owned_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY,
                                                          canonical),
                  PMM_TOML_OWNED_EDIT_FOREIGN);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, foreign_cases[i]);
...
TEST(config_toml_vibe_ambiguous_target_fail_closed) {
    ...
    ASSERT_NOT_NULL(strstr(actual, "name = \"other\""));
    ASSERT_NOT_NULL(strstr(actual, "name = \"project-mind-mcp\""));
    th_cleanup(dir);
    PASS();
}
