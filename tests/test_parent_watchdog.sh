...
set -euo pipefail
exec 3<>"${FIFO}"
"${PMM_BINARY}" <&3 >"${TMPDIR_PATH}/child.out" 2>"${TMPDIR_PATH}/child.err" &
...
PMM_BINARY="${BINARY}" FIFO="${tmpdir}/stdin" TMPDIR_PATH="${tmpdir}" \
  "${tmpdir}/wrapper.sh" &
wrapper_pid=$!
...
echo "project-mind-mcp child ${child_pid} survived parent death" >&2
[[ -s "${tmpdir}/child.err" ]] && cat "${tmpdir}/child.err" >&2
exit 1
