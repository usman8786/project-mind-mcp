...
BINARY="$PROJECT_ROOT/build/c/project-mind-mcp"
FAKE_PROJECT="nonexistent_smoke_test_xyz"
CACHE_DIR="${HOME}/.cache/project-mind-mcp"
GHOST_FILE="$CACHE_DIR/${FAKE_PROJECT}.db"
FAILURES=0

# ── Step 1: Build ─────────────────────────────────────────────────
echo "[smoke_guard] Building project..."
make -f "$PROJECT_ROOT/Makefile.pmm" pmm -C "$PROJECT_ROOT" --quiet 2>&1
if [ ! -x "$BINARY" ]; then
...
check_handler() {
    ...
    # For a missing .db file, pmm_store_open_path_query returns NULL so
