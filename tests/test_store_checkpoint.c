 ...
 *
 * Verifies that pmm_store_checkpoint() does not truncate the on-disk
...
TEST(checkpoint_does_not_truncate_wal) {
    ...
    snprintf(db_path, sizeof(db_path), "%s/pmm_test_ckpt_%d.db", pmm_tmpdir(), (int)getpid());
    ...
    int rc_sql = pmm_store_exec(s, "INSERT OR IGNORE INTO projects(name, indexed_at, root_path) "
    ...
    int rc_ckpt = pmm_store_checkpoint(s);
    ASSERT_EQ(rc_ckpt, 0); /* PMM_STORE_OK */
 ...
 * pmm_store_dump_to_file, reopen — the stale generation's row must NOT be
...
TEST(dump_install_ignores_stale_wal_sidecar) {
    char *td = th_mktempdir("pmm_stalewal");
    char db_path[512];
    char wal_path[512];
    char stale_copy[512];
    ...
    ASSERT_NOT_NULL(s1);
    pmm_store_upsert_project(s1, "walgen", "/tmp/walgen");
    pmm_node_t stale = {.project = "walgen",
                        .label = "Function",
                        .name = "stale_gen_node",
                        ...
                        .start_line = 1,
                        .end_line = 2};
    ASSERT_TRUE(pmm_store_upsert_node(s1, &stale) > 0);
    ...
    pmm_store_close(s1); /* clean close checkpoints + removes the WAL */
    ...
    ASSERT_NOT_NULL(s2);
    pmm_store_upsert_project(s2, "walgen", "/tmp/walgen");
    pmm_node_t fresh = {.project = "walgen",
                        .label = "Function",
                        .name = "fresh_gen_node",
                        ...
                        .start_line = 1,
                        .end_line = 2};
    ASSERT_TRUE(pmm_store_upsert_node(s2, &fresh) > 0);
    ASSERT_EQ(pmm_store_dump_to_file(s2, db_path), PMM_STORE_OK);
    pmm_store_close(s2);

    /* Reader: the stale WAL must not have been replayed onto gen 2. */
    pmm_store_t *s3 = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(s3);
    pmm_node_t *hits = NULL;
    int hit_count = 0;
    ASSERT_EQ(pmm_store_find_nodes_by_name(s3, "walgen", "fresh_gen_node", &hits, &hit_count),
              PMM_STORE_OK);
    ASSERT_TRUE(hit_count >= 1);
    pmm_store_free_nodes(hits, hit_count);
    hits = NULL;
    hit_count = 0;
    ASSERT_EQ(pmm_store_find_nodes_by_name(s3, "walgen", "stale_gen_node", &hits, &hit_count),
              PMM_STORE_OK);
...
TEST(remove_db_sidecars_rejects_truncated_suffix_path) {
    ...
    ASSERT_TRUE(pmm_remove_db_sidecars(db_path) != 0);
    PASS();
}
