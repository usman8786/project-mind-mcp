 ...
 *
 * Two layers:
 *   1. pmm_proc_classify() — pure, exercised on EVERY platform (so the Windows
 *      NTSTATUS crash-code mapping is guarded on Linux/macOS CI too, not just an
 *      untested Windows branch).
 *   2. pmm_subprocess_run() — real spawn/reap, exercised on POSIX via /bin/sh
...
TEST(subprocess_classify_clean) {
    ASSERT_EQ(pmm_proc_classify(true, 0, 0, false), PMM_PROC_CLEAN);
    PASS();
}

TEST(subprocess_classify_exit_nonzero) {
    ASSERT_EQ(pmm_proc_classify(true, 3, 0, false), PMM_PROC_EXIT_NONZERO);
    ASSERT_EQ(pmm_proc_classify(true, 127, 0, false), PMM_PROC_EXIT_NONZERO);
    PASS();
}
...
TEST(subprocess_classify_windows_crash_codes) {
    ASSERT_EQ(pmm_proc_classify(true, (int)0xC0000005u, 0, false), PMM_PROC_CRASH);
    ASSERT_EQ(pmm_proc_classify(true, (int)0xC00000FDu, 0, false), PMM_PROC_CRASH);
    ASSERT_EQ(pmm_proc_classify(true, (int)0xC000001Du, 0, false), PMM_PROC_CRASH);
    ASSERT_EQ(pmm_proc_classify(true, (int)0xC000013Au, 0, false), PMM_PROC_KILLED);
    PASS();
}

TEST(subprocess_classify_posix_fault_signal_is_crash) {
#ifndef _WIN32
    ASSERT_EQ(pmm_proc_classify(false, -1, SIGSEGV, false), PMM_PROC_CRASH);
    ASSERT_EQ(pmm_proc_classify(false, -1, SIGABRT, false), PMM_PROC_CRASH);
    ASSERT_EQ(pmm_proc_classify(false, -1, SIGBUS, false), PMM_PROC_CRASH);
#endif
    PASS();
}
...
#ifndef _WIN32
    ASSERT_EQ(pmm_proc_classify(false, -1, SIGTERM, false), PMM_PROC_KILLED);
    ASSERT_EQ(pmm_proc_classify(false, -1, SIGKILL, false), PMM_PROC_KILLED);
#endif
    PASS();
}
...
TEST(subprocess_classify_timeout_dominates) {
    ASSERT_EQ(pmm_proc_classify(false, -1, 9 /*SIGKILL*/, true), PMM_PROC_HANG);
    ASSERT_EQ(pmm_proc_classify(true, 0, 0, true), PMM_PROC_HANG);
    PASS();
}

TEST(subprocess_outcome_str) {
    ASSERT_STR_EQ(pmm_proc_outcome_str(PMM_PROC_CLEAN), "clean");
    ASSERT_STR_EQ(pmm_proc_outcome_str(PMM_PROC_CRASH), "crash");
    ASSERT_STR_EQ(pmm_proc_outcome_str(PMM_PROC_HANG), "hang");
    ASSERT_STR_EQ(pmm_proc_outcome_str(PMM_PROC_EXIT_NONZERO), "exit_nonzero");
    ASSERT_STR_EQ(pmm_proc_outcome_str(PMM_PROC_KILLED), "killed");
    PASS();
}
...
#ifndef _WIN32
static pmm_proc_result_t run_sh(const char *script, int quiet_timeout_ms) {
    const char *argv[] = {"/bin/sh", "-c", script, NULL};
    pmm_proc_opts_t opts = {0};
    opts.bin = "/bin/sh";
    opts.argv = argv;
    opts.log_file = NULL;
    opts.quiet_timeout_ms = quiet_timeout_ms;
    pmm_proc_result_t r;
    pmm_subprocess_run(&opts, &r);
    return r;
}
#endif
...
#else
    pmm_proc_result_t r = run_sh("exit 0", 0);
    ASSERT_EQ(r.outcome, PMM_PROC_CLEAN);
...
TEST(subprocess_run_exit_nonzero) {
    ...
    SKIP_PLATFORM("POSIX /bin/sh spawn");
#else
    pmm_proc_result_t r = run_sh("exit 7", 0);
    ASSERT_EQ(r.outcome, PMM_PROC_EXIT_NONZERO);
...
TEST(subprocess_run_resolves_literal_binary_name_from_path) {
    ...
    pmm_proc_opts_t opts = {
        .bin = "sh",
        .argv = argv,
    };
    pmm_proc_result_t result;
    int rc = pmm_subprocess_run(&opts, &result);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(result.outcome, PMM_PROC_CLEAN);
...
#else
    pmm_proc_result_t r = run_sh("kill -SEGV $$", 0);
    ASSERT_EQ(r.outcome, PMM_PROC_CRASH);
...
TEST(subprocess_run_hang_is_hang) {
    ...
    SKIP_PLATFORM("POSIX /bin/sh spawn");
#else
    pmm_proc_result_t r = run_sh("sleep 30", 300 /* ms quiet-timeout */);
    ASSERT_EQ(r.outcome, PMM_PROC_HANG);
    PASS();
#endif
}
...
TEST(subprocess_run_spawn_failure) {
    ...
    const char *argv[] = {"/nonexistent/pmm-bogus-binary", NULL};
    pmm_proc_opts_t opts = {0};
    opts.bin = "/nonexistent/pmm-bogus-binary";
    opts.argv = argv;
    pmm_proc_result_t r;
    int rc = pmm_subprocess_run(&opts, &r);
...
TEST(subprocess_run_null_bin_rejected) {
    ...
    int rc = pmm_subprocess_run(&opts, &r);
    ASSERT_EQ(rc, -1);
    ASSERT_EQ(r.outcome, PMM_PROC_SPAWN_FAILED);
    PASS();
}

/* ── Layer 3: nonblocking handle + whole-tree cancellation (POSIX) ──────────
 *
 * The daemon cannot block its coordinator thread in pmm_subprocess_run(). It
...
static bool poll_until_terminal(pmm_subprocess_t *process, int timeout_ms, pmm_proc_result_t *out) {
    ...
    do {
        ...
        if (state == PMM_PROC_POLL_ERROR) {
            return false;
        }
        subprocess_test_pause();
    } while (pmm_now_ms() < deadline);
    return pmm_subprocess_poll(process, out) == PMM_PROC_POLL_TERMINAL;
}

static bool make_tree_pid_path(char path[64]) {
    strcpy(path, "/tmp/pmm-subprocess-tree-XXXXXX");
...
static bool wait_for_tree_pids(const char *path, pmm_subprocess_t *process, pid_t *parent_pid,
    ...
    do {
        ...
        pmm_proc_result_t ignored;
        if (pmm_subprocess_poll(process, &ignored) != PMM_PROC_POLL_RUNNING) {
            return false;
        }
        subprocess_test_pause();
    } while (pmm_now_ms() < deadline);
...
static int spawn_ignoring_tree(const char *pid_path, int quiet_timeout_ms, int cancel_grace_ms,
    ...
    const char *script = "trap '' TERM; "
                         "/bin/sh -c 'trap \"\" TERM; while :; do sleep 1; done' pmm-grandchild & "
                         "grandchild=$!; echo \"$$ $grandchild\" > \"$1\"; wait";
    const char *argv[] = {"/bin/sh", "-c", script, "pmm-parent", pid_path, NULL};
    pmm_proc_opts_t opts = {0};
    opts.bin = "/bin/sh";
    opts.argv = argv;
    opts.quiet_timeout_ms = quiet_timeout_ms;
    opts.cancel_grace_ms = cancel_grace_ms;
    return pmm_subprocess_spawn(&opts, out);
}
...
static void ordered_log_callback(const char *line, void *opaque) {
    ...
    if (index == 399) {
        ...
        capture->late_cancel_accepted = pmm_subprocess_request_cancel(capture->process);
    }
}
...
TEST(subprocess_spawn_returns_while_child_is_running) {
    ...
    uint64_t before_spawn = pmm_now_ms();
    int spawn_rc = pmm_subprocess_spawn(&opts, &process);
    uint64_t spawn_elapsed = pmm_now_ms() - before_spawn;
    ASSERT_EQ(spawn_rc, 0);
    ASSERT_NOT_NULL(process);

    pmm_proc_result_t result;
    uint64_t before_poll = pmm_now_ms();
    pmm_proc_poll_t first_poll = pmm_subprocess_poll(process, &result);
    uint64_t poll_elapsed = pmm_now_ms() - before_poll;
    bool cancel_accepted = pmm_subprocess_request_cancel(process);
     ...
     * PMM_LOCAL_CI_CPUS=4 fidelity pass, CI runners) can never make them
     * test-significant. */
    ASSERT_EQ(first_poll, PMM_PROC_POLL_RUNNING);
...
TEST(subprocess_natural_completion_is_cached_across_polls) {
    ...
    opts.cancel_grace_ms = 100;
    pmm_subprocess_t *process = NULL;
    ASSERT_EQ(pmm_subprocess_spawn(&opts, &process), 0);
    ASSERT_NOT_NULL(process);

    pmm_proc_result_t first;
    ASSERT_TRUE(poll_until_terminal(process, 2000, &first));
    pmm_proc_result_t second;
    pmm_proc_result_t third;
    pmm_proc_poll_t second_poll = pmm_subprocess_poll(process, &second);
    pmm_proc_poll_t third_poll = pmm_subprocess_poll(process, &third);
    pmm_subprocess_destroy(process);

    ASSERT_EQ(second_poll, PMM_PROC_POLL_TERMINAL);
    ASSERT_EQ(third_poll, PMM_PROC_POLL_TERMINAL);
    ASSERT_EQ(first.outcome, PMM_PROC_EXIT_NONZERO);
...
TEST(subprocess_cancel_is_idempotent_and_kills_ignoring_tree) {
    ...
    bool first_cancel = ready && pmm_subprocess_request_cancel(process);
    bool second_cancel = ready && pmm_subprocess_request_cancel(process);
    pmm_proc_result_t result;
    ...
    if (!terminal) {
        ...
        pmm_proc_result_t cleanup_result;
    ...
    ASSERT_TRUE(terminal);
    ASSERT_EQ(result.outcome, PMM_PROC_KILLED);
    ASSERT_TRUE(result.cancellation_requested);
...
TEST(subprocess_quiet_timeout_kills_ignoring_tree) {
    ...
    ASSERT_TRUE(ready);
    ASSERT_TRUE(terminal);
    ASSERT_EQ(result.outcome, PMM_PROC_HANG);
...
TEST(subprocess_cancel_grace_is_hard_capped) {
    ...
    pid_t grandchild_pid = -1;
    bool ready = wait_for_tree_pids(pid_path, process, &parent_pid, &grandchild_pid, 1000);
    bool cancel_accepted = ready && pmm_subprocess_request_cancel(process);
...
TEST(subprocess_poll_log_delivery_is_bounded_and_terminal_is_lossless) {
...
#else
    char log_path[] = "/tmp/pmm-subprocess-log-budget-XXXXXX";
    int log_fd = pmm_mkstemp(log_path);
    ...
    bool backlog_ready = wait_for_log_marker(log_path, "line-399", pmm_now_ms() + 2000U);
    pmm_proc_result_t result = {0};
    bool terminal = false;
    int max_poll_delivery = 0;
    uint64_t deadline = pmm_now_ms() + 5000U;
    while (backlog_ready && pmm_now_ms() < deadline) {
        int before = capture.count;
        pmm_proc_poll_t state = pmm_subprocess_poll(process, &result);
...
TEST(subprocess_final_log_drain_error_is_terminal_and_preserves_classification) {
#ifdef _WIN32
    SKIP_PLATFORM("POSIX log-rename final-drain probe; UTF-8 Windows open uses pmm_fopen");
#else
    char log_path[] = "/tmp/pmm-subprocess-log-drain-XXXXXX";
    int log_fd = pmm_mkstemp(log_path);
    ASSERT_TRUE(log_fd >= 0);
    (void)close(log_fd);
    ...
    bool backlog_ready = wait_for_log_marker(log_path, "line-129", pmm_now_ms() + 2000U);
    pmm_proc_result_t result = {0};
    pmm_proc_poll_t first =
        backlog_ready ? pmm_subprocess_poll(process, &result) : PMM_PROC_POLL_ERROR;
    int first_delivery = capture.count;
    bool moved = first == PMM_PROC_POLL_RUNNING && rename(log_path, saved_path) == 0;
    bool terminal = moved && poll_until_terminal(process, 2000, &result);
    int callbacks_at_terminal = capture.count;
    pmm_proc_result_t cached = {0};
    pmm_proc_poll_t cached_state =
        terminal ? pmm_subprocess_poll(process, &cached) : PMM_PROC_POLL_ERROR;
    ...
    ASSERT_TRUE(backlog_ready);
    ASSERT_EQ(first, PMM_PROC_POLL_RUNNING);
    ASSERT_EQ(first_delivery, 64);
    ASSERT_TRUE(moved);
    ASSERT_TRUE(terminal);
    ASSERT_TRUE(capture.ordered);
    ASSERT_EQ(callbacks_at_terminal, 64);
    ASSERT_EQ(cached_state, PMM_PROC_POLL_TERMINAL);
...
TEST(subprocess_posix_child_closes_unrelated_descriptors) {
...
#else
    char sentinel_path[] = "/tmp/pmm-subprocess-sentinel-XXXXXX";
    int sentinel = pmm_mkstemp(sentinel_path);
    ...
    const char *argv[] = {"/bin/sh", "-c", script, "pmm-fd-probe", fd_text, NULL};
    pmm_proc_opts_t opts = {0};
    opts.bin = "/bin/sh";
    opts.argv = argv;
    pmm_proc_result_t result;
    int run_rc = pmm_subprocess_run(&opts, &result);
    (void)close(sentinel);
...
TEST(subprocess_root_exit_drains_surviving_descendant) {
    ...
    const char *argv[] = {"/bin/sh", "-c", script, "pmm-root-exit", pid_path, NULL};
    pmm_proc_opts_t opts = {0};
    opts.bin = "/bin/sh";
    opts.argv = argv;
    opts.cancel_grace_ms = 100;
    ...
    pid_t descendant = -1;
    uint64_t deadline = pmm_now_ms() + 1000;
    while (descendant <= 1 && pmm_now_ms() < deadline) {
 ...
 * supervisor misreported as a per-file crash. We guard pmm_build_win_cmdline by
...
static bool cmdline_roundtrips(const char *const *argv) {
    char cmd[4096];
    if (!pmm_build_win_cmdline(cmd, sizeof(cmd), argv)) {
        return false;
    }
    char parsed[16][256];
...
TEST(win_cmdline_index_worker_json) {
    ...
    char cmd[4096];
    ASSERT(pmm_build_win_cmdline(cmd, sizeof(cmd), argv));
...
TEST(win_cmdline_overflow_rejected) {
    ...
    char tiny[8];
    ASSERT_FALSE(pmm_build_win_cmdline(tiny, sizeof(tiny), argv));
    PASS();
}
...
TEST(win_cmdline_overflow_leaves_empty_string) {
    ...
    ASSERT_FALSE(pmm_build_win_cmdline(buf, sizeof(buf), argv)); /* overflows cap */
...
TEST(win_cmd_payload_is_verbatim_and_capacity_checked) {
    ...
    ASSERT(pmm_build_win_cmd_payload(result, strlen(expected) + 1, cmd, payload));
    ASSERT_STR_EQ(result, expected);
    ASSERT_NULL(strstr(result, "\\\"C:\\Users"));

    memset(result, 'x', sizeof(result));
    ASSERT_FALSE(pmm_build_win_cmd_payload(result, strlen(expected), cmd, payload));
...
TEST(win_cmd_payload_rejects_short_relative_and_non_cmd_paths) {
    ...
    char result[256];
    ASSERT_FALSE(pmm_build_win_cmd_payload(result, sizeof(result), "", payload));
    ASSERT_FALSE(pmm_build_win_cmd_payload(result, sizeof(result), "C", payload));
    ASSERT_FALSE(pmm_build_win_cmd_payload(result, sizeof(result), "C:", payload));
    ASSERT_FALSE(pmm_build_win_cmd_payload(result, sizeof(result), "cmd.exe", payload));
    ASSERT_FALSE(
        pmm_build_win_cmd_payload(result, sizeof(result), "C:\\tools\\other.exe", payload));
    ASSERT_FALSE(
        pmm_build_win_cmd_payload(result, sizeof(result), "C:\\tools\\cmd.exe\\child", payload));
    ASSERT(
        pmm_build_win_cmd_payload(result, sizeof(result), "c:/Windows/System32/CMD.EXE", payload));
    PASS();
}
