 ...
 *
 * AREA A — CONFIGURES:
 *   Go os.Getenv: GREEN — pass_calls.c PMM_SVC_CONFIG fires on callee_name
 ...
 *     process.env.KEY is a member-access chain, not caught by PMM_SVC_CONFIG
 ...
 *   Sequential path: pass_usages.c pmm_resolve_file_read_writes runs for
...
static pmm_store_t *cp_open_indexed(CP_Proj *lp) {
...
static pmm_store_t *cp_index_files(CP_Proj *lp, const CP_File *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/pmm_cp_XXXXXX");
...
static void cp_cleanup(CP_Proj *lp, pmm_store_t *store) {
...
static int cp_edges(pmm_store_t *store, const char *project, const char *edge_type) {
    if (!store) return -1;
    return pmm_store_count_edges_by_type(store, project, edge_type);
}

/* Count nodes with a given label. */
static int cp_count_label(pmm_store_t *store, const char *project, const char *label) {
    if (!store) return -1;
...
static int cp_callables(pmm_store_t *store, const char *project) {
...
static void cp_diag(pmm_store_t *store, const char *project, const char *label) {
    if (!store) { fprintf(stderr, "  [CP] %s: no graph DB\n", label); return; }
    char line[640] = {0};
    for (int i = 0; CP_ALL_EDGE_TYPES[i]; i++) {
        int c = pmm_store_count_edges_by_type(store, project, CP_ALL_EDGE_TYPES[i]);
...
static pmm_store_t *cp_index_parallel(CP_Proj *lp, const CP_File *meaningful, int n_mean) {
 ...
 * callee QN is classified as PMM_SVC_CONFIG (callee_name matches
 ...
 * "os.Getenv" / "getenv" matches PMM_SVC_CONFIG pattern. */
TEST(cp_configures_go_getenv) {
    ...
    CP_Proj lp;
    pmm_store_t *store = cp_index_files(&lp, f, 1);
    int cfg = cp_edges(store, lp.project, "CONFIGURES");
    if (cfg < 1) cp_diag(store, lp.project, "configures/go_getenv");
    cp_cleanup(&lp, store);
    /* REAL BUG: internal/pmm/extract_env_accesses.c extracts os.Getenv into
 ...
 * or "os.getenv" should match PMM_SVC_CONFIG. */
TEST(cp_configures_python_os_getenv) {
     ...
     * (internal/pmm/extract_env_accesses.c). [KNOWN class 14] */
    ASSERT_TRUE(cfg >= 1);
    PASS();
}
 ...
 * not a call expression; PMM_SVC_CONFIG pattern-matches callee_name
...
TEST(cp_configures_ts_getenv_wrapper) {
    /* A3a: wrapper whose name triggers PMM_SVC_CONFIG */
    static const CP_File f[] = {
        {"env.ts",
     ...
     * its name matches PMM_SVC_CONFIG → CONFIGURES should fire. */
    ASSERT_TRUE(cfg >= 1);
    PASS();
}
 ...
 * form matches the PMM_SVC_CONFIG pattern. */
 ...
 * may be just "var" (too generic to match PMM_SVC_CONFIG) or the full path
...
TEST(cp_configures_rust_env_var) {
     ...
     * consumer of env_accesses (internal/pmm/extract_env_accesses.c).
     * [KNOWN class 14] */
    ASSERT_TRUE(cfg >= 1);
    PASS();
}
 ...
 *
 * pass_usages.c pmm_resolve_file_read_writes() runs on every file
...
TEST(cp_reads_writes_python_global) {
     ...
     * pmm_registry_add ONLY for Function/Method/Class/Interface labels — never
...
TEST(cp_reads_writes_go_global) {
    ...
    cp_cleanup(&lp, store);
    /* REAL BUG: Variable nodes never pmm_registry_add'd (pass_definitions.c
...
TEST(cp_reads_writes_java_static_field) {
    ...
    cp_cleanup(&lp, store);
    /* REAL BUG: Variable/Field nodes never pmm_registry_add'd (pass_definitions.c
     * process_def registers only Function/Method/Class/Interface), so
 ...
 * PMM_SVC_TRPC.  We use a module path "trpc/client.ts" so the QN carries
 * the "trpc" segment. */
 ...
 * PMM_SVC_GRAPHQL.  We place the wrapper in a module "graphql/client.py". */
...
TEST(cp_closure_ts_arrow) {
    ...
    CP_Proj lp;
    pmm_store_t *store = cp_index_files(&lp, f, 2);
    int calls = cp_edges(store, lp.project, "CALLS");
 ...
 * EXPECTED UNCERTAIN: Rust lsp_cross is not wired (pmm_pxc_has_cross_lsp
...
TEST(cp_enum_variant_rust_impl) {
     ...
     * Direction.  Rust is NOT in pmm_pxc_has_cross_lsp (src/pipeline/
...
TEST(cp_constant_go_const) {
    ...
    CP_Proj lp;
    pmm_store_t *store = cp_index_files(&lp, f, 1);
    int total = store ? pmm_store_count_nodes(store, lp.project) : -1;
