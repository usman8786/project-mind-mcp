...
#include <sqlite3.h>
#include "../src/foundation/compat_fs.h" /* pmm_unlink / pmm_rmdir */
#include "../src/foundation/constants.h"
#include "../src/foundation/log.h"
#include "../src/foundation/platform.h" /* pmm_file_size */
...
#ifdef _WIN32
#include <direct.h>
#define pmm_chdir _chdir
#define pmm_getcwd _getcwd
#else
#ifdef __APPLE__
#include <libproc.h>
#endif
...
#include <sys/wait.h>
#include <unistd.h>
#define pmm_chdir chdir
#define pmm_getcwd getcwd
extern char **environ;
#endif
...
static void restore_cache_dir(const char *saved_copy) {
    if (saved_copy) {
        pmm_setenv("PMM_CACHE_DIR", saved_copy, 1);
    } else {
...
typedef struct {
    ...
    int begin_count;
    int end_count;
    pmm_mcp_server_t *cancel_server;
    ...
    char begin_projects[MCP_MUTATION_GUARD_MAX_EVENTS][PMM_SZ_256];
    char end_projects[MCP_MUTATION_GUARD_MAX_EVENTS][PMM_SZ_256];
...
static void mcp_test_restore_env(mcp_test_env_backup_t *backups, size_t count) {
    for (size_t index = 0; index < count; index++) {
        if (backups[index].present) {
            (void)pmm_setenv(backups[index].name, backups[index].value, 1);
        } else {
            (void)pmm_unsetenv(backups[index].name);
        }
...
static int mcp_test_git(const char *root, const char *const *arguments) {
    char empty_config[PMM_SZ_4K];
    int config_length =
        snprintf(empty_config, sizeof(empty_config), "%s/.pmm-empty-gitconfig", root);
    if (config_length <= 0 || (size_t)config_length >= sizeof(empty_config)) {
        return -1;
    }
    FILE *config = pmm_fopen(empty_config, "wb");
    if (!config) {
        return -1;
    }
    if (fclose(config) != 0) {
    ...
    bool environment_ok = pmm_setenv("GIT_CONFIG_GLOBAL", empty_config, 1) == 0 &&
                          pmm_setenv("GIT_CONFIG_SYSTEM", empty_config, 1) == 0 &&
                          pmm_setenv("GIT_CONFIG_NOSYSTEM", "1", 1) == 0 &&
                          pmm_setenv("GIT_CONFIG_COUNT", "0", 1) == 0 &&
                          pmm_unsetenv("GIT_CONFIG_PARAMETERS") == 0;
    if (!environment_ok) {
    ...
    const char *git = "git";
#ifdef _WIN32
    char git_executable[PMM_SZ_4K];
    const char *resolved = pmm_find_cli("git", pmm_get_home_dir());
    int resolved_length =
    ...
    pmm_proc_result_t result = {0};
    int run_result =
        pmm_subprocess_run(&options, &result) == 0 && result.outcome == PMM_PROC_CLEAN ? 0 : -1;
    mcp_test_restore_env(backups, sizeof(backups) / sizeof(backups[0]));
    return run_result;
}
...
static bool mcp_mutation_guard_probe_begin(void *context, const char *project) {
    ...
    if (probe->cancel_on_begin_call > 0 && probe->begin_count == probe->cancel_on_begin_call) {
        ...
        probe->cancel_accepted = pmm_mcp_server_cancel_active(probe->cancel_server);
    }
    if (probe->observed_db_path) {
        probe->db_exists_at_begin = pmm_file_exists(probe->observed_db_path);
    }
    if (probe->observed_backup_path) {
        probe->backup_exists_at_begin = pmm_file_exists(probe->observed_backup_path);
    }
...
static void mcp_mutation_guard_probe_end(void *context, const char *project) {
    ...
    if (probe->observed_db_path) {
        probe->db_exists_at_end = pmm_file_exists(probe->observed_db_path);
    }
    if (probe->observed_backup_path) {
        probe->backup_exists_at_end = pmm_file_exists(probe->observed_backup_path);
    }
}

static bool mcp_make_corrupt_project_store(const char *cache, const char *project) {
    char db_path[PMM_SZ_1K];
     ...
     * pmm_store_check_integrity() and the issue #557 reproduction. */
    bool created = pmm_store_upsert_project(store, project, "826") == PMM_STORE_OK;
...
static pmm_store_t *mcp_open_corrupt_project_store_with_wal(const char *cache,
                                                            const char *project) {
    char db_path[PMM_SZ_1K];
    snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project);
    pmm_store_t *store = pmm_store_open_path(db_path);
    ...
    bool ready =
        pmm_store_exec(store, "PRAGMA wal_autocheckpoint=0;") == PMM_STORE_OK &&
        pmm_store_upsert_project(store, project, "826") == PMM_STORE_OK &&
        pmm_store_exec(store, "CREATE TABLE IF NOT EXISTS guard_wal_sentinel(value TEXT);"
...
static bool mcp_make_valid_project_store_at(const char *path, const char *project,
    ...
    pmm_store_t *store = pmm_store_open_path(path);
    if (!store) {
        return false;
    }
    bool ready = pmm_store_upsert_project(store, project, root_path) == PMM_STORE_OK &&
                 pmm_store_prepare_for_publish(store) == PMM_STORE_OK;
    pmm_store_close(store);
    return ready;
}
...
static int mcp_count_directory_entries_with_prefix(const char *directory, const char *prefix) {
    pmm_dir_t *dir = pmm_opendir(directory);
    if (!dir) {
        return -1;
    }
...
static bool mcp_replacing_mutation_guard_begin(void *context, const char *project) {
    ...
    bool sidecars_removed = pmm_remove_db_sidecars(replacement->live_path) == 0;
    replacement->replacement_succeeded =
        sidecars_removed &&
        pmm_rename_replace(replacement->replacement_path, replacement->live_path) == 0;
    return true;
}
...
TEST(tree_cell_sanitizes_control_and_invalid_utf8) {
    ...
    pmm_sb_t sb;
    pmm_sb_init(&sb);
    pmm_tree_cell_str(&sb,
                      "evil\x01name\xff"
                      "end",
                      true);
    char *out = pmm_sb_finish(&sb);
    ASSERT_NOT_NULL(out);
    ...
    pmm_sb_init(&sb);
    pmm_tree_cell_str(&sb, "b\xC3\xA4r_ok", true);
    out = pmm_sb_finish(&sb);
    ASSERT_NOT_NULL(out);
...
TEST(jsonrpc_parse_request) {
    ...
    int rc = pmm_jsonrpc_parse(line, &req);
    ASSERT_EQ(rc, 0);
...
TEST(jsonrpc_parse_invalid) {
    pmm_jsonrpc_request_t req = {0};
    int rc = pmm_jsonrpc_parse("not json", &req);
    ASSERT_EQ(rc, -1);
    pmm_jsonrpc_request_free(&req);
    PASS();
}
...
TEST(jsonrpc_parse_string_id_issue253) {
    ...
    pmm_jsonrpc_request_t req2 = {0};
    ASSERT_EQ(pmm_jsonrpc_parse(line2, &req2), 0);
    ASSERT_NOT_NULL(req2.id_str);
    ASSERT_STR_EQ(req2.id_str, "xyz");
    pmm_jsonrpc_request_free(&req2);
    PASS();
}
...
TEST(jsonrpc_format_response_string_id_issue253) {
    ...
    char *json = pmm_jsonrpc_format_response(&resp);
...
TEST(jsonrpc_format_response) {
    pmm_jsonrpc_response_t resp = {
        .id = 1,
        .result_json = "{\"name\":\"project-mind-mcp\"}",
    };
    char *json = pmm_jsonrpc_format_response(&resp);
    ASSERT_NOT_NULL(json);
...
TEST(jsonrpc_format_error) {
    char *json = pmm_jsonrpc_format_error(5, -32600, "Invalid Request");
    ASSERT_NOT_NULL(json);
...
TEST(mcp_initialize_response) {
    pmm_cli_set_version("9.8.7-test");

    /* Default (no params): returns latest supported version */
    char *json = pmm_mcp_initialize_response(NULL);
    ASSERT_NOT_NULL(json);
    ASSERT_NOT_NULL(strstr(json, "project-mind-mcp"));
    ...
    json = pmm_mcp_initialize_response("{\"protocolVersion\":\"2024-11-05\"}");
    ASSERT_NOT_NULL(json);
    ASSERT_NOT_NULL(strstr(json, "2024-11-05"));
    free(json);

    json = pmm_mcp_initialize_response("{\"protocolVersion\":\"2025-06-18\"}");
    ASSERT_NOT_NULL(json);
    ASSERT_NOT_NULL(strstr(json, "2025-06-18"));
    free(json);

    /* Client requests unknown version: server returns its latest */
    json = pmm_mcp_initialize_response("{\"protocolVersion\":\"9999-01-01\"}");
    ASSERT_NOT_NULL(json);
    ASSERT_NOT_NULL(strstr(json, "2025-11-25"));
    free(json);
...
TEST(mcp_text_result) {
    char *json = pmm_mcp_text_result("{\"total\":5}", false);
    ASSERT_NOT_NULL(json);
...
TEST(mcp_text_result_wraps_plain_text_as_structured_content) {
    char *json = pmm_mcp_text_result("plain text", false);
    ASSERT_NOT_NULL(json);
...
TEST(mcp_cancel_matches_request_id) {
    ASSERT_TRUE(pmm_mcp_cancel_request_matches("{\"requestId\":7}", 7, NULL));
    ASSERT_FALSE(pmm_mcp_cancel_request_matches("{\"requestId\":8}", 7, NULL));
    ASSERT_TRUE(pmm_mcp_cancel_request_matches("{\"requestId\":\"call-1\"}", -1, "call-1"));
    ASSERT_FALSE(pmm_mcp_cancel_request_matches("{\"requestId\":\"call-2\"}", -1, "call-1"));
    ASSERT_FALSE(pmm_mcp_cancel_request_matches("{\"requestId\":7}", -1, "7"));
    ASSERT_FALSE(pmm_mcp_cancel_request_matches("{}", 7, NULL));
    PASS();
}

TEST(mcp_text_result_error) {
    char *json = pmm_mcp_text_result("something failed", true);
    ASSERT_NOT_NULL(json);
...
TEST(mcp_get_tool_name) {
    ...
    char *name = pmm_mcp_get_tool_name(params);
...
TEST(mcp_get_arguments) {
    ...
    char *args = pmm_mcp_get_arguments(params);
...
TEST(mcp_get_string_arg) {
    ...
    char *val = pmm_mcp_get_string_arg(args, "label");
    ASSERT_NOT_NULL(val);
    ASSERT_STR_EQ(val, "Function");
    free(val);

    val = pmm_mcp_get_string_arg(args, "name_pattern");
    ASSERT_NOT_NULL(val);
    ASSERT_STR_EQ(val, ".*Order.*");
    free(val);

    val = pmm_mcp_get_string_arg(args, "nonexistent");
    ASSERT_NULL(val);
    PASS();
}
...
TEST(mcp_get_int_arg) {
    ...
    int val = pmm_mcp_get_int_arg(args, "limit", 0);
    ASSERT_EQ(val, 10);
    val = pmm_mcp_get_int_arg(args, "offset", 0);
    ASSERT_EQ(val, 5);
    val = pmm_mcp_get_int_arg(args, "missing", 42);
    ASSERT_EQ(val, 42);
    PASS();
}
...
TEST(mcp_get_bool_arg) {
    ...
    bool val = pmm_mcp_get_bool_arg(args, "include_connected");
    ASSERT_TRUE(val);
    val = pmm_mcp_get_bool_arg(args, "regex");
    ASSERT_FALSE(val);
    val = pmm_mcp_get_bool_arg(args, "missing");
    ASSERT_FALSE(val);
    PASS();
}
...
TEST(server_handle_initialize) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\","
                                   "\"params\":{\"capabilities\":{}}}");
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "\"id\":1"));
    ASSERT_NOT_NULL(strstr(resp, "project-mind-mcp"));
...
TEST(server_handle_initialized_notification) {
    ...
    char *resp = pmm_mcp_server_handle(
...
TEST(server_handle_tools_list) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/list\"}");
    ASSERT_NOT_NULL(resp);
...
TEST(server_handle_tools_list_defaults_to_all_tools_and_accepts_cursor) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":200,\"method\":\"tools/list\"}");
    ASSERT_NOT_NULL(resp);
...
TEST(server_handle_analysis_profile_filters_and_rejects_mutators) {
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    ASSERT_NOT_NULL(srv);
    pmm_mcp_server_set_tool_profile(srv, PMM_MCP_TOOL_PROFILE_ANALYSIS);
    ...
    resp = pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":220,\"method\":\"tools/list\"}");
    ASSERT_NOT_NULL(resp);
    ...
    resp = pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":221,\"method\":\"tools/call\","
...
TEST(server_handle_scout_profile_exposes_only_the_fast_tier) {
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    ASSERT_NOT_NULL(srv);
    pmm_mcp_server_set_tool_profile(srv, PMM_MCP_TOOL_PROFILE_SCOUT);
    mcp_saw_autoindex_log = false;
    pmm_log_set_sink_ex(mcp_capture_log, PMM_LOG_SINK_REPLACE);
    ...
    resp = pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":223,\"method\":\"tools/list\"}");
    ASSERT_NOT_NULL(resp);
    ...
    resp = pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":224,\"method\":\"tools/call\","
...
TEST(analysis_profile_arguments_fail_closed_and_disable_http) {
    pmm_mcp_tool_profile_t profile = PMM_MCP_TOOL_PROFILE_ALL;
    const char *no_profile[] = {"project-mind-mcp"};
    const char *analysis_equals[] = {"project-mind-mcp", "--tool-profile=analysis"};
    const char *analysis_pair[] = {"project-mind-mcp", "--tool-profile", "analysis"};
    const char *scout_equals[] = {"project-mind-mcp", "--tool-profile=scout"};
    const char *unknown_equals[] = {"project-mind-mcp", "--tool-profile=analaysis"};
    const char *unknown_pair[] = {"project-mind-mcp", "--tool-profile", "all"};
    const char *missing_value[] = {"project-mind-mcp", "--tool-profile"};

    ASSERT_EQ(pmm_mcp_parse_tool_profile_args(1, no_profile, &profile), 0);
    ASSERT_EQ(profile, PMM_MCP_TOOL_PROFILE_ALL);
    ASSERT_TRUE(pmm_mcp_tool_profile_allows_http(profile));

    ASSERT_EQ(pmm_mcp_parse_tool_profile_args(2, analysis_equals, &profile), 0);
    ASSERT_EQ(profile, PMM_MCP_TOOL_PROFILE_ANALYSIS);
    ASSERT_FALSE(pmm_mcp_tool_profile_allows_http(profile));

    ASSERT_EQ(pmm_mcp_parse_tool_profile_args(3, analysis_pair, &profile), 0);
    ASSERT_EQ(profile, PMM_MCP_TOOL_PROFILE_ANALYSIS);
    ASSERT_EQ(pmm_mcp_parse_tool_profile_args(2, scout_equals, &profile), 0);
    ASSERT_EQ(profile, PMM_MCP_TOOL_PROFILE_SCOUT);
    ASSERT_FALSE(pmm_mcp_tool_profile_allows_http(profile));
    ASSERT_EQ(pmm_mcp_parse_tool_profile_args(2, unknown_equals, &profile), -1);
    ASSERT_EQ(pmm_mcp_parse_tool_profile_args(3, unknown_pair, &profile), -1);
    ASSERT_EQ(pmm_mcp_parse_tool_profile_args(2, missing_value, &profile), -1);
    PASS();
}

TEST(hook_windows_path_containment_is_case_insensitive_and_segment_safe) {
    ASSERT_TRUE(pmm_hook_path_contains_for_testing("C:/Repo", "c:/repo/src/main.c", true));
    ASSERT_FALSE(pmm_hook_path_contains_for_testing("C:/Repo", "c:/repository/src/main.c", true));
    ASSERT_FALSE(pmm_hook_path_contains_for_testing("C:/Repo", "c:/repo/src/main.c", false));
    PASS();
}
...
TEST(server_handle_prompts_list_workflows) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":203,\"method\":\"prompts/list\"}");
    ASSERT_NOT_NULL(resp);
...
TEST(server_handle_prompts_get_workflows) {
    ...
    resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":205,\"method\":\"prompts/get\","
...
TEST(server_handle_prompts_get_validates_arguments) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":206,\"method\":\"prompts/get\","
    ...
    resp = pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":207,\"method\":\"prompts/get\","
                                      "\"params\":{\"name\":\"explore_codebase\",\"arguments\":{"
                                      "\"project\":\"payments\"}}}");
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "\"code\":-32602"));
    ...
    resp = pmm_mcp_server_handle(srv,
    ...
    resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":209,\"method\":\"prompts/get\","
                                   "\"params\":{\"name\":\"review_change_impact\",\"arguments\":{"
                                   "\"project\":\"payments\",\"change\":\"refund retry policy\","
                                   "\"base_branch\":\"\"}}}");
    ASSERT_NOT_NULL(resp);
    ...
    resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":210,\"method\":\"prompts/get\","
                                   "\"params\":{\"name\":\"review_change_impact\",\"arguments\":{"
                                   "\"project\":\"payments\",\"change\":\"refund retry policy\","
                                   "\"base_branch\":17}}}");
    ASSERT_NOT_NULL(resp);
...
TEST(server_handle_logs_request_without_params) {
    mcp_log_buf[0] = '\0';
    CBMLogLevel prev_level = pmm_log_get_level();
    ...
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":210,\"method\":\"tools/list\","
                                   "\"params\":{\"token\":\"secret\"}}");
    ASSERT_NOT_NULL(resp);
    free(resp);
    pmm_mcp_server_free(srv);
...
TEST(server_handle_unknown_method) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":3,\"method\":\"unknown/method\"}");
    ASSERT_NOT_NULL(resp);
...
static pmm_mcp_server_t *setup_mcp_with_data(void) {
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL); /* NULL = in-memory */
    return srv;
}

TEST(tool_list_projects_empty) {
    pmm_mcp_server_t *srv = setup_mcp_with_data();

    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":10,\"method\":\"tools/call\","
...
TEST(tool_get_graph_schema_empty) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":11,\"method\":\"tools/call\","
...
TEST(tool_unknown_tool) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":12,\"method\":\"tools/call\","
...
TEST(tool_search_graph_basic) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":13,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"search_graph\","
...
static pmm_mcp_server_t *setup_snippet_server(char *tmp_dir, size_t tmp_sz);
...
TEST(tool_trace_totals_respect_test_filter) {
    ...
    pmm_store_upsert_project(st, proj, "/tmp/tot");

    pmm_node_t tgt = {.project = proj,
                      .label = "Function",
                      .name = "tgt",
                      ...
                      .start_line = 1,
                      .end_line = 5};
    int64_t tid = pmm_store_upsert_node(st, &tgt);
    ASSERT_GT(tid, 0);
    pmm_node_t prod = {.project = proj,
                       .label = "Function",
                       .name = "prod_caller",
                       ...
                       .start_line = 10,
                       .end_line = 15};
    int64_t pid = pmm_store_upsert_node(st, &prod);
    ASSERT_GT(pid, 0);
    pmm_node_t tst = {.project = proj,
                      .label = "Function",
                      .name = "test_caller",
                      ...
                      .start_line = 1,
                      .end_line = 5};
    int64_t xid = pmm_store_upsert_node(st, &tst);
    ASSERT_GT(xid, 0);
    pmm_edge_t e1 = {.project = proj, .source_id = pid, .target_id = tid, .type = "CALLS"};
    ASSERT_GT(pmm_store_insert_edge(st, &e1), 0);
    pmm_edge_t e2 = {.project = proj, .source_id = xid, .target_id = tid, .type = "CALLS"};
    ASSERT_GT(pmm_store_insert_edge(st, &e2), 0);
...
TEST(tool_get_architecture_cycles_detects_scc) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/cyc");
    ...
    for (int i = 0; i < 5; i++) {
        ...
        pmm_node_t n = {.project = proj,
                        ...
                        .start_line = i + 1,
                        .end_line = i + 2};
        id[i] = pmm_store_upsert_node(st, &n);
        ASSERT_GT(id[i], 0);
    }
    ...
    for (size_t i = 0; i < sizeof(e) / sizeof(e[0]); i++) {
        ...
        ASSERT_GT(pmm_store_insert_edge(st, &ed), 0);
    }
...
TEST(tool_get_code_snippet_clips_whole_file_node) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_snipcap_XXXXXX");
    ASSERT_NOT_NULL(pmm_mkdtemp(tmp));
    char proj_dir[512];
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, proj_dir);
    ...
    ASSERT_GT(pmm_store_upsert_node(st, &mod), 0);
...
TEST(tool_search_graph_includes_node_properties) {
    ...
    char tmp[256];
    pmm_mcp_server_t *srv = setup_snippet_server(tmp, sizeof(tmp));
    ASSERT_NOT_NULL(srv);
...
TEST(tool_lean_defaults_schema_and_status) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":48,\"method\":\"tools/call\","
    ...
    resp = pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":49,\"method\":\"tools/call\","
...
static const char *check_tool_output(pmm_mcp_server_t *srv, const char *req, int ceiling,
                                     const char *floor_marker) {
    char *resp = pmm_mcp_server_handle(srv, req);
    if (!resp) {
        return "no response";
    }
    char *inner = extract_text_content(resp);
...
TEST(tool_search_graph_query_honors_file_pattern_issue552) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/issue-552");

    pmm_node_t lib_status = {0};
    ...
    ASSERT_GT(pmm_store_upsert_node(st, &lib_status), 0);

    pmm_node_t component_status = {0};
    ...
    ASSERT_GT(pmm_store_upsert_node(st, &component_status), 0);

    pmm_store_exec(st, "INSERT INTO nodes_fts(nodes_fts) VALUES('delete-all');");
    ASSERT_EQ(pmm_store_exec(st,
                             "INSERT INTO nodes_fts(rowid, name, qualified_name, label, "
                             "file_path) "
                             "SELECT id, pmm_camel_split(name), qualified_name, label, file_path "
                             "FROM nodes;"),
              PMM_STORE_OK);

    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":552,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"search_graph\","
...
TEST(mcp_resource_discovery_methods_return_empty_lists) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        char *resp = pmm_mcp_server_handle(srv, reqbuf);
        ASSERT_NOT_NULL(resp);
...
TEST(tool_index_status_no_project) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":15,\"method\":\"tools/call\","
...
TEST(tool_check_index_coverage_finds_path_beyond_status_cap) {
    ...
    ASSERT_EQ(pmm_store_upsert_project(st, project, "/tmp/coverage-cap-regression"), PMM_STORE_OK);
    pmm_mcp_server_set_project(srv, project);

    char (*paths)[64] = calloc(ROW_COUNT, sizeof(*paths));
    pmm_coverage_row_t *rows = calloc(ROW_COUNT, sizeof(*rows));
    ...
    for (int i = 0; i < ROW_COUNT; i++) {
        ...
        ASSERT_EQ(pmm_store_upsert_file_hash(st, project, paths[i], "fixture", i + 1, 10),
                  PMM_STORE_OK);
    }
    ASSERT_EQ(pmm_store_coverage_replace(st, project, rows, ROW_COUNT), PMM_STORE_OK);

    char *status =
        pmm_mcp_handle_tool(srv, "index_status", "{\"project\":\"coverage-cap-regression\"}");
    ...
    char *coverage = pmm_mcp_handle_tool(
...
TEST(tool_check_index_coverage_reports_paths_scopes_and_ranges) {
    ...
    ASSERT_EQ(pmm_store_upsert_file_hash(st, "test-project", "main.go", "", 0, 0), PMM_STORE_OK);
    ASSERT_EQ(pmm_store_upsert_file_hash(st, "test-project", "src/skip.c", "", 0, 0), PMM_STORE_OK);
    pmm_coverage_row_t rows[] = {
    ...
    ASSERT_EQ(pmm_store_coverage_replace(st, "test-project", rows, 3), PMM_STORE_OK);

    char *coverage =
        pmm_mcp_handle_tool(srv, "check_index_coverage",
...
static int write_coverage_meta(pmm_store_t *store, const char *generation,
                               const char *recording_status) {
    pmm_coverage_meta_t meta = {
    ...
    return pmm_store_coverage_replace_ex(store, "test-project", NULL, 0, &meta);
}
...
TEST(tool_check_index_coverage_rejects_stale_generation) {
    ...
    pmm_mcp_server_t *srv = setup_snippet_server(tmp, sizeof(tmp));
    ASSERT_NOT_NULL(srv);
    pmm_store_t *store = pmm_mcp_server_store(srv);
    ASSERT_NOT_NULL(store);
    ASSERT_EQ(write_coverage_meta(store, "stale-generation", "complete"), PMM_STORE_OK);

    char *response = pmm_mcp_handle_tool(srv, "check_index_coverage",
...
TEST(tool_check_index_coverage_requires_source_when_file_metadata_changed) {
    ...
    pmm_store_t *store = pmm_mcp_server_store(srv);
    ASSERT_NOT_NULL(store);
    pmm_project_t project = {0};
    ASSERT_EQ(pmm_store_get_project(store, "test-project", &project), PMM_STORE_OK);
    ASSERT_EQ(write_coverage_meta(store, project.indexed_at, "complete"), PMM_STORE_OK);
    pmm_project_free_fields(&project);
    ASSERT_EQ(pmm_store_upsert_file_hash(store, "test-project", "main.go", "fixture", 0, 0),
              PMM_STORE_OK);
...
TEST(tool_check_index_coverage_surfaces_lookup_errors) {
    ...
    pmm_project_free_fields(&project);
    ASSERT_EQ(
        pmm_store_exec(store, "ALTER TABLE index_coverage RENAME COLUMN detail TO broken_detail;"),
        PMM_STORE_OK);

    char *response = pmm_mcp_handle_tool(
        srv, "check_index_coverage",
...
TEST(tool_trace_call_path_not_found) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":20,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"trace_call_path\","
...
TEST(tool_trace_missing_function_name) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":21,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"trace_call_path\","
                                   "\"arguments\":{}}}");
    ASSERT_NOT_NULL(resp);
...
TEST(tool_trace_call_path_ambiguous) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/amb");
    pmm_node_t a = {.project = proj,
                    .label = "Function",
                    .name = "amb",
    ...
    ASSERT_GT(pmm_store_upsert_node(st, &a), 0);
    ASSERT_GT(pmm_store_upsert_node(st, &b), 0);
...
TEST(tool_trace_union_records_min_hop_across_seeds) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/dual");
    ...
    pmm_node_t tgt = {.project = proj,
                      ...
                      .start_line = 10,
                      .end_line = 15};
    int64_t ida = pmm_store_upsert_node(st, &sa);
    int64_t idb = pmm_store_upsert_node(st, &sb);
    int64_t idm = pmm_store_upsert_node(st, &mid);
    int64_t idt = pmm_store_upsert_node(st, &tgt);
    ASSERT_GT(ida, 0);
    ASSERT_GT(idb, 0);
    ASSERT_GT(idm, 0);
    ASSERT_GT(idt, 0);
    pmm_edge_t e1 = {.project = proj, .source_id = ida, .target_id = idm, .type = "CALLS"};
    pmm_edge_t e2 = {.project = proj, .source_id = idm, .target_id = idt, .type = "CALLS"};
    pmm_edge_t e3 = {.project = proj, .source_id = idb, .target_id = idt, .type = "CALLS"};
    ASSERT_GT(pmm_store_insert_edge(st, &e1), 0);
    ASSERT_GT(pmm_store_insert_edge(st, &e2), 0);
    ASSERT_GT(pmm_store_insert_edge(st, &e3), 0);
...
TEST(tool_trace_pagination_exactly_once) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/page");

    pmm_node_t hub = {.project = proj,
                      .label = "Function",
                      .name = "hub",
                      ...
                      .start_line = 1,
                      .end_line = 9};
    int64_t hid = pmm_store_upsert_node(st, &hub);
    ASSERT_GT(hid, 0);
    enum { CALLEES = 12 };
    for (int i = 0; i < CALLEES; i++) {
        ...
        pmm_node_t n = {.project = proj,
                        ...
                        .start_line = 1,
                        .end_line = 3};
        int64_t nid = pmm_store_upsert_node(st, &n);
        ASSERT_GT(nid, 0);
        pmm_edge_t e = {.project = proj, .source_id = hid, .target_id = nid, .type = "CALLS"};
        ASSERT_GT(pmm_store_insert_edge(st, &e), 0);
    }
    ...
    snprintf(req2, sizeof(req2),
             ...
             "\"cursor\":\"%s\"}}}",
             tok1);
    char *resp = pmm_mcp_server_handle(srv, req2);
    ASSERT_NOT_NULL(resp);
    char *inner = extract_text_content(resp);
    free(resp);
    ASSERT_NOT_NULL(inner);
    ...
    snprintf(req2, sizeof(req2),
             ...
             "\"cursor\":\"%s\"}}}",
             tok1);
    resp = pmm_mcp_server_handle(srv, req2);
    ASSERT_NOT_NULL(resp);
    inner = extract_text_content(resp);
    free(resp);
    ASSERT_NOT_NULL(inner);
...
TEST(tool_trace_call_path_prefers_definition) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/pref");
    /* nodes[0]: the WRONG match (a Module, tiny span), inserted first. */
    pmm_node_t wrong = {.project = proj,
                        .label = "Module",
                        .name = "dup",
    ...
    pmm_node_t def = {.project = proj,
                      ...
                      .start_line = 10,
                      .end_line = 50};
    pmm_node_t callee = {.project = proj,
                         .label = "Function",
                         .name = "callee",
                         ...
                         .start_line = 60,
                         .end_line = 70};
    ASSERT_GT(pmm_store_upsert_node(st, &wrong), 0);
    int64_t id_def = pmm_store_upsert_node(st, &def);
    int64_t id_callee = pmm_store_upsert_node(st, &callee);
    ASSERT_GT(id_def, 0);
    ASSERT_GT(id_callee, 0);
    pmm_edge_t e = {.project = proj, .source_id = id_def, .target_id = id_callee, .type = "CALLS"};
    pmm_store_insert_edge(st, &e);
 ...
 * clamped to the MCP ceiling (pmm_mcp_max_depth(), default 15). On origin/main
 ...
 * pmm_store_bfs to arbitrary depth. Over an 18-node call chain, depth=1000
...
TEST(tool_trace_call_path_depth_clamped) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/depth");
    ...
    for (int i = 0; i < 18; i++) {
        ...
        pmm_node_t n = {.project = proj,
                        ...
                        .start_line = 1,
                        .end_line = 2};
        ids[i] = pmm_store_upsert_node(st, &n);
    }
    for (int i = 0; i < 17; i++) {
        pmm_edge_t e = {
...
TEST(tool_trace_call_path_distinct_defs_not_over_unioned) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/ou");
    /* two unrelated real definitions of "dupreal", DIFFERENT body spans */
    pmm_node_t da = {.project = proj,
                     .label = "Function",
                     .name = "dupreal",
    ...
    pmm_node_t cb = {.project = proj,
                     ...
                     .start_line = 50,
                     .end_line = 60};
    int64_t id_da = pmm_store_upsert_node(st, &da);
    int64_t id_db = pmm_store_upsert_node(st, &db);
    int64_t id_ca = pmm_store_upsert_node(st, &ca);
    int64_t id_cb = pmm_store_upsert_node(st, &cb);
    ASSERT_GT(id_da, 0);
    ASSERT_GT(id_db, 0);
    ASSERT_GT(id_ca, 0);
    ASSERT_GT(id_cb, 0);
    pmm_edge_t ea = {.project = proj, .source_id = id_ca, .target_id = id_da, .type = "CALLS"};
    pmm_edge_t eb = {.project = proj, .source_id = id_cb, .target_id = id_db, .type = "CALLS"};
    pmm_store_insert_edge(st, &ea);
    pmm_store_insert_edge(st, &eb);
...
TEST(tool_trace_call_path_dts_stub_unions_with_impl) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/dts");
    pmm_node_t impl = {.project = proj,
                       .label = "Function",
                       .name = "sym546",
    ...
    pmm_node_t cali = {.project = proj,
                       ...
                       .start_line = 1,
                       .end_line = 8};
    int64_t id_impl = pmm_store_upsert_node(st, &impl);
    int64_t id_stub = pmm_store_upsert_node(st, &stub);
    int64_t id_crel = pmm_store_upsert_node(st, &crel);
    int64_t id_cali = pmm_store_upsert_node(st, &cali);
    ASSERT_GT(id_impl, 0);
    ASSERT_GT(id_stub, 0);
    ASSERT_GT(id_crel, 0);
    ASSERT_GT(id_cali, 0);
    /* callers split by import style: relative -> impl, path-alias -> stub */
    pmm_edge_t er = {.project = proj, .source_id = id_crel, .target_id = id_impl, .type = "CALLS"};
    pmm_edge_t el = {.project = proj, .source_id = id_cali, .target_id = id_stub, .type = "CALLS"};
    pmm_store_insert_edge(st, &er);
    pmm_store_insert_edge(st, &el);
...
TEST(tool_delete_project_not_found) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":22,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"delete_project\","
...
TEST(tool_delete_project_mutation_guard_blocks_then_releases) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-mcp-delete-guard-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        PASS();
    }

    const char *saved_cache = getenv("PMM_CACHE_DIR");
    ...
    char db_path[PMM_SZ_1K];
    snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project);
    pmm_store_t *setup = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(setup);
    ASSERT_EQ(pmm_store_upsert_project(setup, project, "/tmp/guard-delete-project"), PMM_STORE_OK);
    pmm_store_close(setup);
    ASSERT_TRUE(pmm_file_exists(db_path));

    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    ASSERT_NOT_NULL(srv);
    mcp_mutation_guard_probe_t probe = {.deny_begin_call = 1};
    pmm_mcp_server_set_project_mutation_guard(srv, mcp_mutation_guard_probe_begin,
                                              mcp_mutation_guard_probe_end, &probe);

    char *resp =
        pmm_mcp_handle_tool(srv, "delete_project", "{\"project\":\"guard-delete-project\"}");
    ASSERT_NOT_NULL(resp);
    ...
    resp = pmm_mcp_handle_tool(srv, "delete_project", "{\"project\":\"guard-delete-project\"}");
    ASSERT_NOT_NULL(resp);
    ...
    ASSERT_FALSE(pmm_file_exists(db_path));
    free(resp);
...
TEST(tool_index_repository_mutation_guard_blocks_before_local_worker) {
    char root[PMM_SZ_1K];
    (void)snprintf(root, sizeof(root), "%s/pmm-index-guard-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(root));
    ...
    (void)snprintf(args, sizeof(args),
                   ...
                   "\"mode\":\"fast\"}",
                   root);
    int spawn_before = pmm_index_supervisor_spawn_count();
    char *response = pmm_mcp_handle_tool(srv, "index_repository", args);
    int spawn_after = pmm_index_supervisor_spawn_count();
...
TEST(tool_get_architecture_empty) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":24,\"method\":\"tools/call\","
 ...
 * pmm_store_get_architecture and surface its sections. Before the fix
...
TEST(tool_get_architecture_emits_populated_sections) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/arch-test");

    pmm_node_t main_fn = {0};
    ...
    ASSERT_GT(pmm_store_upsert_node(st, &main_fn), 0);
     ...
     * when pmm_store_get_architecture is actually called and its result
...
TEST(tool_get_architecture_overview_compact_subset_pr560) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/arch560");
    ...
    pmm_node_t file_node = {.project = proj,
                            .label = "File",
                            .name = "main.go",
                            .qualified_name = "arch560.cmd.main.go",
                            .file_path = "cmd/main.go"};
    ASSERT_GT(pmm_store_upsert_node(st, &file_node), 0);

    /* Sanity: with "all", both entry_points and file_tree surface. */
    char *resp_all = pmm_mcp_server_handle(
...
TEST(tool_get_architecture_rejects_unknown_aspect_pr560) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/arch560v");
...
TEST(tool_get_architecture_accepts_project_name_alias_issue640) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/alias640");
...
TEST(tool_search_graph_accepts_project_name_alias_issue640) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/alias640b");
    ...
    fn.start_line = 1;
    fn.end_line = 2;
    ASSERT_GT(pmm_store_upsert_node(st, &fn), 0);
...
/* #1025: agents pass the repo FOLDER name ("project-mind-mcp"), but
...
TEST(tool_project_arg_resolves_unique_tail_issue1025) {
    char repo_a[PMM_SZ_256];
    char repo_b[PMM_SZ_256];
    char repo_c[PMM_SZ_256];
    char cache[PMM_SZ_256];
    snprintf(repo_a, sizeof(repo_a), "/tmp/pmm-i1025a-XXXXXX");
    snprintf(repo_b, sizeof(repo_b), "/tmp/pmm-i1025b-XXXXXX");
    snprintf(repo_c, sizeof(repo_c), "/tmp/pmm-i1025c-XXXXXX");
    snprintf(cache, sizeof(cache), "/tmp/pmm-i1025d-XXXXXX");
    if (!pmm_mkdtemp(repo_a) || !pmm_mkdtemp(repo_b) || !pmm_mkdtemp(repo_c) ||
        !pmm_mkdtemp(cache)) {
        FAIL("mkdtemp failed");
    }
    const char *saved_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache_copy = saved_cache ? pmm_strdup(saved_cache) : NULL;
    pmm_setenv("PMM_CACHE_DIR", cache, 1);
    pmm_setenv("PMM_INDEX_SUPERVISOR", "0", 1);
    ...
    snprintf(args, sizeof(args), "{\"repo_path\":\"%s\",\"name\":\"E-project-graph-suffix1025\"}",
             repo_a);
    char *r = pmm_mcp_handle_tool(srv, "index_repository", args);
    ASSERT_NOT_NULL(r);
    free(r);
    snprintf(args, sizeof(args), "{\"repo_path\":\"%s\",\"name\":\"F-alpha-amb1025\"}", repo_b);
    r = pmm_mcp_handle_tool(srv, "index_repository", args);
    ASSERT_NOT_NULL(r);
    free(r);
    ...
    r = pmm_mcp_handle_tool(srv, "search_graph",
    ...
    pmm_mcp_server_free(srv);
    if (saved_cache_copy) {
        pmm_setenv("PMM_CACHE_DIR", saved_cache_copy, 1);
        free(saved_cache_copy);
    } else {
        pmm_unsetenv("PMM_CACHE_DIR");
    }
...
TEST(tool_get_architecture_path_scoping) {
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, "/tmp/arch-path");

    pmm_node_t pkg_global = {.project = proj,
                             .label = "Package",
                             .name = "Django",
                             .qualified_name = "arch-path.Django",
                             .file_path = "vendor/django/__init__.py"};
    pmm_store_upsert_node(st, &pkg_global);

    pmm_node_t pkg_local = {.project = proj,
                            .label = "Package",
                            .name = "hoa",
                            .qualified_name = "arch-path.hoa",
                            .file_path = "apps/hoa/main.go"};
    pmm_store_upsert_node(st, &pkg_local);

    pmm_node_t f_hoa = {.project = proj,
                        .label = "File",
                        .name = "main.go",
                        .qualified_name = "arch-path.apps.hoa.main.go",
                        .file_path = "apps/hoa/main.go"};
    pmm_store_upsert_node(st, &f_hoa);

    pmm_node_t f_other = {.project = proj,
                          .label = "File",
                          .name = "other.go",
                          .qualified_name = "arch-path.other.go",
                          .file_path = "lib/other.go"};
    pmm_store_upsert_node(st, &f_other);

    char *resp_root = pmm_mcp_server_handle(
    ...
    char *resp_scoped =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":93,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"get_architecture\","
...
TEST(tool_query_graph_missing_query) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":23,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"query_graph\","
                                   "\"arguments\":{}}}");
    ASSERT_NOT_NULL(resp);
...
TEST(tool_index_repository_missing_path) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":30,\"method\":\"tools/call\","
...
TEST(tool_get_code_snippet_missing_qn) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":31,\"method\":\"tools/call\","
...
TEST(tool_get_code_snippet_not_found) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":32,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"get_code_snippet\","
...
TEST(tool_search_code_missing_pattern) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":33,\"method\":\"tools/call\","
...
TEST(tool_search_code_no_project) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":34,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"search_code\","
...
TEST(search_code_scoped_path_with_spaces_issue687) {
    char tmp[512];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_srch_space_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
    ...
    /* A node so the file is "indexed" (pmm_store_list_files -> scoped grep path)
...
TEST(search_code_scoped_path_with_cjk_root_issue903) {
    char tmp[512];
    snprintf(tmp, sizeof(tmp), "%s/pmm_srch_cjk_XXXXXX", pmm_tmpdir());
    if (!pmm_mkdtemp(tmp)) {
        FAIL("pmm_mkdtemp failed");
    }
...
static pmm_mcp_server_t *setup_prefilter_server(char *tmp, size_t tmp_sz, char *src_path,
                                                size_t src_sz, char *vendor_path,
                                                size_t vendor_sz) {
    snprintf(tmp, tmp_sz, "/tmp/pmm_srch_pref_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
        return NULL;
    }
    char dir[640];
    ...
    pmm_mcp_server_set_project(srv, proj);
    pmm_store_upsert_project(st, proj, tmp);

    pmm_node_t n1 = {.project = proj,
                     .label = "Function",
                     .name = "HandleRequest",
    ...
    pmm_node_t n2 = {.project = proj,
                     ...
                     .start_line = 3,
                     .end_line = 5};
    if (pmm_store_upsert_node(st, &n1) <= 0 || pmm_store_upsert_node(st, &n2) <= 0) {
        pmm_mcp_server_free(srv);
        return NULL;
    }
    return srv;
...
TEST(search_code_path_filter_prefilter_keeps_matches) {
    ...
    pmm_mcp_server_t *srv = setup_prefilter_server(tmp, sizeof(tmp), src_path, sizeof(src_path),
...
TEST(search_code_invalid_regex_errors_issue283) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":91,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"search_code\","
    ...
    resp = pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":92,\"method\":\"tools/call\","
                                      "\"params\":{\"name\":\"search_code\","
...
TEST(search_code_ampersand_accepted_issue272) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":94,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"search_code\","
...
TEST(tool_detect_changes_no_project) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":35,\"method\":\"tools/call\","
...
TEST(tool_manage_adr_no_project) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":36,\"method\":\"tools/call\","
...
TEST(tool_manage_adr_get_with_existing_adr) {
    /* Create a temp directory with .project-mind-mcp/adr.md */
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-adr-test-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        PASS(); /* skip if mkdtemp fails */
    }

    char adr_dir[512];
    snprintf(adr_dir, sizeof(adr_dir), "%s/.project-mind-mcp", tmp_dir);
    pmm_mkdir(adr_dir);
    ...
    pmm_store_t *st = pmm_mcp_server_store(srv);
    ASSERT_NOT_NULL(st);
    pmm_store_upsert_project(st, "test-adr-uaf", tmp_dir);
    pmm_mcp_server_set_project(srv, "test-adr-uaf");

    /* Call manage_adr via full JSON-RPC path to exercise pmm_jsonrpc_format_response.
 ...
 * backend. A manage_adr(update) write must be readable via pmm_store_adr_get
...
TEST(tool_manage_adr_unified_backend_issue256) {
    ...
    pmm_store_t *st = pmm_mcp_server_store(srv);
    ASSERT_NOT_NULL(st);
    pmm_store_upsert_project(st, "adr-unify", "/tmp/adr-unify");
    pmm_mcp_server_set_project(srv, "adr-unify");
    ...
    ASSERT_EQ(pmm_store_adr_get(st, "adr-unify", &adr), PMM_STORE_OK);
...
TEST(tool_manage_adr_mutation_guard_balances_success) {
    ...
    pmm_store_t *store = pmm_mcp_server_store(srv);
    ASSERT_NOT_NULL(store);
    ASSERT_EQ(pmm_store_upsert_project(store, project, "/tmp/guard-adr-success"), PMM_STORE_OK);
    pmm_mcp_server_set_project(srv, project);
    ...
    char *resp = pmm_mcp_handle_tool(srv, "manage_adr",
...
TEST(tool_manage_adr_mutation_guard_releases_on_missing_store) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-mcp-adr-guard-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        PASS();
    }
...
/* A raw pmm_mcp_handle_tool() call is still one request lifetime. Cancellation
...
TEST(tool_raw_dispatch_cancel_is_scoped_non_mutating_and_next_request_clean) {
    ...
    char root[256];
    snprintf(root, sizeof(root), "%s/pmm-mcp-raw-adr-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(root));
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    ASSERT_NOT_NULL(srv);
    pmm_store_t *store = pmm_mcp_server_store(srv);
    ASSERT_NOT_NULL(store);
    ASSERT_EQ(pmm_store_upsert_project(store, project, root), PMM_STORE_OK);
    pmm_mcp_server_set_project(srv, project);
    ...
    char *cancelled_response =
        pmm_mcp_handle_tool(srv, "manage_adr",
    ...
    pmm_adr_t cancelled_adr = {0};
    int cancelled_lookup = pmm_store_adr_get(store, project, &cancelled_adr);
    if (cancelled_lookup == PMM_STORE_OK) {
        pmm_store_adr_free(&cancelled_adr);
    }
    ...
    pmm_adr_t next_adr = {0};
    int next_lookup = pmm_store_adr_get(store, project, &next_adr);
    bool next_write_committed = next_lookup == PMM_STORE_OK && next_adr.content &&
                                strstr(next_adr.content, "Clean next request") &&
                                !strstr(next_adr.content, "MUST NOT COMMIT");
    if (next_lookup == PMM_STORE_OK) {
        pmm_store_adr_free(&next_adr);
    }
    ...
    ASSERT_EQ(cancelled_lookup, PMM_STORE_NOT_FOUND);
...
TEST(tool_outer_request_scope_preserves_predispatch_cancel) {
    ...
    char root[256];
    (void)snprintf(root, sizeof(root), "%s/pmm-mcp-outer-cancel-XXXXXX", pmm_tmpdir());
    bool root_created = pmm_mkdtemp(root) != NULL;
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    pmm_store_t *store = pmm_mcp_server_store(srv);
    bool project_ready =
        root_created && store && pmm_store_upsert_project(store, project, root) == PMM_STORE_OK;
    pmm_mcp_server_set_project(srv, project);
    bool outer_scope = project_ready && pmm_mcp_server_request_scope_begin(srv);
    bool cancel_accepted = outer_scope && pmm_mcp_server_cancel_active(srv);
    char *cancelled_response =
        cancel_accepted
            ? pmm_mcp_handle_tool(srv, "manage_adr",
    ...
    char *next_response = srv ? pmm_mcp_handle_tool(srv, "ingest_traces", "{\"traces\":[]}") : NULL;
...
TEST(tool_index_repository_early_raw_cancel_survives_index_entry) {
    char cache[256];
    char repo[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-raw-index-cache-XXXXXX", pmm_tmpdir());
    snprintf(repo, sizeof(repo), "%s/pmm-mcp-raw-index-repo-XXXXXX", pmm_tmpdir());
    bool cache_created = pmm_mkdtemp(cache) != NULL;
    bool repo_created = pmm_mkdtemp(repo) != NULL;
    ...
    char *project = repo_created ? pmm_project_name_from_path(repo) : NULL;
    pmm_mcp_server_t *srv =
        cache_created && repo_created && project ? pmm_mcp_server_new(NULL) : NULL;
    mcp_mutation_guard_probe_t probe = {
        .cancel_on_begin_call = 1,
        .cancel_server = srv,
    };
    ...
    char *cancelled_response = srv ? pmm_mcp_handle_tool(srv, "index_repository", args) : NULL;
    bool cancellation_reported = cancelled_response && strstr(cancelled_response, "cancelled") &&
                                 strstr(cancelled_response, "\"isError\":true");

    char db_path[PMM_SZ_1K];
    snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project ? project : "missing-project");
    bool no_project_published = !pmm_file_exists(db_path);
...
static bool mcp_cross_repo_create_project_store(const char *cache, const char *project,
    ...
    bool created = pmm_store_upsert_project(store, project, root_path) == PMM_STORE_OK;
    pmm_store_close(store);
    return created;
}
...
static bool mcp_cross_repo_seed_http_match(const char *cache, const char *source_project,
    ...
    char source_path[PMM_SZ_1K];
    char target_path[PMM_SZ_1K];
    ...
    pmm_store_t *source = pmm_store_open_path(source_path);
    pmm_store_t *target = pmm_store_open_path(target_path);
    if (!source || !target) {
        pmm_store_close(source);
        pmm_store_close(target);
        return false;
    }

    bool ok = pmm_store_upsert_project(source, source_project, root_path) == PMM_STORE_OK &&
              pmm_store_upsert_project(target, target_project, root_path) == PMM_STORE_OK;

    pmm_node_t caller = {.project = source_project,
                         .label = "Function",
                         .name = "call_once",
                         ...
                         .start_line = 1,
                         .end_line = 2};
    pmm_node_t local_route = {.project = source_project,
                              .label = "Route",
                              .name = "GET /dedupe",
                              ...
                              .start_line = 3,
                              .end_line = 3};
    int64_t caller_id = ok ? pmm_store_upsert_node(source, &caller) : 0;
    int64_t local_route_id = ok ? pmm_store_upsert_node(source, &local_route) : 0;
    pmm_edge_t http_call = {.project = source_project,
                            .source_id = caller_id,
                            .target_id = local_route_id,
                            .type = "HTTP_CALLS",
                            .properties_json = "{\"url_path\":\"/dedupe\",\"method\":\"GET\"}"};
    ok = ok && caller_id > 0 && local_route_id > 0 && pmm_store_insert_edge(source, &http_call) > 0;

    pmm_node_t target_route = {.project = target_project,
                               .label = "Route",
                               .name = "GET /dedupe",
                               .qualified_name = "__route__GET__/dedupe",
                               .file_path = "server.c",
                               .start_line = 3,
                               .end_line = 3};
    pmm_node_t handler = {.project = target_project,
                          .label = "Function",
                          .name = "handle_once",
                          ...
                          .start_line = 1,
                          .end_line = 2};
    int64_t target_route_id = ok ? pmm_store_upsert_node(target, &target_route) : 0;
    int64_t handler_id = ok ? pmm_store_upsert_node(target, &handler) : 0;
    pmm_edge_t handles = {.project = target_project,
                          .source_id = handler_id,
                          .target_id = target_route_id,
                          .type = "HANDLES"};
    ok = ok && target_route_id > 0 && handler_id > 0 && pmm_store_insert_edge(target, &handles) > 0;
...
TEST(tool_cross_repo_mutation_guard_sorts_dedupes_and_unwinds) {
    char repo[256];
    snprintf(repo, sizeof(repo), "/tmp/pmm-mcp-cross-guard-XXXXXX");
    if (!pmm_mkdtemp(repo)) {
        PASS();
    }
    ...
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    ASSERT_NOT_NULL(srv);
    ASSERT_TRUE(pmm_mcp_server_set_session_context(srv, repo, NULL));
    ...
    snprintf(args, sizeof(args),
             ...
             repo);
    char *resp = pmm_mcp_handle_tool(srv, "index_repository", args);
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "blocked"));
...
TEST(tool_cross_repo_mutation_guard_casefolds_aliases_and_order) {
    char repo[256];
    snprintf(repo, sizeof(repo), "/tmp/pmm-mcp-cross-case-guard-XXXXXX");
    if (!pmm_mkdtemp(repo)) {
        PASS();
    }
    ...
    char first_args[PMM_SZ_2K];
    snprintf(first_args, sizeof(first_args),
             "{\"repo_path\":\"%s\",\"name\":\"Zulu\","
             "\"mode\":\"cross-repo-intelligence\","
             "\"target_projects\":[\"Foo\",\"foo\",\"Alpha\"]}",
             repo);
    char *first_resp = pmm_mcp_handle_tool(srv, "index_repository", first_args);
    ...
    char second_args[PMM_SZ_2K];
    snprintf(second_args, sizeof(second_args),
             "{\"repo_path\":\"%s\",\"name\":\"zULU\","
             "\"mode\":\"cross-repo-intelligence\","
             "\"target_projects\":[\"foo\",\"ALPHA\",\"FOO\"]}",
             repo);
    char *second_resp = pmm_mcp_handle_tool(srv, "index_repository", second_args);
...
TEST(tool_cross_repo_rejects_wildcard_mixed_with_named_targets) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-cross-wildcard-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));

    const char *saved_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache_copy = saved_cache ? strdup(saved_cache) : NULL;
    pmm_setenv("PMM_CACHE_DIR", cache, 1);

    char *project = pmm_project_name_from_path(cache);
    ASSERT_NOT_NULL(project);
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    ASSERT_NOT_NULL(srv);
    ASSERT_TRUE(pmm_mcp_server_set_session_context(srv, cache, NULL));
...
TEST(tool_cross_repo_checks_cancellation_after_acquiring_leases) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-cross-cancel-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
...
/* pmm_store_open_path() creates its path. Cross-repo validation must therefore
...
TEST(tool_cross_repo_missing_inputs_fail_without_creating_ghost_databases) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-cross-missing-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    ...
    char *source_project = pmm_project_name_from_path(cache);
    ...
    char source_db_path[PMM_SZ_1K];
    char missing_target_db_path[PMM_SZ_1K];
    ...
    snprintf(missing_target_db_path, sizeof(missing_target_db_path), "%s/%s.db", cache,
             missing_target);
    ASSERT_FALSE(pmm_file_exists(source_db_path));
    ...
    char *source_resp = pmm_mcp_handle_tool(srv, "index_repository", args);
    ...
    bool source_ghost_created = pmm_file_exists(source_db_path);
    free(source_resp);

    cleanup_project_db(cache, source_project);
    ASSERT_TRUE(mcp_cross_repo_create_project_store(cache, source_project, cache));
    ASSERT_FALSE(pmm_file_exists(missing_target_db_path));

    snprintf(args, sizeof(args),
             "{\"repo_path\":\"%s\",\"mode\":\"cross-repo-intelligence\","
             "\"target_projects\":[\"%s\"]}",
             cache, missing_target);
    char *target_resp = pmm_mcp_handle_tool(srv, "index_repository", args);
    ...
    bool target_ghost_created = pmm_file_exists(missing_target_db_path);
    free(target_resp);
...
TEST(tool_cross_repo_dedupes_targets_before_scanning_and_counting) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-cross-dedupe-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    ...
    char source_db_path[PMM_SZ_1K];
    char target_db_path[PMM_SZ_1K];
    snprintf(source_db_path, sizeof(source_db_path), "%s/%s.db", cache, source_project);
    snprintf(target_db_path, sizeof(target_db_path), "%s/%s.db", cache, target_project);
    pmm_store_t *source = pmm_store_open_path_query(source_db_path);
    pmm_store_t *target = pmm_store_open_path_query(target_db_path);
    int source_cross_edges =
        source ? pmm_store_count_edges_by_type(source, source_project, "CROSS_HTTP_CALLS") : -1;
    int target_cross_edges =
        target ? pmm_store_count_edges_by_type(target, target_project, "CROSS_HTTP_CALLS") : -1;
    pmm_store_close(source);
    pmm_store_close(target);
...
TEST(tool_cross_repo_honors_source_name_override) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-cross-name-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
...
TEST(tool_corrupt_store_cleanup_guard_is_balanced_and_not_nested) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-corrupt-guard-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    ...
    pmm_mcp_server_t *query_srv = pmm_mcp_server_new(NULL);
    ASSERT_NOT_NULL(query_srv);
    mcp_mutation_guard_probe_t query_probe = {
        .observed_db_path = db_path,
    };
    pmm_mcp_server_set_project_mutation_guard(query_srv, mcp_mutation_guard_probe_begin,
                                              mcp_mutation_guard_probe_end, &query_probe);

    char *resp =
        pmm_mcp_handle_tool(query_srv, "search_graph",
                            "{\"project\":\"guard-corrupt-project\",\"name_pattern\":\".*\"}");
    free(resp);
    pmm_mcp_server_free(query_srv);
    char query_backup_path[PMM_SZ_1K];
    ...
    bool query_quarantined =
        !pmm_file_exists(db_path) && query_backup_count == 1 && query_backup_path[0] != '\0';
    ...
    ASSERT_TRUE(mcp_make_corrupt_project_store(cache, project));
    pmm_mcp_server_t *adr_srv = pmm_mcp_server_new(NULL);
    ASSERT_NOT_NULL(adr_srv);
    mcp_mutation_guard_probe_t adr_probe = {
        .observed_db_path = db_path,
    };
    pmm_mcp_server_set_project_mutation_guard(adr_srv, mcp_mutation_guard_probe_begin,
                                              mcp_mutation_guard_probe_end, &adr_probe);
    resp = pmm_mcp_handle_tool(adr_srv, "manage_adr",
                               "{\"project\":\"guard-corrupt-project\",\"mode\":\"get\"}");
    free(resp);
    pmm_mcp_server_free(adr_srv);
    char adr_backup_path[PMM_SZ_1K];
    int adr_backup_count =
        mcp_find_corrupt_backups(cache, project, adr_backup_path, sizeof(adr_backup_path));
    bool adr_quarantined =
        !pmm_file_exists(db_path) && adr_backup_count == 1 && adr_backup_path[0] != '\0';
...
TEST(tool_corrupt_store_cleanup_guard_denial_preserves_db_and_wal) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-corrupt-denied-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    ...
    char db_path[PMM_SZ_1K];
    char wal_path[PMM_SZ_1K];
    snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project);
    snprintf(wal_path, sizeof(wal_path), "%s-wal", db_path);
    pmm_store_t *writer = mcp_open_corrupt_project_store_with_wal(cache, project);
    ASSERT_NOT_NULL(writer);
    ASSERT_TRUE(pmm_file_exists(db_path));
    ASSERT_TRUE(pmm_file_exists(wal_path));
    ...
    char unexpected_backup[PMM_SZ_1K];
    int backup_count =
...
TEST(tool_corrupt_store_cleanup_rechecks_generation_after_guard_wait) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-corrupt-recheck-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    ...
    char db_path[PMM_SZ_1K];
    char replacement_path[PMM_SZ_1K];
    snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project);
    ...
    pmm_mcp_server_set_project_mutation_guard(srv, mcp_replacing_mutation_guard_begin,
    ...
    pmm_store_t *check = pmm_store_open_path_query(db_path);
    bool valid_generation = check && pmm_store_check_integrity(check);
    pmm_project_t stored_project = {0};
    bool replacement_root_visible =
        check && pmm_store_get_project(check, project, &stored_project) == PMM_STORE_OK &&
        stored_project.root_path && strcmp(stored_project.root_path, replacement_root) == 0;
    pmm_project_free_fields(&stored_project);
    pmm_store_close(check);
    char unexpected_backup[PMM_SZ_1K];
    int backup_count =
        mcp_find_corrupt_backups(cache, project, unexpected_backup, sizeof(unexpected_backup));
    bool live_exists = pmm_file_exists(db_path);
    bool replacement_consumed = !pmm_file_exists(replacement_path);
...
TEST(tool_corrupt_store_cleanup_preserves_existing_backup_and_uses_unique_name) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-corrupt-unique-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    ...
    char db_path[PMM_SZ_1K];
    char existing_backup_path[PMM_SZ_1K];
    snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project);
    ...
    char unique_backup_path[PMM_SZ_1K];
    int backup_count =
        mcp_find_corrupt_backups(cache, project, unique_backup_path, sizeof(unique_backup_path));
    pmm_store_t *quarantined =
        unique_backup_path[0] ? pmm_store_open_path_query(unique_backup_path) : NULL;
    bool unique_backup_is_corrupt = quarantined && !pmm_store_check_integrity(quarantined);
    pmm_store_close(quarantined);
    bool live_removed = !pmm_file_exists(db_path);
    int begin_count = probe.begin_count;
    int end_count = probe.end_count;
    bool guarded_project = begin_count == 1 && end_count == 1 &&
...
TEST(tool_corrupt_store_cleanup_publish_failure_preserves_db_and_wal) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-corrupt-publish-fail-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    ...
    pmm_mcp_server_set_quarantine_test_hook(srv, mcp_quarantine_hook_probe, &hook);
    char *resp =
...
TEST(tool_corrupt_store_cleanup_publishes_complete_wal_snapshot_before_delete) {
    char cache[256];
    snprintf(cache, sizeof(cache), "%s/pmm-mcp-corrupt-after-publish-XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(cache));
    ...
    bool db_unchanged = mcp_file_matches_snapshot(db_path, db_before, db_len);
    bool wal_unchanged = mcp_file_matches_snapshot(wal_path, wal_before, wal_len);
    char backup_path[PMM_SZ_1K];
    int backup_count = mcp_find_corrupt_backups(cache, project, backup_path, sizeof(backup_path));
    int artifact_count = mcp_count_corrupt_artifacts(cache, project);
    pmm_store_t *snapshot = backup_path[0] ? pmm_store_open_path_query(backup_path) : NULL;
    pmm_project_t recovered = {0};
    bool recovered_wal_project =
        snapshot && pmm_store_get_project(snapshot, project, &recovered) == PMM_STORE_OK &&
        recovered.root_path && strcmp(recovered.root_path, "826") == 0;
    pmm_project_free_fields(&recovered);
    pmm_store_close(snapshot);
    char backup_wal[PMM_SZ_2K];
    char backup_shm[PMM_SZ_2K];
    snprintf(backup_wal, sizeof(backup_wal), "%s-wal", backup_path);
    snprintf(backup_shm, sizeof(backup_shm), "%s-shm", backup_path);
    bool snapshot_self_contained = !pmm_file_exists(backup_wal) && !pmm_file_exists(backup_shm);
...
TEST(tool_index_repository_reports_store_backed_adr) {
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-index-adr-test-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        PASS();
    }
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-index-adr-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        pmm_rmdir(tmp_dir);
        PASS();
    }

    const char *saved = getenv("PMM_CACHE_DIR");
    ...
    char *project = pmm_project_name_from_path(tmp_dir);
    ASSERT_NOT_NULL(project);
    ...
    snprintf(update_args, sizeof(update_args),
             ...
             project);
    resp = pmm_mcp_handle_tool(srv, "manage_adr", update_args);
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "updated"));
    free(resp);

    resp = pmm_mcp_handle_tool(srv, "index_repository", args);
    ASSERT_NOT_NULL(resp);
    ASSERT(response_contains_json_fragment(resp, "\"status\":\"indexed\""));
    ...
    resp = pmm_mcp_handle_tool(srv, "manage_adr", get_args);
    ASSERT_NOT_NULL(resp);
...
TEST(tool_index_repository_dot_uses_absolute_project_key_and_preserves_adr) {
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-index-dot-adr-test-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        PASS();
    }
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-index-dot-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        pmm_rmdir(tmp_dir);
        PASS();
    }
    ...
    char old_cwd[PMM_SZ_4K];
    ASSERT_NOT_NULL(pmm_getcwd(old_cwd, sizeof(old_cwd)));
    ...
    ASSERT_EQ(pmm_chdir(tmp_dir), 0);
    char *resp =
        pmm_mcp_handle_tool(srv, "index_repository", "{\"repo_path\":\".\",\"mode\":\"fast\"}");
    ASSERT_EQ(pmm_chdir(old_cwd), 0);
    ASSERT_NOT_NULL(resp);
    ...
    ASSERT_EQ(pmm_chdir(tmp_dir), 0);
    resp = pmm_mcp_handle_tool(srv, "index_repository", "{\"repo_path\":\".\",\"mode\":\"fast\"}");
    ASSERT_EQ(pmm_chdir(old_cwd), 0);
    ASSERT_NOT_NULL(resp);
    ASSERT(response_contains_json_fragment(resp, "\"status\":\"indexed\""));
    ASSERT_NOT_NULL(strstr(resp, project));
...
TEST(tool_manage_adr_not_found_rich_error) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-adr-missing-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        PASS();
    }
    ...
    char *resp = pmm_mcp_handle_tool(srv, "manage_adr",
                                     "{\"project\":\"pmm-no-such-project-zzz\",\"mode\":\"get\"}");
    ASSERT_NOT_NULL(resp);
...
TEST(tool_manage_adr_get_accepts_abs_path) {
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-adr-abspath-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        PASS();
    }
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-adr-abspath-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        pmm_rmdir(tmp_dir);
        PASS();
    }
...
#else
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-adr-realpath-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        PASS();
    }
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-adr-realpath-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        pmm_rmdir(tmp_dir);
        PASS();
    }
...
TEST(tool_detect_changes_not_found_rich_error) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-detect-missing-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        PASS();
    }
    ...
    char *resp =
        pmm_mcp_handle_tool(srv, "detect_changes", "{\"project\":\"pmm-no-such-project-zzz\"}");
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "or not indexed"));
    ASSERT_NOT_NULL(strstr(resp, "hint"));
    free(resp);
...
TEST(tool_detect_changes_contained_commands_clean_up_error_and_success) {
    char cache[512];
    (void)snprintf(cache, sizeof(cache), "%s/pmm-detect-contained-XXXXXX", pmm_tmpdir());
    bool cache_created = pmm_mkdtemp(cache) != NULL;
    char repo[PMM_SZ_4K];
    (void)snprintf(repo, sizeof(repo), "%s/pmm-detect-repo-XXXXXX", pmm_tmpdir());
    bool repo_created = pmm_mkdtemp(repo) != NULL;
    char empty_template[PMM_SZ_4K];
    char empty_hooks[PMM_SZ_4K];
    char template_argument[PMM_SZ_4K];
    char hooks_config[PMM_SZ_4K];
    char hostile_template[PMM_SZ_4K];
    char hostile_template_hooks[PMM_SZ_4K];
    char hostile_hooks[PMM_SZ_4K];
    char hostile_hook[PMM_SZ_4K];
    char hostile_config[PMM_SZ_4K];
    int template_length =
        snprintf(empty_template, sizeof(empty_template), "%s/.pmm-empty-template", repo);
    int hooks_length = snprintf(empty_hooks, sizeof(empty_hooks), "%s/.pmm-empty-hooks", repo);
    ...
    int hostile_template_length =
        snprintf(hostile_template, sizeof(hostile_template), "%s/.pmm-hostile-template", repo);
    ...
    int hostile_hooks_length =
        snprintf(hostile_hooks, sizeof(hostile_hooks), "%s/.pmm-hostile-hooks", repo);
    int hostile_hook_length =
        snprintf(hostile_hook, sizeof(hostile_hook), "%s/pre-commit", hostile_hooks);
    int hostile_config_length =
        snprintf(hostile_config, sizeof(hostile_config), "%s/.pmm-hostile-gitconfig", repo);
    bool git_isolation_ready =
        ...
        (size_t)hooks_config_length < sizeof(hooks_config) && pmm_mkdir(empty_template) == 0 &&
        pmm_mkdir(empty_hooks) == 0;
    bool hostile_paths_ready =
        ...
        pmm_mkdir(hostile_template) == 0 && pmm_mkdir(hostile_template_hooks) == 0 &&
        pmm_mkdir(hostile_hooks) == 0;
    FILE *hostile_hook_file = hostile_paths_ready ? pmm_fopen(hostile_hook, "wb") : NULL;
    ...
    FILE *hostile_config_file = hostile_hook_ready ? pmm_fopen(hostile_config, "wb") : NULL;
    ...
    bool hostile_environment_ready = hostile_config_ready && ambient_git_saved &&
                                     pmm_setenv("GIT_CONFIG_GLOBAL", hostile_config, 1) == 0;
    const char *const init_args[] = {"init", "-q", template_argument, NULL};
    const char *const commit_args[] = {
        "-c",      "user.name=pmm-test",
        "-c",      "user.email=pmm-test@example.invalid",
    ...
    const char *saved_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache_copy = saved_cache ? strdup(saved_cache) : NULL;
    bool environment_ready = cache_created && pmm_setenv("PMM_CACHE_DIR", cache, 1) == 0;

    const char *project = "detect-contained-project";
    pmm_mcp_server_t *srv = environment_ready && repo_ready ? pmm_mcp_server_new(NULL) : NULL;
    bool server_ready = srv != NULL;
    pmm_store_t *store = srv ? pmm_mcp_server_store(srv) : NULL;
    bool project_ready = store && pmm_store_upsert_project(store, project, repo) == PMM_STORE_OK;
    mcp_command_hook_probe_t command_probe = {.reject_merge_base = true};
    if (project_ready) {
        pmm_mcp_server_set_project(srv, project);
        pmm_mcp_server_set_command_test_hook(srv, mcp_command_hook_probe, &command_probe);
    }

    char *invalid_response =
        project_ready ? pmm_mcp_handle_tool(srv, "detect_changes",
...
TEST(tool_ingest_traces_empty) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":38,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"ingest_traces\","
                                   "\"arguments\":{\"traces\":[]}}}");
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "accepted"));
...
TEST(store_idle_eviction) {
    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    pmm_mcp_server_set_project(srv, "test-evict");

    /* Trigger resolve_store via a tool call to set store_last_used */
    char *resp = pmm_mcp_handle_tool(srv, "get_graph_schema", "{\"project\":\"test-evict\"}");
    free(resp);

    ASSERT_TRUE(pmm_mcp_server_has_cached_store(srv));

    /* Evict with 0s timeout → should evict immediately */
    pmm_mcp_server_evict_idle(srv, 0);
    ASSERT_FALSE(pmm_mcp_server_has_cached_store(srv));
...
TEST(store_idle_no_eviction_within_timeout) {
    ...
    pmm_mcp_server_evict_idle(srv, 99999);
    ASSERT_TRUE(pmm_mcp_server_has_cached_store(srv));
...
TEST(store_idle_evict_protects_initial_store) {
    ...
    pmm_mcp_server_evict_idle(NULL, 0);
...
TEST(store_idle_evict_access_resets_timer) {
    ...
    resp = pmm_mcp_handle_tool(srv, "get_graph_schema", "{\"project\":\"test-evict\"}");
    free(resp);
...
TEST(parse_file_uri_unix) {
    char path[256];
    ASSERT_TRUE(pmm_parse_file_uri("file:///home/user/project", path, sizeof(path)));
    ASSERT_STR_EQ(path, "/home/user/project");

    ASSERT_TRUE(pmm_parse_file_uri("file:///tmp/test", path, sizeof(path)));
    ASSERT_STR_EQ(path, "/tmp/test");

    ASSERT_TRUE(pmm_parse_file_uri("file:///", path, sizeof(path)));
    ASSERT_STR_EQ(path, "/");
    PASS();
}
...
TEST(parse_file_uri_windows) {
    ...
    ASSERT_TRUE(pmm_parse_file_uri("file:///C:/Users/project", path, sizeof(path)));
    ASSERT_STR_EQ(path, "C:/Users/project");

    ASSERT_TRUE(pmm_parse_file_uri("file:///D:/Projects/myapp", path, sizeof(path)));
...
TEST(parse_file_uri_invalid) {
    char path[256];
    /* Non-file URI */
    ASSERT_FALSE(pmm_parse_file_uri("https://example.com", path, sizeof(path)));
    ASSERT_STR_EQ(path, "");

    /* Empty string */
    ASSERT_FALSE(pmm_parse_file_uri("", path, sizeof(path)));
    ASSERT_STR_EQ(path, "");

    /* NULL */
    ASSERT_FALSE(pmm_parse_file_uri(NULL, path, sizeof(path)));
    PASS();
}
 ...
 * Caller must free the server with pmm_mcp_server_free and
 * unlink the source file + rmdir manually. */
static pmm_mcp_server_t *setup_snippet_server(char *tmp_dir, size_t tmp_sz) {
    /* Create temp dir */
    snprintf(tmp_dir, tmp_sz, "/tmp/pmm_snippet_test_XXXXXX");
    if (!pmm_mkdtemp(tmp_dir))
        return NULL;
    ...
    pmm_mcp_server_set_project(srv, proj_name);
    pmm_store_upsert_project(st, proj_name, proj_dir);
    ...
    n_hr.properties_json = "{\"signature\":\"func HandleRequest() error\","
                           "\"return_type\":\"error\","
                           "\"is_exported\":true}";
    int64_t id_hr = pmm_store_upsert_node(st, &n_hr);
    ...
    int64_t id_po = pmm_store_upsert_node(st, &n_po);
    ...
    n_run1.end_line = 13;
    int64_t id_run1 = pmm_store_upsert_node(st, &n_run1);
    ...
    n_run2.end_line = 13;
    pmm_store_upsert_node(st, &n_run2);

    /* Create edges: HandleRequest -> ProcessOrder, HandleRequest -> Run1 */
    pmm_edge_t e1 = {.project = proj_name, .source_id = id_hr, .target_id = id_po, .type = "CALLS"};
    pmm_store_insert_edge(st, &e1);

    pmm_edge_t e2 = {
        .project = proj_name, .source_id = id_hr, .target_id = id_run1, .type = "CALLS"};
    pmm_store_insert_edge(st, &e2);
...
static char *call_snippet(pmm_mcp_server_t *srv, const char *args_json) {
    char *raw = pmm_mcp_handle_tool(srv, "get_code_snippet", args_json);
...
TEST(snippet_source_invalid_utf8) {
    ...
    char *raw =
        pmm_mcp_handle_tool(srv, "get_code_snippet",
...
TEST(jsonrpc_parse_array_not_object) {
    ...
    pmm_jsonrpc_request_t req = {0};
    int rc = pmm_jsonrpc_parse("[1,2,3]", &req);
    ASSERT_EQ(rc, -1);
    pmm_jsonrpc_request_free(&req);
    PASS();
}
...
TEST(mcp_get_string_arg_empty_json) {
    ...
    char *val = pmm_mcp_get_string_arg("", "key");
    ASSERT_NULL(val);
    PASS();
}
...
TEST(mcp_get_string_arg_nested_value) {
    ...
    char *val = pmm_mcp_get_string_arg(args, "config");
    ASSERT_NULL(val); /* not a string type */
    val = pmm_mcp_get_string_arg(args, "name");
    ASSERT_NOT_NULL(val);
...
TEST(mcp_get_string_arg_int_value) {
    ...
    char *val = pmm_mcp_get_string_arg("{\"count\":42}", "count");
    ASSERT_NULL(val);
    PASS();
}

TEST(mcp_get_int_arg_empty_json) {
    int val = pmm_mcp_get_int_arg("", "key", 99);
    ASSERT_EQ(val, 99);
    PASS();
}
...
TEST(mcp_get_int_arg_string_value) {
    ...
    int val = pmm_mcp_get_int_arg("{\"limit\":\"ten\"}", "limit", 5);
    ASSERT_EQ(val, 5);
    PASS();
}
...
TEST(mcp_get_int_arg_bool_value) {
    ...
    int val = pmm_mcp_get_int_arg("{\"flag\":true}", "flag", -1);
    ASSERT_EQ(val, -1);
    PASS();
}

TEST(mcp_get_bool_arg_empty_json) {
    bool val = pmm_mcp_get_bool_arg("", "key");
    ASSERT_FALSE(val);
    PASS();
}
...
TEST(mcp_get_bool_arg_int_value) {
    ...
    bool val = pmm_mcp_get_bool_arg("{\"flag\":1}", "flag");
    ASSERT_FALSE(val);
    PASS();
}

TEST(mcp_get_tool_name_empty_json) {
    char *name = pmm_mcp_get_tool_name("");
    ASSERT_NULL(name);
    PASS();
}

TEST(mcp_get_tool_name_missing_name) {
    char *name = pmm_mcp_get_tool_name("{\"arguments\":{}}");
    ASSERT_NULL(name);
    PASS();
}

TEST(mcp_get_arguments_empty_json) {
    char *args = pmm_mcp_get_arguments("");
    ASSERT_NULL(args);
    PASS();
}
...
TEST(mcp_get_arguments_no_arguments_key) {
    ...
    char *args = pmm_mcp_get_arguments("{\"name\":\"tool\"}");
    ASSERT_NOT_NULL(args);
    ASSERT_STR_EQ(args, "{}");
    free(args);
    PASS();
...
TEST(parse_file_uri_http_scheme) {
    char path[256];
    ASSERT_FALSE(pmm_parse_file_uri("http://example.com/path", path, sizeof(path)));
    ASSERT_STR_EQ(path, "");
    PASS();
}
...
TEST(parse_file_uri_ftp_scheme) {
    char path[256];
    ASSERT_FALSE(pmm_parse_file_uri("ftp://server/file.txt", path, sizeof(path)));
    ASSERT_STR_EQ(path, "");
    PASS();
}
...
TEST(parse_file_uri_buffer_too_small) {
    ...
    ASSERT_TRUE(pmm_parse_file_uri("file:///usr/local/bin", path, sizeof(path)));
...
TEST(parse_file_uri_spaces_in_path) {
    char path[256];
    ASSERT_TRUE(pmm_parse_file_uri("file:///home/user/my%20project", path, sizeof(path)));
...
TEST(parse_file_uri_null_out_path) {
    ...
    ASSERT_FALSE(pmm_parse_file_uri("file:///tmp", NULL, 256));
    PASS();
}
...
TEST(parse_file_uri_zero_size) {
    ...
    ASSERT_FALSE(pmm_parse_file_uri("file:///tmp", path, 0));
    PASS();
}
...
TEST(server_handle_invalid_json) {
    ...
    char *resp = pmm_mcp_server_handle(srv, "this is not json at all");
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "\"error\""));
...
TEST(server_handle_empty_object) {
    ...
    char *resp = pmm_mcp_server_handle(srv, "{}");
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "\"error\""));
    free(resp);
...
TEST(server_handle_tools_call_missing_name) {
    ...
    char *resp =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":50,\"method\":\"tools/call\","
                                   "\"params\":{\"arguments\":{}}}");
    ASSERT_NOT_NULL(resp);
...
TEST(mcp_server_run_rapid_messages) {
    ...
    /* Install alarm to fail the test if pmm_mcp_server_run blocks */
    signal(SIGALRM, alarm_handler);
    alarm(5);

    int rc = pmm_mcp_server_run(srv, in_fp, out_fp);
 ...
 * error list — collect_db_project_names overflowed projects[PMM_SZ_4K] via
...
TEST(tool_bad_project_name_no_overflow_issue235) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-badproj-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        PASS(); /* skip if mkdtemp fails */
    }
    ...
    for (int i = 0; i < ISSUE235_N; i++) {
        ...
        snprintf(iname, sizeof(iname),
                 ...
                 i);
        pmm_store_t *st = pmm_store_open_path(name);
        if (st) {
            pmm_store_upsert_project(st, iname, cache);
            pmm_store_close(st);
        }
    }
 ...
 * collect_db_project_names overflowed projects[PMM_SZ_4K] and truncated the
...
TEST(tool_bad_project_error_valid_json_issue235) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-badproj-vjson-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        PASS(); /* skip if mkdtemp fails */
    }
 ...
 *
 * Reproduce-first fixture in an isolated PMM_CACHE_DIR:
...
static bool issue704_make_db(const char *dir, const char *filename, const char *internal,
    ...
    pmm_store_t *st = pmm_store_open_path(path);
    if (!st) {
        return false;
    }
    bool ok = (pmm_store_upsert_project(st, internal, dir) == PMM_STORE_OK);
    if (ok) {
        char qn[256];
        ...
        n.start_line = 1;
        n.end_line = 2;
        ok = (pmm_store_upsert_node(st, &n) > 0);
    }
    pmm_store_close(st);
    return ok;
}
...
TEST(tool_resolve_store_by_internal_name_issue704) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-issue704-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
    ...
    char *list =
        pmm_mcp_server_handle(srv, "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/call\","
                                   "\"params\":{\"name\":\"list_projects\",\"arguments\":{}}}");
    ...
    char *q_beta = pmm_mcp_server_handle(
    ...
    char *q_alpha = pmm_mcp_server_handle(
    ...
    /* ── D: the 0-byte ghost is NOT resolvable ─────────────────────── */
    char *q_ghost = pmm_mcp_server_handle(
    ...
    char *q_gamma = pmm_mcp_server_handle(
    ...
    pmm_unlink(a_path);
    pmm_unlink(gamma_path);
    pmm_unlink(ghost_path);
...
TEST(tool_list_projects_ignores_missed_shadow_issue1044) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-issue1044-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
    ...
    pmm_store_t *st = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(st);
    ASSERT_EQ(pmm_store_upsert_project(st, "delta1044::missed", ""), PMM_STORE_OK);
    pmm_store_close(st);
 ...
 *  pmm_store_open_path_query(), which opens the DB SQLITE_OPEN_READWRITE
...
TEST(query_store_reopens_after_database_replacement) {
    static const char project[] = "pmm-store-generation-refresh";
    static const char active_filename[] = "pmm-store-generation-refresh.db";
    static const char staged_filename[] = "pmm-store-generation-next.db";

    char cache[512];
    snprintf(cache, sizeof(cache), "%s/pmm-store-generation-XXXXXX", pmm_tmpdir());
    bool cache_ready = pmm_mkdtemp(cache) != NULL;
    const char *saved = getenv("PMM_CACHE_DIR");
    char *saved_copy = saved ? strdup(saved) : NULL;
    if (cache_ready) {
        pmm_setenv("PMM_CACHE_DIR", cache, 1);
    ...
    pmm_mcp_server_t *srv = generation_a_ready ? pmm_mcp_server_new(NULL) : NULL;
    bool server_ready = srv != NULL;

    char args[512];
    snprintf(args, sizeof(args),
             "{\"project\":\"%s\",\"name_pattern\":\".*Generation.*\",\"limit\":10}", project);
    char *before = srv ? pmm_mcp_handle_tool(srv, "search_graph", args) : NULL;
    ...
    bool replaced = generation_b_ready && pmm_rename_replace(staged_path, active_path) == 0;

    char *after = (srv && replaced) ? pmm_mcp_handle_tool(srv, "search_graph", args) : NULL;
    ...
    if (cache_ready) {
        cleanup_project_db(cache, project);
        cleanup_project_db(cache, "pmm-store-generation-next");
        pmm_rmdir(cache);
    }
    restore_cache_dir(saved_copy);
    free(saved_copy);
...
#define ROQ_PROJECT "pmm-roq-test"
...
TEST(readonly_query_does_not_mutate_db) {
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "%s/pmm_roq_a_XXXXXX", pmm_tmpdir());
    if (!pmm_mkdtemp(tmp_cache)) {
        ASSERT_NOT_NULL(NULL); /* setup failure */
    }
    const char *saved = getenv("PMM_CACHE_DIR");
    char *saved_copy = saved ? strdup(saved) : NULL;
    pmm_setenv("PMM_CACHE_DIR", tmp_cache, 1);
    ...
    pmm_store_t *setup = pmm_store_open_path(db_path);
    ASSERT_NOT_NULL(setup);
    ASSERT_EQ(pmm_store_upsert_project(setup, ROQ_PROJECT, "/tmp/roq"), PMM_STORE_OK);
    pmm_node_t node = {.project = ROQ_PROJECT,
                       .label = "Function",
                       .name = "ReadOnlyProbe",
                       .qualified_name = "roq.mod.ReadOnlyProbe",
                       .file_path = "mod.c"};
    ASSERT_TRUE(pmm_store_upsert_node(setup, &node) > 0);
    ASSERT_EQ(pmm_store_exec(setup, "PRAGMA journal_mode=DELETE;"), 0);
    pmm_store_close(setup);
    ...
    snprintf(args, sizeof(args), "{\"project\":\"%s\",\"name_pattern\":\".*ReadOnlyProbe.*\"}",
             ROQ_PROJECT);
    char *resp = pmm_mcp_handle_tool(srv, "search_graph", args);
    ...
    pmm_mcp_server_free(srv); /* closes the store; header change is persisted */
 ...
 *
 * WHY RED on unfixed code:
 *   pmm_store_open_path_query() runs configure_pragmas(.., false) which
...
TEST(readonly_query_succeeds_on_readonly_fs) {
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "%s/pmm_roq_b_XXXXXX", pmm_tmpdir());
    if (!pmm_mkdtemp(tmp_cache)) {
        ASSERT_NOT_NULL(NULL); /* setup failure */
    }
    const char *saved = getenv("PMM_CACHE_DIR");
    ...
    pmm_node_t node = {.project = ROQ_PROJECT,
                       ...
                       .file_path = "mod.c"};
    ASSERT_TRUE(pmm_store_upsert_node(setup, &node) > 0);
    (void)pmm_store_checkpoint(setup); /* fold WAL frames into the main file */
    pmm_store_close(setup);            /* clean close removes -wal/-shm */
...
static int idx823_supervised_name_override_check(const char *repo_dir, const char *custom_name) {
    ...
    pmm_unsetenv("PMM_INDEX_SUPERVISOR");
    pmm_setenv("PMM_INDEX_MAX_RESTARTS", "1", 1);
    pmm_setenv("PMM_INDEX_WORKER_TIMEOUT_S", "30", 1);
    ...
    if (code == IDX823_OK) {
        char *projects = pmm_mcp_handle_tool(srv, "list_projects", "{}");
        char expected[256];
    ...
    if (code == IDX823_OK) {
        ...
        snprintf(q, sizeof(q),
                 ...
                 custom_name);
        char *sr = pmm_mcp_handle_tool(srv, "search_graph", q);
...
TEST(index_repository_cli_name_override_issue823) {
...
#else
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-idx823-repo-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        FAIL("pmm_mkdtemp repo failed");
    }
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-idx823-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        th_rmtree(tmp_dir);
        FAIL("pmm_mkdtemp cache failed");
    }
    ...
    char *path_project = pmm_project_name_from_path(tmp_dir);
...
/* ══════════════════════════════════════════════════════════════════
 *  #845 — supervisor gate must not wrap embedders of pmm_mcp_handle_tool
 * ══════════════════════════════════════════════════════════════════ */

TEST(index_supervisor_unsafe_clean_is_never_fallback_or_recovery) {
    char response[] = "{\"status\":\"indexed\"}";
    pmm_index_worker_result_t result = {
        .outcome = PMM_PROC_CLEAN,
        .exit_code = 0,
        .tree_quiesced = true,
        .response = response,
    };
    ASSERT_EQ(pmm_mcp_supervised_result_disposition(0, &result), PMM_MCP_SUPERVISED_RESULT_SUCCESS);

    result.cancellation_requested = true;
    ASSERT_EQ(pmm_mcp_supervised_result_disposition(0, &result),
              PMM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL);
    ...
    result.outcome = PMM_PROC_CRASH;
    result.response = NULL;
    ASSERT_EQ(pmm_mcp_supervised_result_disposition(0, &result),
              PMM_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE);
    ASSERT_EQ(pmm_mcp_supervised_result_disposition(-1, &result),
...
static int idx845_index_inprocess_check(const char *repo_dir) {
    int spawns_before = pmm_index_supervisor_spawn_count();
    ...
    int code = IDX845_OK;
    if (pmm_index_supervisor_spawn_count() != spawns_before) {
        code = IDX845_SPAWNED;
    } else if (!resp) {
...
TEST(index_supervisor_gate_requires_marked_host_issue845) {
    /* #845: index_repository via pmm_mcp_handle_tool from an EMBEDDER (this test
     * binary) must index IN-PROCESS even with PMM_INDEX_SUPERVISOR unset. The
     * supervisor gate may only wrap a process that called
     * pmm_index_supervisor_mark_host() — i.e. the real binary's main(). Before
    ...
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-idx845-repo-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        PASS();
    }
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-idx845-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        pmm_rmdir(tmp_dir);
        PASS();
    }
    ...
    const char *saved_sv = getenv("PMM_INDEX_SUPERVISOR");
    ...
    if (saved_sv_copy) {
        pmm_setenv("PMM_INDEX_SUPERVISOR", saved_sv_copy, 1);
        free(saved_sv_copy);
    } else {
        pmm_unsetenv("PMM_INDEX_SUPERVISOR");
    }
...
/* ══════════════════════════════════════════════════════════════════
 *  Mandatory supervision must fail closed in real PMM hosts
 * ══════════════════════════════════════════════════════════════════ */

/* A real PMM host must never turn a supervisor refusal into permission to run
 * the native index pipeline in its own long-lived process. The legacy
 * PMM_INDEX_SUPERVISOR=0 switch is a deterministic start-failure seam here: on
...
int mcp_test_idxfailclosed_supervisor_start_check(const char *repo_dir, const char *cache_dir) {
    (void)pmm_setenv("PMM_CACHE_DIR", cache_dir, 1);
    pmm_index_supervisor_mark_host();
    (void)pmm_setenv("PMM_INDEX_SUPERVISOR", "0", 1);
...
static bool idxfailclosed_self_path(char out[PMM_SZ_4K]) {
#ifdef __APPLE__
    int length = proc_pidpath(getpid(), out, PMM_SZ_4K);
    bool resolved = length > 0 && length < PMM_SZ_4K;
    if (resolved) {
        out[length] = '\0';
    }
    return resolved;
    ...
    bool resolved = length > 0 && length < (ssize_t)PMM_SZ_4K - 1;
    if (resolved) {
        out[length] = '\0';
    }
    return resolved;
...
TEST(index_supervisor_start_failure_is_fail_closed_in_real_host) {
...
#else
    char repo_dir[PMM_SZ_1K];
    char cache_dir[PMM_SZ_1K];
    (void)snprintf(repo_dir, sizeof(repo_dir), "%s/pmm-idx-failclosed-repo-XXXXXX", pmm_tmpdir());
    (void)snprintf(cache_dir, sizeof(cache_dir), "%s/pmm-idx-failclosed-cache-XXXXXX",
                   pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(repo_dir));
    ASSERT_NOT_NULL(pmm_mkdtemp(cache_dir));

    char source_path[PMM_SZ_4K];
    (void)snprintf(source_path, sizeof(source_path), "%s/should_not_index.py", repo_dir);
    FILE *source = pmm_fopen(source_path, "wb");
    ASSERT_NOT_NULL(source);
    ASSERT_TRUE(fputs("def should_not_index():\n    return True\n", source) >= 0);
    ASSERT_EQ(fclose(source), 0);

    char *project = pmm_project_name_from_path(repo_dir);
    ASSERT_NOT_NULL(project);
    char db_path[PMM_SZ_4K];
    (void)snprintf(db_path, sizeof(db_path), "%s/%s.db", cache_dir, project);

    char self_path[PMM_SZ_4K] = {0};
    ASSERT_TRUE(idxfailclosed_self_path(self_path));
    char *const child_argv[] = {
        self_path, "__pmm_mcp_idxfailclosed_probe", repo_dir, cache_dir, NULL,
    };
    (void)fflush(NULL);
    pid_t child = -1;
    ...
    bool database_absent = !pmm_file_exists(db_path);

    cleanup_project_db(cache_dir, project);
    free(project);
    (void)pmm_unlink(source_path);
 ...
 * paths through pmm_mcp_index_run_supervised_path() — the SAME supervised worker
 ...
 * child (pmm_index_supervisor_spawn_count() increases) and (b) actually index the
 * fixture (the worker child writes the Function node). RED on the unfixed
 * in-process routing: it calls pmm_pipeline_run directly, so spawn_count is
...
static int idx832_supervised_route_check(const char *repo_dir) {
    ...
    int spawns_before = pmm_index_supervisor_spawn_count();
    char *resp = pmm_mcp_index_run_supervised_path(repo_dir);
    int spawns_after = pmm_index_supervisor_spawn_count();
...
TEST(index_bg_paths_route_through_supervisor_issue832) {
...
#else
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-idx832-repo-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        PASS();
    }
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-idx832-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        pmm_rmdir(tmp_dir);
        PASS();
    }
    ...
    const char *saved_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache_copy = saved_cache ? strdup(saved_cache) : NULL;
    pmm_setenv("PMM_CACHE_DIR", cache, 1); /* inherited by the worker child */
 ...
 * (pmm_index_supervisor_spawn_st_count stays 0 — RED on the old loop),
...
static int idxpar_recovery_check(const char *repo_dir) {
    ...
    pmm_setenv("PMM_INDEX_MAX_RESTARTS", "5", 1);
    pmm_setenv("PMM_INDEX_WORKER_TIMEOUT_S", "30", 1);
    pmm_setenv("PMM_TEST_CRASH_ON", "idxpar_crasher", 1);

    int st_before = pmm_index_supervisor_spawn_st_count();
    char *resp = pmm_mcp_index_run_supervised_path(repo_dir);
    int st_after = pmm_index_supervisor_spawn_st_count();
    pmm_unsetenv("PMM_TEST_CRASH_ON");
 ...
 * pmm_destroy_thread_parser) on the SECOND index_repository in one server
 ...
 * index); the in-process pipeline — PMM_INDEX_SUPERVISOR=0, and every
...
static int idx773_double_index_check(const char *dir_a, const char *dir_b) {
    ...
    char args[PMM_SZ_512];
    snprintf(args, sizeof(args), "{\"repo_path\":\"%s\",\"mode\":\"full\"}", dir_a);
    char *r1 = pmm_mcp_handle_tool(srv, "index_repository", args);
    ...
    char *r2 = pmm_mcp_handle_tool(srv, "index_repository", args); /* SIGABRT here (RED) */
...
TEST(sequential_service_edge_props_are_valid_json_issue898) {
    char tmp[PMM_SZ_256];
    snprintf(tmp, sizeof(tmp), "/tmp/pmm_seq898_XXXXXX");
    if (!pmm_mkdtemp(tmp)) {
        FAIL("mkdtemp failed");
    }
    char cache[PMM_SZ_256];
    snprintf(cache, sizeof(cache), "/tmp/pmm_seq898_cache_XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        pmm_rmdir(tmp);
    ...
    char *project = pmm_project_name_from_path(tmp);
    ASSERT_NOT_NULL(project);
    char db_path[PMM_SZ_512];
    snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project);
    pmm_store_t *store = pmm_store_open_path_query(db_path);
    ASSERT_NOT_NULL(store);
    struct sqlite3 *db = pmm_store_get_db(store);
    ASSERT_NOT_NULL(db);
...
#else
    char dir_a[PMM_SZ_256];
    char dir_b[PMM_SZ_256];
    char cache[PMM_SZ_256];
    snprintf(dir_a, sizeof(dir_a), "/tmp/pmm-idx773a-XXXXXX");
    snprintf(dir_b, sizeof(dir_b), "/tmp/pmm-idx773b-XXXXXX");
    snprintf(cache, sizeof(cache), "/tmp/pmm-idx773c-XXXXXX");
    if (!pmm_mkdtemp(dir_a) || !pmm_mkdtemp(dir_b) || !pmm_mkdtemp(cache)) {
        FAIL("mkdtemp failed");
    }
    const char *saved_cache = getenv("PMM_CACHE_DIR");
    char *saved_cache_copy = saved_cache ? pmm_strdup(saved_cache) : NULL;
...
#else
    char tmp_dir[PMM_SZ_256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-idxpar-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        FAIL("mkdtemp failed");
    }
    char cache[PMM_SZ_256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-idxpar-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
...
static int auto_watch_connect_watch_count(const char *auto_watch_value) {
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-autowatch-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        return -1;
    }
    ...
    char *project = pmm_project_name_from_path(repodir);
    if (!project) {
        th_rmtree(cache);
        return -3;
    }
    ...
    char old_cwd[1024];
    if (!pmm_getcwd(old_cwd, sizeof(old_cwd)) || pmm_chdir(repodir) != 0) {
        restore_cache_dir(saved_copy);
        free(saved_copy);
        th_rmtree(cache);
        return -5;
    ...
    int count = -6;
    pmm_config_t *cfg = pmm_config_open(cache);
    pmm_store_t *wstore = pmm_store_open_memory();
    pmm_watcher_t *watcher = wstore ? pmm_watcher_new(wstore, NULL, NULL) : NULL;
    if (cfg && watcher) {
        if (auto_watch_value) {
            pmm_config_set(cfg, PMM_CONFIG_AUTO_WATCH, auto_watch_value);
        }

        pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
        if (srv) {
            pmm_mcp_server_set_watcher(srv, watcher);
            pmm_mcp_server_set_config(srv, cfg);
            char *resp = pmm_mcp_server_handle(
                srv, "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{}}");
            free(resp);
            count = pmm_watcher_watch_count(watcher);
            pmm_mcp_server_free(srv);
        }
    }
    ...
    if (wstore) {
        pmm_store_close(wstore);
    }
    if (cfg) {
        pmm_config_close(cfg);
    }

    (void)pmm_chdir(old_cwd);
    restore_cache_dir(saved_copy);
    free(saved_copy);
    th_rmtree(cache);
    return count;
 ...
 * pmm_watcher_watch() guarded only by `if (srv->watcher)` — srv->watcher is set
 ...
 * and auto_watch=false. pmm_mcp_server_free() joins the autoindex thread, so the
 ...
 * pmm_watcher_watch() unconditionally → watch_count == 1 → IDX853_WATCHER_REGISTERED.
...
enum {
    ...
    IDX853_WATCHER_REGISTERED = 61, /* watch_count==1 → RED: ungated pmm_watcher_watch */
...
static int idx853_supervised_autowatch_check(const char *repo_dir, const char *cache_dir) {
    ...
    pmm_config_set(cfg, PMM_CONFIG_AUTO_INDEX, "true");
    pmm_config_set(cfg, PMM_CONFIG_AUTO_WATCH, "false");

    /* detect_session derives session_root/session_project from the cwd. */
    char old_cwd[1024];
    if (!pmm_getcwd(old_cwd, sizeof(old_cwd)) || pmm_chdir(repo_dir) != 0) {
        pmm_watcher_free(watcher);
        pmm_store_close(wstore);
        pmm_config_close(cfg);
        return IDX853_SETUP_FAIL;
    ...
    if (srv) {
        ...
        int spawns_after = pmm_index_supervisor_spawn_count();
        int watch_count = pmm_watcher_watch_count(watcher);
...
#else
    char tmp_dir[256];
    snprintf(tmp_dir, sizeof(tmp_dir), "/tmp/pmm-idx853-repo-XXXXXX");
    if (!pmm_mkdtemp(tmp_dir)) {
        PASS();
    }
    char cache[256];
    snprintf(cache, sizeof(cache), "/tmp/pmm-idx853-cache-XXXXXX");
    if (!pmm_mkdtemp(cache)) {
        pmm_rmdir(tmp_dir);
        PASS();
    }
...
extern bool pmm_path_within_root(const char *root_path, const char *abs_path);

TEST(mcp_path_within_root_rejects_escape) {
#ifdef _WIN32
    char root[512];
    char outside[512];
    snprintf(root, sizeof(root), "%s/pmm_pwr_root_XXXXXX", pmm_tmpdir());
    snprintf(outside, sizeof(outside), "%s/pmm_pwr_outside_XXXXXX", pmm_tmpdir());
    ASSERT_NOT_NULL(pmm_mkdtemp(root));
    ...
    FILE *fp = pmm_fopen(inside, "w");
    ASSERT_NOT_NULL(fp);
    fputs("int inside;\n", fp);
    fclose(fp);
    fp = pmm_fopen(target, "w");
    ASSERT_NOT_NULL(fp);
    fputs("int outside;\n", fp);
    fclose(fp);

    /* pmm_tmpdir() can expose the MSYS spelling C:/msys64/...; cmd's mklink
    ...
    bool linked = pmm_exec_no_shell(junction_argv) == 0;

    ASSERT_TRUE(linked);
    ASSERT_TRUE(pmm_path_within_root(root, inside));
    ASSERT_FALSE(pmm_path_within_root(root, target));
    ASSERT_FALSE(pmm_path_within_root(root, linked_target));
    ...
    ASSERT_TRUE(pmm_path_within_root(case_alias, inside));

    char drive_root[] = {root[0], ':', '\\', '\0'};
    ASSERT_TRUE(((root[0] >= 'A' && root[0] <= 'Z') || (root[0] >= 'a' && root[0] <= 'z')) &&
                root[1] == ':');
    ASSERT_TRUE(pmm_path_within_root(drive_root, inside));
...
#else
    char root[512];
    snprintf(root, sizeof(root), "%s/pmm_pwr_XXXXXX", pmm_tmpdir());
    if (!pmm_mkdtemp(root)) {
        FAIL("pmm_mkdtemp failed");
    }
    char inside[700];
    ...
    ASSERT_TRUE(pmm_path_within_root(root, inside));
    ASSERT_FALSE(pmm_path_within_root(root, escape));
    ASSERT_FALSE(pmm_path_within_root(root, "/etc/hosts"));
    ASSERT_TRUE(pmm_path_within_root("/", "/etc/hosts"));
...
TEST(detect_changes_rejects_option_like_base_branch) {
    ...
    char *resp = pmm_mcp_server_handle(
        srv, "{\"jsonrpc\":\"2.0\",\"id\":77,\"method\":\"tools/call\","
             "\"params\":{\"name\":\"detect_changes\","
             "\"arguments\":{\"project\":\"p\",\"base_branch\":\"--output=/tmp/pmm_pwn\"}}}");
    ASSERT_NOT_NULL(resp);
...
TEST(detect_changes_rejects_windows_cmd_metacharacters_in_base_branch) {
    ...
    for (size_t i = 0; i < sizeof(branches) / sizeof(branches[0]); i++) {
        ...
        snprintf(request, sizeof(request),
                 ...
                 branches[i]);
        char *response = pmm_mcp_server_handle(srv, request);
        ASSERT_NOT_NULL(response);
...
TEST(detect_changes_rejects_windows_cmd_metacharacters_in_project_root) {
#ifdef _WIN32
    const char *const roots[] = {"C:\\pmm-root-%PATH%", "C:\\pmm-root-!name!",
                                 "C:\\pmm-root-^name"};
    ...
    for (size_t i = 0; i < sizeof(roots) / sizeof(roots[0]); i++) {
        ASSERT_EQ(pmm_store_upsert_project(store, project, roots[i]), PMM_STORE_OK);
        char *response = pmm_mcp_handle_tool(
            srv, "detect_changes",
...
/* Opt-in workspace boundary: when PMM_ALLOWED_ROOT is set, index_repository
 * must refuse a repo_path that resolves outside it. Unset (the default) imposes
 * no restriction. */
TEST(index_repository_honors_allowed_root) {
    char allowed[512];
    snprintf(allowed, sizeof(allowed), "%s/pmm_allowed_XXXXXX", pmm_tmpdir());
    if (!pmm_mkdtemp(allowed)) {
        FAIL("pmm_mkdtemp failed");
    }
    pmm_setenv("PMM_ALLOWED_ROOT", allowed, 1);
    ...
    char *resp = pmm_mcp_server_handle(srv, args);
    ASSERT_NOT_NULL(resp);
    ASSERT_NOT_NULL(strstr(resp, "outside the allowed root"));
    free(resp);

    pmm_unsetenv("PMM_ALLOWED_ROOT");
    pmm_mcp_server_free(srv);
    pmm_rmdir(allowed);
    PASS();
}
...
TEST(index_repository_relative_path_uses_explicit_session_root) {
    ...
    char cache[512];
    snprintf(session_root, sizeof(session_root), "%s/pmm_daemon_session_XXXXXX", pmm_tmpdir());
    snprintf(cache, sizeof(cache), "%s/pmm_daemon_cache_XXXXXX", pmm_tmpdir());
    if (!pmm_mkdtemp(session_root) || !pmm_mkdtemp(cache)) {
    ...
    if (saved_supervisor_copy) {
        pmm_setenv("PMM_INDEX_SUPERVISOR", saved_supervisor_copy, 1);
    } else {
        pmm_unsetenv("PMM_INDEX_SUPERVISOR");
    }
...
static int idxcanon_supervised_session_path_check(const char *session_root, const char *decoy_cwd) {
    char saved_cwd[PMM_SZ_4K];
    if (!pmm_getcwd(saved_cwd, sizeof(saved_cwd))) {
        return IDXCANON_GETCWD_FAILED;
    }
    if (pmm_chdir(decoy_cwd) != 0) {
    ...
    char session_repo[PMM_SZ_4K];
    char decoy_repo[PMM_SZ_4K];
    snprintf(session_repo, sizeof(session_repo), "%s/repo", session_root);
    snprintf(decoy_repo, sizeof(decoy_repo), "%s/repo", decoy_cwd);
    char *session_project = pmm_project_name_from_path(session_repo);
    char *decoy_project = pmm_project_name_from_path(decoy_repo);

    pmm_mcp_server_t *srv = pmm_mcp_server_new(NULL);
    int code = IDXCANON_OK;
    if (!srv) {
        code = IDXCANON_NO_SERVER;
    } else if (!pmm_mcp_server_set_session_context(srv, session_root, session_root)) {
        code = IDXCANON_CONTEXT_FAILED;
    }

    int spawns_before = pmm_index_supervisor_spawn_count();
    char *resp = code == IDXCANON_OK
                     ? pmm_mcp_handle_tool(srv, "index_repository",
    ...
    free(decoy_project);
    if (pmm_chdir(saved_cwd) != 0 && code == IDXCANON_OK) {
...
TEST(index_repository_supervisor_uses_canonical_session_path) {
    ...
    char decoy_cwd[512];
    char cache[512];
    snprintf(session_root, sizeof(session_root), "%s/pmm_canonical_session_XXXXXX", pmm_tmpdir());
    snprintf(decoy_cwd, sizeof(decoy_cwd), "%s/pmm_canonical_decoy_XXXXXX", pmm_tmpdir());
    snprintf(cache, sizeof(cache), "%s/pmm_canonical_cache_XXXXXX", pmm_tmpdir());
    if (!pmm_mkdtemp(session_root) || !pmm_mkdtemp(decoy_cwd) || !pmm_mkdtemp(cache)) {
        th_rmtree(session_root);
        th_rmtree(decoy_cwd);
        th_rmtree(cache);
        FAIL("pmm_mkdtemp failed");
    ...
    char session_source[PMM_SZ_4K];
    char decoy_source[PMM_SZ_4K];
