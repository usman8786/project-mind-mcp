...
#define PMM_JSON_LIKE_ENABLE_TEST_API 1
...
static int jl_fixture_open(jl_fixture_t *fixture) {
    snprintf(fixture->directory, sizeof(fixture->directory), "%s/pmm-json-like-XXXXXX",
             pmm_tmpdir());
    if (!pmm_mkdtemp(fixture->directory)) {
        return -1;
    }
    int written =
...
static void jl_fixture_close(jl_fixture_t *fixture) {
    pmm_dir_t *directory = pmm_opendir(fixture->directory);
    if (directory) {
        pmm_dirent_t *entry = NULL;
        while ((entry = pmm_readdir(directory)) != NULL) {
            if (strncmp(entry->name, "config.json.pmm.tmp.", strlen("config.json.pmm.tmp.")) == 0) {
                char temp_path[sizeof(fixture->path) + 96U];
                int written = snprintf(temp_path, sizeof(temp_path), "%s/%s", fixture->directory,
                                       entry->name);
                if (written >= 0 && (size_t)written < sizeof(temp_path)) {
                    (void)pmm_unlink(temp_path);
                }
            }
        }
        pmm_closedir(directory);
    }
    (void)pmm_unlink(fixture->path);
    (void)pmm_rmdir(fixture->directory);
}
...
static int jl_failed_unchanged(const char *path, const char *original,
    ...
    if (pmm_json_like_upsert_entry(path, object_path, path_len, key, value) == 0) {
        return 0;
    }
...
TEST(config_json_like_rejects_stale_content_and_cleans_temp) {
    ...
    pmm_json_like_set_precommit_hook_for_testing(jl_change_before_commit, &change);
    int result = pmm_json_like_upsert_entry(fixture.path, root, 0U, "owned", "true");
    pmm_json_like_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_json_like_missing_target_race_does_not_replace_winner) {
    ...
    pmm_json_like_set_prepublish_hook_for_testing(jl_change_before_commit, &race);
    int result = pmm_json_like_upsert_entry(fixture.path, root, 0U, "owned", "true");
    pmm_json_like_set_prepublish_hook_for_testing(NULL, NULL);
...
TEST(config_json_like_rejects_non_regular_path) {
    jl_fixture_t fixture;
    ASSERT_EQ(jl_fixture_open(&fixture), 0);
    ASSERT_EQ(pmm_mkdir(fixture.path), 0);
    const char *root[] = {NULL};
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, root, 0U, "owned", "true"), -1);
    ASSERT_EQ(jl_temp_file_count(&fixture), 0U);
    ASSERT_EQ(pmm_rmdir(fixture.path), 0);
    jl_fixture_close(&fixture);
    PASS();
}
...
TEST(config_json_like_preserves_owner_group_and_mode) {
    ...
    const char *root[] = {NULL};
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, root, 0U, "owned", "true"), 0);
    struct stat after;
...
TEST(config_json_like_supports_bom_and_common_json5_whitespace) {
    ...
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, path, 2U, "owned", "true"), 0);
    char *content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
...
TEST(config_json_like_fresh_strict_upsert_replace_remove) {
    ...
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, root, 0U, "owned", "{\"command\":\"pmm\"}"),
              0);
    char *content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ...
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, root, 0U, "owned", "[1,2]"), 0);
    content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ASSERT_STR_EQ(content, "{\"keep\":1,\"owned\":[1,2]}\n");
    free(content);

    ASSERT_EQ(pmm_json_like_remove_entry(fixture.path, root, 0U, "owned"), 0);
    content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
...
TEST(config_json_like_preserves_jsonc_comments) {
    ...
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, path, 1U, "project-mind",
                                         "{\"command\":\"pmm\"}"),
              0);
    char *content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ...
    ASSERT(strstr(content, "\"project-mind\": {\"command\":\"pmm\"}") != NULL);
    free(content);
    jl_fixture_close(&fixture);
    PASS();
}
...
TEST(config_json_like_openclaw_json5_nested_servers) {
    ...
    ASSERT_EQ(jl_write(fixture.path, original), 0);
    const char *path[] = {"mcp", "servers"};
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, path, 2U, "project-mind",
                                         "{\"command\":\"pmm\",\"args\":[]}"),
              0);
    char *content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ASSERT(strstr(content, "theme: 'dark'") != NULL);
    ASSERT(strstr(content, "other: { command: 'other' }") != NULL);
    ASSERT(strstr(content, "\"project-mind\"") != NULL);
    free(content);

    ASSERT_EQ(pmm_json_like_remove_entry(fixture.path, path, 2U, "project-mind"), 0);
    content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ASSERT(strstr(content, "project-mind") == NULL);
    ASSERT(strstr(content, "theme: 'dark'") != NULL);
...
TEST(config_json_like_rejects_invalid_and_malformed_byte_identical) {
    ...
    ASSERT_EQ(jl_write(fixture.path, "{\"keep\":1}\n"), 0);
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, root, 0U, "bad\nkey", "true"), -1);
    ASSERT_EQ(pmm_json_like_upsert_entry(NULL, root, 0U, "owned", "true"), -1);
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, NULL, 0U, "owned", "true"), -1);
    const char *deep_path[64];
    for (size_t i = 0; i < sizeof(deep_path) / sizeof(deep_path[0]); i++) {
        deep_path[i] = "level";
    }
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, deep_path, 64U, "owned", "true"), -1);
    char *content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
...
TEST(config_json_like_upsert_is_byte_idempotent) {
    ...
    ASSERT_EQ(jl_write(fixture.path, "{\n  \"mcp\": {}\n}\n"), 0);
    const char *path[] = {"mcp"};
    const char *value = "{\"command\":\"pmm\",\"args\":[\"serve\"]}";
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, path, 1U, "owned", value), 0);
...
TEST(config_json_like_ignores_braces_and_comments_inside_strings) {
    ...
    ASSERT_EQ(jl_write(fixture.path, original), 0);
    const char *path[] = {"mcp", "servers"};
    ASSERT_EQ(pmm_json_like_upsert_entry(fixture.path, path, 2U, "owned", "{\"command\":\"pmm\"}"),
              0);
    char *content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ASSERT(strstr(content, "} // text /* text */ \\\" {") != NULL);
    ASSERT(strstr(content, "\"owned\": {\"command\":\"pmm\"}") != NULL);
    free(content);
    jl_fixture_close(&fixture);
    PASS();
}
...
TEST(config_json_like_handles_trailing_commas) {
    ...
    ASSERT(strstr(content, "\"owned\": true,") != NULL);
    free(content);
    ASSERT_EQ(pmm_json_like_remove_entry(fixture.path, path, 2U, "owned"), 0);
    content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ASSERT(strstr(content, "owned") == NULL);
    ASSERT(strstr(content, "other: [1, 2,]") != NULL);
...
TEST(config_json_like_top_level_array_unique_string) {
    ...
    const char *original = "{\n"
                           ...
                           "  ],\n"
                           "}\n";
    const char *owned = "~/.config/kilo/rules/project-mind-mcp.md";
    ASSERT_EQ(jl_write(fixture.path, original), 0);
    ASSERT_EQ(pmm_json_like_add_unique_string(fixture.path, "instructions", owned), 0);
    char *first = jl_read(fixture.path);
    ASSERT_NOT_NULL(first);
    ...
    ASSERT_EQ(pmm_json_like_remove_string(fixture.path, "instructions", owned), 0);
...
TEST(config_json_like_top_level_array_create_escape_and_fail_closed) {
    ...
    ASSERT_EQ(pmm_json_like_add_unique_string(fixture.path, "instructions", "owned"), -1);
    content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ASSERT_STR_EQ(content, duplicate);
    free(content);
    ASSERT_EQ(pmm_json_like_remove_string(fixture.path, "instructions", "owned"), -1);
    content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ASSERT_STR_EQ(content, duplicate);
    free(content);
...
TEST(config_json_like_nested_array_creates_missing_path_and_escapes) {
    ...
    ASSERT_EQ(pmm_json_like_add_unique_string_at_path(fixture.path, path, 3U,
    ...
    ASSERT_EQ(pmm_json_like_remove_string_at_path(fixture.path, path, 3U, "postCompactionSections",
                                                  owned),
              0);
    content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
...
TEST(config_json_like_nested_array_preserves_jsonc_and_is_idempotent) {
    ...
    ASSERT_EQ(
        pmm_json_like_add_unique_string_at_path(fixture.path, path, 1U, "context_paths", owned), 0);
    char *first = jl_read(fixture.path);
    ASSERT_NOT_NULL(first);
    ...
    ASSERT_EQ(pmm_json_like_remove_string_at_path(fixture.path, path, 1U, "context_paths", owned),
              0);
    char *removed = jl_read(fixture.path);
    ASSERT_NOT_NULL(removed);
...
TEST(config_json_like_nested_array_fails_closed_on_ambiguous_or_invalid_paths) {
    ...
    ASSERT_EQ(jl_write(fixture.path, wrong_type), 0);
    ASSERT_EQ(
        pmm_json_like_add_unique_string_at_path(fixture.path, path, 1U, "context_paths", "owned"),
        -1);
    char *content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
    ASSERT_STR_EQ(content, wrong_type);
    ...
    ASSERT_EQ(
        pmm_json_like_add_unique_string_at_path(fixture.path, NULL, 1U, "context_paths", "owned"),
        -1);
    const char *null_path[] = {NULL};
    ASSERT_EQ(pmm_json_like_add_unique_string_at_path(fixture.path, null_path, 1U, "context_paths",
                                                      "owned"),
              -1);
    const char *control_path[] = {"bad\npath"};
    ASSERT_EQ(pmm_json_like_remove_string_at_path(fixture.path, control_path, 1U, "context_paths",
                                                  "owned"),
              -1);
    const char *deep_path[64];
    for (size_t i = 0; i < sizeof(deep_path) / sizeof(deep_path[0]); i++) {
    ...
    ASSERT_EQ(pmm_json_like_add_unique_string_at_path(fixture.path, deep_path, 64U, "context_paths",
                                                      "owned"),
              -1);
    content = jl_read(fixture.path);
    ASSERT_NOT_NULL(content);
...
TEST(config_json_like_nested_array_rejects_symlink_and_hardlink) {
    ...
    ASSERT_EQ(pmm_json_like_get_string_at_path(fixture.path, path, 1U, "context_paths", &workspace),
              -1);
...
TEST(config_json_like_nested_array_rejects_precommit_content_and_identity_races) {
    ...
    pmm_json_like_set_precommit_hook_for_testing(jl_change_before_commit, &content_change);
    int result =
        pmm_json_like_add_unique_string_at_path(fixture.path, path, 1U, "context_paths", "new");
    pmm_json_like_set_precommit_hook_for_testing(NULL, NULL);
    ...
    pmm_json_like_set_precommit_hook_for_testing(jl_change_before_commit, &identity_change);
    result = pmm_json_like_remove_string_at_path(fixture.path, path, 1U, "context_paths", "owned");
    pmm_json_like_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_json_like_nested_string_lookup_decodes_json5_workspace) {
    ...
    char *workspace = NULL;
    ASSERT_EQ(pmm_json_like_get_string_at_path(fixture.path, path, 2U, "workspace", &workspace), 0);
...
TEST(config_json_like_nested_string_lookup_distinguishes_missing_and_fails_closed) {
    ...
    const char *path[] = {"agents", "defaults"};
    char *workspace = (char *)(uintptr_t)1U;
    ASSERT_EQ(pmm_json_like_get_string_at_path(fixture.path, path, 2U, "workspace", &workspace), 1);
    ASSERT_NULL(workspace);
    ...
    workspace = (char *)(uintptr_t)1U;
    ASSERT_EQ(pmm_json_like_get_string_at_path(fixture.path, path, 2U, "workspace", &workspace),
              -1);
    ASSERT_NULL(workspace);
