...
static bool frontend_maintenance_fixture_start(frontend_maintenance_fixture_t *fixture,
    ...
    fixture->manager = fixture->endpoint ? pmm_version_cohort_manager_new(fixture->endpoint) : NULL;
...
static void frontend_maintenance_fixture_finish(frontend_maintenance_fixture_t *fixture) {
    while (fixture->manager &&
           pmm_version_cohort_manager_free(&fixture->manager) != PMM_PRIVATE_FILE_LOCK_OK) {
        pmm_usleep(1000);
    }
...
static pmm_daemon_build_identity_t frontend_test_identity(void) {
...
static pmm_daemon_runtime_application_session_t *frontend_eof_application_session_open(
...
static pmm_daemon_runtime_application_status_t frontend_eof_application_request(
...
static void *frontend_eof_writer(void *opaque) {
    ...
    uint64_t deadline = pmm_now_ms() + FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS;
    while (
        ok &&
...
static bool frontend_eof_fixture_start(frontend_eof_fixture_t *fixture, const char *parent) {
    ...
    if (log_written <= 0 || log_written >= (int)sizeof(fixture->conflict_log) ||
        !pmm_daemon_rendezvous_key(key) ||
        !pmm_daemon_runtime_process_build_fingerprint((uint64_t)getpid(), build)) {
        return false;
    }
    fixture->endpoint = pmm_daemon_ipc_endpoint_new(key, parent);
    fixture->manager = fixture->endpoint ? pmm_version_cohort_manager_new(fixture->endpoint) : NULL;
    pmm_daemon_build_identity_t identity = {
        .semantic_version = "2.4.0",
    ...
    fixture->service = fixture->manager ? pmm_daemon_runtime_service_start(&config) : NULL;
    pmm_daemon_runtime_connect_result_t connect_result = {0};
    fixture->client = fixture->service
                          ? pmm_daemon_runtime_client_connect(fixture->endpoint, &identity,
                                                              FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS,
                                                              &connect_result)
                          : NULL;
    return fixture->client && connect_result.status == PMM_DAEMON_RUNTIME_CONNECT_ACCEPTED;
}

static bool frontend_eof_fixture_finish(frontend_eof_fixture_t *fixture) {
    bool ok = true;
    if (fixture->client) {
        ok = pmm_daemon_runtime_client_close(fixture->client,
                                             FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS) &&
             ok;
        fixture->client = NULL;
    }
    if (fixture->service) {
        if (pmm_daemon_runtime_service_state(fixture->service) !=
            PMM_DAEMON_RUNTIME_SERVICE_EXITED) {
            ok = pmm_daemon_runtime_service_stop(fixture->service,
                                                 FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS) &&
                 ok;
        }
        ok = pmm_daemon_runtime_service_free(fixture->service) && ok;
        fixture->service = NULL;
    }
    if (fixture->manager) {
        ok = pmm_version_cohort_manager_free(&fixture->manager) == PMM_PRIVATE_FILE_LOCK_OK && ok;
    }
    pmm_daemon_ipc_endpoint_free(fixture->endpoint);
...
static int frontend_eof_child_run(const char *parent, bool overflow) {
    ...
    bool writer_started =
        input && output && pmm_thread_create(&writer_thread, 0, frontend_eof_writer, &writer) == 0;
    if (!writer_started) {
        if (input) {
            (void)fclose(input);
        } else {
    ...
    pmm_daemon_runtime_client_t *frontend_client = fixture.client;
    fixture.client = NULL; /* pmm_daemon_frontend_mcp_run consumes it. */
    int result = pmm_daemon_frontend_mcp_run(frontend_client, fixture.manager, input, output);
    bool joined = pmm_thread_join(&writer_thread) == 0;
...
static bool frontend_eof_run_isolated(const char *tag, bool overflow) {
    char parent[FRONTEND_TEST_PATH_CAP];
    int written =
        snprintf(parent, sizeof(parent), "%s/pmm-frontend-eof-%s-XXXXXX", pmm_tmpdir(), tag);
    if (written <= 0 || written >= (int)sizeof(parent) || !pmm_mkdtemp(parent)) {
        return false;
    }
    pid_t child = fork();
    if (child == 0) {
...
static void frontend_test_release_lease(pmm_version_cohort_lease_t **lease) {
    while (lease && *lease && pmm_version_cohort_lease_release(lease) != PMM_PRIVATE_FILE_LOCK_OK) {
        pmm_usleep(1000);
    }
}

static bool frontend_test_release_lease_until(pmm_version_cohort_lease_t **lease,
                                              uint64_t deadline_ms) {
    while (lease && *lease) {
        if (pmm_version_cohort_lease_release(lease) == PMM_PRIVATE_FILE_LOCK_OK) {
            continue;
        }
...
static pmm_version_cohort_quiesce_result_t frontend_test_quiesce_requested(void *context);

static bool frontend_backpressure_identity(pmm_daemon_build_identity_t *identity,
                                           const char build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!identity || !build || strlen(build) != PMM_DAEMON_BUILD_FINGERPRINT_SIZE - 1) {
        return false;
    }
...
static int frontend_backpressure_daemon_run(const char *parent, int ready_fd, int cancel_fd,
    ...
    char key[PMM_DAEMON_KEY_SIZE];
    char conflict_log[FRONTEND_TEST_PATH_CAP];
    pmm_daemon_build_identity_t identity = {0};
    ...
    pmm_daemon_ipc_endpoint_t *endpoint =
        log_written > 0 && log_written < (int)sizeof(conflict_log) &&
                pmm_daemon_rendezvous_key(key) && frontend_backpressure_identity(&identity, build)
    ...
    pmm_daemon_runtime_service_t *service =
        endpoint ? pmm_daemon_runtime_service_start(&config) : NULL;
    const char ready = 'R';
    bool announced = service && write(ready_fd, &ready, 1) == 1;
    uint64_t cancellation_deadline = pmm_now_ms() + FRONTEND_BACKPRESSURE_RUNTIME_TIMEOUT_MS;
    while (announced &&
           atomic_load_explicit(&application.session_cancels, memory_order_acquire) == 0 &&
           pmm_now_ms() < cancellation_deadline) {
        pmm_usleep(1000);
    }
    ...
    bool shutdown_released =
        ...
        frontend_test_read_byte(control_fd, &control,
                                pmm_now_ms() + FRONTEND_BACKPRESSURE_RUNTIME_TIMEOUT_MS) &&
        control == 'S';
    ...
    bool stopped =
        service && pmm_daemon_runtime_service_stop(service, FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS);
    bool exited = stopped && pmm_daemon_runtime_service_wait_exited(service, 0);
    bool freed = exited && pmm_daemon_runtime_service_free(service);
...
static int frontend_backpressure_frontend_run(const char *parent, int input_fd, int input_write_fd,
    ...
    pmm_version_cohort_status_t admitted =
        manager ? pmm_version_cohort_acquire(manager, &identity, pmm_now_ms() + 2000U, &participant,
                                             &conflict)
                : PMM_VERSION_COHORT_IO;
    pmm_daemon_runtime_connect_result_t connect_result = {0};
    pmm_daemon_runtime_client_t *client =
        admitted == PMM_VERSION_COHORT_OK
            ? pmm_daemon_runtime_client_connect(
    ...
    bool writer_started =
        ...
        pmm_thread_create(&writer_thread, 0, frontend_backpressure_writer, &writer) == 0;
    if (!writer_started) {
        if (client) {
            (void)pmm_daemon_runtime_client_close(client, FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS);
        }
        if (input) {
            (void)fclose(input);
        } else {
    ...
    int result = pmm_daemon_frontend_mcp_run(client, manager, input, output);
    bool joined = pmm_thread_join(&writer_thread) == 0;
    bool writer_ok = atomic_load_explicit(&writer.finished, memory_order_acquire) &&
                     atomic_load_explicit(&writer.succeeded, memory_order_acquire);
    bool input_closed = fclose(input) == 0;
    ...
    frontend_test_release_lease(&participant);
    bool manager_closed = pmm_version_cohort_manager_free(&manager) == PMM_PRIVATE_FILE_LOCK_OK;
    pmm_daemon_ipc_endpoint_free(endpoint);
...
static bool frontend_backpressure_run_isolated(bool maintenance) {
    char parent[FRONTEND_TEST_PATH_CAP];
    char build[PMM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    int path_written = snprintf(parent, sizeof(parent),
                                maintenance ? "%s/pmm-frontend-maintenance-backpressure-XXXXXX"
                                            : "%s/pmm-frontend-backpressure-XXXXXX",
                                pmm_tmpdir());
    int ready_pipe[2] = {-1, -1};
    int cancel_pipe[2] = {-1, -1};
    int control_pipe[2] = {-1, -1};
    bool directory_ready =
        path_written > 0 && path_written < (int)sizeof(parent) && pmm_mkdtemp(parent);
    bool identity_ready =
        directory_ready && pmm_daemon_runtime_process_build_fingerprint((uint64_t)getpid(), build);
    ...
    /* The daemon child runs pmm_daemon_runtime_service_start before it can
    ...
    char sentinel_key[PMM_DAEMON_KEY_SIZE];
    pmm_daemon_build_identity_t sentinel_identity = {0};
    pmm_daemon_ipc_endpoint_t *sentinel_endpoint =
        announced && pmm_daemon_rendezvous_key(sentinel_key) &&
                frontend_backpressure_identity(&sentinel_identity, build)
            ? pmm_daemon_ipc_endpoint_new(sentinel_key, parent)
            : NULL;
    pmm_daemon_runtime_connect_result_t sentinel_connect = {0};
    pmm_daemon_runtime_client_t *sentinel =
        sentinel_endpoint ? pmm_daemon_runtime_client_connect(sentinel_endpoint, &sentinel_identity,
                                                              FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS,
                                                              &sentinel_connect)
                          : NULL;
    bool sentinel_ready =
        sentinel && sentinel_connect.status == PMM_DAEMON_RUNTIME_CONNECT_ACCEPTED;
    ...
    pmm_daemon_ipc_endpoint_t *mutation_endpoint = NULL;
    pmm_version_cohort_manager_t *mutation_manager = NULL;
    ...
    pmm_version_cohort_status_t mutation_status = PMM_VERSION_COHORT_OK;
    if (maintenance) {
        char mutation_key[PMM_DAEMON_KEY_SIZE];
        mutation_endpoint = output_observed && pmm_daemon_rendezvous_key(mutation_key)
                                ? pmm_daemon_ipc_endpoint_new(mutation_key, parent)
                                : NULL;
        mutation_manager =
            mutation_endpoint ? pmm_version_cohort_manager_new(mutation_endpoint) : NULL;
        mutation_status = mutation_manager
                              ? pmm_version_cohort_reserve_for_mutation(
                                    mutation_manager, pmm_now_ms() + 8000U,
                                    frontend_test_quiesce_requested, NULL, &quiesce, &mutation)
                              : PMM_VERSION_COHORT_IO;
        if (mutation_status != PMM_VERSION_COHORT_OK && frontend > 0) {
    ...
    bool checkpoint_read =
        frontend_test_read_byte(ready_pipe[0], &checkpoint_marker, pmm_now_ms() + 4000U);
    ...
    bool sentinel_usable =
        ...
        pmm_daemon_runtime_client_heartbeat(sentinel, FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS);
    bool sentinel_closed =
        sentinel && pmm_daemon_runtime_client_close(sentinel, FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS);
    sentinel = NULL;
    pmm_daemon_ipc_endpoint_free(sentinel_endpoint);
    ...
    char done_marker = '\0';
    bool done_read =
        shutdown_sent && frontend_test_read_byte(ready_pipe[0], &done_marker, pmm_now_ms() + 4000U);
    ...
    uint64_t cleanup_now = pmm_now_ms();
    ...
    while (mutation_released && mutation_manager && pmm_now_ms() < cleanup_deadline &&
           pmm_version_cohort_manager_free(&mutation_manager) != PMM_PRIVATE_FILE_LOCK_OK) {
        pmm_usleep(1000);
    }
    bool mutation_manager_closed = mutation_manager == NULL;
    pmm_daemon_ipc_endpoint_free(mutation_endpoint);
    ...
    bool maintenance_completed =
        !maintenance || (output_observed && mutation_status == PMM_VERSION_COHORT_OK &&
                         quiesce == PMM_VERSION_COHORT_QUIESCE_REQUESTED);
...
TEST(daemon_frontend_recognizes_exact_cancellation_notification) {
    ASSERT_TRUE(pmm_daemon_frontend_is_cancellation_notification(
...
TEST(daemon_frontend_correlates_cancellation_to_exact_request) {
    ASSERT_TRUE(pmm_daemon_frontend_cancellation_matches_request(
        "{\"jsonrpc\":\"2.0\",\"method\":\"notifications/cancelled\","
        "\"params\":{\"requestId\":7}}",
        7, NULL));
    ASSERT_FALSE(pmm_daemon_frontend_cancellation_matches_request(
        "{\"jsonrpc\":\"2.0\",\"method\":\"notifications/cancelled\","
        "\"params\":{\"requestId\":8}}",
        7, NULL));
    ASSERT_TRUE(pmm_daemon_frontend_cancellation_matches_request(
    ...
    ASSERT_FALSE(pmm_daemon_frontend_cancellation_matches_request(
        ...
        7, NULL));
    ASSERT_FALSE(
        pmm_daemon_frontend_cancellation_matches_request("{\"jsonrpc\":\"2.0\",\"id\":9,"
...
TEST(daemon_frontend_ignores_cancellation_text_in_string_content) {
    ASSERT_FALSE(pmm_daemon_frontend_is_cancellation_notification(
...
TEST(daemon_frontend_rejects_non_notification_cancellation_shapes) {
    ...
    ASSERT_FALSE(pmm_daemon_frontend_is_cancellation_notification(
        ...
        "\"notifications/cancelled\"}}"));
    ASSERT_FALSE(pmm_daemon_frontend_is_cancellation_notification("not-json"));
    ASSERT_FALSE(pmm_daemon_frontend_is_cancellation_notification(NULL));
    PASS();
}
...
TEST(daemon_frontend_maintenance_exits_while_stdio_reader_is_blocked) {
    ...
    if (child == 0) {
        ...
        (void)close(ready_pipe[0]);
        pmm_daemon_ipc_endpoint_t *endpoint =
            pmm_daemon_ipc_endpoint_new("0123456789abcdef", fixture.parent);
        pmm_version_cohort_manager_t *manager =
            endpoint ? pmm_version_cohort_manager_new(endpoint) : NULL;
        pmm_version_cohort_lease_t *participant = NULL;
        pmm_daemon_conflict_t conflict;
        pmm_daemon_build_identity_t identity = frontend_test_identity();
        pmm_version_cohort_status_t admitted =
            manager ? pmm_version_cohort_acquire(manager, &identity, pmm_now_ms() + 2000U,
                                                 &participant, &conflict)
                    : PMM_VERSION_COHORT_IO;
        FILE *input = admitted == PMM_VERSION_COHORT_OK ? fdopen(input_pipe[0], "rb") : NULL;
        ...
        int result = pmm_daemon_frontend_mcp_run((pmm_daemon_runtime_client_t *)(uintptr_t)1,
    ...
    bool announced = child > 0 && frontend_test_wait_byte(ready_pipe[0], 'R', pmm_now_ms() + 2000U);
    pmm_version_cohort_lease_t *mutation = NULL;
    pmm_version_cohort_quiesce_result_t quiesce = PMM_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    pmm_version_cohort_status_t status =
        announced ? pmm_version_cohort_reserve_for_mutation(fixture.manager, pmm_now_ms() + 3000U,
                                                            frontend_test_quiesce_requested, NULL,
                                                            &quiesce, &mutation)
                  : PMM_VERSION_COHORT_IO;
    if (status != PMM_VERSION_COHORT_OK && child > 0) {
    ...
    ASSERT_EQ(status, PMM_VERSION_COHORT_OK);
    ASSERT_EQ(quiesce, PMM_VERSION_COHORT_QUIESCE_REQUESTED);
    ASSERT_TRUE(waited);
...
TEST(daemon_local_participant_monitor_cancels_then_bounds_active_operation) {
    ...
    if (child == 0) {
        ...
        pmm_version_cohort_status_t admitted =
            manager ? pmm_version_cohort_acquire(manager, &identity, pmm_now_ms() + 2000U,
                                                 &participant, &conflict)
                    : PMM_VERSION_COHORT_IO;
        pmm_daemon_maintenance_monitor_t *monitor =
            admitted == PMM_VERSION_COHORT_OK
                ? pmm_daemon_maintenance_monitor_start(manager, frontend_test_cancel_active,
    ...
    pmm_version_cohort_quiesce_result_t quiesce = PMM_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    pmm_version_cohort_status_t status =
        announced ? pmm_version_cohort_reserve_for_mutation(fixture.manager, pmm_now_ms() + 5000U,
                                                            frontend_test_quiesce_requested, NULL,
                                                            &quiesce, &mutation)
                  : PMM_VERSION_COHORT_IO;
    bool cancelled = status == PMM_VERSION_COHORT_OK &&
                     frontend_test_wait_byte(cancel_pipe[0], 'C', pmm_now_ms() + 1000U);
    if (status != PMM_VERSION_COHORT_OK && child > 0) {
        (void)kill(child, SIGKILL);
    }
    int child_status = 0;
...
TEST(daemon_local_participant_monitor_allows_supervisor_containment_window) {
    ...
    if (child == 0) {
        ...
        pmm_daemon_maintenance_monitor_t *monitor =
            admitted == PMM_VERSION_COHORT_OK
                ? pmm_daemon_maintenance_monitor_start(manager, frontend_test_cancel_and_mark,
        ...
        pmm_usleep(2100U * 1000U);
        bool stopped = pmm_daemon_maintenance_monitor_stop(&monitor);
        frontend_test_release_lease(&participant);
...
TEST(daemon_local_participant_monitor_joins_before_manager_teardown) {
    ...
    pmm_daemon_maintenance_monitor_t *monitor = pmm_daemon_maintenance_monitor_start(
        fixture.manager, NULL, NULL, EXIT_FAILURE, "test-idle-command");
    bool started = monitor != NULL;
    bool stopped = started && pmm_daemon_maintenance_monitor_stop(&monitor);
