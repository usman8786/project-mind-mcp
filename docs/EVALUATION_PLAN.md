# project-mind-mcp — Evaluation Plan (159 Languages)
...
Measure how well `project-mind-mcp`'s structured knowledge-graph queries answer real developer
...
| Old (v8) | New |
|----------|-----|
| 66 languages | **159 languages** (full `PMM_LANG_*` registry) |
...
The supported set is the `CBMLanguage` enum in `internal/pmm/pmm.h` (`PMM_LANG_GO=0` …
`PMM_LANG_CFML`, before `PMM_LANG_COUNT`). The canonical short name used throughout this plan is the
lowercased enum suffix (`PMM_LANG_GO` → `go`, `PMM_LANG_CSHARP` → `csharp`,
`PMM_LANG_COMMONLISP` → `commonlisp`). The complete repo assignment for all 159 is the master table
in §8.

### The 9 LSP-hybrid languages (deep-dive cohort)

These have dedicated hybrid LSP modules under `internal/pmm/lsp/` and therefore type-aware
...
**One language at a time, in the main channel, never concurrently.** The PMM/graph side of the
...
for each language L in order (skip if manifest says done):
  ...
  8. DELETE L's index from ~/.cache/project-mind-mcp so the next language starts clean,
...
rm -f ~/.cache/project-mind-mcp/*.db
...
for lang in $ALL_LANGS; do                      # ALL_LANGS = full 159-name list (§2 / §8)
  ...
  t0=$(now_ms)
  scripts/benchmark-index.sh ~/.local/bin/project-mind-mcp "$lang" /tmp/bench/"$lang" /tmp/eval-results
...
`MATCH ()-[r]->() RETURN type(r), count(*)`) / `get_graph_schema` / `pmm_store_count_edges_by_type`,
...
file (`internal/pmm/lang_specs.c`) and the tree-sitter node type that would fix it. Plus a
...
#### Challenge Review: EVALUATION_PLAN.md — project-mind-mcp 159-Language Benchmark
