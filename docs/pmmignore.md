# `.pmmignore` — Excluding Files from Indexing

`.pmmignore` is a project-specific ignore file that controls which files the
indexer sees. It uses gitignore-style syntax and is read from the **root of
the indexed directory** (`<repo>/.pmmignore`). Nested `.pmmignore` files in
...
matched by `.pmmignore` never enters the graph. Changes to `.pmmignore` take
...
4. **`.pmmignore`** — a positive match skips the path; a negated match can
...
after them; none of these are overridable from `.pmmignore`. Symlinks are
always skipped.

## Negation (`!`) — current behavior

- **Within `.pmmignore`**: standard gitignore semantics. Patterns are
...
- **Parent pruning** (same caveat as git): when a directory is excluded, the
  ...
  contents.
- **Across layers**: a `.pmmignore` negation overrides the **git global
  excludes** layer only. Example: your `~/.config/git/ignore` ignores
  `*.sql`, but this project's SQL should be indexed — add `!*.sql` to
  `.pmmignore`. Negation cannot override the built-in skip lists, the repo
...
- `!` in `.pmmignore` will be able to un-skip ordinary built-in skip
